...
#if IS_ENABLED(CONFIG_RTK_KDRV_EMCU)
// extern int customer_powerMgr_get_wakeup_source(char *buf);
unsigned int pwrkey_handle_count;
unsigned int poweron_reg_row = 0;
unsigned int power_reason = 0;

static const uint32_t wakeKeyMap[][2] = {
	//tcl-RCA
	{0xf3000003, 0xf300cf00},  // num 0,scancode 11
	{0x73080003, 0x7308cf00},  // num 1,scancode 2
	{0xb3040003, 0xb304cf00},  // num 2,scancode 3
	{0x330c0003, 0x330ccf00},  // num 3,scancode 4
	{0xd3020003, 0xd302cf00},  // num 4,scancode 5
	{0x530a0003, 0x530acf00},  // num 5,scancode 6
	{0x93060003, 0x9306cf00},  // num 6,scancode 7
	{0x130e0003, 0x130ecf00},  // num 7,scancode 8
	{0xe3010003, 0xe301cf00},  // num 8,scancode 9
	{0x63090003, 0x6309cf00},  // num 9,scancode 10
	{0xab050003, 0xab054f00},  // power,scancode 116
	{0xef010003, 0xef010f00},  // home,scancode 102
	{0x080f0003, 0x080f7f00},  // netflix,scancode
	//tcl-NEC (Korea)
	{0x30cf0003, 0x30cf9966},  // num 0,scancode 11
	{0x31ce0003, 0x31ce9966},  // num 1,scancode 2
	{0x32cd0003, 0x32cd9966},  // num 2,scancode 3
	{0x33cc0003, 0x33cc9966},  // num 3,scancode 4
	{0x34cb0003, 0x34cb9966},  // num 4,scancode 5
	{0x35ca0003, 0x35ca9966},  // num 5,scancode 6
	{0x36c90003, 0x36c99966},  // num 6,scancode 7
	{0x37c80003, 0x37c89966},  // num 7,scancode 8
	{0x38c70003, 0x38c79966},  // num 8,scancode 9
	{0x39c60003, 0x39c69966},  // num 9,scancode 10
	{0x2ad50003, 0x2ad59966},  // power,scancode 116
	{0x08f70003, 0x08f79966},  // home,scancode 102
	{0xef100003, 0xef109966},  // netflix,scancode
};

void get_wakeup_reason(unsigned int *reg_row, unsigned int *reason)
{
	int i;
	int cnt;
	*reason = *reg_row & 0xff;

	if(*reason == WKSOR_IRDA) {
		cnt = sizeof(wakeKeyMap) / sizeof(wakeKeyMap[0]);
		for(i = 0; i < cnt; i++) {
			if (wakeKeyMap[i][0] == *reg_row) {
				*reg_row = wakeKeyMap[i][1];
			}
		}
	}
}
...
static ssize_t last_resume_reason_show(struct kobject *kobj, struct kobj_attribute *attr,
		char *buf)
{
//	return customer_powerMgr_get_wakeup_source(buf);
	unsigned int wakeup_reg_row = 0;
	unsigned int reason = 0;
	int buf_offset = 0;
	const char* str[] = {"UNDEF","KEYPAD",/*"WUT"*/"PVR","IR","CEC","PPS","WOW","HML","RTC",/*"WOV"*/"VOICE","WOL","OTHER"};

	if (pwrkey_handle_count != 0) {
		powerMgr_get_wakeup_source(&wakeup_reg_row, NULL);
		get_wakeup_reason(&wakeup_reg_row, &reason);
	}

	buf_offset += sprintf(buf + buf_offset,
			"poweron keycode: %x, poweron reason: %s, wakeup keycode: %x, wakeup reason: %s, resume: %d\n",
			(unsigned int)poweron_reg_row, str[power_reason], (unsigned int)wakeup_reg_row, str[reason], pwrkey_handle_count);
	pr_info("reason info:%s, %d\n",buf,buf_offset);
	return buf_offset;
}
#else
...
#if IS_ENABLED(CONFIG_RTK_KDRV_EMCU)
/* get power on reason and keycode*/
static void get_poweron_reason(void)
{
	if (pwrkey_handle_count != 0)
		return;

	powerMgr_get_wakeup_source(&poweron_reg_row, NULL);
	get_wakeup_reason(&poweron_reg_row, &power_reason);
}
#endif
