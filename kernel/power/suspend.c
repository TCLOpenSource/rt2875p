...
static int enter_state(suspend_state_t state)
{
	...
	if (state == PM_SUSPEND_TO_IDLE) {
		...
		pr_info("DC standby hardcode,fix it later\n");
	}
#endif
	...
	if (error)
		//goto Unlock;
		goto Finish; // for freeze fail case, since return value only -16 & 0, need run resume flow
#ifdef CONFIG_RTK_KDRV_EMCU
	if((state == PM_SUSPEND_STANDBY) && (error == 0)) {
		pr_info("rtk_pm_begin: state:%d, fixit!? \n", state);
		rtk_pm_load_8051(1);
