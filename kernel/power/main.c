...
static suspend_state_t decode_state(const char *buf, size_t n)
{
	...
	if (len == 7 && str_has_prefix(buf, "standby"))
...
static unsigned int pm_str_event_wait_timeout = 0;
#endif
	...
	if(state == PM_SUSPEND_MEM)
	{
		...
		while( (pm_str_event_force_wait == 1) && (str_event_state != STR_EVENT_WAIT_SUSPEND_EVENT_STATE))
		{
			...
			if(time_after(jiffies, timeout_check))
			{
				...
				cnt++;
				str_event_log |= 0x10;
				if(cnt > 10) { //to protect system can be normal, need cold on by 50s timeout.
					pm_str_event_wait_timeout = 1;
					return;
				}
			}
		}
...
echo clear > /sys/power/pm_STR_event
*/
		...
		//||( (str_event_state == STR_EVENT_SUSPEND_EVENT_READY_STATE) && (input_state == STR_EVENT_WAIT_SUSPEND_EVENT_STATE)))
	{
		if(down_trylock(&str_main_flow_sema) != 0)
		{
			if(pm_str_evnet_freeze_fail) {
				...
				pr_err("STR_NotifyAP : freeze task fail happen,notice!!!\n");
				//console_loglevel = 0;
...
#ifdef CONFIG_PM_STR_SUPPORT_EVENT //RTK_FIXME_64
		str_event_before_suspend_peration(state);
		if(pm_str_event_wait_timeout == 1) {
			pm_str_event_wait_timeout = 0;
			pr_info("STR_NotifyAP : Force to standby!!!\n");
