...
// RTK_FIXME_64 (start) : added
#ifdef CONFIG_REALTEK_LOGBUF

__attribute__((weak)) int rtdlog_save_msg(int type, const char * text,rtd_msg_header * header)
{
        return -1;
}
...
void save_early_log(unsigned char type)
{
    ...
    for(i = 0;i <= end_seq;++i)
    {
        ...
        rtdlog_save_msg(type, log_addr, &tmp_header);
        //update index
        now_idx += kernel_log_header->len;
        //check index
        if(now_idx == end_idx)
