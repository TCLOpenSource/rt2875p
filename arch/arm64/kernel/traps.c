...
#if defined(CONFIG_RTK_KDRV_SB2)
/* return 0: recoverable 
 */
__attribute__((weak)) int hw_monitor_check_handler(unsigned long addr, unsigned int fsr, struct pt_regs *regs) {return 0;}

static int rtk_serror_recoverable(struct pt_regs *regs, unsigned int esr)
{
        return hw_monitor_check_handler(-1, esr, regs);
}
#else //#if defined(CONFIG_RTK_KDRV_SB2)

static int rtk_serror_recoverable(struct pt_regs *regs, unsigned int esr) { return -1; }
...
asmlinkage void do_serror(struct pt_regs *regs, unsigned int esr)
{
	...
	if (!was_in_nmi)
        ...
        if (!rtk_serror_recoverable(regs, esr)) {
                if (!was_in_nmi)
                        nmi_exit();
                return;
        }
