...
extern unsigned int arm64_zone_dma32_bits;
static bool is_unbalance_ddr_system()
{
	return arm64_zone_dma32_bits == 32 ? false : true;
}
...
retry:
	...
	for_next_zone_zonelist_nodemask(zone, z, ac->zonelist, ac->high_zoneidx,
		...
		if (is_unbalance_ddr_system() &&
		        (zone_idx(zone) == ZONE_NORMAL) &&
			((gfp_mask & __GFP_MOVABLE) == 0) &&
			(zonelist_rescan == false) &&
			(nr_rescan == 0)) {
//			pr_info("normal, gfp=%lx, no_fallback=%d, alloc_flags=%lx\n", gfp_mask, no_fallback, alloc_flags);
		        continue;
		} else if ((zone_idx(zone) == ZONE_NORMAL) &&
			((gfp_mask & __GFP_MOVABLE) == 0)) {
//			pr_info("normal, gfp=%lx, no_fallback=%d, alloc_flags=%lx, highest=%d, nr_rescan=%d\n",
//					gfp_mask, no_fallback, alloc_flags, ac->high_zoneidx, nr_rescan);
		} else if ((zone_idx(zone) == ZONE_DMA32) &&
			((gfp_mask & __GFP_MOVABLE) == 0)) {
//			pr_info("dma32 , gfp=%lx, no_fallback=%d, alloc_flags=%lx, highest=%d, nr_rescan=%d\n",
//					gfp_mask, no_fallback, alloc_flags, ac->high_zoneidx, nr_rescan);
		}
#endif
		if (cpusets_enabled() &&
...
//		pr_info("normal-rescan(%d), gfp=%lx, no_fallback=%d, alloc_flags=%lx, highest=%d, nr_rescan=%d\n",
//				zone_idx(last_zone), gfp_mask, no_fallback, alloc_flags, ac->high_zoneidx, nr_rescan);
	}
			 ...
			 int migratetype, int flags)
{
	...
	const char *reason_cma = "CMA page";
...
unmovable:
	...
	/*
	 * CMA pages are always on the ZONE_MOVABLE so checking and
	 * warning wheather the zone is the ZONE_MOVABLE is unnecessary.
	 */
	WARN_ON_ONCE(zone_idx(zone) == ZONE_MOVABLE && reason != reason_cma);
