...
#ifdef CONFIG_DWC2_RTK
#ifndef CONFIG_DWC2_RTK_USING_IRQ	
/** * Thread entry for otg polling. */

static int rtk_otg_chk_event(struct dwc2_hsotg* hsotg)
{     
#if 0
	unsigned int status;
	status = rtd_inl(USB2_OTG_USB_INT_reg);     
	return (status & USB2_OTG_USB_INT_int_ehci_dic_mask);    
#else
	u32 intr_status = dwc2_readl(hsotg, GINTSTS);
	u32 gintmsk = dwc2_readl(hsotg, GINTMSK);
	return (intr_status & gintmsk);
#endif
}

static int rtk_otg_polling_thread_func(void * data)
{    
	struct dwc2_hsotg* hsotg = (struct dwc2_hsotg*) data;     
	set_freezable();  
	while(!hsotg->polling_thread_exited_flag) {       
		usleep_range(10,20);         
		if(rtk_otg_chk_event(hsotg))     {   
			dwc2_handle_common_intr(0, hsotg);
			dwc2_hsotg_irq(0, hsotg); 

			//rtd_outl(USB2_OTG_USB_INT_reg, USB2_OTG_USB_INT_int_ehci_dic_mask);   
		}
	}    
	 complete_and_exit(&hsotg->polling_thread_completion, 1);  
	return 0;
}
#endif
#endif
...
#else
	hsotg->polling_thread_exited_flag = 0;
	hsotg->polling_thread_task = kthread_create(rtk_otg_polling_thread_func, hsotg, "rtk_polling_thread");
	if(IS_ERR(hsotg->polling_thread_task)) 
		goto err;
	
	init_completion(&hsotg->polling_thread_completion);
	wake_up_process(hsotg->polling_thread_task);  
#endif
#endif
...
#else
	if(hsotg->polling_thread_task) {
		hsotg->polling_thread_exited_flag = 1;
		wait_for_completion(&hsotg->polling_thread_completion);
		hsotg->polling_thread_task = NULL;
	}
#endif
#endif
	/* all endpoints should be shutdown */
...
#ifdef CONFIG_DWC2_RTK
static struct usb_ep *dwc2_gadget_match_ep(struct usb_gadget *gadget,
        ...
        struct usb_ss_ep_comp_descriptor *ep_comp)
{
	...
	int ep_num = 0;
    	...
    	dev_err(hsotg->dev, "%s: ep_num(%d), dir(%s) \n",
            		__func__, ep_num, dir_in ? "IN" : "OUT");

    	for(ep_num = 1; ep_num < hsotg->num_of_eps; ep_num++) {
		if(dir_in) {
			if(hsotg->eps_in[ep_num] && !hsotg->eps_in[ep_num]->ep.claimed 
					&& (!hsotg->eps_out[ep_num] || !hsotg->eps_out[ep_num]->ep.claimed)) {
				ep = &hsotg->eps_in[ep_num]->ep;
				ep_num = ep_num | USB_DIR_IN;
				break;
			}
		} else {
			if(hsotg->eps_out[ep_num] && !hsotg->eps_out[ep_num]->ep.claimed 
					&& (!hsotg->eps_in[ep_num] || !hsotg->eps_in[ep_num]->ep.claimed)) {
				ep = &hsotg->eps_out[ep_num]->ep;
				break;
			}
		}
    	...
    	if(ep) {
    		desc->bEndpointAddress = ep_num;	
    		dev_err(hsotg->dev, "%s: 0x%02x maping to phys ep_num(%u)\n", __func__,
            		desc->bEndpointAddress, usb_endpoint_num(desc));
	}
