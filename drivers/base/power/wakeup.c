 ...
 */
int enable_skip_wakeup_source = 0;
...
/*ANDROIDTV-933, skip process wakeup event during suspend flow*/
	...
	ret = ret || (atomic_read(&pm_abort_suspend) > 0);
#if 1
	if ((ret == true) && (enable_skip_wakeup_source == 1))
		return false;
	else
		return ret;
#endif
#else
	return false || atomic_read(&pm_abort_suspend) > 0;
#endif
}
