...
struct fbtc_slot s_def[] = {
	...
	{ 15, 0x5a5a5a5a, SLOT_ISO}, /* CXST_W2 */
	...
	{  0, 0x6a5a5a5a, SLOT_MIX}, /* CXST_E2G */
	{  0, 0xffffffff, SLOT_MIX}, /* CXST_E5G */
	{  0, 0x65555555, SLOT_MIX}, /* CXST_EBT */
...
static void _set_policy(struct btc_t *btc, u16 policy_type, const char* action)
{
	...
	if (btc->mdinfo.ant.type == BTC_ANT_SHARED) {
		...
		if (dm->leak_ap &&
			...
			tbl_b1 = cxtbl[3];
			tbl_b4 = cxtbl[3];
		} else if (type == BTC_CXP_FIX || type == BTC_CXP_PFIX) {
			tbl_b1 = cxtbl[3];
			tbl_b4 = cxtbl[3];
		} else {
			tbl_b1 = cxtbl[2];
	...
	case BTC_CXP_OFFE: /* TDMA off + beacon protect + Ext_control */
		...
		case BTC_CXP_OFFE_WL: /*for 4-way  */
			_slot_set(CXST_E2G, 0, cxtbl[1], SLOT_MIX);
