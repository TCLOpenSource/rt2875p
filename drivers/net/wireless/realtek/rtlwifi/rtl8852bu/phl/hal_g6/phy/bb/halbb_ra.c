...
u64 halbb_gen_abg_mask(struct bb_info *bb, struct rtw_phl_stainfo_t *phl_sta_i)
{
	...
	for (i = 0; i < MAX_ABG_RATE_NUM; i++) {
		BB_DBG(bb, DBG_RA, "supported rates = %x\n",
		       asoc_cap_i->supported_rates[i]);
		s_rate[i] = (asoc_cap_i->supported_rates[i] & 0x7f); /*exclude BSS basic bit*/
	}
