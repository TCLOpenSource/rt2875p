...
enum rtw_phl_status rtw_phl_rf_on(void *phl)
{
	...
	phl_data_ctrler(phl_info, &ctl, NULL);
...
enum rtw_phl_status rtw_phl_rf_off(void *phl)
{
...
#endif /* CONFIG_SYNC_INTERRUPT */
	struct phl_data_ctl_t ctl = {0};
	u32 start_t = _os_get_cur_time_ms();
	...
	while (1) {
		if (phl_get_passing_time_ms(start_t) >= MAX_RF_OFF_STOP_TRX_TIME) {
			PHL_ERR("%s : pause trx reach maximum polling time.\n", __func__);
			break;
		}

		if (ops->is_rx_pause(phl_info) && ops->is_tx_pause(phl_info)) {
			PHL_INFO("%s : sw trx pause succeed.\n", __func__);
			break;
		}
