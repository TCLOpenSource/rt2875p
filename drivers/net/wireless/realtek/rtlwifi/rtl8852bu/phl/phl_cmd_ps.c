...
static void _ps_period_chk_done(void *priv, struct phl_msg *msg)
{
...
static void _cmd_ps_timer_cb(void *context)
{
	...
	attr.completion.completion = _ps_period_chk_done;
	attr.completion.priv = ps;

	if (phl_disp_eng_send_msg(ps->phl_info, &msg, &attr, NULL) !=
