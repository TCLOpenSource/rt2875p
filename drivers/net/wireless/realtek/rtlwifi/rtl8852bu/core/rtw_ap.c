...
void rtw_core_ap_swch_start(_adapter *padapter, struct createbss_parm *parm)
{
	...
	if (parm->ch_to_set != 0) {
		...
		rtw_mi_update_union_chan_inf(padapter, parm->ch_to_set, parm->offset_to_set, parm->bw_to_set);
...
int rtw_ap_rx_msdu_act_check(union recv_frame *rframe, const u8 *da,
			     const u8 *sa, u8 *msdu, enum rtw_rx_llc_hdl llc_hdl,
...
//			goto exit;
	...
	/* DA is unknown*/
	act |= RTW_RX_MSDU_ACT_INDICATE;
	/* TODO currently driver does not consider ap_isolate */
	#if 0
	if (adapter->mlmepriv.ap_isolate) {
		/*
...
fwd_chk:
...
#if CONFIG_RTW_AP_DATA_BMC_TO_UC
	if (is_da_bmc) {
		if (rtw_ap_fwd_b2u_policy_chk(adapter->b2u_flags_ap_fwd, da,
					      rattrib->to_fr_ds == 3 &&
					      !IS_MCAST(rattrib->ra))
			&& adapter->registrypriv.wifi_spec == 0
			&& adapter->xmitpriv.free_xmitframe_cnt > (NR_XMITFRAME / 4)) {
				if (rtw_ap_data_bmc_to_uc(adapter, da, sa,
							  rframe->u.hdr.psta->phl_sta->mac_addr,
							  os_qid, f_list) == 0)
					goto set_act_fwd;
			}
	...
	if (adapter_use_wds(adapter) && !in_wds_tbl && !is_da_peer) {
		...
		rtw_ap_data_flood_for_unknown_da(adapter, da, sa,
						 rframe->u.hdr.psta->phl_sta->mac_addr,
						 os_qid, f_list);
		goto set_act_fwd;
	} else
