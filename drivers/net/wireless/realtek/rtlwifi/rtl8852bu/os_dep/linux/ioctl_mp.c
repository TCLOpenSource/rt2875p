				...
				struct iw_point *wrqu, char *extra)
{
	...
	if (strncmp(input, "setbssid=", 8) == 0) {
		...
		int gain_val = 0;
		u8 path_num = 0xff;
		u8 iscck = (u8)rtw_mp_is_cck_rate(pmppriv->rateidx);

		if (sscanf(input, "gaina=%d,iscck=%hhd", &gain_val, &iscck) == 2)
			path_num = 0;
		else if (sscanf(input, "gainb=%d,iscck=%hhd", &gain_val, &iscck) == 2)
			path_num = 1;
		else {
			if (sscanf(input, "gain=%d", &gain_val) == 1)
			path_num = GET_HAL_RFPATH_NUM(adapter_to_dvobj(padapter));
			else if (sscanf(input, "gaina=%d", &gain_val) == 1)
			path_num = 0;
			else if (sscanf(input, "gainb=%d", &gain_val) == 1)
			path_num = 1;
