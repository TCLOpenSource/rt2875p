...
static void _update_adaptivity_mode(struct dvobj_priv *dvobj)
{
	enum rtw_edcca_mode mode = RTW_EDCCA_NORMAL;
	void *phl = GET_PHL_INFO(dvobj);

	if (rtw_adaptivity_en == 0) {
		/* disable */
		mode = RTW_EDCCA_NORMAL;
	} else {
		/* for ETSI */
		if (rtw_adaptivity_mode == 0)
			mode = RTW_EDCCA_ETSI;
		else if (rtw_adaptivity_mode == 1)
		/* for carrier sense */
			mode = RTW_EDCCA_JP;
		else
			mode = RTW_EDCCA_ETSI;
	...
	RTW_INFO("%s: mode=%d\n", __func__, mode);
	rtw_phl_set_edcca_mode(phl, mode);
}
