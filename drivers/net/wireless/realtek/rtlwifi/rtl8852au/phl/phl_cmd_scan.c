 ...
 */
static bool _handle_probing(void *d, struct rtw_phl_scan_param *param)
{
	struct phl_scan_channel *scan_ch = param->scan_ch;
	...
	if (diff_t >= DEF_PROBE_PERIOD) {
		...
		if (param->ops->scan_ch_cancel && check_cancel) {
			if (param->ops->scan_ch_cancel(param->priv, param)) {
		...
		/* core probe handler */
		param->ops->scan_probe(param->priv, param);
	}
	...
	_os_set_timer(d, &param->scan_timer, probe_t);
			...
			if (!param->ops->scan_probe) {
				if (_cmd_chk_ext_act_scan(param)) {
					...
					PHL_INFO("%s :: extend listen state of ch %d by %d ms, and reset timer\n",
						__func__,
						param->scan_ch->channel,
						param->ext_act_scan_period);
					break;
				}
			} else {
				if (_handle_probing(d, param))
					break;
			}
