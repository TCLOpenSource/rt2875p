	...
	if ((MSG_EVT_ID_FIELD(msg->msg_id) == MSG_EVT_DISCONNECT_END) ||
		...
		for (ridx = 0; ridx < MAX_WIFI_ROLE_NUMBER; ridx++) {
			if (band1_ctrl->role_map & BIT(ridx)) {
				wr = &(phl_com->wifi_roles[ridx]);
				phl_wifi_role_realloc_band(phl_info, wr, HW_BAND_0);
			}
		}

		/* 4.dbcc hw cfg (en = false)*/
				   ...
				   struct rtw_wifi_role_t *wrole)
{
	...
	if (rtw_phl_role_is_client_category(wrole)) {
		...
		psts = phl_role_ap_client_notify(phl_info, wrole, MLME_LINKING);
		if (psts != RTW_PHL_STATUS_SUCCESS) {
			PHL_ERR("%s ap role notify failed\n", __func__);
			goto _exit;
		}
	}
	...
	if (!rtw_phl_role_is_ap_category(wrole)) {
		psts = rtw_phl_mr_rx_filter_opt(phl_info, wrole);
		if (psts != RTW_PHL_STATUS_SUCCESS) {
				 ...
				 struct rtw_wifi_role_t *wrole)
{
	...
	if (rtw_phl_role_is_client_category(wrole)) {
		...
		psts = phl_role_ap_client_notify(phl_info, wrole, MLME_LINKED);
		if (psts != RTW_PHL_STATUS_SUCCESS) {
			PHL_ERR("%s ap role notify failed\n", __func__);
			goto _exit;
		}
	...
	if (!rtw_phl_role_is_ap_category(wrole)) {
		...
		psts = rtw_phl_mr_set_rxfltr_type_by_mode(phl_info, wrole,
				...
				struct rtw_wifi_role_t *wrole)
{
	...
	if (rtw_phl_role_is_client_category(wrole)) {
		...
		psts = phl_role_ap_client_notify(phl_info, wrole, MLME_NO_LINK);
		if (psts != RTW_PHL_STATUS_SUCCESS) {
			PHL_ERR("%s ap role notify failed\n", __func__);
			goto _exit;
		}
...
_mrc_module_chg_op_chdef_end_pre_hdlr(u8 *param)
{
	...
	bool (*core_issue_null)(void *, u8, bool) = NULL;
		...
		struct rtw_chan_def *chctx_result)
{
	...
	if (rtw_phl_chanctx_add((void *)phl, wrole, &chan_def, &mr_cc_info)) {
...
enum rtw_phl_status
rtw_phl_mr_rx_filter_opt(void *phl, struct rtw_wifi_role_t *wrole)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	...
	rtw_hal_set_rxfltr_opt_by_mode(phl_info->hal, wrole->hw_band, mode);
	return RTW_PHL_STATUS_SUCCESS;
}
				   ...
				   struct rxfltr_type_cap_to_set *cap_s)
{
	...
	struct rtw_wifi_role_t *wr = NULL;
	struct rxfltr_type_cap *cap_role = NULL;
	u8 ridx = 0;
	...
	for (ridx = 0; ridx < MAX_WIFI_ROLE_NUMBER; ridx++) {
		...
		cap_role = &(wr->rxfltr_type_cap);
		_phl_mr_rxfltr_target_dcsn_ctrl(&(cap_role->rxfltr_ctrl),
						&(cap_s->rxfltr_ctrl));
		_phl_mr_rxfltr_target_dcsn_mgnt(&(cap_role->rxfltr_mgnt),
						&(cap_s->rxfltr_mgnt));
		_phl_mr_rxfltr_target_dcsn_data(&(cap_role->rxfltr_data),
						&(cap_s->rxfltr_data));
	}
}
...
enum rtw_phl_status rtw_phl_mr_set_rxfltr_type_by_mode(void *phl, struct rtw_wifi_role_t *wrole,
						       enum rtw_rxfltr_type_mode mode)
{
	struct phl_info_t *phl_info = (struct phl_info_t*)phl;
	struct rxfltr_type_cap *cap_role = NULL;
	struct rxfltr_type_cap_to_set cap_role_set = {0};
	enum rtw_hal_status hsts = RTW_HAL_STATUS_FAILURE;

	cap_role = &(wrole->rxfltr_type_cap);

	/* set rxfltr type cap of this role by mode */
	switch (mode) {
	case RX_FLTR_TYPE_MODE_ROLE_INIT:
		PHL_MR_SET_RXFLTR_TYPE_CTRL(RXFLTR_TARGET_DROP, cap_role, &cap_role_set);
		PHL_MR_SET_RXFLTR_TYPE_MGNT(RXFLTR_TARGET_TO_HOST, cap_role, &cap_role_set);
		PHL_MR_SET_RXFLTR_TYPE_DATA(RXFLTR_TARGET_TO_HOST, cap_role, &cap_role_set);
		break;
	case RX_FLTR_TYPE_MODE_STA_CONN:
		PHL_MR_SET_RXFLTR_STYPE_CTRL(RXFLTR_STYPE_VHT_HE_NDPA, RXFLTR_TARGET_TO_HOST,
					     cap_role, &cap_role_set);
		PHL_MR_SET_RXFLTR_STYPE_CTRL(RXFLTR_STYPE_BAR, RXFLTR_TARGET_TO_HOST,
					     cap_role, &cap_role_set);
#ifdef RTW_WKARD_RX_FLTR_HE_TF
		if (wrole->cap.wmode & WLAN_MD_11AX) {
	...
	case RX_FLTR_TYPE_MODE_AP_DIS_CONN:
		if (wrole->assoc_sta_queue.cnt < 2)
			PHL_MR_SET_RXFLTR_TYPE_CTRL(RXFLTR_TARGET_DROP, cap_role, &cap_role_set);
		break;
	default:
	...
	/* jointly decide rxfltr type cap among all active roles */
	_phl_mr_rx_filter_type(phl_info, wrole, &cap_role_set);

	/* call HAL API to set rxfltr type cap */
	hsts = rtw_hal_set_rxfltr_by_stype_all(phl_info->hal, wrole->hw_band, &cap_role_set);
