			...
			struct rtw_hal_com_t *hal_com)
{
	...
	bus_cap->rpbd_num = (bus_sw->rpbd_num) ?
		...
		bus_hw->max_rpbd_num;
	/* RXBUF NUM */
	bus_cap->rxbuf_num = (bus_sw->rxbuf_num) ?
		((bus_sw->rxbuf_num > bus_hw->max_rxbuf_num) ?
		 bus_hw->max_rxbuf_num : bus_sw->rxbuf_num) :
		bus_hw->max_rxbuf_num;
	/* RPBUF NUM */
	bus_cap->rpbuf_num = (bus_sw->rpbuf_num) ?
		((bus_sw->rpbuf_num > bus_hw->max_rpbuf_num) ?
		 bus_hw->max_rpbuf_num : bus_sw->rpbuf_num) :
		bus_hw->max_rpbuf_num;
	/* RXBUF SIZE */
	bus_cap->rxbuf_size = (bus_sw->rxbuf_size) ?
		((bus_sw->rxbuf_size > bus_hw->max_rxbuf_size) ?
		 bus_hw->max_rxbuf_size : bus_sw->rxbuf_size) :
