...
rtw_hal_set_rxfltr_opt_by_mode(void *hal, u8 band, enum rtw_rx_fltr_opt_mode mode)
{
	...
	/* Note: @hal_info_t.rx_fltr_opt_mode is used to recored any mode other than
	...
	if (hal_info->monitor_mode && mode != RX_FLTR_OPT_MODE_RESTORE) {
		hal_info->rx_fltr_opt_mode = mode;
	...
	/* Record @hal_info_t.rx_fltr_opt_mode only when the mode is not monitor and
	 * restore, otherwise, it is kept intact.
	 * TODO: The rx fltr opt mode should be recorded  separately for each band.
	 */
