...
static enum rtw_hal_status
_hal_check_rxsc_rdy_8852a(struct hal_info_t *hal,
	...
	u8 *desc, struct rtw_r_meta_data *mdata)
{
	...
	if ((mdata->rpkt_type == RX_8852A_DESC_PKT_T_WIFI)
		...
		if (PHL_MACID_MAX_NUM != rxsc_entry->cached_rx_macid &&
			...
			if (_hal_check_rxsc_rdy_8852a(hal, phl_com, desc, mdata) == RTW_HAL_STATUS_SUCCESS)
				return RTW_HAL_STATUS_SUCCESS;
		}
#endif
				...
				struct rtw_r_meta_data *mdata)
{
	...
	do {
		...
		if (!halmac_rx) {
			...
			rtw_hal_mac_patch_rx_rate(hal, mdata);
		} else {
