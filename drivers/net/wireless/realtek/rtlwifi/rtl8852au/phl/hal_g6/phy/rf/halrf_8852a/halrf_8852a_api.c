...
bool halrf_wl_tx_power_control_8852a(struct rf_info *rf, u32 tx_power_val)
{
	...
	if (pwr->coex_pwr_ctl_enable == false && pwr->dpk_pwr_ctl_enable == false) {
		...
		result = halrf_mac_set_pwr_reg_8852a(rf, phy, 0xd200, 0xfffffc00, 0x0);
		...
		/*all-time control*/
		result = halrf_mac_set_pwr_reg_8852a(rf, phy, 0xd200, 0xfffffc00, ((tmp_pwr & 0x1ff) | BIT(9)));
		if (result) {
			RF_WARNING("=======>%s Set MAC(0xd200) fail, error code=%d\n",
				__func__, result);
			return false;
	...
	if (gnt_bt_control == 0xffff) {
		...
		result = halrf_mac_set_pwr_reg_8852a(rf, phy, 0xd220, 0xfffff007, 0x0);
		if (result) {
		...
		result = halrf_mac_set_pwr_reg_8852a(rf, phy, 0xd220, 0xfffff007, ((gnt_bt_control & 0x1ff) << 3));
		if (result) {
			RF_WARNING("=======>%s Set MAC(0xd220) fail, error code=%d\n",
				__func__, result);
			return false;
