...
static void _phl_rxsc_cache_entry(struct rtw_phl_com_t *phl_com,
								struct phl_tid_ampdu_rx *r,
...
static bool _phl_rxsc_cache_check(struct rtw_phl_com_t *phl_com,
									struct rtw_phl_rx_pkt *phl_rx,
									struct rtw_r_meta_data *meta)
{
	struct rtw_rxsc_cache_entry *rxsc_entry = &phl_com->rxsc_entry;
										...
										_os_list *frames)
{
	...
	if (mpdu_seq_num == r->head_seq_num &&
		...
		r->head_seq_num = seq_inc(r->head_seq_num);
		#ifdef PHL_RXSC_AMPDU
		_phl_rxsc_cache_entry(phl_info->phl_com, r, meta);
		#endif
		return false;
	}
                                   ...
                                   _os_list *frames)
{
	...
	#ifdef PHL_RXSC_AMPDU
	if (_phl_rxsc_cache_check(phl_info->phl_com, phl_rx, meta))
		goto dont_reorder;
	#endif
