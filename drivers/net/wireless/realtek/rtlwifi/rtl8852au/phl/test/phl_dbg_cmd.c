...
void _dump_wifi_role(struct phl_info_t *phl_info, char *output, u32 out_len)
{
	...
	for( j = 0; j < MAX_WIFI_ROLE_NUMBER; j++) {
		...
		PHL_DBG_MON_INFO(out_len, used, output + used,
				 out_len - used, "wrole->chandef.bw = %u \n",
				 wrole->chandef.bw);
		PHL_DBG_MON_INFO(out_len, used, output + used,
				 out_len - used, "wrole->chandef.band = %u \n",
				 wrole->chandef.band);
		PHL_DBG_MON_INFO(out_len, used, output + used,
				 out_len - used, "wrole->chandef.center_ch = %u \n",
				 wrole->chandef.center_ch);
		PHL_DBG_MON_INFO(out_len, used, output + used,
				 out_len - used, "wrole->chandef.chan = %u \n",
				 wrole->chandef.chan);
		PHL_DBG_MON_INFO(out_len, used, output + used,
		...
		PHL_DBG_MON_INFO(out_len, used, output + used,
				 out_len - used, "wrole->chandef.offset = %u \n",
...
void _dump_mr_info(struct phl_info_t *phl_info, char *output, u32 out_len)
{
	...
	for (i = 0; i < MAX_BAND_NUM; i++) {
		...
		if (band_ctrl->chctx_band_map & BIT(CC_BAND_5G_B4))
			PHL_DBG_MON_INFO(out_len, used, output + used, out_len - used,
				" %s ", "B_5G_B4\n");
		if (band_ctrl->chctx_band_map & BIT(CC_BAND_6G))
			PHL_DBG_MON_INFO(out_len, used, output + used, out_len - used,
				" %s ", "B_6G\n");
		      ...
		      u32 input_num, char *output, u32 out_len)
{
	...
	if (1 == ctrl) {
		...
		phl_list_for_loop_safe(psta, n, struct rtw_phl_stainfo_t,
			...
			PHL_DBG_MON_INFO(out_len, used, output + used,
				out_len - used, "WROLE-IDX:%d wlan_mode:0x%02x, chan:%d, bw:%d, link_state:%s\n",
				psta->wrole->id,
				psta->wmode,
				psta->chandef.chan,
				psta->chandef.bw,
				wrole->mstate?((wrole->mstate & MLME_LINKING)?"Linking":"Linked Up"):"No Link");
