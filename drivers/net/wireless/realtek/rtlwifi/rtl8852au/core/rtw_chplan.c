...
{
#if defined(PHL_FEATURE_NIC) && !USE_RTW_REGD
	if (rtw_phl_valid_regulation_domain_6ghz(id))
		return _TRUE;
	else
		return _FALSE;
#else
...
static u8 init_channel_set_from_rtk_priv(_adapter *adapter, RT_CHANNEL_INFO *channel_set)
{
	...
	bool is_2g_5g_null = 0; /* removed after RTW_CHPLAN_NULL is defined officially */
	...
	if (IS_ALPHA2_WORLDWIDE(rfctl->alpha2) || IS_ALPHA2_UNSPEC(rfctl->alpha2)) {
		struct rtw_regulatory_domain domain = {.domain = rfctl->ChannelPlan, .domain_6g = RTW_CHPLAN_6G_NULL};

		is_2g_5g_null = rfctl->ChannelPlan == RTW_CHPLAN_NULL;
		#if CONFIG_IEEE80211_BAND_6GHZ
		domain.domain_6g = rfctl->chplan_6g;
		#endif
		if (rtw_phl_regulation_set_domain_ex(GET_PHL_INFO(dvobj), &domain, REGU_RSN_DEFAULT) != true) {
			RTW_WARN("rtw_phl_regulation_set_domain_ex() != true\n");
			goto exit;
		}
	} else {
	...
	for (i = 0; i < plan.cnt; i++) {
		...
		if (is_2g_5g_null && (plan.ch[i].band == BAND_ON_24G || plan.ch[i].band == BAND_ON_5G))
			continue;
	...
	#if defined(PHL_FEATURE_NIC) && !USE_RTW_REGD
{
	...
	if (rtw_phl_query_country_chplan(code, &plan)) {
		...
		#if CONFIG_IEEE80211_BAND_6GHZ
		phl_ent.chplan_6g = plan.domain_code_6g;
		#endif
