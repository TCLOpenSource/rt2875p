...
static u8 _rtw_dbcc_proto_hdl(void *drv_priv, u8 wr_idx, void *data)
{
	...
	if (!rtw_hw_is_mimo_support(adapter_to_dvobj(adapter))) {
	...
	else if (MLME_IS_AP(adapter) || MLME_IS_GO(adapter)) {
	}
	rst = _SUCCESS;
...
static void rtw_core_set_phl_ops(struct dvobj_priv *dvobj)
{
	...
	evt_ops->wow_handle_sec_info_update = rtw_core_wow_handle_sec_info_update;
...
void rtw_hw_update_chan_def(_adapter *adapter)
{
	...
	/*update chan_def*/
	adapter->phl_role->chandef.band = rtw_phl_get_band_type(mlmeext->chandef.chan);
	...
	adapter->phl_role->chandef.center_ch = rtw_phl_get_center_ch(&mlmeext->chandef);
...
u8 rtw_hw_iface_type_change(_adapter *adapter, u8 iface_type)
{
	...
	if (rtype == PHL_RTYPE_AP) {
		...
		if (sta == NULL) {
			sta = rtw_alloc_stainfo(&adapter->stapriv, adapter_mac_addr(adapter), PHL_CMD_DIRECTLY);
			if (sta == NULL) {
 ...
 */
void rtw_update_phl_sta_edca(struct _ADAPTER *a, enum rtw_ac ac, u32 param)
{
	struct sta_info *sta;
	struct protocol_cap_t *cap;
	u8 *mac;
...
int rtw_hw_connected(struct _ADAPTER *a, struct sta_info *sta)
{
	...
	rtw_join_done_chk_ch(a, 1);
	rtw_phl_connected(phl, a->connect_bidx, a->phl_role, sta->phl_sta);
#ifdef CONFIG_80211AX_HE
