...
void WMMOnAssocRsp(_adapter *padapter)
{
	...
	if (pmlmeinfo->WMM_enable == 0) {
		...
		rtw_update_phl_sta_edca(padapter, RTW_AC_BE, acParm);
		rtw_hw_set_edca(padapter, 1, acParm);
		rtw_update_phl_sta_edca(padapter, RTW_AC_BK, acParm);
		rtw_hw_set_edca(padapter, 2, acParm);
		rtw_update_phl_sta_edca(padapter, RTW_AC_VI, acParm);
		...
		rtw_update_phl_sta_edca(padapter, RTW_AC_VO, acParm);
	} else {
		...
		for (i = 0; i < 4; i++) {
			...
			rtw_update_phl_sta_edca(padapter, ACI, acParm);
...
void process_csa_ie(_adapter *padapter, u8 *ies, uint ies_len)
{
	...
	if (csa_ch != 0) {
		...
		rfctl->csa_chandef.band = rtw_phl_get_band_type(csa_ch);
		rfctl->csa_chandef.chan = csa_ch;
		rfctl->csa_chandef.offset = csa_offset;
		rfctl->csa_chandef.center_freq1= seg_0;
		rfctl->csa_chandef.center_freq2 = seg_1;
...
u8 rtw_get_ch_utilization(_adapter *adapter)
{
	...
	rtw_phl_get_env_rpt(phl, &rpt, adapter->phl_role);
