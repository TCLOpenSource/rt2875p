...
#define KEEP_ALIVE_TRYCNT (3)
	...
	if (chk_alive_num) {
		...
		#ifdef CONFIG_ACTIVE_KEEP_ALIVE_CHECK
		if (pmlmeext->active_keep_alive_check) {
			if (rtw_phl_mr_get_chandef(dvobj->phl, padapter->phl_role, &union_chdef)
			...
			rtw_get_oper_chdef(padapter, &hw_chdef);
			if (switch_channel_by_drv == _TRUE && hw_chdef.chan != pmlmeext->chandef.chan) {
				...
				set_channel_bwmode(padapter, union_chdef.chan, union_chdef.offset, union_chdef.bw, _FALSE);
			}
		}
		#endif /* CONFIG_ACTIVE_KEEP_ALIVE_CHECK */
		...
		if (pmlmeext->active_keep_alive_check) {
			...
			if (switch_channel_by_drv == _TRUE && bk_chdef.chan > 0)
				set_channel_bwmode(padapter, bk_chdef.chan, bk_chdef.offset, bk_chdef.bw, _FALSE);
		}
		#endif
	}
...
void rtw_core_ap_swch_start(_adapter *padapter, struct createbss_parm *parm)
{
	...
	for (i = 0; i < pdvobj->iface_nums; i++) {
		...
		rtw_cfg80211_ch_switch_notify(pdvobj->padapters[i]
			, &pdvobj->padapters[i]->mlmeextpriv.chandef
			, ht_option, 0);
	}
...
void rtw_core_ap_start(_adapter *padapter, struct createbss_parm *parm)
{
	...
	if (parm->is_change_chbw)
		rtw_phl_cmd_stop_beacon(adapter_to_dvobj(padapter)->phl,
					padapter->phl_role,
					_FALSE,
					PHL_CMD_DIRECTLY, 0);
...
chbw_decision:
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		_rtw_memcpy(&(iface->mlmepriv.cur_network.network)
			, &(iface->mlmeextpriv.mlmext_info.network)
			, iface->mlmeextpriv.mlmext_info.network.Length);

		rtw_start_bss_hdl_after_chbw_decided(iface);

		/* Set EDCA param reg after update cur_wireless_mode & update_capinfo */
		if (pregpriv->wifi_spec == 1)
			rtw_set_hw_wmm_param(iface);
	}
	...
	if (parm->is_change_chbw) {
		rtw_phl_cmd_stop_beacon(dvobj->phl,
					padapter->phl_role,
					_TRUE,
					PHL_CMD_DIRECTLY, 0);

		rtw_phl_chanctx_del(dvobj->phl, padapter->phl_role, NULL);
	}
	...
	RTW_INFO("PHL- CH:%d, BW:%d OFF:%d\n", new_chdef.chan, new_chdef.bw, new_chdef.offset);
	if (chdef_to_set.chan != new_chdef.chan) {
...
int rtw_sta_flush(_adapter *padapter, bool enqueue)
{
	...
	_rtw_spinunlock_bh(&pstapriv->asoc_list_lock);
       ...
       _rtw_spinlock_bh(&pstapriv->add_sta_list_lock);

       phead = &pstapriv->add_sta_list;
       plist = get_next(phead);
       while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
               psta = LIST_CONTAINOR(plist, struct sta_info, add_sta_list);
               plist = get_next(plist);

               rtw_list_delete(&psta->add_sta_list);
               pstapriv->add_sta_list_cnt--;
       }

       _rtw_spinunlock_bh(&pstapriv->add_sta_list_lock);
#endif
	...
	, const char *caller)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		mlmeext->chandef.bw = dec_bw[i];
		mlmeext->chandef.offset = dec_offset[i];
		mlmeext->chandef.band = rtw_phl_get_band_type(mlmeext->chandef.chan);
	...
	, s16 req_ch, s8 req_bw, s8 req_offset, struct rtw_chan_def *chdef)
{
...
#ifdef CONFIG_DBCC_SUPPORT
	enum phl_band_idx hw_band = adapter->phl_role->hw_band;
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (req_ch == 0) {
			...
			#ifdef CONFIG_DBCC_SUPPORT
			dec_hwband[i] = iface->phl_role->hw_band;
			#endif
		}
	}
	...
	if (MSTATE_STA_LD_NUM(&mstate_others) || MSTATE_STA_LG_NUM(&mstate_others)
		...
		rtw_warn_on(!rtw_mi_get_ch_setting_union_by_ifbmp(adapter, ifbmp_others, &u_ch, &u_bw, &u_offset));
...
static enum phl_mdl_ret_code _ap_start_req_acquired(void *dispr, void *priv)
{
	...
	padapter->ap_start_cmd_bidx = role->hw_band;
...
static enum phl_mdl_ret_code _ap_stop_req_acquired(void *dispr, void *priv)
{
	...
	padapter->ap_stop_cmd_bidx = role->hw_band;
...
static enum phl_mdl_ret_code _ap_stop_req_ev_hdlr(void *dispr, void *priv, struct phl_msg* msg)
{
	...
	if ((MSG_MDL_ID_FIELD(msg->msg_id) != PHL_FG_MDL_AP_STOP) &&
		...
		RTW_INFO(FUNC_ADPT_FMT ": Message is not from ap stop/mrc module, "
			 "skip msg_id=0x%08x\n", FUNC_ADPT_ARG(padapter), msg->msg_id);
		RTW_DBG(FUNC_ADPT_FMT ": -\n", FUNC_ADPT_ARG(padapter));
		return MDL_RET_IGNORE;
	}
	...
	switch(MSG_EVT_ID_FIELD(msg->msg_id)) {
		...
		case MSG_EVT_AP_STOP:
		{
			...
			RTW_DBG(FUNC_ADPT_FMT ": MSG_EVT_AP_STOP\n", FUNC_ADPT_ARG(padapter));
			stop_ap_hdl(padapter);
			rtw_phl_disconnected_resume_hdlr(GET_PHL_INFO(dvobj),
									wifi_role);
...
static void _ap_add_del_sta_cmd_done(struct _ADAPTER *padapter)
{
	...
	status = rtw_phl_free_cmd_token(GET_PHL_INFO(d),
					role->hw_band, &padapter->ap_add_del_sta_cmd_token);

	padapter->ap_add_del_sta_cmd_token = 0;
	padapter->ap_add_del_sta_cmd_state = 0;
...
static struct sta_info *rtw_ap_get_stainfo_by_add_sta_list(struct _ADAPTER *padapter)
{
	struct sta_priv *pstapriv = &padapter->stapriv;
	struct sta_info *psta = NULL;
	_list	*phead, *plist;
...
static enum phl_mdl_ret_code _ap_add_del_sta_req_acquired(void *dispr, void *priv)
{
	...
	psta = rtw_ap_get_stainfo_by_add_sta_list(padapter);
	...
	if (psta->is_add_sta)
		status = rtw_phl_connect_prepare(adapter_to_dvobj(padapter)->phl,
				role->hw_band, padapter->phl_role, psta->phl_sta->mac_addr);
	else
		status = rtw_phl_disconnect(adapter_to_dvobj(padapter)->phl,
				role->hw_band, padapter->phl_role, _TRUE);
...
static enum phl_mdl_ret_code _ap_add_del_sta_req_ev_hdlr(void *dispr, void *priv, struct phl_msg* msg)
{
	...
	case MSG_EVT_CONNECT_START:
		...
		RTW_INFO(FUNC_ADPT_FMT" add_sta_list, cnt:%u\n"
			, FUNC_ADPT_ARG(padapter), pstapriv->add_sta_list_cnt);

		psta = rtw_ap_get_stainfo_by_add_sta_list(padapter);

		if (!psta) {
			RTW_ERR("psta is null\n");
			_rtw_spinunlock_bh(&pstapriv->add_sta_list_lock);
			goto free_token;
		}
		...
		RTW_INFO(FUNC_ADPT_FMT ": MSG_EVT_CONNECT_START\n", FUNC_ADPT_ARG(padapter));
		...
		status = rtw_phl_connected(adapter_to_dvobj(padapter)->phl,
					padapter->phl_role->hw_band, padapter->phl_role, psta->phl_sta);
	...
	case MSG_EVT_DISCONNECT_PREPARE:
		...
		_rtw_spinunlock_bh(&pstapriv->add_sta_list_lock);
		RTW_INFO(FUNC_ADPT_FMT ": MSG_EVT_DISCONNECT_PREPARE\n", FUNC_ADPT_ARG(padapter));
		...
		status = rtw_phl_disconnect(adapter_to_dvobj(padapter)->phl,
					padapter->phl_role->hw_band, padapter->phl_role, _FALSE);
		if (status == RTW_PHL_STATUS_FAILURE) {
...
static void rtw_cmd_ap_add_del_sta_req_init(struct _ADAPTER *padapter, struct phl_cmd_token_req *fgreq)
{
	// struct _ADAPTER *padapter = pcmd->padapter;

	RTW_DBG(FUNC_ADPT_FMT ": \n", FUNC_ADPT_ARG(padapter));
	/* Fill foreground command request */
	fgreq->module_id= PHL_FG_MDL_AP_ADD_DEL_STA;
	fgreq->priv = padapter;
	fgreq->role = padapter->phl_role;

	fgreq->acquired = _ap_add_del_sta_req_acquired;
	fgreq->abort = _ap_add_del_sta_req_abort;
	fgreq->msg_hdlr = _ap_add_del_sta_req_ev_hdlr;
	fgreq->set_info = _ap_add_del_sta_req_set_info;
	fgreq->query_info = _ap_add_del_sta_req_query_info;
...
enum rtw_phl_status rtw_ap_add_del_sta_cmd(struct _ADAPTER *padapter)
{
	...
	rtw_cmd_ap_add_del_sta_req_init(padapter, &fgreq);

	/* cmd_dispatcher would copy whole phl_cmd_token_req */
	status = rtw_phl_add_cmd_token_req(d->phl, role->hw_band, &fgreq, &padapter->ap_add_del_sta_cmd_token);
	if((status != RTW_PHL_STATUS_SUCCESS) &&
	   (status != RTW_PHL_STATUS_PENDING))
		goto error;
...
u32 rtw_add_del_sta_cmd_check(struct _ADAPTER *padapter, unsigned char *MacAddr, bool add_sta)
{
	...
	if (psta) {
		_rtw_spinlock_bh(&pstapriv->add_sta_list_lock);
		if (rtw_is_list_empty(&psta->add_sta_list)) {
			rtw_list_insert_tail(&psta->add_sta_list, &pstapriv->add_sta_list);
			pstapriv->add_sta_list_cnt++;
			psta->is_add_sta = add_sta;
		}
		_rtw_spinunlock_bh(&pstapriv->add_sta_list_lock);
	}

	if (pstapriv->add_sta_list_cnt == 1) {
