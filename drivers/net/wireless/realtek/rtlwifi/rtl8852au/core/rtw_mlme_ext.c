...
_continue:
	...
	if (check_fwstate(pmlmepriv, WIFI_ASOC_STATE) == _TRUE &&
		...
		if (psta == NULL) {
			...
			psta = rtw_alloc_stainfo_sw(pstapriv, get_sa(pframe));
			if (psta == NULL) {
				/* TODO: */
		...
		if (((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) && (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS)) {
			...
			if (psta != NULL) {
				...
				*/
				...
				psta = rtw_alloc_stainfo_sw(pstapriv, get_addr2_ptr(pframe));
				if (psta == NULL) {
		...
		if (psta != NULL) {
			...
			if (!from_timer && pmlmeext->active_keep_alive_check && (rx_chk == _FAIL || tx_chk == _FAIL)
				...
				if (switch_channel_by_drv) {
					...
					if (hw_chdef.chan != pmlmeext->chandef.chan) {
...
static int rtw_auto_ap_start_beacon(_adapter *adapter)
{
	...
	*(u16 *)ie |= cpu_to_le16(cap_ShortPremble);
	...
	if (rtw_mi_check_status(adapter, MI_LINKED)) {
		if (rtw_phl_mr_get_chandef(dvobj->phl, adapter->phl_role, &u_chdef)
							!= RTW_PHL_STATUS_SUCCESS) {
		...
		oper_channel = adapter->mlmeextpriv.chandef.chan;
	}
...
void update_join_info(struct _ADAPTER *a, struct _WLAN_BSSID_EX *pbuf)
{
	...
	pmlmeext->chandef.band = rtw_phl_get_band_type(pmlmeext->chandef.chan);
...
#define DUMP_ADAPTERS_STATUS 0
	...
	if (join_res >= 0) { /*client join success - restart all ap*/
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			if ((MLME_IS_AP(iface) || MLME_IS_MESH(iface))
				...
				mlmeext->chandef.chan = u_chdef.chan;
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			if ((MLME_IS_AP(iface) || MLME_IS_MESH(iface))
				...
				_rtw_memset(&new_chdef, 0, sizeof(struct rtw_chan_def));
				new_chdef.chan = mlmeext->chandef.chan;
				new_chdef.bw = mlmeext->chandef.bw;
				new_chdef.offset = mlmeext->chandef.offset;
	...
	RTW_INFO("[MI] mr chandef:%u,%u,%u\n", u_chdef.chan, u_chdef.bw, u_chdef.offset);

	set_channel_bwmode(adapter, new_chdef.chan, new_chdef.offset, new_chdef.bw, _FALSE);
...
u8 tdls_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	if (wrole->type != PHL_RTYPE_TDLS) {
		status = rtw_phl_cmd_wrole_change(phl, wrole, WR_CHG_TYPE, (u8*)&rtype, sizeof(enum role_type), PHL_CMD_DIRECTLY, 0);
