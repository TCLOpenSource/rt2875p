...
void rtw_mlme_reset_auto_scan_int(_adapter *adapter, u8 *reason)
{
	...
	*reason = RTW_AUTO_SCAN_REASON_UNSPECIFIED;
	...
	if (rtw_phl_mr_get_chandef(dvobj->phl, adapter->phl_role, &union_chdef)
...
static enum phl_mdl_ret_code _connect_acquired(void* dispr, void *priv)
{
	...
	a->connect_bidx = a->phl_role->hw_band;
	_rtw_spinunlock(&a->connect_st_lock);
					       ...
					       struct phl_msg* msg)
{
	...
	case MSG_EVT_CONNECT_START:
		...
		rtw_free_stainfo(a, sta);
		sta = rtw_alloc_stainfo(&a->stapriv, network->MacAddress, PHL_CMD_DIRECTLY);
		if (sta == NULL) {
		...
		chdef.chan = a->mlmeextpriv.chandef.chan;
		chdef.bw = a->mlmeextpriv.chandef.bw;
		chdef.offset = a->mlmeextpriv.chandef.offset;
		chdef.band = rtw_phl_get_band_type(chdef.chan);
		if (!rtw_phl_chanctx_add(GET_PHL_INFO(d), role, &chdef, &mr_cc_info))
			goto send_disconnect;
	...
	case MSG_EVT_SWCH_START:
		...
		if (rtw_phl_mr_get_chandef(d->phl, a->phl_role, &chdef)
							!= RTW_PHL_STATUS_SUCCESS) {
		...
		set_channel_bwmode(a, chdef.chan, chdef.offset, chdef.bw, _TRUE);
				    ...
				    struct _WLAN_BSSID_EX *network)
{
	...
	if (!rtw_phl_chanctx_chk(GET_PHL_INFO(d), role, chdef, &mr_cc_info)) {
		#ifdef CONFIG_DBCC_SUPPORT
		if (mr_cc_info.sugg_opmode == MR_OP_DBCC) {
...
static enum phl_mdl_ret_code _disconnect_acquired(void* dispr, void *priv)
{
	...
	RTW_DBG(FUNC_ADPT_FMT ": +\n", FUNC_ADPT_ARG(a));
	a->disconnect_bidx = a->phl_role->hw_band;
