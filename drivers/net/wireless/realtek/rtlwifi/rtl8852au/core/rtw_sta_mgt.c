...
void _rtw_init_stainfo(struct sta_info *psta)
{
	...
	_rtw_init_listhead(&psta->add_sta_list);
	psta->is_add_sta = _FALSE;
#endif
...
u32	_rtw_init_sta_priv(struct	sta_priv *pstapriv)
{
	...
	_rtw_spinlock_init(&pstapriv->add_sta_list_lock);
...
#ifdef CONFIG_AP_CMD_DISPR
	_rtw_spinlock_free(&pstapriv->add_sta_list_lock);
#endif
#endif
...
static void _rtw_alloc_phl_stainfo(struct sta_info *sta, struct	sta_priv *stapriv,
								const u8 *hwaddr, enum phl_cmd_type cmd_type)
{
...
struct sta_info *rtw_alloc_stainfo(struct	sta_priv *stapriv, const u8 *hwaddr, enum phl_cmd_type cmd_type)
{
	...
	_rtw_alloc_phl_stainfo(sta, stapriv, hwaddr, cmd_type);
...
struct sta_info *rtw_get_stainfo(struct sta_priv *pstapriv, const u8 *hwaddr)
{
	...
	if (IS_MCAST(hwaddr)) {
		...
		*/
		...
		break;
		case PHL_RTYPE_TDLS:
			if (_rtw_memcmp(hwaddr, pstapriv->padapter->mlmepriv.cur_network.network.MacAddress, ETH_ALEN) == _TRUE)
				index = wifi_mac_hash(phl_role->mac_addr);
			else
				index = wifi_mac_hash(hwaddr);
		break;
