...
ssize_t proc_set_mr_test(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		if (mode == 1) {
			...
			rst = rtw_phl_chanctx_test(dvobj->phl, padapter->phl_role, test_mode, &new_chdef, &mr_cc_info);
			...
			u8 new_port = param;
			u8 org_port = padapter->phl_role->hw_port;

			rtw_phl_wifi_role_realloc_port(dvobj->phl, padapter->phl_role, new_port);
