...
int rtw_mi_get_ch_setting_union_by_ifbmp(_adapter *adapter, u8 ifbmp, u8 *ch, u8 *bw, u8 *offset)
{
...
{
	return rtw_mi_get_ch_setting_union_by_ifbmp(adapter, 0xFF, ch, bw, offset);
}

inline int rtw_mi_get_ch_setting_union_no_self(_adapter *adapter, u8 *ch, u8 *bw, u8 *offset)
{
	return rtw_mi_get_ch_setting_union_by_ifbmp(adapter, 0xFF & ~BIT(adapter->iface_id), ch, bw, offset);
}
...
{
	return rtw_mi_status_by_ifbmp(adapter, 0xFF, mstate);
}

inline void rtw_mi_status_no_self(_adapter *adapter, struct mi_state *mstate)
{
	return rtw_mi_status_by_ifbmp(adapter, 0xFF & ~BIT(adapter->iface_id), mstate);
}

inline void rtw_mi_status_no_others(_adapter *adapter, struct mi_state *mstate)
{
	return rtw_mi_status_by_ifbmp(adapter, BIT(adapter->iface_id), mstate);
}
...
inline void rtw_mi_update_iface_status(struct mlme_priv *pmlmepriv, sint state)
{
	...
	if (rtw_is_adapter_up(adapter))
		hw_band_idx = adapter->phl_role->hw_band;
...
#endif
 	iface_state = &dvobj->iface_state[adapter->phl_role->hw_band];
...
_adapter *rtw_get_iface_by_macddr(_adapter *padapter, const u8 *mac_addr)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (_rtw_memcmp(mac_addr, iface_mac_addr, ETH_ALEN)) {
			bmatch = _TRUE;
			break;
		}
	}
...
bool rtw_iface_at_same_hwband(_adapter *adapter, _adapter *iface)
{
	...
	if (rtw_is_adapter_up(adapter) && rtw_is_adapter_up(iface))
		rst = (adapter->phl_role->hw_band == iface->phl_role->hw_band) ? _TRUE : _FALSE;
	#endif
	return rst;
}

u8 rtw_mi_get_hw_port(_adapter *adapter)
{
	if (rtw_is_adapter_up(adapter))
		return adapter->phl_role->hw_port;
	return 0xFF;
}