...
static int retriveFromFile(const char *path, u8 *buf, u32 sz)
{
	...
	char* const delim = "/";
	char *name = NULL, *token, *cur, path_tmp[1024] = {0};

	if (path && buf) {
		if (strlen(path) > 1024) {
			RTW_INFO("%s() length of path is too long, length : %ld\n", __func__, strlen(path));
			ret = -EINVAL;
			goto RETURN;
		}
		...
		if (ret == 0) {
			if (fw->size > sz) {
				RTW_INFO("%s() file size is too large, size : %zu\n", __func__, fw->size);
				ret = -EINVAL;
				goto RETURN;
			}
