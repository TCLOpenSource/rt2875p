...
int rtw_suspend_wow(_adapter *padapter)
{
	...
	if (pwrpriv->wowlan_mode == _TRUE) {
		...
		if (rtw_mi_check_status(padapter, MI_LINKED)) {
			...
			rtw_phl_mr_get_chandef(dvobj->phl, padapter->phl_role, &union_chdef);
			RTW_INFO(FUNC_ADPT_FMT" back to linked/linking union - band:%u ch:%u, bw:%u, offset:%u\n",
				 FUNC_ADPT_ARG(padapter), union_chdef.band, union_chdef.chan, union_chdef.bw, union_chdef.offset);
