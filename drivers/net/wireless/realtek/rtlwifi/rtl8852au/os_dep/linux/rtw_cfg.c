...
/*0:dynamic ,1~7: force to role.id */
int rtw_dbcc_force_rid = 0;
module_param(rtw_dbcc_force_rid, int, 0644);
MODULE_PARM_DESC(rtw_dbcc_force_rid, "0:Dynamic, [1-7]:Force DBCC to Role Id");
#endif
#endif
...
void rtw_core_update_default_setting (struct dvobj_priv *dvobj)
{
	...
	phl_com->dev_sw_cap.fw_cap.offload_cap = true;
	#endif
...
#ifdef CONFIG_DBCC_FORCE
	if (rtw_dbcc_force_rid >= CONFIG_RTW_STATIC_NDEV_NUM) {
		RTW_ERR("rtw_dbcc_force_rid out of range\n");
		rtw_dbcc_force_rid = 0;
	}
	phl_com->dev_sw_cap.dbcc_force_rid = rtw_dbcc_force_rid;
#endif
#endif
