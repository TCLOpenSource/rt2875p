		...
		u8 ht)
{
	...
	freq = rtw_ch2freq(rtw_chdef->chan);
	if (!freq)
		goto exit;

	cfreq = rtw_phl_get_center_ch(rtw_chdef);
	if (!cfreq)
		goto exit;
...
static void rtw_get_chbw_from_cfg80211_chan_def(struct cfg80211_chan_def *chdef,
		u8 *ht, struct rtw_chan_def *rtw_chdef)
{
	...
	case NL80211_CHAN_WIDTH_40:
		*ht = 1;
		...
		rtw_chdef->offset = (pri_freq > chdef->center_freq1)
				? CHAN_OFFSET_LOWER
				: CHAN_OFFSET_UPPER;
		if (rtw_get_offset_by_chbw(chan->hw_value, rtw_chdef->bw, (u8 *)&rtw_chdef->offset))
			rtw_chdef->chan = chan->hw_value;
		break;
	case NL80211_CHAN_WIDTH_80:
		*ht = 1;
...
static enum nl80211_channel_type rtw_chbw_to_nl80211_channel_type(struct rtw_chan_def *chan_def, u8 ht)
{
	rtw_warn_on(!ht && (chan_def->bw >= CHANNEL_WIDTH_40 || chan_def->offset != CHAN_OFFSET_NO_EXT));

	if (!ht)
		return NL80211_CHAN_NO_HT;
	if (chan_def->bw >= CHANNEL_WIDTH_40) {
		if (chan_def->offset == CHAN_OFFSET_LOWER)
...
static void rtw_get_chbw_from_nl80211_channel_type(struct ieee80211_channel *chan,
			enum nl80211_channel_type ctype,
			u8 *ht, struct rtw_chan_def *chan_def)
{
	int pri_freq;
	...
	chan_def->band = nl80211_band_to_rtw_band(chan->band);
	chan_def->chan = chan->hw_value;
	...
	u8 ht, bool started)
{
	...
	ret = rtw_chbw_to_cfg80211_chan_def(wiphy, &chdef, rtw_chdef, ht);
	if (ret != _SUCCESS)
		goto exit;
...
#else
	int freq = rtw_ch2freq(rtw_chdef->chan);
	enum nl80211_channel_type ctype;
	...
	ctype = rtw_chbw_to_nl80211_channel_type(rtw_chdef, ht);
...
static void rtw_desc_rate_to_nss_mcs(u16 rate_idx, u8 *rate_mode, u8 *nss, u8 *mcs)
{
	...
	if ((RTW_DATA_RATE_MCS0 <= rate_idx) &&
		...
		*rate_mode = RTW_VHT_MODE;
		mcs_in = (rate_idx - RTW_DATA_RATE_VHT_NSS1_MCS0) % 10;
		nss_in = ((rate_idx - RTW_DATA_RATE_VHT_NSS1_MCS0) / 10) + 1;
	} else if ((RTW_DATA_RATE_HE_NSS1_MCS0 <= rate_idx) &&
		   (rate_idx <= RTW_DATA_RATE_HE_NSS4_MCS11)) {
		*rate_mode = RTW_HE_MODE;
		mcs_in = (rate_idx - RTW_DATA_RATE_HE_NSS1_MCS0) % 12;
		nss_in = ((rate_idx - RTW_DATA_RATE_HE_NSS1_MCS0) / 12) + 1;
	} else {
...
#endif
	RTW_INFO(FUNC_ADPT_FMT" ch:%d bw:%d, offset:%d\n",
		FUNC_ADPT_ARG(padapter), target_chdef.chan,
		target_chdef.bw, target_chdef.offset);

	rtw_set_chbw_cmd(padapter, target_chdef.chan, target_chdef.bw,
 ...
 */
	...
	if ((rtw_phl_mr_get_chandef(dvobj->phl, padapter->phl_role, &u_chdef) !=
		RTW_PHL_STATUS_SUCCESS) || (u_chdef.chan == 0)){
...
/* phl_scan will self return opch */
	...
	if (leave_op) {
		if (rtw_mi_check_status(padapter, MI_LINKED))
			set_channel_bwmode(padapter, u_chdef.chan, u_chdef.offset, u_chdef.bw, _FALSE);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 6, 0))
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0))
	rtw_get_chbw_from_cfg80211_chan_def(chdef, &ht, &rtw_chdef);
#else
	rtw_get_chbw_from_nl80211_channel_type(chan, setup->channel_type, &ht, &rtw_chdef);
#endif
	if (!rtw_chdef->chan)
		goto exit;
				...
				struct cfg80211_csa_settings *params)
{
	...
	rtw_get_chbw_from_cfg80211_chan_def(&params->chandef, &ht, &csa_chdef);
