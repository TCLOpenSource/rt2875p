...
static void hw_var_set_monitor(PADAPTER adapter, u8 variable, u8 *val)
{
	...
	mon->rxfilter0 = rtw_read16(adapter, REG_RXFLTMAP0_8188GTV);
	mon->rxfilter1 = rtw_read16(adapter, REG_RXFLTMAP1_8188GTV);
	mon->rxfilter2 = rtw_read16(adapter, REG_RXFLTMAP2_8188GTV);
	rtw_write16(adapter, REG_RXFLTMAP0_8188GTV, 0xFFFF);
	rtw_write16(adapter, REG_RXFLTMAP1_8188GTV, 0xFFFF);
	rtw_write16(adapter, REG_RXFLTMAP2_8188GTV, 0xFFFF);
...
static void hw_var_set_opmode(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	if (isMonitor == _TRUE) {
		...
		if (backup->known_rxfilter) {
			...
			rtw_write16(padapter, REG_RXFLTMAP0_8188GTV, backup->rxfilter0);
			rtw_write16(padapter, REG_RXFLTMAP1_8188GTV, backup->rxfilter1);
			rtw_write16(padapter, REG_RXFLTMAP2_8188GTV, backup->rxfilter2);
		}
#endif /* CONFIG_WIFI_MONITOR */
		isMonitor = _FALSE;
	}
