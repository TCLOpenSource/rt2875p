...
#ifdef CONFIG_PCIE_DMA_COHERENT
#ifdef CONFIG_RL6702_PCI
	pagebyte = len_2_pagebyte(sizeof(*txbd) * entries);
	dma = pli_malloc(pagebyte, GFP_CMA_PCIE);
	txbd = dvr_remap_uncached_memory(dma,pagebyte,__builtin_return_address(0));
	memset((void *)txbd,0,pagebyte);
#else
...
#ifdef CONFIG_PCIE_DMA_COHERENT
#ifdef CONFIG_RL6702_PCI
	dvr_unmap_memory((void *)ring->buf_desc,len_2_pagebyte(sizeof(*ring->buf_desc) * ring->entries)) ;
	pli_free(ring->dma);
#else
