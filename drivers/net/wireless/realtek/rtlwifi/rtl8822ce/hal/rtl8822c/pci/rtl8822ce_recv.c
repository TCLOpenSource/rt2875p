...
#ifdef CONFIG_PCIE_DMA_COHERENT
#ifdef CONFIG_RL6702_PCI
		pagebyte1 = len_2_pagebyte(sizeof(*r_priv->rx_ring[rx_queue_idx].buf_desc) * r_priv->rxringcount);
		r_priv->rx_ring[rx_queue_idx].dma =
			pli_malloc(pagebyte1,GFP_CMA_PCIE);
		r_priv->rx_ring[rx_queue_idx].buf_desc =
			dvr_remap_uncached_memory(
				r_priv->rx_ring[rx_queue_idx].dma,pagebyte1,
				__builtin_return_address(0));
		memset((void *)r_priv->rx_ring[rx_queue_idx].buf_desc,0,pagebyte1);
#else
		r_priv->rx_ring[rx_queue_idx].buf_desc =
...
#ifdef CONFIG_PCIE_DMA_COHERENT
#ifdef CONFIG_RL6702_PCI
			pagebyte2 = len_2_pagebyte(r_priv->rxbuffersize);
			*mapping = pli_malloc(pagebyte2, GFP_CMA_PCIE);
			r_priv->rx_ring[rx_queue_idx].vdma_bpa[i] =
				dvr_remap_uncached_memory(*mapping,pagebyte2,__builtin_return_address(0));
			memset((void *)r_priv->rx_ring[rx_queue_idx].vdma_bpa[i],0,pagebyte2);
#else
			r_priv->rx_ring[rx_queue_idx].vdma_bpa[i] =
				dma_alloc_coherent(&pdev->dev,
...
#ifdef CONFIG_RL6702_PCI
			dvr_unmap_memory(
				(void *)r_priv->rx_ring[rx_queue_idx].vdma_bpa[i],
				len_2_pagebyte(r_priv->rxbuffersize)) ;
			pli_free(*((dma_addr_t *) skb->cb));
#else
...
#ifdef CONFIG_RL6702_PCI
		dvr_unmap_memory(
			(void *)r_priv->rx_ring[rx_queue_idx].buf_desc,
			len_2_pagebyte(sizeof(*r_priv->rx_ring[rx_queue_idx].buf_desc) *
				r_priv->rxringcount)) ;
		pli_free(r_priv->rx_ring[rx_queue_idx].dma);
#else
