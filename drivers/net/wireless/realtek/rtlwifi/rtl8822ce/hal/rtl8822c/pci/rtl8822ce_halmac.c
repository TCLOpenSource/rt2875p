...
static u8 pci_write_port_not_xmitframe(void *d,  u32 size, dma_addr_t mapping,  u8 qsel)
{
	...
	txbd_dma = pli_malloc(len_2_pagebyte(sizeof(struct tx_buf_desc)), GFP_CMA_PCIE);
	txbd = dvr_remap_uncached_memory(txbd_dma, len_2_pagebyte(sizeof(struct tx_buf_desc)), __builtin_return_address(0));
#else
	...
	memset((void *)txbd,0,len_2_pagebyte(sizeof(struct tx_buf_desc)));
...
#ifdef CONFIG_PCIE_DMA_COHERENT
#ifdef CONFIG_RL6702_PCI
	dvr_unmap_memory((void *)txbd,len_2_pagebyte(sizeof(struct tx_buf_desc))) ;
	pli_free(txbd_dma);
#else
...
#ifdef CONFIG_PCIE_DMA_COHERENT
#ifdef CONFIG_RL6702_PCI
	mapping = pli_malloc(len_2_pagebyte(len), GFP_CMA_PCIE);
	buf = dvr_remap_uncached_memory(mapping, len_2_pagebyte(len), __builtin_return_address(0));
#else
...
#ifdef CONFIG_PCIE_DMA_COHERENT
#ifdef CONFIG_RL6702_PCI
	dvr_unmap_memory((void *)buf,len_2_pagebyte(len)) ;
	pli_free(mapping);
#else
