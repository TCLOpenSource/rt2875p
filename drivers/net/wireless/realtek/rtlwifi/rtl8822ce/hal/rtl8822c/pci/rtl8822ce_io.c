...
#elif defined (CONFIG_PLATFORM_ARM_RTD299X)
extern int rtk_pcie_send_mem_rd(u32 addr, int size, u32 *data);
extern int rtk_pcie_send_mem_wr(u32 addr, int size, u32 data);
static u8 _pci_read8_299x(struct intf_hdl *phdl, u32 addr)
{
	struct dvobj_priv *pdvobjpriv = (struct dvobj_priv *)phdl->pintf_dev;
	u32 data;

	//rtk_pcie_send_mem_rd(PCI_MEM_START+addr,1,&data);
	rtk_pcie_send_mem_rd(pdvobjpriv->pci_mem_start+addr,1,&data);
    return (u8)data;
}

static u16 _pci_read16_299x(struct intf_hdl *phdl, u32 addr)
{
	struct dvobj_priv *pdvobjpriv = (struct dvobj_priv *)phdl->pintf_dev;
	u32 data;

	//rtk_pcie_send_mem_rd(PCI_MEM_START+addr,2,&data);
	rtk_pcie_send_mem_rd(pdvobjpriv->pci_mem_start+addr,2,&data);
    return (u16)data;
}

static u32 _pci_read32_299x(struct intf_hdl *phdl, u32 addr)
{
	struct dvobj_priv *pdvobjpriv = (struct dvobj_priv *)phdl->pintf_dev;
	u32 data;

	//rtk_pcie_send_mem_rd(PCI_MEM_START+addr,4,&data);
	rtk_pcie_send_mem_rd(pdvobjpriv->pci_mem_start+addr,4,&data);
    return (u32)data;
}

static int _pci_write8_299x(struct intf_hdl *phdl, u32 addr, u8 val)
{
	struct dvobj_priv *pdvobjpriv = (struct dvobj_priv *)phdl->pintf_dev;
    u32 data = 0;
    data = (u32)val;

	//rtk_pcie_send_mem_wr(PCI_MEM_START+addr,1,data);
	rtk_pcie_send_mem_wr(pdvobjpriv->pci_mem_start+addr,1,data);
	return 1;
}

static int _pci_write16_299x(struct intf_hdl *phdl, u32 addr, u16 val)
{
	struct dvobj_priv *pdvobjpriv = (struct dvobj_priv *)phdl->pintf_dev;
	u32 data = 0;
    data = (u32)val;

	//rtk_pcie_send_mem_wr(PCI_MEM_START+addr,2,data);
	rtk_pcie_send_mem_wr(pdvobjpriv->pci_mem_start+addr,2,data);
	return 2;
}

static int _pci_write32_299x(struct intf_hdl *phdl, u32 addr, u32 val)
{
	struct dvobj_priv *pdvobjpriv = (struct dvobj_priv *)phdl->pintf_dev;
	u32 data = 0;
    data = (u32)val;

	//rtk_pcie_send_mem_wr(PCI_MEM_START+addr,4,data);
	rtk_pcie_send_mem_wr(pdvobjpriv->pci_mem_start+addr,4,data);
	return 4;
}

#if 0
static u8 pci_read8_299x(struct intf_hdl *phdl, u32 addr)
{
    u8  ofst = addr& 0x3;
    u32 v = _pci_read32_299x(phdl, addr - ofst);
    u32 ret = v >> (ofst<<3);  // little endian
    ret &= 0xFF;

	return ret;
}

static u16 pci_read16_299x(struct intf_hdl *phdl, u32 addr)
{
    u8  ofst = addr& 0x3;
    u32 v = _pci_read32_299x(phdl, addr - ofst);
    u32 ret = v >> (ofst<<3);
...
static u32 pci_read32_299x(struct intf_hdl *phdl, u32 addr)
{
    u32 ret  = _pci_read32_299x(phdl, addr);
    udelay(1000);

	return ret ;
}

static int pci_write8_299x(struct intf_hdl *phdl, u32 addr, u8 val)
{
    u8  ofst = addr& 0x3;
    u32 v = _pci_read32_299x(phdl, addr - ofst);
    u32 mask = 0xFF << (ofst<<3);
    u32 ret = v & ~mask;
    ret |= (val & 0xFF) << (ofst<<3);
    udelay(1000);
	_pci_write32_299x(phdl, addr- ofst, ret);

	return 1;
}

static int pci_write16_299x(struct intf_hdl *phdl, u32 addr, u16 val)
{
    u8  ofst = addr& 0x3;
    u32 v = _pci_read32_299x(phdl, addr - ofst);
    u32 mask = 0xFFFF << (ofst<<3);
    u32 ret = v & ~mask;
    ret |= (val & 0xFFFF) << (ofst<<3);
    udelay(1000);
	_pci_write32_299x(phdl, addr- ofst, ret);
...
static int pci_write32_299x(struct intf_hdl *phdl, u32 addr, u32 val)
{
	_pci_write32_299x(phdl, addr, val);
	return 4;
}
#endif
...
#elif defined (CONFIG_PLATFORM_ARM_RTD299X)
	pops->_read8  = &_pci_read8_299x;
	pops->_read16 = &_pci_read16_299x;
	pops->_read32 = &_pci_read32_299x;
	//pops->_read8  = &pci_read8_299x;
	//pops->_read16 = &pci_read16_299x;
	//pops->_read32 = &pci_read32_299x;
#else
...
#elif defined (CONFIG_PLATFORM_ARM_RTD299X)
	pops->_write8  = &_pci_write8_299x;
	pops->_write16 = &_pci_write16_299x;
	pops->_write32 = &_pci_write32_299x;
	//pops->_write8  = &pci_write8_299x;
	//pops->_write16 = &pci_write16_299x;
	//pops->_write32 = &pci_write32_299x;
#else
