...
#ifdef CONFIG_PCIE_DMA_COHERENT
#ifdef CONFIG_RL6702_PCI
		pagebyte = len_2_pagebyte(alloc_sz);
		pxmitbuf->dma_bpa = pli_malloc(pagebyte, GFP_CMA_PCIE);
		pxmitbuf->pallocated_buf =
			dvr_remap_uncached_memory(pxmitbuf->dma_bpa,
				pagebyte, __builtin_return_address(0));
		memset((void *)pxmitbuf->pallocated_buf,0,pagebyte);
#else
...
#ifdef CONFIG_PCIE_DMA_COHERENT
#ifdef CONFIG_RL6702_PCI
			dvr_unmap_memory((void *)pxmitbuf->pallocated_buf,len_2_pagebyte(free_sz)) ;
			pli_free(pxmitbuf->dma_bpa);
#else
