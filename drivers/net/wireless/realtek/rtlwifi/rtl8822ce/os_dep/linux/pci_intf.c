...
static struct dvobj_priv	*pci_dvobj_init(struct pci_dev *pdev, const struct pci_device_id *pdid)
{
	...
	RTW_INFO("RTK TV skip iomap\n");
	dvobj->pci_mem_start = (unsigned long)pmem_start;
#else
	/* shared mem start */
...
#ifndef CONFIG_PLATFORM_ARM_RTD299X
		/* def RTK_DMP_PLATFORM */
...
#define PCI_MEM_START   (0x78000000)
extern int rtk_pcie_set_mem_space_exp(u32 mem_space);
#endif
...
#ifdef CONFIG_RL6702_PCI
	RTW_INFO("%s(%d)RT6702 using CMA for PCIE\n",__func__,__LINE__);
	rtk_pcie_set_mem_space_exp(PCI_MEM_START);
#endif
