...
#include "input_event_intercept.h"
static u32 g_intercept_event_function_enable = 0;
        ...
        if(type == EV_KEY || type == EV_MSC) {
            if (is_event_supported(type, dev->evbit, EV_MAX)) {
                ...
                if (type == EV_KEY) {
                    ...
                    if (value != 2 && !!test_bit(code, dev->intercept_key) == !!value) {
                        spin_unlock_irqrestore(&dev->event_lock, flags);
                        return ;
                    }
                    if(value != 2)
                        __change_bit(code, dev->intercept_key);
                }
        ...
        if(input_dev) {
            if(type == EV_KEY) {
                if(value !=3 ){
                    ...
                    __change_bit(code, input_dev->key);
