...
void drv_osd_sr_1_lti_ctrl0(void)
{
	osd_sr_osd_sr_1_lti_ctrl0_RBUS reg_osd_sr_osd_sr_1_lti_ctrl0_reg;
	
	reg_osd_sr_osd_sr_1_lti_ctrl0_reg.regValue = rtd_inl( OSD_SR_OSD_SR_1_LTI_Ctrl0_reg );
	
	reg_osd_sr_osd_sr_1_lti_ctrl0_reg.sr_lti_ub = 0x40;
	reg_osd_sr_osd_sr_1_lti_ctrl0_reg.sr_lti_lb = 0;
	
	rtd_outl( OSD_SR_OSD_SR_1_LTI_Ctrl0_reg , reg_osd_sr_osd_sr_1_lti_ctrl0_reg.regValue );
}
...
void drv_superresolution_LPF_Gain(GDMA_DISPLAY_PLANE disPlane)
{
	...
	*(unsigned int *)&line_buffer_end_reg = rtd_inl(GDMA_line_buffer_end_reg);
	...
	osd_sr_ctrl_reg.sr_var_thd = 0xf0;
	...
	if (enable && (bypass_flag == 0)) {
		...
		*/
		...
		//	return -1;
			 ...
			 src_width = srcWin.width;
			 src_height = srcWin.height;
			 dis_width = dispWin.width;
			 dis_height = dispWin.height;
			 
			if (dis_width!= 0)
				hor_factor = (src_width<<20)/dis_width;
			if (dis_height != 0)
				ver_factor = (src_height<<20)/dis_height;
			...
			if ((srcWin.height < dispWin.height) && (ver_factor)) { /*  no scaling down */
				...
				if( ver_factor>0x80000 )
				{
					osd_sr_scaleup_ctrl0_reg.v_lti_en = 0;
					osd_sr_scaleup_ctrl0_reg.ver_tap_num = 0;
				}
				else
				if( ver_factor>1 )
				{
					if( srcWin.width<=1920 )
					{
						osd_sr_scaleup_ctrl0_reg.v_lti_en = 1;
						osd_sr_scaleup_ctrl0_reg.ver_tap_num = 0;
					}
					else
					if( srcWin.width<=3840 )
					{
						osd_sr_scaleup_ctrl0_reg.v_lti_en = 1;
			...
			if ((srcWin.width < dispWin.width) && (hor_factor)) { /*  no scaling down */
				...
				if( hor_factor>0x80000 )
				{
					osd_sr_scaleup_ctrl1_reg.h_lti_en = 0;
				}
				else
				if( hor_factor>1 )
				{
					osd_sr_scaleup_ctrl1_reg.h_lti_en = 1;
				}
				else
				{
					osd_sr_scaleup_ctrl1_reg.h_lti_en = 0;
				}
				osd_sr_scaleup_ctrl1_reg.hor_tap_num = 0;
				pReg->osd_sr_scaleup_ctrl1_reg.regValue = osd_sr_scaleup_ctrl1_reg.regValue;
			}
