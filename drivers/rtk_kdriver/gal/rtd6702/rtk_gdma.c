...
#endif
#if (defined(CONFIG_ARCH_RTK288O) || defined(CONFIG_ARCH_RTK2875) || defined(CONFIG_ARCH_RTK2885M) || defined(CONFIG_ARCH_RTK6702)) && defined(THRESHOLD_DETECT)
...
#define BOX_MAX_WIDTH_1920 1920
#define BOX_MAX_HEIGH_1080 1080
int box_mode_enable=0;
...
#define SE_BOX_MODE_ENABLE 0

unsigned char *dst_box=0;
unsigned char *dst_box_2=0;
unsigned int dst_order_box = 0;

int dst_box_buffer_enable=-1;
int dst_box_buffer_enable_pre=-1;
unsigned int dst_box_buffer_w=0;
unsigned int dst_box_buffer_h=0;


int GDMA_BOX_SE_draw_block(int buffer_w,int buffer_h,int box_output_w,int box_output_h,PICTURE_LAYERS_OBJECT *data,int SE_BOX_Enable) ;
...
#if defined(CONFIG_ARCH_RTK288O) || defined(CONFIG_ARCH_RTK2875) || defined(CONFIG_ARCH_RTK2885M) || defined(CONFIG_ARCH_RTK6702)
#else
...
#if defined(CONFIG_ARCH_RTK287O) || defined(CONFIG_ARCH_RTK288O) || defined(CONFIG_ARCH_RTK2875) || defined(CONFIG_ARCH_RTK2885M) || defined(CONFIG_ARCH_RTK6702)
#else
    ...
    case GDMA_GET_FENCE_INFO:
        {
            ...
            if(GDMA_GetFenceInfo(&data) == GDMA_SUCCESS){
                if (copy_to_user((void __user *)arg, &data, sizeof(GDMA_FENCE_INFO))) {
                        pr_debug(" GDMA_GET_GDMA_FEND_INFO ERROR *******\n");
                        ret = -EFAULT;
                }
            }else{
                ret = -EFAULT;
...
#if defined(CONFIG_ARCH_RTK288O) && defined(THRESHOLD_DETECT)
	unsigned int i;
#endif
	volatile int osd1_vend = 0, osd1_vend_3D = 0, osd_vend = 0;/* , i; */
	volatile int osd1_finish = 0;
	volatile int osd1_sync = 0;
#if !defined(CONFIG_ARCH_RTK2875) && !defined(CONFIG_ARCH_RTK2885M) && !defined(CONFIG_ARCH_RTK6702)
...
*/
...
#if defined(CONFIG_ARCH_RTK288O) || defined(CONFIG_ARCH_RTK2885M)
	...
	*/
...
#if (defined(CONFIG_ARCH_RTK288O) || defined(CONFIG_ARCH_RTK2875) || defined(CONFIG_ARCH_RTK2885M)) && defined(THRESHOLD_DETECT)
	...
	printk(KERN_NOTICE"[GDMA]get total memsize = %08lx\n", (unsigned long)gdma_total_ddr_size);

#if defined(CONFIG_ARCH_RTK288O)
	if (gdma_total_ddr_size) {
		rtd_outl(GDMA_AFBC_MIN_reg, 0x0);
		rtd_outl(GDMA_AFBC_MAX_reg, gdma_total_ddr_size);
	} else {
	...
	if (gdma_total_ddr_size) {
		...
		if(gdma_total_ddr_size > (unsigned long)0xFFFFFFFF)
			rtd_outl(GDMA_AFBC1_MAX_reg, 0xFFFFFFFF);
		else
			rtd_outl(GDMA_AFBC1_MAX_reg, gdma_total_ddr_size);
		rtd_outl(GDMA_AFBC2_MIN_reg, 0x0);
		if(gdma_total_ddr_size > (unsigned long)0xFFFFFFFF)
			rtd_outl(GDMA_AFBC2_MAX_reg, 0xFFFFFFFF);
		else
...
#if ESTICKER_778
			if ((change_OSD_mode==GDMA_SPLIT_MODE_0) && (picObj->width==778)) {
#else
...
//for ANDROIDTV-305 test
			if(saveBufAddr >= (unsigned long)gdma_total_ddr_size){
				dump_stack();
				printk(KERN_EMERG"%s, saveBufAddr=%x is over OSD_MAX_PHY_ADDR(%08lx) \n", __FUNCTION__,saveBufAddr,(unsigned long)gdma_total_ddr_size);
				GDMA_EXIT_CRITICAL();
				up(&gdma->sem_receive);
				return GDMA_FAIL;
			}
...
int GDMA_GetFenceInfo(GDMA_FENCE_INFO *getInfo)
{
...

    {
        ...
        if (new_gdma_fence)
        {
            ...
            //printk(KERN_EMERG"fence get %llu\n",getInfo->fence_handle);
        }else{
			getInfo->fence_fd = 0;
...
int GDMA_SignalGetReleaseFenceInfo(GDMA_FENCE_INFO *getInfo)
{
    ...
    {
        struct gdma_fence *new_gdma_fence = alloc_gdma_release_fence();
        if (new_gdma_fence)
        {
            getInfo->fence_fd = new_gdma_fence->gdma_fence_fd;
            getInfo->fence_handle = (uintptr_t) new_gdma_fence;
...
static int GDMA_SignalFence(struct gdma_fence *gf)
{
    ...
    if(ret != 0)
		printk(KERN_EMERG"GDMA Fence singnal fail !!\n");
    return ret;
}
...
int GDMA_ReceiveGraphicLayers(GRAPHIC_LAYERS_OBJECT *data)
{
	...
	*/
	...
	if (gdmaLayerNum > 0){
#if SE_BOX_MODE_ENABLE
		if(dst_box_buffer_enable == 1){
			GDMA_BOX_SE_draw_block(BOX_MAX_WIDTH_1920,BOX_MAX_HEIGH_1080,dst_box_buffer_w,
				dst_box_buffer_h,picLayerObj,dst_box_buffer_enable);
		}
		else{
			GDMA_ReceivePicture(sendGdmaLayerObj);
		} 
		if(dst_box_buffer_enable == 0 && dst_box_buffer_enable_pre ==1){
			GDMA_BOX_SE_draw_block(BOX_MAX_WIDTH_1920,BOX_MAX_HEIGH_1080,dst_box_buffer_w,
				dst_box_buffer_h,picLayerObj,dst_box_buffer_enable);
			dst_box_buffer_enable_pre = 0;
		}
#else
		err = GDMA_ReceivePicture(sendGdmaLayerObj);
#endif
    ...
    //else printk(KERN_EMERG"gdma magic key FAIL! (0x%8x)?\n", data->gdma_fence_magic_key);
...
void GDMA_Clk_Select(int src_type, bool enable)
{
...
#endif
#else
	 //if((sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f2p_gate_sel == 1) && (disp_res.width < FOURK_W))//1 : 60hz	0:120Hz
		if (pll_info_bush() > pll_info_disp())
#endif
	...
	printk(KERN_ERR"[%s]clk pll_info_bush = %08x, pll_info_disp = %08x\n", __func__, pll_info_bush(),pll_info_disp());
...
int GDMA_ConfigSEBoxEnable(unsigned int output_w, unsigned int output_h,int enable)
{
	gdma_dev   *gdma     = &gdma_devices[0];

	down(&gdma->sem);
    dst_box_buffer_w = output_w;
    dst_box_buffer_h = output_h;
    if(dst_box_buffer_enable == -1)
    {
		dst_box_buffer_enable = enable;
    }else{
		dst_box_buffer_enable_pre = dst_box_buffer_enable;
	    dst_box_buffer_enable = enable;
    }
	up(&gdma->sem);
    return 1;
...
int GDMA_BOX_SE_draw_block(int buffer_w,int buffer_h,int box_output_w,
	int box_output_h,PICTURE_LAYERS_OBJECT *data,int SE_BOX_Enable) 
{
	unsigned int dst_phy;
	KGAL_SURFACE_INFO_T dsurf;
	...
	KGAL_DRAW_FLAGS_T sflag = KGAL_DRAW_NOFX;
	KGAL_DRAW_SETTINGS_T sblend;
	KGAL_BLIT_FLAGS_T sflag_blit = KGAL_BLIT_NOFX;
	KGAL_BLIT_SETTINGS_T sblend_blit;

	GDMA_PICTURE_OBJECT *picObj;
	GDMA_PICTURE_OBJECT *data_picObj = NULL;
	VO_RECTANGLE disp_res;
	osd_planealpha_ar osdcmp_plane_ar;
	osd_planealpha_gb osdcmp_plane_gb;
	int output_w=0,output_h=0;
    int src_w=0,src_h=0,src_pitch=0,src_phy=0;

    if(SE_BOX_Enable ==0 )
		goto SE_BOX_DISABLE;
	
	if(data && data->layer_num==1 ){
		if(data->layer[0].fbdc_num == 1){
			data_picObj = &data->layer[0].fbdc[0];
		}else if(data->layer[0].normal_num == 1){
			data_picObj = &data->layer[0].normal[0];
		}
	}
	if(data_picObj == NULL)
		return 0;

	src_w = data_picObj->width;
	src_h = data_picObj->height;
	src_pitch = data_picObj->pitch; 
	src_phy = data_picObj->address;
	...
	memset(&sblend_blit,0, sizeof(KGAL_BLIT_SETTINGS_T));
    
	sblend.srcBlend = KGAL_BLEND_ONE;
	sblend.dstBlend = KGAL_BLEND_ZERO;

	sblend_blit.srcBlend = KGAL_BLEND_ONE;
	sblend_blit.dstBlend = KGAL_BLEND_ZERO;

    if(dst_order_box == 0){	
		if(dst_box == 0){
			dst_box = dvr_malloc_specific(buffer_w*buffer_h*4, GFP_DCU1);
		}
		if(dst_box == 0)
			return 0;
		dst_phy = virt_to_phys(dst_box);
		//memset(dst_AI,0x0,s_w*s_h*2);
		dst_order_box = 1;
	}else{
		if(dst_box_2 == 0){
            dst_box_2 = dvr_malloc_specific(buffer_w*buffer_h*4, GFP_DCU1);
		}
		if(dst_box_2 == 0)
			return 0;
		dst_phy = virt_to_phys((void *)dst_box_2);
		//memset(dst_AI,0x0,s_w*s_h*2);
		dst_order_box = 0;
	}
	if(box_output_w >= buffer_w)
		output_w = buffer_w;
	else
		output_w = box_output_w;

	if(box_output_h >= buffer_h)
		output_h = buffer_h;
	else
		output_h = box_output_h;


	//memset(dst_AI,0x80,s_w*s_h*2);
	dsurf.physicalAddress = dst_phy;
	dsurf.width = output_w;
	dsurf.height = output_h;
	dsurf.bpp = 32;
	dsurf.pitch = buffer_w*4;
	...
	ssurf.height = src_h;
	ssurf.bpp = 32;
	ssurf.pitch = src_pitch;
	ssurf.pixelFormat = KGAL_PIXEL_FORMAT_FBC;

	dsurf.physicalAddress = dst_phy;
	dsurf.width = buffer_w;
	dsurf.height = buffer_h;
	dsurf.bpp = 32;
	dsurf.pitch = buffer_w*4;
	dsurf.pixelFormat = KGAL_PIXEL_FORMAT_ARGB;
	...
	drect.w = output_w;
	drect.h = output_h;
	KGAL_StretchBlit(&ssurf, &srect, &dsurf, &drect, &sflag_blit, &sblend_blit);
	...
	picObj->src_type = SRC_NORMAL;
	picObj->alpha = 0;
	picObj->width = output_w;
	picObj->height = output_h;
	picObj->address = dst_phy;
	picObj->pitch = buffer_w*4;
	picObj->layer_used = 1;	
	picObj->show = 1;
	picObj->paletteformat = 1; /* rgba format */
	picObj->syncInfo.syncstamp = 0;
