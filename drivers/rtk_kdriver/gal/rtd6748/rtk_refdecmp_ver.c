...
void decmp_dump_single_file(const char* path, int size, unsigned long phy_addr)
{
	...
	if( buf_st == NULL ) {
		pr_err("decmp can't map mem 0x%llx %d\n", phy_addr, size);
		return;
	}
	...
	if( filep != NULL )
	{
	     ...
	     pr_err("decmp output file %s  addr:0x%08x %d\n", path, phy_addr, size ); 
...
void decmp_dbg_diff_register( )
{
	...
	for(i=0; i < sizeof(regs_100_match)/sizeof(int); i++)
	{
		...
		if( x_dbg_find_value(reg_addr, &reg_value) == 0 )
		{
			...
			{
				pr_err("[decmpCmp] value not match addr:0x%08x  0x%08x<->0x%08x\n", reg_addr, cur_reg_value, reg_value);
	...
	*/
	...
	for(region_idx =0; region_idx<12; region_idx++)
	{
		...
		for(i=0; i < sizeof(regs_100_diff_per_region)/sizeof(int);)
		{
			...
			{
				pr_err("[decmpCmp] can't find reg 0x%08x! \n", addr1);
			}
...
RTK_DECMP_RECORD_TABLE gRecTable;
 ...
 #define DECMPREC_CHECK_VALUE(addr, idx, val, str, ret) \
    do { \
		...
		if( retXX != val ) { \
			pr_err("[decmprec] reg: 0x%08x region:%d %s notEqual  0x%08x(reg) != 0x%08x\n", addr+idx*4,idx, str, retXX , val); \
			ret = -1; \
		} else {ret = 0;} \
    } while(0)
...
int decmp_rec_resetRec(unsigned long addr, int region_idx)
{
	...
	if( region_idx != REGION_INVALID_INDEX )
	{
		...
		if( item->item_region_idx == REGION_INVALID_INDEX ) {
			pr_err("[decmp rec] invalid region ? addr:0x%"PRIx64 " idx:%d tmp->region_idx:%d\n", addr, region_idx, tmp->region_idx);
		}

		if( item->data_y_addr != addr )
		{
			pr_err("[decmp rec] region/addr mismatch ? addr:0x%"PRIx64 " idx:%d tmp->region_idx:%d\n", addr, region_idx, tmp->region_idx);

			item = decmp_rec_find( addr, NULL);

			if(item == NULL) {
				pr_err("[decmprec] no item found for addr addr:0x%"PRIx64 "\n", addr);
				return -1;
			}
		...
		pr_err("[decmp rec] invalid when reset rec addr:0x%"PRIx64 " idx:%d tmp->region_idx:%d\n", addr, region_idx, tmp->region_idx);
	}
...
int decmp_rec_initRec(rtk_decmp_export* info)
{
	...
	for( ; i < DECMP_REC_NUM_REGIONS; i++ )
	{
		...
		if( tmp->region_idx == REGION_INVALID_INDEX ) {
			...
			{
				pr_err("[decmprec] init temp_rec entry:%d addr:0x%"PRIx64 " \n", i , info->data_y_addr);
	...
	if( tmp == NULL ) {
		...
		pr_err("[decmp] decmp_rec_initRec no free entry !\n");
		GpuDecUtil_dump_exportInfo(info);
	}
...
{ 
	...
	for( idx=0; idx < DECMP_REC_NUM_REGIONS; idx++ )
	{
		...
		pr_err("tmpRec:[%d] region:%d addr:0x%"PRIx64 " \n", idx, tmp->region_idx, tmp->data_y_addr);
	}
...
rtk_decmp_export* decmp_rec_getInfo(int region_idx)
{
	...
	if( item->item_region_idx == REGION_INVALID_INDEX ) {
		pr_err("decmp region:%d is invalid\n", region_idx);
		return NULL;	
	}
	...
	
	{
		...
		if( info->region_idx != region_idx ) {
			...
			pr_err("decmp_rec_getInfo recordInvalid?? recRegion:%d  yStart:0x%"PRIx64 " last_p:%d.\n", 
...
int decmp_rec_diff(int region_idx, unsigned long addr)
{
	...
	if( item->item_region_idx == REGION_INVALID_INDEX ) {
		...
		if( i == DECMP_REC_NUM_REGIONS ) {
			pr_err("can't find from temp_rec. assumeed_region_idx:%d addr:0x%"PRIx64" \n", region_idx, addr );
			...
			pr_err("[decmp_rec] first update region:%d  addr:0x%"PRIx64 " from temp_rec:%d %d %d\n", region_idx, addr, i,  item->rec_wp, item->rec_rp );
		}
	}
	else if( item->data_y_addr != addr )
	{
		...
		if(item) 
		{
			pr_err("[decmp_rec] addr:0x%"PRIx64 " table's regionIdx change from %d to %d.  \n", addr, region_idx, regionIdx_in_table);
			...
			pr_err("[decmp_rec] Error VE change region,   addr:0x%"PRIx64 " org idxInRsgister:%d   \n", addr, region_idx);
		...
		{	
		  //should not happen ?
		  pr_err(" [decmp_rec] region addr mismatch? recRegion:%d  addr:0x%"PRIx64 " itemAddr:0x%"PRIx64 " infoAddr 0x%"PRIx64 "\n", region_idx, addr, 
		  item->data_y_addr, info->data_y_addr );

		}
		else if( info->region_idx != region_idx ) {

			pr_err("[decmp_rec] recordInvalid?? recRegion:%d  yStart:0x%"PRIx64 " last_p:%d  curBind:%d 0x%"PRIx64 " \n", info->region_idx, info->data_y_addr,  last_p,
				region_idx, addr );
		}
...

	{
		...
		if( enabled == 0 ) 
		{
			pr_crit("[decmp_rec] check err. region %d not Enable! \n", region_idx );
		}
		

		if( info == NULL) {
			pr_crit(" [decmp_rec] no info. something wrong in VE/decmp_rec flow for addr:0x%"PRIx64 "idx:%d", addr, region_idx);
...
int decmp_rec_updateItem( RTK_DECMP_RECORD_ITEM* item, rtk_decmp_export* info, int idx)
{
	...
	if( item->item_region_idx == REGION_INVALID_INDEX) {
		...
		pr_err("[decmp_rec] index changed %d to %d\n", item->item_region_idx, idx);
	...
	if( item->rec_rp == item->rec_wp ) {
		//pr_err("[decmp_rec] overflow reg:%d  [%d %d]\n", idx, item->rec_rp, item->rec_wp );
	}
...
static void decomp_dcmt_callback(void)
{
	...
	if( trash_addr > g_uvStart_for_dcmt ) 
	{
		...
		pr_err(" \t trash cord: chroma [x y] = [%d %d] x1:%d pitch:%d\n", x,y, x1, pitch);
	...
	else if ( trash_addr >= g_yStart_for_dcmt && trash_addr <= g_uvStart_for_dcmt ) 
	{
		...
		pr_err(" \t trash cord:  luma [x y] = [%d %d] x1:%d pitch:%d\n", x,y, x1, pitch);
	}
	else 
	{
		pr_err("%s trash cord: invalid address?? 0x%"PRIx64 "  \n", __FUNCTION__, trash_addr);
	}
...
//	int dcmt_idx = 0;
	...
	{ // test when  region bit enable
		...
		if (info == NULL) {
			pr_err("decmp_dbg_dcmt get info fail ! %d %d\n", type, param_1);
			return -1;
		}
	...
	dcmt_set_monitor(dcmt_idx, &mt_desc);
		...
		pr_err("\t dcmt y_start 0x%"PRIx64 " uv_start 0x%"PRIx64 " header_start 0x%"PRIx64 "\n",
	...
	dcmt_set_monitor(dcmt_idx, &mt_desc);
		...
		pr_err("\t dcmt gpuAddr y_start 0x%"PRIx64 " uv_start 0x%"PRIx64 " header_start 0x%"PRIx64 " y_start:%d\n",
	...
	{ // scan top-right corner, co-work with type 2
		...
		pr_err("\t dcmt check right-bottom y_start 0x%"PRIx64 " uv_start 0x%"PRIx64 " header_start 0x%"PRIx64 " \n",
			info->data_y_addr, info->data_uv_addr, info->header_y_addr );
	   	...
	   	for( cur_line=0; cur_line < total_line_count; cur_line++ )
   		{
   			...
   			for( j=0; j < rule_per_set; j++ )
			{
				...
				//check luma  cur_line+1
			        ...
			        mt_desc.ip_entry[0].module = MODULE_ANY; //all module
			        mt_desc.ip_entry[0].prot = PROT_MODE_RW; //access type write
					//pr_err("\t aa2:%d cur_line:%d 0x%x 0x%x \n", dcmt_idx, cur_line, mt_desc.start , mt_desc.end );
				dcmt_set_monitor(dcmt_idx, &mt_desc);
				dcmt_idx++;
			...
			pr_err("\t dcmt line start:%d lastSt 0x%x lastEnd 0x%x y_start:%d dcmtidx:[%d %d]\n",
...
//	long long parsed_data = 0;
	...
	switch (cmd_index) {
		...
		case DECMP_DBG_ERROR_DUMP:
			{
				int enable = 0;
				sscanf(*cmd_pointer, "%d\n", &enable);
				pr_err("[decmprec] enable decmpErrDump:%d\n", enable);
		...
		case DECMP_DBG_DCMT:
			{
				...
				if( ret == 0 ) {
					pr_err("[decmpDBG]dcmt type:%d val:%d ok\n", type, val);
				}
				else 
					pr_err("[decmpDBG]dcmt type:%d val:%d fail %d\n", type, val, ret);

				#else
					pr_crit("decmp kernel doesn't enable CONFIG_RTK_KDRV_DC_MEMORY_TRASH_DETCTER config\n");
		...
		case DECMP_DBG_STATUS:
			...
			break;
		default:
			pr_err("decmp invalid dgb cmd:%d\n", cmd_index);
...
ssize_t GpuDec_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (copy_from_user(str, buf, count)) {
		printk(KERN_ERR "decmp_debug:%d copy_from_user failed! (buf=%p, count=%d)\n",
	...
	if (cmd_index < DECMP_DBG_CMD_NUMBER ) 
	{
		...
		if (cmd_length <= RTKDECMP_MAX_CMD_LENGTH - 1)
			...
			pr_err("rtkgdma_debug:%d array index:%d invalid!\n", __LINE__, cmd_length);
			return -EFAULT;
		}
	}
