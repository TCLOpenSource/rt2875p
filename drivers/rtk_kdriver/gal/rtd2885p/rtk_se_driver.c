...
static int se_setup_cdev(struct se_dev *dev, int index);
...
static void se_debug_info(void)
{
	...
	if(isDcmtTrap("SE2_Q0") || isDcmtTrap("SE2_Q1") || isDcmtTrap("SE2")) {
...
#if defined(CONFIG_ARM64)
				rtd_pr_se_err("%lx ",*((volatile unsigned long*)(cmd_addr)-cnt));
#else
				rtd_pr_se_err("%08x ",*((volatile unsigned int*)(cmd_addr)-cnt));
		...
		/*do nothing, shall we need rtd_pr_se_info ?*/
		rtd_pr_se_err("NOT SE module.\n");
		return;
	}
...
//RTK mark for stop update se
int enable_stop_update_se = 0;
...
inline void InitSeReg(void)
{
...
#if 1
#if 0
	mxx register  mxx*1024, axx *512
	YcrCr_ITU_en = 1 ;															  
	R = m00*y + m10*U + m20*V + a0
	R = Y + 1.4075 * (V - 128)
	m00 = 1, m10 = 0, m20 = 1.4075  a0=0
							
	G=m01*y + m11 *U + m21*V + a1
	G = Y - 0.3455 * (U - 128) - (0.7169 * (V - 128))															  
	m01=1 m11= -0.3455  m21= -0.716     a1= 0													  

	B=m02*y+m12*U + m22*V + a2
	B = Y + 1.7790 * (U - 128)															  
	m02=1 m12=1.7790 m22=0 a2 = 0
#endif
	m00 = (int)1024*1;
	m01 = (int)1024*1;
	m02 = (int)1024*1;
	...
	m21 = (int)1024*(-0.716);
	m22 = (int)1024*0;
	rtd_outl(SEOFFSET(SeConvCoef1), SE_CONVCOEF1_0_m01(m01) | SE_CONVCOEF1_0_m00(m00));
	rtd_outl(SEOFFSET(SeConvCoef2), SE_CONVCOEF2_0_m10(m10)|SE_CONVCOEF2_0_m02(m02));
	rtd_outl(SEOFFSET(SeConvCoef3), SE_CONVCOEF3_0_m12(m12)|SE_CONVCOEF3_0_m11(m11));
	rtd_outl(SEOFFSET(SeConvCoef4), SE_CONVCOEF4_0_m21(m21)|SE_CONVCOEF4_0_m20(m20));
	rtd_outl(SEOFFSET(SeConvCoef5), SE_CONVCOEF5_0_m22(m22));
	rtd_outl(SEOFFSET(SeConvCoef6), SE_CONVCOEF6_0_a2(0)|SE_CONVCOEF6_0_a1(0)|SE_CONVCOEF6_0_a0(0));
#endif
...
inline void InitSeVReg(void)
{
	...
	if(se_v_base) {
		...
		for(cnt=0;cnt<SE_SCAL_COEFH_NUM;cnt++) {
			rtd_outl(SEOFFSET(SeCoefInfo[0].SeRegVCoef[0]) +0x4*cnt, se_v_scal_coef_h[cnt]);
		}

		/*scal coef, 580,5c0*/
		for(cnt=0;cnt<SE_SCAL_COEFV_NUM;cnt++) {
			rtd_outl(SEOFFSET(SeCoefInfo[1].SeRegHCoef32[0]) +0x4*cnt, se_v_scal_coef_v[cnt]);
		}
	}
}
#endif
...
inline void StoreSeVReg(void)
{
	...
	for(cnt=0;cnt<SE_CONV_COEF_NUM;cnt++) {
		se_v_conv_coef[cnt] = rtd_inl(SEOFFSET(SeConvScaling[1]) +0x10*cnt);
	}
	/*scal coef val, 2c4,2d4,2e4*/
	for(cnt=0;cnt<SE_SCAL_VAL_NUM;cnt++) {
		se_v_scal_val[cnt] = rtd_inl(SEOFFSET(SeStretch[1]) +0x10*cnt);
	}

	/*scal coef, 480,500*/
	for(cnt=0;cnt<SE_SCAL_COEFH_NUM;cnt++) {
		se_v_scal_coef_h[cnt] = rtd_inl(SEOFFSET(SeCoefInfo[0].SeRegVCoef[0]) +0x4*cnt);
	}

	/*scal coef, 580,5c0*/
	for(cnt=0;cnt<SE_SCAL_COEFV_NUM;cnt++) {
		se_v_scal_coef_v[cnt] = rtd_inl( SEOFFSET(SeCoefInfo[1].SeRegHCoef32[0]) +0x4*cnt);
	}
}
#endif
#endif
...
{
	if(down_timeout(&sem_cmdqueue, timeout)) {
		rtd_pr_se_err("[SE] Command Quard Interrupt.\n");
		return;
	}
}
...
#if PW_SAVING
/*#define SE_PM_PRINT(s, args...) rtd_pr_se_print(s, ## args)*/
...
static int se_pm_func(void*) {
	...
	while(1) {
		...
		if(down_timeout(&sem_cmdqueue, SE_CMD_TIMEOUT)) {
			rtd_pr_se_err( "[SE]GET Command Queue FAIL@%s \n.", __func__);
			continue;
		}
...
#ifdef CONFIG_COMPAT
#if defined(CONFIG_ARM64)
long compat_rtk_se_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
#else
int compat_rtk_se_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
#endif
{
#if defined(CONFIG_ARM64)
...
long rtk_se_ioctl(struct file *filp, uint32_t cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case SE_IOC_SET_VQUEUE:
	{
		unsigned int pri=0;
		if (copy_from_user((void *)&pri, (const void __user *)arg, sizeof(unsigned int))) {
			retval = -EFAULT;
		}else{
			rtd_outl(SEOFFSET(SeQueuePriority), pri);
		}
		break;
	}
...
#if 0
		//rtk_se_checkFinish();
		rtd_outl(SEOFFSET(ReservedDfbCtrl[4]), 0);
		rtd_outl(SEOFFSET(Se20Dummy[0]), 0xffff0000);
#endif
		break;
	}
...
#endif
		rtk_se_power(1);
		rtd_pr_se_err( "[SE] HW-RESET entry !!!\n");
		InitSeReg();
#if defined(SE_RECOVERY_VCPU)
		InitSeVReg();
#endif
	...
	case SE_IOC_WAIT_FOR_COMPLETE:
	{
#if PW_SAVING
		if(down_timeout(&sem_cmdqueue, SE_CMD_TIMEOUT)) {
			rtd_pr_se_err( "[SE]GET Command Queue FAIL@%s SE_IOC_WAIT_FOR_COMPLETE \n.", __func__);
			break;
		}
		...
		rtd_outl(SEOFFSET( SeInts), (uint32_t)ints.Value);
		...
		if ((rtd_inl(SEOFFSET(SeCtrl)) & 0x2) &&
			...
			rtd_outl(SEOFFSET(SeInte), (uint32_t)inte.Value);
			if(down_timeout(&sem_checkfinish, HZ/2)) {
				rtd_pr_se_err(
					"[SE]WAIT_FOR_COMPLETE timeout!\n");
				retval = -ENOTTY;
				goto out;
			}
		}
	...
	case SE_IOC_PM_CTRL:
	{
		...
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(bool))) {
			retval = -EFAULT;
			//rtd_pr_se_err("[SE]SE_IOC_PM_CTRL get value  failed!\n");
			/*disable pm func if anything gets wrong*/
			set_se_pm_func_clk_ctl_status(false);
		}else{
			rtd_pr_se_err("[SE]SE_IOC_PM_CTRL get value is %d\n",data);
	...
	case SE_IOC_FMT:
	{
		...
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(SE_FMT_ST))) {
			retval = -EFAULT;
			rtd_pr_se_err("[SE]SE_IOC_FMT get value fail!\n");
		}else {
	...
	case SE_IOC_SETPALTE:
	{
		...
		if (copy_from_user((void *)&pale, (const void __user *)arg,
			...
			retval = -EFAULT;
			rtd_pr_se_err("[SE]setpalette get value fail!\n");
		}else {
			lord_kspace_p = current->pid;
	...
	case SE_IOC_BLIT:
	{
		...
		if (copy_from_user((void *)&bst, (const void __user *)arg,
			...
			retval = -EFAULT;
			rtd_pr_se_err("[SE]blit get value fail!\n");
		}else {
			lord_kspace_p = current->pid;
	...
	case SE_IOC_STCHBLIT:
	{
		...
		if (copy_from_user((void *)&bst, (const void __user *)arg,
			...
			retval = -EFAULT;
			rtd_pr_se_err("[SE]stchblit get value fail!\n");
		}else {
			lord_kspace_p = current->pid;
			lord_uspace_p = bst.pinfo.m_pid;
			lord_uspace_pp = bst.pinfo.m_ppid;
	...
	case SE_IOC_FILLRECT:
	{
		...
		if (copy_from_user((void *)&bst, (const void __user *)arg,
			...
			retval = -EFAULT;
			rtd_pr_se_err("[SE]fillrect get value fail!\n");
		}else {
			lord_kspace_p = current->pid;
			lord_uspace_p = bst.pinfo.m_pid;
			lord_uspace_pp = bst.pinfo.m_ppid;
...
static int se_probe(struct platform_device *pdev)
{
	...
	if (result < 0) {
		dev_err(&pdev->dev, "se: can't get major %d\n", se_major);
		return result;
	}
	dev_notice(&pdev->dev, "se init module major number = %d\n", se_major);

	se_class = class_create(THIS_MODULE, "se");
	if (IS_ERR(se_class)) {
		result = PTR_ERR(se_class);
		goto err_class;
	}
	 ...
	 */
	se_devices = (struct se_dev *)kmalloc(se_nr_devs * sizeof(struct se_dev),
		GFP_KERNEL);
	if (!se_devices) {
		result = -ENOMEM;
		goto err_alloc_se_dev;
	}
	memset(se_devices, 0, sizeof(struct se_dev));

	sema_init(&se_devices->sem, 1);

	result = se_setup_cdev(se_devices, 0);
	if (result)
		goto err_setup_cdev;

	noncacheBuffer = dma_alloc_coherent(se_device_st, SE_COMMAND_ENTRIES,
			&noncacheBufferPhy, GFP_KERNEL);
	dev_notice(&pdev->dev, "B:SE_INIT...se_devices = %px(%08x), buffer(noncache) = %px(%08x)\n",
			se_devices, (unsigned int)virt_to_phys(se_devices),
			noncacheBuffer, (unsigned int)noncacheBufferPhy);

	sema_init(&sem_checkfinish, 1);
	sema_init(&sem_cmdqueue, 1);
	sema_init(&sem_nv12, 1);
	...
	se_devices->CmdLimit = (uint32_t) (se_devices->CmdBase + se_devices->size);
	...
	if (irq < 0) {
		dev_err(se_device_st, "se: can't get irq. ret=%d\n", irq);
	} else {
		result = request_irq(irq, rtk_se_irq_handler, IRQF_SHARED, "se", (void *)se_devices);
		if (result) {
			dev_err(se_device_st, "se: can't get request_irq. ret=%d\n", result);
	...
	if(se_pm_thread){
		dev_err(&pdev->dev, "[SE] %s , run pm thread..\n", __func__);
...
err_class:
	unregister_chrdev_region(dev, se_nr_devs);
...
static int se_remove(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "%s: se_major = %d\n", __func__, se_major);

#if IS_ENABLED(CONFIG_RTK_KDRV_DC_MEMORY_TRASH_DETCTER)
	DCMT_DEBUG_INFO_UNREGISTER(se_mdscpu, se_debug_info);
#endif
...
#ifdef CONFIG_PM
static int rtk_se_suspend(struct device *p_dev)
{
#if PW_SAVING
	if(down_timeout(&sem_cmdqueue, SE_CMD_TIMEOUT)) {
		rtd_pr_se_err( "[SE]GET Command Queue FAIL@%s \n.", __func__);
	...
	if(se_pm_func_clk_status == false){
		up(&sem_cmdqueue);
		rtd_pr_se_err( "[RTK-SE]power is down, skip waiting idle bit\n");
		return 0;
	}
#endif
	...
	rtk_se_checkFinishV();
#endif
	rtd_pr_se_err( "[RTK-SE]suspend, wait idle bit..finish.\n");
...
static int rtk_se_resume(struct device *p_dev)
{
	rtd_pr_se_notice( "[RTK-SE]resume\n");
#if PW_SAVING
	if(down_timeout(&sem_cmdqueue, SE_CMD_TIMEOUT)) {
		rtd_pr_se_err( "[SE]GET Command Queue FAIL@%s \n.", __func__);
		return 0;
...
static int rtk_se_suspend_std(struct device *p_dev)
{
#if PW_SAVING
	if(down_timeout(&sem_cmdqueue, SE_CMD_TIMEOUT)) {
		rtd_pr_se_err( "[SE]GET Command Queue FAIL@%s \n.", __func__);
...
static int rtk_se_resume_std(struct device *p_dev)
{
	rtd_pr_se_err( "[RTK-SE]resume\n");
#if PW_SAVING
	if(down_timeout(&sem_cmdqueue, SE_CMD_TIMEOUT)) {
		rtd_pr_se_err( "[SE]GET Command Queue FAIL@%s \n.", __func__);
		return 0;
...
#endif
	rtk_se_power(1);
    rtd_pr_se_err("rtd_inl(SYS_REG_SYS_CLKEN1_reg)=%x\n",rtd_inl(SYS_REG_SYS_CLKEN1_reg));
    rtd_pr_se_err("rtd_inl(SYS_REG_SYS_SRST1_reg)=%x\n",rtd_inl(SYS_REG_SYS_SRST1_reg));
...
#ifdef CONFIG_HIBERNATION
	.freeze     = rtk_se_suspend_std,
	.thaw       = rtk_se_resume_std,
	.poweroff   = rtk_se_suspend_std,
	.restore    = rtk_se_resume_std,
#endif
};
#endif
...
static struct platform_driver se_device_driver = {
	...
	.driver = {
		   ...
		   .pm           = &se_pm_ops,
   #endif
		   .of_match_table = of_match_ptr(se_of_match),
	},
};
		   
#ifdef CONFIG_COMPAT
#if defined(CONFIG_ARM64)
long compat_rtk_se_ioctl(struct file *filp, uint32_t cmd, unsigned long arg);
#else
int compat_rtk_se_ioctl(struct file *filp, uint32_t cmd, unsigned long arg);
#endif
#endif
...
static struct file_operations se_fops = {
	...
	.mmap = rtk_se_mmap,
#ifdef CONFIG_COMPAT
	.compat_ioctl	= compat_rtk_se_ioctl,
#endif
...
static int se_setup_cdev(struct se_dev *dev, int index)
{
	...
	if (err) {
		rtd_pr_se_err( "Error %d adding se%d", err, index);
		return err;
	}

	se_device_st = device_create(se_class, NULL,
			MKDEV(se_major, index), NULL, "se%d", index);
	if (IS_ERR(se_device_st)) {
		err = PTR_ERR(se_device_st);
		goto err_dev_create;
	}

	se_device_st->dma_mask = &se_device_st->coherent_dma_mask;
	if (dma_set_mask(se_device_st, DMA_BIT_MASK(32))) {
		rtd_pr_se_err("SE DMA not supported\n");
	}

#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 0, 0)) && defined(CONFIG_ARM64)
	arch_setup_dma_ops(se_device_st, 0, 0, NULL, false);
#endif
...
{
	return platform_driver_register(&se_device_driver);
}

static void __exit se_exit_module(void)
{
...
void rtk_se_power(int state)
{
   int se_clk = 0;
	...
	if(state ==1) {
		...
		se_clk |= SYS_REG_SYS_CLKEN1_get_clken_se(rtd_inl(SYS_REG_SYS_CLKEN1_reg));
		/*in some project, video enable se for logo animation.
		 we can't reset clk.*/
		//rtd_pr_se_emerg("[%s][$d]	!!\n",__FUNCTION__,__LINE__);
		rtd_pr_se_emerg("[%s][%d]	se_clk=%d!!\n",__FUNCTION__,__LINE__,se_clk);
		se_clk |= SYS_REG_SYS_CLKEN1_get_clken_se(rtd_inl(SYS_REG_SYS_CLKEN1_reg));
		if(!se_clk) {
			msleep(100);
			se_clk |= SYS_REG_SYS_CLKEN1_get_clken_se(rtd_inl(SYS_REG_SYS_CLKEN1_reg));
...
void _WriteCmd(struct se_dev *dev, uint8_t* pbyCommandBuffer, int32_t lCommandLength)
{
	...
	uint32_t wrptr = 0, rdptr = 0, qwrptr = 0;
	...
	qwrptr = (uint32_t)rtd_inl(SEOFFSET(SeCmdWritePtr)) - dev->CmdBase;
	if (lCommandLength % 4 != 0) {
		u32Alignment = 4 - (lCommandLength % 4);
		dev->u64IssuedInstCnt += u32Alignment;
	}
	wrptr = dev->CmdWritePtr - dev->CmdBase;
	while (1) {
		rdptr = (uint32_t)rtd_inl(SEOFFSET(SeCmdReadPtr)) - dev->CmdBase;
		//if (wrptr == qwrptr) {
		//	rtd_pr_se_err("  [%s] (%d), (%d), [%x, %x]!!\n",__func__, (lCommandLength), (u32Alignment), wrptr, rdptr);
		//}
		if ((wrptr == qwrptr) && (wrptr + 800) > dev->size) {
			while (wrptr < dev->size) {
...
//		rtd_pr_se_err("  [%s] (%8x), (%8x)!!\n",__func__, (dev->CmdBase + wrptr), *(uint32_t *) (pbyCommandBuffer + dwDataCounter));
		wrptr += sizeof(uint32_t);
	...
	for (dwDataCounter = 0; dwDataCounter < u32Alignment;
	     dwDataCounter += sizeof(uint32_t)) {
		*(uint32_t *) (phys_to_virt(dev->CmdBase + wrptr)) = 0xf;
//		rtd_pr_se_err("  [%s] (%8x), (       f)!!\n",__func__, (dev->CmdBase + wrptr));
		wrptr += sizeof(uint32_t);
		if (wrptr >= dev->size)
			wrptr = 0;
	}
	...
	//rtd_pr_se_err("[%s] (%x, %x, %x, %x, %x)!!\n",__func__, dev->CmdBase, dev->CmdLimit, dev->size, wrptr, rdptr);
}
...
void _ExecuteCmd(struct se_dev *dev, uint8_t* pbyCommandBuffer, int32_t lCommandLength)
{
	...
	while (1) {
		...
		//	rtd_pr_se_err("  [%s] (%d), (%d), [%x, %x]!!\n",__func__, (lCommandLength), (u32Alignment), wrptr, rdptr);
		//}
		if ((wrptr == qwrptr) && (wrptr + lCommandLength + u32Alignment) > dev->size) {
			while (wrptr < dev->size) {
				*(uint32_t *) (phys_to_virt(dev->CmdBase + wrptr)) = 0xf;
				wrptr += sizeof(uint32_t);
			}
...
#endif
//#if defined(CONFIG_ARM64)
	dmac_flush_range((void*)phys_to_virt(dev->CmdBase), (void*)phys_to_virt(dev->CmdBase+dev->size));
	outer_flush_range((phys_addr_t)dev->CmdBase, (phys_addr_t)(dev->CmdBase + dev->size));
//#endif	
	dev->CmdWritePtr = (uint32_t) wrptr + dev->CmdBase;
...
void WriteCmd(struct se_dev *dev, uint8_t* pbyCommandBuffer,
	...
	se_cmdqueue_lock(HZ/2);

	_ExecuteCmd(dev, pbyCommandBuffer, lCommandLength);
...
static int rtk_se_fmt(SE_FMT_ST *pInfo){
	...
	if(pInfo->width <=32 || pInfo->height <=0 ||pInfo->sAddr==0
		...
		rtd_pr_se_err("[SE] data error,saddr=%08x, daddr=%08x, w=%d, h=%d \n",
...
bool rtk_se_checkFinish(void)
{
	...
	while (!ret) {
		...
		msleep(delayAtomic);
		if(delaycounter++ >5) {
			rtd_pr_se_err( "[SE-S]checkfinihs too long!, break!");
			ret = false;
			break;
		}
	}
...
bool rtk_se_checkFinishV(void)
{
	...
	while (!ret) {
		...
		msleep(delayAtomic);
		if(delaycounter++ >5) {
			rtd_pr_se_err( "[SE-V]checkfinihs too long!, break!");
			ret = false;
			break;
		}
	}
...
unsigned int se_get_mem(void){
	...
	rtd_pr_se_err("[SE]%s-%d malloc memory size=%08x, addr=%08x \n.",__func__,__LINE__, LUT8_WIDTH*LUT8_HEIGHT*4, se_s_index8_mem_phy);
	return se_s_index8_mem_phy;
}

void se_free_mem(void){
	if(se_s_index8_mem_phy) {
		rtd_pr_se_err("[SE]%s-%d free memory size=%08x, addr=%08x \n.",__func__,__LINE__, LUT8_WIDTH*LUT8_HEIGHT*4, se_s_index8_mem_phy);
		if(down_timeout(&sem_cmdqueue, SE_CMD_TIMEOUT)) {
			rtd_pr_se_err("[SE]%s-%d timeout \n.",__func__,__LINE__);
		}
		dvr_free((void*)se_s_index8_mem_vir);
...
int rtk_se_runtime_suspend(void)
{
	rtk_se_checkFinish();
/*
#if defined(SE_RECOVERY_VCPU)
...
EXPORT_SYMBOL(rtk_se_runtime_suspend);

int rtk_se_runtime_resume(void)
{
	rtk_se_power(1);
...
*/
	se_devices->ignore_interrupt = 0;
	enable_stop_update_se = 0;

	return 0;
}
EXPORT_SYMBOL(rtk_se_runtime_resume);



int rtk_se_performance_counter_start()
{
	se_se_pc_ctrl_RBUS se_se_pc_ctrl_reg;

    se_se_pc_ctrl_reg.regValue = rtd_inl(SE_SE_PC_CTRL_reg);
	se_se_pc_ctrl_reg.pc_sel = 0;
	se_se_pc_ctrl_reg.pc_num = 0;	
	se_se_pc_ctrl_reg.pc_go = 1;
    rtd_outl(SE_SE_PC_CTRL_reg,se_se_pc_ctrl_reg.regValue);

	return 1;
}

EXPORT_SYMBOL(rtk_se_performance_counter_start);

int rtk_se_performance_counter_display()
{
    se_se_pc_out_RBUS se_se_pc_out_reg;
    se_se_pc_out_reg.regValue = rtd_inl(SE_SE_PC_OUT_reg);

	printk(KERN_ERR"[%s] se_pc_out=%x \n", __func__, se_se_pc_out_reg.regValue);
    return 1;
}

EXPORT_SYMBOL(rtk_se_performance_counter_display);


int rtk_se_dfmt_position(int index,int x,int y,int x_offset)
{
    se_dfmt_corner_RBUS se_dfmt_corner_reg;
    se_dfmt_corner_reg.dfmt_x = x;

    rtd_outl((SE_DFMT_CORNER_0_reg+index*4),se_dfmt_corner_reg.regValue);

	return 1;
}
EXPORT_SYMBOL(rtk_se_dfmt_position);


int rtk_se_dfmt_set(int enable,int intp_en,int cache_en)
{
	se_dfmt_ctrl_RBUS se_dfmt_ctrl_reg; 
	se_dfmt_ctrl_reg.regValue = rtd_inl(SE_DFMT_CTRL_reg);

	se_dfmt_ctrl_reg.dfmt_intp_en = intp_en;
	se_dfmt_ctrl_reg.dfmt_cache_en = cache_en;		
	se_dfmt_ctrl_reg.stretch_deformat_sel = enable;

    rtd_outl(SE_DFMT_CTRL_reg,se_dfmt_ctrl_reg.regValue);

    return 1;
}
EXPORT_SYMBOL(rtk_se_dfmt_set);

int rtk_se_dfmt_init(int width,int height)
{
    se_dfmt_corner_RBUS se_dfmt_corner_reg;
	
	se_dfmt_corner_reg.regValue = rtd_inl(SE_DFMT_CORNER_0_reg);
	se_dfmt_corner_reg.dfmt_x =1;
	se_dfmt_corner_reg.dfmt_y =1;
	rtd_outl(SE_DFMT_CORNER_0_reg,se_dfmt_corner_reg.regValue);

	se_dfmt_corner_reg.regValue = rtd_inl(SE_DFMT_CORNER_1_reg);
	se_dfmt_corner_reg.dfmt_x =1;
	se_dfmt_corner_reg.dfmt_y =height;
	rtd_outl(SE_DFMT_CORNER_1_reg,se_dfmt_corner_reg.regValue);

	se_dfmt_corner_reg.regValue = rtd_inl(SE_DFMT_CORNER_2_reg);
	se_dfmt_corner_reg.dfmt_x =width;
	se_dfmt_corner_reg.dfmt_y =1;
	rtd_outl(SE_DFMT_CORNER_2_reg,se_dfmt_corner_reg.regValue);

	se_dfmt_corner_reg.regValue = rtd_inl(SE_DFMT_CORNER_3_reg);
	se_dfmt_corner_reg.dfmt_x =width;
	se_dfmt_corner_reg.dfmt_y =height;
	rtd_outl(SE_DFMT_CORNER_3_reg,se_dfmt_corner_reg.regValue);


    return 1;
}
EXPORT_SYMBOL(rtk_se_dfmt_init);


int rtk_se_dfmt_up_offset_x(int off_x)
{

    se_dfmt_corner_RBUS se_dfmt_corner_reg;
	
	se_dfmt_corner_reg.regValue = rtd_inl(SE_DFMT_CORNER_0_reg);
	se_dfmt_corner_reg.dfmt_x += off_x;
	rtd_outl(SE_DFMT_CORNER_0_reg,se_dfmt_corner_reg.regValue);
	se_dfmt_corner_reg.regValue = rtd_inl(SE_DFMT_CORNER_2_reg);
	se_dfmt_corner_reg.dfmt_x -= off_x;
	rtd_outl(SE_DFMT_CORNER_2_reg,se_dfmt_corner_reg.regValue);
...
EXPORT_SYMBOL(rtk_se_dfmt_up_offset_x);

int rtk_se_dfmt_down_offset_x(int off_x)
{
    se_dfmt_corner_RBUS se_dfmt_corner_reg;
...
EXPORT_SYMBOL(rtk_se_dfmt_down_offset_x);


int rtk_se_dfmt_left_offset_y(int off_y)
{
    se_dfmt_corner_RBUS se_dfmt_corner_reg;

	se_dfmt_corner_reg.regValue = rtd_inl(SE_DFMT_CORNER_0_reg);
	se_dfmt_corner_reg.dfmt_y += off_y;
	rtd_outl(SE_DFMT_CORNER_0_reg,se_dfmt_corner_reg.regValue);
	se_dfmt_corner_reg.regValue = rtd_inl(SE_DFMT_CORNER_1_reg);
	se_dfmt_corner_reg.dfmt_y -= off_y;;
	rtd_outl(SE_DFMT_CORNER_1_reg,se_dfmt_corner_reg.regValue);
...
EXPORT_SYMBOL(rtk_se_dfmt_left_offset_y);

int rtk_se_dfmt_right_offset_y(int off_y)
{
    se_dfmt_corner_RBUS se_dfmt_corner_reg;
...
EXPORT_SYMBOL(rtk_se_dfmt_right_offset_y);



module_init(se_init_module);
module_exit(se_exit_module);
