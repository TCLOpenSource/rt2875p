	...
	void*			pExtraParam)
{
	...
	unsigned int max_size=0,max_size_tmp=0;
	...
	if(pSeSourceProcInfo->uAddr!=0 && pSeSourceProcInfo->uAddr < min_addr)
		min_addr = pSeSourceProcInfo->uAddr;

	max_size_tmp = pSeSourceProcInfo->uPitch*(pSeSourceProcInfo->wDataHeight+pSeSourceProcInfo->wDataY);

	if(pSeSourceProcInfo->uAddr!=0 && (pSeSourceProcInfo->uAddr+max_size_tmp) > max_addr){
		max_addr = pSeSourceProcInfo->uAddr;
		max_size = pSeSourceProcInfo->uPitch*(pSeSourceProcInfo->wDataHeight+pSeSourceProcInfo->wDataY);
	}

	if(pSeSourceProcInfo->uAddr_c1!=0 && pSeSourceProcInfo->uAddr_c1 < min_addr)
		min_addr = pSeSourceProcInfo->uAddr_c1;

	max_size_tmp = pSeSourceProcInfo->uPitch_c1*(pSeSourceProcInfo->wDataHeight+pSeSourceProcInfo->wDataY);

	if(pSeSourceProcInfo->uAddr_c1!=0 && (pSeSourceProcInfo->uAddr_c1+max_size_tmp) > max_addr){
		max_addr = pSeSourceProcInfo->uAddr_c1;
	    max_size = pSeSourceProcInfo->uPitch_c1*(pSeSourceProcInfo->wDataHeight+pSeSourceProcInfo->wDataY);
	}

	if(pSeDestinationProcInfo->uAddr!=0 && pSeDestinationProcInfo->uAddr < min_addr)
		min_addr = pSeDestinationProcInfo->uAddr;

	max_size_tmp = pSeDestinationProcInfo->uPitch*(pSeDestinationProcInfo->wDataHeight+pSeDestinationProcInfo->wDataY);

	if(pSeDestinationProcInfo->uAddr!=0 && (pSeDestinationProcInfo->uAddr+max_size_tmp) > max_addr){
		max_addr = pSeDestinationProcInfo->uAddr;
		max_size = pSeDestinationProcInfo->uPitch*(pSeDestinationProcInfo->wDataHeight+pSeDestinationProcInfo->wDataY);
	}

	if(pSeDestinationProcInfo->uAddr_c1!=0 && pSeDestinationProcInfo->uAddr_c1 < min_addr)
		min_addr = pSeDestinationProcInfo->uAddr_c1;

	max_size_tmp = pSeDestinationProcInfo->uPitch_c1*(pSeDestinationProcInfo->wDataHeight+pSeDestinationProcInfo->wDataY);

	if(pSeDestinationProcInfo->uAddr_c1!=0 && (pSeDestinationProcInfo->uAddr_c1+max_size_tmp) > max_addr){
		max_addr = pSeDestinationProcInfo->uAddr_c1;
		max_size = pSeDestinationProcInfo->uPitch_c1*(pSeDestinationProcInfo->wDataHeight+pSeDestinationProcInfo->wDataY);
	}	

	if(max_addr <= min_addr){
		se_ext_stch.WordReglock1.OpCode = SEINFO_OPCODE_NOP;
		se_ext_stch.WordReglock1_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_stch.WordReglock2.OpCode = SEINFO_OPCODE_NOP;
		se_ext_stch.WordReglock2_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_stch.WordReglock3.OpCode = SEINFO_OPCODE_NOP;
		se_ext_stch.WordReglock3_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_stch.WordReglock4.OpCode = SEINFO_OPCODE_NOP;
		se_ext_stch.WordReglock4_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_stch.WordReglock5.OpCode = SEINFO_OPCODE_NOP;
		se_ext_stch.WordReglock5_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_stch.WordReglock6.OpCode = SEINFO_OPCODE_NOP;
		se_ext_stch.WordReglock6_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_stch.WordReglock7.OpCode = SEINFO_OPCODE_NOP;
		se_ext_stch.WordReglock7_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_stch.WordReglock8.OpCode = SEINFO_OPCODE_NOP;
		se_ext_stch.WordReglock8_1.RegisterValue = SEINFO_OPCODE_NOP;
	

	}else{
		se_ext_stch.WordReglock1.Reserved = 0;
		se_ext_stch.WordReglock1.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_stch.WordReglock1.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_stch.WordReglock1.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
		se_ext_stch.WordReglock1_1.RegisterValue = 0;

		se_ext_stch.WordReglock2.Reserved = 0;
		se_ext_stch.WordReglock2.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_stch.WordReglock2.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_stch.WordReglock2.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_STA;
		se_ext_stch.WordReglock2_1.RegisterValue = (min_addr&0xffffff00);

		se_ext_stch.WordReglock3.Reserved = 0;
		se_ext_stch.WordReglock3.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_stch.WordReglock3.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_stch.WordReglock3.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_END;
		se_ext_stch.WordReglock3_1.RegisterValue = ((max_addr+max_size+0x100)&0xffffff00);

		se_ext_stch.WordReglock4.Reserved = 0;
		se_ext_stch.WordReglock4.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_stch.WordReglock4.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_stch.WordReglock4.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
		se_ext_stch.WordReglock4_1.RegisterValue = 1;


		se_ext_stch.WordReglock5.Reserved = 0;
		se_ext_stch.WordReglock5.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_stch.WordReglock5.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_stch.WordReglock5.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
		se_ext_stch.WordReglock5_1.RegisterValue = 0;
	...
	uint32_t  startpale)
{
	...
	if(startpale + size > 256) {
		rtd_pr_se_err(	"%s pale out of size start=%d, size=%d\n",
	...
	for(step=0;step < 128; step++) {
		...
		if(slot1 > size) {
			...
			if((SeCmdCLUT[step].Word2.RegisterValue & 0xff000000) == 0) {
				SeCmdCLUT[step].Word2.RegisterValue = 0;
			}
		}
		slot1 +=1;
		if(slot1 > size) {
			...
			if((SeCmdCLUT[step].Word4.RegisterValue & 0xff000000) == 0) {
				SeCmdCLUT[step].Word4.RegisterValue = 0;
			}
		}
	}
...
)
{
	...
	if(pExtraParam == NULL) {
		rtd_pr_se_err("[%s] get null pointer!!\n",__func__);
		return false;
	}
...
int SE_EXT_FilmDetect(SE_FMT_ST *pInfo)
{
	...
	if (down_timeout(&sem_nv12, SE_CMD_TIMEOUT)) {
		rtd_pr_se_err( "[SE] GET nv12 lock FAIL@%s\n.", __func__);
		return 0;
	}
...
//-------------------------------------------------------------
	...
	if (pEx->result_format == SEINFO_EXT_COLOR_FORMAT_FBDC_U8U8U8U8
		...
		if (se_ext_conv.Word1.lay_num)
			se_ext_conv.Word1.src2_argb_order = ARGBENDIAN; //ABGR
	} else {
...
bool SE_FWSLD_Stretch_Coef(
	SE_PROCINFO_T *pSeSrcYProcInfo, SE_PROCINFO_T *pSeDstYProcInfo,
	// veritcal scaling
	SEINFO_SACLING_TYPE	byVerticalScalingType,
	unsigned int		dwVerticalScalingRatio,
	SEINFO_STRETCH_VTAP	byVerticalTapNumber,
	// horizontal scaling
	SEINFO_SACLING_TYPE	byHorizontalScalingType,
	unsigned int		dwHorizontalScalingRatio,
	SEINFO_STRETCH_HTAP	byHorizontalTapNumber,
...
{
	// reffernce to "bool SE_PQMask_Stretch_Coef()"
	...
	// setup color format
	SeRegColorFormat.Fields.Format = SEINFO_COLOR_FORMAT_Y; /*SEINFO_COLOR_FORMAT_INDEX8;*/
	SeRegColorFormat.Fields.write_enable1  = true;
	...
	se_ext_stch.WordReg1.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_stch.WordReg1.RegisterAddress = (SEINFO_WRITEREG)SEINFO_REG_COLOR_FORMAT;
	se_ext_stch.WordReg1_1.RegisterValue = (unsigned int)SeRegColorFormat.Value;
	...
	dwScalingSetting = (replaceParam |
		((unsigned int) bAlphaEnable << 5) |
		((unsigned int) byHorizontalTapNumber << 3) |
		((unsigned int) byVerticalTapNumber << 2) |
		((unsigned int) ScalingHodd << 1) |
		((unsigned int) ScalingVodd << 0)
	);
	se_ext_stch.WordReg4.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_stch.WordReg4.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_stch.WordReg4.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_STRETCH;
	se_ext_stch.WordReg4_1.RegisterValue = dwScalingSetting;
	// rtd_pr_se_emerg("[%s] dwScalingSetting = 0x%08x\n", "DEBUG", dwScalingSetting);
	...
	se_ext_stch.Word1.opcode = SEINFO_OPCODE_EXT_FMT;
	se_ext_stch.Word1.rslt_fmt = SEINFO_EXT_COLOR_FORMAT_NV12_420_Y; /*SEINFO_EXT_COLOR_FORMAT_YUV444;*/
	se_ext_stch.Word1.lay_ctl = 0;
	se_ext_stch.Word1.lay_num = 0;
	...
	se_ext_stch.Word3.rslt_addr_index = (unsigned int)pSeDstYProcInfo->byBaseAddressIndex;
	se_ext_stch.Word3.rslt_chr1_addr_index = (unsigned int)pSeDstYProcInfo->byBaseAddressIndex; // (UINT32)pSeDstUProcInfo->byBaseAddressIndex;
	se_ext_stch.Word3.rslt_chr2_addr_index = (unsigned int)pSeDstYProcInfo->byBaseAddressIndex; // (UINT32)pSeDstVProcInfo->byBaseAddressIndex;

	se_ext_stch.Word4.src1_fmt = SEINFO_EXT_COLOR_FORMAT_NV12_420_Y; /*SEINFO_EXT_COLOR_FORMAT_YUV444;*/
	...
	se_ext_stch.Word6.src1_addr_index = (unsigned int)pSeSrcYProcInfo->byBaseAddressIndex;
	se_ext_stch.Word6.src1_chr1_addr_index = (unsigned int)pSeDstYProcInfo->byBaseAddressIndex; // (UINT32)pSeSrcUProcInfo->byBaseAddressIndex;
	se_ext_stch.Word6.src1_chr2_addr_index = (unsigned int)pSeDstYProcInfo->byBaseAddressIndex; // (UINT32)pSeSrcVProcInfo->byBaseAddressIndex;
	...
	{
		int i=0;
		unsigned int temp, temp_1;
		for(i=0; i<(sizeof(SECMD_PQMASK_EXT_STCH_BLIT)/8); i++) {
			temp = ((unsigned int *)&se_ext_stch)[i*2];
			temp_1 = ((unsigned int *)&se_ext_stch)[i*2+1];

			if (((temp&0xf) == SEINFO_OPCODE_NOP) && (i<20))
				continue;
			if (i < 12)
				rtd_pr_gdma_info("SE_Reg, WordReg%d[0xB800C%3x]=%x,\n", i+1, ((temp&0xfff0)>>4), temp_1);
			else if (i == 12)
				rtd_pr_gdma_info("SE_Reg, WordAddr0[0xB800C%3x]=%x,\n", ((temp&0xfff0)>>4), temp_1);
			else if (i == 13)
				rtd_pr_gdma_info("SE_Reg, WordPitch0[0xB800C%3x]=%x,\n", ((temp&0xfff0)>>4), temp_1);
			else if (i == 14)
				rtd_pr_gdma_info("SE_Reg, WordAddr1[0xB800C%3x]=%x,\n", ((temp&0xfff0)>>4), temp_1);
			else if (i == 15)
				rtd_pr_gdma_info("SE_Reg, WordPitch1[0xB800C%3x]=%x,\n", ((temp&0xfff0)>>4), temp_1);
			else if (i == 16)
				rtd_pr_gdma_info("SE_Reg, WordAddr2[0xB800C%3x]=%x,\n", ((temp&0xfff0)>>4), temp_1);
			else if (i == 17)
				rtd_pr_gdma_info("SE_Reg, WordPitch2[0xB800C%3x]=%x,\n", ((temp&0xfff0)>>4), temp_1);
			else if (i == 18)
				rtd_pr_gdma_info("SE_Reg, WordAddr3[0xB800C%3x]=%x,\n", ((temp&0xfff0)>>4), temp_1);
			else if (i == 19)
				rtd_pr_gdma_info("SE_Reg, WordPitch3[0xB800C%3x]=%x,\n", ((temp&0xfff0)>>4), temp_1);
			else {
				rtd_pr_gdma_info("SE_Reg, Word%d=%x,\n", (i-19)*2-1, temp);
				rtd_pr_gdma_info("SE_Reg, Word%d=%x,\n", (i-19)*2, temp_1);
			}
		}
	}
	...
	WriteCmd(se_devices, (unsigned char *)&se_ext_stch, sizeof(SECMD_PQMASK_EXT_STCH_BLIT));
	...
	SE_PROCINFO_T *pSeDstVProcInfo)
{
	...
	se_ext_conv.WordReg2.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_conv.WordReg2.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_conv.WordReg2.RegisterAddress = (SEINFO_WRITEREG)SEINFO_REG_COLOR_FORMAT;
	se_ext_conv.WordReg2_1.RegisterValue = SeRegColorFormat.Value;
...
bool SE_PQMask_Bitblit(
	...
	SE_PROCINFO_T *pSeDstYProcInfo, SE_PROCINFO_T *pSeDstUProcInfo, SE_PROCINFO_T *pSeDstVProcInfo,
	unsigned int SrcAlpha, unsigned int DstAlpha, unsigned int RltAlpha, unsigned char RoundSel
)
{
	// local parameters
	...
	se_ext_2layer_bitblit.Word10.round_sel = (RoundSel <= 3)? RoundSel : 0; // 0:rounding, 1:round down, 2:round up, 3:reserved
...
bool SE_PQMask_Stretch_Coef_Bitblit(
	...
	Command.Word10.round_sel = (RoundSel <= 3)? RoundSel : 0; // 0:rounding, 1:round down, 2:round up, 3:reserved
