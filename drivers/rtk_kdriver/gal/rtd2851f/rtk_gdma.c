...
unsigned int recovery_blend_en =1;// for recover default
...
int gdma_probe(struct platform_device* dev);
...
#if (defined(CONFIG_ARCH_RTK288O) || defined(CONFIG_ARCH_RTK2851A) || defined(CONFIG_ARCH_RTK2851F)) && defined(THRESHOLD_DETECT)
...
static void gdma_config_line_buffer(void) {
...
#if defined(CONFIG_ARCH_RTK288O) || defined(CONFIG_ARCH_RTK2851A) || defined(CONFIG_ARCH_RTK2851F)
#else
	...
	line_buffer_sta_reg.l2 = 3;
	line_buffer_sta_reg.l4 = 5;
	line_buffer_sta_reg.l5 = 7;
	line_buffer_size_reg.l1 = 3;
...
#if defined(CONFIG_ARCH_RTK287O) || defined(CONFIG_ARCH_RTK288O) || defined(CONFIG_ARCH_RTK2851A) || defined(CONFIG_ARCH_RTK2851F)
#else
...
int gdma_osd_writedump(unsigned int type, unsigned int osdn, osd_dmem_info* pinfo,osd_dmem_info* pinfo_stch) {
	...
	if(type == 0) {
		...
		rtd_outl(OSDOVL_Mixer_CRC_Ctrl_reg, OSDOVL_Mixer_CRC_Ctrl_mixer_crc_start(1) | OSDOVL_Mixer_CRC_Ctrl_mixer_crc_cont(1) |
			OSDOVL_Mixer_CRC_Ctrl_mixer_crc_done(osdn));
...
//RTK mark for netfix
	...
	case GDMA_SET_OSD_RECOVER_BLENDING:
	{
		unsigned int data;
		osdovl_osd_db_ctrl_RBUS osd_db_ctrl;
		osdovl_mixer_c1_plane_alpha1_RBUS mixer_c1_plane_alpha1;
		osdovl_mixer_layer_sel_RBUS mixer_layer_sel;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(unsigned int))) {
			ret = -EFAULT;
			DBG_PRINT(KERN_EMERG"GDMA: ioctl code = %d failed!!!!!!!!!!!!!!!1\n", GDMA_SET_OSD_RECOVER_BLENDING);
		} else{
			printk(KERN_EMERG"[GDMA] [%s %d]",__FUNCTION__,__LINE__);
			if(data > 0)
			{		
				mixer_c1_plane_alpha1.regValue = rtd_inl(OSDOVL_Mixer_c1_plane_alpha1_reg);
                mixer_c1_plane_alpha1.regValue &= (0xff00ffff);
				mixer_c1_plane_alpha1.regValue |= ((unsigned char)data)<<16;				
				rtd_outl(OSDOVL_Mixer_c0_plane_alpha1_reg,mixer_c1_plane_alpha1.regValue);
				rtd_outl(OSDOVL_Mixer_c1_plane_alpha1_reg,mixer_c1_plane_alpha1.regValue);
				mixer_layer_sel.regValue = rtd_inl(OSDOVL_Mixer_layer_sel_reg);
				mixer_layer_sel.c0_plane_alpha_en = 1;
				mixer_layer_sel.c1_plane_alpha_en = 1;								
				...
				recovery_blend_en =0;
				mixer_c1_plane_alpha1.regValue = 0x00ff00ff;
				osd_db_ctrl.regValue = rtd_inl(OSDOVL_OSD_Db_Ctrl_reg);
				osd_db_ctrl.db_en = 1;
				osd_db_ctrl.db_load = 1;
                rtd_outl(OSDOVL_Mixer_c1_plane_alpha1_reg,mixer_c1_plane_alpha1.regValue);
			...
			printk(KERN_EMERG"[GDMA] [%s %d] en = %d",__FUNCTION__,__LINE__,recovery_blend_en);

			ret = GDMA_SUCCESS;
		}
		DBG_PRINT(KERN_EMERG"GDMA: ioctl GDMA_SET_OSD_RECOVER_BLENDING, enable %d\n", data);
		break;
	}
	default:
...
DCMT_DEBUG_INFO_REGISTER(osd_mdscpu,osd_debug_info);
	...
	if (gdma_total_ddr_size) {
		rtd_outl(GDMA_OSD1_AFBC1_MIN_reg, 0x0);
		rtd_outl(GDMA_OSD1_AFBC1_MAX_reg, gdma_total_ddr_size - 0x1000); // need less than DDR size (transaction less than 4K)
		rtd_outl(GDMA_OSD2_AFBC2_MIN_reg, 0x0);
		rtd_outl(GDMA_OSD2_AFBC2_MAX_reg, gdma_total_ddr_size - 0x1000); // need less than DDR size (transaction less than 4K)
	} else {
  ...
  system_ui_re = get_ui_resolution();
	#if defined(CONFIG_ARCH_RTK2851A) || defined(CONFIG_ARCH_RTK2851F)
...
struct task_struct *gdma_def_thread;
	...
	if(irq_of_parse_and_map(dev->dev.of_node , 0)) {
		gdma->dev = &(dev->dev);
	...
	rtd_outl(GDMA_OSD_INTEN_reg, GDMA_OSD_INTEN_write_data(1) | GDMA_OSD_INTEN_osd1_vsync(1));
#endif
	rtd_outl(GDMA_DMA_BL_2_reg, 0x30303000);
#endif
	...
	dsb();
			...
			*/
			...
			if (gSemaphore[disPlane] < 0) {
				rtd_pr_gdma_info("gdma : gSemaphore[%d] %d\n", disPlane, gSemaphore[disPlane]);
...
int GDMA_ReceiveGraphicLayers(GRAPHIC_LAYERS_OBJECT *data)
{
	...
	if (picLayerObj == NULL || sendCompLayerObj == NULL || sendGdmaLayerObj == NULL || mousepicObj == NULL) {
		...
		rtd_pr_gdma_debug("%s, kmalloc failed... \n", __FUNCTION__);
		return -100;
	}
...
#endif
EXPORT_SYMBOL(GDMA_Clk_Select); //export for Scaler use
/*===============================================*/
