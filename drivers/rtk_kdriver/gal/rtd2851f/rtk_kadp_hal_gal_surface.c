...
int Get_rgb_afbc_size(unsigned int width, unsigned int height, unsigned int *size,unsigned int *w_alig,unsigned int *h_alig)
{
	...
	w_aligned = ALIGN_SIZE(width, AFBC_WIDEBLK_WIDTH_ALIGN);
	h_aligned = ALIGN_SIZE(height, AFBC_WIDEBLK_HEIGHT_ALIGN);
...
//tmp_handle = ion_alloc(gHAL_ION_CLIENT, size, 4096,  ION_HEAP_TYPE_CARVEOUT, flags);
		...
		if(likely (ret == 0 ) ) { //caverout phys() always return 0~
			if( unlikely(phy_size != size) ) {
				/*
				  rtd_pr_se_debug( " %s warning physSize:%d(%d) handle:0x%x\n", __FUNCTION__, phy_size, size,  (unsigned int)tmp_handle);
				*/
			...
			*handle = tmp_handle;
			...
			rtd_pr_se_debug( " GAL %s toPhys fail %d handle:0x%x\n", __FUNCTION__, ret, (unsigned int)handle);
		}
...
int xFreeSurfaceMem(RtSurface* rt_surf)
{
	...
	if( unlikely( (int)GAL_ALLOC_SIZE < 0 ) ) {
		rtd_pr_se_debug( " GAL err!! size <0 handle:0x%x\n", (unsigned int) rt_surf->addr_handle);
	}
						   ...
						   KADP_HAL_GAL_SURFACE_INFO_T *pSurfaceInfo)
{
	...
	if (pixelFormat == KADP_HAL_GAL_PIXEL_FORMAT_FBC) //fixme: for test FBDC and use KADP_HAL_GAL_PIXEL_FORMAT_ARGB1555 as temporary
	{
		if (!use_afbc) {
			...
			size = 0;
			Get_rgb_afbc_size((unsigned int)width, (unsigned int)height, &size, &w_alig, &h_alig);
			tmp_pitch = w_alig * 4;
			
			//rtd_pr_gdma_err(" \n [GAL][FBDC] size=%x, pitch=%x \n", size, tmp_pitch);
		}
	}
	else
	{
...
//gal_surf->paletteInfo = ;
	...
	//add by fox for gfx std
	//rtd_pr_gdma_debug("try to record gfx memory width=%d height=%d\n",width,height );
	if((width*height) <  GFX_SPECIAL_SIZE && total_index < GFX_NUM_MAX && total_size < GFX_CURSOR_SIZE)
	{
		  ...
		  total_index++;
		  total_size += size;
		  //rtd_pr_gdma_debug("record gfx memory width=%d height=%d gal_surf->physicalAddress=%x total_size=%d\n",width,height,gal_surf->physicalAddress,total_size);
	}
...
static int __init early_parse__gal_fbc(char *str)
{
	...
	if (strncmp(str_OFF,str,strlen(str_OFF))==0) {
        fbcOnOff = FBC_OFF;
        //rtd_pr_gdma_info("FBC_OFF\n")
	} else if (strncmp(str_ON,str,strlen(str_ON))==0) {
        fbcOnOff = FBC_ON;
        //rtd_pr_gdma_info("FBC_ON\n");
	} else {
        fbcOnOff = FBC_ON;
        //rtd_pr_gdma_info("using default value = FBC_ON [%s]\n",str);
   }
