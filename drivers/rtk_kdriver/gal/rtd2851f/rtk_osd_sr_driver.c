...
void VPQ_Osdsr1_RegCtrl_SetOsdsr1_reg_lpmm(unsigned char x_lpmm_en)
{
	// RTK2851C add
	#if defined(CONFIG_ARCH_RTK2851F)
	unsigned int z_RBUS_reg;
	// lpmm suggest settings
	z_RBUS_reg = rtd_inl( OSD_SR_OSD_SR_1_GainY_Ctrl0_reg );
	z_RBUS_reg = ( z_RBUS_reg & ~OSD_SR_OSD_SR_1_GainY_Ctrl0_sr_lpmm_corner_mask) | (OSD_SR_OSD_SR_1_GainY_Ctrl0_sr_lpmm_corner( 8 ) );
	z_RBUS_reg = ( z_RBUS_reg & ~OSD_SR_OSD_SR_1_GainY_Ctrl0_sr_lpmm_1px_mask) | (OSD_SR_OSD_SR_1_GainY_Ctrl0_sr_lpmm_1px( 15 ) );
	z_RBUS_reg = ( z_RBUS_reg & ~OSD_SR_OSD_SR_1_GainY_Ctrl0_sr_lpmm_en_mask) | (OSD_SR_OSD_SR_1_GainY_Ctrl0_sr_lpmm_en( x_lpmm_en ) );
	rtd_outl( OSD_SR_OSD_SR_1_GainY_Ctrl0_reg , z_RBUS_reg );
	#endif
}
...
void drv_superresolution_init(void)
{
	...
	*/
	...
	rtd_outl(OSD_SR_OSD_SR_1_GainMap_Ctrl0_reg, osd_sr1_gainmap_ctl0_reg.regValue);
	rtd_outl(OSD_SR_OSD_SR_2_GainMap_Ctrl0_reg, osd_sr1_gainmap_ctl0_reg.regValue);
...
void drv_superresolution_LPF_Gain(GDMA_DISPLAY_PLANE disPlane)
{
	...
	*(unsigned int *)&line_buffer_end_reg = rtd_inl(GDMA_line_buffer_end_reg);
	...
	osd_sr_ctrl_reg.sr_maxmin_shift = 0;
	osd_sr_ctrl_reg.sr_var_thd = 0xff;
	VPQ_Osdsr1_RegCtrl_SetOsdsr1_reg_lpmm( 1 );
	...
	osd_sr_gainy_ctrl0_reg.sr_lpf_gain = 0x3f;
...
int drv_scaleup(uint8_t enable, GDMA_DISPLAY_PLANE disPlane)
{
	...
	*(unsigned int *)&line_buffer_end_reg = rtd_inl(GDMA_line_buffer_end_reg);
	...
	if (enable) {
		...
		rtd_outl(OSD_SR_OSD_SR_1_GainY_Ctrl0_reg+offset, pReg->osd_sr_gainy_ctrl0_reg.regValue);
		rtd_outl(OSD_SR_OSD_SR_1_GainMap_Ctrl0_reg+offset, pReg->osd_sr_gainmap_ctrl_reg.regValue);
	...
	if (enable && (bypass_flag == 0)) {
		...
		*/
		if (srcWin.width == 0 || srcWin.height == 0 || dispWin.width == 0 || dispWin.height == 0) {
			rtd_pr_gdma_debug("GDMA division by :srcWin.width = %d,srcWin.height=%d\n", srcWin.width, srcWin.height) ;
			rtd_pr_gdma_debug("GDMA division by :dispWin.width = %d,dispWin.height=%d\n", dispWin.width, dispWin.height) ;
		}
		...
		//	rtd_pr_gdma_debug("%s:srcWin.width = %d [>=16], srcWin.height = %d [>=4] \n", __FUNCTION__, srcWin.width, srcWin.height) ;
		//	return -1;
		} else {
			if (dispWin.width != 0)
...
#if 1 //for osd quality and performance
			if (disPlane == GDMA_PLANE_OSD1)
...
int osd_sr_init_module(void)
{
	...
	osd_sr_db_ctrl_reg.regValue = rtd_inl(OSD_SR_OSD_SR_DB_CTRL_VADDR);
	osd_sr_db_ctrl_reg.db_en = 1;
	osd_sr_db_ctrl_reg.db_apply = 0;
	rtd_outl(OSD_SR_OSD_SR_DB_CTRL_VADDR, osd_sr_db_ctrl_reg.regValue);
#endif
