...
int gal_gfx_mmap(struct file *filp, struct vm_area_struct *ps_vma)
{
    ...
    rtd_pr_se_debug( " %s: Received mmap(2) request with ui32MMapOffset 0x%x and uiByteSize %d",
			 __FUNCTION__,
			 ps_vma->vm_pgoff,
			 uiByteSize);
	#endif
    ...
    if (((ps_vma->vm_flags & VM_WRITE) != 0) &&
		...
		rtd_pr_se_debug("%s: Cannot mmap non-shareable writable areas", __FUNCTION__);
		ret = -EINVAL;
...
static void gal_gfx_setup_cdev(struct cdev   *gal_gfx_cdev, int index)
{
	...
	if (err)
		rtd_pr_se_notice( "Error %d adding se%d", err, index) ;
...
int gal_gfx_init_module(void)
{
	int result;
	dev_t dev = 0 ;
	rtd_pr_gdma_debug("gal_gfx module_init init \n");
	...
	if (result < 0) {
		rtd_pr_gdma_warn( "gal_gfx: can't get major %d\n", gal_gfx_major) ;
		return result ;
	}

	rtd_pr_gdma_warn( "gal_gfx init module major number = %d\n", gal_gfx_major) ;
	...
	rtd_pr_gdma_debug("gal_gfx module_init finish\n");
	return 0 ;
}
