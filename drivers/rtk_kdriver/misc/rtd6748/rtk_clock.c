...
#define CONFIG_REALTEK_VOLTAGE_CTRL_ONE_PIN 1
...
{
	{SCPU_CORE_sys_pll_scpu2_reg, SCPU_CORE_sys_scpu_dvfs_reg, SCPU_CORE_sys_scpu_status_reg},
	{SCPU_CORE_sys_pll_scpu2_reg, SCPU_CORE_sys_scpu_dvfs_reg, SCPU_CORE_sys_scpu_status_reg}
};
...
{
	SCPU_NONE	= 0,
#ifndef CONFIG_REALTEK_VOLTAGE_CTRL_ONE_PIN
	SCPU_L_L,
	SCPU_L_H,
	SCPU_H_L,
	SCPU_H_H
//TCL board using ONE pin to control
#else
	SCPU_L,
	SCPU_H
#endif
...
#ifdef CONFIG_REALTEK_VOLTAGE_CTRL_ONE_PIN
	//mapping from{1,2,3,4} to {0(SCPU_NONE), 1(SCPU_L), 2(SCPU_H)}
	level = ((level - 1) >> 1) + 1;
#endif
...
void rtk_set_voltage(struct rtk_cpufreq_info *info, unsigned int pll, unsigned int rate)
{
	...
	rtd_pr_cpu_freq_err("rtk_set_vol: pll(%d), rate(%d), vol(%d/%d)\n",pll, rate, voltage_level, old_voltage_level);

	udelay(50);
	old_voltage_level = voltage_level;
}

void rtk_set_default_voltage(int level)
{
	int voltage_level = level;

	if(rtk_cpu_dvs_enable == 0)
		return;

#ifndef CONFIG_REALTEK_VOLTAGE_CTRL_ONE_PIN
	rtd_pr_cpu_freq_err("rtk_set_default_vol before:(%d), GPO(%x/%x)\n", voltage_level, rtd_inl(MIS_GPIO_GP1DATO_reg), rtd_inl(MIS_GPIO_GP1DIR_reg));
#else //ONE PIN GPO_67
	...
	udelay(50);
#ifndef CONFIG_REALTEK_VOLTAGE_CTRL_ONE_PIN
	rtd_pr_cpu_freq_err("rtk_set_default_vol:(%d), GPO(%x/%x)\n", voltage_level, rtd_inl(MIS_GPIO_GP1DATO_reg), rtd_inl(MIS_GPIO_GP1DIR_reg));
#else //ONE PIN GPO_67
	rtd_pr_cpu_freq_err("rtk_set_default_vol:(%d), GPO(%x/%x)\n", voltage_level, rtd_inl(MIS_GPIO_GP2DATO_reg), rtd_inl(MIS_GPIO_GP2DIR_reg));
#endif
}
...
void rtk_clk_set_ratio(int cpu, unsigned int pll, unsigned int rate)
{
	...
	if(rtk_check_control_pll_cpu(cpu))
		value &= (~SCPU_CORE_sys_scpu_clkratio_ctrl_0_clkratio_scpu_a75_mask);
	else
		value &= (~SCPU_CORE_sys_scpu_clkratio_ctrl_0_clkratio_scpu_dsu_mask);

	if(rtk_check_control_pll_cpu(cpu))
		value |= (ratio << SCPU_CORE_sys_scpu_clkratio_ctrl_0_clkratio_scpu_a75_shift);
	else
	...
	if(rtk_check_control_pll_cpu(cpu))
		rtd_outl(SCPU_CORE_sys_scpu_clkratio_ctrl_1_reg, 1<<SCPU_CORE_sys_scpu_clkratio_ctrl_1_clkratio_scpu_a75_active_shift);
	else
...
int __rtk_clk_set_rate_ex(struct clk *clk, unsigned int rate, int algo_id, int cpu, int idx)
{
	...
	else
	{
		...
		if(new_state == RTK_CPUFREQ_STATE_X)
			pll = info->Xpll;
		else if(new_state == RTK_CPUFREQ_STATE_A)
			pll = info->Apll;
