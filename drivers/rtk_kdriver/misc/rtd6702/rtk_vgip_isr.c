...
irqreturn_t OnlineMeasureISR(int irq, void *dev_id)
{
	...
	onms_onms2_interrupt_en_reg.regValue = IoReg_Read32(ONMS_onms2_interrupt_en_reg);
	...
	onms_onms3_interrupt_en_reg.regValue = IoReg_Read32(ONMS_onms3_interrupt_en_reg);
...
int idma_get_field_mode_sub(void)
{
    ...
    return SUB_VGIP_VGIP_CHN2_STATUS_get_ch2_field_pol(IoReg_Read32(SUB_VGIP_VGIP_CHN2_STATUS_reg)) ? 0:1;//1: 0;
}
#endif
...
{//call by dma vgip interrupt
	...
	vodma_vodma_line_st_reg.regValue = IoReg_Read32(VODMA_VODMA_LINE_ST_reg);
...
watermark_watermark_status_RBUS watermark_watermark_status_reg;
...
extern void dynamic_change_hdr_setting_hdmi_finish(unsigned char hdr_enable, I3DDMA_COLOR_SPACE_T color_format, unsigned char hdr_type);
	...
	unsigned char handle_watermark_flag = 0;//for water mark isr
	...
	dma_vgip_chn1_status_reg.regValue = IoReg_Read32(DMA_VGIP_DMA_VGIP_STATUS_reg);
	...
	h3ddma_cap3_cap_status_reg.regValue = IoReg_Read32(H3DDMA_CAP3_Cap_Status_reg);
	ich1_hist_status.regValue = IoReg_Read32(HISTOGRAM_ICH1_Hist_Status_reg);
	//sdnr interrupt
	vgip_vgip_chn1_wde_reg.regValue = IoReg_Read32(VGIP_VGIP_CHN1_WDE_reg);

	ddr_in2status_reg.regValue = IoReg_Read32(MDOMAIN_CAP_DDR_In2Status_reg);
	watermark_watermark_interrupt_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_INTERRUPT_reg);
	watermark_watermark_status_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_STATUS_reg);
	...
	if(watermark_watermark_interrupt_reg.watermark_interrupt_en && watermark_watermark_status_reg.watermark_process_done)
	{//water mark isr case
		handle_watermark_flag = 1;
		watermark_watermark_interrupt_reg.watermark_interrupt_en = 0;
		IoReg_Write32(WATERMARK_WATERMARK_INTERRUPT_reg, watermark_watermark_interrupt_reg.regValue);//disable water mark interrupt
		IoReg_Write32(WATERMARK_WATERMARK_STATUS_reg, watermark_watermark_status_reg.regValue);//clear status 
	}
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	if (handle_ch1_sta_flag == 1 && SysInfo != NULL && Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE) {
		...
		fwif_color_DM2_Sat_Hue_Hist_ch1_sta_ISR();
	}
	...
	if(handle_ch2_end_flag && scaler_scart_out_isr_get_enable())
	{
		if(Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_THRIP))
		{
			if(ddr_in2status_reg.in2_block_select == 0)
			{
				...
				{
					buf_even_addr = IoReg_Read32(MDOMAIN_CAP_DDR_In2_2ndAddr_reg)+TVE_VFLIP_SIZE_OFFSET;
					buf_odd_addr = IoReg_Read32(MDOMAIN_CAP_DDR_In2_3rdAddr_reg)+TVE_VFLIP_SIZE_OFFSET;
				}
				else
				{
					buf_even_addr = IoReg_Read32(MDOMAIN_CAP_DDR_In2_2ndAddr_reg);
					buf_odd_addr = IoReg_Read32(MDOMAIN_CAP_DDR_In2_3rdAddr_reg);
				}
			}
			else if(ddr_in2status_reg.in2_block_select==1)
			{
				if(scaler_get_tve_vflip_enable())
				{
					buf_even_addr = IoReg_Read32(MDOMAIN_CAP_DDR_In2_3rdAddr_reg)+TVE_VFLIP_SIZE_OFFSET;
					buf_odd_addr = IoReg_Read32(MDOMAIN_CAP_DDR_In2Addr_reg)+TVE_VFLIP_SIZE_OFFSET;
				}
				else
				{
					buf_even_addr = IoReg_Read32(MDOMAIN_CAP_DDR_In2_3rdAddr_reg);
					buf_odd_addr = IoReg_Read32(MDOMAIN_CAP_DDR_In2Addr_reg);
				}
			...
			else if(ddr_in2status_reg.in2_block_select==2)
			{
				if(scaler_get_tve_vflip_enable())
				{
					buf_even_addr = IoReg_Read32(MDOMAIN_CAP_DDR_In2Addr_reg)+TVE_VFLIP_SIZE_OFFSET;
					buf_odd_addr = IoReg_Read32(MDOMAIN_CAP_DDR_In2_2ndAddr_reg)+TVE_VFLIP_SIZE_OFFSET;
				}
				else
				{
					buf_even_addr = IoReg_Read32(MDOMAIN_CAP_DDR_In2Addr_reg);
					buf_odd_addr = IoReg_Read32(MDOMAIN_CAP_DDR_In2_2ndAddr_reg);
				}

			}

			if(TVE_syncgen_status_i_get_i_field(IoReg_Read32(TVE_syncgen_status_i_reg)) == 0)
			{
				...
				scaler_rtd_outl(TVE_DMA_TVEVDDMA_SATRT0_reg, buf_odd_addr/8+0x5a);	  //  dma addr  //?M3d dma?t��F8-?
			...
			else
			{
				...
				scaler_rtd_outl(TVE_DMA_TVEVDDMA_SATRT0_reg, buf_even_addr/8);	  //  dma addr  //?M3d dma?t��F8-?
		...
		else
		{
			if(ddr_in2status_reg.in2_block_select == 0)
			{
				if(scaler_get_tve_vflip_enable())
				{
					buf_prog_mode = IoReg_Read32(MDOMAIN_CAP_DDR_In2_3rdAddr_reg)+TVE_VFLIP_SIZE_OFFSET;
				}
				else
				{
					buf_prog_mode = IoReg_Read32(MDOMAIN_CAP_DDR_In2_3rdAddr_reg);
				}
			}
			else if(ddr_in2status_reg.in2_block_select == 1)
			{
				if(scaler_get_tve_vflip_enable())
				{
					buf_prog_mode = IoReg_Read32(MDOMAIN_CAP_DDR_In2Addr_reg)+TVE_VFLIP_SIZE_OFFSET;
				}
				else
				{
					buf_prog_mode = IoReg_Read32(MDOMAIN_CAP_DDR_In2Addr_reg);
				}
			...
			else if(ddr_in2status_reg.in2_block_select == 2)
			{
				if(scaler_get_tve_vflip_enable())
				{
					buf_prog_mode = IoReg_Read32(MDOMAIN_CAP_DDR_In2_2ndAddr_reg)+TVE_VFLIP_SIZE_OFFSET;
				}
				else
				{
					buf_prog_mode = IoReg_Read32(MDOMAIN_CAP_DDR_In2_2ndAddr_reg);
				}
...

			{
				...
				scaler_rtd_outl(TVE_DMA_TVEVDDMA_SATRT0_reg, buf_prog_mode/8+0xb4);	  //  dma addr  //?M3d dma?t��F8-?
...
//<<<End 20171228, pinyen modify for Dolby HDMI to avoid pink gabage when the begining from SDR to DolbyHDR
	...
	if((reg_value & BIT(27))){
		...
		line_info = IoReg_Read32(VGIP_VGIP_CHN1_LC_reg);
		line_cnt = ((line_info & 0x00FFF000)>>12);

		time0 = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
		if(time0s_set==0) {
			time0s_start = time0;
			time0s_set=1;
			time0s_cnt=0;
		}
		time0s_end = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
...
if((reg_value & BIT(29))){
	...
	time1 = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
	if(time1s_set==0) {
		time1s_start = time1;
		time1s_set=1;
		time1s_cnt=0;
	}
	time1s_end = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
...
if((reg_value & BIT(30))){
	...
	time2 = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
	if(time2s_set==0) {
		time2s_start = time1;
		time2s_set=1;
		time2s_cnt=0;
	}
	time2s_end = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
...
// AI application
	if(bAIInited)
	{
		...
		fwif_color_od_table_dynamic_for_VRR();
