...
//#define MISC_DBG_PRINT(s, args...) rtd_pr_misc_err(s, ## args)
...
#define REG_DDR3_OC_CONTROL 0xb80cb028
#define REG_DDR3_OC_DONE 0xb80cb028
#define REG_DDR3_SSC_CONTROL 0xb80cb01c
#define REG_DDR3_NCODE_SSC 0xb80cb028
#define REG_DDR3_FCODE_SSC 0xb80cb024
#define REG_DDR3_GRAN_EST 0xb80cb020
#endif
...
// ML7BU-1259 : disable for bringup
// #define CONFIG_RTK_KDRV_DDR_DEBUG
#ifdef CONFIG_RTK_KDRV_DDR_DEBUG
...
#endif
#ifdef SUPPORT_DCU3
static unsigned int DDR3_freq_old = 0;
static unsigned int DDR3_N_CODE = 0;
static unsigned int DDR3_F_CODE = 0;
static unsigned int DDR3_freq_new = 0;
#endif
...
#ifdef SUPPORT_DCU3
static unsigned int getDDR3Frequency(void)//reference from bootcode:/common/cmd_dssinfo.c  pll_ddr_freq()
{
        unsigned int regValueForN = rtd_inl(REG_DDR3_NCODE_SSC);
        unsigned int regValueForF = rtd_inl(REG_DDR3_FCODE_SSC);
        unsigned int N_CODE, F_CODE, FOUT;
        N_CODE = (regValueForN & 0xFF00) >> 8;
        F_CODE = (regValueForF & 0x7FF0000) >> 16;
...
static void setDDR3Frequency(unsigned int fin)//reference from bootcode:/common/cmd_dssinfo.c  pll_ddr_freq()
{
        unsigned int regValueForN = 0;
        unsigned int regValueForF = 0;
        ...
        regValueForN = (N_CODE&0xFF) << 8;
        regValueForF = ((F_CODE & 0x7FF) <<16);
        rtd_maskl(REG_DDR3_NCODE_SSC, 0xFFFF00FF, regValueForN);
        rtd_maskl(REG_DDR3_FCODE_SSC, 0xF800FFFF, regValueForF);
...
#endif
#ifdef SUPPORT_DCU3
        else if(module == RHAL_SYS_SPREAD_SPECTRUM_MODULE_DDR3)
        {
                rtd_part_outl(REG_DDR3_SSC_CONTROL, 1, 1, 0x0);//En_ssc =0
                if(DDR3_freq_old == 0)
                {
                        DDR3_N_CODE = (rtd_inl(REG_DDR3_NCODE_SSC) & 0xFF00) >> 8; // save ddr freq N_CODE
                        DDR3_F_CODE = (rtd_inl(REG_DDR3_FCODE_SSC) & 0x7FF0000) >> 16; // save ddr freq F_CODE
                        DDR3_freq_old = (unsigned int)(((27 * DDR3_N_CODE + 27 * 3 + (27 * DDR3_F_CODE) / 2048)));
                }

                if(nRatio == RHAL_SYS_SPREAD_SPECTRUM_OFF)
                {
                        rtd_maskl(REG_DDR3_NCODE_SSC, 0xFFFF00FF, ((DDR3_N_CODE&0xFF) << 8));
                        rtd_maskl(REG_DDR3_FCODE_SSC, 0xF800FFFF, ((DDR3_F_CODE & 0x7FF) <<16));
                        //rtd_part_outl(REG_DDR2_SSC_CONTROL, 1, 1, 0x0);//En_ssc =0
                        spin_unlock_irqrestore(&spread_spectrum_lock, flag);
                        return 0;
                }
                ...
                {
                        nRatio = (nRatio - RHAL_SYS_SPREAD_SPECTRUM_RATIO_up_0_25 + 1)*25;
                        DDR3_freq_new = DDR3_freq_old*(nRatio +10000)/10000;
                        setDDR3Frequency(DDR3_freq_new);
                        nRatio = nRatio *2;
                }

                /******************************DCU3********************************/
                freq_MHz = getDDR3Frequency();
                //Step 2. Get Ncode_ssc, Fcode_ssc and Gran
                getSSCParameter(freq_MHz, nRatio, &NCode_ssc, &FCode_ssc, &Gran_est);
                //Step 3. Write to register
                //Step 3.1 Disable ssc
                rtd_part_outl(REG_DDR3_SSC_CONTROL, 3, 3, 0x1);////dpi_ssc_flag_init =1
                rtd_part_outl(REG_DDR3_SSC_CONTROL, 1, 1, 0x0);//////En_ssc =0
                //Step 3.2 Disable oc
                rtd_part_outl(REG_DDR3_OC_CONTROL, 26, 26, 0);//oc_en =0
                //Step 3.3 Set Ncode_ssc, Fcode_ssc
                rtd_part_outl(REG_DDR3_NCODE_SSC, 7, 0, NCode_ssc);//Ncode_ssc
                rtd_part_outl(REG_DDR3_FCODE_SSC, 10, 0, FCode_ssc);//Fcode_ssc
                //Step 3.4 Set Gran_est
                rtd_part_outl(REG_DDR3_GRAN_EST, 22, 20, DOT_GRAN);//Gran_set
                rtd_part_outl(REG_DDR3_GRAN_EST, 18, 0, Gran_est);//Gran_set
                //Step 3.5 Enable oc
                rtd_part_outl(REG_DDR3_OC_CONTROL, 26, 26, 1);//oc_en =1

                waitBit = 0;
                waitBit = rtd_inl(REG_DDR3_OC_DONE);
                while((waitBit & 0x4000000) == 0) {
                        waitBit = rtd_inl(REG_DDR3_OC_DONE); // polling for oc_done
                        rtd_pr_misc_err("waiting.....DDR2  =0x%x\n", waitBit);
                }
                //Step 3.6 Disable oc
                rtd_part_outl(REG_DDR3_OC_CONTROL, 26, 26, 0);//oc_en =0
                //Step 3.7 Enable ssc
                rtd_part_outl(REG_DDR3_SSC_CONTROL, 1, 1, 0x1);//En_ssc =1
                //Step 4.End
                ret = 0;
        }
#endif
...
static int wol_polling(void *arg)
{
        ...
        rtd_outl(GPIO_GPDATO_reg, (rtd_inl(GPIO_GPDATO_reg) | (1 << 6)));
        // Clear WOL Flag
...
#define RTD_SHARE_MEM_BASE      0xb8060500
                ...
                case MISC_SET_DEBUG_LOCK: {
                        ...
                        if(nLock) {
                                ...
                                //disable JTAG debug
                                rtd_maskl(GW_gpu_sw_bist_config_reg, 0xFFFFFFEF, 0x00000010);//ejtag_gpu_bist    set 1 to mux out gpu mbist ejtag port
                                ...
                                //enable JTAG debug
                                rtd_maskl(GW_gpu_sw_bist_config_reg, 0xFFFFFFEF, 0x00000000);//ejtag_gpu_bist
...
#endif
#ifdef SUPPORT_DCU3
						else if(para.module == RHAL_SYS_SPREAD_SPECTRUM_MODULE_DDR3)
                                                para.module_support = 1;
#endif
                                        else
...
#define RTD_SHARE_MEM_BASE      0xb8060500
               ...
               case MISC_GET_MODULE_TEST_PIN:
			   {
                ...
                case MISC_UART_PINMUX_SWITCH:
                {
                        ...
                        switch(param)
                        {
                                case UART0_PINMUX_ENABLE:
                                        rtd_maskl(PINMUX_ST_GPIO_ST_CFG_11_reg, 0xFFFFFF0F, 0x60); //switch uart pinmux 0x1806022c[7:4];6: uart0_rxdi_src0,<I> //7: uart1_rxdi_src1,<I>
                                        rtd_maskl(PINMUX_ST_GPIO_ST_CFG_15_reg, 0x0FFFFFFF, 0x60000000); //switch uart pinmux 0x1806023c[31:28];//6: uart0_tx,<OD>//7: uart1_tx,<OD>
                                        rtd_maskl(PINMUX_ST_Pin_Mux_Ctrl1_reg, 0xFFFFFF0F, 0x0); //switch uart rx src 0xB8060268[7:4]=0 src0
                                        break;
                                case UART1_PINMUX_ENABLE:
                                        rtd_maskl(PINMUX_ST_GPIO_ST_CFG_11_reg, 0xFFFFFF0F, 0x70); //switch uart pinmux 0x1806022c[7:4];6: uart0_rxdi_src0,<I> //7: uart1_rxdi_src1,<I>
                                        rtd_maskl(PINMUX_ST_GPIO_ST_CFG_15_reg, 0x0FFFFFFF, 0x70000000); //switch uart pinmux 0x1806023c[31:28];//6: uart0_tx,<OD>//7: uart1_tx,<OD>
                                        rtd_maskl(PINMUX_Pin_Mux_Ctrl6_reg, 0xFFFFF0FF, 0x100); //switch uart1 rx src 0x180008E8[11:8]=1 src1
                                        break;
                                case UART0_UART1_PINMUX_DISABLE:
                                        rtd_maskl(PINMUX_ST_GPIO_ST_CFG_11_reg, 0xFFFFFF0F, 0xF0); //switch uart pinmux
                                        rtd_maskl(PINMUX_ST_GPIO_ST_CFG_15_reg, 0x0FFFFFFF, 0x60000000); //switch uart pinmux 0x1806023c[31:28];//6: uart0_tx,<OD>//7: uart1_tx,<OD>
                                        break;
                                default:
                ...
                {
                        unsigned int tmp_val = rtd_inl(PINMUX_ST_GPIO_ST_CFG_11_reg);

                        if( (tmp_val & 0x60) == 0x60)
                        {
                                tmp_val = 1;
                        }
                        else
...
long misc_ioctl_compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
