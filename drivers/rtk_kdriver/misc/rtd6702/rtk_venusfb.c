...
//fbdc: 1080P: 126K
static unsigned int gFB_EXTRA_SIZE = 0; //136*1024*3;  // without compression
...
static void reset_video_info(struct venusfb_info *fbi)
{
	if (fbi->video_info.mixer2) {
		rtd_pr_venusfb_err("VenusFB: mixer2 should be NULL...\n");
		BUG();
	}
...
// temp disabling DMA map/unmap for Android 64-bit, it suppose be ok due to flush would be performed when using dvr_malloc_
	...
	fbi->fb.screen_base             = (char*)fbi->video_info.videomemory;//(char __iomem *)(UNCAC_ADDR((unsigned int)fbi->video_info.videomemory));
...
static int video_initDisplayArea(struct venusfb_info *fbi)
{
	...
	rtd_pr_venusfb_debug("\n\r fun=%s line=%d addr=%lx\n",__FUNCTION__,__LINE__,(unsigned long)__builtin_return_address(0));
#else
	rtd_pr_venusfb_debug("\n\r fun=%s line=%d addr=%x\n",__FUNCTION__,__LINE__,(unsigned int)__builtin_return_address(0));
#endif
...
//RPC maintain
	if (send_rpc_command(RPC_VIDEO, RPCCMD_CONFIG_VIDEO_STANDARD, UNCAC_BASE|(unsigned long)page_address(page), 0, &ret))
		goto fail;
	rtd_pr_venusfb_info(" #@# config video standard, ret = %lx \n", ret);

	if (ret != S_OK) {
		rtd_pr_venusfb_err("VenusFB: Config video standard fail...\n");
		goto fail;
	}
...
//RPC maintain
	if (send_rpc_command(RPC_VIDEO, RPCCMD_CONFIG_HDMI, UNCAC_BASE|(unsigned long)page_address(page), 0, &ret))
		goto fail;
	rtd_pr_venusfb_info(" #@# config hdmi, ret = %lx \n", ret);

	if (ret != S_OK) {
		rtd_pr_venusfb_err("VenusFB: Config hdmi fail...\n");
		goto fail;
	}
...
//RPC maintain
	if (send_rpc_command(RPC_VIDEO, RPCCMD_SET_BKGRND, UNCAC_BASE|(unsigned long)page_address(page), 0, &ret))
		goto fail;
	rtd_pr_venusfb_info(" #@# set background, ret = %lx \n", ret);

	if (ret != S_OK) {
		rtd_pr_venusfb_err("VenusFB: Set background fail...\n");
		goto fail;
	}
#endif
...
//RPC maintain
	...
	if (ret != S_OK) {
		rtd_pr_venusfb_err("VenusFB: Config graphic canvas fail...\n");
		goto fail;
	}
...
#if defined(CONFIG_ARM64)
	rtd_pr_venusfb_debug("\n\r fun=%s line=%d addr=%lx cmd=%d\n",__FUNCTION__,__LINE__,(unsigned long)__builtin_return_address(0),cmd);
#else
	rtd_pr_venusfb_debug("\n\r fun=%s line=%d addr=%x cmd=%d\n",__FUNCTION__,__LINE__,(unsigned int)__builtin_return_address(0),cmd);
#endif
...
static int venusfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	if (fbi->video_info.winID == -1) {
		rtd_pr_venusfb_err(" #@# no window no pan...\n");
		return 0;
	}
	...
	if (page != NULL){
		rtd_pr_venusfb_debug(" #@# PBuffer: %p...\n", page_address(page));
	}else{
		rtd_pr_venusfb_err("alloc_page fail !! \n");
		goto fail;
	}
//	rtd_pr_venusfb_info(" #@# PBuffer: %p...\n", page_address(page));
	...
	if (1)
	{
		...
		if(structModifyGraphicWin == NULL){
			rtd_pr_venusfb_err("structModifyGraphicWin is NULL !! \n");
			return 0;
		}
...
//		pitch 	= ((var->xres_virtual * (fbi->video_info.bpp / 8) + 0x0F) >> 4 ) << 4;
	...
	if ((pitch * var->yres_virtual) > fbi->video_info.videomemorysize) {
		if (fbi->video_info.winID != -1) {
			...
			video_freeMemory(fbi);
			fbi->video_info.videomemorysize = PAGE_ALIGN((pitch * var->yres_virtual) + gFB_EXTRA_SIZE);
			rtd_pr_venusfb_info("FB size 3: %d %u\n", fbi->video_info.videomemorysize, gFB_EXTRA_SIZE);
...
static struct venusfb_info *  venusfb_init_fbinfo(struct platform_device *dev)
{
	...
	if (!fbi) {
		rtd_pr_venusfb_err("VenusFB: framebuffer_alloc(%d) failed ..\n", (unsigned int)(sizeof(struct venusfb_info) - sizeof(struct fb_info)));
		goto err;
	}
