...
static int kbrook(void *arg)
{
    ...
    for(;;) {
        ...
        data++;
        if ( retry > 50 )
                rtd_pr_misc_err("%s(): thread%d [%d]   UUID ---[%x] RETRY ---[%d]\n", __FUNCTION__, d, data, SEMA_HW_SEM_0_SCPU_2 | d<<4, retry);
        retry = 0;
        udelay(10);
        rtd_swsem_unlock( SEMA_HW_SEM_0_SCPU_2 | d<<4);

    }
    rtd_pr_misc_err("break\n");
    do_exit(0);
    return 0;
}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
static void timer_do(struct timer_list *data)
#else
                           ...
                           const char *buf, size_t n)
{
	...
	else if ( !strncmp(buf, "thread", 6) )
	{
		for( i=0;i<MTH;i++ )    {
			...
			brook_tsk[i] = kthread_create(kbrook, (void *)&i, "brook");
			if (IS_ERR(brook_tsk[i])) {
				err = PTR_ERR(brook_tsk[i]);
				goto END_TEST;
			}
                }
		for( i=0;i<MTH;i++ )    {
	...
	else if ( !strncmp(buf, "verify", 6) )
	{
		int error = 0;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
		timer_setup(&sema_timer, timer_do, 0);
#else
...
static int __init rtk_semaphore_init(void)
{
    ...
    RTK_SEM_INFO( "Hello, rtk_semaphore\n");
...
{
	RTK_SEM_INFO( "Goodbye, rtk_semaphore\n");
}
