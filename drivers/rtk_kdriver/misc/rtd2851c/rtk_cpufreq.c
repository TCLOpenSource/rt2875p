...
DSU_P0_Rate
*/
int *rtk_cpufreq_tab_ptr = 0;
int rtk_cpufreq_get_pll(int idx)
{
	if(rtk_cpufreq_tab_ptr == 0)
	{
		struct device_node *np = NULL;
		rtk_cpufreq_tab_ptr = kzalloc(sizeof(int) * idx, GFP_KERNEL);

		if(!rtk_cpufreq_tab_ptr)
			panic("%s: failed to kzalloc cpufreq table\n",__func__);

		np = of_find_compatible_node(NULL,NULL,"arm,rtk-cpufreq");
		if (!np)
			panic("%s: failed to find cpufreq table on DT node\n",__func__);

		if(of_property_read_u32_array(np, "freq", rtk_cpufreq_tab_ptr, idx))
			panic("%s: failed to read opp cpufreq array\n",__func__);

		rtd_pr_cpu_freq_info("rtk_cpufreq got data(%d)\n", idx);
		return 0;
	}
	else
		return rtk_cpufreq_tab_ptr[idx];
...
#if 0 //no need R pll due to same with P pll
...
static int rtk_cpufreq_init(struct cpufreq_policy *policy)
{
...

	{
		...
		info->control_pll = ((policy->cpu) == 0) ? (1) : (0);	//2851c have one policy only
		...
		/*to get cpufreq data from dts*/
		rtk_cpufreq_get_pll(max_opps + 2);

		info->Rpll = rtk_cpufreq_get_pll(0); //rtk_cpufreq_get_R_pll();
		info->Ppll = rtk_cpufreq_get_pll(0);
		...
		rtd_pr_cpu_freq_info("(%d)set PLL(%d/%d/%d/%d/%d/%d/%d)\n",
			policy->cpu, info->Rpll, info->Ppll, info->Apll, info->Xpll, rtk_cpufreq_get_pll(0), rtk_cpufreq_get_pll(1), max_opps);
	}
...
/* MAC8PBU-205: no need to run EAS on 2851c
 */
#if 0
...
/*[MAC8PBU-202]*/
#if 1
#define THERMAL_PRODUCTION_PARAMETER	5
#define CPUFREQ_LEVEL_0_UP_DEGREE	(120 - THERMAL_PRODUCTION_PARAMETER)
#define CPUFREQ_LEVEL_1_DOWN_DEGREE	(115 - THERMAL_PRODUCTION_PARAMETER)
