...
#else
        rtk_inv_range((void *)sem_record,(void *) ((unsigned long)(sem_record+9)-1));
#endif
}
...
void dump_sem_data( void )
{
	...
	for (i=0; i<SEMA_HW_SEM_TOP; i++) {
		...
		if ( j & 1 )
			...
			j);
                RTK_SEM_ERR( " SW sema [0x%zx], Owner [0x%x], return address (0x%x) \n",
			(size_t) &sem_record[i],
...
static int __init rtd_semaphore_init(void)
{
...
static void __exit rtd_semaphore_exit(void)
{
	RTK_SEM_INFO(KERN_INFO "Goodbye, rtd_semaphore\n");
}
...
inline void _rtd_swsem_lock(int checkid, enum LOCK_UUID uuid, unsigned long ip)
{
	...
	while ( ( swsem = _rtd_swsem_trylock(checkid, uuid, ip) ) != 1 )
	{
		...
		if (--timeout == 0) {
			...
			if ( swsem == -EAGAIN )
			{
				RTK_SEM_ERR( " SW sem %zx owned by [0x%x] return address [0x%x], but try lock by %x \n",
					(size_t) &sem_record[checkid],
		...
		if ( (timeout & 0x0001FFFF) == 0 )
		{
			...
			if ( swsem == -EAGAIN )
			{
				RTK_SEM_WARNING( " sem 0x%zx owned by 0x%x, RA [0x%x], but wait by 0x%x RA [0x%x]",
					(size_t) &sem_record[checkid],
					(unsigned int) sem_record[checkid].sw_sem_uuid,
					(unsigned int) sem_record[checkid].sw_sem_lr,
					uuid,
...
void _rtd_swsem_unlock( int checkid, enum LOCK_UUID uuid, unsigned long ip )
{
	...
	invalidate_status_flag( );
		...
		RTK_SEM_ERR( "\n\n\n\n"
			"[SW Semaphore] sem 0x%zx owned by 0x%x RA [0x%x], but Unlock by 0x%x"
                        "\n\n\n\n",
                        (size_t) &sem_record[checkid],
			(unsigned int) value,
...
EXPORT_SYMBOL(hw_sema);
EXPORT_SYMBOL(_rtd_swsem_lock);
EXPORT_SYMBOL(_rtd_swsem_unlock);
EXPORT_SYMBOL(_rtd_swsem_trylock);
EXPORT_SYMBOL(_rtd_hwsem_lock);
EXPORT_SYMBOL(_rtd_hwsem_trylock);
EXPORT_SYMBOL(_rtd_hwsem_unlock);
EXPORT_SYMBOL(_rtd_hwsem_unlock_nocheck);
EXPORT_SYMBOL(dump_sem_data);


module_init(rtd_semaphore_init);
module_exit(rtd_semaphore_exit);
