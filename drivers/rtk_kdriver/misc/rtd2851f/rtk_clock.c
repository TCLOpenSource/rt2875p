...
#if 0 //no need to control gpu's dfs on 2851f
...
#define VOLTAGE_LEVEL_DEDAULT	SCPU_L
...
void rtk_set_voltage(struct rtk_cpufreq_info *info, unsigned int pll, unsigned int rate)
{
	...
	/* [MAC8PBU-199]
		2851f use two pin GPIO86/GPIO85 as SCPU_BOOT/CORE_VID
		The kdriver only control the SCPU_BOOT and CORE_VID is control by bootcode set by IDDQ.
		The DTS use four level {1/2/3/4} as original SCPU_BOOT/CORE_VID two pin mapping.
		But kdriver only need to control by using SCPU_BOOT pin only.
	*/
	voltage_level = (rtk_get_voltage_level(info, pll, rate) >> 1) + 1;
...
void rtk_set_default_voltage(void)
{
	...
	udelay(50);
	rtd_pr_cpu_freq_err("rtk_set_default_voltage:(%d), GPO(%x/%x)\n", voltage_level, rtd_inl(MIS_GPIO_GP2DATO_reg), rtd_inl(MIS_GPIO_GP2DIR_reg));
}
...
unsigned int rtk_get_boot_freq(int cluster_id)
{
	...
	if((pllscpu_divmode != 0) || (pllscpu_prediv != 0))
		rtd_pr_cpu_freq_err("%s: pllscpu_divmode(%d), pllscpu_prediv(%d) setting mismatch !!\n", __func__, pllscpu_divmode, pllscpu_prediv);
...
void rtk_clk_set_ratio(int cpu, unsigned int pll, unsigned int rate)
{
	...
	if(cpu == 0)
		...
		value &= (~SCPU_CORE_sys_scpu_clkratio_ctrl_0_clkratio_scpu_dsu_mask);

	if(cpu == 0)
		value |= (ratio << SCPU_CORE_sys_scpu_clkratio_ctrl_0_clkratio_scpu_shift);
	else
		value |= (ratio << SCPU_CORE_sys_scpu_clkratio_ctrl_0_clkratio_scpu_dsu_shift);
	...
	if(cpu == 0)
		...
		rtd_outl(SCPU_CORE_sys_scpu_clkratio_ctrl_1_reg, 1<<SCPU_CORE_sys_scpu_clkratio_ctrl_1_clkratio_scpu_dsu_active_shift);
...
static int rtk_cpufreq_get_dsu_pll(void)
{
	return rtk_cpufreq_get_pll(1);	//DSU_PLL
}

static int rtk_cpufreq_get_dsu_rate(int idx)
{
	return rtk_cpufreq_get_pll(3+idx); //DSU_R_Rate
}
...
int __rtk_clk_set_rate_ex(struct clk *clk, unsigned int rate, int algo_id, int cpu, int idx)
{
	...
	{
		//FIXME : consider put set ratio and dsu ratio together ...
		//set ratio
		rtk_clk_set_ratio(0, info->Ppll, rate);
		//set dsu ratio
		rtk_clk_set_ratio(1, rtk_cpufreq_get_dsu_pll(), rtk_cpufreq_get_dsu_rate(idx));
	}
	else
	{
		int pll;
		...
		//step3: set dsu ratio
		if(new_state == RTK_CPUFREQ_STATE_X || new_state == RTK_CPUFREQ_STATE_A)
			rtk_clk_set_ratio(1, pll, pll);	//set full ratio only, no need real pll value
		else	//RTK_CPUFREQ_STATE_P || RTK_CPUFREQ_STATE_R
			rtk_clk_set_ratio(1, rtk_cpufreq_get_dsu_pll(), rtk_cpufreq_get_dsu_rate(idx));
	}
