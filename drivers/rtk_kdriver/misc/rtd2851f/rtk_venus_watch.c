...
int set_watch_point(unsigned long addr, unsigned long prot)
{
...
#endif
	info->ctrl.privilege = 3;
	info->ctrl.enabled = 1;

	/* Type */
	switch (prot) {
	case HW_BREAKPOINT_X:
		rtd_pr_misc_debug("non support breakpoint\n");
		break;
	...
	case HW_BREAKPOINT_RW:
		...
		break;
	default:
		rtd_pr_misc_debug("no support watchpoint type, assert!\n");
		break;
	}
	...
	default:
		rtd_pr_misc_debug("no support watchpoint len, assert!\n");
		break;
	}

	info->address = addr & ~alignment_mask;
#ifndef CONFIG_ARM64
	rtd_pr_misc_debug("set_watch_point ctrl.en=%x, privilege=%x, type=%x, len=%x, mismatch=%x\n", 
		   info->ctrl.enabled, info->ctrl.privilege, info->ctrl.type, info->ctrl.len, info->ctrl.mismatch);
#else
	rtd_pr_misc_debug("set_watch_point ctrl.en=%x, privilege=%x, type=%x, len=%x\n", 
		   info->ctrl.enabled, info->ctrl.privilege, info->ctrl.type, info->ctrl.len);
#endif
	rtd_pr_misc_debug("set_watch_point addr=%x, ctrl=%x\n", info->address, encode_ctrl_reg(info->ctrl));
...
long venus_watch_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case WATCH_IOQGETASID:
		{
			...
			rtd_pr_misc_debug("get WCR= %x\n", val);

			asm volatile("mrc 14, 0, %0, cr0, cr0, 6" : "=r" (val)); //WVR
			rtd_pr_misc_debug("get WVR= %x\n", val);
#endif
			break;
		}
		case WATCH_IOSSETWATCH:
			...
			if (!ret) {
        #ifdef KERNEL_SPACE_ADDR_TEST
				rtd_pr_misc_debug("addr=%p, prot=%lx\n", &mytest, ws.prot);
				set_watch_point((unsigned long)&mytest, ws.prot);
        #else
				rtd_pr_misc_debug("addr=%lx, prot=%lx\n", ws.addr, ws.prot);
...
static char *venus_watch_devnode(struct device *dev, umode_t *mode)
{
	return NULL;
}
...
int venus_watch_init_module(void) {
	...
	if (result < 0) {
		rtd_pr_misc_err("watchpoint: can not register (%d %d)...\n", watch_major, watch_minor);
		return -ENODEV;
	}
	...
	rtd_pr_misc_info("venus watch major number: %d\n", watch_major);
	return 0;
}
