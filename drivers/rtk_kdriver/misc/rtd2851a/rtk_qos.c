...
#endif
extern unsigned char Get_rotate_function(unsigned char display);
...
int rtk_qos_bw_get_all_client_record(SRAM_RECORD_t *sram_record_p, unsigned int mc_idx, unsigned int record_idx, unsigned int curr_pos)
{
	...
	{
		rtd_pr_qos_err("%s:%d [ERR] record number(%d) large than 7\n", __FUNCTION__, __LINE__, record_idx);
		return 1;
	}
	...
	{
		rtd_pr_qos_err("%s:%d [ERR] wrong mc_idx(%d)\n", __FUNCTION__, __LINE__, mc_idx);
		return 1;
	}
...
int rtk_qos_bw_start_measure(unsigned int n_sec)
{
	...
	{
		rtd_pr_qos_err("%s:%d [ERR] qos_bw_measure_p[MC1] is NULL\n", __FUNCTION__, __LINE__);
		return 1;
	}
	...
	rtd_pr_qos_err("Stop Measure MC1 frame_cnt=%d conflict_cnt=%d\n", cur_record_frame[MC_1], conflict_occur_cnt[MC_1]);
...
{
	rtd_pr_qos_err("Free MC1 memory %p\n", qos_bw_measure_p[MC_1]);
...
{
	if (dram_info == NULL) {
		rtd_pr_qos_err("[ERR]%s(%d): dram_info is NULL\n", __FUNCTION__, __LINE__);
...
RTK_DC_BW_TYPE_t rtk_dc_query_dram_region_by_addr(unsigned int phy_addr)
{
	...
	if (ret != RTK_DC_RET_SUCCESS) {
		rtd_pr_qos_err("[ERR]%s(%d): fail to get dram_info\n", __FUNCTION__, __LINE__);
...
static int __init rtk_dram_info_dump(void)
{
	...
	rtd_pr_qos_err("[RTK-DC] %s:\n", __FUNCTION__);

	ret = rtk_dc_get_dram_info(&dram_info);

	if (ret == RTK_DC_RET_SUCCESS)
	{
		rtd_pr_qos_err("dram_info->dram_num is %x\n", dram_info.dram_num);
		rtd_pr_qos_err("dram_info->swap_en is %x\n", dram_info.swap_en);
		rtd_pr_qos_err("dram_info->sw_mode is %x\n", dram_info.sw_mode);
		rtd_pr_qos_err("dram_info->sw_offset is %x\n", dram_info.sw_offset);
		rtd_pr_qos_err("dram_info->boundary is %x\n", dram_info.boundary);
	}
	else
	{
		rtd_pr_qos_err("fail to get dram info\n");
	}

	rtd_pr_qos_err("0x12000000 is %s bw region\n", rtk_dc_query_dram_region_by_addr(0x12000000)?"lower":"higher");
	rtd_pr_qos_err("0x20000000 is %s bw region\n", rtk_dc_query_dram_region_by_addr(0x20000000)?"lower":"higher");
	rtd_pr_qos_err("0x21000200 is %s bw region\n", rtk_dc_query_dram_region_by_addr(0x21000200)?"lower":"higher");
	rtd_pr_qos_err("0x28004000 is %s bw region\n", rtk_dc_query_dram_region_by_addr(0x28004000)?"lower":"higher");
	rtd_pr_qos_err("0x34004200 is %s bw region\n", rtk_dc_query_dram_region_by_addr(0x34004200)?"lower":"higher");
