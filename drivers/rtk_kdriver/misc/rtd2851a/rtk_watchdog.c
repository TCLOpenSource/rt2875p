	...
	if (fmt) {
		...
		if (debugMode == RELEASE_LEVEL)
			...
			if(rtd_show_log[g_rtdlog_panic_cpu_type]){
			        ...
			        if (rtdlog_watchdog_dump_work((EnumLogbufType)g_rtdlog_panic_cpu_type))
...
//#if 0 /*no need to turn on watchdog, watchdog on/off control by bootcode.*/
	...
	if(enable_check_av_watchgod)
	{
		...
		if (time_is_before_jiffies(to_k)) {
			...
			rtd_pr_misc_err("read 0x18000100 val == %#x read 0x18000110 val == %#x \n", rtd_inl(0xb8000100), rtd_inl(0xb8000110));
...
#if 0  //FIXME: for CARVEDOUT_MAP_GIC not defined  compile errror
...
static int rtk_panic(struct notifier_block *self, unsigned long v, void *p)
{
        if(console_loglevel == 0) {
                ...
                rtd_pr_misc_emerg("Kernel panic - not syncing: %s\n", p);
...
#if 0//def CONFIG_CUSTOMER_TV043
    if(request_irq(WDOG_NMI_IRQ_NUM,
                   wdog_NMI_intr,
                   IRQF_SHARED,
                   "wdog",
                   wdog_NMI_intr)) {
            rtd_pr_misc_err( "wdog: cannot register NMI IRQ %d\n", IRQ_CEC);
            return -1;
    }
#endif   
...
*/
	...
	rtd_pr_misc_info("rtk_wdog_init_module successfully!! \n");
