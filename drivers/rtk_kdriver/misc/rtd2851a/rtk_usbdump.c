...
static int cat_proc_mounts (STRUCT_UINFO * info, char * path)
{
    ...
    {
        //rtd_pr_misc_err("aim=%s\n",aim);
        //rtd_pr_misc_err("aim_block=%s\n",aim_block);
        //rtd_pr_misc_err("aim_fuse=%s\n",aim_fuse);
        //rtd_pr_misc_err("buf_file=%s\n",buf_file);
        if(0 == parse_line(buf_file, aim, aim_block,aim_fuse, path))
        {
            ...
            rtd_pr_misc_err("%s %d  usb_path=%s\n",__func__,__LINE__,usb_path);
            ret = 0;
            goto RET;
        }
        memset(buf_file, '\0', buff_size);
...
void parse_recvmsg (char *recv_buf, int len)
{
    ...
    //rtd_pr_misc_err("%s %d  \e[1;31m%s\e[0m\n\n\n\n",__func__,__LINE__,recv_buf);
    ...
    while (i < len)
    {
        ...
        {
            rtd_pr_misc_err("GET USB_UEVENT  add \n");
            goto USB_ADD;
        }
        else if ( USB_CHECK_REMOVE_AIM == usb_check_process )
        {
            rtd_pr_misc_err("GET USB_UEVENT  remove \n");
            rtk_reset_usb_path();
            return;
        }
        else
