	...
	{//letency parameter change
		...
		if(video_latency_pattern.bOnOff)
		{
				...
				{//PTG disable -> enable
					...
					*/
					IoReg_SetBits(PINMUX_MAIN_ST_GPIO_ST_CFG_3_reg, _BIT15|_BIT14|_BIT13|_BIT12);
					IoReg_Write32(ISO_GPIO_GPDIR_1_reg, _BIT30|_BIT0);//GPIO
					IoReg_Write32(ISO_GPIO_GPDATO_1_reg, _BIT30);//default low
				}
				//set PTG window size
				...
				if(video_latency_pattern.bPatternType == KADP_VSC_PATTERN_WHITE){//white
					...
					IoReg_Write32(ISO_GPIO_GPDATO_1_reg, _BIT30|_BIT0);//white
				}
				else{//balck
					if(i3ddma_rgb2yuv_dither_4xxto4xx_hdmi_rgb2yuv_ctrl_reg.en_rgb2yuv){
					...
					IoReg_Write32(ISO_GPIO_GPDATO_1_reg, _BIT30);//black
				}
		...
		if(get_seamless_trigger_flag())
		{
		    ...
		    if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI))
            {
            	if(seamless_info_sharememory)
            	{
	                ...
	                {
	                    spin_lock(get_hdr_seamless_change_spinlock());//lock i3ddma change spinlock
	                    set_seamless_trigger_flag(FALSE);
                        ...
                        spin_unlock(get_hdr_seamless_change_spinlock());//unlock i3ddma change  spinlock                        						
	                }
            	}
            }
            else
		...
		if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)
		{
        	if(get_hdr_semaless_active())
        	{
        		if(seamless_info_sharememory && seamless_info_sharememory->hdmi_not_ready)
        		{
                    ...
                    spin_unlock(get_hdr_seamless_change_spinlock());//unlock i3ddma change  spinlock
                    dynamic_change_hdr_setting_hdmi_position((Scaler_ChangeUINT32Endian((seamless_info_sharememory->hdmi_trigger_vo_hdr_mode)) & _BIT0)? TRUE : FALSE, seamless_info_sharememory->hdmi_target_color_format, Scaler_ChangeUINT32Endian((seamless_info_sharememory->hdmi_trigger_vo_hdr_mode)) >> 1);
        		}
        	}
			else
			{
	...
	if(handle_ch1_end_flag && hdmi_finish_seamless)
	{
        ...
        if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (get_hdr_semaless_active()))
        {
        	if(seamless_info_sharememory && seamless_info_sharememory->hdmi_not_ready) {
                ...
                dynamic_change_hdr_setting_hdmi_finish((Scaler_ChangeUINT32Endian((seamless_info_sharememory->hdmi_trigger_vo_hdr_mode)) & _BIT0)? TRUE : FALSE, seamless_info_sharememory->hdmi_target_color_format, Scaler_ChangeUINT32Endian((seamless_info_sharememory->hdmi_trigger_vo_hdr_mode)) >> 1);
        	}
        }
		spin_lock(get_hdr_seamless_change_spinlock());//lock i3ddma change spinlock
		set_seamless_trigger_flag(FALSE);
...
if((reg_value & BIT(30))){
	...
	if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) ||(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_CAMERA)|| ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && ((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) || (get_hdmi_4k_hfr_mode() == HDMI_3K120)))) {
		if(handle_ch1_sta_flag)
		{
...
static int __init vgip_isr_init_irq(struct platform_device *pdev)
{
	...
	// enable VGIP interrupt
	rtd_setbits(VGIP_VGIP_CHN1_CTRL_reg, BIT(25));
	// enable i3ddma rooute to SCPU
	//rtd_outl(SYS_REG_INT_CTRL_MEMC_reg, SYS_REG_INT_CTRL_MEMC_i3ddma_int_scpu_routing_en_mask|SYS_REG_INT_CTRL_MEMC_write_data_mask);
