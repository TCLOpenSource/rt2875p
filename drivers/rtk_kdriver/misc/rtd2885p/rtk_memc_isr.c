...
static int memc_isr_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	if( (Scaler_MEMC_CLK_Check() == FALSE) || (get_MEMC_bypass_status_refer_platform_model() == TRUE)){//KTASKWBS-24448
		return FALSE;
	}
	// disable interrupt
	//reg_kmc_int_en
...
static int __init memc_isr_init_irq(struct platform_device *pdev)
{
	...
	ret = request_irq(irq, memc_isr, IRQF_SHARED, "MEMC ISR",
			(void *)pdev);
	if (ret) {
		rtd_pr_memc_notice("memc_isr: cannot register IRQ %d\n", (int)irqd_to_hwirq(irq_get_irq_data(irq)));
		return ret;
	}
	...
	rtd_pr_memc_notice("memc_isr: register IRQ %d\n", (int)irqd_to_hwirq(irq_get_irq_data(irq)));
	//dev_info(&pdev->dev, "memc_isr: register IRQ %d\n", irqd_to_hwirq(irq_get_irq_data(irq)));
...
static int __init memc_isr_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		rtd_pr_memc_notice("memc_isr: can not register irq...\n");
		return ret;
	}
