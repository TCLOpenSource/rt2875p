...
#include <rtd_log/rtd_module_log.h>       //rtd_printk
...
int dbg_help_kmalloc(char **p, int size)
{
        ...
        if(*p == NULL){
                *p = kmalloc(size*sizeof(char), GFP_KERNEL);
                ...
                if(*p == NULL){
                        rtd_pr_online_help_err("kmalloc error\n");
                        return -1;
                }                
        }
        ...
        for(i=1; i<num; i++){
                if(strncasecmp(argv[i],HELP_SUB_RTD_TOOL, sizeof(HELP_SUB_RTD_TOOL)-1) == 0){
                        ...
                        *(p_dcmt_sub_opt+DCMT_SHELL_CMD) = 1;
                        ...
                        rtd_pr_online_help_err("[warning] Unknow operation '%s'\n", argv[i]);
                }
        }
...
int dbg_help_printf(char __user * buf, loff_t *ofst, const char * fmt, ...)
{
        ...
        if(tmp_buf == NULL){
                rtd_pr_online_help_err("kmalloc fail\n");
                return 0;
        }
...
int dbg_help_get (char __user * buf, loff_t *ofst)
{
        ...
        for(opt_main=0; opt_main<OPT_MAX_NUM; opt_main++){
                if(*(p_main_opt+opt_main)){
                        //rtd_pr_online_help_err("p_main_opt[%d]=%d\n", opt_main, *(p_main_opt+opt_main));
...
void dbg_help_set (const char *buffer)
{
        ...
        if(arg_num <=0){
                rtd_pr_online_help_err("have not arg, num=%d\n", arg_num);
                return;
        }
        ...
        //arm_wrapper                
                ...
                //rtd_pr_online_help_err("main arm_wrapper opt=%s\n", argv[0]);
        ...
        //usbdump
                ...
                //rtd_pr_online_help_err("main usbdump opt=%s\n", argv[0]);
        ...
        //rtd_tool
                ...
                *(p_main_opt+HELP) = 1;
                ...
                rtd_pr_online_help_err("unknown main opt=%s\n", argv[0]);     
                return;
        }
...
long dbg_help_ioctl(struct file             *filp,
        ...
        unsigned long           arg)
{
        rtd_pr_online_help_warn("ioctl device node\n");
        return 0;
}

int dbg_help_open(struct inode* inode, struct file* filp)
{
        rtd_pr_online_help_warn("open device node\n");
        return 0;
}

int dbg_help_release(struct inode* inode, struct file* filp)
{
        rtd_pr_online_help_warn("release device node\n");
        return 0;
}
...
ssize_t dbg_help_write(struct file *file,const char __user *buff, size_t size, loff_t *ofst)
{
        ...
        if(size >= DBG_BUF_SIZE){
                rtd_pr_online_help_err("input len beyond bufsize\n");
                goto ret;
        }

        memset(str, 0, sizeof(str));
        if(copy_from_user(str, buff, size) !=0){
                rtd_pr_online_help_err("copy from user error\n");
                goto ret;
        }
...
static int __init dbg_help_module(void)
{
        ...
        rtd_pr_online_help_warn(" ***************** dbg_help  init module ********************* \n");
        ...
        if (result < 0) {
                rtd_pr_online_help_err("dbg_help: can't get major %d\n", dbg_help_major);
                return result;
        }

        rtd_pr_online_help_warn("dbg_help: get result:%d,get dev:0x%08x,major:%d\n", result, dev, dbg_help_major);
        ...
        if (result) {
                ...
                rtd_pr_online_help_err("Error %d adding cdev rtdlog", result);
                goto fail;
        }
...
static void __exit dbg_help_cleanup_module(void)
{
        ...
        rtd_pr_online_help_warn("hw monitor clean module\n");
