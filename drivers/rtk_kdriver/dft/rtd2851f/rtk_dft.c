						...
						char *buf)
{
	...
	if(check_pok_data())
	{
		snprintf(str, sizeof(str), "Get POK Result Fail: checksum error!\n");
	}
	else
	{
		get_ddr_byte_num();
		...
		if(res)
		{
			snprintf(str, sizeof(str), "Get Tx Result Fail\n");
		}
		else
		{
			snprintf(str, sizeof(str), "Tx Vref: %d.%02d%c %d.%02d%c\nTx DQS_PI: ", (tx_vref[0]/100), (tx_vref[0]%100), 0x25, (tx_vref[1]/100), (tx_vref[1]%100), 0x25);
			for(i = 0; i < byte_num; i++)
				snprintf(str + strlen(str), sizeof(str), "0x%02x ", dqspi[i]);

			snprintf(str + strlen(str), sizeof(str), "\n");
		}
		res = get_pok_rx_result((unsigned int *)&rx_vref, (unsigned char *)&te);
		if(res)
		{
			snprintf(str + strlen(str), sizeof(str), "Get Rx Result Fail\n");
		}
		else
		{
			snprintf(str + strlen(str), sizeof(str), "Rx Vref: %d%c %d%c \nRx Te:\n", rx_vref[0], 0x25, rx_vref[1], 0x25);
			for(i = 0; i < byte_num; i++)
			{
				for(j = 0; j < 8; j++)
					snprintf(str + strlen(str), sizeof(str), "0x%02x ", te[i][j]);
