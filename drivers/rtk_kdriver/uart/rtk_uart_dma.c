...
void *g_rtk_uart_dma_vbuf = NULL;
dma_addr_t g_rtk_uart_dma_dbuf = 0;
...
int rtk_serial8250_request_dma(struct uart_8250_port *up)
{
    ...
    if(!g_rtk_uart_dma_vbuf)
        g_rtk_uart_dma_vbuf = dma_alloc_coherent(up->port.dev, UART_XMIT_SIZE,
                                   &g_rtk_uart_dma_dbuf, GFP_KERNEL);
    ...
    rtk_dma->tx_dma_ring_buf_head = g_rtk_uart_dma_vbuf;
    rtk_dma->tx_dma_addr = g_rtk_uart_dma_dbuf;
    ...
    dev_dbg_ratelimited(up->port.dev, "tx_buf_head == 0x%lx, tx_buf_align == 0x%lx, dma_ring_buf_len == %d tx_addr == 0x%08x\n",
                        (unsigned long)rtk_dma->tx_dma_ring_buf_head,
                        (unsigned long)rtk_dma->tx_dma_ring_buf_align, rtk_dma->tx_dma_ring_buf_size, rtk_dma->tx_dma_addr);
    ...
    serial_port_out(port, dma_reg_ofst + UART_DMA_TXTHD,
                    UR1_TX_THD(rtk_up->rtk_dma_config.dma_tx_thd));
    ...
    */
    ...
    serial_port_out(port, dma_reg_ofst + UART_DMA_CTRL,
                    UR1_TXEMPTHR(rtk_up->rtk_dma_config.dma_tx_empty_thd) | UR1_DMAIFEN(0));

    rtk_up->rtk_dma = rtk_dma;
...
static void _rtk_serial8250_force_dma_done(struct uart_8250_port *up)
{
    struct rtk_uart_port *rtk_up = up_to_rtk_uart_port(up);
    struct uart_port *port = &up->port;
    int dma_reg_ofst = rtk_up->rtk_dma_config.dma_membase_ofst;
    ...
    unsigned int dma_read_pos = 0;
    unsigned int dma_write_pos = 0;
    unsigned int dma_tx_limit = 0;
    unsigned int dma_tx_base = 0;

    dma_tx_limit = serial_port_in(port, dma_reg_ofst + UART_DMA_TXLIMIT);
    dma_tx_base = serial_port_in(port, dma_reg_ofst + UART_DMA_TXBASE);
	
    dma_write_pos = serial_port_in(port, dma_reg_ofst + UART_DMA_TXWRPTR);
    dma_read_pos = serial_port_in(port, dma_reg_ofst + UART_DMA_TXRDPTR);
    

    if(dma_write_pos > dma_read_pos) {
	if((dma_write_pos - dma_read_pos) > rtk_up->rtk_dma_config.dma_tx_empty_thd) {
		unsigned int dma_reset_read_pos = dma_write_pos - rtk_up->rtk_dma_config.dma_tx_empty_thd;
		timeout = 0;
		while(1) {
			serial_port_out(port, dma_reg_ofst + UART_DMA_TXRDPTR, dma_reset_read_pos);
			ndelay(1);
			if(serial_port_in(port, dma_reg_ofst + UART_DMA_TXRDPTR) == dma_reset_read_pos)
				break;
			if(timeout++ > 200000)
				break;
		}
	}
    } else if(dma_write_pos < dma_read_pos) {
    	if((dma_tx_limit - dma_read_pos + dma_write_pos - dma_tx_base) > rtk_up->rtk_dma_config.dma_tx_empty_thd) {
		unsigned int dma_reset_read_pos = dma_write_pos - rtk_up->rtk_dma_config.dma_tx_empty_thd;
		if(dma_reset_read_pos < dma_tx_base)
			dma_reset_read_pos = dma_tx_limit - rtk_up->rtk_dma_config.dma_tx_empty_thd;
		timeout = 0;
		while(1) {
			serial_port_out(port, dma_reg_ofst + UART_DMA_TXRDPTR, dma_reset_read_pos);
			ndelay(1);
    ...
    timeout = 0;
    while(serial_port_in(port, dma_reg_ofst + UART_DMA_TXRDPTR) 
		!= serial_port_in(port, dma_reg_ofst + UART_DMA_TXWRPTR)) {
	ndelay(10);
	if(timeout++ > 50000) {
		break;
    ...
    serial_port_out(port, dma_reg_ofst + UART_DMA_CTRL,
                    UR1_TXEMPTHR(rtk_up->rtk_dma_config.dma_tx_empty_thd) | UR1_DMAIFEN(0));
    serial_port_out(port, dma_reg_ofst + UART_DMA_RST, UR1_TX_DMA_FSM(1));
    serial_port_out(port, dma_reg_ofst + UART_DMA_RST, UR1_TX_DMA_FSM(0));
...
void rtk_serial8250_release_dma(struct uart_8250_port *up)
{
    ...
    _rtk_serial8250_force_dma_done(up);
	
    rtk_dma = rtk_up->rtk_dma;
    rtk_up->rtk_dma = NULL;
#ifdef CONFIG_ARCH_RTK2851A 
...
{
    struct rtk_uart_port *rtk_up = up_to_rtk_uart_port(up);
    struct uart_8250_rtk_dma *dma = rtk_up->rtk_dma;
    struct uart_port *port = &up->port;
    struct circ_buf *xmit = &port->state->xmit;
    int dma_reg_ofst = rtk_up->rtk_dma_config.dma_membase_ofst;
    int dma_read_ofst = 0;
    ...
    dma_read_ofst = serial_port_in(port, dma_reg_ofst + UART_DMA_TXRDPTR) - rtk_up->rtk_dma->tx_dma_addr;
    ...
    serial_port_out(port, dma_reg_ofst + UART_DMA_CTRL,
                    UR1_TXEMPTHR(rtk_up->rtk_dma_config.dma_tx_empty_thd) | UR1_DMAIFEN(1));
...
proc_disable_print:
    ...
    serial_port_out(port, dma_reg_ofst + UART_DMA_TXWRPTR, UR1_TX_WRPTR(rtk_up->rtk_dma->tx_dma_addr));
    serial_port_out(port, dma_reg_ofst + UART_DMA_TXRDPTR, UR1_TX_WRPTR(rtk_up->rtk_dma->tx_dma_addr));	
...
void rtk_serial8250_dma_wait_for_xmitr(struct uart_8250_port *up)
{
    ...
    for (;;) {
        dma_read_ofst = serial_port_in(port, dma_reg_ofst + UART_DMA_TXRDPTR) - rtk_up->rtk_dma->tx_dma_addr;
        dma_write_ofst = serial_port_in(port, dma_reg_ofst + UART_DMA_TXWRPTR) - rtk_up->rtk_dma->tx_dma_addr;
...
static void _rtk_serial8250_dma_console_write(struct uart_port *port,
        const unsigned char *buf, int count)
{
    __maybe_unused struct uart_8250_port *up = up_to_u8250p(port);
    int sent_count = 0;
    int ret = 0;
    unsigned int timeout = 500000;/*500ms*/
#if !(IS_ENABLED(CONFIG_REALTEK_RTICE) && IS_ENABLED(CONFIG_RTK_KDRV_RTICE))
