...
#define SPI_RECEIVE_UNIT             384		/*384bytes */
#define SPI_TRANSFER_MAX_LENGTH      256        /*256bytes*/
/*********************************************************
*
*	var define
*
...
static void rtk_spi_master_md_process(unsigned long flash_addr, unsigned int len, const unsigned char *buf, unsigned char is_read)
{
	...
	while(processed_len < total_len){
		if((total_len - processed_len) >= SPI_RECEIVE_UNIT)
			cur_process_len = SPI_RECEIVE_UNIT;
		else
										...
										unsigned int len)
{
	...
	if(addr_mode_enable){
		...
		}else if(len <= SPI_TRANSFER_MAX_LENGTH){
			/*md mode*/
			rtk_spi_master_md_process(0x0, len-1, &tx_data[1], 0);
		}else{
			rtd_pr_spi_dev_err("transfer length %d is bigger than %d \r\n", len, SPI_TRANSFER_MAX_LENGTH);
			return -1;
		}
	}
										...
										unsigned int len)
{
	...
	if(addr_mode_enable){
		...
		}else if(len <= SPI_RECEIVE_UNIT){
			/*md mode*/
			//rtk_spi_master_md_process(0x0, len, rx_data, 1);
			memset(master->p_receive, 0x5A, SPI_RECEIVE_UNIT);  /*set special pattern*/
			rtk_spi_master_md_process(0x0, SPI_RECEIVE_UNIT, master->p_receive, 1);
			memcpy(rx_data, master->p_receive, len);
		}else{
			rtd_pr_spi_dev_err("receive length %d is bigger than %d \r\n", len, SPI_RECEIVE_UNIT);
			return -1;
		}
	}
...
static int rtk_spi_master_probe(struct platform_device *pdev)
{
	...
	/*malloc buffer for recevier data*/
	drv_data->p_receive = (unsigned char *)kmalloc(SPI_RECEIVE_UNIT, GFP_KERNEL);
	if(drv_data->p_receive == NULL){
		rtd_pr_spi_dev_err("malloc private recevier buffer fail!\r\n");
		goto exit;
	}
...
exit:
	if(drv_data->p_receive)
		kfree(drv_data->p_receive);
