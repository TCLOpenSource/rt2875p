...
extern int __init rtk_gpio_init(void);
extern void __exit rtk_gpio_exit(void);
                           ...
                           unsigned int cmd, unsigned long arg)
{
    ...
    memset(&gpio_irq_set,0,sizeof(gpio_irq_set));
    memset(&gpio_irq_en,0,sizeof(gpio_irq_en));
    memset(&gpio_irq_flag,0,sizeof(gpio_irq_flag));
    memset(&gpio_all_gpio_pin,0,sizeof(gpio_all_gpio_pin));

    switch (cmd) {
        case RTK_GPIO_SET_VAL:
            len  = sizeof(gpio_val.pin_name);
            if (copy_from_user ((void *)&gpio_val, (const void __user *)arg, sizeof(RTK_GPIO_VAL_T))) {
                ret = -EFAULT;
                break;
            }
            gpio_val.pin_name[len-1] = 0;

            if(strlen(gpio_val.pin_name) == 0)
                return -EFAULT;
        ...
        case RTK_GPIO_GET_VAL:
            ...
            dir = rtk_gpio_get_dir(pinInfo.gid);
            if(dir==PCB_GPIO_TYPE_OUPUT){
        ...
        case RTK_GPIO_SET_DIR:
            len  = sizeof(gpio_dir.pin_name);
            if (copy_from_user ((void *)&gpio_dir, (const void __user *)arg, sizeof(RTK_GPIO_DIR_T))) {
                return -EFAULT;
            }
            gpio_dir.pin_name[len-1] = 0;
            if(strlen(gpio_dir.pin_name) == 0)
                return -EFAULT;
        ...
        case RTK_GPIO_SET_IRQ:
            len  = sizeof(gpio_irq_set.pin_name);
            if (copy_from_user ((void *)&gpio_irq_set, (const void __user *)arg, sizeof(RTK_GPIO_IRQ_SET_T))) {
                return -EFAULT;
            }
            gpio_irq_set.pin_name[len-1] = 0;
            if(strlen(gpio_irq_set.pin_name) == 0)
                return -EFAULT;
            ...
            if(pinInfo.gpio_type == ISO_GPIO || pinInfo.gpio_type == MIS_GPIO) {
                ...
                if (0 != ret)
                {
                    RTK_GPIO_IOCTL_DBG(" RTK_GPIO_SET_IRQ_PARAM: %s line: %d, set dir error\n", __func__, __LINE__);
                    return -EFAULT;
                }
            }
        ...
        case RTK_GPIO_SET_IRQ_EN:
            len  = sizeof(gpio_irq_en.pin_name);
            if (copy_from_user ((void *)&gpio_irq_en, (const void __user *)arg, sizeof(RTK_GPIO_IRQ_EN_T))) {
                return -EFAULT;
            }
            gpio_irq_en.pin_name[len-1] = 0;
            if(strlen(gpio_irq_en.pin_name) == 0)
                return -EFAULT;
        ...
        {
            len  = sizeof(gpio_irq_flag.pin_name);
            if (copy_from_user ((void *)&gpio_irq_flag, (const void __user *)arg, sizeof(RTK_GPIO_IRQ_FLAG_T))) {
                return -EFAULT;
            }
            // fix string may be not null terminated
            gpio_irq_flag.pin_name[len-1] = 0;

            if(strlen(gpio_irq_flag.pin_name) == 0)
                return -EFAULT;
...
#ifdef CONFIG_COMPAT
long rtk_gpio_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
 void __user *compat_arg = compat_ptr(arg);
 return rtk_gpio_ioctl(filp, cmd, (unsigned long)compat_arg);
}
#endif
...
int __init rtk_gpio_dev_init(void)
{
    ...
    rtk_gpio_init();    //for platform driver register
...
#endif
    rtk_gpio_exit();    //for platform unregister
}

fs_initcall(rtk_gpio_dev_init);
