...
static ssize_t rtk_gpio_proc_write(
    struct file *file,
    ...
    if (copy_from_user(str, buf, count)) {
        RTK_GPIO_ERROR("copy_from_user failed! (buf=%p, count=%zu)\n", buf, count);
        return -EFAULT;
    }
...
static ssize_t rtk_gpio_proc_read(
    struct file *file,
    char __user *buf,
    size_t count,
    loff_t *ppos)
...
static struct proc_ops  rtk_gpio_proc_fops = {
    .proc_write  = rtk_gpio_proc_write,
    .proc_read  = rtk_gpio_proc_read,
};
#else
