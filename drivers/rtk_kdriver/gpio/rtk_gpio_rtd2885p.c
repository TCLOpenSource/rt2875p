...
extern void rtk_hwcnt_clear_count(void);
//=============================================================================
...
#define ISO_GPIO_MIO_REG_SETS            ((ISO_GPIO_MIO_CNT+31)>>5)
...
extern void rtk_panel_error_detect_init(void);
extern void rtk_panel_error_detect_exit(void);
#endif

RTK_GPIO_PCB_T* pPcbInfo = NULL;
static RTK_GPIO_IRQ rtk_gpio_irq[MIS_GPIO_CNT + ISO_GPIO_CNT + GPI_MIS_CNT + GPI_ISO_CNT + MIO_GPIO_CNT + ISO_GPIO_MIO_CNT];

#ifdef ENABLE_GPIO_STR
/* only pcb need to  store and recovery gpio in/output values */
static unsigned int backup_mis_gpie[MIS_GPIO_REG_SETS];
static unsigned int backup_mis_dis_gpie[MIS_GPIO_REG_SETS];
...
static RTK_GPIO_IRQ *_get_gpio_entry(RTK_GPIO_ID gid)
{
    ...
    case MIO_GPIO:
        if (id < MIO_GPIO_CNT)
            return &rtk_gpio_irq[id + MIS_GPIO_CNT + ISO_GPIO_CNT + GPI_MIS_CNT + GPI_ISO_CNT];
        break;
#endif
    case ISO_GPIO_MIO:
        if (id < ISO_GPIO_MIO_CNT)
            return &rtk_gpio_irq[id + MIS_GPIO_CNT + ISO_GPIO_CNT + GPI_MIS_CNT + GPI_ISO_CNT+MIO_GPIO_CNT];
        break;
...
RTK_GPIO_ID rtk_gpio_get_gid_by_pintype(PCB_PIN_TYPE_T pin_type, unsigned char pin_index)
{
    switch (pin_type) {
        ...
        case PCB_PIN_TYPE_MIO_GPIO:
            return rtk_gpio_id(MIO_GPIO, pin_index);
        case PCB_PIN_TYPE_ISO_GPIO_MIO:
            return rtk_gpio_id(ISO_GPIO_MIO, pin_index);
        default:
...
int rtk_gpio_set_dir(RTK_GPIO_ID gid, unsigned char Out)
{
    ...
    case ISO_GPIO:
        ...
        return 0;
    case ISO_GPIO_MIO:
        iowrite_reg_bit(ISO_GPIO_MIO_GPDIR_reg + offset,bit, Out);
        return 0;

#ifdef CONFIG_RTK_KDRV_MIO
    case MIO_GPIO:
        return rtk_mio_gpio_set_dir(gid, Out);
#endif
...
int rtk_gpio_get_dir(RTK_GPIO_ID gid)
{
    ...
    case ISO_GPIO_MIO:
        return ioread_reg_bit(ISO_GPIO_MIO_GPDIR_reg + offset,bit);

#ifdef CONFIG_RTK_KDRV_MIO
    case MIO_GPIO:
        return rtk_mio_gpio_get_dir(gid);
#endif
    default:
        break;
...
int rtk_gpio_set_debounce(RTK_GPIO_ID gid, unsigned char val)
{
    ...
    case MIS_GPIO:
        if (id < MIS_GPIO_CNT && val <= 7)
        {
            ...
            //else
            //    gpio_iowrite32(MIS_GPDEB_1_reg, (0x8 | val) << (((id - 127) >> 4) * 4));
            return 0;
        }
        break;
    ...
    case ISO_GPIO_MIO:
        if (id < ISO_GPIO_MIO_CNT && val <= 7)
        {
            gpio_iowrite32(ISO_GPIO_MIO_GPDEB_reg, (0x8 | val) << ((id >> 4) * 4));
            return 0;
        }
        break;
#ifdef CONFIG_RTK_KDRV_MIO
    case MIO_GPIO:
		return rtk_mio_gpio_set_debounce(gid, val);
#endif
    default:
        break;
...
int rtk_gpio_input(RTK_GPIO_ID gid)
{
    ...
    case ISO_GPIO_MIO:
        return ioread_reg_bit(ISO_GPIO_MIO_GPDATI_reg + offset, bit);

#ifdef CONFIG_RTK_KDRV_MIO
    case MIO_GPIO:
        return rtk_mio_gpio_input(gid);
#endif
    default:
        break;
...
int rtk_gpio_output(RTK_GPIO_ID gid, unsigned char val)
{
    ...
    case ISO_GPIO_MIO:
        iowrite_reg_bit(ISO_GPIO_MIO_GPDATO_reg + offset,bit, val);
        return 0;

#ifdef CONFIG_RTK_KDRV_MIO
    case MIO_GPIO:
        return rtk_mio_gpio_output(gid, val);
#endif
    default:
        break;
...
#endif
    case ISO_GPIO_MIO:
        return ioread_reg_bit(ISO_GPIO_MIO_GPDATO_reg + offset,bit);
        break;

#ifdef CONFIG_RTK_KDRV_MIO
    case MIO_GPIO:
        return rtk_mio_gpio_output_get(gid);
#endif
        ...
        if((memcmp(pcb_enum_all[i].name, "PIN", 3) == 0) && (pcb_enum_all[i].value != 0) && (strstr(pcb_enum_all[i].name,"GPIO_INFO") == NULL))
        {
            ...
            switch (pin_type) {
                ...
                case PCB_PIN_TYPE_EMCU_GPIO:
                    ...
                    break;
                case PCB_PIN_TYPE_MIO_GPIO:
                    gpio_id = rtk_gpio_id(MIO_GPIO, pin_index);
                    gpio_type = MIO_GPIO;
                    break;
                case PCB_PIN_TYPE_ISO_GPIO_MIO:
                    gpio_id = rtk_gpio_id(ISO_GPIO_MIO, pin_index);
                    gpio_type = MIO_GPIO;
                    break;
                default:
                    gpio_pin = false;
...
#ifdef CONFIG_RTK_KDRV_MIO
    /* MIO gpio using default value*/
    else if (group == MIO_GPIO)
    {
        GPIO_ISR = MIO_ISR_reg;
        ISR_GPIO_ASSERT = (0x1 << MIO_ISR_assert_int_shift);
        GPIO_CNT = MIO_GPIO_CNT;
        UMSK_ISR_GPDA = MIO_ISR_GP0DA_reg;
        UMSK_ISR_GPA = MIO_ISR_GP0A_reg;
    }
#endif
    /* ISO gpio MIO using default value*/
    else if (group == ISO_GPIO_MIO)
    {
        GPIO_ISR = ISO_ISR_reg;
        ISR_GPIO_ASSERT = (0x1 << ISO_MISC_ISR_gpio_int_scpu_shift);
        GPIO_CNT = ISO_GPIO_MIO_CNT;
        UMSK_ISR_GPDA = ISO_GPIO_MIO_ISR_GP_DEASSERT_reg;
        UMSK_ISR_GPA = ISO_GPIO_MIO_ISR_GP_ASSERT_reg;
    }
...
static void __inline _rtk_gpio_disable_irq_all(void)
{
    ...
    for (i = 0; i < MIS_GPIO_REG_SETS; i++){
        if(i >= 1){
            ...
            gpio_iowrite32( (MIS_GPIO_GP0IE_DA_reg + 4) + (i << 2), 0xFFFFFFFE);
        }
        else{
            gpio_iowrite32( MIS_GP0IE_reg + (i << 2), 0xFFFFFFFE);
            gpio_iowrite32( MIS_GPIO_GP0IE_DA_reg + (i << 2), 0xFFFFFFFE);
        }

    }

    for (i = 0; i < ISO_GPIO_MIO_REG_SETS; i++)
    {
        gpio_iowrite32(ISO_GPIO_MIO_GPIE_reg + (i << 2), 0xFFFFFFFE);
        gpio_iowrite32(ISO_GPIO_MIO_GPIE_DA_reg + (i << 2), 0xFFFFFFFE);
    }
    ...
    for (i = 0; i < ISO_GPIO_REG_SETS; i++){
        gpio_iowrite32(ISO_GPIO_MIO_GPIE_reg, 0xFFFFFFFE);
    }
#ifdef RTK_SUPPORT_MIS_GPI_BACKUP_CONFIG
...
static void __inline _rtk_gpio_clear_isr_all(void)
{
    ...
    for (i = 0; i < ISO_GPIO_MIO_REG_SETS; i++)
    {
        gpio_iowrite32(ISO_GPIO_MIO_ISR_GP_ASSERT_reg + (i << 2), 0xFFFFFFFE);
        gpio_iowrite32(ISO_GPIO_MIO_ISR_GP_DEASSERT_reg + (i << 2), 0xFFFFFFFE);
    }
...
#endif
    case ISO_GPIO_MIO:
        iowrite_reg_bit(ISO_GPIO_MIO_GPDP_reg + offset, bit, positive);
        return 0;
#ifdef CONFIG_RTK_KDRV_MIO
   case MIO_GPIO:
        return rtk_mio_gpio_set_irq_polarity(gid, positive);
#endif
...
#endif
    case ISO_GPIO_MIO:
        iowrite_reg_bit(ISO_GPIO_MIO_GPINT_SCPU_reg + offset,bit, 1);
        iowrite_reg_bit(ISO_GPIO_MIO_GPIE_reg + offset,bit, On);
        return 0;
#ifdef CONFIG_RTK_KDRV_MIO
    case MIO_GPIO:
        return rtk_mio_gpio_set_irq_enable( gid, On);
#endif
    default:
        break;
...
#endif
    case ISO_GPIO_MIO:
        return ioread_reg_bit(ISO_GPIO_MIO_GPIE_reg + offset,bit);

#ifdef CONFIG_RTK_KDRV_MIO
    case MIO_GPIO:
        return rtk_mio_gpio_chk_irq_enable(gid);
#endif
...
int rtk_gpio_set_dis_irq_enable(RTK_GPIO_ID gid, unsigned char On)
{
    ...
    {
    case MIS_GPIO:
        iowrite_reg_bit(MIS_GPIO_GP0IE_DA_reg + offset,bit, On);
    ...
    case ISO_GPIO:
        if(0 != rtk_gpio_scpu_iso_irq_unmask(gid))
            return -EFAULT;
        iowrite_reg_bit(ISO_GPIO_GPIE_DA_reg + offset,bit, On);
        return 0;

#ifdef RTK_SUPPORT_MIS_GPI_BACKUP_CONFIG
    case MIS_GPI:
        iowrite_reg_bit(MIS_GPIO_GPI0_GPIE_DA_reg + offset,bit, On);
        return 0;
#endif
...
#ifdef CONFIG_RTK_KDRV_MIO
    case MIO_GPIO:
        return rtk_mio_gpio_set_dis_irq_enable(gid, On);
#endif
...
int rtk_gpio_chk_dis_irq_enable(RTK_GPIO_ID gid)
{
    ...
    case MIS_GPIO:
        if(offset >= 4){
            return ioread_reg_bit((MIS_GPIO_GP0IE_DA_reg + 4) + offset, bit);
        }
        else{
            return  ioread_reg_bit(MIS_GPIO_GP0IE_DA_reg + offset, bit);
        }

    case ISO_GPIO:
        return ioread_reg_bit(ISO_GPIO_GPIE_DA_reg + offset,bit);

#ifdef RTK_SUPPORT_MIS_GPI_BACKUP_CONFIG
    case MIS_GPI:
        return ioread_reg_bit(MIS_GPIO_GPI0_GPIE_DA_reg + offset,bit);
#endif
...
#endif
    case ISO_GPIO_MIO:
        return ioread_reg_bit(ISO_GPIO_MIO_GPIE_DA_reg + offset,bit);

#ifdef CONFIG_RTK_KDRV_MIO
    case MIO_GPIO:
        return rtk_mio_gpio_chk_dis_irq_enable(gid);
#endif
...
#endif
    case ISO_GPIO_MIO:
        iowrite_reg_bit(ISO_GPIO_MIO_ISR_GP_ASSERT_reg + offset, bit, 0);
        iowrite_reg_bit(ISO_GPIO_MIO_ISR_GP_DEASSERT_reg + offset, bit, 0);
        return 0;

#ifdef CONFIG_RTK_KDRV_MIO
    case MIO_GPIO:
        return rtk_mio_gpio_clear_isr(gid);
#endif
...
static int rtk_gpio_pm_suspend_noirq_std (struct device *dev)
{
    ...
    for (i = 0; i < MIS_GPIO_REG_SETS; i++)
    {
        if(i >= 1){
            ...
            backup_mis_dis_gpie[i] = gpio_ioread32((MIS_GPIO_GP0IE_DA_reg + 4) + (i << 2));
        }
        else{
            backup_mis_gpie[i] = gpio_ioread32(MIS_GP0IE_reg + (i << 2));
            backup_mis_dis_gpie[i] = gpio_ioread32(MIS_GPIO_GP0IE_DA_reg + (i << 2));
        }
...
static int rtk_gpio_pm_resume_noirq_std(struct device *dev)
{
    ...
    for (i = 0; i < MIS_GPIO_REG_SETS; i++)
    {
        if(i >= 1){
            ...
            gpio_iowrite32_protect( (MIS_GPIO_GP0IE_DA_reg + 4) + (i << 2), backup_mis_dis_gpie[i]);
        }
        else{
            gpio_iowrite32_protect( MIS_GP0IE_reg + (i << 2), backup_mis_gpie[i]);
            gpio_iowrite32_protect( MIS_GPIO_GP0IE_DA_reg + (i << 2), backup_mis_dis_gpie[i]);
        }
...
static int rtk_gpio_probe(struct platform_device *pdev)
{
    ...
    if(MIO_GPIO_CNT)
    {
        ...
        else if(request_irq(iso_irq, gpio_interrupt_handler, IRQF_SHARED, "MIO_GPIO",(void *)~MIO_GPIO)
 < 0)
        {
            RTK_GPIO_DBG("[%s] request iso hwirq %lu failed\n", __func__,
                irqd_to_hwirq(irq_get_irq_data(iso_irq)));
            ret = -EIO;
            goto err_request_mio_irq_failed;
        }
    ...
    if(ISO_GPIO_MIO_CNT)
    {
        ...
        else if(request_irq(iso_irq, gpio_interrupt_handler, IRQF_SHARED, "ISO_GPIO_MIO",(void *)~ISO_GPIO_MIO)
 < 0)
        {
            RTK_GPIO_DBG("[%s] request iso hwirq %lu failed\n", __func__,
                irqd_to_hwirq(irq_get_irq_data(iso_irq)));
...
err_request_mio_irq_failed:
    if (MIO_GPIO_CNT)
    {
        free_irq(iso_irq, (void *)~MIO_GPIO);
    }
...
static int rtk_gpio_remove(struct platform_device *pdev)
{
    ...
    if (iso_irq >= 0) {
        ...
        if (ISO_GPIO_MIO_CNT)
            free_irq(iso_irq, (void *)~ISO_GPIO_MIO);
    }
...
#ifdef CONFIG_RTK_KDRV_MIO
    gpio_iowrite32(MIO_GP_INT_SEL_reg, GPIO_BOTH_INTERRUPT); // set for both direction
#endif
