...
int rpc_poll_init(void)
{
	...
	for (i = 0; i < RPC_POLL_DEV_TOTAL; i++) {
		PDEBUG("rpc_poll_device %d addr: %x \n", i, (unsigned)&(rpc_poll_devices[i].dev));
		...
		rpc_poll_extra[i].nextRpc = (char *)get_rpc_addr(rpc_poll_devices[i].ringOut);
		...
		if (!is_init) {
			...
			case 6: rpc_poll_extra[i].name = "Video3PollWrite"; break;
			case 7: rpc_poll_extra[i].name = "Video3PollRead"; break;
#endif
			}
		}
	}
...
int rpc_poll_release(struct inode *inode, struct file *filp)
{
	...
	if (extra->currProc == proc) {
		...
		if (minor == 2 || minor == 6 || minor == 10
			...
			if (!rpc_done(extra)) {
				rtd_pr_rpc_err("%s: previous rpc hasn't finished, force clear!! ringOut %x => %p\n", __func__,
	...
	if (minor == RPC_POLL_DEV_SA_ID0)
	{
		...
		/* VCPU2 need to do destroy first because VCPU1 destroy will release memory. */
			  ...
			  loff_t *f_pos)
{
	...
	if (rpc_poll_is_paused) {
		rtd_pr_rpc_err("%s:%d thread:%s pid:%d tgid:%d device:%s\n", __func__, __LINE__, current->comm, current->pid, current->tgid, extra->name);
		rtd_pr_rpc_err("RPCpoll: someone access rpc poll during the pause...\n");
		msleep(1000);
		return -EAGAIN;
	}
	...
	if ((extra->currProc != proc) || (ring_empty((RPC_COMMON_Dev *) dev))) {
		if (unlikely(!(filp->f_flags & O_NONBLOCK))) {
			/*rtd_pr_rpc_info("%s:%d:%s Warning: pid:%d use blocking mode with poll buffer!\n", __func__, __LINE__, extra->name, current->pid); */
		}
...
{
	int i;
	RPC_POLL_Dev *rpc_dev = (RPC_POLL_Dev *)get_rpc_addr(RPC_POLL_RECORD_ADDR);
	
	for (i = 0; i < RPC_POLL_DEV_TOTAL; i++)
	{
		if(!(ring_empty((RPC_COMMON_Dev *)(&rpc_dev[i]))))
		{
			rtd_pr_rpc_notice("rpc_poll_device(%d) not empty:%d\n",
			...
			unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == RPC_IOCTRESET) {
		rtd_pr_rpc_err("[RPC]start reset...\n");
		rpc_poll_init();
		rpc_intr_init();
		rpc_kern_init();
		...
		*((int *)phys_to_virt(SYNC_FLAG_V3CPU)) = 0xffffffff;
		...
		rtd_pr_rpc_err("[RPC]done...\n");
	} else {
		rtd_pr_rpc_err("[RPC]: error ioctl command...\n");
	}
...
{
	rtd_pr_rpc_err("[RPC]open for RPC ioctl...\n");
...
static void show_block_user_rpc(int index)
{
	...
	list_for_each_entry(rrpc, &extra->blockrpc_list, list){
		rtd_pr_rpc_info("%s: program:%u version:%u procedure:%u taskID:%u sysPID:%x size:%u context:%x, count:%d\n",
...
{
	RPC_COMMON_Dev *dev = (RPC_COMMON_Dev *)pdev;
	u32 j;

	rtd_pr_rpc_info("\n ringStart: 0x%8x, ringEnd:0x%8x, ringIn: 0x%8x, ringOut: 0x%8x \n", (u32)dev->ringStart,
		(u32)dev->ringEnd, (u32)dev->ringIn, (u32)dev->ringOut);
	rtd_pr_rpc_info("ringbuffer info:\n");
	for(j=0;j<RPC_RING_SIZE ;)
	{
		unsigned char *pbuf = (unsigned char *)get_rpc_addr(dev->ringStart) + j;
		rtd_pr_rpc_info("%08x : %02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x", (u32)(get_rpc_addr(dev->ringStart) + j),
			pbuf[0],pbuf[1],pbuf[2],pbuf[3],pbuf[4],pbuf[5],pbuf[6],pbuf[7],
			pbuf[8],pbuf[9],pbuf[10],pbuf[11],pbuf[12],pbuf[13],pbuf[14],pbuf[15]);
			j += 16;
		// if (j%16 == 0) {
			// rtd_pr_rpc_info("\n%08x :", (u32)(get_rpc_addr(dev->ringStart) + j));
		// }
		// else if(j%4 == 0)
			// rtd_pr_rpc_info(" ");
		// rtd_pr_rpc_info("%02x",(unsigned char)*((unsigned char *)get_rpc_addr(dev->ringStart) + j));
	}
	rtd_pr_rpc_info("\n");
	rtd_pr_rpc_info("=============================================\n");
	...
	const char *buf, size_t count, loff_t *f_pos)
{
	...
	rtd_pr_rpc_debug("%s(): count=%d, buf=0x%08lx\n", __func__, count, (long)buf);
...
{
	rtd_pr_rpc_err("[RPC]open for RPC debug...\n");
