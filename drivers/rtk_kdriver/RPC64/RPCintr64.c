...
//support for mbed audio rpc ipi-interrupt
...
void _show_buffer(RPC_DEV_EXTRA *extra)
{
	...
	data = (u32 *)get_rpc_addr(dev->ringStart);
...
out:
		rtd_pr_rpc_err("RPC intr ringIn pointer is : 0x%8x, minor = %d\n", get_rpc_addr(dev->ringIn),minor);
...
static inline void rpc_unhandle_packet(RPC_PROCESS *proc, char *out, int minor)
{
	   ...
	   if (out) {
			   ...
			   rtd_pr_rpc_err("%s-rpc(%p),program:%lu version:%lu procedure:%lu taskID:%lu sysPID:%lx size:%lu context:%lx %s\n",
		   ...
		   if ((rpc.programID == AUDIO_AGENT) || (rpc.programID == VIDEO_AGENT)){
		   	if(rpc.taskID){	//for blocking mode RPC request
				...
				rtd_pr_rpc_err("%s send reply to rpc-%d \n", extra->name, minor);
		   	}
		   }
	}
...
void rpc_dispatch(unsigned long data)
{
	...
	{
		PDEBUGD("%s: program:%u version:%u procedure:%u taskID:%u sysPID:%x size:%u context:%x %s\n",
		extra->name, rpc.programID, rpc.versionID, rpc.procedureID, rpc.taskID, rpc.sysPID, rpc.parameterSize,
		(unsigned int)rpc.mycontext, in_atomic() ? "atomic" : "");
	}
#endif
...
#endif
	} else if (rpc.programID == REPLYID && rpc.versionID == REPLYID && rpc.parameterSize >= sizeof(unsigned int)) {

		if (get_ring_data(extra->name, dev, out, (char *)&taskID, sizeof(unsigned int)) == NULL)
			return;
	...
	if (proc) {
		...
		if(is_pr_type_match( extra->name))
			{
			PDEBUGD("%s:%d can't find process for handling %s programID:%u \n", extra->name, __LINE__, extra->name, rpc.programID);
	...
	if (found) {
		...
		if(is_pr_type_match( extra->name))
		{
			PDEBUGD("%s:%d Wakeup process:%d and update nextRpc:%px for programID:%u\n", extra->name, __LINE__, proc->pid, extra->nextRpc, rpc.programID);
		}
#endif
			...
			if((rpc.programID == REPLYID)&&( rpc.versionID == REPLYID)){
				if(!is_taskID_alive(taskID)){
					update_nextRpc(extra, out);
					dev->ringOut = (u32)rpc_pa(extra->nextRpc);
...
int rpc_intr_init(void)
{
	...
	for (i = 0; i < RPC_INTR_DEV_TOTAL; i++) {
		...
		PDEBUG("RPC ringStart: 0x%8x\n", (unsigned int)rpc_intr_devices[i].ringStart);
		PDEBUG("RPC ringEnd:   0x%8x\n", (unsigned int)rpc_intr_devices[i].ringEnd);
		PDEBUG("RPC ringIn:    0x%8x\n", (unsigned int)rpc_intr_devices[i].ringIn);
		PDEBUG("RPC ringOut:   0x%8x\n", (unsigned int)rpc_intr_devices[i].ringOut);
		PDEBUG("\n");

		rpc_intr_extra[i].nextRpc =(char *)get_rpc_addr(rpc_intr_devices[i].ringOut);
		...
		if (!is_init) {
			...
			case 6:rpc_intr_extra[i].name = "Video3IntrWrite";break;
			case 7:rpc_intr_extra[i].name = "Video3IntrRead"; break;
#endif
			}
		}
	}
...
int rpc_intr_open(struct inode *inode, struct file *filp)
{
	...
	if (!filp->private_data) {
		...
		if(proc == NULL){
			rtd_pr_rpc_err("%s: failed to allocate RPC_PROCESS\n", __func__);
			return -ENOMEM;
		}
	...
	if (extra->currProc == proc) {
		...
		if (minor == 3 || minor == 7 || minor == 11
			...
			if (!rpc_done(extra)) {
				...
				rpc_unhandle_packet(proc, (char *)get_rpc_addr(dev->ringOut), minor-2);
			  ...
			  loff_t *f_pos)
{
	...
	if (rpc_intr_is_paused) {
		rtd_pr_rpc_err("RPCintr: someone access rpc intr during the pause...\n");
		msleep(1000);
		return -EAGAIN;
	}
...
//			  }
	...
	if (temp >= count) {
		if (my_copy_to_user((int *)buf, (int *)get_rpc_addr(dev->ringOut), count)) {
			ret = -EFAULT;
			goto out;
		}
		ret += count;
		...
		if (my_copy_to_user((int *)buf, (int *)get_rpc_addr(dev->ringOut), temp)) {
			ret = -EFAULT;
			goto out;
		}
		count -= temp;

		if (my_copy_to_user((int *)(buf + temp), (int *)get_rpc_addr(dev->ringStart), count)) {
			ret = -EFAULT;
			goto out;
		}
		ret += (temp + count);
			   ...
			   loff_t *f_pos)
{
	...
	temp = dev->ringEnd - dev->ringIn;
	if (temp >= count) {
		if (my_copy_from_user((int *)get_rpc_addr(dev->ringIn), (int *)buf, count)) {
			ret = -EFAULT;
			goto out;
		}
		ret += count;
		...
		if (my_copy_from_user((int *)get_rpc_addr(dev->ringIn), (int *)buf, temp)) {
			ret = -EFAULT;
			goto out;
		}
		count -= temp;

		if (my_copy_from_user((int *)get_rpc_addr(dev->ringStart), (int *)(buf + temp), count)) {
			ret = -EFAULT;
			goto out;
		}
		ret += (temp + count);
...
long rpc_intr_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case RPC_IOCTHANDLER:
		{
			...
			if (handler == NULL) {
				rtd_pr_rpc_err("%s: failed to allocate RPC_HANDLER\n", __func__);
				return -ENOMEM;
			}
	...
	default:		/* redundant, as cmd was checked against MAXNR */
		rtd_pr_rpc_err("%s:%d unsupported ioctl cmd:%x arg:%lx\n", __func__, __LINE__, cmd, arg);
		return -ENOTTY;
	}
...
{
	int i;
	RPC_INTR_Dev  *rpc_dev = (RPC_INTR_Dev *) get_rpc_addr(RPC_INTR_RECORD_ADDR);
	for (i = 0; i < RPC_INTR_DEV_TOTAL; i++)
	{
		if(!(ring_empty((RPC_COMMON_Dev *)(&rpc_dev[i]))))
		{
			rtd_pr_rpc_notice("rpc_intr_device(%d) not empty:%d\n", i,
			get_ring_data_size((char *)&(rpc_dev[i].ringStart),(char *)&(rpc_dev[i].ringEnd),(char *)&(rpc_dev[i].ringIn), (char *)&(rpc_dev[i].ringOut)));
