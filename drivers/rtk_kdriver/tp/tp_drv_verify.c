...
TP_IOC_SET_TP_RING_BUFF_T_COMPAT_T param_tp_set_buf_compat_t;
...
TP_IOC_TP_READ_DATA_COMPAT_T param_read_data_compat_t;
TP_IOC_TP_RELEASE_DATA_T param_tp_rel_data;
TP_IOC_TP_RELEASE_DATA_COMPAT_T param_tp_rel_data_compat_t;
...
TP_IOC_SET_MTP_BUFF_T_COMPAT_T param_mtp_set_buff_compat_t;
TP_IOC_WRITE_MTP_BUFF_T param_mtp_wrt_buff;
TP_IOC_WRITE_MTP_BUFF_COMPAT_T param_mtp_wrt_buff_compat_t;
TP_IOC_GET_MTP_BUF_STATUS_T param_mtp_buff_status;
TP_IOC_GET_MTP_BUF_STATUS_COMPAT_T param_mtp_buff_status_compat_t;
...
TP_IOC_ADD_SECTION_FILTER_COMPAT_T param_sec_add_filter_compat_t;
TP_IOC_REMOVE_SECTION_FILTER_T param_sec_rmv_filter;
TP_IOC_REMOVE_SECTION_FILTER_COMPAT_T param_sec_rmv_filter_compat_t;
...
TP_IOC_GET_SECTION_DATA_COMPAT_T param_sec_get_data_compat_t;
...
TP_PCR_CNT_SET_INFO_T param_pcr_cnt_set_info;
TP_PCR_STC_INFO_T param_pcr_stc_info;
...
TP_IOC_STARTCODE_SET_BUFFER_T_COMPAT_T param_sc_set_buf_compat_t;
TP_IOC_STARTCODE_READ_INFO_T param_sc_read;
TP_IOC_STARTCODE_READ_INFO_COMPAT_T param_sc_read_compat_t;
TP_IOC_STARTCODE_RELEASE_INFO_T param_sc_relase;
TP_IOC_STARTCODE_RELEASE_INFO_COMPAT_T param_sc_relase_compat_t;
...
/*nagxx */
TPK_NAGXX_Conrol_Data_T  param_nagxx_control_data;
TPK_NAGXX_PID_Updata_Bit_T param_nagxx_pid_updata_bit_T;
#endif
/*------------------------------------------------------------------
 *    COMMON APIS
 *------------------------------------------------------------------*/

void* pli_to_kernel_ex(unsigned long addr, size_t size)
{
        ...
        virt_addr = dvr_remap_uncached_memory(__pfn_to_phys(pfn), size, __builtin_return_address(0)) + (((unsigned long)addr) % PAGE_SIZE) ;
...
INT32 set_optee_tp_register(TPK_TP_ENGINE_T tp_id, UINT8 isEnable)
{
        optee_tp_init();
        TP_WARNING("set protection isEnable=%d\n",isEnable);
        ...
        *pbIsNew = FALSE;
        ...
        for (i = 0; i < SEC_FILTER_BUFFER_NUM; i++) {
                if (pSecbufdata->SecMap[i].bOccupied && pSecbufdata->SecMap[i].bCompleted) {
                        ...
                        *pbIsNew = TRUE;
                        *pHandleID = (UADDRESS)pSecbufdata->SecMap[i].pContext;
...
#ifndef CONFIG_ARM64
        TP_WARNING("TP%d start code buffer :Base=%08x,RP=%08x,WP=%08x,Limit=%08x,*pContinueReadSize=%d!!!!!!!!\n",tp_id,buf_param.Base,buf_param.RP,buf_param.WP,buf_param.Limit,*pContinueReadSize);
#else
        TP_WARNING("TP%d start code buffer :Base=%08lx,RP=%08lx,WP=%08lx,Limit=%08lx,*pContinueReadSize=%d!!!!!!!!\n",tp_id,buf_param.Base,buf_param.RP,buf_param.WP,buf_param.Limit,*pContinueReadSize);
#endif

        /* check if buffer full, flush buffer */
        result = Tp_Get_Buffer_Full_status(buf_idx, (UINT8*)&full_stataus);
        if (result != TPK_SUCCESS) {
...
INT32 TPVerify_DumpTPRegister(OUT TPK_DUMP_REG_INFO_T *dump_reg_info)
{
        ...
        for (buf_idx = 0; buf_idx < MAX_TPP_BUFFER_COUNT; buf_idx++) {
                ...
                dump_reg_info->tp_p_ring_rp_reg[buf_idx]    = READ_REG32(TP_TP_P_RING_RP_reg);
                dump_reg_info->tp_p_ring_wp_reg[buf_idx]    = READ_REG32(TP_TP_P_RING_WP_reg);
...
#ifdef TP_SUPPORT_CI
        /* TPO CLK */
        dump_reg_info->tpout_clk_reg  = READ_REG32(TP_TP_OUT_CLOCK_reg);
#endif
...
long TPVerify_ioctl(struct file* file, unsigned int cmd, unsigned long arg)
{
        ...
        switch (cmd) {
                ...
                case TP_IOC_SET_TP_RING_BUFF:
                        /* copy parameter from user space */
#ifdef CONFIG_COMPAT
                        if (copy_from_user(&param_tp_set_buf_compat_t, (void __user *)arg, sizeof(TP_IOC_SET_TP_RING_BUFF_T_COMPAT_T))) {
                                TP_WARNING("TP_IOC_SET_TP_RING_BUFF, copy parameter from user space failed\n");
                                return -EFAULT;
                        }
                        param_tp_set_buf.tp_id = param_tp_set_buf_compat_t.tp_id;
                        param_tp_set_buf.pPhyAddr= compat_ptr(param_tp_set_buf_compat_t.pPhyAddr);
                        param_tp_set_buf.pVirAddr= compat_ptr(param_tp_set_buf_compat_t.pVirAddr);
                        param_tp_set_buf.size = param_tp_set_buf_compat_t.size;
#else
...
#endif
                        ret = RHAL_SetTPRingBuffer(param_tp_set_buf.tp_id, param_tp_set_buf.pPhyAddr,
                                        (UINT8*)pli_to_kernel_ex((unsigned long)param_tp_set_buf.pVirAddr, param_tp_set_buf.size),
                                        param_tp_set_buf.size);
                        break;
                ...
                case TP_IOC_TP_READ_DATA:
                        /* copy parameter from user space */
#ifdef CONFIG_COMPAT
                        if (copy_from_user(&param_read_data_compat_t, (void __user *)arg, sizeof(TP_IOC_TP_READ_DATA_COMPAT_T))) {
                                TP_WARNING("TP_IOC_TP_READ_DATA, copy parameter from user space failed\n");
                                return -EFAULT;
                        }

                        param_read_data.tp_id = param_read_data_compat_t.tp_id;
                        param_read_data.pReadPhyPtr = compat_ptr(param_read_data_compat_t.pReadPhyPtr);
                        param_read_data.ContinueReadSize = param_read_data_compat_t.ContinueReadSize;
                        param_read_data.Threshold = param_read_data_compat_t.Threshold;
                        ...
                        param_read_data_compat_t.pReadPhyPtr = ptr_to_compat(param_read_data.pReadPhyPtr);
                        param_read_data_compat_t.ContinueReadSize = param_read_data.ContinueReadSize;
                        TP_TRACE("TP_IOC_TP_READ_DATA tp_id=%d pReadPhyPtr=%p ContinueReadSize=%d Threshold=%d\n",
                                param_read_data.tp_id, param_read_data.pReadPhyPtr, param_read_data.ContinueReadSize,  param_read_data.Threshold);

                        if (copy_to_user((void __user *)arg, &param_read_data_compat_t, sizeof(TP_IOC_TP_READ_DATA_COMPAT_T))) {
                ...
                case TP_IOC_TP_RELEASE_DATA:
                        /* copy parameter from user space */
#ifdef CONFIG_COMPAT
                        if (copy_from_user(&param_tp_rel_data_compat_t, (void __user *)arg, sizeof(TP_IOC_TP_RELEASE_DATA_COMPAT_T))) {
                                TP_WARNING("TP_IOC_TP_RELEASE_DATA, copy parameter from user space failed\n");
                                return -EFAULT;
                        }
                        param_tp_rel_data.tp_id = param_tp_rel_data_compat_t.tp_id;
                        param_tp_rel_data.pReadPhyPtr = compat_ptr(param_tp_rel_data_compat_t.pReadPhyPtr);
                        param_tp_rel_data.Size = param_tp_rel_data_compat_t.Size;
                        TP_TRACE("TP_IOC_TP_RELEASE_DATA tp_id=%d pReadPhyPtr=%p Size=%d\n",
                                param_tp_rel_data.tp_id, param_tp_rel_data.pReadPhyPtr, param_tp_rel_data.Size);
                ...
                case TP_IOC_SET_TPOUT_MODE:
                        ...
                        if(param_set_tpout_mode.Param.out_2bit == 0){
                                TP_WARNING("%s mark2/mio use serial mode\n", __func__);
                                RHAL_MIO_SetIOMode(1);
                        }else{
                                TP_WARNING("%s mark2/mio use two-bit mode\n", __func__);
                                RHAL_MIO_SetIOMode(0);
                        }
#endif
                        ret = RHAL_TPOUT_SetMode(param_set_tpout_mode.tpout_id, param_set_tpout_mode.Param);
                        break;
                case TP_IOC_GET_TPOUT_MODE:
                        ...
                        ret = RHAL_TPOUT_GetMode(param_get_tpout_mode.tpout_id, &param_get_tpout_mode.Param);
                ...
                case TP_IOC_SET_TPOUT_DATA_SOURCE:
                        ...
                        if(param_tpout_src.tpout_src == TPK_TPP_TO_TPOUT){
                                RHAL_TPP_Init(param_tpout_src.remux_param.tpp_id);
                                RHAL_TPP_SetDataSource(param_tpout_src.remux_param.tpp_id,param_tpout_src.remux_param.tpp_src);
                                RHAL_TPP_SetSyncReplace(param_tpout_src.remux_param.tpp_id,param_tpout_src.remux_param.output_sync_byte);
                                RHAL_TPP_StreamControl(param_tpout_src.remux_param.tpp_id,TPP_STREAM_START);
                                RHAL_TPP_OutEnable(param_tpout_src.remux_param.tpp_id, 1);
                        }
						
                        ret = RHAL_TPOUT_SetDataSource(param_tpout_src.tpout_id, param_tpout_src.tpout_src);
                        break;
                ...
                case TP_IOC_TPOUT_STREAM_CONTROL:
                        ...
                        if (pTp_drv->tpout[param_tpout_strm_ctrl.tpout_id].tpout_src == TPK_TP_MTP_TO_TPOUT)
                                RHAL_MTP_TPOutEnable(param_mtp_strm_ctrl.mtp_id,  (UINT8)param_mtp_strm_ctrl.ctrl);
                        ret = RHAL_TPOUT_Enable(param_tpout_strm_ctrl.tpout_id, param_tpout_strm_ctrl.ctrl);
                        break;
                ...
                case TP_IOC_GET_TPOUT_STATUS:
                        ...
                        ret = RHAL_TPOUT_GetStatus(param_tpout_status.tpout_id, &param_tpout_status.tpout_status);
                ...
                case TP_IOC_SET_TPOUT_CLK:
                        ...
                        ret = RHAL_TPOUT_SetClockSpeed(param_tpout_clk.tpout_id, param_tpout_clk.pre_div, param_tpout_clk.post_div,0);
                        break;
#endif
                ...
                case TP_IOC_SET_MTP_BUFF:
                        /* copy parameter from user space */
#ifdef CONFIG_COMPAT
                        if (copy_from_user(&param_mtp_set_buff_compat_t, (void __user *)arg, sizeof(TP_IOC_SET_MTP_BUFF_T_COMPAT_T))) {
                                TP_WARNING("TP_IOC_SET_MTP_BUFF, copy parameter from user space failed\n");
                                return -EFAULT;
                        }
                        param_mtp_set_buff.mtp_id = param_mtp_set_buff_compat_t.mtp_id;
                        param_mtp_set_buff.pPhyAddr = compat_ptr(param_mtp_set_buff_compat_t.pPhyAddr);
                        param_mtp_set_buff.pVirAddr = compat_ptr(param_mtp_set_buff_compat_t.pVirAddr);
                        param_mtp_set_buff.size =  param_mtp_set_buff_compat_t.size;
#else
                        ...
                        ret = RHAL_SetMTPBuffer(param_mtp_set_buff.mtp_id, param_mtp_set_buff.pPhyAddr,
                                                (UINT8*)pli_to_kernel_ex((unsigned long)param_mtp_set_buff.pVirAddr, param_mtp_set_buff.size), param_mtp_set_buff.size);
                        break;

                case TP_IOC_WRITE_MTP_BUFF:
                        /* copy parameter from user space */
#ifdef CONFIG_COMPAT
                        if (copy_from_user(&param_mtp_wrt_buff_compat_t, (void __user *)arg, sizeof(TP_IOC_WRITE_MTP_BUFF_COMPAT_T))) {
                                TP_WARNING("TP_IOC_WRITE_MTP_BUFF, copy parameter from user space failed\n");
                                return -EFAULT;
                        }
                        param_mtp_wrt_buff.mtp_id = param_mtp_wrt_buff_compat_t.mtp_id;
                        param_mtp_wrt_buff.pPhyWP = compat_ptr(param_mtp_wrt_buff_compat_t.pPhyWP);
                        param_mtp_wrt_buff.ContinueWrite = param_mtp_wrt_buff_compat_t.ContinueWrite;
                        TP_TRACE("TP_IOC_WRITE_MTP_BUFF mtp_id=%d pPhyWP=0x%p ContinueWrite=%x\n", param_mtp_wrt_buff.mtp_id, param_mtp_wrt_buff.pPhyWP, param_mtp_wrt_buff.ContinueWrite);
#else
                ...
                case TP_IOC_GET_MTP_BUF_STATUS:
                        /* copy parameter from user space */
#ifdef CONFIG_COMPAT
                        if (copy_from_user(&param_mtp_buff_status_compat_t, (void __user *)arg, sizeof(TP_IOC_GET_MTP_BUF_STATUS_COMPAT_T))) {
                                TP_WARNING("TP_IOC_GET_MTP_BUF_STATUS, copy parameter from user space failed\n");
                                return -EFAULT;
                        }
                        param_mtp_buff_status.mtp_id = param_mtp_buff_status_compat_t.mtp_id;
                        param_mtp_buff_status.pWritePtr = compat_ptr(param_mtp_buff_status_compat_t.pWritePtr);
                        param_mtp_buff_status.ContinueWriteSize = param_mtp_buff_status_compat_t.ContinueWriteSize;
                        param_mtp_buff_status.WritableSize = param_mtp_buff_status_compat_t.WritableSize;
                        TP_TRACE("TP_IOC_GET_MTP_BUF_STATUS mtp_id=%d pWritePtr=0x%p ContinueWriteSize=%x WritableSize=%x\n", param_mtp_buff_status.mtp_id, param_mtp_buff_status.pWritePtr, param_mtp_buff_status.ContinueWriteSize, param_mtp_buff_status.WritableSize);
                        ...
                        param_mtp_buff_status_compat_t.mtp_id = param_mtp_buff_status.mtp_id;
                        param_mtp_buff_status_compat_t.pWritePtr = ptr_to_compat(param_mtp_buff_status.pWritePtr);
                        param_mtp_buff_status_compat_t.ContinueWriteSize = param_mtp_buff_status.ContinueWriteSize;
                        param_mtp_buff_status_compat_t.WritableSize = param_mtp_buff_status.WritableSize;
                        TP_TRACE("TP_IOC_GET_MTP_BUF_STATUS mtp_id=%d pWritePtr=0x%p ContinueWriteSize=%x WritableSize=%x\n", param_mtp_buff_status.mtp_id, param_mtp_buff_status.pWritePtr, param_mtp_buff_status.ContinueWriteSize, param_mtp_buff_status.WritableSize);

                        if (copy_to_user((void __user *)arg, &param_mtp_buff_status_compat_t, sizeof(TP_IOC_GET_MTP_BUF_STATUS_COMPAT_T))) {
                ...
                case TP_IOC_MTP_STREAM_CONTROL:
                        ...
                        if (param_mtp_strm_ctrl.mtp_id >= TP_TPP0_MTP)
                                RHAL_TPP_OutEnable(param_mtp_strm_ctrl.mtp_id-TP_TPP0_MTP, (UINT8)param_mtp_strm_ctrl.ctrl);
                        break;
                ...
                case TP_IOC_ADD_SECTION_FILTER:
                        /* copy parameter from user space */
#ifdef CONFIG_COMPAT
                        if (copy_from_user(&param_sec_add_filter_compat_t, (void __user *)arg, sizeof(TP_IOC_ADD_SECTION_FILTER_COMPAT_T))) {
                                TP_WARNING("TP_IOC_ADD_SECTION_FILTER, copy parameter from user space failed\n");
                                return -EFAULT;
                        }
                        param_sec_add_filter.tp_id = param_sec_add_filter_compat_t.tp_id;
                        param_sec_add_filter.pSec_handle = compat_ptr(param_sec_add_filter_compat_t.pSec_handle);

                        param_sec_add_filter.Param.PID = param_sec_add_filter_compat_t.Param.PID;
                        param_sec_add_filter.Param.crc_en = param_sec_add_filter_compat_t.Param.crc_en;
                        param_sec_add_filter.Param.one_shoot = param_sec_add_filter_compat_t.Param.one_shoot;
                        param_sec_add_filter.Param.toggle_mode_en = param_sec_add_filter_compat_t.Param.toggle_mode_en;
                        memcpy(param_sec_add_filter.Param.PosVal, param_sec_add_filter_compat_t.Param.PosVal, sizeof(param_sec_add_filter.Param.PosVal));
                        memcpy(param_sec_add_filter.Param.PosMsk, param_sec_add_filter_compat_t.Param.PosMsk, sizeof(param_sec_add_filter.Param.PosMsk));
                        memcpy(param_sec_add_filter.Param.NegVal, param_sec_add_filter_compat_t.Param.NegVal, sizeof(param_sec_add_filter.Param.NegVal));
                        memcpy(param_sec_add_filter.Param.NegMsk, param_sec_add_filter_compat_t.Param.NegMsk, sizeof(param_sec_add_filter.Param.NegMsk));

                        param_sec_add_filter.Param.pContext = compat_ptr(param_sec_add_filter_compat_t.Param.context_ptr);
                        param_sec_add_filter.Param.Complete = compat_ptr(param_sec_add_filter_compat_t.Param.complete_ptr);
                        param_sec_add_filter.Param.Request_buf = compat_ptr(param_sec_add_filter_compat_t.Param.Request_buf_ptr);
						TP_TRACE("TP_IOC_ADD_SECTION_FILTER tp:%d,PID: 0x%x, crc %d, one_shoot %d, togMode %d\n", param_sec_add_filter.tp_id, param_sec_add_filter.Param.PID, param_sec_add_filter.Param.crc_en, param_sec_add_filter.Param.one_shoot, param_sec_add_filter.Param.toggle_mode_en);
                        ...
                        param_sec_add_filter_compat_t.pSec_handle = ptr_to_compat(param_sec_add_filter.pSec_handle);
                        if (copy_to_user((void __user *)arg, &param_sec_add_filter_compat_t, sizeof(TP_IOC_ADD_SECTION_FILTER_COMPAT_T))) {
                ...
                case TP_IOC_REMOVE_SECTION_FILTER:
                        /* copy parameter from user space */
#ifdef CONFIG_COMPAT
                        if (copy_from_user(&param_sec_rmv_filter_compat_t, (void __user *)arg, sizeof(TP_IOC_REMOVE_SECTION_FILTER_COMPAT_T))) {
...
#ifdef CONFIG_COMPAT
                        param_sec_get_data_compat_t.bIsNew = param_sec_get_data.bIsNew;
                        param_sec_get_data_compat_t.handleId = param_sec_get_data.handleId;
                        memcpy(param_sec_get_data_compat_t.aBuf, param_sec_get_data.aBuf, sizeof(param_sec_get_data_compat_t.aBuf));
                        param_sec_get_data_compat_t.Sec.pid = param_sec_get_data.Sec.pid;
                        param_sec_get_data_compat_t.Sec.p_data = ptr_to_compat(param_sec_get_data.Sec.p_data);
                        param_sec_get_data_compat_t.Sec.buf_idx = param_sec_get_data.Sec.buf_idx;
                        param_sec_get_data_compat_t.Sec.len = param_sec_get_data.Sec.len;
                        param_sec_get_data_compat_t.Sec.crc_err = param_sec_get_data.Sec.crc_err;

                        if (copy_to_user((void __user *)arg, &param_sec_get_data_compat_t, sizeof(TP_IOC_GET_SECTION_DATA_COMPAT_T))) {
                ...
                case TP_IOC_PCR_CNT_SET_INFO:
                        /* copy parameter from user space */
                        if (copy_from_user(&param_pcr_cnt_set_info, (void __user *)arg, sizeof(TP_PCR_CNT_SET_INFO_T))) {
                                TP_WARNING("TP_IOC_PCR_CNT_SET_INFO, copy parameter from user space failed\n");
                                return -EFAULT;
                        }
                        ret = RHAL_setGetTPPcr90KCnt(param_pcr_cnt_set_info.pcr_type,
                                        param_pcr_cnt_set_info.rOrw_flag,
                                        &param_pcr_cnt_set_info.cnt);

                        if (copy_to_user((void __user *)arg, &param_pcr_cnt_set_info, sizeof(TP_PCR_CNT_SET_INFO_T))){
                                TP_WARNING("TP_IOC_PCR_CNT_SET_INFO, copy parameter to user space failed\n");
                                return -EFAULT;
                        }
                        break;

                case TP_IOC_PCR_STC_INFO:
                        /* copy parameter from user space */
                        if (copy_from_user(&param_pcr_stc_info, (void __user *)arg, sizeof(TP_PCR_STC_INFO_T))) {
                                TP_WARNING("TP_IOC_RESET_PCR_TRACKING, copy parameter from user space failed\n");
                                return -EFAULT;
                        }
                        ret = RHAL_adjustStcClock(param_pcr_stc_info.stc_type,
                                        param_pcr_stc_info.fcode,
                                        param_pcr_stc_info.ncode);
                        break;
                ...
                case TP_IOC_STARTCODE_SET_BUFFER:
                        /* copy parameter from user space */
#ifdef CONFIG_COMPAT
                        if (copy_from_user(&param_sc_set_buf_compat_t, (void __user *)arg, sizeof(TP_IOC_STARTCODE_SET_BUFFER_T_COMPAT_T))) {
                                TP_WARNING("TP_IOC_SET_ENCRYPTION, copy parameter from user space failed\n");
                                return -EFAULT;
                        }
                        param_sc_set_buf.tp_id = param_sc_set_buf_compat_t.tp_id;
                        param_sc_set_buf.pPhyAddr = compat_ptr(param_sc_set_buf_compat_t.pPhyAddr);
                        param_sc_set_buf.pVirAddr = compat_ptr(param_sc_set_buf_compat_t.pVirAddr);
                        param_sc_set_buf.size = param_sc_set_buf_compat_t.size;
#else
                        ...
                        pVirAddr = (UINT8*)pli_to_kernel_ex((unsigned long)param_sc_set_buf.pVirAddr, param_sc_set_buf.size);
                ...
                case TP_IOC_STARTCODE_READ_INFO:
                        /* copy parameter from user space */
#ifdef CONFIG_COMPAT
                        if (copy_from_user(&param_sc_read_compat_t, (void __user *)arg, sizeof(TP_IOC_STARTCODE_READ_INFO_COMPAT_T))) {
                                TP_WARNING("TP_IOC_STARTCODE_READ_INFO, copy parameter from user space failed\n");
                                return -EFAULT;
                        }
                        param_sc_read.tp_id = param_sc_read_compat_t.tp_id;
                        param_sc_read.pReadPhyPtr = compat_ptr(param_sc_read_compat_t.pReadPhyPtr);
                        param_sc_read.ContinueReadSize = param_sc_read_compat_t.ContinueReadSize;
                        if(param_sc_read.tp_id < MAX_TP_COUNT) {
                                ret = TPVerify_ReadStartCodeInfo(param_sc_read.tp_id, &param_sc_read.pReadPhyPtr, &param_sc_read.ContinueReadSize);
                                param_sc_read_compat_t.pReadPhyPtr = ptr_to_compat(param_sc_read.pReadPhyPtr);
                                param_sc_read_compat_t.ContinueReadSize = param_sc_read.ContinueReadSize;
                                if (copy_to_user((void __user *)arg, &param_sc_read_compat_t, sizeof(TP_IOC_STARTCODE_READ_INFO_COMPAT_T))) {
                ...
                case TP_IOC_STARTCODE_RELEASE_INFO:
                        /* copy parameter from user space */
#ifdef CONFIG_COMPAT
                        if (copy_from_user(&param_sc_relase_compat_t, (void __user *)arg, sizeof(TP_IOC_STARTCODE_RELEASE_INFO_COMPAT_T))) {
                                TP_WARNING("TP_IOC_STARTCODE_RELEASE_INFO, copy parameter from user space failed\n");
                                return -EFAULT;
                        }
                        param_sc_relase.tp_id = param_sc_relase_compat_t.tp_id;
                        param_sc_relase.pReadPhyPtr = compat_ptr(param_sc_relase_compat_t.pReadPhyPtr);
                        param_sc_relase.Size = param_sc_relase_compat_t.Size;
#else
...
#endif
                        if(param_sc_relase.tp_id < MAX_TP_COUNT)
                ...
                case TP_IOC_TPP_SET_MODE:
                        ...
                        ret = RHAL_TPP_SetMode(param_set_mode.tp_p_id, param_set_mode.param);
                ...
                case TP_IOC_TPP_GET_MODE:
                        ...
                        ret = RHAL_TPP_GetMode(param_get_mode.tp_p_id, &param_get_mode.param);
                ...
                case TP_IOC_TPP_SET_PIDEN:
                        ...
                        ret = RHAL_TPP_EnablePIDFilter(param_tpp_set_piden.tp_p_id, param_tpp_set_piden.pid_en);
                ...
                case TP_IOC_TPP_STREAM_CONTROL:
                        ...
                        RHAL_TPP_StreamControl(param_tpp_strm_ctrl.tp_p_id, param_tpp_strm_ctrl.ctrl);
                ...
                case TP_IOC_GET_TPP_BUF_STATUS:
                        ...
                        ret = RHAL_TPP_GetBufferStatus(param_tpp_buff_status.tp_p_id, &param_tpp_buff_status.BufferSize, &param_tpp_buff_status.DataSize);
                ...
                case TP_IOC_TPP_RINGBUFF_FLUSH:
                        ...
                        ret = RHAL_TPP_FlushBuffer(param_tpp_flush.tp_p_id);
                ...
                case TP_IOC_DUMP_REG_INFO:
                        ...
                        break;
#ifdef CONFIG_RTK_KDRV_SUPPORT_NAGNA
                case TP_IOC_NAGXX_GET_PARAM:
                        /* copy parameter from user space */
                        if (copy_from_user(&param_nagxx_control_data, (void __user *)arg, sizeof(TPK_NAGXX_Conrol_Data_T))) {
                                TP_WARNING("TP_IOC_NAGXX_GET_PARAM, copy parameter from user space failed\n");
                                return -EFAULT;
                        }
                        ret = RHAL_Tp_GetNagxx_param(&param_nagxx_control_data.nagxx_contorl_data);

                        if (copy_to_user((void __user *)arg, &param_nagxx_control_data, sizeof(TPK_NAGXX_Conrol_Data_T))) {
                                TP_WARNING("TP_IOC_NAGXX_GET_PARAM, copy parameter to user space failed\n");
                                return -EFAULT;
                        }
                        break;
                case TP_IOC_NAGXX_PID_UPDATE_BIT:
                        /* copy parameter from user space */
                        if (copy_from_user(&param_nagxx_pid_updata_bit_T, (void __user *)arg, sizeof(TPK_NAGXX_PID_Updata_Bit_T))) {
                                TP_WARNING("TP_IOC_NAGXX_PID_UPDATE_BIT, copy parameter from user space failed\n");
                                return -EFAULT;
                        }
                        ret = RHAL_SetNagxx_PID_update_bit(param_nagxx_pid_updata_bit_T.tp_id, param_nagxx_pid_updata_bit_T.enable);
                        break;
                case TP_IOC_NAGXX_EnableProtect:
                        /* copy parameter from user space */
                        if (copy_from_user(&param_nagxx_pid_updata_bit_T, (void __user *)arg, sizeof(TPK_NAGXX_PID_Updata_Bit_T))) {
                                TP_WARNING("TP_IOC_NAGXX_EnableProtect, copy parameter from user space failed\n");
                                return -EFAULT;
                        }
                        if(param_nagxx_pid_updata_bit_T.enable){
                                ret = set_optee_tp_register(param_nagxx_pid_updata_bit_T.tp_id, 1);
                        }
                        else{
                                ret = set_optee_tp_register(param_nagxx_pid_updata_bit_T.tp_id, 0);
                        }
...
int TPVerify_RunDescTestEx2(IN TPK_TP_ENGINE_T tp_id, DESCRAMBLER_TEST_ITEM* pTest, char* pattern_path)
{
        ...
        if (NULL != pbuff) {
                ...
                RHAL_SetTPRingBuffer((TPK_TP_ENGINE_T)tp_id, (UINT8*)((UADDRESS)pbuff_phy), pbuff, TS_RING_BUFFER_SIZE);
                RHAL_SetMTPBuffer((TPK_TP_MTP_T)tp_id, (UINT8*)((UADDRESS)pbuff_phy + TS_RING_BUFFER_SIZE), (UINT8*)(pbuff + TS_RING_BUFFER_SIZE), TS_RING_BUFFER_SIZE);
                RHAL_MTPFlushBuffer((TPK_TP_MTP_T)tp_id, MTP_BUFF_FLUSH_TO_BASE);
                ...
                for(pid_index = 0; pid_index < pTest->nPID; pid_index++) {
                        ...
                        PIDParm.valid            = 1;
                        RHAL_AddPIDFilter(tp_id, PIDParm, (void*)0xabc);
                }

                RHAL_TPStreamControl(tp_id, TP_STREAM_START);
                RHAL_MTPStreamControl((TPK_TP_MTP_T)tp_id, MTP_STREAM_START);
                RHAL_GetMTPBufferStatus((TPK_TP_MTP_T)tp_id, &mtp_buffer_physical, &mtp_pContinueWriteSize, &mtp_pWritableSize);
                ...
                kernel_read(BinReffile, RefIn_tmp, mtp_pContinueWriteSize, &BinReffile->f_pos);
                kernel_read(BinInfile, mtp_buffer_virtual, mtp_pContinueWriteSize, &BinInfile->f_pos);

                dmac_flush_range(mtp_buffer_virtual, mtp_buffer_virtual + mtp_pContinueWriteSize);
                outer_flush_range((UADDRESS)mtp_buffer_physical, (UADDRESS)(mtp_buffer_physical + mtp_pContinueWriteSize));

                RHAL_WriteMTPBuffer((TPK_TP_MTP_T)tp_id, mtp_buffer_physical, mtp_pContinueWriteSize);
                ...
                outer_flush_range((UADDRESS)tp_buffer, (UADDRESS)(tp_buffer + tp_pContinueReadSize));

#ifdef BINOUT_SAVETO_FILE
                kernel_write(BinOutfile, tp_buffer_virtual, tp_pContinueReadSize, &BinOutfile->f_pos);
#endif
                ...
                RHAL_MTPStreamControl((TPK_TP_MTP_T)tp_id, MTP_STREAM_STOP);
...
)
{
        ...
        TP_DBG_SIMPLE("\n\n");
        TP_DBG_SIMPLE("===============================================================================\n");
        TP_DBG_SIMPLE("= Run TP->CP Descramble Test [%d/%ld] \n", test_id, (unsigned long)MAX_DESC_TEST_NUMBER * MAX_DTV_FRONTEND_NUM - 1);
        TP_DBG_SIMPLE("===============================================================================\n");
