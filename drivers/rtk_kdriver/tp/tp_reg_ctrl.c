...
extern struct file *rec_tlv_fd2;
...
EXPORT_SYMBOL(WRITE_REG32);

void Tp_get_curtime(struct __kernel_old_timeval *time)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
        struct timespec64 t64;
        ktime_get_real_ts64(&t64);
        time->tv_sec = t64.tv_sec;
        time->tv_usec = t64.tv_nsec / 1000;
#else
...
#endif
#ifdef CONFIG_RTK_TP_WITH_MIO
        WRITE_REG32(TP_TP_DMY_A_reg, _BIT1); //[MK2CETOP-174]mark2/mio pll bus/bush not same will cause mosaic when CI path.
#endif
        /* update status */
...
#ifdef TP_USE_IRQ
        if(pTp_drv->tp_irq_enable == 0 && (pTp_drv->irq_number >= 0) ) {
                if(request_irq( pTp_drv->irq_number, tp_isr, IRQF_SHARED | IRQF_TRIGGER_RISING, "REALTEK_TP", (void *)pTp_drv) < 0) {
...
INT32 Tp_uninit(TPK_TP_ENGINE_T tp_id)
{
        ...
        if (uninit_all == 1) {
                if(pTp_drv->tp_irq_enable == 1) {
                        free_irq( pTp_drv->irq_number, (void *)pTp_drv);
...
#ifdef CONFIG_RTK_TP_WITH_MIO
                case TP_TPI_Mio:
                        tp_in_pol_reg.tpi_mio_clk_pol = pol.clk_pol;
                        tp_in_pol_reg.tpi_mio_sync_pol = pol.sync_pol;
                        tp_in_pol_reg.tpi_mio_val_pol = pol.val_pol;
                        break;
#endif
                default:
...
INT32 Tp_TPO_Init(UINT8 tpout_id)
{
        ...
        /* set TPO clk = 9Mhz */
        Tp_TPO_SetClkDiv(TPO_CLK_INIT_PRE_DIV, TPO_CLK_INIT_POST_DIV, 0);
...
#ifdef TP_VDEC_SPEED_MHZ
        *tpo_clk_rate = TP_VDEC_SPEED_MHZ;
#else
...
* Tp_TPO_SetClkDiv *
*  TS bypass mode only.
*  Clock will toggle when meet this register
*  i.e. if((clk_cnt == bypass_out_clk_l) | (clk_cnt == bypass_out_clk_h)) bypass_clk = ~ bypass_clk;
------------------------------------------------------------------*/
INT32 Tp_TPO_SetBypassClk( UINT32 bypass_out_clk_h, UINT32 bypass_out_clk_l)
{
    tp_tp_tf_out_frmcfg_RBUS tp_out_frmcfg_reg;

    tp_out_frmcfg_reg.regValue = READ_REG32(TP_TP_TF_OUT_FRMCFG_reg);
#ifdef REG_TPO_BYPASS_CLK_EXIST
    /*mac7p register doesn't have bypass_out_clk_h and bypass_out_clk_l*/
    tp_out_frmcfg_reg.bypass_out_clk_h = bypass_out_clk_h & 0xF;
    tp_out_frmcfg_reg.bypass_out_clk_l = bypass_out_clk_l & 0xF;
#endif
    WRITE_REG32(TP_TP_TF_OUT_FRMCFG_reg, tp_out_frmcfg_reg.regValue);

	tp_mask_print_emerg( TP_LOG_BLOCK_TPO_CLK, TP_LOG_LEVEL_WARNING, "TPO bypass_out_clk bypass_out_clk_h=%d bypass_out_clk_l=%d \n", bypass_out_clk_h, bypass_out_clk_l);
...
INT32 Tp_TPO_SetClkDiv( UINT32 pre_div, UINT32 post_div, UINT32 div16)
{
        ...
        /* tpo_clk = 1000 / (pre+post+2)  //for mark2/mio     */
...
#ifdef CONFIG_RTK_TP_WITH_MIO
        tp_out_frmcfg.out_2bit       = Param.out_2bit;
#endif
...
#ifdef CONFIG_RTK_TP_WITH_MIO
        pParam->out_2bit                = tp_out_frmcfg.out_2bit;
#endif
        tp_out_ctrl.regValue            = READ_REG32(TP_TP_OUT_CTRL_reg);
...
INT32 Tp_TPO_SetDataSource(TPK_TP_SOURCE_T src_type)
{
        ...
        tp_out_frmcfg_reg.data_src_mio = 0;
#endif
#endif
        switch(src_type) {
                case MTP:
                ...
                case MIO:
                        tp_out_frmcfg_reg.data_src = _BIT0 | _BIT1 | _BIT2;
                        tp_out_frmcfg_reg.data_src_mio = 1;
                        break;
#endif
#endif
                default:
...
UINT8 Tp_Get_RingBuffer_Index(TPK_TP_ENGINE_T tp_id)
{
    if(TP_TP0 == tp_id)
        return TP0_BUFFER_MASS_DATA;
    else if(TP_TP1 == tp_id)
        return TP1_BUFFER_MASS_DATA;
    else if(TP_TP2 == tp_id)
        return TP2_BUFFER_MASS_DATA;
    else if(TP_TP3 == tp_id)
        return TP3_BUFFER_MASS_DATA;
...
#ifdef CONFIG_RTK_TP_WITH_MIO
                                        tp_cntl.src_sel_mio = 1;   //for mark2/mio
#endif
                                        tp_cntl.src_sel_h = 0;
                                        tp_cntl.src_sel_l = 1;
        ...
        *pContinueReadSize = GET_BUFF_DATA_SIZE(buf_param);     /* no Wrap */
        ...
        if(full_stataus) {
                ...
                if(buffer_flag == TP_BUFFER_INFO_PACK) {
                        ...
                        if (pTp_drv->tp[tp_id].tp_src != MTP) {//if playback, do not set frame enable to zero and reset framer
...
#ifdef CONFIG_RTK_KDRV_SUPPORT_NAGNA
INT32 Tp_SetNagxx_PID_update_bit (TPK_TP_ENGINE_T tp_id, BOOL enable)
{
        tp_tp_nag_cntl1_RBUS nag_cntl1_data;
        CHECK_MAX_TP_COUNT(tp_id);

        nag_cntl1_data.regValue = READ_REG32(TP_TP_NAG_CNTL1_reg);
        switch(tp_id){
                case TP_TP0:
                        nag_cntl1_data.data_src = _ZERO;
                        break;
                case TP_TP1:
                        nag_cntl1_data.data_src = _BIT0;
                        break;
                case TP_TP2:
                        nag_cntl1_data.data_src = _BIT1;
                        break;
                default:
                        nag_cntl1_data.data_src = _BIT0 | _BIT1;
                        break;
        }

        if(tp_id >= TP_TP3){
                TP_WARNING("invalid, TP%d not support Nagxx\n", tp_id);
                WRITE_REG32(TP_TP_NAG_CNTL1_reg, nag_cntl1_data.regValue);
                return TPK_NOT_SUPPORT;
        }
        nag_cntl1_data.en_nag_pid_update_bit = enable;
        WRITE_REG32(TP_TP_NAG_CNTL1_reg, nag_cntl1_data.regValue);
        return TPK_SUCCESS;
}

INT32 Tp_GetNagxx_param (TPK_NAGXX_Conrol_Data *nag_ctl_data)
{
        tp_tp_nag_cntl1_RBUS nag_cntl1_data;

        nag_cntl1_data.regValue = READ_REG32(TP_TP_NAG_CNTL1_reg);
		nag_ctl_data->en_nag_pid_update_bit = nag_cntl1_data.en_nag_pid_update_bit;
		nag_ctl_data->msc_sel = nag_cntl1_data.msc_sel;
		nag_ctl_data->data_src = nag_cntl1_data.data_src;
...
#ifdef CONFIG_RTK_KDRV_SUPPORT_NAGNA
		/* nagra setting*/
        if(Param.descramble_sel){
                pid_data2.descramble_sel = Param.descramble_sel;
                pid_data2.keyindex = Param.keyindex;
                pid_data2.key_update = Param.key_update;
                pid_data2.aes_mode = Param.aes_mode;
                pid_data2.des_sel = Param.des_sel;
        }else{
                pid_data2.descramble_sel = Param.descramble_sel;
        }
#endif
...
#ifdef CONFIG_RTK_KDRV_SUPPORT_NAGNA
		/* Nagxx param */
        pParam->descramble_sel = pid_data2.descramble_sel;
        pParam->keyindex = pid_data2.keyindex;
        pParam->key_update = pid_data2.key_update;
        pParam->aes_mode = pid_data2.aes_mode;
        pParam->des_sel = pid_data2.des_sel;
#endif
        _tp_pid_unlock();
...
INT32 Tp_Cmd_Dump_PID_filter_Status(TPK_TP_ENGINE_T tp_id, char *buffer)
{
        ...
        for ( i = 0; i < cur_pid_filter_count; i++) {
                ...
                Tp_GetPidParam(pid_idx_reg, &Param);
                if ( Param.v )
                        pid_filter_count++;
...
INT32 Tp_ParseInfoPack(TPK_TP_ENGINE_T tp_id, UINT8* pData, SEC_INFO_T* pInfo) /* pData is virtual memory */
{
        ...
        UINT8 *tsp_hdr_vir;
        UINT8 tsp_hdr_ecp[4];
        UINT8 info_package_len;
        UADDRESS  info_package_phy;
        /* check parameter */
        CHECK_MAX_TP_COUNT(tp_id);
        CHECK_TP_STRUCTURE();
        ...
        if ((pData[0]& SEC_INFO_PACK_SEL) == SEC_INFO_PACK) {
                ...
                if(pTp_drv->useECP[tp_id] == 0) {
                    ...
                    info_package_phy = (UADDRESS)Tp_Buf_Vir_to_Phy(tp_id, TP_BUFFER_MASS_DATA, pointer_vir);
                    optee_tp_get_section_data(info_package_phy, 1);
                    memcpy(&info_package_len, (UINT8*)tp_shm_va, 1);
                    optee_tp_get_section_data(info_package_phy, info_package_len);
                    memcpy((UINT8*)pTp_drv->tp_pid[tp_id].pcr.adaption_field, (UINT8*)tp_shm_va, info_package_len);
                }
                _tp_ecp_unlock()
        ...
        tsp_hdr_vir = (UINT8*)pInfo->tsp_hdr_vir;
        if (pTp_drv->tp[tp_id].IsPktWithTimeStamp == 1) {
                tsp_hdr_vir  = tsp_hdr_vir + TIME_STAMP_PADDING;
        }

        _tp_ecp_lock()
        if(pTp_drv->useECP[tp_id] == 0) {
                if(pInfo->PID != ( ( (tsp_hdr_vir[1]&0x1f) << 8) | tsp_hdr_vir[2])) {
                        _tp_ecp_unlock()
                        return TPK_NOT_SUPPORT;
                }
        } else {
                if (pTp_drv->tp[tp_id].IsPktWithTimeStamp == 1) {
                        optee_tp_get_section_data(pInfo->tsp_hdr_phy + TIME_STAMP_PADDING, 4);
                }else{
                        optee_tp_get_section_data(pInfo->tsp_hdr_phy, 4);
                }
                memcpy(tsp_hdr_ecp, (UINT8*)tp_shm_va, 4);
                if(pInfo->PID != ( ( (tsp_hdr_ecp[1]&0x1f) << 8) | tsp_hdr_ecp[2])) {
                        _tp_ecp_unlock()
                        return TPK_NOT_SUPPORT;
                }
        }
                ...
                if((pSec->valid) &&
                    ...
                    //(pSec->sec_api_setting.Request_buf != NULL)
                        ...
                        if((get_buf_idx < 0) || (pDeliver_Sec_buf == NULL)) {
                                UINT32 time_cnt;
                                struct __kernel_old_timeval new_time;
                                static UINT8 first = 1;
                                static struct __kernel_old_timeval saved_time;

                                if (first) {
                                        TP_WARNING("SEC[%d] get section deliver buffer failed, skip to deliver this section(tp_id=%u, PID=0x%x)\n",
                                                   i, tp_id, pPid_param->PID);

                                        Tp_get_curtime(&saved_time);

                                        first = 0;
                                }

                                Tp_get_curtime(&new_time);
...
void Tp_Section_assemble(TPK_TP_ENGINE_T tp_id)
{
        ...
        for(Info_process_cnt = 0 ; Info_process_cnt < InfoBufCnt; Info_process_cnt += INFO_PACK_BUFFER_BLOCK_SIZE) {
                ...
                if(sec_info.tail_status) {
                        ...
                        } else if( (sec_info.tail_status & TAIL_STATUS_CRC_STATUS) ==  TAIL_STATUS_CRC_STATUS_TOO_SHORT_FOR_CRC_CHECK) {
                                /* Data too short, ignore it */
                                //TP_WARNING_DEBUG_LEVEL("TP%u PID=0x%x, pid_idx=%u, skip deliver sec data for data to short\n",
                                //                       tp_id, sec_info.PID, sec_info.pid_idx);
                        } else {
...
static INT32 section_assemble_thread(void* arg)
{
        ...
        struct __kernel_old_timeval saved_time, new_time;

        UINT32 time_cnt_check_tpp_buffer;
        UINT32 keep_low_buffer_cnt = 0;
        struct __kernel_old_timeval saved_time_check_tpp_buffer;
        TP_BUFFER_PARAM_T tpp_ring_buf;
        ...
        while(!kthread_should_stop()) {
                Tp_get_curtime(&new_time);
                TP_GET_TIME_CNT(time_cnt_check_tpp_buffer, new_time, saved_time_check_tpp_buffer);
                if( time_cnt_check_tpp_buffer >= check_tpp_buffer_interval_ms ) {
                        for( i = 0; i < MAX_TP_P_COUNT; i++) {
                            if (pTp_drv->tpp[i].tpp_stream_status == TPP_STREAM_START) {
                                  Tp_TPP_Get_Buffer(i, &tpp_ring_buf);
                                  if( tpp_ring_buf.WP < tpp_ring_buf.RP ) {
                                         tpp_ring_buf.WP = tpp_ring_buf.WP + ( tpp_ring_buf.Limit - tpp_ring_buf.Base );
                                  }
                                  if( (tpp_ring_buf.WP - tpp_ring_buf.RP) >= (( tpp_ring_buf.Limit - tpp_ring_buf.Base )/10) ) {
                                        Tp_TPP_GetCtrlReg(i, (UINT32*)&tp_p_cntl);
                                        if( tp_p_cntl.null_en == 0 ) {
                                              tp_p_cntl.regValue   = 0;
                                              tp_p_cntl.null_en    = 1;
                                              Tp_TPP_SetCtrlReg(i, tp_p_cntl.regValue, 1);
                                              TP_WARNING("TPP[%d] ring buffer data size is more than 1/10 buffer szie then enable null packet filter\n", i);
                                        }
                                  }

                                  if( (tpp_ring_buf.WP - tpp_ring_buf.RP) <= 1*1024 ) {
                                        Tp_TPP_GetCtrlReg(i, (UINT32*)&tp_p_cntl);
                                        if( tp_p_cntl.null_en == 1 && keep_low_buffer_cnt>5) {
                                              tp_p_cntl.regValue   = 0;
                                              tp_p_cntl.null_en    = 1;
                                              Tp_TPP_SetCtrlReg(i, tp_p_cntl.regValue, 0);
                                              TP_WARNING("TPP[%d] ring buffer data size is less than 1k then disable null packet filter\n", i);
                                        }
                                        keep_low_buffer_cnt++;
                                  } else {
                                        keep_low_buffer_cnt = 0;
                                  }
                            }
                        }

                        memcpy(&saved_time_check_tpp_buffer, &new_time, sizeof(struct __kernel_old_timeval));
                }
                ...
                for( i = 0; i < MAX_TP_COUNT; i++) {
                        ...
                        if(time_cnt >= debug_log_interval_ms) {
                                ...
                                tp_tf_cntl_reg.regValue = READ_REG32(tp_tf_cntl_reg_addr);
                                _tp_lock();
                                if(tp_tf_int_reg.overflow == 1 && tp_tf_cntl_reg.busy == 1 && (pTp_drv->mtp[i].mtp_stream_status==0)) {
                                        RHAL_SetTPHardwareReset(1);
                                }
...
static INT32 tp_dbg_thread(void* arg)
{
        ...
        struct __kernel_old_timeval saved_time_dbg, saved_time_status, new_time;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
        {
            struct timespec64 t64;
            ktime_get_real_ts64(&t64);
            saved_time_dbg.tv_sec = t64.tv_sec;
            saved_time_dbg.tv_usec = t64.tv_nsec / 1000;
        }
#else
        do_gettimeofday(&saved_time_dbg);
#endif
        memcpy(&saved_time_status, &saved_time_dbg, sizeof(struct __kernel_old_timeval));
        init_waitqueue_head(&timeout_wq);
        ...
        while(!kthread_should_stop()) {
                ...
                for( i = 0; i < MAX_TP_COUNT; i++) {
                        if( tp_log_onoff && (time_cnt_status >= status_log_interval_ms)) {
                                ...
                                if(pTp_drv->tp_japan[i].isJAPAN4K == 1) {
                                        ...
                                        TP_WARNING("[TP]flowrate=%d full=%u\n", pTp_drv->tp_japan[i].for_debug_tlv_total_size / time_cnt_status, pTp_drv->tp_japan[i].for_debug_ring_buffer_full_cnt);

                                        if( pTp_drv->tp_japan[i].src_type  == TPK_DELI_SRC_TYPE_TLV_HW0
                                            || pTp_drv->tp_japan[i].src_type  == TPK_DELI_SRC_TYPE_TLV_HW1
                                            ){
                                            UINT32 wp_temp, rp_temp, spare_size;
                                            wp_temp = TP_TLV_DESCRIPTOR_WRITE_get_pointer(READ_REG32(TP_TLV_DESCRIPTOR_WRITE_reg));
                                            rp_temp = TP_TLV_DESCRIPTOR_READ_get_pointer(READ_REG32(TP_TLV_DESCRIPTOR_READ_reg));
                                            if(wp_temp > rp_temp)
                                                spare_size = (wp_temp - rp_temp)*TLV_DESCRIPTOR_BUFFER_SIZE;
                                            else
                                                spare_size = (TLV_DESCRIPTOR_CNT + wp_temp - rp_temp)*TLV_DESCRIPTOR_BUFFER_SIZE;
                                            TP_DBG_SIMPLE("[TP%d]flowrate=%d full=%u buffer:total_size:%d, spare_size:%d (w_idx:%d, r_idx:%d)\n", i, pTp_drv->tp_japan[i].for_debug_tlv_total_size / time_cnt_status, READ_REG32(TP_TLV_DESCRIPTOR_FLAG_reg)&0x10000,
                                                   TLV_DESCRIPTOR_CNT*TLV_DESCRIPTOR_BUFFER_SIZE, spare_size, wp_temp, rp_temp);

                                        }else if( pTp_drv->tp_japan[i].src_type  == TPK_DELI_SRC_TYPE_TS
                                                  || pTp_drv->tp_japan[i].src_type  == TPK_DELI_SRC_TYPE_TLV_SW0
                                            ){
                                            UINT8 buf_idx = 0;
                                            UINT32 total_size, spare_size;
                                            buf_idx = Tp_Get_RingBuffer_Index(i);
                                            Tp_Get_Buffer(buf_idx, &ring_buf);
                                            total_size = ring_buf.Limit - ring_buf.Base;
                                            if(ring_buf.WP > ring_buf.RP)
                                                spare_size = ring_buf.WP - ring_buf.RP;
                                            else
                                                spare_size = total_size + ring_buf.WP - ring_buf.RP;

                                            TP_DBG_SIMPLE("[TP%d]flowrate=%d full=%u buffer:total_size:%d, spare_size:%d (R="PT_FMT_UADDR", W="PT_FMT_UADDR", B="PT_FMT_UADDR", L="PT_FMT_UADDR") \n", i, pTp_drv->tp_japan[i].for_debug_tlv_total_size / time_cnt_status, pTp_drv->tp_japan[i].for_debug_ring_buffer_full_cnt,
                                                   total_size, spare_size, ring_buf.RP, ring_buf.WP, ring_buf.Base, ring_buf.Limit);
                                        }
                                }
                        }
                }
...
#else
                                tp_mask_print_emerg( TP_LOG_BLOCK_PCR, TP_LOG_LEVEL_DEBUG, "misc90k_new=0x%08x misc90k_old=0x%08x tp90k_new=0x%x tp90k_old=0x%08x misc90k_diff=%d tp90k_diff=%d DISPA_SD2=0x%08x\n", misc90k_new, misc90k_old, tp90k_new, tp90k_old, misc90k_new - misc90k_old, tp90k_new - tp90k_old, READ_REG32(PLL_REG_SYS_PLL_DISPA_SD2_reg));
#endif
                ...
                if(time_cnt_dbg >= debug_log_interval_ms) {
                        ...
                        memcpy(&saved_time_dbg, &new_time, sizeof(struct __kernel_old_timeval));
                ...
                if ( time_cnt_status >= status_log_interval_ms ) {
                        memcpy(&saved_time_status, &new_time, sizeof(struct __kernel_old_timeval));
                }
                            ...
                            ( pTp_drv->tpp[tpp_id].tpp_src == TS_SRC_0
                              ...
                              || pTp_drv->tpp[tpp_id].tpp_src == TS_SRC_1
                                ...
                                if ( now_casType == TPK_WITH_CIP_CAS ) {
                                        // fix CI+1.4 to 12Mhz
                                        Tp_TPO_SetClkDiv(TPO_CLK_CIP_PRE_DIV, TPO_CLK_CIP_POST_DIV, 0);
...
{
        struct task_struct *sec_assemble = NULL;
        INT32 result = 0;
...
{
        struct task_struct *tp_dbg_t = NULL;
        INT32 ret = 0;
...
{
        struct task_struct *tpi_rate_cnt_t = NULL;
        INT32 ret = 0;
...
INT32 Tp_Update_PIDFilter_predes( TPK_TP_ENGINE_T tp_id,  UINT8 predes_value)
{
        ...
        for( i = 0; i < pid_cnt; i++) {
                ...
                if( (pid_reg.v == 1) && (pid_reg.sec_en == 1)) {
                        if( pid_reg.pre_des != predes_value ) {
...
INT32 Tp_FRC_SetPrefix( TPK_TP_ENGINE_T tp_id, TPK_FRC_PREFIX_T prefix_sel, UINT32 meta_data)
{
        ...
        TPK_FRC_PREFIX_EX_T prefix_selex = TP_FRC_EX_NO_PREFIX;
        ...
        if(prefix_sel == TP_FRC_DEBUG_01) { // TP_FRC_DEBUG_01 -> 90K
                prefix_selex = TP_FRC_EX_CNT_90K;
        } else if(prefix_sel == TP_FRC_DEBUG_02) { // TP_FRC_DEBUG_02 ->  27M
                prefix_selex = TP_FRC_EX_CNT_27M;
        } else if(prefix_sel == TP_FRC_META_DATA) { // TP_FRC_META_DATA
                prefix_selex = TP_FRC_EX_META_DATA;
        } else if(prefix_sel == TP_FRC_NO_PREFIX) { // TP_FRC_NO_PREFIX
                prefix_selex = TP_FRC_EX_NO_PREFIX;
        } else if(prefix_sel == TP_FRC_KEPP) { // TP_FRC_KEPP
                prefix_selex = TP_FRC_EX_KEPP;
        }

        prefix_ctrl.regValue = READ_REG32(TP_PREFIX_CTRL_reg);
        switch(tp_id) {
                case TP_TP0:
                        prefix_ctrl.tp0_sel = prefix_selex;
                        break;
                case TP_TP1:
                        prefix_ctrl.tp1_sel = prefix_selex;
                        break;
                case TP_TP2:
                        prefix_ctrl.tp2_sel = prefix_selex;
                        break;
#ifdef TP_SUPPORT_TP3
                case TP_TP3:
                        prefix_ctrl.tp3_sel = prefix_selex;
                        break;
#endif
                default:
...
INT32 TP_MTPStreamControl(TPK_TP_MTP_T mtp_id, TPK_MTP_STREAM_CTRL_T ctrl)
{
        ...
        tp_tp_tf0_int_RBUS    tp_int_ctrl;
        REGADDR m2m_ctrl_reg, m2m_limit_reg, m2m_base_reg, m2m_rp_reg, m2m_wp_reg, tp_int_en_reg, tp_int_reg;
        ...
        switch (mtp_id) {
                ...
                case TP_TPP0_MTP:
                        ...
                        tp_int_reg    = TP_TP_TF0_P_INT_reg;
                        break;
                case TP_TPP1_MTP:
                        ...
                        tp_int_reg    = TP_TP_TF1_P_INT_reg;
                        break;
                case TP_TPP2_MTP:
                        ...
                        tp_int_reg    = TP_TP_TF2_P_INT_reg;
                        break;
#endif
                default:
        ...
        if (ctrl == MTP_STREAM_START) {
                ...
                /* workaround:MAC8PQC-443 MTP has data, but TP blocked, so reset TP DMA state */
                WRITE_REG32(TP_TP_DMY_B_reg, READ_REG32(TP_TP_DMY_B_reg) | 0x1);
                WRITE_REG32(TP_TP_DMY_B_reg, READ_REG32(TP_TP_DMY_B_reg) & (~0x1));
#endif
                ...
                tp_int_ctrl.regValue = 0;
                Tp_Write_Data_Reg_Write(tp_int_reg, tp_int_ctrl.regValue);
...
INT32 Tp_TPP_MassBuffInit(TPK_TPP_ENGINE_T tpp_id)
{
        ...
        pTp_drv->tpp[tpp_id].mass_buffer.LimitVir = virt_addr + TP_P_BUFFER_SIZE;
        pTp_drv->tpp[tpp_id].mass_buffer.now_rp = phys_addr;

        pTp_drv->tpp[tpp_id].mass_buffer.bock_size = MASS_DATA_BUFFER_BLOCK_SIZE;

        pTp_drv->tpp[tpp_id].mass_buffer.RP_Connect[tpp_id] = phys_addr;
        pTp_drv->tpp[tpp_id].mass_buffer.RP_Connect_cnt = 1;
...
INT32 Tp_TPP_FlushBuf(TPK_TPP_ENGINE_T tpp_id)
{
		...
		/*	clear rp/wp record */
        pTp_drv->tpp[tpp_id].mass_buffer.RP					= buf_base_reg.regValue;
        pTp_drv->tpp[tpp_id].mass_buffer.WP					= buf_base_reg.regValue;
        pTp_drv->tpp[tpp_id].mass_buffer.RP_Connect[tpp_id] = buf_base_reg.regValue;
...
/***************************************************************
  Tp_Tpp_Set_Buffer : Set tpp ring buffer status
***************************************************************/
INT32 Tp_Tpp_Set_Buffer(UINT8 buf_idx, TP_BUFFER_PARAM_T param, UINT8 write_mask)
{
        tp_tp_p_ring_ctrl_RBUS buf_ctrl_reg;
        tp_tp_p_ring_limit_RBUS buf_limit_reg;
        tp_tp_p_ring_base_RBUS buf_base_reg;
        ...
        buf_limit_reg.res1     = 0;
        if(buf_limit_reg.ring_limit == 0) {
                TP_WARNING("[%s]bad parameter, buf_idx=%u, PhyBase="PT_FMT_UADDR", size=%u, PhyLimit="PT_FMT_UADDR"\n",
                           __func__, buf_idx, pPhyBase, (UINT32)(pPhyLimit - pPhyBase), pPhyLimit);
                return TPK_BAD_PARAMETER;
        }
        ...
        WRITE_REG32(TP_TP_P_RING_LIMIT_reg, buf_limit_reg.regValue);
        WRITE_REG32(TP_TP_P_RING_BASE_reg, buf_base_reg.regValue);
        WRITE_REG32(TP_TP_P_RING_RP_reg, buf_rp_reg.regValue);
        WRITE_REG32(TP_TP_P_RING_WP_reg, buf_wp_reg.regValue);
...
/***************************************************************
  Tp_TPP_rst :  reset framer
***************************************************************/
INT32 Tp_TPP_rst(TPK_TPP_ENGINE_T tpp_id)
{
        tp_tp_tf0_p_cntl_RBUS tp_p_cntl;
...
/***************************************************************
  Tp_TPP_Update_Buffer_RP :  Updat tpp buffer RP point
***************************************************************/
void Tp_TPP_Update_Buffer_RP(TPK_TPP_ENGINE_T tpp_id, TPK_TP_BUF_T *pBuf_status)
{
    TP_BUFFER_PARAM_T Param;
    UINT32 max_remain = 0, i = 0;
    UINT32 remain;
    ...
    Tp_TPP_Get_Buffer(tpp_id, &Param);
    ...
    Tp_Tpp_Set_Buffer(tpp_id, Param, (BUFFER_WRITE_MASK_LIMIT | BUFFER_WRITE_MASK_BASE | BUFFER_WRITE_MASK_WP));

    _tp_update_buf_rp_unlock();
}

/***************************************************************
  Tp_TPP_Buf_Phy_to_Vir :  TPP phy change to vir
***************************************************************/
UINT8* Tp_TPP_Buf_Phy_to_Vir(TPK_TPP_ENGINE_T tpp_id, UINT8* pPhyAddr)
{
	CHECK_MAX_TPP_COUNT_RT_NULL(tpp_id);

	if(pTp_drv->tpp[tpp_id].mass_buffer.inited == 0) {
	        TP_WARNING("TPP%u buffer not init\n", tpp_id);
	        return NULL;
	}

	if(((UADDRESS)pPhyAddr < pTp_drv->tpp[tpp_id].mass_buffer.BasePhy) ||
	    ((UADDRESS)pPhyAddr > pTp_drv->tpp[tpp_id].mass_buffer.LimitPhy)) {
	        TP_WARNING("TPP%u buffer over range (base="PT_FMT_UADDR", limit="PT_FMT_UADDR", pAddr=%p)\n",
	                   tpp_id, pTp_drv->tpp[tpp_id].mass_buffer.BasePhy, pTp_drv->tpp[tpp_id].mass_buffer.LimitPhy, pPhyAddr);
	        return NULL;
	}

	return (UINT8*)(pTp_drv->tpp[tpp_id].mass_buffer.BaseVir + ((UADDRESS)pPhyAddr - pTp_drv->tpp[tpp_id].mass_buffer.BasePhy));
}

UINT8* Tp_TPP_Buf_Vir_to_Phy(TPK_TPP_ENGINE_T tpp_id, UINT8* pVirAddr)
{
	CHECK_MAX_TPP_COUNT_RT_NULL(tpp_id);
    ...
    if(((UADDRESS)pVirAddr < pTp_drv->tpp[tpp_id].mass_buffer.BaseVir) ||
        ((UADDRESS)pVirAddr >pTp_drv->tpp[tpp_id].mass_buffer.LimitVir)) {
            TP_WARNING("TPP%u buffer over range (base="PT_FMT_UADDR", limit="PT_FMT_UADDR", pAddr=%p)\n",
                       tpp_id, pTp_drv->tpp[tpp_id].mass_buffer.BaseVir, pTp_drv->tpp[tpp_id].mass_buffer.LimitPhy, pVirAddr);
            return NULL;
    }

	return (UINT8*)(pTp_drv->tpp[tpp_id].mass_buffer.BasePhy + ((UADDRESS)pVirAddr - pTp_drv->tpp[tpp_id].mass_buffer.BaseVir));
}

INT32 Tp_TPP_Set_Buffer_Avail_status(TPK_TPP_ENGINE_T tpp_id, UINT8 data)
{
        tp_tp_p_ring_avail_int_RBUS reg;
        ...
        WRITE_REG32(TP_TP_P_RING_AVAIL_INT_reg, reg.regValue);
        return TPK_SUCCESS;
}

INT32 Tp_TPP_Set_Buffer_Full_status(TPK_TPP_ENGINE_T tpp_id, UINT8 data)
{
        tp_tp_p_ring_full_int_RBUS reg;
        ...
        reg.write_data = data;
        WRITE_REG32(TP_TP_P_RING_FULL_INT_reg, reg.regValue);
        return TPK_SUCCESS;
}


INT32 Tp_TPP_Get_Buffer_Full_status(TPK_TPP_ENGINE_T tpp_id, UINT8* data)
{
        tp_tp_p_ring_full_int_RBUS reg;
        ...
        reg.regValue = READ_REG32(TP_TP_P_RING_FULL_INT_reg);
...
INT32 Tp_TPP_Set_Buffer_Avail_int_En(TPK_TPP_ENGINE_T tpp_id, UINT8 Enable)
{
        tp_tp_p_ring_avail_int_en_RBUS reg;
        ...
        reg.write_data = Enable;
        WRITE_REG32(TP_TP_P_RING_AVAIL_INT_EN_reg, reg.regValue);
        return TPK_SUCCESS;
}


INT32 Tp_TPP_Set_Buffer_Full_int_En(TPK_TPP_ENGINE_T tpp_id, UINT8 Enable)
{
        tp_tp_p_ring_full_int_en_RBUS reg;
        ...
        reg.write_data = Enable;
        WRITE_REG32(TP_TP_P_RING_FULL_INT_EN_reg, reg.regValue);
        return TPK_SUCCESS;
}

INT32 Tp_TPP_ReadData(TPK_TPP_ENGINE_T tpp_id, UINT8** ppReadPhyPtr, UINT32* pContinueReadSize, UINT32 Threshold)
{
        TP_BUFFER_PARAM_T buf_param;
        INT32 result, block_size, buf_cnt_tmp;
        UINT8 full_stataus;
        TPK_TP_BUF_T *pBuf;
        char *ppReadVirPtr = NULL;
        UINT8 *pFirst_byte_vir;
        tp_tp_tf0_p_cntl_RBUS tpp_ctrl;
        tp_tp_tf0_p_frmcfg_RBUS tpp_frm;
        ...
        pBuf = &pTp_drv->tpp[tpp_id].mass_buffer;
        block_size = pTp_drv->tpp[tpp_id].mass_buffer.bock_size;

        if((pBuf->inited == 0) ||
            (pTp_drv->tpp[tpp_id].tpp_stream_status != TPP_STREAM_START)) {
                return TPK_NOT_INIT;
        }

        /* get buffer register */
		result = Tp_TPP_Get_Buffer(tpp_id, &buf_param);
        if(result != TPK_SUCCESS) {
                TP_WARNING("TPP%d get buffer status fail!\n", tpp_id);
                return result;
        }

        if(pTp_drv->tpp[tpp_id].IsMassBufferFull){
                buf_cnt_tmp = GET_BUFF_DATA_ALL_SIZE(buf_param);
                if(buf_cnt_tmp < ( GET_BUFF_SIZE(buf_param) / 10)) {
                        TP_WARNING("TPP%u mass data buffer empty detected, RESTART streaming\n",tpp_id);
                        /* clear full and avail int status */
                        Tp_TPP_Set_Buffer_Avail_status(tpp_id, _ZERO);
                        Tp_TPP_Set_Buffer_Full_status(tpp_id, _ZERO);

                        /* frame enable = 1 */
                        TP_TPP_StreamControl(tpp_id,TPP_STREAM_START);

                        pTp_drv->tpp[tpp_id].IsMassBufferFull = 0;
                }
                Tp_TPP_Update_Buffer_RP(tpp_id, pBuf);
        }

        /* replace RP with mass data RP */
        buf_param.RP  = pBuf->RP_Connect[tpp_id];
        *ppReadPhyPtr = (UINT8*)pBuf->RP_Connect[tpp_id];

        /* check limit and base */
        if((buf_param.Base == 0) || (buf_param.Limit == 0)) {
                TP_WARNING("buffer not init!(tpp_id=%u, Base="PT_FMT_UADDR", Limit="PT_FMT_UADDR")\n",
                           tpp_id, buf_param.Base, buf_param.Limit);
                return TPK_NOT_INIT;
        }

        /* check aligment for WP */
        if((buf_param.WP - buf_param.Base) % block_size) {
                TP_WARNING("WP error, not alignment (tpp_id=%u, Base="PT_FMT_UADDR", Limit="PT_FMT_UADDR", WP="PT_FMT_UADDR", block_size=%d)\n",
                           tpp_id, buf_param.Base, buf_param.Limit, buf_param.WP, block_size);
        ...
        /* calucate length */
		*pContinueReadSize = GET_BUFF_DATA_SIZE(buf_param);
        if(*pContinueReadSize != 0){
                /* check if first data is not 0x47 */
                pFirst_byte_vir = Tp_TPP_Buf_Phy_to_Vir(tpp_id, *ppReadPhyPtr);
                if(pFirst_byte_vir == NULL) {
                        TP_WARNING("TPP%u get TSP start virtual pointer error\n", tpp_id);
                        return TPK_NOT_INIT;
                }

                if (pTp_drv->tp[tpp_id].IsPktWithTimeStamp == 1) {
                        pFirst_byte_vir = pFirst_byte_vir + TIME_STAMP_PADDING;
                }
                Tp_TPP_GetCtrlReg(tpp_id, (UINT32*)&tpp_ctrl);
                if (pFirst_byte_vir[0] != 0x47 && !tpp_ctrl.dir_dma) {
#ifndef TP_VERIFY_ENABLE // verify sync byte replace in TP
#if 1
                        TP_VERBOSE("TPP%u first byte != 0x47 (base="PT_FMT_UADDR", limit="PT_FMT_UADDR", rp="PT_FMT_UADDR", wp="PT_FMT_UADDR", data= 0x%x 0x%x 0x%x 0x%x)\n",  tpp_id, buf_param.Base, buf_param.Limit, buf_param.RP, buf_param.WP,
                        pFirst_byte_vir[0], pFirst_byte_vir[1], pFirst_byte_vir[2], pFirst_byte_vir[3]);
#endif
#endif
                }
        ...
        /* check aligment for length */
		if((*pContinueReadSize) % block_size) {
			TP_WARNING("mass data length error, not alignment (tpp_id=%u,Base="PT_FMT_UADDR",Limit="PT_FMT_UADDR",WP="PT_FMT_UADDR", MASS_DATA_RP="PT_FMT_UADDR" block_size=%d *pContinueReadSize=%d)\n",tpp_id, buf_param.Base, buf_param.Limit , buf_param.WP, buf_param.RP, block_size, *pContinueReadSize);
			return TPK_BUFFER_UNALIGNMENT;
		}
        /* check if buffer full, flush buffer */
        result = Tp_TPP_Get_Buffer_Full_status(tpp_id, &full_stataus);
        if(result != TPK_SUCCESS) {
                TP_WARNING("get TPP%u buffer full status fail (result=%d)\n", tpp_id, result);
                return result;
        }

        if(full_stataus) {
                if(pTp_drv->tpp[tpp_id].tpp_src != MTP) {
                        TP_WARNING("TPP%u buffer full detect (threshold=%u, rp="PT_FMT_UADDR"("PT_FMT_UADDR"), wp="PT_FMT_UADDR", base="PT_FMT_UADDR", limit="PT_FMT_UADDR", real_sz=%ld, rt_sz=%u)\n", tpp_id, READ_REG32(TP_TP_P_THRESHOLD_reg), buf_param.RP, pBuf->RP_Connect[tpp_id], buf_param.WP, buf_param.Base, buf_param.Limit,(unsigned long)GET_BUFF_DATA_ALL_SIZE(buf_param), *pContinueReadSize);
                }
                /*  Mass data buffer will be released by upper api */
                if (!pTp_drv->tpp[tpp_id].IsMassBufferFull) {
                        TP_WARNING("TPP%u mass data buffer full detected, STOP streaming\n", tpp_id);
                        /*  frame enable =0 */
                        Tp_TPP_GetFrmReg(tpp_id, &tpp_frm.regValue);
						tpp_frm.frm_en   = 0;
                        Tp_TPP_SetFrmReg(tpp_id, tpp_frm.regValue);

                        /*  reset framer */
                        Tp_TPP_rst(tpp_id);
                        pTp_drv->tpp[tpp_id].IsMassBufferFull = 1;
                }
                return  TPK_BUFFER_FULL;
        } else {
                if(pTp_drv->tpp[tpp_id].tpp_src != MTP && !(pTp_drv->tpp[tpp_id].IsMassBufferFull)) {
                        TP_VERBOSE("TPP%u mass buffer normally!(threshold=%u, rp="PT_FMT_UADDR"("PT_FMT_UADDR"), wp="PT_FMT_UADDR", base="PT_FMT_UADDR", limit="PT_FMT_UADDR", real_sz=%ld, rt_sz=%u)\n",
                        tpp_id, READ_REG32(TP_TP_P_THRESHOLD_reg), buf_param.RP, pBuf->RP_Connect[tpp_id], buf_param.WP, buf_param.Base, buf_param.Limit,
                        (unsigned long)GET_BUFF_DATA_ALL_SIZE(buf_param), *pContinueReadSize);
                }
        }

        /*  check if length is less than threshold */
        if( ( *pContinueReadSize >= Threshold ) || ( buf_param.RP > buf_param.WP) ) {
                ppReadVirPtr = Tp_TPP_Buf_Phy_to_Vir(tpp_id, (unsigned char*)*ppReadPhyPtr);
...
INT32 Tp_TPP_ReleaseData(TPK_TPP_ENGINE_T tpp_id, UINT8* pReadPhyPtr, UINT32 Size)
{
        TP_BUFFER_PARAM_T buf_param;
        INT32 result = 0, block_size;
        UADDRESS Offset;
        ...
        if((pBuf->inited == 0) || (pTp_drv->tpp[tpp_id].tpp_stream_status != TPP_STREAM_START)) {
                TP_WARNING("buffer not init, tpp_id=%d, pBuf->inited=%u, tpp_stream_status=%u\n",
                           tpp_id, pBuf->inited, pTp_drv->tpp[tpp_id].tpp_stream_status);
                return TPK_NOT_INIT;
        }

        /* check if pReadPhyPtr is same as driver buffer rp saving */
		if((UADDRESS)pReadPhyPtr != pBuf->RP_Connect[tpp_id]) {
                TP_WARNING("release buff data pointer(%p) is not equal to the driver saved RP("PT_FMT_UADDR"), do nothing \n",
                           pReadPhyPtr, pBuf->RP_Connect[tpp_id]);
                return TPK_BAD_PARAMETER;
        }

        /* check if length alignment (188 bytes or 8 bytes) */
		if(Size % block_size) {
		    TP_WARNING("TPP%d released size of buffer is %u bytes not alignment(block_size=%u)\n",tpp_id, Size, block_size);
		    return TPK_BUFFER_UNALIGNMENT;
		}
		/* get buffer status */
        result = Tp_TPP_Get_Buffer(tpp_id, &buf_param);
        if(result != TPK_SUCCESS) {
                TP_WARNING("get buffer status fail (tpp_id=%d)\n", tpp_id);
                return result;
        }

        buff_data_size = GET_BUFF_DATA_ALL_SIZE(buf_param);
        if( Size > buff_data_size ) {
		    TP_WARNING("TPP%d release buffer size(%u) is large than actual buffer size(%u), (0x%p/"PT_FMT_UADDR") ("PT_FMT_UADDR"/"PT_FMT_UADDR"/"PT_FMT_UADDR"/"PT_FMT_UADDR")!\n", tpp_id, Size, buff_data_size, pReadPhyPtr, pBuf->RP_Connect[tpp_id], buf_param.Base, buf_param.RP, buf_param.WP, buf_param.Limit);
		}
        /* update to rp record */
        Offset = (UADDRESS)pReadPhyPtr;
        Offset += Size;
...
{
        UINT32 tp_cnt_reg = 0, tp_drp_cnt_reg = 0, tp_err_cnt_reg = 0;
        UINT32 tp_cntl_reg = 0, tp_frmcfg_reg = 0;
        UINT8  buf_idx = 0;
        TP_BUFFER_PARAM_T buf_param;
        tp_tp_tf0_frmcfg_RBUS tp_frm;
        tp_tp_tf0_cntl_RBUS tp_cntl;
        ...
        {
                pTp_drv->tp_input_monitor_virt_addr = (UADDRESS)dvr_malloc_specific(TP_P_MONITOR_BUFFER_SIZE, GFP_DCU2_FIRST);
                ...
                pTp_drv->tp_input_monitor_phys_addr = (UADDRESS)dvr_to_phys((void *)pTp_drv->tp_input_monitor_virt_addr);
        ...
        buf_param.Base  =(UADDRESS)pTp_drv->tp_input_monitor_phys_addr;
        buf_param.Limit =(UADDRESS)pTp_drv->tp_input_monitor_phys_addr + TP_P_BUFFER_SIZE;
        buf_param.RP    =(UADDRESS)pTp_drv->tp_input_monitor_phys_addr;
        buf_param.WP    =(UADDRESS)pTp_drv->tp_input_monitor_phys_addr;
        Tp_Set_Buffer(buf_idx, buf_param, BUFFER_WRITE_MASK_NONE);
...
)
{
        ...
        if(pTimeinfo[0] != 0x00 && pTp_drv->tp_atsc[p_this->tp_id].clock_recovery == 1) {
                ...
                if(pTimeinfo[0] == 0x81) {
                        ...
                        if( pTp_drv->tp_atsc[p_this->tp_id].wall_clock_value != STC_value) {
                                ...
                                tp_mask_print_emerg( TP_LOG_BLOCK_PCR, TP_LOG_LEVEL_DEBUG, "wall=%d.%03d STC_Local=%lld STC diff=%lld\n", sec, msec, STC_Local, STC_value - pTp_drv->tp_atsc[p_this->tp_id].wallclock_stc_offset - STC_Local);
                                pTp_drv->tp_atsc[p_this->tp_id].wall_clock_value = STC_value;
                        }
        ...
        if(pTp_drv->tp_atsc[p_this->tp_id].for_debug_first_valid_bbf_in_time == 0) {
                ...
                TP_WARNING("#DeliveryPerf# [%d] TP%d First valid bbf data in.pPhyWP:%ld\n", READ_REG32(TIMER_SCPU_CLK90K_LO_reg) / 90, p_this->tp_id, pPhyWP);
        }
...
void TP_TLV_FilterInit(RTK_TLV_REG_T *ptlv_regs)
{
    unsigned int i;
    tp_tlv_filter_cfg_RBUS tlv_filter_cfg_reg;
    tp_tlv_filter_cfg_ctrl_RBUS tlv_filter_cfg_ctrl_reg;

    tlv_filter_cfg_reg.regValue = READ_REG32(ptlv_regs->TLV_FILTER_CFG);
    tlv_filter_cfg_reg.idx_num = TP_TLV_FILTER_NUM - 1;
    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG, tlv_filter_cfg_reg.regValue);

    for(i = 0; i < TP_TLV_FILTER_NUM; i++){
        WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_EN, 0);
        WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D0, 0);
        WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D1, 0);
        WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D2, 0);
        WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D3, 0);
        WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D4, 0);
        WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D5, 0);
        WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D7, 0);
        WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D8, 0);
        WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D9, 0);
        WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D10, 0);
        WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D11, 0);

        tlv_filter_cfg_ctrl_reg.act = 1;
        tlv_filter_cfg_ctrl_reg.rw  = 1;
        tlv_filter_cfg_ctrl_reg.idx = i;
        WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_CTRL, tlv_filter_cfg_ctrl_reg.regValue);
    }
}

void TP_TLV_FilterUnInit(RTK_TLV_REG_T *ptlv_regs)
{
    tp_tlv_filter_cfg_RBUS tlv_filter_cfg_reg;

    tlv_filter_cfg_reg.regValue = READ_REG32(ptlv_regs->TLV_FILTER_CFG);
    tlv_filter_cfg_reg.dmy_en  = 0;
    tlv_filter_cfg_reg.null_en = 0;
    tlv_filter_cfg_reg.tcsp_en = 1;
    tlv_filter_cfg_reg.enable  = 0;
    tlv_filter_cfg_reg.idx_num = 0;
    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG, tlv_filter_cfg_reg.regValue);
}

void TP_TLV_FilterEnable(RTK_TLV_REG_T *ptlv_regs)
{
    tp_tlv_filter_cfg_RBUS tlv_filter_cfg_reg;
    tlv_filter_cfg_reg.regValue = READ_REG32(ptlv_regs->TLV_FILTER_CFG);
    tlv_filter_cfg_reg.enable  = 1;
    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG, tlv_filter_cfg_reg.regValue);
}

void TP_TLV_FilterDisable(RTK_TLV_REG_T *ptlv_regs)
{
    tp_tlv_filter_cfg_RBUS tlv_filter_cfg_reg;
...
void TP_TLV_SetFilterParams(RTK_TLV_REG_T *ptlv_regs, UINT8 filter_idx, RTK_TLV_FILTER_PARAMS *p_filter_param)
{
    UINT32 filter_type = TPK_DELI_TLV_FILTER_ALL_INVALID;
    UINT32 src_ipdata[4] = {0}, dst_ipdata[4]={0};
    tp_tlv_filter_cfg_ctrl_RBUS tlv_filter_cfg_ctrl_reg;
    tp_tlv_filter_cfg_d5_RBUS   tlv_filter_cfg_d5_reg;
    tp_tlv_filter_cfg_d0_RBUS   tlv_filter_cfg_d0_reg;
    tp_tlv_filter_cfg_en_RBUS   tlv_filter_cfg_en_reg;

    tlv_filter_cfg_d5_reg.regValue = 0;
    tlv_filter_cfg_d0_reg.regValue = 0;
    tlv_filter_cfg_en_reg.regValue = filter_type;

    if(NULL != p_filter_param){
        /*no ip params filter*/
        if(0 == p_filter_param->flag){
            filter_type = TPK_DELI_TLV_FILTER_PKT_ID_VALID;
            tlv_filter_cfg_d5_reg.pkt_id = (p_filter_param->packet_id & 0xFFFF);
        }
        /*has ipv4 params filter*/
        else if(1 == p_filter_param->flag){
            src_ipdata[3] = (p_filter_param->src_ip[0] << 24) | (p_filter_param->src_ip[1] << 16)
                                 | (p_filter_param->src_ip[2] << 8) | p_filter_param->src_ip[3];

            dst_ipdata[3] = (p_filter_param->dst_ip[0] << 24) | (p_filter_param->dst_ip[1] << 16)
                                 | (p_filter_param->dst_ip[2] << 8) | p_filter_param->dst_ip[3];

            filter_type = TPK_DELI_TLV_FILTER_IPSRC_ADDR_VALID | TPK_DELI_TLV_FILTER_IPDST_ADDR_VALID;
            if(p_filter_param->packet_id){
                filter_type |= TPK_DELI_TLV_FILTER_PKT_ID_VALID;
                tlv_filter_cfg_d5_reg.pkt_id = (p_filter_param->packet_id & 0xFFFF);
            }
        }
        /*has ipv6 params filter*/
        else if(2 == p_filter_param->flag){
            src_ipdata[0] = (p_filter_param->src_ip[0] << 24) | (p_filter_param->src_ip[1] << 16)
                             | (p_filter_param->src_ip[2] << 8) | p_filter_param->src_ip[3];
            src_ipdata[1] = (p_filter_param->src_ip[4] << 24) | (p_filter_param->src_ip[5] << 16)
                             | (p_filter_param->src_ip[6] << 8) | p_filter_param->src_ip[7];
            src_ipdata[2] = (p_filter_param->src_ip[8] << 24) | (p_filter_param->src_ip[9] << 16)
                             | (p_filter_param->src_ip[10] << 8) | p_filter_param->src_ip[11];
            src_ipdata[3] = (p_filter_param->src_ip[12] << 24) | (p_filter_param->src_ip[13] << 16)
                             | (p_filter_param->src_ip[14] << 8) | p_filter_param->src_ip[15];

            dst_ipdata[0] = (p_filter_param->dst_ip[0] << 24) | (p_filter_param->dst_ip[1] << 16)
                             | (p_filter_param->dst_ip[2] << 8) | p_filter_param->dst_ip[3];
            dst_ipdata[1] = (p_filter_param->dst_ip[4] << 24) | (p_filter_param->dst_ip[5] << 16)
                             | (p_filter_param->dst_ip[6] << 8) | p_filter_param->dst_ip[7];
            dst_ipdata[2] = (p_filter_param->dst_ip[8] << 24) | (p_filter_param->dst_ip[9] << 16)
                             | (p_filter_param->dst_ip[10] << 8) | p_filter_param->dst_ip[11];
            dst_ipdata[3] = (p_filter_param->dst_ip[11] << 24) | (p_filter_param->dst_ip[13] << 16)
                             | (p_filter_param->dst_ip[14] << 8) | p_filter_param->dst_ip[15];
        ...
        /*mmtp si filter*/
        else if(3 == p_filter_param->flag){
            filter_type = TPK_DELI_TLV_FILTER_PYLD_TYPE_VALID;
            tlv_filter_cfg_d5_reg.pyld_type= (TLV_PAKET_MMTP_PAYLAD_SI & 0x3F);
        }
        /*init cid mapping table and valid this filter*/
        tlv_filter_cfg_d0_reg.vld = 1;
        tlv_filter_cfg_d0_reg.ini = 1;
    }else{    /*remove this filter*/
        /*uninit cid mapping table and invalid this filter*/
        tlv_filter_cfg_d0_reg.vld = 0;
        tlv_filter_cfg_d0_reg.ini = 0;
    }
    /*valid filter types*/
    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_EN, filter_type);
    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D0, tlv_filter_cfg_d0_reg.regValue);

    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D8,  src_ipdata[0]);
    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D9,  src_ipdata[1]);
    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D10, src_ipdata[2]);
    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D11, src_ipdata[3]);

    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D1, dst_ipdata[0]);
    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D2, dst_ipdata[1]);
    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D3, dst_ipdata[2]);
    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D4, dst_ipdata[3]);

    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_D5, tlv_filter_cfg_d5_reg.regValue);

    tlv_filter_cfg_ctrl_reg.act = 1;
    tlv_filter_cfg_ctrl_reg.rw  = 1;
    tlv_filter_cfg_ctrl_reg.idx = filter_idx;
    WRITE_REG32(ptlv_regs->TLV_FILTER_CFG_CTRL, tlv_filter_cfg_ctrl_reg.regValue);
}

UINT32 TP_TLV_GetCIDMappedwithIP(RTK_TLV_REG_T *ptlv_regs, UINT8 filter_idx)
{
    tp_tlv_filter_cfg_ctrl_RBUS tlv_filter_cfg_ctrl_reg;

    tlv_filter_cfg_ctrl_reg.act = 1;
    tlv_filter_cfg_ctrl_reg.rw  = 0;
    tlv_filter_cfg_ctrl_reg.idx = filter_idx;

    return READ_REG32(ptlv_regs->TLV_FILTER_CFG_D6) & 0xFFF;
}

void TP_TLV_EnableHWParseNTP(RTK_TLV_REG_T *ptlv_regs)
{
    tp_tlv_ntp_RBUS tlv_ntp_reg;
    tp_tlv_ntp_stc_RBUS tlv_ntp_stc_reg;
    tp_tlv_ntp_cfg0_RBUS tlv_ntp_port_reg;

    /*fix hw wrong default value */
    tlv_ntp_port_reg.regValue = READ_REG32(ptlv_regs->TLV_NTP_CFG0);
    tlv_ntp_port_reg.port = 0x7B;
    WRITE_REG32(ptlv_regs->TLV_NTP_CFG0, tlv_ntp_port_reg.regValue);

    tlv_ntp_reg.regValue = READ_REG32(ptlv_regs->TLV_NTP);
    tlv_ntp_reg.dma_en = 1;
    tlv_ntp_reg.en = 1;
    WRITE_REG32(ptlv_regs->TLV_NTP, tlv_ntp_reg.regValue);

    tlv_ntp_stc_reg.regValue = READ_REG32(ptlv_regs->TLV_STC);
    tlv_ntp_stc_reg.st = 1;
    WRITE_REG32(ptlv_regs->TLV_STC, tlv_ntp_stc_reg.regValue);
}

void TP_TLV_DisableHWParseNTP(RTK_TLV_REG_T *ptlv_regs)
{
    tp_tlv_ntp_RBUS tlv_ntp_reg;
    tp_tlv_ntp_stc_RBUS tlv_ntp_stc_reg;

    tlv_ntp_reg.regValue = READ_REG32(ptlv_regs->TLV_NTP);
    tlv_ntp_reg.dma_en = 0;
    tlv_ntp_reg.en = 0;
    WRITE_REG32(ptlv_regs->TLV_NTP, tlv_ntp_reg.regValue);

    tlv_ntp_stc_reg.regValue = READ_REG32(ptlv_regs->TLV_STC);
    tlv_ntp_stc_reg.st = 0;
    WRITE_REG32(ptlv_regs->TLV_STC, tlv_ntp_stc_reg.regValue);
}

void TP_TLV_SetNTPFilterParams(RTK_TLV_REG_T *ptlv_regs, RTK_TLV_NTP_FILTER_PARAMS *p_params)
{
    tp_tlv_ntp_mask_RBUS tlv_ntp_mask_reg;
    tp_tlv_ntp_cfg0_RBUS tlv_ntp_port_reg;
    tp_tlv_ntp_cfg1_RBUS tlv_ntp_cfg1_reg;
    tp_tlv_ntp_cfg2_RBUS tlv_ntp_cfg2_reg;
    tp_tlv_ntp_cfg3_RBUS tlv_ntp_cfg3_reg;
    tp_tlv_ntp_cfg4_RBUS tlv_ntp_cfg4_reg;

    tlv_ntp_mask_reg.regValue = READ_REG32(ptlv_regs->TLV_NTP_MASK);
    if(TLV_NTP_SET_PORT == p_params->flag || TLV_NTP_SET_PORT_AND_IP == p_params->flag )
    {
        tlv_ntp_mask_reg.port = p_params->port_mask;
        WRITE_REG32(ptlv_regs->TLV_NTP_MASK, tlv_ntp_mask_reg.regValue);

        tlv_ntp_port_reg.regValue = READ_REG32(ptlv_regs->TLV_NTP_CFG0);
        tlv_ntp_port_reg.port = p_params->port;
        WRITE_REG32(ptlv_regs->TLV_NTP_CFG0, tlv_ntp_port_reg.regValue);
    }
    if(TLV_NTP_SET_IP == p_params->flag || TLV_NTP_SET_PORT_AND_IP == p_params->flag )
    {
        tlv_ntp_mask_reg.addr = p_params->ip_mask;
        WRITE_REG32(ptlv_regs->TLV_NTP_MASK, tlv_ntp_mask_reg.regValue);

        tlv_ntp_cfg1_reg.regValue = READ_REG32(ptlv_regs->TLV_NTP_CFG1);
        tlv_ntp_cfg1_reg.addr0 = p_params->ip_addr[0];
        WRITE_REG32(ptlv_regs->TLV_NTP_CFG1, tlv_ntp_cfg1_reg.regValue);

        tlv_ntp_cfg2_reg.regValue = READ_REG32(ptlv_regs->TLV_NTP_CFG2);
        tlv_ntp_cfg2_reg.addr1 = p_params->ip_addr[1];
        WRITE_REG32(ptlv_regs->TLV_NTP_CFG1, tlv_ntp_cfg2_reg.regValue);

        tlv_ntp_cfg3_reg.regValue = READ_REG32(ptlv_regs->TLV_NTP_CFG3);
        tlv_ntp_cfg3_reg.addr2 = p_params->ip_addr[2];
        WRITE_REG32(ptlv_regs->TLV_NTP_CFG3, tlv_ntp_cfg3_reg.regValue);

        tlv_ntp_cfg4_reg.regValue = READ_REG32(ptlv_regs->TLV_NTP_CFG4);
        tlv_ntp_cfg4_reg.addr3 = p_params->ip_addr[3];
        WRITE_REG32(ptlv_regs->TLV_NTP_CFG4, tlv_ntp_cfg4_reg.regValue);
    }
}
#endif
/****************************************************************************************/

void TP_TLV_Reset(TPK_TP_ENGINE_T tp_id, RTK_TLV_REG_T *ptlv_regs)
{
    int i;
    tp_tlv_ctrl_RBUS tlv_ctrl_reg;
    UINT32 temp1;

    WRITE_REG32( ptlv_regs->TLV_CTRL, 0x10000001 );

    do {
        temp1 = READ_REG32( ptlv_regs->TLV_CTRL );
        if( (temp1 & 0x20000000) ==  0x00)	{
            UINT32 temp_main = READ_REG32( ptlv_regs->TLV_PARSER_MAIN );
            UINT32 temp_dma = READ_REG32( ptlv_regs->TLV_PARSER_DMA );
    ...
    WRITE_REG32( ptlv_regs->TLV_SYNC, 0x00000000 | TLV_SYNC_BYTE);
    WRITE_REG32( ptlv_regs->TLV_CFG , 0x00000000 | TLV_DESCRIPTOR_BUFFER_SIZE);

    WRITE_REG32( ptlv_regs->TLV_DESCRIPTOR_WRITE, 0x00000000 );
    WRITE_REG32( ptlv_regs->TLV_DESCRIPTOR_READ , 0x00000000 );

    WRITE_REG32( ptlv_regs->TLV_DESCRIPTOR_TIME,	 0xffffffff );
    WRITE_REG32( ptlv_regs->TLV_DESCRIPTOR_PKT_NUM , 0x000000ff );

    for(i=0; i<TLV_DESCRIPTOR_CNT; i++) {
        memset( (UINT8*)(pTp_drv->tp_japan[tp_id].tlv_descripor_buffer[i].BaseVir), 0xff, TLV_DESCRIPTOR_BUFFER_SIZE);

        WRITE_REG32( ptlv_regs->TLV_DESCRIPTOR_DATA0,  pTp_drv->tp_japan[tp_id].tlv_descripor_buffer[i].BasePhy);
        WRITE_REG32( ptlv_regs->TLV_DESCRIPTOR_DATA1, 0x00000000);
        WRITE_REG32( ptlv_regs->TLV_DESCRIPTOR		, 0x00001100 | i);
    }
    ...
    WRITE_REG32(ptlv_regs->TLV_DESCRIPTOR_FLAG	, 0x00010000 );
    ...
    switch(pTp_drv->tp_japan[tp_id].inputType) {
    ...
    WRITE_REG32( ptlv_regs->TLV_CTRL, tlv_ctrl_reg.regValue );
    TP_WARNING("reset TLV engine[%d]\n", tp_id);
}

void TP_TLV_Release(RTK_TLV_REG_T *ptlv_regs)
{
    UINT32 temp1;
    WRITE_REG32( ptlv_regs->TLV_CTRL, 0x10000001 );
    do {
    ...
    WRITE_REG32( ptlv_regs->TLV_CTRL, 0x00000000 );
}

INT32 TP_TLV_Config(TPK_TP_ENGINE_T tp_id, RTK_TLV_REG_T *ptlv_regs)
{
    UINT32   i;
    tp_tlv_ctrl_RBUS tlv_ctrl_reg;
...
void Tp_japan4k_dmx_callback(void* p_ctx, unsigned char *p_data, unsigned int len)
{
        ...
        // To get the NTP information from the TLV packet
        if(p_data[0] == 0x7f && (p_data[1] == 0x1 || p_data[1] == 0x2) && pTp_drv->tp_atsc[p_this->tp_id].clock_recovery) {
                UINT32 sec = 0, msec = 0, usec = 0, nsec = 0;
                UINT64 STC_value = 0, STC_Local = 0;
                UINT32 STC_value_Hi = 0, STC_value_Low = 0;
#if 0
                TP_WARNING("Deliver TLV, %d bytes {%02x %02x %02x %02x_%02x %02x %02x %02x_%02x %02x %02x %02x_%02x %02x %02x %02x_%02x %02x %02x %02x_%02x %02x %02x %02x_%02x %02x %02x %02x... %02X %02X %02X %02X %02X %02X %02X %02X}\n",
                           len,
                           p_data[0], p_data[1], p_data[2], p_data[3],
                           p_data[4], p_data[5], p_data[6], p_data[7],
                           p_data[8], p_data[9], p_data[10], p_data[11],
                           p_data[12], p_data[13], p_data[14], p_data[15],
                           p_data[16], p_data[17], p_data[18], p_data[19],
                           p_data[20], p_data[21], p_data[22], p_data[23],
                           p_data[24], p_data[25], p_data[26], p_data[27],
                           p_data[len - 8], p_data[len - 7], p_data[len - 6], p_data[len - 5],
                           p_data[len - 4], p_data[len - 3], p_data[len - 2], p_data[len - 1]);
#endif
                if(p_data[1] == 0x1) {
                        // IPv4
                        // No need to implement, Japan4k no support IPV4
                }
                if(p_data[1] == 0x2) {
                        // IPv6
                        if(p_data[4] == 0x60 && p_data[42] == 0x01 && p_data[43] == 0x01 && p_data[47] == 0x7b && len == 100) {
                                memcpy(pTimeinfo, &p_data[len - 8], 8);
                                sec  = pTimeinfo[0] << 24 | pTimeinfo[1] << 16 | pTimeinfo[2] << 8 | pTimeinfo[3];
                                msec = ((pTimeinfo[4] & 0xff) << 2) | ((pTimeinfo[5] & 0xc0) >> 6);
                                usec = ((pTimeinfo[5] & 0x3f) << 4) | ((pTimeinfo[6] & 0xf0) >> 4);
                                nsec = ((pTimeinfo[6] & 0x0f) << 6) | ((pTimeinfo[7] & 0xfc) >> 2);
                                STC_value = div64_u64(((UINT64)sec * 1000000 + (UINT64)msec * 1000 + (UINT64)usec) * (UINT64)90 , 1000);

                                tp_mask_print_emerg( TP_LOG_BLOCK_PCR, TP_LOG_LEVEL_DEBUG, "NTP = %02x %02x %02x %02x %02x %02x %02x %02x\n", pTimeinfo[0], pTimeinfo[1], pTimeinfo[2], pTimeinfo[3], pTimeinfo[4], pTimeinfo[5], pTimeinfo[6], pTimeinfo[7]);
                                tp_mask_print_emerg( TP_LOG_BLOCK_PCR, TP_LOG_LEVEL_DEBUG, "sec=0x%x msec=0x%x usec=0x%x nsec=0x%x\n", sec, msec, usec, nsec);

                                Tp_Get_PCR(p_this->tp_id, &STC_value_Hi, &STC_value_Low);
                                STC_Local = ((UINT64)STC_value_Hi << 32) + STC_value_Low;

                                if( abs(STC_value - (STC_Local + pTp_drv->tp_atsc[p_this->tp_id].wallclock_stc_offset)) > 45000 ) {
                                        TP_WARNING("resync wallclock offset=%lld wall=%lld stc=%lld modify_offset=%lld\n", pTp_drv->tp_atsc[p_this->tp_id].wallclock_stc_offset, STC_value, STC_Local, STC_value - STC_Local);
                                        pTp_drv->tp_atsc[p_this->tp_id].wallclock_stc_offset = STC_value - STC_Local;
                                }
        ...
        if( (pTp_drv->tp_japan[p_this->tp_id].dmx->rhal_tlv_buffer_max_size - pTp_drv->tp_japan[p_this->tp_id].dmx->rhal_tlv_buffer_size) > len ) {
                if(copy_to_user((void __user *)to_user_ptr((pTp_drv->tp_japan[p_this->tp_id].dmx->rhal_tlv_buffer + pTp_drv->tp_japan[p_this->tp_id].dmx->rhal_tlv_buffer_size)), (const void *)p_data, len)) {
...
{
        *isNeedProtect = (UINT8)((READ_REG32(ECP_CTRL_REG) & ECP_CTRL_BIT) != 0);
        return TPK_SUCCESS;
}
...
#ifdef CONFIG_RTK_TP_WITH_MIO
/* MIO TP init*/
INT32 TP_MIO_init(void)
{
    iso_spi_synchronizer_tp_ctrl_RBUS mio_tp_cntl;
    TPK_MIO_T2P_PARAM_T T2P_param;
    TPK_MIO_P2T_PARAM_T P2T_param;

    mio_tp_cntl.regValue = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_CTRL_reg);
    mio_tp_cntl.sw_reset     = 1;
    WRITE_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_CTRL_reg, mio_tp_cntl.regValue);

    ndelay(110);

    mio_tp_cntl.regValue = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_CTRL_reg);
    mio_tp_cntl.sw_reset     = 0;
    WRITE_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_CTRL_reg, mio_tp_cntl.regValue);

    /*set mio out_clk pol*/
    TP_MIO_Get_T2PMode(&T2P_param);
    T2P_param.out_clk_pol = 1;
    TP_MIO_Set_T2PMode(T2P_param);

    TP_MIO_Get_P2TMode(&P2T_param);
    P2T_param.out_clk_pol = 1;
    TP_MIO_Set_P2TMode_(P2T_param);

    TP_WARNING("TP_mio init success!\n");
    return TPK_SUCCESS;
}

/*Enable circuits. Gated operating clock. default:on*/
INT32 TP_MIO_Enable(BOOL enable)
{
    iso_spi_synchronizer_tp_ctrl_RBUS mio_tp_cntl;

    mio_tp_cntl.regValue = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_CTRL_reg);
    mio_tp_cntl.en     = enable;
    WRITE_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_CTRL_reg, mio_tp_cntl.regValue);
...
/*T2P mode setting*/
/*T2P: 2bit to parallel, mark2 to mio path*/
INT32 TP_MIO_Set_T2PMode(TPK_MIO_T2P_PARAM_T Param)
{
	iso_spi_synchronizer_tp_t2p_cfg_RBUS mio_t2p_cfg;
	iso_spi_synchronizer_tp_t2p_ctrl_RBUS mio_t2p_ctrl;

	/* set t2p in parameter */
	mio_t2p_cfg.regValue            = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_T2P_CFG_reg);
	mio_t2p_cfg.in_sync_pol         = Param.in_sync_pol;
	mio_t2p_cfg.in_val_pol          = Param.in_val_pol;
	mio_t2p_cfg.in_clk_pol          = Param.in_clk_pol;

	/* set t2p out parameter */
	mio_t2p_cfg.out_sync_pol        = Param.out_sync_pol;
	mio_t2p_cfg.out_val_pol         = Param.out_val_pol;
	mio_t2p_cfg.out_clk_pol         = Param.out_clk_pol;

	mio_t2p_cfg.out_clk_high        = Param.out_clk_high;
	mio_t2p_cfg.out_clk_low         = Param.out_clk_low;
	WRITE_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_T2P_CFG_reg, mio_t2p_cfg.regValue);

	/* set ts out sync_dur */
	mio_t2p_ctrl.regValue           = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_T2P_CTRL_reg);
	mio_t2p_ctrl.in_data_order      = Param.T2P_ctrl_param.data_order;
	mio_t2p_ctrl.in_data_swap       = Param.T2P_ctrl_param.data_swap;
	mio_t2p_ctrl.in_mode            = Param.T2P_ctrl_param.mode;
	WRITE_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_T2P_CTRL_reg, mio_t2p_ctrl.regValue);

	return TPK_SUCCESS;
}

INT32 TP_MIO_Get_T2PMode(TPK_MIO_T2P_PARAM_T *Param)
{
	iso_spi_synchronizer_tp_t2p_cfg_RBUS mio_t2p_cfg;
	iso_spi_synchronizer_tp_t2p_ctrl_RBUS mio_t2p_ctrl;

	/* Get t2p in parameter */
	mio_t2p_cfg.regValue                = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_T2P_CFG_reg);
	Param->in_sync_pol                  = mio_t2p_cfg.in_sync_pol;
	Param->in_val_pol                   = mio_t2p_cfg.in_val_pol;
	Param->in_clk_pol                   = mio_t2p_cfg.in_clk_pol;

	/* Get t2p out parameter */
	Param->out_sync_pol                 = mio_t2p_cfg.out_sync_pol;
	Param->out_val_pol                  = mio_t2p_cfg.out_val_pol;
	Param->out_clk_pol                  = mio_t2p_cfg.out_clk_pol;

	Param->out_clk_high                 = mio_t2p_cfg.out_clk_high;
	Param->out_clk_low                  = mio_t2p_cfg.out_clk_low;

	mio_t2p_ctrl.regValue               = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_T2P_CTRL_reg);
	Param->T2P_ctrl_param.data_order    = mio_t2p_ctrl.in_data_order;
	Param->T2P_ctrl_param.data_swap     = mio_t2p_ctrl.in_data_swap;
	Param->T2P_ctrl_param.mode          = mio_t2p_ctrl.in_mode;

	return TPK_SUCCESS;
}

/*P2T mode setting*/
/*P2T: parallel to 2bit, mio to mark2 path*/
INT32 TP_MIO_Set_P2TMode_(TPK_MIO_P2T_PARAM_T Param)
{
	iso_spi_synchronizer_tp_p2t_cfg_RBUS mio_p2t_cfg;
	iso_spi_synchronizer_tp_p2t_ctrl_RBUS mio_p2t_ctrl;

	/* set t2p in parameter */
	mio_p2t_cfg.regValue            = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_P2T_CFG_reg);
	mio_p2t_cfg.in_sync_pol         = Param.in_sync_pol;
	mio_p2t_cfg.in_val_pol          = Param.in_val_pol;
	mio_p2t_cfg.in_clk_pol          = Param.in_clk_pol;

	/* set t2p out parameter */
	mio_p2t_cfg.out_sync_pol        = Param.out_sync_pol;
	mio_p2t_cfg.out_val_pol         = Param.out_val_pol;
	mio_p2t_cfg.out_clk_pol         = Param.out_clk_pol;

	mio_p2t_cfg.out_sync_duration   = Param.out_sync_duration;
	WRITE_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_P2T_CFG_reg, mio_p2t_cfg.regValue);

	/* set ts out sync_dur */
	mio_p2t_ctrl.regValue           = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_P2T_CTRL_reg);
	mio_p2t_ctrl.out_data_order     = Param.P2T_ctrl_param.data_order;
	mio_p2t_ctrl.out_data_swap      = Param.P2T_ctrl_param.data_swap;
	mio_p2t_ctrl.out_mode           = Param.P2T_ctrl_param.mode;
	WRITE_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_P2T_CTRL_reg, mio_p2t_ctrl.regValue);

	return TPK_SUCCESS;
}

INT32 TP_MIO_Get_P2TMode(TPK_MIO_P2T_PARAM_T *Param)
{
	iso_spi_synchronizer_tp_p2t_cfg_RBUS mio_p2t_cfg;
	iso_spi_synchronizer_tp_p2t_ctrl_RBUS mio_p2t_ctrl;
	...
	/* Get t2p in parameter */
	mio_p2t_cfg.regValue                = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_P2T_CFG_reg);
	Param->in_sync_pol                  = mio_p2t_cfg.in_sync_pol;
	Param->in_val_pol                   = mio_p2t_cfg.in_val_pol;
	Param->in_clk_pol                   = mio_p2t_cfg.in_clk_pol;

	/* Get t2p out parameter */
	Param->out_sync_pol                 = mio_p2t_cfg.out_sync_pol;
	Param->out_val_pol                  = mio_p2t_cfg.out_val_pol;
	Param->out_clk_pol                  = mio_p2t_cfg.out_clk_pol;

	Param->out_sync_duration            = mio_p2t_cfg.out_sync_duration;

	mio_p2t_ctrl.regValue               = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_P2T_CTRL_reg);
	Param->P2T_ctrl_param.data_order    = mio_p2t_ctrl.out_data_order;
	Param->P2T_ctrl_param.data_swap     = mio_p2t_ctrl.out_data_swap;
	Param->P2T_ctrl_param.mode          = mio_p2t_ctrl.out_mode;
