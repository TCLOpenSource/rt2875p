...
#ifdef CONFIG_COMPAT
        if (copy_from_user((void *) buff, param->key, param->key_len))
#else
...
#ifdef CONFIG_COMPAT
        if (copy_from_user((void *) &buff[MAX_CIPHER_KEY_SIZE], param->iv, param->iv_len))
#else
...
#else
        if (copy_to_user((void __user *) ((unsigned long)out), buff, outlen))
#endif
        {
...
#else
        if (copy_from_user((void *) buff, (void __user *)((unsigned long)in), len))
#endif
        {
...
#else
            if (copy_to_user((void __user *) ((unsigned long)out), buff, ret))
#endif
                return -EFAULT;
        }
    	...
    	case MCP_CIPHER_IOCTL_UPDATE:
      	{
			...
			return -EFAULT;	      
        	return mcp_cipher_do_update(ctx, (unsigned long) data.in, data.len, (unsigned long) data.out);
    	}
    	...
    	case MCP_CIPHER_IOCTL_FREE_BUF:
       	{
        	...
        	return mcp_cipher_do_free_buf((unsigned long) data.in, data.len);
    	}
	default:
    ...
    )
{
    ...
    MCP_CIPHER_PARAM  param;
    MCP_CIPHER_DATA   data;
    COMPAT_MCP_CIPHER_PARAM  compat_param;
    COMPAT_MCP_CIPHER_DATA   compat_data;
    ...
    switch (cmd)
    {
    	case MCP_CIPHER_IOCTL_INIT:
        {
        	if (copy_from_user(&compat_param, (void __user *)(arg), sizeof(COMPAT_MCP_CIPHER_PARAM)))
        	{
            	mcp_warning("[MCP][CIPHER] WARNING, do cipher init failed, copy cipher data from user failed\n");
				return -EFAULT;
        	}

        	param.enc = compat_param.enc;
        	param.type = compat_param.type;
        	param.iv = (unsigned char*)(unsigned long)compat_param.iv;
        	param.iv_len = compat_param.iv_len;
        	param.key = (unsigned char*)(unsigned long)compat_param.key;
        	param.key_len = compat_param.key_len;

        	return mcp_cipher_do_init(ctx, &param);
    	}
    	case MCP_CIPHER_IOCTL_UPDATE:
      	{
        	if (copy_from_user(&compat_data, (void __user *)(arg), sizeof(COMPAT_MCP_CIPHER_DATA)))
        	{
            	mcp_warning("[MCP][CIPHER] WARNING, do cipher update failed, copy cipher data from user failed\n");
				return -EFAULT;
        	}

        	data.in = (unsigned char*)(unsigned long)compat_data.in;
        	data.len = (unsigned long)compat_data.len;
        	data.out = (unsigned char*)(unsigned long)compat_data.out;
		if(data.len > 0x7fffffff)
			return -EFAULT;	   	
        	return mcp_cipher_do_update(ctx, (unsigned int)(unsigned long)data.in, data.len, (unsigned int)(unsigned long)data.out);
    	}
    	case MCP_CIPHER_IOCTL_FINAL:
     	{
        	return mcp_cipher_do_final(ctx, (unsigned int)arg);
    	...
    	case MCP_CIPHER_IOCTL_FREE_BUF:
       	{
        	...
        	data.in = (unsigned char*)(unsigned long)compat_data.in;
        	data.len = (unsigned long)(unsigned long)compat_data.in;
        	return mcp_cipher_do_free_buf((unsigned long)data.in, data.len);
    	}
	default:
		mcp_warning("[MCP] WARNING, unknown command\n");
...
int __init mcp_cipher_init(void)
{
