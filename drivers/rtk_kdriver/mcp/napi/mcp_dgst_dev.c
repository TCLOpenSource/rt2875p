...
)
{
    ...
    switch (cmd) {
        ...
        case MCP_DGST_IOCTL_UPDATE: {
            ...
            if (mcp_is_pli_address((unsigned long)mcpb.head)) {
                mcpb.head = (unsigned char*)pli_to_kernel((unsigned long)mcpb.head);
                mcpb.data = (unsigned char*)pli_to_kernel((unsigned long)mcpb.data);
                mcpb.tail = (unsigned char*)pli_to_kernel((unsigned long)mcpb.tail);
                mcpb.end  = (unsigned char*)pli_to_kernel((unsigned long)mcpb.end);

                ret = MCP_DigestUpdate(ctx, &mcpb);
            } else {
                if(mcpb.len > 0x7fffffff)
		     return -EFAULT;
        ...
        default:
            mcp_warning("[MCP][DGST] WARNING, unknown command - %08x\n", cmd);
            mcp_warning("[MCP][DGST] MCP_DGST_IOCTL_INIT - %lx\n", (unsigned long)MCP_DGST_IOCTL_INIT);
            mcp_warning("[MCP][DGST] MCP_DGST_IOCTL_UPDATE - %lx\n", (unsigned long)MCP_DGST_IOCTL_UPDATE);
            mcp_warning("[MCP][DGST] MCP_DGST_IOCTL_FINAL - %lx\n", (unsigned long)MCP_DGST_IOCTL_FINAL);
            mcp_warning("[MCP][DGST] MCP_DGST_IOCTL_PEEK - %lx\n", (unsigned long)MCP_DGST_IOCTL_PEEK);
            return -EFAULT;
    }
    ...
    switch (cmd) {
        ...
        case MCP_DGST_IOCTL_UPDATE: {
            ...
            if (mcp_is_pli_address(compat_mcpb.head)) {
                mcpb.head = (unsigned char*)pli_to_kernel(compat_mcpb.head);
                mcpb.data = (unsigned char*)pli_to_kernel(compat_mcpb.data);
                mcpb.tail = (unsigned char*)pli_to_kernel(compat_mcpb.tail);
                mcpb.end  = (unsigned char*)pli_to_kernel(compat_mcpb.end);
                mcpb.len = compat_mcpb.len;

                ret = MCP_DigestUpdate(ctx, &mcpb);
            } else {
                if(compat_mcpb.len > 0x7fffffff)
		     return -EFAULT;
...
int __init mcp_dgst_dev_init(void)
{
