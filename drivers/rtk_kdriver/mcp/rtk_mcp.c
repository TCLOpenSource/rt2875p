...
void mcp_dump_all_desc(char * buf)
{
    ...
    if (offset < MCP_DESC_ENTRY_COUNT) {
        snprintf(buf, PAGE_SIZE - strlen(buf), "\n\nXXXXXXXX desc (%p) XXXXXXXXXX\n", &desc_list[offset]);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "flag=%08x\n", (&desc_list[offset])->flags);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "Key0=%08x\n", (&desc_list[offset])->key[0]);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "Key1=%08x\n", (&desc_list[offset])->key[1]);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "Key2=%08x\n", (&desc_list[offset])->key[2]);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "Key3=%08x\n", (&desc_list[offset])->key[3]);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "Key4=%08x\n", (&desc_list[offset])->key[4]);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "Key5=%08x\n", (&desc_list[offset])->key[5]);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "IV0=%08x\n", (&desc_list[offset])->iv[0]);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "IV1=%08x\n", (&desc_list[offset])->iv[1]);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "IV2=%08x\n", (&desc_list[offset])->iv[2]);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "IV3=%08x\n", (&desc_list[offset])->iv[3]);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "di=%08x\n", (&desc_list[offset])->data_in);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "do=%08x\n", (&desc_list[offset])->data_out);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "len=%08x\n", (&desc_list[offset])->length);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "XXXXXXXX desc end XXXXXXXXXX\n\n");
    }
}
/*------------------------------------------------------------------
...
bool is_cp_clken(void)
{
    if(rtd_inl(SYS_REG_SYS_CLKEN1_reg) & SYS_REG_SYS_CLKEN1_clken_cp_mask) {
        if ((GET_MCP_SCTCH() & MCP_SCTCH_REG0_DUMMY0_DECLARE_CLK_OFF)){
            return false;
        }
        return true;
    }
...
static int _mcp_phy_init(void)
{
    ...
    SET_MCP_CTRL1(MCP_DEFAULT_DMA_BUST_LEN_SETTING | MCP_AES_PAD_OFF(1) |                  /* disable AES_H auto padding*/
                  MCP_CSA_ENTROPY(ORIGIONAL_MODE) |    /* setup csa mode*/
                  MCP_ROUND_NO(0) | DMA_WRITE_FINISH(1));                     /* set round number of multi-2 to 1*/
    SET_MCP_SCTCH(GET_MCP_SCTCH() | MCP_SCTCH_REG0_DUMMY0_KCPU_IDLE);
    return 0;
}
...
)
{
    ...
    if (((limit - base) % 64))
        mcp_warning("Set MCP Descriptor Buffer size (%lu) is not multiple of 64\n", limit - base);
...
int _mcp_start_xfer(void)
{
    ...
    while(complete == 0 && try_count-- > 0) {
        ...
        if ((status & MCP_ERROR)) {
            ...
            complete = 1;
            ret = -1;
#ifdef CONFIG_MCP_USING_NEW_POLL_FINISH_MECHANISM
	} else if (!(ctrl & MCP_GO)) {
#else
...
dma_addr_t _mcp_map_single(void *ptr, size_t size, enum dma_data_direction dir)
{
    return dma_map_single(mcp_device, ptr, size, dir);
}

void _mcp_unmap_single(dma_addr_t addr, size_t size, enum dma_data_direction dir)
{
    dma_unmap_single(mcp_device, addr, size, dir);
}

void* mcp_dma_alloc_coherent(unsigned int size, dma_addr_t* p_dma_addr, unsigned long flags)
{
    return dma_alloc_coherent(mcp_device, size, p_dma_addr, flags);

}

void mcp_dma_free_coherent(unsigned int size, unsigned char* p_buffer, dma_addr_t p_dma_addr)
{
    if(p_buffer)
        dma_free_coherent(mcp_device, size, p_buffer, p_dma_addr);
}
...
bool mcp_check_flags(mcp_desc* p_desc, int n_desc)
{
#ifdef CONFIG_MCP_SCPU_OTP_KEY_REMOVED
    int i = 0;
    for(i = 0; i < n_desc; i++) {
        if(MCP_GET_KEY_SEL_FLAG(p_desc[i].flags) == MCP_KEY_SEL_OTP)
	      return false;
    }
    return true;
#else
...
static int do_mcp_command(mcp_desc* p_desc, int n_desc, int is_user_pointer)
{
    ...
    while(n_desc) {
        ...
        if(mcp_check_flags(desc_list, desc_cnt) != true) {
            mutex_unlock(&mcp_mutex);
            return -1;
        }
...
EXPORT_SYMBOL(MCP_AES_H_HASH_INIT);
...
EXPORT_SYMBOL(MCP_AES_H_IV_UPDATE);
...
EXPORT_SYMBOL(MCP_AES_H_PADDING);
...
EXPORT_SYMBOL(MCP_AES_H_Hashing);
...
EXPORT_SYMBOL(MCP_AES_H_DataHash);
...
EXPORT_SYMBOL(MCP_DES_DESC_INIT);
...
EXPORT_SYMBOL(MCP_DES_Decryption);
...
EXPORT_SYMBOL(MCP_DES_Encryption);
...
EXPORT_SYMBOL(MCP_TDES_DESC_INIT);
...
EXPORT_SYMBOL(MCP_TDES_Decryption);
...
EXPORT_SYMBOL(MCP_TDES_Encryption);
...
EXPORT_SYMBOL(MCP_AES_DESC_INIT);

/*======================================================================
 * Func : MCP_AES_Pattern_Decryption
 *
 * Desc : Do AES Pattern Decryption
 *
 ...
 *        p_out            : Data Out
 *        len              : Data Length
 *        crypt_byte_block : Crypto blocks number
 *        skip_byte_block  : Skip blocks number
 * Retn : 0 for success, others failed
 *======================================================================*/
int MCP_AES_Pattern_Decryption(
    unsigned char               Mode,
    unsigned char               Key[16],
    unsigned char               IV[16],
    unsigned char*              p_in,
    unsigned char*              p_out,
    unsigned long               len,
    unsigned char               crypt_byte_block,
    unsigned char               skip_byte_block
)
{
    mcp_desc desc;
    int ret = -1;
    ...
    switch (Mode) {
        case MCP_BCM_CBC:
        case MCP_BCM_CTR:
            ret = MCP_AES_DESC_INIT(&desc, 0, Mode, Key, IV);
            if(ret != 0)
                return ret;

            if (crypt_byte_block && skip_byte_block)
                desc.flags = desc.flags | MCP_PATTERN(MCP_CRYPT_SKIP);
            else if (crypt_byte_block)
                desc.flags = desc.flags | MCP_PATTERN(MCP_FULL_CRYPT);
            else
                desc.flags = desc.flags | MCP_PATTERN(MCP_FULL_SKIP);
            ...
            desc.data_in  = addr1;
            desc.data_out = addr2;
            desc.length   = ((len) & ~0xFF00000F) | ((crypt_byte_block & 0xF) << 28) | ((skip_byte_block & 0xF) << 24);
...
EXPORT_SYMBOL(MCP_AES_Pattern_Decryption);

/*======================================================================
 * Func : MCP_AES_Pattern_Decryption_DoubleDADR
 *
 * Desc : Do AES Pattern Decryption output to 2 addr
 *
 * Parm : mode             : Operation Mode
 *        key              : Key Value
 *        iv               : Initial Vector
 ...
 * Retn : 0 for success, others failed
 *======================================================================*/
int MCP_AES_Pattern_Decryption_DoubleDADR(
    unsigned char               Mode,
    unsigned char               Key[16],
    unsigned char               IV[16],
    unsigned char*              p_in,
    unsigned char*              p_out[2],
    unsigned long               len[2],
    unsigned char               crypt_byte_block,
    unsigned char               skip_byte_block
)
{
    ...
    switch (Mode) {
        ...
        case MCP_BCM_CTR:
            ...
            desc.flags = desc.flags | MCP_DOUBLE_DADR(MCP_TWO_DADR);

            addr1 = _mcp_map_single(p_in, len[0] + len[1], DMA_TO_DEVICE);
            addr2 = _mcp_map_single(p_out[0], len[0], DMA_FROM_DEVICE);
            addr3 = _mcp_map_single(p_out[1], len[1], DMA_FROM_DEVICE);

            desc.data_in  = addr1;
            desc.data_out = addr2;
            desc.data_out2 = addr3;
            desc.data_out_length = len[0];

            desc.length   = ((len[0] + len[1]) & ~0xFF00000F) | ((crypt_byte_block & 0xF) << 28) | ((skip_byte_block & 0xF) << 24);

            ret = mcp_do_command(&desc, 1);

            _mcp_unmap_single(addr1, len[0] + len[1], DMA_TO_DEVICE);
            _mcp_unmap_single(addr2, len[0], DMA_FROM_DEVICE);
            _mcp_unmap_single(addr3, len[1], DMA_FROM_DEVICE);
...
EXPORT_SYMBOL(MCP_AES_Pattern_Decryption_DoubleDADR);
            ...
            while(cur_pos < len) {
                if((len - cur_pos ) > MAX_CP_EN_DE_SEGMENT_SIZE) { // for len of image more than 16M
                    ...
                    if(cur_pos == 0) {
                        ...
                        ret = MCP_AES_DESC_INIT(&desc, 0, Mode, Key, cur_iv_key);
                    }
			if(ret != 0)
				break;
...
EXPORT_SYMBOL(MCP_AES_Decryption);
...
)
{
    ...
    ret = MCP_AES_DESC_INIT(&desc, 1, mode, key, iv);
    if(ret != 0)
        return ret;
...
EXPORT_SYMBOL(MCP_AES_Encryption);
...
EXPORT_SYMBOL(MCP_AES_192_DESC_INIT);
...
)
{
    ...
    key_buff = dma_alloc_coherent(mcp_device, 24, &key_buff_phy, GFP_KERNEL);
    if(!key_buff) {
    ...
    _mcp_unmap_single(addr1, len, DMA_TO_DEVICE);
    _mcp_unmap_single(addr2, len, DMA_FROM_DEVICE);
    dma_free_coherent(mcp_device, 24, key_buff, key_buff_phy);

    return ret;
}
EXPORT_SYMBOL(MCP_AES_192_Decryption);
...
EXPORT_SYMBOL(MCP_AES_192_Encryption);
...
EXPORT_SYMBOL(MCP_AES_256_DESC_INIT);
...
)
{
    ...
    key_buff = dma_alloc_coherent(mcp_device, 32, &key_buff_phy, GFP_KERNEL);
    if(!key_buff) {
    ...
    _mcp_unmap_single(addr1, len, DMA_TO_DEVICE);
    _mcp_unmap_single(addr2, len, DMA_FROM_DEVICE);
    dma_free_coherent(mcp_device, 32, key_buff, key_buff_phy);

    return ret;
}
EXPORT_SYMBOL(MCP_AES_256_Decryption);
...
EXPORT_SYMBOL(MCP_AES_256_Encryption);
...
EXPORT_SYMBOL(MCP_CAM_DESC_INIT);
...
)
{
    ...
    ret = MCP_CAM_DESC_INIT(&desc, 0, Mode, Key, IV);
    if(ret != 0)
        return ret;
...
EXPORT_SYMBOL(MCP_CAM_Decryption);
...
)
{
    ...
    ret = MCP_CAM_DESC_INIT(&desc, 1, mode, key, iv);
    if(ret != 0)
        return ret;
...
EXPORT_SYMBOL(MCP_CAM_Encryption);
...
EXPORT_SYMBOL(MCP_CAM_192_DESC_INIT);
...
EXPORT_SYMBOL(MCP_CAM_192_Decryption);
...
EXPORT_SYMBOL(MCP_CAM_192_Encryption);
...
EXPORT_SYMBOL(MCP_CAM_256_DESC_INIT);
...
EXPORT_SYMBOL(MCP_CAM_256_Decryption);
...
EXPORT_SYMBOL(MCP_CAM_256_Encryption);
...
EXPORT_SYMBOL(MCP_SHA1_PADDING);
...
EXPORT_SYMBOL(MCP_SHA1_Hashing);
...
EXPORT_SYMBOL(MCP_SHA1_DataHash);
...
EXPORT_SYMBOL(MCP_SHA256_PADDING);
 ...
 * Retn : S_OK  /  S_FALSE
 *------------------------------------------------------------------*/
#ifdef CONFIG_MCP_SHA256_USE_SW
...
)
{
	...
	iv[6] = p_desc->iv[0];
	iv[7] = p_desc->iv[1];
	ret = sw_sha256_ce(false, iv, p_in, len, p_out);
...
#endif
EXPORT_SYMBOL(MCP_SHA256_Hashing);
...
EXPORT_SYMBOL(MCP_SHA256_DataHash);

/*------------------------------------------------------------------
 * Func : MCP_SHA256
 *
 * Desc : Compute hash value of Data via 256 bits SHA256
 *
 * Parm : p_data        : Start address of data
 *        data_len      : Length of data
 *        hash          : 32 Bytes (256 bits) hash value of the file
 *        iv              : if iv is NULL, the iv of the descriptor will be reset to default
 *
 * Retn : S_OK  /  S_FALSE
 *------------------------------------------------------------------*/
int MCP_SHA256(unsigned char* p_data, unsigned long data_len, unsigned char hash[SHA256_DIGEST_SIZE])
{  
    int ret = 0;
    mcp_desc desc;
    int pad_size = 0;
    ...
    src = kmalloc(data_len + SHA256_PADDING_MAX_SIZE, GFP_KERNEL);
    if(src == NULL){
        mcp_warning("%s(%d) kmalloc failed...\n", __FUNCTION__, __LINE__);
        ret = -1;
        goto err;
    }
    
    memset(src, 0x0, data_len + SHA256_PADDING_MAX_SIZE);
    memcpy(src, p_data, data_len);
    
    pad_size = MCP_SHA256_PADDING(src + data_len, SHA256_PADDING_MAX_SIZE, data_len);
    if(pad_size < 0) {
        mcp_warning("%s(%d) MCP_SHA256_PADDING failed...\n", __FUNCTION__, __LINE__);
        ret = -1;
        goto err;
    }
    
    ret = MCP_SHA256_Hashing(&desc, src, data_len + pad_size, hash);
...
void do_mcp_throughput_calculate(mcp_desc* desc, dma_addr_t* data_in, dma_addr_t* data_out, unsigned long data_len, int ende, int bcm, char *str)
{
    ...
    mcp_notice("[MCP] %s %s BCM(%d) Throughput %ld Kbytes elapsed %d us, \033[0;32;34m %d ms\n\033[m", 
            str, ende ? "EN" : "DE", bcm, data_len / 1024, (pts_e - pts_s) * (1000000 / 90090), jiffies_to_msecs(jiffies - stime));
}
...
{
#ifdef USE_DVR_MALLOC
    return (size >= PAGE_SIZE) ? (void*) MCP_MALLOC_ACTION(size)
...
int mcp_pm_on(void)
{
    ...
    SET_MCP_SCTCH(GET_MCP_SCTCH() | MCP_SCTCH_REG0_DUMMY0_KCPU_IDLE & (~MCP_SCTCH_REG0_DUMMY0_DECLARE_CLK_OFF));
    /* reset MCP phy */
    _mcp_phy_init();
...
int mcp_pm_off(void)
{
    ...
    SET_MCP_SCTCH(GET_MCP_SCTCH() | MCP_SCTCH_REG0_DUMMY0_DECLARE_CLK_OFF);	
    ...
    mcp_notice("scpu mcp idle\n");

    kcpu_status = GET_MCP_SCTCH();
    if(kcpu_status & MCP_SCTCH_REG0_DUMMY0_KCPU_SUSPENDED) {
        CRT_CLK_OnOff(CP, CLK_OFF, NULL);
    }
    else {
        /* set the dummy bit for kcpu to know scpu suspend ok */
        SET_MCP_SCTCH(kcpu_status | MCP_SCTCH_REG0_DUMMY0_SCPU_SUSPENDED);
    }
    return 0;
}

int mcp_pm_runtime_off(void)
{
    int log_flag = 0;
    ...
    // mcp_debug("MCP (GET_MCP_CTRL() & MCP_IDEL)>>2=0x%x, !((GET_MCP_CTRL() & MCP_IDEL)>>2)=0x%x\n", (GET_MCP_CTRL() & MCP_IDEL)>>2, !((GET_MCP_CTRL() & MCP_IDEL)>>2));
    /* check scpu mcp idle */
    while(!((GET_MCP_CTRL() & MCP_IDEL) >> 2) || !(GET_MCP_SCTCH() & MCP_SCTCH_REG0_DUMMY0_KCPU_IDLE)) {
        if ( log_flag == 0 ) {
            mcp_notice("wait for cp engine idle ...\n");
            if (!((GET_MCP_CTRL() & MCP_IDEL) >> 2))
                mcp_notice("scpu mcp not idle ...\n");
            if ( !(GET_MCP_SCTCH() & MCP_SCTCH_REG0_DUMMY0_KCPU_IDLE))
                mcp_notice("kcpu not ok ...\n");
            log_flag++;
        }
    }
    mcp_notice("kcpu and scpu mcp idle now, start to clk off\n");
    CRT_CLK_OnOff(CP, CLK_OFF, NULL);
...
int __init mcp_core_init(struct device *dev_device)
{
    ...
    if(g_rtk_mcp_irq_num <= 0) {
        mcp_err("fail to get mcp irq_num=%u\n",g_rtk_mcp_irq_num);
        ret = -EINVAL;
        goto  FAIL_REQUEST_IRQ;
    }
    
    /*mcp_warning("get mcp irq_num=%u\n",g_rtk_mcp_irq_num);*/
    
    if (request_irq(g_rtk_mcp_irq_num, mcp_isr, IRQF_SHARED, "MCP", &mcp_wait_queue) < 0) {
        mcp_err("Request mcp irq %d failed\n", g_rtk_mcp_irq_num);
        ret = -ENODEV;
        goto FAIL_REQUEST_IRQ;
    }
#endif
...
#ifdef MCP_INTERRUPT_ENABLE
    free_irq(g_rtk_mcp_irq_num, &mcp_wait_queue);
FAIL_REQUEST_IRQ:
#endif
FAIL_INIT_PHY:
    return ret;
...
/*********************only for chip mac8p and after*************************/
bool mcp_enable_cp_power_saving(bool on)
{
	if (!(rtd_inl(SYS_REG_SYS_CLKEN1_reg) & SYS_REG_SYS_CLKEN1_clken_cp_mask))
		return false;
	if(on)
		rtd_outl(CP_CP_PWM_CTRL_reg, rtd_inl(CP_CP_PWM_CTRL_reg) | CP_CP_PWM_CTRL_en_cp_mask);
	else
		rtd_outl(CP_CP_PWM_CTRL_reg, rtd_inl(CP_CP_PWM_CTRL_reg) & (~CP_CP_PWM_CTRL_en_cp_mask));
	return true;
}
EXPORT_SYMBOL(mcp_enable_cp_power_saving);
