...
#if IS_ENABLED(CONFIG_RTK_KDRV_CW)
...
#else
#define RTK_RCP_LOCK(lock, flags)       ((void)flags)
#define RTK_RCP_UNLOCK(lock, flags)     ((void)flags)
#endif
...
static int RTK_RCP_CRYPTION_BY_MCP(unsigned int rcp_crypt_set, unsigned char *key, unsigned int key_len,
                                   unsigned char *iv, unsigned int iv_len, unsigned char *pDataIn,
                                   unsigned char *pDataOut, unsigned long DataLen)
{
    unsigned char *mcp_tmp_buf = NULL;
    int ret;
    //RCP_ERR("%s %d\n", __func__, __LINE__);

    mcp_tmp_buf = mcp_malloc(DataLen);
    if (mcp_tmp_buf == NULL) {
        return 0;
    }
    if ((unsigned long)pDataIn > MAX_CW_ENTRY) {
        memcpy(mcp_tmp_buf, pDataIn, DataLen);
    }
    else {
        RTK_RCP_GET_CW((unsigned long)pDataIn, mcp_tmp_buf, DataLen);
    }
    switch (rcp_crypt_set) {
        /* DES */
        case RCP_DES_ECB_ENC:
            ret = MCP_DES_ECB_Encryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_DES_ECB_DEC:
            ret = MCP_DES_ECB_Decryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_DES_CBC_ENC:
            ret = MCP_DES_CBC_Encryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_DES_CBC_DEC:
            ret = MCP_DES_CBC_Decryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        /* TDES */
        case RCP_TDES_ECB_ENC:
            ret = MCP_TDES_ECB_Encryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_TDES_ECB_DEC:
            ret = MCP_TDES_ECB_Decryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_TDES_CBC_ENC:
            ret = MCP_TDES_CBC_Encryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_TDES_CBC_DEC:
            ret = MCP_TDES_CBC_Decryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        /* AES 128 */
        case RCP_AES_128_ECB_ENC:
            ret = MCP_AES_ECB_Encryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_128_ECB_DEC:
            ret = MCP_AES_ECB_Decryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_128_CBC_ENC:
            ret = MCP_AES_CBC_Encryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_128_CBC_DEC:
            ret = MCP_AES_CBC_Decryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_128_CTR_ENC:
            ret = MCP_AES_CTR_Encryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_128_CTR_DEC:
            ret = MCP_AES_CTR_Decryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        /* AES 192 */
        case RCP_AES_192_ECB_ENC:
            ret = MCP_AES_192_ECB_Encryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_192_ECB_DEC:
            ret = MCP_AES_192_ECB_Decryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_192_CBC_ENC:
            ret = MCP_AES_192_CBC_Encryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_192_CBC_DEC:
            ret = MCP_AES_192_CBC_Decryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_192_CTR_ENC:
            ret = MCP_AES_192_CTR_Encryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_192_CTR_DEC:
            ret = MCP_AES_192_CTR_Decryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        /* AES 256 */
        case RCP_AES_256_ECB_ENC:
            ret = MCP_AES_256_ECB_Encryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_256_ECB_DEC:
            ret = MCP_AES_256_ECB_Decryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_256_CBC_ENC:
            ret = MCP_AES_256_CBC_Encryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_256_CBC_DEC:
            ret = MCP_AES_256_CBC_Decryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_256_CTR_ENC:
            ret = MCP_AES_256_CTR_Encryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_AES_256_CTR_DEC:
            ret = MCP_AES_256_CTR_Decryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        /* CAM 128 */
        case RCP_CAM_128_ECB_ENC:
            ret = MCP_CAM_ECB_Encryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_128_ECB_DEC:
            ret = MCP_CAM_ECB_Decryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_128_CBC_ENC:
            ret = MCP_CAM_CBC_Encryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_128_CBC_DEC:
            ret = MCP_CAM_CBC_Decryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_128_CTR_ENC:
            ret = MCP_CAM_CTR_Encryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_128_CTR_DEC:
            ret = MCP_CAM_CTR_Decryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        /* CAM 192 */
        case RCP_CAM_192_ECB_ENC:
            ret = MCP_CAM_192_ECB_Encryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_192_ECB_DEC:
            ret = MCP_CAM_192_ECB_Decryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_192_CBC_ENC:
            ret = MCP_CAM_192_CBC_Encryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_192_CBC_DEC:
            ret = MCP_CAM_192_CBC_Decryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_192_CTR_ENC:
            ret = MCP_CAM_192_CTR_Encryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_192_CTR_DEC:
            ret = MCP_CAM_192_CTR_Decryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        /* CAM 256 */
        case RCP_CAM_256_ECB_ENC:
            ret = MCP_CAM_256_ECB_Encryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_256_ECB_DEC:
            ret = MCP_CAM_256_ECB_Decryption(key, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_256_CBC_ENC:
            ret = MCP_CAM_256_CBC_Encryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_256_CBC_DEC:
            ret = MCP_CAM_256_CBC_Decryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_256_CTR_ENC:
            ret = MCP_CAM_256_CTR_Encryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        case RCP_CAM_256_CTR_DEC:
            ret = MCP_CAM_256_CTR_Decryption(key, iv, mcp_tmp_buf, mcp_tmp_buf, DataLen);
            break;
        default:
            ret = -1;
            break;
    ...
    if (ret != 0) {
        mcp_free(mcp_tmp_buf, DataLen);
        return 0;
    }

    if ((unsigned long)pDataOut > MAX_CW_ENTRY) {
        memcpy(pDataOut, mcp_tmp_buf, DataLen);
    }
    else {
        RTK_RCP_SET_CW((unsigned long)pDataOut, mcp_tmp_buf, DataLen);
    }
    mcp_free(mcp_tmp_buf, DataLen);
    return DataLen;
}
...
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_DES_ECB_ENC : RCP_DES_ECB_DEC,
                                      Key, 7, NULL, 0, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_DES_ECB_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_DES_CBC_ENC : RCP_DES_CBC_DEC, NULL, 8,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_DES_CBC_ENC : RCP_DES_CBC_DEC,
                                      Key, 7, IV, 8, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_DES_CBC_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_TDES_ECB_ENC : RCP_TDES_ECB_DEC, Key, 8,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_TDES_ECB_ENC : RCP_TDES_ECB_DEC,
                                      Key, 21, NULL, 0, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_TDES_ECB_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_TDES_CBC_ENC : RCP_TDES_CBC_DEC, Key, 8,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_TDES_CBC_ENC : RCP_TDES_CBC_DEC,
                                      Key, 21, IV, 8, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_TDES_CBC_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_AES_128_ECB_ENC : RCP_AES_128_ECB_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_AES_128_ECB_ENC : RCP_AES_128_ECB_DEC,
                                      Key, 16, NULL, 0, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_AES_ECB_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_AES_128_CBC_ENC : RCP_AES_128_CBC_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_AES_128_CBC_ENC : RCP_AES_128_CBC_DEC,
                                      Key, 16, IV, 16, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_AES_CBC_Cipher);
...
#else
    return RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_AES_128_CTR_ENC : RCP_AES_128_CTR_DEC,
                                   Key, 16, IV, 16, pDataIn, pDataOut, DataLen);
#endif
}
EXPORT_SYMBOL(RTK_RCP_AES_CTR_Cipher_ex);
...
EXPORT_SYMBOL(RTK_RCP_AES_CTR_Cipher);
...
EXPORT_SYMBOL(RTK_RCP_WIDEVINE_AES_CBC_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_AES_192_ECB_ENC : RCP_AES_192_ECB_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_AES_192_ECB_ENC : RCP_AES_192_ECB_DEC,
                                      Key, 24, NULL, 0, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
...
EXPORT_SYMBOL(RTK_RCP_AES_192_ECB_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_AES_192_CBC_ENC : RCP_AES_192_CBC_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_AES_192_CBC_ENC : RCP_AES_192_CBC_DEC,
                                      Key, 24, IV, 16, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_AES_192_CBC_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_AES_192_CTR_ENC : RCP_AES_192_CTR_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_AES_192_CTR_ENC : RCP_AES_192_CTR_DEC,
                                      Key, 24, IV, 16, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_AES_192_CTR_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_AES_256_ECB_ENC : RCP_AES_256_ECB_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_AES_256_ECB_ENC : RCP_AES_256_ECB_DEC,
                                      Key, 32, NULL, 0, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
...
EXPORT_SYMBOL(RTK_RCP_AES_256_ECB_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_AES_256_CBC_ENC : RCP_AES_256_CBC_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_AES_256_CBC_ENC : RCP_AES_256_CBC_DEC,
                                      Key, 32, IV, 16, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_AES_256_CBC_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_AES_256_CTR_ENC : RCP_AES_256_CTR_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_AES_256_CTR_ENC : RCP_AES_256_CTR_DEC,
                                      Key, 32, IV, 16, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_AES_256_CTR_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_CAM_128_ECB_ENC : RCP_CAM_128_ECB_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_CAM_128_ECB_ENC : RCP_CAM_128_ECB_DEC,
                                      Key, 16, NULL, 0, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_CAM_ECB_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_CAM_128_CBC_ENC : RCP_CAM_128_CBC_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_CAM_128_CBC_ENC : RCP_CAM_128_CBC_DEC,
                                      Key, 16, IV, 16, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_CAM_CBC_Cipher);
...
EXPORT_SYMBOL(RTK_RCP_WIDEVINE_CAM_CBC_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_CAM_128_CTR_ENC : RCP_CAM_128_CTR_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_CAM_128_CTR_ENC : RCP_CAM_128_CTR_DEC,
                                      Key, 16, IV, 16, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_CAM_CTR_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_CAM_192_ECB_ENC : RCP_CAM_192_ECB_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_CAM_192_ECB_ENC : RCP_CAM_192_ECB_DEC,
                                      Key, 24, NULL, 0, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_CAM_192_ECB_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_CAM_192_CBC_ENC : RCP_CAM_192_CBC_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_CAM_192_CBC_ENC : RCP_CAM_192_CBC_DEC,
                                      Key, 24, IV, 16, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_CAM_192_CBC_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_CAM_192_CTR_ENC : RCP_CAM_192_CTR_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_CAM_192_CTR_ENC : RCP_CAM_192_CTR_DEC,
                                      Key, 24, IV, 16, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_CAM_192_CTR_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_CAM_256_ECB_ENC : RCP_CAM_256_ECB_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_CAM_256_ECB_ENC : RCP_CAM_256_ECB_DEC,
                                      Key, 32, NULL, 0, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_CAM_256_ECB_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_CAM_256_CBC_ENC : RCP_CAM_256_CBC_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_CAM_256_CBC_ENC : RCP_CAM_256_CBC_DEC,
                                      Key, 32, IV, 16, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_CAM_256_CBC_Cipher);
...
)
{
    ...
    out_len = RTK_RCP_DO_CRYPTION((EnDe) ? RCP_CAM_256_CTR_ENC : RCP_CAM_256_CTR_DEC, NULL, 16,
                                  pDataIn, pDataOut, DataLen);
#else
    out_len = RTK_RCP_CRYPTION_BY_MCP((EnDe) ? RCP_CAM_256_CTR_ENC : RCP_CAM_256_CTR_DEC,
                                      Key, 32, IV, 16, pDataIn, pDataOut, DataLen);
#endif
    RTK_RCP_UNLOCK(rtk_rcp_lock, flags);
    return out_len;
}
EXPORT_SYMBOL(RTK_RCP_CAM_256_CTR_Cipher);
...
static int RTK_RCP_CW_LOGIC_OP(RCP_CW_LOGIC_MODE_TYPE cw_logic_mode, int CwIn1, int CwIn2, int CwOut)
{
    ...
    GET_CP_REG_MOD_DATAOUT(7);// wait complete
    ...
    switch (RCP_MODE(p_desc->flags)) {
        ...
        case RCP_ALGO_CW_LOGIC: {
            switch (RCP_CW_LOGIC(p_desc->flags)) {
                case RCP_CW_LOGIC_XOR:
                    ret = RTK_RCP_CW_XOR((unsigned long)p_desc->key, (unsigned long)p_desc->iv, (unsigned long)p_desc->data_out);
                    break;
                default:
                    ret = 0;
                    break;
        ...
        case RCP_ALGO_CW_OPSEL: {
            switch (RCP_CW_OPSEL(p_desc->flags)) {
                case GET_CW:
                    ret = _RCP_GET_CW((unsigned long)p_desc->data_in, p_desc->data_out, p_desc->len);
                    ret = (ret == 0) ? p_desc->len : 0;
                    break;
                case SET_CW:
                    ret = _RCP_SET_CW((unsigned long)p_desc->data_out, p_desc->data_in, p_desc->len);
                    ret = (ret == 0) ? p_desc->len : 0;
                    break;
            }
            break;
...
EXPORT_SYMBOL(rtk_rcp_do_command);
...
int __init rtk_rcp_module_init(void)
{
    int ret;
