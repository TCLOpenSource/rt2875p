...
int rtk_rcp_do_command_user(RCP_DESC_USER __user * p_desc)
{
    ...
    if(RCP_KEY_FROM(desc_user.flags) == RCP_KEY_FROM_OTP) {
        ...
        unsigned long  tmp = desc_user.key[0];
        desc.key = (unsigned char*)tmp;
    } else {
    ...
    if(RCP_IV_FROM(desc_user.flags) == RCP_IV_FROM_OTP) {
        ...
        unsigned long  tmp = desc_user.iv[0];
        desc.iv = (unsigned char*)tmp;
    } else {
...
int rtk_rcp_do_command_user_compat(COMPAT_RCP_DESC_USER __user * p_desc)
{
    int ret = -1;
    unsigned char *buffer = NULL;
    COMPAT_RCP_DESC_USER desc_user;
    RCP_DESC desc;
    ...
    memset(&desc_user, 0, sizeof(COMPAT_RCP_DESC_USER));
    memset(&desc, 0, sizeof(RCP_DESC));

    if(copy_from_user(&desc_user, (COMPAT_RCP_DESC_USER __user *)p_desc, sizeof(COMPAT_RCP_DESC_USER))) {
        RCP_WARNING("do ioctl command failed - copy desc from user failed\n");
        goto fail;
    }
    
    if(desc_user.len == 0 || desc_user.data_in == 0 || desc_user.data_out == 0)
        goto fail;
    ...
    desc.len = desc_user.len;
    desc.data_out = (unsigned char*)(unsigned long)desc_user.data_out;
    desc.data_in = (unsigned char*)(unsigned long)desc_user.data_in;
    ...
    if(ret == 0) {
        ...
        if(copy_to_user((void __user *)p_desc, &desc_user, sizeof(COMPAT_RCP_DESC_USER))) {
            ret = -1;
            RCP_WARNING("do ioctl command failed - copy data to user failed\n");
            goto fail;
        }
        if(RCP_IV_POST_CRYPT(desc_user.flags) == RCP_IV_NEED_UPDATE
            && RCP_IV_FROM(desc_user.flags) == RCP_IV_FROM_DDR) {
            if(!desc.iv || copy_to_user((void __user *)p_desc->iv, desc.iv , 32)) {
                ret = -1;
                RCP_WARNING("do ioctl command failed - copy data to user failed\n");
                goto fail;
            }
                                     ...
                                     unsigned long arg)
{
    ...
    switch(cmd) {
        case RCP_IOCTL_DO_COMMAND:
            return rtk_rcp_do_command_user_compat((COMPAT_RCP_DESC_USER __user *)arg);
            break;
        default:
            retval = -ENOTTY;
            break;
...
int __init rtk_rcp_dev_init(void)
{
    int ret = 0;
