...
#define DMX_DBG_LOG(fmt, ...) rtd_pr_demux_emerg(fmt, ##__VA_ARGS__)

#define RTKDEMUX_PROC_DIR                        "rtkdemux"
#define RTKDEMUX_PROC_ENTRY                      "dbg"
#define RTKDEMUX_STATUS_ENTRY                    "status"
...
enum {
	...
	DEMUX_DBG_CMD_MONITOR_IO = 21,
	DEMUX_DBG_CMD_ESBUF_MONITOR = 22,
...
static const char *rtkdemux_cmd_str[] = {
	...
	"monitorio=",		/* DEMUX_DBG_CMD_MONITOR_IO */
	"esbufmonitor=",	/* DEMUX_DBG_CMD_ESBUF_MONITOR */
};
...
unsigned char rtkdemux_dbg_monitorio_en = 1;
unsigned char rtkdemux_dbg_printclock_en;
unsigned char rtkdemux_dbg_esbufmonitor_en = 1;
...
unsigned long rtkdemux_dbg_monitorio_read_count[DEMUX_CH_NUM];
unsigned long rtkdemux_dbg_monitorio_deliver_count[DEMUX_CH_NUM][NUMBER_OF_PINS];

void rtkdemux_dbg_reset_monitorio_counter(void)
{
    int i, j;
    for (i = 0; i < DEMUX_CH_NUM; i++)
    	rtkdemux_dbg_monitorio_read_count[i] = 0;
    	
    for (i = 0; i < DEMUX_CH_NUM; i++)
    	for (j = 0; j < NUMBER_OF_PINS; j++)
    		rtkdemux_dbg_monitorio_deliver_count[i][j] = 0;
}

void rtkdemux_dbg_monitorio_counter_init(void)
{    
    static int monitor_count_init = 0;
    if (monitor_count_init==0) {
        rtkdemux_dbg_reset_monitorio_counter();                    
        monitor_count_init=1;
    }    
}
...
void rtkdemux_dbg_printio_read_size_calc(void *ch_pointer, unsigned long read_size)
{
	...
	if (ch_idx >= 0) {
		...
		if (rtkdemux_dbg_monitorio_en) {
		    rtkdemux_dbg_monitorio_counter_init();    
		    rtkdemux_dbg_monitorio_read_count[ch_idx] += read_size;
		}
	}
}
...
void rtkdemux_dbg_printio_deliver_size_calc(void *ch_pointer, int pin_idx, unsigned long deliver_size)
{
	...
	if (ch_idx >= 0 && pin_idx >= 0 && pin_idx < NUMBER_OF_PINS) {
		...
		if (rtkdemux_dbg_monitorio_en) {
		    rtkdemux_dbg_monitorio_counter_init();    
		    rtkdemux_dbg_monitorio_deliver_count[ch_idx][pin_idx] += deliver_size;
		}
	}
}
	...
	bool is_reverse)
{
	...
	if (is_reverse) {
		phyWp = DEMUX_reverseInteger(*wp);
		phyRp = DEMUX_reverseInteger(*rp);
	} else {
		phyWp = *wp;
		phyRp = *rp;
	}
	...
	if (phyRp > phyWp)
		...
		return (upper - lower) - (phyWp - phyRp);
}
	...
	for (i = 0; i < demux_device->chNum; i++) {
		...
		if (demux_device->isInit && pCh->isInit) {
			...
			if (i == 0)
				...
				Tp_Get_Buffer(TP3_BUFFER_MASS_DATA, &tp_ring_buffer);
			_tp_unlock();
			...
			pes_used_num = 0;
/*
			for (j = 0 ; j < PES_FILTER_BUFFER_NUM ; j++) {
				if (pCh->pesMap[j].occupied) {
					pes_used_size += PES_FILTER_BUFFER_EACH_SIZE;
					pes_used_num++;
				}
			}
			DMX_DBG_LOG(DGB_COLOR_GREEN"    PES: %uB allocated, %uB size, %uB(%u/%d) used, %uB free\n"DGB_COLOR_NONE,
				pCh->pesFilterBuffer.allocSize,
				pCh->pesFilterBuffer.size,
				pes_used_size,
				pes_used_num,
				PES_FILTER_BUFFER_NUM,
				pCh->pesFilterBuffer.size - pes_used_size);
*/
			...
			for (j = 0; j < MAX_PID_FILTER_CB_NUM; j++) {
				...
				if(pPes->occupied) {
					pes_used_num++;
					pes_used_size += PES_FILTER_RING_BUFFER_SIZE;
				}
			...
			DMX_DBG_LOG(DGB_COLOR_GREEN"    PES: %uB allocated, %uB size, %uB(%u/%d) used, %uB free"DGB_COLOR_NONE_N,
				pCh->pesFilterBuffer.allocSize,
				pCh->pesFilterBuffer.size,
				pes_used_size,
				pes_used_num,
				MAX_PID_FILTER_CB_NUM,
				pCh->pesFilterBuffer.size - pes_used_size);

			total_allocated += pCh->pesFilterBuffer.allocSize;
#endif
			pin_info = NULL;
			pin = 0;
			/* video */
			for (j = 0 ; j < demux_device->numberOfVD ; j++) {
				...
				DMX_DBG_LOG(DGB_COLOR_GREEN"    Video%d BS: %uB allocated, %uB size, %uB used, %uB free"DGB_COLOR_NONE_N,
					j, demux_device->bsVideoBuf[j].allocSize, demux_device->bsVideoBuf[j].size, demux_device->bsVideoBuf[j].size - free_size, free_size);
				total_allocated += demux_device->bsVideoBuf[j].allocSize;
			#else
				DMX_DBG_LOG(DGB_COLOR_GREEN"    Video%d BS: %uB allocated, %uB size, %uB used, %uB free"DGB_COLOR_NONE_N,
					j, pCh->bsVideoBuf[j].allocSize, pCh->bsVideoBuf[j].size, pCh->bsVideoBuf[j].size - free_size, free_size);
				total_allocated += pCh->bsVideoBuf[j].allocSize;
			#endif
				...
				DMX_DBG_LOG(DGB_COLOR_GREEN"    Video%d IB: %uB allocated, %uB size, %uB used, %uB free"DGB_COLOR_NONE_N,
					j, demux_device->ibVideoBuf[j].allocSize, demux_device->ibVideoBuf[j].size, demux_device->ibVideoBuf[j].size - free_size, free_size);
				total_allocated += demux_device->ibVideoBuf[j].allocSize;
			#else
				DMX_DBG_LOG(DGB_COLOR_GREEN"    Video%d IB: %uB allocated, %uB size, %uB used, %uB free"DGB_COLOR_NONE_N,
					j, pCh->ibVideoBuf[j].allocSize, pCh->ibVideoBuf[j].size, pCh->ibVideoBuf[j].size - free_size, free_size);
				total_allocated += pCh->ibVideoBuf[j].allocSize;
			#endif
			...
			for (j = 0 ; j < demux_device->numberOfAD ; j++) {
				...
				DMX_DBG_LOG(DGB_COLOR_GREEN"    Audio%d BS: %uB Index:%d,status:%d, allocated, %uB size, %uB used, %uB free"DGB_COLOR_NONE_N,
					j, pCh->pinInfo[pin].pBuffer->bsBuffer.allocSize, pCh->pinInfo[pin].pBuffer->bufferIndex, pCh->pinInfo[pin].pBuffer->status,  pCh->pinInfo[pin].pBuffer->bsBuffer.size, pCh->pinInfo[pin].pBuffer->bsBuffer.size - free_size, free_size);
				total_allocated += pCh->pinInfo[pin].pBuffer->bsBuffer.allocSize;
			#else
				DMX_DBG_LOG(DGB_COLOR_GREEN"    Audio%d BS: %uB allocated, %uB size, %uB used, %uB free"DGB_COLOR_NONE_N,
					j, pCh->bsAudioBuf[j].allocSize, pCh->bsAudioBuf[j].size, pCh->bsAudioBuf[j].size - free_size, free_size);
				total_allocated += pCh->bsAudioBuf[j].allocSize;
			#endif
				/* inband buffer */
				pin_info = &pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_IB];
				free_size = rtkdemux_dbg_get_free_size(
				...
				DMX_DBG_LOG(DGB_COLOR_GREEN"    Audio%d IB: %uB allocated, %uB size, %uB used, %uB free"DGB_COLOR_NONE_N,
					j, pCh->pinInfo[pin].pBuffer->ibBuffer.allocSize, pCh->pinInfo[pin].pBuffer->ibBuffer.size, pCh->pinInfo[pin].pBuffer->ibBuffer.size - free_size, free_size);
				total_allocated += pCh->pinInfo[pin].pBuffer->ibBuffer.allocSize;
			#else
				DMX_DBG_LOG(DGB_COLOR_GREEN"    Audio%d IB: %uB allocated, %uB size, %uB used, %uB free"DGB_COLOR_NONE_N,
					j, pCh->ibAudioBuf[j].allocSize, pCh->ibAudioBuf[j].size, pCh->ibAudioBuf[j].size - free_size, free_size);
				total_allocated += pCh->ibAudioBuf[j].allocSize;
			#endif
			}
...
static const char *rtkdemux_get_pidtype_string(int pid_type)
{
	switch (pid_type)
	{
		case DEMUX_PID_VIDEO:
			return "DEMUX_PID_VIDEO";
		case DEMUX_PID_AUDIO:
		/*case DEMUX_PID_AUDIO_DESCRIPTION:*/
			return "DEMUX_PID_AUDIO";
		case DEMUX_PID_PCR:
			return "DEMUX_PID_PCR";
		case DEMUX_PID_TELETEXT:
			return "DEMUX_PID_TELETEXT";
		case DEMUX_PID_SUBTITLE:
		/*case DEMUX_PID_DCC:*/
			return "DEMUX_PID_SUBTITLE";
		case DEMUX_PID_PSI:
			return "DEMUX_PID_PSI";
		case DEMUX_PID_GP:
			return "DEMUX_PID_GP";
		default:
			return "DEMUX_PID_UNDEFINED";
	}
}
...
static char *rtkdemux_get_pesdest_string(int pid_dest)
{
	switch (pid_dest)
	{
		case DEMUX_DEST_NONE:
			return "DEMUX_DEST_NONE";
		case DEMUX_DEST_ADEC0:
			return "DEMUX_DEST_ADEC0";
		case DEMUX_DEST_ADEC1:
			return "DEMUX_DEST_ADEC1";
		case DEMUX_DEST_VDEC0:
			return "DEMUX_DEST_VDEC0";
		case DEMUX_DEST_VDEC1:
			return "DEMUX_DEST_VDEC1";
		case DEMUX_DEST_STC0:
			return "DEMUX_DEST_STC0";
		case DEMUX_DEST_STC1:
			return "DEMUX_DEST_STC1";
		case DEMUX_DEST_BUFFER:
			return "DEMUX_DEST_BUFFER";
		case DEMUX_DEST_RECORD_A:
			return "DEMUX_DEST_RECORD_A";
		case DEMUX_DEST_RECORD_B:
			return "DEMUX_DEST_RECORD_B";
		case DEMUX_DEST_TSO:
			return "DEMUX_DEST_TSO";
		default:
			return "DEMUX_DEST_UNDEFINED";
	}
}
		...
		for (i = 0 ; i < demux_device->chNum ; i++) {
			...
			if (pCh) {
				...
				for (j = 0 ; j < MAX_SECTION_FILTER_NUM ; j++) {
					...
					DMX_DBG_LOG(DGB_COLOR_GREEN"    SEC_FILTER: pRd=0x%lx,pWr=0x%lx,pBase=0x%lx,pLimit=0x%lx\n"DGB_COLOR_NONE,
...
static inline void rtkdemux_dbg_EXECUTE(const int cmd_index, char **cmd_pointer)
{
	...
	case DEMUX_DBG_CMD_PRINT_IO:
		...
		break;		
    case DEMUX_DBG_CMD_MONITOR_IO:
        rtkdemux_dbg_parse_value(*cmd_pointer, &parsed_data);
		rtkdemux_dbg_monitorio_en = (unsigned char)parsed_data;
		DMX_DBG_LOG("rtkdemux_dbg_monitorio_en=%d\n",
			rtkdemux_dbg_monitorio_en);			        
	    if (rtkdemux_dbg_monitorio_en == 0)
	        rtkdemux_dbg_reset_monitorio_counter();		
        break;        		
	...
	case DEMUX_DBG_CMD_ESBUF_MONITOR:
	{
		rtkdemux_dbg_parse_value(*cmd_pointer, &parsed_data);
		rtkdemux_dbg_esbufmonitor_en = (unsigned char)parsed_data;
		DMX_DBG_LOG("rtkdemux_dbg_esbufmonitor_en=%d\n", rtkdemux_dbg_esbufmonitor_en);
		break;
	}
	default:
	...
	loff_t *ppos)
{
	...
	if (console_dmx_info[0] == 0)
		console_dmx_info[0] = 1;
...
int rtkdemux_print_demux_status_str(char* buff, int buff_size)
{
	int len = 0;
	int i, j;
	int n;
	...
	if (demux_device) {	    
		...
		for (i = 0 ; i < demux_device->chNum ; i++) {
			demux_channel *pCh = &demux_device->pChArray[i];			
			if (pCh) {			    
				unsigned long * const deliver_counters = rtkdemux_dbg_monitorio_deliver_count[i];			    			    
	    
				if (rtkdemux_dbg_monitorio_en)
					n = snprintf(buff, buff_size, "------------------------------------------------------\nCH%d: streaming=%d, RX Count :%lu\n", 
						i, pCh->startStreaming, rtkdemux_dbg_monitorio_read_count[i]);                		    
				else 
					n = snprintf(buff, buff_size, "------------------------------------------------------\nCH%d: streaming=%d\n", 
				...
				for (j = 0 ; j < pCh->pidFilterCount ; j++) {
					...
					if (target >= 0 && target < DEMUX_NUM_OF_TARGETS) {
						if (rtkdemux_dbg_monitorio_en)
    							n = snprintf(buff, buff_size, "(%d/%d): pid=%d(0x%x),type=%s(%d),dest=%s(%d),target_pin=%s (CNT:%lu)\n",
    								j+1,
	    							pCh->pidFilterCount,
    								pCh->pidFilterList[j].pid, pCh->pidFilterList[j].pid,
    								rtkdemux_get_pidtype_string(pCh->pidFilterList[j].pidType), pCh->pidFilterList[j].pidType,
    								rtkdemux_get_pesdest_string(pCh->pidFilterList[j].pesDest), pCh->pidFilterList[j].pesDest,
    								rtkdemux_outputNames[pCh->demuxTargetInfo[pCh->demuxTargetTable[pCh->pidFilterList[j].pid]].pinIndex],
    								deliver_counters[pCh->demuxTargetInfo[pCh->demuxTargetTable[pCh->pidFilterList[j].pid]].pinIndex]);
						else
							n = snprintf(buff, buff_size, "(%d/%d): pid=%d(0x%x),type=%s(%d),dest=%s(%d),target_pin=%s\n",
    								j+1,
	    							pCh->pidFilterCount,
    								pCh->pidFilterList[j].pid, pCh->pidFilterList[j].pid,
    								rtkdemux_get_pidtype_string(pCh->pidFilterList[j].pidType), pCh->pidFilterList[j].pidType,
						...
						n = snprintf(buff, buff_size, "(%d/%d): pid=%d(0x%x),type=%s(%d),dest=%s(%d),target_pin=NOT_DEMUX\n",
							j+1,
							pCh->pidFilterCount,
							pCh->pidFilterList[j].pid, pCh->pidFilterList[j].pid,
							rtkdemux_get_pidtype_string(pCh->pidFilterList[j].pidType), pCh->pidFilterList[j].pidType,
	...
	{
	    dmx_info(KERN_WARNING "missing dmx device\n");
	}
	return len;	
}
...
static int rtkdemux_status_read(
	struct file *file,
	char __user *buf,
	size_t count,
	loff_t *ppos)
{
    ...
    proc_out_size = rtkdemux_print_demux_status_str(proc_out_buff, sizeof(proc_out_buff));
    
    dmx_info(KERN_WARNING "rtkdemux_status_read = %d\n", proc_out_size);   
    
    if (proc_out_size < 0)
    {
        DMX_DBG_LOG("rtkdemux_status_read failed - get demux target stream failed\n");
        return 0;
    }
...
static const struct file_operations rtkdemux_status_fops = {
	.owner = THIS_MODULE,		
	.read = rtkdemux_status_read,
};

struct proc_dir_entry *rtkdemux_proc_dir;
struct proc_dir_entry *rtkdemux_proc_entry;
struct proc_dir_entry *rtkdemux_status_entry = NULL;
...
bool demux_init_debug_proc(void)
{
	...
	if (rtkdemux_proc_dir == NULL && rtkdemux_proc_entry == NULL) {
		...
		if (rtkdemux_proc_dir != NULL) {
			...
			rtkdemux_status_entry =
				proc_create(RTKDEMUX_STATUS_ENTRY, 0666,
					rtkdemux_proc_dir, &rtkdemux_status_fops);
...
void demux_uninit_debug_proc(void)
{
	...
	if (rtkdemux_status_entry) {
		proc_remove(rtkdemux_status_entry);
		rtkdemux_status_entry = NULL;
	}
