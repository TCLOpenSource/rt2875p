...
long DEMUX_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case DEMUX_IOC_GET_PID_FILTER_INDEX:
		{
			DEMUX_SET_PID_T data;
			if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_SET_PID_T))) {
				ret = -EFAULT;
	...
	case DEMUX_IOC_SWITCH_DESCRAMBLER:
		{
	...
	case DEMUX_IOC_TPP_MTP_GET_BUF:
	{
		DEMUX_TPP_MTP_BUF_INFO_T data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_TPP_MTP_BUF_INFO_T))
				||  IOCTL_TPPMTPGetBuffer(&data) < 0
				||  copy_to_user((void __user *)arg, (void *)&data, sizeof(DEMUX_TPP_MTP_BUF_INFO_T))) {
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_TPP_MTP_GET_BUF failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = 0;
		break;
	}
	case DEMUX_IOC_FREE_TPP_MTP:
	{
		DEMUX_CHANNEL_T data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_CHANNEL_T))) {
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_FREE_TPP_MTP failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = IOCTL_FreeTPPMTPBuf(&data);
		break;
	}
	case DEMUX_IOC_TPP_MTP_GET_WRITE_STATUS:
	{
		DEMUX_MTP_WRITE_STATUS_T data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_MTP_WRITE_STATUS_T))
				||  IOCTL_TPPMTPGetWriteStatus(&data) < 0
				||  copy_to_user((void __user *)arg, (void *)&data, sizeof(DEMUX_MTP_WRITE_STATUS_T))) {
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_TPP_MTP_GET_WRITE_STATUS failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = 0;
		break;
	}
	case DEMUX_IOC_GET_TP_BUF_STATUS:
	{
		...
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_TP_BUF_STATUS_T))
			...
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_GET_TP_BUF_STATUS failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = 0;
		break;
	}
	case DEMUX_IOC_GET_TPP_BUF_STATUS:
	{
		DEMUX_TPP_BUF_STATUS_T data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_TPP_BUF_STATUS_T))
				||  IOCTL_GetTPPBufStatus(&data) < 0
				||  copy_to_user((void __user *)arg, (void *)&data, sizeof(DEMUX_TPP_BUF_STATUS_T))) {
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_GET_TPP_MTP_BUF_STATUS failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = 0;
		break;
	}
	case DEMUX_IOC_SET_TPP_MTP_STREAMING_STATUS:
	{
		DEMUX_MTP_STREAMING_STATUS_T data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_MTP_STREAMING_STATUS_T))) {
			ret = -EFAULT;
			...
			ret = IOCTL_SetTPPMtpStreaming(&data);
		break;
	}
	case DEMUX_IOC_TPP_MTP_WRITE_BUF:
	{
		DEMUX_MTP_WRITE_T data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_MTP_WRITE_T))) {
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_TPP_MTP_WRITE_BUF failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = IOCTL_TPPMTPWriteBuffer(&data);
		break;
	}
	case DEMUX_IOC_GET_CI_DL_BUF_STATUS:
	{
		DEMUX_CI_DL_BUF_STATUS_T data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_CI_DL_BUF_STATUS_T))
				||  IOCTL_GetCIDLBufferStatus(&data) < 0
				||  copy_to_user((void __user *)arg, (void *)&data, sizeof(DEMUX_CI_DL_BUF_STATUS_T))) {
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_TPP_MTP_GET_BUF failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = 0;
	...
	case DEMUX_IOC_SET_CI_DL_BUF_RP:
	{
		DEMUX_CI_DL_BUF_RP_T data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_CI_DL_BUF_RP_T))) {
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_TPP_MTP_WRITE_BUF failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = IOCTL_SetCIDLBufferRP(&data);
		break;
	}
	case DEMUX_IOC_SET_TP_PID_FLT_EN:
	{
		DEMUX_TP_PID_ENABLE_T data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_TP_PID_ENABLE_T))) {
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_TPP_MTP_WRITE_BUF failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = IOCTL_SetTpPidEnable(&data);
		break;
	}
	case DEMUX_IOC_SET_TPP_PID_FLT_EN:
	{
		DEMUX_TPP_PID_ENABLE_T data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_TPP_PID_ENABLE_T))) {
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_TPP_MTP_WRITE_BUF failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = IOCTL_SetTppPidEnable(&data);
		break;
	}
	case DEMUX_IOC_SET_TP_SYNC_CP14:
	{
		DEMUX_TP_SYNC_CP14_T data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_TP_SYNC_CP14_T))) {
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_SET_TP_SYNC_CP14 failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = IOCTL_SetTpSyncCP14(&data);
		break;
	}
	case DEMUX_IOC_SET_BYPASS_PID_MULTI:
	{
		DEMUX_PID_TABLE_T data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_PID_TABLE_T))) {
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_SET_BYPASS_PID_MULTI failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = IOCTL_SetBypassPIDs(&data);
		break;
	}
	case DEMUX_IOC_REMOVE_BYPASS_PID_MULTI:
	{
		DEMUX_PID_TABLE_T data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_PID_TABLE_T))) {
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_REMOVE_BYPASS_PID_MULTI failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = IOCTL_RemoveBypassPIDs(&data);
		break;
	}
	case DEMUX_IOC_GET_BYPASS_PID_MULTI:
	{
		DEMUX_PID_TABLE_T data;
		if(copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_PID_TABLE_T))
				||  IOCTL_GetBypassPIDs(&data) < 0
				||  copy_to_user((void __user *)arg, (void *)&data, sizeof(DEMUX_PID_TABLE_T))){
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_GET_BYPASS_PID_MULTI failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = 0;
		break;
	}
	case DEMUX_IOC_TPP_MTP_FLUSH:
	{
		DEMUX_MTP_FLUSH_T data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_MTP_FLUSH_T)))	{
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_TPP_MTP_FLUSH failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = IOCTL_TPPMTPFlush(&data);
		break;
	}
	case DEMUX_IOC_TPP_FLUSH:
	{
		UINT8 data;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(UINT8)))	{
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_TPP_FLUSH failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = IOCTL_TPPFlush(&data);
		break;
	}
	case DEMUX_IOC_GET_CAM_BITRATE:
	{
		DEMUX_CAM_BITRATE_T data;
		if (IOCTL_GetCAMBitrate(&data) < 0
				||  copy_to_user((void __user *)arg, (void *)&data, sizeof(DEMUX_CAM_BITRATE_T))) {
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_GET_CAM_BITRATE failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = 0;
		break;
	}
	case DEMUX_IOC_TPP_STREAM_CONTROL:
	{
		DEMUX_TP_STREAM_CONTROL_T data;

		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_TP_STREAM_CONTROL_T)))	{
			ret = -EFAULT;
			dmx_err("RTKDEMUX[%d]: ioctl DEMUX_IOC_TPP_STREAM_CONTROL failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else
			ret = IOCTL_SetTPPStreamControl(&data);
		break;
	}
	default:
...
static char *DEMUX_devnode(struct device *dev, umode_t *mode)
{
	return NULL;
}
...
int DEMUX_init_module(void)
{
	...
	if (result < 0) {
		dmx_info("demux: can not get chrdev region...\n");
		return result;
	}

	demux_class = class_create(THIS_MODULE, DRV_NAME);
	if (IS_ERR(demux_class)) {
		dmx_info("demux: can not create class...\n");
	...
	if (platform_driver_register(&demux_device_driver) != 0) {
		dmx_info("demux: can not register platform driver...\n");
		result = -EINVAL;
	...
	if (cdev_add(&demux->cdev, dev, 1) < 0) {
		dmx_info("demux: can not add character device...\n");
		goto fail_cdev_init;
	}
	...
	if(IOCTL_InitModule() ==-1)
		dmx_info("demux: Demux init fail!! \n");
	return 0;
