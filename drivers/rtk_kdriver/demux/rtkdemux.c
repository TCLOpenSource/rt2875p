...
/* Mac3 */
#define MIS_CLK90K_TM_LO_reg    TIMER_SCPU_CLK90K_LO_reg
#define MIS_CLK90K_TM_HI_reg    TIMER_SCPU_CLK90K_HI_reg
...
int DEMUX_DeliverData(demux_channel *pCh, NAVDEMUXOUT *pDemuxOut)
{
	...
	for (i = 0 ; i < NUMBER_OF_PINS ; i++) {
		...
		if (pPinInfo->pRBH != NULL && pPinIBInfo->pRBH != NULL) { /* for connected pin only */
			...
			/*                    dmx_info("RTKDemux: ring buffer(%s), WP = %lxh, RP = %lxh, size = %ld, req-size = %d\n",
...
#define IS_AUDIO_PIN(_pin_) ((_pin_) == AUDIO_PIN || (_pin_) == AUDIO2_PIN)
			...
			if(pCh->startStreaming && IS_VIDEO_PIN(pNavBuffer->data.pinIndex)){
				pCh->video_deliver_size = pCh->video_deliver_size +(unsigned long)pNavBuffer->data.payloadSize;
			}
			if(pCh->startStreaming && IS_AUDIO_PIN(pNavBuffer->data.pinIndex)){
				pCh->audio_deliver_size = pCh->audio_deliver_size + (unsigned long)pNavBuffer->data.payloadSize;
			}
			rtkdemux_dbg_dump_bitstream(pCh->tunerHandle, pNavBuffer->data.pinIndex, pNavBuffer->data.payloadData, pNavBuffer->data.payloadSize);
		}
	}
...
/*			if (!got) {
				for (j = 0; j < pCh->bypassPidFilterCount; j++) {
					if (pCh->bypassPidFilterList[j] == pCh->pidActiveList[i].PID) {
						got = true;
						break;
					}
				}
			...
			if (!got) {
				...
				/* Reset active PID & dest */
				if(pCh->active_video_pid == pCh->pidActiveList[i].PID){
					Demux_Flush(pCh,GetTargetIdxFromType(DEMUX_PID_VIDEO, pCh->active_video_dest) );
					dmx_mask_print(DEMUX_PVR_ES_MONITOR_DEBUG, "%s_%d ch:%d, target:%d, pid:0x%x, dest:%d flushed\n" ,
						__func__, __LINE__, pCh->tunerHandle, GetTargetIdxFromType(DEMUX_PID_VIDEO, pCh->active_video_dest), pCh->active_video_pid, pCh->active_video_dest);
					pCh->active_video_pid = 0x1fff;
					pCh->active_video_dest= DEMUX_DEST_NONE;
					pCh->video_deliver_size = 0;
				}
				if(pCh->active_audio_pid == pCh->pidActiveList[i].PID){
					Demux_Flush(pCh,GetTargetIdxFromType(DEMUX_PID_AUDIO, pCh->active_audio_dest) );
					dmx_mask_print(DEMUX_PVR_ES_MONITOR_DEBUG, "%s_%d ch:%d, target:%d, pid:0x%x, dest:%d flushed\n" ,
						__func__, __LINE__, pCh->tunerHandle, GetTargetIdxFromType(DEMUX_PID_AUDIO, pCh->active_audio_dest), pCh->active_audio_pid,  pCh->active_audio_dest);
					pCh->active_audio_pid = 0x1fff;
					pCh->active_audio_dest= DEMUX_DEST_NONE;
					pCh->audio_deliver_size = 0;
				}
		...
		for (i = 0 ; i < pCh->pidFilterCount ; i++) {
			...
			if(pCh->pidFilterList[i].pidType == DEMUX_PID_VIDEO
				&&(pCh->pidFilterList[i].pesDest == DEMUX_DEST_VDEC0 || pCh->pidFilterList[i].pesDest == DEMUX_DEST_VDEC1)){
				pCh->active_video_pid = pCh->pidFilterList[i].pid;
				pCh->active_video_dest= pCh->pidFilterList[i].pesDest;
			}
			if(pCh->pidFilterList[i].pidType == DEMUX_PID_AUDIO
				&&(pCh->pidFilterList[i].pesDest == DEMUX_DEST_ADEC0 || pCh->pidFilterList[i].pesDest == DEMUX_DEST_ADEC1)){
				pCh->active_audio_pid = pCh->pidFilterList[i].pid;
				pCh->active_audio_dest= pCh->pidFilterList[i].pesDest;
			}
		}
		...
		for (i = 0; i < pCh->bypassPidFilterCount; i++) {
			for (j = 0; j < count; j++) {
				if (pCh->bypassPidFilterList[i] == add_pidEntry[j].PID)
					break;
			}
			if (j == count) {
				add_pidEntry[count].valid        = 1 ;
				add_pidEntry[count].DescrambleEn = 0 ;
				add_pidEntry[count].KeySel       = 0 ;
				add_pidEntry[count].PID          = pCh->bypassPidFilterList[i];
				count++ ;
				//dmx_mask_print(DEMUX_PVR_ES_MONITOR_DEBUG, "ch:%d, bypass Pid:0x%x\n" , pCh->tunerHandle, pCh->bypassPidFilterList[i]);
		...
		/*	if ( (pCh->tp_src== MTP)&&(pCh->activeTarget[i].pid == -1 )){
				//removed
				Demux_Flush(pCh, i);
				//dmx_mask_print(DEMUX_PVR_ES_MONITOR_DEBUG, "ch:%d, target:%d flushed\n" , pCh->tunerHandle, i);
			}*/
		}
...
int DEMUX_EVENT_AudioNewFormat(demux_channel *pCh, int pin, DEMUX_AUDIO_DEC_T audioFormat, long privateInfo[8])
{
	...
	dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG|DEMUX_PVR_ES_MONITOR_DEBUG,"%s,%d, ch:%d, pin:%d, audioFormat:%d\n", __func__, __LINE__, pCh->tunerHandle, pin, audioFormat);
	...
	if (pCh->demuxTargetInfo[target].contiCounter == -1)
		...
		if (pCh->demuxTargetInfo[target].contiCounter == count) { /* duplicated packet*/
			...
			if (pCh->demuxTargetInfo[target].contiCounter != count) { /* continuity_counter is not continuous */
				DMX_WARNING_CC(("[CH%d]ParsePacket: [target:%d] [target_pid:%d]detect discontinuous continuity-counter (%d != %d)\n", 
					(int)pCh->tunerHandle, target, pCh->activeTarget[target].pid, pCh->demuxTargetInfo[target].contiCounter, count));
...
void Demux_Flush(demux_channel *pCh, int target)
{
	...
	for ( i=0;i<pCh->demuxOutReserved.numBuffers; i++)
	{
		if (pCh->demuxOutReserved.pNavBuffers[i].data.pinIndex == target){
			dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG, "%s_%d, flush dirty data of pin %d ,pts:%lld\n", __func__, __LINE__, target, pCh->demuxOutReserved.pNavBuffers[i].data.pts);
			pCh->demuxOutReserved.pNavBuffers[i].type = NAVBUF_NONE;
			pCh->demuxOutReserved.pNavBuffers[i].data.pinIndex = DEMUX_TARGET_DROP;
		}
	}
	if(pCh->tp_src == MTP) 
		if (DEMUX_IS_AUDIO_TARGET(target)){
			REFCLOCK * const pRefClock = DEMUX_GetReferenceClock(pCh);
			dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG, "%s_%d, reset pin:%d,  \n", __func__, __LINE__, target);
			...
			if (buf.type == NAVBUF_DATA || buf.type == NAVBUF_DATA_EXT) {
				...
				if (buf.data.payloadSize > 0) {
					...
					if(pCh->startStreaming){
						pCh->read_TP_size = pCh->read_TP_size+ (unsigned long)buf.data.payloadSize;
					}
				}
...
void DEMUX_CheckPrintDemuxInfo(demux_channel *pCh)
{
	...
	if(TIMEOUT_CHECK_DEMUX_INFO * DEMUX_CLOCKS_PER_SEC <= passed__ ) {
		/* Case 1: didn't receive data from TP but PID has been set */
		if(pCh->read_TP_size == 0){
			if((pCh->active_video_pid > 0) && (pCh->active_video_pid != 0x1FFF) ){
				dmx_notice("(CH%d)RTKDEMUX ERROR(video PID:0x%x): should check TP side\n", pCh->tunerHandle, pCh->active_video_pid);
			}
			if((pCh->active_audio_pid > 0) && (pCh->active_audio_pid  != 0x1FFF)){
				dmx_notice("(CH%d)RTKDEMUX ERROR(audio PID:0x%x): should check TP side\n", pCh->tunerHandle, pCh->active_audio_pid);
			}
		}
		pCh->read_TP_size = 0;
		
		/* Case 3: demux receives video ts packet from TP,  but doesn¡¦t deliver video es*/
		if(pCh->video_deliver_size == 0){
			if((pCh->active_video_pid == 0) || (pCh->active_video_pid != 0x1FFF)){
				if(pCh->active_video_dest== DEMUX_DEST_VDEC0 || pCh->active_video_dest == DEMUX_DEST_VDEC1){
					dmx_notice("(CH%d)RTKDEMUX ERROR(video dest:0x%x): should check SDEC side\n", pCh->tunerHandle, pCh->active_video_dest);
				}
			}else if(pCh->active_video_dest== DEMUX_DEST_VDEC0 || pCh->active_video_dest == DEMUX_DEST_VDEC1){
				dmx_notice("(CH%d)RTKDEMUX ERROR(video PID:0x%x): should check SDEC side\n", pCh->tunerHandle, pCh->active_video_pid);
			}
		}
		if(pCh->audio_deliver_size == 0){
			if((pCh->active_audio_pid == 0) ||(pCh->active_audio_pid  != 0x1FFF)){
				if(pCh->active_audio_dest== DEMUX_DEST_ADEC0 || pCh->active_audio_dest == DEMUX_DEST_ADEC1){
					dmx_notice("(CH%d)RTKDEMUX ERROR(audio dest:0x%x): should check SDEC side\n", pCh->tunerHandle, pCh->active_audio_dest);
				}
			}else if(pCh->active_video_dest== DEMUX_DEST_ADEC0 || pCh->active_video_dest == DEMUX_DEST_ADEC1){
				dmx_notice("(CH%d)RTKDEMUX ERROR(audio PID:0x%x): should check SDEC side\n", pCh->tunerHandle, pCh->active_audio_pid);
			}
		}
		pCh->video_deliver_size = 0;
		pCh->audio_deliver_size = 0;
		
		/* Case 2: demux sends video es¡Abut video system pts is not updated anymore */
		if((pCh->preAudioSystemPTS == curAudioSystemPTS) && (pCh->pcrProcesser.lastDemuxAudioPTS > 0)){
			dmx_notice("(CH%d)RTKDEMUX ERROR : should check ADEC side, audio system PTS(%lld) no change during %d sec\n", pCh->tunerHandle, curAudioSystemPTS, TIMEOUT_CHECK_DEMUX_INFO);
		}
		if((pCh->preVideoSystemPTS == curVideoSystemPTS) && (pCh->pcrProcesser.lastDemuxVideoPTS > 0)){
			dmx_notice("(CH%d)RTKDEMUX ERROR : should check VDEC side, video system PTS(%lld) no change during %d sec\n", pCh->tunerHandle, curVideoSystemPTS, TIMEOUT_CHECK_DEMUX_INFO);
		}
