...
#define DEMUX_GET_TPP_MTP_ID(_ch) (((_ch) == DEMUX_CH_A) ? TP_TPP0_MTP : (((_ch) == DEMUX_CH_B) ? TP_TPP1_MTP : TP_TPP2_MTP))
#define DEMUX_GET_TPP_ID(_ch) (((_ch) == DEMUX_CH_A) ? TP_TPP0 : (((_ch) == DEMUX_CH_B) ? TP_TPP1 : TP_TPP2))
...
static void IOCTL_ShutDownModule(void)
{
	...
	if (demux_device->pChArray) {
		for (i = 0; i < demux_device->chNum; i++) {
			...
			DEMUX_BUF_FreeBuffer(&pCh->tppMtpBuffer);
...
static void IOCTL_CheckAvSync(demux_channel *pCh)
{
	...
	if (pCh->avSyncMode != MTP_AVSYNC_MODE && (pCh->tp_src == MTP)) {
		dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,"%s_%d, avsynMode \n", __func__, __LINE__);
...
int IOCTL_AllocateTPBuffer(int chNum, int size)
{
	...
	unsigned long tpBufferZoneStartPhyAddr = 0;
	...
	dmx_notice("tpBuffer=> startPhyAdd:0x%lx, totalSize:%d, ch:%d, chTpSize:%d\n",
		...
		if (haveSamePIDType && haveSameDest) {
			...
			if (currentFilter->pid == pInfo->pid) {
				...
				if (HAS_FLAG(pCh->status, STATUS_AUDIO_PAUSED) && IOCTL_IsAudioDest(pInfo->dest)) {
					dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,"[PVR] (CH%d) DONOT RemovePID: pid=%d(0x%x),type=%d,dest=%d\n", pCh->tunerHandle, currentFilter->pid, currentFilter->pid, pInfo->pidType, pInfo->dest);
					return 0;
				} else {
					dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,"[dtv] (CH%d)RemovePID: pid=%d(0x%x),type=%d,dest=%d\n", pCh->tunerHandle, currentFilter->pid, currentFilter->pid, pInfo->pidType, pInfo->dest);
...
int IOCTL_ResetFilters(DEMUX_RESET_FILTERS_T *pInfo)
{
	...
	if (bBypass) {
		for (i = 0; i < pCh->bypassPidFilterCount; i++)
			pCh->bypassPidFilterList[i] = -1;
		pCh->bypassPidFilterCount = 0;
	}
...
int IOCTL_SetPrivateInfo(DEMUX_PRIVATE_INFO_T *pInfo)
{
	...
	case DEMUX_PRIVATEINFO_AUDIO_FORMAT:
		{
			dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,"(%s-%d): DEMUX_PRIVATEINFO_AUDIO_FORMAT: dest:%d \n", __FILE__, __LINE__, pInfo->dest);
	...
	case DEMUX_PRIVATEINFO_AUDIO_PAUSE:
	{
		 ...
		 */
		dmx_mask_print(DEMUX_AUDIO_STABLE_DEBUG,"[%s %d] audio paused \n",__func__, __LINE__);
...
int IOCTL_ReConfigTPBuffer(DEMUX_CHANNEL_T ch)
{
	...
	tpp_mtp_id = DEMUX_GET_TPP_MTP_ID(ch);
	...
	if (pCh->tppMtpBuffer.allocSize > 0) {
		...
		dmx_crit("[%s %d] pCh->mtpBuffer.phyAddr = 0x%x\n", __func__, __LINE__, pCh->tppMtpBuffer.phyAddr);
...
{
	if (RHAL_TPOUT_Enable(pInfo->tpout_id, pInfo->ctrl) != TPK_SUCCESS)
		return -1;
...
int IOCTL_ChangeAvSyncMode(DEMUX_SET_SPEED_T *pInfo)
{
	 ...
	 * 3. pause -> fast rewind
	 */
	dmx_mask_print(DEMUX_PVR_ES_MONITOR_DEBUG, "%s_%d: ch:%d, speed:%d \n", __func__, __LINE__, pCh->tunerHandle, pInfo->speed);
...
int IOCTL_SetTPInputConfig(DEMUX_TP_SOURCE_CONFIG_T *pInfo)
{
	...
	int tp_id = 0, tmp_tp_id = 0;
	DEMUX_CHANNEL_T tmp_dmx_ch;
	demux_channel *pCh, *tmp_pCh;
	TPK_TP_STATUS_T tpStatus, tmp_tpStatus;
	...
	/* find the ch with same setting, if found, set the same tpp ch, tmp_tp_id = -1 means not found */
	for( tmp_dmx_ch = DEMUX_CH_A; tmp_dmx_ch < DEMUX_CH_NUM; tmp_dmx_ch++ ) {
		tmp_pCh = &demux_device->pChArray[tmp_dmx_ch];

		//dmx_notice("%s find ch%d portType=%d inputType=%d casType=%d\n", __func__, tmp_dmx_ch, tmp_pCh->inputConfigInfo.portType, tmp_pCh->inputConfigInfo.inputType, tmp_pCh->inputConfigInfo.casType, tmp_tp_id);
		if ( tmp_pCh->inputConfigInfo.portType == pInfo->portType &&
			 tmp_pCh->inputConfigInfo.inputType == pInfo->inputType &&
			 tmp_pCh->inputConfigInfo.casType == pInfo->casType )
			break;
	}
	tmp_tp_id = (tmp_dmx_ch == DEMUX_CH_NUM)?(-1):tmp_pCh->tunerHandle;

	/* record ch setting */
	pCh->inputConfigInfo.portType = pInfo->portType;
	pCh->inputConfigInfo.inputType = pInfo->inputType;
	pCh->inputConfigInfo.casType = pInfo->casType;
	dmx_notice("%s --- ch%d portType=%d inputType=%d casType=%d, tmp_tp_id=%d\n", __func__, pInfo->ch, pInfo->portType, pInfo->inputType, pInfo->casType, tmp_tp_id);
	...
	if (RHAL_GetTpStatus(tp_id, &tpStatus) != TPK_SUCCESS)
		return TPK_FAIL;
	if ((tmp_tp_id != -1) && RHAL_GetTpStatus(tmp_tp_id, &tmp_tpStatus) != TPK_SUCCESS)
		return TPK_FAIL;
	...
	if(pInfo ->casType == TPK_WITH_CIP_CAS) {
		if ( tmp_tp_id == -1 )
			tpStatus.tp_param.sync_byte = tpStatus.tp_param.sync_byte_cip14;
		else
			tpStatus.tp_param.sync_byte = tmp_tpStatus.tp_param.sync_byte_cip14;
	} else {
	...
	else if (pInfo->inputType == TPK_INPUT_PARALLEL || pInfo->inputType == TPK_INPUT_MEM)
	{
		tpStatus.tp_param.serial            = 0 ;
	...
	/* 4. set tpo */
	if(pInfo->casType!=TPK_WITHOUT_CAS)
	{
		...
		if(pInfo->casType==TPK_WITH_CIP_CAS){
			if ( tmp_tp_id != -1 ) {
				remux_param.tpp_id = (TPK_TPP_ENGINE_T)tmp_tp_id;
				remux_param.output_sync_byte = tmp_tpStatus.tp_param.sync_byte_cip14;
			} else {
				remux_param.tpp_id = (TPK_TPP_ENGINE_T)tp_id;
				remux_param.output_sync_byte = tpStatus.tp_param.sync_byte_cip14;
			}
		} else {
			remux_param.output_sync_byte = 0x47;
			remux_param.tpp_id = 0; /*default use tpp0 */
		}
		dmx_notice("%s Set CI channel to TPP%d! tp%d_sync_byte=%X tpp_out_sync_byte=%X\n", __func__, remux_param.tpp_id, tp_id, tpStatus.tp_param.sync_byte, remux_param.output_sync_byte);
		...
		if (RHAL_TPOUT_Enable(0,TPOUT_STREAM_START) != TPK_SUCCESS)
			return TPK_FAIL;
...
int IOCTL_TPPMTPGetBuffer(DEMUX_TPP_MTP_BUF_INFO_T *pInfo)
{
	demux_channel *pCh;
	...
	pCh = &demux_device->pChArray[pInfo->ch];
	if (pCh->tppMtpBuffer.allocSize == 0) {
		if (DEMUX_BUF_AllocBuffer(&pCh->tppMtpBuffer,  TPP_MTP_BUFFER_SIZE, 1, 0) < 0) {
			dmx_crit("[%s %d] fail to allocate MTP buffer\n", __func__, __LINE__);
			return -1;
		}
		IOCTL_ReConfigTPBuffer(pInfo->ch);
	}
	pInfo->bufPhyAddr = pCh->tppMtpBuffer.phyAddr;
	pInfo->bufSize = pCh->tppMtpBuffer.allocSize;
	return 0;
}

int IOCTL_FreeTPPMTPBuf(DEMUX_CHANNEL_T *pInfo)
{
	demux_channel *pCh;
	UINT8 tpp_mtp_id;
	...
	DEMUX_CHECK_CH(*pInfo);
	pCh = &demux_device->pChArray[*pInfo];
	tpp_mtp_id = DEMUX_GET_TPP_MTP_ID(*pInfo);
	RHAL_SetMTPBuffer(tpp_mtp_id, NULL, NULL, 0); /*Notify TP that mtp buffer was freed*/
	DEMUX_BUF_FreeBuffer(&pCh->tppMtpBuffer);
	return 0;
}

int IOCTL_TPPMTPGetWriteStatus(DEMUX_MTP_WRITE_STATUS_T *pInfo)
{
	...
	tpp_mtp_id = DEMUX_GET_TPP_MTP_ID(pInfo->ch);

	if (RHAL_GetMTPBufferStatus(tpp_mtp_id, &pWritePhyAddr, &contiguousWritableSize, &WritableSize) != TPK_SUCCESS)
		return -1;
...
int IOCTL_GetTPBufStatus(DEMUX_TP_BUF_STATUS_T *pInfo)
{
	demux_channel *pCh;
	UINT32 bufSize;
	UINT32 dataSize;
	...
	if (RHAL_GetTPBufferStatus(pCh->tunerHandle, &bufSize, &dataSize) != TPK_SUCCESS)
		return -1;
...
int IOCTL_GetTPPBufStatus(DEMUX_TPP_BUF_STATUS_T *pInfo)
{
	...
	tpp_id = DEMUX_GET_TPP_ID(pInfo->ch);

	if (RHAL_GetTPPBufferStatus(tpp_id, &bufSize, &dataSize) != TPK_SUCCESS)
		return -1;
...
int IOCTL_SetTPPMtpStreaming(DEMUX_MTP_STREAMING_STATUS_T *pInfo)
{
	demux_channel *pCh;
	UINT8 tpp_mtp_id;
	DEMUX_CHECK_CH(pInfo->ch);
	...
	if(pInfo->bStartStreaming)
	{
		if (pCh->tppMtpBuffer.allocSize == 0) {
			if (DEMUX_BUF_AllocBuffer(&pCh->tppMtpBuffer, TPP_MTP_BUFFER_SIZE, 1, 0) < 0) {
				dmx_crit("[%s %d] fail to allocate TPP_MTP buffer\n", __func__, __LINE__);
				return -1;
			}
			IOCTL_ReConfigTPBuffer(pInfo->ch);
		}
	...
	if (TPK_SUCCESS != RHAL_MTPStreamControl(tpp_mtp_id, pInfo->bStartStreaming ? MTP_STREAM_START : MTP_STREAM_STOP))
		return -1;

	return 0;
}

int IOCTL_TPPMTPFlush(DEMUX_MTP_FLUSH_T *pInfo)
{
	UINT8 tpp_mtp_id;
	MTP_BUFF_FLUSH_MODE_T mode;
	DEMUX_CHECK_CH(pInfo->ch);
	...
	if (RHAL_MTPFlushBuffer(tpp_mtp_id, mode) != TPK_SUCCESS)
		return -1;
...
int IOCTL_TPPMTPWriteBuffer(DEMUX_MTP_WRITE_T *pInfo)
{
	UINT8 tpp_mtp_id;
	DEMUX_CHECK_CH(pInfo->ch);

	tpp_mtp_id = DEMUX_GET_TPP_MTP_ID(pInfo->ch);
	if (RHAL_WriteMTPBuffer(tpp_mtp_id, (UINT8 *)pInfo->writeAddr, pInfo->writeSize) != TPK_SUCCESS)
		return -1;

	return 0;
}

int IOCTL_GetCIDLBufferStatus(DEMUX_CI_DL_BUF_STATUS_T *pInfo)
{
	demux_channel *pCh;
	...
	if(RHAL_GetCIDLBufferStatus(pCh->tunerHandle, &pInfo->phyBase, &pInfo->phyLimit,
			&pInfo->phyRead, &pInfo->phyWrite) != TPK_SUCCESS)
		return -1;

	return 0;
}

int IOCTL_SetCIDLBufferRP(DEMUX_CI_DL_BUF_RP_T *pInfo)
{
	demux_channel *pCh;
	...
	if(RHAL_SetCIDLBufferRP(pCh->tunerHandle, pInfo->phyRead) != TPK_SUCCESS)
		return -1;

	return 0;
}

int IOCTL_SetTpPidEnable(DEMUX_TP_PID_ENABLE_T *pInfo)
{
	demux_channel *pCh;
	...
	if(RHAL_EnablePIDFilter(pCh->tunerHandle, pInfo->bEnable) != TPK_SUCCESS)
		return -1;

	return 0;
}

int IOCTL_SetTppPidEnable(DEMUX_TPP_PID_ENABLE_T *pInfo)
{
	if(RHAL_EnableTPPPIDFilter((TPK_TP_ENGINE_T)pInfo->tpp_id, pInfo->bEnable) != TPK_SUCCESS)
		   return -1;
...
int IOCTL_TPPFlush(UINT8 *pInfo)
{
	if(RHAL_TPPFlushBuffer((TPK_TP_ENGINE_T)*pInfo) != TPK_SUCCESS)
		   return -1;

	return 0;
}

int IOCTL_SetTpSyncCP14(DEMUX_TP_SYNC_CP14_T *pInfo)
{
	demux_channel *pCh;

	DEMUX_CHECK_CH(pInfo->ch);
	pCh = &demux_device->pChArray[pInfo->ch];
	if(RHAL_SetTpSyncCP14(pCh->tunerHandle, pInfo->u8SyncByte) != TPK_SUCCESS)
		   return -1;

	return 0;
}
int IOCTL_SetBypassPIDs(DEMUX_PID_TABLE_T *pInfo)
{
	demux_channel *pCh;
	int i, j;
	dmx_function_call();
	...
	pCh = &demux_device->pChArray[pInfo->ch];
	if (pInfo->count > DEMUX_MAX_NUM_TO_SET || (pInfo->count + pCh->bypassPidFilterCount) > DEMUX_MAX_NUM_TO_SET) {
		dmx_warn("[%s %d] too many pids! already have %d and you want to insert %d pids\n", __func__, __LINE__, pCh->bypassPidFilterCount, pInfo->count);
		return -1;
	}
	mutex_lock(&pCh->mutex);
	...
	for (i = 0; i < pInfo->count; i++) {
		for (j = 0; j < pCh->bypassPidFilterCount; j++) {
			if (pInfo->pid[i] == pCh->bypassPidFilterList[j])
				break;
		}
		if (j == pCh->bypassPidFilterCount) {
			pCh->bypassPidFilterList[pCh->bypassPidFilterCount] = pInfo->pid[i];
			pCh->bypassPidFilterCount++;
		}
	}
	pCh->bUpdateDemuxTarget = true;
	if (!pCh->startStreaming) {
...
static size_t IOCTL_RelocatePIDFilters(short* const list, size_t size)
{

	short* const end = list + size;
...
int IOCTL_RemoveBypassPIDs(DEMUX_PID_TABLE_T *pInfo)
{
	demux_channel *pCh;
	int i, j;
	dmx_function_call();
	...
	pCh = &demux_device->pChArray[pInfo->ch];
	if (pInfo->count > DEMUX_MAX_NUM_TO_SET) {
		dmx_warn("[%s %d] too many pids! you want to remove %d pids\n", __func__, __LINE__, pInfo->count);
		return -1;
	}
	mutex_lock(&pCh->mutex);
	...
	for (i = 0; i < pInfo->count; i++) {
		for (j = 0; j < pCh->bypassPidFilterCount; j++) {
			if (pInfo->pid[i] == pCh->bypassPidFilterList[j]) {
				pCh->bypassPidFilterList[j] = -1;
				break;
			}
		}
	}
	pCh->bypassPidFilterCount = IOCTL_RelocatePIDFilters(pCh->bypassPidFilterList, pCh->bypassPidFilterCount);
	pCh->bUpdateDemuxTarget = true;

	mutex_unlock(&pCh->mutex);
	return 0;
}
int IOCTL_GetBypassPIDs(DEMUX_PID_TABLE_T *pInfo)
{
	demux_channel *pCh;
	dmx_function_call();
	DEMUX_CHECK_CH(pInfo->ch);
	...
	pInfo->count = pCh->bypassPidFilterCount;
	memcpy(pInfo->pid, pCh->bypassPidFilterList, sizeof(short)*DEMUX_MAX_NUM_TO_SET);

	mutex_unlock(&pCh->mutex);
	return 0;
}

int IOCTL_GetCAMBitrate(DEMUX_CAM_BITRATE_T *pInfo)
{
	if(RHAL_GetCAMBitRate(&pInfo->inputBitrate,&pInfo->outputBitrate)!= TPK_SUCCESS)
		return -1;

	return 0;
}

int IOCTL_SetTPPStreamControl(DEMUX_TP_STREAM_CONTROL_T *pInfo)
{
	DEMUX_CHECK_CH(pInfo->ch);

	TPK_TPP_ENGINE_T tpp_id = DEMUX_GET_TPP_ID(pInfo->ch);

	if (RHAL_TPPStreamControl(tpp_id, pInfo->ctrl) != TPK_SUCCESS)
		return -1;
