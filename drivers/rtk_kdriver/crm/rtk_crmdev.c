...
static const char *_codec_type_to_name(int32_t codec_type) {
  switch (codec_type) {
    ...
    case RTK_CRMDEV_CODEC_TYPE_VP9:
      return "vp9";
    case RTK_CRMDEV_CODEC_TYPE_AV1:
      return "av1";
    case RTK_CRMDEV_CODEC_TYPE_VVC:
      return "vvc";
    ...
    case RTK_CRMDEV_CODEC_TYPE_AVS:
      return "avs";
    case RTK_CRMDEV_CODEC_TYPE_QWER3:
      return "qwer3";
    case RTK_CRMDEV_CODEC_TYPE_DOLBY_VISION:
      return "dolby_vision";
    case RTK_CRMDEV_CODEC_TYPE_JPEG:
      return "jpeg";
    case RTK_CRMDEV_CODEC_TYPE_JPS:
      return "jps";
    case RTK_CRMDEV_CODEC_TYPE_PNG:
      return "png";
    case RTK_CRMDEV_CODEC_TYPE_BMP:
      return "bmp";
    case RTK_CRMDEV_CODEC_TYPE_GIF:
      return "gif";
    case RTK_CRMDEV_CODEC_TYPE_TIFF:
      return "tiff";
    ...
    case RTK_CRMDEV_CODEC_TYPE_SOURCE_ATV:
      return "source_atv";
    case RTK_CRMDEV_CODEC_TYPE_SOURCE_DTV:
      return "source_dtv";
    ...
    case RTK_CRMDEV_CODEC_TYPE_AVS2:
      return "avs2";
    case RTK_CRMDEV_CODEC_TYPE_AVS3:
      return "avs3";
    ...
    case RTK_CRMDEV_CODEC_TYPE_DTS_HD:
      return "dts_hd";
    case RTK_CRMDEV_CODEC_TYPE_DTS_X:
      return "dts_x";
    ...
    case RTK_CRMDEV_CODEC_TYPE_ALAC:
      return "alac";
    case RTK_CRMDEV_CODEC_TYPE_DISPLAY_VP:
      return "display_vp";
    case RTK_CRMDEV_CODEC_TYPE_DISPLAY_VP0:
      return "display_vp0";
    case RTK_CRMDEV_CODEC_TYPE_DISPLAY_VP1:
      return "display_vp1";
    case RTK_CRMDEV_CODEC_TYPE_VSC_WIN0:
      return "vsc_win0";
    case RTK_CRMDEV_CODEC_TYPE_VSC_WIN1:
      return "vsc_win1";
    default:
      return "none";
  }
...
static inline bool _codec_info_is_display(const rtk_crmdev_codec_info_t *info) {
  return ((info->codec & RTK_CRMDEV_CODEC_TYPE_VIDEO_DISPLAY) ? true : false);
}

static inline bool _codec_info_is_vsc(const rtk_crmdev_codec_info_t *info) {
  return ((info->codec & RTK_CRMDEV_CODEC_TYPE_VIDEO_SCALE_CONTROL) ? true : false);
}
...
static const char *_codec_info_to_string(const rtk_crmdev_codec_info_t *info) {
  ...
  const char *adaptive = info->adaptive ? "adaptive" : "static";
  ...
  } else if (_codec_info_is_display(info)) {
    snprintf(_device_table->buffer, sizeof(_device_table->buffer),
             "codec-info: %s %s", codec_name, encoder);
  } else if (_codec_info_is_vsc(info)) {
    snprintf(_device_table->buffer, sizeof(_device_table->buffer),
             "codec-info: %s %s", codec_name, encoder);
  } else {
    snprintf(_device_table->buffer, sizeof(_device_table->buffer),
...
static void _codec_resource_set_from_providers(
  ...
  for (i = 0; i < provider_count; i++) {
    ...
    } else if (!strncmp(provider->name, "ip3", sizeof(provider->name))) {
      resource->core_type |= RTK_CRMDEV_CORE_TYPE_IP3;
...
static void _codec_resource_get_providers(
    const rtk_crmdev_codec_resource_t *resource, int32_t unique_id, bool all_client,
  ...
  for (i = 0; i < _device_table->provider_count; i++) {
    ...
    if (all_client  || _resource_provider_contain_unique_id(provider, unique_id, NULL, NULL)) {
      ...
      } else if (_resource_provider_is_name(provider, "ip3")) {
        if (resource->core_type & RTK_CRMDEV_CORE_TYPE_IP3) {
          providers[(*provider_count)++] = provider;
        }
...
static inline const bool _resource_client_is_contain_name(
    const resource_client_t *client, const char *clientName) {
  return strncmp(client->name, clientName, sizeof(client->name)) == 0 ? false : true;
}
...
static void _resource_client_wake_event(resource_client_t *client,
  ...
  rtd_pr_crm_debug("[crmmgr] events   : %s wake up with %s status\n",
...
static void _resource_client_finalize(struct kref *ref) {
  ...
  rtd_pr_crm_debug("[crmmgr] finalize : %s %p\n", _resource_client_get_name(client),
          client);
  kfree(client);
}
...
static bool _resource_client_create(struct file *filp) {
  ...
  rtd_pr_crm_debug("[crmmgr] open     : %p %p\n", client, filp);
...
static void _resource_client_release(struct file *filp) {
  if (filp && filp->private_data) {
    ...
    if (test_and_clear_bit(RESOURCE_CLIENT_STATUS_RECLAIM_CLOSE,
                           &client->state)) {
      rtd_pr_crm_debug("[crmmgr] events   : %s response reclaim close\n",
              _resource_client_get_name(client));
    ...
    if (test_and_clear_bit(RESOURCE_CLIENT_STATUS_UPDATE_PRIORITY_CLOSE,
                           &client->state)) {
      rtd_pr_crm_debug("[crmmgr] events   : %s response update priority close\n",
              _resource_client_get_name(client));
      _resource_client_wake_event(client,
                                  RESOURCE_CLIENT_STATUS_UPDATE_PRIORITY_DONE);
    }
    mutex_unlock(&_table_lock);
    rtd_pr_crm_debug("[crmmgr] release  : %s %p %p\n", _resource_client_get_name(client),
            client, filp);
...
static int _resource_client_reclaim(resource_client_t *client, bool force) {
  ...
  if(client->is_reclaiming == 1) {
      rtd_pr_crm_err("[crmmgr] reclaim  : %s is in reclaim process\n", _resource_client_get_name(client));
      //wait reclaim end
      mutex_unlock(&_table_lock);
      while(client->is_reclaiming == 1 && cnt++ < 30) {
        usleep_range(10000,10000);
      }
      mutex_lock(&_table_lock);
      if(client->is_reclaiming == 1){
        rtd_pr_crm_err("[crmmgr] reclaim  : %s is still in reclaim process, just return reject\n",
           _resource_client_get_name(client));
        return -ENOMEM;;
      }else {
        rtd_pr_crm_err("[crmmgr] reclaim  : %s available wait %d ms, return 0\n",
          _resource_client_get_name(client),cnt*10);
        return 0;
      }
  }
  ...
  rtd_pr_crm_err("[crmmgr] events   : %s wait to reclaim response\n",
          _resource_client_get_name(client));
  ...
  if (unlikely(ret)) {
    if (signal_pending(current)) {
      rtd_pr_crm_warn("[crmmgr] events   : %s caught pending signal when reclaim\n",
                 _resource_client_get_name(client));
      return -EINTR;
    }
    return -ERESTARTSYS;
...
static int _resource_client_update_priority(resource_client_t *client) {
  ...
  rtd_pr_crm_debug("[crmmgr] events   : %s wait to update priority\n",
          _resource_client_get_name(client));
...
static bool _resource_client_is_monitor(resource_provider_t *provider,
  ...
  if (_resource_provider_is_group(provider)) {
    rtd_pr_crm_err("[crmmgr] release.error call release on group provider");
    return false;
  }
...
static void _resource_record_clear_owner(resource_record_t *record,
  ...
  if (record != NULL && record->owner &&
    ...
    rtd_pr_crm_debug("[crmmgr] release  : %s owner was remove from %s (%s %d)\n",
            _resource_client_get_name(client),
...
static void _resource_record_clear_monitor(resource_record_t *record,
  ...
  for (i = 0; i < size; i++) {
    ...
    if (monitor && _resource_client_is_equal(monitor, client)) {
      rtd_pr_crm_debug("[crmmgr] release  : %s monitor was remove from %s (%s %d)\n",
              _resource_client_get_name(client),
              _resource_record_get_name(record),
              _resource_provider_get_name(record->provider), record->cost);
...
static bool _resource_record_contain_client(const resource_record_t *record,
  ...
  if (record->owner != NULL) {
    if(_resource_client_is_contain_name(record->owner, clientName))
      return true;
  }

  for (i = 0; i < size; i++) {
    resource_client_t *monitor = record->monitors[i];
    if (monitor && _resource_client_is_contain_name(monitor, clientName)) {
      return true;
    }
  }
  return false;
...
static int _resource_record_reclaim_priority_compare(const void *lhs,
  ...
  rtd_pr_crm_debug("[crmmgr] compare: %s %d %d %llu\n", _resource_record_get_name(l),
...
static int _resource_record_reclaim(resource_record_t *record, bool force) {
  ...
  while ((ret == 0) && (client = _resource_record_peek_monitor(record))) {
    rtd_pr_crm_err("[crmmgr] reclaim  : %s monitor is reclaiming for %s (%s %d)\n",
            _resource_client_get_name(client),
            _resource_record_get_name(record),
            _resource_provider_get_name(record->provider), record->cost);
    ...
    rtd_pr_crm_err("[crmmgr] reclaim  : %s monitor was %s\n",
            _resource_client_get_name(client),
      ...
      if (_resource_provider_is_leaf(provider)) {
        ...
        if (_resource_provider_contain_client(provider, client,
          ...
          for (j = 0; j < record_count; j++) {
            ...
            rtd_pr_crm_err("[crmmgr] reclaim  : check record %s\n", _resource_provider_get_name(monitor_record->provider));
            if((monitor_client = _resource_record_peek_monitor(monitor_record)) != NULL){
              rtd_pr_crm_err("[crmmgr] reclaim  : %s monitor found %s \n",
  ...
  if ((ret == 0) && (client = _resource_record_peek_owner(record))) {
    rtd_pr_crm_err("[crmmgr] reclaim  : %s owner is reclaiming for %s (%s %d)\n",
            _resource_client_get_name(client),
            _resource_record_get_name(record),
            _resource_provider_get_name(record->provider), record->cost);
    ret |= _resource_client_reclaim(client, force);
    client->is_reclaiming = 0;
    rtd_pr_crm_err("[crmmgr] reclaim  : %s owner was %s\n",
            _resource_client_get_name(client),
            (ret ? "rejected" : "reclaimed"));
    _resource_client_unref(client);
  }
...
static int _resource_record_update_priority(resource_record_t *record) {
  ...
  if ((client = _resource_record_peek_owner(record))) {
    rtd_pr_crm_debug("[crmmgr] priority : %s owner is updating priority\n",
            _resource_client_get_name(client));
    ret |= _resource_client_update_priority(client);
    rtd_pr_crm_debug("[crmmgr] priority : %s owner has priority %d\n",
            _resource_client_get_name(client),
...
static bool _resource_provider_set_acquire_priority(
  ...
  if (priority) {
    rtd_pr_crm_warn("[crmmgr] acquire  : set acquire priority %d in %s\n", priority,
...
static int _resource_provider_reclaim_priority_compare(const void *lhs,
  ...
  rtd_pr_crm_debug("[crmmgr] compare  : %s avail: %d prio: %d acqtime: %llu\n",
...
static bool _resource_provider_add_record(resource_provider_t *provider,
  ...
  if (!record) {
    rtd_pr_crm_err("[crmmgr] record is full in %s provider",
           _resource_provider_get_name(provider));
    return false;
  }
  ...
  rtd_pr_crm_debug("[crmmgr] acquire  : %s owner was insert into %s (%s %d)\n",
...
static bool _resource_provider_acquire(resource_provider_t *provider,
  ...
  rtd_pr_crm_debug("[crmmgr] acquire  : %s acquire (%s %d)\n",
          _resource_client_get_name(client),
          _resource_provider_get_name(provider), cost);
  ...
  if (provider->reference_count > 0) {
    ...
    if (cost > 0) {
      rtd_pr_crm_err("[crmmgr] resource table is confused at %s\n", __func__);
      return false;
    }
  } else {
    /* leaf provider */
...
static int _resource_provider_update_reclaim_priority(
  ...
  if (provider->reference_count > 0) {
    ...
    rtd_pr_crm_debug("[crmmgr] reclaim  : updating reclaim priority (%s)\n",
            _resource_provider_get_name(provider));
    ...
    rtd_pr_crm_debug("[crmmgr] reclaim  : updated reclaim priority (%s)\n",
            _resource_provider_get_name(provider));
  }
...
static int _resource_provider_reclaim(resource_provider_t *provider,
  ...
  if (_resource_provider_available(provider, cost) >= 0) {
    rtd_pr_crm_err("[crmmgr] reclaim  : %s is available\n",
            _resource_provider_get_name(provider));
    return 0;
  }

  rtd_pr_crm_err("[crmmgr] reclaim  : reclaiming (%s %d)\n",
          _resource_provider_get_name(provider), cost);
  ...
  if (update_reclaim_priority) {
    ...
    if (_resource_provider_available(provider, cost) >= 0) {
      rtd_pr_crm_debug("[crmmgr] reclaim  : %s is available after update priority\n",
              _resource_provider_get_name(provider));
      return 0;
    }
  }
...
static bool _resource_provider_is_display(const resource_provider_t *provider) {
  if (_resource_provider_is_name(provider, "vdp0"))
    return true;
  else if (_resource_provider_is_name(provider, "vdp1"))
    return true;
  else
...
static int _resource_provider_clear(resource_provider_t *provider) {
  int ret = 0;
  size_t j = 0;
  bool display_provider = false;
  resource_record_t *records[MAX_RECORDS_COUNT];
  size_t record_count = 0;

  rtd_pr_crm_err("[crmmgr] clear  : clear %s\n", _resource_provider_get_name(provider));

  //if display, only clean hwc
  if (_resource_provider_is_display(provider)) {
    display_provider = true;
  }

  /* sort records by reclaim priority */
  for (j = 0; j < MAX_RECORDS_COUNT; j++) {
    if (_resource_record_is_used(&provider->records[j])) {
        records[record_count++] = &provider->records[j];
    }
  }
  ...
  rtd_pr_crm_debug("[crmmgr] clear  : start to check display %s record\n",
         _resource_provider_get_name(provider));

    /* reclaim for each record */
  for (j = 0; j < record_count; j++) {
    if (!display_provider || (_resource_record_contain_client(records[j], "hwc-"))) {
        rtd_pr_crm_err("[crmmgr] clear  : clear %s record %s\n",
            _resource_provider_get_name(provider),
            _resource_record_get_name(records[j]));
        ret = _resource_record_reclaim(records[j], false);
        if (ret == 0) {
            continue;
        } else if (ret == -ERESTARTSYS || ret == -EINTR) {
            return ret;
  ...
  rtd_pr_crm_err("[crmmgr] clear  : clear %s end\n", _resource_provider_get_name(provider));

  return 0;

}

static int _resource_provider_clear_display_internal(resource_client_t *client) {
  size_t provider_count = _device_table->provider_count;
  size_t i = 0;
  int ret = 0;

  rtd_pr_crm_err("[crmmgr] clear : %s clear display internal\n", _resource_client_get_name(client));
  ...
  for (i = 0; i < provider_count; i++) {
    ...
    ret = _resource_provider_clear(provider);
    if (ret != 0) {
        rtd_pr_crm_err("[crmmgr] clear : %s clear internal %s error\n",
                _resource_client_get_name(client), _resource_provider_get_name(provider));
    }
  }
...
static inline bool _resource_format_is_display(const resource_format_t *format) {
  return ((format->codec & RTK_CRMDEV_CODEC_TYPE_VIDEO_DISPLAY) ? true : false);
}

static inline bool _resource_format_is_vsc(const resource_format_t *format) {
  return ((format->codec & RTK_CRMDEV_CODEC_TYPE_VIDEO_SCALE_CONTROL) ? true : false);
}
...
static const char *_resource_format_to_string(const resource_format_t *format) {
  ...
  } else if (_resource_format_is_display(format)) {
    snprintf(_device_table->buffer, sizeof(_device_table->buffer),
             "%s (%04x) %s", _codec_type_to_name(format->codec), format->codec,
             format->encoder ? "encoder" : "decoder");
  } else if (_resource_format_is_vsc(format)) {
    snprintf(_device_table->buffer, sizeof(_device_table->buffer),
             "%s (%04x) %s", _codec_type_to_name(format->codec), format->codec,
             format->encoder ? "encoder" : "decoder");
  } else {
...
static bool _resource_format_is_dtv(const resource_format_t *format) {
  switch(format->codec) {
    case RTK_CRMDEV_CODEC_TYPE_DTV_AVC:
        return true;
    case RTK_CRMDEV_CODEC_TYPE_DTV_HEVC:
...
static bool _resource_format_acquire(resource_format_t *format,
  ...
  resource_provider_t *providers[MAX_PROVIDER_COUNT];
  bool all_vbm_used = false;
  const resource_provider_t *provider = NULL;

  rtd_pr_crm_err("[crmmgr] acquire  : %s acquire format %s\n",
          _resource_client_get_name(client),
          _resource_format_to_string(format));

  /* check all of requires */
  if (!_resource_format_is_available(format)) {
    rtd_pr_crm_err("[crmmgr] acquire  : no resource available, need reclaim for %s\n", _resource_format_to_string(format));
    return false;
  }
  ...
  for (i = 0; i < provider_count; i++) {
      provider = providers[i];
      if (!strncmp(provider->name, "vbm", sizeof(provider->name)) &&
              _resource_provider_available(provider, 0) <= 0) {
          all_vbm_used = true;
      }
  }

  //if DTV and all vbm used, reclaim hwc display resource to stop dvo (free vbm buffer) if needed.
  if (_resource_format_is_dtv(format) && all_vbm_used) {
      _resource_provider_clear_display_internal(client);
  }
...
static int _resource_format_reclaim(resource_format_t *format) {
  ...
  for (i = 0; i < format->require_count; i++) {
    ...
    if (_resource_require_is_available(require)) {
      rtd_pr_crm_debug("[crmmgr] reclaim  : (%s %d) is available\n",
...
static bool _resource_table_match(resource_table_t *table,
  ...
  if(in_capability) {
    if(capability_count <= 0) {
      rtd_pr_crm_err("[crmmgr] capability     : capability_count %d is not vaild\n", capability_count);
    } else {
      ...
      if (_resource_provider_is_leaf(provider)) {
        ...
        if (_resource_provider_contain_client(provider, client,
            ...
            for (j = 0; j < record_count; j++) {
                const resource_record_t *record = records[j];
                name = _resource_provider_get_name(record->provider);
                seq_printf(m, "(monitor %s %03d %llu) ", name, record->cost,
                        record->acquire_time);
            }
        }
      }
...
static void _resource_table_add_provider(rtk_crmdev_request *req) {
  ...
  if (_device_table->provider_count < MAX_PROVIDER_COUNT) {
    ...
    rtd_pr_crm_debug("[crmmgr] init     : add %s\n",
            _resource_provider_get_name(provider));
    req->status = RTK_CRMDEV_STATUS_OK;
  } else {
    rtd_pr_crm_err("[crmmgr] init     : provider count is exceeded %u\n",
           MAX_PROVIDER_COUNT);
...
static void _resource_table_add_reference(rtk_crmdev_request *req) {
  ...
  if (_device_table->provider_count > 0) {
    ...
    if (provider->reference_count < MAX_REFERENCES_COUNT) {
      ...
      if (reference) {
        rtd_pr_crm_debug("[crmmgr] init     : add %s %s\n",
                _resource_provider_get_name(provider),
        ...
        rtd_pr_crm_err("[crmmgr] init     : no provider %s found\n", item->name);
        req->status = RTK_CRMDEV_STATUS_INVALID_XML;
      }
    } else {
      rtd_pr_crm_err("[crmmgr] init     : %s reference count is exceeded %u\n",
             _resource_provider_get_name(provider), MAX_REFERENCES_COUNT);
      req->status = RTK_CRMDEV_STATUS_INVALID_XML;
    }
  } else {
    rtd_pr_crm_err("[crmmgr] init     : no provider available\n");
    req->status = RTK_CRMDEV_STATUS_INVALID_XML;
  }
}
...
static void _resource_table_add_format(rtk_crmdev_request *req) {
  ...
  if (_device_table->format_count < MAX_FORMAT_COUNT) {
    ...
    rtd_pr_crm_err("[crmmgr] init     : format count is exceeded %u\n",
           MAX_FORMAT_COUNT);
    req->status = RTK_CRMDEV_STATUS_INVALID_XML;
  }
}
...
static void _resource_table_add_require(rtk_crmdev_request *req) {
  ...
  if (_device_table->format_count > 0) {
    ...
    if (format->require_count < MAX_REQUIRES_COUNT) {
      ...
      if (provider) {
        rtd_pr_crm_debug("[crmmgr] init     : add %s format require (%s %d)\n",
      ...
      rtd_pr_crm_err("[crmmgr] init     : %s require count is exceeded %u\n",
             _resource_format_get_name(format), MAX_REQUIRES_COUNT);
      req->status = RTK_CRMDEV_STATUS_INVALID_XML;
    }
  } else {
    rtd_pr_crm_err("[crmmgr] init     : no format available\n");
    req->status = RTK_CRMDEV_STATUS_INVALID_XML;
  }
}
...
static void _resource_table_add_capability(rtk_crmdev_request *req) {
  ...
  if (_device_table->capability_count < MAX_CAPABILITY_COUNT) {
    ...
    rtd_pr_crm_err("[crmmgr] init     : capability count is exceeded %u\n",
...
static void _resource_service_check_support_lock_table(
  ...
  rtd_pr_crm_err("[crmmgr] support  : %s check %s is %s",
...
static void _resource_service_check_capability_lock_table(
  ...
  rtd_pr_crm_err("[crmmgr] capability  : %s check %s is %s",
...
static void _resource_service_release_lock_table(resource_client_t *client,
  ...
  for (i = 0; i < _device_table->provider_count; i++) {
    ...
    if (_resource_provider_is_leaf(provider)) {
      if(_resource_client_is_monitor(provider, client)) {
        rtd_pr_crm_err("[crmmgr] release  : %s is monitor, release directly\n", _resource_client_get_name(client));
        is_monitor = 1;
        break;
      }
    }
  ...
  if(!is_monitor) {
    rtd_pr_crm_debug("[crmmgr] release  : %s check if the resources have registed monitor to reclaim\n", _resource_client_get_name(client));
    for (i = 0; i < _device_table->provider_count; i++) {
      const resource_provider_t *provider = &_device_table->providers[i];
      if (_resource_provider_is_leaf(provider)) {
        size_t record_count = 0;
    ...
    if(!monitor_reclaim)
      rtd_pr_crm_err("[crmmgr] release  : %s no registed monitor found, just release client\n", _resource_client_get_name(client));
  }
  ...
  if (req) {
    req->status = RTK_CRMDEV_STATUS_OK;
    rtd_pr_crm_err("[crmmgr] release  : %s done\n", _resource_client_get_name(client));
  }
}
...
static int _resource_service__acquire_lock_table(resource_client_t *client,
  ...
  if (!_resource_table_match(_device_table, info, NULL, match_formats,
                             &match_count)) {
    rtd_pr_crm_err("[crmmgr] acquire  : %s acquire %s is not support",
           _resource_client_get_name(client), _codec_info_to_string(info));
    req->status = RTK_CRMDEV_STATUS_UNSUPPORT;
    return 0;
  } else {
    rtd_pr_crm_err("[crmmgr] acquire  : %s acquire %s match_count %d\n",
            _resource_client_get_name(client), _codec_info_to_string(info), match_count);
  }

  for (i = 0; i < match_count; i++) {
    resource_format_t *format = match_formats[i];
    rtd_pr_crm_err("[crmmgr] acquire  : %s match format %s, avail %d\n",
            _resource_client_get_name(client),
            _resource_format_to_string(format),
  ...
  if (_resource_table_contain_client(_device_table, client,
    ...
    rtd_pr_crm_err("[crmmgr] acquire  : %s acquire again\n",
            _resource_client_get_name(client));
      ...
      if (_resource_provider_is_leaf(provider)) {
        ...
        if (_resource_provider_contain_client(
          ...
          for (j = 0; j < record_count; j++) {
            ...
            rtd_pr_crm_err("[crmmgr] acquire  : %s owner has %s (%s %d)\n",
                    _resource_client_get_name(client),
                    _resource_record_get_name(record),
                    _resource_provider_get_name(provider),
    ...
    rtd_pr_crm_err("[crmmgr] acquire  : %s acquire first\n",
            _resource_client_get_name(client));
  }

  for (i = 0; i < match_count; i++) {
    if (_resource_format_acquire(match_formats[i], client, info, resource)) {
      rtd_pr_crm_err("[crmmgr] acquire  : %s acquire success %s\n",
              _resource_client_get_name(client),
  ...
  if(info->try_acquire == 1){
    ...
    rtd_pr_crm_err("[crmmgr] try_acquire  : %s acquire failure, no resource available\n",
            _resource_client_get_name(client));
    return 0;
  }else {
  ...
  if (ret == -ENOMEM) {
    req->status = RTK_CRMDEV_STATUS_NO_RESOURCE;
    rtd_pr_crm_err("[crmmgr] acquire  : %s acquire failure, no resource available\n",
            _resource_client_get_name(client));
    ret = 0;
  }
...
static int _resource_service_register_lock_table(resource_client_t *client,
  ...
  if (!_resource_table_contain_unique_id(_device_table, unique_id,
                                         &target_client)) {
    rtd_pr_crm_debug("[crmmgr] register : %s does not contain unique id %08x",
  ...
  if (_resource_table_contain_client(_device_table, client,
                                     RESOURCE_FIND_TARGET_MONITOR)) {
    rtd_pr_crm_debug("[crmmgr] register : %s register again %s for %08x\n",
            _resource_client_get_name(client),
    ...
    rtd_pr_crm_debug("[crmmgr] register : %s register first %s for %08x\n",
            _resource_client_get_name(client),
            _codec_resource_to_string(resource), unique_id);
  }

  _codec_resource_get_providers(resource, unique_id, false, providers,
                                &provider_count);

  /* if there are no providers that found in resource */
  if (provider_count == 0) {
    rtd_pr_crm_debug("[crmmgr] register : %s register invalid\n",
           _resource_client_get_name(client));
  ...
  for (i = 0; i < provider_count; i++) {
    ...
    if (_resource_provider_contain_unique_id(provider, unique_id, records,
      ...
      rtd_pr_crm_err("[crmmgr] register : %s does not has (%s)\n",
             _resource_client_get_name(target_client),
             _resource_provider_get_name(provider));
      req->status = RTK_CRMDEV_STATUS_INVALID_RESOURCE;
    }
...
static int _resource_service_clear_lock_table(resource_client_t *client,
                                              rtk_crmdev_request *req) {
  rtk_crmdev_codec_resource_t *resource = &req->data.clear_resource.resource;
  resource_provider_t *providers[MAX_PROVIDER_COUNT];
  size_t provider_count = 0;
  size_t i = 0;
  int ret = 0;

  rtd_pr_crm_err("[crmmgr] clear : %s clear %s\n",
    _resource_client_get_name(client), _codec_resource_to_string(resource));

  _codec_resource_get_providers(resource, 0, true, providers, &provider_count);

  /* if there are no providers that found in resource */
  if (provider_count == 0) {
    rtd_pr_crm_err("[crmmgr] clear : %s clear invalid\n",
        _resource_client_get_name(client));
    req->status = RTK_CRMDEV_STATUS_INVALID_RESOURCE;
    return 0;
  }
  ...
  for (i = 0; i < provider_count; i++) {
    ...
    ret = _resource_provider_clear(provider);
    if (ret != 0) {
      rtd_pr_crm_err("[crmmgr] clear : %s clear %s error\n",
        _resource_client_get_name(client),  _resource_provider_get_name(provider));
      req->status = RTK_CRMDEV_STATUS_INVALID_RESOURCE;
    }
  }
...
static long _rtk_crmmgr_ioctl_reset(resource_client_t *client,
  ...
  if (count == 1) {
    ...
    rtd_pr_crm_err("[crmmgr] init     : table reset success\n");
    req->status = RTK_CRMDEV_STATUS_OK;
  } else {
    rtd_pr_crm_err("[crmmgr] init     : table reset failure, use by %u instance\n",
           count);
...
static long _rtk_crmmgr_ioctl_wait_event(resource_client_t *client,
  ...
  rtd_pr_crm_debug("[crmmgr] events   : %s wait event\n",
          _resource_client_get_name(client));
  ...
  if (ret) {
    if (signal_pending(current)) {
      rtd_pr_crm_warn("[crmmgr] events   : %s caught pending signal when wait\n",
                 _resource_client_get_name(client));
      return -EINTR;
    }
    return -ERESTARTSYS;
  ...
  rtd_pr_crm_err("[crmmgr] events   : %s process %s\n",
          _resource_client_get_name(client),
          _resource_client_get_state_name(client));
...
static long _rtk_crmmgr_ioctl_release_resource(resource_client_t *client,
                                               rtk_crmdev_request *req) {
  rtd_pr_crm_err("[crmmgr] release  : %s\n", _resource_client_get_name(client));
...
static long _rtk_crmmgr_ioctl_acquire_resource(resource_client_t *client,
                                               rtk_crmdev_request *req) {
  int ret = 0;
  rtd_pr_crm_err("[crmmgr] acquire  : %s\n", _resource_client_get_name(client));
...
static long _rtk_crmmgr_ioctl_register_resource(resource_client_t *client,
                                                rtk_crmdev_request *req) {
  int ret = 0;
  rtd_pr_crm_debug("[crmmgr] register : %s\n", _resource_client_get_name(client));
...
static long _rtk_crmmgr_ioctl_clear_resource(resource_client_t *client,
                                             rtk_crmdev_request *req) {
  int ret = 0;
  rtd_pr_crm_err("[crmmgr] clear : %s\n", _resource_client_get_name(client));

  mutex_lock(&_table_lock);
  ret = _resource_service_clear_lock_table(client, req);
  mutex_unlock(&_table_lock);
...
static long _rtk_crmmgr_ioctl_check_support(resource_client_t *client,
                                            rtk_crmdev_request *req) {
  rtd_pr_crm_err("[crmmgr] support  : %s\n", _resource_client_get_name(client));
...
static long _rtk_crmmgr_ioctl_check_capability(resource_client_t *client,
                                            rtk_crmdev_request *req) {
  rtd_pr_crm_err("[crmmgr] capability  : %s\n", _resource_client_get_name(client));
...
static long _rtk_crmmgr_ioctl_set_name(resource_client_t *client,
  ...
  rtd_pr_crm_debug("[crmmgr] create   : %s %p\n", client->name, client);
...
static long _rtk_crmmgr_ioctl_cancel_event(resource_client_t *client,
                                           rtk_crmdev_request *req) {
  rtd_pr_crm_debug("[crmmgr] events   : %s cancel event\n",
          _resource_client_get_name(client));
...
static long _rtk_crmmgr_ioctl_response_reclaim(resource_client_t *client,
  ...
  if (req->data.response_reclaim.accept) {
    rtd_pr_crm_err("[crmmgr] events   : %s response reclaim accept\n",
            _resource_client_get_name(client));
    _resource_client_wake_event(client, RESOURCE_CLIENT_STATUS_RECLAIM_ACCEPT);
  } else {
    rtd_pr_crm_err("[crmmgr] events   : %s response reclaim reject\n",
            _resource_client_get_name(client));
...
static long _rtk_crmmgr_ioctl_response_priority(resource_client_t *client,
                                                rtk_crmdev_request *req) {
  rtd_pr_crm_debug("[crmmgr] events   : %s response priority\n",
          _resource_client_get_name(client));
...
static long _rtk_crmmgr_ioctl_unknown(resource_client_t *client,
  ...
  rtd_pr_crm_err("[crmmgr] ioctl    : invalid command %d\n", req->cmd);
  return 0;
}
  ...
  if (cmd == RTK_CRMMGR_IOC_REQUEST) {
    ...
    switch (req.cmd) {
      ...
      case RTK_CRMDEV_CMD_REGISTER_RESOURCE:
        ...
        break;
      case RTK_CRMDEV_CMD_CLEAR_RESOURCE:
        ret = _rtk_crmmgr_ioctl_clear_resource(client, &req);
        break;
    ...
    if (!ret) {
      ...
      rtd_pr_crm_warn("[crmmgr] ioctl    : %s command %d, ret %ld\n",
...
static const struct proc_ops rtk_crmmgr_proc_status_fops = {
    .proc_open = rtk_crmmgr_proc_status_open,
    .proc_read = seq_read,
...
static int rtk_crmmgr_proc_init(void) {
  if (_proc_dir == NULL && _proc_entry == NULL) {
    ...
    if (_proc_dir == NULL) {
      rtd_pr_crm_err("[crmmgr] failure to create proc dir (%s)\n", RTK_CRMMGR_PROC_DIR);
      goto error_proc_mkdir;
    }
    _proc_entry =
        proc_create("status", 0666, _proc_dir, &rtk_crmmgr_proc_status_fops);
    if (_proc_entry == NULL) {
      rtd_pr_crm_err("[crmmgr] failure to create proc log entry\n");
...
#ifdef CONFIG_COMPAT
static long _rtk_crmmgr_compat_ioctl(struct file *filp, unsigned int cmd,  unsigned long arg)
{
    long ret = 0;
    ret = _rtk_crmmgr_unlocked_ioctl(filp,cmd,arg);
    return ret;
}
#endif
...
static const struct file_operations rtk_crmmgr_fops = {
    ...
    #ifdef CONFIG_COMPAT
        .compat_ioctl = _rtk_crmmgr_compat_ioctl,
    #endif
};
...
static int __init rtk_crmmgr_init(void) {
  int ret = 0;
  rtd_pr_crm_debug("[crmmgr] init\n");
  _device_table = kzalloc(sizeof(*_device_table), GFP_KERNEL);
  if (!_device_table) {
    rtd_pr_crm_err("[crmmgr] failure to allocate resource table\n");
    return -ENOMEM;
  }
  ret = misc_register(&rtk_crmmgr_miscdev);
  if (ret) {
    rtd_pr_crm_err("[crmmgr] failure to misc_register\n");
    return ret;
  }
  ret = rtk_crmmgr_proc_init();
  if (ret) {
    rtd_pr_crm_err("[crmmgr] failure to rtk_crmmgr_proc_init\n");
    return ret;
  }
  return ret;
}
