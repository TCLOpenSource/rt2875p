...
unsigned long carvedout_buf[MM_LAYOUT_CARVEDOUT_NUM][CARVEDOUT_NUM][2] = {
  {
	...
	{0x1a500000, (10+1)*_MB_},                          // CARVEDOUT_AV_OS. [NUMDEV-302] handle HDCP in Video FW. enlarge memory size for fw
...
//#endif
		...
		if (carvedout[1]) {
			if ((carvedout[1] != (unsigned long)size) || (carvedout[0] != (unsigned long)r.start)) {
				rtd_pr_rmm_err("%s%s (0x%08x/0x%08x)vs(0x%08x/0x%08x) ERROR Mismatched \n",
