...
//	{0x12280000,  0*_MB_/*17920*_KB_*/},			// CARVEDOUT_VDEC_COMEM
	...
	{0x1a900000, 7*_MB_},							// CARVEDOUT_V_OS
...
//	{AUDIO_RINGBUFFER_START,  AUDIO_RINGBUFFER_SIZE}, // CARVEDOUT_AUDIO_RINGBUF
  ...
  // MM_LAYOUT_1G5    for DIAS advanced 1.5GB
  {
	{0x00000000,  1*_MB_},							// CARVEDOUT_BOOTCODE
...
//	{DDR_1G5_AUDIO_RINGBUFFER_START,  DDR_1G5_AUDIO_RINGBUFFER_SIZE}, // CARVEDOUT_AUDIO_RINGBUF
  ...
  // MM_LAYOUT_1GB    for DIAS entry 1GB
  {
	{0x00000000,  1*_MB_},							// CARVEDOUT_BOOTCODE
	{DEMOD_CARVED_OUT,  0*_MB_},					// CARVEDOUT_DEMOD
	{0x10000000,  0*_MB_},							// CARVEDOUT_AV_DMEM
...
//	{DDR_1GB_AUDIO_RINGBUFFER_START,  DDR_1GB_AUDIO_RINGBUFFER_SIZE}, // CARVEDOUT_AUDIO_RINGBUF
  ...
  // MM_LAYOUT_2K_MODEL    for TV 2K 1.5GB
  {
	{0x00000000,  1*_MB_},							// CARVEDOUT_BOOTCODE
	{DEMOD_CARVED_OUT,  8*_MB_},					// CARVEDOUT_DEMOD
	{0x10000000,  0*_MB_},							// CARVEDOUT_AV_DMEM
...
unsigned int carvedout_buf_get_layout_idx(void)
{
	...
	if (get_product_type() == PRODUCT_TYPE_DIAS) {
		if (DRAM_size >= 0x60000) { //(get_memory_size(GFP_DCU1) + get_memory_size(GFP_DCU2))
...
unsigned long carvedout_buf_query(carvedout_buf_t what, void **addr)
{
	...
	case CARVEDOUT_GPU_RESERVED:
		...
		} else if (layout_idx == MM_LAYOUT_1G5) {
			if (DRAM_size == 0x80000)
				size = CMA_HIGHMEM_DIAS_2GB;
			else
				size = CMA_HIGHMEM_DIAS_1G5;
		} else if (layout_idx == MM_LAYOUT_1GB) {
			size = CMA_HIGHMEM_DIAS_1GB;
		} else if (layout_idx == MM_LAYOUT_2K_MODEL) {
			size = CMA_HIGHMEM_TV_2K;
		} else {
...
void __init rtk_reserve(void)
{
	...
	rtd_pr_rmm_info("[MEM] rtd2851f rtk_reserve, DRAM_size = 0x%x, layout_idx = 0x%x\n", (unsigned int)DRAM_size, layout_idx);
