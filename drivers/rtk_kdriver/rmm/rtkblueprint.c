...
int add_ban_info(unsigned long addr, unsigned long size, int type)
{
	...
	if (ban_info.nr_regions >= NR_REGIONS) {
		rtd_pr_rmm_err("rtkbp: NR_REGIONS too low, ignoring memory at 0x%08lx\n", addr);
		return -EINVAL;
	}
	...
	if (type == BAN_NOT_USED) {
		if ((addr != ALIGN(addr, PAGE_SIZE << pageblock_order)) || 
			...
			rtd_pr_rmm_err("#########################################################\n");
			rtd_pr_rmm_err("rtkbp: %s addr: %08lx size: %08lx\n", __FUNCTION__, addr, size);
			rtd_pr_rmm_err("rtkbp: addr & size should be aligned to pageblock size...\n");
			rtd_pr_rmm_err("#########################################################\n");
			return -EINVAL;
		}
#ifndef CONFIG_REALTEK_MEMORY_MANAGEMENT_MODULE
		if (memblock_reserve(addr, size) < 0) {
			rtd_pr_rmm_err("rtkbp: can not ban region %08lx - %08lx\n", addr, addr + size);
			BUG();
		}
#endif
	}
...
//		rtd_pr_rmm_info("rtkbp: bad index: %d \n", unit - bp->rtk_mem_map);
		return 1;
	}
}
...
//	rtd_pr_rmm_info(" @@ 1unit: %p index: %lu \n", unit, unit_idx);
...
void init_rtkbp(struct mem_bp *bp, unsigned long base_pfn, int count)
{
	...
	while ((count -= 256) >= 0) {
		...
		if (memblock_is_region_reserved((base_pfn + count) << PAGE_SHIFT, 256 << PAGE_SHIFT))
			...
			rtd_pr_rmm_info("rtkbp: memory addr: %08lx size: %08x is not reserved by memblock...\n", 
...
got:
	rtd_pr_rmm_info("+++ ALLOC pfn: %lu order: %d \n", bp->base_pfn + (unit - bp->rtk_mem_map), order);
	rtd_pr_rmm_info("rtkbp: get physical address %08lx @ %08lx \n", 
...
{
	rtd_pr_rmm_info("--- FREE pfn: %lu order: %d \n", bp->base_pfn + pageno, order);
...
void show_rtkbp(struct mem_bp *bp)
{
	...
	if (bp == NULL) {
		rtd_pr_rmm_err("\tNo BP info here...\n");
		return;
	}

	rtd_pr_rmm_warn("\t<<<FREE AREA BASE: %08lx>>>\n", bp->base_pfn << PAGE_SHIFT);
    spin_lock_irq(&rtkbp_lock);
	for (idx = 0; idx < RTK_BP_MAX_ORDER; idx++) {
		...
		list_for_each(up, &bp->rtk_free_area[idx].free_list) {
			...
			rtd_pr_rmm_warn("\t\tAddr: %08lx, order: %d \n",
