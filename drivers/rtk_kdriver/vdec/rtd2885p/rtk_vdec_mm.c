...
int rtkvdec_vbm_alloc_by_chunk(int size, bool from_luma)
{
	...
	if ( from_luma )
		dmabuf_Ptr = dvr_dmabuf_chunk_malloc(size, VBM_BUF_DECOMP, DMA_HEAP_USER_VIDEO_4K_LUMA, false, (unsigned long **)&phy_addr);
	else
...
int rtkvdec_vbm_alloc_by_frame(int NumofUse)
{
	...
	unsigned int check, type, LuSize, ChSize, hdrsize, cnt;
	int i, get_Lucnt, get_Chcnt;
	bool bProt = false;
	...
	if ( type == VBM_REQ_TYPE_4K ) {
		...
		dmabuf_LuPtr = dvr_dmabuf_frames_malloc(LuSize + ChSize, cnt, VBM_BUF_DECOMP, DMA_HEAP_USER_VIDEO_SUB_2K, bProt, (unsigned long **)&first_phy_addr);
		if ( IS_ERR_OR_NULL(dmabuf_LuPtr) ) {
			rtd_pr_vdec_err("[%s %d] alloc dmabuf fail\n", __func__, __LINE__);
			goto ERR_CASE_LUMA_FAIL;
		}
...
{
	return VBM_start + 450 * 1024 * 1024;
}


int rtkvdec_borrow_ve_sram (void)
{
	if ( 0 != Scaler_Set_DI_SNR_Borrow_Sram_Mode(SNR_DI_Sram_VE_USE) ) {
		rtd_pr_vdec_err("[%s %d] get fail!!!\n", __func__, __LINE__);
		return 1; /* fail */
	}
	else {
		rtd_pr_vdec_debug("[%s %d] get success\n", __func__, __LINE__);
		return 0;
	}
}


int rtkvdec_return_ve_sram (void)
{
	Scaler_Set_DI_SNR_Borrow_Sram_Mode(SNR_DI_Sram_Release_From_VE);
	rtd_pr_vdec_debug("[%s %d] return\n", __func__, __LINE__);
	return 0;
}
...
//extern void vbm_heap_dump_chunk_list(void);
void rtkvdec_vbm_by_frame_testcase3 (void)
{
	unsigned long tmpAddr = 0;

	rtd_pr_vdec_err("allocate 4K main start\n");
	rtkvdec_vbm_by_frame_testcase_alloc(0, VBM_REQ_TYPE_4K, 2560*2176, 0x4C800, 13);
	rtd_pr_vdec_err("allocate 2K sub start\n");
	rtkvdec_vbm_by_frame_testcase_alloc(1, VBM_REQ_TYPE_2K, 1280*1152, 0x8700 ,  9);
	rtd_pr_vdec_err("allocate 2K decimate start\n");
	tmpAddr = rtkvdec_vbm_alloc_by_chunk(35*1024*1024, false);

	//vbm_heap_dump_chunk_list();

	rtd_pr_vdec_err("free 2K decimate start\n");
	rtkvdec_vbm_free_by_chunk(tmpAddr);
	rtd_pr_vdec_err("free 2K sub start\n");
	rtkvdec_vbm_by_frame_testcase_free(1);

	rtd_pr_vdec_err("allocate co-buffer start\n");
	tmpAddr = rtkvdec_vbm_alloc_by_chunk(48*1024*1024, false);

	//vbm_heap_dump_chunk_list();

	rtd_pr_vdec_err("free co-buffer start\n");
	rtkvdec_vbm_free_by_chunk(tmpAddr);
	...
	//vbm_heap_dump_chunk_list();
	rtd_pr_vdec_err("testcase3 done\n");
}


void rtkvdec_vbm_by_frame_testcase4 (void)
{
	unsigned long tmpAddr1 = 0;
	unsigned long tmpAddr2 = 0;

	rtd_pr_vdec_err("allocate 2K main start\n");
	rtkvdec_vbm_by_frame_testcase_alloc(0, VBM_REQ_TYPE_2K, 1280*1152, 0x8700 , 13);
	rtd_pr_vdec_err("allocate 4K sub start\n");
	rtkvdec_vbm_by_frame_testcase_alloc(1, VBM_REQ_TYPE_4K, 2560*2176, 0x4C800,  9);
	rtd_pr_vdec_err("allocate 4K decimate start\n");
	tmpAddr1 = rtkvdec_vbm_alloc_by_chunk(35*1024*1024, true);

	//vbm_heap_dump_chunk_list();

	rtd_pr_vdec_err("free 2K main start\n");
	rtkvdec_vbm_by_frame_testcase_free(0);

	rtd_pr_vdec_err("allocate co-buffer start\n");
	tmpAddr2 = rtkvdec_vbm_alloc_by_chunk(48*1024*1024, false);

	//vbm_heap_dump_chunk_list();

	rtd_pr_vdec_err("free co-buffer start\n");
	rtkvdec_vbm_free_by_chunk(tmpAddr2);

	rtd_pr_vdec_err("free 4K decimate start\n");
	rtkvdec_vbm_free_by_chunk(tmpAddr1);
	rtd_pr_vdec_err("free 4K start\n");
	rtkvdec_vbm_by_frame_testcase_free(1);

	//vbm_heap_dump_chunk_list();
	rtd_pr_vdec_err("testcase4 done\n");
}
#endif
