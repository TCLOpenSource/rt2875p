...
void rtkvdec_fw_remote_malloc_summary(ret_info_t *info) {
	...
	list_for_each(list, &vfw_remote_malloc_info_specific_head) {
		...
		if (mem_info->dcu == RTKVDEC_DCU2) {
			rtd_pr_vdec_notice("phy_addr %lx, alloc_size %lx\n", mem_info->phy_addr, mem_info->malloc_size);
		}
	}
	rtd_pr_vdec_notice("\nDCU2_FIRST:\n");
	list_for_each(list, &vfw_remote_malloc_info_specific_head) {
...
unsigned long rtkvdec_remote_malloc(unsigned long para1, unsigned long para2)
{
	...
	if (para1 & 0x80000000) {
		...
		if (VBM_total >= size) {
			for (i = 0; i < RTKVDEC_MAX_VBM_MEM_NUM; i++) {
				if ( VBM_Memory[i].used == 0 ) {
					...
					VBM_Memory[i].used = 1 ;
#ifdef CONFIG_OPTEE_SECURE_SVP_PROTECTION
					VBM_Memory[i].aligned_addr = ROUNDUP(VBM_Memory[i].addr, 6 * 1024);
					VBM_Memory[i].aligned_size = ROUNDDOWN(VBM_Memory[i].size, 6 * 1024);
#endif
					VBM_total -= size ;
					return phy_addr ;
				}
		...
		if (!phy_addr) {
			...
			rtd_pr_vdec_err("\nVBM chunk:\n");
			for (i = 0; i < RTKVDEC_MAX_VBM_MEM_NUM; i++) {
				rtd_pr_vdec_err("VBM chunk[%d]%d %lx %x\n", i, VBM_Memory[i].used, VBM_Memory[i].addr, VBM_Memory[i].size);
			}
			BUG();
			return 0;
		}
...
#endif
			if (phy_addr == INVALID_VAL) {
				rtd_pr_vdec_warn("[%s %d]alloc DCU2 memory fail\n", __func__, __LINE__);
				return 0;
			}
			DCU = RTKVDEC_DCU2 ;
		}
...
void rtkvdec_dump_videofw_data(void)
{
	...
	filePtr = file_open("/tmp/var/log/video_fw_data.bin", O_TRUNC | O_RDWR | O_CREAT, 0666) ;
#else
	filePtr = file_open("/data/data/video_fw_data.bin", O_TRUNC | O_RDWR | O_CREAT, 0666) ;
#endif
	if (filePtr == 0) {
		rtd_pr_vdec_err("[%s %d]open file fail\n",__FUNCTION__,__LINE__);
		return ;
	}

	cnt = (end_addr - start_addr) >> 4 ;
	for (i = 0 ; i < cnt; i++) {
		rtkvdec_memset(LogBuf, 0, 128);
	...
	filePtr = 0 ;
#ifndef CONFIG_ANDROID
	rtd_pr_vdec_notice("rtkvdec_dump_videofw_data: check /tmp/var/log/video_fw_data.bin\n") ;
#else
	rtd_pr_vdec_notice("rtkvdec_dump_videofw_data: check /data/data/video_fw_data.bin\n") ;
#endif
}
...
void rtkvdec_dump_esbuf(void)
{
	...
	for (i = 0; i < RTKVDEC_MAX_SVPMEM_NUM; i++) {
		rtkvdec_memset((void *)&ringBuf_H, 0, sizeof(RINGBUFFER_HEADER));
		if (svp_Memory[i].pid != 0 && svp_Memory[i].used != 0) {
			...
			if (ringBuf_H.beginAddr == 0) {
				rtd_pr_vdec_err("[%s %d] beginAddr is NULL!!\n",__FUNCTION__,__LINE__);
				continue;
			}

			if (i == 0) filePtr = file_open("/tmp/var/log/video_esbuf1.bin", O_TRUNC | O_RDWR | O_CREAT,0666) ;
			else        filePtr = file_open("/tmp/var/log/video_esbuf2.bin", O_TRUNC | O_RDWR | O_CREAT,0666) ;

			if (filePtr == 0) {
				rtd_pr_vdec_err("[%s %d]open %d file fail\n",__FUNCTION__,__LINE__, i);
				return ;
			}
			...
			rtd_pr_vdec_notice("Ptr w(%lx) r(%lx) b(%lx) l(%lx)\n", (unsigned long)wrPtr, (unsigned long)rdPtr, (unsigned long)basePtr, (unsigned long)limitPtr);
			...
			filePtr = 0 ;
			f_offset = 0 ;
			rtd_pr_vdec_notice("rtkvdec_dump_esbuf: check /tmp/var/log/video_esbuf%d.bin\n", i+1) ;
		}
	}
}
...
static int rtkvdec_debug_check_thread(void *arg)
{
	...
	for (;;) {
		...
		if (SharedMemAddr && !vcpu1_is_crash) {
			rtkvdec_memcpy((void *)&show_shmem, (void *)SharedMemAddr, sizeof(VDEC_SHARE_MEM_STRUCT));
...
static int rtkvdec_vcreate_thread(void *arg)
{
	...
	for (;;) {
		ret = send_rpc_command(RPC_VIDEO, RPCCMD_VIDEO_CREATE, (unsigned long)init_addr, 0, &return_value) ;
	...
	for (;;) {
		ret = send_rpc_command(RPC_VIDEO2, RPCCMD_VIDEO_CREATE, (unsigned long)init_addr, 0, &return_value) ;
...
void rtkvdec_send_sharedmemory()
{
	...
	rtkvdec_memset((void *)SharedMemAddr, '\0', sizeof(VDEC_SHARE_MEM_STRUCT));
...
void rtkvdec_show_sharedmemory(void)
{
	...
	rtd_pr_vdec_err("rtkvdec_user_alloc_summary %d %ld\n", rtkdev_free_flow_check, vcpu1_is_crash);

	if (!SharedMemAddr) return ;

	rtkvdec_memcpy((void *)&show_shmem, (void *)SharedMemAddr, sizeof(VDEC_SHARE_MEM_STRUCT));

	rtd_pr_vdec_err("record video fw isr :\n");
	rtd_pr_vdec_err("last %d\n", ntohl(show_shmem.now));
	for (i = 0; i < 10; i++) {
		rtd_pr_vdec_err("%d: v%d %x/%x, %x/%x, %x/%x/%x\n"
			, i, ntohl(show_shmem.isr_vector[i]), ntohl(show_shmem.isr[i]), ntohl(show_shmem.isr_done[i])
			, ntohl(show_shmem.isr_in_time[i]), ntohl(show_shmem.isr_out_time[i])
			, ntohl(show_shmem.epc[i]), ntohl(show_shmem.isr_in_sb2[i]), ntohl(show_shmem.isr_out_sb2[i]));
	}
}
...
void rtkvdec_show_calltrace(void)
{
	...
	/* rtd_pr_vdec_notice("rtkvdec_show_calltrace %x %x\n", show_shmem->show_ct_no_crash, *((volatile unsigned int *) SB2_INV_INTSTAT_reg)); */
}
...
void rtkvdec_user_alloc_summary(struct ret_info *info)
{
	...
	down(&rtkvdec_alloc_sem);
	rtd_pr_vdec_notice("rtkvdec_user_alloc_summary %d\n", rtkdev_free_flow_check);
	list_for_each_entry(buf_info, &rtkvdec_alloc_head, buffer_list)
	{
...
void rtkvdec_free_svpmem(unsigned int addr)
{
	int i, svp_free_size = 0;
...
long rtkvdec_alloc_svpmem(VDEC_SVPMEM_STRUCT *svpmem, bool secured) {
	int i;
	...
	switch (cmd) {
		case VDEC_IOC_INIT:
		{
			...
			if (!VideoCreatDone) {
				...
				if ((ret = send_rpc_command(RPC_VIDEO, RPCCMD_VIDEO_CREATE, (unsigned long)init_addr, 0, &return_value))) {
					rtd_pr_vdec_debug("rtkvdec: RPCCMD_VIDEO_CREATE V1 fail %lx\n", ret);
				}
				else {
					VideoCreatDone = 1 ;
					rtd_pr_vdec_debug("rtkvdec: RPCCMD_VIDEO_CREATE V1 success %lx\n", ret);

#ifdef SUPPORT_VCPU2_VERSION
					init->boardType = htonl(0) ;
					if ((ret = send_rpc_command(RPC_VIDEO2, RPCCMD_VIDEO_CREATE, (unsigned long)init_addr, 0, &return_value))) {
						rtd_pr_vdec_debug("rtkvdec: RPCCMD_VIDEO_CREATE V2 fail %lx\n", ret);
					}
					else {
						VideoCreatDone2 = 1 ;
						rtd_pr_vdec_debug("rtkvdec: RPCCMD_VIDEO_CREATE V2 success %lx\n", ret);
					}
#endif
				}
				...
				p = dma_alloc_coherent(rtkvdec_device, 4096, &dat, GFP_KERNEL);
				if (!p) {
					rtd_pr_vdec_debug("[%s %d]alloc INIT memory fail\n",__FUNCTION__,__LINE__);
					return -ENOMEM;
				}
				...
				rtd_pr_vdec_debug("rtkvdec: video create already done!\n");
			}
		...
		{
			/* rtd_pr_vdec_debug("rtkvdec: deinit\n"); */
			break;
		}

		case VDEC_IOC_OPEN:
		{
			/* rtd_pr_vdec_debug("rtkvdec: open\n"); */
			VDEC_FLOW_CMD_T vdec_cmd ;
			if (copy_from_user((void *)&vdec_cmd, (const void __user *)arg, sizeof(vdec_cmd))) {
				ret = -EFAULT;
				rtd_pr_vdec_err("rtkvdec ioctl code=VDEC_IOC_OPEN failed!!!!!!!!!!!!!!!\n");
				goto out;
			}
			/*
			send_rpc_command
		...
		case VDEC_IOC_CLOSE:
		{
			/* rtd_pr_vdec_debug("rtkvdec: close\n"); */
			VDEC_FLOW_CMD_T vdec_cmd ;
			if (copy_from_user((void *)&vdec_cmd, (const void __user *)arg, sizeof(vdec_cmd))) {
				ret = -EFAULT;
				rtd_pr_vdec_err("rtkvdec ioctl code=VDEC_IOC_CLOSE failed!!!!!!!!!!!!!!!\n");
				goto out;
			}
			/*
			send_rpc_command
		...
		case VDEC_IOC_PLAY:
		{
			/* rtd_pr_vdec_debug("rtkvdec: play\n"); */
			VDEC_FLOW_CMD_T vdec_cmd ;
			if (copy_from_user((void *)&vdec_cmd, (const void __user *)arg, sizeof(vdec_cmd))) {
				ret = -EFAULT;
				rtd_pr_vdec_err("rtkvdec ioctl code=VDEC_IOC_PALY failed!!!!!!!!!!!!!!!\n");
				goto out;
			}
			/*
			send_rpc_command
		...
		case VDEC_IOC_PAUSE:
		{
			/* rtd_pr_vdec_debug("*****************  rtkvdec pause  *********************\n"); */
			break;
		}

		case VDEC_IOC_FLUSH:
		{
			/* rtd_pr_vdec_debug("rtkvdec: flush\n"); */
			VDEC_FLOW_CMD_T vdec_cmd ;
			if (copy_from_user((void *)&vdec_cmd, (const void __user *)arg, sizeof(vdec_cmd))) {
				ret = -EFAULT;
				rtd_pr_vdec_err("rtkvdec ioctl code=VDEC_IOC_FLUSH failed!!!!!!!!!!!!!!!\n");
				goto out;
			}
			/*
			send_rpc_command
		...
		case VDEC_IOC_STOP:
		{
			/* rtd_pr_vdec_debug("rtkvdec: stop\n"); */
			VDEC_FLOW_CMD_T vdec_cmd ;
			if (copy_from_user((void *)&vdec_cmd, (const void __user *)arg, sizeof(vdec_cmd))) {
				ret = -EFAULT;
				rtd_pr_vdec_err("rtkvdec ioctl code=VDEC_IOC_STOP failed!!!!!!!!!!!!!!!\n");
				goto out;
			}
			/*
			send_rpc_command
		...
		{
			/* rtd_pr_vdec_debug("rtkvdec: connect\n"); */
			VDEC_FLOW_CMD_T vdec_cmd ;
			if (copy_from_user((void *)&vdec_cmd, (const void __user *)arg, sizeof(vdec_cmd))) {
				ret = -EFAULT;
				rtd_pr_vdec_err("rtkvdec ioctl code=VDEC_IOC_CONNECT failed!!!!!!!!!!!!!!!\n");
				goto out;
			}
			/*
			send_rpc_command
		...
		case VDEC_IOC_SET:
		{
			/* rtd_pr_vdec_debug("rtkvdec: set\n"); */
			VDEC_SET_CMD_T vdec_cmd;
			if (copy_from_user((void *)&vdec_cmd, (const void __user *)arg, sizeof(vdec_cmd))) {
				ret = -EFAULT;
				rtd_pr_vdec_err("rtkvdec ioctl code=VDEC_IOC_SET failed!!!!!!!!!!!!!!!\n");
				goto out;
			}
			/*
			send_rpc_command
		...
		case VDEC_IOC_ALLOCSVPM:
		{
			...
			ret = rtkvdec_alloc_svpmem(&svpmem, cmd == VDEC_IOC_ALLOCSVPMP);
			if (ret != 0) {
				break;
			}
