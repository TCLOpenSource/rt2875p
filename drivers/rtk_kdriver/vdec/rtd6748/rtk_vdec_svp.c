...
#ifndef AI_INCLUDE_PREFETCH_BARRIER
////////////////////////////////////////////////////////////////////////////////
...
#endif //AI_INCLUDE_PREFETCH_BARRIER
...
int rtkvdec_svp_cma_memory_init(unsigned long phys_addr,unsigned int size )
{
	...
	rtd_pr_vdec_err("remove %lx, %x va=%lx\n", phys_addr, size, phys_to_virt(phys_addr));
...
static unsigned int  cma_region_start = 0;
static unsigned int  cma_region_size = 0;

int rtkvdec_svp_hdmi_cma_region_record (unsigned int addr, unsigned int size)
{
	cma_region_start = addr;
	cma_region_size = size;	
	return 0;
}	


EXPORT_SYMBOL(rtkvdec_svp_hdmi_cma_region_record);

static int rtkvdec_svp_check_hdmi_cma_region (unsigned int addr, unsigned int size)
{
	if(cma_region_start == addr && cma_region_size == size)	
		return 1;
	else 
		return 0;
}	
...
int rtkvdec_svp_enable_protection(unsigned int addr, unsigned int size,
	...
	if (type == TYPE_SVP_PROTECT_CPB)
		...
		if(rtkvdec_svp_check_hdmi_cma_region(addr, size)){
			rtkvdec_svp_cma_memory_init((unsigned long) addr, size);
		}
#endif
    } else if (type == TYPE_SVP_PROTECT_DISPLAY){
        arg.func = MEMPRT_CMD_OpenDisplayProtectedBufferScaler;
...
int rtkvdec_svp_disable_protection(unsigned int addr, unsigned int size,
	...
	if (type == TYPE_SVP_PROTECT_CPB)
        ...
        arg.func = MEMPRT_CMD_CloseDisplayProtectedBufferScaler;
	} else if (type == TYPE_SVP_PROTECT_VBM) {
