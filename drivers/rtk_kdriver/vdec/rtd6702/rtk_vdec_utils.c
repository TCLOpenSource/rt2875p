...
/* for debug : vcpu2 shared memory */
unsigned long SharedMemUNVirtAddr_V3 ;
unsigned long SharedMemPhyAddr_V3 ;
unsigned long SharedMemVirtAddr_V3 ;
#endif
...
#ifdef CONFIG_RTK_KDRV_SUPPORT_VCPU3
	SharedMemVirtAddr_V3 = (unsigned long)dvr_malloc_uncached_specific(sizeof(VDEC_SHARE_MEM_STRUCT), GFP_DCU1_LIMIT, (void **)(&SharedMemUNVirtAddr_V3));
	if (!SharedMemVirtAddr_V3) {
		rtd_pr_vdec_err("[%s %d]alloc sharedmemory fail\n",__FUNCTION__,__LINE__);
		return;
	}
	SharedMemPhyAddr_V3 = (unsigned long)dvr_to_phys((void *)SharedMemVirtAddr_V3);

	rtkvdec_memset((void *)SharedMemUNVirtAddr_V3, '\0', sizeof(VDEC_SHARE_MEM_STRUCT));

	rtd_pr_vdec_notice("rtkvdec: sharedmemory %lx %lx %lx\n", SharedMemVirtAddr_V3, SharedMemUNVirtAddr_V3, SharedMemPhyAddr_V3);

	if ( send_rpc_command(RPC_VIDEO3, RPC_VCPU_SET_SHARED_MEMORY, SharedMemPhyAddr_V3, 0, &return_value) )
		rtd_pr_vdec_debug("rtkvdec: RPC_VCPU_SET_SHARED_MEMORY fail %ld\n", return_value);
#endif
...
#ifdef CONFIG_RTK_KDRV_SUPPORT_VCPU3
	if ( !SharedMemUNVirtAddr_V3 ) goto FREE_MEMORY ;

	rtkvdec_memcpy((void *)gpShmem, (void *)SharedMemUNVirtAddr_V3, sizeof(VDEC_SHARE_MEM_STRUCT));

	rtd_pr_vdec_err("record VCPU3 isr :\n");
	rtd_pr_vdec_err("last %d\n", ntohl(gpShmem->now));
	for (i = 0; i < 10; i++) {
		rtd_pr_vdec_err("%d: v%d %x/%x, %x/%x, %x/%x/%x\n"
			, i, ntohl(gpShmem->isr_vector[i]), ntohl(gpShmem->isr[i]), ntohl(gpShmem->isr_done[i])
...
{
	rtd_pr_vdec_debug("rtkvdec_set_command(%lx/%lx)\n", para1, para2);
	if ( para1 == RTKVDEC_STECMD_TYPE_AV18K ) {
		if ( para2 == 1 )
			rtk_qos_set_vdec_codec_type(VDEC_CODEC_TYPE_AV18K);
		else
			rtk_qos_set_vdec_codec_type(VDEC_CODEC_TYPE_NOSET);
	}
	return 0;
}
