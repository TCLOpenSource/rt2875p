...
unsigned long rtkvdec_remote_malloc_specific(unsigned long para1, unsigned long para2)
{
	...
	if (para1 == RTKVDEC_DCU1) {
		...
		if ( para2 >= 1024 * 1024 )
			return rtkvdec_vbm_alloc_by_chunk(para2);
		else
...
static int rtkvdec_vcreate_thread(void *arg)
{
	...
	init_addr = (VIDEO_INIT_DATA *)(dvr_to_phys(p) | 0xa0000000);
	...
	for (;;) {
		ret = send_rpc_command(RPC_VIDEO3, RPCCMD_VIDEO_CREATE, (unsigned long)init_addr, 0, &return_value) ;

		if (!ret && ntohl(init->boardType) == 0) {
			VideoCreatDone3 = 1 ;
			rtd_pr_vdec_debug("rtkvdec: RPCCMD_VIDEO_CREATE V3 success %ld\n", ret);
			break ;
		}
...
#ifdef CONFIG_RTK_KDRV_SUPPORT_VCPU3
	if (send_to & 0x4)
		if (send_rpc_command(RPC_VIDEO3, RPC_VCPU_DEBUG_COMMAND, phy_addr, length, &ret))
			rtd_pr_vdec_err("rtkvdec: V3 debug string(%s) fail %d\n", pattern, (int)ret);
#endif
...
ssize_t rtkvdec_write(struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	...
	if ((cmd_buf[0] == 'f') && (count == 2)) {
		...
		rtkvdec_send_string(&cmd_buf[4], (count-4)-1, 0x7);
		rtd_pr_vdec_notice("\n=======================================\n");
		...
		rtd_pr_vdec_notice("\n=======================================\n");
	} else if ((cmd_buf[0] == 'f') && (cmd_buf[1] == 'w') && (cmd_buf[2] == '3') && (cmd_buf[3] == '@')) {
		rtd_pr_vdec_notice("\n=== rtkvdec: send debug string: =============\n");
		rtkvdec_send_string(&cmd_buf[5], (count-5)-1, 0x4);
		rtd_pr_vdec_notice("\n=======================================\n");
	} else {
		rtd_pr_vdec_notice("%s\n", cmd_buf);
	}
...
#ifdef CONFIG_RTK_KDRV_VDEC_MODULE	
