...
{
    //IQADC_I_VREF_SEL
    rtd_maskl(0xb8033130, 0x77ffffff, 0x88000000);		//<bit31:30>REG_QADC_I_VREF_SEL=0x10(1.5X),<bit27:26>REG_IADC_I_VREF_SEL=0x10(1.5X)    
	...
	case RTK_DEMOD_MODE_ATV:
        ...
        rtd_maskl(0xb8033104, 0xfffffcff, 0x00000300);              //<9:8>IPGA LPF BW = 0x3(37.5M)
		rtd_maskl(0xb8033124, 0xfffffefe, 0x00000101);		        //I_PGA<bit8> power on, <bit0>I_ADC power on
		rtd_maskl(0xb8033128, 0x8ff8ffff, 0x00020000);		        //<bit30:28>REG_QPGA_INPUT_SEL=0x0(No input),<bit18:16>REG_IPGA_INPUT_SEL=0x2(IFD input)
		rtd_maskl(0xb803312c, 0xffeecc00, 0x00000000);	            //Select to Differential, Q single_end<bit20>=0x0(disable), I single_end<bit16>=0(disable), Q_CKIN_Sel<bit13:12>=00, I_CKIN_Sel<bit9:8>=00: From PLL27X_IFADC, reg_qpga_d<bit7:4>=0x0(0dB), reg_ipga_d<bit3:0>=0x0(0dB)
...
/*----------------------------------------------------------------*
* OTP Address : 0xB80380C4 = EFUSE_DATAO_49_reg                   * 
* OTP_QADC_LDO range : <bit27:24>                                 *
*                     <bit27>Qotp_done_flag, <bit26:24>Q_LDO_sel  *
* OTP_IADC_LDO range : <bit23:20>                                 *
*                     <bit27>Iotp_done_flag, <bit26:24>I_LDO_sel  *
*----------------------------------------------------------------*/
int IQAdcLdoSetting(void)
{
	int Iotp=0, Qotp=0;					
    Iotp = ((rtd_inl(EFUSE_DATAO_49_reg)>>23)&0x1);     //check Iotp done flag 
    if(Iotp==1){
	    rtd_maskl(TV_SB1_ANA_IQADC_CTRL7_reg, ~TV_SB1_ANA_IQADC_CTRL7_reg_iadc_ldo_sel_mask, (rtd_inl(EFUSE_DATAO_49_reg)>>20)&0x7);       //IADC_LDO_set
    }else{
    ...
    Qotp = ((rtd_inl(EFUSE_DATAO_49_reg)>>27)&0x1);     //check Qotp done flag 
    if(Qotp==1){
	    rtd_maskl(TV_SB1_ANA_IQADC_CTRL7_reg, ~TV_SB1_ANA_IQADC_CTRL7_reg_qadc_ldo_sel_mask, (rtd_inl(EFUSE_DATAO_49_reg)>>20)&0x70);       //QADC_LDO_set
    }else{
...
int DemodFrontendPathSetting(RTK_DEMOD_MODE mode)
{
	...
	case RTK_DEMOD_MODE_DTMB:
		...
		rtd_maskl(0xb80008b4, ~(0xf << 20), (0x3 << 20));           //Set  GPIO_106 to DEMOD_IF_AGC
	...
	case RTK_DEMOD_MODE_SPEC_AN:
		...
		rtd_maskl(0xb8000820, 0xFF0FFFFF, 0x00600000); 				//GPIO_09, Set pinmux = 0x6(For DEMOD_S_IF_AGC)
	...
	case RTK_DEMOD_MODE_ATV:
		...
		rtd_maskl(0xb80008b4, ~(0xf << 20), (0x5 << 20));           //Set  GPIO_106 to DEMOD_IF_AGC
		break;
