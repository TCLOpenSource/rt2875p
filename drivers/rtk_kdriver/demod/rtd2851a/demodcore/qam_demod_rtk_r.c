...
)
{
	...
	rtd_maskl(0xb8158d9c, ~0xe0, 0xe0);//increase FSR acc num to 31
	rtd_maskl(0xb8158da0, ~0x60, 0x60);
	rtd_maskl(0xb8158dbc, ~0x0f, 0x00);	//[11:8]larger FSR search range half add
	rtd_maskl(0xb8158db8, ~0xff, 0xa0);	//[7:0] FSR search range half add 0x72=500K 0x89=600K 0xe4=1000K 0xcd=900K 0x94=650K 0xa0=700K
	rtd_maskl(0xb8158de8, ~0x0f, 0x00);	//[11:8]larger FSR search range half sub
	rtd_maskl(0xb8158de4, ~0xff, 0xab);	//[7:0] FSR search range half sub 0x72=500K 0x89=600K 0xe4=1000K 0xcd=900K 0x94=650K 0xa0=700K 0xab=750K
	rtd_maskl(0xb815881c, ~0xff, 0xd0);	//0x23e=0x73, [7:0]opt_pos_frq_max_sr[7:0]=0x73
	rtd_maskl(0xb8158820, ~0x0f, 0x0c);
	rtd_maskl(0xb8158824, ~0xff, 0x63);
	rtd_maskl(0xb8158828, ~0x0f, 0x06);
	...
	rtd_maskl(0xb8158da0, ~0x1f, 0x0f); //increase CCFO acc num to 15
	rtd_outl(0xb815845C, 0xa4);//for CR range
	rtd_maskl(0xb8158C14, ~0x01, 0x01); //enable papr ccfo
	//------------------------For Auto mode Qamd  setting-----------------------//
	rtd_maskl(0xb8158898, ~0x0f, 0x05); //opt_qamd_start_state[3:0]
	rtd_maskl(0xb8158C88, ~0xff, 0x3d);//opt_qamd_eqp_zn9_min[7:0] for auto qam match C platform
	rtd_maskl(0xb8158C90, ~0xff, 0x98);//opt_qamd_eqp_zn10_min[7:0]
	...
	//--------------------------Enable erasure decode---------------------------//
	rtd_maskl(0xb8158C40, ~0x01, 0x01);
	rtd_maskl(0xb8158C44, ~0x07, 0x04);
...
)
{
	...
	rtd_maskl(0xb8158D48, ~0x0f, 0x04); //reset csr th
	rtd_maskl(0xb8158dbc, ~0x0f, 0x00); //[11:8]larger FSR search range half add
	rtd_maskl(0xb8158db8, ~0xff, 0xa0); //[7:0] FSR search range half add 0x72=500K 0x89=600K 0xe4=1000K 0xcd=900K 0x94=650K 0xa0=700K
	rtd_maskl(0xb8158de8, ~0x0f, 0x00); //[11:8]larger FSR search range half sub
	rtd_maskl(0xb8158de4, ~0xff, 0xab); //[7:0] FSR search range half sub 0x72=500K 0x89=600K 0xe4=1000K 0xcd=900K 0x94=650K 0xa0=700K 0xab=750K
	...
	while ((tv_osal_time() - stime) < 300)
	{
			...
			else
			{
				...
				*pFSR = fsr_KHz *1000;
				...
				if((low_sr_retry_cnt <= DVBC_AUTUMODE_LOW_SR_CNT) && (csr_KHz >= 3100 && csr_KHz < 4410 && fsr_KHz >= 4499 && fsr_KHz <= 4501))
				{
					...
					rtd_maskl(0xb8158D48, ~0x0f, 0x0a); //encrease th to 10
					rtd_maskl(0xb8158dbc, ~0x0f, 0x00); //[11:8]larger FSR search range half add
					rtd_maskl(0xb8158db8, ~0xff, 0x40); //[7:0] FSR search range half add 0x72=500K 0x89=600K 0xe4=1000K 0xcd=900K 0x94=650K 0xa0=700K
					rtd_maskl(0xb8158de8, ~0x0f, 0x00); //[11:8]larger FSR search range half sub
					rtd_maskl(0xb8158de4, ~0xff, 0x40); //[7:0] FSR search range half sub 0x72=500K 0x89=600K 0xe4=1000K 0xcd=900K 0x94=650K 0xa0=700K 0xab=750K
					goto AutoModeBegin;
				}
			}
		}
		...
		else if(srs_nbd_done == 0x1 && ccfo_done == 0x1 && fsm_state_o >= 4 && fsm_state_o < 14)
		{
			...
			if(tr_ld_o < 32)
			{
				...
				//New PAPR CCFO
				papr_ccfo_max_value = (rtd_inl(0xb8158e00) | (rtd_inl(0xb8158e04) << 8));
				est_papr_ccfo_idx = ((rtd_inl(0xb8158e0c) & 0x1f) << 8) + (rtd_inl(0xb8158e08) & 0xff);
