...
enum DTMB_REG_BIT_NAME {
	...
	DTMB_FFTMASK_EN,			// For RTL2836B DTMB only
...
typedef int
(*DTMB_DEMOD_FP_SET_BANDWIDTH_MODE)(
	DTMB_DEMOD_MODULE *pDemod,
	int BandwidthMode
);
...
@brief   DTMB demod inner lock asking function pointer

One can use DTMB_DEMOD_FP_IS_INNER_LOCKED() to ask DTMB demod if it is inner-locked.
...
@note
	-# Demod building function will set DTMB_DEMOD_FP_IS_INNER_LOCKED() with the corresponding function.

*/
typedef int
(*DTMB_DEMOD_FP_IS_TPS_LOCKED)(
	DTMB_DEMOD_MODULE *pDemod,
	int *pAnswer
);
/*
...
@brief   DTMB demod TPS lock asking function pointer

*/


typedef int
(*DTMB_DEMOD_FP_IS_INNER_LOCKED)(
	DTMB_DEMOD_MODULE *pDemod,
	int *pAnswer
);
...
@brief   DTMB demod Nosig asking function pointer

One can use DTMB_DEMOD_FP_IS_NO_SIG() to ask DTMB demod if it is Nosig.
...
typedef int
(*DTMB_DEMOD_FP_GET_SIGNAL_PRESENT_STATUS)(
	DTMB_DEMOD_MODULE *pDemod,
	int *signal_present_ans
);
...
/// RTK_R DTMB extra module
typedef struct RTK_R_DTMB_EXTRA_MODULE_TAG RTK_R_DTMB_EXTRA_MODULE;

struct RTK_R_DTMB_EXTRA_MODULE_TAG {
...
struct DTMB_DEMOD_MODULE_TAG {
	...
	union {										///<   Demod extra module used by driving module
		RTK_R_DTMB_EXTRA_MODULE RtkrDtmb;
	}
	Extra;
	...
	DTMB_DEMOD_FP_SET_BANDWIDTH_MODE    SetBandwidthMode;
    ...
    DTMB_DEMOD_FP_IS_TPS_LOCKED               IsTpsLocked;

	DTMB_DEMOD_FP_IS_INNER_LOCKED       IsInnerLocked;
	DTMB_DEMOD_FP_IS_NO_SIG             IsNoSig;
	...
	DTMB_DEMOD_FP_GET_FSM_STAGE   GetFsmStage;
...
#define REALTEK_R_DTMB_DBG(fmt, args...)         rtd_pr_demod_debug("Dtmb Debug, " fmt, ##args);
#define REALTEK_R_DTMB_INFO(fmt, args...)        rtd_pr_demod_info("Dtmb Info, " fmt, ##args);
#define REALTEK_R_DTMB_WARNING(fmt, args...)     rtd_pr_demod_warn("Dtmb Warning, " fmt, ##args);
