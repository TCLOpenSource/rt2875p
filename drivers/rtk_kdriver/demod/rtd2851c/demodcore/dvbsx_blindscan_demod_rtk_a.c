...
void RTK_A_DVBSX_AddChannel(DVBSX_DEMOD_MODULE *pDemod, unsigned int u32CenterFrequencyk)
{
	...
	unsigned int u32SymbolRFFreq = 0;
	int i32TmpFreq = 0;
	...
	for(u32Iter = 0; u32Iter < gScanedParams.u8ScanedSymbolNum; u32Iter++)
	{
		...
		if((!u8SameSymbolFound) && (gScanedParams.ScanedSymbols[u32Iter].u32SymbolRateKHz > u32MinSymbolRateLimit) && (gScanedParams.ScanedSymbols[u32Iter].u32SymbolRateKHz < u32MaxSymbolRateLimit))
		{
			...
			if(u32index < TOTAL_MAX_SCANED_SYMBOL)
			{
				...
				#ifdef BS_SCAN_DEBUG
				REALTEK_A_DVBSX_INFO("Add founded new signal frequency: %d Symbolrate: %d\n", u32SymbolRFFreq, gScanedParams.ScanedSymbols[u32Iter].u32SymbolRateKHz);
				#endif
			}
		}
	}
...
RTK_A_DVBSX_STATUS RTK_A_DVBSX_BlindScan_GetTunerFreq(DVBSX_DEMOD_MODULE *pDemod, unsigned int *TunerCenterFreqMhz, unsigned int *TunerCutOffFreqKhz)
{
	...
	if(gDvbSxBsSpecialParams.u8IsScanSymbolMode)
	{
		...
		if(gDvbSxScanParams.u32BlindScanCheckedSignalIndex >= gScanedAllSymbols.u32ScanedSignalNum)
		{
			...
			if(gDvbSxBsSpecialParams.u8UnicableEnabled)
			{
				REALTEK_A_DVBSX_INFO("[Unicable Blindscan]\n");
	...
	else
	{
		...
		*TunerCenterFreqMhz = gDvbSxBsLocalParams.u32TuneCenterFrequencyKHz/1000;
...
RTK_A_DVBSX_STATUS RTK_A_DVBSX_BlindScan_NextFreq(DVBSX_DEMOD_MODULE *pDemod, unsigned int *bBlindScanEnd)
{
	...
	else //scan mode
	{
		if(gDvbSxScanParams.u32BlindScanCheckedSignalIndex >= gScanedAllSymbols.u32ScanedSignalNum)
		{
			...
			if(pDemod->CurrentTunerId == TV_TUNER_RT710)
			{
				REALTEK_A_DVBSX_INFO("Adjust TV_TUNER_RT710 target level in BS\n");
				status |= RTK_A_DVBSX_Write(pDemod, 0x10, 0x05, 0x03);    // default 0x03
				status |= RTK_A_DVBSX_Write(pDemod, 0x10, 0x04, 0x18);    // default 0x12
				status |= RTK_A_DVBSX_Write(pDemod, 0x10, 0x03, 0x00);    // default 0xff
			}
...
re_check_signal:
	...
	//{
	//	RTK_A_DVBSX_Write(pDemod, 0x00, 0x05, 0x01);
	...
	if((status == RTK_DEMOD_SUCCESSFUL) && (ChannelExist != 1))
	{
		...
		#endif
		if(u32SymbolrateHz < 6000000 && ((i32CFOKHz > 5500)||(i32CFOKHz < -5500)) && ReCheckCnt < 3)
		{
			ReCheckCnt++;
		...
		if(u8DemodLockedFlag)
		{
			...
			#ifdef BS_SCAN_DEBUG
			REALTEK_A_DVBSX_INFO("LockedCFO: %d\n", i32LockedCFO);
			#endif
			...
			if(i32LockedCFO == 0)
			{
				i32TmpCenterFreqKHz = ((i32TmpCenterFreqKHz + 500)/1000)*1000 + i32CFOKHz;
				REALTEK_A_DVBSX_INFO("Use CoarseCFO for LockedCFO i32LockedCFO: %d i32CFOKHz: %d\n", i32LockedCFO, i32CFOKHz);
			}
			else
			{
				i32TmpCenterFreqKHz = ((i32TmpCenterFreqKHz + 500)/1000)*1000 + i32LockedCFO;
			}
	...
	{
		gDvbSxScanParams.DvbSxChannel[gDvbSxScanParams.u32ValidChannelNumber].u32FrequencyKHz = u32CenterFrequencyKHz;
...
unsigned int RTK_A_DVBSX_NotDuplicateTpCheck(DVBSX_DEMOD_MODULE *pDemod, unsigned int mode, unsigned int check_freq, unsigned int check_sr)
{
	...
	//{	
		for(i=0; i<gDvbSxScanParams.u32GetFoundChannelNumber; i++)
		{
			...
			if(check_sr <= 5000000)
			{
				if(check_freq <= gDvbSxScanParams.GetFoundChannel[i].u32FrequencyKHz+1500 && check_freq >= gDvbSxScanParams.GetFoundChannel[i].u32FrequencyKHz-1500 &&
				...
				{
					REALTEK_A_DVBSX_INFO("Low Duplicate Tp\n");
					return 0;
				}
			}
			else
			{
				...
				{
					REALTEK_A_DVBSX_INFO("High Duplicate Tp\n");
					return 0;
				}
			}
		}
