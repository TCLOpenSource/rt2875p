...
)
{
	...
	int spectrum_mode = SpectrumMode;
	U32BITS SpecInv;

	// Get SPEC_INV value from spectrum inverse value table according to spectrum mode.
	if((spectrum_mode!=0) && (spectrum_mode!=1))
		spectrum_mode = 0;
	
	SpecInv = SpecInvValueTable[spectrum_mode];
	...
	pDemod->SpectrumMode      = spectrum_mode;
...
)
{
	...
	// OC IsTrLocked need double confirm
...
)
{
	...
	unsigned char csr_retry_cnt = 0, fsr_7p2_retry_cnt = 0, fsr_retry_cnt = 0, tr_retry_cnt = 0, ccfo_retry_cnt = 0, low_sr_retry_cnt = 0;
