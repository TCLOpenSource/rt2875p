...
{
	U32BITS TargetBytePos, TargetBitPos;
...
)
{
	...
	if (TargetBytePos > MPI_VALUE_BYTE_NUM_MAX-1)
	{
		RTK_DEMOD_WARNING("MpiGetBit_Array Index Out of Bounds\n");
		TargetBytePos = MPI_VALUE_BYTE_NUM_MAX-1;
	}
...
)
{
	...
	memset(&MiddleResult, 0 , sizeof(MiddleResult));
...
)
{
	...
	unsigned char MultiplicandSignedBit = 0, MultiplicatorSignedBit = 0;
	MPI MultiplicandAbs, MultiplicatorAbs;

	unsigned char CurrentBit = 0;


	memset(&MultiplicandAbs, 0 , sizeof(MultiplicandAbs));
	memset(&MultiplicatorAbs, 0 , sizeof(MultiplicatorAbs));
...
)
{
	...
	unsigned char DividendSignedBit = 0, DivisorSignedBit = 0;
	MPI DividendAbs, DivisorAbs;

	U32BITS PrimaryDividendBitNum = 0;
	unsigned char ShiftBit = 0;

	MPI Const;
	MPI MiddleResult;

	memset(&DividendAbs, 0 , sizeof(DividendAbs));
	memset(&DivisorAbs, 0 , sizeof(DivisorAbs));
	memset(&Const, 0 , sizeof(Const));
	memset(&MiddleResult, 0 , sizeof(MiddleResult));
