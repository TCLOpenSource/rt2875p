...
//#include <stdio.h>
//#include <string.h>
#include "atsc_demod_rtk_r_base.h"
...
)
{
	...
	for (i = 0; i < ByteNum; i += ReadingByteNumMax) {
		...
		if (pBaseInterface->I2cRead(pBaseInterface, DeviceAddr, &RegReadingAddr, LEN_1_BYTE, &pReadingBytes[i], ReadingByteNum) != FUNCTION_SUCCESS)
...
{
	int i;
	unsigned char ReadingBytes[LEN_4_BYTE] = {0};
	unsigned char WritingBytes[LEN_4_BYTE];
	unsigned char ByteNum;
	U32BITS Mask;
	unsigned char Shift;
...
int atsc_rtd_4bytes_i2c_mask(ATSC_DEMOD_MODULE *pDemod, unsigned int addr, unsigned int mask, unsigned int data)
{
	...
	ret = atsc_rtd_4bytes_i2c_rd(pDemod, addr, &readingData);
	writingData = (readingData & mask) | data;
	ret |= atsc_rtd_4bytes_i2c_wr(pDemod, addr, writingData);
