...
#if IQ_SWAP_DET
	rtd_outl(RTK_DTV_DEMOD_ISDBT_IQ_SWAP_DET_EN_reg, 0x01);//enable iq swap det
	rtd_outl(RTK_DTV_DEMOD_ISDBT_BB_SP_EN_reg, 0x02);//enable sp_inv
	RTK_R_ISDB_DBG("[IQSWAP]iq swap det = 0x%x, sp_inv = 0x%x", rtd_inl(RTK_DTV_DEMOD_ISDBT_IQ_SWAP_DET_EN_reg), rtd_inl(RTK_DTV_DEMOD_ISDBT_BB_SP_EN_reg));
#endif
...
#if IQ_SWAP_DET
	tv_osal_msleep(5);
	rtd_outl(RTK_DTV_DEMOD_ISDBT_IQ_SWAP_SYS_RST_reg, 0x00);//iq_swap_rst
	rtd_outl(RTK_DTV_DEMOD_ISDBT_IQ_SWAP_SYS_RST_reg, 0x01);//iq_swap_rst
#endif
	pDemodKernel->Unlock(pDemodKernel);
...
void ReleaseRtkrDriver(RTK_R_ISDBT_DRIVER_DATA *pDriver)
{
	if (pDriver->pShareMemory) {
		...
		if(rtd_inl(TVSB5_TV_SB5_DCU1_ARB_CR1_reg)&0x01){
