...
static int rtk_demod_suspend(struct device *dev)
{
	...
	DEMOD_MGR_INFO("realtek_demod rtk_demod_suspend begin\n");
	atsc_demod_resume_done = 0;
...
static int rtk_demod_resume(struct device *dev)
{
	...
	/*
		...
		if (IsAtsc3McmPackage() == 1) {
			pDemod_globe->m_pDemod->SetTvMode(pDemod_globe->m_pDemod, TV_SYS_TYPE_ATSC);
			DEMOD_MGR_INFO("MCM Package! Init ATSC First\n");
	               atsc_demod_resume_done = 1;
		}
...
{
	DEMOD_MGR_CALLER("func %s, line %d, NOT implemented!!\n", __func__, __LINE__);
	return 0;
}
...
static long rtk_demod_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	pDemodData = kmalloc(sizeof(RTK_DEMOD_DATA), GFP_KERNEL);

	if (pDemodData == NULL) {
		ret = -EFAULT;
		DEMOD_MGR_WARNING("pDemodData kmalloc Failed (rtk_demod_ioctl) !!!!!!!!!!!!!!!\n");
		kfree(pDemodData);
	...
	switch (cmd) {
	case IOCTL_RTK_DEMOD_ATS_GET_DUMMY:
		if (isDemodConstructors != 1) {
			DEMOD_MGR_WARNING("RTKDEMOD[%d]: need do IOCTL_RTK_DEMOD_CONSTRUCTORS first !!!!!!!!!!!!!!!\n", __LINE__);
			break;
		}

		isUseDDemod = 1;
		ats_data = kmalloc(sizeof(RTK_DEMOD_ATS_GET_DUMMY), GFP_KERNEL);
		if (ats_data != NULL) {
			ret = pDemod_globe->m_pDemod->ATSgetdummy(ats_data);
			if (copy_to_user((void __user *)arg, ats_data, sizeof(RTK_DEMOD_ATS_GET_DUMMY)))
				DEMOD_MGR_WARNING("RTKDEMOD[%d]: ioctl IOCTL_RTK_DEMOD_ATS_GET_DUMMY failed !!!!!!!!!!!!!!!\n", __LINE__);
			kfree(ats_data);
		}
		break;
	case IOCTL_RTK_DEMOD_CONSTRUCTORS:
		//DEMOD_MGR_CALLER("[realtek_demod] IOCTL_RTK_DEMOD_CONSTRUCTORS\n");
		if (copy_from_user((void *)&pDemodData->demod_init_data, (const void __user *)arg, sizeof(RTK_DEMOD_CONSTRUCTORS))) {
			ret = -EFAULT;
...
#else
			Comm_Constructors(pDemod_globe->m_comm, 4);	//Demo Board
#endif
			...
			REALTEK_ALL_Constructors(pDemod_globe->m_pDemod, pDemodData->demod_init_data.m_addr, pDemodData->demod_init_data.m_output_mode, pDemodData->demod_init_data.m_output_freq, pDemod_globe->m_comm);
	...
	case IOCTL_RTK_DEMOD_SETIF:
		...
		if (copy_from_user((void *)&pDemodData->set_if_data, (const void __user *)arg, sizeof(RTK_DEMOD_SETIF))) {
			ret = -EFAULT;
			...
			pDemod_globe->m_pDemod->SetIF(pDemod_globe->m_pDemod, (IF_PARAM*) &pDemodData->set_if_data);
			ret = 0;
		}
		break;
	...
	case IOCTL_RTK_DEMOD_SETTVSYS:
		...
		if (copy_from_user((void *)&pDemodData->set_tv_sys_ex_data, (const void __user *)arg, sizeof(RTK_DEMOD_SETTVSYSEX))) {
			ret = -EFAULT;
			...
			pDemod_globe->m_pDemod->SetTVSysEx(pDemod_globe->m_pDemod, pDemodData->set_tv_sys_ex_data.sys, NULL);
			ret = 0;
		}
		break;
	...
	case IOCTL_RTK_DEMOD_SETTVSYSEX:
		...
		if (copy_from_user((void *)&pDemodData->set_tv_sys_ex_data, (const void __user *)arg, sizeof(RTK_DEMOD_SETTVSYSEX))) {
			...
			pDemod_globe->m_pDemod->SetTVSysEx(pDemod_globe->m_pDemod, pDemodData->set_tv_sys_ex_data.sys, &pDemodData->set_tv_sys_ex_data.pSigInfo);
			ret = 0;
		}
		break;
	...
	case IOCTL_RTK_DEMOD_ACQUIRESIGNAL:
		...
		if (copy_from_user((void *)&pDemodData->set_acq_sig_data, (const void __user *)arg, sizeof(RTK_DEMOD_SET_ACQ_SIG))) {
			//ret = -EFAULT;
			ret = 0;
			...
			pDemodData->set_acq_sig_data.return_value = pDemod_globe->m_pDemod->AcquireSignal(pDemod_globe->m_pDemod, pDemodData->set_acq_sig_data.WaitSignalLock);
			if (copy_to_user((void __user *)arg, &pDemodData->set_acq_sig_data, sizeof(RTK_DEMOD_SET_ACQ_SIG)))
				ret = 0;
			else
				ret = -EFAULT;
		}
	...
	case IOCTL_RTK_DEMOD_SETTSMODE:
		...
		if (copy_from_user((void *)&pDemodData->set_ts_mode_data, (const void __user *)arg, sizeof(RTK_DEMOD_SET_TS_MODE))) {
			ret = -EFAULT;
			...
			pDemod_globe->m_pDemod->SetTSMode(pDemod_globe->m_pDemod, (TS_PARAM*)&pDemodData->set_ts_mode_data);
			ret = 0;
		}
		break;
	...
	case IOCTL_RTK_DEMOD_GETLOCK:
		...
		pDemod_globe->m_pDemod->GetLockStatus(pDemod_globe->m_pDemod, &pDemodData->get_lock_data.lock);
		ret = 0;

		if (copy_to_user((void __user *)arg, &pDemodData->get_lock_data, sizeof(RTK_DEMOD_GET_LOCK)))
	...
	case IOCTL_RTK_DEMOD_GETCROFFSET:
		...
		pDemod_globe->m_pDemod->GetCarrierOffset(pDemod_globe->m_pDemod, &pDemodData->get_croffset_data.Offset);
		ret = 0;
		if (copy_to_user((void __user *)arg, &pDemodData->get_croffset_data, sizeof(RTK_DEMOD_GET_CROFFSET)))
	...
	case IOCTL_RTK_DEMOD_GETCHANNELINFO:
		...
		pDemod_globe->m_pDemod->GetChannelInfo(pDemod_globe->m_pDemod, &pDemodData->get_channelinfo_data.Standard, &pDemodData->get_channelinfo_data.Modulation, &pDemodData->get_channelinfo_data.CodeRate, &pDemodData->get_channelinfo_data.PilotOnOff, &pDemodData->get_channelinfo_data.Inverse);
		ret = 0;
		if (copy_to_user((void __user *)arg, &pDemodData->get_channelinfo_data, sizeof(RTK_DEMOD_GET_CHANNELINFO)))
	...
	case IOCTL_RTK_DEMOD_SETMODE:
		...
		if (copy_from_user((void *)&pDemodData->set_mode_data, (const void __user *)arg, sizeof(RTK_DEMOD_SETMODE))) {
			ret = -EFAULT;
			...
			pDemod_globe->m_pDemod->SetMode(pDemod_globe->m_pDemod, (TV_OP_MODE)pDemodData->set_mode_data.mode);
			ret = 0;
		}
	...
	case IOCTL_RTK_DEMOD_SETTVPARA:
		...
		if (copy_from_user((void *)&pDemodData->set_tv_para, (const void __user *)arg, sizeof(RTK_DEMOD_SET_TVPARA))) {
			ret = -EFAULT;
			...
			pDemod_globe->m_pTuner->TunerParam.id = pDemodData->set_tv_para.id;
			pDemod_globe->m_pTuner->TunerParam.std = pDemodData->set_tv_para.std;
			pDemod_globe->m_pTuner->TunerParam.step_freq = pDemodData->set_tv_para.step_freq;
			pDemod_globe->m_pTuner->TunerParam.if_freq = pDemodData->set_tv_para.if_freq;
			pDemod_globe->m_pTuner->TunerParam.if_inversion = pDemodData->set_tv_para.if_inversion;
			pDemod_globe->m_pTuner->TunerParam.if_agc_pola = pDemodData->set_tv_para.if_agc_pola;
			pDemod_globe->m_pTuner->TunerParam.min_freq = pDemodData->set_tv_para.min_freq;
			pDemod_globe->m_pTuner->TunerParam.max_freq = pDemodData->set_tv_para.max_freq;
			pDemod_globe->m_pTuner->TunerParam.lowband_midband_limit_freq = pDemodData->set_tv_para.lowband_midband_limit_freq;
			pDemod_globe->m_pTuner->TunerParam.midband_highband_limit_freq = pDemodData->set_tv_para.midband_highband_limit_freq;
			ret = 0;
		}
	...
	case IOCTL_RTK_DEMOD_GETQUALITY:
		...
		if (copy_from_user((void *)&pDemodData->get_quality, (const void __user *)arg, sizeof(RTK_DEMOD_GET_QUALITY))) {
			ret = -EFAULT;
			DEMOD_MGR_WARNING("RTKDEMOD[%d]: ioctl IOCTL_RTK_DEMOD_GETQUALITY failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else {

			quality_temp.rflevel = pDemodData->get_quality.pQuality.rflevel;
			pDemod_globe->m_pDemod->GetSignalQuality(pDemod_globe->m_pDemod, (ENUM_TV_QUAL)pDemodData->get_quality.id, (TV_SIG_QUAL*)&quality_temp);
			ret = 0;

			switch (pDemodData->get_quality.id) {
			case TV_QUAL_NOSIG :
				pDemodData->get_quality.pQuality.nosig = quality_temp.nosig;
				break;

			case TV_QUAL_INNERLOCK :
				pDemodData->get_quality.pQuality.inner = quality_temp.inner;
				break;

			case TV_QUAL_SNR :
				pDemodData->get_quality.pQuality.snr = quality_temp.snr;

				break;
			case TV_QUAL_BEC :
			case TV_QUAL_BER : {
				if (pDemodData->get_quality.id == TV_QUAL_BER)
					pDemodData->get_quality.pQuality.ber = quality_temp.ber;
				if (pDemodData->get_quality.id  == TV_QUAL_BEC) {
					pDemodData->get_quality.pQuality.layer_ber.ber_num = quality_temp.layer_ber.ber_num;
					pDemodData->get_quality.pQuality.layer_ber.ber_den = quality_temp.layer_ber.ber_den;
				}
			...
			case TV_QUAL_UCBLOCKS : {
				if (pDemodData->get_quality.id == TV_QUAL_PER)
					pDemodData->get_quality.pQuality.per = quality_temp.per;
				else if (pDemodData->get_quality.id == TV_QUAL_UCBLOCKS)
					pDemodData->get_quality.pQuality.layer_per.per_num = quality_temp.ucblocks;
				else if (pDemodData->get_quality.id == TV_QUAL_PEC) {
					pDemodData->get_quality.pQuality.layer_per.per_num = quality_temp.layer_per.per_num;
					pDemodData->get_quality.pQuality.layer_per.per_den = quality_temp.layer_per.per_den;
				}
			...
			case TV_QUAL_SIGNAL_STRENGTH:
				pDemodData->get_quality.pQuality.strength = quality_temp.strength;
				break;

			case TV_QUAL_SIGNAL_QUALITY:
				pDemodData->get_quality.pQuality.quality = quality_temp.quality;
				break;

			case TV_QUAL_AGC:
				pDemodData->get_quality.pQuality.agc = quality_temp.agc;
				break;
			case TV_QUAL_DEMODFWVERSION:
				pDemodData->get_quality.pQuality.DemodFwVersion = quality_temp.DemodFwVersion;
				break;

			case TV_QUAL_EMERGENCY_FLAG:
				pDemodData->get_quality.pQuality.emergency_flag = quality_temp.emergency_flag;
				break;
			...
			if (copy_to_user((void __user *)arg, &pDemodData->get_quality, sizeof(RTK_DEMOD_GET_QUALITY)))
	...
	case IOCTL_RTK_DEMOD_GET_DEMOD_INFO:
		...
		pDemod_globe->m_pDemod->GetDemodInfo(pDemod_globe->m_pDemod, (TV_DEMOD_INFO*)&pDemodData->get_demod_info_data);
		if (copy_to_user((void __user *)arg, &pDemodData->get_demod_info_data, sizeof(RTK_DEMOD_GET_DEMOD_INFO)))
	...
	case IOCTL_RTK_DEMOD_GET_DEBUG_INFO:
		...
		if (copy_from_user((void *)&pDemodData->get_debug_info_data, (const void __user *)arg, sizeof(RTK_DEMOD_GET_DEBUG_INFO))) {
			ret = -EFAULT;
			DEMOD_MGR_WARNING("RTKDEMOD[%d]: ioctl IOCTL_RTK_DEMOD_GET_DEBUG_INFO failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else {
			pDemod_globe->m_pDemod->DebugInfo(pDemod_globe->m_pDemod, pDemodData->get_debug_info_data.debug_mode);
			ret = 0;
		}
		break;
	...
	case IOCTL_RTK_DEMOD_GET_SIGNAL_INFO:
		...
		pDemod_globe->m_pDemod->GetSignalInfo(pDemod_globe->m_pDemod, (TV_SIG_INFO*)&pDemodData->get_signal_info_data);
		if (copy_to_user((void __user *)arg, &pDemodData->get_signal_info_data, sizeof(RTK_DEMOD_GET_SIGNAL_INFO)))
	...
	case IOCTL_RTK_DEMOD_CHANGET2PLP:
		...
		if (copy_from_user((void *)&pDemodData->get_signal_info_data, (const void __user *)arg, sizeof(RTK_DEMOD_GET_SIGNAL_INFO))) {
			ret = -EFAULT;
			DEMOD_MGR_WARNING("RTKDEMOD[%d]: IOCTL_RTK_DEMOD_CHANGET2PLP failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else {
			//DEMOD_MGR_DBG("realtek_demod set_tv_sys_ex_data.sys = %d\n", pDemod_globe->set_tv_sys_ex_data.sys);

			pDemod_globe->m_pDemod->ChangeT2PLP(pDemod_globe->m_pDemod, (TV_SIG_INFO*)&pDemodData->get_signal_info_data);
			ret = 0;
		}
		break;
	...
	case IOCTL_RTK_DEMOD_GETT2PLPINFO:
		...
		pDemod_globe->m_pDemod->GetT2PLPInfo(pDemod_globe->m_pDemod, (TV_SIG_INFO*)&pDemodData->get_signal_info_data);
	...
	case IOCTL_RTK_DEMOD_CHANGEATSC3PLP:
		...
		if (copy_from_user((void *)&pDemodData->get_signal_info_data, (const void __user *)arg, sizeof(RTK_DEMOD_GET_SIGNAL_INFO))) {
			ret = -EFAULT;
			DEMOD_MGR_WARNING("RTKDEMOD[%d]: IOCTL_RTK_DEMOD_CHANGEATSC3PLP failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else {
			//DEMOD_MGR_DBG("realtek_demod set_tv_sys_ex_data.sys = %d\n", pDemod_globe->set_tv_sys_ex_data.sys);

			pDemod_globe->m_pDemod->ChangeAtsc3PLP(pDemod_globe->m_pDemod, (TV_SIG_INFO*)&pDemodData->get_signal_info_data);
			ret = 0;
		}
		break;

	case IOCTL_RTK_DEMOD_GETATSC3PLPINFO:
		...
		if (copy_from_user((void *)&pDemodData->get_demod_atsc3_PLPinfo_data, (const void __user *)arg, sizeof(RTK_DEMOD_GETATSC3PLPINFO))) {
			//ret = -EFAULT;
			ret = 0;
			DEMOD_MGR_WARNING("RTKDEMOD[%d]: ioctl IOCTL_RTK_DEMOD_GETATSC3PLPINFO failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else {
			//DEMOD_MGR_DBG("realtek_demod set_acq_sig_data.WaitSignalLock = %d\n", pDemod_globe->set_acq_sig_data.WaitSignalLock);

			pDemod_globe->m_pDemod->GetAtsc3PLPInfo(pDemod_globe->m_pDemod, (TV_SIG_INFO*)&pDemodData->get_demod_atsc3_PLPinfo_data.pSigInfo, pDemodData->get_demod_atsc3_PLPinfo_data.Atsc3MPlpMode);
			if (copy_to_user((void __user *)arg, &pDemodData->get_demod_atsc3_PLPinfo_data, sizeof(RTK_DEMOD_GETATSC3PLPINFO)))
				ret = 0;
			else
				ret = -EFAULT;
		}
	...
	case IOCTL_RTK_DEMOD_SETDISEQC_22K:
		...
		if (copy_from_user((void *)&pDemodData->set_diseqc_22k_data, (const void __user *)arg, sizeof(RTK_DEMOD_SET_DISEQC_22K))) {
			ret = -EFAULT;
			DEMOD_MGR_WARNING("RTKDEMOD[%d]: ioctl IOCTL_RTK_DEMOD_SETDISEQC_22K failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else {
			pDemod_globe->m_pDemod->SetDiseqcContinue22kOnOff(pDemod_globe->m_pDemod, pDemodData->set_diseqc_22k_data.onoff);
			ret = 0;
		}
		break;
	...
	case IOCTL_RTK_DEMOD_SETDISEQC_CMD:
		...
		if (copy_from_user((void *)&pDemodData->set_diseqc_cmd_data, (const void __user *)arg, sizeof(RTK_DEMOD_SET_DISEQC_CMD))) {
			ret = -EFAULT;
			DEMOD_MGR_WARNING("RTKDEMOD[%d]: ioctl IOCTL_RTK_DEMOD_SETDISEQC_CMD failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else {
			if (pDemodData->set_diseqc_cmd_data.length > 8) {
				pDemodData->set_diseqc_cmd_data.length = 8;
			}
			pDemod_globe->m_pDemod->SetDiseqcCmd(pDemod_globe->m_pDemod, pDemodData->set_diseqc_cmd_data.length, pDemodData->set_diseqc_cmd_data.data);
			ret = 0;
		}
		break;
		
	case IOCTL_RTK_DEMOD_SETDISEQC_2P0_CMD:
		...
		//DEMOD_MGR_CALLER("[realtek_demod] IOCTL_RTK_DEMOD_SETDISEQC_2P0_CMD\n");
		if (copy_from_user((void *)&pDemodData->set_diseqc_cmd_data, (const void __user *)arg, sizeof(RTK_DEMOD_SET_DISEQC_CMD))) {
			ret = -EFAULT;
			DEMOD_MGR_WARNING("RTKDEMOD[%d]: ioctl IOCTL_RTK_DEMOD_SETDISEQC_2P0_CMD failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else {
			if (pDemodData->set_diseqc_cmd_data.length > 8) {
				pDemodData->set_diseqc_cmd_data.length = 8;
			}
			pDemod_globe->m_pDemod->SetDiseqc2p0Cmd(pDemod_globe->m_pDemod, &pDemodData->set_diseqc_cmd_data.length, pDemodData->set_diseqc_cmd_data.data);
			if (copy_to_user((void __user *)arg, &pDemodData->set_diseqc_cmd_data, sizeof(RTK_DEMOD_SET_DISEQC_CMD)))
				DEMOD_MGR_DBG("[realtek_demod] IOCTL_RTK_DEMOD_SETDISEQC_2P0_CMD\n");
			ret = 0;
		}
		break;
	...
	case IOCTL_RTK_DEMOD_SETDISEQC_BURST:
		...
		if (copy_from_user((void *)&pDemodData->set_diseqc_toneburst_data, (const void __user *)arg, sizeof(RTK_DEMOD_SET_DISEQC_TONEBURST))) {
			ret = -EFAULT;
			DEMOD_MGR_WARNING("RTKDEMOD[%d]: ioctl IOCTL_RTK_DEMOD_SETDISEQC_BURST failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else {
			pDemod_globe->m_pDemod->SetDiseqcToneBurst(pDemod_globe->m_pDemod, pDemodData->set_diseqc_toneburst_data.mode);
			ret = 0;
		}
		break;
	...
	case IOCTL_RTK_DEMOD_GETREGCMD:
		...
		if (copy_from_user((void *)&pDemodData->get_reg_data, (const void __user *)arg, sizeof(RTK_DEMOD_GET_REG_DATA))) {
			ret = -EFAULT;
			DEMOD_MGR_WARNING("RTKDEMOD[%d]: ioctl IOCTL_RTK_DEMOD_GETREGCMD failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else {
			pDemod_globe->m_pDemod->GetRegCmd(pDemod_globe->m_pDemod, pDemodData->get_reg_data.RegAddr, &pDemodData->get_reg_data.RegValue);
			if (copy_to_user((void __user *)arg, &pDemodData->get_reg_data, sizeof(RTK_DEMOD_GET_REG_DATA)))
	...
	case IOCTL_RTK_DEMOD_SETREGCMD:
		...
		if (copy_from_user((void *)&pDemodData->set_reg_data, (const void __user *)arg, sizeof(RTK_DEMOD_SET_REG_DATA))) {
			ret = -EFAULT;
			DEMOD_MGR_WARNING("RTKDEMOD[%d]: ioctl IOCTL_RTK_DEMOD_SETREGCMD failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else {
			pDemod_globe->m_pDemod->SetRegCmd(pDemod_globe->m_pDemod, pDemodData->set_reg_data.RegAddr, pDemodData->set_reg_data.RegValue);
			ret = 0;
		}
		break;
	...
	case IOCTL_RTK_DEMOD_SETTUNER_INPUT_MODE:
		...
		if (copy_from_user((void *)&pDemodData->set_tuner_input, (const void __user *)arg, sizeof(RTK_DEMOD_SET_TUNER_INPUT))) {
			ret = -EFAULT;
			DEMOD_MGR_WARNING("RTKDEMOD[%d]: ioctl IOCTL_RTK_DEMOD_SETTUNER_INPUT_MODE failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else {
			pDemod_globe->m_pDemod->SetTunerInputMode(pDemod_globe->m_pDemod, pDemodData->set_tuner_input.Mode);
			ret = 0;
		}
		break;
	...
	case IOCTL_RTK_DEMOD_BLINDSCAN_INIT:
		...
		if (copy_from_user((void *)&pDemodData->blindscan_init_data, (const void __user *)arg, sizeof(RTK_DEMOD_BLINDSCAN_INIT))) {
			ret = -EFAULT;
			DEMOD_MGR_WARNING("RTKDEMOD[%d]: ioctl IOCTL_RTK_DEMOD_BLINDSCAN_INIT failed !!!!!!!!!!!!!!!\n", __LINE__);
		} else {
			pDemod_globe->m_pDemod->BlindScan_Init(pDemod_globe->m_pDemod, pDemodData->blindscan_init_data.StartFreqMhz, pDemodData->blindscan_init_data.EndFreqMhz);
			ret = 0;
		}
		break;
	...
	case IOCTL_RTK_DEMOD_BLINDSCAN_NEXTFREQ:
		...
		pDemod_globe->m_pDemod->BlindScan_NextFreq(pDemod_globe->m_pDemod, &pDemodData->blindscan_nextfreq_data.bBlindScanEnd);
		ret = 0;
		if (copy_to_user((void __user *)arg, &pDemodData->blindscan_nextfreq_data, sizeof(RTK_DEMOD_BLINDSCAN_NEXTFREQ)))
	...
	case IOCTL_RTK_DEMOD_BLINDSCAN_GETTUNERFREQ:
		...
		pDemod_globe->m_pDemod->BlindScan_GetTunerFreq(pDemod_globe->m_pDemod, &pDemodData->blindscan_get_tunerfreq_data.TunerCenterFreqMhz, &pDemodData->blindscan_get_tunerfreq_data.TunerCutOffFreqKhz);
		ret = 0;
		if (copy_to_user((void __user *)arg, &pDemodData->blindscan_get_tunerfreq_data, sizeof(RTK_DEMOD_BLINDSCAN_GET_TUNERFREQ)))
	...
	case IOCTL_RTK_DEMOD_BLINDSCAN_GETFOUNDTP:
		...
		pDemod_globe->m_pDemod->BlindScan_GetFoundTP(pDemod_globe->m_pDemod, &pDemodData->blindscan_get_foundtp_data.FreqMhz, &pDemodData->blindscan_get_foundtp_data.SymbolRateKhz);
		ret = 0;
		if (copy_to_user((void __user *)arg, &pDemodData->blindscan_get_foundtp_data, sizeof(RTK_DEMOD_BLINDSCAN_GET_FOUNDTP)))
	...
	case IOCTL_RTK_DEMOD_BLINDSCAN_GETSTATUS:
		...
		pDemod_globe->m_pDemod->BlindScan_GetStatus(pDemod_globe->m_pDemod, &pDemodData->blindscan_get_status_data.Status, &pDemodData->blindscan_get_status_data.bBlindScanLock);
		ret = 0;
		if (copy_to_user((void __user *)arg, &pDemodData->blindscan_get_status_data, sizeof(RTK_DEMOD_BLINDSCAN_GET_STATUS)))
	...
	case IOCTL_RTK_DEMOD_BLINDSCAN_ACQUIRE_SIGNAL:
		...
		if (copy_from_user((void *)&pDemodData->blindscan_acquire_signal_data, (const void __user *)arg, sizeof(RTK_DEMOD_BLINDSCAN_ACQUIRE_SIGNAL))) {
			ret = -EFAULT;
			...
			pDemod_globe->m_pDemod->BlindScan_AcquireSignal(pDemod_globe->m_pDemod, &pDemodData->blindscan_acquire_signal_data.FrequencyKhz, &pDemodData->blindscan_acquire_signal_data.SymbolRate, &pDemodData->blindscan_acquire_signal_data.Fcfo, &pDemodData->blindscan_acquire_signal_data.bTpLock);
			ret = 0;
			if (copy_to_user((void __user *)arg, &pDemodData->blindscan_acquire_signal_data, sizeof(RTK_DEMOD_BLINDSCAN_ACQUIRE_SIGNAL)))
...
#ifdef CONFIG_COMPAT
long rtk_demod_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	void __user *compat_arg = compat_ptr(arg);
	return rtk_demod_ioctl(filp, cmd, (unsigned long)compat_arg);
}
#endif
...
struct file_operations demod_fops = {
	...
	.poll               = rtk_demod_poll,
#ifdef CONFIG_COMPAT
	.compat_ioctl = rtk_demod_compat_ioctl,
#endif

};

static char *rtk_demod_devnode(struct device *dev, umode_t *mode)
{
	return NULL;
}
...
static const struct proc_ops demod_proc_fops = {
	.proc_write  = demod_proc_write,
	.proc_read  = demod_proc_read,
};
#else
...
static int __init rtk_demod_init(void)
{
	...
	if (get_product_type() == PRODUCT_TYPE_DIAS) {
		DEMOD_MGR_WARNING("demod not support on dias platform\n");
		return -ENODEV;
	}
	...
	gDemodDev = device_create(rtk_demod_class, NULL, dev, NULL, DRV_NAME);
	gDemodDev->coherent_dma_mask = DMA_BIT_MASK(32);
	gDemodDev->dma_mask = &gDemodDev->coherent_dma_mask;

	if(dma_set_mask(gDemodDev, DMA_BIT_MASK(32))) {
		DEMOD_MGR_WARNING("DMA not supported\n");
	}
...
int rtk_demod_get_debugbuf(unsigned int *buf_addr, unsigned int *buf_size)
{
	...
	if (pDemod_globe->m_pDemod != NULL) {
		...
		ret = 0;
		DEMOD_MGR_INFO("demod debugbuf, type=%d, addr=0x%x, size=0x%x\n", buf_type, *buf_addr, *buf_size);
	}
