...
int DtvDemodEnable(RTK_DEMOD_MODE mode)
{
	...
	case RTK_DEMOD_MODE_ISDB:
		CRT_CLK_OnOff(DTVATBwoWrap, CLK_ON, NULL);
		CRT_CLK_OnOff(DTVRTK, CLK_ON, NULL);
		RTK_DEMOD_INFO("DtvDemodEnable to ISDBT(ATB_DFE)\n");
		break;
	case RTK_DEMOD_MODE_DVBC:
	case RTK_DEMOD_MODE_ATSC:
	case RTK_DEMOD_MODE_QAM:
		CRT_CLK_OnOff(DTVRTK, CLK_ON, NULL);
		RTK_DEMOD_INFO("DtvDemodEnable to DVBC ATSC OC\n");
		break;
...
/*
	if (mode == RTK_DEMOD_MODE_DTMB) {
		...
		*/
	if (mode == RTK_DEMOD_MODE_ISDB) {
		SysClkCheckMode = SYS_REG_SYS_CLKEN0_get_clken_atb_demod(sysclk0) & SYS_REG_SYS_CLKEN0_get_clken_rtk_demod(sysclk0);
		SysRstCheckMode = SYS_REG_SYS_SRST0_get_rstn_atb_demod(sysrst0) & SYS_REG_SYS_SRST0_get_rstn_rtk_demod(sysrst0);
	} else if ((mode == RTK_DEMOD_MODE_ATSC) || (mode == RTK_DEMOD_MODE_QAM) || (mode == RTK_DEMOD_MODE_DVBC) || (mode == RTK_DEMOD_MODE_DTMB)) {
...
void DemodPllDefault(void)
{
	...
	rtd_outl(0xb8000504, 0x000e0104);				//PLL27X_IFADC VCO M,N, M<14>+2=16(VCO=432M)
...
void DemodPllSetting(RTK_DEMOD_MODE mode)
{
	switch (mode) {
	case RTK_DEMOD_MODE_DTMB:
		//**PLL setting for DTMB_RTK_864M_144M Setting**//			//VCO and DIV setting
		//ADC CLK and System CLK setting(ADC=144M, Sys=384M)
		...
		rtd_maskl(0xb8000504, 0xFC80FC38, 0x001e0143);				//VCO = M<6:0>+2 = 30+2, N:00=1, 27 /1 *32=864MHz, PLL27X_IFADC_M<bit22:16> = 0x1E(30), PLL27X_IFADC_N<bit25:24>=0x0(1), <9:6>PLL27X_IFADC_LF_RS=0101, PLL27X_IFADC PUMP current<bit2:0>=0x3(011:30uF)
		rtd_maskl(0xb80004d8, 0xFFFF00FF, 0x00003200);				//<15:8>PLL_RESERVE=00110010
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000069);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0110 = 0x6(/2.25), sys_clk = 864 / 2.25 = 384MHz
																	//ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 1001 = 0x9(/6), Adc_clk = 864 / 6=144MHz
		rtd_maskl(0xb80004b8, 0xFFFD8B3B, 0x00021000);				//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF432M_DIV=0x0(div2), <bit13>DIF432M_EN=0x0, <bit12>DTVDIG_EN=0x1, <bit10>LDPC_EN=0x0, <7:6>LDPC_DIV=0x0(div1.75), <bit2>PLL27X_IFADC_REF_SEL=0(XTAL)
		RTK_DEMOD_INFO("DemodPllSetting Pll Change to DTMB \n");
		break;

	case RTK_DEMOD_MODE_ISDB:
	case RTK_DEMOD_MODE_ATSC:
		//**PLL setting for ATSC_ISDBT_RTK Setting**//				//VCO and DIV setting
		//ADC CLK and System CLK setting(ADC=54M, Sys=432M)
		rtd_maskl(0xb8000214, 0xFFFFB7FF, 0x00004000); 				//(B14,B11) = 10, Demod Sel = RTK Demod(ATSC/DVBC/ISDBT/DTMB)
		rtd_maskl(0xb80004dc, 0xFDFFFFFE, 0x02000000);				//PLL_L2H_CML_POW <bit25> = 1(on), PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
		rtd_maskl(0xb8000500, 0xFFFFFFEF, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80FC38, 0x000e0145);				//VCO = M<6:0>+2 = 14+2, N:00=1, 27 /1 *16=432MHz, PLL27X_IFADC_M<bit22:16> = 0xE(14), PLL27X_IFADC_N<bit25:24>=0x0(1), <9:6>PLL27X_IFADC_LF_RS=0101, PLL27X_IFADC PUMP current<bit2:0>=0x5(101:15uF)
		rtd_maskl(0xb80004d8, 0xFFFF00FF, 0x00003000);				//<15:8>PLL_RESERVE=00110000
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x000000A0);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 1010 = 0xA(/1), sys_clk = 432 / 1 = 432MHz
	...
	case RTK_DEMOD_MODE_QAM:
		//**PLL setting for OpenCable_RTK_25p2M Setting**//			//VCO and DIV setting
		//ADC CLK and System CLK setting(ADC=36M, Sys=432M)
		rtd_maskl(0xb8000214, 0xFFFFB7FF, 0x00004000); 				//(B14,B11) = 10, Demod Sel = RTK Demod(ATSC/DVBC/ISDBT/DTMB)
		rtd_maskl(0xb80004dc, 0xFDFFFFFE, 0x02000000);				//PLL_L2H_CML_POW <bit25> = 1(on), PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
		rtd_maskl(0xb8000500, 0xFFFFFFEF, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80FC38, 0x001A0204);				//VCO = M<6:0>+2 = 26+2, N:00=1, 27 /1 *28=756MHz, PLL27X_IFADC_M<bit22:16> = 0x1A(26), PLL27X_IFADC_N<bit25:24>=0x0(1), <9:6>PLL27X_IFADC_LF_RS=1000, PLL27X_IFADC PUMP current<bit2:0>=0x5(100:12.5uF)
		rtd_maskl(0xb80004d8, 0xFFFF00FF, 0x00003000);				//<15:8>PLL_RESERVE=00110000
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000018);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0001 = 0x1(/2.5), sys_clk = 756 / 2.5 = 302.4MHz
	...
	case RTK_DEMOD_MODE_DVBC:
		//**PLL setting for DVBC_RTK_36M Setting**//				//VCO and DIV setting
		//ADC CLK and System CLK setting(ADC=36M, Sys=432M)
		rtd_maskl(0xb8000214, 0xFFFFB7FF, 0x00004000); 				//(B14,B11) = 10, Demod Sel = RTK Demod(ATSC/DVBC/ISDBT/DTMB)
		rtd_maskl(0xb80004dc, 0xFDFFFFFE, 0x02000000);				//PLL_L2H_CML_POW <bit25> = 1(on), PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
		rtd_maskl(0xb8000500, 0xFFFFFFEF, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80FC38, 0x00120145);				//VCO = M<6:0>+2 = 18+2, N:00=1, 27 /1 *20=540MHz, PLL27X_IFADC_M<bit22:16> = 0x12(18), PLL27X_IFADC_N<bit25:24>=0x0(1), <9:6>PLL27X_IFADC_LF_RS=0101, PLL27X_IFADC PUMP current<bit2:0>=0x5(101:15uF)
		rtd_maskl(0xb80004d8, 0xFFFF00FF, 0x00003000);				//<15:8>PLL_RESERVE=00110000
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000031);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0011 = 0x3(/1.25), sys_clk = 864 / 1.25 = 432MHz
	...
	case RTK_DEMOD_MODE_DVBT:
	case RTK_DEMOD_MODE_DVBT2:
		//**PLL setting for DVB-T/T2/C2 Setting**//					//VCO and DIV setting
		...
		rtd_maskl(0xb80004dc, 0xFDFFFFFE, 0x02000000);				//PLL_L2H_CML_POW <bit25> = 1(on), PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
		rtd_maskl(0xb8000500, 0xFFFFFFEF, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80FC38, 0x00160145);				//VCO = M<6:0>+2 = 22+2, N:00=1, 27 /1 *24=648MHz, PLL27X_IFADC_M<bit22:16> = 0x16(22), PLL27X_IFADC_N<bit25:24>=0x0(1), <9:6>PLL27X_IFADC_LF_RS=0101, PLL27X_IFADC PUMP current<bit2:0>=0x5(101:15uF)
		rtd_maskl(0xb80004d8, 0xFFFF00FF, 0x00003000);				//<15:8>PLL_RESERVE=00110000
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000037);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0x3(/1.25)(40/60), sys_clk = 648 / 1.25 = 518.4 MHz
																	//ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0x7(/4.5)(33/67)), Adc_clk = 648 / 4.5 =144MHz															
		rtd_maskl(0xb80004b8, 0xFFFD8B3B, 0x00027440);				//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF432M_DIV=0x1(div3), <bit13>DIF432M_EN=0x1, <bit12>DTVDIG_EN=0x1,
	...
	case RTK_DEMOD_MODE_DVBS2X:
	case RTK_DEMOD_MODE_SPEC_AN:
		//**PLL setting for DVB-S/S2 Setting**//					//VCO and DIV setting
		//ADC CLK and System CLK setting(ADC=121.5M, Sys=486M)
		rtd_maskl(0xb8000214, 0xFFFDB7FF, 0x00020000);				//(B14,B11) = 00, Demod Sel = ATB Demod(DVB-T/T2/S/S2), <bit17>Dtv_demod_atb_clk_sel=0x1(1: ldpc from system clk)
		rtd_maskl(0xb80004dc, 0xFDFFFFFE, 0x02000000);				//PLL_L2H_CML_POW <bit25> = 1(on), PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
		rtd_maskl(0xb8000500, 0xFFFFFFEF, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80FC38, 0x001901C4);				//VCO = M<6:0>+2 = 25+2, N:00=1, 27 /1 *27=729MHz, PLL27X_IFADC_M<bit22:16> = 0x19(25), PLL27X_IFADC_N<bit25:24>=0x0(1), <9:6>PLL27X_IFADC_LF_RS=0111, PLL27X_IFADC PUMP current<bit2:0>=0x4(100:12.5uF)
		rtd_maskl(0xb80004d8, 0xFFFF00FF, 0x00003000);				//<15:8>PLL_RESERVE=00110000
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000049);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0x4(/1.5)(50/50), sys_clk = 729 / 1.5 = 486 MHz
																	//ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0x9(/6)(33/67)), Adc_clk = 729 / 6 =121.5MHz
		rtd_maskl(0xb80004b8, 0xFFFD8B3B, 0x000214C0);				//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF432M_DIV=0x0(div2), <bit13>DIF432M_EN=0x0, <bit12>DTVDIG_EN=0x1,
																	//<bit10>LDPC_EN=0x1, <7:6>LDPC_DIV=0x3(div1), <bit2>PLL27X_IFADC_REF_SEL=0(XTAL)
...
//MERLIN5
	...
	if (mode == RTK_DEMOD_MODE_DTMB) {
		...
		// down sample 3 filter setting
		rtd_outl(0xb8033900, 0x00000000);//tap0&1
		rtd_outl(0xb8033904, 0x1FFF0000);//tap2&3
		rtd_outl(0xb8033908, 0x00001FFF);//tap4&5
		rtd_outl(0xb803390C, 0x00040001);//tap6&7
		rtd_outl(0xb8033910, 0x00030006);//tap8&9
		rtd_outl(0xb8033914, 0x1FF21FFC);//tap10&11
		rtd_outl(0xb8033918, 0x1FF01FEB);//tap12&13
		rtd_outl(0xb803391C, 0x00230005);//tap14&15
		rtd_outl(0xb8033920, 0x0033003A);//tap16&17
		rtd_outl(0xb8033924, 0x1FB80003);//tap18&19
		rtd_outl(0xb8033928, 0x1F791F79);//tap20&21
		rtd_outl(0xb803392C, 0x00791FD6);//tap22&23
		rtd_outl(0xb8033930, 0x013A011A);//tap24&25
		rtd_outl(0xb8033934, 0x1F53009E);//tap26&27
		rtd_outl(0xb8033938, 0x1D2A1DE1);//tap28&29
		rtd_outl(0xb803393C, 0x00D61E06);//tap30&31
		rtd_outl(0xb8033940, 0x09FC0533);//tap32&33
		rtd_outl(0xb8033944, 0x0F1B0DB4);//tap34&35
/*
...
int DemodFrontendPathSetting(RTK_DEMOD_MODE mode)
{
	...
	switch (mode) {
	case RTK_DEMOD_MODE_DTMB:
		rtd_outl(0xb8033800, 0x10050010);							//<bit29:28>dtmb_data_valid_delay=0x1(delay data valid), <bit21>rtk_dtv_demod_sel=0, <bit18:16>Dtv_frontend_mode=0x5, <bit8>dtv_idata_sel=0, <bit4>Dtv_dnflt_en=1
		...
		rtd_maskl(0xb8000844, 0xFFFFFF0F, 0x00000030);				//GPIO_13, Set pinmux DEMOD_IF_AGC
	...
	case RTK_DEMOD_MODE_ATSC:
	case RTK_DEMOD_MODE_ISDB:
		rtd_outl(0xb8033800, 0x10040000);							//<bit29:28>dtmb_data_valid_delay=0x1(delay data valid), <bit21>rtk_dtv_demod_sel=0, <bit18:16>Dtv_frontend_mode=0x4, <bit8>dtv_idata_sel, <bit4>Dtv_dnflt_en for RTK Demod don't care
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00040400);				//input fifo clr, dtv_input_fifo_clr<bit18>, iq_adc_fifo<bit10> = 0x1, FIFO waddr & raddr clear
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00000000);				//input fifo clr release, dtv_input_fifo_clr<bit18>, iq_adc_fifo<bit10> = 0x0, FIFO waddr & raddr clear
		rtd_maskl(0xb8000844, 0xFFFFFF0F, 0x00000030);				//GPIO_13, Set pinmux DEMOD_IF_AGC
	...
	case RTK_DEMOD_MODE_QAM:
		rtd_outl(0xb8033800, 0x10240000);							//<bit29:28>dtmb_data_valid_delay=0x1(delay data valid), <bit21>rtk_dtv_demod_sel=1, <bit18:16>Dtv_frontend_mode=0x4, <bit8>dtv_idata_sel=0, <bit4>Dtv_dnflt_en=0
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00040400);				//input fifo clr, dtv_input_fifo_clr<bit18>, iq_adc_fifo<bit10> = 0x1, FIFO waddr & raddr clear
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00000000);				//input fifo clr release, dtv_input_fifo_clr<bit18>, iq_adc_fifo<bit10> = 0x0, FIFO waddr & raddr clear
		rtd_maskl(0xb8000844, 0xFFFFFF0F, 0x00000030);				//GPIO_13, Set pinmux DEMOD_IF_AGC
	...
	case RTK_DEMOD_MODE_SPEC_AN:
		...
		rtd_maskl(0xb8000848, 0xFF0FFFFF, 0x00600000);      		//GPIO_15, Set pinmux = 0x6(For DEMOD_S_IF_AGC)
	...
	case RTK_DEMOD_MODE_ATV:
		...
		rtd_maskl(0xb8000844, 0xFFFFFF0F, 0x00000050);				//GPIO_13, Set pinmux ATV_IF_AGC
		break;
...
int DemodBisrCheck(void)
{
	...
	rtd_maskl(TOPBIST_afbc_line_buffer_bisr_rstn_reg, ~TOPBIST_afbc_line_buffer_bisr_rstn_bisr_rstn_memory_bank_07_2_mask, TOPBIST_afbc_line_buffer_bisr_rstn_bisr_rstn_memory_bank_07_2(1));

	while (TOPBIST_afbc_line_buffer_bisr_done_get_memory_bank_07_2_bisr_done(rtd_inl(TOPBIST_afbc_line_buffer_bisr_done_reg)) != 1) {
		loopcnt++;
		tv_osal_usleep(100);
		if (loopcnt > 120)
			break;
	...
	RTK_DEMOD_INFO("BisrCheck done_status= %d (loopcnt=%d) \n", TOPBIST_afbc_line_buffer_bisr_done_get_memory_bank_07_2_bisr_done(rtd_inl(TOPBIST_afbc_line_buffer_bisr_done_reg)), loopcnt);

	rtd_maskl(TOPBIST_afbc_line_buffer_bisr_remap_reg, ~TOPBIST_afbc_line_buffer_bisr_remap_bisr_remap_memory_bank_07_2_mask, TOPBIST_afbc_line_buffer_bisr_remap_bisr_remap_memory_bank_07_2(1));
