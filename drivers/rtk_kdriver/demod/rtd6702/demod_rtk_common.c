...
#define TPO_DIVIDEND_CONSTANT 1000000 //250*1000 modify for mark2
...
int DtvGetICVersion()
{
	...
	*/
	...
	ICVersion = rtd_inl(0xb8060000);//mark2 ChipID

	if(ICVersion == 0x67020000)
		return 1;//Version A
	else
...
{
	//past from script
	//Mark2 ADC register default
	//rtd_outl(0xb8033100, 0x00010900);					//<bit16=1>IFD MBIAS power on,<bit11:10>REG_I_VCM_SEL=0x2(0.656V),<bit9:8>ADC2X_BG_SEL= 0x1(0.606V),  <bit7:0>REG_IFD_RESERVED=0x00
	//rtd_outl(0xb8033104, 0x00000100);					//<bit24>IQADC input cm power off, <bit9:8>REG_IPGA_BW=0x1(75M)
	//rtd_outl(0xb8033118, 0x00000010);					//<bit4>REG_CKOUT_SEL_Q: the default is "1:I_CKOUT" on AIF spec
	//rtd_outl(0xb8033120, 0x00000000);					//IF ADC GAIN FIX EN <bit0> = 0(SIF mode)
	//rtd_outl(0xb8033124, 0x00000000);					//PGA/ADC Power Off
	//rtd_outl(0xb8033128, 0x00100000);					//<bit21:20>REG_QPGA_BW=0x1(75M)
	...
	//rtd_outl(0xb803312C, 0x00000800);					//default setting, <bit11:10>REG_Q_VCM_SEL=0x2(0.656V)
	//rtd_outl(0xb8033130, 0xCC000000);					//default setting, <bit31:30>REG_QADC_I_VREF_SEL=0x3(2X),<bit27:26>REG_IADC_I_VREF_SEL=0x3(2X)
	//rtd_outl(0xb8033134, 0x22000055);					//default setting, <bit29:28>REG_QADC_VREF_SEL=0x2(1.4V), <bit25:24>REG_IADC_VREF_SEL=0x2(1.4V)
...
{
	if(DtvGetICVersion()) {
		rtd_maskl(0xb8033100, 0xfffffcff, 0x00000300);		//REG_ADC2X_BG_SEL<1:0>=11  for Mark2 VerA only
	}
	rtd_maskl(0xb8033130, 0x33ffffff, 0xCC000000);      //<bit31:30>REG_QADC_I_VREF_SEL=0x3(2X),<bit27:26>REG_IADC_I_VREF_SEL=0x3(2X)
	...
	rtd_maskl(0xb803312c, 0xffeecc00, 0x00000000);		//<bit20>Q single_end=0x0, <bit16>I single_end=0, <bit13:12>Q_CKIN_Sel=00(from PLL27x_IFADC), <bit9:8>I_CKIN_Sel=00: From PLL27X_IFADC, <bit7:4>reg_qpga_d=0x0(0dB), <bit3:0>reg_ipga_d=0x0(0dB)
	if(DtvGetICVersion()) {
		rtd_maskl(0xb8033134, 0xffffff88, 0x00000077);		//<bit6:4>REG_QADC_LDO_SEL=0x7(0.959V), <bit2:0>REG_IADC_LDO_SEL=0x7(0.959V)
	}
	rtd_maskl(0xb803311c, 0xfff8fff8, 0x00060006);		//<18:16>REG_QADC_CAPCAL= 0x6, <2:0>REG_IADC_CAPCAL=0x6
	...
	case RTK_DEMOD_MODE_SPEC_AN:
		...
		if(DtvGetICVersion()) {//Ver.A DVBS use ATV mode
...
{
	int Iotp=0, Qotp=0;					
	unsigned int ifd_iadc_ldo_data = 0, ifd_qadc_ldo_data = 0;

	if (rtk_otp_field_read_by_name("hw_ifd_iadc_ldo", (unsigned char*)&ifd_iadc_ldo_data, 1) != true) {
		RTK_DEMOD_INFO("hw_ifd_iadc_ldo OTP data read fail \n");
		ifd_iadc_ldo_data = 0;
	}

	if (rtk_otp_field_read_by_name("hw_ifd_qadc_ldo", (unsigned char*)&ifd_qadc_ldo_data, 1) != true) {
		RTK_DEMOD_INFO("hw_ifd_qadc_ldo OTP data read fail \n");
		ifd_qadc_ldo_data = 0;
	}
	...
	if (Iotp == 1) {
		...
		if(!DtvGetICVersion()) {//~mark2 Ver.A: set default
...
void DemodPllDefault(void)
{
	...
	rtd_outl(0xb80004c4, 0x00000000);				//DTV PLLDIF PLLDIF_DIVDIG_SEL = 0000(2), PLLDIF_DIVADC_SEL = 0000(divide by 8)
	//rtd_outl(0xb80004d0,0x0044000e);				//PLL27X, LFP Setting, M<14>+2=16(VCO=432M)
	//rtd_outl(0xb80004d4,0x00086021);				//<bit24:21>VDD11_PAD_SEL = 0x0,<bit20:19>PLL_LDO_XTAL_SEL=0x1(1.76V)
	//<bit5:4>PLL27X_PS_54M_DELAY=0x2(VCO=432M),<bit0>PLL27X_PS_DUTY_SEL = larger duty cycle
	//rtd_outl(0xb80004d8,0x00003004);				//Default setting, <15:8>PLL_RESERVE=0x30, <bit2>PLL27X_VCORSTB=0x1(normal), <bit1>PLL27X_RSTB=0x0(Active Reset), <bit0>PLL27X_POW=0x0(Off).
	...
	rtd_outl(0xb8000514, 0x00000009);				//<bit3>PLL27x 108M_EN clk to digital=0x1(enable), <bit0>write bit used
	...
	rtd_outl(0xb8000504, 0x000e0404);				//PLL27X_IFADC VCO M,N, M<14>+2=16(VCO=432M)
	rtd_outl(0xb8000508, 0x00000000);				//PLL27X_IFADC Poweroff and VCORSTB reset
	tv_osal_usleep(10);

	//PLL27X
	//rtd_maskl(0xb80004d8, 0xfffffff0, 0x00000004);  //<bit2>PLL27X_VCORSTB=0x1(normal), <bit1>PLL27X_RSTB=0x0(Active Reset), <bit0>PLL27X_POW=0x0(Off).
	//rtd_maskl(0xb80004dc, 0xfdfffffe, 0x02000000);	//Set PLL_L2H_CML_POW<bit25> = enable(0x1) and MUX to DTV Mode<bit0>=0x0

	//PLL27X_IFADC
	//rtd_maskl(0xb8000500, 0xfffffff3, 0x0000000C);	//PLL27X_IFADC_ATV_CKO_SEL = 11 (135M or 162M (depend on VCO and REG_PLL27x_IFADC_DIV).
...
{
	rtd_maskl(0xb8033130, 0xFFFFFCFF, 0x00000000);					//Default setting: IADC_CKOUT_DIV<bit9:8>=0(/1)
	switch (mode) {
	case RTK_DEMOD_MODE_DTMB:
		...
		rtd_maskl(0xb80004d8, 0xFFFF00FF, 0x00003000);				//PLL_RESERVE<bit15:8> = 0x1, Select ADC CLK From PLLDIF
		rtd_maskl(0xb8000500, 0xFFFFFFEF, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80F0F8, 0x00100505);				//VCO = M<6:0>+2 = 16+2, N:00=1, 27 /1 *27=486MHz, PLL27X_IFADC_M<bit22:16> = 0x10(16), PLL27X_IFADC_N<bit25:24>=0x0(1), <11:8>PLL27X_IFADC_LF_RS=0101, PLL27X_IFADC PUMP current<bit2:0>=0x5(101:15uF)
		rtd_maskl(0xb80004d8, 0xFFFFEDFF, 0x00001000);				//<12>PLL_RESERVE[4]=1, <9>PLL_RESERVE[1]=0
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000084);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0b1000(/4), sys_clk = 486 / 4 = 121.5 MHz, ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0b0100(/4), Adc_clk = 486 / 4 =121.5MHz
		if(DtvGetICVersion()) {//Ver.A DTMB use ATV mode
			rtd_maskl(0xb80004dc, 0x9FFFFDFE, 0x40000201);				//<30:29>PLL27X_IFADC_DUTY = 2:(VCO/2case)25/75, <9>PLL27X_IFADC_DIV = 1(divide by 2), <0>PLL_CK_IFADC_MUX = 1(ATV mode)
			rtd_maskl(0xb8033130, 0xFFFFFCFF, 0x00000100);				//IADC_CKOUT_DIV<bit9:8>=1(/2)
		}
		rtd_maskl(0xb80004b8, 0xFFFD893B, 0x00021200);				//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF216M_DIV=0x0(div2), <bit13>DIF216M_EN=0x0, <7:6>LDPC_DIV=0x0(div1.75), <bit10>LDPC_EN=0x0, <bit2>PLL27X_IFADC_REF_SEL=XTAL
		break;
	...
	case RTK_DEMOD_MODE_ATSC:
		...
		rtd_maskl(0xb80004dc, 0xFDFFFFFE, 0x02000000);				//PLL_L2H_CML_POW <bit25> = 1(on), PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
		rtd_maskl(0xb80004d8, 0xFFFF00FF, 0x00003000);				//PLL_RESERVE<bit15:8> = 0x30, Select ADC CLK From PLLDIF
		rtd_maskl(0xb8000500, 0xFFFFFFEF, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80F0F8, 0x000e0505);				//VCO = M<6:0>+2 = 14+2, N:00=1, 27 /1 *16=432MHz, PLL27X_IFADC_M<bit22:16> = 0xE(14), PLL27X_IFADC_N<bit25:24>=0x0(1), <11:8>PLL27X_IFADC_LF_RS=0101, PLL27X_IFADC PUMP current<bit2:0>=0x5(101:15uF)
		rtd_maskl(0xb80004d8, 0xFFFFEDFF, 0x00001000);				//<12>PLL_RESERVE[4]=1, <9>PLL_RESERVE[1]=0
		if(!DtvGetICVersion()) {
			rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000000);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0x0(/2), sys_clk = 432 / 2 = 216MHz, ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0x0(/8), Adc_clk = 432 / 8=54MHz
		}else{
			rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000004);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0x0(/2), sys_clk = 432 / 2 = 216MHz, ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0x4(/4), Adc_clk = 432 / 4=108MHz
	...
	case RTK_DEMOD_MODE_QAM:
		...
		rtd_maskl(0xb80004d8, 0xFFFF00FF, 0x00003000);				//PLL_RESERVE<bit15:8> = 0x30, Select ADC CLK From PLLDIF
		rtd_maskl(0xb8000500, 0xFFFFFFEF, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80F0F8, 0x001A0804);				//VCO = M<6:0>+2 = 26+2, N:00=1, 27 /1 *28=756MHz, PLL27X_IFADC_M<bit22:16> = 0x1A(26), PLL27X_IFADC_N<bit25:24>=0x0(1), <11:8>PLL27X_IFADC_LF_RS=1000, <2:0>PLL27X_IFADC_IP=100
		rtd_maskl(0xb80004d8, 0xFFFFEDFF, 0x00001000);				//<12>PLL_RESERVE[4]=1, <9>PLL_RESERVE[1]=0
		if(!DtvGetICVersion()) {
			rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000078);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0b0111(/5), sys_clk = 756 / 5 = 151.2MHz, ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0b1000(/30), Adc_clk = 756 / 30 = 25.2MHz
		}else{
			rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000071);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0b0111(/5), sys_clk = 756 / 5 = 151.2MHz, ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0b0001(/15), Adc_clk = 756 / 15 = 50.4MHz
			rtd_maskl(0xb8033130, 0xFFFFFCFF, 0x00000100);				//IADC_CKOUT_DIV<bit9:8>=1(/2)
		}
		rtd_maskl(0xb80004b8, 0xFFFD893B, 0x00021200);				//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF216M_DIV=0x0(div2), <bit13>DIF216M_EN=0x0, <7:6>LDPC_DIV=0x0(div1.75), <bit10>LDPC_EN=0x0, <bit2>PLL27X_IFADC_REF_SEL=XTAL
	...
	case RTK_DEMOD_MODE_DVBC:
		...
		rtd_maskl(0xb8000500, 0xFFFFFFEF, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		if(!DtvGetICVersion()) {
			rtd_maskl(0xb8000504, 0xFC80F0F8, 0x00120505);				//VCO = M<6:0>+2 = 18+2, N:00=1, 27 /1 *20=540MHz, PLL27X_IFADC_M<bit22:16> = 0x12(18), PLL27X_IFADC_N<bit25:24>=0x0(1), <11:8>PLL27X_IFADC_LF_RS=0101, <2:0>PLL27X_IFADC_IP=101
		}else{
			rtd_maskl(0xb8000504, 0xFC80F0F8, 0x000e0505);				//VCO = M<6:0>+2 = 14+2, N:00=1, 27 /1 *16=432MHz, PLL27X_IFADC_M<bit22:16> = 0x0e(14), PLL27X_IFADC_N<bit25:24>=0x0(1), <11:8>PLL27X_IFADC_LF_RS=0101, <2:0>PLL27X_IFADC_IP=101
		}
		rtd_maskl(0xb80004d8, 0xFFFFEDFF, 0x00001000);				//<12>PLL_RESERVE[4]=1, <9>PLL_RESERVE[1]=0
		if(!DtvGetICVersion()) {
			rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000011);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0b0001(/2.5), sys_clk = 540 / 2.5 = 216MHz, ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0b0001(/15), Adc_clk = 540 / 15 = 36MHz
		}else{
			rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000006);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0b0000(/2), sys_clk = 432 / 2 = 216MHz, ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0b0110(/3), Adc_clk = 432 /3  = 144MHz
			rtd_maskl(0xb8033130, 0xFFFFFCFF, 0x00000300);				//IADC_CKOUT_DIV<bit9:8>=3(/4)
		}
		rtd_maskl(0xb80004b8, 0xFFFD893B, 0x00021200);				//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF216M_DIV=0x0(div2), <bit13>DIF216M_EN=0x0, <7:6>LDPC_DIV=0x0(div1.75), <bit10>LDPC_EN=0x0, <bit2>PLL27X_IFADC_REF_SEL=XTAL
	...
	case RTK_DEMOD_MODE_DVBT2:
		...
		rtd_maskl(0xb80004d8, 0xFFFF00FF, 0x00003000);				//PLL_RESERVE<bit15:8> = 0x30, Select ADC CLK From PLLDIF
		rtd_maskl(0xb8000500, 0xFFFFFFEF, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80F0F8, 0x00160505);				//VCO = M<6:0>+2 = 22+2, N:00=1, 27 /1 *24=648MHz, PLL27X_IFADC_M<bit22:16> = 0x16(22), PLL27X_IFADC_N<bit25:24>=0x0(1), <11:8>PLL27X_IFADC_LF_RS=0101, PLL27X_IFADC PUMP current<bit2:0>=0x5(101:15uF)
		rtd_maskl(0xb80004d8, 0xFFFFEDFF, 0x00001000);				//<12>PLL_RESERVE[4]=1, <9>PLL_RESERVE[1]=0
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000037);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0x3(/1.25)(40/60), sys_clk = 648 / 1.25 = 518.4 MHz, ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0x7(/4.5)(33/67)), Adc_clk = 648 / 4.5 =144MHz
		rtd_maskl(0xb80004b8, 0xFFFD893B, 0x00027400);				//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF216M_DIV=0x1(div3), <bit13>DIF216M_EN=0x1, <bit12>DTVDIG_EN=0x1, <bit10>LDPC_EN=0x1, <7:6>LDPC_DIV=0x0(div1.75), <bit2>PLL27X_IFADC_REF_SEL=0(XTAL)
	...
	case RTK_DEMOD_MODE_SPEC_AN:
		...
		rtd_maskl(0xb80004d8, 0xFFFF00FF, 0x00003000);				//PLL_RESERVE<bit15:8> = 0x30, Select ADC CLK From PLLDIF
		rtd_maskl(0xb8000500, 0xFFFFFFEF, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80F0F8, 0x00100505);				//VCO = M<6:0>+2 = 16+2, N:00=1, 27 /1 *18=486MHz, PLL27X_IFADC_M<bit22:16> = 0x10(16), PLL27X_IFADC_N<bit25:24>=0x0(1), <11:8>PLL27X_IFADC_LF_RS=0101, <2:0>PLL27X_IFADC_IP=101
		rtd_maskl(0xb80004d8, 0xFFFFEDFF, 0x00001000);				//<12>PLL_RESERVE[4]=1, <9>PLL_RESERVE[1]=0
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x000000A4);				//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0b1010(/1), sys_clk = 486 / 1 = 486 MHz, ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0b0100(/4), Adc_clk = 486 / 4 =121.5MHz
		rtd_maskl(0xb80004b8, 0xFFFD893B, 0x00021440);				//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF216M_DIV=0x0(div2), <bit13>DIF216M_EN=0x0, <bit12>DTVDIG_EN=0x1, <7:6>LDPC_DIV=0x1(div1.25), <bit10>LDPC_EN=0x1, <bit2>PLL27X_IFADC_REF_SEL=0(XTAL)
		if(DtvGetICVersion()) {//Ver.A DVBS use ATV mode{
			rtd_maskl(0xb80004dc, 0x9FFFFDFE, 0x40000201);				//<30:29>PLL27X_IFADC_DUTY = 2:(VCO/2case)25/75, <9>PLL27X_IFADC_DIV = 1(divide by 2), <0>PLL_CK_IFADC_MUX = 1(ATV mode)
			rtd_maskl(0xb8033130, 0xFFFFCCFF, 0x00001100);				//IADC_CKOUT_DIV<bit9:8>=1(/2),QADC_CKOUT_DIV<bit13:12>=1(/2)
		}
	...
	case RTK_DEMOD_MODE_ATV:
		...
		rtd_maskl(0xb80004d8, 0xFFFFDEFF, 0x00002000);				//<13>PLL_RESERVE[5]=1, <8>PLL_RESERVE[0]=0
		rtd_outl(0xb80004dc, 0x42010001);           				//[30:29]PLL27X_IFADC_DUTY=0x01(vco/4), [25]PLL_L2H_CML_POW=0x1, [17:16]PLL27X_CK108M_DIV=0x1(vco=432M), [9]PLL27X_IFADC_DIV=0x0(divide 4), [0]PLL_CK_IFADC_MUX=0x1(ATV mode)
		...
		rtd_maskl(0xb8000504, 0xFC80F0F8, 0x00160505);				//VCO = M<6:0>+2 = 22+2, N:00=1, 27 /1 *24=648MHz, PLL27X_IFADC_M<bit22:16> = 0x16(22), PLL27X_IFADC_N<bit25:24>=0x0(1), <11:8>PLL27X_IFADC_LF_RS=0101, <bit2:0>PLL27X_IFADC_IP=0x5(101:15uF)
		rtd_maskl(0xb80004d8, 0xFFFFEDFF, 0x00001000);				//<12>PLL_RESERVE[4]=1, <9>PLL_RESERVE[1]=0
...
int DemodFrontendPathSetting(RTK_DEMOD_MODE mode)
{
	...
	case RTK_DEMOD_MODE_DTMB:
		...
		rtd_maskl(0xb806024C, 0xFFFF0FFF, 0x00004000);				//ST_GPIO_89, Set pinmux DEMOD_IF_AGC
	...
	case RTK_DEMOD_MODE_SPEC_AN:
		...
		//rtd_maskl(0xb8000838, 0x0FFFFFFF, 0xa0000000); 	//scart_fsw_0_ps/MIS_GPIO_bit52, Set pinmux  DEMOD_IF_AGC
		rtd_maskl(0xb806023C, 0xFFFFFF0F, 0x000000C0);				//<7:4>st_gpio_74_ps, Set pinmux = 0xC(For DEMOD_IF_AGC)
	...
	case RTK_DEMOD_MODE_ATV:
		...
		//rtd_maskl(0xb8000844, ~(0xf << 28), (0x3 << 28));	//gpio_61: ATV_IF_AGC
		rtd_maskl(0xb806024C, 0xFFFF0FFF, 0x00003000);				//ST_GPIO_89, Set pinmux ATV_IF_AGC
		break;
...
int DemodTsOutClkCtrl(unsigned int TsClkKHz)
{
	...
	//result = TPO_DIVIDEND_CONSTANT / TsClkKHz;  //parallel mode
	result = TPO_DIVIDEND_CONSTANT / (TsClkKHz * 4); /*2  bits mode */
...
int DemodBisrCheck(void)
{
	...
	rtd_maskl(TOPBIST_demod_bisr_rstn_reg, ~TOPBIST_demod_bisr_rstn_bisr_rstn_memory_bank_07_2_mask, TOPBIST_demod_bisr_rstn_bisr_rstn_memory_bank_07_2(1));

	while (TOPBIST_demod_bisr_done_get_memory_bank_07_2_bisr_done(rtd_inl(TOPBIST_demod_bisr_done_reg)) != 1) {
		loopcnt++;
		tv_osal_usleep(100);
		if (loopcnt > 120)
			break;
	...
	RTK_DEMOD_INFO("BisrCheck done_status= %d (loopcnt=%d) \n", TOPBIST_demod_bisr_done_get_memory_bank_07_2_bisr_done(rtd_inl(TOPBIST_demod_bisr_done_reg)), loopcnt);

	rtd_maskl(TOPBIST_demod_bisr_remap_reg, ~TOPBIST_demod_bisr_remap_hold_remap_memory_bank_07_2_mask, TOPBIST_demod_bisr_remap_hold_remap_memory_bank_07_2(1));
...
int AtvDemodInitialization(RTK_DEMOD_MODE mode)
{
	...
	DemodPllSetting(mode);
	tv_osal_usleep(50);
	IQAdcLdoSetting();                          //add IFADC_LDO setting from OTP setting
	DemodAdcSetting(mode);
	tv_osal_usleep(50);
	TvFrontendPowerControl(1);                    //Turn on TVFrontend
