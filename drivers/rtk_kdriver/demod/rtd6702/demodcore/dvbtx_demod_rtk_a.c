...
)
{
	...
	rtd_maskl(0xb816008c, 0xFFFFFFF0, 0x2); //data delay for sd ram to sync buf  // need check setting at Mark2
...

{
	...
	for (i = 0; i < 20; i++) {
		...
		CellIdVld = ATB_DEMOD_TIME_RO_RESERV_BIT_5_0_get_cell_id_vld(rtd_inl(ATB_DEMOD_TIME_RO_RESERV_BIT_5_0_reg));
		if (CellIdVld == 1) {
