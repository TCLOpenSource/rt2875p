...
int main(void)
{
	...
	U32BITS BerNum, BerDen, PerNum, PerDen;
	double Ber, Per;
	S32BITS SnrDbNum, SnrDbDen;
	double SnrDb;
	U32BITS SignalStrength, SignalQuality;
...
typedef enum {
	REALTEK_R_DVBC_SRS_FULL = 0,
	REALTEK_R_DVBC_SRS_HBR = 1,
	REALTEK_R_OC_SRS = 2,
	REALTEK_R_CCFO = 3,
} FFTMODE;
...
enum QAM_REG_BIT_NAME {
	...
	QAM_OPT_MPEG_OUT_SEL,					// for RTD2974 QAM only
	QAM_CKOUTPAR,
	QAM_CKOUT_PWR,
	QAM_CDIV_PH0,
	QAM_CDIV_PH1,
	...
	//FW used dummy regiser
	QAM_DUMMY_AUTO_SYMBOLRATE_SHIFT_ENABLE,
	QAM_DUMMY_AUTO_SYMBOLRATE_SHIFT_START,
	QAM_DUMMY_TR_LOCK_FLAG,
	QAM_DUMMY_TR_LOCK_SIGDETECT_DONE,
	QAM_DUMMY_AUTO_QAM_DETECT_ENABLE,
	QAM_DUMMY_AUTO_QAMMODE_DETECT_START,
	QAM_DUMMY_AUTO_QAMMODE_UNLOCK_FLAG,
	QAM_DUMMY_AUTO_QAMMODE_DETECT_INITIAL_FLAG,
	QAM_DUMMY_AUTO_QAMMODE_SEARCH,
	QAM_DUMMY_AUTO_QAMMODE_CHANGE,
	QAM_DUMMY_TR_LOCK_THD,
	QAM_DUMMY_AUTO_QAMMODE_NEXT,
	QAM_DUMMY_AUTO_QAMMODE_STORE,
	QAM_DUMMY_SYMBOLRATE_CENTER_15_8,
	QAM_DUMMY_SYMBOLRATE_CENTER_7_0,
	QAM_DUMMY_SYMBOLRATE_OFFSET_INDEX,
};
...
@brief   QAM demod aagc loop polarity setting function pointer

One can use QAM_DEMOD_FP_SET_AAGC_LOOP_POL() to set QAM demod aagc loop polarity.


@param [in]   pDemod         The demod module pointer
@param [in]   AagcLoopPol    AAGC Loop Polarity for setting


@retval   FUNCTION_SUCCESS   Set demod aagc loop polarity successfully.
@retval   FUNCTION_ERROR     Set demod aagc loop polarity unsuccessfully.


@note
	-# Demod building function will set QAM_DEMOD_FP_SET_AAGC_LOOP_POL() with the corresponding function.
...
*/
typedef int
(*QAM_DEMOD_FP_SET_AAGC_LOOP_POL)(
	QAM_DEMOD_MODULE *pDemod,
	int AagcLoopPol
);
...
@brief   QAM demod tr lock asking function pointer

One can use QAM_DEMOD_FP_IS_TR_LOCKED() to ask QAM demod if it is tr-locked.
...
@note
	-# Demod building function will set QAM_DEMOD_FP_IS_TR_LOCKED() with the corresponding function.

*/
typedef int
(*QAM_DEMOD_FP_IS_TR_LOCKED)(
	QAM_DEMOD_MODULE *pDemod,
	int *pAnswer
);
...
@brief   QAM demod Cr lock asking function pointer

One can use QAM_DEMOD_FP_IS_CR_LOCKED() to ask QAM demod if it is cr-locked.
...
@note
	-# Demod building function will set QAM_DEMOD_FP_IS_CR_LOCKED() with the corresponding function.

*/
typedef int
(*QAM_DEMOD_FP_IS_CR_LOCKED)(
	QAM_DEMOD_MODULE *pDemod,
	int *pAnswer
);
...
@brief   QAM demod inner lock asking function pointer

One can use QAM_DEMOD_FP_IS_INNER_LOCKED() to ask QAM demod if it is inner-locked.
...
@note
	-# Demod building function will set QAM_DEMOD_FP_IS_INNER_LOCKED() with the corresponding function.

*/
typedef int
(*QAM_DEMOD_FP_IS_INNER_LOCKED)(
	QAM_DEMOD_MODULE *pDemod,
	int *pAnswer
);
...
@brief   QAM demod Nosig asking function pointer

One can use QAM_DEMOD_FP_IS_NO_SIG() to ask QAM demod if it is Nosig.
...
@brief   QAM demod NBD and SRS function pointer
@One can use QAM_DEMOD_FP_NBD_SRS() to get no signal flag and symbol rate.

@param [in]	pDemod  	The demod module pointer
@param [out]	pNosig   		Return No signal Flag
@param [out] 	pPowerDiff   	Inpunt Signal Spectrum magnitude for debug no signal flag
@param [out] 	pSymbolRate  Return Auto detected symbol rate

@retval   FUNCTION_SUCCESS   Get demod no signal flag and symbol rate successfully.
@retval   FUNCTION_ERROR     Get demod no signal flag and symbol rate unsuccessfully.

*/
typedef int
(*QAM_DEMOD_FP_NBD_SRS)(
	QAM_DEMOD_MODULE *pDemod,
...
@brief   QAM demod Auto Qam Detection function pointer
@One can use QAM_DEMOD_FP_AUTO_QAMD() to get the qam mode.

@param [in]	pDemod  	The demod module pointer
@param [out] 	pQammode    Return Auto detected current qam mode
...
typedef int
(*QAM_DEMOD_FP_AUTO_QAMD_REG_PRESET)(
	QAM_DEMOD_MODULE *pDemod,
	U32BITS SymbolRateHz
);

typedef int
(*QAM_DEMOD_FP_AUTO_QAMD_REG_POSTSET)(
	QAM_DEMOD_MODULE *pDemod,
	U32BITS SymbolRateHz
);

typedef int
(*QAM_DEMOD_FP_AUTO_QAMD_PRECHECK)(
	QAM_DEMOD_MODULE *pDemod,
	U32BITS SymbolRateHz
);
typedef int
(*QAM_DEMOD_FP_SET_CCFO_METHOD_EN)(
	QAM_DEMOD_MODULE *pDemod,
...
typedef int
(*QAM_DEMOD_FP_IS_CCFO_METHOD_EN)(
	QAM_DEMOD_MODULE *pDemod,
...
typedef void
(*QAM_DEMOD_FP_GET_CCFO_INIT_FREQ)(
	QAM_DEMOD_MODULE *pDemod,
	U32BITS SymbolRate,
...
typedef int
(*QAM_DEMOD_FP_SET_CCFO_INIT_FREQ)(
	QAM_DEMOD_MODULE *pDemod,
...
typedef int
(*QAM_DEMOD_FP_GET_SRS_CCFO_DONE_FLAG)(
	QAM_DEMOD_MODULE *pDemod,
	unsigned int *SrsCcfoDoneFlag
);

typedef int
(*QAM_DEMOD_FP_GET_TS_THROUGHPUT)(
	QAM_DEMOD_MODULE *pDemod,
	unsigned int *TSThroughputKBps
);

typedef int
(*QAM_DEMOD_FP_SET_AUTOMODE_EN)(
	QAM_DEMOD_MODULE *pDemod,
	unsigned char enable
);
...
@brief   Auto mode SRS,CCFO, and QAM mode function pointer
@One can use QAM_DEMOD_FP_AUTO_SRS() to get csr,fsr,ccfo,qam mode.
...
typedef int
(*QAM_DEMOD_FP_AUTO_MODE_PRESET)(
	QAM_DEMOD_MODULE *pDemod
);

typedef int
(*QAM_DEMOD_FP_AUTO_MODE_POSTSET)(
	QAM_DEMOD_MODULE *pDemod
);

typedef int
(*QAM_DEMOD_FP_AUTO_MODE_SET_SR_HZ)(
	QAM_DEMOD_MODULE *pDemod,
	U32BITS SymbolRateHz
);

typedef int
(*QAM_DEMOD_FP_SET_SFI_CANCEL)(
	QAM_DEMOD_MODULE *pDemod
);

typedef int
(*QAM_DEMOD_FP_SFI_FULL_TRACKING)(
	QAM_DEMOD_MODULE *pDemod
);
/**

@brief   QAM demod Set FFT mode function pointer
@One can use QAM_DEMOD_FP_SET_FFT_MODE() to set the fft mode.

@param [in]	pDemod  	The demod module pointer
@param [in] 	mode    Select FFT mode

@retval   FUNCTION_SUCCESS   Set demod FFT mode successfully.
@retval   FUNCTION_ERROR     Set demod FFT mode unsuccessfully.

*/
typedef int
(*QAM_DEMOD_FP_SET_FFT_MODE)(
	QAM_DEMOD_MODULE *pDemod,
	FFTMODE mode
);
...
/// RTD2974 QAM extra module alias
typedef struct RTD2974_QAM_EXTRA_MODULE_TAG RTD2974_QAM_EXTRA_MODULE;

// RTD2974 QAM extra module
struct RTD2974_QAM_EXTRA_MODULE_TAG {
	// Variables
	int ConfigMode;
	int MpegOffSyncCount;
};

/// RTD2945 QAM extra module alias
typedef struct RTD2945_QAM_EXTRA_MODULE_TAG RTD2945_QAM_EXTRA_MODULE;

// RTD2945 QAM extra module
struct RTD2945_QAM_EXTRA_MODULE_TAG {
	// Variables
	int ConfigMode;
	int MpegOffSyncCount;
};

/// REALTEK_R QAM extra module alias
typedef struct REALTEK_R_QAM_EXTRA_MODULE_TAG REALTEK_R_QAM_EXTRA_MODULE;

// REALTEK_R QAM extra module
struct REALTEK_R_QAM_EXTRA_MODULE_TAG {
	// Variables
	int ConfigMode;
	int MpegOffSyncCount;
};
...
struct QAM_DEMOD_MODULE_TAG {
	...
	unsigned int Symbolratedelay;
	unsigned int TrLdThreshold;
	unsigned int AutoModeSRSCnt;

	union {										///<   Demod extra module used by driving module
		REALTEK_R_QAM_EXTRA_MODULE  RealtekRQam;
	}
	Extra;
	...
	QAM_DEMOD_FP_SET_AAGC_LOOP_POL     SetAagcLoopPol;
	...
	QAM_DEMOD_FP_IS_TR_LOCKED          IsTrLocked;
	QAM_DEMOD_FP_IS_CR_LOCKED          IsCrLocked;
	QAM_DEMOD_FP_IS_INNER_LOCKED       IsInnerLocked;
	QAM_DEMOD_FP_IS_NO_SIG             IsNoSig;
	...
	QAM_DEMOD_FP_RS_RESET     		   RSReset;
	QAM_DEMOD_FP_NBD_SRS        	   NbdAndSrs;
	QAM_DEMOD_FP_AUTO_QAMD        	   AutoQamd;
	QAM_DEMOD_FP_AUTO_QAMD_REG_PRESET        	   AutoQamdRegPreset;
	QAM_DEMOD_FP_AUTO_QAMD_REG_POSTSET       AutoQamdRegPostset;
	QAM_DEMOD_FP_AUTO_QAMD_PRECHECK        	   AutoQamdPreCheck;
	QAM_DEMOD_FP_SET_CCFO_METHOD_EN	SetCcfoMethodEn;
	QAM_DEMOD_FP_IS_CCFO_METHOD_EN	IsCcfoMethodEn;
	QAM_DEMOD_FP_GET_CCFO_INIT_FREQ	GetCcfoInitFreq;
	QAM_DEMOD_FP_SET_CCFO_INIT_FREQ		SetCcfoInitFreq;
	QAM_DEMOD_FP_GET_SRS_CCFO_DONE_FLAG	GetSrsCcfoDoneFlag;
	QAM_DEMOD_FP_GET_TS_THROUGHPUT 	GetTsThroughput;

	QAM_DEMOD_FP_SET_FFT_MODE		SetFftMode;
	QAM_DEMOD_FP_SET_AUTOMODE_EN	SetAutoModeEn;
	QAM_DEMOD_FP_AUTO_MODE        	AutoMode;
	QAM_DEMOD_FP_AUTO_MODE_PRESET	AutoModePreset;
	QAM_DEMOD_FP_AUTO_MODE_POSTSET	AutoModePostset;
	QAM_DEMOD_FP_AUTO_MODE_SET_SR_HZ	AutoModeSetSrHz;
	QAM_DEMOD_FP_SET_SFI_CANCEL	SetSfiCancel;
	QAM_DEMOD_FP_SFI_FULL_TRACKING SfiFullTracking;
};
...
#define REALTEK_R_QAM_BASE_PRINT_LVL_DBG      0
#define REALTEK_R_QAM_BASE_PRINT_LVL_INFO     1
#define REALTEK_R_QAM_BASE_PRINT_LVL_WARNING  2


#ifndef REALTEK_R_QAM_BASE_PRINT_LVL
#define REALTEK_R_QAM_BASE_PRINT_LVL          REALTEK_R_QAM_BASE_PRINT_LVL_INFO
#endif


#define REALTEK_R_QAM_BASE_DBG(fmt, args...)        rtd_demod_print(KERN_DEBUG,"QAM_BASE_DEBUG, " fmt, ##args);
#define REALTEK_R_QAM_BASE_INFO(fmt, args...)       rtd_demod_print(KERN_INFO,"QAM_BASE_INFO, " fmt, ##args);
#define REALTEK_R_QAM_BASE_WARNING(fmt, args...)    rtd_demod_print(KERN_WARNING,"QAM_BASE_WARNING, " fmt, ##args);
