...
static unsigned char m_atv_video_mode = 0;
static ATV_SOUND_STD_MAIN_SYSTEM m_atv_audio_mode = ATV_SOUND_UNKNOWN_SYSTEM;
static ATV_SOUND_STD_MAIN_SYSTEM g_atv_audio_mode = ATV_SOUND_UNKNOWN_SYSTEM;
static int m_atv_scan_mode = 0xf;
static char m_atv_video_modulation = 0;
...
void drv_ifd_init(void)
{
	...
	IFD_DEBUG_MSG(IFD_MSG_INFO, "drv_ifd_init....\n");

	m_atv_video_mode = 0;
	m_atv_audio_mode = ATV_SOUND_UNKNOWN_SYSTEM;
	g_atv_audio_mode = ATV_SOUND_UNKNOWN_SYSTEM;
	m_atv_scan_mode = 0xf;
	...
	drv_ifd_set_cr_lock_freq_thr(IFD_CR_LOCK_OFFSET_THD_1050K);
	...
	rtd_maskl(IFD_108_cr_auto_reset_en_reg, ~(0x1<<4), (0x1<<4)); //cr_unlock_track_en
	...
	drv_ifd_agc_set_peak_slowcharge_val(GET_ATV_IFD_AGC_PEAK_SLOWCHARGE_VAL());
	...
	rtd_outl(IFD_108_agc_max_countforcoarsemode_agc_lock2_reg, GET_ATV_IFD_AGCLOOP1_DNSAMPLE() << 4);
	...
	drv_ifd_agc_loop1_peak_cnt(GET_ATV_IFD_AGCLOOP1_PEAKCNT());
	drv_ifd_agc_loop2_peak_cnt(GET_ATV_IFD_AGCLOOP2_PEAKCNT());
	...
	IFD_DEBUG_MSG(IFD_MSG_INFO, "drv_ifd_init....done\n");
}
...
void drvif_ifd_set_scanchannel(StructATVModule *pAtvModule)
{
	...
	CH_mode = 1;		// scan_mode
	g_atv_audio_mode = pAtvModule->audio_mode;
	...
	if ((pAtvModule->video_mode != m_atv_video_mode) || (pAtvModule->audio_mode  != m_atv_audio_mode) || (pAtvModule->scan_mode != m_atv_scan_mode)) {
	...
	*/
	...
	m_atv_video_mode = pAtvModule->video_mode;
	m_atv_audio_mode = pAtvModule->audio_mode;
	m_atv_scan_mode = CH_mode;
...
{
	bool CH_mode;
	char atv_video_modulation = 0;

	CH_mode = 0;		// normal_mode

	//CH_mode = pAtvModule->scan_mode;
	g_atv_audio_mode = pAtvModule->audio_mode;
	...
	*/
		...
		if (((pAtvModule->freq < 162300000)
			...
			if ((drv_ifd_adc_clk_sel_get_info() != IFD_CLK_RATE_135M) || ((rtd_inl(PLL27X_REG_SYS_PLL_27X4_reg) & 0x0100) != 0x0100)) {
				IFD_DEBUG_MSG(IFD_MSG_INFO,
							  "Freq="PT_U32BITS"; IFD ADC CLOCK=135MHz\n",
							  pAtvModule->freq);
			...
			if ((drv_ifd_adc_clk_sel_get_info() !=
				...
				IFD_DEBUG_MSG(IFD_MSG_INFO,
							  "Freq="PT_U32BITS"; IFD ADC CLOCK=DEFAULT\n",
							  pAtvModule->freq);
				drv_ifd_adc_clk_sel((IFD_CLK_RATE)
	...
	if ((pAtvModule->video_mode != m_atv_video_mode) || (pAtvModule->audio_mode  != m_atv_audio_mode) || (pAtvModule->scan_mode != m_atv_scan_mode)) {
		...
		*/
		...
		if (atv_video_modulation != m_atv_video_modulation) {
	...
	m_atv_audio_mode = pAtvModule->audio_mode;
	m_atv_scan_mode = CH_mode;
	m_atv_video_modulation = atv_video_modulation;
...
void drv_ifd_set_video_ntsc(bool scan_mode)
{
	...
	//drv_ifd_agc_dgain_en(1);

	if (scan_mode) {
		drv_ifd_agc_adaptivetarget_en(IFD_DISABLE);
		//drv_ifd_agc_loop2_dn_sample(GET_ATV_IFD_AGCLOOP2_DNSAMPLE());
	} else {
		drv_ifd_agc_adaptivetarget_en(IFD_ENABLE);
	}
...
void drv_ifd_set_video_pal(bool scan_mode)
{
	...
	if (scan_mode) {
		...
		if (g_atv_audio_mode == ATV_SOUND_I_SYSTEM) {
			curTunerIFVal = stIFDUserData.tunerPara.tunerPIF_I;
		} else if (g_atv_audio_mode == ATV_SOUND_BG_SYSTEM) {
			curTunerIFVal = stIFDUserData.tunerPara.tunerPIF_B;
		} else if (g_atv_audio_mode == ATV_SOUND_MN_SYSTEM) {
			curTunerIFVal = stIFDUserData.tunerPara.tunerPIF_MN;
		} else if (g_atv_audio_mode == ATV_SOUND_L_SYSTEM) {
			curTunerIFVal = stIFDUserData.tunerPara.tunerPIF_L;
		} else if (g_atv_audio_mode == ATV_SOUND_LA_SYSTEM) {
			curTunerIFVal = stIFDUserData.tunerPara.tunerPIF_LA;
		} else {
			curTunerIFVal = stIFDUserData.tunerPara.tunerPIF_DK;
		}
		IFD_DEBUG_MSG(IFD_MSG_SETCH, "g_atv_audio_mode=%d, curTunerIFVal=%d\n",
			g_atv_audio_mode, (UINT32) curTunerIFVal);
...
void drv_ifd_set_video_secaml(bool scan_mode)
{
	...
	rtd_outl(IFD_108_agc_enable_sel_reg, 0x000000FF);

	if (scan_mode) {
		drv_ifd_agc_adaptivetarget_en(IFD_DISABLE);
		drv_ifd_agc_dgain_en(IFD_DISABLE);
	} else {
		drv_ifd_agc_dgain_en(IFD_ENABLE);
		drv_ifd_agc_adaptivetarget_en(IFD_ENABLE);
	}
	...
	drv_ifd_cr_loop_filter_kp(GET_ATV_IFD_CR_PGAIN());
	drv_ifd_cr_loop_filter_ki(GET_ATV_IFD_CR_IGAIN());
	drv_ifd_set_agc_loop1_peak_force(IFD_DISABLE);
	drv_ifd_set_agc_loop2_peak_force(IFD_DISABLE);
	drv_ifd_agc_loop1_kp_settings(GET_ATV_IFD_AGCLOOP1_KPGAIN());
	drv_ifd_agc_loop1_ki_settings(GET_ATV_IFD_AGCLOOP1_KIGAIN());
	drv_ifd_agc_loop1_pishift_settings(GET_ATV_IFD_AGCLOOP1_KPKISHIFT());
...
void drvif_ifd_set_scan_mode_detect_cr_lock(UINT32 curr_tuner_if_freq)
{
	...
	IFD_DEBUG_MSG(IFD_MSG_SETCH, "[CR_Power]ifd_adc_clock(%d)\n", ifd_adc_clock);
	IFD_DEBUG_MSG(IFD_MSG_SETCH, "[CR_Power]curr_tuner_if_freq(%d)\n", curr_tuner_if_freq);
	IFD_DEBUG_MSG(IFD_MSG_SETCH, "[CR_Power]tuner_if_freq(%d)\n", tuner_if_freq);
	IFD_DEBUG_MSG(IFD_MSG_SETCH, "[CR_Power]if_nco(%x)\n", if_nco);
	...
	for (nco_count = 0; nco_count <= 7; nco_count++) {
		...
		IFD_DEBUG_MSG(IFD_MSG_SETCH, "[CR_Power]if_noise_point[%d](%x)\n", nco_count, noise_point[nco_count]);
	}
