                                 ...
                                 sony_i2c_t * pDemodI2c)
{
    ...
    pDemod->iffreqConfig.configISDBT_6 = SONY_DEMOD_MAKE_IFFREQ_CONFIG(5.0);//SONY_DEMOD_MAKE_IFFREQ_CONFIG(3.55);
    pDemod->iffreqConfig.configISDBT_7 = SONY_DEMOD_MAKE_IFFREQ_CONFIG(5.0);//SONY_DEMOD_MAKE_IFFREQ_CONFIG(4.15);
    pDemod->iffreqConfig.configISDBT_8 = SONY_DEMOD_MAKE_IFFREQ_CONFIG(5.0);//SONY_DEMOD_MAKE_IFFREQ_CONFIG(4.75);
                                    ...
                                    int32_t value)
{
    ...
    case SONY_DEMOD_CONFIG_SAT_SINGLECABLE_SPECTRUM_INV:
        pDemod->isSinglecableIqInv = value ? 1 : 0;
        break;
    ...
    case SONY_DEMOD_CONFIG_TLVVALID_MASK_IN_NULL:
        /* This register can change only in Sleep state */
        if (pDemod->state != SONY_DEMOD_STATE_SLEEP) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_SW_STATE);
        }
...
sony_result_t sony_demod_SetTSClockModeAndFreq (sony_demod_t * pDemod, sony_dtv_system_t system)
{
    ...
    /* slave    Bank    Addr    Bit    default    Name
     * -----------------------------------------------------
     * <SLV-T>  01h     E7h     [0]    1'b1       OCHG_BYPASS (0: TS serial in ISDB-S3,   1: others(default))
     */
    if ((system == SONY_DTV_SYSTEM_ISDBS3) && (serialTS & 0x80)){
...
sony_result_t sony_demod_SetTLVClockModeAndFreq (sony_demod_t * pDemod, sony_dtv_system_t system)
{
    ...
    /* slave    Bank    Addr    Bit    default    Name
     * -----------------------------------------------------
     * <SLV-T>  01h     E7h     [0]    1'b1       OCHG_BYPASS (0: TLV serial, 2bit parallel in ISDB-S3, 1: others(default))
     */
    if ((system == SONY_DTV_SYSTEM_ISDBS3) && ((serialTLV & 0x80) || (twoBitParallel & 0x01))) {
        result = sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xE7, 0, 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
