...
ssize_t demod_proc_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	case DEMOD_PROC_ATSONOFF: {
	        ats_onoff = (unsigned char)input_data;

		if (input_data == 1) {
	            DEMOD_MGR_WARNING("Enable Demod ATS\n");
		} else {
	            DEMOD_MGR_WARNING("Disable Demod ATS\n");
	        }
	        break;
	    }
...
ssize_t demod_proc_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	if(str == NULL){
		DEMOD_MGR_WARNING("demod_proc_read kmalloc failed\n");
		return 0;
	}
