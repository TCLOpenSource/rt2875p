...
extern unsigned char IsATSC3McmPkgFlag;
...
)
{
	...
	static const INIT_REG_ENTRY InitRegTable[] = {
		...
		{0xde53,			7,		5,		0x3 			},// ori 0x3, cr_cr2_carb_wb
		...
		{0xd06c,		   16,		6,		0x3c0 			},
		{0xd06e,		   11,		1,		0x3c0 			},
		{0xd54c,		   3,		0,		0x5 			},// ori 0x3, check_blk_times
		{0xd552,		   12,		0,		0x142 			},// ori 0x123, fast_thd
		{0xd556,		   0,		0,		0x1 			},// ori 0x1, stay_carb_bn_enable
		{0xd565,		   12,		0,		0x38 			},// ori 0x1f, rm_mean_thd
		//{0xd56d,		   7,		0,		0x0 			},// Burst Noise, [7:0]: atsc_soft_llr_cal_opt
		{0xd56f,		   7,		0,		0x0 			},// Burst Noise, [5]: opt_erasure_df, [4]: opt_erasure_num_fix, [3:0]opt_erasure_num, 0x2: enable
		//{0xd8b6,		   15,		0,		0x100 			},// Burst Noise, [15:0]reg_delta
		{0xd572,		   7,		0,		0x14 			},// Monitoring CR offset diff, [7:0]cr_offset_dif_start_unit=0x14 (20ms)
		{0xd57f,		   0,		0,		0x0 			},// Monitoring CR offset diff, [0]cr_offset_dif_read_en, 0x1: enable
		//{0xd8b5,		   0,		0,		0x1 			},// ori 0x0, atsc_iter_en
		{0xd8b8,		   0,		0,		0x1 			},// ori 0x0, reg_follow_carb_nb_flag
		{0xde40,		   21,		3,		0x6a00 			},// ori 0x8000, cr_cr2_cr_offset_cnt_win
		{0xdeb8,		   5,		5,		0x1 			},// ori 0x0, cci_all_reset_en
		...
		{0xde60,			2,		2,		0x1				},// ori 0x0, comp_buf_out
	#else
		{0xde60,			2,		2,		0x0				},// ori 0x0, comp_buf_out
	#endif
		...
		{0xdea8,		   11,		6,		0x27			},// ori 0x4, cci_det_start_st=39
		{0xdea9,			9,		4,		0x1f			},// ori 0x10, cci_det_fft_n (0x3f for R2.2)
		{0xdeab,		    5,		0,		0x10			},// ori 0x4, cci_pos_around_cnt_thd
		{0xdeab,		   11,		6,		0x20			},// ori 0x6, cci_rej_start_st=32
		...
		{0xd1e0,			7, 		0,		0x6				},
		{0xd1e2,			7, 		0,		0x95			},
		{0xd1e0,			7, 		0,		0xc				},//st20, fsm_adj_detect_reset_n=0x0
		{0xd1e2,			7, 		0,		0xe6			},//st20, fsm_adj_detect_reset_n=0x0
		{0xd1e1,			7, 		0,		0x18			},
		{0xd1e0,			7, 		0,		0x14			},
		{0xd1e2,			7, 		0,		0x88			},
		{0xd1e0,			7, 		0,		0x16			},
	...
	if (IsATSC3McmPkgFlag == 2) {
		if (pDemod->RegAccess.Addr16Bit.SetRegBits(pDemod, ATSC_INBUF_EN, 1) != FUNCTION_SUCCESS)
			goto error_status_set_demod_registers;

		if (pDemod->RegAccess.Addr16Bit.SetRegBits(pDemod, ATSC_ITER_EN, 1) != FUNCTION_SUCCESS)
			goto error_status_set_demod_registers;

		if (pDemod->RegAccess.Addr16Bit.SetRegBits(pDemod, ATSC_CR_CR2_SWBW_COND_SEL, 0) != FUNCTION_SUCCESS)
			goto error_status_set_demod_registers;
		
	} else {
		rtd_maskl(DTV_DEMOD_MISC_ATSC_ITER_OUTER_DMA_CTRL_reg, ~DTV_DEMOD_MISC_ATSC_ITER_OUTER_DMA_CTRL_atsc_iter_dma_sw_rst_mask, DTV_DEMOD_MISC_ATSC_ITER_OUTER_DMA_CTRL_atsc_iter_dma_sw_rst(0x0));	//ori 0x0, atsc_iter_dma_sw_rst (misc)
		
		if (pDemod->RegAccess.Addr16Bit.SetRegBits(pDemod, ATSC_ITER_EN, 1) != FUNCTION_SUCCESS)
			goto error_status_set_demod_registers;

		if (pDemod->RegAccess.Addr16Bit.SetRegBits(pDemod, ATSC_CR_CR2_SWBW_COND_SEL, 15) != FUNCTION_SUCCESS)
			goto error_status_set_demod_registers;
	}
...
)
{
	...
	U32BITS TrPhaseIncBinary = 0;
	S32BITS TrOffsetN2Int = 0;
	S32BITS TrFsFreqInt = 0;
	S32BITS TrPhaseIncInt = 0, TrPhaseInc_set = 0;
	S32BITS TrOffset = 0, TrFsFreq = 0, TrPhaseInc = 0, TrPhaseIncBinary_Final = 0;
...
)
{
	static const ATSC_PRIMARY_REG_ENTRY_ADDR_16BIT PrimaryRegTable[] = {
		...
		{ATSC_CR_CR2_CARB_WB,                               0xde53,         7,     	5   },
		{ATSC_CR_CR2_SWBW_COND_SEL,                         0xde5c,         4,     	1   },
		{ATSC_CR_CR2_SWBW,									0xde61,			4,		4	},
		...
		{ATSC_INBUF_EN,						0xd8b5,			1,		1	},
		{ATSC_ITER_EN,						0xd8b5,			0,		0	},
		...
		{Dummy_8051_STAY_CARB_NB_IND,			0xf091,			7,		0	},
		{Dummy_8051_CR_GL_DIFF_IND,				0xf092,			7,		0	},
		{Dummy_CHANGE_CHANNEL_IND,              0xf095,         7,      0   },
		{Dummy_8051_CR_SW_MODE_IND,				0xf098,			7,		0	},
...
)
{
	...
	if (atsc_demod_resume_done != 1) {
		if (IsATSC3McmPkgFlag == 2) {
			if (pDemod->RegAccess.Addr16Bit.SetRegBytes(pDemod, 0x3000, realtek_r_atsc_FirewareCodeTableMcm,
		...
		REALTEK_R_ATSC_BASE_DBG("ATSC 1.0 DownloadFirmwareCode Done!!!\n");

		if (IsAtsc3McmPackage(RTK_DEMOD_MODE_ATSC) == 1)
			atsc_demod_resume_done = 1;

	} else {
		REALTEK_R_ATSC_BASE_DBG("Skip ATSC 1.0 DownloadFirmwareCode Flow!!!\n");
	}
...
// ATSC Demod 8051 FW V2.B (B815C1D4 = 0x2b)
// fsm_cr_fmu/fsm_cr_pmu (st32,33,34), KTASKWBS-5745 patch, ADJ_DET, and FS2 range,
// CR flag SW mode Merlin7 (LGE K8), 20210427
// REALTEK_R ATSC 8051 demod firmware code table
unsigned char realtek_r_atsc_FirewareCodeTableMcm[REALTEK_R_ATSC_FIRMWARE_CODE_TABLE_LEN] = {
0x75,
0x54,
0x0,
0x75,
