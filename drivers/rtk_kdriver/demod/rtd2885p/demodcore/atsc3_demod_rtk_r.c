...
int  realtek_r_atsc3_mplp_cti_detect(ATSC3_DEMOD_MODULE * pDemod, unsigned char* flag)
{
	...
	unsigned char L1DPlpALayer = 0, L1DPlpBLayer = 0, L1DPlpCLayer = 0, L1DPlpDLayer = 0;
	unsigned char plpSelectedCnt = 0, plpCoreLayerCnt = 0;
	int  i32CntTmp = 0;
		...
		if (L1ParsingDone == 0x3) {
			...
			*flag = 0x0;
			...
			if (*flag == 1) {
				...
				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED50, &ReadingData);
				L1DPlpBLayer = (ReadingData >> 16) & 0x3;
				if (L1DPlpBLayer == 0)
					plpCoreLayerCnt++;

				if ((plpCoreLayerCnt == 1) && (plpSelectedCnt > 2)) {
					ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED98, &ReadingData);
					L1DPlpCLayer = (ReadingData >> 16) & 0x3;
					if (L1DPlpCLayer == 0)
						plpCoreLayerCnt++;
				}

				if ((plpCoreLayerCnt == 1) && (plpSelectedCnt > 3)) {
					ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDE0, &ReadingData);
					L1DPlpDLayer = (ReadingData >> 16) & 0x3;
					if (L1DPlpDLayer == 0)
						plpCoreLayerCnt++;
			}

				if (plpCoreLayerCnt == 1)
					*flag = 0;
