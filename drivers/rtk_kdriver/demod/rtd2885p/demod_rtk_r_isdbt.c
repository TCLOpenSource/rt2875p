...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^  ERROR   need to modify
	...
	if (!ptr) {
		RTK_R_ISDB_WARNING("[PLI] allocation failure... Allocate size = %d\n", size);
		return 0;
	}

	nonCachedAddr_local = (unsigned char *)(ptr + DEF_MEM_SIZE);
	phyAddr_local = (unsigned long *)(ptr + DEF_MEM_SIZE);
	RTK_R_ISDB_INFO("realtek_demod pli_allocContinuousMemoryMesg size=0x%x ptr=0x%p nonCachedAddr_local=0x%p phyAddr_local=0x%lx DEF_MEM_SIZE=0x%x\n", size, ptr, nonCachedAddr_local, phyAddr_local, DEF_MEM_SIZE);
...
kmalloc_fail:
	RTK_R_ISDB_WARNING("RTK_R_ISDBT_Constructors kmalloc Failed\n");
}
...
int RTK_R_ISDBT_Init(DEMOD* pDemodKernel)
{
	...
	//REG_VALUE_ENTRY_RBUS*  OptRegTable = NULL;
	int i;
	...
	if (pDemodKernel->m_pTuner) {
		...
		if (pDemodKernel->m_pTuner->GetParam(pDemodKernel->m_pTuner, TV_SYS_ISDBT_6M, &Param) == TUNER_CTRL_OK) {
			...
			case TV_TUNER_NXP_TDA18275A:
				...
				pRtkrISDB->m_pTunerOptRegRbus = (REG_VALUE_ENTRY_RBUS*) TDA18275A_OPT_VALUE_ISDBT;
				pRtkrISDB->m_TunerOptRegCnt = sizeof(TDA18275A_OPT_VALUE_ISDBT) / sizeof(REG_VALUE_ENTRY_RBUS);
				break;
			...
			case TV_TUNER_NO_TUNER:
				...
				pRtkrISDB->m_pTunerOptRegRbus = (REG_VALUE_ENTRY_RBUS*) SI2158_OPT_VALUE_ISDBT;
				pRtkrISDB->m_TunerOptRegCnt  = sizeof(SI2158_OPT_VALUE_ISDBT) / sizeof(REG_VALUE_ENTRY_RBUS);
				break;
			case TV_TUNER_MAXLINEAR_MXL661:
				RTK_R_ISDB_INFO("Tuner id %d, TV_TUNER_MAXLINEAR_MXL6x1\n", Param.id);
				pRtkrISDB->m_pTunerOptRegRbus = (REG_VALUE_ENTRY_RBUS*) MXL601_OPT_VALUE_ISDBT;
				pRtkrISDB->m_TunerOptRegCnt  = sizeof(MXL601_OPT_VALUE_ISDBT) / sizeof(REG_VALUE_ENTRY_RBUS);
				break;
			case TV_TUNER_RAFAEL_R840:
				RTK_R_ISDB_INFO("Tuner id %d, TV_TUNER_RAFAEL_R840\n", Param.id);
				pRtkrISDB->m_pTunerOptRegRbus = (REG_VALUE_ENTRY_RBUS*) R840_OPT_VALUE_ISDBT;
				pRtkrISDB->m_TunerOptRegCnt  = sizeof(R840_OPT_VALUE_ISDBT) / sizeof(REG_VALUE_ENTRY_RBUS);
				break;
			case TV_TUNER_SONY_SUTRX243:
			case TV_TUNER_SONY_SUTRX243_RA:
				RTK_R_ISDB_INFO("Tuner id %d, TV_TUNER_SONY_SUTRC234\n", Param.id);
				pRtkrISDB->m_pTunerOptRegRbus = (REG_VALUE_ENTRY_RBUS*) SUTRX243_OPT_VALUE_ISDBT;
				pRtkrISDB->m_TunerOptRegCnt  = sizeof(SUTRX243_OPT_VALUE_ISDBT) / sizeof(REG_VALUE_ENTRY_RBUS);
				break;
			default:
	...
	for (i = 0; i < pRtkrISDB->m_TunerOptRegCnt; i++) {
		U32BITS Reg = pRtkrISDB->m_pTunerOptRegRbus[i].RegBitNameRbus;
		U32BITS Val = pRtkrISDB->m_pTunerOptRegRbus[i].ValueRbus;
	...
	RTK_R_ISDB_INFO("RTK_R_ISDBT Init Complete\n");
...
int RTK_R_ISDBT_SetTVSysEx(DEMOD* pDemodKernel, TV_SYSTEM sys, const TV_SIG_INFO* pSigInfo)
{
	...
	*/
	...
	pRtkrISDB->m_status_checking_stime = tv_osal_time() + ISDBT_STATUS_CHECK_INTERVAL_MS;
...
int RTK_R_ISDBT_GetLockStatus(DEMOD* pDemodKernel, unsigned char* pLock)
{
	...
	if (IsCheckLockBy != BY_PREFRM_LOCK) { //checked by inner/outer lock
		...
		if (*pLock == DTV_SIGNAL_LOCK) {
			...
			snr = (SnrDen!=0) ? (SnrNum * 10000 / SnrDen) : 0;
			if (snr <= SNR_THD_FOR_FRM_LOCK) {
				OuterLock = (rtd_inl(0xB816E4DC) & 0x7);//[0]: layerA, [1]:layerB, [2]:layerC
				RsberValid[0] = OuterLock & 0x1;
				RsberValid[1] = (OuterLock >> 1) &0x1;
				RsberValid[2] = (OuterLock >> 2) &0x1;
...
int RTK_R_ISDBT_GetSignalQuality(DEMOD* pDemodKernel, ENUM_TV_QUAL id, TV_SIG_QUAL* pQuality)
{
	...
	case TV_QUAL_EMERGENCY_FLAG:
		ret = DECODE_RET(pDemod->GetRegBits(pDemod, ISDBT_EMERGENCY_FLAG, &EmergencyFlag));
		if (ret == TUNER_CTRL_OK)
			pQuality->emergency_flag = (unsigned char)EmergencyFlag;
		break;
	...
	case TV_QUAL_CUSTOMER_LG:
		...
		RTK_R_ISDB_INFO("\033[1;32;32m""ISDBT_layer = %d, BerNum = "PT_U32BITS", PerNum = "PT_U32BITS"\n""\033[m", MaxSeglayer , BerNum, PerNum);
...
int RTK_R_ISDBT_KernelAutoTune(DEMOD* pDemodKernel)
{
	...
	*/
	...
	cur_time = tv_osal_time();
#ifndef CONFIG_RTK_KDRV_DEMOD_SCAN_THREAD_ENABLE
	if ((cur_time > pRtkrISDB->m_status_checking_stime) || ((pRtkrISDB->m_status_checking_stime - cur_time) > ISDBT_STATUS_CHECK_INTERVAL_MS)) {
		RTK_R_ISDBT_GetLockStatus(pDemodKernel, &LockStatus);
		if (LockStatus == 1)
			RTK_R_ISDBT_GetSignalQuality(pDemodKernel, TV_QUAL_CUSTOMER_LG, &Quality);
		pRtkrISDB->m_status_checking_stime = tv_osal_time() + ISDBT_STATUS_CHECK_INTERVAL_MS;
	}
#endif
...
RTK_R_ISDBT_DRIVER_DATA* AllocRtkrDriver_ISDBT(COMM* pComm,	unsigned char Addr, U32BITS CrystalFreq, int TSMode)
{
	...
	if (pDriver) {
		...
		if (RTK_R_ISDBTT_DEMUX_DDR_SIZE) {
			...
			ret = DtvDemodMemRemappingInt(RTK_R_ISDBTT_DEMUX_DDR_SIZE, &pDriver->ShareMemoryPhysicalAddress, &AllocateSize);
			...
			RTK_R_ISDB_INFO("allocate share memory - addr=%p/%08lx", pDriver->pShareMemory, pDriver->ShareMemoryPhysicalAddress);
			if (pDriver->ShareMemoryPhysicalAddress & 0xfff) {
				RTK_R_ISDB_WARNING("AllocRtkrDriver_ISDBT failed, share memory allocation is not 4KB align\n");
				//assert(0);
			}
