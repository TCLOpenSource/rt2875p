...
int REALTEK_R_ATSC_Init(DEMOD* pDemodKernel)
{
	...
	if (IsAtsc3McmPackage(RTK_DEMOD_MODE_ATSC) == 0) {
		...
		rtd_maskl(RTK_DTV_DEMOD_SYS_DEMOD_SEL_reg, 0xFFFFFFFC, 0x00000002); //[1:0]demod_sel, 0x2: ATSC
		pDemod->RegAccess.Addr16Bit.SetRegBits(pDemod, DW_8051_RST, 0); //dw51_rst_in_n
		tv_osal_msleep(1);
...
int REALTEK_R_Ext_ATSC_System_Init(DEMOD* pDemodKernel, unsigned char On)
{
	...
	ret |= REALTEK_R_ATSC_Ipga_Vcm_Setting(pDemodKernel);
...
/*
	...
	if(OtpDone) {
		ret |= atsc_rtd_4bytes_i2c_mask(pDemod, 0xb8033100, 0xFFFFF3FF, ((rtd_inl(EFUSE_DATAO_98_reg) >> 24) & 0x03)<< 10);
	}
*/
	return ret;
}