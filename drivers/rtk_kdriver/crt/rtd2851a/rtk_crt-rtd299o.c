...
#include <mach/io.h>
#include <rtk_kdriver/tvscalercontrol/vdc/video.h>
...
/**
   ...
   return 1: get lock
**/
inline int rtd_swsem_trylock(int bit)
{
          int swsem;
          int ret;
...
void rtd_swsem_lock(int bit)
{
          int retry = 0;

          while (!rtd_swsem_trylock(bit)) {
#if !defined (RTD_SWSEM_DEBUG_DISABLE)		  
		  if (retry++ == 0x100000) {
...
int APLL_ADC_clock(enum CRT_CLK clken)
{
	...
	if (CLK_ON == clken) {
		if (((rtd_inl(SYS_REG_SYS_SRST1_reg)&SYS_REG_SYS_SRST1_rstn_apll_adc_mask) == 0) ||
			((rtd_inl(SYS_REG_SYS_CLKEN1_reg)&SYS_REG_SYS_CLKEN1_clken_apll_adc_mask) == 0)) {
			rtd_setbits(SYS_REG_SYS_SRST1_reg, SYS_REG_SYS_SRST1_rstn_apll_adc_mask);
			rtd_setbits(SYS_REG_SYS_CLKEN1_reg, SYS_REG_SYS_CLKEN1_clken_apll_adc_mask);
		}
	}
	hw_semaphore_unlock();
	return CRT_SUCCESS;
...
{
	hw_semaphore_try_lock();
#define USB_PHY_DELAY	1000
	if (CLK_ON == clken) {
		rtd_maskl(SYS_REG_SYS_CLKEN2_reg, 0xfffbffff, 0x00000000); // CLKEN_USB bit[18]=1 Disable(clock off)
		//rst = 0
		rtd_maskl(SYS_REG_SYS_SRST2_reg, 0xfffbffff, 0x00000000); // RSTN_USB MAC bit[18](reset off,active on)
		udelay(USB_PHY_DELAY);
		rtd_maskl(SYS_REG_SYS_SRST3_reg, 0xffffff0c, 0x00000000); // RSTN_USB2_PHY_P3~P0 bit[7:4]=1(reset off,active on)
		udelay(USB_PHY_DELAY);

		rtd_maskl(SYS_REG_SYS_SRST3_reg, 0xffffff0c, 0x000000f3); // RSTN_USB2_PHY_P3~P0 bit[7:4]=1(reset off,active on)
		udelay(USB_PHY_DELAY);
		rtd_maskl(SYS_REG_SYS_CLKEN2_reg, 0xfffbffff, 0x00040000); // CLKEN_USB bit[18]=1 Enable(clock on)
		udelay(USB_PHY_DELAY);
		rtd_maskl(SYS_REG_SYS_CLKEN2_reg, 0xfffbffff, 0x00000000); // CLKEN_USB bit[18]=1 Disable(clock off)
		udelay(USB_PHY_DELAY);
		rtd_maskl(SYS_REG_SYS_SRST2_reg, 0xfffbffff, 0x00040000); // RSTN_USB MAC bit[18](reset off,active on)
		udelay(USB_PHY_DELAY);
		rtd_maskl(SYS_REG_SYS_CLKEN2_reg, 0xfffbffff, 0x00040000); // CLKEN_USB bit[18]=1 Enable(clock on)

		//TODO: check the usage of this bit
		rtd_maskl(SYS_REG_SYS_SRST0_reg, 0xFFFFFFFF, 0x0100);

		// USB MAC Reset
		rtd_maskl(USB2_PHY_UTMI_reg, 0xfffffff0, 0x0000000f);
		udelay(USB_PHY_DELAY);
		//rtd_outl(0xb8013008,0x0000000f);  //UTMI USB2 reset to PHY bit[3:0]P0~P3 reset off
		rtd_maskl(USB2_PHY_UTMI_reg, 0xfffffff0, 0x00000000);
		udelay(USB_PHY_DELAY);
		//rtd_outl(0xb8013008,0x00000000);  //UTMI USB2 reset to PHY bit[3:0]P0~P3 reset on

#ifdef CONFIG_USB_XHCI_AUTO_SUSPEND
		rtd_outl(USB_TMP_1_reg, 0x1);      //USB3.0 avoid no UTMI clock, only for RTD299O
		udelay(USB_PHY_DELAY);
		rtd_outl(USB3_PHY_PIPE_reg, 0x1);     //pipe3
		udelay(USB_PHY_DELAY);
		rtd_outl(USB3_PHY_PIPE_reg, 0x0);
		udelay(USB_PHY_DELAY);
#else
		rtd_outl(ANA_PHY0_reg,0xe);  //USB3 ANA PHY bit[3:0]
		udelay(USB_PHY_DELAY);
#endif
		rtd_outl(ANA_PHY1_reg, 0xf); //USB2 POW gating bit[3:0]P0~P3
		udelay(USB_PHY_DELAY);
		...
		// enable interrupt
		rtd_setbits(SYS_REG_INT_CTRL_SCPU_reg, BIT(19));
		udelay(USB_PHY_DELAY);
	} else {

		// disable interrupt
		rtd_clearbits(SYS_REG_INT_CTRL_SCPU_reg, BIT(19));
		udelay(USB_PHY_DELAY);
	
		// stop xhci
		//rtd_outl(0xb8050020,0x00000000);
		//udelay(USB_PHY_DELAY);

		rtd_maskl(SYS_REG_SYS_CLKEN2_reg, 0xfffbffff, 0x00000000); // CLKEN_USB bit[18]=1 Disable(clock off)
		//rst = 0
		rtd_maskl(SYS_REG_SYS_SRST2_reg, 0xfffbffff, 0x00000000); // RSTN_USB MAC bit[18](reset on,active on)
		udelay(USB_PHY_DELAY);
		rtd_maskl(SYS_REG_SYS_SRST3_reg, 0xffffff0c, 0x00000000); // RSTN_USB2_PHY_P3~P0 bit[7:4]=1(reset on,active on)
		udelay(USB_PHY_DELAY);
	}
#undef USB_PHY_DELAY
	hw_semaphore_unlock();
...
int IrDA_CriticalRegister(enum CRT_CLK clken, void *private_data)
{
	...
	int raw_mode = *(int *)private_data;

	hw_semaphore_try_lock();

	if (clken == CLK_ON) {
		rtd_clearbits(STB_ST_CLKEN1_reg, BIT(2));   //clken_irda:0
		rtd_clearbits(STB_ST_SRST1_reg, BIT(2));    //RSTN_irda:0
		rtd_setbits(STB_ST_CLKEN1_reg, BIT(2));   	//clken_irda:1
		rtd_clearbits(STB_ST_CLKEN1_reg, BIT(2));   //clken_irda:0
		rtd_setbits(STB_ST_SRST1_reg, BIT(2));    	//RSTN_irda:1
		rtd_setbits(STB_ST_CLKEN1_reg, BIT(2));   	//clken_irda:1
		rtd_setbits(STB_ST_CLKEN0_reg, BIT(0)); // ISO_MISC clock
		rtd_setbits(STB_ST_SRST0_reg, BIT(0));  // ISO_MISC reset

		if (raw_mode == 1) {
			rtd_clearbits(STB_ST_CLKEN0_reg, BIT(4));   //clken_irda_off:0
			rtd_clearbits(STB_ST_SRST0_reg, BIT(4));    //RSTN_irda_off:0
			rtd_setbits(STB_ST_CLKEN0_reg, BIT(4));   	//clken_irda_off:1
			rtd_clearbits(STB_ST_CLKEN0_reg, BIT(4));   //clken_irda_off:0
			rtd_setbits(STB_ST_SRST0_reg, BIT(4));    	//RSTN_irda_off:1
			rtd_setbits(STB_ST_CLKEN0_reg, BIT(4));   	//clken_irda_off:1
			rtd_setbits(STB_ST_CLKEN0_reg, BIT(1)); // ISO_MISC clock
			rtd_setbits(STB_ST_SRST0_reg, BIT(1));  // ISO_MISC reset
		}
	} else {
		rtd_setbits(STB_ST_SRST1_reg, BIT(2));  // ISO_MISC_IRDA reset
		if (raw_mode == 1) {
			rtd_setbits(STB_ST_SRST0_reg, BIT(4));  // ISO_MISC_OFF reset
		}
...
int I2C_CriticalRegister(enum CRT_CLK clken , void *private_data)
{
	...
	if (clken == CLK_ON) {
		if (i2c_phy & BIT(0)) {
			rtd_clearbits(STB_ST_CLKEN0_reg, BIT(STB_ST_CLKEN0_clken_i2c0_off_shift));
			rtd_clearbits(STB_ST_SRST0_reg, BIT(STB_ST_SRST0_rstn_i2c0_off_shift));
			rtd_setbits(STB_ST_CLKEN0_reg, BIT(STB_ST_CLKEN0_clken_i2c0_off_shift));
			rtd_clearbits(STB_ST_CLKEN0_reg, BIT(STB_ST_CLKEN0_clken_i2c0_off_shift));
			rtd_setbits(STB_ST_SRST0_reg, BIT(STB_ST_SRST0_rstn_i2c0_off_shift));
			rtd_setbits(STB_ST_CLKEN0_reg, BIT(STB_ST_CLKEN0_clken_i2c0_off_shift));
		}
		if (i2c_phy & BIT(1)) {
			rtd_clearbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_i2c1_shift));
			rtd_clearbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_i2c1_shift));
			rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_i2c1_shift));
			rtd_clearbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_i2c1_shift));
			rtd_setbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_i2c1_shift));
			rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_i2c1_shift));
		}
		if (i2c_phy & BIT(2)) {
			rtd_clearbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_i2c2_shift));
			rtd_clearbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_i2c2_shift));
			rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_i2c2_shift));
			rtd_clearbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_i2c2_shift));
			rtd_setbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_i2c2_shift));
			rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_i2c2_shift));
		}
		if (i2c_phy & BIT(3)) {
			rtd_clearbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_i2c3_shift));
			rtd_clearbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_i2c3_shift));
			rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_i2c3_shift));
			rtd_clearbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_i2c3_shift));
			rtd_setbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_i2c3_shift));
			rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_i2c3_shift));
		}
...
int UART_CriticalRegister(enum CRT_CLK clken, void *private_data)
{
	...
	case 1:
		rtd_clearbits(SYS_REG_SYS_CLKEN2_reg, BIT(8));
		rtd_clearbits(SYS_REG_SYS_SRST2_reg, BIT(8));
		rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(8));
		rtd_clearbits(SYS_REG_SYS_CLKEN2_reg, BIT(8));
		rtd_setbits(SYS_REG_SYS_SRST2_reg, BIT(8));
		rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(8) | BIT(12));
		rtd_clearbits(SYS_REG_SYS_CLKSEL_reg, BIT(0));	 // set uart1 to 98MHz
		rtd_maskl(PLL27X_REG_SYS_PLL_AUD3_reg, 0xfffffff2, BIT(3));
		break;
	case 2:
		rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(7));
		break;
	case 3:
		rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(6));
		break;
	default:
		hw_semaphore_unlock();
		return CRT_FAIL;
...
int SE_CriticalRegister(enum CRT_CLK clken)
{
	...
	if (clken == CLK_ON) {
		...
		rtd_clearbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_se_shift));
		udelay(10);

		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_se_shift));
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_se2_shift));
		udelay(10);

		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_se_shift));
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_se2_shift));
		udelay(10);

		rtd_clearbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_se_shift));
		udelay(10);

		rtd_setbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_se_shift));
		udelay(10);
...
int NAND_CriticalRegister(enum CRT_CLK clken, void *private_data)
{
	...
	if (p_nf_clk == NULL) {
		...
		if (clken == CLK_ON) {
			rtd_outl(SYS_REG_SYS_CLKEN2_reg, rtd_inl(SYS_REG_SYS_CLKEN2_reg) | SYS_REG_SYS_CLKEN2_clken_nf_mask);
		} else {
			rtd_outl(SYS_REG_SYS_CLKEN2_reg, rtd_inl(SYS_REG_SYS_CLKEN2_reg) & (~SYS_REG_SYS_CLKEN2_clken_nf_mask));
		}
	} else {
...
int MD_CriticalRegister(enum CRT_CLK clken)
{
	...
	if (clken == CLK_ON) {
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, BIT(6));
		rtd_clearbits(SYS_REG_SYS_SRST1_reg, BIT(6));
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, BIT(6));
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, BIT(6));
		rtd_setbits(SYS_REG_SYS_SRST1_reg, BIT(6));
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, BIT(6));
	} else {
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, BIT(6));
	}
...
int GPU_CriticalRegister(enum CRT_CLK clken)
{
	...
	if (clken == CLK_ON) {
		...
		//OSWaitus(100);
		//enable GPU CLK
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, BIT(3));
		udelay(10);
		...
		//disable GPU RESET
		rtd_setbits(SYS_REG_SYS_SRST1_reg, BIT(3));
		udelay(10);
...
int PWM_CriticalRegister(enum CRT_CLK clken, void *private_data)
{
	...
	if (clken == CLK_ON) {
		if (pwm_type == 0) {
			/*iso*/
			rtd_setbits(STB_ST_CLKEN0_reg,
						BIT(STB_ST_CLKEN0_clken_pwm_shift));
			rtd_clearbits(STB_ST_CLKEN0_reg,
						  BIT(STB_ST_CLKEN0_clken_pwm_shift));
			rtd_clearbits(STB_ST_SRST0_reg,
						  BIT(STB_ST_SRST0_rstn_pwm_shift));
			rtd_setbits(STB_ST_SRST0_reg,
						BIT(STB_ST_SRST0_rstn_pwm_shift));
			rtd_setbits(STB_ST_CLKEN0_reg,
						BIT(STB_ST_CLKEN0_clken_pwm_shift));
		} else {
			/*misc*/
			rtd_setbits(SYS_REG_SYS_CLKEN2_reg,
						BIT(SYS_REG_SYS_CLKEN2_clken_pwm_shift));
			rtd_clearbits(SYS_REG_SYS_CLKEN2_reg,
						  BIT(SYS_REG_SYS_CLKEN2_clken_pwm_shift));
			rtd_clearbits(SYS_REG_SYS_SRST2_reg,
						  BIT(SYS_REG_SYS_SRST2_rstn_pwm_shift));
			rtd_setbits(SYS_REG_SYS_SRST2_reg,
						BIT(SYS_REG_SYS_SRST2_rstn_pwm_shift));
			rtd_setbits(SYS_REG_SYS_CLKEN2_reg,
						BIT(SYS_REG_SYS_CLKEN2_clken_pwm_shift));
		}
...
int PCMCIA_CriticalRegister(enum CRT_CLK clken)
{
	...
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_outl(SYS_REG_SYS_CLKEN2_reg, rtd_inl(SYS_REG_SYS_CLKEN2_reg) & ~SYS_REG_SYS_CLKEN2_clken_pc_mask);	/* disable clock */
		udelay(10);
		rtd_outl(SYS_REG_SYS_SRST2_reg, rtd_inl(SYS_REG_SYS_SRST2_reg) & ~SYS_REG_SYS_SRST2_rstn_pc_mask);	/* assert pcmcia */
		udelay(10);
		rtd_outl(SYS_REG_SYS_SRST2_reg, rtd_inl(SYS_REG_SYS_SRST2_reg) | SYS_REG_SYS_SRST2_rstn_pc_mask); 	/* deassert pcmcia */
		udelay(10);
		rtd_outl(SYS_REG_SYS_CLKEN2_reg, rtd_inl(SYS_REG_SYS_CLKEN2_reg) | SYS_REG_SYS_CLKEN2_clken_pc_mask);
	} else {
		rtd_outl(SYS_REG_SYS_CLKEN2_reg, rtd_inl(SYS_REG_SYS_CLKEN2_reg) & ~SYS_REG_SYS_CLKEN2_clken_pc_mask);	/* disable clock  */
	}
	hw_semaphore_unlock();
...
int GPIO_CriticalRegister(enum CRT_CLK clken)
{
	...
	if (clken == CLK_ON) {
		/*
		   rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_gpio_shift));
		   rtd_clearbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_gpio_shift));
		   rtd_clearbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_gpio_shift));
		   rtd_setbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_gpio_shift));
		   rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_gpio_shift));
		   */
		rtd_setbits(STB_ST_CLKEN0_reg, BIT(0)); // ISO_MISC clock
		rtd_setbits(STB_ST_SRST0_reg, BIT(0));  // ISO_MISC reset
...
int EMMC_CriticalRegister(enum CRT_CLK clken, void *private_data)
{
	...
	if (clken == CLK_ON) {
		...
		rtd_clearbits(SYS_REG_SYS_CLKEN2_reg, BIT(23));
		rtd_clearbits(SYS_REG_SYS_SRST2_reg, BIT(23));
		udelay(10);
		rtd_setbits(SYS_REG_SYS_SRST2_reg, BIT(23));
		rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(23));
...
int SDIO_CriticalRegister(enum CRT_CLK clken, void *private_data)
{
	...
	if (clken == CLK_ON) {
		...
		rtd_clearbits(SYS_REG_SYS_CLKEN3_reg, BIT(16));
		rtd_clearbits(SYS_REG_SYS_SRST3_reg, BIT(16));
		udelay(10);
		rtd_setbits(SYS_REG_SYS_SRST3_reg, BIT(16));
		rtd_setbits(SYS_REG_SYS_CLKEN3_reg, BIT(16));

		/* enable interrupt */
		rtd_setbits(SYS_REG_INT_CTRL_SCPU_reg, BIT(16));
		udelay(10);
		...
		/* disable interrupt */
		rtd_clearbits(SYS_REG_INT_CTRL_SCPU_reg, BIT(16));
		rtd_clearbits(SYS_REG_INT_CTRL_KCPU_reg, BIT(16));
		udelay(10);
	}
	hw_semaphore_unlock();
	return CRT_SUCCESS;
...
int SMC_CriticalRegister(enum CRT_CLK clken)
{
	...
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_clearbits(SYS_REG_SYS_CLKEN2_reg, SYS_REG_SYS_CLKEN2_clken_sc_mask);   /* Disable Clock */
		udelay(100);
		rtd_clearbits(SYS_REG_SYS_SRST2_reg, SYS_REG_SYS_SRST2_rstn_sc_mask);   /*HW Reset*/
		udelay(100);
		rtd_setbits(SYS_REG_SYS_SRST2_reg, SYS_REG_SYS_SRST2_rstn_sc_mask);     /* HW Reset*/
		udelay(100);
		rtd_setbits(SYS_REG_SYS_CLKEN2_reg, SYS_REG_SYS_CLKEN2_clken_sc_mask);     /* Enable Clock*/
	} else {
		rtd_clearbits(SYS_REG_SYS_CLKEN2_reg, SYS_REG_SYS_CLKEN2_clken_sc_mask);   /* Disable Clock */
	}
	hw_semaphore_unlock();
...
int CP_CriticalRegister(enum CRT_CLK clken)
{
	...
	if (clken == CLK_ON) {
		...
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, BIT(7));
		udelay(10);
		rtd_clearbits(SYS_REG_SYS_SRST1_reg, BIT(7));
		udelay(10);
		rtd_setbits(SYS_REG_SYS_SRST1_reg, BIT(7));
		udelay(10);
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, BIT(7));
		udelay(10);
	} else {
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, BIT(7));
		udelay(10);
...
int TP_CriticalRegister(enum CRT_CLK clken)
{
	...
	if (clken == CLK_ON) {
		...
		rtd_clearbits(SYS_REG_SYS_SRST1_reg, SYS_REG_SYS_SRST1_rstn_tp_mask);
		udelay(100);

		/* enable TP clock */
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, SYS_REG_SYS_CLKEN1_clken_tp_mask);
		udelay(100);

		/*  External Reset TP */
		rtd_setbits(SYS_REG_SYS_SRST1_reg, SYS_REG_SYS_SRST1_rstn_tp_mask);
		udelay(100);

	} else {
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, SYS_REG_SYS_CLKEN1_clken_tp_mask);
	}
...
int TPOUT_CriticalRegister(enum CRT_CLK clken)
{
	...
	if (clken == CLK_ON) {
		/* enable TPOUT clock */
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, SYS_REG_SYS_CLKEN1_clken_tp_div_mask);
		udelay(100);
	} else {
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, SYS_REG_SYS_CLKEN1_clken_tp_div_mask);
	}
...
int HDMI_CriticalRegister(enum CRT_CLK clken)
{
	...
	if (clken == CLK_ON) {
		...
		rtd_maskl(SYS_REG_SYS_SRST1_reg,
				  ...
				  (SYS_REG_SYS_SRST1_rstn_hdmi(1) |
				   SYS_REG_SYS_SRST1_rstn_apll_adc(1) |
				   SYS_REG_SYS_SRST1_rstn_offms(1) |
				   SYS_REG_SYS_SRST1_rstn_cbus(1)));

		rtd_maskl(SYS_REG_SYS_CLKEN1_reg,
				  ...
				  (SYS_REG_SYS_CLKEN1_clken_hdmi(1) |
				   SYS_REG_SYS_CLKEN1_clken_apll_adc(1) |
				   SYS_REG_SYS_CLKEN1_clken_offms(1) |
				   SYS_REG_SYS_CLKEN1_clken_cbus(1)));
		/* DDC */
		rtd_maskl(SYS_REG_SYS_SRST3_reg,
				  ~(SYS_REG_SYS_SRST3_rstn_ddc_mask),
				  (SYS_REG_SYS_SRST3_rstn_ddc(1)));
		rtd_maskl(SYS_REG_SYS_CLKEN3_reg,
				  ~(SYS_REG_SYS_CLKEN3_clken_ddc_mask),
				  (SYS_REG_SYS_CLKEN3_clken_ddc(1)));
	} else {
		/* HDMI & APLL &cbus & off measure */
		rtd_maskl(SYS_REG_SYS_SRST1_reg,
				  ~(SYS_REG_SYS_SRST1_rstn_hdmi_mask |
					...
					SYS_REG_SYS_SRST1_rstn_offms_mask |
					SYS_REG_SYS_SRST1_rstn_cbus_mask),
				  (SYS_REG_SYS_SRST1_rstn_hdmi(0) |
				   SYS_REG_SYS_SRST1_rstn_apll_adc(0) |
				   SYS_REG_SYS_SRST1_rstn_offms(0) |
				   SYS_REG_SYS_SRST1_rstn_cbus(0)));
		...
		rtd_maskl(SYS_REG_SYS_CLKEN1_reg,
				  ~(SYS_REG_SYS_CLKEN1_clken_hdmi_mask |
					...
					SYS_REG_SYS_CLKEN1_clken_offms_mask |
					SYS_REG_SYS_CLKEN1_clken_cbus_mask),
				  (SYS_REG_SYS_CLKEN1_clken_hdmi(0) |
				   SYS_REG_SYS_CLKEN1_clken_apll_adc(0) |
				   SYS_REG_SYS_CLKEN1_clken_offms(0) |
				   SYS_REG_SYS_CLKEN1_clken_cbus(0)));
		/* DDC */
		rtd_maskl(SYS_REG_SYS_SRST3_reg,
				  ~(SYS_REG_SYS_SRST3_rstn_ddc_mask),
				  (SYS_REG_SYS_SRST3_rstn_ddc(0)));
		rtd_maskl(SYS_REG_SYS_CLKEN3_reg,
				  ~(SYS_REG_SYS_CLKEN3_clken_ddc_mask),
				  (SYS_REG_SYS_CLKEN3_clken_ddc(0)));
	}
...
int VBI_CriticalRegister(enum CRT_CLK clken)
{
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_setbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_vbis0_shift));
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_vbis0_shift));
		udelay(10);
	} else {
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_vbis0_shift));
		rtd_clearbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_vbis0_shift));
		udelay(10);
	}
	hw_semaphore_unlock();
...
int ADEMOD_CriticalRegister(enum CRT_CLK clken)
{
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_setbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_if_demod_shift));
		rtd_setbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_ifadc_shift));
		rtd_setbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_atvin2_shift));
		rtd_setbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_atvin_shift));
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_if_demod_shift));
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_ifadc_shift));
		rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_atvin_ifd_shift));
		//rtd_clearbits(SYS_REG_SYS_CLKSEL_reg, BIT(SYS_REG_SYS_CLKSEL_atvin_clk_sel_shift));//Don't need to set when clk en = 0
		udelay(10);
	} else {
		rtd_clearbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_if_demod_shift));
		rtd_clearbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_ifadc_shift));
		rtd_clearbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_atvin2_shift));
		rtd_clearbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_atvin_shift));
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_if_demod_shift));
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_ifadc_shift));
		rtd_clearbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_atvin_ifd_shift));
		udelay(10);
	}
	hw_semaphore_unlock();
...
int ADEMOD_RST_CriticalRegister(enum CRT_CLK clken)
{
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_setbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_if_demod_shift));
...
int ADEMOD_IN_RST_CriticalRegister(enum CRT_CLK clken)
{
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_setbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_atvin2_shift));
...
int ADEMOD_CLK_PROTECT_BIT_CriticalRegister(enum CRT_CLK clken, void *private_data)
{
	int bits = *(int *)private_data;

	hw_semaphore_try_lock();
	if (clken == CLK_OFF)
		rtd_setbits(SB2_DUMMY_1_reg, BIT(bits));
	else
		rtd_clearbits(SB2_DUMMY_1_reg, BIT(bits));
...
int DDEMOD_CriticalRegister(enum CRT_CLK clken)
{
	...
	if (clken == CLK_ON) {
		rtd_setbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_if_demod_shift));
		rtd_setbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_ifadc_shift));
		rtd_setbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_dtv_demod_shift));
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_if_demod_shift));
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_ifadc_shift));
		rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_dtv_demod_shift));
		udelay(10);
	} else {
		rtd_clearbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_if_demod_shift));
		rtd_clearbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_ifadc_shift));
		rtd_clearbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_dtv_demod_shift));
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_if_demod_shift));
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_ifadc_shift));
		rtd_clearbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_dtv_demod_shift));
		udelay(10);
	}
	hw_semaphore_unlock();
...
int DDEMOD_CLK_EN_CriticalRegister(enum CRT_CLK clken)
{
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_dtv_demod_shift));
...
int DDEMOD_Power_EN_CriticalRegister(enum CRT_CLK clken)
{
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_setbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_dtv_demod_shift));
...
int ADEMOD_CLK_EN_CriticalRegister(enum CRT_CLK clken)
{
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_setbits(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_if_demod_shift));
...
int ADEMOD_INPUT_CLK_SEL_CriticalRegister(enum CRT_CLK clken)
{
	hw_semaphore_try_lock();
	rtd_clearbits(SYS_REG_SYS_CLKSEL_reg, BIT(SYS_REG_SYS_CLKSEL_atvin_clk_sel_shift)); //Don't need to set when clk en = 0
	hw_semaphore_unlock();
...
int ADEMOD_ADC_CLK_EN_CriticalRegister(enum CRT_CLK clken)
{
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_if_demod_shift));
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_ifadc_shift));
		//rtd_setbits(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_atvin_ifd_shift));
		//rtd_clearbits(SYS_REG_SYS_CLKSEL_reg, BIT(SYS_REG_SYS_CLKSEL_atvin_clk_sel_shift));
...
int ADEMOD_ADC_RST_CriticalRegister(enum CRT_CLK clken)
{
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		//rtd_setbits(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_atvin_shift));
...
int ADC_CriticalRegister(enum CRT_CLK clken)
{
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_setbits(SYS_REG_SYS_SRST1_reg, _BIT11 | _BIT1); 	/* Reset release in APLL_ADC&I/M domain */
		udelay(100);//wait apll stable
		rtd_setbits(SYS_REG_SYS_CLKEN1_reg, _BIT11 | _BIT1); /* APLL_ADC  & I/M domain CRT on */
	} else {
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, _BIT11); /* APLL_ADC CRT off */
		rtd_clearbits(SYS_REG_SYS_SRST1_reg, _BIT11); 	/* APLL_ADC CRT off */
	}
	hw_semaphore_unlock();
...
int Auto2_CriticalRegister(enum CRT_CLK clken)
{
	...
	if (clken == CLK_ON) {
		...
		rtd_clearbits(SYS_REG_SYS_CLKEN1_reg, _BIT11); /* APLL_ADC CRT off */
		rtd_clearbits(SYS_REG_SYS_SRST1_reg, _BIT11); 	/* Reset  in I/M domain */
	}
	hw_semaphore_unlock();
...
int VO_CriticalRegister(enum CRT_CLK clken)
{
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_maskl(SYS_REG_SYS_CLKEN1_reg, ~(_BIT2), 0);
		rtd_maskl(SYS_REG_SYS_SRST1_reg, ~(_BIT2), 0);
		rtd_maskl(SYS_REG_SYS_CLKEN1_reg, ~(_BIT2), (_BIT2));
		rtd_maskl(SYS_REG_SYS_CLKEN1_reg, ~(_BIT2), 0);
		rtd_maskl(SYS_REG_SYS_SRST1_reg, ~(_BIT2), (_BIT2));
		rtd_maskl(SYS_REG_SYS_CLKEN1_reg, ~(_BIT2), (_BIT2));
	} else {
...
{
	sys_clken1_RBUS sys_clken1_reg;
	...
	if (clken == CLK_ON) {
		...
		sys_srst1_reg.rstn_offms = 1;
		rtd_outl(SYS_REG_SYS_SRST1_reg, sys_srst1_reg.regValue);

		// 0xb8000114
		sys_clken1_reg.regValue = rtd_inl(SYS_REG_SYS_CLKEN1_reg);
		sys_clken1_reg.clken_offms = 1;
		rtd_outl(SYS_REG_SYS_CLKEN1_reg, sys_clken1_reg.regValue);
	} else {
...
int TVE1_CriticalRegister(enum CRT_CLK clken)
{
	...
	if (clken == CLK_ON) {
		...
		//check[22]CLKEN_VDEC
		if (!(rtd_inl(SYS_REG_SYS_CLKEN1_reg) & _BIT22)) {
			rtd_maskl(SYS_REG_SYS_SRST1_reg, 0xFF9FF7FF, 0x00600000); //[22]RSTN_VDEC
			rtd_maskl(SYS_REG_SYS_CLKEN1_reg, 0xFFBFF7FF, 0x00400000); //[22]CLKEN_VDEC
		}

		//check [11]RSTN_APLL_ADC
		if (!(rtd_inl(SYS_REG_SYS_CLKEN1_reg) & _BIT11)) {
			rtd_maskl(SYS_REG_SYS_SRST1_reg, 0xFF9FF7FF, 0x00000800); //[11]RSTN_APLL_ADC
			rtd_maskl(SYS_REG_SYS_CLKEN1_reg, 0xFFBFF7FF, 0x00000800); //[11]CLKEN_APLL_ADC
		}

		//check [27]RSTN_ATVIN2[26]RSTN_ATVIN
		if (!(rtd_inl(SYS_REG_SYS_CLKEN2_reg) & _BIT27)) {
			rtd_maskl(SYS_REG_SYS_SRST2_reg, 0xF3FFFFFF, 0x0C000000); //[27]RSTN_ATVIN2[26]RSTN_ATVIN
			rtd_maskl(SYS_REG_SYS_CLKEN2_reg, 0xFFBFFFFF, 0x00400000); //[27]CLKEN_ATVIN_VD
		}

		//check [26]CLKEN_ATVIN_IFD
		if (!(rtd_inl(SYS_REG_SYS_CLKEN2_reg) & _BIT26)) {
			rtd_maskl(SYS_REG_SYS_CLKEN2_reg, 0xFbfFFfFF, 0x04000000); //[26]CLKEN_ATVIN_IFD
		}

		//check [15]CLKEN_IFADC
		if (!(rtd_inl(SYS_REG_SYS_CLKEN1_reg) & _BIT15)) {
			rtd_maskl(SYS_REG_SYS_SRST1_reg, 0xFffF7fFF, 0x00008000); //[15]RSTN_IFADC
			rtd_maskl(SYS_REG_SYS_CLKEN1_reg, 0xFFff7FFF, 0x00008000); //[15]CLKEN_IFADC
		}

		//check [23]CLKEN_IF_DEMOD
		if (!(rtd_inl(SYS_REG_SYS_CLKEN1_reg) & _BIT23)) {
			rtd_maskl(SYS_REG_SYS_CLKEN1_reg, 0xFF7fFFFF, 0x00800000); //[23]CLKEN_IF_DEMOD
		}
		...
		//tve reset
		rtd_maskl(SYS_REG_SYS_SRST1_reg, 0xFbfFFfFF, 0x04000000); //[26]RSTN_TVE
		//tve clock enable - digital
		rtd_maskl(SYS_REG_SYS_CLKEN1_reg, 0xFbfFFfFF, 0x04000000); //[26]CLKEN_TVE
	} else {
...
int TVE2_CriticalRegister(enum CRT_CLK clken)
{
	...
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_maskl(SYS_REG_SYS_CLKEN1_reg, ~_BIT26, _BIT26);
	} else {
		rtd_maskl(SYS_REG_SYS_CLKEN1_reg, ~_BIT26, 0);
	}
	hw_semaphore_unlock();
...
int TVE3_CriticalRegister(enum CRT_CLK clken)
{
	...
	if (clken == CLK_ON) {
		...
		rtd_maskl(SYS_REG_SYS_CLKEN1_reg, 0xFbfFFfFF, 0x00000000); //[26]CLKEN_TVE
	} else {
...
int TVE4_CriticalRegister(enum CRT_CLK clken)
{
	...
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_maskl(SYS_REG_SYS_SRST1_reg, 0xFbfFFfFF, 0x04000000);
	} else {
...
{
	sys_srst1_RBUS sys_srst1_reg;		/* 0_0104 */
	sys_srst2_RBUS sys_srst2_reg;		/* 0_0108 */
	sys_clken1_RBUS sys_clken1_reg;	/* 0_0114 */
	sys_clken2_RBUS sys_clken2_reg;	/* 0_0118 */

	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		sys_srst1_reg.regValue = rtd_inl(SYS_REG_SYS_SRST1_reg);
		sys_srst2_reg.regValue = rtd_inl(SYS_REG_SYS_SRST2_reg);
		/* 0_0104: [22]RSTN_VDEC[21]RSTN_VDEC2[15]RSTN_IFADC for vdac[11]RSTN_APLL_ADC */
		sys_srst1_reg.rstn_vdec = 1;
		sys_srst1_reg.rstn_vdec2 = 1;
		sys_srst1_reg.rstn_ifadc = 1;
		sys_srst1_reg.rstn_apll_adc = 1;
		/* 0_0108: [27]RSTN_ATVIN2[26]RSTN_ATVIN */
		sys_srst2_reg.rstn_atvin = 1;
		sys_srst2_reg.rstn_atvin2 = 1;
		rtd_outl(SYS_REG_SYS_SRST1_reg, sys_srst1_reg.regValue);
		rtd_outl(SYS_REG_SYS_SRST2_reg, sys_srst2_reg.regValue);

		sys_clken1_reg.regValue = rtd_inl(SYS_REG_SYS_CLKEN1_reg);
		sys_clken2_reg.regValue = rtd_inl(SYS_REG_SYS_CLKEN2_reg);
		/* 0_0114: [22]CLKEN_VDEC[15]CLKEN_IFADC for vdac[11]CLKEN_APLL_ADC */
		sys_clken1_reg.clken_vdec = 1;
		sys_clken1_reg.clken_ifadc = 1;
		sys_clken1_reg.clken_apll_adc = 1;
		/* 0_0118: [27]CLKEN_ATVIN_VD[26]CLKEN_ATVIN_IFD for vd_ifd_inuput */
		sys_clken2_reg.clken_atvin_vd = 1;
		sys_clken2_reg.clken_atvin_ifd = 1;
		rtd_outl(SYS_REG_SYS_CLKEN1_reg, sys_clken1_reg.regValue);
		rtd_outl(SYS_REG_SYS_CLKEN2_reg, sys_clken2_reg.regValue);
...
{
	sys_pll_27x3_RBUS sys_pll_27x3_reg;	/* 0_04d8 */
...
#if 0
		rtd_pr_crt_emerg( "0xb8000204=%x\n", rtd_inl(0xb8000204));

		rtd_pr_crt_emerg( "0xb8000104=%x\n", rtd_inl(0xb8000104));
		rtd_pr_crt_emerg( "0xb8000108=%x\n", rtd_inl(0xb8000108));
		rtd_pr_crt_emerg( "0xb800010c=%x\n", rtd_inl(0xb800010c));
		rtd_pr_crt_emerg( "0xb8000110=%x\n", rtd_inl(0xb8000110));
		rtd_pr_crt_emerg( "0xb8000114=%x\n", rtd_inl(0xb8000114));
		rtd_pr_crt_emerg( "0xb8000118=%x\n", rtd_inl(0xb8000118));
		rtd_pr_crt_emerg( "0xb800011c=%x\n", rtd_inl(0xb800011c));

		rtd_pr_crt_emerg( "0xb80004d4=%x\n", rtd_inl(0xb80004d4));
...
{
	sys_pll_27x1_RBUS sys_pll_27x1_reg;	/* 0_04d0 */
	sys_pll_27x2_RBUS sys_pll_27x2_reg;	/* 0_04d4 */
	sys_pll_27x3_RBUS sys_pll_27x3_reg;	/* 0_04d8 */
	sys_pll_27x4_RBUS sys_pll_27x4_reg;	/* 0_04dc */
...
int VDC6_CriticalRegister(enum CRT_CLK clken, void *private_data)
{
	...
	if (clken == CLK_ON) {
		...
		misc_control_reg.regValue = rtd_inl(VDTOP_MISC_CONTROL_VADDR);
		switch (clkReg) {
		case VDC_PLL27x_CLK_648M:
		...
		rtd_outl(VDTOP_MISC_CONTROL_VADDR, misc_control_reg.regValue);
		rtd_outl(PLL27X_REG_SYS_PLL_27X3_reg, sys_pll_27x3_reg.regValue);
	} else {
...
{
	int flag_pll27x3, flag_avdin;
	
	/* b800_0104[21] */
	sys_srst1_RBUS sys_srst1_reg;

	/* b800_0118[27] */
	sys_clken2_RBUS sys_clken2_reg;
...
//	mdelay(1);
	...
	// 0xb8000118[27]=0  // disable clock
	sys_clken2_reg.clken_atvin_vd = 0;
	rtd_outl(SYS_REG_SYS_CLKEN2_reg, sys_clken2_reg.regValue);
...
int Thermal_CriticalRegister(enum CRT_CLK clken)
{
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_part_outl(SYS_REG_SYS_SRST2_reg, SYS_REG_SYS_SRST2_rstn_tm_shift, SYS_REG_SYS_SRST2_rstn_tm_shift, 0x1);//release thermal rst
		rtd_part_outl(SYS_REG_SYS_CLKEN2_reg, SYS_REG_SYS_CLKEN2_clken_tm_shift, SYS_REG_SYS_CLKEN2_clken_tm_shift, 0x1);//enable thermal clk
	} else {
	}
	hw_semaphore_unlock();
	return CRT_SUCCESS;
...
void CRT_CLK_OnOff(enum CRT_MODULE module_ip, enum CRT_CLK clken, void *private_data)
{
	...
	case MHL:
		if (MHL_CriticalRegister(clken) == CRT_FAIL) {
			rtd_pr_crt_warn("MHL CRT setting fail\n");
		}
		break;
	...
	case ANADEMOD:
		if (ADEMOD_CriticalRegister(clken) == CRT_FAIL) {
			rtd_pr_crt_warn("ANADEMOD CRT setting fail\n");
		}
		break;
	case ANADEMOD_RST_EN:
		if (ADEMOD_RST_CriticalRegister(clken) == CRT_FAIL) {
			rtd_pr_crt_warn("ANADEMOD_RST_EN setting fail\n");
		}
		break;
	case ANADEMOD_IN_RST_EN:
		if (ADEMOD_IN_RST_CriticalRegister(clken) == CRT_FAIL) {
			rtd_pr_crt_warn("ANADEMOD_RST_EN setting fail\n");
		}
		break;
	case ANADEMOD_CLK_PROTECT_BIT:
		if (ADEMOD_CLK_PROTECT_BIT_CriticalRegister(clken, private_data) == CRT_FAIL) {
			rtd_pr_crt_warn("ANADEMOD_RST_EN setting fail\n");
		}
		break;                
	case DIGDEMOD:
		if (DDEMOD_CriticalRegister(clken) == CRT_FAIL) {
			rtd_pr_crt_warn("DIGDEMOD CRT setting fail\n");
		}
		break;
	case ANADEMOD_CLK_EN:
		if (ADEMOD_CLK_EN_CriticalRegister(clken) == CRT_FAIL) {
			rtd_pr_crt_warn("ANADEMOD_CLK_EN CRT setting fail\n");
		}
		break;
	case ANADEMOD_INPUT_CLK_SEL:
		if (ADEMOD_INPUT_CLK_SEL_CriticalRegister(clken) == CRT_FAIL) {
			rtd_pr_crt_warn("ADEMOD_INPUT_CLK_SEL_CriticalRegister fail\n");
		}
		break;
	case ANADEMOD_ADC_CLK_EN:
		if (ADEMOD_ADC_CLK_EN_CriticalRegister(clken) == CRT_FAIL) {
			rtd_pr_crt_warn("ANADEMOD_CLK_EN CRT setting fail\n");
		}
		break;
	case ANADEMOD_ADC_RST:
		if (ADEMOD_ADC_RST_CriticalRegister(clken) == CRT_FAIL) {
			rtd_pr_crt_warn("ANADEMOD_CLK_EN CRT setting fail\n");
		}
		break;
	case DIGDEMOD_CLK_EN:
		if (DDEMOD_CLK_EN_CriticalRegister(clken) == CRT_FAIL) {
			rtd_pr_crt_warn("DIGDEMOD_CLK_EN CRT setting fail\n");
		}
		break;

	case DIGDEMOD_POWER_EN:
		if (DDEMOD_Power_EN_CriticalRegister(clken) == CRT_FAIL) {
			rtd_pr_crt_warn("DIGDEMOD_POWER_EN CRT setting fail\n");
		}
		break;
...
static int __init crt_module_init(void)
{
	...
	rtd_pr_crt_info("CRT module init, major number = %d, device name = %s \n", crt_major, dev_name(crt_device));
