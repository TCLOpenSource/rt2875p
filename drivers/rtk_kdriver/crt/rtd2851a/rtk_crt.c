...
int IrDA_CriticalRegister(enum CRT_CLK clken, void *private_data)
{
	...
	if (clken == CLK_ON) {
		/* HW mode  and RAW mod has share the same setting*/
...
int TV_All_CriticalRegister(enum CRT_CLK clken)
{
    ...
    //ATV turn off
    rtd_outl(0xb8000104, 0x00800000);							//RSTN_IF_DEMOD<bit23> set to 0(active low)
    rtd_outl(0xb8000108, 0x00400800);							//RSTN_IF_DEMOD_logic<bit22> set to 0(active low)
    rtd_outl(0xb8000114, 0x00800000);							//CLKEN_IF_DEMOD<bit23> set to 0(CLK off)
    ...
    rtd_outl(0xb8000104, 0x00000002);							//RSTN_dtv_demod_mb<bit2> set to 0(active low)
    ScalerTimer_DelayXms(1);									//driver delay 1mS

    //DTV Demod turn off
    rtd_outl(0xb8000110, 0x81d82000);							//Demod CLK<bit31, bit24, bit23, bit22, bit20, bit19, bit13> set to 0
    rtd_outl(0xb8000118, 0x00000800);							//CLKEN_DTV_DEMOD<bit11> set to 0
...
/*  //from script
    ...
    //rtd_outl(0xb8000114, 0x00800001);							//<bit23>CLKEN_IF_DEMOD, <bit0>write bit(1)
    //rtd_outl(0xb8000114, 0x00800000);							//<bit23>CLKEN_IF_DEMOD, <bit0>write bit(0)
    rtd_outl(0xb8000104, 0x00800001);							//<bit23>RSTN_IF_DEMOD, <bit0>write bit(1) Reset release
    rtd_outl(0xb8000114, 0x00800001);							//<bit23>CLKEN_IF_DEMOD, <bit0>write bit(1)
    ScalerTimer_DelayXms(1);									//Driver delay 10uS
    //rtd_outl(0xb8000118, 0x04000001);							//<bit26>CLKEN_ATVIN_IFD, <bit0>write bit(1)
    //rtd_outl(0xb8000118, 0x04000000);							//<bit26>CLKEN_ATVIN_IFD, <bit0>write bit(0)
								  ...
								  const char *buf, size_t count)
{
	...
	if ((register_addr == 0xabcd1234) || (register_addr == 0xabcd1357))
		register_password = 0;
    
	if (register_addr == 0x1357abcd)
		register_password = 2;
