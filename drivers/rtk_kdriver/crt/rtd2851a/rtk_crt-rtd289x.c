...
#define USB_PHY_DELAY	1000
	if (CLK_ON == clken) 
	{
		...
		// clock off : USB MAC and OTG   	                                 
		rtd_outl(SYS_REG_SYS_CLKEN2_reg, SYS_REG_SYS_CLKEN2_clken_usb_mask |
						 SYS_REG_SYS_CLKEN2_clken_otg_mask | 
						 SYS_REG_SYS_CLKEN2_write_data(0));  		  	

		udelay(USB_PHY_DELAY);
        
		// Reset MAC 		
		rtd_outl(SYS_REG_SYS_SRST2_reg,  SYS_REG_SYS_SRST2_rstn_usb(1) |
						 SYS_REG_SYS_SRST2_rstn_otg(1) |
		...
		rtd_outl(SYS_REG_SYS_SRST3_reg, SYS_REG_SYS_SRST3_rstn_usb2_phy_p0_mask |
						SYS_REG_SYS_SRST3_rstn_usb2_phy_p1_mask |
						SYS_REG_SYS_SRST3_rstn_usb2_phy_p2_mask |
						SYS_REG_SYS_SRST3_rstn_usb2_phy_p3_mask |
						SYS_REG_SYS_SRST3_rstn_usb3_phy_p0_mask |
		...
		// enable interrupt
		rtd_setbits(SYS_REG_INT_CTRL_SCPU_reg, BIT(SYS_REG_INT_CTRL_SCPU_usb3_int_scpu_routing_en_shift));
		udelay(USB_PHY_DELAY);
	} else {

		// disable interrupt
		rtd_clearbits(SYS_REG_INT_CTRL_SCPU_reg, BIT(SYS_REG_INT_CTRL_SCPU_usb3_int_scpu_routing_en_shift));
		udelay(USB_PHY_DELAY);										 		
...
int GPU_CriticalRegister(enum CRT_CLK clken)
{
	...
	if (clken == CLK_ON) {
		...
		rtd_outl(SYS_REG_INT_CTRL_SCPU_reg, rtd_inl(SYS_REG_INT_CTRL_SCPU_reg) | SYS_REG_INT_CTRL_SCPU_gpu_int_scpu_routing_en_mask);
...
int PCMCIA_CriticalRegister(enum CRT_CLK clken)
{
	...
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_outl(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_pc_shift));	/* disable clock */
		udelay(10);
		rtd_outl(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_pc_shift));	/* assert pcmcia */
		udelay(10);
		rtd_outl(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_pc_shift) | BIT(SYS_REG_SYS_SRST2_write_data_shift)); 	/* deassert pcmcia */
		udelay(10);
		rtd_outl(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_pc_shift) | BIT(SYS_REG_SYS_CLKEN2_write_data_shift));
	} else {
		rtd_outl(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_pc_shift));	/* disable clock  */
	}
	hw_semaphore_unlock();
...
int ETH_CriticalRegister(enum CRT_CLK clken, void *private_data)
{
	...
	if (clken == CLK_ON && wol_mode == 0) {
		rtd_clearbits(SYS_REG_INT_CTRL_KCPU_reg, BIT(17));
		rtd_setbits(SYS_REG_INT_CTRL_SCPU_reg, BIT(17));
	}
	else if(clken == CLK_ON && wol_mode == 1) {
		/* GETN_RSTN_MASK bit[30]=0 */
		rtd_clearbits(STB_ST_CTRL0_reg, STB_ST_CTRL0_getn_rstn_mask_mask);
		/* GETN RBUS & GETN clock */
		rtd_outl(STB_ST_CLKEN0_reg, STB_ST_CLKEN0_clken_getn_rbus_mask | STB_ST_CLKEN0_clken_getn_mask | STB_ST_CLKEN0_write_data_mask); // CLKEN_GETN_RBUS bit[11]=1,CLKEN_GETN bit[8]=1
		rtd_outl(STB_ST_SRST0_reg, STB_ST_SRST0_rstn_getn_mask | STB_ST_SRST0_write_data_mask); // RSTN_GETN_bit[8]=1
		...
		rtd_outl(STB_ST_CLKEN0_reg, STB_ST_CLKEN0_clken_getn_rbus_mask | STB_ST_CLKEN0_clken_getn_mask); // CLKEN_GETN_RBUS bit[11]=0,CLKEN_GETN bit[8]=0
		rtd_outl(STB_ST_SRST0_reg, STB_ST_SRST0_rstn_getn_mask); //RSTN_GETN bit[8] = 0
		rtd_outl(STB_ST_SRST0_reg, STB_ST_SRST0_rstn_getn_mask | STB_ST_SRST0_write_data_mask);  // RSTN_GETN bit[8] = 1
		rtd_outl(STB_ST_CLKEN0_reg, STB_ST_CLKEN0_clken_getn_rbus_mask | STB_ST_CLKEN0_clken_getn_mask | STB_ST_CLKEN0_write_data_mask); // CLKEN_GETN_RBUS bit[11]=1,CLKEN_GETN bit[8]=1
		...
		rtd_clearbits(SYS_REG_INT_CTRL_SCPU_reg, BIT(17));
	}
...
int ADEMOD_CriticalRegister(enum CRT_CLK clken)
{
	hw_semaphore_try_lock();
	if (clken == CLK_ON) {
		rtd_outl(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_if_demod_shift) | SYS_REG_SYS_SRST1_write_data_mask);
		rtd_outl(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_ifadc_shift) | SYS_REG_SYS_SRST1_write_data_mask);
		rtd_outl(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_atvin2_shift));
		rtd_outl(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_atvin_shift));
		rtd_outl(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_if_demod_shift) | SYS_REG_SYS_CLKEN1_write_data_mask);
		rtd_outl(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_ifadc_shift) | SYS_REG_SYS_CLKEN1_write_data_mask);
		rtd_outl(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_atvin_ifd_shift) | SYS_REG_SYS_CLKEN2_write_data_mask);
		//rtd_clearbits(SYS_REG_SYS_CLKSEL_reg, BIT(SYS_REG_SYS_CLKSEL_atvin_clk_sel_shift));//Don't need to set when clk en = 0
		udelay(10);
	} else {
		rtd_outl(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_if_demod_shift));
		rtd_outl(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_ifadc_shift));
		rtd_outl(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_atvin2_shift) | SYS_REG_SYS_SRST2_write_data_mask);
		rtd_outl(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_atvin_shift) | SYS_REG_SYS_SRST2_write_data_mask);
		rtd_outl(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_if_demod_shift));
		rtd_outl(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_ifadc_shift));
		rtd_outl(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_atvin_ifd_shift));
		udelay(10);
	}
	hw_semaphore_unlock();
...
int DDEMOD_CriticalRegister(enum CRT_CLK clken)
{
	...
	if (clken == CLK_ON) {
		rtd_outl(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_if_demod_shift) | SYS_REG_SYS_SRST1_write_data_mask);
		rtd_outl(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_ifadc_shift) | SYS_REG_SYS_SRST1_write_data_mask);
		rtd_outl(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_dtv_demod_shift) | SYS_REG_SYS_SRST2_write_data_mask);
		rtd_outl(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_if_demod_shift) | SYS_REG_SYS_CLKEN1_write_data_mask);
		rtd_outl(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_ifadc_shift) | SYS_REG_SYS_CLKEN1_write_data_mask);
		rtd_outl(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_dtv_demod_shift) | SYS_REG_SYS_CLKEN2_write_data_mask);
		udelay(10);
	} else {
		rtd_outl(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_if_demod_shift));
		rtd_outl(SYS_REG_SYS_SRST1_reg, BIT(SYS_REG_SYS_SRST1_rstn_ifadc_shift));
		rtd_outl(SYS_REG_SYS_SRST2_reg, BIT(SYS_REG_SYS_SRST2_rstn_dtv_demod_shift));
		rtd_outl(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_if_demod_shift));
		rtd_outl(SYS_REG_SYS_CLKEN1_reg, BIT(SYS_REG_SYS_CLKEN1_clken_ifadc_shift));
		rtd_outl(SYS_REG_SYS_CLKEN2_reg, BIT(SYS_REG_SYS_CLKEN2_clken_dtv_demod_shift));
		udelay(10);
	}
	hw_semaphore_unlock();
