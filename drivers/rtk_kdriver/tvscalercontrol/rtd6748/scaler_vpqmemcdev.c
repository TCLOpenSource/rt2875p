...
static unsigned int DISP_DHTotal = 0;
...
void Scaler_MEMC_SetInOutFrameRateByDisp(unsigned char _channel)
{
        ...
        if(get_scaler_qms_mode_flag() && (_channel == SLR_MAIN_DISPLAY))
	{
		...
		if(ulVFreq == 0)
		{
			rtd_pr_memc_err("Scaler_MEMC_SetInOutFrameRateByDisp Err QMS vfreq is zero, fallback to using disp framerate \n");
		}
	}
        ...
        if (ulVFreq == 0)
	{
                ...
                {
                        pr_emerg("modestate_get_mdomain_frc_flag = TRUE\n");
	...
	else	if((ulVFreq > 475) && (ulVFreq < 485))	//48Hz
	{
		...
		//if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) == PIXEL_MODE_2_PIXEL){
		if(Get_DISPLAY_REFRESH_RATE() > 60){
			memc_output_frame_rate = VPQ_OUTPUT_120HZ; //48Hz => 120Hz
		}
		else{
			if((Scaler_MEMC_GetMotionType() == VPQ_MEMC_TYPE_OFF) && (fwif_color_get_cinema_mode_en() == 1)){
				memc_output_frame_rate = VPQ_OUTPUT_48HZ; //24Hz => 48Hz	//tru-motion off + real cinema
...
unsigned char Scaler_MEMC_GetPanelSizeByDisp(void)
{
	...
	if( ((print_cnt<=10) && (print_cnt>1)) || (memc_output_panelsize!=pre_PanelSize) ){
		rtd_printk(KERN_NOTICE, TAG_NAME_MEMC, "[%s][%d][ACTEND,H=%d,V=%d,PANELTYPE=%d,][lbmc,hact=%d,vact=%d,][%d]\n",__FUNCTION__, __LINE__,
...
//					}	
			...
			rtd_pr_memc_notice("[%s][%d][%d][%d,%d]\n", __FUNCTION__, __LINE__, (rtd_inl(PPOVERLAY_MEMC_MUX_CTRL_reg)>>12)&0x1, wait_cnt, (rtd_inl(TIMER_SCPU_CLK90K_LO_reg)-time2));
...
void Scaler_set_MEMC_gamemode_lowdelay(unsigned char enable){
...

    {
    	...
    	if(enable){
    		...
    		if(force_enable_two_step_uzu() || (get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) || get_hdmi_vrr_4k60_mode())
    			vbe_disp_set_dtgM2uzu_lowdelay_mode(2);
    		else
            {
    			vbe_disp_set_dtgM2uzu_lowdelay_mode(1);
                if((get_panel_pixel_mode() <= PANEL_1_PIXEL) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ) >= 994))
                {   
                    if(Get_DISPLAY_REFRESH_RATE() >= 120)
                    {
                        vbe_disp_set_dtgM2uzu_lowdelay_mode(2);
...
void Scaler_MEMC_allocate_memory(void){
	...
	if(Scaler_MEMC_GetPanelSizeByDisp() == MEMC_SIZE_2K){//if(Get_DISPLAY_PANEL_TYPE() == P_LVDS_2K1K){
		...
		IoReg_Write32(DBUS_WRAPPER_Me_uplimit_addr_reg, (gphy_addr_kme + KME_TOTAL_SIZE/*KME_TOTAL_SIZE_8K*/));
		//MC memory boundary
		#if CONFIG_MC_8_BUFFER
		if(Get_DISPLAY_REFRESH_RATE() <= 60){
			Scaler_MEMC_Set_MC_MEMBoundary(gphy_addr_kmc00, (gphy_addr_kmc00 + KMC_TOTAL_SIZE_8buf_8bit/*KMC_TOTAL_SIZE_8buf_8bit_8K*/));	
		}else{
			Scaler_MEMC_Set_MC_MEMBoundary(gphy_addr_kmc00, (gphy_addr_kmc00 + KMC_TOTAL_SIZE_8buf_10bit/*KMC_TOTAL_SIZE_8buf_10bit_8K*/));	
		}
		#else
		Scaler_MEMC_Set_MC_MEMBoundary(gphy_addr_kmc00, (gphy_addr_kmc00 + KMC_TOTAL_SIZE_6buf_10bit/*KMC_TOTAL_SIZE_6buf_10bit_8K*/));	
		#endif
	}else{
		//ME memory boundary
		IoReg_Write32(DBUS_WRAPPER_Me_downlimit_addr_reg, gphy_addr_kme);
...
void Scaler_MEMC_output_force_bg_enable(unsigned char enable,char const* pfunc,unsigned int line){
	...
	rtd_pr_memc_notice("%s,MEMC out FBG = %d, from %s @line : %d\n", __FUNCTION__, enable, pfunc, line);
	//enable MEMC output fbg
	memc_mux_ctrl_reg.regValue = IoReg_Read32(PPOVERLAY_MEMC_MUX_CTRL_reg);
	if(memc_mux_ctrl_reg.memc_out_bg_en != enable)
...
void vpq_memc_disp_suspend_instanboot(void)
{
	...
	//vpq_memc_suspend_resume.MEMC_DBUS_WRAP_ME_UPLIMIT = IoReg_Read32(DBUS_WRAPPER_Me_uplimit_addr_reg);
	//MC memory boundary
	//vpq_memc_suspend_resume.MC_dma_low_limit= MC_DMA_MC_LF_DMA_WR_Rule_check_low_get_dma_low_limit(IoReg_Read32(MC_DMA_MC_LF_DMA_WR_Rule_check_low_reg));
	//vpq_memc_suspend_resume.MC_dma_up_limit= MC_DMA_MC_LF_DMA_WR_Rule_check_up_get_dma_up_limit(IoReg_Read32(MC_DMA_MC_LF_DMA_WR_Rule_check_up_reg));
...
//#endif
	...
	u1_DoPowerSaving = 0;
	rtd_pr_memc_info("[PS_resume_vpqmemc]\n");
...
//#endif
	...
	//MEMC mux enable
	if((rtd_inl(MEMC_MUX_MEMC_MUX_FUNC_CTRL_reg)&0x1) == 0){//if((rtd_inl(0xb802e800)&0x1) == 0){
		drvif_memc_mux_en(TRUE);
	}
