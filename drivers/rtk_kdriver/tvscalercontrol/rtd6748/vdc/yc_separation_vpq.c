...
//#define VDDMA_SELF_TEST
#ifdef VDDMA_SELF_TEST
...
static unsigned char ucHandlerStatus = VPQ_HANDLE_STATUS_ENABLE;
#ifdef FUNC_CHECK_2D_H2V_LK
...
#endif
static unsigned int VPQ_3D_FrameMemStaAddr[4] = {0};
static unsigned int VPQ_3D_FrameMemEndAddr[4] = {0};
static unsigned char VPQ_2D_TableIndex = 0;
static unsigned char VPQ_DCTI_TableIndex = 0;
...
/* FUNCTION IMPLMENT */
void drvif_module_vpq_YcSeparation(unsigned char ucInputSrc, unsigned char uc3DEnable)
{
#ifdef DEBUG_INTERFACE
	...
	// 24		: Fix_VDPQTableEnable, force PQ table enable
	// 25		: VDPQ_AutoMA_bypass
	// 26       : Fix_3DStatusEnable, force 3D status enable
	// 27~28    : Fix_3DStatusSel, force to select a PQ table
	// 29       : Fix_InputSourceEnable, force InputSrc enable
	// 30~31    : Fix_ATV/AV/ExTV, force to select a PQ table
	vdpq_vdpq_fw_used0_RBUS vdpq_vdpq_fw_used0_reg;
...
#ifdef VDDMA_SELF_TEST
	static unsigned char ucDMATestTimeCnt = 0;
	unsigned char ucDMATestResult = 0;
	unsigned char ucDMATestFIFOStatus = 0;
#endif
	...
	if(ucHandlerStatus != VPQ_HANDLE_STATUS_ENABLE)
		return;

#ifdef VDDMA_SELF_TEST
	if( ucDMATestTimeCnt == 0 )
	{
		module_vpq_dma_check_alive(3, &ucDMATestResult, &ucDMATestFIFOStatus);
		rtd_pr_vpq_info("[%s] dma check status = %d, FIFO = %x\n", __func__, ucDMATestResult, ucDMATestFIFOStatus);
	}

	ucDMATestTimeCnt++;
	if( ucDMATestTimeCnt >= TEST_PERIOD )
		ucDMATestTimeCnt = 0;
#endif
	...
	if(!vdc_LockStatus)
		...
		ucCurFormat = drvif_module_vdc_ReadMode(VDC_SET);//do not use detect mode (VDC_DETECT)!
...
#ifdef DEBUG_INTERFACE
	vdpq_vdpq_fw_used0_reg.regValue = vdcf_inl(VDPQ_VDPQ_FW_USED0_reg);

	if( scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue, 24, 24, 0, VD_BIT_GET) )
	{
		ucCurFormat = scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue, 19, 21, 0, VD_BIT_GET);
	}
	if( scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue, 26, 26, 0, VD_BIT_GET) )
	{
		// 0: default 1: 1d 2: 2d 3: 3d
		ucCurStatus = scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue, 27, 28, 0, VD_BIT_GET);
	}
	if( scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue, 29, 29, 0, VD_BIT_GET) )
	{
		// 0:av 1:tv 2:exTv
		ucCurInputAVTV = scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue, 30, 31, 0, VD_BIT_GET);
	}
#endif
	...
	{
		rtd_pr_vpq_info("[%s] InputSrc = %d, YcStatus = %d, InputFormat = %d\n", __func__, ucCurInputAVTV, ucCurStatus, ucCurFormat);

		module_vpq_Get_PrjIDFrom_VpqPrjID();
	...
	if( ucDebugPreStatus != ucCurStatus )
	{
		...
		vdpq_vdpq_fw_used0_reg.regValue = scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue,  0,  4, Debug1DCnt, VD_BIT_SET);
		vdpq_vdpq_fw_used0_reg.regValue = scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue,  5,  9, Debug2DCnt, VD_BIT_SET);
		vdpq_vdpq_fw_used0_reg.regValue = scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue, 10, 14, Debug3DCnt, VD_BIT_SET);
		vdpq_vdpq_fw_used0_reg.regValue = scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue, 22, 23, ucCurStatus, VD_BIT_SET);
		vdcf_outl(VDPQ_VDPQ_FW_USED0_reg, vdpq_vdpq_fw_used0_reg.regValue);
		if(ucDebugPrint == _ENABLE)
		{
			rtd_pr_vpq_info("[VDPQ_INTERFACE] Status change, CurStatus = %d, PreStatus =%d\n", ucCurStatus, ucDebugPreStatus);
		}
	}
	...
	if( ucDebugPreFormat != ucCurFormat)
	{
		...
		vdpq_vdpq_fw_used0_reg.regValue = scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue, 15, 17, DebugFormatCnt, VD_BIT_SET);
		vdcf_outl(VDPQ_VDPQ_FW_USED0_reg, vdpq_vdpq_fw_used0_reg.regValue);
		if(ucDebugPrint == _ENABLE)
		{
			rtd_pr_vpq_info("[VDPQ_INTERFACE] Format change, CurFormat = %d, PreFormat =%d \n", ucCurFormat, ucDebugPreFormat);
		}
	}

	if(scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue, 18, 18, 0, VD_BIT_GET))
	{
		//reset cnt to 0
		Debug1DCnt = 0;
		Debug2DCnt = 0;
		Debug3DCnt = 0;
		DebugFormatCnt = 0;
		vdpq_vdpq_fw_used0_reg.regValue = scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue,  0,  4, 0, VD_BIT_SET);
		vdpq_vdpq_fw_used0_reg.regValue = scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue,  5,  9, 0, VD_BIT_SET);
		vdpq_vdpq_fw_used0_reg.regValue = scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue, 10, 14, 0, VD_BIT_SET);
		vdpq_vdpq_fw_used0_reg.regValue = scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue, 15, 17, 0, VD_BIT_SET);
		vdpq_vdpq_fw_used0_reg.regValue = scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used0_reg.regValue, 18, 18, 0, VD_BIT_SET);
		vdcf_outl(VDPQ_VDPQ_FW_USED0_reg, vdpq_vdpq_fw_used0_reg.regValue);
	}
...
/**
 * @brief
 * check the vd 3d enable status
 * @param PrintStatus
 * @return unsigned char
 * 0 : fail
 * others : ok
 */
unsigned char drvif_module_vpq_check_3d_off_on(unsigned char PrintStatus) {
	...
	vdtop_video_status_reg3_RBUS vdtop_video_status_reg3_reg;
	vdtop_signal_stm_control_RBUS vdtop_signal_stm_control_reg;
	vdpq_frame_buf_chk3d_ctrl0_RBUS vdpq_frame_buf_chk3d_ctrl0_reg;
	vdpq_frame_buf_chk3d_ctrl1_RBUS vdpq_frame_buf_chk3d_ctrl1_reg;
	vdpq_frame_buf_chk3d_ctrl2_RBUS vdpq_frame_buf_chk3d_ctrl2_reg;
	vdtop_video_status_reg1_RBUS vdtop_video_status_reg1_reg;
	vdtop_video_status_reg2_RBUS vdtop_video_status_reg2_reg;
	vdtop_sync_midpt_v2_RBUS vdtop_sync_midpt_v2_reg;


	vdpq_dma_ctrl4_reg.regValue = vdcf_inl(VDPQ_DMA_CTRL4_reg);

	if(vdpq_dma_ctrl4_reg.dma_3d_on_off == 1)
		Ret = 1;
	else
		Ret = 0;
	...
	if( PrintStatus != 0 ) {
		...
		vdcf_maskl(VDPQ_FRAME_BUF_CHK3D_CTRL1_reg, ~(1<<6), 0x0); // clear wclr bit

		vdtop_video_status_reg3_reg.regValue = vdcf_inl(VDTOP_VIDEO_STATUS_REG3_reg);
		vdtop_signal_stm_control_reg.regValue = vdcf_inl(VDTOP_SIGNAL_STM_CONTROL_reg);
		vdpq_frame_buf_chk3d_ctrl0_reg.regValue = vdcf_inl(VDPQ_FRAME_BUF_CHK3D_CTRL0_reg);
		vdpq_frame_buf_chk3d_ctrl1_reg.regValue = vdcf_inl(VDPQ_FRAME_BUF_CHK3D_CTRL1_reg);
		vdpq_frame_buf_chk3d_ctrl2_reg.regValue = vdcf_inl(VDPQ_FRAME_BUF_CHK3D_CTRL2_reg);
		vdtop_video_status_reg1_reg.regValue = vdcf_inl(VDTOP_VIDEO_STATUS_REG1_reg);
		vdtop_video_status_reg2_reg.regValue = vdcf_inl(VDTOP_VIDEO_STATUS_REG2_reg);
		vdtop_sync_midpt_v2_reg.regValue = vdcf_inl(VDTOP_SYNC_MIDPT_V2_reg);

		rtd_pr_vpq_info("[%s] noisy en = %d, val = %d(0)\n", __FUNCTION__, vdtop_signal_stm_control_reg.auto_3dchange_noisy, vdtop_video_status_reg3_reg.noisy);
		rtd_pr_vpq_info("[%s] VCR en = %d, val = %d(0)\n", __FUNCTION__, vdtop_signal_stm_control_reg.auto_3dchange_vcr, vdtop_video_status_reg3_reg.vcr);
		rtd_pr_vpq_info("[%s] num en = %d, val = %d(0), hrgn = %d, crgn = %d\n", __FUNCTION__,
			vdpq_frame_buf_chk3d_ctrl1_reg.f_num_chk_3d_en, vdpq_frame_buf_chk3d_ctrl1_reg.f_num_3d_fail_det,
			vdpq_frame_buf_chk3d_ctrl0_reg.f_hennum_range, vdpq_frame_buf_chk3d_ctrl1_reg.f_cennum_range
		);
		rtd_pr_vpq_info("[%s] Proscan = %d(0)\n", __FUNCTION__, vdtop_video_status_reg2_reg.proscan_detected);
		rtd_pr_vpq_info("[%s] Hlock = %d(1), Clock = %d(1)\n", __FUNCTION__, vdtop_video_status_reg1_reg.hlock, vdtop_video_status_reg1_reg.chomalock);
		rtd_pr_vpq_info("[%s] Vlock = %d(1), ModSel = %d(0)\n", __FUNCTION__, vdtop_video_status_reg1_reg.vlock, vdtop_sync_midpt_v2_reg.vlock_sel);
		rtd_pr_vpq_info("[%s] open = %d, close = %d\n", __FUNCTION__, vdpq_frame_buf_chk3d_ctrl2_reg.f_num_open3d_cnt, vdpq_frame_buf_chk3d_ctrl2_reg.f_num_close3d_cnt);
	}
...
void module_vpq_VDPQTable_1DSet(unsigned char ucInputSrc, unsigned char ucFormat)
{
	...
	module_vpq_demodulation_region_table_write(VPQ_DEMOD_REGION[ucFormat]);
...
#endif
void module_vpq_Get_PrjIDFrom_VpqPrjID()
{
	static bool bInitPrjId = false; // Just for RTICE debug
	if(bInitPrjId == false)
	{
		switch(vpqex_project_id)
		{
			case 0x00010000:
				ucVdpqPrjId = 0;
				break;
...
void module_vpq_3D_MemCheck(void)
{
	...
	{
		rtd_pr_vpq_err("VDPQ Memory Check Error\n");
		for(i=0; i<4; i++)
		{
			rtd_pr_vpq_err("Frame %d Sta = %x, End = %x, Offset = %x\n",
				i, VPQ_3D_FrameMemStaAddr[i], VPQ_3D_FrameMemEndAddr[i], VPQ_3D_FrameMemEndAddr[i]-VPQ_3D_FrameMemStaAddr[i]);
		}
	}
}

void module_vpq_demodulation_region_table_write(unsigned char *table)
{
	vdpq_auto_mode_ycsep_demod_region_RBUS vdpq_auto_mode_ycsep_demod_region_reg;
	vdpq_auto_mode_ycsep_demod_region_reg.regValue = vdcf_inl(VDPQ_AUTO_MODE_YCSEP_DEMOD_REGION_reg);
	vdpq_auto_mode_ycsep_demod_region_reg.ycsep_demod_region_always_on = table[0];
	vdpq_auto_mode_ycsep_demod_region_reg.hactivestartreg_en_ycsep = table[1];
	vdpq_auto_mode_ycsep_demod_region_reg.hactive_start_ycsep = table[2];
	vdcf_outl(VDPQ_AUTO_MODE_YCSEP_DEMOD_REGION_reg, vdpq_auto_mode_ycsep_demod_region_reg.regValue);
}

void module_vpq_postp_alpha_weight_table_write(unsigned char *table)
{
	vdpq_postp_3d2dalpha_blend_ctrl_1_RBUS vdpq_postp_3d2dalpha_blend_ctrl_1_reg;

	vdpq_postp_3d2dalpha_blend_ctrl_1_reg.regValue = vdcf_inl(VDPQ_POSTP_3D2DALPHA_BLEND_CTRL_1_reg);

	vdpq_postp_3d2dalpha_blend_ctrl_1_reg.xnr_alpha_2d_weight = table[0];
	vdpq_postp_3d2dalpha_blend_ctrl_1_reg.xnr_alpha_bld_weight = table[1];
	vdpq_postp_3d2dalpha_blend_ctrl_1_reg.xnr_alpha_3d_weight = table[2];

	vdcf_outl(VDPQ_POSTP_3D2DALPHA_BLEND_CTRL_1_reg, vdpq_postp_3d2dalpha_blend_ctrl_1_reg.regValue);
}
...
{
	int RegValue = vdcf_inl(VDPQ_VDPQ_FW_USED2_reg);
	RegValue = ( RegValue | (_BIT14) );
	vdcf_outl(VDPQ_VDPQ_FW_USED2_reg, RegValue);
}
...
void module_vpq_table_info_to_fw(unsigned char ucFormat, unsigned char ucTBL)
{
  ...
  vdpq_vdpq_fw_used1_RBUS vdpq_vdpq_fw_used1_reg;
  vdpq_vdpq_fw_used1_reg.regValue = vdcf_inl(VDPQ_VDPQ_FW_USED1_reg);
  vdpq_vdpq_fw_used1_reg.regValue = (((vdpq_vdpq_fw_used1_reg.regValue) & (0x00ffffff)) | (((ucTBL+1)<<26)+(ucFormat<<24)));
  vdcf_outl(VDPQ_VDPQ_FW_USED1_reg, vdpq_vdpq_fw_used1_reg.regValue);
}
...
void module_vpq_format_change_bit_to_fw(void)
{
  ...
  vdpq_vdpq_fw_used2_reg.regValue = vdcf_inl(VDPQ_VDPQ_FW_USED2_reg);
  vdpq_vdpq_fw_used2_reg.regValue = ((vdpq_vdpq_fw_used2_reg.regValue) | (0x00200000));
  vdcf_outl(VDPQ_VDPQ_FW_USED2_reg, vdpq_vdpq_fw_used2_reg.regValue);
}
...
void module_vpq_YCbCrDelay(unsigned char ucEnable, unsigned char ucInYcSep, unsigned char ucInputSrc, unsigned char ucInputFormat)
{
	...
	vdpq_auto_pos_ctrl_RBUS vdpq_auto_pos_ctrl_reg;
	vdpq_auto_pos_ctrl_2_RBUS vdpq_auto_pos_ctrl_2_reg;
	...
	vdpq_auto_pos_ctrl_reg.regValue = vdcf_inl(VDPQ_AUTO_POS_CTRL_reg);
	vdpq_auto_pos_ctrl_2_reg.regValue = vdcf_inl(VDPQ_AUTO_POS_CTRL_2_reg);

	vdpq_auto_pos_ctrl_reg.y1delayreg_en   = ucEnable;
	vdpq_auto_pos_ctrl_reg.cb1delayreg_en  = ucEnable;
	vdpq_auto_pos_ctrl_reg.cr1delayreg_en  = ucEnable;
	vdpq_auto_pos_ctrl_reg.new_y1delay_en  = ucEnable;
	vdpq_auto_pos_ctrl_reg.new_cb1delay_en = ucEnable;
	vdpq_auto_pos_ctrl_reg.new_cr1delay_en = ucEnable;
	vdpq_auto_pos_ctrl_reg.y1delay  = ucNewYDelay;
	vdpq_auto_pos_ctrl_reg.cb1delay = ucNewCbDelay;
	vdpq_auto_pos_ctrl_reg.cr1delay = ucNewCrDelay;

	vdpq_auto_pos_ctrl_2_reg.ckill_y1delayreg_en  = 1;
	vdpq_auto_pos_ctrl_2_reg.ckill_cb1delayreg_en = 1;
	vdpq_auto_pos_ctrl_2_reg.ckill_cr1delayreg_en = 1;
	vdpq_auto_pos_ctrl_2_reg.ckill_y1delay = ucNewCKYDelay;

	vdcf_outl(VDPQ_AUTO_POS_CTRL_reg, vdpq_auto_pos_ctrl_reg.regValue);
	vdcf_outl(VDPQ_AUTO_POS_CTRL_2_reg, vdpq_auto_pos_ctrl_2_reg.regValue);
	// -----------------------------------------------------------------------------------------------------
...
{
	// DMA related setting
	vdcf_maskl(VDPQ_DMA_CTRL6_reg, 0xfffffffb, 0x00000000); // dma field inverse reset

	// PQ related registers
	vdcf_outl(VDPQ_BPF_BW_SEL_Y_reg, 0x00000550);
	vdcf_outl(VDPQ_BPF_BW_SEL_C_reg, 0x00000440);
	vdcf_outl(VDPQ_ADAP_BPF_C_TH1_reg, 0x0000000a);
	vdcf_outl(VDPQ_ADAP_BPF_C_TH2_reg, 0x00000077);
	vdcf_outl(VDPQ_COMB_FILTER_CONFIG_reg, 0x00000042);
	vdcf_outl(VDPQ_COMB_FILTER_THRESHOLD1_reg, 0x00000007);
	vdcf_outl(VDPQ_ADAP_BPF_Y_TH3_reg, 0x00000000);
	vdcf_outl(VDPQ_YC_BW_CTRL_reg, 0x00000001);
	vdcf_outl(VDPQ_DEBUG_2D_MODE_reg, 0x00000000);
	vdcf_outl(VDPQ_BW_DETECTION_reg, 0x00000010);
	vdcf_outl(VDPQ_BW_EDGE_THR_reg, 0x00000020);
	vdcf_outl(VDPQ_BLEND_1D_THR_reg, 0x00000008);
	vdcf_outl(VDPQ_BLEND_1D_BPF_SEL_reg, 0x00000002);
	vdcf_outl(VDPQ_ALPHA_2D_MOD_ENABLE_reg, 0x04053222);
	vdcf_outl(VDPQ_H2V_CORING_THR_reg, 0x00020221);
	vdcf_outl(VDPQ_H2V_Y_NOISE_THR_reg, 0x00000006);
	vdcf_outl(VDPQ_H2V_CVBS_NOISE_THR_reg, 0x00000015);
	vdcf_outl(VDPQ_H2V_NOISE_MAX_HDY_reg, 0x0000000f);
	vdcf_outl(VDPQ_H2V_NOISE_Y_ADD_DC_reg, 0x0000000a);
	vdcf_outl(VDPQ_H2V_BLEND_RATIO_reg, 0x00000004);
	vdcf_outl(VDPQ_ADAP_BPF_Y_TH2_reg, 0x00000000);
	vdcf_outl(VDPQ_CDIFF_ADAP_BPF_TH1_reg, 0x00081e00);
	vdcf_outl(VDPQ_CDIFF_ADAP_BPF_TH2_reg, 0x0019a221);
	vdcf_outl(VDPQ_BPF_VEDGE_CTRL1_reg, 0x00325050);
	vdcf_outl(VDPQ_BPF_VEDGE_CTRL2_reg, 0x0000190c);
	vdcf_outl(VDPQ_BYPASS_BPF_CTRL1_reg, 0x519321e0);
	vdcf_outl(VDPQ_BYPASS_BPF_CTRL2_reg, 0x00283194);
	vdcf_outl(VDPQ_DIAMC_CVBS_AMP_TH_reg, 0x00000a00);
	vdcf_outl(VDPQ_VMC_CTRL_reg, 0x19114144);
	vdcf_outl(VDPQ_VMC_SPATIAL_CORR_CTRL_reg, 0x000042b7);
	vdcf_outl(VDPQ_yc2d_postlp_ctrl_reg, 0x0000000c);
	vdcf_outl(VDPQ_yc2d_postlp_flat_1_reg, 0x08070504);
	vdcf_outl(VDPQ_yc2d_postlp_flat_2_reg, 0x0a05040a);
	vdcf_outl(VDPQ_yc2d_postlp_flat_3_reg, 0x0003100d);
	vdcf_outl(VDPQ_yc2d_postlp_flat_4_reg, 0x00060402);
	vdcf_outl(VDPQ_yc2d_postlp_grad_1_reg, 0x50405808);
	vdcf_outl(VDPQ_yc2d_postlp_grad_2_reg, 0x00a0c051);
	vdcf_outl(VDPQ_yc2d_postlp_grad_3_reg, 0x00000608);
	vdcf_outl(VDPQ_yc2d_postlp_dia_1_reg, 0x02141601);
	vdcf_outl(VDPQ_yc2d_postlp_dia_2_reg, 0x00a81914);
	vdcf_outl(VDPQ_yc2d_postlp_dia_3_reg, 0x06080a0c);
	vdcf_outl(VDPQ_yc2d_postlp_dia_4_reg, 0x00000023);
	vdcf_outl(VDPQ_YC_SEP_MODE_CTRL_reg, 0x00000000);
	vdcf_outl(VDPQ_Dummy_3_reg, 0x00000000);
	vdcf_outl(VDPQ_YC_3DSEP_CHROMA_BPF_SEL_reg, 0x00000080);
	//vdcf_outl(VDPQ_YC_SEP_YUV_DELAY_reg, 0x00000000); // yc delay related register
	vdcf_outl(VDPQ_YC_3D_F4_STAT_1_reg, 0x00000000);
	vdcf_outl(VDPQ_YC_3D_F4_STAT_2_reg, 0x00050000);
	vdcf_outl(VDPQ_YC_3D_F4_STAT_3_reg, 0x00050000);
	vdcf_outl(VDPQ_YC_3D_F4_NTSC_ABA_1_reg, 0x0050f140);
	vdcf_outl(VDPQ_YC_3D_F4_NTSC_ABA_2_reg, 0x0305140a);
	vdcf_outl(VDPQ_Dummy_4_reg, 0x00000000);
	vdcf_outl(VDPQ_YC_3D_F4_Ctrl_reg, 0x23264000);
	vdcf_outl(VDPQ_YC_3D_F4_Spa_Cnt_CVBS_Amp_reg, 0x140db976);
	vdcf_outl(VDPQ_YC_3D_F4_HV_Edge_1_reg, 0x00151442);
	vdcf_outl(VDPQ_YC_3D_F4_HV_Edge_2_reg, 0x000000f0);
	vdcf_outl(VDPQ_YC_3D_F4_Hifreq_Det_reg, 0x00000140);
	vdcf_outl(VDPQ_YC_3D_F4_DC_Compare_reg, 0x0000000a);
	vdcf_outl(VDPQ_YC_3D_F4_PAL_Err_Com_1_reg, 0x19103251);
	vdcf_outl(VDPQ_YC_3D_F4_PAL_Err_Com_2_reg, 0x07010814);
	vdcf_outl(VDPQ_YC_3D_F4_PAL_Err_Com_3_reg, 0x00a0a20a);
	vdcf_outl(VDPQ_YC_3D_F4_Still_Vector_1_reg, 0x0125060a);
	vdcf_outl(VDPQ_YC_3D_F4_Still_Vector_2_reg, 0x0000000f);
	vdcf_outl(VDPQ_YC_3D_F4_Still_Gray_HiFreq_1_reg, 0x0001e3e0);
	vdcf_outl(VDPQ_YC_3D_F4_Still_Gray_HiFreq_2_reg, 0x00f50c0a);
	vdcf_outl(VDPQ_YC_3D_F4_Pair_Th_Multiple_1_reg, 0x08090b0b);
	vdcf_outl(VDPQ_YC_3D_F4_Pair_Th_Multiple_2_reg, 0x06070707);
	vdcf_outl(VDPQ_YC_3D_F4_Pair_Th_Multiple_3_reg, 0x0f0f0606);
	vdcf_outl(VDPQ_YC_3D_F4_Pair_Th_Base_1_reg, 0x0f141414);
	vdcf_outl(VDPQ_YC_3D_F4_Pair_Th_Base_2_reg, 0x0f0f0f0f);
	vdcf_outl(VDPQ_YC_3D_F4_Pair_Th_Base_3_reg, 0x050f0f0f);
	vdcf_outl(VDPQ_YC_3D_F4_Pair_Ctrl_1_reg, 0x1e214002);
	vdcf_outl(VDPQ_YC_3D_F4_Pair_Ctrl_2_reg, 0x0000050f);
	vdcf_outl(VDPQ_YC_3D_F4_Pair_Ctrl_3_reg, 0x02614082);
	vdcf_outl(VDPQ_YC_3D_F4_Pair_Ctrl_4_reg, 0x26050f1e);
	vdcf_outl(VDPQ_YC_3D_F4_Pair_Ctrl_5_reg, 0x000140f1);
	vdcf_outl(VDPQ_YC_3D_F4_Pair_Ctrl_6_reg, 0x000a1108);
	vdcf_outl(VDPQ_YC_3D_F4_Global_Diff_Th_Hifreq_Offset_1_reg, 0x00000000);
	vdcf_outl(VDPQ_YC_3D_F4_Global_Diff_Th_Hifreq_Offset_2_reg, 0x00000000);
	vdcf_outl(VDPQ_YC_3D_F4_Global_Diff_Th_Hifreq_Offset_3_reg, 0x00000000);
	vdcf_outl(VDPQ_YC_3D_F4_Global_Diff_Th_Hifreq_Offset_4_reg, 0x00000000);
	vdcf_outl(VDPQ_YC_3D_F4_MGHF_Ctrl_1_reg, 0x0000f050);
	vdcf_outl(VDPQ_YC_3D_F4_MGHF_Ctrl_2_reg, 0x440f0712);
	vdcf_outl(VDPQ_YC_3D_F4_Global_Diff_Th_Com_reg, 0x00000000);
	vdcf_outl(VDPQ_YC_Post2DBPF_DBWline_1_reg, 0x192170f0);
	vdcf_outl(VDPQ_YC_Post2DBPF_DBWline_2_reg, 0x190f2d14);
	vdcf_outl(VDPQ_YC_Post2DBPF_DBWline_3_reg, 0x0000000f);
	vdcf_outl(VDPQ_YC_Post2DBPF_Ctrl_reg, 0x00a29240);
	vdcf_outl(VDPQ_YC_Post2DBPF_ThBase_reg, 0x00001914);
	vdcf_outl(VDPQ_YC_Post2DBPF_AdapTh_1_reg, 0x0190c30a);
	vdcf_outl(VDPQ_YC_Post2DBPF_AdapTh_2_reg, 0x0f0a0a0a);
	vdcf_outl(VDPQ_YC_Post2DBPF_AdapTh_3_reg, 0x05050505);
	vdcf_outl(VDPQ_YC_Post2DBPF_AdapTh_4_reg, 0x0a191404);
	vdcf_outl(VDPQ_YC_Post2DBPF_AdapTh_5_reg, 0x00001919);
	vdcf_outl(VDPQ_YC_Post2DBPF_AdapTh_6_reg, 0x1e0f4b5a);
	vdcf_outl(VDPQ_YC_Post2DBPF_AdapTh_7_reg, 0x0000c30a);
	vdcf_outl(VDPQ_YC_Post2DBPF_AdapTh_8_reg, 0x050a1428);
	vdcf_outl(VDPQ_YC_Post2DBPF_AdapTh_9_reg, 0x0a19141e);
	vdcf_outl(VDPQ_YC_Post2DBPF_AdapTh_10_reg, 0x1e1e190c);
	vdcf_outl(VDPQ_YC_Post2DBPF_AdapTh_11_reg, 0x051e0f0f);
	vdcf_outl(VDPQ_YC_Post2DBPF_AdapTh_12_reg, 0x23661e0f);
	vdcf_outl(VDPQ_YC_Post2DBPF_AdapTh_13_reg, 0x00ffa006);
	vdcf_outl(VDPQ_YC_Post2DBPF_XC_reg, 0x2680230a);
	vdcf_outl(VDPQ_SECAM_YADAP_MODE_reg, 0x00000000);
	vdcf_outl(VDPQ_SECAM_YADAP_CTRL1_reg, 0x00000044);
	vdcf_outl(VDPQ_SECAM_YADAP_CTRL2_reg, 0x00000080);
	vdcf_outl(VDPQ_XNR_CTRL_reg, 0x00000000);
	vdcf_outl(VDPQ_XNR_THR_reg, 0x00000015);
	vdcf_outl(VDPQ_XNR_IDX_THR_reg, 0x00000033);
	vdcf_outl(VDPQ_XNR_RATIO_CTRL_reg, 0x00000022);
	vdcf_outl(VDPQ_POSTP_MF_COMPENSATION_CTRL1_reg, 0x0d08143f);
	vdcf_outl(VDPQ_POSTP_MF_COMPENSATION_CTRL2_reg, 0x0000d086);
	vdcf_outl(VDPQ_POSTP_3D2DALPHA_BLEND_CTRL_1_reg, 0x00000444);
	vdcf_outl(VDPQ_POSTP_3D2DALPHA_BLEND_CTRL_2_reg, 0x00000000);
	vdcf_outl(VDPQ_SPNR_CP_Ctrl_reg, 0x0000000e);
	vdcf_outl(VDPQ_Dummy_Status_reg, 0x00000000);
	vdcf_outl(VDPQ_ALPHA_BLENDING_CTRL0_reg, 0x00000000);
	vdcf_outl(VDPQ_ALPHA_BLENDING_CTRL1_reg, 0x00000000);
	vdcf_outl(VDPQ_ALPHA_BLENDING_CTRL2_reg, 0x00040808);
	vdcf_outl(VDPQ_ALPHA_BLENDING_CTRL3_reg, 0x0c0c0e0e);
	vdcf_outl(VDPQ_ALPHA_BLENDING_CTRL4_reg, 0x10101010);
	vdcf_outl(VDPQ_ALPHA_TEMPORAL_SMOOTH_reg, 0x000ff880);
	vdcf_outl(VDPQ_DE_XC_CTRL_reg, 0x00000003);
	vdcf_outl(VDPQ_DE_XC_CTRL_NEW_reg, 0x23141d03);
	vdcf_outl(VDPQ_CADAP_ENABLE_CTRL_reg, 0x00000018);
	vdcf_outl(VDPQ_CADAP_ENABLE_CTRL2_reg, 0x00000007);
	vdcf_outl(VDPQ_CADAP_GAIN_CTRL_reg, 0x00000068);
	vdcf_outl(VDPQ_CADAP_ZC_TH_reg, 0x00000034);
	vdcf_outl(VDPQ_CADAP_ZC_TH2_reg, 0x00000074);
	vdcf_outl(VDPQ_CADAP_VER_UNIFORMITY_TH_reg, 0x00000055);
	vdcf_outl(VDPQ_CADAP_SYNC_UNIFORMITY_TH_reg, 0x00000055);
	vdcf_outl(VDPQ_CADAP_LUT_LEVEL_TH_reg, 0x00000067);
	vdcf_outl(VDPQ_CADAP_LUT_GAIN_TH_reg, 0x0000007d);
	vdcf_outl(VDPQ_DCTI_FILTER_COEF_B1_reg, 0x00000000);
	vdcf_outl(VDPQ_DCTI_FILTER_COEF_B2_reg, 0x00000000);
	vdcf_outl(VDPQ_DCTI_FILTER_COEF_B3_reg, 0x00000000);
	vdcf_outl(VDPQ_DCTI_FILTER_COEF_A2_reg, 0x00000000);
	vdcf_outl(VDPQ_DCTI_FILTER_COEF_A3_reg, 0x00000000);
	vdcf_outl(VDPQ_DCTI_FILTER_EN_reg, 0x00000000);
	vdcf_outl(VDPQ_YC_3D_F4_SMD_EN_reg, 0x00000000);
	vdcf_outl(VDPQ_YC_3D_F4_SMD_TH1_reg, 0x04040a0a);
	vdcf_outl(VDPQ_YC_3D_F4_SMD_TH2_reg, 0x00003232);
	vdcf_outl(VDPQ_YC_3D_F4_Pair_SMD_TH1_reg, 0x01000519);
	vdcf_outl(VDPQ_YC_3D_F4_Pair_SMD_TH2_reg, 0x00000400);
	//vdcf_outl(VDPQ_NS_R1_reg, 0x00000000); // read only register
	//vdcf_outl(VDPQ_NS_R2_reg, 0x00000000); // read only register
	//vdcf_outl(VDPQ_NS_R3_reg, 0x00000000); // read only register
	vdcf_outl(VDPQ_NS_RW1_reg, 0x000a0000);
	vdcf_outl(VDPQ_NS_RW2_reg, 0x00000000);
	vdcf_outl(VDPQ_NS_GATE_reg, 0x00001031);
	vdcf_outl(VDPQ_POSTP_SECAM_FIX_CTRL_reg, 0x00041400);
	vdcf_outl(VDPQ_POSTP_SECAM_FIX_Y_DETECT0_reg, 0x04010314);
	vdcf_outl(VDPQ_POSTP_SECAM_FIX_Y_DETECT1_reg, 0x00000403);
	vdcf_outl(VDPQ_DIRECTIONAL_3D_reg, 0x00000000);
	vdcf_outl(VDPQ_AUTO_MODE_YCSEP_DEMOD_REGION_reg, 0x00000082); // demod region control defualt disable to avoid burst disappear and cgain overflow
	//vdcf_outl(VDPQ_AUTO_POS_CTRL_3_reg, 0x00000000); // yc delay related register
	//vdcf_outl(VDPQ_AUTO_POS_CTRL_reg, 0x70000000); // yc delay related register
	//vdcf_outl(VDPQ_AUTO_POS_CTRL_2_reg, 0x00000000); // yc delay related register
	//vdcf_outl(VDPQ_VD_FORCE_CTRL_reg, 0x00000000); // yc delay related register
	vdcf_outl(VDPQ_BPF_COEF_Y_WIDE_reg, 0x00000000);
	vdcf_outl(VDPQ_BPF_COEF_Y_MID_reg, 0x00000000);
	vdcf_outl(VDPQ_BPF_COEF_Y_NARROW_reg, 0x00000000);
	vdcf_outl(VDPQ_BPF_COEF_C_WIDE_reg, 0x00000000);
	vdcf_outl(VDPQ_BPF_COEF_C_MID_reg, 0x00000000);
	vdcf_outl(VDPQ_BPF_COEF_C_NARROW_reg, 0x00000000);
	vdcf_outl(VDPQ_BLEND_1D_BPF_SEL_COEF_Y_reg, 0x00000000);
	vdcf_outl(VDPQ_BLEND_1D_BPF_SEL_COEF_C_WIDE_reg, 0x00000000);
	vdcf_outl(VDPQ_BLEND_1D_BPF_SEL_COEF_C_NARROW_reg, 0x00000000);
	vdcf_outl(VDPQ_FIX_WINDOW_CTRL_reg, 0x00000000);
	vdcf_outl(VDPQ_FIX_WINDOW_POSX_reg, 0x00000000);
	vdcf_outl(VDPQ_FIX_WINDOW_POSY_reg, 0x00000000);

}

void module_vpq_dma_check_alive(unsigned char ucWrtClearTimes, unsigned char *ucRetResult, unsigned char *ucRetFIFOStatus )
{
	// Ret = 5'b[fifoStatus]3'b[checkResult]
	// fifoStatus = b8019850[7:3]
	// checkResult :
	// 0 : fail due to vd error flag
	// 1 : fail due to memory is off
	// 2 : pass
	unsigned char i = 0;
	vdpq_dma_ctrl5_RBUS vdpq_dma_ctrl5_reg;
	tvsb1_tv_sb1_dcu1_arb_cr1_RBUS tvsb1_tv_sb1_dcu1_arb_cr1_reg;

	*ucRetResult = 2;
	*ucRetFIFOStatus = 0;

	vdpq_dma_ctrl5_reg.regValue = vdcf_inl(VDPQ_DMA_CTRL5_reg);
	tvsb1_tv_sb1_dcu1_arb_cr1_reg.regValue = rtd_inl(TVSB1_TV_SB1_DCU1_ARB_CR1_reg);

	if( vdpq_dma_ctrl5_reg.dma_reset == 1 || vdpq_dma_ctrl5_reg.mem_off == 1 || ((tvsb1_tv_sb1_dcu1_arb_cr1_reg.arb_en & 0x1) == 0) )
	{
		rtd_pr_vpq_info("[%s] Mem/Arb (%d %d %d) is off, cannot check\n",
			__func__, vdpq_dma_ctrl5_reg.dma_reset, vdpq_dma_ctrl5_reg.mem_off, (tvsb1_tv_sb1_dcu1_arb_cr1_reg.arb_en & 0x1));
		*ucRetResult = 1;
		*ucRetFIFOStatus = 0;
	}
	else
	{
		for(i=0; i<ucWrtClearTimes; i++)
		{
			vdpq_dma_ctrl5_reg.regValue = vdcf_inl(VDPQ_DMA_CTRL5_reg);
		...
		vdpq_dma_ctrl5_reg.regValue = vdcf_inl(VDPQ_DMA_CTRL5_reg);
		if( vdpq_dma_ctrl5_reg.ififo_err_flag_1 == 1 || vdpq_dma_ctrl5_reg.ififo_err_flag_2 == 1 ||
			vdpq_dma_ctrl5_reg.ififo_err_flag_3 == 1 || vdpq_dma_ctrl5_reg.ififo_err_flag_4 == 1 ||
			vdpq_dma_ctrl5_reg.ofifo_err_flag == 1 )
		{
			*ucRetResult = 0;
			*ucRetFIFOStatus = ((vdpq_dma_ctrl5_reg.regValue >> 3) & 0x1f);
		}
		else
		{
			*ucRetResult = 2;
...
void module_vpq_Enable3D(void)
{
	// This is for DMA hw bug
#if 0//def M6_FORCE_2D
	if( (get_ic_version() == VERSION_A) && (((rtd_inl(EFUSE_DATAO_137_reg)>>1)& 0x7) == 0x0) )
	{
		vdpq_yc_sep_mode_ctrl_RBUS vdpq_yc_sep_mode_ctrl_reg;
		vdpq_yc_sep_mode_ctrl_reg.regValue = vdcf_inl(VDPQ_YC_SEP_MODE_CTRL_reg);
		vdpq_yc_sep_mode_ctrl_reg.yc_sep_mode = 0;
		vdcf_outl(VDPQ_YC_SEP_MODE_CTRL_reg, vdpq_yc_sep_mode_ctrl_reg.regValue);
	}
	else
	{
		vdpq_dummy_5_RBUS vdpq_dummy_5_reg;
		vdpq_dummy_5_reg.regValue = vdcf_inl(VDPQ_Dummy_5_reg);
		vdpq_dummy_5_reg.regValue = vdpq_dummy_5_reg.regValue | 0x1; // eco bit
		vdcf_outl(VDPQ_Dummy_5_reg, vdpq_dummy_5_reg.regValue);
	}
#endif
}
...
void drvif_module_vpq_3D_mem_Initial(unsigned int mem_addr)
{
	...
	rtd_pr_vpq_info("[%s] Initial set addr = %x, _SDRAM_3D_ONE_FRAME_USE = %x\n", __func__, nRegAddr, _SDRAM_3D_ONE_FRAME_USE);
...
void drvif_module_vpq_DisableMemory(void)
{
	...
	if(dma_ctrl5_reg.mem_off == 0)	// if memory on
	{
		...
		drvif_module_vpq_DmaAccessCtrl(OFF);
		rtd_pr_vpq_debug("[%s] 3D disable!! memory off\n", __func__);
	}
}
...
void drvif_module_yc_boot_init(void)
{
	/* YC init at boot, ycsep_demod_region_always_on init */
	vdcf_maskl(VDPQ_AUTO_MODE_YCSEP_DEMOD_REGION_reg, ~(_BIT16), _ZERO);
}

unsigned char drvif_module_vpq_disable_handler(void)
{
	ucHandlerStatus = VPQ_HANDLE_STATUS_DISABLE;
	rtd_pr_vpq_info("[%s] set vpq handler disable\n", __func__);
	return ucHandlerStatus;
}

unsigned char drvif_module_vpq_enable_handler(void)
{
	ucHandlerStatus = VPQ_HANDLE_STATUS_ENABLE;
	rtd_pr_vpq_info("[%s] set vpq handler enable\n", __func__);
	return ucHandlerStatus;
}

unsigned char drvif_module_vpq_get_handler_status(void)
{
	return ucHandlerStatus;
}