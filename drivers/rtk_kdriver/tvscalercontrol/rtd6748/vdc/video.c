...
#ifdef CONFIG_CUSTOMER_TV010
#define VDC_Error_case1 0	/* ATSC_081202_0925_337.00M_-37.18_20M_-32.77_LAN_FieldSignal(ch3)*/
//#define VDC_Error_case2 0	/* do not enable this case!! */
#define VDC_Error_case3 0	/* over modulation issue */
#define VDC_Error_case4 0	/* ATSC_110225_0925_196.50M_-38.86_20M_-35.42_NON_FieldSignal(ch11)*/
#define VDC_Error_case5 0	/* sync reduction to 20% */
#define VDC_Error_case6 0	/* ATSC_081202_0925_337.00M_-37.18_20M_-32.77_LAN_FieldSignal(ch2)*/
#define VDC_Error_case7 0	/* ntsc_070615_1512_201 */
#define VDC_Error_case8 0	/* atsc_080922_1518_5790 */
#define VDC_Error_case9 1	/* skyworth non standard */
#else
...
#define VDC_Error_case10 1	/* Vsync 525 change to 625, can't detect correct 625flag */
#endif
#define AV_FastBlack	1

extern KADP_VFE_AVD_DEMOD_TYPE Scaler_AVD_GetDemodType(void);
...
static bool vdc_needopenpowerflag = TRUE;//True now vd power off/ saving need to set on
...
unsigned char vdc_resetTV_Count = 0x0;
...
static int g_hstate_vsync_error_unbalance_status_cnt_check = 0 ;
static unsigned int g_hstate_vsync_error_unbalance_status_result_check = 0;
...
static int g_dgain_status_flag  = 0 ;

extern void module_vdc_Clear_SECAM_Setting(void);
extern void module_vdc_Set_SECAM_Setting(void);

#ifdef CONFIG_ENABLE_VDC_BIG_SIGNAL_CHECK
extern unsigned char  per_Peak_data_sel_value=0;
extern unsigned char  count_big_signal_fail=0;
extern unsigned char count_big_signal_ok=0;
#endif

// refactor sta ==================================================>

bool drvif_module_vdc_get_needopenpowerflag(void)
{
	return vdc_needopenpowerflag;
}

void drvif_module_vdc_set_needopenpowerflag(bool bOnOff)
{
	vdc_needopenpowerflag = bOnOff;
}

VDC_FRAME_RATE drvif_module_vdc_GetFrameRate(void)
{
	return field;
}
...
void drvif_module_vdc_ClockOn(void)
{
	...
	rtd_pr_vdc_debug("0xb8022284=%x\n", IoReg_Read32(0xb8022284));
	IoReg_Write32(SYS_REG_SYS_CLKEN2_reg, _BIT24 | _BIT25 | _BIT0);
	IoReg_Write32(0xb8000108, _BIT24 | _BIT25 | _BIT0);
	IoReg_Write32(SYS_REG_SYS_CLKEN1_reg, _BIT2 | _BIT0);
	IoReg_Write32(SYS_REG_SYS_SRST1_reg, _BIT2 | _BIT0);
#endif
...
bool module_vdc_check_vd_clock(void)
{
	...
	if (!ucRet) {
		VDC_DEBUG_MSG(VDC_MSG_DEBUG1, "VD clock is not setted, set it !\n");
	}

	return ucRet;
}

bool drvif_module_vdc_check_pll27x_setting(void)
{
	enum CRT_CLK clken = CLK_ON;
	unsigned char ucRet = _TRUE;
...
void drvif_module_vdc_Power_Setting(VDC_POWER_SETTING mode, unsigned char a_ucInputSrc)
{
	...
	//rtd_pr_vdc_info("drvif_module_vdc_Power_Setting vdc_needopenpowerflag=%d\n",vdc_needopenpowerflag);
}

void drvif_module_vd_ADC_PowerCtrl(unsigned char on_off, unsigned char a_ucInputSrc)
{	/* 2_0048, 2_0050, 2_0054, 2_005c */
	tv_sb1_ana_vd_adc_clamp_ctl_1_RBUS tv_sb1_ana_vd_adc_clamp_ctl_1_reg; // 0xb8020048
	tv_sb1_ana_vd_adc_updn_ctl_RBUS tv_sb1_ana_vd_adc_updn_ctl_reg;       // 0xb8020050
	tv_sb1_ana_vdadc_pow_ctl_RBUS tv_sb1_ana_vdadc_pow_ctl_reg;           // 0xb802005c

	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.regValue = rtdf_inl(TV_SB1_ANA_VD_ADC_CLAMP_CTL_1_reg);
	tv_sb1_ana_vd_adc_updn_ctl_reg.regValue = rtdf_inl(TV_SB1_ANA_VD_ADC_UPDN_CTL_reg);
	tv_sb1_ana_vdadc_pow_ctl_reg.regValue = rtdf_inl(TV_SB1_ANA_VDADC_POW_CTL_reg);

	if (on_off == ON) {
		/*=== ADC power on/off ===*/
		down(get_adc_power_semaphore());
#ifdef CONFIG_SUPPORT_SRC_ADC
		if (SRC_CONNECT_DONE == get_ADC_Global_Status()) {
			drvif_adc_power_control(ADC_POWER_YPP_ONLY_DETECT_CONTROL, __func__, __LINE__); //make sure adc alive
		} else
		...
		if (_SRC_TV != a_ucInputSrc) {
			tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_pow = 1;	/* 2_005c[0] VDADC & PGA Power */
			//VDC_DEBUG_MSG(VDC_MSG_INIT, "drvif_module_vdc_PowerCtrl [Set VD-1]\n");
		}
		// tv_sb1_ana_vdadc_pow_ctl_reg.ldo100ma_pow = 1;	/* the major power of vdadc */
		rtdf_outl(TV_SB1_ANA_VDADC_POW_CTL_reg, tv_sb1_ana_vdadc_pow_ctl_reg.regValue);
		...
		if ((SRC_NOTHING == get_ADC_Global_Status()) || (SRC_INIT_DONE == get_ADC_Global_Status())) {
			...
			if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_ADC){
				//ATV recording, change to YPP source, stop recording, make sure adc power open
				drvif_adc_power_control(ADC_POWER_YPP_SCALER_CONTROL,__func__,__LINE__);
			}else{
				drvif_adc_power_control(ADC_POWER_YPP_ONLY_DETECT_CONTROL,__func__,__LINE__);//make sure adc alive
		...
		tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_pow = 1;	/* 2_005c[0] VDADC & PGA Power */
#else
		tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_pow = 0;	/* 2_005c[0] VDADC & PGA Power */
// #ifdef ENABLE_PWR_IP_CLOSE_SIRIUS
// 		tv_sb1_ana_vdadc_pow_ctl_reg.ldo100ma_pow = 0;	/* 2_005c[1]=1, enable LDO power */
// #endif
#endif
		tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_15 = 0x0;
		tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_14 = 0x0;
		tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_13 = 0x0;
		tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_7 = 0;
		tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_6 = 0x0;
		tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_5 = 0x0;
		tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_2 = 0x0;	/* 2_0050[2:0] = 0, clamp up/dn vin9~14 */
		rtdf_maskl(TV_SB1_ANA_SCART_CTRL1_reg, ~(_BIT0), _ZERO);

		rtdf_outl(TV_SB1_ANA_VD_ADC_CLAMP_CTL_1_reg, tv_sb1_ana_vd_adc_clamp_ctl_1_reg.regValue);
		rtdf_outl(TV_SB1_ANA_VD_ADC_UPDN_CTL_reg, tv_sb1_ana_vd_adc_updn_ctl_reg.regValue);
		rtdf_outl(TV_SB1_ANA_VDADC_POW_CTL_reg, tv_sb1_ana_vdadc_pow_ctl_reg.regValue);
	}
}
...
void drvif_module_vdc_108M_FrontDigitalFilterEnable(VDC_VIDEO_108M_FrontDigitalFilter mode)
{
	...
	//0118[27]CLKEN_ATVIN_VD enable
	rtd_outl(SYS_REG_SYS_CLKEN2_reg,SYS_REG_SYS_CLKEN2_clken_atvin_vd_mask | SYS_REG_SYS_CLKEN2_write_data_mask);
	...
	case VDC_108M_FDF_80MHZ:
		...
		vd_inout_ctrl_reg.fe_fil_sel = 1;
		break;
	case VDC_108M_FDF_60_75MHZ:
		drvif_module_vdc_input_filter_sel(0, INPUT_FILTER_60_75MHZ);
		vd_inout_ctrl_reg.fe_fil_sel = 1;
		break;
...
unsigned char drvif_module_vdc_Read_108M_FrontDigitalFilter_info(void)
{
	...
	if (vd_inout_ctrl_reg.fe_fil_sel != 0) {
		...
		else if (input_flt_tap_0_1_reg.input_flt_tap1 == INPUT_FILTER_60_75MHZ[1])
			return VDC_108M_FDF_60_75MHZ;
...
void module_vdc_adcNegativeAdjustment(unsigned char ucValue)
{
	...
	if (drvif_module_vdc_CheckFactoryModEnable(ON))
		return;
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.regValue = rtdf_inl(TV_SB1_ANA_VD_ADC_CLAMP_CTL_1_reg);
	if (tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl0_7_4 != ucValue) {
		tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl0_7_4 = ucValue;
		rtdf_outl(TV_SB1_ANA_VD_ADC_CLAMP_CTL_1_reg, tv_sb1_ana_vd_adc_clamp_ctl_1_reg.regValue);
	}
}

void module_vdc_BlankLevelAdjustment(unsigned int uiValue)
{
	/* 1_96A0 */
	vdtop_blank_level_manu_comp_value_RBUS blank_level_manu_comp_value_reg;
	if (drvif_module_vdc_CheckFactoryModEnable(ON))
		return;
	blank_level_manu_comp_value_reg.regValue = rtdf_inl(VDTOP_BLANK_LEVEL_MANU_COMP_VALUE_reg);
	if (blank_level_manu_comp_value_reg.blank_level_comp_manu_add != uiValue)
	{
		blank_level_manu_comp_value_reg.blank_level_comp_manu_add = uiValue;
		rtdf_outl(VDTOP_BLANK_LEVEL_MANU_COMP_VALUE_reg, blank_level_manu_comp_value_reg.regValue);
	}
}
...
unsigned int drvif_video_status_reg(VDC_STATUS_REG status)
{
	...
	case VDC_mv_detect:
		video_status_reg1_reg.regValue = vdcf_inl(VDTOP_VIDEO_STATUS_REG1_reg);
		result = video_status_reg1_reg.mov_vbi_detected;
		break;
	case VDC_cid_detect:
		video_status_reg1_reg.regValue = vdcf_inl(VDTOP_VIDEO_STATUS_REG1_reg);
		result = video_status_reg1_reg.mov_colour_inverse;
		break;
...
unsigned int drvif_module_vdc_read_Cgain_data(void)
{
       /* 9028[29:16] */
       vdtop_cgain_status_RBUS cgain_status_reg;
       cgain_status_reg.regValue =  vdcf_inl(VDTOP_CGAIN_STATUS_reg);
       return cgain_status_reg.status_cgain_data;
}
...
void drvif_module_vdc_set_Control2_Register(VDC_VIDEO_CONTROL2 mode)
{
	...
	case VDC_mv_hagc_mode_on:
		video_control2_reg.mov_hagc_mode = 1;
		break;
	case VDC_mv_hagc_mode_off:
		video_control2_reg.mov_hagc_mode = 0;
		break;
	...
	case VDC_mv_detect_sel_hpll:
		video_control2_reg.mov_detect_sel = 0;
		break;
	case VDC_mv_detect_sel_vpll:
		video_control2_reg.mov_detect_sel = 1;
		break;
	}
...
bool drvif_module_vdc_read_spike_detected(void)
{
	/* 9114[10] */
	vdtop_spike_detected_up_RBUS spike_detected_reg;
	spike_detected_reg.regValue = vdcf_inl(VDTOP_spike_detected_up_reg);
	return spike_detected_reg.spike_detected;
}
...
void drvif_module_vdc_set_second_pll(VDC_SECOND_PLL_ENABLE_REG on_off)
{
	...
	second_pll_gain_reg.regValue = vdcf_inl(VDTOP_Second_pll_gain_reg);
	new_clampagc_ctrl_reg.regValue = vdcf_inl(VDTOP_NEW_CLAMPAGC_CTRL_reg);
	//if (second_pll_gain_reg.second_pll_en != on_off) { //AGC Stability - Picture Shaking,0x2 -->FIR mode/0x0 -->IIR mode, RL
...
void drvif_module_vdc_cpll_reset(void)
{
	...
	//printk("#####[%s(%d)] enter cpll_cgain_reset_en, 9218=0x%08x \n", __func__, __LINE__, vdcf_inl(VDTOP_CSTATE_STATUS_reg));
	vdcf_outl(VDTOP_CSTATE_STATUS_reg, vdtop_cstate_status_reg.regValue);
	mdelay(50);
	vdtop_cstate_status_reg.cpll_cgain_reset_en = 0;
	//printk("#####[%s(%d)] delay 50ms cpll_cgain_reset_en, 9218=0x%08x \n", __func__, __LINE__, vdcf_inl(VDTOP_CSTATE_STATUS_reg));
	vdcf_outl(VDTOP_CSTATE_STATUS_reg, vdtop_cstate_status_reg.regValue);
	//printk("#####[%s(%d)] exit cpll_cgain_reset_en, 9218=0x%08x \n", __func__, __LINE__, vdcf_inl(VDTOP_CSTATE_STATUS_reg));
}
...
void  drvif_module_vdc_vloop_control_set(VDC_Vloop_Control value)
{
	...
	case VDC_vloop_speed0:	/* for AV source */
		/* default setting */
		vsync_time_cont1_reg.vloop_tc1 = 0x80;
		vsync_time_cont2_reg.vloop_tc2 = 0x10;
		vsync_time_cont_reg.vloop_tc3 = 0x02;
		vloop2_tc1_reg.vloop2_tc1 = 0x10;
		vloop2_tc2_reg.vloop2_tc2 = 0x02;
		vloop_tc3_reg.vloop2_tc3 = 0x02;
		vloop0_tc1_reg.vloop0_tc1 = 0x80;
		vloop0_tc2_reg.vloop0_tc2 = 0x08;
		vloop_tc3_reg.vloop0_tc3 = 0x02;
		break;
	case VDC_vloop_speed1:	/* for TV source */
		...
		vsync_time_cont2_reg.vloop_tc2 = 0x10;	/*state1 */
		vsync_time_cont1_reg.vloop_tc1 = 0x60;
		vloop2_tc2_reg.vloop2_tc2 = 0x10;		/*state2 */
		vloop2_tc1_reg.vloop2_tc1 = 0x60;
		vloop_tc3_reg.vloop0_tc3 = 0x00;	/*0x02 */
		break;
	case VDC_vloop_FAST:
		break;
	}
...
void module_vdc_Set_lose_chromalock_level(unsigned char ucValue)
{
	/* 9408[3:1]=0x5 */
	vdtop_chroma_lock_config_RBUS vdtop_chroma_lock_config_reg;
	vdtop_chroma_lock_config_reg.regValue = vdcf_inl(VDTOP_CHROMA_LOCK_CONFIG_reg);
...
unsigned char drvif_module_vdc_Get_CKill_Auto(void)
{
	...
	vdpq_auto_pos_ctrl_RBUS auto_pos_ctrl_reg;
	vdpq_auto_pos_ctrl_2_RBUS auto_pos_ctrl_2_reg;
	auto_pos_ctrl_reg.regValue = vdcf_inl(VDPQ_AUTO_POS_CTRL_reg);
	auto_pos_ctrl_2_reg.regValue = vdcf_inl(VDPQ_AUTO_POS_CTRL_2_reg);
	if ((auto_pos_ctrl_reg.regValue >> 24 == 0x77) && ((auto_pos_ctrl_2_reg.regValue >> 20) & 0xf) == 0x7) {
		return CKill_ON;
	} else {
		return CKill_OFF;
	}
return 0 ;//after remove VPQ_COMPILER_ERROR_ENABLE also remove this line
}
...
void drvif_module_vdc_Cpeak_set(unsigned char on_off)
{
	...
	//rtd_pr_vdc_emerg("[VDC] drvif_module_vdc_Cpeak_set %d   %d  ", on_off, chroma_agc_peak_ctrl_reg.chroma_peak_en);
...
#ifdef CONFIG_CUSTOMER_TV010
void module_vdc_set_chroma_agc_peak_nominal(unsigned char value)
{
	/* 9434[7:0] */
	vdtop_chroma_agc_peak_nominal_RBUS chroma_agc_peak_nominal_reg;
	chroma_agc_peak_nominal_reg.regValue = vdcf_inl(VDTOP_CHROMA_AGC_PEAK_NOMINAL_reg);
	chroma_agc_peak_nominal_reg.chroma_agc_peak_nominal = value;
	vdcf_outl(VDTOP_CHROMA_AGC_PEAK_NOMINAL_reg, chroma_agc_peak_nominal_reg.regValue);
}
#endif

void drvif_module_vdc_set_CbCrDelay(unsigned char ucCb_delay, unsigned char ucCr_delay)
{
	/* 9448[6:4][2:0] */
	vdtop_chroma_delay_ctrl_RBUS chroma_delay_ctrl_reg;
	chroma_delay_ctrl_reg.regValue = vdcf_inl(VDTOP_CHROMA_DELAY_CTRL_reg);
	if ((chroma_delay_ctrl_reg.cb_delay != ucCb_delay) ||
		(chroma_delay_ctrl_reg.cr_delay != ucCr_delay)) {
		chroma_delay_ctrl_reg.cb_delay = ucCb_delay;
		chroma_delay_ctrl_reg.cr_delay = ucCr_delay;
...
void drvif_module_vdc_Disablemode(unsigned char on_off)
{
	/* 94a8[6]*/
	vdtop_statistic_and_disable_mode_RBUS disable_mode_reg;
	disable_mode_reg.regValue = vdcf_inl(VDTOP_STATISTIC_AND_DISABLE_MODE_reg);
	if (on_off != disable_mode_reg.disable_mode)
	{
		disable_mode_reg.disable_mode = on_off;
		vdcf_outl(VDTOP_STATISTIC_AND_DISABLE_MODE_reg, disable_mode_reg.regValue);
	}
}

void drvif_module_vdc_DisablemodeEN(unsigned char on_off)
{
	/* 94a8[7]*/
	vdtop_statistic_and_disable_mode_RBUS disable_mode_reg;
	disable_mode_reg.regValue = vdcf_inl(VDTOP_STATISTIC_AND_DISABLE_MODE_reg);
	if (on_off != disable_mode_reg.disable_mode_en)
	{
		disable_mode_reg.disable_mode_en = on_off;
		vdcf_outl(VDTOP_STATISTIC_AND_DISABLE_MODE_reg, disable_mode_reg.regValue);
	}
}

bool drvif_module_vdc_ReadDisablemodeEN(void)
{
	/* 94a8[7] */
	vdtop_statistic_and_disable_mode_RBUS disable_mode_reg;
	disable_mode_reg.regValue = vdcf_inl(VDTOP_STATISTIC_AND_DISABLE_MODE_reg);
	return disable_mode_reg.disable_mode_en;
}
...
void drvif_module_vdc_set_Basic_Register(VDC_VIDEO_BASIC_REGISTER mode)
{
	...
	case VDC_BASIC_REG_ClearAll:
		vd_basic_register_enable_reg.regValue = 0x00000000;
		break;
...
/******************************************************************************
*
* FUNCTION       :  void drvif_module_vdc_Set_NewY1Cb1Cr1_delay(bool bEnable, unsigned char InYcsep, unsigned char InSource, unsigned char InMode)
*
* USAGE          	:  Set Y/Cb/Cr DELAY POSITION
*
* INPUT         	 	:  	< ucMode > :
*                                { 0 : ntsc
*                                   1 : palm
...
void drvif_module_vdc_vline_625_src_set(unsigned char on_off)
{
	/* 9508[30] */
	...
	if (vdtop_flag_625_counter_reg.vline_625_src_sel != on_off) {
		vdtop_flag_625_counter_reg.vline_625_src_sel = on_off;
...
#if 1
unsigned char drvif_module_vdc_read_internal_auto_scan_flag(void)
{
...
void drvif_module_vdc_set_internal_auto_scan_flag(unsigned char on_off)
{
	/* 96b4[27], for internal demod auto scan do softreset */
	vdtop_fpga_test_reg2_RBUS fpga_test_reg2_reg;
	fpga_test_reg2_reg.regValue = vdcf_inl(VDTOP_FPGA_TEST_REG2_reg);
...
unsigned char drvif_module_vdc_read_internal_ch_change_flag(void)
{
	/* 96b4[27] */
	vdtop_fpga_test_reg2_RBUS fpga_test_reg2_reg;
	fpga_test_reg2_reg.regValue = vdcf_inl(VDTOP_FPGA_TEST_REG2_reg);
	return (unsigned char)((fpga_test_reg2_reg.fpga_test2 & VDC_MSG_INT_CH_FLAG) >> 24);
}

void drvif_module_vdc_set_internal_ch_change_flag(unsigned char on_off)
{
	/* 96b4[27] */
	vdtop_fpga_test_reg2_RBUS fpga_test_reg2_reg;
	fpga_test_reg2_reg.regValue = vdcf_inl(VDTOP_FPGA_TEST_REG2_reg);
	if (on_off)
		fpga_test_reg2_reg.regValue = fpga_test_reg2_reg.regValue | VDC_MSG_INT_CH_FLAG;
	else
		fpga_test_reg2_reg.regValue = fpga_test_reg2_reg.regValue & ~VDC_MSG_INT_CH_FLAG;
	vdcf_outl(VDTOP_FPGA_TEST_REG2_reg, fpga_test_reg2_reg.regValue);
}
...
void drvif_module_vdc_ResetReg(void)
{
	...
	module_vdc_Set_lose_chromalock_level(0x05);		/* set to default */
...
//	Scaler_SetYcSep(VDC_YCSEPARATE_DEFAULT);
	...
	/* Reset lock & no-signal counters for re-enabling peak white control procedure. */
	...
	if(get_ic_version() == VERSION_A){
		if(get_AVD_Input_Source() == _SRC_CVBS){
			if(drvif_module_vdc_ReadMode(VDC_DETECT) == VDC_MODE_SECAM){
				module_vdc_Hactive_Start(FwMode, 0xb5);//for AV SECAM
				drvif_module_vdc_set_Hsync_start(0x3b);//for AV SECAM
				rtd_pr_vdc_info("#####[%s(%d)] for AV SECAM,0xB80194b4=0x%08x([7]=1),0xB801916C=0x%08x([7:0]=0x3b),0xB8019208=0x%08x([7:0]=0xb5)\n",__func__,__LINE__,vdcf_inl(VDTOP_VD_BASIC_REGISTER_ENABLE_reg),vdcf_inl(VDTOP_HSYNC_PULSE_START_reg),vdcf_inl(VDTOP_HACTIVE_START_reg));
			}else{
				module_vdc_Hactive_Start(FwMode, 0xdf);//for AV
				drvif_module_vdc_set_Hsync_start(0x65);//for AV
				rtd_pr_vdc_info("#####[%s(%d)] for AV,0xB80194b4=0x%08x([7]=1),0xB801916C=0x%08x([7:0]=0x65),0xB8019208=0x%08x([7:0]=0xdf)\n",__func__,__LINE__,vdcf_inl(VDTOP_VD_BASIC_REGISTER_ENABLE_reg),vdcf_inl(VDTOP_HSYNC_PULSE_START_reg),vdcf_inl(VDTOP_HACTIVE_START_reg));
			}
		}else{
			module_vdc_Hactive_Start(FwMode, 0xc2);//for ATV
			drvif_module_vdc_set_Hsync_start(0);//for ATV
			rtd_pr_vdc_info("#####[%s(%d)] for ATV,0xB80194b4=0x%08x([7]=1),0xB801916C=0x%08x([7:0]=0),0xB8019208=0x%08x([7:0]=0xc2)\n",__func__,__LINE__,vdcf_inl(VDTOP_VD_BASIC_REGISTER_ENABLE_reg),vdcf_inl(VDTOP_HSYNC_PULSE_START_reg),vdcf_inl(VDTOP_HACTIVE_START_reg));
		}
	}
#endif
...
void drvif_module_vdc_tv_Reset(void)
{
...
#endif
#if VDC_Error_case10
	drvif_module_vdc_vline_625_src_set(_DISABLE);
#endif
	drvif_module_vdc_Disable_Auto_Burst_position();
#ifdef VD_hdto_adjust
	module_vdc_set_hdto_inc(HDTO_Default);
	...
	if (tvAutoScanFlag) {
		...
		drvif_module_vdc_set_hlock_field_cnt_Value(0x02); //[JF] ori is 0x5, modify for all mode scan(include SECAM-L)
#else
...
unsigned char drvif_module_vdc_CheckHVisLocked(void)
{
	...
	if (drvif_module_vdc_read_Clamping_path()) { /* is TV */
		...
		if (drvif_module_vdc_read_scan_flag()) {
			...
			//fix_625_flag HW san and scan speed up, RL
			drvif_module_vdc_DisablemodeEN(_ENABLE);
			drvif_module_vdc_Disablemode(_DISABLE);
			drvif_module_vdc_set_noSignalCounter_Value(0x02);
			VDC_DEBUG_MSG(VDC_MSG_TVSCAN, "[Video][CheckHV][DetectTiming]**DisablemodeAddress=%x, noSignalCounter_ValueAddress=%x\n",  vdcf_inl(VDTOP_STATISTIC_AND_DISABLE_MODE_reg), vdcf_inl(VDTOP_NO_SIGNAL_DEC_reg));
			...
			if ((drvif_module_vdc_ReadDisablemodeEN() == 1)&&drvif_module_vdc_read_scan_flag()){//fix_625_flag HW san and scan speed up old method, RL
...
unsigned char drvif_video_ChromaLockStatus(void)
{
...
void drvif_module_vdc_CheckInputStatus(unsigned char ucInputSrc, unsigned char ucCheckStart)
{
	...
	vdtop_fpga_test_reg1_RBUS fpga_test_reg1_reg;
    //adc_vdadc_core_lpf_ctrl_RBUS adc_vdadc_core_lpf_ctrl_reg; 
    //adc_vd_adc_clamp_ctl_1_RBUS adc_vd_adc_clamp_ctl_1_reg;
	unsigned int ui_9008 ;
	bool unBalance_flag ;
	bool unBalance_flag_check ;
	...
	if (fpga_test_reg2_reg.regValue != VDreg_NoAck)
	{
		...
		rtd_pr_vdc_info("(dead==>)VDTOP_FPGA_TEST_REG2_reg==%x\n", fpga_test_reg2_reg.regValue);
		return;
	}

	//drvif_module_vdc_Status_Loop();
#if 0
	//[start]ryanliao 20181025 [9488]
	display_timing_ctrl1_REG.regValue = vdcf_inl(PPOVERLAY_Display_Timing_CTRL1_reg);

	if (ucInputSrc == _SRC_CVBS && display_timing_ctrl1_REG.disp_fix_last_line == 0x00) {
		//rtd_pr_vdc_emerg("ADC3_test_0\n");
		vd_ifd_input_vd_adc_lsb_ctrl_reg.regValue = vdcf_inl(VD_IFD_INPUT_VD_ADC_LSB_CTRL_reg);
		if (vd_ifd_input_vd_adc_lsb_ctrl_reg.adc3_lsb_force0_num != 0x02){
			vd_ifd_input_vd_adc_lsb_ctrl_reg.adc3_lsb_force0_num = 0x02;
			vdcf_outl(VD_IFD_INPUT_VD_ADC_LSB_CTRL_reg, vd_ifd_input_vd_adc_lsb_ctrl_reg.regValue);
			rtd_pr_vdc_emerg("ADC3_test_1\n");
		}
	}else{
		//rtd_pr_vdc_emerg("ADC3_test_2\n");
		vd_ifd_input_vd_adc_lsb_ctrl_reg.regValue = vdcf_inl(VD_IFD_INPUT_VD_ADC_LSB_CTRL_reg);
		if (vd_ifd_input_vd_adc_lsb_ctrl_reg.adc3_lsb_force0_num == 0x02){
			vd_ifd_input_vd_adc_lsb_ctrl_reg.adc3_lsb_force0_num = 0x00;
			vdcf_outl(VD_IFD_INPUT_VD_ADC_LSB_CTRL_reg, vd_ifd_input_vd_adc_lsb_ctrl_reg.regValue);
			rtd_pr_vdc_emerg("ADC3_test_3\n");
		}
	}
	//[end]
#endif
	...
	if (fpga_test_reg1_reg.regValue != VDreg_NoAck)
	{
		if (fpga_test_reg1_reg.factory_flag)
		{
			...
			if( ucTestDmaLength != 0 )
			{
				rtd_pr_vdc_emerg("VDTOP_FPGA_TEST_REG1_reg = %x, length = %x\n", fpga_test_reg1_reg.regValue, ucTestDmaLength);
				...
				drvif_module_vpq_disable_handler();
				drvif_module_vdc_factory_TestDMA_Enable(ucTestDmaLength);
				drvif_module_vdc_clean_factory_flag();
				drvif_module_vpq_enable_handler();
			}
			else
			{
				rtd_pr_vdc_emerg("dump size length = %x error\n", fpga_test_reg1_reg.regValue);
			}
		}
	} else {
		rtd_pr_vdc_emerg("(dead==>)VDTOP_FPGA_TEST_REG1_reg==%x\n", fpga_test_reg1_reg.regValue);
		// return;
	}
	...
	if (vdc_LockStatus & ucCheckStart) {
		vdtop_adc_over_under_flag_RBUS adc_over_under_flag_reg;
		vdtop_agc_gate_width_RBUS agc_gate_width_reg;
		...
		//for secam fliter_2018.05.07_john
		if (drvif_module_vdc_ReadMode(VDC_SET) == VDC_MODE_SECAM)
		{
			vdtop_video_control0_RBUS video_control0_reg;
			video_control0_reg.regValue = vdcf_inl(VDTOP_VIDEO_CONTROL0_reg);
			video_control0_reg.secam_eq_share_sel = 1;
			vdcf_outl(VDTOP_VIDEO_CONTROL0_reg, video_control0_reg.regValue);
		}
		else
		{
			vdtop_video_control0_RBUS video_control0_reg;
			video_control0_reg.regValue = vdcf_inl(VDTOP_VIDEO_CONTROL0_reg);
			video_control0_reg.secam_eq_share_sel = 0;
			vdcf_outl(VDTOP_VIDEO_CONTROL0_reg, video_control0_reg.regValue);
		}

		//Adjust the limit low of dgain
		adc_over_under_flag_reg.regValue = vdcf_inl(VDTOP_ADC_OVER_UNDER_FLAG_reg);
		vdcf_outl(VDTOP_ADC_OVER_UNDER_FLAG_reg, adc_over_under_flag_reg.regValue);
		adc_over_under_flag_reg.regValue = vdcf_inl(VDTOP_ADC_OVER_UNDER_FLAG_reg);
		agc_gate_width_reg.regValue = vdcf_inl(VDTOP_AGC_GATE_WIDTH_reg);
#ifdef VD_SHGain_SONY       //S/H gain = 1.6x, vdadc_clamp_n setup 0x6, dgain limit low set to 0x230, DC restore offset 0.4V disable
		#if 0
        adc_vdadc_core_lpf_ctrl_reg.regValue = vdcf_inl(ADC_VDADC_CORE_LPF_CTRL_reg);   //02_0054
        adc_vdadc_core_lpf_ctrl_reg.vdadc_adc_ctrl_0 = 0x1;                             //<bit16> VDADC_ADC_CTRL[0] = 1(S/H gain=1.6x)
        vdcf_outl(ADC_VDADC_CORE_LPF_CTRL_reg, adc_vdadc_core_lpf_ctrl_reg.regValue);
        
        adc_vd_adc_clamp_ctl_1_reg.regValue = vdcf_inl(ADC_VD_ADC_CLAMP_CTL_1_reg);     //02_0048
        adc_vd_adc_clamp_ctl_1_reg.vdadc_clamp_ctrl1_15 = 0;                            //<bit15> VDADC_CLAMP_CTRL1[15] = 0(disable dc_restore offset_0.4V)
        adc_vd_adc_clamp_ctl_1_reg.vdadc_clamp_ctrl0_7_4 = 0x6;                         //<bit15> VDADC_CLAMP_CTRL0[7:4] = 0x6
        vdcf_outl(ADC_VD_ADC_CLAMP_CTL_1_reg, adc_vd_adc_clamp_ctl_1_reg.regValue);
		#endif
		if ((adc_over_under_flag_reg.ad4_value_big == 0) && (vdcf_inl(VDTOP_NOISE_STATUS_reg) <= 0x10) && (vdcf_inl(VDTOP_AGC_DGAIN_STATUS_reg) < 0x230) && (agc_gate_width_reg.data_path_dgain_limit_low != 0x230))
		{
		    agc_gate_width_reg.data_path_dgain_limit_low= 0x230;                        //limit low of dgain set to 0x230
		    vdcf_outl(VDTOP_AGC_GATE_WIDTH_reg, agc_gate_width_reg.regValue);
        }
#else                       //S/H gain = 1.0x for default setting
		#if 0
        adc_vdadc_core_lpf_ctrl_reg.regValue = vdcf_inl(ADC_VDADC_CORE_LPF_CTRL_reg);
        adc_vdadc_core_lpf_ctrl_reg.vdadc_adc_ctrl_0 = 0;
        vdcf_outl(ADC_VDADC_CORE_LPF_CTRL_reg, adc_vdadc_core_lpf_ctrl_reg.regValue);
        
        
        adc_vd_adc_clamp_ctl_1_reg.regValue = vdcf_inl(ADC_VD_ADC_CLAMP_CTL_1_reg);
        adc_vd_adc_clamp_ctl_1_reg.vdadc_clamp_ctrl1_15 = 0x1;
        adc_vd_adc_clamp_ctl_1_reg.vdadc_clamp_ctrl0_7_4 = 0xC;
        vdcf_outl(ADC_VD_ADC_CLAMP_CTL_1_reg, adc_vd_adc_clamp_ctl_1_reg.regValue);
		#endif
		if ((adc_over_under_flag_reg.ad4_value_big == 0) && (vdcf_inl(VDTOP_NOISE_STATUS_reg) <= 0x10) && (vdcf_inl(VDTOP_AGC_DGAIN_STATUS_reg) < 0x350) && (agc_gate_width_reg.data_path_dgain_limit_low != 0x350))
		{
			agc_gate_width_reg.data_path_dgain_limit_low= 0x350;
			vdcf_outl(VDTOP_AGC_GATE_WIDTH_reg, agc_gate_width_reg.regValue);

		}else if((adc_over_under_flag_reg.ad4_value_big == 0) && agc_gate_width_reg.data_path_dgain_limit_low == 0x350 && vdcf_inl(VDTOP_AGC_DGAIN_STATUS_reg) > 0x400){

			g_dgain_status_flag =0;
		}
#endif

		//when sync level too small, let hsync locking early
		if (drvif_module_vdc_read_spike_detected() == 1)
			module_vdc_set_vsync_h_max(0x0);	/* 9324[6:0] set lower */
		else
			module_vdc_set_vsync_h_max(0xe);	/* 9324[6:0] set default */

		if (((adc_over_under_flag_reg.ad4_value_big == 1) ||(vdcf_inl(VDTOP_NOISE_STATUS_reg) > 0x10)) && (agc_gate_width_reg.data_path_dgain_limit_low != 0x200))
		{
			if (g_dgain_status_flag ==1){
				agc_gate_width_reg.data_path_dgain_limit_low= 0x350;
				vdcf_outl(VDTOP_AGC_GATE_WIDTH_reg, agc_gate_width_reg.regValue);
				
				if (vdcf_inl(VDTOP_AGC_DGAIN_STATUS_reg) > 0x350){
					g_dgain_status_flag =0;
				}
			}

			if (g_dgain_status_flag !=1){ // check normal case,because change the 0x200, it should be >350 level of DGAIN level
				agc_gate_width_reg.data_path_dgain_limit_low= 0x200;
				vdcf_outl(VDTOP_AGC_GATE_WIDTH_reg, agc_gate_width_reg.regValue);
			}

			if(vdcf_inl(VDTOP_AGC_DGAIN_STATUS_reg) < 0x300){ //fake sync level small, keep 0x350

				agc_gate_width_reg.data_path_dgain_limit_low= 0x350;
				vdcf_outl(VDTOP_AGC_GATE_WIDTH_reg, agc_gate_width_reg.regValue);

				g_dgain_status_flag =1;
			}
		}
		...
		drvif_module_vpq_YcSeparation(ucInputSrc, _ENABLE);
		...
		if (ucInputSrc == _SRC_CVBS) {
			...
			drvif_module_vdc_big_signal_detect_function(1);
...
//					}
...
#endif
#if VDC_Error_case7
				if (drvif_module_vdc_Read_Hstate_set() == 0x03){ //AGC Stability - Picture Shaking, RL
					unBalance_flag_check = (bool)drvif_module_vdc_read_hstate_vsync_error_unbalance_check();
					VDC_DEBUG_MSG(VDC_MSG_DEBUG2, "drv_module_vdc_Hstate_control_check!!flag=%d \n",unBalance_flag_check);
					if(g_hstate_vsync_error_unbalance_status_cnt_check == 8){
						g_hstate_vsync_error_unbalance_status_cnt_check = 0;
						if (_FALSE == unBalance_flag_check){
							VDC_DEBUG_MSG(VDC_MSG_DEBUG2, "[ATV 7] drv_module_vdc_Hstate_control fix Hstate=1!! check flag=%d \n", unBalance_flag_check);
							drvif_module_vdc_Set_Hstate(VDC_NOT_FIX_HSTATE5);
						}
					}
				}else{
				...
				HState = drvif_module_vdc_Read_Hstate_Factory_Mode_Get_info();
					VDC_DEBUG_MSG(VDC_MSG_DEBUG2, "drv_module_vdc_Hstate_control!! Hstate=%d,err=%x,flag=%d \n",HState,Hdetect_err,unBalance_flag);
...
#else
				if (vdc_NoiseStatus > 0x70 && drvif_video_status_reg(VDC_Lines_625_detected) == 0) { //fix fluke<30db mode dectet and no color, RL
					module_vdc_set_Mode_443358pk_sel(VDC_old443358);
					VDC_DEBUG_MSG(VDC_MSG_DEBUG2, "weak_signal case6 improve\n");
				}
#endif
			}
#endif
	...
	//VDC_DEBUG_MSG(VDC_MSG_TVCHCH, "TVGetNonStandardFlag=%08x\n",nstvalue);
	if(ucInputSrc == _SRC_TV){
		if(nstvalue & ATVNONSTANDFLAG_0){
		}

		if(nstvalue & ATVNONSTANDFLAG_1){
			vdtop_avout_fifo_rd_ptr_RBUS avout_fifo_rd_ptr_reg;
			avout_fifo_rd_ptr_reg.regValue = vdcf_inl(VDTOP_AVOUT_FIFO_RD_PTR_reg);
			if(avout_fifo_rd_ptr_reg.avout_fifo_rd_ptr > 0x508)
			{
				avout_fifo_rd_ptr_reg.avout_fifo_rd_ptr = 0x508;
			}
			vdcf_outl(VDTOP_AVOUT_FIFO_RD_PTR_reg, avout_fifo_rd_ptr_reg.regValue);
		}

		if(nstvalue & ATVNONSTANDFLAG_2){

			vdpq_frame_buf_chk3d_ctrl1_RBUS frame_buf_chk3d_ctrl1_reg;
			vdpq_frame_buf_chk3d_ctrl2_RBUS frame_buf_chk3d_ctrl2_reg;
			vdpq_yc_sep_mode_ctrl_RBUS yc_sep_mode_ctrl_reg;

			//TODO, Merlin3 tv010 sync, need wait vip to add
			//frame_buf_chk3d_ctrl1_reg.regValue = vdcf_inl(VDPQ_FRAME_BUF_CHK3D_CTRL1_VADDR);
			//frame_buf_chk3d_ctrl2_reg.regValue = vdcf_inl(VDPQ_FRAME_BUF_CHK3D_CTRL2_VADDR);
			yc_sep_mode_ctrl_reg.regValue = vdcf_inl(VDPQ_YC_SEP_MODE_CTRL_reg);

			if((frame_buf_chk3d_ctrl1_reg.regValue & 0x000000ff) != 0xc2
			||(frame_buf_chk3d_ctrl2_reg.regValue & 0x000000ff) != 0x81
			||(yc_sep_mode_ctrl_reg.regValue & 0x000000ff) != 0x01)
			{
				//TODO, Merlin3 tv010 sync, need wait vip to add
				//vdcf_maskl(VDPQ_FRAME_BUF_CHK3D_CTRL1_VADDR, 0xffffff00, 0x000000c2);
				//vdcf_maskl(VDPQ_FRAME_BUF_CHK3D_CTRL2_VADDR, 0xffffff00, 0x00000081);
				vdcf_maskl(VDPQ_YC_SEP_MODE_CTRL_reg, 0xffffff00, 0x00000001);
			}
		}

		if(nstvalue & ATVNONSTANDFLAG_3){
		}

		if(nstvalue & ATVNONSTANDFLAG_4){
		}

		if(nstvalue & ATVNONSTANDFLAG_5){
		}

		if(nstvalue & ATVNONSTANDFLAG_6){
		}

		if(nstvalue & ATVNONSTANDFLAG_7){
		}

		if(nstvalue & ATVNONSTANDFLAG_8){
		...
		if(nstvalue & ATVNONSTANDFLAG_9){
		}
	}
	else if(ucInputSrc == _SRC_CVBS){
		if(nstvalue & AVNONSTANDFLAG_0)//flash white line
		{
			vdpq_frame_buf_chk3d_ctrl1_RBUS frame_buf_chk3d_ctrl1_reg;
			vdpq_frame_buf_chk3d_ctrl2_RBUS frame_buf_chk3d_ctrl2_reg;
			vdpq_yc_sep_mode_ctrl_RBUS yc_sep_mode_ctrl_reg;
		...
		if(nstvalue & AVNONSTANDFLAG_1)//color standard detect error as secam
		{
			vdtop_state_machine_fix_mode_RBUS state_machine_fix_mode_reg;
			state_machine_fix_mode_reg.regValue = vdcf_inl(VDTOP_STATE_MACHINE_FIX_MODE_reg);
		...
		if(nstvalue & AVNONSTANDFLAG_2)//flash abnormal color
		{
			drvif_module_vdc_Set_Cstate(VDC_NOT_FIX_CSTATE2);
		}

		if(nstvalue & AVNONSTANDFLAG_3){
		}

		if(nstvalue & AVNONSTANDFLAG_4)//flash abnormal color
		{
			drvif_module_vdc_Set_Cstate(VDC_NOT_FIX_CSTATE3);
		}

		if(nstvalue & AVNONSTANDFLAG_5)//flash abnormal color
		{
			drvif_module_vdc_Disable_Auto_Burst_position();
		}

		if(nstvalue & AVNONSTANDFLAG_6){
		}

		if(nstvalue & AVNONSTANDFLAG_7){
		}

		if(nstvalue & AVNONSTANDFLAG_8){
		}

		if(nstvalue & AVNONSTANDFLAG_9){
		}
	}
...
#if VDC_Error_case10
		/* Vsync 525 change to 625, VD can't detect correct 625flag and detect PAL60 mode, RL*/
		if ((vdcf_inl(VDTOP_PEAK_VSAG_reg) & 0x00001FFF) > 0x100 && drvif_video_status_reg(VDC_Lines_625_detected) == 0) {
			drvif_module_vdc_vline_625_src_set(_ENABLE);
			rtd_pr_vdc_emerg("weak_signal case10 Vsync issue, and 625flag old V change to new V detection\n");
		}else{
			drvif_module_vdc_vline_625_src_set(_DISABLE);
		}
		...
		if (vdc_ucNoSignalCounter == 0) {
            ...
            //rtd_pr_vdc_info("[VD_CheckInputStatus]VD status No_Signal = 0x%x \n", (vdcf_inl(VDTOP_VIDEO_STATUS_REG1_reg)&&0x1));
#ifdef VD_SHGain_SONY   //H, V, C unlocked, S/H gain set to 1.0x, DC restore offset(0.4V) enable
		#if 0
        adc_vdadc_core_lpf_ctrl_reg.regValue = vdcf_inl(ADC_VDADC_CORE_LPF_CTRL_reg);   //02_0054
        adc_vdadc_core_lpf_ctrl_reg.vdadc_adc_ctrl_0 = 0;                               //<bit16> VDADC_ADC_CTRL[0] = 0(S/H gain=1.0x)
        vdcf_outl(ADC_VDADC_CORE_LPF_CTRL_reg, adc_vdadc_core_lpf_ctrl_reg.regValue);

        adc_vd_adc_clamp_ctl_1_reg.regValue = vdcf_inl(ADC_VD_ADC_CLAMP_CTL_1_reg);     //02_0048
        adc_vd_adc_clamp_ctl_1_reg.vdadc_clamp_ctrl1_15 = 0x1;                          //<bit15> VDADC_CLAMP_CTRL1[15] = 1(Ensable dc_restore offset_0.4V)
        vdcf_outl(ADC_VD_ADC_CLAMP_CTL_1_reg, adc_vd_adc_clamp_ctl_1_reg.regValue);
		#endif
#endif
...
#ifdef VDC_SyncReduction_test
		if (ucInputSrc == _SRC_CVBS) {
			module_vdc_adcNegativeAdjustment(ucAdcNeg_Sing); //KTASKWBS-12484
		}
...
//			}
#endif
	}else if (vdc_LockStatus & (!ucCheckStart))  vdc_ucLockCounter = 0x0;  //fix fluke<30db mode dectet and no color, RL
	/* ========================================================================= */
...
bool drvif_module_vdc_OutputStable(void)
{
    ...
    if ((drvif_module_vdc_ReadMode(VDC_DETECT) == VDC_MODE_SECAM)
        ...
        ||(drvif_module_vdc_ReadMode(VDC_DETECT) == VDC_MODE_NTSC443)
        ||(VDTOP_VIDEO_STATUS_REG3_get_vcr(vdcf_inl(VDTOP_VIDEO_STATUS_REG3_reg)))){

            uc3dNoSupport = 1;
    }

    if (((drvif_module_vpq_GetYcSep() & 0xF) == VDC_YCSEPARATE_3D) && (uc3dNoSupport == 0)){

        if ((drvif_module_vdc_Read_Vstate() == 3) && (avd_drv_vdpq_dma_3d_ready() == 1) && (g_uiBuffer_Ready == 1)){
			//M6PRTANPM-765 improve chroma lock speed ryanliao 20190226 [start]
			if(g_ucSrcType == _SRC_CVBS)
			{
				module_vdc_Set_lose_chromalock_level(0x05); /* set to default ref jj*/
			}else{ //TV
				module_vdc_Set_lose_chromalock_level(0x04);
			}
			//[end]
            VDC_DEBUG_MSG(VDC_MSG_TVCHCH, "[%s(%d)][3D check] VD ready !! Vstate=3\n",__func__,__LINE__);
            return TRUE;
        }else{

            VDC_DEBUG_MSG(VDC_MSG_TVCHCH, "[%s(%d)][3D check] ucVstate=%d,avd_drv_vdpq_dma_3d_ready=%d,g_uiBuffer_Ready=%d\n",__func__,__LINE__,drvif_module_vdc_Read_Vstate(),avd_drv_vdpq_dma_3d_ready(),g_uiBuffer_Ready);
            return FALSE;
        }
    }else{

        if (drvif_module_vdc_Read_Vstate() == 3 && g_uiBuffer_Ready == 1){

            VDC_DEBUG_MSG(VDC_MSG_TVCHCH, "[%s(%d)] VD ready !! Vstate=3 && g_uiBuffer_Ready=1\n",__func__,__LINE__);
			//M6PRTANPM-765 improve chroma lock speed ryanliao 20190226 [start]
			if(g_ucSrcType == _SRC_CVBS)
			{
				module_vdc_Set_lose_chromalock_level(0x05); /* set to default ref jj*/
            ...
            VDC_DEBUG_MSG(VDC_MSG_TVCHCH, "[%s(%d)] ucVstate=%d,g_uiBuffer_Ready=%d\n",__func__,__LINE__,drvif_module_vdc_Read_Vstate(),g_uiBuffer_Ready);
            return FALSE;
        }
    }
}
...
void drvif_module_vdc_set_ColorMode(unsigned char mode)
 {
       //rtd_pr_vdc_info("[lish av]drvif_module_vdc_set_ColorMode: mode=%d \n", mode);
       rtd_pr_vdc_emerg("0...[cstate][AV]drvif_module_vdc_set_ColorMode: mode=%d \n", mode);
       ...
       if (mode == VDC_MODE_NTSC) {
               ...
               drvif_module_vdc_Cstate_fast_locked(VDC_CSTATE_FastLock_DISABLE);
               drvif_module_vdc_Set_Cstate(VDC_CSTATE_RESET);
			   //drvif_module_vdc_27M_FrontDigitalFilterEnable(VDC_27M_FDF_55MHZ);
       }else if (mode == VDC_MODE_NTSC443) { //fixed_cstate, display flash, add c state speed, chroma lock speed don't enough
               drvif_video_set_control_reg(VDC_set_Colour_mode_NTSC443);
               drvif_module_vdc_set_Basic_Register(VDC_colourmodereg_enable);
               drvif_module_vdc_Cstate_fast_locked(VDC_CSTATE_FastLock_ENABLE);
               drvif_module_vdc_Set_Cstate(VDC_NOT_FIX_CSTATE1);
			  // drvif_module_vdc_27M_FrontDigitalFilterEnable(VDC_27M_FDF_55MHZ);
       }else if (mode == VDC_MODE_PAL60) {  //fixed_cstate, display flash, add c state speed, chroma lock speed don't enough
               drvif_video_set_control_reg(VDC_set_Colour_mode_PAL60);
               drvif_module_vdc_set_Basic_Register(VDC_colourmodereg_enable);
               drvif_module_vdc_Cstate_fast_locked(VDC_CSTATE_FastLock_ENABLE);
               drvif_module_vdc_Set_Cstate(VDC_NOT_FIX_CSTATE1);
       }else if (mode == VDC_MODE_PALM) {
               drvif_video_set_control_reg(VDC_set_Colour_mode_NTSC);
               drvif_module_vdc_set_Basic_Register(VDC_colourmodereg_disable);
               drvif_module_vdc_Cstate_fast_locked(VDC_CSTATE_FastLock_ENABLE);
               drvif_module_vdc_Set_Cstate(VDC_NOT_FIX_CSTATE1);
...
//	unsigned char ucFreeRunStatus = drvif_module_vdc_read_hv_freerun_sel();
	...
	if (result == VDC_FREERUN_DISABLE) {
		if (drvif_module_vdc_read_hv_freerun_sel() != _DISABLE) {
			...
			module_vdc_BlankLevelAdjustment(0x3c0);//change solution for WOSQRTK-8849
		}
	} else {
		if (drvif_module_vdc_read_hv_freerun_sel() != _ENABLE) {
			...
			module_vdc_BlankLevelAdjustment(0x0);//change solution for WOSQRTK-8849
...
{
	/*rtd_pr_vdc_debug("[vdc]drvif_module_vdc_FixCKillMode a_ucFixed=%x\n", a_ucFixed);*/
...
unsigned char module_video_check_Hsync_power_2(unsigned char mode)
{
       /* 956c[31:24][23:16] */
       unsigned char uc443power, uc358power;
       unsigned char i = 0;
       ...
       if (mode == VDC_MODE_NTSC) {
               ...
               VDC_CpeakSet = vd_cpeak_disable;
               //rtd_pr_vdc_emerg("[cpeak_2] [mode == VDC_MODE_NTSC] NTSC 443_p=%x, 358_p=%x, c(%d) n(%d) ", uc443power, uc358power, color_count, normal_count);
       } else if (mode == VDC_MODE_PALI) {

               for (i = 0; i < 3; i++) {
       
                        if ((uc358power < 0x50) && (uc443power < 0xf0) && (drvif_module_vdc_read_Cgain_data() >0x3ef0)){
                               color_count++;
                               //Rt_Delay(1);
                               //rtd_pr_vdc_emerg("[cpeak_1] [mode == VDC_MODE_PALI] PAL 443_p=%x, 358_p=%x, c(%d) n(%d) %x", uc443power, uc358power, color_count, normal_count, drvif_module_vdc_read_Cgain_data());

                       }else if ((uc358power < 0x50) && (uc443power < 0xf0)){
                               color_count++;
                               //rtd_pr_vdc_emerg("[cpeak_2] [mode == VDC_MODE_PALI] PAL 443_p=%x, 358_p=%x, c(%d) n(%d) %x", uc443power, uc358power, color_count, normal_count, drvif_module_vdc_read_Cgain_data());
                               //VDC_CpeakSet = vd_cpeak_enable;
                               break;
               ...
               //rtd_pr_vdc_emerg("[cpeak_3] [mode == VDC_MODE_PALI] PAL 443_p=%x, 358_p=%x, c(%d) n(%d) %x", uc443power, uc358power, color_count, normal_count, drvif_module_vdc_read_Cgain_data());
       }

               if (color_count > 2) {
                       VDC_CpeakSet = vd_cpeak_enable;
                       color_count = 0;
                       //rtd_pr_vdc_emerg("[cpeak] VDC_CpeakSet :%x  %x", VDC_CpeakSet, vdc_LoopCount);
               }
...
void drvif_module_vdc_Cpeak_control(unsigned char ucInputSrc)
{
	...
	drvif_module_vdc_Cpeak_set(_ENABLE);
       } else if (cpeak_check_flag ==1){// av source, hisense set-topbox display abnormal, gain data overflow
       	//drvif_module_vdc_Cpeak_set(_ENABLE); 
       	// cpeak enable need time to reduce overflow of cgain data, so change to use cagc_enable bit
...
#else // Color distortion when burst level reduced to 20%, Set the chroma peak enable when the burst level too low.
	//unsigned char average_burst;
	unsigned char vd_mode;
...
unsigned int drvif_module_vdc_read_hstate_vsync_error_unbalance_check(void)
{
	// 9134[25]
	vdtop_h_loop_maxstate_RBUS vdtop_h_loop_maxstate_reg;
	vdtop_h_loop_maxstate_reg.regValue =  vdcf_inl(VDTOP_H_LOOP_MAXSTATE_reg);

	g_hstate_vsync_error_unbalance_status_cnt_check++;
	if (g_hstate_vsync_error_unbalance_status_cnt_check > 8) {
		g_hstate_vsync_error_unbalance_status_cnt_check = 8;
		return g_hstate_vsync_error_unbalance_status_result_check;
	}
	...
	vdtop_h_loop_maxstate_reg.regValue =  vdcf_inl(VDTOP_H_LOOP_MAXSTATE_reg);
	g_hstate_vsync_error_unbalance_status_result_check = vdtop_h_loop_maxstate_reg.hstate_vsync_error_unbalance_status;
...
unsigned char dvrif_module_vdc_get_frequency_bias( unsigned int frm_num )
{
	...
	while( counter < 128 && stopUpdateFrequencyOffset == 0)
	{
		...
		if( timeout <= 0 )
		{
			rtd_pr_vdc_info("[scalerVideo] %s TIMEOUT\n", __func__ );
			break;
		}
	}
	//rtd_pr_vdc_info("[scalerVideo] timeout left = %d\n", timeout );
	...
	if( frm_num == 7 )
	{
		...
		//unsigned int temp = IoReg_Read32(PLL_REG_THERMAL_RESULT_1_reg);
...
#ifdef CONFIG_ENABLE_VDC_BIG_SIGNAL_CHECK
void  drvif_module_vdc_big_signal_detect_function(bool enable)
{
           UINT8 i=0;
           vdtop_adc_over_under_flag_RBUS vdtop_adc_over_under_flag_reg;
           vdtop_agc_peak_nominal_RBUS  vdtop_agc_peak_nominal_reg;
           vdtop_noise_status_RBUS vdtop_noise_status_reg;
           vdtop_agc_peak_ctrl_RBUS    vdtop_agc_peak_ctr_reg;

            vdtop_adc_over_under_flag_reg.adc_value_max_thr=0x300;
            vdcf_outl(VDTOP_ADC_OVER_UNDER_FLAG_reg, vdtop_adc_over_under_flag_reg.regValue);
            vdtop_noise_status_reg.regValue= vdcf_inl(VDTOP_NOISE_STATUS_reg);


  if(enable==1)
    {
  if(vdtop_noise_status_reg.status_noise<=0x0a)
    {
                   rtd_pr_vdc_info("[VDC] big signal function \n ");
                  count_big_signal_fail=0;
                  count_big_signal_ok=0;
                if(per_Peak_data_sel_value==1)
                        {

                             for (i = 0; i < 4; ++i) {
                                vdtop_adc_over_under_flag_reg.ad4_value_big=1;
                                vdcf_outl(VDTOP_ADC_OVER_UNDER_FLAG_reg, vdtop_adc_over_under_flag_reg.regValue);
                                Rt_Delay(1);
                                vdtop_adc_over_under_flag_reg.regValue= vdcf_inl(VDTOP_ADC_OVER_UNDER_FLAG_reg);

                                if(vdtop_adc_over_under_flag_reg.ad4_value_big==0)
                                        {
                                                count_big_signal_fail=count_big_signal_fail+1;
                                                rtd_pr_vdc_info("[VDC] big signal function  ,count_big_signal_fail=%d\n ",count_big_signal_fail);

                                        }
                                }

                                    if(count_big_signal_fail>2)
                                        {
                                                vdtop_agc_peak_nominal_reg.peak_data_sel=0;
                                                vdcf_outl(VDTOP_AGC_PEAK_NOMINAL_reg, vdtop_agc_peak_nominal_reg.regValue);
                                                per_Peak_data_sel_value=0;

                                                 VDC_DEBUG_MSG(VDC_MSG_DEBUG1, "[VDC] big signal is not coming ,change the normal signal");
                                              rtd_pr_vdc_info("[VDC] big signal is not coming ,change the normal signal \n");
                                                count_big_signal_fail=0;
                ...
                else
                        {
                                ...
                                vdcf_outl(VDTOP_ADC_OVER_UNDER_FLAG_reg, vdtop_adc_over_under_flag_reg.regValue);
   															Rt_Delay(1);
                               vdtop_adc_over_under_flag_reg.regValue= vdcf_inl(VDTOP_ADC_OVER_UNDER_FLAG_reg);
                                if(vdtop_adc_over_under_flag_reg.ad4_value_big==1)
                                        {

                                                                        count_big_signal_ok=count_big_signal_ok+1;
																						   rtd_pr_vdc_info("[VDC] big signal function  ,count_big_signal_ok=%d\n ",count_big_signal_ok);


                                                        }

                                        }
                                         if(count_big_signal_ok>3)
                                        {
                                                vdtop_agc_peak_nominal_reg.peak_data_sel=1;
                                                vdcf_outl(VDTOP_AGC_PEAK_NOMINAL_reg, vdtop_agc_peak_nominal_reg.regValue);
                                                per_Peak_data_sel_value=1;
                                                 VDC_DEBUG_MSG(VDC_MSG_DEBUG1, "[VDC] big signal  is coming");
                                                rtd_pr_vdc_info("[VDC] big signal  is coming \n");
                                                 count_big_signal_ok=0;
...
void drvif_module_DMA_CHK3D_ENABLE_control(VDC_DMA_CHK3D_ENABLE_MODE value)
{
	...
	if(value == VDC_CHANEL_DISPLAY_MODE) {
		...
		DMA_CHK3D_TABLE.open_3d_cnt = 0x9;
...
void module_vdc_Hactive_Start(Hactive_Mode a_ucHactiveMode,unsigned int a_uiHactiveStart)
{
	...
	{
		rtd_pr_vdc_info("a_uiHactiveStart > 0xff\n");
...
unsigned char module_vdc_FhAdjust(void)
{
	...
	if(!Temp_diff)
	{
		rtd_pr_vdc_debug("[Fh] h/v unlock return : %x\n",Temp_diff);
		...
		||(VD_mode == VDC_MODE_PALM)||(VD_mode == VDC_MODE_PALN)))
	{
		...
		{
			ucFhBug_Counter++;
//			rtd_pr_vdc_info("[1]ucFhBug_Counter: %x\n", ucFhBug_Counter);
			if ( ucFhBug_Counter >= 3 )	// 10
			{
				ucFhBug_Counter = 2;	// 8
				if(cvideo_fh_set_1d != ON)
				{
					rtd_pr_vdc_debug("Fh 1D 1D 1D 1D 1D .....!\n");
...
//					drvif_module_vdc_set_Control2_Register(VDC_dc_clamp_mode_synctip);
		...
		{
			ucFhBug_Counter--;
//			rtd_pr_vdc_info("[2]ucFhBug_Counter: %x\n", ucFhBug_Counter);
			if ( ucFhBug_Counter == 0 )
			{
				ucFhBug_Counter = 1;	// 3
				if(cvideo_fh_set_1d != OFF)
				{
					rtd_pr_vdc_debug("Fh 3D~~~~~\n");
					drvif_module_vdc_Swallow_on_Vsyng(_DISABLE);
...
void module_vdc_Set_bgate_Setting(bool enable)
{
	vdtop_charge_pump_adjustment_RBUS charge_pump_adjustment_reg;
	...
	if (enable == 1)
		{
			...
			charge_pump_adjustment_reg.regValue = vdcf_inl(VDTOP_CHARGE_PUMP_ADJUSTMENT_reg);
			charge_pump_adjustment_reg.bgate_raw_length = 0x78;	/* 90c8[15:8] = 0x78 */
			vdcf_outl(VDTOP_CHARGE_PUMP_ADJUSTMENT_reg, charge_pump_adjustment_reg.regValue);
		}
	else{
			//clamping pulse
			drvif_module_vdc_set_Control2_Register(VDC_bgate_raw_sel_disable);
			charge_pump_adjustment_reg.regValue = vdcf_inl(VDTOP_CHARGE_PUMP_ADJUSTMENT_reg);
			charge_pump_adjustment_reg.bgate_raw_length = 0x0c;	/* 90c8[15:8] = 0x0c */
			vdcf_outl(VDTOP_CHARGE_PUMP_ADJUSTMENT_reg, charge_pump_adjustment_reg.regValue);
		}
	//[End]
}
...
#ifdef CONFIG_CUSTOMER_TV010
unsigned char drvif_module_vdc_GetMVStatus(void)
{
	return vdc_MvFlag;
}
#endif

unsigned char drvif_module_vdc_Get625Flag(void)
{
	return vdc_625flag;
}

#ifdef CONFIG_SUPPORT_SCALER_MODULE
int __init video_module_init(void)
#else
...
#ifdef CONFIG_SUPPORT_SCALER_MODULE
void __exit video_module_exit(void)
#else
...
void drvif_module_vdc_ADC_Vrefp_Setting(VDC_AVD_AV_COEF AvConfig)
{
	AD_vdadc_adc_ctrl_9_8_Setting(AvConfig);
}

void drvif_module_vdc_FDF_Table_Select(VDC_AVD_AV_COEF AvConfig)
{
	FrontDigitalFilter_Table_Setting(AvConfig);
}
