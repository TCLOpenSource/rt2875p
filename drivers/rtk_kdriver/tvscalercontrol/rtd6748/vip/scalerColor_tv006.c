...
unsigned char OETF_set_by_user=0;
extern unsigned int pre_sourceMaxL_a;
extern unsigned char Need_recheck_hdr_type;
extern BRIGHTNESS_LUT_OETF BRIGHTNESS_LUT_T;
extern unsigned int EOTF_LUT_2084_nits[1025];
#if 0
...
void fwif_color_Get_sharpness_table_TV006(short *xTmp)
{
	...
	{
		DRV_Sharpness_Level sharp_gain = {0};
		DRV_Sharpness_Other_Ctrl sharp_other_ctrl = {0};
	...
	{
		DRV_VipNewIDcti_Table i_dcti = {0};
		DRV_VipNewDDcti_Table d_dcti = {0};
		VIP_D_vcti_t v_cti = {0};
...
UINT8 fwif_color_DM2_EOTF_OETF_3D_LUT_by_HFC(UINT8 HFC_bit, HDR_flow_control_timing HDR_timing)
{
	...
	printk("DM_OETF_DMA, set by HFC, HFC_bit_oetf=%d,ret_val=%d,ctrl_bit=%d,tbl_idx=%d,\n",
	...
	if(Need_recheck_hdr_type){
		if(VIP_system_info_structure_table != NULL){
			if(VIP_system_info_structure_table->HDR_flag == HAL_VPQ_HDR_MODE_HDR10){
				...
				printk("[HDR10] Need_recheck_hdr_type done \n");
			}
		}
		Need_recheck_hdr_type = 0;
	}
	if(VIP_system_info_structure_table->HDR_flag == HAL_VPQ_HDR_MODE_HDR10)
		drvif_color_Set_ST2094_3Dlut_CInv(1024<<5, 0);
	else
...
void fwif_color_Set_De_Contour_tv006(RTK_DECONTOUR_T *level)
{
        ...
        DRV_LC_decontour ptr_LC_de = {0};
        VIP_I_De_Contour_TBL ptr_i_de = {0};
