...
#undef VIPprintf
#define VIPprintf(fmt, args...)	VIPprintfPrivate(VIP_LOGLEVEL_DEBUG,VIP_LOGMODULE_VIP_PQA_DEBUG,fmt,##args)
...
void drvif_color_PQ_Adaptive_offset_temp(unsigned int *PQA_TABLE, unsigned char src, unsigned char index, unsigned int *PQA_TABLE_OFFSET_TEMP)
{
	...
	if (!PQA_TABLE) {
		...
		VIPprintfPrivate(VIP_LOGLEVEL_ERR, VIP_LOGMODULE_VIP_PQA_interface1_DEBUG, "PQ data ERROR, PQA_TABLE is NULL!!\n");
		return;
	}
	...
	for (PQA_ITEM_count = 0; PQA_ITEM_count < PQA_ITEM_MAX; PQA_ITEM_count++) {
		...
		if (index == PQA_Input_type && mask_idx < 32) {
			PQA_TABLE_OFFSET_TEMP[PQA_ITEM_count] = (IoReg_Read32(PQA_Reg) >> PQA_Bitlow) & andMASK_Table2[(PQA_Bitup-PQA_Bitlow+1)];	/*data*/
		}
		//VIPprintf( "PQ_Adaptive PQA_Reg %x PQA_Bitup %d PQA_Bitlow %d %d\n", PQA_Reg, PQA_Bitup, PQA_Bitlow, (IoReg_Read32(PQA_Reg) >> PQA_Bitlow) & andMASK_Table2[(PQA_Bitup-PQA_Bitlow+1)]);
		VIPprintfPrivate(VIP_LOGLEVEL_INFO, VIP_LOGMODULE_VIP_PQA_interface1_DEBUG, "PQ_Adaptive PQA_Reg %x PQA_Bitup %d PQA_Bitlow %d %d\n", PQA_Reg, PQA_Bitup, PQA_Bitlow, (IoReg_Read32(PQA_Reg) >> PQA_Bitlow) & andMASK_Table2[(PQA_Bitup-PQA_Bitlow+1)]);
	}
