...
void drvif_color_set_LD_BL_Profile_Interpolation_Table_HV(unsigned int *Hptr, unsigned int *Vptr)
{
	...
	blu_ld_inter_hctrl1.regValue = IoReg_Read32(BLU_LD_Inter_HCtrl1_reg);
	blu_ld_inter_vctrl1.regValue = IoReg_Read32(BLU_LD_Inter_VCtrl1_reg);
	tab_hsize = blu_ld_inter_hctrl1.ld_tab_hsize+1;
	tab_vsize = blu_ld_inter_vctrl1.ld_tab_vsize+1;

	if (tab_hsize > (BL_Profile_Table_COLUMN/5))
		tab_hsize = BL_Profile_Table_COLUMN/5;

	if (tab_vsize > (BL_Profile_Table_COLUMN/5))
		tab_vsize = BL_Profile_Table_COLUMN/5;
...
char drvif_color_Set_Local_Dimming_Object_Gain_DMA_Enable(unsigned char Enable, unsigned int phy_Addr, unsigned short* vir_Addr, unsigned short H_Num, unsigned short V_Num)
{	
	blu_ld_global_ctrl0_RBUS blu_ld_global_ctrl0;
	...
	if (phy_Addr == 0 || vir_Addr == NULL) {
		rtd_pr_vpq_info("drvif_color_Set_Local_Dimming_Object_Gain_DMA_Enable NUL\n");
		return -1;
	}
	...
	blu_ld_rd_num_bl_wrap_ctl_reg.ld_line_num = V_Num;	// + 1;
...
void drvif_color_set_LDSPI_SRAM_HW_WRITE_done_IntEn(unsigned char enable, unsigned short hw_write_num)
{
	ldspi_rx_data_ctrl_RBUS ldspi_rx_data_ctrl;
	ldspi_ld_spi_sram_hw_write_done_RBUS ldspi_ld_spi_sram_hw_write_done;
	ldspi_rx_data_ctrl.regValue = IoReg_Read32(LDSPI_RX_data_CTRL_reg);
	ldspi_ld_spi_sram_hw_write_done.regValue = IoReg_Read32(LDSPI_ld_spi_sram_hw_write_done_reg);

	ldspi_rx_data_ctrl.sram_hw_write_done_int_en = enable;
	ldspi_rx_data_ctrl.ld_spi_int2_en = 1;
	ldspi_ld_spi_sram_hw_write_done.sram_hw_write_num_set = hw_write_num;
	
	IoReg_Write32(LDSPI_ld_spi_sram_hw_write_done_reg, ldspi_ld_spi_sram_hw_write_done.regValue);
	IoReg_Write32(LDSPI_RX_data_CTRL_reg, ldspi_rx_data_ctrl.regValue);
	//printk("sram_hw_write_done_int_en = %d hw_write_num=%d\n",enable,hw_write_num);
}
...
char drvif_color_get_LD_AlgoSRAM(unsigned int length, unsigned short *BLtable) 
{
	...
	if(BLtable == NULL) {
		//printk("BLtable is NULL\n");
		ret = 0;
		return ret;
	}


	//rtd_printk(KERN_EMERG, TAG_NAME, "drvif_color_set_LD_AlgoSRAM");
	blu_ld_sram_ctrl.regValue = IoReg_Read32(BLU_LD_SRAM_CTRL_reg);
	...
	if (blu_ld_sram_ctrl.ld_blight_sw_mode == 1) {/*SW Mode enabled*/
		...
		IoReg_Write32(BLU_LD_BLIGHT_AP_reg, blu_ld_blight_ap.regValue);
		//dummy = IoReg_Read32(BLU_LD_BLIGHT_DP_reg);
		
		if (ld_blk_type == 1 || ld_blk_type==2) {/*edge type*/
			if (ld_blk_vnum == 0 || ld_blk_hnum == 0) {/*1 sided*/
				for(i=0;i<length;i++) {
					blu_ld_blight_dp.regValue = IoReg_Read32(BLU_LD_BLIGHT_DP_reg);
					//rtd_printk(KERN_EMERG, TAG_NAME, "A BLValue[%d] = %d\n", i, ldspi_data_rwport.data_rw_data_port);
			...
			//printk("Get LD AlgoSRAM is not correct, id type = %d\n", ld_blk_type);
		
		}
	} else {
		//printk("Please enable SWMode before writing the algo SRAM\n");
	}
...
#if 1
void drvif_color_LD_Write_DataSRAM_Continuous(unsigned short SRAM_Position, unsigned short length, unsigned short* SRAM_Data)
{
	 unsigned short i;
	 ...
	 ldspi_data_addr_ctrl.regValue = rtd_inl(LDSPI_Data_Addr_CTRL_reg);
	 ldspi_data_addr_ctrl.data_rw_pointer_set = SRAM_Position;
	 rtd_outl(LDSPI_Data_Addr_CTRL_reg, ldspi_data_addr_ctrl.regValue); /*Select SRAM Position*/
	 for (i=0;i<length;i++) {
	  ldspi_data_rwport.data_rw_data_port = *(SRAM_Data+i);
	  rtd_outl(LDSPI_Data_Rwport_reg, ldspi_data_rwport.regValue); /*SRAM Data*/
	 }
}
#endif
