...
const char PQMaskBlockModuleName[PQMASK_BLKMOD_NUM][PQMASK_STRING_LEN] = {
	/* 0 */ "SPNR",
	/* 1 */ "SHARP",
	/* 2 */ "ICM",
	/* 3 */ "LC",
...
static int PQMaskDumpData2File(char* tmpname, unsigned int * vir_y, unsigned int * vir_c, unsigned int y_size, unsigned int c_size)
{
	...
	if( rtk_get_usb_path(usbPath, 256) != 0) {
		...
		if( sprintf(filename, "%s/%s.raw", "/tmp/usb/sda/sda1", tmpname) < 0 ) {
			rtd_printk(KERN_ERR, TAG_NAME, "[%s] construct file name error\n", __FUNCTION__);
			return 0;
		}
	} else {
...
static void scalerPQMask_UpdateOrbitInfo(void) {
	...
	if( PQMaskGlbMgr.Status.OrbitStatus.bIsOled != bIsOledPanel || PQMaskGlbMgr.Status.OrbitStatus.bOrbitEn != bOribitEnable ||
		...
		rtd_printk(KERN_EMERG, TAG_NAME, "[%s] display = %d, isOlde = %d, orbitEn = %d\n", __FUNCTION__, Display, bIsOledPanel, bOribitEnable);
		rtd_printk(KERN_EMERG, TAG_NAME, "shift offset/range = %2d/%4d, %2d/%4d\n",
...
static inline void scalerPQMask_Memset(void *VirtAddr, void *PhysAddr, unsigned char Val, unsigned int SetSize) {
	...
	// error checking
	if( VirtAddr == 0 ) {
		rtd_printk(KERN_EMERG, TAG_NAME, "[%s] error address\n", __FUNCTION__);
		return;
	}
...
static void scalerPQMask_SETask(struct work_struct *work) {
	...
	if( PQMaskGlbMgr.Status.bAllowExec == 0 ) {
		...
		if( bPreAllowExec != PQMaskGlbMgr.Status.bAllowExec ) {
			...
			rtd_printk(KERN_EMERG, TAG_NAME, "change from allow to dis-allow pqmask\n");
		}
...
void scalerPQMask_CheckSystem(void) {
	...
	if( preAllowExec!=PQMaskGlbMgr.Status.bAllowExec ) {
		rtd_printk(KERN_INFO, TAG_NAME, "[%s] secure = %d\n", __FUNCTION__, isSecurePath);
		rtd_printk(KERN_INFO, TAG_NAME, "[%s] update to %d\n", __FUNCTION__, PQMaskGlbMgr.Status.bAllowExec);
	}

	// rtd_printk(KERN_INFO, TAG_NAME, "[%s] allow = %d end\n", __FUNCTION__, PQMaskGlbMgr.Status.bAllowExec);
		...
		if( PQMaskGlbMgr.Status.ErrRecoverTimes > 0 ) {
			...
			if( RecoverStatus != RET_OK ) {
				...
				if( PQMaskGlbMgr.Status.PQMaskStatus == PQMASK_STAT_ERR ) {
					...
					rtd_printk(KERN_EMERG, TAG_NAME, "[%s] try to change status to %s at status %s, recover flow interrupted!\n",
...
void scalerPQMask_disableModuleDMA(void) {
	...
	drvif_color_set_pqmask_local_contrast_enable(0);
	drvif_module_set_pqmask_idomain_db_apply();
	drvif_module_set_pqmask_ddomain_db_apply();
	drvif_module_wait_pqmask_idomain_db_apply_done(40);
	drvif_module_wait_pqmask_ddomain_db_apply_done(40);
...
void scalerPQMask_SetModuleAddr(unsigned char ModuleSel, unsigned long TargetAddr) {
	...
	switch(ModuleSel) {
		...
		case PQMASK_MODULE_INT:
			...
			break;
		case PQMASK_MODULE_LC:
			drvif_color_set_pqmask_local_contrast_Addr(TargetAddr>>4);
			break;
		default:
...
void scalerPQMask_SetBlkModuleDmaConfig(unsigned char BlkModuleSel) {
	...
	ModuleDmaCfg.LineStep = PQMASK_PROC_WIDTH>>4; // LineAddrOffset (align 16Byte)
	...
	switch(BlkModuleSel) {
		...
		case PQMASK_BLKMOD_LOCALCONTRAST:
			...
			drvif_color_set_pqmask_ddomain_DMA_config(&DmaCfg);
			drvif_color_set_pqmask_ddomain_DMA_enable(1);
			drvif_color_set_pqmask_local_contrast_DMA_config(&ModuleDmaCfg);
			if( pPQMaskColorMgr->ModuleSetting.LocalContrast.bSharpEnable != 0 || pPQMaskColorMgr->ModuleSetting.LocalContrast.bToneMappingEnable != 0 ) {
				drvif_color_set_pqmask_local_contrast_enable(1);
			} else {
				drvif_color_set_pqmask_local_contrast_enable(0);
			}
			break;
...
void scalerPQMask_SetBlkModuleIoSize(unsigned char BlkModuleSel) {
	...
	switch(BlkModuleSel) {
		...
		case PQMASK_BLKMOD_ICM:
			...
			break;
		case PQMASK_BLKMOD_LOCALCONTRAST:
			drvif_color_set_pqmask_local_contrast_size(&PQMaskBlkModuleCfg[BlkModuleSel].IoSize);
			drvif_color_set_pqmask_local_contrast_scaling(&PQMaskBlkModuleCfg[BlkModuleSel].Scale);
			drvif_color_set_pqmask_local_contrast_scaling_enable(1, 1);
			break;
		default:
			rtd_printk(KERN_ERR, TAG_NAME, "[%s] index %d overflow\n", __FUNCTION__, BlkModuleSel);
			break;
...
void scalerPQMask_GetDMALimit(unsigned char domain, PQMASK_DMA_CONFIG_T *DmaCfg) {
	...
	if( domain == PQMASK_DOMAIN_I ) {
		...
		for(i=PQMASK_MODULE_EDGE; i<=PQMASK_MODULE_LC; i++) {
			for(j=0; j<PQMASK_MODULE_DMA_NUM; j++) {
				if( DmaCfg->AddrUpLimit <= PQMaskBufMgr.ModuleDma[i][j].AddrInfo.PhysAddr) {
					DmaCfg->AddrUpLimit = PQMaskBufMgr.ModuleDma[i][j].AddrInfo.PhysAddr;
				}
...
int scalerPQMask_CheckResolution(void) {
	...
	PQMASK_BLK_MODULE_CONFIG_T CurrentModuleCfg[PQMASK_BLKMOD_NUM];
	memcpy(CurrentModuleCfg, PQMaskBlkModuleCfg, sizeof(PQMASK_BLK_MODULE_CONFIG_T)*PQMASK_BLKMOD_NUM);

	scalerPQMask_UpdateOrbitInfo();
	scalerPQMask_UpdateModuleResolution();

	if( memcmp(CurrentModuleCfg, PQMaskBlkModuleCfg, sizeof(PQMASK_BLK_MODULE_CONFIG_T)*PQMASK_BLKMOD_NUM) != 0 ) {
		rtd_printk(KERN_EMERG, TAG_NAME, "[%s] module size setting change\n", __FUNCTION__);
		Ret = RET_FAIL;
	}
