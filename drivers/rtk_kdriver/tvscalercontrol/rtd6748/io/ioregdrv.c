...
unsigned int IoReg_Read32 (unsigned int addr) {
	...
	if (Scaler_I2rnd_get_timing_enable()) {
		//if(Scaler_I2rnd_get_force_cmd() && (Scaler_I2rnd_get_display() == I2RND_MAIN_S0_TABLE))
		...
		if(curr_addr){
			...
			unsigned int curr_md_addr;
			if (Scaler_I2rnd_get_timing_enable()) {
				if((Scaler_I2rnd_get_display() == I2RND_MAIN_S0_TABLE) && Scaler_main_md_pst_get_enable()){
					curr_md_addr = Scaler_PST_IP_register_get_cur_read_addr(addr,I2RND_MAIN_S0_TABLE);
					if(curr_md_addr){
						curr_md_addr += 4;
						value = *(volatile unsigned int *)(curr_md_addr);
						curr_md_addr-=4;
						tmp_addr = *(volatile unsigned int *)(curr_md_addr);
						//printk(KERN_EMERG"i2rnd_buffer read: %x,%x, %x\n",addr, value, tmp_addr);
						...
						if((value == 0) && (tmp_addr == 0)){
							...
							curr_md_addr += 4;
							value =  __bswap_32(*(volatile unsigned int *)(curr_md_addr));
							//printk(KERN_EMERG"i2rnd_buffer read (after swap): %x,%x\n",addr,value);
							return value;
						}
					}
				...
				else if((Scaler_I2rnd_get_display() ==I2RND_SUB_S1_TABLE) && Scaler_sub_md_pst_get_enable()){
					curr_md_addr = Scaler_PST_IP_register_get_cur_read_addr(addr,I2RND_SUB_S1_TABLE);
					if(curr_md_addr){
						curr_md_addr += 4;
						value = *(volatile unsigned int *)(curr_md_addr);
						curr_md_addr-=4;
...
#endif
#ifdef CONFIG_DUMP_REGISTER
					bForceDumpFlag = (*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VGIP_VGIP_VBISLI_reg) & _BIT18);

					bForcebg = (((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(PPOVERLAY_Main_Display_Control_RSV_reg))&0x00000002)>>1);

					if(Get_dump_function()
#ifdef CONFIG_DUMP_REGISTER_WHEN_ONLY_DISCONNECT // dump only in VO_DisConnectVDO()  ~  forcebg disable
		&& ((flag_vdo_disconnect==1) && ( bForcebg ==1))
#endif
						/*&& (main_display_control_rsv_reg.m_force_bg ==0)*/
#if 1 // TEST -- [KTASKWBS-6583] disable dump when CRC error > 20
						&& ((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VGIP_VGIP_VBISLI_reg) & _BIT17) == 0)
#endif
						&&(
						...
						//rtd_pr_ioreg_emerg("[tjw]register_counter=%d\n",dump_register_info.register_counter);
...
EXPORT_SYMBOL(IoReg_Read32);

void IoReg_Write32 (unsigned int addr, unsigned int value) {
#ifdef CONFIG_I2RND_ENABLE
		unsigned int curr_addr = 0;

		if (Scaler_I2rnd_get_display() != 0) {
			//update DI write bit of control register @Crixus 20180109
			if(addr == DI_IM_DI_CONTROL_reg){
				value = scaler_i2rnd_enable_di_write_bit(value);
			}

			curr_addr = Scaler_i2rnd_IP_register_get_cur_write_addr(addr, Scaler_I2rnd_get_display());
			if (curr_addr) {
				...
				*(volatile unsigned int *)(curr_addr) = __bswap_32(tmp_addr);
				//Eric@0802 I2rnd dual vo case default write cmd buffer
				if(Scaler_I2rnd_get_force_cmd() && (Scaler_I2rnd_get_display() == I2RND_MAIN_S0_TABLE))
		...
		#ifdef CONFIG_PST_ENABLE
				unsigned int curr_md_addr;
				unsigned int tmp_md_addr;
				if((Scaler_I2rnd_get_display() == I2RND_MAIN_S0_TABLE) && Scaler_main_md_pst_get_enable()){
					curr_md_addr = Scaler_PST_IP_register_get_cur_write_addr(addr, I2RND_MAIN_S0_TABLE);
					if (curr_md_addr) {
						tmp_md_addr =((addr>>2)&0x000fffff)|0x80000000;
						...
						*(volatile unsigned int *)(curr_md_addr) = __bswap_32(tmp_md_addr);
						//Eric@20171212 pst default write register
						(*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(addr) = value);
					}  else {
						(*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(addr) = value);
					}
					...
					curr_md_addr = Scaler_PST_IP_register_get_cur_write_addr(addr, I2RND_SUB_S1_TABLE);
					if (curr_md_addr) {
						tmp_md_addr =((addr>>2)&0x000fffff)|0x80000000;
						//printk(KERN_EMERG"pst_buffer write: %x->%x,%x->%x\n",addr,value,curr_md_addr,tmp_md_addr);
						curr_md_addr +=4;
	...
	if(Get_dump_function()
		...
		dump_register_info.vgip_line_cnt=(((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VGIP_VGIP_CHN1_LC_reg))&0x7FFF0000)>>16);
		dump_register_info.uzudtg_line_cnt=(((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(PPOVERLAY_new_meas1_linecnt_real_reg))&0x1FFF0000)>>16);
		dump_register_info.memcdtg_line_cnt=((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(PPOVERLAY_new_meas1_linecnt_real_reg))&0x1FFF);
		dump_register_info.registeraddr=addr;
...
unsigned char IoReg_BusyCheckRegBit(unsigned int addr, unsigned int value, unsigned int timeoutcnt)
{
	...
	do {
		if (addr == PPOVERLAY_DTG_pending_status_2_reg) {
			if (value == _BIT0 && end_count != ddomain_isr_get_mv_den_end_count())
				return TRUE;
			else if (value == _BIT1 && start_count != ddomain_isr_get_mv_den_sta_count())
				return TRUE;
		} else if(IoReg_Read32(addr) & value) {
			return TRUE;
		}
	} while(timeoutcnt-- != 0);
...
unsigned char IoReg_BusyCheckRegInverseBit(unsigned int addr, unsigned int value, unsigned int timeoutcnt)
{
	...
	if((addr == VGIP_VGIP_CHN1_STATUS_reg)&&((value&(_BIT24|_BIT25))>0)){
		;
#ifdef CONFIG_DUAL_CHANNEL
	}else if((addr == SUB_VGIP_VGIP_CHN2_STATUS_reg)&&((value&(_BIT24|_BIT25))>0)){
		;
...
int PlayFile(char* f_name)
	...
	AIO_PATH_CFG	path_cfg;
	  ...
	  if(player == NULL &&  RMP_Player_New(&player) != RMP_ERR_OK) {
		...
		rtd_pr_ioreg_info("Refresh DDR refresh rate\n");
		IoReg_Write32(DC1_MC_DC_MIS2_reg, (IoReg_Read32(DC1_MC_DC_MIS2_reg) & ~0x3f00)| 0x00001a00);
			...
			rtd_pr_ioreg_info("st load %s\n", mediapath);
			rtd_pr_ioreg_info("orgFname:%s\n", f_name);
		...
		if( retval == RMP_ERR_OK ) {
			if( player->Play(player) != RMP_ERR_OK) {
				...
				rtd_pr_ioreg_info("play %s fail\n", mediapath);
				exit(-1);
			}
		}
		else {
			rtd_pr_ioreg_info("load media %s fail:\n", mediapath);
			NAVLOADFAILURE	*pFailure = &media_param.retFailure;
			rtd_pr_ioreg_info("res: mType:%d vType:%d(%d) aType:%d\n", pFailure->reason,
...
static unsigned long vir_addr=0;
...
void register_dump_data_to_file(void)
{
	if(read_flag)
	{
		...
		if (filp == NULL) {
			rtd_pr_ioreg_emerg("file open fail\n");
			return;
		}
		else if(rd_ptr==NULL)
		{
			rtd_pr_ioreg_emerg("rd_ptr=NUlL");
			return;
		}

		//rtd_pr_ioreg_emerg("[Eric] dump_pRdptr = %x, dump_pWdptr = %x\n",dump_pRdptr,dump_pWdptr);
		...
		if((rd_ptr->registeraddr != NR_SNR_DB_CTRL_reg)|| (rd_ptr->value != dumpreg_last_value)){
			if(rd_ptr->rw_flag == 2){
				if((rd_ptr->registeraddr == pre_reg_addr) && (rd_ptr->value == pre_reg_val) && (rd_ptr->rw_flag) == pre_reg_rw_flag){
					reg_iteration++;
				}
				else{
					if(reg_iteration > 0)
						snprintf(tmp_buf, buf_size, "[DUMP.S] [0x%08x] scnt=0x%08x,vline_cnt=0x%08x,uline_cnt=0x%08x,mline_cnt=0x%08x,addr=0x%08x,val=0x%08x,rw=0x%x, iteration=0x%x\n",
				            pre_reg_pts, rd_ptr->register_counter,pre_reg_vgip_line_cnt,pre_reg_uzudtg_line_cnt,pre_reg_memcdtg_line_cnt,
				           pre_reg_addr,pre_reg_val,pre_reg_rw_flag,reg_iteration);
					reg_iteration = 0;
					rd_ptr->pts = ((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(TIMER_VCPU_CLK90K_LO_reg)));
					rd_ptr->vgip_line_cnt=(((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VGIP_VGIP_CHN1_LC_reg))&0x7FFF0000)>>16);
					rd_ptr->uzudtg_line_cnt=(((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(PPOVERLAY_new_meas1_linecnt_real_reg))&0x1FFF0000)>>16);
					rd_ptr->memcdtg_line_cnt=((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(PPOVERLAY_new_meas1_linecnt_real_reg))&0x1FFF);
					snprintf(tmp_buf, buf_size, "[DUMP.S] [0x%08x] scnt=0x%08x,vline_cnt=0x%08x,uline_cnt=0x%08x,mline_cnt=0x%08x,addr=0x%08x,val=0x%08x,rw=0x%x, iteration=0x%x\n",
			            rd_ptr->pts, rd_ptr->register_counter,
			            rd_ptr->vgip_line_cnt,rd_ptr->uzudtg_line_cnt,rd_ptr->memcdtg_line_cnt,
			            rd_ptr->registeraddr,rd_ptr->value,rd_ptr->rw_flag,reg_iteration);
				}
				pre_reg_pts = ((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(TIMER_VCPU_CLK90K_LO_reg)));
				pre_reg_scnt = rd_ptr->register_counter;
				pre_reg_vgip_line_cnt = (((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VGIP_VGIP_CHN1_LC_reg))&0x7FFF0000)>>16);
				pre_reg_uzudtg_line_cnt = (((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(PPOVERLAY_new_meas1_linecnt_real_reg))&0x1FFF0000)>>16);
				pre_reg_memcdtg_line_cnt = ((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(PPOVERLAY_new_meas1_linecnt_real_reg))&0x1FFF);
				pre_reg_addr = rd_ptr->registeraddr;
				pre_reg_val = rd_ptr->value;
				pre_reg_rw_flag = rd_ptr->rw_flag;
			}
			else{
				if((rd_ptr->registeraddr == pre_reg_addr) && (rd_ptr->value == pre_reg_val) && (rd_ptr->rw_flag == pre_reg_rw_flag)){
					reg_iteration++;
	        ...
	        if(rd_ptr->registeraddr == NR_SNR_DB_CTRL_reg)
	            dumpreg_last_value = rd_ptr->value;
	    }
		//rtd_pr_ioreg_emerg("[tjw]%s",tmp_buf);
		ptr=&tmp_buf[0];
		if (ptr == NULL) {
			rtd_pr_ioreg_emerg("ptr == NULL\n");
			return;
		}
...
void register_dump_data_to_buffer(DUMP_REGISTER_INFO dump_register_info)
{
	...
	//rtd_pr_ioreg_emerg("[1]dump_pWdptr=%x,dump_pRdptr=%x\n",dump_pWdptr,dump_pRdptr);
	remained_size=dump_bufStar+dump_bufSize-wr_ptr-1;
	length=sizeof(DUMP_REGISTER_INFO);

	// timing monitor init 
	wrt_pnt1_reg_val = (*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VODMA_vodma_timing_monitor_ctrl_reg)); 
	wrt_pnt1_reg_val &= 0xFFFF00FF;
	wrt_pnt1_reg_val |= 0x0000f300;  //B[15:8]
	(*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VODMA_vodma_timing_monitor_ctrl_reg) = wrt_pnt1_reg_val);
	regValue5680 = (*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VODMA_vodma_timing_monitor_ctrl_reg)); 
	regValue5680 &= 0x6FFFFFFF;
	regValue5680 |= 0x90000000; //B[28] & [31]
	(*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VODMA_vodma_timing_monitor_ctrl_reg) = regValue5680);

#if 1 // dump log directory
    if((dump_register_info.registeraddr != NR_SNR_DB_CTRL_reg)|| (dump_register_info.value != dumpreg_last_value)){
		if(dump_register_info.rw_flag == 2){
			if((dump_register_info.registeraddr == pre_reg_addr) && (dump_register_info.value == pre_reg_val) && (dump_register_info.rw_flag) == pre_reg_rw_flag){
				...
				if(reg_iteration > 0){
					...
					rtd_pr_ioreg_info("[DUMP.S] [0x%08x] scnt=%4d,LCVO=%4d,LCV=%4d,LCU=0x%4d,LCM=%4d,MomSize=%8d,addr=0x%08x,val=0x%08x,rw=%d, rpt=%d\n",
			           pre_reg_pts, dump_register_info.register_counter,pre_reg_vo_line_cnt,pre_reg_vgip_line_cnt,pre_reg_uzudtg_line_cnt,pre_reg_memcdtg_line_cnt,pre_timing_monitor,
			           pre_reg_addr,pre_reg_val,pre_reg_rw_flag,reg_iteration);
					#else
					rtd_pr_ioreg_info("[DUMP.S] [0x%08x] scnt=0x%08x,vline_cnt=0x%08x,uline_cnt=0x%08x,mline_cnt=0x%08x,addr=0x%08x,val=0x%08x,rw=0x%x, iteration=0x%x\n",
				...
				dump_register_info.pts = ((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(TIMER_VCPU_CLK90K_LO_reg)));
				dump_register_info.vo_line_cnt = (((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VODMA_VODMA_LINE_ST_reg))&0x00000FFF)); 
				dump_register_info.vgip_line_cnt=(((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VGIP_VGIP_CHN1_LC_reg))&0x7FFF0000)>>16);
				dump_register_info.uzudtg_line_cnt=(((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(PPOVERLAY_new_meas1_linecnt_real_reg))&0x1FFF0000)>>16);
				dump_register_info.memcdtg_line_cnt=((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(PPOVERLAY_new_meas1_linecnt_real_reg))&0x1FFF);
				dump_register_info.timing_monitor = ((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VODMA_vodma_timing_monitor_result1_reg))&0xFFFFFFFF);
				#ifdef CONFIG_DUMP_REGISTER_WHEN_ONLY_DISCONNECT
				rtd_pr_ioreg_info("[DUMP.S] [0x%08x] scnt=%4d,LCVO=%4d,LCV=%4d,LCU=0x%4d,LCM=%4d,MomSize=%8d,addr=0x%08x,val=0x%08x,rw=%d, rpt=%d\n",
		            dump_register_info.pts, dump_register_info.register_counter,
		            dump_register_info.vo_line_cnt,dump_register_info.vgip_line_cnt,dump_register_info.uzudtg_line_cnt,dump_register_info.memcdtg_line_cnt,dump_register_info.timing_monitor,
			...
			pre_reg_vo_line_cnt = (((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VODMA_VODMA_LINE_ST_reg))&0x00000FFF)); 
			pre_reg_vgip_line_cnt = (((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VGIP_VGIP_CHN1_LC_reg))&0x7FFF0000)>>16);
			pre_reg_uzudtg_line_cnt = (((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(PPOVERLAY_new_meas1_linecnt_real_reg))&0x1FFF0000)>>16);
			pre_reg_memcdtg_line_cnt = ((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(PPOVERLAY_new_meas1_linecnt_real_reg))&0x1FFF);
			pre_timing_monitor = ((*(volatile unsigned int *)GET_MAPPED_RBUS_ADDR(VODMA_vodma_timing_monitor_result1_reg))&0xFFFFFFFF);
        ...
        if(dump_register_info.registeraddr == NR_SNR_DB_CTRL_reg)
            dumpreg_last_value = dump_register_info.value;
    }
#else
	...
	else
	{
		...
		rtd_pr_ioreg_emerg("memory over,dump stop");
...
int register_dump_check_counter(void)
{
	...
	if(dr_info_ptk==NULL)
		rtd_pr_ioreg_emerg("dr_info_ptk=NULL\n");
	if (dr_info_ptk) {
...
void register_dump_send_info(VIDEO_RPC_DUMP_REGISTER dr_rpc_date)
{
	...
	if(pulDataTemp==NULL){
		rtd_pr_ioreg_emerg("[tjw]pulDataTemp==NULL!!!\n");
		return;
	}
	for (i = 0; i < ulCount; i++)
	...
	{
		rtd_pr_ioreg_emerg("ret=%d, SCALERIOC_DUMP_REGISTER_INFO to driver fail !!!\n", ret);
	}
}
...
//	rtd_pr_ioreg_emerg("register_dump_test !!!\n");
	if(IoReg_Read32(VGIP_VGIP_CHN1_STATUS_reg)&(1<<17)){
		if(Get_dump_function()==0){

		vir_addr = (unsigned long)dvr_malloc_uncached_specific(REGISTER_DUMP_BUFFER_SIZE*sizeof(unsigned char), GFP_DCU1, (void **)&vir_addr_noncache);
		if(0==vir_addr)
			return;
		...
		*dr_info_ptk=0;
		...
		Set_dump_function(1);
		//rtd_pr_ioreg_emerg("register_dump_test rpc done !!!\n");
		}
	}
	else{
		if(Get_dump_function()==1){
		//rtd_pr_ioreg_emerg("register_dump_test stop !!!\n");
		Set_dump_function(0);
...
void reg_dump_set (const char *buffer)
{
	...
	{
			//rtd_pr_ioreg_debug("register dump set failed, invalid argument - %s\n", buffer);
			goto Fail;
	}
...
#endif
Fail:
    rtd_pr_ioreg_emerg("%s fail.  %s\n\n\n",__func__,buffer);

    rtd_pr_ioreg_emerg("\e[1;32m reg_dump related commands as follow\e[0m: (set no must between 1 and 4)\n");
    rtd_pr_ioreg_emerg("echo clear > /sys/realtek_boards/reg_dump\n\n\n");
    rtd_pr_ioreg_emerg("echo \"set 0x00cd2d00-0x00cd2d40\" > /sys/realtek_boards/reg_dump\n");
    rtd_pr_ioreg_emerg("echo start > /sys/realtek_boards/reg_dump\n\n\n");
    rtd_pr_ioreg_emerg("echo stop > /sys/realtek_boards/reg_dump\n\n\n");
    return ;
}
#endif
#endif
