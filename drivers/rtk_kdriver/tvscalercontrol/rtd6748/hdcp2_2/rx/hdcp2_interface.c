...
static void spu_printBytes(const H2uint8 *bytes, H2uint32 len)
{
	unsigned int ii;
	for (ii = 0; ii < len; ii++) {
		rtd_pr_hdcp_debug("%02x ", bytes[ii]);
		if (ii % 16 == 15) {
			rtd_pr_hdcp_debug("\r\n");
		}
	}
...
#define spu_printBytes_with_text(x, y, fmt, args...)    do {rtd_pr_hdcp_debug(fmt, ## args); spu_printBytes(x, y); } while (0)
...
int read_binary_file(const char *path, unsigned char *buff, unsigned int len)
{
	...
	if (IS_ERR(file)) {
		rtd_pr_hdcp_debug("create file error\n");
		return -1;
	}
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
...
int write_binary_file(const char *path, unsigned char *buff, unsigned int len)
{
    ...
    kernel_write(file, buff, len, &pos);
#else
...
{
	static H2uint8 Km_tmp[MASTERKEY_SIZE];
	H2uint8 Kd_tmp[KD_SIZE];
	static H2uint8 KPriv_tmp[KPRIVRX_SIZE];
	H2status rc;
...
{
	H2uint8 Kh[KH_SIZE] = {0};
	H2uint8 Km[MASTERKEY_SIZE] = {0};
	spu_GetKH(Kh);
...
H2status hdcp2_Rx_SetEdKeyKs(H2uint8 *EdKeyKs)
{
	...
	unsigned char dkey2[16] = {0};
	H2uint8 rtx[RTX_SIZE] = {0};
	H2uint8 rrx[RRX_SIZE] = {0};
	H2uint8 rn[RN_SIZE] = {0};
	unsigned char ks[16] = {0};
	unsigned char lc128[LC128_SIZE] = {0};
...
{
	unsigned char dkey2[DKEY_SIZE] = {0};
	unsigned char ks[KS_SIZE] = {0};
...
H2status hdcp2_save_keyset_file(unsigned char *pKeySet, unsigned char *dst_file_path, int keyLength)
{
	...
	{
		rtd_pr_hdcp_emerg("rhal don't save keyset to file\n");
		return H2_OK;
	}
...
H2status hdcp2_load_keyset_file(unsigned char *pKeySet, unsigned char *src_file_path, int keyLength)
{
	...
	{
		rtd_pr_hdcp_emerg("rhal don't load keyset from file\n");
		return H2_OK;
	}
...
H2status hdcp2_load_from_default(void)
{
    ...
    rtd_pr_hdcp_emerg("hdcp2_load_from_default Device ID = %x %x %x %x  %x \n ",CTS_key[CTS_KEY_CERTRX_OFFSET],CTS_key[CTS_KEY_CERTRX_OFFSET+1],
...
#else
		rtd_pr_hdcp_emerg("hdcp2 default key can't use\n");
#endif
	return H2_OK;
}
...
H2status hdcp2_load_from_ap(void)
{
	...
	if (newbase_hdmi_get_customer_hdcp2p2() && h2p2table) {
		rtd_pr_hdcp_emerg("Load AP Customer HDCP 2.2 Key successfully!!!");
			
		spu_SetLc128(h2p2table);
		spu_SetCertRx(&h2p2table[16]);
		rtd_pr_hdcp_emerg("AP HDCP2.2 KEY Device ID = %x %x %x %x  %x \n ",h2p2table[16],h2p2table[17],h2p2table[18],h2p2table[19],h2p2table[20]);
		...
		rtd_pr_hdcp_emerg("Failed to load HDCP 2.2 Key from AP!!!");
		return H2_ERROR;
	}
...
H2status hdcp2_load_from_sstorage(unsigned char *pKeySet)
{
	...
	if(pKeySet == NULL)
	{
		rtd_pr_hdcp_notice("hdcp2_load_from_sstorage FAILED\n");
		return H2_ERROR;
	}
	...
	rtd_pr_hdcp_emerg("hdcp2_load_from_sstorage Device ID = %x %x %x %x  %x \n ",pKeySet[CTS_KEY_CERTRX_OFFSET],pKeySet[CTS_KEY_CERTRX_OFFSET+1],
