...
#include <hdmirx/hdmiPlatform.h>


#include <rtd_log/rtd_module_log.h>
#define HDCP2_PRINTF(fmt, args...) 		rtd_pr_hdcp_emerg("HDMI"fmt, ##args)
...
int h2MsgAkeSendCertPack(unsigned char *pMsg, unsigned int *pLength, const H2_AKESendCertPayLoad *payload)
{
	...
	do {
       ...
       */
		...
		//rtd_pr_hdcp_info("func=%s,line=%d\n",__FUNCTION__, __LINE__);
		#if ENABLE_DDC_WRITE
		if (pLength)
		{
			#if defined(NEWBASE_FUNC)
...
extern unsigned char g_TX_msg_id[];
...
int h2MsgAkeSendHprimePack(unsigned char *pMsg, unsigned int *pLength, const H2_AKESendHprimePayLoad *payload)
{
	...
	#if ENABLE_DDC_WRITE
	#if 1
	if(g_TX_msg_id[nport] ==  AKE_NO_Stored_Km)
	{
		#if defined(NEWBASE_FUNC)
	...
	rtd_pr_hdcp_debug("cloud write teset ");
	  #endif
		rc = 0;
		break;
	} while (0);
		...
		if (pLength) {
			*pLength = EKHKM_SIZE + 1;
				...
				if (HDMI_ABS(hdmi_get_system_time_ms(), current_ms) > 1000)
				{
					...
					hdmi_flow_flag[nport].hdcp22_error_type = HDCP22_ERROR_TX_READ_HPRIME_TIMEOUT;
				}
#endif
