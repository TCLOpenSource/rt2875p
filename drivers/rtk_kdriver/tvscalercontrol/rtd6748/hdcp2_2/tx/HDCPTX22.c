...
HDCP_ERR send_ake_nostored_km(void)
{
	...
	if(pDB == NULL){
		LOG("[HDCP] kcalloc failed, pDB is NULL\n");
		err_code = HDCP_ERR_UNDEFINED;
		goto kcalloc_error;
	}
	offset=0;
	...
	if(pdbMask == NULL){
		LOG("[HDCP] kcalloc failed, pdbMask is NULL\n");
		err_code = HDCP_ERR_UNDEFINED;
		goto kcalloc_error;
	}
	...
	if(pMaskedDB == NULL){
		LOG("[HDCP] kcalloc failed, pMaskedDB is NULL\n");
		err_code = HDCP_ERR_UNDEFINED;
		goto kcalloc_error;
	}
...
HDCP_ERR send_repeater_auth_content_stream_manage(void)
{
	...
	if(out == NULL){
		LOG("[HDCP] kcalloc failed, out is NULL\n");
		return HDCP_ERR_UNDEFINED;
	}
...
HDCP_ERR read_repeater_auth_content_stream_ready(UINT32 time_out, UINT32 *time_consumed)
{
	...
	if(IS_SUCCESS(err_code))
	{
		...
		if(input == NULL){
			LOG("[HDCP] kcalloc failed, input is NULL\n");
			return HDCP_ERR_UNDEFINED;
		}
...
void generate_random_number(UINT8 *dst, INT32 size, const char *msg)
{
	...
	if(tmp == NULL){
		LOG("ERROR - kcalloc failed, tmp is NULL\n");
		return;
	}
