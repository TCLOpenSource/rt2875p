...
#include <dolby_idk_1_6_1_1/dolby_gdbs_delay.h>
...
unsigned char LocalContrast_EN_UI=1;

extern unsigned int pre_sourceMaxL_a;
extern unsigned char pre_hdrtype;
...
unsigned int hack_source_index = 0xffffffff;
...
//rtd_printk(KERN_DEBUG, TAG_NAME, "\n rord get_HDMI_HDR_mode()=%d,get_OTT_HDR_mode()=%d  \n",get_HDMI_HDR_mode(),get_OTT_HDR_mode());
  ...
  /*Need SW sync,  SW Team not sync from TV001*/
	...
	#if 0
	printk( "++++++++++++fwif_vip_source_check debug Part.A+++++++++++++++++++\n");
	printk( "[source_check]input_src_from = %d \n", input_src_form);
	printk( "[source_check]input_src_type = %d \n", input_src_type);
	printk( "[source_check]Scaler_VOFrom = %d \n", Scaler_VOFrom(Scaler_Get_CurVoInfo_plane()));
	printk( "[source_check]drvif_scalerVideo_readMode = %d \n", drvif_module_vdc_ReadMode(VDC_SET));
	printk( "[source_check]Scaler_DispGetInputInfo = %d \n", Scaler_DispGetInputInfo(SLR_INPUT_MODE_CURR));
	printk( "[source_check]Scaler_InputSrcFormat = %d \n", Scaler_InputSrcFormat(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DATA_ARRAY_IDX)));
	printk( "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	#endif
	...
	if( input_src_form == _SRC_FROM_VDC )
	{
		...
		if (is_DTV_flag_get()==0)	{		/* USB source*/
			if(hack_source_index<VIP_QUALITY_TOTAL_SOURCE_NUM){
				which_source = hack_source_index;
				printk("Use HAL source %d\n",hack_source_index);

				if(fwif_VIP_get_Project_ID() == VIP_Project_ID_TV011 && hack_source_index==73){	//Hisense network use YPP source
			...
			}else if (Scaler_VOFromJPEG(Scaler_Get_CurVoInfo_plane()) == 1) {
			...
			case _MODE_480I:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					which_source = VIP_QUALITY_HDR_PVR_480I;
				else if (bHDR_flag == HAL_VPQ_HDR_MODE_ST2094)
					which_source = VIP_QUALITY_ST2094_PVR_480I;
				else if(bDolby_HDR_flag)
					which_source = VIP_QUALITY_DolbyHDR_PVR_480I;
				else
					which_source = VIP_QUALITY_PVR_480I;
				break;
			case _MODE_576I:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					which_source = VIP_QUALITY_HDR_PVR_576I;
				else if (bHDR_flag == HAL_VPQ_HDR_MODE_ST2094)
					which_source = VIP_QUALITY_ST2094_PVR_576I;
				else if(bDolby_HDR_flag)
					which_source = VIP_QUALITY_DolbyHDR_PVR_576I;
				else
			...
			case  _MODE_480P:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					...
					which_source = VIP_QUALITY_ST2094_PVR_480P;
				else if(bDolby_HDR_flag)
			...
			case _MODE_576P:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					...
					which_source = VIP_QUALITY_ST2094_PVR_576P;
				else if(bDolby_HDR_flag)
			...
			case _MODE_720P60:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					...
					which_source = VIP_QUALITY_ST2094_PVR_720P;
				else if(bDolby_HDR_flag)
			...
			case _MODE_1080I30:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					which_source = VIP_QUALITY_HDR_PVR_1080I;
				else if (bHDR_flag == HAL_VPQ_HDR_MODE_ST2094)
					which_source = VIP_QUALITY_ST2094_PVR_1080I;
				else if(bDolby_HDR_flag)
					which_source = VIP_QUALITY_DolbyHDR_PVR_1080I;
				else
		...
		which_source = VIP_QUALITY_DTV_JPEG;
			...
			case _MODE_480I:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					which_source = VIP_QUALITY_HDR_DTV_480I;
				else if (bHDR_flag == HAL_VPQ_HDR_MODE_ST2094)
					which_source = VIP_QUALITY_ST2094_DTV_480I;
				else if(bDolby_HDR_flag)
					which_source = VIP_QUALITY_DolbyHDR_DTV_480I;
				else
					which_source = VIP_QUALITY_DTV_480I;
				break;
			case _MODE_576I:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					which_source = VIP_QUALITY_HDR_DTV_576I;
				else if (bHDR_flag == HAL_VPQ_HDR_MODE_ST2094)
					which_source = VIP_QUALITY_ST2094_DTV_576I;
				else if(bDolby_HDR_flag)
					which_source = VIP_QUALITY_DolbyHDR_DTV_576I;
				else
			...
			case  _MODE_480P:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					...
					which_source = VIP_QUALITY_ST2094_DTV_480P;
				else if(bDolby_HDR_flag)
			...
			case _MODE_576P:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					...
					which_source = VIP_QUALITY_ST2094_DTV_576P;
				else if(bDolby_HDR_flag)
			...
			case _MODE_720P60:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					...
					which_source = VIP_QUALITY_ST2094_DTV_720P;
				else if(bDolby_HDR_flag)
			...
			case _MODE_1080I30:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					which_source = VIP_QUALITY_HDR_DTV_1080I;
				else if (bHDR_flag == HAL_VPQ_HDR_MODE_ST2094)
					which_source = VIP_QUALITY_ST2094_DTV_1080I;
				else if(bDolby_HDR_flag)
					which_source = VIP_QUALITY_DolbyHDR_DTV_1080I;
				else
		...
		#if 0
		printk("++++++++++++fwif_vip_source_check debug Part.B+++++++++++++++++++\n");
		printk("[source_check] drvif_IsHDMI()= %d			     $$$$$$$$\n", drvif_IsHDMI());
		printk("[source_check] input_timming_info= %d   $$$$$$$$\n",input_timming_info);
		printk("[source_check] video_format = %d				  $$$$$$$$\n", video_format);
		//printk("[source_check] ForceHDMIIntoVIPMode()= %d	     $$$$$$$$\n", ForceHDMIIntoVIPMode);
		printk("[source_check] drvif_Hdmi_IsAudioLock() = %d    $$$$$$$$\n", drvif_Hdmi_IsAudioLock());
		printk("[source_check] Scaler_DispGetInputInfo() = %d    $$$$$$$$\n", Scaler_DispGetInputInfo(SLR_INPUT_MODE_CURR));
		printk("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
		#endif
		...
		if (drvif_IsHDMI()) { /* HDMI*/
			...
			case _MODE_480I:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					which_source = VIP_QUALITY_HDR_HDMI_480I;
				else if (bHDR_flag == HAL_VPQ_HDR_MODE_ST2094)
					which_source = VIP_QUALITY_ST2094_HDMI_480I;
				else if(bDolby_HDR_flag)
					which_source = VIP_QUALITY_DolbyHDR_HDMI_480I;
				else
					which_source = VIP_QUALITY_HDMI_480I;
				break;
			case _MODE_576I:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					which_source = VIP_QUALITY_HDR_HDMI_576I;
				else if (bHDR_flag == HAL_VPQ_HDR_MODE_ST2094)
					which_source = VIP_QUALITY_ST2094_HDMI_576I;
				else if(bDolby_HDR_flag)
					which_source = VIP_QUALITY_DolbyHDR_HDMI_576I;
				else
			...
			case  _MODE_480P:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					...
					which_source = VIP_QUALITY_ST2094_HDMI_480P;
				else if(bDolby_HDR_flag)
			...
			case _MODE_576P:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					...
					which_source = VIP_QUALITY_ST2094_HDMI_576P;
				else if(bDolby_HDR_flag)
			...
			case _MODE_720P60:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					...
					which_source = VIP_QUALITY_ST2094_HDMI_720P;
				else if(bDolby_HDR_flag)
			...
			case _MODE_1080I30:
				if((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
					which_source = VIP_QUALITY_HDR_HDMI_1080I;
				else if (bHDR_flag == HAL_VPQ_HDR_MODE_ST2094)
					which_source = VIP_QUALITY_ST2094_HDMI_1080I;
				else if(bDolby_HDR_flag)
					which_source = VIP_QUALITY_DolbyHDR_HDMI_1080I;
				else
	...
	/*==USER: Vincent_Lee  DATE: 2011/08/12  TODO: Add new CASE for 1080P24 mode==*/
			...
			if(rtk_hal_vsc_GetRGB444Mode()){ // PC mode
				if(bHDR_flag == HAL_VPQ_HDR_MODE_SDR_MAX_RGB){
					which_source = VIP_QUALITY_VGA;
					printk("RGB444mode: PC mode go VIP VGA \n");
				}
			}
		} else { /* DVI*/
			which_source = VIP_QUALITY_VGA;
...
void fwif_color_video_quality_handler(void)/*TV Process*/
{
	...
	extern VIP_YUV2RGB_LEVEL_SELECT uvoffset_Level_UI;
	VIPprintf("+VIP color handler+\n");

	if (fwif_VIP_get_Project_ID() == VIP_Project_ID_TV030){
		#ifdef CONFIG_DYNAMIC_PANEL_SELECT
		 if(s_panelDlgMode != PANEL_DLG_DEFAULT){
			framerate_update_delay = 0;
...
/*o------ VIP table : VIP_QUALITY_Coef ------o*/
	...
	//rtd_printk(KERN_EMERG, TAG_NAME,"==========qua  source = %d ===================\n", source);
	printk("==========quahandler  source = %d ===================\n", source);
	...
	if (source >= VIP_QUALITY_TOTAL_SOURCE_NUM || pVipCoefArray == NULL) {
		...
		if(fwif_VIP_get_Project_ID() == VIP_Project_ID_TV030)
			fwif_color_set_UV_Offset_tv043(uvoffset_Level_UI);
...
/*o------ VIP table : VIP_QUALITY_Extend_Coef ------o*/
	...
	if (!(source >= VIP_QUALITY_TOTAL_SOURCE_NUM || pVipCoefArray == NULL)) {
		...
		rtd_printk(KERN_EMERG, TAG_NAME,"[qua hand] i_decontour_index=%d\n",i_decontour_index);
...
/*o------ VIP table : VIP_QUALITY_Extend3_Coef ------o*/
	...
	if (!(source >= VIP_QUALITY_TOTAL_SOURCE_NUM || pVipCoefArray == NULL)) {
		...
		if (vpq_project_id != 0x00060000 || vpqex_project_id != 0x00060000) { /*pre_id:16 main_id:8 sub1_id:8 sub2_id:8, example->lg=tv006*/
			//if(fwif_VIP_get_Project_ID() != VIP_Project_ID_TV011){ //fix testcase ICM NULL issue
			...
			Scaler_SetDCC_Mode(aipq_DynamicContrastLevel&0xff);
		}
	...
	if (fwif_VIP_get_Project_ID() == VIP_Project_ID_TV030) {
		rtd_printk(KERN_EMERG, TAG_NAME, "yuan,qua end===\n");
	...
	if(vcom_flag){
		printk("qua handler,vcom_flag=%d \n",vcom_flag);
		Scaler_color_Set_PQ_ByPass_Lv(8);
...
void Scaler_VIP_Project_ID_init()
{
	...
	#elif defined(CONFIG_TV011_PLATFORM)
		/*VIP_system_info_structure_table->Project_ID	= VIP_Project_ID_TV011;*/
		fwif_VIP_set_Project_ID(VIP_Project_ID_TV011);
...
void Scaler_SetSharpness(unsigned char value)
{
	...
	printk("Scaler_SetSharpness ==> %d", value);
	pData = (SLR_PICTURE_MODE_DATA *)fwif_color_get_cur_picture_mode_data(src_idx);
	if (pData == NULL)
		return;
...
unsigned char Scaler_get_HDR_YUV2RGB_ColorFMT_601_in_IMD_Domain(unsigned char color_format)
{
	...
	if (srcType == VSC_INPUTSRC_HDMI) {
		...
		video_format = drvif_Hdmi_GetColorSpace();
	} else if (srcType == VSC_INPUTSRC_VDEC) {
	...
	rtd_printk(KERN_INFO, TAG_NAME, "CSC1, display=%d,srcType=%d,color_fmt=%d,video_fmt=%d,FullRange=%d,ret=%d\n",
	...
	else
	{
		...
		if (HDR_mode == HAL_VPQ_HDR_MODE_SDR_MAX_RGB) {
			...
			drvif_HDR_YUV2RGB(1, hdr_YUV2RGB[index]);
				...
				drvif_HDR_YUV2RGB(0, hdr_YUV2RGB[index]);		//RGB input
...
void Scaler_hdr_setting_SEAMLESS(unsigned char hdr_enable)
{
	...
	*/
	...
	printk("[seamless]Scaler_hdr_setting_SEAMLESS hdrType = %d \n",hdrType);
	if(pre_hdrtype != PQModeInfo_flag_seamless[0]){
		if(OETF_set_by_user)
			OETF_set_by_user = 0;
		pre_hdrtype = PQModeInfo_flag_seamless[0];
	}
...
//	_system_setting_info *system_info_struct = (_system_setting_info *)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_system_info_structure);
	...
	printk("[seamless]Scaler_hdr_setting_SEAMLESS_hdmi_position hdrType = %d \n",hdrType);
	if(pre_hdrtype != PQModeInfo_flag_seamless[0]){
		if(OETF_set_by_user)
			OETF_set_by_user = 0;
		pre_hdrtype = PQModeInfo_flag_seamless[0];
...
static int frame_to_msec(int frame, int msec)
{
	return (frame > 0) ? msec * frame : 0;
}

static void set_dolby_delay_table(frame_delay_T frame_delay)
{
	struct dolby_vision_gdbs_delay_data delay_table = { 0 };

	delay_table.delay_hdmi_fps_24 = frame_to_msec(frame_delay.hdmi_delay_frame[0], 41);
	delay_table.delay_hdmi_fps_30 = frame_to_msec(frame_delay.hdmi_delay_frame[0], 33);
	delay_table.delay_hdmi_fps_50 = frame_to_msec(frame_delay.hdmi_delay_frame[1], 20);
	delay_table.delay_hdmi_fps_60 = frame_to_msec(frame_delay.hdmi_delay_frame[1], 17);

	delay_table.delay_ott_fhd_fps_24 = frame_to_msec(frame_delay.ott_delay_frame[0], 41);
	delay_table.delay_ott_fhd_fps_30 = frame_to_msec(frame_delay.ott_delay_frame[0], 33);
	delay_table.delay_ott_fhd_fps_50 = frame_to_msec(frame_delay.ott_delay_frame[1], 20);
	delay_table.delay_ott_fhd_fps_60 = frame_to_msec(frame_delay.ott_delay_frame[1], 17);

	delay_table.delay_ott_uhd_fps_24 = frame_to_msec(frame_delay.ott_delay_frame[0], 41);
	delay_table.delay_ott_uhd_fps_30 = frame_to_msec(frame_delay.ott_delay_frame[0], 33);
	delay_table.delay_ott_uhd_fps_50 = frame_to_msec(frame_delay.ott_delay_frame[1], 20);
	delay_table.delay_ott_uhd_fps_60 = frame_to_msec(frame_delay.ott_delay_frame[1], 17);

	set_gdbs_delay_matrix_table(delay_table);
}

void Scaler_color_vpq_set_dolbylight_init(KADP_VPQ_DOBYBAKLIGHT_INI dolbybacklight_ini)
{
	frame_delay.ott_delay_frame[0] = dolbybacklight_ini.ott_24_delay_frame;
	frame_delay.ott_delay_frame[1] = dolbybacklight_ini.ott_60_delay_frame;
	frame_delay.hdmi_delay_frame[0] = dolbybacklight_ini.hdmi_24_delay_frame;
	frame_delay.hdmi_delay_frame[1] = dolbybacklight_ini.hdmi_60_delay_frame;
	...
	//ui 0~100, dm 0~256
	dm_node.dm_node_0 = 26;
	dm_node.dm_node_1 = 83;
	dm_node.dm_node_2 = 140;
	dm_node.dm_node_3 = 198;
	...
	set_dolby_delay_table(frame_delay);
...
static int dump_debug_duty_pre, dump_debug_duty;

static void rtk_pwm_backlight_set_duty_by_adim_pdim(int getui_area,int getui_value)
{
    RTK_PWM_PARAM_EX_T param_adim, param_pdim;
    extern int rtk_pwm_set_param(RTK_PWM_PARAM_EX_T* param);

    switch(getui_area)
    {
        case UI_AREA_0_TO_25:
            adim_value = ui_value.ui_0_adim + ((ui_value.ui_25_adim - ui_value.ui_0_adim) * getui_value / (25 - 0));
            pdim_value = ui_value.ui_0_pdim + ((ui_value.ui_25_pdim - ui_value.ui_0_pdim) * getui_value / (25 - 0));
            break;
        case UI_AREA_25_TO_50:
            adim_value = ui_value.ui_25_adim + ((ui_value.ui_50_adim - ui_value.ui_25_adim) * (getui_value - 25) / (50 - 25));
            pdim_value = ui_value.ui_25_pdim + ((ui_value.ui_50_pdim - ui_value.ui_25_pdim) * (getui_value - 25) / (50 - 25));
            break;
        case UI_AREA_50_TO_75:
            adim_value = ui_value.ui_50_adim + ((ui_value.ui_75_adim - ui_value.ui_50_adim) * (getui_value - 50) / (75 - 50));
            pdim_value = ui_value.ui_50_pdim + ((ui_value.ui_75_pdim - ui_value.ui_50_pdim) * (getui_value - 50) / (75 - 50));
            break;
        case UI_AREA_75_TO_100:
            adim_value = ui_value.ui_75_adim + ((ui_value.ui_100_adim - ui_value.ui_75_adim) * (getui_value - 75) / (100 - 75));
            pdim_value = ui_value.ui_75_pdim + ((ui_value.ui_100_pdim - ui_value.ui_75_pdim) * (getui_value - 75) / (100 - 75));
            break;
        case UI_AREA_0:
            adim_value = ui_value.ui_0_adim;
            pdim_value = ui_value.ui_0_pdim;
            break;
        case UI_AREA_100:
            adim_value = ui_value.ui_100_adim;
            pdim_value = ui_value.ui_100_pdim;
            break;
    }
    ...
    memset(param_adim.pcbname, 0, sizeof(param_adim.pcbname));
    strcpy(param_adim.pcbname, A_DIM);
    param_adim.id = PWM_PARAM_DUTY;
    param_adim.val = adim_value;

    memset(param_pdim.pcbname, 0, sizeof(param_pdim.pcbname));
    strcpy(param_pdim.pcbname, P_DIM);
    param_pdim.id = PWM_PARAM_DUTY;
    param_pdim.val = pdim_value;

    {//debug area
        static const int DEBUG_DIM_THRESHOLD = 50;
        static const int DEBUG_DUMP = 1000;
        static int debug_dump_count = 0;
        static int debug_adim_value, debug_pdim_value, debug_ui_area, debug_ui_value;
        if(debug_adim_value >= adim_value + DEBUG_DIM_THRESHOLD
             || debug_adim_value + DEBUG_DIM_THRESHOLD <= adim_value
             || debug_pdim_value >= pdim_value + DEBUG_DIM_THRESHOLD
             || debug_pdim_value + DEBUG_DIM_THRESHOLD <= pdim_value
             || debug_adim_value > ui_value.ui_100_adim
             || debug_pdim_value > ui_value.ui_100_pdim
             || debug_adim_value < ui_value.ui_0_adim
             || debug_pdim_value < ui_value.ui_0_pdim
             || !debug_dump_count
        ) {
            pr_notice("[Dolby][%s-%d][%d] adim: %d, %d, %d, %d, %d, pdim: %d, %d, %d, %d, %d\n", __func__, __LINE__,
                     debug_dump_count,
                     ui_value.ui_0_adim,
                     ui_value.ui_25_adim,
                     ui_value.ui_50_adim,
                     ui_value.ui_75_adim,
                     ui_value.ui_100_adim,
                     ui_value.ui_0_pdim,
                     ui_value.ui_25_pdim,
                     ui_value.ui_50_pdim,
                     ui_value.ui_75_pdim,
                     ui_value.ui_100_pdim
                );
            pr_notice("[Dolby][%s-%d] NOTICE dim values, adim: %d -> %d, pdim: %d -> %d, adim range: %d - %d, pdim range: %d - %d, duty: %d -> %d, getui_area: %d -> %d, getui_value: %d -> %d\n", __func__, __LINE__, debug_adim_value, adim_value, debug_pdim_value, pdim_value, ui_value.ui_0_adim, ui_value.ui_100_adim, ui_value.ui_0_pdim, ui_value.ui_100_pdim, dump_debug_duty_pre, dump_debug_duty, debug_ui_area, getui_area, debug_ui_value, getui_value);
        }
        debug_adim_value = adim_value;
        debug_pdim_value = pdim_value;
        debug_ui_area = getui_area;
        debug_ui_value = getui_value;
        debug_dump_count++;
        debug_dump_count %= DEBUG_DUMP;
    }

    rtk_pwm_set_param(&param_adim);
    rtk_pwm_set_param(&param_pdim);
}
...
static int transfer_duty_to_ui(int current_duty)
{
	...
	ui = ((current_duty - dm_node.dm_node_0) * (ui_node.ui_node_4 - ui_node.ui_node_0) ) / (dm_node.dm_node_4 - dm_node.dm_node_0);

	return ui;
}

int set_dolby_duty_by_adim_pdim(int duty)
{
	int ui_area=0;
	int ui_value=0;
	...
	if((duty >= dm_node.dm_node_0) && (duty < dm_node.dm_node_1))
		...
		if(duty < dm_node.dm_node_0)
			ui_area = UI_AREA_0;
		else if(duty >= dm_node.dm_node_4)
			ui_area = UI_AREA_100;
		...
		//else {
			...
			//return 1;
		//}
		if(duty < dm_node.dm_node_0 || duty > dm_node.dm_node_4)
			printk(KERN_EMERG "[ERROR]Dolby get ERROR PWM from INI talbe!! duty: %d, duty table %d - %d\n", duty, dm_node.dm_node_0, dm_node.dm_node_4);
	}

	{//debug area
		static const int DEBUG_DUTY_THRESHOLD = 30;
		static int debug_duty;
		if(debug_duty >= duty + DEBUG_DUTY_THRESHOLD
				|| debug_duty + DEBUG_DUTY_THRESHOLD <= duty
		  ) {
			pr_info("[Dolby][%s-%d] notice duty: %d -> %d\n", __func__, __LINE__, debug_duty, duty);
		}
		dump_debug_duty_pre = debug_duty;
		dump_debug_duty = duty;
		debug_duty = duty;
	}

	rtk_pwm_backlight_set_duty_by_adim_pdim(ui_area,ui_value);
...
//==========================================
	...
	printk("HDR auto, HDR_Type=%d, input_src_type=%d, i3d_en=%d, Get_DisplayMode_Src=%d\n",
...
void Scaler_color_HDRInfo_Update(UINT32* modeinfo)
{
	...
	printk("Scaler_color_HDRInfo_Update, modeinfo = %d, Colorimetry info = %d\n", modeinfo[0], modeinfo[1]);
	if (modeinfo[1] == 0) modeinfo[1] = 2;	//data protect
	//SET_HDR_mode
	switch(modeinfo[0])	//HDR mode
	{
...
HDR_flow_control_timing Scaler_color_get_HDR_timing(void)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "[HFC]HDR timing = %d\n", result);
	return result;
}
...
char Scaler_color_Get_HDR10_EOTF_ByMaxCLL_ByPanelLuminance(unsigned short forceIndex)
{
	...
	if (pVOInfo == NULL) {
		rtd_printk(KERN_EMERG, TAG_NAME, "Scaler_color_Get_HDR10_EOTF_ByMaxCLL_ByPanelLuminance, table=NULL, pVOInfo=%p, return !!!!\n", pVOInfo);
		return -1;
	}
	...
	if(input_src_type == _SRC_VO) {
		...
		rtd_printk(KERN_EMERG, TAG_NAME, "Scaler_color_Get_HDR10_EOTF_ByMaxCLL_ByPanelLuminance, is not VO or HDMI, SRC=%d\n", input_src_type);
		return 1;
	}
	...
	rtd_printk(KERN_EMERG, TAG_NAME, "EOTF get, input_src_type=%d, panel_Luminance=%d, MaxCLL=%d,MaxDML=%d,MaxFALL=%d, norm_div=%d, forceIndex=%d\n",
		input_src_type, panel_Luminance, MaxCLL, MaxDML, MaxFALL, norm_div, forceIndex);

	/*MaxCLL == 0, can not get maxCLL*/
	if ((max(MaxCLL, MaxDML) == 0) || (max(MaxCLL, MaxDML) < panel_Luminance))
		MaxLevel = 1000;
	else
...
static unsigned int OETFyi_calman[][65] = 	{
									{400, 9664,9664,11924,13400,13580,14048,14560,14775,15009,15173,15290,15289,15297,15435,15526,15604,15623,15748,15805,15831,15858,15858,15885,15912,15938,15965,15989,16015,16039,16064,16087,16110,16133,16157,16180,16202,16224,16246,16247,16270,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,},
									{600, 13415,13415,13415,13400,13583,13916,14524,14720,14943,15111,15229,15223,15227,15416,15450,15555,15545,15680,15735,15765,15794,15797,15819,15846,15875,15899,15924,15949,15974,15997,16021,16045,16067,16090,16113,16136,16158,16180,16181,16203,16225,16247,16269,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16383,16384,},
...
char Scaler_color_Get_HDR10_OETF_ByMaxCLL_ByPanelLuminance(unsigned short forceIndex)
{
	...
	unsigned int OETF_value_max=0, OETF_value_max_index=0, OETF_shift_bit=0;
	...
	if (pVOInfo == NULL) {
		rtd_printk(KERN_EMERG, TAG_NAME, "Scaler_color_Get_HDR10_OETF_ByMaxCLL_ByPanelLuminance, table=NULL, pVOInfo=%p return !!!!\n", pVOInfo);
		return -1;
	}
	...
	rtd_printk(KERN_INFO, TAG_NAME, "OETF get, input_src_type=%d, panel_Luminance=%d, MaxCLL=%d,MaxFALL=%d,MaxDML=%d\n", input_src_type, panel_Luminance, MaxCLL, MaxFALL, MaxDML);
	...
	if(MaxLevel<=OETFyi_calman[0][0]){
		...
		for(i=1;i<OETFyi_calman_index_max;i++){
			if(MaxLevel < OETFyi_calman[i][0]){
				for(j=1;j<(oetfin+1);j++)
					OETFyi_calman_inner_int[j-1] = ((((((int)OETFyi_calman[i][j]-(int)OETFyi_calman[i-1][j])*((MaxLevel-(int)OETFyi_calman[i-1][0])/16)))/(((int)OETFyi_calman[i][0]-(int)OETFyi_calman[i-1][0])/16))) + (int)OETFyi_calman[i-1][j];
				break;
			}
		}
	}
	...
	if(MaxLevel<=OETFyi[0][0]){
		...
		for(i=1;i<OETFyi_index_max;i++){
			if(MaxLevel < OETFyi[i][0]){
				...
				for(j=1;j<(oetfin+1);j++)
					OETFyi_inner_int[j-1] = ((((int)OETFyi[i][j]-(int)OETFyi[i-1][j])*((MaxLevel-(int)OETFyi[i-1][0])/16))/(((int)OETFyi[i][0]-(int)OETFyi[i-1][0])/16)) + (int)OETFyi[i-1][j];
				break;
			}
		}
	}
	...
	rtd_printk(KERN_INFO, TAG_NAME, "[OETF]OETF_value_max(before)=%d, OETF_value_max_index=%d\n",OETF_value_max, OETF_value_max_index);
	...
	rtd_printk(KERN_INFO, TAG_NAME, "[OETF]OETF_value_max(after) = %d, OETF_shift_bit = %d\n",OETF_value_max,OETF_shift_bit);

	if(OETF_shift_bit<=4){
		drvif_color_set_BBC_shift_bit(OETF_shift_bit);
		if(panel_Luminance!=0)
			drvif_color_Set_ST2094_3Dlut_CInv(((1024<<3)*200+(panel_Luminance/2))/panel_Luminance, 0);
		else
			drvif_color_Set_ST2094_3Dlut_CInv((1024<<3), 0);
	}else{
		drvif_color_set_BBC_shift_bit(4);
		if(panel_Luminance!=0)
			drvif_color_Set_ST2094_3Dlut_CInv(((1024<<(3+OETF_shift_bit-(4)))*200+(panel_Luminance/2))/panel_Luminance, 0);
		else
			drvif_color_Set_ST2094_3Dlut_CInv((1024<<(3+OETF_shift_bit-(4))), 0);
	}
...
void Scaler_color_Get_HDR10_EOTF_OETF_ByMaxCLL_ByPanelLuminance_for_task()
{
	...
	static unsigned int calman_rate = 256, PRE_calman_rate=0xffffffff/*, calman_rate_target=0*/;
	unsigned char EOTF_task_run = HDR10_EOTF_ByMaxCLL_ByPanelLuminance_for_task_run;
	unsigned char OETF_task_run = HDR10_OETF_ByMaxCLL_ByPanelLuminance_for_task_run;
	...
	if(VGIP_VGIP_CHN1_CTRL_get_ch1_vact_end_ie(IoReg_Read32(VGIP_VGIP_CHN1_CTRL_reg))){
		/*
		#ifdef CONFIG_HDR_SDR_SEAMLESS
		extern unsigned char hdmi_position_seamless, hdmi_finish_seamless;
		_RPC_system_setting_info* RPC_SysInfo = scaler_GetShare_Memory_RPC_system_setting_info_Struct();
		if (!(hdmi_position_seamless || hdmi_finish_seamless) && !(RPC_SysInfo != NULL && RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Status] != 0)) {//not during seamless
		#endif
		...
		*/
		...
		if(EOTF_task_run==1 || OETF_task_run==1){
			calman_rate = 256;
	...
	if((calman_rate!=PRE_calman_rate && eotf_lut_enable == 1 && b0103_or_eotf == 1) || EOTF_task_run==1){
		Scaler_color_Get_HDR10_EOTF_apply_rate(EOTF_LUT_ByMaxCLL_ByPanelLuminance, EOTF_LUT_ByMaxCLL_ByPanelLuminance, calman_rate);
	}

	if((calman_rate!=PRE_calman_rate && oetf_lut_enable == 1 && b0105_or_oetf == 1) || OETF_task_run==1){
...
void Scaler_color_set_HDR_Type(unsigned char HDR_Type)
{
	...
	printk("Scaler_color_set_HDR_Type=%d,\n", HDR_Type);
	if(VIP_system_info_structure_table == NULL){
		rtd_printk(KERN_EMERG, TAG_NAME, "[%s:%d] error here!! system_info=NULL! \n",__FILE__, __LINE__);
		return;
	}

	if(HDR_Type != HDR_DM_HDR10 && HDR_Type != HDR_DM_HLG)
		OETF_set_by_user = 0;
...
void Scaler_color_copy_HDR_table_from_AP(SLR_HDR_TABLE* ptr_HDR_bin)
{
	...
	extern unsigned int HLG_EOTF_LUT_R[EOTF_size];
	extern unsigned short HLG_OETF_LUT_R[OETF_size];
	extern UINT32 HFC_EOTF_LUT_R[4][1025];
	extern unsigned short HFC_OETF_LUT_R[5][1025];
	extern VIP_HDR10_OETF_ByLuminace_TBL HDR10_OETF_ByLuminace_TBL;
	...
	return;
#ifndef	CONFIG_CUSTOMER_TV002
	//memcpy(hdr_table, ptr_HDR_bin->hdr_table, sizeof(hdr_table));
	//memcpy(hdr_RGB2OPT, ptr_HDR_bin->hdr_RGB2OPT, sizeof(hdr_RGB2OPT));
#endif
	memcpy(HDR10_3DLUT_17x17x17, ptr_HDR_bin->HDR10_3DLUT_17x17x17, sizeof(HDR10_3DLUT_17x17x17));
	//memcpy(HLG_3DLUT_17x17x17, ptr_HDR_bin->HDR10_3DLUT_17x17x17, sizeof(HLG_3DLUT_17x17x17));
	memcpy(EOTF_LUT_R, ptr_HDR_bin->EOTF_LUT_R, sizeof(EOTF_LUT_R));
	memcpy(OETF_LUT_R, ptr_HDR_bin->OETF_LUT_R, sizeof(OETF_LUT_R));
	...
	memcpy(HFC_EOTF_LUT_R, ptr_HDR_bin->HFC_EOTF_LUT_R, sizeof(HFC_EOTF_LUT_R));
	memcpy(HFC_OETF_LUT_R, ptr_HDR_bin->HFC_OETF_LUT_R, sizeof(HFC_OETF_LUT_R));
	...
	memcpy(TCHDR_Table, ptr_HDR_bin->TCHDR_Table, sizeof(TCHDR_Table));
	...
	memcpy(TCHDR_v130_D_Table_settings, ptr_HDR_bin->TCHDR_v130_D_Table_settings, sizeof(TCHDR_v130_D_Table_settings));
...
/**table_iDX
 * Level = 0 => no bypass?? * Level = 1 => I DCTI?．_DCTI?〃_DCTI?．LTI : bypass?? * Level = 2 => I DCTI?．_DCTI?〃_DCTI?．LTI?ヽTNR Y?ヽTNR C?｀CNR?．I IP Enabl
...
void Scaler_recheck_sharpness_table(void)
{
	...
	if (!(source >= VIP_QUALITY_TOTAL_SOURCE_NUM || pVipCoefArray == NULL)) {
		...
		if (Scaler_GetMBPeaking() != pVipCoefArray[VIP_QUALITY_FUNCTION_MB_Peaking]) {
			Scaler_SetMBPeaking(pVipCoefArray[VIP_QUALITY_FUNCTION_MB_Peaking]);
		}
		...
		if(Scaler_GetDCC_Table()!= pVipCoefArray[VIP_QUALITY_FUNCTION_DCC_Table_Select]){
			Scaler_SetDCC_Table(pVipCoefArray[VIP_QUALITY_FUNCTION_DCC_Table_Select]);
		}
         	 if(Scaler_Get_DCC_Color_Independent_Table()!=pVipCoefArray[VIP_QUALITY_FUNCTION_DCC_Skin_Tone_Table_Select])
			Scaler_Set_DCC_Color_Independent_Table(pVipCoefArray[VIP_QUALITY_FUNCTION_DCC_Skin_Tone_Table_Select]);
		if(Scaler_Get_DCC_chroma_compensation_Table()!=pVipCoefArray[VIP_QUALITY_FUNCTION_DCC_Chroma_comp_t_sel])
			Scaler_Set_DCC_chroma_compensation_Table(pVipCoefArray[VIP_QUALITY_FUNCTION_DCC_Chroma_comp_t_sel]);
		Scaler_SetDCC_Mode(Scaler_GetDCC_Mode());
	}
}
...
{
	if(fwif_VIP_get_Project_ID() == VIP_Project_ID_TV030){
		rtd_printk(KERN_EMERG, "OD", "Do not Scaler_Set_OD_Bit_By_XML!! \n");
		return TRUE;
	}
...
void Scaler_color_set_RTNR_H_Rotate(unsigned char display, unsigned char En)
{
	...
	if (display == SLR_MAIN_DISPLAY) {
		...
		rtd_printk(KERN_INFO, TAG_NAME, "RTNR_H_Rotate, display=%d, En=%d\n",
			display, En);
...
void Scaler_color_decide_RTNR_H_Rotate(unsigned char display)
{
	...
	if (display == SLR_MAIN_DISPLAY) {
		...
		if ((get_rotate_hflip_flag(SLR_MAIN_DISPLAY) == 0)  || (isInterlace) || (!DI_10bit))
			Scaler_color_access_RTNR_H_Rotate_Flag(0, 1);
		else
			Scaler_color_access_RTNR_H_Rotate_Flag(1, 1);
		rtd_printk(KERN_INFO, TAG_NAME, "RTNR_H_Rotate,decide, display=%d, isInterlace=%d, DI_10bit=%d, En=%d\n",
				display, isInterlace, DI_10bit, get_rotate_hflip_flag(SLR_MAIN_DISPLAY));
	}
...
void Scaler_color_Dither_Table_Ctrl(void)
{
	...
	for(nIdex = 0;nIdex<VIP_DITHER_IP_MAX;nIdex++ )
	{
		...
		switch(nIdex) {
			...
			case VIP_DITHER_IP_PANEL:
				if(fwif_VIP_get_Project_ID() != VIP_Project_ID_TV030){
					rtd_printk(KERN_EMERG, "paneldither", "qua handler,set paneldither table!! \n");
...
void Scaler_SetNNSR_Ctrl_Table(unsigned char TableIdx)
{
	...
	if (pTable == NULL) {
		rtd_printk(KERN_DEBUG, TAG_NAME, " NNSR Scaler_SetNNSR_Ctrl_Table error %d \n", TableIdx);
		return;
	}
...
void Scaler_Set_Camera_PQsetting(unsigned char get_vo_camera_flow_flag)
{
	...
	printk("[srgb]Scaler_Set_Camera_PQsetting(%d) \n",get_vo_camera_flow_flag);
  if(get_vo_camera_flow_flag){
    rtd_printk(KERN_EMERG, TAG_NAME, "get_vo_camera_flow_flag : ON\n");
    ...
    printk("[srgb]get_vo_camera_flow_flag : OFF\n");
    /* set gamma */
    IoReg_SetBits(GAMMA_GAMMA_CTRL_2_reg, 0x00000040);//_BIT6
    IoReg_SetBits(INV_GAMMA_INV_GAMMA_CTRL_2_reg, 0x00000040);//_BIT6
    /* set DCC */
    ...
    IoReg_Write32(YUV2RGB_D_YUV2RGB_Control_reg, d_yuv2rgb_control_REG.regValue);
    /* set SNR_Y */
    dch1_cp_ctrl_reg.cp_spatialenabley = Ori_SNR_spatialenabley;
	IoReg_Write32(NR_DCH1_CP_Ctrl_reg, dch1_cp_ctrl_reg.regValue);
	IoReg_SetBits(NR_SNR_DB_CTRL_reg, NR_SNR_DB_CTRL_snr_db_apply_mask); /* DB apply */
    /* set De_Contour */
    drvif_color_Set_I_De_Contour_DB_En( 1 );
	ipq_decontour_de_contour_ctrl_reg.decont_en=Ori_I_De_Contour_en;
    IoReg_Write32(IPQ_DECONTOUR_De_Contour_CTRL_reg, ipq_decontour_de_contour_ctrl_reg.regValue);
    drvif_color_Set_I_De_Contour_DB_Apply();
    /* set block-decontour */
...
void fwif_color_get_VDBlackLevel_from_vip_table(unsigned char *Contrast, unsigned char *Brightness, unsigned char *Hue, unsigned char *Sat)
{
	SLR_VIP_TABLE *gVip_Table = fwif_colo_get_AP_vip_table_gVIP_Table();
	unsigned char source = 255;
	unsigned char *pVipCoefArray = NULL;
	unsigned char VD_tbl_inx = 0;
	source = fwif_vip_source_check(3, NOT_BY_DISPLAY);
	pVipCoefArray = (unsigned char *)m_cbFunc_getQualityExtend3((VIP_SOURCE_TIMING)source);
	VD_tbl_inx = pVipCoefArray[VIP_QUALITY_FUNCTION_VD_ConBriHueSat_Table];
	*Contrast = gVip_Table->VD_ConBriHueSat[VD_tbl_inx].VD_contrast;
	*Brightness = gVip_Table->VD_ConBriHueSat[VD_tbl_inx].VD_brightness;
	*Hue = gVip_Table->VD_ConBriHueSat[VD_tbl_inx].VD_hue;
	*Sat = gVip_Table->VD_ConBriHueSat[VD_tbl_inx].VD_saturation;
}
