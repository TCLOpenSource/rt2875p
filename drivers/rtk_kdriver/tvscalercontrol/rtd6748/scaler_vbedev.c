...
extern void tconless_CS602_cs603_reserve(void);
extern void tconless_cs602_cs603_reset(void);
char tconless_cs602_cs603_reset_is_done = 0;
static DEFINE_SEMAPHORE(panel_on_flow_sema);
...
unsigned char vbe_disp_decide_frc2fsync_using_I2D(void)
{
	if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 235) &&
		...
		&& (vbe_disp_get_VRR_timingMode_flag()|| vbe_disp_get_freesync_mode_flag() || get_scaler_qms_mode_flag())){//VRR
		return FALSE;
	}
	return TRUE;
}
...
void scaler_disp_dtg_set_s1ip_dleay(unsigned char bEnable_DB)
{
    ...
    if((Get_DISPLAY_PANEL_OLED_TYPE() == TRUE))
	{
		...
		{
			rtd_pr_vbe_notice("[%s]slow tuning s1ip delay\n",__FUNCTION__);
			while(s1ip_delay != ppoverlay_uzudtg_control3_reg.s1ip_max_line_delay)
			{
				timeout = 10;
				
				if((ppoverlay_uzudtg_control3_reg.s1ip_max_line_delay-s1ip_delay) > SLOW_TUNNING_OLED_OFFSET)
				{
					ppoverlay_uzudtg_control3_reg.s1ip_max_line_delay -= SLOW_TUNNING_OLED_OFFSET; 
				}
				else
				{
					ppoverlay_uzudtg_control3_reg.s1ip_max_line_delay = s1ip_delay;
				}
				rtd_pr_vbe_notice("[%s]slow tuning s1ip_max_line_delay=%d\n",__FUNCTION__, ppoverlay_uzudtg_control3_reg.s1ip_max_line_delay);
				IoReg_Write32(PPOVERLAY_uzudtg_control3_reg, ppoverlay_uzudtg_control3_reg.regValue);
				if(bEnable_DB)
				{
					ppoverlay_double_buffer_ctrl2_reg.regValue = IoReg_Read32(PPOVERLAY_Double_Buffer_CTRL2_reg);
...
unsigned int scaler_disp_dtg_get_time_borrow_value(void)
{
	...
	}else if((get_panel_res() == PANEL_RES_FHD) || (get_panel_res() == PANEL_RES_HD) || (Get_DISP_DEN_STA_VPOS() < (DTG_DEFAULT_TIME_BORROW_V+_M_PRE_READ_MARGIN)))
...
*/
	...
	//		Scaler_MEMC_DTV_ME_CLK_OnOff(0, 1);
			...
			rtd_pr_vbe_notice("[Scaler_disp_setting][AA][%d,%d]\n", ((rtd_inl(PPOVERLAY_MEMC_MUX_CTRL_reg)>>13)&0x1), Get_tv006_wb_pattern());
		#endif
	...
	//		vbe_dtg_memec_bypass_switch_set(1);
			rtd_pr_vbe_notice("[Scaler_disp_setting][BB][%d,%d,%d]\n", ((rtd_inl(PPOVERLAY_MEMC_MUX_CTRL_reg)>>13)&0x1), Get_tv006_wb_pattern(), drv_memory_get_vdec_direct_low_latency_mode());
			//Scaler_MEMC_Bypass_CLK_OnOff(0);
	...
	if((Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI)){
		...
		if((FALSE == Get_Val_first_boot_run_main())/*||(drvif_scalerdisplay_get_force_bg_status())*/){
...
void HAL_VBE_DISP_SetDisplayOutput(BOOLEAN bOnOff)
{
	...
	if(bOnOff)
	{
		...
		if((Get_DISPLAY_PANEL_TYPE() == P_CEDS)||(Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE)||(Get_DISPLAY_PANEL_TYPE() == P_CMPI)||(Get_DISPLAY_PANEL_TYPE() == P_ISP)){
			msleep(25);
...
void vbe_disp_panel_suspend(void)
{
	...
	else if((Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE)||(Get_DISPLAY_PANEL_TYPE() == P_CEDS)||(Get_DISPLAY_PANEL_TYPE() == P_CMPI))
	{
    ...
    else if(Get_DISPLAY_PANEL_TYPE() == P_ISP)
    {
        ...
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING04_00_S_R =	IoReg_Read32(ISP_ISP_SETTING04_00_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING04_04_S_R =	IoReg_Read32(ISP_ISP_SETTING04_04_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING04_08_S_R =	IoReg_Read32(ISP_ISP_SETTING04_08_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING05_00_S_R =	IoReg_Read32(ISP_ISP_SETTING05_00_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING05_04_S_R =	IoReg_Read32(ISP_ISP_SETTING05_04_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING05_08_S_R =	IoReg_Read32(ISP_ISP_SETTING05_08_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING06_00_S_R =	IoReg_Read32(ISP_ISP_SETTING06_00_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING06_04_S_R =	IoReg_Read32(ISP_ISP_SETTING06_04_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING06_08_S_R =	IoReg_Read32(ISP_ISP_SETTING06_08_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING07_00_S_R =	IoReg_Read32(ISP_ISP_SETTING07_00_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING07_04_S_R =	IoReg_Read32(ISP_ISP_SETTING07_04_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING07_08_S_R =	IoReg_Read32(ISP_ISP_SETTING07_08_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING08_00_S_R =	IoReg_Read32(ISP_ISP_SETTING08_00_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING08_04_S_R =	IoReg_Read32(ISP_ISP_SETTING08_04_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING08_08_S_R =	IoReg_Read32(ISP_ISP_SETTING08_08_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING09_00_S_R =	IoReg_Read32(ISP_ISP_SETTING09_00_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING09_04_S_R =	IoReg_Read32(ISP_ISP_SETTING09_04_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING09_08_S_R =	IoReg_Read32(ISP_ISP_SETTING09_08_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING10_S_R =	IoReg_Read32(ISP_ISP_SETTING10_reg);
        vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING11_S_R =	IoReg_Read32(ISP_ISP_SETTING11_reg);
...
void vbe_disp_panel_resume(void)
{
	...
	else if(Get_DISPLAY_PANEL_TYPE() == P_ISP)
	{
        ...
        IoReg_Write32(ISP_ISP_SETTING04_00_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING04_00_S_R);
        IoReg_Write32(ISP_ISP_SETTING04_04_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING04_04_S_R);
        IoReg_Write32(ISP_ISP_SETTING04_08_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING04_08_S_R);
        IoReg_Write32(ISP_ISP_SETTING05_00_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING05_00_S_R);
        IoReg_Write32(ISP_ISP_SETTING05_04_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING05_04_S_R);
        IoReg_Write32(ISP_ISP_SETTING05_08_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING05_08_S_R);
        IoReg_Write32(ISP_ISP_SETTING06_00_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING06_00_S_R);
        IoReg_Write32(ISP_ISP_SETTING06_04_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING06_04_S_R);
        IoReg_Write32(ISP_ISP_SETTING06_08_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING06_08_S_R);
        IoReg_Write32(ISP_ISP_SETTING07_00_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING07_00_S_R);
        IoReg_Write32(ISP_ISP_SETTING07_04_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING07_04_S_R);
        IoReg_Write32(ISP_ISP_SETTING07_08_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING07_08_S_R);
        IoReg_Write32(ISP_ISP_SETTING08_00_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING08_00_S_R);
        IoReg_Write32(ISP_ISP_SETTING08_04_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING08_04_S_R);
        IoReg_Write32(ISP_ISP_SETTING08_08_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING08_08_S_R);
        IoReg_Write32(ISP_ISP_SETTING09_00_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING09_00_S_R);
        IoReg_Write32(ISP_ISP_SETTING09_04_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING09_04_S_R);
        IoReg_Write32(ISP_ISP_SETTING09_08_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING09_08_S_R);
        IoReg_Write32(ISP_ISP_SETTING10_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING10_S_R);
        IoReg_Write32(ISP_ISP_SETTING11_reg, vbe_disp_panel_isp_suspend_resume.ISP_ISP_SETTING11_S_R);
...
void vbe_disp_suspend(void)
{
	...
	tconless_cs602_cs603_reset_is_done = 0;
	rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] tconless_cs602_cs603_reset_is_done = %d \n",__FUNCTION__,tconless_cs602_cs603_reset_is_done);
}
...
void vbe_disp_init_flow_finish(void)
{
	...
	if((Get_DISPLAY_PANEL_TYPE() == P_CEDS)||(Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE)||
        (Get_DISPLAY_PANEL_TYPE() == P_VBY1_4K2K)||(Get_DISPLAY_PANEL_TYPE() == P_ISP)){
        //control on HAL_VBE_DISP_SetDisplayOutput
    }else
	    vbe_disp_lane_power_on();
}

static bool vbe_panel_tsk_running_flag = FALSE;//Record vbe_panel_tsk status. True: Task is running
static struct task_struct *p_vbe_panel_tsk = NULL;

static int vbe_resume_panel_tsk(void *p)//This task run to set panel on sequence
{
    rtd_pr_vbe_print(KERN_NOTICE, "####[%s] thread start...####\n",__FUNCTION__);
    while (!kthread_should_stop() && vbe_panel_tsk_running_flag == TRUE)
    {
        if(down_interruptible(&panel_on_flow_sema))
            rtd_pr_vbe_print(KERN_NOTICE, "interrupted, no semaphore held\n");
        //vbe_panel_tsk_running_flag = FALSE;
        Panel_resume_TurnOn();
        //msleep(10); // sleep 10 ms
    }
    rtd_pr_vbe_print(KERN_NOTICE, "####[%s] thread end...####\n",__FUNCTION__);
    vbe_panel_tsk_running_flag = FALSE;
    do_exit(0);
    return 0;
}

static void vbe_resume_Panel_TurnOn(void)
{
    int err;
    rtd_pr_vbe_print(KERN_NOTICE, "####vbe_panel_tsk_running_flag=%d...####\n",vbe_panel_tsk_running_flag);
    if (vbe_panel_tsk_running_flag == FALSE) {
        p_vbe_panel_tsk = kthread_create(vbe_resume_panel_tsk, NULL, "vbe_resume_panel_tsk");
        if (p_vbe_panel_tsk)
        {
            vbe_panel_tsk_running_flag = TRUE;
            msleep(10);
            wake_up_process(p_vbe_panel_tsk);
        }
        else
        {
            err = PTR_ERR(p_vbe_panel_tsk);
            rtd_pr_vbe_print(KERN_NOTICE, "Unable to start vbe_resume_panel_tsk (err_id = %d)./n", err);
        }
    }
    else
        up(&panel_on_flow_sema);
}
...
//	ppoverlay_dispd_smooth_toggle1_RBUS ppoverlay_dispd_smooth_toggle1_reg;
    ...
    vbe_resume_Panel_TurnOn();
    rtd_pr_vbe_print(KERN_NOTICE, "[%s] %d\n", __FUNCTION__, __LINE__);
...
static int vbe_resume_instanboot (struct device *p_dev)
{
    ...
    if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 15)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 239)){
        rtd_pr_vbe_print(KERN_NOTICE, " 8051 wake up !!!\n");
        rtd_outl(0xB8060044, _BIT9);
        rtd_outl(0xB8060034, _BIT9);      // rst on
        rtd_outl(0xB8060044, _BIT9|_BIT0);
        rtd_outl(0xb8060110, 0x2379c0c0);
        rtd_outl(0xB8060034, _BIT9|_BIT0);
    }
#if 0	//remove tve
	//VBE_AVE resume
	if(VBE_AVE_Global_Status == VBE_AVE_OPEN_DONE){
...
void vbe_disp_orbit_justscan_HW_Ctrl(int shift_x, int shift_y, unsigned char justscan_position, unsigned char bEnable_DB)
{
	...
	if(((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1)||force_enable_two_step_uzu() || get_hdmi_vrr_4k60_mode() || (ppoverlay_uzudtg_control1_reg.uzu_4k_120_mode == 1)) && (Get_DISPLAY_REFRESH_RATE() == 120)/* && justscan_position ==1*/)
	{
		if(shift_x <= 0)
		{
			if((shift_x%2) ==0)
...
UINT32 vbe_disp_get_VRR_timing_real_vtotal(void)
{
    ...
    if(vertical_total == 0)
    {
    	if(Get_DISPLAY_REFRESH_RATE() == 165){
			...
			{
				rtd_pr_vbe_notice("[%s] vertical_total is wrong(%d), input is 240Hz/288Hz, set 240Hz/288Hz to protect vtotal!!!\r", __FUNCTION__, vertical_total);
				vertical_total = Get_DISP_VERTICAL_TOTAL();
			}
			else
			{
	...
	switch (cmd)
	{
		...
		case VBE_IOC_PANEL_MODE_SET:
		{
			...
			else
			{
                ...
                else if(vbe_get_cur_dlg_mode())
                {
                    vbe_set_TC901_hsr_mode(1);
                }
                if(panel_set.ctrl_set == 0)
                {
                    if (panel_set.power_enable == false)
                    {
                        GDMA_Clear();
                        if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 15)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 239)){
                            IoReg_Write32(0xb8060110, 0);
                            msleep(1);
                            IoReg_Write32(0xB8060044, _BIT9);
                            IoReg_Write32(0xB8060034, _BIT9);      // rst off
                            IoReg_Write32(0xB8060044, _BIT9|_BIT0);
                            rtd_pr_vbe_print(KERN_NOTICE, " 8051 close  !!!\n");
                            msleep(10);
                        }
                        tconless_CS602_cs603_reserve(); //mark by max
                        vbe_disp_lane_power_off();

                        if((pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 0xE4)
                         ||(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 0xE6)
                         ||(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 0xE9)
                         ||(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 0xEB))
                        {
                            Panel_SetPanelOnOff(0);
                        }

                        tconless_cs602_cs603_reset_is_done = 0;
                        rtd_pr_vbe_print(KERN_NOTICE, "[%s] tconless_cs602_cs603_reset_is_done = %d \n",__FUNCTION__,tconless_cs602_cs603_reset_is_done);
                    }
                    else
                    {
                        if(tconless_cs602_cs603_reset_is_done==1)
                            break;
                        tconless_cs602_cs603_reset();
                        vbe_disp_lane_power_on();
                        if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 15)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 239)){
                            //msleep(100);
                            rtd_outl(0xB8060044, _BIT9);
