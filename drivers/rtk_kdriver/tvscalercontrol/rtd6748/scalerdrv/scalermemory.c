...
#define MAIN_FIFO_GATE_TH 500;//DIC suggestion for k8 
...
{//3k 4k use data frc need borrow memory
	...
	if ((!Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC))
			&& ((unsigned int)(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE) * Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE)) >  ((unsigned int)M_DOMAIN_WIDTH_CARVEOUT * M_DOMAIN_LEN_CARVEOUT)
...
//	unsigned int  TotalSizeFrame=0;
	...
	if(((vsc_get_adaptivestream_flag(SLR_MAIN_DISPLAY))||(m_dispwin.srcy <= 100)) && (!drv_memory_get_vdec_direct_low_latency_mode() || ((Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI)))) {
...
unsigned int drv_get_caveout_buffer_size_by_platform(unsigned int pixel_bits)
{
...
#ifndef CONFIG_RTK_BUSINESS_DISPLAY_MODEL
    ...
    if (Get_rotate_function(SLR_MAIN_DISPLAY)){
	    width = M_DOMAIN_WIDTH_BUSINESS_DISPLAY;
	    len = M_DOMAIN_LEN_BUSINESS_DISPLAY;
    } else {
        width = M_DOMAIN_WIDTH_CARVEOUT_2K;
        len = M_DOMAIN_LEN_CARVEOUT_2K;
    }
	...
	if (display == SLR_MAIN_DISPLAY)	 {
...

		{
			...
			// 4k60 vrr/game mode
				...
				//under 2k timing only use 2k16bit buffer size
				if ((dvrif_memory_get_compression_bits(SLR_MAIN_DISPLAY)==COMPRESSION_16_BITS)
					||(dvrif_memory_get_compression_bits(SLR_MAIN_DISPLAY)==COMPRESSION_18_BITS)
				    ||(dvrif_memory_get_compression_bits(SLR_MAIN_DISPLAY)==COMPRESSION_20_BITS)){
...
//-		rtd_pr_scaler_memory_debug("\n Allocate Sub Disp Memory !\n");
		...
		if(get_hdmi_4k_hfr_mode()== HDMI_4K120_2_1)
		{
			...
			IoReg_Write32(MDOMAIN_CAP_DDR_In2_3rdAddr_reg, nStartAddr & 0xfffffff0);
		//}
	#endif

		if(bDouble)
		{
			IoReg_Write32(MDOMAIN_CAP_DDR_In2_2ndAddr_reg, drvif_memory_alloc_block(MEMIDX_SUB_SEC,buffer_size, MEMALIGN_ANY)& 0xfffffff0);
		}
...
//-		rtd_pr_scaler_memory_debug("Get allocated sub display addr1=%x, addr2=%x, size=%x\n", StartAddr, StartAddr1, TotalSize);
	...
	if(display){
		...
		mdomain_cap_ddr_in2ctrl_reg.in2_freeze_enable = 0;
...
void dvrif_memory_handler(unsigned char display)
{
	...
	if (display == SLR_MAIN_DISPLAY) {	// Main
		...
		if((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) && (get_panel_pixel_mode() > PANEL_1_PIXEL))
...
//remove in k4lp
		...
		if(((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_ADC)
			...
			&&(!is_game_mode_keep_buffer_mode())&&(!drvif_i3ddma_triplebuf_by_timing_protect_panel()) && (display == SLR_MAIN_DISPLAY)
...
void dvrif_memory_comp_setting(UINT8 enable_compression, UINT16 comp_wid, UINT16 comp_len, UINT8 comp_ratio){
	...
	if(enable_compression == TRUE){
		...
		*/
		...
		if(Get_PANEL_VFLIP_ENABLE() || get_hdmi_vrr_4k60_mode())
