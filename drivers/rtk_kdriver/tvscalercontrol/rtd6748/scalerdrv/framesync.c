    ...
    if(get_panel_pixel_mode() > PANEL_1_PIXEL)
    {
        if((removeRatio == 1) && (multipleRatio == 1))
        {
            if(Get_DISPLAY_REFRESH_RATE() == 120)
            {
                ...
                //2k1k 288 input, panel is 4k2k120, output framerate is 120Hz
                else if((intputFrameRate >= 28750) && (intputFrameRate <= 28850))
                {
                    removeRatio = 36;
                    multipleRatio = 15;
                }
            ...
            {
                //2k1k 200 or 240 input, panel is 4k2k144, output framerate is 144Hz
                if((intputFrameRate > 23950 && intputFrameRate < 24050)
                        || (intputFrameRate > 19950 && intputFrameRate < 20050))
                {
                    removeRatio = 15;
                ...
                //2k1k 288 input, panel is 4k2k144, output framerate is 144Hz
                else if((intputFrameRate >= 28750) && (intputFrameRate <= 28850))
                {
                    removeRatio = 2;
                    multipleRatio = 1;
            ...
            {
                if((intputFrameRate >= 28750) && (intputFrameRate <= 28850))
                {
                    removeRatio = 18;
                    multipleRatio = 15;
                }
    ...
    else
    {
        if((removeRatio == 1) && (multipleRatio == 1))
        {
            if(Get_DISPLAY_REFRESH_RATE() == 60)
            {
                ...
                //2k1k144 input, 4k2k60Hz panel, output framerate 60
                else if(intputFrameRate > 14350 && intputFrameRate <= 14450)
                {
                    removeRatio = 36;
                    multipleRatio = 15;
                }
...
//	extern SMT_STATUS_UNDER_GAME_MODE zoom_smoothtoggle_gamemode_get_smt_status(unsigned char display);
    ...
    if(dispTimingMode == DISPLAY_MODE_RESET_VODMA){
		...
		if(drv_memory_get_low_delay_game_mode_dynamic()){//if game mode case, need to use calculation result @Crixus 20170712
			...
			#endif
				else if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) && (!drv_memory_get_vdec_direct_low_latency_mode())){  //vdec no need calculate iv2dv delay
					fs_iv_dv_fine_tuning5_reg.iv2dv_line = 2;
					fs_iv_dv_fine_tuning5_reg.iv2dv_line_2 = 2;
...
#endif
    if ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) != VSC_INPUTSRC_VDEC) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE) > DATAFS_VDEC_RUN_DATAFS_WIDTH) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) > DATAFS_VDEC_RUN_DATAFS_HEIGHT)){
