...
void drvif_scalerdisplay_set_bg_color(unsigned char display, eDISPLAYBORDERTYPE BorderType, unsigned int ColorR, unsigned int ColorG, unsigned int ColorB)
{
	...
	rtd_pr_vbe_debug("#########Main DISPLAY_BG Color is (%x,%x,%x)\n",ColorR,ColorG,ColorB);
...
#ifdef CONFIG_I2RND_ENABLE
		//Eric@0808 Dual vo i2rnd set sub path display size
...
unsigned char fw_scaler_cal_sdnr_capture(unsigned char display)
{
	...
	if(Scaler_InputRegionType_Get(display) == INPUT_BASE_ON_DISP_WIN){
...
#ifdef CONFIG_I2RND_ENABLE
		//Eric@20170918 i2rnd sub rerun main flow use sub input size to calc sdnr size
		if((Scaler_I2rnd_get_timing_enable() && ((vsc_i2rnd_dualvo_sub_stage == I2RND_DUALVO_STAGE_RERUN_MAIN_FOR_S1) || (vsc_i2rnd_sub_stage == I2RND_STAGE_MAIN_GET_RPC)))){
			newInput.srcx = Sub_InputRegion_x_temp;
			newInput.srcy = Sub_InputRegion_y;
			newInput.src_wid = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DI_WID);
			newInput.src_height = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DI_LEN);

			rtd_pr_vbe_emerg("[crixus][sdnr]newInput.srcx = %d\n", newInput.srcx);
			rtd_pr_vbe_emerg("[crixus][sdnr]newInput.srcy = %d\n", newInput.srcy);
			rtd_pr_vbe_emerg("[crixus][sdnr]newInput.src_wid = %d\n", newInput.src_wid);
			rtd_pr_vbe_emerg("[crixus][sdnr]newInput.src_height = %d\n", newInput.src_height);
		}
#endif
		...
		//else
		{
			...
			if (Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE)) {
				...
				if ((Main_InputRegion_h == 0) || (Main_InputRegion_h > (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPV_ACT_LEN_PRE)<<1)))/*Not reasonable input*/
  				...
  				if(Scaler_InputSrcGetMainChType() == _SRC_VO && get_vsc_run_adaptive_stream(display)){
					StructSrcRect m_dispwin = Scaler_DispWindowGet();
					StructSrcRect m_input_size = Scaler_AdaptiveStream_InputWindowGet(display);
...
//	sfg_sfg_ctrl_c_RBUS    sfg_ctrl_c_reg;
	...
	sfg_ctrl_7_reg.regValue = IoReg_Read32(SFG_SFG_CTRL_7_reg);
	sfg_ctrl_7_reg.hsync_width = Get_DISP_HSYNC_WIDTH();//HS_Width = 16 dclk = 16*Pixel_Mode*n DH_Width (must be 4 * n and n >= 1 where n is a integer)
	sfg_ctrl_7_reg.hsync_delay = Get_DISP_HORIZONTAL_TOTAL() -13;
	IoReg_Write32(SFG_SFG_CTRL_7_reg, sfg_ctrl_7_reg.regValue);
...
unsigned int scalerdisplay_get_vtotal_by_vfreq(UINT16 vsyncFreq)
{
	...
	//else if ((vsyncFreq > 60) && (Get_DISPLAY_REFRESH_RATE() != 144) && (Get_DISPLAY_REFRESH_RATE() != 165))
	//	vsyncFreq /= 2;
#if 0
...
//	if(vertical_total==0){
	if (0 != h_total)
    {
        if(get_panel_pixel_mode() > PANEL_1_PIXEL)
        {
            if(vsyncFreq > 60)
            {
                vertical_total = Get_DISPLAY_CLOCK_TYPICAL() / (h_total * vsyncFreq / 2);
            }
            else
            {
                vertical_total = Get_DISPLAY_CLOCK_TYPICAL() / (h_total * vsyncFreq);
            }
        }
        else
        {
	...
	rtd_pr_vbe_notice("######## vertical_total:%d ###############\n",
		vertical_total);
...
{
    extern void forcebg_memc_force_unfreeze(void);
	unsigned long flags = 0;//for spin_lock_irqsave
	...
	{
		rtd_pr_vbe_notice("%s,Disp[%d]=%d WB=%d\n", __FUNCTION__, channel, enable, Get_tv006_wb_pattern());
		return;
	}
  ...
  if(enable) //different algo. enable:new   disable:old
  {
	if(channel == SLR_MAIN_DISPLAY){//main path
        ...
        forcebg_memc_force_unfreeze();//check memc unfreeze
//#ifdef CONFIG_ORBIT_PIXEL_SHIFT_ALGO
...
{//chnage 0xb802850c and 0xb8028510 delay
	...
	{// only 120 panel need to change the setting
		if(force_enable_two_step_uzu() || (get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1))
		{
