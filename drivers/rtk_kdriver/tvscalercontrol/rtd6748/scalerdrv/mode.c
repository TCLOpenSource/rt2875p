...
//#endif
	...
	if(src == VSC_INPUTSRC_HDMI)
	{
		...
		else
		{
			...
			hdmi_framerate = Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ);
			if(hdmi_framerate)
				onms_onms3_demode_vs_uf_reg.on3_de_vsper_uf_th = ((unsigned int)CONFIG_ONLINE_XTAL_FREQ*(unsigned int)98)/hdmi_framerate/10;//for panel compability
			else
				onms_onms3_demode_vs_uf_reg.on3_de_vsper_uf_th = 0;
...
//	sub_vgip_vgip_chn2_act_hsta_width_RBUS vgip_chn2_act_hsta_width_reg;
	...
	if (display == SLR_MAIN_DISPLAY) {
		...
		if (RPC_VIP_sysInfo) {
            pr_info("DM_freeze, reset mode, unfreeze,\n");
            MEMC_Lib_Freeze(0);
