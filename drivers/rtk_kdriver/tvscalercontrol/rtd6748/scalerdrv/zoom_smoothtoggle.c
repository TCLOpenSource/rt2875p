...
unsigned char imd_smooth_framesync_gatting_do(unsigned char display)
{
	...
	if(!((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_IPH_ACT_WID_PRE)> DATAFS_DISABLE_GATTING_WIDTH) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_IPV_ACT_LEN_PRE) > DATAFS_DISABLE_GATTING_WIDTH)))
#endif
	...
	if (sm_info_pkg) {
        ...
        if(true == update_info.sm_info[display].done_flag){
			...
			if (0==scaler_before) {//after scaler need to update smoothtoggle
				...
				if((display == SLR_MAIN_DISPLAY) && ((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) ||(get_hdmi_vrr_4k60_mode())))
				...
				Scaler_DispSetScaleStatus((SCALER_DISP_CHANNEL)display, SLR_SCALE_V_UP, update_info.sm_info[display].smooth_toggle_info.IMD_SMOOTH_V_SCALE_UP);
				Scaler_DispSetScaleStatus((SCALER_DISP_CHANNEL)display, SLR_SCALE_H_UP, update_info.sm_info[display].smooth_toggle_info.IMD_SMOOTH_H_SCALE_UP);
			}
#ifndef UT_flag
			//vdec source new voinfo,re-config i3ddma nn cap
			if((display == SLR_MAIN_DISPLAY) && ((Get_DisplayMode_Src(display) == VSC_INPUTSRC_VDEC) || (Get_DisplayMode_Src(display) == VSC_INPUTSRC_JPEG)) && (scalerAI_get_AIPQ_mode_enable_flag()) &&
