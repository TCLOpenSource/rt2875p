...
static unsigned char hdmi_dp_force_framerate_flag = 0;
...
#define HDMI_RX_PORT_MAX_NUM 4
static long dolby_hdmi_vsif_mode[HDMI_RX_PORT_MAX_NUM];
static long dolby_h14b_vsif_mode[HDMI_RX_PORT_MAX_NUM];
...
void set_HDMI_Dolby_H14B_VSIF_mode(DOLBY_HDMI_VSIF_T vsif_mode, unsigned char port)
{
	extern unsigned char newbase_hdmi_get_current_display_port(void);
	unsigned char current_port = newbase_hdmi_get_current_display_port();

	if (port >= HDMI_RX_PORT_MAX_NUM)
		return;

	if (dolby_h14b_vsif_mode[port] != vsif_mode) {
		pr_info("[Dolby][%s:%d] (%ld -> %d) port (%d %d)\n", __func__, __LINE__
				, dolby_h14b_vsif_mode[port]
				, vsif_mode
				, port
				, current_port
		       );
	...
	dolby_h14b_vsif_mode[port] = vsif_mode;
...
void set_HDMI_Dolby_VSIF_mode(DOLBY_HDMI_VSIF_T vsif_mode, unsigned char port)
{
	extern unsigned char newbase_hdmi_get_current_display_port(void);
	unsigned char current_port = newbase_hdmi_get_current_display_port();
	...
	if (dolby_hdmi_vsif_mode[port] != vsif_mode) {
		pr_info("[Dolby][%s:%d] (%ld -> %d) port %d %d\n", __func__, __LINE__
				, dolby_hdmi_vsif_mode[port]
				, vsif_mode
				, port
				, current_port
		       );
	...
	dolby_hdmi_vsif_mode[port] = vsif_mode;
...
DOLBY_HDMI_VSIF_T get_HDMI_Dolby_VSIF_mode(void)
{
	...
	flag_dolby_vsif = dolby_hdmi_vsif_mode[current_port];

	flag_dolby_h14b_vsif = dolby_h14b_vsif_mode[current_port];
...
#define VODMA_IV2DV_0_RESET_TARGET_CP_OFFSET 0
...
//	speedup_rerun_resetvo_by_90K = (90000*10/Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ));//*3/4; //3/4 frame time
    ...
    if(bUseIv2dv0)
    {
       ...
       /* --- Restore PQ for delay time --- */
	// iv2dv need set after reset vodma
	framesync_set_iv2dv_delay(DISPLAY_MODE_FRAME_SYNC);

	if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI){
		if(drvif_get_hdmi_dp_force_framerate_flag())
		{
			rtd_pr_vbe_notice("[%s] hdmi does not enable vodma tracking i3 because frame rate frc case\n", __FUNCTION__);
		}
		else
...
//		drvif_scaler_enable_frc2fsync_interrupt(0);
...
#endif
	{
		...
		if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 235) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 245)){
...
//			Scaler_DispSetInputInfo(SLR_INPUT_DI_WID,Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID));
			...
			if(_DISP_WID == 1920 && _DISP_LEN == 1080) {
				...
				if(Get_rotate_function(SLR_MAIN_DISPLAY)&&(get_rotate_curState(SLR_MAIN_DISPLAY)==MAIN_ROTATE_INIT_UZD)){
					...
					tmpwid = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_CAP_WID);
					tmplen = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_CAP_LEN);
					if(((tmpwid > carveout_width) || (tmplen>carveout_len)))
					{
						if((get_rotate_mode(SLR_MAIN_DISPLAY)==(DIRECT_VO_FRAME_ORIENTATION)ROTATE_MODE_90)||(get_rotate_mode(SLR_MAIN_DISPLAY)==(DIRECT_VO_FRAME_ORIENTATION)ROTATE_MODE_270))
						{
							tmpwid = min(Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DISP_LEN),Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID));
							...
							if(tmpwid >= carveout_width)
							{
								rotatetmp=carveout_width;
								Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_CAP_WID,rotatetmp);
							}

							if(tmplen >= carveout_len)
							{
								rotatetmp=carveout_len;
								Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_CAP_LEN,rotatetmp);
							}
...
//	tRPCShareMem->ulAddr[EMF_COEF_MEM_IDX] = g_atShareMemInfo[SCALERIOC_SETEMF].ulOffset + g_ulPhyAddr + MEM_OFFSET;
	...
	tRPCShareMem->ulAddr[VIDEO_FW_CONTROL_MEMC_FREEZE_IDX] = g_atShareMemInfo[SCALERIOC_VIDEO_FW_CONTROL_MEMC_FREEZE].ulOffset + g_ulPhyAddr + MEM_OFFSET;
	...
	tRPCShareMem->ulAddr[VIDEO_FW_CONTROL_MEMC_FREEZE_IDX] = htonl(tRPCShareMem->ulAddr[VIDEO_FW_CONTROL_MEMC_FREEZE_IDX]);
...
//#endif
		...
		SetRPCShareMem(SCALERIOC_VIDEO_FW_CONTROL_MEMC_FREEZE, sizeof(unsigned int));
...
//		SetRPCShareMem(SCALERIOC_VIP_PQ_DEVICE, sizeof(PQ_device_struct));
		...
		if(g_ulCachedAddr != 0)
		{
			...
			dmac_flush_range((const void *)g_ulCachedAddr, (const void *)(g_ulCachedAddr + g_ulShareMemOffset)); //cache flush to ddr
		...
		if(Get_DISPLAY_REFRESH_RATE() >= 120){//120 panel
			if(Get_DISPLAY_REFRESH_RATE() == 144){
				...
				Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 60);
                        ...
                        if(get_vsc_gamemode_datafrc_mode_flag()==1){
                                Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 60);
			...
			(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) <= 605))	//60hz
	{
		...
		if(Get_DISPLAY_REFRESH_RATE() >= 120){
			if(((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= 994) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) <= 1005))
				...
				rtd_pr_vbe_debug("1/1, 1pixel (input 100Hz/120Hz/240Hz) \n");
				Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_REMOVE_RATIO, 1);
				Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_MULTIPLE_RATIO, 1);
			}
			else{
...
//			UINT32 i = 0, j=0, tmp=0;
            ...
            //i3ddma_timing.v_freq = Scaler_DispGetInputInfo(SLR_INPUT_SRC_FRAMERATE) * 100;
            if( Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)
            {
            ...
            {
                i3ddma_timing.v_freq = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_SRC_FRAMERATE) * 100;
            }
...
unsigned char drivf_i3ddma_config(unsigned char display, unsigned char src)
{
	...
	if(src != VGIP_SRC_TMDS && src != VGIP_SRC_ADC && src != VGIP_SRC_ATV && src != VGIP_SRC_VODMA1)
	{
...
// timing dectect done
	...
	if(Get_PANEL_VFLIP_ENABLE()) {
		if((get_rotate_mode(SLR_MAIN_DISPLAY) == DIRECT_VO_FRAME_ORIENTATION_ROTATE_180) && (Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE) == FALSE)) {
			rtd_pr_vbe_emerg("for i3ddma, vflip progressive rotate 180, send vo vflip=false to vcpu\n");
		...
		if((get_rotate_mode(SLR_MAIN_DISPLAY) == DIRECT_VO_FRAME_ORIENTATION_ROTATE_180) && (Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE) == FALSE)) {
			rtd_pr_vbe_emerg("for i3ddma, progressive rotate 180, send vo vflip=true to vcpu\n");
			rtd_pr_vsc_debug("vflip=%d\n", TRUE);
			scaler_send_rotate_vflip_rpc(TRUE);
		} else {
			rtd_pr_vsc_debug("vflip=%d\n", FALSE);
    ...
    && ((I3DDMA_COLOR_DEPTH_T) Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_COLOR_DEPTH) == I3DDMA_COLOR_DEPTH_8B))
{
    ...
    //i3ddma_timing.v_freq = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_SRC_FRAMERATE) * 100;
	...
	//drvif_I3DDMA_Set_Input_Info_Vaule(I3DDMA_INFO_FRAMERATE_X2, true);
	if((src == VGIP_SRC_TMDS) || (src == VGIP_SRC_VODMA1))
		mode = VGIP_MODE_DIGITAL;
	else
		mode = VGIP_MODE_ANALOG;
...
unsigned int drvif_get_hdmi_dp_force_framerate_flag(void)
{
	return hdmi_dp_force_framerate_flag;
}

void drvif_set_hdmi_dp_force_framerate_flag(unsigned char flag)
{
	hdmi_dp_force_framerate_flag = flag;
}

void decide_hdmi_dp_framerate_for_timing_protect(unsigned char display)
{
    unsigned int ori_framerate = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_SRC_FRAMERATE);
    unsigned int new_framerate = ori_framerate;
    ...
    if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI )
    {
        ...
        {//120 panel
             ...
             && ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) <= HDMI_TO_VO_FRC_60_MAX_LENTH_120_PANEL ) )
            {
                ...
                if(Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) > 95000) // hfr case
                {
                    if((Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) > 139000 && Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) < 149000)
                        && ((Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) <= 143136) || (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) >= 144864)))
                    {
                        new_framerate = 1440;//go 144 hz
                    }
                    else if((Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) > 115000 && Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) < 125000)
                        && ((Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) <= 119100) || (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) >= 120720)))
                    {
                        new_framerate = 1200; //go 120hz
                    }
                    else if((Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) > 95000 && Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) < 105000)
                        && ((Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) <= 99400) || (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) >= 100600)))
                    {
                        new_framerate = 1000; //go 100hz
                    }
                    else
                    {
                        new_framerate = ori_framerate;
                    }
                }
                else
                {
                    if((Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) > 60100)   && (!(( Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag()))))//60.2Hz~99.4Hz //to be compatible with OLED panel
                    {
                        rtd_printk(KERN_INFO, TAG_NAME_VSC, "input frame big than 60 and VO force to 60hz\n");
                        new_framerate = 600;//i domain go 60
                    }
                    ...
                    {
                        rtd_printk(KERN_INFO, TAG_NAME_VSC, "[OLED] input frame bigger than 24, VO force to 24hz\n");
                        new_framerate = 240;//i domain go 24
                    }
                    ...
                    {
                        rtd_printk(KERN_INFO, TAG_NAME_VSC, "[OLED] input frame bigger than 30, VO force to 30hz\n");
                        new_framerate = 300;//i domain go 30
                    }
                    else
                    {
                        new_framerate = ori_framerate;
        ...
        else
        {
             ...
             && ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) <= HDMI_TO_VO_FRC_60_MAX_LENTH_60_PANEL ) )
            {
                ...
                else if((get_MEMC_bypass_status_refer_platform_model() == TRUE ) && get_current_driver_pattern(DRIVER_FREERUN_PATTERN))
                {
                    if ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ) >= 245) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ) <= 255))
                    {
                        rtd_pr_vsc_info("25hz memc bypass, VO force to 50hz\n");
                        new_framerate = 500;
                    } else {
                        rtd_pr_vsc_info("not 25hz, memc bypass, VO force to 60hz\n");
                        new_framerate = 600;
                    }
                }
            }
    ...
    {
        rtd_pr_vsc_info("[%s] interlace timing donnt change frametrate!!\n", __FUNCTION__);
    }

    if(new_framerate != ori_framerate)
    {
        drvif_set_hdmi_dp_force_framerate_flag(TRUE);
        //pass i3ddma will force framerate,otherwise just switch timing freerun
        if(get_force_i3ddma_enable(display))
        {
            Scaler_DispSetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ, new_framerate);
            rtd_pr_vsc_info("[%s] force framerate %d -> %d", __FUNCTION__, ori_framerate, new_framerate);
        }
        else
        {
            rtd_pr_vsc_info("[%s] bypass i3ddma keep framerate but timing will freerun", __FUNCTION__);
        }
    }
    else
    {
...
unsigned char is_game_mode_keep_buffer_mode(void)
{
    if(Get_PANEL_VFLIP_ENABLE() || get_scaler_qms_mode_flag() || drvif_get_hdmi_dp_force_framerate_flag())
    {
        rtd_pr_vsc_info("[%s] game mode dont change buffer mode (%d %d %d)\n", __FUNCTION__,
        Get_PANEL_VFLIP_ENABLE(), get_scaler_qms_mode_flag(), drvif_get_hdmi_dp_force_framerate_flag());
        return TRUE;
    }
    else
    {
...
int drvif_i3ddma_triplebuf_flag(void)
{
	...
	{
		if(get_scaler_qms_mode_flag() || drvif_get_hdmi_dp_force_framerate_flag())
		{//qms use 3 buffer
			rtd_printk(KERN_INFO, TAG_NAME_VSC, " QMS or force framerate i3ddma run 3buffer \n");
			return TRUE;
		}
		else
			return FALSE;
		...
		{//4k2k input
			...
			if((drvif_Hdmi_GetColorSpace() == COLOR_YUV444) || (drvif_Hdmi_GetColorSpace() == COLOR_RGB))
			{
				if(get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1)
					...
					dm_input_format_reg.dm_in_sel = 7;//1pixel 444 3k120
			}
			else
			{
				if(get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1)
					...
					dm_input_format_reg.dm_in_sel = 6;//1pixel 422 3k120
			}
...
void scaler_hdr_mode_setting(unsigned char display, unsigned char hdrtype)
{
	...
	printk("scaler_hdr_mode_setting(hdrtype=%d) \n",hdrtype);
...
VSC_DRIVER_PATTERN vsc_driver_pattern_table[] ={
#ifdef CONFIG_RTK_BUSINESS_DISPLAY_MODEL
  	{HDMI_2K1K_30,		0, 2, 0, 0, 0, 1, 1},
	{HDMI_2K1K_60, 		0, 2, 0, 0, 0, 1, 1},
	{HDMI_2K1K_120,	 	0, 2, 0, 0, 0, 1, 1},
	...
	{VO_2K1K_30, 		0, 2, 0, 0, 1, 1, 1},
	{VO_2K1K_60, 		0, 2, 0, 0, 1, 1, 1},
	{VO_2K1K_120, 		0, 2, 0, 0, 1, 1, 1},
	...
	{CP_2K1K_30, 		0, 2, 0, 1, 1, 1, 1},
	{CP_2K1K_60, 		0, 2, 0, 1, 1, 1, 1},
	{CP_2K1K_120, 		0, 2, 0, 0, 1, 1, 1},
	...
	{JPEG_2K1K_30, 		0, 2, 0, 0, 1, 0, 0},
	{JPEG_4K2K_30, 		0, 2, 0, 1, 1, 0, 0},
	{JPEG_8K4K_30, 		0, 2, 0, 1, 1, 0, 0},
	{TIMING_INVAILD,	0, 1, 0, 0, 0, 0, 0},
#else
	{HDMI_2K1K_30,		1, 2, 0, 0, 0, 1, 1},
	{HDMI_2K1K_60, 		1, 2, 0, 0, 0, 1, 1},
	{HDMI_2K1K_120,	 	1, 2, 0, 0, 0, 1, 1},
	{HDMI_4K2K_30, 		1, 2, 0, 1, 0, 1, 0},
	{HDMI_4K2K_60, 		1, 2, 0, 1, 0, 1, 0},
	{HDMI_4K2K_120, 		1, 2, 0, 0, 0, 1, 0},
	{HDMI_8K4K_30, 		0, 2, 0, 1, 0, 1, 0},
	{HDMI_8K4K_60, 		0, 2, 0, 1, 0, 1, 0},
	{VO_2K1K_30, 		1, 2, 0, 0, 1, 1, 1},
	{VO_2K1K_60, 		1, 2, 0, 0, 1, 1, 1},
	{VO_2K1K_120, 		1, 2, 0, 0, 1, 1, 1},
	{VO_4K1K_120, 		0, 2, 0, 1, 1, 0, 0},
	{VO_4K2K_30, 		1, 2, 0, 1, 1, 1, 0},
	{VO_4K2K_60, 		0, 2, 0, 1, 1, 0, 0},
	{VO_8K4K_30, 		0, 2, 0, 1, 1, 0, 0},
	{VO_8K4K_60, 		0, 2, 0, 1, 1, 0, 0},
	{CP_2K1K_30, 		1, 2, 0, 1, 1, 1, 1},
	{CP_2K1K_60, 		1, 2, 0, 1, 1, 1, 1},
	{CP_2K1K_120, 		1, 2, 0, 1, 1, 1, 1},
	{CP_4K1K_120, 		0, 2, 0, 1, 1, 0, 0},
...
unsigned char decide_seamless_freeze_condition(void)
{//return TRUE: can freeze return False: can not freeze
    ...
    if(ppoverlay_memc_mux_ctrl_reg.memc_outmux_sel)
        return TRUE;
    return FALSE;
}
...
void set_video_fw_memc_freeze_type(unsigned char display,unsigned char value)
{
	VIDEO_FW_CONTRL_MEMC_TYPE * video_fw_memc_freeze_type = NULL;

	if(display == SLR_MAIN_DISPLAY){
		video_fw_memc_freeze_type = (VIDEO_FW_CONTRL_MEMC_TYPE *)Scaler_GetShareMemVirAddr(SCALERIOC_VIDEO_FW_CONTROL_MEMC_FREEZE);
		if(video_fw_memc_freeze_type == NULL){
			pr_emerg("video_fw_memc_freeze_type is NULL!!!\n");
		}

		*video_fw_memc_freeze_type = Scaler_ChangeUINT32Endian(value);
		pr_emerg("memc_freeze_type =%d\n",value);
	}
}

VIDEO_FW_CONTRL_MEMC_TYPE get_video_fw_memc_freeze_type(unsigned char display)
{
	VIDEO_FW_CONTRL_MEMC_TYPE * video_fw_memc_freeze_type = NULL;
	VIDEO_FW_CONTRL_MEMC_TYPE memc_freeze_type = VIDEO_FW_CONTRL_MEMC_NONE;

	if(!PPOVERLAY_MEMC_MUX_CTRL_get_memc_outmux_sel(IoReg_Read32(PPOVERLAY_MEMC_MUX_CTRL_reg))){
		return VIDEO_FW_CONTRL_MEMC_NONE;
	}
	...
	if(display == SLR_MAIN_DISPLAY){
		...
		memc_freeze_type = Scaler_ChangeUINT32Endian(*video_fw_memc_freeze_type);

	}
	return memc_freeze_type;
}
...
void WaitFor_DEN_STOP_MEMCDTG(void)
{
    UINT32 timeoutcnt = 0xFFFFFF;
    UINT32 denEnd = 0;
    ...
    denEnd = ppoverlay_memcdtg_dv_den_start_end_reg.memcdtg_dv_den_end;

    do
    {
        ppoverlay_new_meas1_linecnt_real_reg.regValue = IoReg_Read32(PPOVERLAY_new_meas1_linecnt_real_reg);
        if(ppoverlay_new_meas1_linecnt_real_reg.memcdtg_line_cnt_rt > denEnd)
        {
            return;
        }
    } while(--timeoutcnt != 0);
