...
static unsigned char ypp_auto_phase_waitfinish(unsigned char display)
{
	...
	do {
		...
		msleep(1);//can't remove, keep enough time wait auto phase finish
	} while (--Wait_Time_Cnt);
...
//	StructDisplayInfo *p_dispinfo = Get_ADC_Dispinfo();
	...
	if(phase_flag){
		...
		Result  = auto_phasesearch_start(Hardware_Auto_Phase, display, Pulse_Detection_Enable, _COLORS_GREEN, HWAUTO_STEP_4, HWAUTOSTEPNUM(16), HWAUTOSTEPSTART(0), &phase_tmp,Low_Pass_Filter_0);
	}
	#else
...
void drvif_self_awb_initial_setting(void)//Auto color initial seeting at beginning
{
	...
	adc_adc_vbias_reg.adc_vbias1_7_6 = 3;
