...
int newbase_hdmi_enter_power_saving(unsigned char port)
{
    ...
    default:
        PWR_EMG("HDMI[p%d] enter power saving failed, unknwon ps mode (%d)\n", port, ps_mode);
        return -1;
    }
...
void newbase_hdmi_power_saving_handler(unsigned char port)
{
    ...
    if(ps_msg_cnt++ >= g_pwr_print_cycle_cnt_thd)
    {
        ...
        {
            PWR_EMG("PWR_STS : FG_HDMI=%d, P0/P1/P2/P3(active,fsm,vs_cnt,pol,int) =  P0(%d,%s,%d,%d,%d)/P1(%d,%s,%d,%d,%d)/P2(%d,%s,%d,%d,%d)/P3(%d,%s,%d,%d,%d)\n",
                ...
                _is_active_source(3), _hdmi_power_saving_fsm_str(hdmi_ps_st[3].ps_fsm), hdmi_rx[3].video_t.vs_cnt, hdmi_rx[3].video_t.vs_pol, GET_H_INTERLACE(3));
        }
...
void lib_hdmi_mac_exit_power_saving_mode_hw_mac(unsigned char nport, unsigned char frl_mode, unsigned char scrambled, unsigned char hdcp_mode)
{
    ...
    else
    {
        ...
        PWR_FLOW_INFO("HDMI[p%d] Win opp detected time=%d\n", nport, 1000 - timeout);
...
void lib_hdmi_mac_dump_power_saving_reg(unsigned char nport)
{
    if (hdmi_rx[nport].hdmi_2p1_en)
    {
        PWR_FLOW_INFO("HDMI[p%d] mac setting : HD21_ps_ct=%08x (ps_mode=%d, hdcp_mode=%d, imer_cnt_end=%d, mac_out=%d), HD21_ps_st=%08x (phy_pwr_on=%d)\n",
            nport,
...
void lib_hdmi_phy_reset_power_saving_state(unsigned char nport)
{
    ...
    case 0:
        ...
        // DPHY
        // reset HD21 DPHY
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<31), (1<<31));   // bypass power save (HDMI-2.1)
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<30), 0);       // power on cntrol by MAC (HDMI-2.1)
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<29), (1<<29));   // manual power setting = On (HDMI-2.1)

        // reset HD20 DPHY
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<15), (1<<15)); // bypass power save (HDMI-1.4)
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<10), 1<<10);    // Power on reg_p0_hd14_pow_save_bypass_r_only by FW
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<9), 1<<9);    // Power on reg_p0_hd14_pow_save_bypass_g_only by FW
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<8), 1<<8);    // Power onreg_p0_hd14_pow_save_bypass_b_only by FW
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<14), 0);       // power on cntrol by MAC  (HDMI-1.4)
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<13), (1<<13));   // manual power setting = On (HDMI-1.4)

        // release reset
        hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<29), (1<<29));  // fast switch release reset p0 b
        hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<30), (1<<30));  // fast switch release reset p0 g
        hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<31), (1<<31));  // fast switch release reset p0 r

        hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<26), (1<<26));  // fast switch release reset p0 b
        hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<27), (1<<27));  // fast switch release reset p0 g
        hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<28), (1<<28));  // fast switch release reset p0 r
        break;
    ...
    case 1:
        ...
        // reset HD21 DPHY
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<23), (1<<23)); // bypass power save (HDMI-2.1)
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<22), 0);       // power on cntrol by MAC (HDMI-2.1)
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<21), (1<<21)); // manual power setting = On (HDMI-2.1)

        // reset HD20 DPHY
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<7), (1<<7));   // bypass power save (HDMI-1.4)
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<2), 1<<2);      // Power on reg_p1_hd14_pow_save_bypass_r_only by FW
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<1), 1<<1);      // Power on reg_p1_hd14_pow_save_bypass_g_only by FW
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<0), 1<<0);      // Power on reg_p1_hd14_pow_save_bypass_b_only by FW

        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<6), 0);        // power on cntrol by MAC  (HDMI-1.4)
        hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<5), (1<<5));   // manual power setting = On (HDMI-1.4)

        // release reset
        hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<29), (1<<29));  // fast switch release reset p1 b
        hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<30), (1<<30));  // fast switch release reset p1 g
        hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<31), (1<<31));  // fast switch release reset p1 r

        hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<26), (1<<26));  // fast switch release reset p1 b
        hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<27), (1<<27));  // fast switch release reset p1 g
        hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<28), (1<<28));  // fast switch release reset p1 g
        break;
    ...
    case 2:
        ...
        // reset HD20 DPHY
        hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(1<<15), (1<<15));    // bypass power saving (HDMI-1.4)
        hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<10), 1<<10);    //reg_p0_hd14_pow_save_bypass_r_only Power on by FW
        hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<9), 1<<9);    // reg_p0_hd14_pow_save_bypass_g_onlyPower on by FW
        hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<8), 1<<8);    //reg_p0_hd14_pow_save_bypass_b_only Power on by FW

        hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(1<<14), 0x0);        // power on cntrol by MAC  (HDMI-1.4)
        hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(1<<13), (1<<13));    // manual power setting = On (HDMI-1.4)

        // release reset
        hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<29), (1<<29));  // fast switch release reset p2 b
        hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<30), (1<<30));  // fast switch release reset p2 g
        hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<31), (1<<31));  // fast switch release reset p2 r

        hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<26), (1<<26));  // fast switch release reset p2 b
        hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<27), (1<<27));  // fast switch release reset p2 g
        hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<28), (1<<28));  // fast switch release reset p2 r
        break;
    ...
    case 3:
        ...
        // reset HD20 DPHY
        hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(1<<7), (1<<7));       // bypass power saving (HDMI-1.4)
        hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<2), 1<<2);    // Power on reg_p1_hd14_pow_save_bypass_r_only by FW
        hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<1), 1<<1);    // Power on reg_p1_hd14_pow_save_bypass_g_only by FW
        hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<0), 1<<0);    // Power on reg_p1_hd14_pow_save_bypass_b_only by FW

        hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(1<<6), 0x0);          // power on cntrol by MAC  (HDMI-1.4)
        hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(1<<5), (1<<5));       // manual power setting = On (HDMI-1.4)

        // release reset
        hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<29), (1<<29));  // fast switch release reset p3 b
        hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<30), (1<<30));  // fast switch release reset p3 g
        hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<31), (1<<31));  // fast switch release reset p3 r

        hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<26), (1<<26));  // fast switch release reset p3 b
        hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<27), (1<<27));  // fast switch release reset p3 g
        hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<28), (1<<28));  // fast switch release reset p3 r
        break;
    }
}
...
void lib_hdmi_phy_enter_power_saving_mode(unsigned char nport, HDMI_MODE mode, unsigned char br_swap)
{
    ...
    case HDMI_20:
        ...
        case 0:
            ...
            // Digital Phy Setting
            hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<30), 0);      // fast switch reset p0 g
            hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<29), 0);      // fast switch reset p0 b
            hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<27), 0);      // fast switch reset p0 g
            hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<26), 0);      // fast switch reset p0 b

            // POWR OFF
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<15), 1<<15);  // enable power saving byass (should be default value)
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<10), 1<<10);    // Power on reg_p0_hd14_pow_save_bypass_r_only by FW
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<9), 1<<9);    // Power on reg_p0_hd14_pow_save_bypass_g_only by FW
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<8), 1<<8);    // Power onreg_p0_hd14_pow_save_bypass_b_only by FW
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<14), 1<<14);  // Power on by FW
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<13), 0);      // manul_power_on=OFF (should be default value)
            break;
        ...
        case 1:
            ...
            //HDM 2.1 Port1
            //PHY Setting
            //0x1800DA14 [31:24] REG_TOP_IN_7  for port1
            //REG_TOP_IN_7<0>=1'b0
            hdmi_mask(HDMIRX_2P1_PHY_TOP_IN_1_reg, ~(1<<16), 0);
            ...
            // Digital
            hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<30), 0);      // fast switch reset p1 g
            hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<29), 0);      // fast switch reset p1 b
            hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<27), 0);      // fast switch reset p1 g
            hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<26), 0);      // fast switch reset p1 b

            // POWR OFF DPHY
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<7), 1<<7);     // enable power saving byass (should be default value)
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<2), 1<<2);      // Power on reg_p1_hd14_pow_save_bypass_r_only by FW
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<1), 1<<1);      // Power on reg_p1_hd14_pow_save_bypass_g_only by FW
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<0), 1<<0);      // Power on reg_p1_hd14_pow_save_bypass_b_only by FW
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<6), 1<<6);     // Power on by FW
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<5), 0);        // manul_power_on=OFF (should be default value)
            break;
        ...
        case 2:  // waiting for DIC to realease
            ...
            if (br_swap)
            {
                ...
                // Digital
                hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<30), 0);  // fast switch reset p2 g
                hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<31), 0);  // fast switch reset p2 r
                hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<27), 0);  // fast switch reset p2 g
                hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<28), 0);  // fast switch reset p2 r
            }
            else
            {
                // Analog
                ...
                // Digital
                hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<30), 0);  // fast switch reset p2 g
                hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<29), 0);  // fast switch reset p2 b
                hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<27), 0);  // fast switch reset p2 g
                hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<26), 0);  // fast switch reset p2 b
            }

            // POWR OFF DPHY
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<15), 1<<15);    // enable power saving byass (should be default value)
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<10), 1<<10);    //reg_p0_hd14_pow_save_bypass_r_only Power on by FW
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<9), 1<<9);    // reg_p0_hd14_pow_save_bypass_g_onlyPower on by FW
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<8), 1<<8);    //reg_p0_hd14_pow_save_bypass_b_only Power on by FW
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<14), 1<<14);    // Power on by FW
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<13), 0);        // manul_power_on=OFF (should be default value)
            break;
        ...
        case 3:
            ...
            if (br_swap)
            {
                ...
                // Digital
                hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<30), 0);      // fast switch reset p3 g
                hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<31), 0);      // fast switch reset p3 r
                hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<27), 0);      // fast switch reset p3 g
                hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<28), 0);      // fast switch reset p3 r
            }
            else
            {
                // Analog
                ...
                // Digital
                hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<30), 0);      // fast switch reset p3 g
                hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<29), 0);      // fast switch reset p3 b
                hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<27), 0);      // fast switch reset p3 g
                hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<26), 0);      // fast switch reset p3 b
            }

            // POWR OFF DPHY
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<7), 1<<7);    // enable power saving byass (should be default value)
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<2), 1<<2);    // Power on reg_p1_hd14_pow_save_bypass_r_only by FW
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<1), 1<<1);    // Power on reg_p1_hd14_pow_save_bypass_g_only by FW
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<0), 1<<0);    // Power on reg_p1_hd14_pow_save_bypass_b_only by FW
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<6), 1<<6);    // Power on by FW
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<5), 0);       // manul_power_on=OFF (should be default value)
            break;
        }
    ...
    case HDMI_20_VS_WIN_OPP:
        ...
        case 0:
            ...
            // 0x1800DA88 [31:24] REG_P0_ACDR_G_8
            hdmi_mask(HDMIRX_2P1_PHY_P0_ACDR_G1_reg, ~(0x1<<25), 0);    //G Lane : fast switch circuit disable
            ...
            // Digital Phy Setting
            hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<30), 0);      // fast switch reset p0 g G OFF
            //hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<29), 0);      // fast switch reset p0 b
            hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<27), 0);      // fast switch reset p0 g G OFF
            //hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<26), 0);      // fast switch reset p0 b
            ...
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<10), 1<<10);    // Power on reg_p0_hd14_pow_save_bypass_r_only by FW
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<9), 1<<9);    // Power on reg_p0_hd14_pow_save_bypass_g_only by FW
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<8), 0<<8);    // Power onreg_p0_hd14_pow_save_bypass_b_only by Mac
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<14), 0<<14);  // Power on by Mac
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<13), 0);      // manul_power_on=OFF (should be default value)
            break;
        ...
        case 1:
            ...
            // 0x1800DB88 [31:24] REG_P1_ACDR_G_8
            hdmi_mask(HDMIRX_2P1_PHY_P1_ACDR_G1_reg, ~(0x1<<25), 0);    // G Lane : fast switch circuit disable
            ...
            // Digital
            hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<30), 0);      // fast switch reset p1 G OFF
            //hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<29), 0);      // fast switch reset p1 b
            hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<27), 0);      // fast switch reset p1 G OFF
            //hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<26), 0);      // fast switch reset p1 b

            // POWR OFF DPHY
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<7), 0<<7);     // Disable power saving byass (Control by Mac)
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<2), 1<<2);      // Power on reg_p1_hd14_pow_save_bypass_r_only by FW
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<1), 1<<1);      // Power on reg_p1_hd14_pow_save_bypass_g_only by FW
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<0), 0<<0);      // Power on reg_p1_hd14_pow_save_bypass_b_only by Mac
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<6), 0<<6);     // Power on by Mac
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<5), 0);        // manul_power_on=OFF (should be default value)
            break;
        ...
        case 2:  // waiting for DIC to realease
            ...
            if (br_swap)
            {
                ...
                // Digital
                hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<30), 0);  // fast switch reset p2 g OFF
                //hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<31), 0);  // fast switch reset p2 r
                hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<27), 0);  // fast switch reset p2 g OFF
                //hdmi_mask(HDMIRX_2P0_PHY_REGD0_reg, ~(0x1<<28), 0);  // fast switch reset p2 r

                // POWR OFF DPHY
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<15), 0<<15);    //  Disable power saving byass (Control by Mac)
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<10), 0<<10);    //reg_p0_hd14_pow_save_bypass_r_only Power on by Mac
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<9), 1<<9);    // reg_p0_hd14_pow_save_bypass_g_onlyPower on by FW
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<8), 1<<8);    //reg_p0_hd14_pow_save_bypass_b_only Power on by FW
            }
            else
            {
                ...
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<10), 1<<10);    //reg_p0_hd14_pow_save_bypass_r_only Power on by FW
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<9), 1<<9);    // reg_p0_hd14_pow_save_bypass_g_onlyPower on by FW
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<8), 0<<8);    //reg_p0_hd14_pow_save_bypass_b_only Power on by Mac
            }

            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<14), 0<<14);    // Power on by Mac
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<13), 0<<13);    // manul_power_on=OFF (should be default value)
            break;
        ...
        case 3:
            ...
            if (br_swap)
            {
                ...
                // Digital
                hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<30), 0);      // fast switch reset p3 g OFF
                //hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<31), 0);      // fast switch reset p3 r
                hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<27), 0);      // fast switch reset p3 g OFF
                //hdmi_mask(HDMIRX_2P0_PHY_REGD5_reg, ~(0x1<<28), 0);      // fast switch reset p3 r

                // POWR OFF DPHY
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<7), 0<<7);    //  Disable power saving byass (Control by Mac)
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<2), 0<<2);    // Power on reg_p1_hd14_pow_save_bypass_r_only by Mac
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<1), 1<<1);    // Power on reg_p1_hd14_pow_save_bypass_g_only by FW
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<0), 1<<0);    // Power on reg_p1_hd14_pow_save_bypass_b_only by FW
            }
            else
            {
                ...
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<2), 1<<2);    // Power on reg_p1_hd14_pow_save_bypass_r_only by FW
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<1), 1<<1);    // Power on reg_p1_hd14_pow_save_bypass_g_only by FW
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<0), 0<<0);    // Power on reg_p1_hd14_pow_save_bypass_b_only by Mac
            }

            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<6), 0<<6);    // Power on by Mac
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<5), 0<<5);       // manul_power_on=OFF (should be default value)
            break;
        }
        break;
    ...
    case HDMI_21:
        ...
        case 0:
          ...
          hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<10), 0);    // Power on reg_p0_hd14_pow_save_bypass_r_only by MAC
          hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<9), 0);    // Power on reg_p0_hd14_pow_save_bypass_g_only by MAC
          hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<8), 0);    // Power onreg_p0_hd14_pow_save_bypass_b_only by MAC
            }
            break;
        ...
        case 1:
            ...
            if (mode==HDMI_21)
            {
                hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<21), 1<<21);   // manual power on = ON  (HDMI-2.1)
                hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<22), 0);       // power on cntrol by MAC (HDMI-2.1)
                hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<23), 0);       // disable fast switch bypass (HDMI-2.1)
            }
            else
            {
                hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<5), 1<<5);    // manual power on = ON  (HDMI-2.1)
                hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<6), 0);       // power on cntrol by MAC (HDMI-2.1)
                hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<7), 0);       // disable fast switch bypass (HDMI-2.1)
             hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<2), 0);      // Power on reg_p1_hd14_pow_save_bypass_r_only by MAC
             hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<1), 0);      // Power on reg_p1_hd14_pow_save_bypass_g_only by MAC
             hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<0), 0);      // Power on reg_p1_hd14_pow_save_bypass_b_only by MAC
            }
            break;
        ...
        case 2:
            ...
            if (mode==HDMI_14)
            {
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(1<<13), 1<<13);   // manual power on = ON  (HDMI-1.4)
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(1<<14), 0x0);     // power on cntrol by MAC (HDMI-1.4)
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(1<<15), 0x0);     // disable fast switch bypass (HDMI-1.4)
             hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<10), 0);    //reg_p0_hd14_pow_save_bypass_r_only Power on by MAC
             hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<9), 0);    // reg_p0_hd14_pow_save_bypass_g_onlyPower on by MAC
             hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<8), 0);    //reg_p0_hd14_pow_save_bypass_b_only Power on by MAC
        ...
        case 3:
            ...
            if (mode==HDMI_14)
            {
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(1<<5), 1<<5);    // manual power on = ON  (HDMI-1.4)
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(1<<6), 0x0);     // power on cntrol by MAC (HDMI-1.4)
                hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(1<<7), 0x0);     // disable fast switch bypass (HDMI-1.4)
          hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<2), 0);    // Power on reg_p1_hd14_pow_save_bypass_r_only by MAC
          hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<1), 0);    // Power on reg_p1_hd14_pow_save_bypass_g_only by MAC
          hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<0), 0);    // Power on reg_p1_hd14_pow_save_bypass_b_only by MAC
...
void lib_hdmi_phy_exit_power_saving_mode(unsigned char nport, HDMI_MODE mode, unsigned char br_swap)
{
    ...
    case HDMI_20:
        ...
        case 1:
            ...
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<0), 1<<0);      // Power on reg_p1_hd14_pow_save_bypass_b_only by FW
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<6), 1<<6);      // Power on by FW
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<5), (1<<5));    // manual power=ON
            udelay(10);
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<6), 0);         // Power on by MAC
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<5), 0);         // manual power=OFF
            // DPHY RESET
            hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<30), (1<<30));  // fast switch release reset p1 g
            hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<29), (1<<29));  // fast switch release reset p1 b
            hdmi_mask(HDMIRX_2P1_PHY_P1_HD21_Y1_REGD00_reg, ~(0x1<<27), (1<<27));  // fast switch release reset p1 g
        ...
        case 2: // HDMI20 P0
            ...
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<14), 1<<14);      // Power on by FW
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<13), (1<<13));    // manual power=ON
            udelay(10);
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<14), 0);          // Power on by MAC
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<13), 0);          // manual power=OFF
        ...
        case 3: // HDMI20 P1
            ...
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<0), 1<<0);    // Power on reg_p1_hd14_pow_save_bypass_b_only by FW
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<6), 1<<6);          // Power on by FW
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<5), (1<<5));        // manual power=ON
            udelay(10);
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<6), 0);             // Power on by MAC
            hdmi_mask(HDMIRX_2P0_PHY_REGD85_reg, ~(0x1<<5), 0);             // manual power=OFF
    ...
    case HDMI_20_VS_WIN_OPP:
        ...
        case 0:
            ...
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<10), 1<<10);    // Power on reg_p0_hd14_pow_save_bypass_r_only by MAC
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<9),  1<<9);     // Power on reg_p0_hd14_pow_save_bypass_g_only by MAC
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<8),  1<<8);     // Power onreg_p0_hd14_pow_save_bypass_b_only by MAC
            ...
            //hdmi_mask(HDMIRX_2P1_PHY_P0_HD21_Y1_REGD00_reg, ~(0x1<<26), (1<<26));  // fast switch release reset p0 b
            // 0x1800DA68 [31:24] REG_P0_ACDR_B_8
            hdmi_mask(HDMIRX_2P1_PHY_P0_ACDR_B1_reg, ~(0x1<<25), 0);    //B lane fast switch circuit disable
            break;

        case 1:
            // DPHY ON
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<5), 1<<5);      // manual power on = ON  (HDMI-1.4)
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<6), 0);         // Power on by MAC (HDMI-1.4)
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<7), 1<<7);      // enable power saving byass  (HDMI-1.4)
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<2), 1<<2);      // Power on reg_p1_hd14_pow_save_bypass_r_only by MAC
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<1), 1<<1);      // Power on reg_p1_hd14_pow_save_bypass_g_only by MAC
            hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<0), 1<<0);      // Power on reg_p1_hd14_pow_save_bypass_b_only by MAC
    ...
    case HDMI_21:
        ...
        case 1:
            ...
            else
            {
                hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<5), 1<<5);      // manual power on = ON  (HDMI-1.4)
                hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<6), 0);         // Power on by MAC (HDMI-1.4)
                hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<7), 0);         // enable power saving byass  (HDMI-1.4)
             hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<2), 0);      // Power on reg_p1_hd14_pow_save_bypass_r_only by MAC
             hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<1), 0);      // Power on reg_p1_hd14_pow_save_bypass_g_only by MAC
             hdmi_mask(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg, ~(0x1<<0), 0);      // Power on reg_p1_hd14_pow_save_bypass_b_only by MAC
            }
...
void lib_hdmi_phy_dump_pow_saving_reg(unsigned char nport)
{
    ...
    case 1:
        ...
        PWR_EMG("HDMI[p%d] dphy power saving setting (%08x=%08x, b[23:21]=%d%d%d, b[7:5]=%d%d%d)\n",
            ...
            (HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg),
            (hdmi_in(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg)),
            (hdmi_in(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg)>>23) & 1,
            (hdmi_in(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg)>>22) & 1,
            (hdmi_in(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg)>>21) & 1,
            (hdmi_in(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg)>>7) & 1,
            (hdmi_in(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg)>>6) & 1,
            (hdmi_in(HDMIRX_2P1_PHY_GLOBAL_HD21_Y1_REGD32_reg)>>5) & 1);
        break;
    ...
    case 2:
        ...
        PWR_EMG("HDMI[p%d] dphy power saving setting (%08x=%08x, b13=%d, b14=%d, b15=%d)\n",
            nport,
            (HDMIRX_2P0_PHY_REGD85_reg),
            (hdmi_in(HDMIRX_2P0_PHY_REGD85_reg)),
            (hdmi_in(HDMIRX_2P0_PHY_REGD85_reg)>>13) & 1,
            (hdmi_in(HDMIRX_2P0_PHY_REGD85_reg)>>14) & 1,
            (hdmi_in(HDMIRX_2P0_PHY_REGD85_reg)>>15) & 1);
        break;
    ...
    case 3:
        ...
        PWR_EMG("HDMI[p%d] dphy power saving setting (%08x=%08x, b5=%d, b6=%d, b7=%d)\n",
            ...
            (HDMIRX_2P0_PHY_REGD85_reg),
            (hdmi_in(HDMIRX_2P0_PHY_REGD85_reg)),
            (hdmi_in(HDMIRX_2P0_PHY_REGD85_reg)>>5) & 1,
            (hdmi_in(HDMIRX_2P0_PHY_REGD85_reg)>>6) & 1,
            (hdmi_in(HDMIRX_2P0_PHY_REGD85_reg)>>7) & 1);
        break;
    }
}
