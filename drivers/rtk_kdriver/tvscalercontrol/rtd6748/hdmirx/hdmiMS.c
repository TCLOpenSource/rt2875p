...
#define VRR_NO_VIC_MAX_VFREQ_TB_NUM             6
/**********************************************************************************************
*
*	Const Declarations
*
**********************************************************************************************/
const HDMI_VIDEO_FREQ_TABLE_T g_video_freq_table[VRR_NO_VIC_MAX_VFREQ_TB_NUM] = {
	{240000, 268100},
	{165000, 179000},
	{144000, 155000},
	{120000, 127000},
	...
	if(is_hv_sync_mode == TRUE)
	{
		...
		if(is_vrr_mode == TRUE)
		{
			...
			{// no VIC, no VTEM base v info, force set max V freq,
				...
				unsigned long long temp_v_total = tm->v_total;
				unsigned long long pixel_clock = temp_pixel_clock;
				do_div(temp_pixel_clock, (unsigned long long) tm->h_total);
				temp_v_total = tm->v_act_len + tm->v_act_sta;
				tm->v_freq = (unsigned int)(temp_pixel_clock)/temp_v_total;
				HDMI_OFMS_WARN("port=%d, PixelColok(x10)=%lld, H total=%lld, v_freq = %d\n", port, pixel_clock, temp_h_total,tm->v_freq);

				for(i = 0; i < VRR_NO_VIC_MAX_VFREQ_TB_NUM; i++){
					if(tm->v_freq >= g_video_freq_table[i].freq && tm->v_freq < g_video_freq_table[i].freq_max){
						tm->v_freq = g_video_freq_table[i].freq;
						temp_v_total = (pixel_clock/temp_h_total)/tm->v_freq;
						break;
					}
				}
				if(i == VRR_NO_VIC_MAX_VFREQ_TB_NUM){
					HDMI_EMG("can't find vreq in video_freq_table\n");
				}

				tm->v_total = (unsigned int)temp_v_total;
				tm->h_total = (unsigned int)temp_h_total;
				HDMI_EMG("Measure mode: VRR3 , without VIC, without VTEM Base, port=%d, is_get_vtem =%d, Force V freq=%d, V total=%d, Final H total=%d, H act=%d, V act=%d\n",
