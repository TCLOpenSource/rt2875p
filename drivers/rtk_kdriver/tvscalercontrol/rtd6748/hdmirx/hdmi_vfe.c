...
extern void newbase_hdmi_set_cecoption_flag(unsigned int option_type,unsigned int enable);
...
bool cec_wakeup_enable=0;
bool cec_ctrl_enable=0;
#ifdef HDMI_PORT_MUX
...
static void HdmiTable_Init(void)
{
	...
	for (ch=0; ch < HDMI_RX_PORT_MAX_NUM; ch++)
	{
		...
		// load configuration of specified hdmi port
		if (0)//(!newbase_hdmi_is_edid_initialized(hdmi_port))
		{
...
int vfe_hdmi_drv_init(void)
{
	...
	{		
		...
		memset(&(hdmi_flow_flag[port].write_edid_ex), 0, sizeof(hdmi_flow_flag[port].write_edid_ex));
	}
...
{    
	...
	timing_info->v_freq    = lib_hdmims_vfreq_prescion_3_to_1(tm.v_freq); //0.1hz
	...
	timing_info->scan_type = (!tm.is_interlace);
        ...
        else
        {
           ...
           else if (get_HDMI_Dolby_VSIF_mode() == DOLBY_HDMI_VSIF_STD
			   || get_HDMI_Dolby_VSIF_mode() == DOLBY_HDMI_h14B_VSIF
			   || dolby_adapter_get_dolby_vsem_type(port) == DOLBY_VISION_VSEM_FORM_1
			   || dolby_adapter_get_dolby_vsem_type(port) == DOLBY_VISION_VSEM_FORM_2)
		   timing_info->isDolbyHDR = 1;//
	   else
		   timing_info->isDolbyHDR = 0;
        }
    #else
        timing_info->isDolbyHDR = (get_HDMI_Dolby_VSIF_mode() == DOLBY_HDMI_VSIF_STD || get_HDMI_Dolby_VSIF_mode() == DOLBY_HDMI_h14B_VSIF);
    #endif
#else
	timing_info->isDolbyHDR = 0;
#endif
...
//=============================================
	...
	if((1 == timing_info->isDolbyHDR))
	{
		if(true == drvif_Hdmi_GetDolbyVsInfoFrame(&hdmi_dolby_vsi_content))
		{
			if(hdmi_dolby_vsi_content.payload[1] & 0x20 && (2 == (hdmi_dolby_vsi_content.payload[6] & 0xf)))
			{
				timing_info->isDolbyGame = 1;
			}
		}
	}
...
int vfe_hdmi_drv_enable_hpd(void)
{
	...
	for (ch=0; ch<HDMI_PORT_TOTAL_NUM; ch++)
	{
		...
		if (hdmi_vfe_get_hdmi_port(ch, &port)==0)
		{
			lib_hdmi_z0_set(port, LN_ALL, 1, CLK_Wake_Up);   // Q10235 PS4 toggle HPD only, it causes noise
...
int vfe_hdmi_drv_disable_hpd(void)
{
	...
	for (ch=0; ch<HDMI_PORT_TOTAL_NUM; ch++)
	{
		...
		if (newbase_hdmi_get_video_state(port) >= MAIN_FSM_HDMI_DISPLAY_ON)
			lib_hdmi_z0_set(port, LN_ALL, 0, CLK_Wake_Up);
	...
	)
{
	...
	if(newbase_hdmi_save_edid(port, edid, len))
	{
		...
		hdmi_flow_flag[port].write_edid_ex=1;
	}
	else
	{
...
#ifdef CONFIG_OPTEE_HDCP2
	optee_hdcp2_get_receiver_id(HDCP2_CMD_GET_RECEIVER_ID, HDCP_2p2_ReceiverdId, HDCP_2P2_RECEIVER_ID_SIZE);
#endif
...
int vfe_hdmi_drv_set_hpd(vfe_hdmi_hpd_t* hpd_value)
{
	...
	if((hdmi_flow_flag[port].write_edid_ex==0)&&(hpd_value->hpd_ctl))
		{
		HDMI_EDID_ERR("EDID not set !! don't plug hotplug\n");
		return HDMI_DRV_ARG_ERR;
	}
	...
	if(hpd_value->hpd_ctl) {
		...
		lib_hdmi_z0_set(port, LN_ALL, 0, CLK_Wake_Up);   // Q10235 PS4 toggle HPD only, it causes noise
		newbase_hdmi_set_hpd(port, 0);
	}
    ...
    )
{
	...
	case VFE_HDMI_PHY_CEC_OPTION:

		newbase_hdmi_set_cecoption_flag(param, reserved);
		break;
    ...
    )
{
    ...
    case VFE_HDMI_PHY_FRL_STATUS :
	{
	 unsigned char support = 0;
	 result = vfe_hdmi_drv_get_frl_support(ch,&support);
	 *param = (unsigned int)support;
	 HDMI_INFO("[%s]  zxc__frl_support ==%d",__FUNCTION__,*param);
	 break;
	}
    default:
        return -1;
...
int vfe_hdmi_drv_get_currentVRRVFrequency(unsigned char ch, unsigned short *vFreq)
{
	...
	*vFreq = 0;
	...
	if (newbase_hdmi_get_vrr_enable(port) ||newbase_hdmi_get_qms_enable(port) || newbase_hdmi_get_freesync_enable(port))
	{
#if IS_ENABLED(CONFIG_SUPPORT_SCALER)
		unsigned short get_v_freq = 0;
        if(newbase_hdmi_get_video_state(port)  == MAIN_FSM_HDMI_VIDEO_READY)
        {
                ...
                else
                {
                    ...
                    memset(&timing, 0, sizeof(HDMI_TIMING_T));
                    newbase_hdmi_get_timing(port, &timing);
                    *vFreq = (unsigned short)(timing.v_freq/100);
                    result = HDMI_DRV_NO_ERR;
                }
        }
        else if(newbase_hdmi_get_video_state(port)  != MAIN_FSM_HDMI_VIDEO_READY)
        {
                *vFreq = 0;
                 result = HDMI_DRV_NO_ERR;
        }
...
int vfe_hdmi_drv_get_SCDC_Status(unsigned char ch, unsigned int *val)
{
	...
	*val = lib_hdmi_scdc_read(port,  SCDC_TMDS_CONFIGURATION) |
	(hdmi_flow_flag[port].scdc_tmds_config_update<<8) |
...
int vfe_hdmi_drv_get_frl_support(unsigned  char ch,unsigned char * support)
{
	unsigned char port = 0x0F;
	unsigned char result = HDMI_DRV_HW_ERR;
	...
	if(hdmi_vfe_get_hdmi_port(ch, &port) < 0)
	{
	       HDMI_PRINTF("[WARNING1][%s] vfe_hdmi_drv_get_port  invalid ch:%d, port: %d\n",__FUNCTION__, ch, port);
		return HDMI_DRV_ARG_ERR;
	}
	
	*support = lib_hdmi_is_hdmi_21_available(port);
...
int vfe_hdmi_drv_get_AMDFreesyncInfo(VFE_HDMI_AMD_FREE_SYNC_INFO* p_info)
{
	unsigned char port;
	HDMI_AMD_FREE_SYNC_INFO freesyncinfo;
	VFE_FLOW_PRINTF("[%s %d] starts\n", __func__, __LINE__);

	if (g_hdmi_drv_state != VFE_HDMI_DRV_SERVICE_STATE) {
		HDMI_EMG("[vfe_hdmi_drv_get_AMDFreesync_info][- STATE ERR] fsm = %d\n", g_hdmi_drv_state);
		return HDMI_DRV_STATE_ERR;
	}

	if (hdmi_vfe_get_connected_port(&port)<0) {
		memset(p_info, 0, sizeof(VFE_HDMI_AMD_FREE_SYNC_INFO));
		HDMI_EMG("[WARNING1] AMDFreesync_info forget to connect hdmi port\n");
		return HDMI_DRV_ARG_ERR;
	}
	
	memset(p_info, 0, sizeof(VFE_HDMI_AMD_FREE_SYNC_INFO));

	if(newbase_hdmi_get_freesync_info(port,&freesyncinfo))
	{
        p_info->version = freesyncinfo.version;
        p_info->freesync_supported        = freesyncinfo.freesync_supported;
        p_info->freesync_enabled          = freesyncinfo.freesync_enabled;
        p_info->freesync_activate         = freesyncinfo.freesync_activate;
        p_info->native_color_space_active = freesyncinfo.native_color_space_active;
        p_info->brightness_control_active = freesyncinfo.brightness_control_active;
        p_info->local_diming_disalbe      = freesyncinfo.local_diming_disalbe;
        p_info->min_refresh_rate          = freesyncinfo.min_refresh_rate;
        p_info->max_refresh_rate          = freesyncinfo.max_refresh_rate;
        p_info->srgb_eotf_active          = freesyncinfo.srgb_eotf_active;
        p_info->bt709_eotf_active         = freesyncinfo.bt709_eotf_active;
        p_info->gamma_2p2_eotf_active     = freesyncinfo.gamma_2p2_eotf_active;
        p_info->gamma_2p6_eotf_active     = freesyncinfo.gamma_2p6_eotf_active;
        p_info->brightness_control        = freesyncinfo.brightness_control;      
	}
    else
	{
		HDMI_EMG("HDMI[p%d] %s : NOT_AMDFreesync_info !!!\n",port,__func__);
	}
...
void vfe_hdmi_drv_suspend(void)
{
	...
	if (g_hdmi_drv_state == VFE_HDMI_DRV_SERVICE_STATE) 
	{
		...
		for (ch = 0; ch < HDMI_RX_PORT_MAX_NUM; ch++) 
		{
			...
			if(!cec_ctrl_enable || !cec_wakeup_enable)
				newbase_hdmi_set_hpd(port, 0);
