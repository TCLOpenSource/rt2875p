...
static DEFINE_SPINLOCK(HDMI_SIGNAL_ERR_HANDLER_SYNC);//sync for hdmi_can_do_error_handler_done
...
unsigned char hdmi_can_do_error_handler_done = TRUE;//TRUE means can do error handler 
	
unsigned char decide_do_hdmi_error_handler(void)
{//return TRUE: can do error handler
	unsigned char result = FALSE;
	spin_lock(&HDMI_SIGNAL_ERR_HANDLER_SYNC);
	if(hdmi_can_do_error_handler_done)
	{
		hdmi_can_do_error_handler_done = FALSE;
		result = TRUE;
	}
	spin_unlock(&HDMI_SIGNAL_ERR_HANDLER_SYNC);
	return result;
}

void reset_hdmi_error_hanler_ctrl(void)
{//reset hdmi_can_do_error_handler_done to true
	spin_lock(&HDMI_SIGNAL_ERR_HANDLER_SYNC);
	hdmi_can_do_error_handler_done = TRUE;
	spin_unlock(&HDMI_SIGNAL_ERR_HANDLER_SYNC);
}
...
{
	unsigned short AV_INPUT_LIST[] = {_AV_INPUT2, _AV_INPUT4, _AV_INPUT1};
	...
	if ((VBI_support_type_get() !=KADP_VFE_AVD_CC_IS_SUPPORTED) &&((vdc_mode == VDC_MODE_PALI)||(vdc_mode == VDC_MODE_NTSC50)||(vdc_mode == VDC_MODE_SECAM))) {//tt setting
		...
		if (0 ==(IoReg_Read32(VBI_TT_VBI_DATA_STATUS_reg) &VBI_TT_VBI_DATA_STATUS_tt_vps_int_en_mask)) {
			if ((drvif_module_vdc_read_coch_detect_status() == 1)  &&
				(((drvif_video_status_reg(VDC_noisy_vbi) == 0) &&
				(IoReg_Read32(VDTOP_DC_COCH_SEPERATE_1_reg) & VDTOP_DC_COCH_SEPERATE_1_coch_detected_by_check_dc_mask)))) {
...
void VBI_Suspend(void)
{
	...
	if (vbi_connect_flag&(VBI_NTSC_CC_BIT|VBI_PALN_CC_BIT)) {/* cc open */
		...
		if (VBI_CC_Register_Backup) {
			...
			//VBI_CC_Register_Backup[index++] = vdcf_inl(ADC_ADC_CLKGEN_RGB_Capture_reg);//Fix Me. ADC register spec out
	...
	if (vbi_connect_flag & VBI_AV_CGMS_BIT) {/* cgms_cvbs open */
		...
		if (VBI_CC_Register_Backup) {
			...
			if (VBI_CGMS_Register_Backup) {
				...
				//VBI_CGMS_Register_Backup[index++] = vdcf_inl(ADC_ADC_CLKGEN_RGB_Capture_reg);//Fix Me. ADC register spec out
				VBI_CGMS_Register_Backup[index++] = vdcf_inl(VBI_CGMS_C0_reg);
				VBI_CGMS_Register_Backup[index++] = vdcf_inl(VBI_CGMS_C1_reg);
				VBI_CGMS_Register_Backup[index++] = vdcf_inl(VBI_CGMS_C2_reg);
				VBI_CGMS_Register_Backup[index++] = vdcf_inl(VBI_CGMS_C3_reg);
...
void VBI_Resume(void)
{
	...
	if (VBI_CC_Register_Backup) {
		...
		//IoReg_Mask32(ADC_ADC_CLKGEN_RGB_Capture_reg, ~_BIT2,((VBI_CC_Register_Backup[index++])&_BIT2));//Fix Me. ADC register spec out
	...
	if (VBI_CGMS_Register_Backup) {
		...
		if (VBI_CC_Register_Backup) {
			...
			//IoReg_Mask32(ADC_ADC_CLKGEN_RGB_Capture_reg, ~_BIT2,((VBI_CGMS_Register_Backup[index++])&_BIT2));//Fix Me. ADC register spec out
			vdcf_outl(VBI_CGMS_C0_reg, VBI_CGMS_Register_Backup[index++]);
			vdcf_outl(VBI_CGMS_C1_reg, VBI_CGMS_Register_Backup[index++]);
			vdcf_outl(VBI_CGMS_C2_reg, VBI_CGMS_Register_Backup[index++]);
			vdcf_outl(VBI_CGMS_C3_reg, VBI_CGMS_Register_Backup[index++]);
...
{//return FALSE: for hdmi check spec. frame rat unit is real framerate*100
	...
	if(!(get_scaler_qms_mode_flag() || vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag()))
	{
		*framerate = 0;
...
//	abl_reserved_RBUS abl_Reserved_reg;//for debug test
	...
	KADP_VFE_AVD_AVDECODER_VIDEO_MODE_T ColorSystem3 = KADP_VIDEO_AVDEC_MODE_NTSC;
...
//	VSC_INPUT_TYPE_T vo_src;
	...
	VFE_HDMI_AMD_FREE_SYNC_INFO pFreesyncinfo;
	...
	switch(cmd)
	{
		...
		case VFE_IOC_HDMI_GET_AMDFREESYNCINFO:
			if (copy_from_user((void *)&pFreesyncinfo, (const void __user *)arg, sizeof(pFreesyncinfo)))
			{
				rtd_pr_vfe_debug("[ERR] copy_from_user error\n");
				ret = EFAULT;
			}
			...
			{
				/* excute driver api */
				if (vfe_hdmi_drv_get_AMDFreesyncInfo(&pFreesyncinfo) != 0)
				{
					ret = -1;
				}

				/* update info to user */
				if (copy_to_user((void *)arg, &pFreesyncinfo, sizeof(pFreesyncinfo)))
				{
					rtd_pr_vfe_debug("[ERR] copy_to_user error\n");
					ret = EFAULT;
				}
...
//			DRB_VOINFO  voTimingInfo_pr;
			...
			voTimingInfo_drvbase.src_v_len = voTimingInfo->src_v_len;
                        voTimingInfo_drvbase.launcher = voTimingInfo->launcher;
...
*/
		...
		case VO_GET_USBVOINFO:
		{
            ...
            {
                rtd_pr_vfe_debug("===VFE_IOCTL: copy_to_user retrun -1====\nreceive Voinfo:\n");
                ret = -1;
            }
            else{
                ret = 0;
		...
		case VFE_IOC_AVD_GET_AUTO_MODE:
        {
            rtd_pr_vfe_debug("Kernel VFE_IOC_AVD_GET_AUTO_MODE Start\n");
            rtd_pr_vfe_debug("VFE_IOC_AVD_GET_AUTO_MODE size=%zu\n", sizeof(VFE_IOC_AVD_GET_AUTO_MODE));
            switch(drvif_module_vdc_ReadMode(VDC_DETECT))
            {
                case VDC_MODE_NTSC:
                    ColorSystem3=KADP_VIDEO_AVDEC_MODE_NTSC;
                    break;
                case VDC_MODE_PALI:
                    ColorSystem3=KADP_VIDEO_AVDEC_MODE_PAL;
                    break;
                case VDC_MODE_PALM:
                    ColorSystem3=KADP_VIDEO_AVDEC_MODE_PAL_M;
                    break;
                case VDC_MODE_PALN:
                    ColorSystem3=KADP_VIDEO_AVDEC_MODE_PAL_NC_358;
                    break;
                case VDC_MODE_SECAM:
                    ColorSystem3=KADP_VIDEO_AVDEC_MODE_SECAM;
                    break;
            }
            ret = 0;
            if (copy_to_user((void __user *)arg, (const void * )&ColorSystem3, sizeof(ColorSystem3)))
            {
                rtd_pr_vfe_err("[ERR] copy_to_user error\n");
                ret = -1;
            }
            else
            {
            ...
            rtd_pr_vfe_debug("Kernel VFE_IOC_AVD_GET_AUTO_MODE End\n");
            break;
        }
		...
		{
			ret = 0;
			avOfflineStatus= drvif_AV_Detect_Signal_PlugInOut(0);
...
unsigned int rpc_main_onlinemeasure_isr(unsigned long para1, unsigned long para2)
{
	...
	drvif_scalerdisplay_set_bg_color(SLR_MAIN_DISPLAY, _DISPLAY_BG, 0x0000, 0x0000, 0x0000);
			...
			if (0 == Scaler_AVD_Init(KADP_VFE_AVD_CC_AND_TTX_ARE_SUPPORTED)){
				...
				if (0 == VFE_AVD_OPEN()){
					...
					if(AUTO_BRINGUP_SOURCE == _SRC_TV){
						...
						if (0 == Scaler_AVD_AV_Connect(_AV_INPUT4)){
							AVD_Global_Status = SRC_CONNECT_DONE;
							AV_Status = SRC_CONNECT_DONE;
							AVD_Input_Source = _SRC_CVBS;
							VDC_set_detect_flag(TRUE);
