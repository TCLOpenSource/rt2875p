...
unsigned char UI_aipq=0;
unsigned char UI_aipq_en=0;
...
extern unsigned char od_en;
unsigned char valc_en = 0;
unsigned char s_pcid_en = 0;
...
extern unsigned char aiaq_mode;
// end 0622 lsy
...
void vpq_boot_init(void)
{
	...
	fwif_VIP_set_Project_ID(VIP_Project_ID_TV030);
	...
	rtd_pr_vpq_info("boot init PID=%d !!!\n",fwif_VIP_get_Project_ID());
	/* set panel dither*/
	fwif_color_set_Panel_Dither(TRUE, VIP_PANEL_BIT8, VIP_DITHER_PANELTABLE_DEFAULT);
	...
	if(fwif_VIP_get_Project_ID() != VIP_Project_ID_TV030){
		if (Get_DISPLAY_REFRESH_RATE() == 60)
			drvif_color_set_Od_4K120_mode(0);
		else
			drvif_color_set_Od_4K120_mode(1);

		rtd_pr_vpq_info("vpq boot init,set OD table!! \n");
	...
	fwif_color_od_ctrl_get_setting();
	od_en = drvif_color_od_enable_get();
	Scaler_color_Set_HDR_AutoRun(TRUE); // use HFC to set HDR inv gamma and EOTF for TV006

	/* demura ini */
	if(fwif_VIP_get_Project_ID() != VIP_Project_ID_TV030){
		rtd_pr_vpq_info("vpq boot init,set demura table!! \n");
...
#if IS_ENABLED(CONFIG_RTK_AI_DRV)
	rtd_pr_vpq_info("[boot init]aipq init!! \n");
	/* NN init*/
	scalerAI_Init();
#endif
...
typedef struct{
    ...
    unsigned char output_Gamma_fmt;
    unsigned char output_Gamma_Location;
}Output_Gamma_Ctrl;

Output_Gamma_Ctrl output_Gamma_Ctrl_back = {0};

void acc_do_suspend(void)
{
	extern unsigned int TconOutputGamma_R[];
	extern unsigned int TconOutputGamma_R_cmp[16];
	extern unsigned int TconOutputGamma_G[];
	extern unsigned int TconOutputGamma_B[];
	//int i = 0;
	...
	output_Gamma_Ctrl_back.output_Gamma_En = outputgamma_out_gamma_ctrl_1_reg.out_gamma_en;
	output_Gamma_Ctrl_back.output_Gamma_Location = outputgamma_out_gamma_ctrl_1_reg.out_gamma_location;
	//output_Gamma_Ctrl_back.output_Gamma_fmt = outputgamma_out_gamma_ctrl_2_reg.out_gamma_format;

	if (0 == output_Gamma_Ctrl_back.output_Gamma_En) {
		return;
	}
	...
	if (0 == memcmp(TconOutputGamma_R_cmp, TconOutputGamma_R, sizeof(TconOutputGamma_R_cmp))) {
		printk(" no valid OutputGamma data, read data, Gamma_En:%d \n", output_Gamma_Ctrl_back.output_Gamma_En);
		drvif_color_colorRead_outGamma(TconOutputGamma_R, GAMMA_CHANNEL_R);
		drvif_color_colorRead_outGamma(TconOutputGamma_G, GAMMA_CHANNEL_G);
		drvif_color_colorRead_outGamma(TconOutputGamma_B, GAMMA_CHANNEL_B);
	}
...
void od_do_suspend(void)
{
	//od_en = drvif_color_od_enable_get();
	 ...
	 fwif_color_od_ctrl_get_setting();
	 fwif_color_od_get_table();
}

void pcid_do_suspend(void)
{
	s_pcid_en = fwif_color_get_pcid();
	printk("[ZYY][LINEOD]pcid_do_suspend,  s_pcid_en = %d",s_pcid_en);
}

void valc_do_suspend(void)
{
	valc_en = fwif_color_get_valc();
	printk("[ZYY][VALC]valc_do_suspend,  valc_en = %d",valc_en);
}
...
void vpq_do_suspend(void)
{
	...
	*((unsigned char *)Scaler_GetShareMemVirAddr(SCALERIOC_SETAUTOMAFLAG)) = 0;
	...
	if(PQ_LED_Dev_Status == PQ_LED_DEV_INIT_DONE){
		fwif_color_LD_SPI_init(PQ_LED_DEV_INIT_STANBY);
	}
	//od,acc
	fwif_color_dlg_do_suspend();
...
void acc_do_resume(void)
{
	...
	fwif_color_out_gamma_control_front();
	drvif_color_colorwrite_output_gamma(TconOutputGamma_R, GAMMA_CHANNEL_R);
	drvif_color_colorwrite_output_gamma(TconOutputGamma_G, GAMMA_CHANNEL_G);
	drvif_color_colorwrite_output_gamma(TconOutputGamma_B, GAMMA_CHANNEL_B);
	fwif_color_out_gamma_control_back();
	drvif_color_set_output_gamma_format(output_Gamma_Ctrl_back.output_Gamma_En,
			output_Gamma_Ctrl_back.output_Gamma_fmt, output_Gamma_Ctrl_back.output_Gamma_Location, 0);

	printk("------ acc_do_resume done\n");
}
...
void od_do_resume(void)
{
	...
	rtd_pr_vpq_emerg("VPQ_OD,  %s begin, bODInited:%d,  od_en:%d\n", __func__,bODInited, od_en);
	//rtd_printk(KERN_EMERG,"VPQ_OD", "%s begin, bODInited:%d \n", __func__,bODInited);
	if (!od_en)
		return;
	...
	if (Scaler_Init_OD()) {
		//rtd_pr_vpq_emerg("VPQ_OD,  %s , bODTableLoaded:%d, od_table_mode_store:%d \n", __func__,bODTableLoaded,od_table_mode_store);
		//rtd_printk(KERN_EMERG,"VPQ_OD", "%s , bODTableLoaded:%d, od_table_mode_store:%d \n", __func__,bODTableLoaded,od_table_mode_store);

#if 1
		fwif_color_set_od_table();
		drvif_color_od_enable_set(od_en);
#else
		...
		rtd_pr_vpq_emerg("VPQ_OD,  %s finished\n", __func__);
		//rtd_printk(KERN_EMERG,"VPQ_OD", "%s finished\n", __func__);
	}
	else {
		rtd_pr_vpq_emerg("VPQ_OD,  %s Scaler_Init_OD fail\n", __func__);
		//rtd_printk(KERN_EMERG,"VPQ_OD", "%s Scaler_Init_OD fail\n", __func__);
...
{
	printk("[ZYY][LINEOD]pcid_do_resume,  s_pcid_en = %d",s_pcid_en);
	if (0 == s_pcid_en)
		return;
...
void valc_do_resume(void)
{
	printk("[ZYY][VALC]valc_do_resume,  valc_en = %d",valc_en);
	if (0 == valc_en)
		return;

	fwif_color_set_valc(valc_en);
}
...
void vpq_do_resume(void)
{
	...
	if(PQ_LED_Dev_Status == PQ_LED_DEV_INIT_STANBY){
		fwif_color_LD_Table_init(0,vpq_led_LD_lutTableIndex);
		fwif_color_LD_SPI_init(PQ_LED_DEV_INIT);
	}
...
static int se_tsk(void *p)//This task run se proc
{
	...
	while (1)
	{
		...
		if(od_ctrl_reg.dummy1802ca00_31_12>>18 & 1) vt_en = 1; // bit 30
...
static int VPQ_detect_tsk(void *p)
{
    ...
    while (1)
    {
		...
		fwif_color_set_LD_hvsize_change(_DISP_WID,_DISP_LEN);
...
*/
	...
	*/
			...
			if (copy_from_user(&DeMura_Info, (int __user *)arg, sizeof(HAL_VPQ_DATA_T))) {
				...
				if(DeMura_Info.length == sizeof(VIP_DeMura_TBL))
				{
					 if (copy_from_user((void *)&DeMura_TBL, (int __user *)DeMura_Info.pData, (DeMura_Info.length)*sizeof(unsigned char))) {
						 rtd_pr_vpq_err("kernel copy DeMura_TBL fail 222\n");
						 ret = -1;
					 } else {
	...
	case VPQ_IOC_AI_DETECTION_ON:
	{
		...
		if (list & AI_AQ_DETECT)
			scalerAI_aq_mode_ctrl_tv001(1);

        if(fwif_VIP_get_Project_ID() == VIP_Project_ID_TV030 && (list & AI_SCENE_DETECT) ){
	        //scalerAI_apply_by_SCENE();
			UI_aipq=1;//this is for apply pq setting immediately
			UI_aipq_en=1;//this is call scalerAI_apply_by_SCENE
        printk("[AIPQ]VPQ_IOC_AI_DETECTION_ON scene on\n");
		}
		ret = 0;
		break;
	}
	case VPQ_IOC_AI_DETECTION_OFF:
	{
		...
		if (list & AI_AQ_DETECT)
			scalerAI_aq_mode_ctrl_tv001(0);

        if(fwif_VIP_get_Project_ID() == VIP_Project_ID_TV030 && (list & AI_SCENE_DETECT)){
			UI_aipq=0;//this is for apply pq setting immediately
			UI_aipq_en=0;//this is "NO" call scalerAI_apply_by_SCENE
			fwif_color_reset_AIPQ_offset_table();
            printk("[AIPQ]VPQ_IOC_AI_DETECTION_OFF scene off\n");
		}
		ret = 0;
		break;
	}
...
#endif
    case VPQ_IOC_GET_AI_ENABLE_LIST:
    {
        unsigned int list=0;

        if(aipq_mode.face_mode)
            list|=AI_FACE_DETECT;
        if(aipq_mode.scene_mode)
            list|=AI_SCENE_DETECT;
        if(aipq_mode.sqm_mode)
            list|=AI_SQM_DETECT;
        if(aipq_mode.depth_mode)
            list|=AI_DEPTH_DETECT;
        if(aipq_mode.noise_mode)
            list|=AI_NOISE_DETECT;
        if(aiaq_mode)
            list|=AI_AQ_DETECT;

        //printk("[AI]kernel VPQ_IOC_GET_AI_ENABLE_LIST list=%d\n",list);
        if (copy_to_user((void __user *)arg,(void *) &list, sizeof(unsigned int))) {
            rtd_pr_vpq_emerg("[AI]kernel VPQ_IOC_GET_AI_ENABLE_LIST fail\n");
            ret = -1;
        } else
            ret = 0;
        break;
