...
#define CS602_DEV_ADDR         0x33	//  ic slave address
#define CS602_DATA_NUM          42
unsigned char cs602_reg[49];
char cs602_i2c_port = 1;
extern char tconless_cs602_cs603_reset_is_done;

unsigned char CS602_Data_BankA_ST5461D17_1[49] =
{
    0x00, 0xC8, 0x19, 0x1B, 0x00, 0x2B, 0x23, 0xFC, 0x86, 0x77, 0x25, 0x23, 0x00, 0x11, 0x11, 0x0B, 0x0B,
    0x3E, 0xC3, 0xE0, 0x33, 0xE2, 0xED, 0x2C, 0xB2, 0x5D, 0x25, 0x31, 0x97, 0x18, 0xd1, 0x35, 0x0F,
    0x20, 0x8A, 0x01, 0xA0, 0x0D, 0x16, 0x51, 0x65, 0xDE, 0xAB, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
...
unsigned char Panel_SetMCUMode(unsigned char enable)
{
	int pinindex = 0;
	int ret = 0;
	int invert = 0;
	...
	if (pcb_mgr_get_enum_info_byname("PIN_BL_MCU_EN", &param) == 0)
	{
...
void cs602_i2c_port_check(void)
{
	unsigned char sub_addr = 0x00;
	int ret,i;
	unsigned char tmp_cs602_reg[49];
	for(i=0;i<10;i++)
	{
		ret = i2c_master_recv_ex(cs602_i2c_port, CS602_DEV_ADDR, &sub_addr, 1, &tmp_cs602_reg[0], 1);
		if(ret < 0)
			{
				rtd_pr_vbe_print(KERN_NOTICE, "[%s] cs602_i2c_port is wrong  i2c-port= %d\n",__FUNCTION__,cs602_i2c_port);
				if(cs602_i2c_port == 1)
					cs602_i2c_port = 2;
				else
					cs602_i2c_port = 1;
				...
				if(ret < 0)
				{
					rtd_pr_vbe_print(KERN_NOTICE, "[%s] cs 602_i2c_port is wrong after change the i2c-port %d\n",__FUNCTION__,cs602_i2c_port);
				}
				else
				{
					rtd_pr_vbe_print(KERN_NOTICE, "[%s]cs602_i2c_port is right after change the i2c-port %d\n",__FUNCTION__,cs602_i2c_port);
					msleep(5);
					return;
				}
			}
		...
		{
			rtd_pr_vbe_print(KERN_NOTICE, "[%s]cs 602_i2c_port is right i2c-port %d\n",__FUNCTION__,cs602_i2c_port);
			msleep(5);
			return;
		}
		...
		if(cs602_i2c_port == 1)
			...
			cs602_i2c_port = 1;
		msleep(10);
		rtd_pr_vbe_print(KERN_NOTICE, "[%s]cs602_i2c_port_check %d time\n",__FUNCTION__,i+1);
		if(i== 2)
		{
			cs602_i2c_port = 1;
			rtd_pr_vbe_print(KERN_NOTICE, "[%s]cs602_i2c_port is wrong after check 3 times the i2c-port %d\n",__FUNCTION__,cs602_i2c_port);
			return;
		}
	}
}

static int read_cs602_cs603_data_from_reg(void)
{
    int ret,i,sum = 0;
    unsigned char temp_cs602[48];

    rtd_pr_vbe_print(KERN_NOTICE, "kernel type = %d , index = %d\n",pPanelCOnfigParameter->iCONFIG_PANEL_TYPE,pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX);

    if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 15 && pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 239)
     ||(pPanelCOnfigParameter->iCONFIG_PMIC_TYPE == 2))
    {
            temp_cs602[0] = 0x00;
            for(i=0 ;i<43 ;i++)
            {
                temp_cs602[i+1] = cs602_reg[i];
                sum = sum + cs602_reg[i];
            }
            rtd_pr_vbe_print(KERN_NOTICE, "[%s] tconless_cs602 sum reg is %d \n",__FUNCTION__,sum);
            cs602_i2c_port_check();
            if(sum == 0)
            {
                rtd_pr_vbe_print(KERN_NOTICE, "[%s] tconless_cs602 reg is all zero\n",__FUNCTION__);
                return -1;
            }
            ret = i2c_master_send_ex(cs602_i2c_port,CS602_DEV_ADDR,&temp_cs602[0],43);
            if(ret < 0)
            { 
                rtd_pr_vbe_print(KERN_NOTICE, "[%s] i2c tconless_cs602 write fail\n",__FUNCTION__);
            }
            else
            {
                rtd_pr_vbe_print(KERN_NOTICE, "[%s] i2c tconless_cs602 write ok\n",__FUNCTION__);
                return 1;
            }
            for(i=0; i<43; i++) //for debug
                rtd_pr_vbe_print(KERN_NOTICE, "[%s]tconless [%x]\n",__FUNCTION__,temp_cs602[i]);
        }
    return -1;
}

void tconless_cs602_cs603_reset(void)
{
    int ret,i;

    if(tconless_cs602_cs603_reset_is_done == 0)
    {
        rtd_pr_vbe_print(KERN_NOTICE, "[%s] tconless_reset_flag = %d \n",__FUNCTION__,tconless_cs602_cs603_reset_is_done);
        tconless_cs602_cs603_reset_is_done = 1;
    }
    else
    {
        rtd_pr_vbe_print(KERN_NOTICE, "[%s] tconless_reset_flag = %d \n",__FUNCTION__,tconless_cs602_cs603_reset_is_done);
    ...
    if(read_cs602_cs603_data_from_reg() != -1)
    {
        rtd_pr_vbe_print(KERN_NOTICE, "[%s] write cs601 data from reserve\n",__FUNCTION__);
        return;
    }
    else
    {
        for(i = 0; i < 3; i++)
       {
            rtd_pr_vbe_print(KERN_NOTICE, "[%s] write PMIC data from code\n",__FUNCTION__);
            if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 15)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 239))
            {
                cs602_i2c_port_check();
                ret = i2c_master_send_ex(cs602_i2c_port,CS602_DEV_ADDR,&CS602_Data_BankA_ST5461D17_1[0],CS602_DATA_NUM+1);
                if(ret < 0){
                    rtd_pr_vbe_print(KERN_NOTICE, "[%s] i2c cs602_cs601 write fail\n",__FUNCTION__);
                }
                else{
                    rtd_pr_vbe_print(KERN_NOTICE, "[%s] i2c cs602_cs601 write ok\n",__FUNCTION__);
                    return;
                }
            }
        }
...
void tconless_CS602_cs603_reserve(void)
{
    int ret;
    int i;
    unsigned char sub_addr = 0x00;
    unsigned char cs602_cs603_reg_temp1[49] = {0xff};
    unsigned char cs602_cs603_reg_temp2[49] = {0x00};
     ...
     ||(pPanelCOnfigParameter->iCONFIG_PMIC_TYPE == 2))
    {
        ...
        {
            cs602_i2c_port_check();
            ret = i2c_master_recv_ex(cs602_i2c_port, CS602_DEV_ADDR, &sub_addr, 1, &cs602_cs603_reg_temp1[0], 42);
            if(ret < 0)
            {
                rtd_pr_vbe_print(KERN_NOTICE, "[%s] i2c tconless_cs602 read fail \n",__FUNCTION__);
            }
            else
            {
                rtd_pr_vbe_print(KERN_NOTICE, "[%s] i2c tconless_cs602 read ok\n",__FUNCTION__);
            }
            msleep(20);
            ret = i2c_master_recv_ex(cs602_i2c_port, CS602_DEV_ADDR, &sub_addr, 1, &cs602_cs603_reg_temp2[0], 42);
            if(ret < 0)
            {
                rtd_pr_vbe_print(KERN_NOTICE, "[%s] i2c tconless_cs602 read fail \n",__FUNCTION__);
            }
            ...
            for(i=0 ; i< 42; i++)
            {
                if(cs602_cs603_reg_temp1[i] != cs602_cs603_reg_temp2[i])
                {
                    rtd_pr_vbe_print(KERN_NOTICE, "[%s] i2c tconless_cs602 reg match fail\n",__FUNCTION__);
                    match_flag = 0;
                    break;
                }
                else
            ...
            if(match_flag == 1)
            {
                for(i=0 ; i< 42; i++)
                {
                    cs602_reg[i] = cs602_cs603_reg_temp2[i];
                    rtd_pr_vbe_print(KERN_NOTICE, "[%s] i2c tconless_cs602 reg 0x%x\n",__FUNCTION__,cs602_reg[i]);
                }
                return;
            }
        }
...
#define WKSOR_WOL 6
#define WKSOR_RTC 8
extern void vbe_disp_lane_power_on(void);
extern void vbe_disp_lane_power_off(void);

void Panel_resume_TurnOn(void)
{
    unsigned int regValue;
    rtd_pr_vbe_print(KERN_NOTICE, "[%s]\n",__FUNCTION__);
    regValue = rtd_inl(WKSOR_REG);
    if(((regValue & 0xff) != 0x0) && ((regValue & 0xff) != WKSOR_WOL) && ((regValue & 0xff) != WKSOR_RTC))
    {
        rtd_pr_vbe_print(KERN_NOTICE, "[%s] Normal wakeup, ctrl panel on now\n",__FUNCTION__);
        Panel_TurnOn_By_KernelAp(0);
    }
    else
        rtd_pr_vbe_print(KERN_NOTICE, "[%s] RTC/WOW wakeup, ctrl panel on later\n",__FUNCTION__);
}
extern HAL_VBE_DISP_SetMute(BOOLEAN bOnOff);

void Panel_TurnOn_By_KernelAp(unsigned int value)
{
    unsigned int regValue;
    ...
    rtd_pr_vbe_print(KERN_NOTICE, "[%s] STR wakeup, panel on by kernel(0) or ap(1)? %d\n",__FUNCTION__, value);
    Panel_SetPanelOnOff(1);
    msleep(Get_PANEL_TO_LVDS_ON_ms());
    tconless_cs602_cs603_reset();
    msleep(100);    //VDD3.3V to data delay 60ms
    vbe_disp_lane_power_on();

    regValue = rtd_inl(WKSOR_REG);
    if((regValue & 0xff) == WKSOR_WUT){
        HAL_VBE_DISP_SetMute(1);
        rtd_pr_vbe_print(KERN_NOTICE, "[%s] WUT wakeup, open backlight later\n",__FUNCTION__);
        }
    else
    {
        rtd_pr_vbe_print(KERN_NOTICE, "Backlight_TurnOn BL_delay=%d(ms) \n",Get_LVDS_TO_LIGHT_ON_ms());
...
/*
    merlin7 dpll support max dpll 700M
    panel dclk less than dpll run one pixel
    panel dclk more than dpll run two pixel
*/
PANEL_PIXEL_MODE get_panel_pixel_mode(void)
{
    unsigned long long dclk = 0;
    
    dclk = ((unsigned long long)Get_DISP_VERTICAL_TOTAL()) * Get_DISP_HORIZONTAL_TOTAL() * Get_DISPLAY_REFRESH_RATE();

    if(dclk <= 700000000)
    {
        return PANEL_1_PIXEL;
    }
    
    return PANEL_2_PIXEL;
}
