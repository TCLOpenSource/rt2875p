...
I3DDMA_ERR_T I3DDMA_MeasureActiveSpace(I3DDMA_TIMING_T *tx_timing, I3DDMA_TIMING_T *gen_timing) {
	...
	while(i3ddma_active_space_table[i].name) {
		...
		//	gen_timing->progressive = tx_timing->progressive;
			...
			gen_timing->v_freq = ( gen_timing->progressive ? ( drvif_scaler3d_decide_3d_SG_data_FRC() ? ( tx_timing->v_freq * 2 ) : 120000 ) : ( tx_timing->v_freq * fields ) );
...
//	gen_timing->progressive = tx_timing->progressive;
	gen_timing->color = tx_timing->color;
	gen_timing->depth = tx_timing->depth;
	gen_timing->v_freq = ( gen_timing->progressive ? 120000 : ( tx_timing->v_freq * fields ) );
	gen_timing->v_total = tx_timing->v_total / fields;
	gen_timing->h_freq = tx_timing->h_freq;
	gen_timing->h_act_len = tx_timing->h_act_len;
	gen_timing->h_total = tx_timing->h_total;
...
//	gen_timing->quincunx_en = 0;
    ...
    {
        gen_timing->v_freq  = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ) * 100;
    }
...
//			rtd_maskl(I3DDMA_CAP_CTL0_VADDR, ~(I3DDMA_CAP_CTL0_cap_en_mask), I3DDMA_CAP_CTL0_cap_en(0));
		...
		case I3DDMA_3D_OPMODE_HW:
			...
			if(get_i3ddma_4k120_flag())
				i3ddma_cap_ctl0_reg.cap0_auto_block_sel_en = 0;//single buffer sw mode to select buffer
			else
...
//#define _2K_1K_10BIT_SIZE      (1920 * 1080 * 30/8)
	...
	#define _2K_1K_24BIT_SIZE ((_2K_WID * 24 + 256)  * _1K_LEN / 8)
	#define _3K_1K_12BIT_SIZE ((_3K_1K_WID * 12 + 256)  * _3K_1K_LEN / 8)
	#define _3K_1K_20BIT_SIZE ((_3K_1K_WID * 20 + 256)  * _3K_1K_LEN / 8)
#endif
...
///qms			size_video = dvr_size_alignment(size_video);
		...
		if(get_i3ddma_4k120_flag())
		{
			allocSize = _48M_SIZE; //hdmi2.0 4k120 alloc vbm chuck 48M
	...
	*/
    ...
    {//alloc need  disable i3ddma capture first
		...
		if(I3DDMA_Hdmi_Is_Need_Protection()){
			rtd_pr_i3ddma_warn("@@enable_hdmi_protect,%x %x,%s,%d",
								i3ddmaCtrl.cap_buffer[0].phyaddr_unalign ,i3ddmaCtrl.cap_buffer[0].getsize,
								__FUNCTION__, __LINE__);
			if(i3ddma_alloc_memory_cma_flag){
				rtkvdec_svp_hdmi_cma_region_record(i3ddmaCtrl.cap_buffer[0].phyaddr_unalign ,i3ddmaCtrl.cap_buffer[0].getsize);
			}else {
				rtkvdec_svp_hdmi_cma_region_record(0, 0);
			}
			rtkvdec_svp_enable_hdmi_protection(i3ddmaCtrl.cap_buffer[0].phyaddr_unalign ,i3ddmaCtrl.cap_buffer[0].getsize);
			set_hdmi_hdcp_secure_status(true);
		}	
#endif
    }
	if(m_domain_borrow_case)
    ...
    if(i3ddmaCtrl.cap_buffer[0].phyaddr)
    {
		...
		else if(i3ddmaCtrl.cap_buffer[2].phyaddr && (drvif_i3ddma_triplebuf_flag() || drvif_i3ddma_triplebuf_by_timing_protect_panel() || get_scaler_qms_mode_flag()) && (i3ddmaCtrl.cap_buffer[2].getsize != 0))
		{
			i3ddmaCtrl.cap_buffer[2].size = size_video;
...
void I3DDMA_DolbyVision_HDMI_DeInit(void) {
	...
	if(i3ddma_alloc_memory_cma_flag && i3ddmaCtrl.cap_buffer[0].cache)
	{
		...
		if(I3DDMA_Hdmi_Is_Need_Protection()){
			...
			set_hdmi_hdcp_secure_status(false);
		}
#endif
...
{
			if(is_game_mode_set_line_buffer() && Scaler_DispWindowGet().src_height == _DISP_LEN)
			{
				...
				rtd_pr_i3ddma_emerg("[%s]HDMI timing sync, game mode data fs, use increased voClk(%d)", __FUNCTION__, VoClock);
			}
			else
			{
				VoClock = VSYNCGEN_get_vodma_clk_0();
				rtd_pr_i3ddma_emerg("[%s]HDMI timing sync, game mode data frc, use origin voClk(%d)", __FUNCTION__, VoClock);
			}
		}
		else
		...
		i3ddmaVfreq = i3ddmaCtrl.pgen_timing->v_freq / 100;
		//if ((get_current_driver_pattern(DRIVER_FREERUN_PATTERN) == 1) && (get_hdmi_4k_hfr_mode() == HDMI_NON_4K120)) {//fix 4k max size
		rtd_pr_i3ddma_info("[iv2dv_dly][%d,%d,%d,%d,%d,%d,%d,%d]",voHtotal, voVactive, voVstart, VoClock,i3ddmaVtotal, i3ddmaVstart, i3ddmaVactive, i3ddmaVfreq);
		/*if(i3ddmaVfreq % 10 > 4)
...
void drv_I3ddmaVodma_config_hdmi_ivs_src(void)
{
	...
	if((drvif_i3ddma_triplebuf_by_timing_protect_panel()== FALSE)&&(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)){
		...
		if(get_i3ddma_4k120_flag() == FALSE){  // 4k120 don't enable hw mode
...
*/
	...
	unsigned long vir_addr, vir_addr_noncache;
	unsigned int phy_addr;
	unsigned int fixed_vfreq = timing->v_freq;
	unsigned int fixed_src_vfreq = timing->src_v_freq;
	...
	if(drvif_i3ddma_triplebuf_flag())
		...
		config_data->hw_auto_mode = 1;
#endif
	if(drvif_i3ddma_triplebuf_by_timing_protect_panel() || get_scaler_qms_mode_flag())
...
{//decide i3ddma output color format
	...
	if((timing->i3ddma_3dformat == I3DDMA_VERTICAL_STRIPE)|| (timing->i3ddma_3dformat == I3DDMA_CHECKER_BOARD))
	{
        ...
        if((timing->h_act_len >= 3800) && (timing->v_freq > 48000)
...
void dvrif_i3ddma_comp_setting(UINT8 enable_compression, UINT16 comp_wid, UINT16 comp_len, UINT8 comp_ratio){
	...
	if(enable_compression == TRUE){
		...
		*/
		...
		if(Get_PANEL_VFLIP_ENABLE() || ((get_rotate_mode(SLR_MAIN_DISPLAY) == DIRECT_VO_FRAME_ORIENTATION_ROTATE_180) && (Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE) == FALSE)))
			h3ddma_h3ddma_pq_cmp_pair_reg.two_line_prediction_en = 0;
		else {
...
void dvrif_vodma_pqdecomp_setting(VIDEO_RPC_VOUT_CONFIG_HDMI_3D *HDMI_INFO)
{
	...
	unsigned int comp_ratio = i3ddma_get_compression_bits();

	ModulePrintf(LOGLEVEL_WARNING, MODULE_VOUT,"enable_compression = %d, comp_wid = %d, comp_len=%d, comp_mode = %d, comp_ratio = %d\n",i3ddma_compression_get_enable(), comp_wid, comp_len, i3ddma_get_compression_mode(), comp_ratio);

    if (i3ddma_compression_get_enable() == TRUE)
    {
    		/*
			width need to align 32
		...
		*/
		...
		//i3ddma capture compression setting
		h3ddma_h3ddma_pq_cmp_reg.regValue = vo_rtd_inl(H3DDMA_H3DDMA_PQ_CMP_reg);
		h3ddma_h3ddma_pq_cmp_reg.cmp_width_div32 = comp_wid / 32;//set width

		//compression bits setting
		h3ddma_h3ddma_pq_cmp_bit_reg.regValue = vo_rtd_inl(H3DDMA_H3DDMA_PQ_CMP_BIT_reg);
		if(i3ddma_get_compression_mode() == I3DDMA_COMP_LINE_MODE)
		{
			h3ddma_h3ddma_pq_cmp_bit_reg.frame_limit_bit = comp_ratio;
			h3ddma_h3ddma_pq_cmp_bit_reg.line_limit_bit = comp_ratio;

			h3ddma_h3ddma_pq_cmp_reg.cmp_height = config_hdmi_info_save.height;//set original source length when line mode
		}
		else{//frame mode, line limit bit need to add margin to avoid the peak bandwidth
			h3ddma_h3ddma_pq_cmp_bit_reg.frame_limit_bit = comp_ratio;
			h3ddma_h3ddma_pq_cmp_bit_reg.line_limit_bit = comp_ratio + I3DDMA_PQC_LINE_MARGIN;
		...
		h3ddma_h3ddma_pq_cmp_bit_reg.block_limit_bit = 0x3f;//PQC fine tune setting
		h3ddma_h3ddma_pq_cmp_bit_reg.first_line_more_bit = 0x20;
		vo_rtd_outl(H3DDMA_H3DDMA_PQ_CMP_BIT_reg, h3ddma_h3ddma_pq_cmp_bit_reg.regValue);
		vo_rtd_outl(H3DDMA_H3DDMA_PQ_CMP_reg, h3ddma_h3ddma_pq_cmp_reg.regValue);

		//compression other setting
		h3ddma_h3ddma_pq_cmp_pair_reg.regValue = vo_rtd_inl(H3DDMA_H3DDMA_PQ_CMP_PAIR_reg);
		...
		if(Get_PANEL_VFLIP_ENABLE())
			...
			if ((i3ddma_get_compression_mode() == I3DDMA_COMP_LINE_MODE) && (get_current_driver_pattern(DRIVER_FREERUN_PATTERN) == 1)) {
				h3ddma_h3ddma_pq_cmp_pair_reg.two_line_prediction_en = 0;
			} else {
				h3ddma_h3ddma_pq_cmp_pair_reg.two_line_prediction_en = 1;
			}
		...
		h3ddma_h3ddma_pq_cmp_pair_reg.cmp_dic_mode_en = 0;
		h3ddma_h3ddma_pq_cmp_pair_reg.cmp_jump4_en = 1;
		vo_rtd_outl(H3DDMA_H3DDMA_PQ_CMP_PAIR_reg, h3ddma_h3ddma_pq_cmp_pair_reg.regValue);

                //>>>>[start]K5LG-1492:modify for i3ddma and mdomain compression
		h3ddma_h3ddma_pq_cmp_enable_reg.regValue = vo_rtd_inl(H3DDMA_H3DDMA_PQ_CMP_ENABLE_reg);
                //RGB444 PQC mode do_422_mode=2,comment from DIC Tien-Hung
		#if 0
        if(get_vsc_run_pc_mode()){
			if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_COLOR_SPACE) == I3DDMA_COLOR_YUV422))
		...
		h3ddma_h3ddma_pq_cmp_enable_reg.poor_limit_min_qp_en = 1;
		h3ddma_h3ddma_pq_cmp_enable_reg.cmp_ctrl_para0 = 0x10;
		vo_rtd_outl(H3DDMA_H3DDMA_PQ_CMP_ENABLE_reg,h3ddma_h3ddma_pq_cmp_enable_reg.regValue);

		h3ddma_h3ddma_pq_cmp_allocate_reg.regValue = vo_rtd_inl(H3DDMA_H3DDMA_PQ_CMP_ALLOCATE_reg);
		//RGB444 setting for PQC ratio_max and ratio_min, comment from DIC Tien-Hung
		if(pc_mode_run_422())
		{//hdmi input yuv422 pc mode
			h3ddma_h3ddma_pq_cmp_allocate_reg.dynamic_allocate_ratio_max = 16;
		...
		h3ddma_h3ddma_pq_cmp_allocate_reg.dynamic_allocate_more = 2;
		h3ddma_h3ddma_pq_cmp_allocate_reg.dynamic_allocate_line = 4;
		vo_rtd_outl(H3DDMA_H3DDMA_PQ_CMP_ALLOCATE_reg, h3ddma_h3ddma_pq_cmp_allocate_reg.regValue);

		h3ddma_h3ddma_pq_cmp_poor_Reg.regValue=vo_rtd_inl(H3DDMA_H3DDMA_PQ_CMP_POOR_reg);
		...
		h3ddma_h3ddma_pq_cmp_poor_Reg.poor_limit_th_qp3 = 12;
		h3ddma_h3ddma_pq_cmp_poor_Reg.poor_limit_th_qp4 = 20;
		vo_rtd_outl(H3DDMA_H3DDMA_PQ_CMP_POOR_reg, h3ddma_h3ddma_pq_cmp_poor_Reg.regValue);

		// Guarantee
		h3ddma_h3ddma_pq_cmp_guarantee_reg.regValue = vo_rtd_inl(H3DDMA_H3DDMA_PQ_CMP_Guarantee_reg);
		h3ddma_h3ddma_pq_cmp_guarantee_reg.guarantee_max_g_qp = 0;
		h3ddma_h3ddma_pq_cmp_guarantee_reg.guarantee_max_rb_qp = 0;
		// Dictionary
		h3ddma_h3ddma_pq_cmp_guarantee_reg.dic_mode_qp_th_g = 0;
		h3ddma_h3ddma_pq_cmp_guarantee_reg.dic_mode_qp_th_rb = 0;
		vo_rtd_outl(H3DDMA_H3DDMA_PQ_CMP_Guarantee_reg, h3ddma_h3ddma_pq_cmp_guarantee_reg.regValue);

		// Quota distribution
		h3ddma_h3ddma_pq_cmp_blk0_add0_reg.regValue = vo_rtd_inl(H3DDMA_H3DDMA_PQ_CMP_BLK0_ADD0_reg);
		h3ddma_h3ddma_pq_cmp_blk0_add0_reg.blk0_add_value0 = 1;
		h3ddma_h3ddma_pq_cmp_blk0_add0_reg.blk0_add_value16 = 8;
		h3ddma_h3ddma_pq_cmp_blk0_add0_reg.blk0_add_value32 = 16;
		...
		vo_rtd_outl(H3DDMA_H3DDMA_PQ_CMP_BLK0_ADD0_reg, h3ddma_h3ddma_pq_cmp_blk0_add0_reg.regValue);


		h3ddma_h3ddma_pq_cmp_blk0_add1_reg.regValue = vo_rtd_inl(H3DDMA_H3DDMA_PQ_CMP_BLK0_ADD1_reg);
		h3ddma_h3ddma_pq_cmp_blk0_add1_reg.blk0_add_value1 = 1;
		h3ddma_h3ddma_pq_cmp_blk0_add1_reg.blk0_add_value2 = 2;
		h3ddma_h3ddma_pq_cmp_blk0_add1_reg.blk0_add_value4 = 4;
		h3ddma_h3ddma_pq_cmp_blk0_add1_reg.blk0_add_value8 = 6;
		vo_rtd_outl(H3DDMA_H3DDMA_PQ_CMP_BLK0_ADD1_reg, h3ddma_h3ddma_pq_cmp_blk0_add1_reg.regValue);

		// Balance
		h3ddma_h3ddma_pq_cmp_balance_reg.regValue = vo_rtd_inl(H3DDMA_H3DDMA_PQ_CMP_BALANCE_reg);
		h3ddma_h3ddma_pq_cmp_balance_reg.balance_g_give = 3;
		h3ddma_h3ddma_pq_cmp_balance_reg.balance_g_ud_th = 3;
		h3ddma_h3ddma_pq_cmp_balance_reg.balance_g_ov_th = 0;
		h3ddma_h3ddma_pq_cmp_balance_reg.balance_rb_give = 3;
		...
		h3ddma_h3ddma_pq_cmp_balance_reg.variation_maxmin_th = 30;
		h3ddma_h3ddma_pq_cmp_balance_reg.variation_maxmin_th2 = 3;
		vo_rtd_outl(H3DDMA_H3DDMA_PQ_CMP_BALANCE_reg, h3ddma_h3ddma_pq_cmp_balance_reg.regValue);

		// Smooth
		h3ddma_h3ddma_pq_cmp_smooth_reg.regValue = vo_rtd_inl(H3DDMA_H3DDMA_PQ_CMP_SMOOTH_reg);
		h3ddma_h3ddma_pq_cmp_smooth_reg.variation_near_num_th = 4;
		h3ddma_h3ddma_pq_cmp_smooth_reg.variation_value_th = 3;
		h3ddma_h3ddma_pq_cmp_smooth_reg.variation_num_th = 3;
		...
		// DIctionary
		h3ddma_h3ddma_pq_cmp_smooth_reg.dic_mode_entry_th = 15;
		vo_rtd_outl(H3DDMA_H3DDMA_PQ_CMP_SMOOTH_reg, h3ddma_h3ddma_pq_cmp_smooth_reg.regValue);

		h3ddma_pq_cmp_qp_st_reg.regValue = vo_rtd_inl(H3DDMA_H3DDMA_PQ_CMP_QP_ST_reg);
		h3ddma_pq_cmp_qp_st_reg.cmp_ctrl_para1 = 0x44;
		h3ddma_pq_cmp_qp_st_reg.cmp_ctrl_para2 = 0xD0;
		vo_rtd_outl(H3DDMA_H3DDMA_PQ_CMP_QP_ST_reg, h3ddma_pq_cmp_qp_st_reg.regValue);

		h3ddma_pq_cmp_fifo_st1_reg.regValue = vo_rtd_inl(H3DDMA_H3DDMA_PQ_CMP_FIFO_ST1_reg);
		h3ddma_pq_cmp_fifo_st1_reg.cmp_ctrl_para3 = 0x0;
		vo_rtd_outl(H3DDMA_H3DDMA_PQ_CMP_FIFO_ST1_reg, h3ddma_pq_cmp_fifo_st1_reg.regValue);

        //vodma de-compression setting
		vodma_vodma_pq_decmp_reg.regValue = vo_rtd_inl(VODMA_VODMA_PQ_DECMP_reg);
		vodma_vodma_pq_decmp_reg.decmp_height = comp_len;
		vodma_vodma_pq_decmp_reg.decmp_width_div32 = comp_wid / 32;
		vo_rtd_outl(VODMA_VODMA_PQ_DECMP_reg, vodma_vodma_pq_decmp_reg.regValue);

        vodma_vodma_pq_decmp_pair_reg.regValue = vo_rtd_inl(VODMA_VODMA_PQ_DECMP_PAIR_reg);
		if(pc_mode_run_422())
		{
			vodma_vodma_pq_decmp_pair_reg.decmp_data_format = 1; // 422
		}
		...
		if(Get_PANEL_VFLIP_ENABLE())
		{
			...
			if ((i3ddma_get_compression_mode()) && (get_current_driver_pattern(DRIVER_FREERUN_PATTERN) == 1 )) {
				vodma_vodma_pq_decmp_pair_reg.decmp_two_line_prediction_en = 0;
			} else {
				vodma_vodma_pq_decmp_pair_reg.decmp_two_line_prediction_en = 1;
			}
		...
		vodma_vodma_pq_decmp_pair_reg.decmp_jump4_en = 1;
		vodma_vodma_pq_decmp_pair_reg.decmp_pair_para = 1;
	    vo_rtd_outl(VODMA_VODMA_PQ_DECMP_PAIR_reg, vodma_vodma_pq_decmp_pair_reg.regValue);

		vodma_vodma_pq_decmp_sat_en_reg.regValue = vo_rtd_inl(VODMA_VODMA_PQ_DECMP_SAT_EN_reg);
		vodma_vodma_pq_decmp_sat_en_reg.decmp_ctrl_para = 4;
		vodma_vodma_pq_decmp_sat_en_reg.saturation_en = 1;
		vo_rtd_outl(VODMA_VODMA_PQ_DECMP_SAT_EN_reg, vodma_vodma_pq_decmp_sat_en_reg.regValue);
