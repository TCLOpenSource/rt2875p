...
HAL_LED_PANEL_INFO_T PANEL_INFO_T = {55,HAL_BL_DIRECT_L,0,0,0,0};

DRV_LDM_CUSTOME_INFO ldm_custome_info = {0};
...
long vpq_led_ioctl(struct file *file, unsigned int cmd,  unsigned long arg)
{
	...
	case VPQ_LED_IOC_SET_LD_INIT:
		{
			...
			printk("[LDM]Warning!!!kernel VPQ_LED_IOC_SET_LD_INIT NOT use!!\n");
			return -1;
	...
	case VPQ_IOC_SET_LD_INIT:
		{
			//unsigned char src_idx=0,TableIdx=3;
			unsigned char temp;
			
			rtd_pr_vpq_err("VPQ_IOC_SET_LD_INIT start!!\n");
			if (copy_from_user(&temp, (int __user *)arg, sizeof(unsigned char))) {
				rtd_pr_vpq_err("kernel VPQ_IOC_SET_LD_INIT fail\n");
				ret = -1;
			}
			#if 0
			fwif_color_set_LD_Data_Compensation(src_idx, TableIdx);
			...
			printk("[LDM]VPQ_LED_IOC_SET_LD_INIT_SLAVE done!!temp=%d \n",temp);
		}
		break;
	...
	case VPQ_LED_IOC_SET_LDSetLUT:
		{
			...
			if (copy_from_user(&vpq_led_LD_lutTableIndex, (int __user *)arg, sizeof(UINT8))) {
				...
				printk("[ldm] VPQ_LED_LDSetLUT success,lutTableIndex=%d \n",vpq_led_LD_lutTableIndex);
				ret = 0;
			}
	...
	case VPQ_LED_IOC_SET_SLR_VIP_TABLE_LD:
		{
			...
			if (copy_from_user(&Local_Dimming_Table[0],  (int __user *)arg,sizeof(Local_Dimming_Table))) {
				...
				/*LD table initialization*/
				vpq_led_LD_lutTableIndex = 3;
				fwif_color_LD_Table_init(0,vpq_led_LD_lutTableIndex);		
				printk("[LDM] VPQ_LED_IOC_SET_SLR_VIP_TABLE_LD success\n");
				ret = 0;
			}
		}
		break;
      ...
      case VPQ_LED_IOC_SET_SLR_VIP_TABLE_LDSPI:
		{
			...
			if (copy_from_user(&LDSPI_Table[0],  (int __user *)arg,sizeof(LDSPI_Table))) {
					...
					rtd_pr_vpq_debug("kernel VPQ_LED_IOC_SET_LD_INIT start init\n");
					...
					fwif_color_Set_LD_Init_Done(true);
					printk("[ldm]kernel VPQ_LED_IOC_SET_LD_INIT success\n");
					ret = 0;	
			}
		}
		break;
	...
	case VPQ_LED_IOC_SET_LD_CUSTOME_INFO:
		{						
			if (copy_from_user(&ldm_custome_info,	(int __user *)arg,sizeof(ldm_custome_info))) {
				ret = -1;
				printk("[ldm]VPQ_LED_IOC_SET_LD_CUSTOME_INFO fail!\n");
			} else {
				printk("**[ldm]head ext length = %d,%d \n",ldm_custome_info.u8cmdlength,ldm_custome_info.u8ExtDataLength);
				printk("[ldm]head ext data length = %d,%d \n",sizeof(ldm_custome_info.u8MspiHead),sizeof(ldm_custome_info.u8ExtData));				
				printk("[ldm]tail0-6: %d,%d,%d,%d,%d,%d,\n",ldm_custome_info.u8ExtData[0],ldm_custome_info.u8ExtData[1],
					ldm_custome_info.u8ExtData[2],ldm_custome_info.u8ExtData[3],ldm_custome_info.u8ExtData[4],
					ldm_custome_info.u8ExtData[5],ldm_custome_info.u8ExtData[6]);
				if((ldm_custome_info.u8cmdlength > sizeof(ldm_custome_info.u8MspiHead))
					||(ldm_custome_info.u8ExtDataLength > sizeof(ldm_custome_info.u8ExtData))
				){
					printk("[ldm]Warnning:head/ext length invalid,use default setting!\n");
					ldm_custome_info.u8cmdlength = 4;
					ldm_custome_info.u8MspiHead[0] = 0x55;
					ldm_custome_info.u8MspiHead[1] = 0xaa;
					ldm_custome_info.u8MspiHead[2] = 0x00;
					ldm_custome_info.u8MspiHead[3] = 0x34;
					
					ldm_custome_info.u8ExtDataLength = 3;
					ldm_custome_info.u8ExtData[0] = 0xff;
					ldm_custome_info.u8ExtData[1] = 0xff;
					ldm_custome_info.u8ExtData[2] = 0xff;
				}
			}
		}
		break;
	...
	case VPQ_LED_IOC_LDSetDuty:
		{
			...
			if (copy_from_user(&Duty, (int __user *)arg, sizeof(unsigned short))) {
				...
				printk("[ldm] VPQ_LED_IOC_LDSetDuty success\n");
				ret = 0;
			}
	...
	case VPQ_LED_IOC_SET_LDSPI_SRAM_Data:
		{						
			...
			if (copy_from_user(&tLD_LDSPI_DataSRAM,  (int __user *)arg,sizeof(tLD_LDSPI_DataSRAM))) {
				...
				if(m_pCacheStartAddr == NULL){
					printk("[ERROR]VPQ_LED_IOC_SET_LDSPI_SRAM_Data Allocate SRAM_Length=%x fail\n",tLD_LDSPI_DataSRAM.SRAM_Length);
					return -1;
				}
				...
				if (copy_from_user((void *)m_pCacheStartAddr, (int __user *)tLD_LDSPI_DataSRAM.SRAM_Value,tLD_LDSPI_DataSRAM.SRAM_Length*sizeof(unsigned short))) {
					printk("kernel VPQ_LED_IOC_SET_LDSPI_SRAM_Data copy SRAM_Data fail\n");
					ret = -1;
				} else {
					if(*m_pCacheStartAddr < 10 || *m_pCacheStartAddr > 250){
						printk("[ldm]***SRAM_Position=0x%x, SRAM_Value[0]=%d\n",tLD_LDSPI_DataSRAM.SRAM_Position,*m_pCacheStartAddr);
					}
					tLD_LDSPI_DataSRAM.SRAM_Value = m_pCacheStartAddr;
					tLD_LDSPI_DataSRAM.SRAM_Position = 0x404;
		...
		break;
	case VPQ_LED_IOC_SET_LDSPI_SRAM_Data_TV030:
		{						
			DRV_LD_LDSPI_DATASRAM_TYPE_TV030 tLD_LDSPI_DataSRAM;

			if (copy_from_user(&tLD_LDSPI_DataSRAM,  (int __user *)arg,sizeof(tLD_LDSPI_DataSRAM))) {
				printk("kernel VPQ_LED_IOC_SET_LDSPI_SRAM_Data_TV030 fail\n");
				ret = -1;
			} else {
				if(tLD_LDSPI_DataSRAM.SRAM_Value < 10 || tLD_LDSPI_DataSRAM.SRAM_Value > 250){
					printk("[ldm]SRAM_posi=0x%x,value=%d\n", tLD_LDSPI_DataSRAM.SRAM_Position,tLD_LDSPI_DataSRAM.SRAM_Value);
				}
				ldm_custome_info.u8ExtData[0] = tLD_LDSPI_DataSRAM.SRAM_Value;
				fwif_color_Set_LD_SPI_Header(ldm_custome_info.u8cmdlength,&ldm_custome_info.u8MspiHead[0],ldm_custome_info.u8ExtDataLength,&ldm_custome_info.u8ExtData[0]);
