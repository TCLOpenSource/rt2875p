...
char I3DDMA_4xxto4xx_Downsample(I3DDMA_COLOR_SPACE_T source, I3DDMA_COLOR_SPACE_T target) {
	...
	h3ddma_rgb2yuv_dither_4xxto4xx_hdmi_444to422_ctrl_RBUS h3ddma_rgb2yuv_dither_4xxto4xx_hdmi_444to422_ctrl_reg;
	...
	//sync i3ddma uzd color issue from S4AP @Crixus 20170620
	   ...
	   if(source == I3DDMA_COLOR_YUV422 && (drvif_I3DDMA_Get_Input_Info_Vaule(I3DDMA_INFO_TARGET_COLOR)==I3DDMA_COLOR_YUV444)){
	...
	h3ddma_rgb2yuv_dither_4xxto4xx_hdmi_444to422_ctrl_reg.regValue = IoReg_Read32(H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_444to422_CTRL_reg);
	if(source == I3DDMA_COLOR_YUV422)
		h3ddma_rgb2yuv_dither_4xxto4xx_hdmi_444to422_ctrl_reg.en_444to422 = 0; //yuv422 bypass (0)
	else if(source == I3DDMA_COLOR_RGB)
	{
		if(target == I3DDMA_COLOR_YUV422)
			h3ddma_rgb2yuv_dither_4xxto4xx_hdmi_444to422_ctrl_reg.en_444to422 = 1; //444 to 422
	 	else
			h3ddma_rgb2yuv_dither_4xxto4xx_hdmi_444to422_ctrl_reg.en_444to422 = 0;//keep 444
	}
	else
	{//input 444
		if(target == I3DDMA_COLOR_YUV422)
	...
	h3ddma_rgb2yuv_dither_4xxto4xx_hdmi_444to422_ctrl_reg.sel_444to422 = 0; //sel after rgb2yuv output
	IoReg_Write32(H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_444to422_CTRL_reg, h3ddma_rgb2yuv_dither_4xxto4xx_hdmi_444to422_ctrl_reg.regValue);
	...
	if (source == I3DDMA_COLOR_RGB) {
		...
		{
			rtd_pr_i3ddma_debug("av source, idma bypass rgb2yuv\n");
			i3ddma_rgb2yuv_dither_4xxto4xx_hdmi_rgb2yuv_ctrl_reg.regValue = rtd_inl(H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_RGB2YUV_CTRL_reg);
			i3ddma_rgb2yuv_dither_4xxto4xx_hdmi_rgb2yuv_ctrl_reg.en_rgb2yuv = 0;
			rtd_outl(H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_RGB2YUV_CTRL_reg, i3ddma_rgb2yuv_dither_4xxto4xx_hdmi_rgb2yuv_ctrl_reg.regValue);
		}
...
char I3DDMA_Dithering12to10(I3DDMA_COLOR_DEPTH_T source, I3DDMA_COLOR_DEPTH_T target) {
	...
	if(target >= I3DDMA_COLOR_DEPTH_10B)
	{
		for(ii = 0; ii < I3DDMA_DITHERLUT_ROW_MAX; ii++)
		{
			for(jj = 0; jj < I3DDMA_DITHERLUT_COL_MAX; jj++)
			{
				...
				if(jj == 1)
				{
					...
					{
						case 0: // red
								rtd_outl(H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_DITHER_TB_R_00_reg, dither_32BIT_DATA);
								break;
						case 1: // green
								rtd_outl(H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_DITHER_TB_G_00_reg, dither_32BIT_DATA);
								break;
						case 2: // blue
								rtd_outl(H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_DITHER_TB_B_00_reg, dither_32BIT_DATA);
								break;
					}
				}
				...
				if(jj == 3)
				{
					...
					{
						case 0: // red
								rtd_outl(H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_DITHER_TB_R_02_reg, dither_32BIT_DATA);
								break;
						case 1: // green
								rtd_outl(H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_DITHER_TB_G_02_reg, dither_32BIT_DATA);
								break;
						case 2: // blue
								rtd_outl(H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_DITHER_TB_B_02_reg, dither_32BIT_DATA);
								break;
					}
				}
		...
		rtd_outl(H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_DITHER_Temp_TB_reg, dither_32BIT_DATA);
...
#if 0 //@jiawei_tang for K6 compiler error
	else
	{
		for(ii = 0; ii < I3DDMA_DITHERLUT_ROW_MAX; ii++)
		{
...
static unsigned short tRGB2YUV_COEF_601_RGB_0_255[] = {
	...
	0x7d40,   /* m21 >> 2 0x01D5 up bits at AT Name: cychen2 , Date: 2010/3/25*/
	0x7a93,   /* m22 >> 1*/
	0x082d,   /* m23 >> 2*/
	0x082d,   /* m31 >> 2*/
	0x7927,   /* m32 >> 2 0x0195  up bits at AT Name: cychen2 , Date: 2010/3/25*/
	0x7eac,   /* m33 >> 1*/
...
//#endif
	...
	rtd_pr_i3ddma_debug("i3ddma_rgb2yuv_tab1_y_reg.regValue = %x\n", i3ddma_rgb2yuv_tab1_y_reg.regValue);
