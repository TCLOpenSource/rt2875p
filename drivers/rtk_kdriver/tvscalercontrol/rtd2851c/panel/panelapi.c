...
UINT16                               panel_support_max_frame_rate = 0;
...
void Panel_InitParameter(PANEL_CONFIG_PARAMETER *parameter)
{
	...
	panel_support_max_frame_rate = pPanelCOnfigParameter->iCONFIG_DISPLAY_REFRESH_RATE;
...
unsigned int Get_DISPLAY_CLOCK_TYPICAL(void)
{
	...
	if(vbe_disp_get_game_mode_dynamic_change_clk_flag()) {
		if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= 235) &&
			(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) <= 605))
			return Get_DISPLAY_REFRESH_RATE() * Get_DISP_HORIZONTAL_TOTAL() * Get_DISP_VERTICAL_TOTAL();
		else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= 995) &&
			(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) <= 1205))
			return Get_DISPLAY_REFRESH_RATE() * Get_DISP_HORIZONTAL_TOTAL() * Get_DISP_VERTICAL_TOTAL();
	}
...
void Panel_TurnOff(void)
{
	...
	if(Panel_Get_STR_BackLight_Ctrl_Mode() == _BACKLIGHT_STR_CTRL_MODE_BY_KERNEL){
		...
		rtd_pr_vbe_notice("[%s] STR BACKLIGHT CTRL MODE_BY_AP\n",__FUNCTION__);
	}
	else{
		Panel_SetBackLightMode(0);
		rtd_pr_vbe_notice("[%s]  BACKLIGHT OFF\n",__FUNCTION__);
	}
...
void panel_setup_PMIC(void)
{
	...
	unsigned char pmic_1F_42_data[2][2] = {{0x00, 0x1F}, {0x01, 0x42}};
	unsigned char pmic_F_42_data[2][2] = {{0x00, 0x0F}, {0x01, 0x42}};
	unsigned char pmic_F_65_data[2][2] = {{0x00, 0x0F}, {0x01, 0x65}};
	unsigned char pmic_25_data[2] = {0x00, 0x25};
