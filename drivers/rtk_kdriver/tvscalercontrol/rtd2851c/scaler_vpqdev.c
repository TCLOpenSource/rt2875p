...
struct semaphore VPQ_DDomain_DMA_TBL_Semaphore;
...
char vpq_IP_Status_Check_handler(void)
{
	...
	if (VIP_IP_Status_Check_List_TBL == NULL) {
		rtd_pr_vpq_emerg("VIP_IP_Status,VIP_IP_Status_Check_List_TBL = NULL\n");
		return -1;
	}
	...
	for (i=0;i<num_max;i++) {
		...
		if (reg_addr_clk != 0) {	// need to check clk	
			...
			if (clk_val == 1) { // clk is ok
				...
				rtd_pr_vpq_emerg("VIP_IP_Status,%d.%s(0x%x[%d:%d])=%d,clk(0x%x[%d])=%d,\n", 
					i ,name ,reg_addr, reg_shift+bit_size-1, reg_shift, val, reg_addr_clk, reg_shift_clk, clk_val);
				
			} else {
				rtd_pr_vpq_emerg("VIP_IP_Status,%d.%s(0x%x[%d:%d]),clk(0x%x[%d])=%d,\n", 
			...
			val = drvif_color_get_reg_Val(reg_addr, reg_shift, reg_mask);
			rtd_pr_vpq_emerg("VIP_IP_Status,%d.%s(0x%x[%d:%d])=%d,\n", 
...
char vpq_self_Check_handler(void)
{
	...
	if(vipSys_info == NULL){
		rtd_pr_vpq_emerg("VIP_self_Check,vipSys_info = NULL, fail\n");
		ch_result--;
	} else {		
		if (vipSys_info->DolbyHDR_flag == 1)
			//rtd_pr_vpq_emerg("VIP_self_Check,self check with dolby vision content\n");			
			sprintf(HDR_type_str, "Dolby Vision HDR");
		else if (vipSys_info ->HDR_flag == HAL_VPQ_HDR_MODE_SDR)
			//rtd_pr_vpq_emerg("VIP_self_Check,self check with HAL_VPQ_HDR_MODE_SDR\n");
			sprintf(HDR_type_str, "SDR Content");
		else if (vipSys_info ->HDR_flag == HAL_VPQ_HDR_MODE_HDR10)
			//rtd_pr_vpq_emerg("VIP_self_Check,self check with HAL_VPQ_HDR_MODE_HDR10\n");
			sprintf(HDR_type_str, "HDR10 Content");
		else if (vipSys_info ->HDR_flag == HAL_VPQ_HDR_MODE_HLG)
			//rtd_pr_vpq_emerg("VIP_self_Check,self check with HAL_VPQ_HDR_MODE_HLG\n");
			sprintf(HDR_type_str, "HLG Content");
		else if (vipSys_info ->HDR_flag == HAL_VPQ_HDR_MODE_SDR_MAX_RGB)
			//rtd_pr_vpq_emerg("VIP_self_Check,self check with HAL_VPQ_HDR_MODE_SDR_MAX_RGB\n");
			sprintf(HDR_type_str, "SDR Max RGB Content");
		else
			//rtd_pr_vpq_emerg("VIP_self_Check,self check with unknow stream=%d\n", vipSys_info ->HDR_flag);
			sprintf(HDR_type_str, "Unknow Content");
		rtd_pr_vpq_emerg("VIP_self_Check,self check with %s\n", HDR_type_str);

		// hdr dm check
		drvif_Self_Check_Reg_Get(SLR_MAIN_DISPLAY, &Self_Check_Reg_Get);
		// check eotf
		rtd_pr_vpq_emerg("VIP_self_Check,EOTF check\n");	
		if (Self_Check_Reg_Get.eotf_lut_enable == 1) {
			...
			if (EOTF_table32 != NULL) {
				...
				if (curve_isFail == 0)
					rtd_pr_vpq_emerg("VIP_self_Check,EOTF is monotonically increasing\n");
				else {
					rtd_pr_vpq_emerg("VIP_self_Check,EOTF is not monotonically increasing, EOTF check fail\n");
					ch_result--;
				}
				...
				if (curve_isFail == 0)
					rtd_pr_vpq_emerg("VIP_self_Check,EOTF is not all 0\n");
				else {
					rtd_pr_vpq_emerg("VIP_self_Check,EOTF is all 0, EOTF check fail\n");
					ch_result--;
				}
				...
				ch_result--;
				rtd_pr_vpq_emerg("VIP_self_Check,EOTF malloc fail\n");
			}
		} else {
			rtd_pr_vpq_emerg("VIP_self_Check,EOTF is off, skip check\n");
		}

		// check b01-04
		rtd_pr_vpq_emerg("VIP_self_Check,dm 3x3 check\n");	
		if (Self_Check_Reg_Get.dm_b0104_enable == 1) {
			...
			if (coef != NULL) {
				...
				for (i=0;i<9;i++)
					temp = temp + coef[i];
				if (temp != 0)
					rtd_pr_vpq_emerg("VIP_self_Check,dm 3x3 is not all 0\n");
				else {
					rtd_pr_vpq_emerg("VIP_self_Check,dm 3x3 is all 0, dm 3x3  check fail\n");
					ch_result--;
				}
				...
				ch_result--;
				rtd_pr_vpq_emerg("VIP_self_Check,dm 3x3 malloc fail\n");
			}
		} else {
			rtd_pr_vpq_emerg("VIP_self_Check,dm 3x3 is off, skip check\n");
		}

		// check oetf
		rtd_pr_vpq_emerg("VIP_self_Check,OETF check\n");	
		if (Self_Check_Reg_Get.oetf_lut_enable == 1) {
			...
			if (OETF_table16 != NULL) {
				...
				if (curve_isFail == 0)
					rtd_pr_vpq_emerg("VIP_self_Check,OETF is monotonically increasing\n");
				else {
					rtd_pr_vpq_emerg("VIP_self_Check,OETF is not monotonically increasing, OETF check fail\n");
					ch_result--;
				}
#endif
				// table check, curve is all 0 or not
				...
				if (curve_isFail == 0)
					rtd_pr_vpq_emerg("VIP_self_Check,OETF is not all 0\n");
				else {
					rtd_pr_vpq_emerg("VIP_self_Check,OETF is all 0, OETF check fail\n");
					ch_result--;
				}
				...
				ch_result--;
				rtd_pr_vpq_emerg("VIP_self_Check,OETF malloc fail\n");
			}
		} else {
			rtd_pr_vpq_emerg("VIP_self_Check,OETF is off, skip check\n");
		}

		// check b05-01
		rtd_pr_vpq_emerg("VIP_self_Check,CSC3 check\n");	
		if (Self_Check_Reg_Get.dm_b0501_enable == 1) {
			...
			if (pMax_Min_Inv != NULL) {
				...
				if ((pInv[0] != 0) && (pInv[1] != 0) && (pInv[2] != 0))
					rtd_pr_vpq_emerg("VIP_self_Check,CSC3 Inv c1-3 is not 0\n");
				else {
					rtd_pr_vpq_emerg("VIP_self_Check,CSC3 Inv c1-3 check fail, CINV=%d,%d,%d,\n", pInv[0], pInv[1], pInv[2]);
					ch_result--;
				}
				...
				ch_result--;
				rtd_pr_vpq_emerg("VIP_self_Check,CSC3 malloc fail\n");
			}
		} else {
			rtd_pr_vpq_emerg("VIP_self_Check,CSC3 is off, skip check\n");
		}	
		...
		rtd_pr_vpq_emerg("VIP_self_Check,Color bar check, panel_width=%d, panel_heigth=%d, h_s=%d, h_e=%d, v_s=%d, v_e=%d, color_bar_wid=%d, dataaccess_bit_num=%d, dataaccess_half_val=%d\n", 
		...
		if ((R>dataaccess_half_val) && (G>dataaccess_half_val) && (B>dataaccess_half_val)) {
			rtd_pr_vpq_emerg("VIP_self_Check,white point is ok, [%d,%d]=R:G:B=%d,%d,%d, ch_result=%d\n", x, y, R, G, B, ch_result);
		} else {
			ch_result--;
			rtd_pr_vpq_emerg("VIP_self_Check,white point fail, [%d,%d]=R:G:B=%d,%d,%d, ch_result=%d\n", x, y, R, G, B, ch_result);
		}
		...
		drvif_color_get_pcid_data_access_read(&R, &G, &B);
		if ((R>dataaccess_half_val) && (G>dataaccess_half_val) && (B<dataaccess_half_val)) {
			rtd_pr_vpq_emerg("VIP_self_Check,yellow point is ok, [%d,%d]=R:G:B=%d,%d,%d, ch_result=%d\n", x, y, R, G, B, ch_result);
		} else {
			ch_result--;
			rtd_pr_vpq_emerg("VIP_self_Check,yellow point fail, [%d,%d]=R:G:B=%d,%d,%d, ch_result=%d\n", x, y, R, G, B, ch_result);
		}
		...
		drvif_color_get_pcid_data_access_read(&R, &G, &B);
		if ((R<dataaccess_half_val) && (G>dataaccess_half_val) && (B>dataaccess_half_val)) {
			rtd_pr_vpq_emerg("VIP_self_Check,cyan point is ok, [%d,%d]=R:G:B=%d,%d,%d, ch_result=%d\n", x, y, R, G, B, ch_result);
		} else {
			ch_result--;
			rtd_pr_vpq_emerg("VIP_self_Check,cyan point fail, [%d,%d]=R:G:B=%d,%d,%d, ch_result=%d\n", x, y, R, G, B, ch_result);
		}
		...
		drvif_color_get_pcid_data_access_read(&R, &G, &B);
		if ((R<dataaccess_half_val) && (G>dataaccess_half_val) && (B<dataaccess_half_val)) {
			rtd_pr_vpq_emerg("VIP_self_Check,green point is ok, [%d,%d]=R:G:B=%d,%d,%d, ch_result=%d\n", x, y, R, G, B, ch_result);
		} else {
			ch_result--;
			rtd_pr_vpq_emerg("VIP_self_Check,green point fail, [%d,%d]=R:G:B=%d,%d,%d, ch_result=%d\n", x, y, R, G, B, ch_result);
		}
		...
		drvif_color_get_pcid_data_access_read(&R, &G, &B);
		if ((R>dataaccess_half_val) && (G<dataaccess_half_val) && (B>dataaccess_half_val)) {
			rtd_pr_vpq_emerg("VIP_self_Check,Magenta point is ok, [%d,%d]=R:G:B=%d,%d,%d, ch_result=%d\n", x, y, R, G, B, ch_result);
		} else {
			ch_result--;
			rtd_pr_vpq_emerg("VIP_self_Check,Magenta point fail, [%d,%d]=R:G:B=%d,%d,%d, ch_result=%d\n", x, y, R, G, B, ch_result);
		}
		...
		drvif_color_get_pcid_data_access_read(&R, &G, &B);
		if ((R>dataaccess_half_val) && (G<dataaccess_half_val) && (B<dataaccess_half_val)) {
			rtd_pr_vpq_emerg("VIP_self_Check,Red point is ok, [%d,%d]=R:G:B=%d,%d,%d, ch_result=%d\n", x, y, R, G, B, ch_result);
		} else {
			ch_result--;
			rtd_pr_vpq_emerg("VIP_self_Check,Red point fail, [%d,%d]=R:G:B=%d,%d,%d, ch_result=%d\n", x, y, R, G, B, ch_result);
		}
		...
		drvif_color_get_pcid_data_access_read(&R, &G, &B);
		if ((R<dataaccess_half_val) && (G<dataaccess_half_val) && (B>dataaccess_half_val)) {
			rtd_pr_vpq_emerg("VIP_self_Check,Blue point is ok, [%d,%d]=R:G:B=%d,%d,%d, ch_result=%d\n", x, y, R, G, B, ch_result);
		} else {
			ch_result--;
			rtd_pr_vpq_emerg("VIP_self_Check,Blue point fail, [%d,%d]=R:G:B=%d,%d,%d, ch_result=%d\n", x, y, R, G, B, ch_result);
		}
	}
	
	if (ch_result == 0) {
		rtd_pr_vpq_emerg("VIP_self_Check,!!!!%s VIP self check final result is ok!!!!\n", HDR_type_str);
	} else {
		rtd_pr_vpq_emerg("VIP_self_Check,!!!!%s VIP self check final result is fail!!!!, fail number=%d\n", HDR_type_str, (ch_result*(-1)));
	}
...
//AT V6.3 Patch 2022/08/04 - END
	...
	if(strcmp(cmd_buf, "testmode=1\n") == 0){
		...
		rtd_pr_vpq_emerg("PQ_bypass_mode1\n");
...
{
	extern struct semaphore Gamma_Semaphore;
    rtd_pr_vpq_emerg("vpq_set_gamma\n");
...
if(is_QS_pq_enable()==1) { //current is QS flow	pq_quick_show flow
    ...
    rtd_pr_vpq_info("Panel_Dither_Enable()=%d\n",Get_PANEL_DITHER_ENABLE());
    rtd_pr_vpq_info("Panel_Dither_Temporal_Enabl()=%d",Get_PANEL_PANEL_DITHER_TEMPORAL_ENABLE());
    rtd_pr_vpq_info("Setditherbit()=%d",Get_PANEL_PANEL_DITHER_BIT_TABLE_SELECT());
    rtd_pr_vpq_info("Panel_Dither_OLED_Enable()=%d",Get_PANEL_PANEL_DITHER_OLED_TH_EN());
    rtd_pr_vpq_info("Panel_Dither_OLED_Th()=%d",Get_PANEL_PANEL_DITHER_OLED_TH());
	...
	Set_Var_VPQ_rtpm_tsk_block_SE_done(0);
	Set_Var_VPQ_rtpm_tsk_SE_freeze_stauts(0);
...
void od_do_suspend(void)
{
	...
	drvif_color_od_table_read(&(od_table_store_r[0]), 0, 0);//R
	drvif_color_od_table_read(&(od_table_store_g[0]), 1, 0);//G
	drvif_color_od_table_read(&(od_table_store_b[0]), 2, 0);//B
}
...
void od_do_resume(void)
{
	...
	if (Scaler_Init_OD()) {
		...
		drvif_color_od_table_write(&(od_delta_table_resume[0]), 0, 0);// 0:delta mode 1:target mode(output mode) R
		fwif_color_od_table_17x17_transform(0, od_table_store_g, od_delta_table_resume);
		drvif_color_od_table_write(&(od_delta_table_resume[0]), 0, 1);// 0:delta mode 1:target mode(output mode) G
		fwif_color_od_table_17x17_transform(0, od_table_store_b, od_delta_table_resume);
		drvif_color_od_table_write(&(od_delta_table_resume[0]), 0, 2);// 0:delta mode 1:target mode(output mode) B
	...
	//drvif_color_od_table_write(&(od_table_store_w[0]), 1, 4);// 0:delta mode 1:target mode(output mode) W
...
static int se_tsk(void *p)//This task run se proc
{
	...
	while (1)
	{
   	...
   	_RPC_system_setting_info* RPC_system_info_structure_table = NULL;
		...
		if (freezing(current))
		{
			Set_Var_VPQ_rtpm_tsk_SE_freeze_stauts(1);
			try_to_freeze();
		}
		Set_Var_VPQ_rtpm_tsk_SE_freeze_stauts(0);
		...
		if (vpq_rtpm_tsk_block == 1) {
			msleep(1);
			Set_Var_VPQ_rtpm_tsk_block_SE_done(1);
			continue;
		} else {
			Set_Var_VPQ_rtpm_tsk_block_SE_done(0);
		}
		...
		if ((try_lock_times <= 0) || (vpq_rtpm_tsk_block == 1)) {
			//rtd_printk(KERN_INFO, TAG_NAME, "VPQ_RTPM, try_lock_times=%d, vpq_rtpm_tsk_block=%d,,\n", try_lock_times, vpq_rtpm_tsk_block);
			continue;
		}
...
static void create_se_tsk(void)
{
	...
	if (Get_Var_VPQ_se_tsk_running_flag() == FALSE) {
		p_se_tsk = kthread_create(se_tsk, NULL, "se_tsk");

	    if (p_se_tsk) {
			wake_up_process(p_se_tsk);
			Set_Var_VPQ_se_tsk_running_flag(TRUE);
	    } else {
...
{
	int ret;
	if (Get_Var_VPQ_se_tsk_running_flag()) {
 		ret = kthread_stop(p_se_tsk);
 		if (!ret) {
 			p_se_tsk = NULL;
 			Set_Var_VPQ_se_tsk_running_flag(FALSE);
...
int vpq_rumtime_resume(struct device *p_dev)
{
	...
	fwif_color_inv_gamma_control_back(SLR_MAIN_DISPLAY, 0);
	//vpq_set_gamma(1, 0);
	//fwif_color_gamma_control_back(SLR_SUB_DISPLAY, 0);
	fwif_color_colorwrite_Output_gamma(Output_InvOutput_Gamma_TBL_Reserved04);
	drvif_color_inv_gamma_enable(SLR_MAIN_DISPLAY, 0, 0);
	drvif_color_inv_gamma_enable(SLR_SUB_DISPLAY, 0, 0);
	drvif_color_set_Vivid_Color_Enable(TRUE);
...
{
	unsigned char ret;
	unsigned char freeze_VPQ_done=0,se_done=0;
	...
	if (Get_Var_VPQ_se_tsk_running_flag() == 1)
		se_done = Get_Var_VPQ_rtpm_tsk_block_SE_done();
	else
		se_done = 1;

	if ((Get_Var_VPQ_rtpm_tsk_VPQ_freeze_stauts() == 1)&&(Get_Var_VPQ_rtpm_tsk_SE_freeze_stauts()==1)){
		ret = 1;
	}
	else if (((freeze_VPQ_done == 1)&&(se_done == 1)) && (Get_Var_VPQ_rtpm_tsk_block() == 1) ){
		ret = 1;
	}
	else{
		ret = 0;
...
static long VPQ_IOC_SET_HDR_Eotf_impl(unsigned long arg)
{
	...
	else
	{
		...
		//UINT16 i;
		//for (i = 0; i < EOTF_size; i++)
			//rtd_pr_vpq_info("roger, Eotf[%d] = %d\n", i, Eotf_table32[i]);

		fwif_color_set_DM2_EOTF_TV006(Eotf_table32, Eotf_table32, Eotf_table32); //SEAMLESS mode block it
	}
	return ret;
}
...
static long VPQ_IOC_SET_HDR_Oetf_impl(unsigned long arg)
{
	...
	else
	{
		...
		//UINT16 i;
		//for (i = 0; i < OETF_size; i++)
			//rtd_pr_vpq_info("roger, Oetf[%d] = %d\n", i, Oetf_table16[i]);

		fwif_color_set_DM2_OETF_TV006(Oetf_table16, Oetf_table16, Oetf_table16);
	}
	return ret;
}
	...
	if (Get_Factory_SelfDiagnosis_Mode()) {
		...
		if (copy_from_user(&lowdelaymode, (int __user *)arg, sizeof(bool))) {
...
#endif
			rtd_pr_vpq_notice("VPQ_IOC_SET_LOWDELAY_MODE %d\n", lowdelaymode);
			if(lowdelaymode) {
				//rtd_pr_vpq_emerg("[crixus]VPQ_IOC_SET_LOWDELAY_MODE1\n");
				//game mode dynamic, check in vsc task @Crixus 20151228
				drv_memory_set_game_mode_dynamic(_ENABLE);
			} else {
				//rtd_pr_vpq_emerg("[crixus]VPQ_IOC_SET_LOWDELAY_MODE0\n");
				//game mode dynamic, check in vsc task @Crixus 20151228
				drv_memory_set_game_mode_dynamic(_DISABLE);
			}
			...
			if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE){
				...
				//rtd_pr_vpq_emerg("[crixus]game_mode_cmd_check = %d\n", game_mode_cmd_check);
			}
			ret = 0;
		}
	}
...
#if defined(CONFIG_RTK_AI_DRV) || defined(CONFIG_RTK_AI_DRV_MODULE)
static long VPQ_IOC_SET_AI_PA_impl(struct file *file,unsigned long arg)
{
	int ret = 0;
	AI_NN_BUF_INFO info;
	VIP_NN_CTRL *pNN;
...
static long VPQ_IOC_GET_AI_MODE_impl(unsigned long arg)
{
	int ret = 0;
...
static long VPQ_IOC_AI_DETECTION_ON_impl(unsigned long arg)
{
	int ret = 0;
	unsigned int list;
	...
	{
		ret = -1;
		rtd_pr_vpq_info("VPQ_IOC_AI_DETECTION_ON fail\n");
		return ret;
	}
...
static long VPQ_IOC_AI_DETECTION_OFF_impl(unsigned long arg)
{
	int ret = 0;
	unsigned int list;
...
long vpq_ioctl(struct file *file, unsigned int cmd,  unsigned long arg)
{
	...
	case VPQ_IOC_SET_AI_PA:
		ret = VPQ_IOC_SET_AI_PA_impl(file,arg);
	break;
	case VPQ_IOC_GET_AI_MODE:
		ret = VPQ_IOC_GET_AI_MODE_impl(arg);
	break;
	case VPQ_IOC_AI_DETECTION_ON:
		ret = VPQ_IOC_AI_DETECTION_ON_impl(arg);
	break;
	case VPQ_IOC_AI_DETECTION_OFF:
		ret = VPQ_IOC_AI_DETECTION_OFF_impl(arg);
	break;
#endif
	default:
...
int vpq_module_init(void)
{
	...
	sema_init(&VPQ_DDomain_DMA_TBL_Semaphore, 1);
...
unsigned char Get_Var_VPQ_rtpm_tsk_block_SE_done(void)
{
	return vpq_rtpm_tsk_block_SE_done;
}

void Set_Var_VPQ_rtpm_tsk_block_SE_done(unsigned char block_SE_done)
{
	vpq_rtpm_tsk_block_SE_done = block_SE_done;
}

unsigned char Get_Var_VPQ_rtpm_tsk_SE_freeze_stauts(void)
{
	return vpq_rtpm_tsk_SE_freeze_stauts;
}

void Set_Var_VPQ_rtpm_tsk_SE_freeze_stauts(unsigned char SE_freeze_stauts)
{
	vpq_rtpm_tsk_SE_freeze_stauts = SE_freeze_stauts;
}

unsigned char Get_Var_VPQ_se_tsk_running_flag(void)
{
#if defined(CONFIG_RTK_AI_DRV) || defined(CONFIG_RTK_AI_DRV_MODULE)
	return se_tsk_running_flag;
#else
	return 0;
#endif
	}

void Set_Var_VPQ_se_tsk_running_flag(unsigned char se_tsk_run_flag)
{
#if defined(CONFIG_RTK_AI_DRV) || defined(CONFIG_RTK_AI_DRV_MODULE)
	se_tsk_running_flag = se_tsk_run_flag;
#endif
}
