...
#include <scaler_vfedev.h> //ryanliao 2018/09/27
#include <ioctrl/scaler/vfe_cmd_id.h>
#include <tvscalercontrol/avd/avdctrl.h>
#include "rbus/efuse_reg.h"                 //Kenliu 2020/06/05
...
/*
#define SB2_HD_SEM_NEW_5_reg 0xB801A634
#define Get_HD_SEM_NEW_5() while(!IoReg_Read32(SB2_HD_SEM_NEW_5_reg)){rtd_pr_vdc_debug("#@# %s wait SB2_HD_SEM_NEW_5_reg ready\n", moduleName); msleep(20);}
...
unsigned char AD_vdadc_adc_ctrl_9_8 = 0x2;
unsigned char FrontDigitalFilter_Table = VDC_216M_FDF_75MHZ;
...
void AD_vdadc_adc_ctrl_9_8_Setting (VDC_AVD_AV_COEF config)
{
	switch (config)
	{
	case VDC_AVD_AV_CONFIG_0:
		AD_vdadc_adc_ctrl_9_8 = 0x2; //for resistor   75 ohm & capacitor   47 nF
		break;
	case VDC_AVD_AV_CONFIG_1:
		AD_vdadc_adc_ctrl_9_8 = 0x3; //for resistor 110 ohm & capacitor 100 nF
		break;
	}
}

void FrontDigitalFilter_Table_Setting (VDC_AVD_AV_COEF config)
{
	switch (config)
	{
	case VDC_AVD_AV_CONFIG_0:
		FrontDigitalFilter_Table = VDC_216M_FDF_75MHZ;	 //for resistor   75 ohm & capacitor   47 nF
		break;
	case VDC_AVD_AV_CONFIG_1:
		FrontDigitalFilter_Table = VDC_108M_FDF_60_75MHZ; //for resistor 110 ohm & capacitor 100 nF
		break;
	}
}

unsigned char module_vdc_ldo_selection(void)
{
	unsigned int vd_adc_ldo_data = 0;
	unsigned char VDADC_ADC_CTRL_LDO1V0_sel;        //VDADC_ADC_CTRL[5:3]  Kenliu 2020/06/05

	if (rtk_otp_field_read_by_name("hw_vd_adc_ldo", (unsigned char*)&vd_adc_ldo_data, 1) != true) {
		VDC_DEBUG_MSG(VDC_MSG_INIT,"hw_vd_adc_ldo OTP data read fail \n");
		vd_adc_ldo_data = 0;
	}

	VDADC_ADC_CTRL_LDO1V0_sel = (vd_adc_ldo_data & 0x8) ? (vd_adc_ldo_data & 0x7) : 0x1;
	// VDC_DEBUG_MSG(VDC_MSG_INIT,"check to OTP value..... %x \n", vd_adc_ldo_data);

	return VDADC_ADC_CTRL_LDO1V0_sel;
}
...
void module_vdc_ADC_common_setting(void)
{
	...
	tv_sb1_ana_vd_adc_clamp_ctl_2_RBUS tv_sb1_ana_vd_adc_clamp_ctl_2_reg;	// 0xb802004c
	tv_sb1_ana_vd_adc_updn_ctl_RBUS tv_sb1_ana_vd_adc_updn_ctl_reg; 		// 0xb8020050
	tv_sb1_ana_vdadc_core_lpf_ctrl_RBUS tv_sb1_ana_vdadc_core_lpf_ctrl_reg; // 0xb8020054
	tv_sb1_ana_vdadc_ck_ctl_RBUS tv_sb1_ana_vdadc_ck_ctl_reg;				// 0xb8020058
	...
	pll27x_reg_pll27x_vdadc_RBUS reg_pll27x_vdadc_reg; //0xb80004bc
	//T12 modify parameters
	reg_pll27x_vdadc_reg.regValue = rtdf_inl(PLL27X_REG_PLL27X_VDADC_reg);
	reg_pll27x_vdadc_reg.fast_sel = 0x0; // bit[6]
	reg_pll27x_vdadc_reg.pll27x_vdadc_cko_en = 0x1; //bit[5] VDADC CLK enable
	reg_pll27x_vdadc_reg.pll27x_vdadc_cko_sel = 0x3; //bit[4:3] PLL27x VDADC clock output mux select
	reg_pll27x_vdadc_reg.pll27x_vdadc_div = 0x1; // bit[2] Divide value
	reg_pll27x_vdadc_reg.pll27x_vdadc_duty = 0x0; //bit[1:0] VDADC clk duty select
	rtdf_outl(PLL27X_REG_PLL27X_VDADC_reg, reg_pll27x_vdadc_reg.regValue);

	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.regValue = rtdf_inl(TV_SB1_ANA_VD_ADC_CLAMP_CTL_1_reg);
	// VDC_DEBUG_MSG(VDC_MSG_INIT,"[0] 2_0048=%x\n", tv_sb1_ana_vd_adc_clamp_ctl_1_reg.regValue);
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl0_15_14 = 0x0;	/* 2_0048[31:30]  V Channel Input- Select */
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl0_13 = 0x0;		/* 2_0048[29] = 0 */
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl0_11_10 = 0x2;	/* 2_0048[27:26] = 10, swap default */
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl0_9 = 0;		/* 2_0048[25] = 00 */
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl0_8 = 0;		/* 2_0048[24] = 00 */
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl0_7_4 = ucAdcNeg_Sing;	/* 2_0048[23:20] */
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl0_3_0 = 0x7;	/* 2_0048[19:16] = 0x7, default */
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_15 = 0x1;		/* 2_0048[15] = 1, dc restore offset enable(+0.4V) */
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_14 = 0x0;		/* 2_0048[14] = 0x0, Reserved  */
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_13 = 0x0;		/* 2_0048[13] = 0x0, dc restore enable */
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_12 = 0x0;		/* 2_0048[12] = 0 */
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_8 = 0x1;		/* 2_0048[8] = 1, diff. */
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_6 = 0x0;		/* 2_0048[6] = 0x0, Reserved */
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_5 = 0x0;		/* 2_0048[5] = 0x0, clamp en */
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_1_0 = 0x0;	/* 2_0048[1:0] = 0, dc restore */
	...
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.regValue = rtdf_inl(TV_SB1_ANA_VD_ADC_CLAMP_CTL_2_reg);
	// VDC_DEBUG_MSG(VDC_MSG_INIT,"[0] 2_004c=%x\n", tv_sb1_ana_vd_adc_clamp_ctl_2_reg.regValue);
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_14_12 = 0x0;	/* 2_004c[14:12] test mode */
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_11 = 0x0;		/* 2_004c[11] default */
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_10 = 0x0;		/* 2_004c[10] VIN9PP select */
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_9 = 0x0;		/* 2_004c[9] VIN11PP select */
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_8 = 0x0;		/* 2_004c[8] VIN11PP select */
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_7 = 0x0;		/* 2_004c[7] = 0, VDC Slelction */
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_6 = 0x1;		/* 2_004c[6] = 1, clamp current */
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_5 = 0x0;		/* 2_004c[5] = 0, yc swap disable */
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_4_2 = 0x0;	/* 2_004c[4:2] Negative clamp enable */
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_1 = 0x0;		/* 2_004c[1] = 0, Reserved */
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_0 = 0x0;		/* 2_004c[0] = 0, SF avout enable */
	// VDC_DEBUG_MSG(VDC_MSG_INIT,"[1] 2_004c=%x\n", tv_sb1_ana_vd_adc_clamp_ctl_2_reg.regValue);
	rtdf_outl(TV_SB1_ANA_VD_ADC_CLAMP_CTL_2_reg, tv_sb1_ana_vd_adc_clamp_ctl_2_reg.regValue);

	tv_sb1_ana_vd_adc_updn_ctl_reg.regValue = rtdf_inl(TV_SB1_ANA_VD_ADC_UPDN_CTL_reg);
	// VDC_DEBUG_MSG(VDC_MSG_INIT,"[0] 2_0050=%x\n", tv_sb1_ana_vd_adc_updn_ctl_reg.regValue);
	tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_15_14 = 0x0;		/* 2_0050[15:14]=00, ADC Gain Calibration */
	tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_13_11 = 0x0;		/* 2_0050[13:11]=000, default */
	tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_10_8 = 0x0;		/* 2_0050[10:8]=000, Negative UP/DN NCLAMP_EN Enable */
	tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_6_3 = 0x4;		/* 2_0050[6:3]=0100, clamp current */
	tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_1 = 0x0;			/* 2_0050[1]=1, */
	tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_0 = 0x1;			/* 2_0050[0]=01, AV clamp up/dn enable */
	...
	// VDC_DEBUG_MSG(VDC_MSG_INIT,"[1] 2_005c=%x\n", tv_sb1_ana_vdadc_pow_ctl_reg.regValue);
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_15_11= 0x0;	/* 2_005c[31:27]=0, default */
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_10 = 0x0;		/* 2_005c[26]=0, default */
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_9 = 0x0;		/* 2_005c[25]=0, default */
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_8 = 0x0;		/* 2_005c[24]=0, default */
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_7 = 0x0;		/* 2_005c[23]=0, default */
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_6 = 0x0;		/* 2_005c[22]=0, default */
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_5 = 0x0;		/* 2_005c[21]=0, default */
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_4 = 0x0;		/* 2_005c[20]=0, default */
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_3 = 0x0;		/* 2_005c[19]=0, default */
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_2_1 = 0x2;	/* 2_005c[18:17]=2, default */
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_0 = 0x0;		/* 2_005c[16]=0, default */
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_ldo100ma_pow = 0x1;		/* 2_005c[1]=1, LDO100MA_POW */
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_pow = 0x1;			/* 2_005c[0]=1, power on */
	// VDC_DEBUG_MSG(VDC_MSG_INIT,"[1] 2_005c=%x\n", tv_sb1_ana_vdadc_pow_ctl_reg.regValue);
	rtdf_outl(TV_SB1_ANA_VDADC_POW_CTL_reg, tv_sb1_ana_vdadc_pow_ctl_reg.regValue);	// Clayton Note[1/2]: PGA power on first "vdadc_pow"

	tv_sb1_ana_vdadc_core_lpf_ctrl_reg.regValue = rtdf_inl(TV_SB1_ANA_VDADC_CORE_LPF_CTRL_reg);
	// VDC_DEBUG_MSG(VDC_MSG_INIT,"[0] 2_0054=%x\n", tv_sb1_ana_vdadc_core_lpf_ctrl_reg.regValue);
	tv_sb1_ana_vdadc_core_lpf_ctrl_reg.reg_vdadc_adc_ctrl_15_14 = 0x3;	/* 2_0054[31:30]=01 */ /*T12 modify, Reference Buffer Source Current Selection 0x3*/
	//tv_sb1_ana_vdadc_core_lpf_ctrl_reg.reg_vdadc_adc_ctrl_13_12 = 0x0;	/* 2_0054[29:28]=00 */ // T12 reserved
	tv_sb1_ana_vdadc_core_lpf_ctrl_reg.reg_vdadc_adc_ctrl_11_10 = 0x1;	// 2_0054[27:26]=1 T12 modify parameters SAR ADC sample time reduction
	tv_sb1_ana_vdadc_core_lpf_ctrl_reg.reg_vdadc_adc_ctrl_9_8 = AD_vdadc_adc_ctrl_9_8;		/* 2_0054[25:24]=10 */
	tv_sb1_ana_vdadc_core_lpf_ctrl_reg.reg_vdadc_adc_ctrl_7_6 = 0x2;		/* 2_0054[23:22]=01, 1.3V LDO Selection *//* T12 modify parameters, DAC Shifting Selection */
	tv_sb1_ana_vdadc_core_lpf_ctrl_reg.reg_vdadc_adc_ctrl_5_3 = module_vdc_ldo_selection();	/* 2_0054[21:19]=001, 1V LDO Selection(001:0.95V) */
	tv_sb1_ana_vdadc_core_lpf_ctrl_reg.reg_vdadc_adc_ctrl_1 = 0x1;		/* 2_0054[17]=0 */ /*T12 modify parameters, SAR ADC DAC Reset*/	
	tv_sb1_ana_vdadc_core_lpf_ctrl_reg.reg_vdadc_adc_ctrl_0 = 0x0;		/* 2_0054[16]=0, default ADC S/H gain. (0:1X, 1:1.6X) normal signal setting=0x0 after m7p */
	tv_sb1_ana_vdadc_core_lpf_ctrl_reg.reg_vdadc_lpf_ctrl_1_0 = 0x1;		/* 2_0054[1:0]=01, LPF 12.6MHz */
	// VDC_DEBUG_MSG(VDC_MSG_INIT,"[1] 2_0054=%x\n", tv_sb1_ana_vdadc_core_lpf_ctrl_reg.regValue);
	rtdf_outl(TV_SB1_ANA_VDADC_CORE_LPF_CTRL_reg, tv_sb1_ana_vdadc_core_lpf_ctrl_reg.regValue);

	tv_sb1_ana_vdadc_ck_ctl_reg.regValue = rtdf_inl(TV_SB1_ANA_VDADC_CK_CTL_reg);
	// VDC_DEBUG_MSG(VDC_MSG_INIT,"[0] 2_0058=%x\n", tv_sb1_ana_vdadc_ck_ctl_reg.regValue);
	tv_sb1_ana_vdadc_ck_ctl_reg.reg_vdadc_ckin_sel_6 = 0x0;		/* 2_0058[14]=0, clock selection */
	tv_sb1_ana_vdadc_ck_ctl_reg.reg_vdadc_ckin_sel_5 = 0x0;		/* 2_0058[13]=0, 54M clock polarity */
	tv_sb1_ana_vdadc_ck_ctl_reg.reg_vdadc_ckin_sel_4_2 = 0x0;	/* 2_0058[12:10]=0x0 */
	tv_sb1_ana_vdadc_ck_ctl_reg.reg_vdadc_ckout_sel_6 = 0x1;	/* 2_0058[6]=1, default */
	tv_sb1_ana_vdadc_ck_ctl_reg.reg_vdadc_ckout_sel_4 = 0x1;	/* 2_0058[4]=0 */ /*T12 modify parameters ADADC 108M output clock drivider polarity */
	tv_sb1_ana_vdadc_ck_ctl_reg.reg_vdadc_ckout_sel_3_2 = 0x0;	/* 2_0058[3:2]=00 */
	tv_sb1_ana_vdadc_ck_ctl_reg.reg_vdadc_ckout_sel_1_0 = 0x0;	/* 2_0058[1:0]=00, the default should be X1 multiple */
	// VDC_DEBUG_MSG(VDC_MSG_INIT,"[1] 2_0058=%x\n", tv_sb1_ana_vdadc_ck_ctl_reg.regValue);
	rtdf_outl(TV_SB1_ANA_VDADC_CK_CTL_reg, tv_sb1_ana_vdadc_ck_ctl_reg.regValue);
...
unsigned char module_vdc_SetADCPath(unsigned char path_signal, unsigned char path_gnd)
{
	...
	ucNClampUpDn = tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_10_8;

	switch (path_signal) {
	/* V Channel Input+ : 1P,4P,7P	*/
	/* 2_0008[11:10]=VD Channel Input+ Select VIN_VD_i(00:GND,01:VIN1P,10:VIN4P,11:VIN7P)*/
	case VIDEO_INPUT1:
		// adc_ctl_rgb12_reg.adc_ctl_rgb1_11_10 = 0x1; /* 2_0008[11:10]=01:VIN1P */
		// adc_ctl_rgb12_reg.adc_ctl_rgb1_7 = 0x0; 	/* 2_0008[7]=0:G-ch in VIN1PP floating */
		//adc_ctl_rgb12_reg.adc_ctl_rgb1_3_2 = 0x3; /*[JF]YPP's setting, so mark it!! 2_0008[3:2]=11:X */
		goto _VIN_VD_i;
	case VIDEO_INPUT4:
	...
	case VIDEO_INPUT7:
		...
		goto _VIN_VD_i;
_VIN_VD_i:
		tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_10_9 = 0x0;	/* 2_0048[10:9]=00, VIN9PP[VIN_VD_i] */
		// vd_adc_clamp_ctl_2_reg.vdadc_clamp_ctrl2_10 = 0x1;	/* 2_004c[10]=1, VIN13P */
		tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_9 = 0x0;	/* 2_004c[9]=0, VIN1P/4P/7P */
		tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_8 = 0x1;	/* 2_004c[8]=1, VIN1P/4P/7P */
		goto _V_Ch_Set;

	case VIDEO_INPUT9:
		tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_10_9 = 0x0;	/* 2_0048[10:9]=00, VIN9PP[VIN_VD_i] */
		// vd_adc_clamp_ctl_2_reg.vdadc_clamp_ctrl2_10 = 0x0;	/* 2_004c[10]=0, VIN9P */
		tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_9 = 0x0;	/* 2_004c[9]=1, VIN10P */
		tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_8 = 0x1;	/* 2_004c[8]=1, VIN9P */
		goto _V_Ch_Set;

	case VIDEO_INPUT10:
		tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_10_9 = 0x1;	/* 2_0048[10:9]=01, VIN10P */
		tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_9 = 0x1;	/* 2_004c[9]=1, VIN10P */
		tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_8 = 0x0;	/* 2_004c[8]=0, VIN10P */
		goto _V_Ch_Set;
	/* Passive MUX : VIN11PP=12P, 13P */
	default:
	case VIDEO_INPUT12:
		tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_9 = 0x0;	/* 2_004c[9]=0, VIN12P */
		tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_8 = 0x0;	/* 2_004c[8]=0, VIN12P */
		goto _VIN11PP_set;
	case VIDEO_INPUT13:
		tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_9 = 0x1;	/* 2_004c[9]=1, VIN13P */
		tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_8 = 0x1;	/* 2_004c[8]=1, VIN13P */
		goto _VIN11PP_set;

_VIN11PP_set:
		tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_10_9 = 0x2;	/* 2_0048[10:9]=10, VIN11P */
		tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_10 = 0x1;	/* 2_004c[10] VIN9PP select */
		goto _V_Ch_Set;
...
_V_Ch_Set:		/* GND set */
		...
		default:
		case VIDEO_GNDA0:
			tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl0_15_14 = 0x0;				/* 2_0048[31:30] = 00, A0N */
			tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_10_8 = (ucNClampUpDn | _BIT0); /* 2_0050[8]=1 */
			break;
		case VIDEO_Y0N: 	/* Y0N */
		case VIDEO_GNDA1:	/* A1N from YPP */
			// adc_adc_vbias_reg.adc_vbias1_7 = 0x0;					/* 2_000c[15] = 0,	*/
			// adc_adc_vbias_reg.adc_vbias1_6 = 0x0;					/* 2_000c[14] = 0, */
			// adc_adc_vbias_reg.adc_vbias1_5_4 = 0x2; 			/* 2_000c[13:12] = 10, VINN mux to D */
			// adc_adc_vbias_reg.adc_vbias1_3_2 = 0x0; 			/* 2_000c[11:10] = 00,	*/
			// adc_adc_vbias_reg.adc_vbias1_1 = 0x0;					/* 2_000c[9] = 0,  */
			// adc_adc_vbias_reg.adc_vbias1_0 = 0x0;					/* 2_000c[8] = 0,  */
			tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl0_15_14 = 0x1;				/* 2_0048[31:30] = 01, A1N from YPP */
			tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_10_8 = (ucNClampUpDn | _BIT1); /* 2_0050[9]=1 */
			break;
		case VIDEO_GNDA2:
			tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl0_15_14 = 0x2;				/* 2_0048[31:30] = 10, A2N */
			tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_10_8 = (ucNClampUpDn | _BIT2); /* 2_0050[10]=1 */
			break;
		case VIDEO_GN:
			// adc_adc_vbias_reg.adc_vbias1_5_4 = 0x1; 			/* 2_000c[13:12] = 01, VINN mux to GN */
			tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl0_15_14 = 0x1;				/* 2_0048[31:30] = 01, A1N from YPP */
			tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_10_8 = (ucNClampUpDn | _BIT1); /* 2_0050[9]=1, A1N */
			break;
		}
		ucTemp = 3;
		break;
	...
	// rtdf_outl(ADC_ADC_VBIAS_reg, adc_adc_vbias_reg.regValue );		/* 2_000c */
	rtdf_outl(TV_SB1_ANA_VD_ADC_CLAMP_CTL_1_reg, tv_sb1_ana_vd_adc_clamp_ctl_1_reg.regValue); /* 2_0048 */
	rtdf_outl(TV_SB1_ANA_VD_ADC_CLAMP_CTL_2_reg, tv_sb1_ana_vd_adc_clamp_ctl_2_reg.regValue); /* 2_004c */
	rtdf_outl(TV_SB1_ANA_VD_ADC_UPDN_CTL_reg, tv_sb1_ana_vd_adc_updn_ctl_reg.regValue);		/* 2_0050 */
...
void module_vdc_SetInput_Y(unsigned char Y_path, unsigned char Y_Gnd_path)
{
	...
	VDC_DEBUG_MSG(VDC_MSG_INIT,"#####[%s(%d)][Y_path] 0x%x, [Y_Gnd_path] 0x%x\n",__func__,__LINE__, Y_path, Y_Gnd_path);
...
void module_vdc_SetInput_YC(unsigned char Y_path, unsigned char Y_Gnd_path, unsigned char C_path, unsigned char C_Gnd_path, unsigned char SV_Type)
{
	...
	//tv_sb1_ana_vd_adc_clamp_ctl_2_RBUS tv_sb1_ana_vd_adc_clamp_ctl_2_reg;	/* 2_004c */
	tv_sb1_ana_vd_adc_updn_ctl_RBUS tv_sb1_ana_vd_adc_updn_ctl_reg;		/* 2_0050 */
	tv_sb1_ana_vdadc_ck_ctl_RBUS tv_sb1_ana_vdadc_ck_ctl_reg;				/* 2_0058 */
	...
	if (SV_Type == VIDEO_SV_SWAP) {
		...
		// tv_sb1_ana_vd_adc_clamp_ctl_2_reg.regValue = rtdf_inl(ADC_VD_ADC_CLAMP_CTL_2_reg);
		// tv_sb1_ana_vd_adc_clamp_ctl_2_reg.vdadc_clamp_ctrl2_5 = 0x1;	/* 2_004c[5] = 1, yc swap enable */
		// rtdf_outl(ADC_VD_ADC_CLAMP_CTL_2_reg, tv_sb1_ana_vd_adc_clamp_ctl_2_reg.regValue);

		tv_sb1_ana_vd_adc_updn_ctl_reg.regValue = rtdf_inl(TV_SB1_ANA_VD_ADC_UPDN_CTL_reg);
		tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_1 = 0x1;	/* 2_0050[1]=1, SV */
		tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_0 = 0x1;	/* 2_0050[0]=1, SV */
		rtdf_outl(TV_SB1_ANA_VD_ADC_UPDN_CTL_reg, tv_sb1_ana_vd_adc_updn_ctl_reg.regValue);

		tv_sb1_ana_vdadc_ck_ctl_reg.regValue = rtdf_inl(TV_SB1_ANA_VDADC_CK_CTL_reg);
		tv_sb1_ana_vdadc_ck_ctl_reg.reg_vdadc_ckin_sel_7 = 0x1;	/* 2_0058[15]=1, PLL27X_CK54MHz clock enable */
		rtdf_outl(TV_SB1_ANA_VDADC_CK_CTL_reg, tv_sb1_ana_vdadc_ck_ctl_reg.regValue);
...
void module_vdc_SetInput_YRGB(unsigned char Y_path, unsigned char B_path, unsigned char G_path, unsigned char R_path, unsigned char YRGB_Gnd_path)
{
	...
	vdtop_ad_sw_ctrl0_RBUS ad_sw_ctrl0_reg;		/* 9608 */
	vdtop_clamp_rgb_updn_en_RBUS clamp_rgb_updn_en_reg;	/* 9610 */
	//tv_sb1_ana_scart_ctrl1_RBUS tv_sb1_ana_scart_ctrl1_reg;
	...
	drvif_adc_power_control(ADC_POWER_ALL_ENABLE_CONTROL,__func__,__LINE__);/* enable all ADC power */
	...
	vdcf_outl(VDTOP_CLAMP_RGB_UPDN_EN_reg, clamp_rgb_updn_en_reg.regValue);
#ifdef CONFIG_SUPPORT_SRC_ADC
	tv_sb1_ana_scart_ctrl1_reg.regValue = rtdf_inl(ADC_SCART_CTRL1_reg);
	tv_sb1_ana_scart_ctrl1_reg.vdadc_sct_ctrl_7_6= 0x1;
	tv_sb1_ana_scart_ctrl1_reg.vdadc_sct_ctrl_5_4= 0x0;
	tv_sb1_ana_scart_ctrl1_reg.vdadc_sct_ctrl_3 = 0x0;
	tv_sb1_ana_scart_ctrl1_reg.vdadc_sct_ctrl_2 = 0x1;
	tv_sb1_ana_scart_ctrl1_reg.vdadc_sct_ctrl_1 = 0x1;
	tv_sb1_ana_scart_ctrl1_reg.vdadc_sct_ctrl_0 = 0x1;
	rtdf_outl(ADC_SCART_CTRL1_reg, tv_sb1_ana_scart_ctrl1_reg.regValue);
#endif
}
#endif
...
unsigned char drvif_AV_Detect_OffLine_Signal(unsigned char source)
{
	...
	tv_sb1_ana_vd_adc_updn_ctl_RBUS tv_sb1_ana_vd_adc_updn_ctl_reg; 		// 0xb8020050
	tv_sb1_ana_vdadc_pow_ctl_RBUS tv_sb1_ana_vdadc_pow_ctl_reg; 			// 0xb802005c
	tv_sb1_ana_adc_sog_RBUS  tv_sb1_ana_vd_adc_ADC_SOG_reg;	// 0xb8020060

	drvif_module_vdc_ClockOn(); //for av offline measure yiteng_huang 20170602

	tv_sb1_ana_vdadc_pow_ctl_reg.regValue =  IoReg_Read32(TV_SB1_ANA_VDADC_POW_CTL_reg);//005c
#if 1//fix-me:mac6p bringup
	//adc_clamp_ctrl2_reg.adc_sog_cmp_3 = 1;
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_4 = 0x0;	//0x0 VIN10P
#endif
	IoReg_Write32(TV_SB1_ANA_VDADC_POW_CTL_reg, tv_sb1_ana_vdadc_pow_ctl_reg.regValue);

	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.regValue = IoReg_Read32(TV_SB1_ANA_VD_ADC_CLAMP_CTL_1_reg);//0048
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_10_9=1;
#if 1//fix-me:mac6p bringup
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_13=1;
#endif
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_15=0;//hyt add for av picture too light
	IoReg_Write32(TV_SB1_ANA_VD_ADC_CLAMP_CTL_1_reg, tv_sb1_ana_vd_adc_clamp_ctl_1_reg.regValue);

	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.regValue = IoReg_Read32(TV_SB1_ANA_VD_ADC_CLAMP_CTL_2_reg);//004c
#if 1//fix-me:mac6p bringup
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_9=0x1;
#endif
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_6=0x0;

	IoReg_Write32(TV_SB1_ANA_VD_ADC_CLAMP_CTL_2_reg, tv_sb1_ana_vd_adc_clamp_ctl_2_reg.regValue);

	tv_sb1_ana_vd_adc_updn_ctl_reg.regValue = IoReg_Read32(TV_SB1_ANA_VD_ADC_UPDN_CTL_reg);//0050
	//vd_adc_updn_ctl_reg.vdadc_updn_ctrl0_1_0=0;
#if 1//fix-me:mac6p bringup
	tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_0=1;//hyt modify for av picture too light
#endif
	tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_2=0;
	tmp_vdadc_clamp_current=tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_6_3;
	tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_6_3 = 0x2;
	IoReg_Write32(TV_SB1_ANA_VD_ADC_UPDN_CTL_reg, tv_sb1_ana_vd_adc_updn_ctl_reg.regValue);

	tv_sb1_ana_vdadc_pow_ctl_reg.regValue = IoReg_Read32(TV_SB1_ANA_VDADC_POW_CTL_reg);//005c
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_pow = 1;
	IoReg_Write32(TV_SB1_ANA_VDADC_POW_CTL_reg, tv_sb1_ana_vdadc_pow_ctl_reg.regValue);

	tv_sb1_ana_vdadc_pow_ctl_reg.regValue = IoReg_Read32(TV_SB1_ANA_VDADC_POW_CTL_reg); //005c
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_15_11 = 0x9; //0x01001
	IoReg_Write32(TV_SB1_ANA_VDADC_POW_CTL_reg, tv_sb1_ana_vdadc_pow_ctl_reg.regValue);

	tv_sb1_ana_vdadc_pow_ctl_reg.regValue = IoReg_Read32(TV_SB1_ANA_VDADC_POW_CTL_reg); //005c
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_10= 0x0;
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_reserve_9= 0x1; 
	IoReg_Write32(TV_SB1_ANA_VDADC_POW_CTL_reg, tv_sb1_ana_vdadc_pow_ctl_reg.regValue);

	for(i=0;i<10;i++)
	{

		tv_sb1_ana_vd_adc_ADC_SOG_reg.regValue = IoReg_Read32(TV_SB1_ANA_ADC_SOG_reg);//20060
		//tv_sb1_ana_vd_adc_ADC_SOG_reg.adc_out_sig1 = 1;
		//IoReg_Write32(TV_SB1_ANA_ADC_SOG_reg, tv_sb1_ana_vd_adc_ADC_SOG_reg.regValue);
		msleep(1);
		if(tv_sb1_ana_vd_adc_ADC_SOG_reg.adc_out_sig1)
			toggle_cnt++;
	}

	#ifdef CONFIG_CUSTOMER_TV030
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.regValue = IoReg_Read32(TV_SB1_ANA_VD_ADC_CLAMP_CTL_1_reg);//0048
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl0_13=0;
	IoReg_Write32(TV_SB1_ANA_VD_ADC_CLAMP_CTL_1_reg, tv_sb1_ana_vd_adc_clamp_ctl_1_reg.regValue);

	tv_sb1_ana_vd_adc_updn_ctl_reg.regValue = IoReg_Read32(TV_SB1_ANA_VD_ADC_UPDN_CTL_reg);//0050
	tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_6_3 = tmp_vdadc_clamp_current;
	IoReg_Write32(TV_SB1_ANA_VD_ADC_UPDN_CTL_reg, tv_sb1_ana_vd_adc_updn_ctl_reg.regValue);

	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.regValue = IoReg_Read32(TV_SB1_ANA_VD_ADC_CLAMP_CTL_2_reg);//004c
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_6=0x1;
	IoReg_Write32(TV_SB1_ANA_VD_ADC_CLAMP_CTL_2_reg, tv_sb1_ana_vd_adc_clamp_ctl_2_reg.regValue);
	#endif
	...
	if (toggle_cnt >= 8)
	{
		...
		//rtd_pr_avd_info("detect source %d, no_signal %d 000000000000", source, toggle_cnt);
		return FALSE;
	}
	else
	{
		//rtd_pr_avd_info("detect source %d, signal %d 111111111111", source, toggle_cnt);
		return TRUE;
	}
#endif
}
...
unsigned char drvif_AV_Detect_Signal_PlugInOut(unsigned char source)
{
#if 1
	int i;
	int toggle_cnt=0;
	...
	vd_ifd_input_vd_inout_ctrl_RBUS vd_inout_ctrl_reg;

	vdtop_video_status_reg1_RBUS video_status_reg1_reg;//9008
	...
	if (Scaler_AVD_Get_g_ucSrcType() == _SRC_CVBS){ //VD clk source
		video_status_reg1_reg.regValue = vdcf_inl(VDTOP_VIDEO_STATUS_REG1_reg);

		if (video_status_reg1_reg.no_signal){
			return FALSE;
		}else{
			return TRUE;
		}
	...
	sys_clksel_reg.regValue = rtdf_inl(SYS_REG_SYS_CLKSEL_reg);	/* 0_0204 */
	sys_clksel_reg.atvin_vd_clk_sel = 1;
	...
	vd_inout_ctrl_reg.regValue = vdcf_inl(VD_IFD_INPUT_VD_INOUT_CTRL_reg); //180180ac
	vd_inout_ctrl_reg.adc27_mod = 1;
	vdcf_outl(VD_IFD_INPUT_VD_INOUT_CTRL_reg, vd_inout_ctrl_reg.regValue);

	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.regValue = vdcf_inl(TV_SB1_ANA_VD_ADC_CLAMP_CTL_1_reg);//0048
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_10_9= 1;
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_13= 1;
	tv_sb1_ana_vd_adc_clamp_ctl_1_reg.reg_vdadc_clamp_ctrl1_15= 0;
	rtdf_outl(TV_SB1_ANA_VD_ADC_CLAMP_CTL_1_reg, tv_sb1_ana_vd_adc_clamp_ctl_1_reg.regValue);

	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.regValue = vdcf_inl(TV_SB1_ANA_VD_ADC_CLAMP_CTL_2_reg);//004c
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_9= 0x1;
	tv_sb1_ana_vd_adc_clamp_ctl_2_reg.reg_vdadc_clamp_ctrl2_6= 0x0;
	rtdf_outl(TV_SB1_ANA_VD_ADC_CLAMP_CTL_2_reg, tv_sb1_ana_vd_adc_clamp_ctl_2_reg.regValue);

	tv_sb1_ana_vd_adc_updn_ctl_reg.regValue = vdcf_inl(TV_SB1_ANA_VD_ADC_UPDN_CTL_reg);//0050
	tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_0= 1;
	tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_2= 0;
	tv_sb1_ana_vd_adc_updn_ctl_reg.reg_vdadc_updn_ctrl0_6_3= 0x2;
	rtdf_outl(TV_SB1_ANA_VD_ADC_UPDN_CTL_reg, tv_sb1_ana_vd_adc_updn_ctl_reg.regValue);

	//tv_sb1_ana_vdadc_core_lpf_ctrl_reg.regValue = vdcf_inl(TV_SB1_ANA_VDADC_CORE_LPF_CTRL_reg);//0054
	//tv_sb1_ana_vdadc_core_lpf_ctrl_reg.reg_vdadc_adc_ctrl_0= 1;
	//rtdf_outl(TV_SB1_ANA_VDADC_CORE_LPF_CTRL_reg, tv_sb1_ana_vdadc_core_lpf_ctrl_reg.regValue);

	tv_sb1_ana_vdadc_pow_ctl_reg.regValue = vdcf_inl(TV_SB1_ANA_VDADC_POW_CTL_reg);//005c
	tv_sb1_ana_vdadc_pow_ctl_reg.reg_vdadc_pow= 1;
	rtdf_outl(TV_SB1_ANA_VDADC_POW_CTL_reg, tv_sb1_ana_vdadc_pow_ctl_reg.regValue);

	adc_over_under_flag_reg.regValue = vdcf_inl(VDTOP_ADC_OVER_UNDER_FLAG_reg);//95bc
	adc_over_under_flag_reg.adc_value_max_thr = 0x062;
	adc_over_under_flag_reg.adc_value_min_thr= 0x000;
	rtdf_outl(VDTOP_ADC_OVER_UNDER_FLAG_reg, adc_over_under_flag_reg.regValue);

	msleep(100); //waitting HW setting stable, don't remove

	for(i=0;i<5;i++)
	{
		//write clear
		adc_over_under_flag_reg.regValue = vdcf_inl(VDTOP_ADC_OVER_UNDER_FLAG_reg);//95bc
		adc_over_under_flag_reg.ad4_value_small = 1;
		adc_over_under_flag_reg.ad4_value_big= 1;
		rtdf_outl(VDTOP_ADC_OVER_UNDER_FLAG_reg, adc_over_under_flag_reg.regValue);

		msleep(2);
		adc_over_under_flag_reg.regValue = vdcf_inl(VDTOP_ADC_OVER_UNDER_FLAG_reg);

		if((adc_over_under_flag_reg.ad4_value_big) && (adc_over_under_flag_reg.ad4_value_small))
			toggle_cnt++;
	}
	...
	if (toggle_cnt >= 4)
	{
		//rtd_pr_vdc_notice("detect source %d, signal %d111111111111", source,toggle_cnt);
		return TRUE;
	}
	else
	{
		//rtd_pr_vdc_notice("detect source %d, signal %d 000000000000", source,toggle_cnt);
		return FALSE;
	}
#endif
}
...
void module_vdc_PathSelect(unsigned char ucSourceIndex, void* stPath)
{
	...
	if (ucSourceIndex != _SRC_TV) {
		...
		// for  CVBS plug_inout detection
		sys_clksel_reg.regValue = rtdf_inl(SYS_REG_SYS_CLKSEL_reg);	/* 0_0204 */
		sys_clksel_reg.atvin_vd_clk_sel = 0;
		rtdf_outl(SYS_REG_SYS_CLKSEL_reg, sys_clksel_reg.regValue);

		vd_inout_ctrl_reg.regValue = vdcf_inl(VD_IFD_INPUT_VD_INOUT_CTRL_reg); //180180ac
		vd_inout_ctrl_reg.adc27_mod = 0;
		vdcf_outl(VD_IFD_INPUT_VD_INOUT_CTRL_reg, vd_inout_ctrl_reg.regValue);

		adc_over_under_flag_reg.regValue = vdcf_inl(VDTOP_ADC_OVER_UNDER_FLAG_reg);//95bc
		adc_over_under_flag_reg.adc_value_max_thr = 0x3f0;
		adc_over_under_flag_reg.adc_value_min_thr= 0x010;
		rtdf_outl(VDTOP_ADC_OVER_UNDER_FLAG_reg, adc_over_under_flag_reg.regValue);
	...
	tv_sb1_ana_scart_ctrl1_reg.vdadc_sct_ctrl_5_4= 0x0;
	tv_sb1_ana_scart_ctrl1_reg.vdadc_sct_ctrl_3 = 0x0;
	tv_sb1_ana_scart_ctrl1_reg.vdadc_sct_ctrl_2 = 0x0;
	tv_sb1_ana_scart_ctrl1_reg.vdadc_sct_ctrl_1 = 0x1;
	tv_sb1_ana_scart_ctrl1_reg.vdadc_sct_ctrl_0 = 0x0;
	rtdf_outl(ADC_SCART_CTRL1_reg, tv_sb1_ana_scart_ctrl1_reg.regValue);
#endif
	...
	drvif_video_set_control_reg(VDC_set_src_AV);
	//drvif_module_vdc_set_Control2_Register(VDC_bgate_raw_sel_enable); //always high
...
//#ifdef CONFIG_TV_ENABLE
	...
	drvif_module_vdc_108M_FrontDigitalFilterEnable(FrontDigitalFilter_Table);
