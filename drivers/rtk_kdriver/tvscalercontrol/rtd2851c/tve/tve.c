...
void drv_tve_set_video_ntsc_m(void)
{
		...
		rtdf_outl(TVE_ip_phs_0_reg,0x00000022);//33
		rtdf_outl(TVE_ip_phs_1_reg,0x0000005e);//7b
		rtdf_outl(TVE_ip_phs_2_reg,0x000000f0);
		rtdf_outl(TVE_ip_phs_3_reg,0x00000021);
		rtdf_outl(TVE_ip_sctoh_0_reg,0x00000000);
		rtdf_outl(TVE_ip_sctoh_1_reg,0x00000055);
		rtdf_outl(TVE_ip_sctoh_2_reg,0x00000002);
		rtdf_outl(TVE_ip_sctoh_3_reg,0x0000004b);
		...
		rtdf_outl(TVE_ip_synclpf_gain_reg,0x00000036);///1a
...
void drv_tve_set_video_pal_i(void)
{
	...
	rtdf_outl(TVE_video_dac_ctrl2_0_reg,0x00005b1e);
	rtdf_outl(TVE_video_dac_ctrl2_1_reg,0x00005b1e);
	rtdf_outl(TVE_ip_y_gain_reg,0x00000226);//208
	...
	rtdf_outl(TVE_ip_sctoh_3_reg,0x00000071);
	rtdf_outl(TVE_ip_f_ctrl_reg,0x00000002);
	rtdf_outl(TVE_ip_blck_lvl_reg,0x00000078);
	rtdf_outl(TVE_ip_blnk_lvl_reg,0x00000078);
	rtdf_outl(TVE_ip_clamp_lvl_reg,0x00000010);
	...
	rtdf_outl(TVE_ip_synclpf_gain_reg,0x00000040);//29
	rtdf_outl(TVE_ip_sync_strt_reg,0x00000000);
...
void drvif_module_tve_video_memory_setting(unsigned char vflip_en)
{
	...
	if(vflip_en == false){
		...
		rtdf_outl(TVE_DMA_TVEVDDMA_SATRT0_reg,rtdf_inl(MDOMAIN_CAP_DDR_In2Addr_reg)/8);   //  dma addr	//�M3d dma�t�F8��
		rtdf_outl(TVE_DMA_TVEVDDMA_SATRT1_reg,rtdf_inl(MDOMAIN_CAP_DDR_In2_2ndAddr_reg)/8);   //  dma addr
		rtdf_outl(TVE_DMA_TVEVDDMA_SATRT2_reg,rtdf_inl(MDOMAIN_CAP_DDR_In2_3rdAddr_reg)/8);   //  dma addr
		//rtdf_outl(TVE_DMA_TVEVDDMA_SATRT3_reg,rtdf_inl(H3DDMA_Cap_L4_Start_reg)/8);	  //  dma addr
	}
	else if(vflip_en == true){
		rtdf_outl(TVE_DMA_TVEVDDMA_SATRT0_reg,(rtdf_inl(MDOMAIN_CAP_DDR_In2Addr_reg)+TVE_VFLIP_SIZE_OFFSET)/8 );	  //  dma addr	//�M3d dma�t�F8��
		rtdf_outl(TVE_DMA_TVEVDDMA_SATRT1_reg,(rtdf_inl(MDOMAIN_CAP_DDR_In2_2ndAddr_reg)+TVE_VFLIP_SIZE_OFFSET)/8);   //  dma addr
		rtdf_outl(TVE_DMA_TVEVDDMA_SATRT2_reg,(rtdf_inl(MDOMAIN_CAP_DDR_In2_3rdAddr_reg)+TVE_VFLIP_SIZE_OFFSET)/8);   //  dma addr
...
void drvif_module_tve_vbi_memory_setting(unsigned int vbi_addr)
{
	...
	//rtdf_outl(TVE_DMA_TVEVBIDMA_START_VADDR,vbi_addr / 8);//dma addr   //�M3d dma�t�F8��
...
void drvif_module_tve_vps_data(Struct_TVE_VPS_DATA *VPS_DATA)
{
	...
	//check pointer
	if(VPS_DATA == NULL) {
		rtd_pr_vdc_debug("NULL pointer in %s\n",__FUNCTION__);
		return;
	}
...
void drvif_module_tve_interrupt(int enable)
{
	if(enable)
	{
		rtd_pr_vdc_debug("drvif_module_tve_interrupt\n");
...
void drvif_module_tve_change_memory_setting(unsigned int in_width, unsigned char vflip_en){
	...
	rtd_pr_vdc_debug("[crixus]line_step = %x\n",line_step);
...
void drvif_module_tve_vbi_tt_line_num(UINT32 line_num)
{
	...
	//rtd_pr_vdc_debug("finial_value = %x\n",finial_value);
