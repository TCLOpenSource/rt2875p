...
#define HDMI_FIX_RL6641_1363        //fix issue of video broken on 480i/240p/576i/288p. JIRA MAC8PQC-576, do not remove in 2851 series, because video path do not through DMAVGIP.
...
typedef struct {
	...
	unsigned int  sbtm_emp_cnt;
	unsigned int  no_sbtm_emp_cnt;
	unsigned int  rsv0_emp_cnt;
	unsigned int  no_rsv0_emp_cnt;
	unsigned int  rsv1_emp_cnt;
	unsigned int  no_rsv1_emp_cnt;
	...
	unsigned char sbtm_emp[EM_SBTM_INFO_LEN];
	unsigned char rsv0_emp[RSV0_EM_LEN];
	unsigned char rsv1_emp[RSV1_EM_LEN];
