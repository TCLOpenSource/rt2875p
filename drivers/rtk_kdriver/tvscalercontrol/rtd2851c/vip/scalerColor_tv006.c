...
extern unsigned short ModuleTest_HDR10_3DLUT_24x24x24[HDR_24x24x24_size][3];
extern unsigned int ModuleTest_EOTF_LUT_R[EOTF_size];
extern unsigned short ModuleTest_OETF_LUT_R[OETF_size];
...
unsigned int OutGamma_PTG_R[128];
unsigned int OutGamma_PTG_G[128];
unsigned int OutGamma_PTG_B[128];
unsigned int OutGamma_PTG_W[128];

unsigned int OutGamma_Ori_R[128];
unsigned int OutGamma_Ori_G[128];
unsigned int OutGamma_Ori_B[128];
unsigned int OutGamma_Ori_W[128];
...
void fwif_color_SetDNR_tv006(unsigned char level)
{
	...
	//gVip_Table->Manual_NR_Table[pqa_table_idx][level].S_SNR_TABLE.S_SNR_Spatial_NR_Y.SNR_zoranweight4y = pNR_Table->LPF_Y.Weight4Y;//mac8p change
	...
	//gVip_Table->Manual_NR_Table[pqa_table_idx][level].S_SNR_TABLE.S_SNR_Spatial_NR_C.SNR_zoranweight4c = pNR_Table->LPF_C.Weight4C;//mac8p change
	...
	gVip_Table->Manual_NR_Table[pqa_table_idx][level].S_RTNR_TABLE.S_RTNR_General_ctrl.RTNR_y_enable = pNR_Table->TNR_Y.Y_enable;
...
void fwif_color_Set_MPEGNR_tv006(unsigned char level)
{
	...
	if (pMPEGNR_Table->MPEG_LEVEL < 4) {
		...
		pNR_ITEM->S_MPG_TABLE.S_MPEGNR_H.S_MPEGNR_16_LPF.result_weight = pMPEGNR_Table->MPEGNR_UI_T.MpegResultWeight_H;
...
void fwif_color_set_ICM_table_driver_init_elem_tv006(COLORELEM_TAB_T *pIcm_tab_elem)
{
	...
	for (i = 0; i < 9; i++) {
		...
		for (s = 0; s < 12; s++) {
			for (h = 0; h < 62; h++) {
				pIcm_tab_elem->elem[i][s][h].H = chip_hue_idx[h]>>2;
...
void fwif_color_set_ICM_table_driver_tv006(COLORELEM_TAB_T *pIcm_tab_elem, unsigned char waiting)
{
	...
	ret = drvif_color_icm_Write3DTable_By_DMA(pIcm_tab_elem->elem, addr, pVir_addr, PILLAR_NUM_INFO_TYPE_FROM_REG_GET);
...
{
	SLR_VIP_TABLE *gVip_Table = NULL;
	static DRV_VIP_YUV2RGB_CSMatrix drv_vipCSMatrix_t = {0};
	static unsigned int table_idx[3][3][VIP_YUV2RGB_Y_Seg_Max];
	static short satTmp[3][3], hueTmp[3][3], kTmp[3][3], kTmp1[3][3];
	static VPQ_SAT_LUT_T sat_LUT, sat_gain, total_sat_gain;
...
#ifdef CONFIG_ARM64
	memset_io(satTmp, 0, sizeof(satTmp));
	memset_io(hueTmp, 0, sizeof(hueTmp));
	memset_io(kTmp, 0, sizeof(kTmp));
	memset_io(kTmp1, 0, sizeof(kTmp1));
#else
...
void fwif_color_set_sharpness_table_TV006(CHIP_SHARPNESS_UI_T *ptCHIP_SHARPNESS_UI_T, CHIP_EDGE_ENHANCE_UI_T *ptCHIP_EDGE_ENHANCE_UI_T, CHIP_SR_UI_T *ptCHIP_SR_UI_T)
{
	...
	sharp.Edge_Coring.Edge_Coring_En = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCoringMode.edgeCoringEn;
	sharp.Edge_Coring.Edge_Gain_En = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCoringMode.edgeGainEn;
	sharp.Edge_Coring.DeltaEdge_Ulevel = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCoringMode.deltaEdgeUlevel;
	sharp.Edge_Coring.Edge_Coring_Clip = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCoringMode.edgeCoringClip;
	sharp.Edge_Coring.Edge_Gain_Bound = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCoringMode.edgeGainBound;
	sharp.Edge_Coring.Edge_Ser_Range = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCoringMode.edgeSerRange;
	sharp.Edge_Coring.Bigedge_Ratio = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCoringMode.bigEdgeRatio;
	sharp.Edge_Coring.Smalledge_Ratio = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCoringMode.smallEdgeRatio;
	sharp.Edge_Coring.Deltaedge_Ratio = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCoringMode.deltaEdgeRatio;
	sharp.Edge_Coring.Smalledge_Ulevel = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCoringMode.smallEdgeUlevel;
	sharp.Edge_Coring.Edge_Delta_Ext_Range = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCoringMode.Edge_delta_ext_R;

	/* Remapping CHIP_Y_REMAPPING_T
	sharp.Y_Remap.LinearY_LB = ptCHIP_SHARPNESS_UI_T->stSharpness.stYRemapping.linearY_LB;
	sharp.Y_Remap.LinearY_UB = ptCHIP_SHARPNESS_UI_T->stSharpness.stYRemapping.linearY_UB;
	sharp.Y_Remap.LinearY_LM = ptCHIP_SHARPNESS_UI_T->stSharpness.stYRemapping.linearY_Lmode;
	sharp.Y_Remap.LinearY_UM = ptCHIP_SHARPNESS_UI_T->stSharpness.stYRemapping.linearY_Umode;
	//*/

	/* Remapping CHIP_EDGE_HPF_T*/
	// sharp.TwoD_Peak_AdaptCtrl2.Edge_Filter_Sel = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.edge_select_filter;
	sharp.Edge_Shp_coef1.C00 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c0_0;
	sharp.Edge_Shp_coef1.C10 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c1_0;
	sharp.Edge_Shp_coef1.C20 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c2_0;
	sharp.Edge_Shp_coef1.C30 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c3_0;
	sharp.Edge_Shp_coef1.C40 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c4_0;
	sharp.Edge_Shp_coef1.C50 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c5_0;
	sharp.Edge_Shp_coef1.C60 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c6_0;
	sharp.Edge_Shp_coef1.C70 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c7_0;
	sharp.Edge_Shp_coef1.C80 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c8_0;
	sharp.Edge_Shp_coef1.C90 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c9_0;
	sharp.Edge_Shp_coef1.C100 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c10_0;
	sharp.Edge_Shp_coef2.C01 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c0_1;
	// sharp.Edge_Shp_coef2.C11 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c1_1; //mac6p removed
	// sharp.Edge_Shp_coef2.C21 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c2_1; //mac6p removed
	// sharp.Edge_Shp_coef2.C31 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c3_1; //mac6p removed
	// sharp.Edge_Shp_coef2.C41 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c4_1; //mac6p removed
	// sharp.Edge_Shp_coef2.C51 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c5_1; //mac6p removed
	sharp.Edge_Shp_coef3.C02 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c0_2;
	// sharp.Edge_Shp_coef3.C12 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c1_2; //mac6p removed
	// sharp.Edge_Shp_coef3.C22 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c2_2; //mac6p removed
	// sharp.Edge_Shp_coef3.C32 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c3_2; //mac6p removed
	// sharp.Edge_Shp_coef3.C42 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c4_2; //mac6p removed
	// sharp.Edge_Shp_coef3.C52 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c5_2; //mac6p removed
	// sharp.Edge_Shp_coef4.C03 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c0_3;
	// sharp.Edge_Shp_coef5.C04 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c0_4;
	...
	*/
	...
	sharp.Edge_Shp_set.Edg_D2_Shift_Bit = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCurveMappingUI.Edge_Shift;
	sharp.Edge_Shp_set.G_Neg2 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCurveMappingUI.Gain_Neg2;
	sharp.Edge_Shp_set.G_Pos2 = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCurveMappingUI.Gain_Pos2;

	/* Remapping CHIP_TEXTURE_HPF_T*/
	sharp.TwoD_Peak_AdaptCtrl.Tex_Filter_Sel = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.text_select_filter;
	sharp.Texture_Shp_coef1.C00 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c0_0;
	sharp.Texture_Shp_coef1.C10 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c1_0;
	sharp.Texture_Shp_coef1.C20 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c2_0;
	sharp.Texture_Shp_coef1.C30 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c3_0;
	sharp.Texture_Shp_coef1.C40 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c4_0;
	sharp.Texture_Shp_coef1.C50 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c5_0;
	sharp.Texture_Shp_coef1.C60 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c6_0;
	sharp.Texture_Shp_coef1.C70 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c7_0;
	sharp.Texture_Shp_coef1.C80 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c8_0;
	sharp.Texture_Shp_coef1.C90 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c9_0;
	sharp.Texture_Shp_coef1.C100 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c10_0;

	sharp.Texture_Shp_coef2.C01 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c0_1;
	// sharp.Texture_Shp_coef2.C11 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c1_1; //mac6p removed
	// sharp.Texture_Shp_coef2.C21 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c2_1; //mac6p removed
	// sharp.Texture_Shp_coef2.C31 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c3_1; //mac6p removed
	// sharp.Texture_Shp_coef2.C41 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c4_1; //mac6p removed
	// sharp.Texture_Shp_coef2.C51 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c5_1; //mac6p removed
	sharp.Texture_Shp_coef3.C02 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c0_2;
	// sharp.Texture_Shp_coef3.C12 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c1_2; //mac6p removed
	// sharp.Texture_Shp_coef3.C22 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c2_2; //mac6p removed
	// sharp.Texture_Shp_coef3.C32 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c3_2; //mac6p removed
	// sharp.Texture_Shp_coef3.C42 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c4_2; //mac6p removed
	// sharp.Texture_Shp_coef3.C52 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c5_2; //mac6p removed
	// sharp.Texture_Shp_coef4.C03 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c0_3;
	// sharp.Texture_Shp_coef5.C04 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c0_4;
	...
	*/
	...
	sharp.Texture_Shp_set.Edg_D2_Shift_Bit = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureCurveMappingUI.Tex_Shift;
	sharp.Texture_Shp_set.G_Neg2 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureCurveMappingUI.Gain_Neg2;
	sharp.Texture_Shp_set.G_Pos2 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureCurveMappingUI.Gain_Pos2;

	sharp.Slope_gain_mask.Slope_gain_en = ptCHIP_SHARPNESS_UI_T->stSharpness.stSlopeGainMask.Slope_Gain_Enable;
	sharp.Slope_gain_mask.dir = ptCHIP_SHARPNESS_UI_T->stSharpness.stSlopeGainMask.DIR;
	sharp.Slope_gain_mask.FH = ptCHIP_SHARPNESS_UI_T->stSharpness.stSlopeGainMask.FH;
	sharp.Slope_gain_mask.Gain_boundPos = ptCHIP_SHARPNESS_UI_T->stSharpness.stSlopeGainMask.Gain_boundPos;
	sharp.Slope_gain_mask.Gain_boundNeg = ptCHIP_SHARPNESS_UI_T->stSharpness.stSlopeGainMask.Gain_boundNeg;
	sharp.Slope_gain_mask.Gain_externdPos = ptCHIP_SHARPNESS_UI_T->stSharpness.stSlopeGainMask.Gain_extendPos;
	sharp.Slope_gain_mask.Gain_externdNeg = ptCHIP_SHARPNESS_UI_T->stSharpness.stSlopeGainMask.Gain_extendNeg;

	// sharp.Slope_gain_mask.flat_enable = ptCHIP_SHARPNESS_UI_T->stSharpness.stSlopeGainMask.flat_enable;
	// sharp.Slope_gain_mask.flat_offset = ptCHIP_SHARPNESS_UI_T->stSharpness.stSlopeGainMask.flat_offset;
	// sharp.Slope_gain_mask.flat_gain = ptCHIP_SHARPNESS_UI_T->stSharpness.stSlopeGainMask.flat_gain;
	...
	*/
	...
	sharp.Gain_By_Y.Gain_By_Y_En = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.gain_by_y_en;
	sharp.Gain_By_Y.gain_by_y_v_en = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.gain_by_y_v_en;
	sharp.Gain_By_Y.gain_by_y_tex_en = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.gain_by_y_tex_en;
	sharp.Gain_By_Y.gain_by_y_edg_en = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.gain_by_y_edg_en;
	sharp.Max_Min_Lpf.Weigh_En = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.lpf_weight_en;
	sharp.Max_Min_Lpf.lpf_weigh_edg_en = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.lpf_weight_edg_en;
	sharp.Max_Min_Lpf.lpf_weigh_tex_en = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.lpf_weight_tex_en;

	sharp.Max_Min_Gain.Max_Min_Gain_En = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.max_min_gain_en;
	sharp.Max_Min_Gain.max_min_gain_v_en = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.max_min_v_en;
	sharp.Max_Min_Gain.max_min_gain_tex_en = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.max_min_tex_en;
	sharp.Max_Min_Gain.max_min_gain_edg_en = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.max_min_edg_en;

	sharp.Gain_By_Y.lv_by_y_enable = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.lv_by_Y_enable;
	sharp.Gain_By_Y.lv_by_y_edg_enable = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.lv_by_y_edg_enable;
	sharp.Gain_By_Y.lv_by_y_v_edg_enable = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.lv_by_y_v_edg_enable;
	sharp.Gain_By_Y.lv_by_y_tex_enable = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.lv_by_y_tex_enable;
	sharp.Gain_By_Y.lv_by_y_v_enable = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.lv_by_y_v_enable;

	sharp.Mk3_Lpf.C0 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.coef_c0;
	sharp.Mk3_Lpf.C1 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.coef_c1;
	sharp.Mk3_Lpf.C2 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.coef_c2;
	sharp.Mk3_Lpf.C3 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.coef_c3;
	sharp.Mk3_Lpf.C4 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.coef_c4;
	sharp.Mk3_Lpf.C5 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.coef_c5;

	sharp.Max_Min_Gain.Step_1 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.maxmin_s1;
	sharp.Max_Min_Gain.Step_2 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.maxmin_s2;
	sharp.Max_Min_Gain.Step_3 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.maxmin_s3;
	sharp.Max_Min_Gain.Step_4 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.maxmin_s4;
	sharp.Max_Min_Gain.Step_5 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.maxmin_s5;
	sharp.Max_Min_Gain.Step_6 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.maxmin_s6;

	sharp.Max_Min_Gain.Gain_0 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.maxmin_g0;
	sharp.Max_Min_Gain.Gain_1 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.maxmin_g1;
	sharp.Max_Min_Gain.Gain_2 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.maxmin_g2;
	sharp.Max_Min_Gain.Gain_3 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.maxmin_g3;
	sharp.Max_Min_Gain.Gain_4 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.maxmin_g4;
	sharp.Max_Min_Gain.Gain_5 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.maxmin_g5;
	sharp.Max_Min_Gain.Gain_6 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.maxmin_g6;

	sharp.Max_Min_Lpf.Step_1 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.blending_s1;
	sharp.Max_Min_Lpf.Step_2 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.blending_s2;
	sharp.Max_Min_Lpf.Step_3 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.blending_s3;
	sharp.Max_Min_Lpf.Step_4 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.blending_s4;
	sharp.Max_Min_Lpf.Step_5 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.blending_s5;
	sharp.Max_Min_Lpf.Step_6 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.blending_s6;

	sharp.Max_Min_Lpf.Weit_0 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.blending_g0;
	sharp.Max_Min_Lpf.Weit_1 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.blending_g1;
	sharp.Max_Min_Lpf.Weit_2 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.blending_g2;
	sharp.Max_Min_Lpf.Weit_3 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.blending_g3;
	sharp.Max_Min_Lpf.Weit_4 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.blending_g4;
	sharp.Max_Min_Lpf.Weit_5 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.blending_g5;
	sharp.Max_Min_Lpf.Weit_6 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.blending_g6;

	sharp.Gain_By_Y.Step_1 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.y_s1;
	sharp.Gain_By_Y.Step_2 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.y_s2;
	sharp.Gain_By_Y.Step_3 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.y_s3;
	sharp.Gain_By_Y.Step_4 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.y_s4;
	sharp.Gain_By_Y.Step_5 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.y_s5;
	sharp.Gain_By_Y.Step_6 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.y_s6;

	sharp.Gain_By_Y.Gain_0 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.y_g0;
	sharp.Gain_By_Y.Gain_1 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.y_g1;
	sharp.Gain_By_Y.Gain_2 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.y_g2;
	sharp.Gain_By_Y.Gain_3 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.y_g3;
	sharp.Gain_By_Y.Gain_4 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.y_g4;
	sharp.Gain_By_Y.Gain_5 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.y_g5;
	sharp.Gain_By_Y.Gain_6 = ptCHIP_SHARPNESS_UI_T->stSharpness.stMKIII.y_g6;
	...
	sharp.Edge_EMF_Table.EMF_fixentent_neg = ptCHIP_EDGE_ENHANCE_UI_T->stEdgeEnhance.stEdgeEMF.fix_extend_neg;
	sharp.Edge_EMF_Table.EMF_fixentent_pos = ptCHIP_EDGE_ENHANCE_UI_T->stEdgeEnhance.stEdgeEMF.fix_extend_pos;
	...
	sharp.Texture_EMF_Table.EMF_En = ptCHIP_EDGE_ENHANCE_UI_T->stEdgeEnhance.stTextureEMF.emf_enable;
	sharp.Texture_EMF_Table.EMF_Shift = ptCHIP_EDGE_ENHANCE_UI_T->stEdgeEnhance.stTextureEMF.emf_shiftbit;
	sharp.Texture_EMF_Table.EMF_Range = ptCHIP_EDGE_ENHANCE_UI_T->stEdgeEnhance.stTextureEMF.emf_range;
	sharp.Texture_EMF_Table.EMF_fixentent_neg = ptCHIP_EDGE_ENHANCE_UI_T->stEdgeEnhance.stTextureEMF.fix_extend_neg;
	sharp.Texture_EMF_Table.EMF_fixentent_pos = ptCHIP_EDGE_ENHANCE_UI_T->stEdgeEnhance.stTextureEMF.fix_extend_pos;
	sharp.Texture_EMF_Table.Seg_0_x = ptCHIP_EDGE_ENHANCE_UI_T->stEdgeEnhance.stTextureEMF.seg0_x;
	sharp.Texture_EMF_Table.Seg_1_x = ptCHIP_EDGE_ENHANCE_UI_T->stEdgeEnhance.stTextureEMF.seg1_x;
	sharp.Texture_EMF_Table.Blend_wt = ptCHIP_EDGE_ENHANCE_UI_T->stEdgeEnhance.stTextureEMF.blend_wt;
	sharp.Texture_EMF_Table.Seg_0_Gain = ptCHIP_EDGE_ENHANCE_UI_T->stEdgeEnhance.stTextureEMF.gain0;
	sharp.Texture_EMF_Table.Seg_1_Gain = ptCHIP_EDGE_ENHANCE_UI_T->stEdgeEnhance.stTextureEMF.gain1;
	sharp.Texture_EMF_Table.Seg_2_Gain = ptCHIP_EDGE_ENHANCE_UI_T->stEdgeEnhance.stTextureEMF.gain2;
	sharp.Texture_EMF_Table.Seg_0_Offset = ptCHIP_EDGE_ENHANCE_UI_T->stEdgeEnhance.stTextureEMF.Offset0;
	/*Calc offset 1, 2*/
	if (sharp.Texture_EMF_Table.Seg_0_Gain > 6)
		sharp.Texture_EMF_Table.Seg_0_Gain = 6;
	if (sharp.Texture_EMF_Table.Seg_1_Gain > 6)
		sharp.Texture_EMF_Table.Seg_1_Gain = 6;
	sharp.Texture_EMF_Table.Seg_1_Offset = (EMFGainMap[sharp.Texture_EMF_Table.Seg_0_Gain]<<6) + sharp.Texture_EMF_Table.Seg_0_Offset;
	if (sharp.Texture_EMF_Table.Seg_1_Offset < 0)
		sharp.Texture_EMF_Table.Seg_1_Offset = 0;
	sharp.Texture_EMF_Table.Seg_2_Offset = (EMFGainMap[sharp.Texture_EMF_Table.Seg_1_Gain]<<6) + sharp.Texture_EMF_Table.Seg_1_Offset;
	if (sharp.Texture_EMF_Table.Seg_2_Offset < 0)
		sharp.Texture_EMF_Table.Seg_2_Offset = 0;
	...
	sharp.Slope_gain_mask.v_slope_gain_en =  ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalSlopeGainMask.Slope_Gain_V_Enable;
	sharp.Slope_gain_mask.dir_v= ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalSlopeGainMask.DIR;
	sharp.Slope_gain_mask.gain_boundpos_v= ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalSlopeGainMask.Gain_boundPos;
	sharp.Slope_gain_mask.gain_boundneg_v= ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalSlopeGainMask.Gain_boundNeg;
	sharp.Slope_gain_mask.gain_extendpos_v= ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalSlopeGainMask.Gain_extendPos;
	sharp.Slope_gain_mask.gain_extendneg_v= ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalSlopeGainMask.Gain_extendNeg;

	// sharp.Slope_gain_mask.flat_v_enable= ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalSlopeGainMask.flat_v_enable;
	// sharp.Slope_gain_mask.flat_offset_v= ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalSlopeGainMask.flat_offset_v;
	// sharp.Slope_gain_mask.flat_gain_v= ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalSlopeGainMask.flat_gain_v;

	sharp.Vertical_edg.enable= ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalCurveMappingUI.V_Parameters_En;
	sharp.Vertical_edg.G_Pos2= ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalCurveMappingUI.Edge_Gain_Pos2;
	sharp.Vertical_edg.G_Neg2= ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalCurveMappingUI.Edge_Gain_Neg2;
	...
	sharp.TwoD_Peak_AdaptCtrl2.PkDlti_comsign = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.PkDlti_comsign;
	...
	sharp.Edge_Shp_set.LV2 = sharpLV2.Edge_LV2;/*gVip_Table->Ddomain_SHPTable[value].Edge_Shp_set.LV2;*/
	sharp.Texture_Shp_set.LV2 = sharpLV2.Texture_LV2;/*gVip_Table->Ddomain_SHPTable[value].Texture_Shp_set.LV2;*/ //merlin
	sharp.Vertical.LV2 = sharpLV2.Vertical_LV2;/*gVip_Table->Ddomain_SHPTable[value].Vertical.LV2;*/

	sharp.TwoD_Peak_AdaptCtrl2.Edg_Tex_Blending_Lpf_En = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.Edg_Tex_Blending_Lpf_En;
	sharp.TwoD_Peak_AdaptCtrl2.Edg_Tex_Blending_Lpf_Filter = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.Edg_Tex_Blending_Lpf_Filter;
	...
	*/
	...
	sharp.TwoD_Peak_AdaptCtrl.sobelrange_h= gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl.sobelrange_h;//merlin
	sharp.TwoD_Peak_AdaptCtrl.sobelrange_v= gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl.sobelrange_v;//merlin
	sharp.TwoD_Peak_AdaptCtrl.acts_offset= gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl.acts_offset;//merlin
	sharp.TwoD_Peak_AdaptCtrl.small_th1= gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl.small_th1;//merlin
...
void fwif_color_set_sharpness_table_init_TV006(unsigned char src_idx, unsigned char value)
{
	...
	sharp.Edge_Shp_set.LV2 = gVip_Table->Ddomain_SHPTable[value].Edge_Shp_set.LV2;
	sharp.Texture_Shp_set.LV2 = gVip_Table->Ddomain_SHPTable[value].Texture_Shp_set.LV2;
...
unsigned char  fwif_color_set_dcc_Curve_Write_tv006(signed int *sAccLUT, unsigned char triggerEnable)
{
	...
	if (VIP_RPC_system_info_structure_table == NULL) {
		VIPprintf("_RPC_system_setting_info NULL!!!");
		return FALSE;
	}

	if (VIP_system_info_structure_table == NULL) {
		VIPprintf("VIP_system_info_structure_table NULL!!!");
		return FALSE;
	}
	...
	if(VIP_system_info_structure_table->DCC_FW_ONOFF)
		VIP_RPC_system_info_structure_table->SCPU_ISRIINFO_TO_VCPU.DCC_Curve_apply_flag = TRUE;
...
unsigned char  fwif_color_set_bypass_dcc_Curve_Write_tv006(void)
{
	...
	if (VIP_RPC_system_info_structure_table == NULL) {
		VIPprintf("VIP_RPC_system_info_structure_table NULL!!!");
		return FALSE;
	}
...
void fwif_color_set_DCC_Init_tv006(void)
{
	...
	FreshContrastLUT[DCC_Curve_Node_MAX-1] = 4095;

	fwif_color_set_dcc_Curve_Write_tv006(FreshContrastLUT, 0);
}
#if 0
...
char fwif_color_set_DDomain_3dLUT_16_TV006(unsigned short *p, unsigned char enable)
{
	...
	if (enable == 0 || p == NULL) {
		...
		fwif_color_D_3dLUT_Encode_16(p, D_3D_DRV_LUT);	//fwif_color_D_3dLUT_Encode(pArray, D_3D_DRV_LUT);
...
void fwif_color_SetDataFormatHandler_tv006(void)
{
	...
	unsigned char nMode = 0, is_hdr_block_used = 0;
	...
	rtd_pr_vpq_info("DataFormatHandler_tv006,display=%d,is_hdr_block_used=%d,en_422to444=%d,\n",
...
void fwif_color_SetDataFormatHandler_tv006_Seamless(unsigned char HDR_Mode)
{
	...
	fwif_color_colorspacergb2yuvtransfer_Seamless(HDR_Mode, InputSrcGetType);
	fwif_color_setCon_Bri_Color_Tint_tv006(0, system_info_struct);
}
#endif
...
void fwif_color_set_Mplus_ModeChange_TV006(DISP_MLE_MODE_T mode_sel)
{
	...
	//51c remove drvif_color_set_Mplus_ModeChange((DRV_Mplus_ModeChange *) &data);
}

static unsigned char gDM2_OE_EO_CSC3_Sram_init_Flag = 0;
void fwif_color_set_DM2_OE_EO_CSC3_Sram_init_Flag_TV006(unsigned char Flag)
{
	if (Flag == 1)
		gDM2_OE_EO_CSC3_Sram_init_Flag = 1;
	else
		gDM2_OE_EO_CSC3_Sram_init_Flag = 0;
}

unsigned char fwif_color_get_DM2_OE_EO_CSC3_Sram_init_Flag_TV006(void)
{
	return gDM2_OE_EO_CSC3_Sram_init_Flag;
}
...
char fwif_color_set_DM_HDR_3dLUT_24x24x24_16_TV006(void *p)
{
	...
	if (enable == 1) {
			...
			((get_HDMI_HDR_mode() == HDR_DOLBY_HDMI) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)))
		{
			...
			fwif_color_DM2_3D_LUT_BIT_Change(pArray, &HDR10_3DLUT_Compact[0], NEW_24x24x24);
	...
	if (fwif_HDR_compare_3DLUT(p, NEW_24x24x24) != 1) {
		rtd_pr_vpq_info("Rtice HDR 3D LUT same\n");
		ret = 1;
	} else {
...
char fwif_color_set_DM_HDR_3dLUT_17x17x17_16_TV006(void *p)
{
	...
	if (enable == 0) {
		...
		if (fwif_HDR_compare_3DLUT(p, OLD_17x17x17) != 1) {
			rtd_pr_vpq_info("Rtice HDR 3D LUT same, sRam\n");
			ret = 1;
		} else {
			fwif_color_DM2_3D_LUT_BIT_Change(pArray, &HDR_3DLUT_Compact[0], OLD_17x17x17);
			ret = drvif_DM_HDR_3dLUT_16(1, &HDR_3DLUT_Compact[0]);
			if (ret < 0) {
				drvif_Set_DM_HDR_3dLUT_17x17x17_16_By_DMA(0, NULL, 0, NULL);
				rtd_pr_vpq_emerg("DM_3dLUT_DMA, Sram can't write W\n");
				ret = -1;
			} else {
				rtd_pr_vpq_info("DM_3dLUT_DMA, done sram\n");
				ret = 0;
			}
		}
	} else {
		if (fwif_HDR_compare_3DLUT(p, OLD_17x17x17) != 1) {
			...
			fwif_color_DM2_3D_LUT_BIT_Change(pArray, &HDR_3DLUT_Compact[0], OLD_17x17x17);
			ret = fwif_color_DM_3D_17x17x17_LUT_Set_By_DMA(1, &HDR_3DLUT_Compact[0]);
		}
	}
...
{//Decide HDR setting
	...
	rtd_pr_vpq_info("fwif_color_set_HDR_Dolby_mode_Enable,display=%d,vsc_Src=%d,2pixel=%d,hdmi_in=%d,colorFMT=%d,\n",
		display, Get_DisplayMode_Src(display), twopixel_mode, hdmi_input, color_format);

	if (display == SLR_MAIN_DISPLAY) {
		if (HDR_type == HDR_DM_MODE_NONE) {
			if (color_format == COLOR_YUV422)
				drvif_DM_HDR_Vtop_set(display, 0, 1, 0);
			else
				drvif_DM_HDR_Vtop_set(display, 0, 0, 0);
		} else if (HDR_type == HDR_DM_HDR10_VIVID) {
			if (color_format == COLOR_YUV422)
				drvif_DM_HDR_Vtop_set(display, 1, 0, 1);
			else
				drvif_DM_HDR_Vtop_set(display, 1, 0, 0);
		} else {
			...
			{//no need to check dm connect status
				if (HDR_type == HDR_DM_TECHNICOLOR) {
					...
					{//Playback TC mode and compress mode
						drvif_DM_HDR_Vtop_set(display, 9, 0, 1);
					}
					else
					{//Playback TC mode and no compress mode
						drvif_DM_HDR_Vtop_set(display, 8, 0, 1);
					}
                    #else
                    drvif_DM_HDR_Vtop_set(display, 8, 0, 1);
                    #endif
...
void set_DM2_Hist_AutoMode_Flag(unsigned char value)
{
	DM2_Hist_AutoMode_Flag = value;
}

unsigned char get_DM2_Hist_AutoMode_Flag_impl(void)
{
	return DM2_Hist_AutoMode_Flag;
}
unsigned char (*get_DM2_Hist_AutoMode_Flag)(void) = get_DM2_Hist_AutoMode_Flag_impl;

void set_DM2_Hist_AutoMode_Counter(unsigned char value)
{
	DM2_Hist_AutoMode_Counter = value;
}

unsigned char get_DM2_Hist_AutoMode_Counter_impl(void)
{
	return DM2_Hist_AutoMode_Counter;
}
unsigned char (*get_DM2_Hist_AutoMode_Counter)(void) = get_DM2_Hist_AutoMode_Counter_impl;
...
#endif
	if (En == 1) {
		set_DM2_Hist_AutoMode_Flag(1);
		set_DM2_Hist_AutoMode_Counter(5);
		drvif_DM2_HDR_histogram_Auto_Mode_Init(1);
	} else {
		set_DM2_Hist_AutoMode_Flag(0);
		set_DM2_Hist_AutoMode_Counter(0);
		drvif_DM2_HDR_histogram_Auto_Mode_Init(0);
	}
}

void fwif_color_DM2_HDR_histogram_Mode_set_ISR(unsigned char Mode)
{
	drvif_Set_dm_sel_hdr_hist_position_ISR(Mode);
}

unsigned int HDR_Hist_Auto_Mode[DM2_Hist_MODE_MAX][HDR_Histogram_size] = {{0}};
unsigned int HDR_Hist_Auto_Mode_temp[HDR_Histogram_size] = {0};
unsigned char fwif_color_DM2_Hist_AutoMode_ch1_sta_ISR(void)
{
	...
	if ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE) && (get_DM2_Hist_AutoMode_Flag() == 1)) {
		IRQ_Status = drvif_DM2_HDR_histogram_IRQ_Status_Get_ISR();
		if (get_DM2_Hist_AutoMode_Counter() == 0) {
			if (IRQ_Status == 1) {
				hist_position = drvif_DM2_HDR_histogram_Mode_Get_ISR();
				hist_position = DM2_Hist_Y_Mode;
				if (hist_position == DM2_Hist_Y_Mode) {
					...
					fwif_color_DM2_HDR_histogram_Mode_set_ISR(DM2_Hist_RGB_MAX_Mode);
				} else {
					if (drvif_DM2_HDR_histogram_Get_ISR(&HDR_Hist_Auto_Mode_temp[0]) == 0)
						memcpy(HDR_Hist_Auto_Mode[DM2_Hist_Y_Mode], &HDR_Hist_Auto_Mode_temp[0], sizeof(int)*HDR_Histogram_size);
					fwif_color_DM2_HDR_histogram_Mode_set_ISR(DM2_Hist_Y_Mode);
				}
...
#ifndef UT_flag
void fwif_color_set_DM2_EOTF_TV006(UINT32* EOTF_table32_R, UINT32* EOTF_table32_G, UINT32* EOTF_table32_B)
{
#if 0	/* use DMA driver*/
	UINT16 i = 0;
	while (i < 1025)
...
//#endif
	...
	char ret;
	down(&VPQ_DM_DMA_TBL_Semaphore);
	if ((drvif_DM_HDR10_enable_Get() == 0) || ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) != _MODE_STATE_ACTIVE))) {
		//drvif_DM2_EOTF_Enable(0, 1); // use sRam
		rtd_pr_vpq_info("DM_EOTF_DMA, use Sram\n");
		...
		if ((db_apply == 1 || b02b03_no_sram_mode == 0) && (fwif_color_get_DM2_OE_EO_CSC3_Sram_init_Flag_TV006() == 0)) {
			drvif_DM2_B0203_noSram_Enable(1);
			ret = WaitDM_DB_Apply_Clean();
		} else {
			ret = 1;
		}
		...
		if (ret < 0) {
			...
			ret = drvif_DM2_EOTF_Set(EOTF_table32_R, EOTF_table32_G, EOTF_table32_B);
			if (ret < 0) {
				drvif_DM2_EOTF_Enable(1, 0);
				rtd_pr_vpq_emerg("DM_EOTF_DMA, Sram, write fail.\n");
			} else {
				drvif_DM2_EOTF_Enable(1, 1);
				rtd_pr_vpq_info("DM_EOTF_DMA, Sram write done 0,\n");
			}
		}
	} else {
		drvif_DM2_EOTF_Enable(1, 1);
...
void fwif_color_set_DM2_OETF_TV006(UINT16* OETF_table16_R, UINT16* OETF_table16_G, UINT16* OETF_table16_B)
{
#if 0	/* use DMA driver*/
	UINT16 i = 0;
	while (i < 1025)
	...
	unsigned char oetf_lut_enable, b0103_or_oetf, b02b03_no_sram_mode;
	unsigned char db_en, db_lv, db_apply;
	char ret;

	down(&VPQ_DM_DMA_TBL_Semaphore);
	if ((drvif_DM_HDR10_enable_Get() == 0) || ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) != _MODE_STATE_ACTIVE))) {
		//drvif_DM2_OETF_Enable(0, 1); // use sRam
		rtd_pr_vpq_info("DM_OETF_DMA, use Sram\n");

		drvif_Get_DM2_DB_Status(&db_en, &db_lv, &db_apply);
		drvif_Get_DM2_OETF_Enable_Status(&oetf_lut_enable, &b0103_or_oetf, &b02b03_no_sram_mode);
		if ((db_apply == 1 || b02b03_no_sram_mode == 0) && (fwif_color_get_DM2_OE_EO_CSC3_Sram_init_Flag_TV006() == 0)) {
			drvif_DM2_B0203_noSram_Enable(1);
			ret = WaitDM_DB_Apply_Clean();
		} else {
		...
		if (ret < 0) {
			...
			ret = drvif_DM2_OETF_Set(OETF_table16_R, OETF_table16_G, OETF_table16_B);
			if (ret < 0) {
				drvif_DM2_OETF_Enable(1, 0);
				rtd_pr_vpq_emerg("DM_OETF_DMA, Sram, write fail.\n");
			} else {
				drvif_DM2_OETF_Enable(1, 1);
				rtd_pr_vpq_info("DM_OETF_DMA, Sram write done 0\n");
			}
		}
	} else {
		drvif_DM2_B0203_noSram_Enable(1);
...
void fwif_color_set_HDR10_runmode(void)
{
	...
	get_vtop_input_info(SLR_MAIN_DISPLAY, &twopixel_mode, &hdmi_input, &color_format);
	...
	if ((DisplayMode_Src == VSC_INPUTSRC_HDMI)
		       || (DisplayMode_Src == VSC_INPUTSRC_DP)) {
		...
		if (VIP_system_info_structure_table->HDR_flag == HAL_VPQ_HDR_MODE_HDR10_VIVID)
			in_sel = 2;
		else if (color_format == COLOR_YUV422)
			in_sel = 1;
		else
			in_sel = 3;	//444, TC, TC need to disable
	} /*else {
...
void fwif_color_set_output_gamma_PTG_TV006 (unsigned char bCtrl, unsigned short R_value,unsigned short G_value,unsigned short B_value,unsigned short W_value)
{
	...
	if (bCtrl == 1) {
		...
		drvif_color_set_output_gamma_format(1, output_Location_ori, 0);
		...
		drvif_color_set_output_gamma_format(output_Gamma_En_ori, output_Location_ori, 0);
...
char fwif_color_DM2_set_EOTF_OETF_3D_LUT_Sram(void)
{
	unsigned char eotf_lut_enable, b0103_or_eotf, b02b03_no_sram_mode;
	unsigned char db_en, db_lv, db_apply;
	char ret = 0;
	...
	// check b0203 no sram for OE/EO
	if (fwif_color_get_DM2_OE_EO_CSC3_Sram_init_Flag_TV006() == 0 || b02b03_no_sram_mode == 0) {
		rtd_pr_vpq_info("DMA_LUT_Handler, Sram need to enable, Flag=%d, b0203=%d\n",
			fwif_color_get_DM2_OE_EO_CSC3_Sram_init_Flag_TV006(), b02b03_no_sram_mode);
		drvif_DM2_B0203_noSram_Enable(1);
		ret = WaitDM_DB_Apply_Clean();
	}
#if 0
...
char fwif_color_DM2_EOTF_OETF_3D_LUT_Handler(HAL_VPQ_HDR_MODE_T HDR_MODE)
{
	...
	if ((HDR_MODE != HAL_VPQ_HDR_MODE_HDR10) && (HDR_MODE != HAL_VPQ_HDR_MODE_HLG) && (HDR_MODE != HAL_VPQ_HDR_MODE_ST2094)
		...
		ret_val = fwif_color_DM2_set_EOTF_OETF_3D_LUT_Sram();
		if (ret_val < 0)
			rtd_pr_vpq_emerg("DMA_LUT_Handler, fwif_color_DM2_set_EOTF_OETF_3D_LUT_Sram fail\n");

		for (DMA_set_times = 0;DMA_set_times<DMA_set_times_max;DMA_set_times++) {
			ret_val = -1;
			if (HDR_MODE == HAL_VPQ_HDR_MODE_HDR10) {
					args.a3dLUT_16_pArray = (unsigned short*)HDR10_3DLUT_17x17x17;
					...
					if (fwif_VIP_get_Project_ID() == VIP_Project_ID_TV030) {
						...
						drvif_DM2_OETF_Set_Prog_Index_th(&(OETF_prog_idx_tbl[1].oetf_th[0]));
						Scaler_color_Get_HDR10_OETF_ByMaxCLL_ByPanelLuminance(0);
					}
					drvif_color_Set_HDR_OETF_Decrease(0);
					drvif_DM2_Wait_for_DMA_Apply_Done();
			} else if (HDR_MODE == HAL_VPQ_HDR_MODE_HLG) {
					args.a3dLUT_16_pArray = (unsigned short*)HLG_3DLUT_17x17x17;
					fwif_color_set_DM_HDR_3dLUT_17x17x17_16_TV006(&args);
					drvif_DM_set_HDR_3dLUT_byoffset(); //20221017, yvonne, need execute after fwif_color_set_DM_HDR_3dLUT_17x17x17_16_TV006 function
					drvif_color_set_BBC_shift_bit(1);
					fwif_color_set_DM2_EOTF_TV006(HLG_EOTF_LUT_R, HLG_EOTF_LUT_R, HLG_EOTF_LUT_R);
					drvif_DM2_OETF_Set_Prog_Index(&OETF_prog_idx_tbl[1]);
					drvif_DM2_OETF_Set_Prog_Index_th(&(OETF_prog_idx_tbl[1].oetf_th[0]));
					fwif_color_set_DM2_OETF_TV006(HLG_OETF_LUT_R, HLG_OETF_LUT_R, HLG_OETF_LUT_R);
					drvif_color_Set_HDR_OETF_Decrease(0);
					drvif_DM2_Wait_for_DMA_Apply_Done();
			} else if (HDR_MODE == HAL_VPQ_HDR_MODE_ST2094) {
					args.a3dLUT_16_pArray = &HDR10_3DLUT_17x17x17[0][0];
					fwif_color_set_DM_HDR_3dLUT_17x17x17_16_TV006(&args);
					drvif_color_set_BBC_shift_bit(0);
					fwif_color_set_DM2_EOTF_TV006(EOTF_LUT_R, EOTF_LUT_R, EOTF_LUT_R);
					drvif_DM2_OETF_Set_Prog_Index(&OETF_prog_idx_tbl[0]);
					drvif_DM2_OETF_Set_Prog_Index_th(&(OETF_prog_idx_tbl[0].oetf_th[0]));
					fwif_color_set_DM2_OETF_TV006(OETF_LUT_R, OETF_LUT_R, OETF_LUT_R);
					drvif_color_Set_HDR_OETF_Decrease(0);
					drvif_DM2_Wait_for_DMA_Apply_Done();
			} else if (HDR_MODE == HAL_VPQ_HDR_MODE_HDR10_VIVID) {
					// write linear then disable
					args.a3dLUT_16_pArray = &HDR10_3DLUT_17x17x17[0][0]; // use linear
					args.c3dLUT_16_enable = 1;
					fwif_color_set_DM_HDR_3dLUT_17x17x17_16_TV006(&args);
					args.c3dLUT_16_enable = 0;
					fwif_color_set_DM_HDR_3dLUT_17x17x17_16_TV006(&args);
					drvif_color_set_BBC_shift_bit(0);
					fwif_color_set_DM2_EOTF_TV006(EOTF_LUT_R, EOTF_LUT_R, EOTF_LUT_R);
					fwif_color_set_DM2_OETF_TV006(OETF_LUT_R, OETF_LUT_R, OETF_LUT_R);
...
{
	//51c remove drvif_color_SetFrameGainLimit(nFrameGainLimit);

}
void fwif_color_GetFrameGainLimit_TV006 (unsigned short *nFrameGainLimit)
{
	//51c remove drvif_color_GetFrameGainLimit(nFrameGainLimit);

}
void fwif_color_SetPixelGainLimit_TV006 (unsigned short nPixelGainLimit)
{
	//51c remove drvif_color_SetPixelGainLimit(nPixelGainLimit);

}
void fwif_color_GetPixelGainLimit_TV006 (unsigned short *nPixelGainLimit)
{
	//51c remove drvif_color_GetPixelGainLimit(nPixelGainLimit);
...
void fwif_color_set_LocalContrast_table_TV006(CHIP_LOCAL_CONTRAST_T *param)
{
    ...
    data.lc_local_sharp_en= gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_local_sharp_en;
	data.lc_tone_mapping_en= gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_tone_mapping_en;
	data.lc_subblk_mode = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_subblk_mode;
	data.lc_gcon_mode = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_gcon_mode;
	data.lc_hist_mode = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_hist_mode;
	data.lc_tenable = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_tenable;
	data.lc_valid = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_valid;
    #else
	data.lc_local_sharp_en= gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_local_sharp_en;
	data.lc_tone_mapping_en= gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_tone_mapping_en;
	//data.lc_db_en = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_db_en;
	//data.lc_db_apply = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_db_apply;
	//data.lc_db_read_level = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_db_read_level;
	data.lc_3d_mode = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_3d_mode;
	//data.lc_blk_hnum = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_blk_hnum;
	//data.lc_blk_vnum = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_blk_vnum;
	data.lc_subblk_mode = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_subblk_mode;
	data.lc_gcon_mode = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_gcon_mode;
	data.lc_hist_mode = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_hist_mode;
	data.lc_tenable = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_tenable;
	data.lc_valid = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_valid;
	//data.lc_blk_hsize = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_blk_hsize;
	//data.lc_blk_vsize = gVip_Table->Local_Contrast_Table[TableIdx].LC_Global_Ctrl.lc_blk_vsize;
    #endif
    /* Control by xxLGxx DB */
	...
	fwif_color_set_LC_Global_Region_Num_Size(TableIdx, LC_BLOCK_32x16);
...
void fwif_color_set_LocalContrast_ToneMappingXY_SlopePoint_TV006(CHIP_LOCAL_CONTRAST_T *param)
{
	...
	*/
	...
	grid0.lc_tmap_g0_th1 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th1;
	delta_grid_X[0][0]= grid0.lc_tmap_g0_th1;
	grid0.lc_tmap_g0_th2 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th2;
	delta_grid_X[0][1]= grid0.lc_tmap_g0_th2 -grid0.lc_tmap_g0_th1;
	grid0.lc_tmap_g0_th3 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th3;
	delta_grid_X[0][2]= grid0.lc_tmap_g0_th3 -grid0.lc_tmap_g0_th2;
	grid0.lc_tmap_g0_th4 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th4;
	delta_grid_X[0][3]= grid0.lc_tmap_g0_th4 -grid0.lc_tmap_g0_th3;
	grid0.lc_tmap_g0_th5 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th5;
	delta_grid_X[0][4]= grid0.lc_tmap_g0_th5 -grid0.lc_tmap_g0_th4;
	grid0.lc_tmap_g0_th6 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th6;
	delta_grid_X[0][5]= grid0.lc_tmap_g0_th6 -grid0.lc_tmap_g0_th5;
	grid0.lc_tmap_g0_th7 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th7;
	delta_grid_X[0][6]= grid0.lc_tmap_g0_th7 -grid0.lc_tmap_g0_th6;
	grid0.lc_tmap_g0_th8 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th8;
	delta_grid_X[0][7]= grid0.lc_tmap_g0_th8 -grid0.lc_tmap_g0_th7;
	grid0.lc_tmap_g0_th9 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th9;
	delta_grid_X[0][8]= grid0.lc_tmap_g0_th9 -grid0.lc_tmap_g0_th8;
	grid0.lc_tmap_g0_th10 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th10;
	delta_grid_X[0][9]= grid0.lc_tmap_g0_th10 -grid0.lc_tmap_g0_th9;
	grid0.lc_tmap_g0_th11 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th11;
	delta_grid_X[0][10]= grid0.lc_tmap_g0_th11 -grid0.lc_tmap_g0_th10;
	grid0.lc_tmap_g0_th12 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th12;
	delta_grid_X[0][11]= grid0.lc_tmap_g0_th12 -grid0.lc_tmap_g0_th11;
	grid0.lc_tmap_g0_th13 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th13;
	delta_grid_X[0][12]= grid0.lc_tmap_g0_th13 -grid0.lc_tmap_g0_th12;
	grid0.lc_tmap_g0_th14 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th14;
	delta_grid_X[0][13]= grid0.lc_tmap_g0_th14 -grid0.lc_tmap_g0_th13;
	grid0.lc_tmap_g0_th15 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th15;
	delta_grid_X[0][14]= grid0.lc_tmap_g0_th15 -grid0.lc_tmap_g0_th14;
	grid0.lc_tmap_g0_th16 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th16;
	delta_grid_X[0][15]= grid0.lc_tmap_g0_th16 -grid0.lc_tmap_g0_th15;
	grid0.lc_tmap_g0_th17 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th17;
	delta_grid_X[0][16]= grid0.lc_tmap_g0_th17 -grid0.lc_tmap_g0_th16;
	grid0.lc_tmap_g0_th18 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th18;
	delta_grid_X[0][17]= grid0.lc_tmap_g0_th18 -grid0.lc_tmap_g0_th17;
	grid0.lc_tmap_g0_th19 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th19;
	delta_grid_X[0][18]= grid0.lc_tmap_g0_th19 -grid0.lc_tmap_g0_th18;
	grid0.lc_tmap_g0_th20 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th20;
	delta_grid_X[0][19]= grid0.lc_tmap_g0_th20 -grid0.lc_tmap_g0_th19;
	grid0.lc_tmap_g0_th21 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th21;
	delta_grid_X[0][20]= grid0.lc_tmap_g0_th21 -grid0.lc_tmap_g0_th20;
	grid0.lc_tmap_g0_th22 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th22;
	delta_grid_X[0][21]= grid0.lc_tmap_g0_th22 -grid0.lc_tmap_g0_th21;
	grid0.lc_tmap_g0_th23 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th23;
	delta_grid_X[0][22]= grid0.lc_tmap_g0_th23 -grid0.lc_tmap_g0_th22;
	grid0.lc_tmap_g0_th24 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th24;
	delta_grid_X[0][23]= grid0.lc_tmap_g0_th24 -grid0.lc_tmap_g0_th23;
	grid0.lc_tmap_g0_th25 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th25;
	delta_grid_X[0][24]= grid0.lc_tmap_g0_th25 -grid0.lc_tmap_g0_th24;
	grid0.lc_tmap_g0_th26 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th26;
	delta_grid_X[0][25]= grid0.lc_tmap_g0_th26 -grid0.lc_tmap_g0_th25;
	grid0.lc_tmap_g0_th27 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th27;
	delta_grid_X[0][26]= grid0.lc_tmap_g0_th27 -grid0.lc_tmap_g0_th26;
	grid0.lc_tmap_g0_th28 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th28;
	delta_grid_X[0][27]= grid0.lc_tmap_g0_th28 -grid0.lc_tmap_g0_th27;
	grid0.lc_tmap_g0_th29 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th29;
	delta_grid_X[0][28]= grid0.lc_tmap_g0_th29 -grid0.lc_tmap_g0_th28;
	grid0.lc_tmap_g0_th30 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th30;
	delta_grid_X[0][29]= grid0.lc_tmap_g0_th30 -grid0.lc_tmap_g0_th29;
	grid0.lc_tmap_g0_th31 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid0.lc_tmap_g0_th31;
	...
	grid2.lc_tmap_g2_th1 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th1;
	delta_grid_X[2][0]= grid2.lc_tmap_g2_th1;
	grid2.lc_tmap_g2_th2 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th2;
	delta_grid_X[2][1]= grid2.lc_tmap_g2_th2-grid2.lc_tmap_g2_th1;
	grid2.lc_tmap_g2_th3 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th3;
	delta_grid_X[2][2]= grid2.lc_tmap_g2_th3-grid2.lc_tmap_g2_th2;
	grid2.lc_tmap_g2_th4 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th4;
	delta_grid_X[2][3]= grid2.lc_tmap_g2_th4-grid2.lc_tmap_g2_th3;
	grid2.lc_tmap_g2_th5 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th5;
	delta_grid_X[2][4]= grid2.lc_tmap_g2_th5-grid2.lc_tmap_g2_th4;
	grid2.lc_tmap_g2_th6 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th6;
	delta_grid_X[2][5]= grid2.lc_tmap_g2_th6-grid2.lc_tmap_g2_th5;
	grid2.lc_tmap_g2_th7 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th7;
	delta_grid_X[2][6]= grid2.lc_tmap_g2_th7-grid2.lc_tmap_g2_th6;
	grid2.lc_tmap_g2_th8 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th8;
	delta_grid_X[2][7]= grid2.lc_tmap_g2_th8-grid2.lc_tmap_g2_th7;
	grid2.lc_tmap_g2_th9 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th9;
	delta_grid_X[2][8]= grid2.lc_tmap_g2_th9-grid2.lc_tmap_g2_th8;
	grid2.lc_tmap_g2_th10 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th10;
	delta_grid_X[2][9]= grid2.lc_tmap_g2_th10-grid2.lc_tmap_g2_th9;
	grid2.lc_tmap_g2_th11 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th11;
	delta_grid_X[2][10]= grid2.lc_tmap_g2_th11-grid2.lc_tmap_g2_th10;
	grid2.lc_tmap_g2_th12 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th12;
	delta_grid_X[2][11]= grid2.lc_tmap_g2_th12-grid2.lc_tmap_g2_th11;
	grid2.lc_tmap_g2_th13 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th13;
	delta_grid_X[2][12]= grid2.lc_tmap_g2_th13-grid2.lc_tmap_g2_th12;
	grid2.lc_tmap_g2_th14 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th14;
	delta_grid_X[2][13]= grid2.lc_tmap_g2_th14-grid2.lc_tmap_g2_th13;
	grid2.lc_tmap_g2_th15 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th15;
	delta_grid_X[2][14]= grid2.lc_tmap_g2_th15-grid2.lc_tmap_g2_th14;
	grid2.lc_tmap_g2_th16 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th16;
	delta_grid_X[2][15]= grid2.lc_tmap_g2_th16-grid2.lc_tmap_g2_th15;
	grid2.lc_tmap_g2_th17 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th17;
	delta_grid_X[2][16]= grid2.lc_tmap_g2_th17-grid2.lc_tmap_g2_th16;
	grid2.lc_tmap_g2_th18 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th18;
	delta_grid_X[2][17]= grid2.lc_tmap_g2_th18-grid2.lc_tmap_g2_th17;
	grid2.lc_tmap_g2_th19 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th19;
	delta_grid_X[2][18]= grid2.lc_tmap_g2_th19-grid2.lc_tmap_g2_th18;
	grid2.lc_tmap_g2_th20 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th20;
	delta_grid_X[2][19]= grid2.lc_tmap_g2_th20-grid2.lc_tmap_g2_th19;
	grid2.lc_tmap_g2_th21 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th21;
	delta_grid_X[2][20]= grid2.lc_tmap_g2_th21-grid2.lc_tmap_g2_th20;
	grid2.lc_tmap_g2_th22 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th22;
	delta_grid_X[2][21]= grid2.lc_tmap_g2_th22-grid2.lc_tmap_g2_th21;
	grid2.lc_tmap_g2_th23 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th23;
	delta_grid_X[2][22]= grid2.lc_tmap_g2_th23-grid2.lc_tmap_g2_th22;
	grid2.lc_tmap_g2_th24 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th24;
	delta_grid_X[2][23]= grid2.lc_tmap_g2_th24-grid2.lc_tmap_g2_th23;
	grid2.lc_tmap_g2_th25 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th25;
	delta_grid_X[2][24]= grid2.lc_tmap_g2_th25-grid2.lc_tmap_g2_th24;
	grid2.lc_tmap_g2_th26 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th26;
	delta_grid_X[2][25]= grid2.lc_tmap_g2_th26-grid2.lc_tmap_g2_th25;
	grid2.lc_tmap_g2_th27 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th27;
	delta_grid_X[2][26]= grid2.lc_tmap_g2_th27-grid2.lc_tmap_g2_th26;
	grid2.lc_tmap_g2_th28 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th28;
	delta_grid_X[2][27]= grid2.lc_tmap_g2_th28-grid2.lc_tmap_g2_th27;
	grid2.lc_tmap_g2_th29 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th29;
	delta_grid_X[2][28]= grid2.lc_tmap_g2_th29-grid2.lc_tmap_g2_th28;
	grid2.lc_tmap_g2_th30 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th30;
	delta_grid_X[2][29]= grid2.lc_tmap_g2_th30-grid2.lc_tmap_g2_th29;
	grid2.lc_tmap_g2_th31 = gVip_Table->Local_Contrast_Table[TableIdx].LC_ToneMapping_Grid2.lc_tmap_g2_th31;
...
char fwif_color_ModuleTest_DM2_EOTF_OETF_3D_24x24x24_LUT_Handler(HAL_VPQ_HDR_MODE_T HDR_MODE)
{
	char ret_val = -1;
	unsigned char check_times = 0, check_times_max = 20;
	unsigned char DMA_set_times = 0, DMA_set_times_max = 3;
	static PQ_HDR_3dLUT_16_PARAM ModuleTest_args;
	ModuleTest_args.c3dLUT_16_enable = 1;

	if ((HDR_MODE != HAL_VPQ_HDR_MODE_HDR10) && (HDR_MODE != HAL_VPQ_HDR_MODE_HLG)) {
		ret_val = 1;
		rtd_pr_vpq_info("DMA_LUT_Handler, SDR\n");
	} else {
		for (DMA_set_times = 0;DMA_set_times<DMA_set_times_max;DMA_set_times++) {
			ret_val = -1;
			if (HDR_MODE == HAL_VPQ_HDR_MODE_HDR10) {
					ModuleTest_args.a3dLUT_16_pArray = &ModuleTest_HDR10_3DLUT_24x24x24[0][0];
					fwif_color_set_DM_HDR_3dLUT_24x24x24_16_TV006(&ModuleTest_args);
					fwif_color_set_DM2_EOTF_TV006(ModuleTest_EOTF_LUT_R, ModuleTest_EOTF_LUT_R, ModuleTest_EOTF_LUT_R);
					fwif_color_set_DM2_OETF_TV006(ModuleTest_OETF_LUT_R, ModuleTest_OETF_LUT_R, ModuleTest_OETF_LUT_R);
					drvif_DM2_Wait_for_DMA_Apply_Done();
			} else if (HDR_MODE == HAL_VPQ_HDR_MODE_HLG) {
					ModuleTest_args.a3dLUT_16_pArray = &ModuleTest_HDR10_3DLUT_24x24x24[0][0];
					fwif_color_set_DM_HDR_3dLUT_24x24x24_16_TV006(&ModuleTest_args);
