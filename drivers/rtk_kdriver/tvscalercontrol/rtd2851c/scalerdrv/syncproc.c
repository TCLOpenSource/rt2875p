...
{//Detect sync type
	...
	if(inverted == 1 && src  == _SRC_YPBPR)
	{
		...
		rtd_pr_adc_info("###func:%s inverted so change to no support####\r\n", __FUNCTION__);
	}
	else if(ucCounter != 0)
	{
...
//		RTD_Log(LOGGER_INFO,"\n SyncProc_Detector SyncType:%x\n", ucResult);
	...
	{
		ucResult = NO_SIGNAL;
		//rtd_pr_adc_info("SyncProc_TypeDetect timeout\n");
		rtd_pr_adc_info("#####rzhen##### [%s(%d)] #####\n", __func__, __LINE__);
	}
...
static unsigned short SyncProc_GetStablePeriod(unsigned char indicator)
 {
	...
	if(SYNC_PROCESSOR1 == indicator)
	{
		...
		{
			rtd_pr_adc_debug("n HS Overflow, Stable Period Change, Stable Polarity Change");
			goto error_result;
		}
...
static unsigned char SyncProc_SetVsyncCounterLevel(unsigned char indicator)
{
	...
	rtd_pr_adc_info("#####rzhen##### [%s(%d)] Vsyn Level = 0x%08x #####\n", __func__, __LINE__, result);
	...
	if ((result > HSYNC_ONLY)||(result < YPBPR_SYNC)) // No Sync type found
	{
		...
		if ((result_ret > HSYNC_ONLY)||(result_ret < YPBPR_SYNC)){ // No Sync type found
			...
			if ((result_ret <= HSYNC_ONLY)&&(result_ret >= YPBPR_SYNC))
				rtd_pr_adc_debug("n 3. Find Sync type in the compare level %x !!", usAdaptiveCompareLevel);
		}
		else
			rtd_pr_adc_debug("n 2. Find Sync type in the compare level %x !!", usAdaptiveCompareLevel);
	}
	else
		rtd_pr_adc_debug("n 1. Find Sync type in the compare level %x !!", usAdaptiveCompareLevel);
...
unsigned char SyncProc_SyncHandler(int src)
{
	...
	rtd_pr_adc_info("#####[%s(%d)] Detected signal-0 => %x\n", __FUNCTION__, __LINE__, ucTempSync);
	...
	switch(ucTempSync)
	{
		...
		case SEPARATE_SYNC:
			//rtd_pr_adc_info("#####rzhen##### [%s(%d)] ucTempSync=%d #####\n", __func__, __LINE__, ucTempSync);
			...
			{//WOSQRTK-4614 patch 20160108
				...
				msleep(1);
				sync_processor_sp1_stbrst0_reg.regValue = IoReg_Read32(SYNC_PROCESSOR_SP1_StbRst0_reg);
				if(sync_processor_sp1_stbrst0_reg.s1_cap_unlock == 1) {
					rtd_pr_adc_info("##### [%s(%d)] vsync non standard so change to no support####\n", __func__, __LINE__);
					return _MODE_NOSUPPORT;
				}
		...
		#else
			ret = SyncProc_SetVsyncCounterLevel(SYNC_PROCESSOR1);

			if(ucTempSync == HSYNC_ONLY){

				StructDisplayInfo *p_adcdispinfo=Get_ADC_Dispinfo();
				ModeInformationType *p_adctiminginfo=Get_ADC_Timinginfo();

				if (drvif_mode_offlinemeasure(_OFFLINE_MS_SRC_DEHV1, ANALOG_MODE_MEASUREMENT, p_adcdispinfo, p_adctiminginfo) != _MODE_SUCCESS){

					rtd_pr_adc_notice("#####rzhen##### [%s(%d)] #####\n", __func__, __LINE__);
					return _MODE_NOSUPPORT; //sync is detected, but not the YPbPr valid sync, treat as no support
				}

				//rtd_pr_adc_notice("#####rzhen##### [%s(%d)] 0xb8021104[12:0]=0x%08x #####\n", __func__, __LINE__, IoReg_Read32(0xb8021104));
			}
...
void Adjust_ADC_Clock(unsigned char display, unsigned char fast_lock_enable)
{
	...
	rtd_pr_adc_notice("#####[%s(%d)] p_dispinfo->INPUT_ADC_CLOCK=%d\n", __FUNCTION__, __LINE__, p_dispinfo->INPUT_ADC_CLOCK);
...
unsigned char SyncProc_OffMeasure(int src, unsigned char OfflineSrcType)//This is for YPP and VGA offline measure
 {
	...
	if (drvif_mode_offlinemeasure(OfflineSrcType, ANALOG_MODE_MEASUREMENT, p_adcdispinfo, p_adctiminginfo) != _MODE_SUCCESS) {
		...
		rtd_pr_adc_notice("#####rzhen##### [%s(%d)]result=0x%08x #####\n", __func__, __LINE__, result);
		goto error_result;
	}
	up(get_offline_semaphore());
	...
	//rtd_pr_adc_info("#####rzhen##### [%s(%d)] result = 0x%08x#####\n", __func__, __LINE__, result);
		...
		case _SRC_VGA:
			if((_ENABLE == Scaler_Get_VGA_Support_SOG()) || (_ENABLE == Scaler_Get_VGA_Support_Composite_Sync()))
			{
				...
				if(SEPARATE_MODE == VGA_Detect_SyncTypeMode())
				{
					...
					rtd_pr_adc_info("###### [%s(%d)] result = 0x%08x#####\n", __func__, __LINE__, result);
				}
				else
				{
					result = SyncProc_OffMeasure(src, _OFFLINE_MS_SRC_DEHV1);
...
unsigned char SyncProc_CheckMode(void)
{
	if(!drvif_mode_onlinemeasure(ANALOG_MODE_MEASUREMENT)){
		rtd_pr_adc_debug("nSyncProcCheckModeChange\n");
		return  FALSE;
	}
	return TRUE;
}
...
void YPbPr_SyncProc(void)
{
	...
	sync_processor_ef_fld_pos_RBUS sync_processor_ef_fld_pos_reg;
	...
	IoReg_Write32(DDS_PLL_DDS_MIX_W2_reg, 0x000002a0);  // Delay chain select (Default value))
		...
		sync_processor_sp1_stbctrl_reg.s1_dds_cap_win_h2l_enable = 0;
		sync_processor_sp1_stbctrl_reg.s1_stable_en = 0;
		IoReg_Write32(SYNC_PROCESSOR_SP1_StbCtrl_reg, sync_processor_sp1_stbctrl_reg.regValue);
		...
		sync_processor_sp1_clmpmsk_reg.s1_cap_miss_flag2hswin_enable = 0;
		...
		sync_processor_ef_fld_pos_reg.regValue = IoReg_Read32(SYNC_PROCESSOR_Ef_fld_Pos_reg);
		sync_processor_ef_fld_pos_reg.s1_miss_flag2dds_enable = 0;
		IoReg_Write32(SYNC_PROCESSOR_Ef_fld_Pos_reg, sync_processor_ef_fld_pos_reg.regValue);
	...
	rtd_pr_adc_debug("YPbPr in SyncProc_InitSrc\n");
}
...
//	adc_adc_dcrestore_ctrl_RBUS adc_adc_dcrestore_ctrl_reg;
	...
	if (src == _SRC_VGA)
	{
		...
		rtd_pr_adc_notice("#####[%s(%d)] soy_path = %d \n", __FUNCTION__, __LINE__, soy_path);
		...
		rtd_pr_adc_notice("#####[%s(%d)] adc_path = %d \n", __FUNCTION__, __LINE__, adc_path);
...
void SyncProc_coast_front_back(UINT8 front, UINT8 back)
{
	...
	sync_processor_sp1_coastftbk_reg.s1_coast_front= front;
	sync_processor_sp1_coastftbk_reg.s1_coast_back = back;
...
void SyncProc_Updw_Clamp_Ctrl(int src){
	...
	if(src == _SRC_YPBPR){
		sync_processor_updw_global_ctrl1_reg.updw_ny_mode_r = 0x0;	//fixed, Non-SOY
		sync_processor_updw_global_ctrl1_reg.updw_ny_mode_g = 0x0;	//fixed, SOY
		sync_processor_updw_global_ctrl1_reg.updw_ny_mode_b = 0x0;	//fixed, Non-SOY
	}else{
	...
	if(src == _SRC_YPBPR){
		...
		//rtd_pr_adc_emerg("#####[%s(%d)]updn_end = %d #####\n", __func__, __LINE__,updn_end);
	...
	sync_processor_updw_ctrl1_r_reg.charge_mode_sel_r = 0x0;	//fixed, auto mode for up/dn clamp
...
void SyncProc_Updw_Lock_Range(void){
	...
	sync_processor_updw_st2_ctrl_r_reg.regValue = IoReg_Read32(SYNC_PROCESSOR_updw_St2_CTRL_R_reg);
	sync_processor_updw_st2_ctrl_r_reg.lock_range_r = 0x5;		// Fine tune
	IoReg_Write32(SYNC_PROCESSOR_updw_St2_CTRL_R_reg, sync_processor_updw_st2_ctrl_r_reg.regValue);

	sync_processor_updw_st2_ctrl_g_reg.regValue = IoReg_Read32(SYNC_PROCESSOR_updw_St2_CTRL_G_reg);
	sync_processor_updw_st2_ctrl_g_reg.lock_range_g = 0x5;		// Fine tune
	IoReg_Write32(SYNC_PROCESSOR_updw_St2_CTRL_G_reg, sync_processor_updw_st2_ctrl_g_reg.regValue);

	sync_processor_updw_st2_ctrl_b_reg.regValue = IoReg_Read32(SYNC_PROCESSOR_updw_St2_CTRL_B_reg);
	sync_processor_updw_st2_ctrl_b_reg.lock_range_b = 0x5;		// Fine tune
	IoReg_Write32(SYNC_PROCESSOR_updw_St2_CTRL_B_reg, sync_processor_updw_st2_ctrl_b_reg.regValue);
}
...
void SyncProc_Check_UpDn_Stable(unsigned char display){
	...
	unsigned char count = (IoReg_Read32(DI_IM_DI_CONTROL_reg)&_BIT3)?4:7; //5a mode,3*msleep(5)
	...
	while(count > 0){//wait stage2 stable
		...
		if(count<5 && updw_stage_flag_r_reg.lock_flag_r && updw_stage_flag_g_reg.lock_flag_g && updw_stage_flag_b_reg.lock_flag_b){

			rtd_pr_adc_notice("#####[%s(%d)] ADC Up/Dn clamp stable count=%d!!!\n", __FUNCTION__, __LINE__, count);
			break;
		}else{
			count--;

			if(count == 0){
				rtd_pr_adc_notice("#####[%s(%d)] ADC Up/Dn clamp timeout !!!\n", __FUNCTION__, __LINE__);
			}
...
//	sync_processor_sp1_clmpmsk_RBUS  sync_processor_sp1_clmpmsk_reg;
	...
	rtd_pr_adc_info("VGA in SyncProc_InitSrc\n");
}
