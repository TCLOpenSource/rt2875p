...
*/
	...
	if((Get_GOSUBandVODMA()==TRUE)&&((get_rotate_mode(SLR_MAIN_DISPLAY)==(DIRECT_VO_FRAME_ORIENTATION)ROTATE_MODE_90)||(get_rotate_mode()==(DIRECT_VO_FRAME_ORIENTATION)ROTATE_MODE_270))&&(Scaler_InputSrcGetMainChType() == _SRC_VO))
	{
...
StructSrcRect cal_new_input_base_on_display_win(unsigned char display)
{
    ...
    {
        if (m_input_size.src_wid > _DISP_WID)
            m_input_size.src_wid = _DISP_WID;

        if (m_input_size.src_height > _DISP_LEN)
            m_input_size.src_height = _DISP_LEN;
...
StructSrcRect cal_new_inputRegion_base_on_hal_orgInput(unsigned char display)
{
    ...
    rtd_pr_vbe_info("func:%s, inputRegion:x:%d,y:%d,wid:%d,len:%d\n", __FUNCTION__, inputRegion.srcx,inputRegion.srcy,inputRegion.src_wid,inputRegion.src_height);
    rtd_pr_vbe_info("func:%s, inputTiming:x:%d,y:%d,wid:%d,len:%d\n", __FUNCTION__, inputTiming.srcx,inputTiming.srcy,inputTiming.src_wid,inputTiming.src_height);

	//check 4k2k input and output ,overflow
	if(inputTiming.src_wid && inputTiming.src_height)
    {
        if (inputRegion.src_wid > inputTiming.src_wid)
             inputRegion.src_wid = inputTiming.src_wid;

         if (inputRegion.src_height > inputTiming.src_height)
             inputRegion.src_height = inputTiming.src_height;
...
unsigned char fw_scaler_cal_sdnr_capture_for_hal_orgInput(unsigned char display)
{
	...
	if ((SCALER_DISP_CHANNEL)display == SLR_SUB_DISPLAY)
	{
		...
		ucSDNR_W = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID);
	...
	else// SLR_MAIN_DISPLAY
	{
		...
		if (Scaler_Get_di_hsd_flag()){
			...
			if ((VD_27M_HSD960_DI5A == fw_video_get_27mhz_mode(display))&&(Get_DisplayMode_Src(display) == VSC_INPUTSRC_AVD)&&
				...
				rtd_pr_vbe_warn("[HSD] new sdnr size:x :%d, wid:%d\n",temp_x,temp_w);
			}
		} else {
	...
	if(Scaler_DispGetStatus((SCALER_DISP_CHANNEL)display,SLR_DISP_422CAP) && Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display, SLR_INPUT_FRAMESYNC))
	{
...
StructSrcRect cal_InputRegion_Base_Scaler_InputRegionType(unsigned char display, SCALER_VSC_INPUT_REGION_TYPE type)
{
    StructSrcRect newInputRegion = {0};

    // cal inputregion base input type
    if (INPUT_BASE_ON_HAL_ORIGINAL_INPUT == type)
    {
        newInputRegion = cal_new_inputRegion_base_on_hal_orgInput(display);
    }
    else if (INPUT_BASE_ON_DISP_WIN == type)
    {
        newInputRegion = cal_new_input_base_on_display_win(display);
    }
    else //INPUT_BASE_ON_TIMING
    {
        get_input_for_vgip(display, &newInputRegion);
    }

    // if inputregion w and h is zero, need add protection
    if (newInputRegion.src_wid == 0)
    {
		if (Scaler_Get_di_hsd_flag() && (SLR_MAIN_DISPLAY == display) && (VD_27M_HSD960_DI5A == fw_video_get_27mhz_mode(display)) && (Get_DisplayMode_Src(display) == VSC_INPUTSRC_AVD)&&
			...
			newInputRegion.src_wid = AVD_27M_VGIP_WIDTH/2;
			newInputRegion.srcx = 0;
		} else {
			newInputRegion.src_wid = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_WID);
			newInputRegion.srcx = 0;
		}
	}
    if (newInputRegion.src_height == 0)
    {
    	newInputRegion.src_height = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_LEN);
    	newInputRegion.srcy = 0;
    }

    rtd_pr_vbe_notice("function=%s, line=%d, inputRegionType=%d, input region(%d,%d,%d,%d)\n", __FUNCTION__, __LINE__, type,
    newInputRegion.srcx, newInputRegion.srcy, newInputRegion.src_wid, newInputRegion.src_height);

    return newInputRegion;
}
...
unsigned char fw_scaler_cal_sdnr_capture(unsigned char display)
{
    unsigned char src = 0;
    unsigned char colorspace = 0;
    unsigned int uDI_WID = 0;
    unsigned int uINPUT_WID = 0x00;
    unsigned int ucSDNR_W = 0, ucSDNR_X = 0;
    StructSrcRect newInput = {0};
    ...
    newInput = cal_InputRegion_Base_Scaler_InputRegionType(display, Scaler_InputRegionType_Get(display));
    ...
    else if ((SLR_MAIN_DISPLAY == display) && (Scaler_Get_di_hsd_flag()))
    {
        uDI_WID = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DI_WID);
        uINPUT_WID = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID);
    ...
    if (((SCALER_DISP_CHANNEL)display == SLR_SUB_DISPLAY) && (Get_sub_from_DI_mux_path()))
    {
    ...
    {
         if (newInput.src_height > Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_DI_LEN))/*Not reasonable input height*/
    ...
    if (newInput.src_wid > Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_DI_WID))/*Not reasonable input wid*/
    ...
    colorspace = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_COLOR_SPACE);
    if (((src == _SRC_HDMI) || (src == _SRC_VO)) && ((colorspace != VODMA_COLOR_YUV444) && (colorspace!= VODMA_COLOR_RGB)))
    {
        //This means that shift value need even <= need to do by SDNR side
        if((Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA)%2) != (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPH_ACT_STA_PRE)%2))
            Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA, Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA) -1);
    }

    if(Scaler_DispGetStatus((SCALER_DISP_CHANNEL)display,SLR_DISP_422CAP) && (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display, SLR_INPUT_FRAMESYNC)||(dvrif_memory_compression_get_enable(display) == FALSE)))
    {
        Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA, (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA) & ~0x1));
        Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID) & ~0x1));
    }
	...
	if(Scaler_InputRegionType_Get(display) == INPUT_BASE_ON_DISP_WIN){
		...
		if (newInput.src_wid == 0) {
			if (Scaler_Get_di_hsd_flag()&&(SLR_MAIN_DISPLAY==display) &&(VD_27M_HSD960_DI5A == fw_video_get_27mhz_mode(display))&&(Get_DisplayMode_Src(display) == VSC_INPUTSRC_AVD)&&
				(Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPH_ACT_WID) == AVD_27M_VGIP_WIDTH))	{
				// AV function need to convert to  original source
				newInput.src_wid = AVD_27M_VGIP_WIDTH/2;
				newInput.srcx = 0;
			} else {
...
{//disable main forcebg double buffer, becasue the double buffer is changed
	...
	if (display == SLR_MAIN_DISPLAY) {
		...
		if(En) {
			IoReg_SetBits(PPOVERLAY_Double_Buffer_CTRL3_reg, _BIT2|_BIT0);
		} else {
			IoReg_Mask32(PPOVERLAY_Double_Buffer_CTRL3_reg, ~_BIT2, 0);
		}
		spin_unlock_irqrestore(get_DBreg_spinlock(SLR_MAIN_DISPLAY), flags);//unlock double buffer spinlock
	} else {
		spin_lock_irqsave(get_DBreg_spinlock(SLR_MAIN_DISPLAY), flags);//lock double buffer spinlock //also use the main path spinlock because also control 0xb8028028
...
void  fw_scaler_4k2k_color_config(void)
{
	...
	//if((IoReg_Read32(0xB801bc3c) & _BIT15))
	{
		...
		//check 0xb8029004[31:30]=0  //disable adaptive uzu
		    ...
		    IoReg_Write32(0xb802a000,0x4000000C); //disable sharpness,modify for PQ setting for M3 0723
...
//	sfg_sfg_ctrl_c_RBUS    sfg_ctrl_c_reg;
	...
	sfg_ctrl_7_reg.regValue = IoReg_Read32(SFG_SFG_CTRL_7_reg);
	sfg_ctrl_7_reg.hsync_width = Get_DISP_HSYNC_WIDTH();//HS_Width = 16 dclk = 16*Pixel_Mode*n DH_Width (must be 4*N, N is an integer and >= 1)
	sfg_ctrl_7_reg.hsync_delay = Get_DISP_HORIZONTAL_TOTAL() -13;
	IoReg_Write32(SFG_SFG_CTRL_7_reg, sfg_ctrl_7_reg.regValue);
...
extern unsigned int scaler_disp_smooth_get_memc_protect_vtotal(PROTECT_VTOTAL_TYPE eType);
...
unsigned int scalerdisplay_get_vtotal_by_vfreq(UINT16 vsyncFreq)
{
	...
	if(vbe_disp_get_adaptivestream_fs_mode()){
		...
		vertical_total = scaler_disp_smooth_get_memc_protect_vtotal(VTOTAL_FOR_NORMAL);
	}
#endif
//	if(vertical_total==0){
	if (0 != h_total)
	...
	rtd_pr_vbe_info("[%s %d]######## vertical_total:%d ###############\n",
...
{
	extern void enable_data_access(void);//enable data access
	unsigned long flags = 0;//for spin_lock_irqsave
...
#ifdef CONFIG_DUAL_CHANNEL
    if(channel > SLR_SUB_DISPLAY || channel < SLR_MAIN_DISPLAY){
        rtd_pr_vbe_info("[%s] display source is error!! %d\n", __FUNCTION__, channel);
        return;
    }
#else
...
//#endif
  ...
  else
  {
	if(channel == SLR_MAIN_DISPLAY){//main path
		...
		//apply double buffer
		drvif_scalerdisplay_wait_d0_apply(channel);
        RUTIL_TM_END_AND_DUMP(eRUtilTM_VSC_Unmute, "scalerdisplay_force_bg_enable: Unmuted", CONFIG_RUTIL_TM_LOG_DUMP_TO_CONSOLE);
#ifndef BUILD_QUICK_SHOW
		set_atv_go_smooth_toggle_flag(SLR_MAIN_DISPLAY, TRUE);
		wakeup_mute_off_callback(SLR_MAIN_DISPLAY);
#endif
		rtd_pr_vbe_notice("####RTK Disable Main ForceBG (90K=%d)!!!####\r\n", IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg));
#ifndef BUILD_QUICK_SHOW
		enable_data_access();
#endif
//#ifdef CONFIG_ORBIT_PIXEL_SHIFT_ALGO
