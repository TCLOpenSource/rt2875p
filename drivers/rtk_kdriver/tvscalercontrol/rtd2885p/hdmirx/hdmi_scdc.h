...
#define SCDC_DAISY_ERR                       (0x1<<2)
#define SCDC_MONO_DIR_ON                     (0x1<<3)
#define SCDC_MONO_DIR_ERR                    (0x1<<4)
#define SCDC_CA_PWR_ERR                      (0x1<<5)
...
typedef struct
{
    ...
    unsigned char is_need_recover_by_onms;
...
extern unsigned char lib_hdmi_scdc_get_sink_config_30(unsigned char nport);
...
extern unsigned char lib_hdmi_scdc_get_frl_start(unsigned char nport);
...
extern void newbase_hdmi_sync_qs_scdc_status(unsigned char port);

#ifdef HDMI_SCDC_DBG_EN                        
    #define HDMI_SCDC_DBG(fmt, args...)       rtd_pr_hdmi_debug("[HDMI][SCDC][DBG]" fmt, ##args)
#else
    #define HDMI_SCDC_DBG(fmt, args...)
#endif

#define HDMI_SCDC_INFO(fmt, args...)          rtd_pr_hdmi_info("[HDMI][SCDC][Info]" fmt, ##args)       
#define HDMI_SCDC_WARN(fmt, args...)          rtd_pr_hdmi_warn("[HDMI][SCDC][WARN]" fmt, ##args) 
