...
extern unsigned char g_ddc_wakeup_enable;
#endif
    ...
    )
{
    ...
    n = scnprintf(ptr, count, "HDMI2.1 Status : FRL Mode=%s (%d), is_DSC=%d, DSC_FSM:%s, LT FSM = %s (%d), LT Error Status: %x\n",
                   _hdmi_hd21_frl_mode_str(p_phy_st->frl_mode), p_phy_st->frl_mode,
                    GET_H_RUN_DSC(port), _hdmi_hd21_dsc_fsm_str(GET_DSCD_FSM()),
    ...
    )
{
    ...
    if (strcmp(attr->attr.name, "debug_ctrl")==0)
    {
        ...
        // factory_mode_en
        //---------------------------------------------------
        n = scnprintf(ptr, count, "factory_or_power_only_mode_en=%d   (Force Run Factory flow)\n", 
            newbase_hdmi_get_factory_or_power_only_mode_enable());
        ptr+=n; count-=n;

        n = scnprintf(ptr, count, "factory_or_power_only_mode_bch_irq_en=%d   (0: disable, 1: enable)\n", 
            factory_or_power_only_mode_bch_irq_en);
        ptr+=n; count-=n;

        n = scnprintf(ptr, count, "factory_or_power_only_mode_hd21_eq_ofst=%d   (-10~10)\n", 
            factory_or_power_only_mode_hd21_eq_ofst);
        ptr+=n; count-=n;
...
#ifdef CONFIG_RTK_KDRV_DDC_WAKEUP
        {
            ...
            unsigned char addr_cnt = 0;
            int ret = 0;

            for (i=0; i<HDMI_PORT_TOTAL_NUM; i++)
            {
                n = scnprintf(ptr, count, "ddc_wakeup_en [%d]=%02d\n", RTK_HDMI_DDC(i), g_ddc_wakeup_enable);
                ptr+=n; count-=n;

                ret = rtk_ddc_lib_get_ddc_wakeup_addr(RTK_HDMI_DDC(i), addr, 12, &addr_cnt);
                n = scnprintf(ptr, count, "ddc_wakeup_addr [%d]=", RTK_HDMI_DDC(i));
                ptr+=n; count-=n;
                if (ret == 0)
                {
                    for (j=0;j<addr_cnt;j++)
                    {
                        n = scnprintf(ptr, count, "%2X ", addr[j]);
                        ptr+=n; count-=n;
                    }
                }
                n = scnprintf(ptr, count, "\n");
                ...
                addr[0] = 0;
                ret = rtk_ddc_lib_get_ddc_hitted_wakeup_addr(RTK_HDMI_DDC(i), addr);
                n = scnprintf(ptr, count, "ddc_wakeup_hit_addr [%d]=%02x\n", RTK_HDMI_DDC(i), ret?0:addr[0]);
                ptr+=n; count-=n;
            }
        }
#endif
                   ...
                   const char *buf, size_t count)
{
    ...
    if (strcmp(attr->attr.name, "debug_ctrl")==0)
    {
        ...
        else if (sscanf(buf, "factory_or_power_only_mode_en=%1d", &val)==1)
        {
            newbase_hdmi_set_factory_or_power_only_mode_enable((val) ? 1 : 0);
        }
        else if (sscanf(buf, "factory_or_power_only_mode_bch_irq_en=%1d", &val)==1)
        {
            factory_or_power_only_mode_bch_irq_en = (val) ? 1 : 0;
        }
        else if (sscanf(buf, "factory_or_power_only_mode_hd21_eq_ofst=%5d", &val)==1)
        {
            if (val < -10) 
                val = -10;
            else if (val > 10)  
                val = 10;
            factory_or_power_only_mode_hd21_eq_ofst = val;
        }
...
#ifdef CONFIG_RTK_KDRV_DDC_WAKEUP
        else if (sscanf(buf, "ddc_wakeup_en=%d", &val)==1) 
        {
            unsigned char i;

            for (i=0; i<HDMI_PORT_TOTAL_NUM; i++)
                rtk_ddc_lib_enable_wakeup(RTK_HDMI_DDC(i), val?1:0);
            g_ddc_wakeup_enable = val?1:0;
        }
        else if (sscanf(buf, "ddc_wakeup_clr_hit=%d", &val)==1) 
        {
            unsigned char i;
            if (val)
            {
                for (i=0; i<HDMI_PORT_TOTAL_NUM; i++)
                {
                    rtk_ddc_lib_clr_ddc_hitted_wakeup_addr(RTK_HDMI_DDC(i));
                }
            }
        }
        else if (sscanf(buf, "ddc_wakeup_reset=%d", &val)==1) 
        {
            unsigned char i;
            if (val)
            {
                for (i=0; i<HDMI_PORT_TOTAL_NUM; i++)
                    rtk_ddc_lib_wakeup_reset(RTK_HDMI_DDC(i));
            }
        }
        else if (sscanf(buf, "ddc_wakeup_default_addr=%d", &val)==1) 
        {
            unsigned char i;
            if (val)
            {
                for (i=0; i<HDMI_PORT_TOTAL_NUM; i++)
                {
                    rtk_ddc_lib_set_ddc_wakeup_addr(RTK_HDMI_DDC(i), 0x50, 1); // 0xA0 EDID
                    rtk_ddc_lib_set_ddc_wakeup_addr(RTK_HDMI_DDC(i), 0x52, 1); // 0xA4 EDID
                    rtk_ddc_lib_set_ddc_wakeup_addr(RTK_HDMI_DDC(i), 0x30, 1); // 0x60 E-EDID
                    rtk_ddc_lib_set_ddc_wakeup_addr(RTK_HDMI_DDC(i), 0x37, 1); // 0x6E reserved for DDC/CI
                    rtk_ddc_lib_set_ddc_wakeup_addr(RTK_HDMI_DDC(i), 0x3A, 1); // 0x74 HDCP
                    rtk_ddc_lib_set_ddc_wakeup_addr(RTK_HDMI_DDC(i), 0x3B, 1); // 0x76 HDCP
                    rtk_ddc_lib_set_ddc_wakeup_addr(RTK_HDMI_DDC(i), 0x54, 1); // 0xA8 SCDC
                    rtk_ddc_lib_set_ddc_wakeup_addr(RTK_HDMI_DDC(i), 0x40, 1); // 0x80 reserved for display port(Dual mode video Adaptor[2])
                }
            }
        }
        else if (sscanf(buf, "ddc_wakeup_addr=%2x", &val)==1) 
        {
            unsigned char wakeup_addr_en = 0;
            unsigned char wakeup_addr = 0;
            unsigned char i;

            wakeup_addr_en = (val&0x80)?1:0;
            wakeup_addr = val&0x7F;
            if (val)
            {
                for (i=0; i<HDMI_PORT_TOTAL_NUM; i++)
                    rtk_ddc_lib_set_ddc_wakeup_addr(RTK_HDMI_DDC(i), wakeup_addr, wakeup_addr_en);
            }
        }
#endif
        else
    ...
    )
{
    ...
    if (strcmp(attr->attr.name, "meas")==0)
    {
        ...
        if(lib_offline_measure_native(OFFMS_SOURCE_HDMI,&tm, frl_mode))
        {
            ...
            measure_result2 &= lib_offline_measure_ex(OFFMS_SOURCE_HDMI,&tm2, frl_mode, OFFMS_ONE_SHOT);
...
int __init hdmi_module_init(void)
{
    ...
    {
        HDMI_WARN("hdmi_module_init failed - register misc device failed\n");
        return -ENODEV;
    }
#ifdef MODULE
