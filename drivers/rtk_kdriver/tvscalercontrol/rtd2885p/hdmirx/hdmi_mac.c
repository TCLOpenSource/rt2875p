...
void lib_hdmi_sys_crt_on(void)
{
    ...
    hdmi_out(SYS_REG_INT_CTRL_VCPU_reg, (SYS_REG_INT_CTRL_VCPU_standby_hdmi_stb_int_scpu_routing_en_mask|SYS_REG_INT_CTRL_VCPU_write_data_mask));
}
...
void lib_hdmi_crt_reset_hdmi_common(void)
{
    ...
    // do not close rst because merlin8 yuv420 circuit needed
    // RST = 0
    //hdmi_mask(HDMI_P0_hdmi_rst0_reg,~HDMI_hdmi_rst0_rstn_hdmi_common_mask, 0x0);
    //udelay(5);
...
void lib_hdmi_420_en(unsigned char nport, unsigned char en)
{
    ...
    hdmi_mask(COMMON_RX_420_HDMIRX_YUV420_CTRL_0_reg, ~COMMON_RX_420_HDMIRX_YUV420_CTRL_0_yuv420_cr_en_mask, COMMON_RX_420_HDMIRX_YUV420_CTRL_0_yuv420_cr_en(en?1:0));
}
...
void lib_hdmi_set_420_vactive(unsigned char nport, unsigned int vactive)
{
    ...
    hdmi_mask(COMMON_RX_420_HDMIRX_YUV420_CTRL_1_reg, ~COMMON_RX_420_HDMIRX_YUV420_CTRL_1_yuv420_cr1_mode_mask, COMMON_RX_420_HDMIRX_YUV420_CTRL_1_yuv420_cr1_mode_mask);
    hdmi_mask(COMMON_RX_420_HDMIRX_YUV420_CTRL_1_reg, ~COMMON_RX_420_HDMIRX_YUV420_CTRL_1_yuv420_cr1_vactive_mask, COMMON_RX_420_HDMIRX_YUV420_CTRL_1_yuv420_cr1_vactive(vactive));
...
void lib_hdmi_420_fva_ds_mode(unsigned char nport, unsigned char en)
{
    ...
    hdmi_mask(COMMON_RX_420_HDMIRX_YUV420_CTRL_1_reg, ~COMMON_RX_420_HDMIRX_YUV420_CTRL_1_yuv420_cr1_fva_ds_mode_mask, COMMON_RX_420_HDMIRX_YUV420_CTRL_1_yuv420_cr1_fva_ds_mode(en?1:0));
}
...
unsigned int lib_hdmi_get_420_fva_ds_mode(unsigned char nport)
{
    ...
    return COMMON_RX_420_HDMIRX_YUV420_CTRL_1_get_yuv420_cr1_fva_ds_mode(hdmi_in(COMMON_RX_420_HDMIRX_YUV420_CTRL_1_reg));
}
...
void lib_hdmi_video_pll_init(unsigned char nport)
{
    ...
    //hdmi_mask(HDMI_YUV420_CR1_reg, ~HDMI_YUV420_CR1_mode_mask, 0);// no use
    hdmi_mask(COMMON_RX_420_HDMIRX_YUV420_CTRL_1_reg, ~COMMON_RX_420_HDMIRX_YUV420_CTRL_1_yuv420_cr1_mode_mask, 0);
    //hdmi_mask(HDMI_YUV420_CR1_reg, ~HDMI_YUV420_CR1_vactive_mask, HDMI_P0_YUV420_CR1_vactive(0));
    hdmi_mask(COMMON_RX_420_HDMIRX_YUV420_CTRL_1_reg, ~COMMON_RX_420_HDMIRX_YUV420_CTRL_1_yuv420_cr1_vactive_mask, COMMON_RX_420_HDMIRX_YUV420_CTRL_1_yuv420_cr1_vactive(0));
    lib_hdmi_420_clkgen_en(nport, FALSE);
...
#define FVCO_MAX    500
    ...
    *pll_pixelclockx1024 =pixel_clockx1024;
    ...
    {//TBD, FRL MODE should set dpc corresponding Deep color value. Need more test on non FRL mode.
        hdmi_mask(COMMON_RX_420_HDMIRX_YUV420_CTRL_1_reg, ~COMMON_RX_420_HDMIRX_YUV420_CTRL_1_yuv420_clk_valid_en_mask, COMMON_RX_420_HDMIRX_YUV420_CTRL_1_yuv420_clk_valid_en_mask);
        if(cd!=0)
            lib_hdmi_set_dpc_enable(nport, 1);  // enable deep color support
    }
    else
    {
        //hdmi_mask(HDMI_YUV420_CR1_reg, ~HDMI_YUV420_CR1_yuv420_clk_valid_en_mask, 0);
        hdmi_mask(COMMON_RX_420_HDMIRX_YUV420_CTRL_1_reg, ~COMMON_RX_420_HDMIRX_YUV420_CTRL_1_yuv420_clk_valid_en_mask, 0);
...
// Packet Absence
void lib_hdmi_pa_irq_en(unsigned char nport, unsigned char enable)
{
    if (hdmi_rx[nport].hdmi_2p1_en)
    {
        hdmi_mask(HD21_HDMI_PA_IRQ_EN_reg , ~(HD21_HDMI_PA_IRQ_EN_hfvs_absence_irq_en_mask), HD21_HDMI_PA_IRQ_EN_hfvs_absence_irq_en(enable));
    }
    else
    {
        hdmi_mask(HDMI_HDMI_PA_IRQ_EN_reg , ~(HDMI_HDMI_PA_IRQ_EN_hfvs_absence_irq_en_mask), HDMI_HDMI_PA_IRQ_EN_hfvs_absence_irq_en(enable));
    }
}


unsigned char lib_hdmi_get_pa_irq_status(unsigned char nport)
{
    if (hdmi_rx[nport].hdmi_2p1_en)
    {
...
void lib_hdmi_clr_pa_irq_status(unsigned char nport)
{
    if (hdmi_rx[nport].hdmi_2p1_en)
    {
        hdmi_out(HD21_HDMI_PA_IRQ_STA_reg, HD21_HDMI_PA_IRQ_STA_hfvs_absence_irq_sta_mask);
    }
    else
    {
        hdmi_out(HDMI_HDMI_PA_IRQ_STA_reg, HDMI_HDMI_PA_IRQ_STA_hfvs_absence_irq_sta_mask);
    }
}

unsigned char lib_hdmi_get_pa_frame_count(unsigned char nport)
{
    if (hdmi_rx[nport].hdmi_2p1_en)
    {
        return HD21_HDMI_PA_FRAME_CNT_get_hfvs_absence_frame_cnt(hdmi_in(HD21_HDMI_PA_FRAME_CNT_reg));
    }
    else
    {
        return HDMI_HDMI_PA_FRAME_CNT_get_hfvs_absence_frame_cnt(hdmi_in(HDMI_HDMI_PA_FRAME_CNT_reg));
    }
}

void lib_hdmi_set_pa_frame_count(unsigned char nport, unsigned char frame_cnt)
{
    if (hdmi_rx[nport].hdmi_2p1_en)
    {
        hdmi_mask(HD21_HDMI_PA_FRAME_CNT_reg, ~(HD21_HDMI_PA_FRAME_CNT_hfvs_absence_frame_cnt_mask), HD21_HDMI_PA_FRAME_CNT_hfvs_absence_frame_cnt(frame_cnt));
    }
    else
    {
        hdmi_mask(HDMI_HDMI_PA_FRAME_CNT_reg, ~(HDMI_HDMI_PA_FRAME_CNT_hfvs_absence_frame_cnt_mask), HDMI_HDMI_PA_FRAME_CNT_hfvs_absence_frame_cnt(frame_cnt));
    }
}
