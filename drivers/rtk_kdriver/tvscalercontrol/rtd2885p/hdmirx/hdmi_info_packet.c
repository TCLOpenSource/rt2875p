...
#ifndef UT_flag
static DEFINE_SEMAPHORE(sem_dolby_vsif);
#else
struct semaphore sem_dolby_vsif;
#endif // UT_flag

void  _init_dolby_vsif_sem(void)      { } /* using static initialize */
...
HDMI_QMS_STATUS_T m_qms_status[4];
/*---------------------------------------------------
...
// FVS
//-------------------------------------------------------------------------
 static unsigned char fvs_present[4] = {0, 0, 0, 0};
...
INFOFRAME_READ_RESULT newbase_hdmi_update_vsi_info(unsigned char port, unsigned char *pkt_buf, INFO_PACKET_CONFIG *config)
{
    ...
    else if ((result = lib_hdmi_read_fvs_info_packet(port, pkt_buf))==INFOFRAME_READ_SUCCESS)
    {
        ...
        if (fvs_present[port])
        {
            if (lib_hdmi_read_fvs_pa_status(port))
            {
                INFOFRAME_FLOW_PRINTF("FVS absence triggered (no FVS packet)!!\n");
                fvs_present[port] = 0;
                lib_hdmi_clear_fvs_pa_status(port);
                lib_hdmi_clear_fvs_received_status(port);
            }
        }
        else
        {
            ...
            {
                INFOFRAME_FLOW_PRINTF("FVS present here!!\n");
                fvs_present[port] = 1;
                lib_hdmi_clear_fvs_pa_status(port);
            }
        }
...
#ifndef UT_flag
        if (fvs_present[port] && lib_hdmi_read_fvs_pa_status(port))
        {
            INFOFRAME_FLOW_PRINTF("FVS absence triggered (no VSI packet)!!\n");
            fvs_present[port] = 0;
            lib_hdmi_clear_fvs_pa_status(port);
            lib_hdmi_clear_fvs_received_status(port);
        }
...
void newbase_hdmi_update_infoframe(unsigned char port)
{
    ...
    if (is_avi_here != hdmi_rx[port].avi_t.type_code) {
        HDMI_WARN("GET AVI INFO=%x, pkt_cnt=%d, VIC=%d, pixel_repeat=%d, scaling=%d, Q=%d, YQ=%d, Y= %d\n",
            hdmi_rx[port].avi_t.type_code, hdmi_rx[port].pkt_cnt, hdmi_rx[port].avi_t.VIC, hdmi_rx[port].avi_t.PR, hdmi_rx[port].avi_t.SC, hdmi_rx[port].avi_t.Q, hdmi_rx[port].avi_t.YQ, hdmi_rx[port].avi_t.Y);
    }
...
/*------------------------------------------------------------------
 * Func : newbase_hdmi_get_dolby_vsem_infoframe
 *
 * Desc : get Dolby Vision VSEM from a given hdmi port
 *
 * Para : [IN] port  : HDMI port
 *        [IN] p_vsem : VSEM output
 *
 * Retn : 0 : no Dolby VSEM received
 *        1 : Dolby VSEM received
 *------------------------------------------------------------------*/
unsigned char newbase_hdmi_get_dolby_vsem_infoframe(unsigned char port, unsigned char *p_vsem)
{
    if (FALSE == newbase_hdmi_check_infoframe_config(port, INFO_TYPE_EMP))
        return 0;

    if (hdmi_rx[port].no_dolby_vsem_emp_cnt > 0)
        return 0;

    if (lib_hdmi_get_fw_debug_bit(DEBUG_21_FORCE_NO_INFOPKT))
        return 0;

    memcpy(p_vsem, &hdmi_rx[port].dolby_vsem, MAX_EM_HDR_INFO_LEN);
    return 1;
}
...
unsigned char newbase_hdmi_get_fvs_allm(unsigned char port, unsigned char* allm_mode)
{
    ...
    //Packet Absence detect
    if(lib_hdmi_get_pa_irq_status(port)){
        lib_hdmi_clr_pa_irq_status(port);// wclr
        HDMI_EMG("[newbase_hdmi_get_fvs_allm port:%d]hdmi packet abence frame count : %d\n", port, lib_hdmi_get_pa_frame_count(port));
        *allm_mode = 0;
        return FALSE;
    }
...
static int _infoframe_rx_thread(void* arg)
{
    ...
    while (!kthread_freezable_should_stop(NULL) && infoframe_flag)
    {
        ...
        wait_event_interruptible_hrtimeout(hdmi_thd_wait_qu,(!infoframe_flag),ktime_set(0,40*NSEC_PER_MSEC));
    }
...
void hdmi_set_infoframe_thread_stop(unsigned char ucStop)
{
    ...
    ucInfoframe_Thread_Stop = ucStop;
#endif
    HDMI_INFO("[hdmi_set_infoframe_thread_stop] ucStop=%d\n", ucStop);
}
...
{
    ucInfoframe_Thread_Stop = ucStop;
    HDMI_INFO("[hdmi_set_infoframe_thread_stop_nonseamphore] ucStop=%d\n", ucStop);
}
    ...
    else
    {
        if (infoframe_flag)
        {
            ...
            if(infoframe_task)
            {
                ...
                HDMI_EMG("hdmi info thread stopped already\n");
        }
    }
}
#endif //UT_flag
...
void newbase_hdmi_qms_handler(unsigned char port)
{
    ...
    onms_vfreq = lib_measure_calc_vfreq(lib_measure_get_vs_period());
