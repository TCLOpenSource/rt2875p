...
void lib_hdmi_hd21_clk_div_sel(unsigned char nport, unsigned char pixel_mode)
{
    if (!lib_hdmi_is_hdmi_21_available(nport))
        return;
    ...
    case 2: // 2 pixel mode
        ...
        hdmi_mask(COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_reg, ~(COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_pixel_transform_mode_sel_mask),
                COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_pixel_transform_mode_sel(1));
        break;
...
void lib_hdmi_hd21_timing_gen_reset_toggle(unsigned char nport)
{
    ...
    #endif
    hdmi_mask(HD21_TIMING_GEN_CR_reg, ~HD21_TIMING_GEN_CR_tg_en_mask, 0);     // disable timing gen
    hdmi_mask(HDMI_hdmi_v_bypass_reg, ~HDMI_hdmi_v_bypass_reg_hd21_eco_option_mask , HDMI_hdmi_v_bypass_get_reg_hd21_eco_option(1));
...
HDMI21_MEASURE_RESULT lib_hdmi21_hd21_measure(unsigned char nport, unsigned char is_dsc_mode, MEASURE_TIMING_T* update_timing)
{
...
void lib_hdmi_hd21_mac_init(unsigned char nport)
{
    ...
    hdmi_mask(HD21_PHY_FIFO_CR1_reg,
        ~(HD21_PHY_FIFO_CR1_port0_ckclk_inv_mask),
...
{//enable before link training
    ...
    lib_hdmi_hdcp_set_enc_tog_sel(port, 1);// hd21  set enc_tog_sel  1
...
void newbase_hdmi_hd21_mac_disable(unsigned char port)
{
    ...
    lib_hdmi_hdcp_set_enc_tog_sel(port, 0); // hd2.0  set enc_tog_sel  0
#ifdef HDMI_DISABLE_TMDS_INPUT_ON_FRL_MODE
...
void newbase_hdmi_hd21_clk_gen(unsigned char nport, unsigned char dsc_enable, unsigned char pixel_mode)
{
    ...
    lib_hdmi_hd21_clk_div_sel(nport, pixel_mode);
    ...
    if ( dsc_enable)
    {
        hdmi_mask(COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_reg, ~COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel_mask, COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel(2));
        newbase_hdmi_hd21_dsc_enanble(nport, 1);
    }
    else
    {
        hdmi_mask(COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_reg, ~COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel_mask, COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel(0)); 
    }
...
unsigned char newbase_hdmi_hd21_is_timing_change(unsigned char port, unsigned char is_dsc_mode, MEASURE_TIMING_T* last_timing)
{
