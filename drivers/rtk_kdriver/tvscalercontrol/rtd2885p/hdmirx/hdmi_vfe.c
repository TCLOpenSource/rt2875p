...
unsigned char g_ddc_wakeup_enable = 0;
#endif
...
int vfe_hdmi_qs_pcb_info_init(void)
{
	HdmiTable_Init();
	return 0;
}
...
int vfe_hdmi_qs_drv_init(void)
{
	...
	ch = (unsigned char)qs_portnum;
	HdmiTable_Init();

	if (hdmi_vfe_get_hdmi_port(ch, &nport)<0)
	{
		HDMI_EMG("%s, channel number %d error!\n", __func__, ch);
		return HDMI_DRV_ARG_ERR;
	}
	
	// sync qs scdc status
	newbase_hdmi_sync_qs_scdc_status(nport);

	//sync qs phy status
	newbase_set_qs_rxphy_status(nport);


	newbase_hdmi_set_edid_initialized(nport, 1);// do not write edid in qs mode


	newbase_hdmi_qs_init(nport);
	...
	g_hdmi_channel_connect_flag[ch] = 1;

	for(ch = 0; ch < HDMI_RX_PORT_MAX_NUM; ch++)
	{
		if (hdmi_vfe_get_hdmi_port(ch, &tmp_port)<0)
			continue;  // no HDMI port associated
		newbase_hdmi_qs_init_source(tmp_port);
	...
	newbase_hdmi_power_saving_wakeup(nport, PS_WAKEUP_ACTIVE_SOURCE | PS_WAKEUP_IMMEDIATELY);  // wakeup hdmi port
	newbase_hdmi_hdr10_enable(_ENABLE);     //Minchay@20160607-a add hdr10

	if(newbase_hdmi_get_thermal_mode())
	{
		newbase_hdmi_set_hpd(nport, 1);//set hpd high let phy pw open in thermal mode
	}
...
void vfe_hdmi_sync_qs_status(void)
{
	...
	if (hdmi_vfe_get_hdmi_port((unsigned char)qs_portnum, &nport) < 0){
		HDMI_EMG("%s fail : can not get physical number\n", __func__);
		return;
	}
	
	memset(&avi_info, 0, sizeof(HDMI_AVI_T));
	// need to wait infoframe thread run in qs init
	is_get_avi = newbase_hdmi_get_avi_infoframe(nport, &avi_info);
	if(is_get_avi){
		newbase_hdmi_set_current_vic(nport, avi_info.VIC) ;
	}
	newbase_hdmi_read_edid(nport, edid, 256);
	newbase_hdmi_save_edid(nport, edid, 256);
}
...
#ifdef CONFIG_RTK_KDRV_DDC_WAKEUP
		if (g_ddc_wakeup_enable)
		{
			for (ch = 0; ch < HDMI_PORT_TOTAL_NUM; ch++)
				lib_hdmiddc_wakeup_init(RTK_HDMI_DDC(ch));
			emcu_set_ddc(1);
		}
		else
			emcu_set_ddc(0);
