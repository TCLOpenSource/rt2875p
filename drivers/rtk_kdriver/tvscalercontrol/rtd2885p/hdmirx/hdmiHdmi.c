...
#endif // UT_flag
extern void Hdmi_Av_Out_Disable_Vsc_Pre_Action(void);
/**********************************************************************************************
*
...
int rtk_ddc_hdcp_scdc_change_slave_address(int enable);
...
#endif
OFFMS_SOURCE_SEL g_offms_out_sel[HDMI_PORT_TOTAL_NUM] = {OFFMS_SOURCE_HDMI};
...
static int _hdmi_interrupt_handler(unsigned char port)
{
	...
	if(GET_FLOW_CFG(HDMI_FLOW_CFG_ERR_DET, HDMI_FLOW_CFG4_HDMI_DVI_MODE_IRQ_ENABLE))
	{
		if (port == newbase_hdmi_get_current_display_port() && !hdmi_rx[port].hdmi_2p1_en)
		{
			if (lib_hdmi_get_hdirq_en(port)) {
				unsigned char mode = lib_hdmi_get_hdmi_mode_reg(port);

				if (mode != GET_H_MODE(port)) {
					mute_av = 1; // disable video output
					lib_hdmi_set_hdirq_en(port, 0);
					HDMI_WARN("[HDMI_ISR][p%d] video off due to hdmi DVI/HDMI mode chg (%d -> %d),  is_fwmode:%d\n", port, GET_H_MODE(port), mode, lib_hdmi_is_modedet_fwmode(port) );
				}
			}
		}
	}
	...
	if(GET_FLOW_CFG(HDMI_FLOW_CFG_ERR_DET, HDMI_FLOW_CFG4_BCH_ERR_IRQ_ENABLE) ||
	    (newbase_hdmi_get_factory_or_power_only_mode_enable() && factory_or_power_only_mode_bch_irq_en))
	{
...
void newbase_hdmi_init(void)
{
	...
	for (j=0; j<HDMI_PORT_TOTAL_NUM; j++)
	{
		...
		if (newbase_hdmi_ddc_channel_status(j,&ddc_ch) != 0 )
			continue;
#ifndef BUILD_QUICK_SHOW
		lib_hdmiddc_crt_on(RTK_HDMI_DDC(ddc_ch));
	...
	rtk_ddc_hdcp_scdc_change_slave_address(0);
	newbase_hdmi_port_var_init_all();
	newbase_hdmi_flow_cfg_info_init();
...
void newbase_hdmi_qs_init_source(unsigned char port)
{
	...
	if ((hdmi_vfe_get_hdmi_port((unsigned char)get_QS_portnum(), &tmp_port)==0) \
		&& (port == tmp_port))
	{
...
{
    hdmi_rx[port].timing_t.avi_vic = vic;
}
...
unsigned char newbase_hdmi_detect_mode(unsigned char port)
{
	...
	switch (GET_H_VIDEO_FSM(port)) {
		...
		case MAIN_FSM_HDMI_DISPLAY_ON:
			HDMI_WARN("FSM_HDMI_DISPLAY_ON[%d][frl_mode:%d]\n", port, p_phy_st->frl_mode);
			...
			if (newbase_hdmi_display_on(port, p_phy_st->frl_mode)) {
				...
				if (GET_FLOW_CFG(HDMI_FLOW_CFG_ERR_DET, HDMI_FLOW_CFG4_HDMI_DVI_MODE_IRQ_ENABLE))
				{
					...
					if ((p_phy_st->frl_mode == MODE_TMDS) && (lib_hdmi_is_modedet_fwmode(port)== 0))// only work for HW auto mode)
					{
		...
		case MAIN_FSM_HDMI_VIDEO_READY:
			...
			if (newbase_hdmi_video_ready(port, p_phy_st->frl_mode)) {
				...
				if(!hdmi_good_timing_ready)
				{
					if (newbase_hdmi_good_timing_check(port, p_phy_st->frl_mode))
					{
						hdmi_good_timing_ready = TRUE;
						SET_REPLY_ZERO_TIMING_FLAG(VSC_INPUTSRC_HDMI, SOURCE_GOOD_TIMING_READY);
				...
				if(is_QS_hdmi_enable() && qs_init == 0){
					newbase_hdmi_measure(port, p_phy_st->frl_mode);
					qs_init = 1;
				}
#endif
				return _MODE_SUCCESS;
...
unsigned char newbase_hdmi_is_need_check_vic_table (HDMI_AVI_T* avi_info,  MEASURE_TIMING_T *tm)
{
	...
	avi_vic = avi_info->VIC;
	for(i = 0; i<g_vic_check_list_num; i++)
	{
		if(avi_vic == g_vic_check_list[i])
		{
			is_need_check |= TRUE;
...
unsigned char newbase_hdmi_vic_correction(unsigned char port, MEASURE_TIMING_T *tm, unsigned char force_assign_tm)
{
	...
	if(avi_state == 0)
	{
		...
		hdmi_rx[port].timing_t.avi_vic = 0;
		is_correction = FALSE;
	}
	else
	{
		...
		if (vic_timing_info_ptr)
		{
			...
			{
				if(newbase_hdmi_is_need_check_vic_table(&avi_info, tm) ==TRUE || (force_assign_tm == TRUE))
				{//YUV420
...
#define HDMI_CLK_GEN_PIXEL_MODE_THD    6000000  // Pixel clock , 0.1KHZ
#define HDMI_TG_OUTPUT_BW_THD    12000000  // Pixel clock , 0.1KHZ
#define HDMI_4K240_PIXEL_CLK_THD    22340000  // Pixel clock , 0.1KHZ
...
#ifndef UT_flag
    hdmi_rx[nport].timing_t.is_v_drop = 0;
    //RL6856-1671  4k2k240 need to vertical drop
    if(hdmi_rx[nport].timing_t.v_act_len == 2160 && hdmi_rx[nport].timing_t.v_freq >= 239000){
        hdmi_mask(COMMON_RX_420_LINE_DROP_CTRL_0_reg, ~(COMMON_RX_420_LINE_DROP_CTRL_0_line_drop_valid_delay_mask),
                    COMMON_RX_420_LINE_DROP_CTRL_0_line_drop_valid_delay(hdmi_rx[nport].timing_t.h_total/4/2));
        hdmi_mask(COMMON_RX_420_LINE_DROP_CTRL_0_reg, ~(COMMON_RX_420_LINE_DROP_CTRL_0_line_drop_en_mask), COMMON_RX_420_LINE_DROP_CTRL_0_line_drop_en_mask);
        HDMI_EMG("[Port %d]set line drop enable, line drop valid delay %d\n", nport, COMMON_RX_420_LINE_DROP_CTRL_0_get_line_drop_valid_delay(hdmi_in(COMMON_RX_420_LINE_DROP_CTRL_0_reg)));
        hdmi_rx[nport].timing_t.is_v_drop = 1;
    }else{
        hdmi_mask(COMMON_RX_420_LINE_DROP_CTRL_0_reg, ~(COMMON_RX_420_LINE_DROP_CTRL_0_line_drop_en_mask), 0);
    }
#endif // UT_flag
}
...
unsigned char newbase_hdmi_measure(unsigned char port, unsigned char frl_mode)
{
	...
	unsigned char use_vic_measure = 0;
	...
	g_offms_out_sel[port] = OFFMS_SOURCE_HDMI;
	...
	if (frl_mode != MODE_TMDS)
	{
		g_offms_out_sel[port] = OFFMS_SOURCE_HDMI21;
		if ( GET_H_RUN_DSC(port) == TRUE)
		{
			unsigned char dscd_output_vaild = FALSE;

			dscd_output_vaild = newbase_hdmi_check_dscd_output_valid();
			HDMI_WARN("[newbase_hdmi_measure port:%d] DSCD dscd_output_vaild = %d\n", port, dscd_output_vaild);
			g_offms_out_sel[port] = OFFMS_SOURCE_DSCD;
			if(dscd_output_vaild == 0)
			{
				HDMI_EMG("[newbase_hdmi_measure port:%d] DSCD output  Fail\n", port);
				return FALSE;
			}
		}
	...
	is_get_h_sync = lib_hdmi_get_mac_h_sync(port, 0); // lib_hdmims_offms_is_get_hsync(); maybe has some problem, remark it.
	is_hv_sync_mode = (frl_mode == MODE_TMDS) ? 1 : is_get_h_sync;
	if(GET_H_RUN_VRR(port) != 0)
	{
		//measure_result = newbase_hdmi_vic_measure(port, &g_video_timing[port]);
		//if(measure_result == MEASURE_FAIL_CORRECTION_BY_VIC)
		if((newbase_hdmi_vic_correction(port,  &hdmi_rx[port].timing_t, 1)) == FALSE)
		{
			measure_result = newbase_hdmi_dp_measure_video_ex(g_offms_out_sel[port], port, &hdmi_rx[port].timing_t, frl_mode,is_hv_sync_mode);
			if(MEASURE_SUCCESS_NORMAL_BY_HV == measure_result && (frl_mode == MODE_TMDS))
			{
				//measure_result =  newbase_hdmi_vrr_no_vic_measure(&g_video_timing[port]);
			}
		}else{
			measure_result = TRUE;
		}
	...
	use_vic_measure = GET_FLOW_CFG(HDMI_FLOW_CFG_GENERAL, HDMI_FLOW_CFG0_FORCE_VIC_MEASURE);
	HDMI_INFO("[FSM_HDMI_MEASURE Port:%d][OFMS] measure_result=%d, timing_valid_check=%d,use_vic_measure=%d\n", port, measure_result, timing_valid_check,use_vic_measure);
	if (use_vic_measure == FALSE && (measure_result == FALSE|| timing_valid_check == FALSE))
	...
	if(newbase_hdmi_vic_correction(port,  &hdmi_rx[port].timing_t, use_vic_measure) == TRUE)
	{
	...
	if (frl_mode != MODE_TMDS)
	{
		...
		lib_hdmi_hd21_fec_measure_restart(port, GET_H_RUN_DSC(port));
	...
	{//single pixel mode for lower pixel clock
		...
		hdmi_rx[port].timing_t.pixel_mode = ONE_PIXEL_MODE;
	}
	else
	{//Multi pixel mode for lower pixel clock
		lib_hdmi_set_hd20_21_ctrl_pixel_mode(port, DEFAULT_HDMI_MULTI_PIXEL_MODE);// 2/pixel
		newbase_hdmi_hd21_clk_gen(port, GET_H_RUN_DSC(port), DEFAULT_HDMI_MULTI_PIXEL_MODE); // 2/4 pixel
		hdmi_rx[port].timing_t.pixel_mode = TWO_PIXEL_MODE;
	}
...
unsigned char newbase_hdmi_good_timing_check(unsigned char port, unsigned char frl_mode)
{
	...
	unsigned char is_get_h_sync = lib_hdmi_get_mac_h_sync(port, 0); // lib_hdmims_offms_is_get_hsync(); maybe has some problem, remark it.
	unsigned char is_hv_sync_mode = (frl_mode == MODE_TMDS) ? 1 : is_get_h_sync;
	...
	if(newbase_hdmi_is_dispsrc(SLR_MAIN_DISPLAY) && (GET_FLOW_CFG(HDMI_FLOW_CFG_GENERAL, HDMI_FLOW_CFG0_VIDEO_FSM_MONITOR_ENABLE) == TRUE))
	{
		...
		else if((measure_result = newbase_hdmi_dp_measure_video_ex(g_offms_out_sel[port], port, &tm,  frl_mode,is_hv_sync_mode)) ==FALSE)
		{
...
unsigned char newbase_hdmi_check_mode(void)
{
	...
	if((is_get_avi != 0)  && ( hdmi_rx[port].timing_t.avi_vic != avi_info.VIC))
	{
		HDMI_WARN("[newbase_hdmi_check_mode port: %d] VIC change!!, from %d -> %d\n", port, hdmi_rx[port].timing_t.avi_vic, avi_info.VIC);
		return FALSE;
	}

	//Check offline measure continuous mode
	if(g_hdmi_debug_offms_continuous)
	{
	        MEASURE_TIMING_T tm;

	        memset(&tm, 0, sizeof(MEASURE_TIMING_T));

	        lib_offline_measure_ex(OFFMS_SOURCE_HDMI, &tm,  newbase_rxphy_get_frl_mode(port), OFFMS_ONE_SHOT);
	}
...
#define HDCP_SLAVE_ADDRESS 0x3A
    int i = 0;
    unsigned char ddc_ch = 0 ;

    if(enable == 1) {
        HDMI_WARN("Change right ddc/hdcp/scdc slave address\n");
        for (i = 0; i < HDMI_PORT_TOTAL_NUM; i++) {
            if (newbase_hdmi_phy_port_status(i)==HDMI_PHY_PORT_NOTUSED)
                continue;

            lib_hdmi_scdc_write(i, 0xDE, SCDC_SLAVE_ADDRESS);
            lib_hdmi_hdcp_set_i2c_addr(i, HDCP_SLAVE_ADDRESS);

            if (newbase_hdmi_ddc_channel_status(i,&ddc_ch) != 0)
                continue;
            lib_hdmiddc_set_edid_address(ddc_ch, 0);
        }
    }
    else {
        HDMI_WARN("Change wrong ddc/hdcp/scdc slave address\n");
        for (i = 0; i < HDMI_PORT_TOTAL_NUM; i++) {
            if (newbase_hdmi_phy_port_status(i)==HDMI_PHY_PORT_NOTUSED)
                continue;
            lib_hdmi_scdc_write(i, 0xDE, 0x40);
            lib_hdmi_hdcp_set_i2c_addr(i, 0x41);

            if (newbase_hdmi_ddc_channel_status(i,&ddc_ch) != 0)
                continue;
            lib_hdmiddc_set_edid_address(ddc_ch, 0x7);
        }
    }
    return 0;
}
...
void newbase_hdmi_set_hpd(unsigned char nport, unsigned char high)
{
    ...
    if (high)
    {
        ...
        rtk_ddc_hdcp_scdc_change_slave_address(1);
#ifndef BUILD_QUICK_SHOW
...
void _hdmi_hpd_delay_work_func(struct work_struct *work)
{
    ...
    if (mwork== NULL) {
        ...
        newbase_hdmi_set_hpd(port, 1);
        lib_hdmi_z0_set(port, LN_ALL, 1, CLK_Wake_Up);
    }
...
void newbase_hdmi_force_stop_scdc_extend(unsigned char port)
{
	...
	hdmi_mask(HD21_cercr_reg, ~(HD21_cercr_reset_err_det_mask), HD21_cercr_reset_err_det_mask);
	hdmi_mask(HD21_cercr_reg, ~(HD21_cercr_reset_err_det_mask), 0x0);
		...
		if(newbase_hdmi_hd21_is_data_align_lock(port) == TRUE)
		{
			if((lt_fsm_status[port].tx_flt_no_train == TRUE)&&(GET_FLOW_CFG(HDMI_FLOW_CFG_HDMI21_LT1 , HDMI_FLOW_CFG2_CTS_EXTEND_MODE) == TRUE))
			{
				...
				if(m_scdc_ced_extend_state[port] == SCDC_EXTEND_FSM_READY)
				{
					...
					// before 850 = 6~9, otherwise 0
					if (m_scdc_rst_handle_once == TRUE)
						scdc_extend_random = 0;
					else
...
void newbase_hdmi_error_handler(unsigned char port)
{
            ...
            *--------------------------------------------------------------*/
            ...
            else
            {
                ...
                if (ch_status || hdmi_rx[port].rgb_de_align==0)
                {
                    ...
                    {
                        ERP_WARN("HDMI[p%d] TMDS Channel Buffer Error, RGB DE dealigned (ud/ov/flush) R:%d/%d/%d, G:%d/%d/%d, B:%d/%d/%d (cnt=%d->0, error_cnt=%d, raw_tmds_phy_md_clk=%d)\n",
                            port,
                ...
                else
                {
                    ...
                    {
                        ERP_WARN("HDMI[p%d] TMDS Channel Buffer Error, RGB DE dealigned without error (de_align=%d, cnt=%d, error_cnt=%d, raw_tmds_phy_md_clk=%d)\n",
                            port, hdmi_rx[port].rgb_de_align, hdmi_rx[port].rgb_de_align_cnt, hdmi_rx[port].ch_fifo_error, hdmi_rx[port].raw_tmds_phy_md_clk);
                    }
                }
            }
    ...
    else if (p_phy_st->frl_mode >= MODE_FRL_3G_3_LANE )
...
unsigned char newbase_hdmi_compatibility_run_action(unsigned char port)
{
	...
	if(lib_hdmi_get_specific_device(port) != HDMI_SPECIFIC_DEVICE_NONE)
	{
		...
		{//STR HPD toggle
			...
			newbase_hdmi_set_comp_hpd_low_ms(port, g_device_compatibility_config[table_index].action_str_hpd_low_ms);
...
//			return _FALSE;
	...
	memset(&avi_info, 0, sizeof(HDMI_AVI_T)); //init avi_info
	avi_state = newbase_hdmi_get_avi_infoframe(port, &avi_info);
	if(hdmi_rx[port].timing_t.avi_vic != avi_info.VIC)
	{
	...
	HDMI_INFO("[FETM] DRM state:%d, EOTF:%d; AVI state:%d, VIC=%d, scan_info:%d, scaling:%d, ColorRange(RGB/YCC)=(%d, %d),ColorMetry:%s \n",
		drm_state,
		info.eEOTFtype,
		avi_state,
		avi_info.VIC,
...
unsigned char lib_hdmi_toggle_hpd_z0_check(unsigned char nport, unsigned char ucInit)
{
	...
	if(current_time >= start_time)
	{
		if(current_time - start_time >= HDMI_START_TOGGLE_HPD_Z0_MS)
		{
			...
			lib_hdmi_z0_set(nport, LN_ALL, 0, CLK_Wake_Up);
			msleep(HDMI_TOGGLE_HPD_Z0_MS);
			lib_hdmi_z0_set(nport, LN_ALL, 1, CLK_Wake_Up);
