...
void set_vr_dc2h_capture_sdnr_in_clock(UINT8 enable)
{
	...
	if(in_position == KADP_VR_SCALER_OUTPUT)
	{
		if(Get_DISPLAY_REFRESH_RATE() >= 120)
		{
			*dc2h_in_sel = _TWO_SECOND_UZU;
			...
			{
				if(vr_is_4K2K_capture() == TRUE)
				{
					if((get_VR_Pixel_Format() == VR_CAP_NV12)||(get_VR_Pixel_Format() == VR_CAP_NV21))  //4k2k120 NV12
						dc2h_rgb2yuv_ctrl_reg.dc2h_drop_en = 2;
					else
						rtd_pr_vr_emerg("[error] only support 4K2K120 nv12:%d\n",__LINE__);
				}
				else
				{
	...
	else if(in_position == KADP_VR_DISPLAY_OUTPUT)
	{
		...
		*dc2h_in_sel = _Memc_Mux_Input;
		...
		if(Get_DISPLAY_REFRESH_RATE() >= 120)
		{
			...
			{
				dc2h_rgb2yuv_ctrl_reg.regValue = rtd_inl(DC2H_RGB2YUV_DC2H_RGB2YUV_CTRL_reg); //9c00
				if(vr_is_4K2K_capture() == TRUE) //4k2k120 NV12
				{
	...
	else if(in_position == KADP_VR_OSDVIDEO_OUTPUT)
	{
		...
		*dc2h_in_sel = _OSD_MIXER;
		...
		{			
			...
			if(vr_is_4K2K_capture() == TRUE)
			{
				if(get_VR_Pixel_Format() == VR_CAP_NV12)  //4k2k120 NV12
					dc2h_rgb2yuv_ctrl_reg.dc2h_drop_en = 2;
				else
					rtd_pr_vr_err("[error] only support 4K2K120 nv12:%d\n",__LINE__);
			}
			else
			{
				dc2h_rgb2yuv_ctrl_reg.dc2h_drop_en = 1; /* drop 1 pixel */
