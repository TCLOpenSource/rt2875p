...
//#else
...
#ifndef BUILD_QUICK_SHOW //Get_MEMC_Enable_Dynamic always return 1    
    if(Get_MEMC_Enable_Dynamic() == 0){
            rtd_pr_vbe_debug("\n #### [%s]Not Vx1 panel. MEMC bypass. \n", __FUNCTION__);
            return 0;
    }
	...
	{
		printk("camera setting dclk_fract_a/b=%d/%d @%s,%d\n",sys_dispclksel_reg.dclk_fract_a,sys_dispclksel_reg.dclk_fract_b,__FUNCTION__,__LINE__);///////////////////////
	    sys_dispclksel_reg.dclk_fract_a = 24;
	    sys_dispclksel_reg.dclk_fract_b = 31;
	}*/
    //sys_dispclksel_reg.dclk_fract_b_eco = (dclk_fract_b&_BIT5)?1:0; //k3l error will mark 0603
    IoReg_Write32(SYS_REG_SYS_DISPCLKSEL_reg, sys_dispclksel_reg.regValue);
...
//#ifndef CONFIG_MEMC_BYPASS
	...
	if(get_panel_pixel_mode() > PANEL_1_PIXEL){
		...
		if((Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY) && ((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) || force_enable_two_step_uzu()))
...
//#ifndef CONFIG_MEMC_BYPASS
	if(get_MEMC_bypass_status_refer_platform_model() == FALSE){
	    ...
	    if(get_panel_pixel_mode() > PANEL_1_PIXEL){
	        ...
	        else if((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) || force_enable_two_step_uzu())
	            sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel = 0;//two pixel mode
	        else
	            sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel = 1;
	        sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f1p_gate_sel = 0;
