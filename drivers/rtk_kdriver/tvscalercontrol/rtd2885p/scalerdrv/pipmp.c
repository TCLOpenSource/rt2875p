 ...
 * @date 	$Date: 2006-06-09 16:01:50 +0800 (?��?�? 09 ?��? 2006) $
...
static struct semaphore sub_window_ctrl_semaphore;


void initial_sub_window_ctrl_semaphore(void)
{	
	sema_init(&sub_window_ctrl_semaphore, 1);
}
...
//extern void set_vsc_SetCircle_OutputRegion(VIDEO_RECT_T outregion, unsigned short W, unsigned short H, UINT32 num);
...
void Pipmp_SetPipCtrl(void)
{
	...
	//if (subWin_LGowner)
		//set_vsc_SetCircle_OutputRegion(outregion, true_wide, true_high, sub_shape_type);
	//else
		//set_vsc_SetCircle_OutputRegion(outregion, true_wide, true_high, sub_window_type);
	...
	unsigned short DispVEnd, unsigned char Border)
{
	...
	down(&sub_window_ctrl_semaphore);
...
void PipmpSetSubMuteWindow(unsigned short DispHSta, unsigned short DispHAct, unsigned short DispVSta, unsigned short DispVAct)
{//this api is for sub mute off to set window. ANDROIDTV-2297 issue
...
//	ppoverlay_sub_active_v_start_end_RBUS sub_active_v_start_end_Reg;
	down(&sub_window_ctrl_semaphore);
	sub_den_h_start_width_Reg.regValue = IoReg_Read32(PPOVERLAY_SUB_DEN_H_Start_Width_reg);
	if((sub_den_h_start_width_Reg.sh_den_sta != DispHSta) || (sub_den_h_start_width_Reg.sh_den_width != DispHAct))
	{
		sub_den_h_start_width_Reg.sh_den_sta = DispHSta;
		sub_den_h_start_width_Reg.sh_den_width = DispHAct;
		IoReg_Write32(PPOVERLAY_SUB_DEN_H_Start_Width_reg, sub_den_h_start_width_Reg.regValue);
	}

	sub_den_v_start_length_Reg.regValue = IoReg_Read32(PPOVERLAY_SUB_DEN_V_Start_Length_reg);
	if((sub_den_v_start_length_Reg.sv_den_sta != DispVSta) || (sub_den_v_start_length_Reg.sv_den_length != DispVAct))
	{
		sub_den_v_start_length_Reg.sv_den_sta = DispVSta;
		sub_den_v_start_length_Reg.sv_den_length = DispVAct;
		IoReg_Write32(PPOVERLAY_SUB_DEN_V_Start_Length_reg, sub_den_v_start_length_Reg.regValue);
	}
	

	sub_background_h_start_end_Reg.regValue = IoReg_Read32(PPOVERLAY_SUB_Background_H_Start_End_reg);
	if((sub_background_h_start_end_Reg.sh_bg_sta != 0) || (sub_background_h_start_end_Reg.sh_bg_end != DispHAct))
	{
		sub_background_h_start_end_Reg.sh_bg_sta = 0;
		sub_background_h_start_end_Reg.sh_bg_end = DispHAct;
		IoReg_Write32(PPOVERLAY_SUB_Background_H_Start_End_reg, sub_background_h_start_end_Reg.regValue);
	}
	
	sub_background_v_start_end_Reg.regValue = IoReg_Read32(PPOVERLAY_SUB_Background_V_Start_End_reg);
	if((sub_background_v_start_end_Reg.sv_bg_sta != 0) || (sub_background_v_start_end_Reg.sv_bg_end != DispVAct))
	{
		sub_background_v_start_end_Reg.sv_bg_sta = 0;
		sub_background_v_start_end_Reg.sv_bg_end = DispVAct;
		IoReg_Write32(PPOVERLAY_SUB_Background_V_Start_End_reg, sub_background_v_start_end_Reg.regValue);
	}
	up(&sub_window_ctrl_semaphore);

/* //force bg no need to set active
	sub_active_h_start_end_Reg.regValue = IoReg_Read32(PPOVERLAY_SUB_Active_H_Start_End_reg);
	if((sub_active_h_start_end_Reg.sh_act_sta != DispHSta) || (sub_active_h_start_end_Reg.sh_act_end != (DispHAct - 1)))
	{
		sub_active_h_start_end_Reg.sh_act_sta = DispHSta;
		sub_active_h_start_end_Reg.sh_act_end = DispHAct - 1 ;
		IoReg_Write32(PPOVERLAY_SUB_Active_H_Start_End_reg, sub_active_h_start_end_Reg.regValue);
	}

	sub_active_v_start_end_Reg.regValue = IoReg_Read32(PPOVERLAY_SUB_Active_V_Start_End_reg);
	if((sub_active_v_start_end_Reg.sv_act_sta != DispVSta) || (sub_active_v_start_end_Reg.sv_act_end != (DispVAct - 1)))
	{
		sub_active_v_start_end_Reg.sv_act_sta = DispVSta;
		sub_active_v_start_end_Reg.sv_act_end = DispVAct - 1;
		IoReg_Write32(PPOVERLAY_SUB_Active_V_Start_End_reg, sub_active_v_start_end_Reg.regValue);
	}
*/
