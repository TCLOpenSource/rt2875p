...
// th0 : 512, clk gating 25%
// th1 : 768, clk gating 50%
// th2 : 896, clk gating 75%
// th3 : 1024, clk gating 100%
#define MAIN_FIFO_GATE_TH 512;//DIC suggestion for merlin8
#define MAIN_FIFO_GATE_TH_1 768;
#define MAIN_FIFO_GATE_TH_2 896;
#define MAIN_FIFO_GATE_TH_3 960;
...
bool data_frc_need_borrow_cma_buffer(unsigned char display)
{
	return false;
#if 0
...
{//4k memory size borrow type
	...
	if ((srctype!= VSC_INPUTSRC_VDEC) && (srctype!= VSC_INPUTSRC_JPEG) && (srctype!= VSC_INPUTSRC_CAMERA) && (srctype!= VSC_INPUTSRC_AVD)
	...
	else if(get_force_i3ddma_enable(SLR_MAIN_DISPLAY))    //dolby vision or QMS case
		return BORROW_FROM_CMA;//from CMA
	else if((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) && (get_force_i3ddma_enable(SLR_MAIN_DISPLAY) == FALSE))
		return BORROW_FROM_VBM;//from vbm
	else if((get_hdmi_4k_hfr_mode() == HDMI_3K120) && (get_force_i3ddma_enable(SLR_MAIN_DISPLAY) == FALSE))
		return BORROW_FROM_VBM;//from vbm
		...
		return BORROW_FROM_MEMC;//from memc
			...
			&& (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 990))	//3K 100/120 case borrow from MEMC
		return BORROW_FROM_MEMC;//from memc
	else
		return BORROW_FROM_CMA;//from CMA
}

extern unsigned int max_width,max_len;
...
{//3k 4k use data frc need borrow memory
	...
	if ((!Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC))
			&& ((unsigned int)(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE) * Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE)) >  (max_width * max_len)
...
void drvif_memory_enable_outstanding(void)
{
	mdomain_cap_ddr_outstanding_ctrl_main_cap_RBUS mdomain_cap_ddr_outstanding_ctrl_main_cap_reg;
	mdomain_cap_ddr_outstanding_ctrl_sub_cap_RBUS mdomain_cap_ddr_outstanding_ctrl_sub_cap_reg;

	mdomain_disp_ddr_outstanding_ctrl_main_RBUS mdomain_disp_ddr_outstanding_ctrl_main_reg;
	mdomain_disp_ddr_outstanding_ctrl_sub_RBUS mdomain_disp_ddr_outstanding_ctrl_sub_reg;

	rtd_pr_scaler_memory_info("drvif_memory_init set Max Outstanding\n");

	// Main Cap max outstanding
	mdomain_cap_ddr_outstanding_ctrl_main_cap_reg.regValue = IoReg_Read32(MDOMAIN_CAP_DDR_Outstanding_Ctrl_Main_Cap_reg);
	mdomain_cap_ddr_outstanding_ctrl_main_cap_reg.in1_max_outstanding = _CAP_MAX_OUTSTANDING;
	mdomain_cap_ddr_outstanding_ctrl_main_cap_reg.in1_force_all_rst = 1;
	IoReg_Write32(MDOMAIN_CAP_DDR_Outstanding_Ctrl_Main_Cap_reg, mdomain_cap_ddr_outstanding_ctrl_main_cap_reg.regValue);

	// Main Disp max outstanding
	mdomain_disp_ddr_outstanding_ctrl_main_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_Outstanding_Ctrl_Main_reg);
	mdomain_disp_ddr_outstanding_ctrl_main_reg.max_outstanding = _DISP_MAX_OUTSTANDING;
    mdomain_disp_ddr_outstanding_ctrl_main_reg.force_all_rst = 1;
	IoReg_Write32(MDOMAIN_DISP_DDR_Outstanding_Ctrl_Main_reg, mdomain_disp_ddr_outstanding_ctrl_main_reg.regValue);

#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
	// Sub Cap max outstanding
	mdomain_cap_ddr_outstanding_ctrl_sub_cap_reg.regValue = IoReg_Read32(MDOMAIN_CAP_DDR_Outstanding_Ctrl_Sub_Cap_reg);
	mdomain_cap_ddr_outstanding_ctrl_sub_cap_reg.in2_max_outstanding = _CAP_MAX_OUTSTANDING;
	mdomain_cap_ddr_outstanding_ctrl_sub_cap_reg.in2_force_all_rst = 1;
	IoReg_Write32(MDOMAIN_CAP_DDR_Outstanding_Ctrl_Sub_Cap_reg, mdomain_cap_ddr_outstanding_ctrl_sub_cap_reg.regValue);

	// Sub Disp max outstanding
	mdomain_disp_ddr_outstanding_ctrl_sub_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_Outstanding_Ctrl_Sub_reg);
	mdomain_disp_ddr_outstanding_ctrl_sub_reg.sub_max_outstanding = _DISP_MAX_OUTSTANDING;
	IoReg_Write32(MDOMAIN_DISP_DDR_Outstanding_Ctrl_Sub_reg, mdomain_disp_ddr_outstanding_ctrl_sub_reg.regValue);
#endif
}
...
unsigned int memory_get_line_size(unsigned int LineSize, unsigned char InputFormat, unsigned char BitNum)
{
	...
	rtd_pr_scaler_memory_debug("memory_get_line_size LineSize=%d\n",LineSize);
	return LineSize;
}
...
unsigned int memory_get_capture_size(unsigned char display, eMemCapAccessType AccessType)
{
	...
	rtd_pr_scaler_memory_debug("10 bit:%d;;422cap:%d\n",(Scaler_DispGetStatus((SCALER_DISP_CHANNEL)display,SLR_DISP_10BIT)),(Scaler_DispGetStatus((SCALER_DISP_CHANNEL)display,SLR_DISP_422CAP)));

	ddrbit =  (((Scaler_DispGetStatus((SCALER_DISP_CHANNEL)display,SLR_DISP_10BIT)) << 1 ))  | ( !Scaler_DispGetStatus((SCALER_DISP_CHANNEL)display,SLR_DISP_422CAP));
...
unsigned int memory_get_capture_line_size_compression(unsigned char compression_bits)
{
	...
	rtd_pr_scaler_memory_debug("compression_bits=%x, capWid=%x\n", compression_bits, capWid);
	...
	rtd_pr_scaler_memory_debug("[crixus]memory_get_capture_line_size_compression LineSize=%d\n",TotalSize);
...
/* FIXME 
 * should find a way didn't hard code the m-domain memory size
 * 2560 * 1440 * 16 * 3 / 8 = 21 MB */
#define MDOMAIN_MAIN_CARVEDOUT_SIZE    (1024*1024*22)
...
unsigned long get_query_start_address(unsigned char idx){
	...
	//do nothing
		...
		size = (unsigned int)carvedout_buf_query(CARVEDOUT_SCALER_MDOMAIN, (void **)&addr) ;
#endif
		addr += MDOMAIN_MAIN_CARVEDOUT_SIZE;
		if(size == 0){
			BUG();
		}
	}else if(idx == QUERY_IDX_DI) {
...
//	static unsigned int dcu2_address = 0;
	...
	if((Idx >= MEMIDX_VDC) && (Idx < MEMIDX_ALL)) {
		...
		if(MEMIDX_DI == Idx){//DI, RTNR allocate memory by overlap memory @Crixus 20151122
			...
			rtd_pr_scaler_memory_emerg("[crixus]DI sub memory address = %x\n", m_allocate_memory);
		...
		if(MEMIDX_DI == Idx){
			...
			IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr1_reg,(MemTag[MEMIDX_MAIN_THIRD].StartAddr & 0xfffffff0));
			IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg,(MemTag[MEMIDX_MAIN_THIRD].StartAddr+space-CONFIG_MDOMAIN_BURST_SIZE) & 0xfffffff0);
		}else if(MEMIDX_MAIN_SEC == Idx ){
			IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr1_reg,(MemTag[MEMIDX_MAIN_SEC].StartAddr & 0xfffffff0));
			IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg,(MemTag[MEMIDX_MAIN_SEC].StartAddr+space-CONFIG_MDOMAIN_BURST_SIZE) & 0xfffffff0);
		}else if(MEMIDX_MAIN == Idx){
			IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr1_reg,(MemTag[MEMIDX_MAIN].StartAddr & 0xfffffff0));
			IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg,(MemTag[MEMIDX_MAIN].StartAddr+space-CONFIG_MDOMAIN_BURST_SIZE) & 0xfffffff0);
		}else if(MEMIDX_SUB_THIRD == Idx){
			IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr1_reg,(MemTag[MEMIDX_SUB_THIRD].StartAddr & 0xfffffff0));
			IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_reg,(MemTag[MEMIDX_SUB_THIRD].StartAddr+space-CONFIG_MDOMAIN_BURST_SIZE) & 0xfffffff0);
		}else if(MEMIDX_SUB_SEC == Idx){
			IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr1_reg,(MemTag[MEMIDX_SUB_SEC].StartAddr & 0xfffffff0));
			IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_reg,(MemTag[MEMIDX_SUB_SEC].StartAddr+space-CONFIG_MDOMAIN_BURST_SIZE) & 0xfffffff0);
		}else if(MEMIDX_SUB == Idx){
			IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr1_reg,(MemTag[MEMIDX_SUB].StartAddr & 0xfffffff0));
			IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_reg,(MemTag[MEMIDX_SUB].StartAddr+space-CONFIG_MDOMAIN_BURST_SIZE) & 0xfffffff0);
		}
#else
...
{
	mdomain_cap_ddr_in1wtlvl_num_RBUS mdomain_cap_ddr_in1wtlvl_num_reg;
	mdomain_cap_ddr_in1wrlen_rem_RBUS mdomain_cap_ddr_in1wrlen_rem_reg;
#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
	mdomain_cap_ddr_in2wtlvl_RBUS mdomain_cap_ddr_in2wtlvl_reg;
	mdomain_cap_ddr_in2wrlen_rem_RBUS  mdomain_cap_ddr_in2wrlen_rem_reg;
	if(channel)
	{//sub
		mdomain_cap_ddr_in2wtlvl_reg.regValue = IoReg_Read32(MDOMAIN_CAP_DDR_In2WTLVL_reg);
		mdomain_cap_ddr_in2wtlvl_reg.in2_wtlvl = (WaterLevel>>1);
		mdomain_cap_ddr_in2wtlvl_reg.in2_write_num = Quotient;
		IoReg_Write32(MDOMAIN_CAP_DDR_In2WTLVL_reg, mdomain_cap_ddr_in2wtlvl_reg.regValue);
		
		mdomain_cap_ddr_in2wrlen_rem_reg.regValue = IoReg_Read32(MDOMAIN_CAP_DDR_In2WrLen_Rem_reg);
		mdomain_cap_ddr_in2wrlen_rem_reg.in2_wrlen = WaterLevel;
		mdomain_cap_ddr_in2wrlen_rem_reg.in2_write_remain = Remainder;
		IoReg_Write32(MDOMAIN_CAP_DDR_In2WrLen_Rem_reg, mdomain_cap_ddr_in2wrlen_rem_reg.regValue);
		
	}
	else
	{//main
		mdomain_cap_ddr_in1wtlvl_num_reg.regValue = IoReg_Read32(MDOMAIN_CAP_DDR_In1WTLVL_Num_reg);
		mdomain_cap_ddr_in1wtlvl_num_reg.in1_wtlvl = (WaterLevel>>1);
		mdomain_cap_ddr_in1wtlvl_num_reg.in1_write_num = Quotient;
		IoReg_Write32(MDOMAIN_CAP_DDR_In1WTLVL_Num_reg, mdomain_cap_ddr_in1wtlvl_num_reg.regValue);

		
		mdomain_cap_ddr_in1wrlen_rem_reg.regValue = IoReg_Read32(MDOMAIN_CAP_DDR_In1WrLen_Rem_reg);
		mdomain_cap_ddr_in1wrlen_rem_reg.in1_wrlen = WaterLevel;
		mdomain_cap_ddr_in1wrlen_rem_reg.in1_write_remain = Remainder;
		IoReg_Write32(MDOMAIN_CAP_DDR_In1WrLen_Rem_reg, mdomain_cap_ddr_in1wrlen_rem_reg.regValue);
	}
#else
	mdomain_cap_ddr_in1wtlvl_num_reg.regValue = IoReg_Read32(MDOMAIN_CAP_DDR_In1WTLVL_Num_reg);
	mdomain_cap_ddr_in1wtlvl_num_reg.in1_wtlvl = (WaterLevel>>1);
...
{
	//Crixus@20140722 - Mac3 no in2
	IoReg_Write32(/*channel ? MDOMAIN_CAP_DDR_In2Addr_reg : */MDOMAIN_CAP_DDR_In1Addr_reg, (Addr1& 0xfffffff0));
	IoReg_Write32(/*channel ? MDOMAIN_CAP_DDR_In2_2ndAddr_reg : */MDOMAIN_CAP_DDR_In1_2ndAddr_reg, (Addr2 & 0xfffffff0));
}
...
static void MemSetDisplayAddr(unsigned char display, unsigned int Addr1, unsigned int Addr2)
{
	...
	if(display == SLR_MAIN_DISPLAY){
		//rtd_pr_scaler_memory_debug("Before %s %d IoReg_Read32(MDOMAIN_DISP_DDR_MainAddr_reg)=%x\n",__FUNCTION__,__LINE__,IoReg_Read32(MDOMAIN_DISP_DDR_MainAddr_reg));
		...
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr1_reg, address& 0xfffffff0);
		address = IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg);
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr2_reg, address& 0xfffffff0);
	}
#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
	else{
		IoReg_Write32(MDOMAIN_DISP_DDR_SubAddr_reg, Addr1& 0xfffffff0);
		IoReg_Write32(MDOMAIN_DISP_DDR_Sub2ndAddr_reg, Addr2& 0xfffffff0);
		  
		//update m_disp boundary the same as m_cap boundary @Crixus 20171227
		address = IoReg_Read32(MDOMAIN_CAP_CapSub_BoundaryAddr1_reg);
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr1_reg, address& 0xfffffff0);
		address = IoReg_Read32(MDOMAIN_CAP_CapSub_BoundaryAddr2_reg);
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_reg, address& 0xfffffff0);
...
unsigned int memory_get_display_size(unsigned char display)
{
...
#if 0
	if(Get_rotate_function(SLR_MAIN_DISPLAY) && (get_rotate_curState(SLR_MAIN_DISPLAY)==MAIN_ROTATE_INIT_VI) && ((Get_Rotate_Mode(SLR_MAIN_DISPLAY)==ROTATE_MODE_90)||(Get_Rotate_Mode(SLR_MAIN_DISPLAY)==ROTATE_MODE_270))){
	...
	//rtd_pr_scaler_memory_debug("[Memory display]:TotalSize=%x\n", TotalSize);
	return TotalSize;
}
...
{
	mdomain_disp_ddr_subblkstep_RBUS mdomain_disp_ddr_subblkstep_reg;
	...
	mdomain_disp_ddr_subpixnumwtl_reg.sub_pixel_num = Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_WID) & 0x3FFF;
	mdomain_disp_ddr_subpixnumwtl_reg.sub_wtlvl = (fifoLen-burstLen);
	...
	if((Scaler_PipGetInfo(SLR_PIP_TYPE) >= SUBTYPE_MP4X4) && (Scaler_PipGetInfo(SLR_PIP_TYPE) <= SUBTYPE_MP1L7B)){ // for sub size can be divisible by 4 in case 1C12B, 4x4, 1L7B
		...
		mdomain_disp_ddr_sublinenum_reg.sub_line_number = (Scaler_DispGetInputInfo(SLR_INPUT_DISP_LEN)<<2) & 0x0FFF;
		IoReg_Write32(MDOMAIN_DISP_DDR_SubLineNum_reg, mdomain_disp_ddr_sublinenum_reg.regValue);
	}
	else
	{
		mdomain_disp_ddr_sublinenum_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_SubLineNum_reg);
		mdomain_disp_ddr_sublinenum_reg.sub_line_number = (_DISP_LEN & 0x0FFF);
		IoReg_Write32(MDOMAIN_DISP_DDR_SubLineNum_reg, mdomain_disp_ddr_sublinenum_reg.regValue);
	}
	...
	mdomain_disp_ddr_subrdnumlenrem_reg.sub_read_len = (burstLen & 0xff);
	mdomain_disp_ddr_subrdnumlenrem_reg.sub_read_remain = 0xff;
	...
	mdomain_disp_ddr_sublinestep_reg.sub_line_step = linestep;//bit 3 need to be 0. rbus rule
	...
	mdomain_disp_ddr_subblkstep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_SubBlkStep_reg);
	mdomain_disp_ddr_subblkstep_reg.sub_block_step = blockstep;//bit 3 need to be 0. rbus rule
	IoReg_Write32(MDOMAIN_DISP_DDR_SubBlkStep_reg, mdomain_disp_ddr_subblkstep_reg.regValue);
...
static void memory_set_sub_for_pip(unsigned char display)
{
	...
	mdomain_disp_ddr_sublinenum_RBUS mdomain_disp_ddr_sublinenum_reg;
	mdomain_disp_ddr_outstanding_ctrl_sub_RBUS ddr_outstanding_ctrl_sub_reg;

	//if hfr mode, main and sub path capture same data, so sub path need set burst/lenth setting as main path
	if((SLR_SUB_DISPLAY == display) && ((is_4k_hfr_mode(SLR_MAIN_DISPLAY)) || get_hdmi_vrr_4k60_mode())){
		fifoLen = _FIFOLENGTH;
		if(!Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP))
			burstLen = 0xf0; //align 96-bytes
		else
			burstLen = _BURSTLENGTH;
	}

	//	enable outstanding burstLen = 80
	if(get_hdmi_vrr_4k60_mode() || (is_4k_hfr_mode(SLR_MAIN_DISPLAY))
         #ifndef BUILD_QUICK_SHOW
        || ((get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_ROTATE_0)&&(get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_DEFAULT))
         #endif
    )
	{
		ddr_outstanding_ctrl_sub_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_Outstanding_Ctrl_Sub_reg);
		ddr_outstanding_ctrl_sub_reg.sub_max_outstanding = 0;
		IoReg_Write32(MDOMAIN_DISP_DDR_Outstanding_Ctrl_Sub_reg, ddr_outstanding_ctrl_sub_reg.regValue);
	}
	else
	{
		burstLen = 0x50;
		ddr_outstanding_ctrl_sub_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_Outstanding_Ctrl_Sub_reg);
		ddr_outstanding_ctrl_sub_reg.sub_max_outstanding = 0x3;
		IoReg_Write32(MDOMAIN_DISP_DDR_Outstanding_Ctrl_Sub_reg, ddr_outstanding_ctrl_sub_reg.regValue);
	}
	...
	if ((Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_HSTA) == 0) && (Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_VSTA) == 0) ) {
		//frank@0323 change waterlevel/2 for AT,because water level nit is 128 bits
		mdomain_disp_ddr_subpixnumwtl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_SubPixNumWTL_reg);
		mdomain_disp_ddr_subpixnumwtl_reg.sub_pixel_num = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_WID) & 0x3FFF;
	...
	rtd_pr_scaler_memory_debug("TotalSize=%x, burstLen=%x, Quotient=%x, Remainder=%x\n", TotalSize, burstLen, Quotient, Remainder);
	...
	{
		rtd_pr_scaler_memory_debug("Sub compress\n");
		if (dvrif_memory_get_compression_mode(SLR_SUB_DISPLAY) == COMPRESSION_LINE_MODE)
		{
			...
			rtd_pr_scaler_memory_debug("ulLineStep=%x\n", ulLineStep);
		}
	}
	// Not compress, check if m-cap line mode?
	else if (MEMCAPTYPE_LINE == dvrif_memory_get_cap_mode(display))
	{
		rtd_pr_scaler_memory_debug("Sub not compres, cap line mode\n");
		//merlin4 memory do 96 bytes align @Crixus 20180321
		ulLineStep = drvif_memory_get_data_align(TotalSize, DMA_SPEEDUP_ALIGN_VALUE/8);
	} else{
		rtd_pr_scaler_memory_debug("Sub not compres, cap frame mode\n");
		ulLineStep = memory_get_capture_size(display, MEMCAPTYPE_LINE);
    }


	rtd_pr_scaler_memory_debug("memory_set_sub_for_pip() Sub cap access mode=%x, display=%x ulLineStep=%x\n", dvrif_memory_get_cap_mode(display), display, ulLineStep);
	if(get_hdmi_vrr_4k60_mode())
	{
		mdomain_disp_ddr_sublinestep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_SubLineStep_reg);
		mdomain_disp_ddr_sublinestep_reg.sub_line_step = ((ulLineStep + ulLineStep%2) * 2);//bit 3 need to be 0. rbus rule
		IoReg_Write32(MDOMAIN_DISP_DDR_SubLineStep_reg, mdomain_disp_ddr_sublinestep_reg.regValue);
	}
	else
	{
		mdomain_disp_ddr_sublinestep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_SubLineStep_reg);
		mdomain_disp_ddr_sublinestep_reg.sub_line_step = (ulLineStep + ulLineStep%2);//bit 3 need to be 0. rbus rule
		IoReg_Write32(MDOMAIN_DISP_DDR_SubLineStep_reg, mdomain_disp_ddr_sublinestep_reg.regValue);
	}
	...
	IoReg_Write32(MDOMAIN_DISP_DDR_SubLineNum_reg, mdomain_disp_ddr_sublinenum_reg.regValue);
	if(get_hdmi_vrr_4k60_mode())
		MemSetDisplayAddr(SLR_SUB_DISPLAY, drvif_memory_get_block_addr(MEMIDX_MAIN) + ((ulLineStep + ulLineStep%2) << 3), drvif_memory_get_block_addr(MEMIDX_MAIN_SEC) + ((ulLineStep + ulLineStep%2) << 3));
	else
	...
	if(get_hdmi_vrr_4k60_mode())
		IoReg_Write32(MDOMAIN_DISP_DDR_Sub3rdAddr_reg, (drvif_memory_get_block_addr(MEMIDX_MAIN_THIRD) + ((ulLineStep + ulLineStep%2) << 3))& 0xfffffff0);
	else
		IoReg_Write32(MDOMAIN_DISP_DDR_Sub3rdAddr_reg, (drvif_memory_get_block_addr(MEMIDX_SUB_THIRD) + MemShiftAddr)& 0xfffffff0);
#endif
...
#ifndef BUILD_QUICK_SHOW    
#ifdef CONFIG_ENABLE_SE_ROTATE_M_DOMAIN_BLOCK_MODE
	if((display == SLR_MAIN_DISPLAY) && Get_rotate_function(SLR_MAIN_DISPLAY) && (get_rotate_mode(SLR_MAIN_DISPLAY)==(DIRECT_VO_FRAME_ORIENTATION)ROTATE_MODE_90) ){//block mode:8 bit
		K = 64;
		L = (HOffset * 8) / K;
		S = (HOffset * 8 - L * K) / 8;
...
void memory_set_main_3D_setting(unsigned int LineStep)
{
		...
		&& ((Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE) == SLR_3D_TOP_AND_BOTTOM)||(Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE) == SLR_3D_FRAME_PACKING)))
	{
		...
		// ddr_mainctrl_reg.main_pr_our_merge_en = 1; // Merlin8 remove
#endif
		// ddr_mainctrl_reg.main_merge_en0 = 1; // Merlin8 remove
		ddr_mainctrl_reg.main_double_en = 1;
		IoReg_Write32(MDOMAIN_DISP_DDR_MainAddr_reg,IoReg_Read32(MDOMAIN_CAP_DDR_In1Addr_reg)& 0xfffffff0);
		IoReg_Write32(MDOMAIN_DISP_DDR_Main2ndAddr_reg,IoReg_Read32(MDOMAIN_CAP_DDR_In1_2ndAddr_reg)& 0xfffffff0);
		IoReg_Write32(MDOMAIN_DISP_DDR_Main3rdAddr_reg,IoReg_Read32(MDOMAIN_CAP_DDR_In1_3rdAddr_reg)& 0xfffffff0);
		rtd_pr_scaler_memory_debug("\n@@PR TBFP[%d] merge enable\n", Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE));
	}
	else
	{
		ddr_mainblkstep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainBlkStep_reg);
		//rtd_pr_scaler_memory_debug("[DBG][TaB] Step Line/Block1=[%x/%x->%x]\n", LineStep, ddr_mainblkstep_reg.main_block_step, TotalSize);
		ddr_mainblkstep_reg.main_block_step = 0;

		// PR output enable
		ddr_mainctrl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainCtrl_reg);
#ifndef CONFIG_DARWIN_SCALER
		// ddr_mainctrl_reg.main_pr_our_merge_en = 0; // Merlin8 remove
#endif
		// ddr_mainctrl_reg.main_merge_en0 = 0; // Merlin8 remove
...
void memory_set_main_SG_3D_setting(unsigned int LineStep, unsigned int Addr1, unsigned int Addr2, unsigned int Addr3)
{
	...
	if(drvif_scaler3d_decide_3d_SG_data_FRC()){
		...
		if(rFrameOffset != (drvif_memory_get_block_size(MEMIDX_MAIN) >> 1))
			rtd_pr_scaler_memory_debug("[SG] BlkStep cap/blk=%x/%x@IDMA\n", rFrameOffset, drvif_memory_get_block_size(MEMIDX_MAIN) >> 1);
		...
		if(Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE) == SLR_3D_2D_CVT_3D){
			...
			rtd_pr_scaler_memory_debug("[DBG][TaB2] Step Line/Block1=[%x/%x->%x(%d)]\n", LineStep, ddr_mainblkstep_reg.main_block_step, TotalSize, Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_MEM_LEN_PRE));
			ddr_mainblkstep_reg.main_block_step = TotalSize;

			rFrameOffset = TotalSize << 3;
			if(rFrameOffset != (drvif_memory_get_block_size(MEMIDX_MAIN) >> 1))
				rtd_pr_scaler_memory_debug("[SG] BlkStep cap/blk=%x/%x@TaB\n", rFrameOffset, drvif_memory_get_block_size(MEMIDX_MAIN) >> 1);
			...
			rtd_pr_scaler_memory_debug("[DBG][SBS] Step Line/Block1=[%x/%x->%x(%d)]\n", LineStep, ddr_mainblkstep_reg.main_block_step, TotalSize, Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_MEM_LEN_PRE));
			ddr_mainblkstep_reg.main_block_step = TotalSize;

			rFrameOffset = TotalSize << 3;
			if(rFrameOffset != (drvif_memory_get_block_size(MEMIDX_MAIN) >> 1))
				rtd_pr_scaler_memory_debug("[SG] BlkStep cap/blk=%x/%x@SBS\n", rFrameOffset, drvif_memory_get_block_size(MEMIDX_MAIN) >> 1);
		}
		ddr_mainblkstep_reg.main_block_step = (ddr_mainblkstep_reg.main_block_step % 2)? ddr_mainblkstep_reg.main_block_step + 1 : ddr_mainblkstep_reg.main_block_step;
		//bit 3 need to be 0. rbus rule
		if(Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE) == SLR_3D_2D_CVT_3D){
...
void memory_set_main_3D_setting_dispchange(unsigned int LineStep)
{
		...
		&& ((Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE) == SLR_3D_TOP_AND_BOTTOM)||(Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE) == SLR_3D_FRAME_PACKING)))
	{
		...
		TotalSize = LineStep * (Scaler_DispGetInputInfoByDisp(display, (SLR_INPUT_INFO)SLR_DISP_3D_MEM_LEN_PRE)>>1);//for overscan modify
...
{
#ifndef BUILD_QUICK_SHOW
	mdomain_disp_ddr_mainblkstep_RBUS mdomain_disp_ddr_mainblkstep_reg;
	unsigned int TotalSize;
	unsigned int rFrameOffset=0;
	//mdomain_disp_ddr_mainctrl_RBUS ddr_mainctrl_reg;

        mdomain_disp_ddr_mainblkstep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainBlkStep_reg);
	...
	if(drvif_scaler3d_decide_3d_SG_data_FRC()){
		...
		if(Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE) == SLR_3D_2D_CVT_3D){
			...
			rtd_pr_scaler_memory_debug("[DBG][TaB2] Step Line/Block1=[%x/%x->%x(%d)]\n", LineStep, mdomain_disp_ddr_mainblkstep_reg.main_block_step, TotalSize, Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_MEM_LEN_PRE));
			mdomain_disp_ddr_mainblkstep_reg.main_block_step = TotalSize;
		...
		mdomain_disp_ddr_mainblkstep_reg.main_block_step = (mdomain_disp_ddr_mainblkstep_reg.main_block_step % 2)? mdomain_disp_ddr_mainblkstep_reg.main_block_step + 1 : mdomain_disp_ddr_mainblkstep_reg.main_block_step;
		//bit 3 need to be 0. rbus rule
		if(Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE) == SLR_3D_2D_CVT_3D){
			IoReg_Write32(MDOMAIN_DISP_DDR_Main_R_1ST_Addr_reg, Addr1 + rFrameOffset);
			IoReg_Write32(MDOMAIN_DISP_DDR_Main_R_2ND_Addr_reg, Addr2 + rFrameOffset);
			IoReg_Write32(MDOMAIN_DISP_DDR_Main_R_3RD_Addr_reg, Addr3 + rFrameOffset);
			IoReg_Write32(MDOMAIN_DISP_DDR_MainBlkStep_reg, mdomain_disp_ddr_mainblkstep_reg.regValue);
		}else{
			// non-IDMA SG 3D mode L/R order need do invert
			IoReg_Write32(MDOMAIN_DISP_DDR_MainAddr_reg,(Addr1 + rFrameOffset)& 0xfffffff0);
			IoReg_Write32(MDOMAIN_DISP_DDR_Main2ndAddr_reg,(Addr2 + rFrameOffset)& 0xfffffff0);
...
static void memory_set_main(unsigned char bDouble, unsigned char display)
{
	mdomain_disp_ddr_mainblkstep_RBUS mdomain_disp_ddr_mainblkstep_reg;
	mdomain_disp_ddr_mainlinestep_RBUS mdomain_disp_ddr_mainlinestep_reg;
	mdomain_disp_ddr_mainlinenum_RBUS mdomain_disp_ddr_mainlinenum_reg;
	mdomain_disp_ddr_mainrdnumlenrem_RBUS mdomain_disp_ddr_mainrdnumlenrem_reg;
	mdomain_disp_ddr_mainnumwtl_RBUS mdomain_disp_ddr_mainnumwtl_reg;
	unsigned int TotalSize;
//	unsigned int  TotalSizeFrame=0;
	...
	mdomain_disp_ddr_outstanding_ctrl_main_RBUS ddr_outstanding_ctrl_main_reg;
	...
	if((display == SLR_MAIN_DISPLAY) && ( !Scaler_DispGetStatus(display, SLR_DISP_422CAP))){
		burstLen=0xf0; //align 96-bytes
	}
#endif

	//	enable outstanding burstLen = 80
	if(get_hdmi_vrr_4k60_mode() || (is_4k_hfr_mode(display))
         #ifndef BUILD_QUICK_SHOW
        || ((get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_ROTATE_0)&&(get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_DEFAULT))
           #endif
         )
	{
		ddr_outstanding_ctrl_main_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_Outstanding_Ctrl_Main_reg);
		ddr_outstanding_ctrl_main_reg.max_outstanding = 0;
		IoReg_Write32(MDOMAIN_DISP_DDR_Outstanding_Ctrl_Main_reg, ddr_outstanding_ctrl_main_reg.regValue);
	}
	else
	{
		burstLen = 0x50;
		ddr_outstanding_ctrl_main_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_Outstanding_Ctrl_Main_reg);
		ddr_outstanding_ctrl_main_reg.max_outstanding = 0x3;
		IoReg_Write32(MDOMAIN_DISP_DDR_Outstanding_Ctrl_Main_reg, ddr_outstanding_ctrl_main_reg.regValue);
	}
	...
	if((get_vsc_run_adaptive_stream(SLR_MAIN_DISPLAY)) ||(m_dispwin.srcy <= 100)) {
		ddr_mainprevstart_reg.main_pre_rd_v_start = drv_Calculate_m_pre_read_value();//7;//MDOM_PREREAD_START;
	} else {
		ddr_mainprevstart_reg.main_pre_rd_v_start = (m_dispwin.srcy-10);
	}
	//ddr_mainprevstart_reg.main_fifo_gate_th = MAIN_FIFO_GATE_TH;//for dic suggestion fs
	IoReg_Write32(MDOMAIN_DISP_DDR_MainPreVStart_reg, ddr_mainprevstart_reg.regValue);
	//DIC recommand, db latch line = pre_read+2
	scaler_disp_dtg_set_db_latch_line(ddr_mainprevstart_reg.main_pre_rd_v_start+2);

	IoReg_Write32(MDOMAIN_DISP_DDR_MainAddrDropBits_reg,0x0);

	mdomain_disp_ddr_mainlinenum_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainLineNum_reg);
	mdomain_disp_ddr_mainlinenum_reg.main_line_number = (Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_LEN) & 0x0FFF);
	IoReg_Write32(MDOMAIN_DISP_DDR_MainLineNum_reg, mdomain_disp_ddr_mainlinenum_reg.regValue);
	
#ifdef CONFIG_3D_ASPECT_RATIO
	rtd_pr_scaler_memory_debug("mem act info %d %d %d %d\n",Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_HSTA),Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_VSTA), Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_WID),Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_LEN));
	...
	if ((Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_HSTA) == 0) && (Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_VSTA) == 0) )
	{
		...
		if((display == SLR_MAIN_DISPLAY) && (dvrif_memory_compression_get_enable(display) == TRUE)){
			...
			mdomain_disp_ddr_mainnumwtl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainNumWTL_reg);
			mdomain_disp_ddr_mainnumwtl_reg.main_pixel_num = comp_width & 0x03FFF;//14 bit
			mdomain_disp_ddr_mainnumwtl_reg.main_wtlvl = ((fifoLen-(burstLen>>1))>>1);
			IoReg_Write32(MDOMAIN_DISP_DDR_MainNumWTL_reg, mdomain_disp_ddr_mainnumwtl_reg.regValue);

			TotalSize = memory_get_capture_line_size_compression(dvrif_memory_get_compression_bits(display));
		}
		else{
			mdomain_disp_ddr_mainnumwtl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainNumWTL_reg);
			mdomain_disp_ddr_mainnumwtl_reg.main_pixel_num = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_WID) & 0x03FFF;//14 bit
			mdomain_disp_ddr_mainnumwtl_reg.main_wtlvl = ((fifoLen-(burstLen>>1))>>1);
			IoReg_Write32(MDOMAIN_DISP_DDR_MainNumWTL_reg, mdomain_disp_ddr_mainnumwtl_reg.regValue);
			TotalSize = memory_get_display_size(display);	// calculate the memory size of capture
#ifndef BUILD_QUICK_SHOW
			...
			if((display == SLR_MAIN_DISPLAY) && Get_rotate_function(SLR_MAIN_DISPLAY)) {
				TotalSize = drvif_memory_get_data_align(Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_WID)*8, 64);
	...
	else
	{
    ...
    #ifdef CONFIG_ENABLE_SE_ROTATE_M_DOMAIN_BLOCK_MODE
		if((display == SLR_MAIN_DISPLAY) && Get_rotate_function(SLR_MAIN_DISPLAY) && (get_rotate_mode(SLR_MAIN_DISPLAY)==(DIRECT_VO_FRAME_ORIENTATION)ROTATE_MODE_90) ) {
			TotalSize = drvif_memory_get_data_align((Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_WID) + droppixel)*8, 64);
			TotalSize = (unsigned int)SHR(TotalSize, 6);	// (pixels of per line * 24 / 64), unit: 64bits
		} else
	#endif
#endif    
		...
		mdomain_disp_ddr_mainnumwtl_reg.main_wtlvl = ((fifoLen-(burstLen>>1))>>1);
		IoReg_Write32(MDOMAIN_DISP_DDR_MainNumWTL_reg, mdomain_disp_ddr_mainnumwtl_reg.regValue);
		rtd_pr_scaler_memory_debug("drop pixel=%d shift addr=%x\n",droppixel,MemShiftAddr);
	...
	if(droppixel != 0){
		...
		rtd_pr_scaler_memory_emerg("@@@@@@@@@@@@drop_line = %d\n",drop_line);
	...
	//rtd_pr_scaler_memory_emerg("TotalSize=%x, burstLen=%x, Quotient=%x, Remainder=%x\n", TotalSize, burstLen, Quotient, Remainder);
	...
	if(((dvrif_memory_compression_get_enable(display)) == TRUE) && (dvrif_memory_get_compression_mode(display) == COMPRESSION_FRAME_MODE)){
		...
		mdomain_disp_ddr_mainrdnumlenrem_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainRdNumLenRem_reg);
		mdomain_disp_ddr_mainrdnumlenrem_reg.main_read_num = 0xffff;
		mdomain_disp_ddr_mainrdnumlenrem_reg.main_read_len = burstLen;
		mdomain_disp_ddr_mainrdnumlenrem_reg.main_read_remain = burstLen;
		IoReg_Write32(MDOMAIN_DISP_DDR_MainRdNumLenRem_reg, mdomain_disp_ddr_mainrdnumlenrem_reg.regValue);		
	}
	else{
		mdomain_disp_ddr_mainrdnumlenrem_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainRdNumLenRem_reg);
		mdomain_disp_ddr_mainrdnumlenrem_reg.main_read_num = Quotient;
		mdomain_disp_ddr_mainrdnumlenrem_reg.main_read_len = burstLen;
		mdomain_disp_ddr_mainrdnumlenrem_reg.main_read_remain = Remainder;
		IoReg_Write32(MDOMAIN_DISP_DDR_MainRdNumLenRem_reg, mdomain_disp_ddr_mainrdnumlenrem_reg.regValue);	
	}
	...
	//rtd_pr_scaler_memory_emerg("file=%s, function=%s, line=%d\n", __FILE__, __FUNCTION__, __LINE__);
	//rtd_pr_scaler_memory_emerg("TotalSize=%x\n", TotalSize);
#ifndef BUILD_QUICK_SHOW
	...
	//rtd_pr_scaler_memory_emerg("Line Step=0x%x\n",LineStep);
	TotalSize = SHL(TotalSize, 3); // unit conversion from 64bits to 8bits
	...
	{
		//rtd_pr_scaler_memory_emerg("file=%s, function=%s, line=%d\n", __FILE__, __FUNCTION__, __LINE__);
		//rtd_pr_scaler_memory_emerg("m-cap compress enable\n");
		if (dvrif_memory_get_compression_mode(display) == COMPRESSION_LINE_MODE)
		{
			...
			//rtd_pr_scaler_memory_emerg("m-cap compress line mode\n");
		}
		else
		{
			//rtd_pr_scaler_memory_emerg("file=%s, function=%s, line=%d\n", __FILE__, __FUNCTION__, __LINE__);
			//rtd_pr_scaler_memory_emerg("m-cap compress frame mode\n");
		}
	}
	// Not compress, check if m-cap line mode?
	else if (MEMCAPTYPE_LINE == dvrif_memory_get_cap_mode(display))
	{
		LineStep_96Align = drvif_memory_get_data_align(LineStep, DMA_SPEEDUP_ALIGN_VALUE/8);
		//rtd_pr_scaler_memory_emerg("file=%s, function=%s, line=%d\n", __FILE__, __FUNCTION__, __LINE__);
		//rtd_pr_scaler_memory_emerg("m-cap not compress line mode\n");
	}
	if(get_hdmi_vrr_4k60_mode())
	{
		mdomain_disp_ddr_mainlinestep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainLineStep_reg);
		mdomain_disp_ddr_mainlinestep_reg.main_line_step = ((LineStep_96Align + LineStep_96Align%2) * 2);//bit 3 need to be 0. rbus rule
		IoReg_Write32(MDOMAIN_DISP_DDR_MainLineStep_reg, mdomain_disp_ddr_mainlinestep_reg.regValue);	
	}
	else if((is_4k_hfr_mode(display)) || ((get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_ROTATE_0)&&(get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_DEFAULT)))
	{
		mdomain_disp_ddr_mainlinestep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainLineStep_reg);
		mdomain_disp_ddr_mainlinestep_reg.main_line_step = (LineStep_96Align + LineStep_96Align%2);//bit 3 need to be 0. rbus rule
		IoReg_Write32(MDOMAIN_DISP_DDR_MainLineStep_reg, mdomain_disp_ddr_mainlinestep_reg.regValue);	
	}
	else
	{
		mdomain_disp_ddr_mainlinestep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainLineStep_reg);
		mdomain_disp_ddr_mainlinestep_reg.main_line_step = (LineStep + LineStep%2);//bit 3 need to be 0. rbus rule
		IoReg_Write32(MDOMAIN_DISP_DDR_MainLineStep_reg, mdomain_disp_ddr_mainlinestep_reg.regValue);	
	}
#ifndef BUILD_QUICK_SHOW
	if(Get_PANEL_3D_DISPLAY_SUPPORT_ENABLE() && Get_PANEL_3D_LINE_ALTERNATIVE_SUPPORT_ENABLE()
		&& ((Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE)==SLR_3D_SIDE_BY_SIDE_HALF)||(Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE)==SLR_3D_TOP_AND_BOTTOM)||(Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE)==SLR_3D_FRAME_PACKING)))
	{
		...
		unsigned int temp;
		ddr_bit=((Scaler_DispGetStatus(SLR_MAIN_DISPLAY,SLR_DISP_10BIT) << 1))  | ( !Scaler_DispGetStatus(SLR_MAIN_DISPLAY,SLR_DISP_422CAP));
		...
		if(Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE)==SLR_3D_FRAME_PACKING || Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE)==SLR_3D_TOP_AND_BOTTOM)
		{
			...
			{
				MemShiftAddr=((2*Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_MEM_LEN_PRE)-2*Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_LEN))>>2)*LineStep;
			}
			else
			{
				MemShiftAddr=((Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_MEM_LEN_PRE)-Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_LEN))>>2)*LineStep;
		...
		{
			MemShiftAddr=((Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_MEM_LEN_PRE)-Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_LEN))>>1)*LineStep;
			if(get_3D_overscan_enable())
			{
		...
		//rtd_pr_scaler_memory_debug("[3d]shift addr v=%x\n",MemShiftAddr);
		temp=memory_get_line_size(((Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_MEM_WID_PRE)-Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_WID))>>1), (ddr_bit & _BIT0), ((ddr_bit & _BIT1) >> 1));
		if(get_3D_overscan_enable())
		{
			if(Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE)==SLR_3D_FRAME_PACKING || Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE)==SLR_3D_TOP_AND_BOTTOM)
		...
		//rtd_pr_scaler_memory_debug("[3d]shift addr h=%x\n",MemShiftAddr);
		MemShiftAddr=MemShiftAddr<<3;
		//rtd_pr_scaler_memory_debug("[3d]shift addr after <<3 =%x\n",MemShiftAddr);
	}
#endif
#if 0
...
#endif
	{
		...
		if(Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ)>650)
		{
			...
			IoReg_Write32(MDOMAIN_DISP_DDR_Main3rdAddr_reg, ucStartAddress3& 0xfffffff0);
		}
...
#ifndef BUILD_QUICK_SHOW
	Scaler_Disp3dSetInfo(display, SLR_DISP_3D_DISPLAY_SIZE, memory_get_display_size(display));
#endif
	...
	//rtd_pr_scaler_memory_debug("%s %d bDouble=%d\n",__FUNCTION__,__LINE__,bDouble);
	if(bDouble) {	// use double buffer
		...
		TotalSize = LineStep * Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_MEM_ACT_LEN);
		mdomain_disp_ddr_mainblkstep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainBlkStep_reg);
		mdomain_disp_ddr_mainblkstep_reg.main_block_step = (TotalSize + TotalSize%2);//bit 3 need to be 0. rbus rule
		IoReg_Write32(MDOMAIN_DISP_DDR_MainBlkStep_reg, mdomain_disp_ddr_mainblkstep_reg.regValue);	
	}
#ifndef BUILD_QUICK_SHOW
	if(Get_PANEL_3D_DISPLAY_SUPPORT_ENABLE() && Get_PANEL_3D_LINE_ALTERNATIVE_SUPPORT_ENABLE()){
...
#endif
	Scaler_Disp3dSetInfo(display, SLR_DISP_3D_MAIN_ADDR, ucStartAddress);
	Scaler_Disp3dSetInfo(display, SLR_DISP_3D_MAIN_SEC_ADDR, ucStartAddress2);
	Scaler_Disp3dSetInfo(display, SLR_DISP_3D_MAIN_THIRD_ADDR, ucStartAddress3);
#endif
...
//	unsigned int  TotalSizeFrame=0;
	...
	if((_CHANNEL1 == (SCALER_CHANNEL_NUM)Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL)) && ( !Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP))){
		burstLen=0xf0;
	}
	...
	if(m_dispwin.srcy <= 100) {
		ddr_mainprevstart_reg.main_pre_rd_v_start = drv_Calculate_m_pre_read_value();//7;
	} else {
		ddr_mainprevstart_reg.main_pre_rd_v_start = (m_dispwin.srcy-10);
	}
	IoReg_Write32(MDOMAIN_DISP_DDR_MainPreVStart_reg, ddr_mainprevstart_reg.regValue);
	...
	//frank@0323 change waterlevel/2 for AT,because water level nit is 128 bits
	mdomain_disp_ddr_mainnumwtl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainNumWTL_reg);
	mdomain_disp_ddr_mainnumwtl_reg.main_pixel_num = Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_WID) & 0x03FFF;//14 bit
	mdomain_disp_ddr_mainnumwtl_reg.main_wtlvl = ((fifoLen-(burstLen>>1))>>1);
	IoReg_Write32(MDOMAIN_DISP_DDR_MainNumWTL_reg, mdomain_disp_ddr_mainnumwtl_reg.regValue);	

	mdomain_disp_ddr_mainlinenum_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainLineNum_reg);
	mdomain_disp_ddr_mainlinenum_reg.main_line_number = (Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_LEN) & 0x0FFF);
	IoReg_Write32(MDOMAIN_DISP_DDR_MainLineNum_reg, mdomain_disp_ddr_mainlinenum_reg.regValue);
	...
	//rtd_pr_scaler_memory_debug("disp buffernum=%x, ucStartAddress=%x ucStartAddress2=%x\n",buffernum,ucStartAddress,ucStartAddress2);
...
void memory_set_input_format(unsigned char display, unsigned char bDouble)
{
	...
	mdomain_disp_main_disp_blocksel_option_RBUS mdomain_disp_main_disp_blocksel_option_reg;
	mdomain_disp_sub_disp_blocksel_option_RBUS mdomain_disp_sub_disp_blocksel_option_reg;
	...
	//run scaler clear force_blocksel_mode
...
#endif        
	{
           ...
           )
		{
			...
			if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 650) 
                #ifndef BUILD_QUICK_SHOW
                && (Scaler_Get3DMode((SCALER_DISP_CHANNEL)display) == SLR_3DMODE_2D)
                #endif
                && (display == SLR_MAIN_DISPLAY))
			...
			if(Get_PANEL_3D_LINE_ALTERNATIVE_SUPPORT_ENABLE()){
				if((Scaler_Disp3dGetInfo((SCALER_DISP_CHANNEL)display, SLR_DISP_3D_3DTYPE) == SLR_3D_TOP_AND_BOTTOM)||(Scaler_Disp3dGetInfo((SCALER_DISP_CHANNEL)display, SLR_DISP_3D_3DTYPE) == SLR_3D_FRAME_PACKING)){
					//temp1 |=_BIT14;
					// mdomain_disp_ddr_mainctrl_reg.main_pr_out_merge_triple = 1; // Merlin8 remove
				}
			}
#endif 
		}
		...
		if(Get_PANEL_3D_DISPLAY_SUPPORT_ENABLE() && Get_PANEL_3D_LINE_ALTERNATIVE_SUPPORT_ENABLE()){
			// mdomain_disp_ddr_mainctrl_reg.main_pr_our_merge_en = (mdomain_disp_ddr_mainctrl_reg.regValue&MDOMAIN_DISP_DDR_MainCtrl_main_pr_our_merge_en_mask)>>MDOMAIN_DISP_DDR_MainCtrl_main_pr_our_merge_en_shift; // Merlin8 remove
			// mdomain_disp_ddr_mainctrl_reg.main_merge_en0 =  (mdomain_disp_ddr_mainctrl_reg.regValue&MDOMAIN_DISP_DDR_MainCtrl_main_merge_en0_mask)>>MDOMAIN_DISP_DDR_MainCtrl_main_merge_en0_shift; // Merlin8 remove
		}
#endif        
	}
	...
	if(display == SLR_MAIN_DISPLAY){
		...
		if(get_hdmi_vrr_4k60_mode())
			mdomain_disp_ddr_subctrl_reg.sub_source_sel = 0;	//from main capture
		else
    ...
    if (display == SLR_MAIN_DISPLAY) {
		if (get_hdmi_vrr_4k60_mode())
			mdomain_disp_ddr_mainctrl_reg.sub_sm_1stden_sel = 1;
		else
			mdomain_disp_ddr_mainctrl_reg.sub_sm_1stden_sel = 0;
	} else {
		/* clear sub_sm_1stden_sel if we want show sub display */
		if (!(get_hdmi_vrr_4k60_mode())) {
			if (mdomain_disp_ddr_mainctrl_reg.sub_sm_1stden_sel == 1)
				mdomain_disp_ddr_mainctrl_reg.sub_sm_1stden_sel = 0;
		}
	}
	...
	//VRR 4K60 mode, sub disp set same as main disp.
	if((display == SLR_SUB_DISPLAY) && get_hdmi_vrr_4k60_mode())
		mdomain_disp_display_sub_byte_channel_swap_reg.regValue = display_byte_channel_swap_reg.regValue;
	IoReg_Write32(MDOMAIN_DISP_Display_byte_channel_swap_reg, display_byte_channel_swap_reg.regValue);
...
unsigned int drv_get_caveout_buffer_size_by_platform(unsigned int pixel_bits)
{
	...
	buffersize = (drvif_memory_get_data_align((width * pixel_bits + 256) / 8, DMA_SPEEDUP_ALIGN_VALUE) * len + FINAL_BURST_LENTH);
#else
	buffersize = (((width * pixel_bits + 256) / 8) * len + FINAL_BURST_LENTH);
#endif //CONFIG_BW_96B_ALIGNED
	return buffersize;
}
...
//	unsigned char lineRepeatEnable = FALSE;
	...
	if(Cap_type == MEMCAPTYPE_LINE){
		//rtd_pr_scaler_memory_debug("[2D3D] Rpt/Multi[%d/x%d], LineWidth/Size[%d/%d]\n", lineRepeatEnable, multiOf2D3DBuf, TotalSize, TotalSize*multiOf2D3DBuf);
		...
		if(get_hdmi_vrr_4k60_mode() || (is_4k_hfr_mode(SLR_MAIN_DISPLAY)) || ((get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_ROTATE_0)&&(get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_DEFAULT)))
			...
			IoReg_Write32(display ? MDOMAIN_CAP_DDR_In2LineStep_reg: MDOMAIN_CAP_DDR_In1LineStep_reg, (TotalSize + TotalSize%2) << 3);
...
//-	rtd_pr_scaler_memory_debug("\n Calculate FIFO Size !");
...
#endif       
	{
		//rtd_pr_scaler_memory_emerg("0  [Memory]TotalSize=%x, WL=%x,Q=%x,Rm=%x\n", TotalSize, burstLen,Quotient,Remainder);
		memory_division(TotalSize, burstLen, &Quotient, &Remainder);	// Do a division
	}


	//rtd_pr_scaler_memory_debug("[Memory]WL=%x,Q=%x,Rm=%x\n", burstLen,Quotient,Remainder);
	...
	if((dvrif_memory_compression_get_enable(display) == TRUE) &&(dvrif_memory_get_compression_mode(display) == MEMCAPTYPE_FRAME)){
		...
		rtd_pr_scaler_memory_debug("line mode\n");
	...
	//rtd_pr_scaler_memory_debug("[3D]TotalSize_ori= %x,TotalSize1_ori= %x,INPUT_CAP_LEN=%x, SG_FRC=%d\n", TotalSize,TotalSize1
//		,Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY), SLR_INPUT_CAP_LEN), drvif_scaler3d_decide_3d_SG_data_FRC());
	...
	if (display == SLR_MAIN_DISPLAY)	 {
...

		{
			...
			{
				buffer_size = MDOMAIN_MAIN_BUFFER_SIZE_16BITS_4K120_CO_BUFFER;//using pc mode size
				//if(get_panel_pixel_mode() > PANEL_1_PIXEL)
                {
                    buffer_size = buffer_size/2;//4k1k for 4k120 case
                }
			}
			// 4k60 vrr/game mode
			else if (is_m_domain_data_frc_need_borrow_memory()) {
			...
			if(data_frc_need_borrow_cma_buffer(display)){
				buffer_size = MDOMAIN_MAIN_BUFFER_SIZE_16BITS_4K;
			}
			...
			{
#ifndef BUILD_QUICK_SHOW
				if (is_m_domain_data_frc_need_borrow_memory() && m_domain_4k_memory_from_type() == BORROW_FROM_CMA) {//4k frc and need borrow memory from cma
			...
			if(drv_memory_Get_multibuffer_flag()){
				rtd_pr_scaler_memory_emerg("[crixus]M-domain Multi-buffer Enable!!!\n");
			...
			//virtual memory allocation	and just use one buffer
			rtd_pr_scaler_memory_debug("MemTag[MEMIDX_MAIN_THIRD].Allocate_VirAddr=%x\n", (unsigned int)MemTag[MEMIDX_MAIN_THIRD].Allocate_VirAddr);
			if (0 == MemTag[MEMIDX_MAIN_THIRD].Allocate_VirAddr)
			{
				...
				#ifdef CONFIG_ARM64
					rtd_pr_scaler_memory_debug("vir_addr=%lx, align_vir_addr=%lx\n", vir_addr, align_vir_addr);
				#else
					rtd_pr_scaler_memory_debug("vir_addr=%x, align_vir_addr=%x\n", vir_addr, align_vir_addr);
				#endif
			}
			//M-domain start address
			IoReg_Write32(MDOMAIN_CAP_DDR_In1_3rdAddr_reg, (MemTag[MEMIDX_MAIN_THIRD].StartAddr & 0xfffffff0));
			IoReg_Write32(MDOMAIN_CAP_DDR_In1_2ndAddr_reg, (MemTag[MEMIDX_MAIN_SEC].StartAddr & 0xfffffff0));
			IoReg_Write32(MDOMAIN_CAP_DDR_In1Addr_reg, (MemTag[MEMIDX_MAIN].StartAddr & 0xfffffff0));
			//M-domain boundary
			...
			#endif
			{
				unsigned long low_addr =  min(MemTag[MEMIDX_MAIN_THIRD].StartAddr & 0xfffffff0,
						MemTag[MEMIDX_MAIN_SEC].StartAddr & 0xfffffff0);

				unsigned long high_addr = max((MemTag[MEMIDX_MAIN].StartAddr+buffer_size) & 0xfffffff0
						, (MemTag[MEMIDX_MAIN_THIRD].StartAddr+buffer_size) & 0xfffffff0);

				IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr1_reg, low_addr);
				...
				IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg, high_addr);
	...
	drvif_memory_free_block(MEMIDX_MAIN_THIRD);
		...
		//rtd_pr_scaler_memory_debug("memory_get_capture_max_size:TotalSize1 == %d\n",TotalSize1);
		...
		//if(TRUE!=get_delayframe_fuc())
		{
			...
			if((Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ)>650) && (display ==_CHANNEL1))
			{
				...
				IoReg_Write32(MDOMAIN_CAP_DDR_In1_3rdAddr_reg, nStartAddr & 0xfffffff0);
			}
			else if(_ENABLE == Scaler_Get_Enable_3Buffer_NewRuleCondition())
			{
				if(_TRUE == Scaler_DispGetStatus(display, SLR_DISP_VFLIP3))
		...
		//rtd_pr_scaler_memory_debug("###################main buffer TotalSize=%d;;TotalSize1=%d;;TotalSize2=%d\n",TotalSize,TotalSize1,TotalSize2);
		if(bDouble) {
			if(Scaler_InputSrcGetType(display) == _SRC_VO){
			...
			IoReg_Write32(MDOMAIN_CAP_DDR_In1_2ndAddr_reg, nStartAddr & 0xfffffff0);
		}
#if 0
		//20130905qiang_zhou:added for video delay frames for bluetooth+
		if(TRUE==get_delayframe_fuc())
		...
		IoReg_Write32(MDOMAIN_CAP_DDR_In1Addr_reg, nStartAddr& 0xfffffff0);
		//rtd_pr_scaler_memory_debug("After %s--%d  IoReg_Read32(MDOMAIN_CAP_DDR_In1LineStep_reg)=%x\n",__FUNCTION__,__LINE__,IoReg_Read32(MDOMAIN_CAP_DDR_In1LineStep_reg));
#endif
	}
#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
	else {
//-		rtd_pr_scaler_memory_debug("\n Allocate Sub Disp Memory !\n");
		...
		if(is_4k_hfr_mode(SLR_MAIN_DISPLAY))
		{
			...
			rtd_pr_scaler_memory_debug("set M-domain sub 3rd buffer 0x%px 0x%08X\n", sub_3rd_buffer, MemTag[MEMIDX_SUB_THIRD].StartAddr);
		...
		if(bDouble)
		{
			...
			rtd_pr_scaler_memory_debug("set M-domain sub 2nd buffer 0x%px paddr 0x%08X\n", sub_2nd_buffer, MemTag[MEMIDX_SUB_SEC].StartAddr);

		}

		MemTag[MEMIDX_SUB].StartAddr = get_query_start_address(QUERY_IDX_MDOMAIN_SUB);
		IoReg_Write32(MDOMAIN_CAP_DDR_In2Addr_reg, MemTag[MEMIDX_SUB].StartAddr & 0x7ffffff0);
		//IoReg_Write32(MDOMAIN_CAP_DDR_In2Addr_reg, drvif_memory_alloc_block(MEMIDX_SUB, buffer_size, MEMALIGN_ANY) & 0xfffffff0);

		rtd_pr_scaler_memory_debug("###################sub buffer TotalSize=%d;;\n", buffer_size);
//-		rtd_pr_scaler_memory_debug("Get allocated sub display addr1=%x, addr2=%x, size=%x\n", StartAddr, StartAddr1, TotalSize);
	...
	if(display){
		...
		// mdomain_cap_ddr_in2ctrl_reg.in2_en_vs_rst = 1; // Merlin8 remove
		if (get_sub_m_domain_2nd_buf_address() != NULL && get_sub_m_domain_3rd_buf_address() != NULL) {
		...
		} else if (get_sub_m_domain_2nd_buf_address() != NULL) {
			mdomain_cap_ddr_in2ctrl_reg.in2_double_enable = 1;
	...
	#ifdef ENABLE_3_BUFFER_DELAY_MODE
		//triple_buf_flag always equal to 1
		// mdomain_cap_ddr_in1ctrl_reg.in1_en_vs_rst = 1; // Merlin8 remove
    ...
    rtd_pr_scaler_memory_debug("double_buf_flag=%d, triple_buf_flag=%d\n", double_buf_flag, triple_buf_flag);
#ifndef BUILD_QUICK_SHOW
...
void memory_set_memory_size(unsigned char bDouble, eMemCapAccessType Cap_type)
{
	...
	if (Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY){
		...
		if((Scaler_I2rnd_get_timing_enable() == _ENABLE) && ((vsc_i2rnd_sub_stage == I2RND_STAGE_MAIN_GET_RPC) || (vsc_i2rnd_dualvo_sub_stage == I2RND_DUALVO_STAGE_RERUN_MAIN_FOR_S1))){
			rtd_pr_scaler_memory_emerg("[I2RND][%s]Enter cap2 memory allocate\n", __FUNCTION__);
...
#endif
		{
			...
			{
				IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr1_reg,(MemTag[MEMIDX_MAIN_THIRD].StartAddr& 0xfffffff0));
				IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg,(MemTag[MEMIDX_MAIN].StartAddr+buffer_size)& 0xfffffff0);

				//Update m-cap limit boundary
				drv_memory_cap_limit_boundary_setting(SLR_MAIN_DISPLAY, buffer_size);

				IoReg_Write32(MDOMAIN_DISP_DDR_MainAddr_reg, drvif_memory_get_block_addr(MEMIDX_MAIN)& 0xfffffff0);
				IoReg_Write32(MDOMAIN_DISP_DDR_Main2ndAddr_reg, drvif_memory_get_block_addr(MEMIDX_MAIN_SEC)& 0xfffffff0);
				IoReg_Write32(MDOMAIN_DISP_DDR_Main3rdAddr_reg, drvif_memory_get_block_addr(MEMIDX_MAIN_THIRD)& 0xfffffff0);
				//update m_disp boundary the same as m_cap boundary @Crixus 20171227
				IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr1_reg, MemTag[MEMIDX_MAIN_THIRD].StartAddr& 0xfffffff0);
				IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr2_reg, (MemTag[MEMIDX_MAIN].StartAddr+buffer_size)& 0xfffffff0);
				//Update s-cap limit boundary
				drv_memory_disp_limit_boundary_setting(SLR_MAIN_DISPLAY);
			}
		}
...
//-		rtd_pr_scaler_memory_debug("\n Allocate Sub Disp Memory !\n");
		...
		drvif_memory_free_block(MEMIDX_SUB_THIRD);	// free block befor getting a new one
		//sub use cap max size @Crixus 20151231
		if((is_4k_hfr_mode(SLR_SUB_DISPLAY)) || get_hdmi_vrr_4k60_mode())
		{
			if((Scaler_DispGetStatus(SLR_SUB_DISPLAY, SLR_DISP_422CAP) == FALSE) && get_vsc_run_pc_mode()) {
			...
			set_sub_m_domain_3rd_buf_address(sub_3rd_buffer);
			MemTag[MEMIDX_SUB_THIRD].StartAddr = dvr_to_phys(sub_3rd_buffer);
			rtd_pr_scaler_memory_debug("set M-domain sub 3rd buffer 0x%px paddr 0x%08X\n", sub_3rd_buffer, MemTag[MEMIDX_SUB_THIRD].StartAddr);
			IoReg_Write32(MDOMAIN_CAP_DDR_In2_3rdAddr_reg, MemTag[MEMIDX_SUB_THIRD].StartAddr & 0x7ffffff0);
		//}
#endif
		...
		nStartAddr = get_query_start_address(QUERY_IDX_MDOMAIN_SUB);
		MemTag[MEMIDX_SUB].StartAddr = nStartAddr;
		IoReg_Write32(MDOMAIN_CAP_DDR_In2Addr_reg, MemTag[MEMIDX_SUB].StartAddr & 0x7ffffff0);
...
void dvrif_memory_setting_for_data_fs(void)
{
	...
	if (display == SLR_MAIN_DISPLAY) {	// Main
		...
		if(is_4k_hfr_mode(SLR_MAIN_DISPLAY))
		{
			IoReg_Write32(MDOMAIN_DISP_DDR_SubPreVStart_reg, IoReg_Read32(MDOMAIN_DISP_DDR_MainPreVStart_reg));
		}
    }
#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
	else {	//Sub enable
...
//	unsigned int  TotalSizeFrame;
	...
	//frank@0325 double the M domain burst length to avoid 444 10 bits peak BW not enough
	if(((unsigned short)_CHANNEL1 == Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL)) && ( !Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP))){
		burstLen=0xf0;
	}
...
//-	rtd_pr_scaler_memory_debug("\n Calculate FIFO Size !");
	...
	rtd_pr_scaler_memory_debug("[ZOOM][Memory]WL=%x,Q=%x,Rm=%x\n", burstLen,Quotient,Remainder);
	memory_set_capture_fifo(Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL), Quotient, Remainder, burstLen);	// set capture FIFO
	...
	if (Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY)	 {
		...
		{
			rtd_pr_scaler_memory_debug("TOTAL SIZE1 == 0\n");
			TotalSize1 = 0;
		}
...
//	unsigned int  TotalSizeFrame;
	...
	//-		rtd_pr_scaler_memory_debug("\n Allocate Sub Disp Memory !\n");
			...
			if(bVflip)
				IoReg_Write32(MDOMAIN_CAP_DDR_In2_3rdAddr_reg, drvif_memory_alloc_block(MEMIDX_SUB_THIRD,TotalSize+CONFIG_MDOMAIN_BURST_SIZE, MEMALIGN_ANY)& 0xfffffff0);
	//-		rtd_pr_scaler_memory_debug("Get allocated sub display addr1=%x, addr2=%x, size=%x\n", StartAddr, StartAddr1, TotalSize);
		}
#endif
...
static void MemSetDisplayVFLIPAddr(unsigned char display, unsigned int Addr1, unsigned int Addr2, unsigned int Addr3)
{
	...
	//	dvrif_memory_set_addr_check(SLR_MAIN_DISPLAY, _TRUE);
		...
		IoReg_Write32(MDOMAIN_DISP_DDR_SubAddr_reg, Addr1& 0xfffffff0);
		IoReg_Write32(MDOMAIN_DISP_DDR_Sub2ndAddr_reg, Addr2& 0xfffffff0);
		IoReg_Write32(MDOMAIN_DISP_DDR_Sub3rdAddr_reg,Addr3& 0xfffffff0);
...
//	unsigned int  TotalSizeFrame=0;
	...
	else
	{
		...
		mdomain_disp_ddr_mainnumwtl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainNumWTL_reg);
		mdomain_disp_ddr_mainnumwtl_reg.main_pixel_num = (Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_WID) +droppixel) & 0x03FFF;//14 bit
		mdomain_disp_ddr_mainnumwtl_reg.main_wtlvl = ((fifoLen-(burstLen>>1))>>1);
		IoReg_Write32(MDOMAIN_DISP_DDR_MainNumWTL_reg, mdomain_disp_ddr_mainnumwtl_reg.regValue);
		rtd_pr_scaler_memory_debug("[vFlip] drop pixel=%d shift addr=%x\n",droppixel,MemShiftAddr);

	}

	rtd_pr_scaler_memory_debug("[crixus][v-flip]TotalSize = %d\n", TotalSize);
	...
	LineStep = TotalSize;
	LineStep_96Align = TotalSize;
	//rtd_pr_scaler_memory_debug("[vFlip]line step=0x%x\n",LineStep);
	...
	if(get_hdmi_vrr_4k60_mode() || (is_4k_hfr_mode(display)) || ((get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_ROTATE_0)&&(get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_DEFAULT)))
	{
		LineStep_96Align = LineStep_96Align + LineStep_96Align%2;
		rtd_pr_scaler_memory_emerg("[crixus][v-flip]Align_LineStep = 0x%x", (~LineStep_96Align + 1));


		mdomain_disp_ddr_mainlinestep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainLineStep_reg);
		mdomain_disp_ddr_mainlinestep_reg.main_line_step = (~LineStep_96Align + 1);//bit 3 need to be 0. rbus rule
		IoReg_Write32(MDOMAIN_DISP_DDR_MainLineStep_reg, mdomain_disp_ddr_mainlinestep_reg.regValue);
	}
	else{
		LineStep = LineStep + LineStep%2;
		rtd_pr_scaler_memory_emerg("[crixus][v-flip]Align_LineStep = 0x%x", (~LineStep + 1));


		mdomain_disp_ddr_mainlinestep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainLineStep_reg);
		mdomain_disp_ddr_mainlinestep_reg.main_line_step = (~LineStep + 1);//bit 3 need to be 0. rbus rule
		IoReg_Write32(MDOMAIN_DISP_DDR_MainLineStep_reg, mdomain_disp_ddr_mainlinestep_reg.regValue);
	}
		...
		&& ((Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE)==SLR_3D_SIDE_BY_SIDE_HALF)||(Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE)==SLR_3D_TOP_AND_BOTTOM)||(Scaler_Disp3dGetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE)==SLR_3D_FRAME_PACKING)))
	{
		...
		//rtd_pr_scaler_memory_debug("[3d]shift addr h=%x\n",MemShiftAddr);
		MemShiftAddr=MemShiftAddr<<3;
		rtd_pr_scaler_memory_debug("[3d][vFlip]shift addr after <<3 =%x\n",MemShiftAddr);
	}
	...
	rtd_pr_scaler_memory_debug("[vFlip]before address %x %x %x\n",ucStartAddress,ucStartAddress2, ucStartAddress3);
	if(get_hdmi_vrr_4k60_mode() || (is_4k_hfr_mode(display)) || ((get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_ROTATE_0)&&(get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_DEFAULT)))
	{
		LineStep_96Align = drvif_memory_get_data_align(LineStep_96Align, DMA_SPEEDUP_ALIGN_VALUE/8);
		ucStartAddress = memory_cal_vflip_block_addr(ucStartAddress, LineStep_96Align , Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_LEN));
		ucStartAddress2 = memory_cal_vflip_block_addr(ucStartAddress2, LineStep_96Align ,Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_LEN));
		ucStartAddress3 = memory_cal_vflip_block_addr(ucStartAddress3, LineStep_96Align , Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_LEN));
		rtd_pr_scaler_memory_debug("[vFlip]after address %x %x %x\n",ucStartAddress,ucStartAddress2, ucStartAddress3);
	...
	rtd_pr_scaler_memory_debug("[vFlip]block step %x\n", ((TotalSize + TotalSize%2) << 3));
...
//	unsigned int  TotalSizeFrame=0;
	...
	mdomain_disp_ddr_mainlinestep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainLineStep_reg);
	mdomain_disp_ddr_mainlinestep_reg.main_line_step = ((~Align_LineStep) + 1);//bit 3 need to be 0. rbus rule
	IoReg_Write32(MDOMAIN_DISP_DDR_MainLineStep_reg, mdomain_disp_ddr_mainlinestep_reg.regValue);	
...
void memory_set_main_displaywindow_change(void)
{
	...
	if ((Scaler_DispGetInputInfoByDisp(display,SLR_INPUT_MEM_ACT_HSTA) == 0) && (Scaler_DispGetInputInfoByDisp(display,SLR_INPUT_MEM_ACT_VSTA) == 0) )
	{
		if((Scaler_DispGetInputInfoByDisp(display,SLR_INPUT_CHANNEL) == _CHANNEL1) && (dvrif_memory_compression_get_enable(Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL)) == TRUE)){
			...
			mdomain_disp_ddr_mainnumwtl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainNumWTL_reg);
			mdomain_disp_ddr_mainnumwtl_reg.main_pixel_num = (comp_width * dvrif_memory_get_compression_bits(display) / 32) & 0x03FFF;//14 bit
			mdomain_disp_ddr_mainnumwtl_reg.main_wtlvl = ((fifoLen - (burstLen>>1))>>1);
			IoReg_Write32(MDOMAIN_DISP_DDR_MainNumWTL_reg, mdomain_disp_ddr_mainnumwtl_reg.regValue);
			TotalSize = memory_get_capture_line_size_compression(dvrif_memory_get_compression_bits(display));
		} else {
		...
		TotalSize = memory_get_disp_line_size1(display,Scaler_DispGetInputInfoByDisp(display,SLR_INPUT_MEM_ACT_WID) + droppixel);	// calculate the memory size of capture
		mdomain_disp_ddr_mainnumwtl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainNumWTL_reg);
		mdomain_disp_ddr_mainnumwtl_reg.main_pixel_num = (Scaler_DispGetInputInfoByDisp(display,SLR_INPUT_MEM_ACT_WID) +droppixel) & 0x03FFF;//14 bit
		mdomain_disp_ddr_mainnumwtl_reg.main_wtlvl = ((fifoLen - (burstLen>>1))>>1);
		IoReg_Write32(MDOMAIN_DISP_DDR_MainNumWTL_reg, mdomain_disp_ddr_mainnumwtl_reg.regValue);
		rtd_pr_scaler_memory_debug("drop pixel=%d shift addr=%x\n",droppixel,MemShiftAddr);
...
#endif
	LineStep = TotalSize;
	//rtd_pr_scaler_memory_debug("Line Step=0x%x\n",LineStep);
	TotalSize = SHL(TotalSize, 3); // unit conversion from 64bits to 8bits
		...
		&& ((Scaler_Disp3dGetInfo(display, SLR_DISP_3D_3DTYPE)==SLR_3D_SIDE_BY_SIDE_HALF)||(Scaler_Disp3dGetInfo(display, SLR_DISP_3D_3DTYPE)==SLR_3D_TOP_AND_BOTTOM)||(Scaler_Disp3dGetInfo(display, SLR_DISP_3D_3DTYPE)==SLR_3D_FRAME_PACKING)))
	{
		...
		if(Scaler_Disp3dGetInfo(display, SLR_DISP_3D_3DTYPE)==SLR_3D_FRAME_PACKING || Scaler_Disp3dGetInfo(display, SLR_DISP_3D_3DTYPE)==SLR_3D_TOP_AND_BOTTOM)
		{
			...
			{
				MemShiftAddr=((2*Scaler_DispGetInputInfoByDisp(display, (SLR_INPUT_INFO)SLR_DISP_3D_MEM_LEN_PRE)-2*Scaler_DispGetInputInfoByDisp(display,SLR_INPUT_MEM_ACT_LEN))>>2)*LineStep;
			}
			else
			{
				MemShiftAddr=((Scaler_DispGetInputInfoByDisp(display, (SLR_INPUT_INFO)SLR_DISP_3D_MEM_LEN_PRE)-Scaler_DispGetInputInfoByDisp(display,SLR_INPUT_MEM_ACT_LEN))>>2)*LineStep;
				if(get_3D_overscan_enable())
				{
					MemShiftAddr=(get_3D_overscan_input_vsta()>>1)*LineStep;
				}
		...
		{
			MemShiftAddr=((Scaler_DispGetInputInfoByDisp(display, (SLR_INPUT_INFO)SLR_DISP_3D_MEM_LEN_PRE)-Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_LEN))>>1)*LineStep;
			if(get_3D_overscan_enable())
			{
				MemShiftAddr=(get_3D_overscan_input_vsta())*LineStep;
			}
		...
		//rtd_pr_scaler_memory_debug("[3d]shift addr v=%x\n",MemShiftAddr);
		temp=memory_get_line_size(((Scaler_DispGetInputInfoByDisp(display, (SLR_INPUT_INFO)SLR_DISP_3D_MEM_WID_PRE) - Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_WID))>>1), (ddr_bit & _BIT0), ((ddr_bit & _BIT1) >> 1));
		if(get_3D_overscan_enable())
		{
			if(Scaler_Disp3dGetInfo(display, SLR_DISP_3D_3DTYPE)==SLR_3D_FRAME_PACKING || Scaler_Disp3dGetInfo(display, SLR_DISP_3D_3DTYPE)==SLR_3D_TOP_AND_BOTTOM)
				temp=memory_get_line_size(get_3D_overscan_input_hsta(), (ddr_bit & _BIT0), ((ddr_bit & _BIT1) >> 1));
...
#endif
	ucStartAddress = (drvif_memory_get_block_addr(MEMIDX_MAIN) + MemShiftAddr) & 0xfffffff0;

	ucStartAddress2 = (drvif_memory_get_block_addr(MEMIDX_MAIN_SEC) + MemShiftAddr)& 0xfffffff0;
#ifndef BUILD_QUICK_SHOW
	if(Get_PANEL_3D_DISPLAY_SUPPORT_ENABLE() && Get_PANEL_3D_LINE_ALTERNATIVE_SUPPORT_ENABLE() && (Scaler_Disp3dGetInfo(display, SLR_DISP_3D_3DTYPE) != SLR_3D_2D_ONLY))
	{
		MemDispStartAddr_3D[0]=ucStartAddress;
	...
	if(Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ)>650)
	{
		ucStartAddress3 = (drvif_memory_get_block_addr(MEMIDX_MAIN_THIRD) + MemShiftAddr)& 0xfffffff0;
		IoReg_Write32(MDOMAIN_DISP_DDR_Main3rdAddr_reg, ucStartAddress3& 0xfffffff0);
	}
	else if(_ENABLE == Scaler_Get_Enable_3Buffer_NewRuleCondition())
	{
			...
			|| (modestate_get_pr3d_in_fp_3d_mode() && ((Scaler_InputSrcGetMainChType() == _SRC_VO)|| (Scaler_DispGetInputInfoByDisp(display,SLR_INPUT_V_FREQ) >= 310))))
	{
		...
		if(SLR_MAIN_DISPLAY ==  display)//frank@1007 solve VGA source M domain hang problem
			//set Mdomain display triple buffer
			mainctrltemp |=_BIT7;
...
//	mdomain_disp_ddr_sublinestep_RBUS mdomain_disp_ddr_sublinestep_reg;
	...
	mdomain_disp_ddr_subprevstart_Reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_SubPreVStart_reg);
		...
		mdomain_disp_ddr_subprevstart_Reg.sub_pre_rd_v_start = SUB_MDOM_PREREAD_START;
#endif
	...
	if(changetomain)
		MemSetDisplayAddr(SLR_SUB_DISPLAY, (drvif_memory_get_block_addr(MEMIDX_MAIN) + MemShiftAddr) & 0xfffffff0, (drvif_memory_get_block_addr(MEMIDX_MAIN_SEC) + MemShiftAddr) & 0xfffffff0);
	else
		MemSetDisplayAddr(SLR_SUB_DISPLAY, (drvif_memory_get_block_addr(MEMIDX_SUB) + MemShiftAddr) & 0xfffffff0, (drvif_memory_get_block_addr(MEMIDX_SUB_SEC) + MemShiftAddr) & 0xfffffff0);

#ifdef ENABLE_3_BUFFER_DELAY_MODE
	if(changetomain)
		IoReg_Write32(MDOMAIN_DISP_DDR_Sub3rdAddr_reg, (drvif_memory_get_block_addr(MEMIDX_MAIN_THIRD) + MemShiftAddr)& 0xfffffff0);
	else
		IoReg_Write32(MDOMAIN_DISP_DDR_Sub3rdAddr_reg, (drvif_memory_get_block_addr(MEMIDX_SUB_THIRD) + MemShiftAddr) & 0xfffffff0);
#endif
...
static void memory_set_sub_vflip_for_pip(unsigned char display)
{
    ...
    ucStartAddress3 = drvif_memory_get_block_addr(MEMIDX_SUB_THIRD);

   if (dvrif_memory_compression_get_enable(SLR_SUB_DISPLAY) == TRUE)
   {
       TotalSize = memory_get_disp_line_size_compression(display, Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_WID));  // calculate the memory size of capture
   }
   else
   {
       TotalSize = memory_get_display_size(display);   // calculate the memory size of capture
	...
	LineStep = TotalSize;
	Align_LineStep = drvif_memory_get_data_align(TotalSize, DMA_SPEEDUP_ALIGN_VALUE/8);
	Align_LineStep = (Align_LineStep % 2) ? (Align_LineStep + 1) : Align_LineStep;
	mdomain_disp_ddr_sublinestep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_SubLineStep_reg);
	mdomain_disp_ddr_sublinestep_reg.sub_line_step = ((~Align_LineStep) + 1);//bit 3 need to be 0. rbus rule
	IoReg_Write32(MDOMAIN_DISP_DDR_SubLineStep_reg, mdomain_disp_ddr_sublinestep_reg.regValue);
	
	ucStartAddress = memory_cal_vflip_block_addr(ucStartAddress, Align_LineStep , Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_LEN));
	ucStartAddress2 = memory_cal_vflip_block_addr(ucStartAddress2, Align_LineStep , Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_LEN));
    ucStartAddress3 = memory_cal_vflip_block_addr(ucStartAddress3, Align_LineStep , Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_LEN));

    MemSetDisplayVFLIPAddr(SLR_SUB_DISPLAY, ucStartAddress,ucStartAddress2,ucStartAddress3);
	...
	mdomain_disp_ddr_subblkstep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_SubBlkStep_reg);
	mdomain_disp_ddr_subblkstep_reg.sub_block_step = 0;//bit 3 need to be 0. rbus rule
	IoReg_Write32(MDOMAIN_DISP_DDR_SubBlkStep_reg, mdomain_disp_ddr_subblkstep_reg.regValue);
...
void dvrif_memory_handler(unsigned char display)
{
	...
	if (display == SLR_MAIN_DISPLAY) {	// Main
		...
		else
		{
			...
			*/
			if((Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL) == _CHANNEL1) && (dvrif_memory_compression_get_enable(Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL)) == TRUE)){
				...
				if(dvrif_memory_get_compression_mode(display) == COMPRESSION_LINE_MODE)
				{
					rtd_pr_scaler_memory_debug("comp line mode\n");
					memory_set_capture(TRUE, MEMCAPTYPE_LINE, display);
				}
				else
				{
					rtd_pr_scaler_memory_debug("comp frame mode\n");
					memory_set_capture(TRUE, MEMCAPTYPE_FRAME, display);
				}
			}
			else{
				rtd_pr_scaler_memory_debug("no-comp frame mode\n");
		...
		if((Scaler_InputSrcGetMainChType() == _SRC_VO) && (Scaler_VOFrom(Scaler_Get_CurVoInfo_plane()) == 1))	// yychao+ : use single buffer for JPEG source
			memory_set_main(FALSE,display);		//  CSW 0961105 change single buffer to double to slove 1080i moving picture not smooth
		/*else if((Get_HDMI_SingleBuffer_GameMode()==TRUE) && (Scaler_InputSrcGetMainChType() == _SRC_HDMI))
			memory_set_main(FALSE);*/
		else
		{
#ifndef BUILD_QUICK_SHOW					
            if (get_mdomain_vflip_flag(SLR_MAIN_DISPLAY))
            {
		...
		if((is_4k_hfr_mode(display)) && (get_panel_pixel_mode() > PANEL_1_PIXEL)
#ifndef BUILD_QUICK_SHOW
		...
		{//run sub mdomain for two pixel mode only for 120 panel
...
#ifndef BUILD_QUICK_SHOW            
            if (get_mdomain_vflip_flag(SLR_MAIN_DISPLAY)){
                 memory_set_sub_vflip_for_pip(SLR_SUB_DISPLAY);
            } else {
			...
			ddr_mainprevstart_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainPreVStart_reg);
			mdomain_disp_ddr_subprevstart_Reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_SubPreVStart_reg);
			mdomain_disp_ddr_subprevstart_Reg.sub_pre_rd_v_start = ddr_mainprevstart_reg.main_pre_rd_v_start;//sub sync main pre read
			IoReg_Write32(MDOMAIN_DISP_DDR_SubPreVStart_reg, mdomain_disp_ddr_subprevstart_Reg.regValue);
	...
	if(drv_memory_Get_multibuffer_flag() && (Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL) == _CHANNEL1)){
		rtd_pr_scaler_memory_emerg("[crixus]drv_memory_Get_multibuffer_number() = %x\n", drv_memory_Get_multibuffer_number());
...
//remove in k4lp
		...
		if(((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_ADC)
			...
			||((false == is_hdmi_dolby_vision_sink_led()) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)))//dolby case set false
...
//		value =(info->IPV_ACT_LEN_PRE *info->IPH_ACT_WID_PRE * bitNum)>>4;
	...
	if(!((Scaler_I2rnd_get_timing_enable() == _ENABLE) && ((vsc_i2rnd_sub_stage == I2RND_STAGE_MAIN_GET_RPC)|| (vsc_i2rnd_dualvo_sub_stage == I2RND_DUALVO_STAGE_RERUN_MAIN_FOR_S1)))){
		//rtd_pr_scaler_memory_emerg("[%s][crixus]DI non-i2rnd memory allocation\n", __FUNCTION__);
#endif
...
#ifdef CONFIG_I2RND_ENABLE
		//rtd_pr_scaler_memory_emerg("[%s][crixus]DI non-i2rnd MemStartAddr = %x\n", __FUNCTION__, MemStartAddr);

	}
	else{//I2rnd case - allocate DI 2 memory
		//rtd_pr_scaler_memory_emerg("[%s][crixus]DI i2rnd second memory allocation\n", __FUNCTION__);
		if(drvif_scaler3d_get_IDMA3dMode()
			|| (((Get_PANEL_3D_DISPLAY_SUPPORT_ENABLE() && (Get_PANEL_3D_LINE_ALTERNATIVE_SUPPORT_ENABLE() == TRUE)
			&& (Scaler_Disp3dGetInfo((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY), SLR_DISP_3D_3DTYPE)==SLR_3D_FRAME_PACKING)) || (drvif_scaler3d_decide_3d_SG_Disable_IDMA(SLR_3D_FRAME_PACKING) == TRUE))
			&& drvif_scaler3d_decide_HDMI_framePacking_interlaced_status())){
		...
		if(MemStartAddr == _DUMMYADDR){
			rtd_pr_scaler_memory_emerg("set MEMIDX_DI_2  to size fail !!!\n");
		}
		//rtd_pr_scaler_memory_emerg("[%s][crixus]DI i2rnd MemStartAddr = %x\n", __FUNCTION__, MemStartAddr);
	}
#endif
#else
	if(access != 2)
	...
	//add I2rnd condition @Crixus 20160716
	if(!((Scaler_I2rnd_get_timing_enable() == _ENABLE) && ((vsc_i2rnd_sub_stage == I2RND_STAGE_MAIN_GET_RPC) || (vsc_i2rnd_dualvo_sub_stage == I2RND_DUALVO_STAGE_RERUN_MAIN_FOR_S1)))){
		//rtd_pr_scaler_memory_emerg("[%s][crixus]DI non-i2rnd set start address\n", __FUNCTION__);
#endif
		if(drvif_scaler3d_get_IDMA3dMode()
				|| (((Get_PANEL_3D_DISPLAY_SUPPORT_ENABLE() && (Get_PANEL_3D_LINE_ALTERNATIVE_SUPPORT_ENABLE() == TRUE)
					&& (Scaler_Disp3dGetInfo((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY), SLR_DISP_3D_3DTYPE)==SLR_3D_FRAME_PACKING)) || (drvif_scaler3d_decide_3d_SG_Disable_IDMA(SLR_3D_FRAME_PACKING) == TRUE))
		...
		//rtd_pr_scaler_memory_emerg("[%s][crixus]DI i2rnd set start address\n", __FUNCTION__);
		if(drvif_scaler3d_get_IDMA3dMode()
				|| (((Get_PANEL_3D_DISPLAY_SUPPORT_ENABLE() && (Get_PANEL_3D_LINE_ALTERNATIVE_SUPPORT_ENABLE() == TRUE)
					&& (Scaler_Disp3dGetInfo((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY), SLR_DISP_3D_3DTYPE)==SLR_3D_FRAME_PACKING)) || (drvif_scaler3d_decide_3d_SG_Disable_IDMA(SLR_3D_FRAME_PACKING) == TRUE))
					&& drvif_scaler3d_decide_HDMI_framePacking_interlaced_status()))
...
void drvif_memory_set_fs_display_fifo(void)
{
	...
	//frank@0325 double the M domain burst length to avoid 444 10 bits peak BW not enough
	if((Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL) == (unsigned short)_CHANNEL1) && ( !Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP))){
		burstLen=0xf0;
	}
	...
	//frank@0323 change waterlevel/2 for AT,because water level nit is 128 bits
	mdomain_disp_ddr_mainnumwtl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainNumWTL_reg);
	mdomain_disp_ddr_mainnumwtl_reg.main_pixel_num = 64 & 0x03FFF;//14 bit
	mdomain_disp_ddr_mainnumwtl_reg.main_wtlvl = ((fifoLen-(burstLen>>1))>>1);
	IoReg_Write32(MDOMAIN_DISP_DDR_MainNumWTL_reg, mdomain_disp_ddr_mainnumwtl_reg.regValue);

	mdomain_disp_ddr_mainlinenum_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainLineNum_reg);
	mdomain_disp_ddr_mainlinenum_reg.main_line_number = 0;
	IoReg_Write32(MDOMAIN_DISP_DDR_MainLineNum_reg, mdomain_disp_ddr_mainlinenum_reg.regValue);
	...
	if(is_4k_hfr_mode(SLR_MAIN_DISPLAY))
	{
		IoReg_Write32(MDOMAIN_DISP_Display_byte_channel_swap_reg, IoReg_Read32(MDOMAIN_DISP_Display_sub_byte_channel_swap_reg));
		IoReg_Write32(MDOMAIN_CAP_Capture1_byte_swap_reg, IoReg_Read32(MDOMAIN_CAP_Capture2_byte_swap_reg));
		IoReg_Write32(MDOMAIN_DISP_DDR_MainNumWTL_reg, IoReg_Read32(MDOMAIN_DISP_DDR_SubPixNumWTL_reg));
		IoReg_Write32(MDOMAIN_DISP_DDR_MainLineNum_reg, IoReg_Read32(MDOMAIN_DISP_DDR_SubLineNum_reg));
		IoReg_Write32(MDOMAIN_DISP_DDR_MainRdNumLenRem_reg, IoReg_Read32(MDOMAIN_DISP_DDR_SubRdNumLenRem_reg));
		IoReg_Write32(MDOMAIN_DISP_DDR_SubAddrDropBits_reg, IoReg_Read32(MDOMAIN_DISP_DDR_MainAddrDropBits_reg));
		//apply sub
		IoReg_SetBits(MDOMAIN_DISP_DDR_MainSubCtrl_reg, MDOMAIN_DISP_DDR_MainSubCtrl_disp2_double_apply_mask);
	}
...
void drvif_memory_rotate_mp(unsigned short MpIdx)
{
	...
	IoReg_Write32(MDOMAIN_CAP_DDR_In2Addr_reg, (tmpValue << 1) & 0xfffffff0);
...
static unsigned int memory_get_jpeg_capture_size(StructJPEGDisplayInfo *jpeg_info, eMemCapAccessType AccessType)
{
	...
	if ( jpeg_info == NULL ) {
		rtd_pr_scaler_memory_debug("[memory_get_jpeg_capture_size] jpeg_info cannot be NULL.\n");
		return 0;
	}
...
void drvif_memory_set_jpeg_disp_mem(StructJPEGDisplayInfo *jpeg_info)
{
	...
	//frank@0323 change waterlevel/2 for AT,because water level nit is 128 bits
	mdomain_disp_ddr_mainnumwtl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainNumWTL_reg);
	mdomain_disp_ddr_mainnumwtl_reg.main_pixel_num = jpeg_info->MemWid & 0x03FFF;//14 bit
	mdomain_disp_ddr_mainnumwtl_reg.main_wtlvl = ((fifoLen-(burstLen>>1))>>1);
	IoReg_Write32(MDOMAIN_DISP_DDR_MainNumWTL_reg, mdomain_disp_ddr_mainnumwtl_reg.regValue);

	mdomain_disp_ddr_mainlinenum_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainLineNum_reg);
	mdomain_disp_ddr_mainlinenum_reg.main_line_number = (jpeg_info->MemLen & 0x0FFF);
	IoReg_Write32(MDOMAIN_DISP_DDR_MainLineNum_reg, mdomain_disp_ddr_mainlinenum_reg.regValue);
	...
	// set memory address
//--	WaitFor_DEN_STOP();
	IoReg_Write32(MDOMAIN_DISP_DDR_MainAddr_reg, (jpeg_info->MemAddr)& 0xfffffff0);
}
...
//	unsigned int  nStartAddr;
	...
	IoReg_ClearBits(Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL) ? MDOMAIN_CAP_Cap_In2_enable_reg : MDOMAIN_CAP_Cap_In1_enable_reg, _BIT0);	// disable Capture
#else
	rtd_pr_scaler_memory_debug("[TV]: %s:%s(%d) addr=0x%x W=%d H=%d\n",__FILE__,__func__,__LINE__,CapBuf_addr,width,Hight);
	IoReg_ClearBits(/*Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL) ? MDOMAIN_CAP_DDR_In2Ctrl_reg : */MDOMAIN_CAP_DDR_In1Ctrl_reg, MDOMAIN_CAP_DDR_In1Ctrl_in1_double_enable_mask);	// disable Capture
	IoReg_ClearBits(/*Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL) ? MDOMAIN_CAP_DDR_In2Ctrl_reg : */MDOMAIN_CAP_Cap_In1_enable_reg, _BIT0);	// disable Capture
#endif
	//frank@0503 add below delay to confirm M capture have disable
...
#endif
	IoReg_Write32(MDOMAIN_CAP_DDR_In1Addr_reg, CapBuf_addr& 0xfffffff0);
	rtd_pr_scaler_memory_debug("[TV]: %s:%s(%d) write addr=0x%x\n",__FILE__,__func__,__LINE__,CapBuf_addr);
...
void drv_memory_set_3D_SBS_LR_swap(unsigned char lr_swap)
{
	...
	if(Get_PANEL_VFLIP_ENABLE() && Scaler_InputSrcGetMainChType() == _SRC_VO)
		lr_swap = !lr_swap;
	rtd_pr_scaler_memory_debug("drv_memory_set_3D_SBS_LR_swap , lr swap=%d\n",lr_swap);
	if(lr_swap==_TRUE)
	{
	...
	WaitFor_DEN_STOP();
	IoReg_Write32(MDOMAIN_CAP_DDR_In1Addr_reg , ddr_in1addr_reg.regValue & 0xfffffff0);
	IoReg_Write32(MDOMAIN_CAP_DDR_In1_2ndAddr_reg , ddr_in1_2ndaddr_reg.regValue & 0xfffffff0);
	if(Scaler_DispGetStatus((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY),SLR_DISP_VFLIP3))
		IoReg_Write32(MDOMAIN_CAP_DDR_In1_3rdAddr_reg , ddr_in1_3rdaddr_reg.regValue& 0xfffffff0);
	IoReg_Write32(MDOMAIN_CAP_CapMain_SBS_VSD_CAP_OFFSET_reg, capmain_sbs_vsd_cap_offset_reg.regValue);
...
void drv_memory_set_3D_TB_or_FP_LR_swap(unsigned char lr_swap)
{
	...
	if(lr_swap==_TRUE)
	{
		...
		TotalSize=(~TotalSize)+1;
		ddr_mainblkstep_reg.main_block_step=TotalSize;
		//rtd_pr_scaler_memory_debug("[23D]TT&B LR main_1=%x,second_1=%x,,third_1=%x,TotalSize=%x\n",ddr_mainaddr_reg.regValue,ddr_main2ndaddr_reg.regValue,ddr_main3rdaddr_reg.regValue,TotalSize);
	}
	else
	{

		//rtd_pr_scaler_memory_debug("[33D]TT&B LR main_1=%x,second_1=%x,,third_1=%x,TotalSize=%x\n",ddr_mainaddr_reg.regValue,ddr_main2ndaddr_reg.regValue,ddr_main3rdaddr_reg.regValue,TotalSize);
		...
		ddr_mainblkstep_reg.main_block_step=TotalSize;
		//rtd_pr_scaler_memory_debug("[43D]TT&B LR main_1=%x,second_1=%x,,third_1=%x,TotalSize=%x\n",ddr_mainaddr_reg.regValue,ddr_main2ndaddr_reg.regValue,ddr_main3rdaddr_reg.regValue,TotalSize);
	}
	rtd_pr_scaler_memory_debug("[3D]TT&B LR main_1=%x,second_1=%x,,third_1=%x,TotalSize=%x\n",ddr_mainaddr_reg.regValue,ddr_main2ndaddr_reg.regValue,ddr_main3rdaddr_reg.regValue,TotalSize);
	WaitFor_DEN_STOP();
	IoReg_Write32(MDOMAIN_DISP_DDR_MainAddr_reg , ddr_mainaddr_reg.regValue& 0xfffffff0);

	IoReg_Write32(MDOMAIN_DISP_DDR_Main2ndAddr_reg , ddr_main2ndaddr_reg.regValue& 0xfffffff0);
	...
	if ((Get_PANEL_VFLIP_ENABLE() && Scaler_DispGetStatus((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY),SLR_DISP_VFLIP3)))
		IoReg_Write32(MDOMAIN_DISP_DDR_Main3rdAddr_reg , (ddr_main3rdaddr_reg.regValue)& 0xfffffff0);
...
void drvif_memory_set_MVC_FRC_LR_swap(unsigned char bInvMode)
{
	...
	rtd_pr_scaler_memory_debug("[SG] MVC FRC L/R Swap=%d\n", ucLRseq);
...
void drvif_memory_block_mode_enable(bool enable)
{
	if(enable == TRUE){
		...
		rtd_pr_scaler_memory_debug("@@@@@Enable block mode@@@@@\n");
	}
	else{
		IoReg_ClearBits(MDOMAIN_DISP_DDR_Main_BM_DMA_reg, _BIT0);
		rtd_pr_scaler_memory_debug("@@@@@Disable block mode & run sequence mode@@@@@\n");
	}
}
...
void drv_memory_Main_Disp_Enable(bool enable)
{
	...
	if(enable == TRUE){
		...
		{
			ddr_mainsubctrl_reg.regValue =IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg);
			while(ddr_mainsubctrl_reg.disp1_double_apply && apply_timeout_cnt--){
				usleep_range(10000, 10000);// msleep(10);
				ddr_mainsubctrl_reg.regValue =IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg);
			}
			if(!apply_timeout_cnt)
				rtd_pr_scaler_memory_err("#####func:%s using db to disable wait timeout#####\r\n", __FUNCTION__);
			else
				rtd_pr_scaler_memory_info("###func:%s using db to disable ####\r\n", __FUNCTION__);
		}
...
void dvrif_memory_comp_setting(UINT8 enable_compression, UINT16 comp_wid, UINT16 comp_len, UINT8 comp_ratio){
	...
	mdomain_cap_cap_pq_cmp_qp_st_RBUS mdomain_cap_cap_pq_cmp_qp_st_reg;
	mdomain_cap_cap_pq_cmp_fifo_st1_RBUS mdomain_cap_cap_pq_cmp_fifo_st1_reg;
	...
	if(enable_compression == TRUE){
		...
		*/
		...
		mdomain_cap_cap_pq_cmp_bit_reg.first_line_more_bit = 0x20;
		...
		if ((Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP) == FALSE)){
			...
			mdomain_cap_cap_pq_cmp_pair_reg.cmp_data_format = 0; // 422
		}

		mdomain_cap_cap_pq_cmp_pair_reg.cmp_data_color = 1;
		mdomain_cap_cap_pq_cmp_pair_reg.cmp_pair_para = 1;
		...
		if ((Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP) == FALSE)){
			if(pc_mode_run_422())
			{
				mdomain_cap_cap_pq_cmp_enable_reg.g_ratio = 16;
			...
			mdomain_cap_cap_pq_cmp_enable_reg.do_422_mode = 0;//422 format
		}
		...
		mdomain_cap_cap_pq_cmp_enable_reg.variation_maxmin_en = 0;
		...
		mdomain_cap_cap_pq_cmp_enable_reg.cmp_ctrl_para0 = 0x10;
		...
		mdomain_cap_cap_pq_cmp_allocate_reg.dynamic_allocate_less = 2;
		mdomain_cap_cap_pq_cmp_allocate_reg.dynamic_allocate_more = 2;
		...
		mdomain_cap_cap_pq_cmp_balance_reg.balance_rb_ov_th = 0;
		mdomain_cap_cap_pq_cmp_balance_reg.balance_rb_ud_th = 3;
		...
		mdomain_cap_cap_pq_cmp_qp_st_reg.regValue = IoReg_Read32(MDOMAIN_CAP_CAP_PQ_CMP_ST_reg);
		mdomain_cap_cap_pq_cmp_qp_st_reg.cmp_ctrl_para1 = 0x44;
		mdomain_cap_cap_pq_cmp_qp_st_reg.cmp_ctrl_para2 = 0xD0;
		IoReg_Write32(MDOMAIN_CAP_CAP_PQ_CMP_ST_reg, mdomain_cap_cap_pq_cmp_qp_st_reg.regValue);

		mdomain_cap_cap_pq_cmp_fifo_st1_reg.regValue = IoReg_Read32(MDOMAIN_CAP_CAP_PQ_CMP_FIFO_ST1_reg);
		mdomain_cap_cap_pq_cmp_fifo_st1_reg.cmp_ctrl_para3 = 0x0;
		IoReg_Write32(MDOMAIN_CAP_CAP_PQ_CMP_FIFO_ST1_reg, mdomain_cap_cap_pq_cmp_fifo_st1_reg.regValue);
		...
		if ((Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP) == FALSE)){
			...
			mdomain_disp_dispm_main_pq_decmp_pair_reg.decmp_data_format = 0; //422
		}
		...
		mdomain_disp_dispm_main_pq_decmp_pair_reg.decmp_pair_para = 1;
		...
		mdomain_disp_dispm_main_pq_decmp_sat_en_reg.decmp_ctrl_para = 4;
...
void dvrif_memory_comp_setting_sub(UINT8 enable_compression, UINT16 comp_wid, UINT16 comp_len, UINT8 comp_ratio){
	...
	if(enable_compression == TRUE){
		...
		*/
		...
		mdomain_cap_caps_pq_cmp_bit_reg.s_first_line_more_bit = 0x20;
		...
		if(Scaler_DispGetStatus(SLR_SUB_DISPLAY, SLR_DISP_422CAP) == FALSE){
			...
			mdomain_cap_caps_pq_cmp_pair_reg.s_cmp_data_format = 0; // 422
		}

		mdomain_cap_caps_pq_cmp_pair_reg.s_cmp_data_color = 1;
		mdomain_cap_caps_pq_cmp_pair_reg.s_cmp_pair_para = 1;
		...
		if ((Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP) == FALSE)){
			if(pc_mode_run_422())
			{
				mdomain_cap_caps_pq_cmp_enable_reg.s_g_ratio = 16;
			...
			mdomain_cap_caps_pq_cmp_enable_reg.s_do_422_mode = 0;//422 format
		}
		...
		mdomain_cap_caps_pq_cmp_enable_reg.s_variation_maxmin_en = 0;
		...
		mdomain_cap_caps_pq_cmp_enable_reg.s_cmp_ctrl_para0 = 5;
		...
		mdomain_cap_caps_pq_cmp_allocate_reg.s_dynamic_allocate_less = 2;
		mdomain_cap_caps_pq_cmp_allocate_reg.s_dynamic_allocate_more = 2;
		...
		mdomain_cap_caps_pq_cmp_balance_reg.s_balance_rb_ov_th = 0;
		mdomain_cap_caps_pq_cmp_balance_reg.s_balance_rb_ud_th = 3;
		...
		if(Scaler_DispGetStatus(SLR_SUB_DISPLAY, SLR_DISP_422CAP) == FALSE){
			...
			mdomain_disp_dispm_sub_pq_decmp_pair_reg.s_decmp_data_format = 0;//422
		}
		...
		mdomain_disp_dispm_sub_pq_decmp_pair_reg.s_decmp_pair_para = 1;
		...
		mdomain_disp_dispm_sub_pq_decmp_sat_en_reg.s_decmp_ctrl_para = 1;
...
//fixme:vip
	...
	*/
	...
	if (access == SLR_DI_5A_mode) {
		...
		if (isCompression_Mode != PQC_Disable)
			data_r.length = 30;	// for outstanding enable.
	...
	//rtd_pr_scaler_memory_debug("\n Data BitNum=%d, Info BitNum=%d", data_bitNum, info_bitNum);
	//rtd_pr_scaler_memory_emerg("\n Data BitNum=%d, Info BitNum=%d", data_bitNum, info_bitNum);
        rtd_pr_scaler_memory_debug("\n data_num=%d, info_num=%d, DIAllocateBlock=%d\n", data_num, info_num, DIAllocateBlock);
	...
	ddr_rdatdma_wtlennum.dat_len_r		= data_r.length;
	ddr_rdatdma_wtlennum.dat_num_r		= data_r.number;
	if (isCompression_Mode != PQC_Disable)	// enable outstanding in pqc on
		ddr_rdatdma_wtlennum.dma_outstanding_en = 1;
	else
		ddr_rdatdma_wtlennum.dma_outstanding_en = 0;
	ddr_wdatdma_wtlennum.dat_water_w	= data_w.water_level;
	ddr_wdatdma_wtlennum.dat_len_w		= data_w.length;
	ddr_wdatdma_wtlennum.dat_num_w		= data_w.number;
...
//		value =(info->IPV_ACT_LEN_PRE *info->IPH_ACT_WID_PRE * bitNum)>>4;
	...
	if (aligned_val != 0) {
		...
		data_size_aligned = drvif_memory_get_data_align(data_size, 16);
	...
	for (i=0;i<4;i++) {
		if (aligned_val != 0) {
			...
			MemAddr_aligned[i] = drvif_memory_get_data_align(MemAddr[i], 16);
		}
	}
	...
	return MemStartAddr;
#endif
//#endif  //compiler error
        return 0; // please remove it
...
{
	ipq_decontour_bdecont_dma_rd_ctrl_RBUS 			bdecont_dma_rd_ctrl_reg;
	ipq_decontour_bdecont_dma_wr_ctrl_RBUS 			bdecont_dma_wr_ctrl_reg;	
	ipq_decontour_bdecont_datmemorystart_RBUS 		bdecont_datmemorystart_reg;
	ipq_decontour_bdecont_wddr_dma_wtlennum_RBUS 	bdecont_wddr_dma_wtlennum_reg;
	ipq_decontour_bdecont_wddr_datdma_rm_RBUS 		bdecont_wddr_datdma_rm_reg;
	ipq_decontour_bdecont_rddr_dma_wtlennum_RBUS 	bdecont_rddr_dma_wtlennum_reg;
	ipq_decontour_bdecont_rddr_datdma_rm_RBUS 		bdecont_rddr_datdma_rm_reg;
	ipq_decontour_bdecont_dma_rule_check_up_RBUS 	bdecont_dma_rule_check_up_reg;
	ipq_decontour_bdecont_dma_rule_check_low_RBUS 	bdecont_dma_rule_check_low_reg;
	ipq_decontour_de_contour_db_reg_ctl_RBUS 		ipq_decontour_de_contour_db_reg_ctl_reg;
	//nr_spnr_act_width_RBUS 							nr_spnr_act_width_reg;
	//nr_spnr_act_length_RBUS 						nr_spnr_act_length_reg;
	...
	//unsigned int height = Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN);
	//unsigned int width = Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID);
	unsigned int bus_rw_bits=128; // h5c2 256, ml8 128
	unsigned int info_bit;
	unsigned int rw_num=0;
	unsigned short len, remain, num;
	unsigned int mem, mem_aligned, mem_end;
	unsigned int /*buffer=0, */buffer_aligned;
	//unsigned short aligned_val;
	unsigned int blk_h, blk_w;
	#define BLK_DECONT_DMA_SIZE (28*1024) // ((blk_h * blk_w * info_bit)>>3) = ((103*54*40)>>3) = 27810

	fwif_color_Set_Block_DeContour_Size();
	
	// enable DB
	IoReg_SetBits(IPQ_DECONTOUR_De_Contour_db_reg_ctl_reg, IPQ_DECONTOUR_De_Contour_db_reg_ctl_db_en_mask);

	bdecont_dma_rd_ctrl_reg.regValue 		= IoReg_Read32(IPQ_DECONTOUR_BDECONT_DMA_RD_Ctrl_reg);
	bdecont_dma_wr_ctrl_reg.regValue 		= IoReg_Read32(IPQ_DECONTOUR_BDECONT_DMA_WR_Ctrl_reg);
	bdecont_datmemorystart_reg.regValue 	= IoReg_Read32(IPQ_DECONTOUR_BDECONT_DATMemoryStart_reg);
	bdecont_wddr_dma_wtlennum_reg.regValue 	= IoReg_Read32(IPQ_DECONTOUR_BDECONT_WDDR_DMA_WTLENNUM_reg);
	bdecont_wddr_datdma_rm_reg.regValue 	= IoReg_Read32(IPQ_DECONTOUR_BDECONT_WDDR_DATDMA_RM_reg);
	bdecont_rddr_dma_wtlennum_reg.regValue 	= IoReg_Read32(IPQ_DECONTOUR_BDECONT_RDDR_DMA_WTLENNUM_reg);
	bdecont_rddr_datdma_rm_reg.regValue 	= IoReg_Read32(IPQ_DECONTOUR_BDECONT_RDDR_DATDMA_RM_reg);		
	bdecont_dma_rule_check_up_reg.regValue 	= IoReg_Read32(IPQ_DECONTOUR_BDECONT_DMA_Rule_check_up_reg);
	bdecont_dma_rule_check_low_reg.regValue = IoReg_Read32(IPQ_DECONTOUR_BDECONT_DMA_Rule_check_low_reg);		
	//nr_spnr_act_width_reg.regValue 			= IoReg_Read32(NR_SPNR_ACT_Width_reg);
	//nr_spnr_act_length_reg.regValue 		= IoReg_Read32(NR_SPNR_ACT_Length_reg);
	...
	// buffer size
	blk_h = blk_de_contour_size0_reg.bdecont_blk_hnum;
	blk_w = blk_de_contour_size1_reg.bdecont_blk_vnum;
	info_bit = 40;
#if 0
	buffer = ((blk_h * blk_w * info_bit)>>3) + 1024;
	//aligned_val = 0; //h5x no 96 alignment
	aligned_val = drv_memory_get_DMA_Aligned_Val();

	if (aligned_val != 0) {
		buffer_aligned = dvr_size_alignment(buffer);
	} else {
		buffer_aligned = drvif_memory_get_data_align(buffer, 16);
	}

	//buffer_aligned = buffer;
	mem = (phyAddr);
	mem_end = phyAddr + buffer_aligned;

	if (aligned_val != 0) {
		mem_aligned = dvr_memory_alignment((unsigned long)mem, buffer_aligned);		

	} else {
		mem_aligned = drvif_memory_get_data_align(mem, 16);		
	}

	//mem = ((mem % 16)==0)?(mem):(mem + (16 - (mem % 16)));	// need 16 aligned to avoid DCMT
	//mem_aligned = mem;
#else
	buffer_aligned = BLK_DECONT_DMA_SIZE;
	mem = (phyAddr);
	mem_end = phyAddr + buffer_aligned;
	mem_aligned = mem;
#endif	

	//bdecont_datmemorystart_reg.regValue = mem_aligned;
	bdecont_datmemorystart_reg.mstart = mem_aligned>>4;

	// boundary 
	bdecont_dma_rule_check_up_reg.dma_up_limit = mem_end>>4;
	bdecont_dma_rule_check_low_reg.dma_low_limit = mem_aligned>>4; 

	// water/ len	
	rw_num = ((blk_w*blk_h*info_bit)%bus_rw_bits)?((blk_w*blk_h*info_bit)/bus_rw_bits+1):((blk_w*blk_h*info_bit)/bus_rw_bits);
	len = 16;
	num = rw_num / len;
	remain = rw_num - len * num;
	if (remain == 0) { /* remain can't be 0*/
		num -= 1;
		remain = len;
	}

	bdecont_wddr_dma_wtlennum_reg.wr_dat_len = len;
	bdecont_wddr_dma_wtlennum_reg.wr_dat_water = len; // wr_water = len
	bdecont_wddr_dma_wtlennum_reg.wr_dat_num = num;
	bdecont_wddr_datdma_rm_reg.wr_dat_remain = remain;
	
	bdecont_rddr_dma_wtlennum_reg.rd_dat_len = len;
	bdecont_rddr_dma_wtlennum_reg.rd_dat_water = 32-len; // rd_water = fifo_depth - len
	bdecont_rddr_dma_wtlennum_reg.rd_dat_num = num;
	bdecont_rddr_datdma_rm_reg.rd_dat_remain = remain;
	bdecont_rddr_datdma_rm_reg.rd_dma_remain_en = 1;	

	// image h/w
	//nr_spnr_act_length_reg.spnr_image_height	= height;
	//nr_spnr_act_width_reg.spnr_image_width	= width;
	//if(Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_THRIP) == TRUE)//i mode
	//	nr_spnr_act_length_reg.spnr_image_height = nr_spnr_act_length_reg.spnr_image_height<<1;

	// dma enable
	bdecont_dma_rd_ctrl_reg.rd_dma_enable = 1;
	bdecont_dma_wr_ctrl_reg.wr_dma_enable = 1;
	
	IoReg_Write32(IPQ_DECONTOUR_BDECONT_DMA_RD_Ctrl_reg, bdecont_dma_rd_ctrl_reg.regValue);
	IoReg_Write32(IPQ_DECONTOUR_BDECONT_DMA_WR_Ctrl_reg, bdecont_dma_wr_ctrl_reg.regValue);
	IoReg_Write32(IPQ_DECONTOUR_BDECONT_DATMemoryStart_reg, bdecont_datmemorystart_reg.regValue);
	IoReg_Write32(IPQ_DECONTOUR_BDECONT_WDDR_DMA_WTLENNUM_reg, bdecont_wddr_dma_wtlennum_reg.regValue);
	IoReg_Write32(IPQ_DECONTOUR_BDECONT_WDDR_DATDMA_RM_reg, bdecont_wddr_datdma_rm_reg.regValue);
	IoReg_Write32(IPQ_DECONTOUR_BDECONT_RDDR_DMA_WTLENNUM_reg, bdecont_rddr_dma_wtlennum_reg.regValue);
	IoReg_Write32(IPQ_DECONTOUR_BDECONT_RDDR_DATDMA_RM_reg, bdecont_rddr_datdma_rm_reg.regValue);
	IoReg_Write32(IPQ_DECONTOUR_BDECONT_DMA_Rule_check_up_reg, bdecont_dma_rule_check_up_reg.regValue);
	IoReg_Write32(IPQ_DECONTOUR_BDECONT_DMA_Rule_check_low_reg, bdecont_dma_rule_check_low_reg.regValue);
	//IoReg_Write32(NR_SPNR_ACT_Width_reg, nr_spnr_act_width_reg.regValue);
	//IoReg_Write32(NR_SPNR_ACT_Length_reg, nr_spnr_act_length_reg.regValue);
	...
	drv_memory_Set_I_BLK_decontour_Mem_Ready_Flag(0, 1);
	return mem_end;
}
...
unsigned int drv_memory_MA_SNR_Config(unsigned char access ,unsigned int phyAddr, unsigned char DI_MEM_IDX)
{
	...
	if (drv_memory_MA_SNR_Enable_Check(access, phyAddr, DI_MEM_IDX, width, height) == 0) {
		...
		// Merlin6: decontour use SPNR_image_width instead, remove this part, jimmy 20200406
		/*
	...
	if(access == SLR_DI_5A_mode)
	{
		...
		if (aligned_val != 0) {
			...
			mem_aligned[0] = drvif_memory_get_data_align(mem[0], 16);
			mem_aligned[1] = drvif_memory_get_data_align(mem[1], 16);
			mem_aligned[2] = drvif_memory_get_data_align(mem[2], 16);
			mem_aligned[3] = drvif_memory_get_data_align(mem[3], 16);
		}
	}
	else
	{
...
char drv_memory_MA_SNR_Enable_Check(unsigned char access ,unsigned int phyAddr, unsigned char DI_MEM_IDX, unsigned int DI_Width, unsigned int DI_Height)
{
	...
	adaptvie_stream_flag = get_vsc_run_adaptive_stream(SLR_MAIN_DISPLAY);
	...
	if ((decide_by_AP == 0) || ((vsc_src == VSC_INPUTSRC_VDEC || vsc_src == VSC_INPUTSRC_CAMERA) && (adaptvie_stream_flag >= ADAPTIVE_STREAM_ON)) || ((DISP_THRIP  == FALSE) && (DISP_RTNR == FALSE))) {
		ret = 0;
	} else {
		ret = 1;
	}
...
void drv_memory_wait_cap_last_write_done(unsigned char display, unsigned char wait_frame, unsigned char game_mode_case){
	...
	if(display == SLR_MAIN_DISPLAY){
		do{
    		...
    		//rtd_pr_scaler_memory_notice("M domain main done-0, cap pre idx = %d\n", pre_cap_idx);
        	...
        	while(!(MDOMAIN_CAP_Cap_DDR_FIFOStatus_get_in1_cap_last_wr_flag(IoReg_Read32(MDOMAIN_CAP_Cap_DDR_FIFOStatus_reg))) && --count){
				...
				{
					rtd_pr_scaler_memory_notice("drv_memory_wait_cap_last_write_done game mode case not active return\n");
					return;
				}
#ifdef BUILD_QUICK_SHOW
    		...
    		rtd_pr_scaler_memory_notice("M domain main done-1, cap pre index = %d new idx = %d count:%d\n", pre_cap_idx, cur_cap_idx, count);

			if(count == 0){
                #ifdef BUILD_QUICK_SHOW
        		printf2("M domain main timeout-1 !\n");
                #else
        		rtd_pr_scaler_memory_err("M domain main timeout-1 !\n");
                #endif
            }
    		...
    		if(MDOMAIN_CAP_DDR_In1Status_get_in1_block_select(IoReg_Read32(MDOMAIN_CAP_DDR_In1Status_reg)) == pre_cap_idx)
    		{
				...
				cur_cap_idx = MDOMAIN_CAP_DDR_In1Status_get_in1_block_select(IoReg_Read32(MDOMAIN_CAP_DDR_In1Status_reg));
        		rtd_pr_scaler_memory_notice("M domain main done-2(check again), cap new idx = %d count:%d\n", cur_cap_idx, count);
        		if(count == 0)
            		rtd_pr_scaler_memory_err("M domain main timeout-2 !\n");
    		}
			wait_frame--;
		...
		do{
            ...
            if(count != 0) {
				while((MDOMAIN_DISP_DDR_MainSubCtrl_get_disp2_double_apply(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg))) && --count) {
#ifdef BUILD_QUICK_SHOW
                                           msleep(10);//no busy delay 10 ms
#else
                                       msleep(0);//no busy delay 10 ms
			...
			cur_cap_idx = MDOMAIN_CAP_DDR_In2Status_get_in2_block_select(IoReg_Read32(MDOMAIN_CAP_DDR_In2Status_reg));
			rtd_pr_scaler_memory_notice("M domain sub done-1, cap pre index = %d new idx = %d count:%d mdisp apply:%d\n", pre_cap_idx, cur_cap_idx, count, (MDOMAIN_DISP_DDR_MainSubCtrl_get_disp2_double_apply(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg))));
            
    		if(count == 0)
        		rtd_pr_scaler_memory_err("M domain sub timeout1 !\n");
    		...
    		if(MDOMAIN_CAP_DDR_In2Status_get_in2_block_select(IoReg_Read32(MDOMAIN_CAP_DDR_In2Status_reg)) == pre_cap_idx)
    		{
                ...
                cur_cap_idx = MDOMAIN_CAP_DDR_In2Status_get_in2_block_select(IoReg_Read32(MDOMAIN_CAP_DDR_In2Status_reg));
                rtd_pr_scaler_memory_notice("M domain sub done-2(check again), cap new idx = %d count:%d\n", cur_cap_idx, count);
...
void drv_memory_wait_mdom_vi_last_write_done(unsigned char display, unsigned char wait_frame){
	...
	if(display == SLR_MAIN_DISPLAY){
		do{
			...
			if(count == 0)
        		rtd_pr_scaler_memory_err("M domain vi main timeout-1 !\n");
        	...
        	if(count == 1)
            	rtd_pr_scaler_memory_err("M domain vi main timeout-2 !\n");
    		}
			wait_frame--;
...
unsigned char drv_memory_get_low_delay_game_mode_dynamic(void)
{
	...
	low_delay_game = (drv_memory_get_game_mode_dynamic() || ((get_vsc_src_is_hdmi_or_dp()) && (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())));
...
void drv_memory_SingleBuffer_GameMode(unsigned char enable){
	...
	IoReg_Write32(MDOMAIN_CAP_DDR_In1Ctrl_reg, mdomain_cap_ddr_in1ctrl_Reg.regValue);
	IoReg_Write32(MDOMAIN_DISP_DDR_MainCtrl_reg, mdomain_disp_ddr_mainctrl_Reg.regValue);
	rtd_pr_scaler_memory_debug("[Game Mode]M-domain game mode done enable=%d!!\n",enable);
}
...
{//enable: true = single   false: triple  4k120 case
    ...
    mdomain_cap_cap_reg_doublbuffer_RBUS cap_reg_doublbuffer_reg;
    mdomain_disp_ddr_mainsubctrl_RBUS ddr_mainsubctrl_reg;
    mdomain_cap_ddr_in2ctrl_RBUS mdomain_cap_ddr_in1ctr2_reg;
    mdomain_disp_ddr_subctrl_RBUS mdomain_disp_ddr_subctrl_reg;
    mdomain_cap_cap_ddr_fifostatus_RBUS cap_ddr_fifostatus_reg;
    ...
    {//one buffer
        ...
        cap_db_en = cap_reg_doublbuffer_reg.cap2_db_en;
    ...
    /////////////////////////////////////////////////////////////////////////////////////////
        ...
        mdomain_disp_ddr_subctrl_reg.sub_double_en = 0;
        mdomain_disp_ddr_subctrl_reg.sub_v_flip_3buf_en = 0;
        mdomain_disp_ddr_subctrl_reg.sub_single_buffer_select = 2;
        IoReg_Write32(MDOMAIN_DISP_DDR_SubCtrl_reg, mdomain_disp_ddr_subctrl_reg.regValue);


        ddr_mainsubctrl_reg.disp2_double_apply = 1;
        IoReg_Write32(MDOMAIN_DISP_DDR_MainSubCtrl_reg,ddr_mainsubctrl_reg.regValue);
        while((ddr_mainsubctrl_reg.disp2_double_apply == 1) && (timeout--)){
            ddr_mainsubctrl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg);
        }
        ...
        if(!get_hdmi_vrr_4k60_mode()){
            //change M-cap
            mdomain_cap_ddr_in1ctr2_reg.regValue = IoReg_Read32(MDOMAIN_CAP_DDR_In2Ctrl_reg);
            mdomain_cap_ddr_in1ctr2_reg.in2_v_flip_3buf_en = 0;
            mdomain_cap_ddr_in1ctr2_reg.in2_double_enable = 0;
            mdomain_cap_ddr_in1ctr2_reg.in2_single_buffer_select = 2;
            IoReg_Write32(MDOMAIN_CAP_DDR_In2Ctrl_reg, mdomain_cap_ddr_in1ctr2_reg.regValue);

            cap_reg_doublbuffer_reg.cap2_db_apply = 1;
            IoReg_Write32(MDOMAIN_CAP_cap_reg_doublbuffer_reg,cap_reg_doublbuffer_reg.regValue);
            while((cap_reg_doublbuffer_reg.cap2_db_apply == 1) && (timeout--)){
                cap_reg_doublbuffer_reg.regValue = IoReg_Read32(MDOMAIN_CAP_cap_reg_doublbuffer_reg);
            }
        }
        ...
        if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_SEARCH) {
            ...
            cap_reg_doublbuffer_reg.cap2_db_en = cap_db_en;
        }
        IoReg_Write32(MDOMAIN_CAP_cap_reg_doublbuffer_reg, cap_reg_doublbuffer_reg.regValue);
    }
    else
    {//three buffer
        ...
        if(!get_hdmi_vrr_4k60_mode()){
            ...
            mdomain_cap_ddr_in1ctr2_reg.regValue = IoReg_Read32(MDOMAIN_CAP_DDR_In2Ctrl_reg);
	        mdomain_cap_ddr_in1ctr2_reg.in2_v_flip_3buf_en = 1;
            mdomain_cap_ddr_in1ctr2_reg.in2_double_enable = 1;
            mdomain_cap_ddr_in1ctr2_reg.in2_single_buffer_select = 0;
            IoReg_Write32(MDOMAIN_CAP_DDR_In2Ctrl_reg, mdomain_cap_ddr_in1ctr2_reg.regValue);
        ...
        cap_ddr_fifostatus_reg.regValue = IoReg_Read32(MDOMAIN_CAP_Cap_DDR_FIFOStatus_reg);
        while(!(cap_ddr_fifostatus_reg.in2_cap_last_wr_flag) && --timeout){
            cap_ddr_fifostatus_reg.regValue = IoReg_Read32(MDOMAIN_CAP_Cap_DDR_FIFOStatus_reg);
        }
        ...
        mdomain_disp_ddr_subctrl_reg.sub_double_en = 1;
        mdomain_disp_ddr_subctrl_reg.sub_v_flip_3buf_en = 1;
        mdomain_disp_ddr_subctrl_reg.sub_single_buffer_select = 0;
        IoReg_Write32(MDOMAIN_DISP_DDR_SubCtrl_reg, mdomain_disp_ddr_subctrl_reg.regValue);
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	if((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) ||(get_hdmi_vrr_4k60_mode()))
    {
        if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE)
        {
            drv_memory_gamemode_sub_switch_single_buffer(TRUE);//sub switch single buffer
        }
    }
#if 0
...
//	ppoverlay_double_buffer_ctrl_RBUS double_buffer_ctrl_reg;
	...
	if((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) ||(get_hdmi_vrr_4k60_mode()))
    {
        if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE)
        {
            drv_memory_gamemode_sub_switch_single_buffer(FALSE);//sub switch 3 buffer
        }
    }
#if 0
	//resume M-disp double buffer setting
...
unsigned int drv_GameMode_iv2dv_delay_compress_margin(unsigned int iv2dv_ori){
	...
	iv2dv_ori = iv2dv_ori + ((_FIFOLENGTH * _BURSTLENGTH / dvrif_memory_get_compression_bits(SLR_MAIN_DISPLAY)) / active_size) + MDOMAIN_COMPRESSION_MARGIN;
...
//	ppoverlay_fs_iv_dv_fine_tuning5_RBUS fs_iv_dv_fine_tuning5_reg;
	...
	UINT32 orbit_front;
	UINT32 snrVstart;
	UINT32 hdmi_vfreq = Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ);
	...
	UINT32 vfreq = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ);
	...
	unsigned char scaler_input_hdmi = FALSE;//scaler directly input main vgip
	...
	scaler_input_hdmi = ((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) || (get_hdmi_4k_hfr_mode() == HDMI_3K120));

	if(scaler_input_hdmi)
	{
		if((hdmi_vfreq == 0)|| (hdmi_vtotal == 0) || (hdmi_htotal == 0))
		{
	...
	if(!get_force_i3ddma_enable(SLR_MAIN_DISPLAY))
	{
		if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) &&
			((hdmi_htotal != 0) && (hdmi_vtotal != 0) && (hdmi_vfreq != 0))){
				voClock = hdmi_htotal * hdmi_vtotal * (hdmi_vfreq/10);
		}
		else{
			voClock = htotal * vtotal * (vfreq/10);
		}
	}
	else{
	...
	if((Get_DISPLAY_REFRESH_RATE()>=120)&& (PPOVERLAY_uzudtg_control1_get_uzu_4k_120_mode(rtd_inl(PPOVERLAY_uzudtg_control1_reg)))){
...
unsigned int drv_GameMode_decided_iv2dv_delay_new_mode(void)
{
	...
	//rtd_pr_scaler_memory_emerg("[ben]drv_GameMode_decided_iv2dv_delay_new_mode \n");
...
#else
		if((Get_DISPLAY_REFRESH_RATE() >= 120) && (get_vsc_src_is_hdmi_or_dp()) && (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())){
			return;
		}
...
void drv_memory_SingleBuffer_GameMode_Dynamic(unsigned char enable){
    ...
    if(vbe_disp_gamemode_use_fixlastline()){
...
#endif
		rtd_pr_scaler_memory_debug("[Game Mode]M-domain game mode enable!!\n");
	}
	else{
#if 0
		...
		if(!((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC || Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_CAMERA) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_IPH_ACT_WID_PRE) > DATAFS_DISABLE_GATTING_WIDTH) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_IPV_ACT_LEN_PRE) > DATAFS_DISABLE_GATTING_HEIGHT)))
#endif
		...
		rtd_pr_scaler_memory_debug("[Game Mode]M-domain game mode disable!!\n");
	}
...
//	unsigned long flags;//for spin_lock_irqsave
	...
	if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_FRAMESYNC)
		...
		|| (((get_vsc_src_is_hdmi_or_dp()) && (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag()))==1)){
		return;
	}
...
void drv_memory_capture_multibuffer_update_addr(unsigned int buffer_size, unsigned int start_addr){
	...
	if(drv_memory_Get_multibuffer_number() > 3){
		//rtd_pr_scaler_memory_emerg("[crixus]buffer_size = %x\n", buffer_size);
		...
		#ifdef CONFIG_ARM64
		rtd_pr_scaler_memory_emerg("[crixus]MemTag[MEMIDX_MAIN_FOURTH].StartAddr = %lx\n", MemTag[MEMIDX_MAIN_FOURTH].StartAddr);
		#else
		rtd_pr_scaler_memory_emerg("[crixus]MemTag[MEMIDX_MAIN_FOURTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_FOURTH].StartAddr);
		#endif
	}
	...
	if(drv_memory_Get_multibuffer_number() > 4){
		...
		#ifdef CONFIG_ARM64
		rtd_pr_scaler_memory_emerg("[crixus]MemTag[MEMIDX_MAIN_FIFTH].StartAddr = %lx\n", MemTag[MEMIDX_MAIN_FIFTH].StartAddr);
		#else
		rtd_pr_scaler_memory_emerg("[crixus]MemTag[MEMIDX_MAIN_FIFTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_FIFTH].StartAddr);
		#endif
	}
	...
	if(drv_memory_Get_multibuffer_number() > 5){
		...
		#ifdef CONFIG_ARM64
		rtd_pr_scaler_memory_emerg("[crixus]MemTag[MEMIDX_MAIN_SIXTH].StartAddr = %lx\n", MemTag[MEMIDX_MAIN_SIXTH].StartAddr);
		#else
		rtd_pr_scaler_memory_emerg("[crixus]MemTag[MEMIDX_MAIN_SIXTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_SIXTH].StartAddr);
		#endif
	}
	...
	if(drv_memory_Get_multibuffer_number() > 6){
		...
		#ifdef CONFIG_ARM64
		rtd_pr_scaler_memory_emerg("[crixus]MemTag[MEMIDX_MAIN_SEVENTH].StartAddr = %lx\n", MemTag[MEMIDX_MAIN_SEVENTH].StartAddr);
		#else
		rtd_pr_scaler_memory_emerg("[crixus]MemTag[MEMIDX_MAIN_SEVENTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_SEVENTH].StartAddr);
		#endif
	}
	...
	if(drv_memory_Get_multibuffer_number() > 7){
		...
		#ifdef CONFIG_ARM64
		rtd_pr_scaler_memory_emerg("[crixus]MemTag[MEMIDX_MAIN_EIGHTH].StartAddr = %lx\n", MemTag[MEMIDX_MAIN_EIGHTH].StartAddr);
		#else
		rtd_pr_scaler_memory_emerg("[crixus]MemTag[MEMIDX_MAIN_EIGHTH].StartAddr = %x\n", MemTag[MEMIDX_MAIN_EIGHTH].StartAddr);
		#endif
	}
}
...
void drv_memory_capture_multibuffer_update_boundary(unsigned char buf_num, unsigned int buffer_size){
	IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr1_reg,(MemTag[MEMIDX_MAIN_THIRD].StartAddr& 0xfffffff0));
	if(buf_num>3)
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg,(MemTag[MEMIDX_MAIN + (buf_num - 1)].StartAddr+buffer_size-CONFIG_MDOMAIN_BURST_SIZE)& 0xfffffff0);
	else
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg,(MemTag[MEMIDX_MAIN].StartAddr+buffer_size-CONFIG_MDOMAIN_BURST_SIZE)& 0xfffffff0);
}
...
void drv_memory_capture_multibuffer_control(unsigned char buf_num, unsigned char disp2cap_distance){
	...
	//rtd_pr_scaler_memory_emerg("[crixus]buf_num = %x\n", buf_num);
	//rtd_pr_scaler_memory_emerg("[crixus]disp2cap_distance = %x\n", disp2cap_distance);
...
//	mdomain_cap_ddr_in1ctrl_reg.regValue=IoReg_Read32(MDOMAIN_CAP_DDR_In1Status_reg);
	...
	rtd_pr_scaler_memory_debug("Captblock=%d,Dispblock=%d\n",mdomain_cap_ddr_in1status_reg.in1_block_select,mdomain_disp_ddr_mainstatus_reg.main_block_select);
...
void drv_memory_display_multibuffer_switch_buffer_number(void)
{
	...
	if(mdomain_cap_ddr_in1status_reg.in1_block_select ==(buf_num-1))
	{
		...
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr1_reg, IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr1_reg)& 0xfffffff0);
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr2_reg, IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg)& 0xfffffff0);
...
void dvrif_memory_main_comp_sub_decomp_setting(UINT8 enable_compression, UINT16 comp_wid, UINT16 comp_len, UINT8 comp_ratio, UINT8 cap_type){
	...
	rtd_pr_scaler_memory_emerg("[crixus][%s]enable_compression = %d, comp_wid = %d, comp_len = %d, comp_ratio = %d\n", __FUNCTION__, enable_compression, comp_wid, comp_len, comp_ratio);
...
static void memory_set_capture_i2rnd_sdisp(unsigned char cap_type)
{
	...
	IoReg_Mask32(MDOMAIN_DISP_DDR_SubLineNum_reg, 0xfffff000, (Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_LEN) & 0x0FFF));
	IoReg_Write32(MDOMAIN_DISP_DDR_SubAddr_reg, (MemTag[MEMIDX_MAIN_2].StartAddr + MemShiftAddr)& 0xfffffff0);
	IoReg_Write32(MDOMAIN_DISP_DDR_Sub2ndAddr_reg, (MemTag[MEMIDX_MAIN_2_SEC].StartAddr + MemShiftAddr)& 0xfffffff0);
	IoReg_Write32(MDOMAIN_DISP_DDR_Sub3rdAddr_reg, (MemTag[MEMIDX_MAIN_2_THIRD].StartAddr + MemShiftAddr)& 0xfffffff0);
	IoReg_Write32(MDOMAIN_DISP_DDR_SubAddrDropBits_reg,droppixel);
...
void mdomain_handler_i2rnd_mcap_apvr(void)
{
	...
	rtd_pr_scaler_memory_emerg("#################[%s][%d]Done!!@@@@@@@@@@@@@@@@@\n", __FUNCTION__, __LINE__);
}
...
void mdomain_handler_i2rnd_mcap_sdisp(void)
{
	...
	rtd_pr_scaler_memory_emerg("[I2RND]#################[%s][%d]Done!!@@@@@@@@@@@@@@@@@\n", __FUNCTION__, __LINE__);
}
#endif
...
void i2rnd_default_allocate_mcap2(void){
	...
	//M-domain start address
	IoReg_Write32(MDOMAIN_CAP_DDR_In1_3rdAddr_reg, (MemTag[MEMIDX_MAIN_2_THIRD].StartAddr& 0xfffffff0));
	IoReg_Write32(MDOMAIN_CAP_DDR_In1_2ndAddr_reg, (MemTag[MEMIDX_MAIN_2_SEC].StartAddr & 0xfffffff0));
	IoReg_Write32(MDOMAIN_CAP_DDR_In1Addr_reg, (MemTag[MEMIDX_MAIN_2].StartAddr & 0xfffffff0));
	//M-domain boundary
	IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr1_reg,(MemTag[MEMIDX_MAIN_2_THIRD].StartAddr & 0xfffffff0));
	IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg,(MemTag[MEMIDX_MAIN_2].StartAddr+buffer_size-CONFIG_MDOMAIN_BURST_SIZE)& 0xfffffff0);
}
...
void i2rnd_default_allocate_DI1(void){
	...
	IoReg_Write32(DI_DI_DATMemoryStartAdd1_reg, (di_address & 0xfffffff0));
	IoReg_Write32(DI_DI_DATMemoryStartAdd2_reg, ((di_address + default_buffer_size)& 0xfffffff0));
	IoReg_Write32(DI_DI_DATMemoryStartAdd3_reg, ((di_address + default_buffer_size * 2) & 0xfffffff0));
	IoReg_Write32(DI_DI_DATMemoryStartAdd4_reg, ((di_address + default_buffer_size * 3) & 0xfffffff0));
	IoReg_Write32(DI_DI_INFMemoryStartAdd1_reg, ((di_address + default_buffer_size * 4) & 0xfffffff0));
	IoReg_Write32(DI_DI_INFMemoryStartAdd2_reg, ((di_address + default_buffer_size * 4 + default_info_size)& 0xfffffff0));

	//boundary setting
	IoReg_Write32(DI_DI1_WRRD_BoundaryAddr1_reg, (di_address & 0xfffffff0));
	IoReg_Write32(DI_DI1_WRRD_BoundaryAddr2_reg, ((di_address + default_buffer_size * 4 + default_info_size * 2) & 0xfffffff0));
}
...
void i2rnd_default_allocate_DI2(void){
	...
	//start address setting
	IoReg_Write32(DI_DI_I2R_DATMemoryStartAdd1_reg, (di_address& 0xfffffff0));
	IoReg_Write32(DI_DI_I2R_DATMemoryStartAdd2_reg, ((di_address + default_buffer_size) & 0xfffffff0));
	IoReg_Write32(DI_DI_I2R_DATMemoryStartAdd3_reg, ((di_address + default_buffer_size * 2) & 0xfffffff0));
	IoReg_Write32(DI_DI_I2R_DATMemoryStartAdd4_reg, ((di_address + default_buffer_size * 3) & 0xfffffff0));
	IoReg_Write32(DI_DI_I2R_INFMemoryStartAdd1_reg, ((di_address + default_buffer_size * 4) & 0xfffffff0));
	IoReg_Write32(DI_DI_I2R_INFMemoryStartAdd2_reg, ((di_address + default_buffer_size * 4 + default_info_size) & 0xfffffff0));

	//boundary setting
	IoReg_Write32(DI_DI1_I2R_WRRD_BoundaryAddr1_reg, (di_address & 0xfffffff0));
	IoReg_Write32(DI_DI1_I2R_WRRD_BoundaryAddr2_reg, ((di_address + default_buffer_size * 4 + default_info_size * 2) & 0xfffffff0));
...
void drv_memory_cap_limit_boundary_setting(unsigned char display, unsigned int buffer_size){
	...
	mdomain_cap_capmain_boundaryaddr_ctrl2_RBUS mdomain_cap_capmain_boundaryaddr_ctrl2_reg;
	mdomain_cap_capsub_boundaryaddr_ctrl2_RBUS mdomain_cap_capsub_boundaryaddr_ctrl2_reg;

	if(display == SLR_MAIN_DISPLAY){

		// Set boundary for each block
		mdomain_cap_capmain_boundaryaddr_ctrl2_reg.regValue = IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr_CTRL2_reg);
		mdomain_cap_capmain_boundaryaddr_ctrl2_reg.in1_blk0_sel = 1; //use 0xb8027754 boundary
		mdomain_cap_capmain_boundaryaddr_ctrl2_reg.in1_blk1_sel = 2; //use 0xb802775C boundary
		mdomain_cap_capmain_boundaryaddr_ctrl2_reg.in1_blk2_sel = 3; //use 0xb8027764 boundary
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_CTRL2_reg, mdomain_cap_capmain_boundaryaddr_ctrl2_reg.regValue);

		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK0_reg, (MemTag[MEMIDX_MAIN].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK0_reg, (MemTag[MEMIDX_MAIN].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0xfffffff0);//need cut burstLen
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK1_reg, (MemTag[MEMIDX_MAIN_SEC].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK1_reg, (MemTag[MEMIDX_MAIN_SEC].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0xfffffff0);//need cut burstLen
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK2_reg, (MemTag[MEMIDX_MAIN_THIRD].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK2_reg, (MemTag[MEMIDX_MAIN_THIRD].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0xfffffff0);//need cut burstLen
	}
	else if(display == SLR_SUB_DISPLAY){

		// Set boundary for each block
		mdomain_cap_capsub_boundaryaddr_ctrl2_reg.regValue = IoReg_Read32(MDOMAIN_CAP_CapSub_BoundaryAddr_CTRL2_reg);
		mdomain_cap_capsub_boundaryaddr_ctrl2_reg.in2_blk0_sel = 5; //use 0xb8027774 boundary
		mdomain_cap_capsub_boundaryaddr_ctrl2_reg.in2_blk1_sel = 6; //use 0xb802777c boundary
		mdomain_cap_capsub_boundaryaddr_ctrl2_reg.in2_blk2_sel = 7; //use 0xb8027784 boundary
		IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_CTRL2_reg, mdomain_cap_capsub_boundaryaddr_ctrl2_reg.regValue);

		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK0_reg, (MemTag[MEMIDX_SUB].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK0_reg, (MemTag[MEMIDX_SUB].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0xfffffff0);//need cut burstLen
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK1_reg, (MemTag[MEMIDX_SUB_SEC].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK1_reg, (MemTag[MEMIDX_SUB_SEC].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0xfffffff0);//need cut burstLen
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK2_reg, (MemTag[MEMIDX_SUB_THIRD].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK2_reg, (MemTag[MEMIDX_SUB_THIRD].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0xfffffff0);//need cut burstLen
	}
}

void drv_memory_disp_limit_boundary_setting(unsigned char display){

	mdomain_disp_dispmain_boundaryaddr_ctrl2_RBUS mdomain_disp_dispmain_boundaryaddr_ctrl2_reg;
	mdomain_disp_dispsub_boundaryaddr_ctrl2_RBUS mdomain_disp_dispsub_boundaryaddr_ctrl2_reg;

	if(display == SLR_MAIN_DISPLAY){

		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DispMain_BoundaryAddr_CTRL2_reg);
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.main_blk0_sel = 1; //use 0xb8027754 boundary
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.main_blk1_sel = 2; //use 0xb802775C boundary
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.main_blk2_sel = 3; //use 0xb8027764 boundary
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr_CTRL2_reg, mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.regValue);
		...
		mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DispSub_BoundaryAddr_CTRL2_reg);
		mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.sub_blk0_sel = 5; //use 0xb8027774 boundary
		mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.sub_blk1_sel = 6; //use 0xb802777c boundary
		mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.sub_blk2_sel = 7; //use 0xb8027784 boundary
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_CTRL2_reg, mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.regValue);

		if(get_hdmi_vrr_4k60_mode()){
			//4K60Hz VRR flow
			IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK0_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK0_reg)& 0xfffffff0));
			IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK0_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK0_reg) & 0xfffffff0));
			IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK1_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK1_reg) & 0xfffffff0));
			IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK1_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK1_reg) & 0xfffffff0));
			IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK2_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK2_reg) & 0xfffffff0));
			IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK2_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK2_reg) & 0xfffffff0));
		}
#if 0
...
void Scaler_memory_setting_through_CMA(void)
{
	...
	if (drvif_memory_get_block_addr(MEMIDX_MAIN_THIRD) == _DUMMYADDR) {
		...
		IoReg_Write32(MDOMAIN_DISP_DDR_Main3rdAddr_reg, nStartAddr& 0xfffffff0);
		nStartAddr = drvif_memory_get_block_addr(MEMIDX_MAIN_SEC);
		IoReg_Write32(MDOMAIN_DISP_DDR_Main2ndAddr_reg, nStartAddr& 0xfffffff0);
		nStartAddr = drvif_memory_get_block_addr(MEMIDX_MAIN);
		IoReg_Write32(MDOMAIN_DISP_DDR_MainAddr_reg, nStartAddr& 0xfffffff0);
		nStartAddr = IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr1_reg);
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr1_reg, nStartAddr& 0xfffffff0);
		nStartAddr = IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg);
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr2_reg, nStartAddr& 0xfffffff0);
...
bool is_panel_size_bigger_than_m_size(unsigned char display)
{
    ...
    if (display == SLR_MAIN_DISPLAY
    		&& ((_DISP_WID * _DISP_LEN) > (max_width * max_len)))
    	ret = true;
...
{//return true: use
    if((get_scaler_run_hfr_mode()) && (is_4k_hfr_mode(SLR_MAIN_DISPLAY)))
        return TRUE;//current for 4k2k 120 case
    return FALSE;
...
{
	unsigned int cnt = 40;
	unsigned int uzudtg_dvs_cnt = 0;
	unsigned char frc_style = 0;
	if (!get_sub_OutputVencMode()) {//sub m domain iterrupt open so don't need wait lastwrite @qing_liu --s4ap
		...
		uzudtg_dvs_cnt = PPOVERLAY_uzudtg_DVS_cnt_uzudtg_dvs_cnt(IoReg_Read32(PPOVERLAY_uzudtg_DVS_cnt_reg));

		if(!get_hdmi_vrr_4k60_mode() && ((get_hdmi_4k_hfr_mode() == HDMI_NON_4K120) || (get_hdmi_4k_hfr_mode() == HDMI_3K120))&&
			  (uzudtg_dvs_cnt != 0)&&((270000000/uzudtg_dvs_cnt) < (Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY, SLR_INPUT_V_FREQ)-5)))//input fast
		{
			frc_style = 1;
		}else{
			frc_style = 0;
		...
		if(MDOMAIN_DISP_DDR_SubCtrl_get_sub_frc_style(IoReg_Read32(MDOMAIN_DISP_DDR_SubCtrl_reg)) == frc_style)
			return; //no need do

		//enable double buffer for input style switch
		IoReg_SetBits(MDOMAIN_DISP_DDR_MainSubCtrl_reg, MDOMAIN_DISP_DDR_MainSubCtrl_disp2_double_enable_mask);
		if(1 == frc_style){
			IoReg_SetBits(MDOMAIN_DISP_DDR_SubCtrl_reg, MDOMAIN_DISP_DDR_SubCtrl_sub_frc_style_mask);//set input fast
		}else{
			IoReg_ClearBits(MDOMAIN_DISP_DDR_SubCtrl_reg, MDOMAIN_DISP_DDR_SubCtrl_sub_frc_style_mask);//set input slow
		}//apply setting
		IoReg_SetBits(MDOMAIN_DISP_DDR_MainSubCtrl_reg, MDOMAIN_DISP_DDR_MainSubCtrl_disp2_double_apply_mask);
		//wait apply
		while(MDOMAIN_DISP_DDR_MainSubCtrl_get_disp2_double_apply(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg)) && --cnt){
			msleep(1);
		}
		if(cnt == 0)
			rtd_pr_scaler_memory_err("apply sub input fast timeout!\n");
	}
}
#endif
...
{
	unsigned int m_pre_read = 0;
	unsigned int DVStar,timeborrow;
	...
	DVStar = Get_DISP_DEN_STA_VPOS();

	//ppoverlay_uzudtg_stage1_time_borrow_reg.regValue = IoReg_Read32(PPOVERLAY_uzudtg_stage1_time_borrow_reg);
	timeborrow = scaler_disp_dtg_get_time_borrow_value();
	//timeborrow = 0; //scaler_disp_dtg_get_time_borrow_value();
	
	m_pre_read = DVStar - timeborrow - _M_PRE_READ_MARGIN;
	if((Get_DISPLAY_PANEL_OLED_TYPE() == TRUE))
	{
		m_pre_read = m_pre_read - DTG_ORBIT_MAX_EXTEND_V;
	}
...
{
	mdomain_disp_disp_sub_fsync_ctrl_0_RBUS mdomain_disp_disp_sub_fsync_ctrl_0_reg;


	mdomain_disp_disp_sub_fsync_ctrl_0_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DISP_SUB_FSYNC_CTRL_0_reg);

	mdomain_disp_disp_sub_fsync_ctrl_0_reg.sub_fifo_gate_to_vo1_en = a_bEnable;

	IoReg_Write32(MDOMAIN_DISP_DISP_SUB_FSYNC_CTRL_0_reg, mdomain_disp_disp_sub_fsync_ctrl_0_reg.regValue);
}
...
void drv_memory_set_vo_gating_threshold(void)
{
	// main mdisp
	mdomain_disp_ddr_mainprevstart_RBUS mdomain_disp_ddr_mainprevstart_reg;
	mdomain_disp_disp_main_fsync_ctrl_0_RBUS mdomain_disp_disp_main_fsync_ctrl_0_reg;
	mdomain_disp_disp_main_fsync_ctrl_1_RBUS mdomain_disp_disp_main_fsync_ctrl_1_reg;

	// sub mdisp
	mdomain_disp_ddr_subprevstart_RBUS mdomain_disp_ddr_subprevstart_reg;
	mdomain_disp_disp_sub_fsync_ctrl_0_RBUS mdomain_disp_disp_sub_fsync_ctrl_0_reg;
	mdomain_disp_disp_sub_fsync_ctrl_1_RBUS mdomain_disp_disp_sub_fsync_ctrl_1_reg;

	// Set main mdisp vo gating threshold
	mdomain_disp_ddr_mainprevstart_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainPreVStart_reg);
	mdomain_disp_ddr_mainprevstart_reg.main_fifo_gate_th = MAIN_FIFO_GATE_TH;
	IoReg_Write32(MDOMAIN_DISP_DDR_MainPreVStart_reg, mdomain_disp_ddr_mainprevstart_reg.regValue);

	mdomain_disp_disp_main_fsync_ctrl_1_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DISP_MAIN_FSYNC_CTRL_1_reg);
	mdomain_disp_disp_main_fsync_ctrl_1_reg.main_fifo_gate_th1 = MAIN_FIFO_GATE_TH_1;
	mdomain_disp_disp_main_fsync_ctrl_1_reg.main_fifo_gate_th2 = MAIN_FIFO_GATE_TH_2;
	IoReg_Write32(MDOMAIN_DISP_DISP_MAIN_FSYNC_CTRL_1_reg, mdomain_disp_disp_main_fsync_ctrl_1_reg.regValue);

	mdomain_disp_disp_main_fsync_ctrl_0_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DISP_MAIN_FSYNC_CTRL_0_reg);
	mdomain_disp_disp_main_fsync_ctrl_0_reg.main_fifo_gate_th3 = MAIN_FIFO_GATE_TH_3;
	IoReg_Write32(MDOMAIN_DISP_DISP_MAIN_FSYNC_CTRL_0_reg, mdomain_disp_disp_main_fsync_ctrl_0_reg.regValue);

	// Set sub mdisp vo gating threshold
	mdomain_disp_ddr_subprevstart_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_SubPreVStart_reg);
	mdomain_disp_ddr_subprevstart_reg.sub_fifo_gate_th = MAIN_FIFO_GATE_TH;
	IoReg_Write32(MDOMAIN_DISP_DDR_SubPreVStart_reg, mdomain_disp_ddr_subprevstart_reg.regValue);

	mdomain_disp_disp_sub_fsync_ctrl_1_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DISP_SUB_FSYNC_CTRL_1_reg);
	mdomain_disp_disp_sub_fsync_ctrl_1_reg.sub_fifo_gate_th1 = MAIN_FIFO_GATE_TH_1;
	mdomain_disp_disp_sub_fsync_ctrl_1_reg.sub_fifo_gate_th2 = MAIN_FIFO_GATE_TH_2;
	IoReg_Write32(MDOMAIN_DISP_DISP_SUB_FSYNC_CTRL_1_reg, mdomain_disp_disp_sub_fsync_ctrl_1_reg.regValue);

	mdomain_disp_disp_sub_fsync_ctrl_0_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DISP_SUB_FSYNC_CTRL_0_reg);
	mdomain_disp_disp_sub_fsync_ctrl_0_reg.sub_fifo_gate_th3 = MAIN_FIFO_GATE_TH_3;
	IoReg_Write32(MDOMAIN_DISP_DISP_SUB_FSYNC_CTRL_0_reg, mdomain_disp_disp_sub_fsync_ctrl_0_reg.regValue);
}
