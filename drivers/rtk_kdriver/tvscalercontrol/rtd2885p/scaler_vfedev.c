...
#include "hdmirx/hdmi_power_saving.h"
...
//	VSC_INPUT_TYPE_T vo_src;
	...
	switch(cmd)
	{
		...
		case VFE_IOC_AVD_AV_CONNECT:
			...
			if(port == 0)
				{
					AVD_Input = _AV_INPUT2;  // Orignal, Vin=10, A0N
				}
				// CVBS, YPP use the same Y
			else if(port == 1)
				{
					AVD_Input = _AV_INPUT4;  // Vin=9, A0N
				}
...
{//force connect bringup source
	...
	switch(AUTO_BRINGUP_SOURCE){
		...
		case _SRC_HDMI:
			if(get_HDMI_Global_Status() == SRC_CONNECT_DONE)
				break;
			rtd_pr_vfe_emerg("#####[Bring-up][%s(%d)]HDMI Init start\n",__func__,__LINE__);
			vfe_hdmi_drv_init();
			vfe_hdmi_drv_open();
		...
		case _SRC_MINI_DP:
			if(DPRX_Global_Status == SRC_CONNECT_DONE)
				break;
			rtd_pr_vfe_emerg("#####[Bring-up][%s(%d)]DPRX Init start\n",__func__,__LINE__);
