...
typedef enum
{
	...
	VPQ_INPUT_125HZ,						///< input 125Hz
	VPQ_INPUT_144HZ,						///< input 144Hz
	VPQ_INPUT_FRAME_MAX,
...
typedef enum
{
	...
	VPQ_OUTPUT_125HZ,						///< output 125Hz
	VPQ_OUTPUT_144HZ,						///< output 144Hz
    VPQ_OUTPUT_150HZ,
...
void Scaler_MEMC_Set_DISP_DHTOTAL(unsigned int DHTotal);//k23
unsigned int Scaler_MEMC_Get_DISP_DHTOTAL(void);//k23
...
void Scaler_MEMC_fbg_control(char const* pfunc,UINT32 line);
void Scaler_MEMC_SetInOutFrameRate(void);
void Scaler_MEMC_SetInOutFrameRateByDisp(unsigned char _channel);
...
void Scaler_SetMEMCLowDelay(UINT8 enable);
unsigned char Scaler_GetMEMCLowDelay(void);
void Scaler_set_MEMC_gamemode_lowdelay(unsigned char enable);
...
typedef struct{
	...
	unsigned int MV_INFO_RD_up_limit;
	//MC memory boundary
	UINT32 MC_dma_low_limit;
	UINT32 MC_dma_up_limit;
