...
AV_VDC_PATH 				tAv1Path = {VIDEO_INPUT4, VIDEO_Y0N};
AV_VDC_PATH 				tAv2Path = {VIDEO_INPUT10, VIDEO_GNDA0};
AV_VDC_PATH 				tAv3Path = {VIDEO_INPUT1, VIDEO_GN};
AV_VDC_PATH 				tAv4Path = {VIDEO_INPUT9, VIDEO_GNDA0};
...
StructSourceInput g_pcbSourceTable[] = {
	...
	{ TVD_INFMT_HDMI0, SET_SOURCE(_SRC_FROM_TMDS, _SRC_HDMI),	_DP_INPUT1,	0,  _SRC_OBJ_DP,  (void *)NULL, 				AV1_AUTOSWITCH_DB },
...
{
	rtd_pr_vfe_debug("PcbSource_Init\n");
...
void InitPinDefineFromBookCode(void)
{
	...
	rtd_pr_avd_notice("#####[%s(%d)]tAv1Path.VideoIn=%d, tAv1Path.GroundIn=%d\n",__func__,__LINE__,tAv1Path.VideoIn, tAv1Path.GroundIn);
	...
	rtd_pr_avd_notice("#####[%s(%d)]tAv2Path.VideoIn=%d, tAv2Path.GroundIn=%d\n",__func__,__LINE__, tAv2Path.VideoIn, tAv2Path.GroundIn);
	...
	rtd_pr_avd_notice("#####[%s(%d)]tAv3Path.VideoIn=%d, tAv3Path.GroundIn=%d\n",__func__,__LINE__, tAv3Path.VideoIn, tAv3Path.GroundIn);
	...
	rtd_pr_avd_debug("tScart1Path.Y_VideoIn=%d, tScart1Path.R_VideoIn=%d, tScart1Path.G_VideoIn=%d, tScart1Path.B_VideoIn=%d, tScart1Path.GroundIn=%d\n", tScart1Path.Y_VideoIn, tScart1Path.R_VideoIn, tScart1Path.G_VideoIn, tScart1Path.B_VideoIn, tScart1Path.GroundIn);
	...
	rtd_pr_avd_debug("tSv1Path.Y_VideoIn=%d, tSv1Path.Y_GroundIn=%d, tSv1Path.C_VideoIn=%d, tSv1Path.C_GroundIn=%d, tSv1Path.SV_Type=%d\n", tSv1Path.Y_VideoIn, tSv1Path.Y_GroundIn, tSv1Path.C_VideoIn, tSv1Path.C_GroundIn, tSv1Path.SV_Type);
	...
	rtd_pr_avd_debug("tSv2Path.Y_VideoIn=%d, tSv2Path.Y_GroundIn=%d, tSv2Path.C_VideoIn=%d, tSv2Path.C_GroundIn=%d, tSv2Path.SV_Type=%d\n", tSv2Path.Y_VideoIn, tSv2Path.Y_GroundIn, tSv2Path.C_VideoIn, tSv2Path.C_GroundIn, tSv2Path.SV_Type);
	...
	rtd_pr_adc_debug("tVGAPath.DATA_INPUT=%d, tVGAPath.SOY_INPUT=%d, tVGAPath.SWAP_PATH=%d\n", tVGAPath.DATA_INPUT, tVGAPath.SOY_INPUT, tVGAPath.SWAP_PATH);
	...
	rtd_pr_adc_debug("tYPbPrPath1.DATA_INPUT=%d, tYPbPrPath1.SOY_INPUT=%d, tYPbPrPath1.SWAP_PATH=%d\n", tYPbPrPath1.DATA_INPUT, tYPbPrPath1.SOY_INPUT, tYPbPrPath1.SWAP_PATH);
...
// #if 0
	...
	rtd_pr_adc_debug("tYPbPrPath2.DATA_INPUT=%d, tYPbPrPath2.SOY_INPUT=%d, tYPbPrPath2.SWAP_PATH=%d\n", tYPbPrPath2.DATA_INPUT, tYPbPrPath2.SOY_INPUT, tYPbPrPath2.SWAP_PATH);
#endif
}
...
int GetAVPath(unsigned char a_ucPort, unsigned char *a_pucVideoIn, unsigned char *a_pucGroundIn)
{
	...
	rtd_pr_avd_debug("GetAVPath, a_ucPort=%d\n", a_ucPort);
	...
	rtd_pr_avd_debug("AV path lRet=%d, VideoIn=%d, GroundIn=%d\n", lRet, *a_pucVideoIn, *a_pucGroundIn);
...
int GetScartPath(unsigned char a_ucPort, unsigned char *a_pucYIn, unsigned char *a_pucRIn, unsigned char *a_pucGIn, unsigned char *a_pucBIn, unsigned char *a_pucGroundIn)
{
	...
	rtd_pr_avd_debug("GetScartPath, a_ucPort=%d\n", a_ucPort);
	...
	rtd_pr_avd_debug("Scart path lRet=%d, YIn=%d, RIn=%d, GIn=%d, BIn=%d, GroundIn=%d\n", lRet, *a_pucYIn, *a_pucRIn, *a_pucGIn, *a_pucBIn, *a_pucGroundIn);
...
int GetSVPath(unsigned char a_ucPort, unsigned char *a_pucYIn, unsigned char *a_pucYGround, unsigned char *a_pucCIn, unsigned char *a_pucCGround, unsigned char *a_pucType)
{
	...
	rtd_pr_avd_debug("GetSVPath, a_ucPort=%d\n", a_ucPort);
	...
	rtd_pr_avd_debug("SV path lRet=%d, YIn=%d, YGround=%d, CIn=%d, CGround=%d, Type=%d\n", lRet, *a_pucYIn, *a_pucYGround, *a_pucCIn, *a_pucCGround, *a_pucType);
...
int GetVGAPath(unsigned char a_ucPort, unsigned char *a_pucData, unsigned char *a_pucSOY, unsigned char *a_pucSWAP)
{
	...
	rtd_pr_adc_debug("GetVGAPath, a_ucPort=%d\n", a_ucPort);
	...
	rtd_pr_adc_debug("VGA path lRet=%d, a_pucData=%d, a_pucSOY=%d, a_pucSWAP=%d\n", lRet, *a_pucData, *a_pucSOY, *a_pucSWAP);
...
int GetYPPPath(unsigned char a_ucPort, unsigned char *a_pucData, unsigned char *a_pucSOY, unsigned char *a_pucSWAP)
{
	...
	rtd_pr_adc_debug("YPP path lRet=%d, a_pucData=%d, a_pucSOY=%d, a_pucSWAP=%d\n", lRet, *a_pucData, *a_pucSOY, *a_pucSWAP);
