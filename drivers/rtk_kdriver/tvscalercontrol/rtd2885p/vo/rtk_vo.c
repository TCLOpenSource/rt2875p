...
void __drv_I3ddmaVodma_SingleBuffer_GameMode(unsigned char enable)
{
	...
	HDMI_INFO.framerate =  timing->v_freq;
	HDMI_INFO.framerate_ori = timing->src_v_freq;
	HDMI_INFO.srcframerate =  timing->src_v_freq;
	...
	if(((Get_DISPLAY_REFRESH_RATE() == 120) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && is_vrr_or_freesync_mode())
	...
	else if(get_current_driver_pattern(DRIVER_FREERUN_PATTERN) && (is_vrr_or_freesync_mode()==FALSE)){
		if(!VOInfo){
