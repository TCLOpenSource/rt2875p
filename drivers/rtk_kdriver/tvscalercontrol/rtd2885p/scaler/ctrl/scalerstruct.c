...
#endif
extern unsigned int Scaler_Get_HDMI_VFREQ(unsigned int orig_v_freq);
extern unsigned int Scaler_Get_HDMI_HFREQ(unsigned int orig_h_freq);
...
DEBUG_SCALER_VSC_STRUCT DbgSclrFlgTkr=
{
	...
	0, //Gamemode_break_flag
	0, //game_mode_rerun_scaler_flag
}
...
{
	if((Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI))
	{//hdmi case to check qms status
		if(get_scaler_qms_mode_flag())
		{//qms mode
			if(check_hdmi_current_qms_status(NULL) != HDMI_QMS_FSM_READY)
			{//qms not stable yet
				return FALSE;
			}
		}
	...
	if(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY)
		update_hdmi_qms_info_shamemory();//update qms status to video fw
...
unsigned int Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_INFO infoList)
{
	switch (infoList)
	{
		...
		case SLR_MODE_IHFREQ:
			return Scaler_Get_HDMI_HFREQ(stHdmiModeInfo.IHFreq);
		case SLR_MODE_IVFREQ:
			return Scaler_Get_HDMI_VFREQ(stHdmiModeInfo.IVFreq);
		case SLR_MODE_IHLEN:
		...
		case SLR_MODE_IHSYNCPULSECOUNT:
			return stHdmiModeInfo.IHSyncPulseCount;
                case SLR_MODE_IHFREQ_NEW:
                        return stHdmiModeInfo.IHFreq;
                case SLR_MODE_IVFREQ_NEW:
...
void Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_INFO infoList, unsigned int data)
{
	switch (infoList)
	{
		...
		case SLR_MODE_POLARITY:
			...
			break;
		case SLR_MODE_IHFREQ:
			stHdmiModeInfo.IHFreq = (unsigned int)data;
			break;
		case SLR_MODE_IVFREQ:
			stHdmiModeInfo.IVFreq = (unsigned int)data;
			break;
		case SLR_MODE_IHLEN:
...
StructSrcRect* Scaler_GetDispWindowPnt(void)
{
...
// FixMe, 20140928
	...
	if(drvif_scaler3d_get_IDMA3dMode() != SLR_IDMA_MODE_DISABLE){
		...
		if(i3ddma_timing){
			...
			ivFreq = i3ddma_timing->v_freq / 100;
			// monitor always show 72.8xx as 72Hz
			tempHz = ( i3ddma_timing->v_freq + 500 ) / 1000;
		}
	}else
#endif
