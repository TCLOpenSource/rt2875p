...
extern void MEMC_Lib_quick_show_memc_mc_dma_db(void);
...
void memc_realcinema_cinematic_framerate(void)
{
    ...
    if(0/*Scaler_get_cinematic_mode_condition()*/)
    {
        ...
        if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI){
            source_vfreq = Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ);
        }
        else{
            source_vfreq = pVOInfo->src_v_freq;
        }
    }
    else
    {
        source_vfreq = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_V_FREQ);
    }

	/*input frame rate must be 24hz or output is 48hz*/
	if(((source_vfreq > 235) && (source_vfreq < 245)) || \
			Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_DTG_MASTER_V_FREQ) == 48) {
		unsigned int vfreq_ori = 0;
		vfreq_ori = memc_realcinema_get_current_framerate(); //Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_DTG_MASTER_V_FREQ);
		modestate_decide_dtg_m_modeByDisp((unsigned char)SLR_MAIN_DISPLAY);
			...
			if(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY){
				if(memc_mux_ctrl_reg.memc_outmux_sel)
				{
					...
					if (scaler_vsc_get_adaptive_pst_lowdelay_mode() == 1 && !drv_memory_get_vdec_direct_low_latency_mode()){
						...
						if((sys_reg_sys_clken3_reg.clken_memc_me == 0)/* && (pVOInfo->src_v_freq <= 470)*/
...
/*
void HAL_VPQ_MEMC_GetFirmwareVersion(unsigned short *pVersion){
...
//		if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) == PIXEL_MODE_2_PIXEL){
    ...
    else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 475) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 485))	//48Hz
	{
...
//		if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) == PIXEL_MODE_2_PIXEL){
	...
	else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 995) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 1005))	//100hz
	{
	...
	else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 1195) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 1205))	//120hz
	{
    ...
    else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 1435) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 1445))	//120hz //k24 else if((Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) > 1195) && (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 1205))	//120hz
	{
...
void Scaler_MEMC_SetInOutFrameRateByDisp(unsigned char _channel)
{
	...
	{
		ulVFreq = get_qms_current_framerate()/100;
		if(ulVFreq == 0)
		{
    ...
    if (ulVFreq == 0)
	{
      	ulVFreq = Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ);
       ...
       if (TRUE == modestate_get_mdomain_frc_flag(_channel))
        {
            ...
            ulVFreq = 	modestate_get_mdomain_frc_vfreq(_channel);
       	}
#endif
    }        
#endif
	...
	if((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE) && (_channel == SLR_SUB_DISPLAY)){
		rtd_pr_memc_err( "[%s]SLR_SUB_DISPLAY don't config memc inout framerate.\n",__FUNCTION__);
		return;
	}
...
#if 1//#if RTK_MEMC_Performance_tunging_from_tv001//k24
	if((ulVFreq > 235) && (ulVFreq < 245))	//24Hz
#else
...
#if 1//#if RTK_MEMC_Performance_tunging_from_tv001//k24
	else if((ulVFreq > 245) && (ulVFreq < 255))	//25Hz
#else
...
#if 1//#if RTK_MEMC_Performance_tunging_from_tv001//k24
	else if((ulVFreq > 295) && (ulVFreq < 305))	//30Hz
#else
...
#if RTK_MEMC_Performance_tunging_from_tv001
	#ifdef CONFIG_MDOMAIN_FORCE_FRC
	else if((ulVFreq > 475) && (ulVFreq < 485))	//48Hz
	{
		memc_input_frame_rate = VPQ_INPUT_48HZ;
		//if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) == PIXEL_MODE_2_PIXEL){
		if(Get_DISPLAY_REFRESH_RATE() > 60){
...
#endif
#if 1//#if RTK_MEMC_Performance_tunging_from_tv001//k24
	else if((ulVFreq > 495) && (ulVFreq < 505))	//50Hz
#else
...
#if 1//#if RTK_MEMC_Performance_tunging_from_tv001//k24
	else if((ulVFreq > 595) && (ulVFreq < 605))	//60hz
#else
	...
	else if((ulVFreq > 995) && (ulVFreq < 1005))	//100hz //k24 //else if((Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) > 995) && (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 1005))	//100hz
	{
		memc_input_frame_rate = VPQ_INPUT_100HZ;
		//if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) == PIXEL_MODE_2_PIXEL){
		if(Get_DISPLAY_REFRESH_RATE() > 60){
	...
	else if((ulVFreq > 1195) && (ulVFreq < 1205))	//120hz //k24 else if((Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) > 1195) && (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 1205))	//120hz
	{
		memc_input_frame_rate = VPQ_INPUT_120HZ;
		//if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) == PIXEL_MODE_2_PIXEL){
		if(Get_DISPLAY_REFRESH_RATE() > 60){
    ...
    else if((ulVFreq > 1435) && (ulVFreq < 1445))	//120hz //k24 else if((Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) > 1195) && (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 1205))	//120hz
	{
		memc_input_frame_rate = VPQ_INPUT_144HZ;
		//if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) == PIXEL_MODE_2_PIXEL){
		if(Get_DISPLAY_REFRESH_RATE() > 60){
...
int Scaler_MEMC_MEMC_CLK_OnOff(unsigned char ME_enable, unsigned char MC_enable, unsigned char bMute)
{
	...
	down(&MEMC_CLK_Semaphore);
		if(do_power_saving == 2){  //MC clk on, ME clk off
			...
			rtd_pr_memc_notice("[%s][%d][%d][%d,%d][%x]\n", __FUNCTION__, __LINE__, (rtd_inl(PPOVERLAY_MEMC_MUX_CTRL_reg)>>12)&0x1, wait_cnt, (rtd_inl(0xB801B6B8)-time2), rtd_inl(DBUS_WRAPPER_MEMC_DCU1_ARB_SR0_reg));
	...
	if(get_MEMC_bypass_status_refer_platform_model() == FALSE){
		...
		/*if((Get_DISPLAY_REFRESH_RATE() == 120) && !(vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())
			&& (((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ) > 495) &&
			   (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ) < 605)) || (enable == 0)))*/
		//k23
		if((Get_DISPLAY_REFRESH_RATE() >= 120) && !(vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())
			&& (((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ) > 495) &&
			   (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ) < 605)) || (enable == 0)))
		{
			...
			rtd_pr_memc_emerg("Scaler_set_MEMC_gamemode_lowdelay (%d)\n", enable);
			...
			if(enable){
				...
				if(force_enable_two_step_uzu() || (is_4k_hfr_mode(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY))) || get_hdmi_vrr_4k60_mode())
					vbe_disp_set_dtgM2uzu_lowdelay_mode(2);
				else
					vbe_disp_set_dtgM2uzu_lowdelay_mode(1);
...
#ifdef BUILD_QUICK_SHOW
		MEMC_Lib_quick_show_memc_mc_dma_db();
#endif
	//MEMC mux
	//if (Mc_clk_state == 1)
