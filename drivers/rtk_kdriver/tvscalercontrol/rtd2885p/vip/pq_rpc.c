...
//#define VIPprintf(fmt, args...)	rtd_pr_vpq_debug(fmt, ##args)	//20150822 roger mark for image release
...
unsigned char fw_film_GMV_ratio;
int fw_film_suspend=0;
unsigned char fw_film_Table_Idx=0;
unsigned char fw_film_Timing=0;
unsigned char fw_film_is_dtv=0;
#ifndef BUILD_QUICK_SHOW
int z_film_scenechange_32=0;
int a_film_framediff_hist[9]={0,0,0,0,0,0,0,0,0};
int is_24to25=0;
int is_known_24to25=0;
...
void scalerVIP_film22_pair(
	unsigned int FramePairPattern,int PrevDiff,int NextDiff
    ,unsigned int* NextStaticPattern
    ,int bitpos
   // ,unsigned char* PrevStaticPattern
    ,unsigned int* NextFieldMotion
    ,unsigned int* PrevFieldMotion
){
	switch(FramePairPattern&3){
		case 1:
		NextStaticPattern[0]|=2;
		//PrevStaticPattern[0]|=1;
		//NextFieldMotion[0]&=-3;
		//PrevFieldMotion[0]&=-2;
		PrevFieldMotion[0]&=~(1<<bitpos);
		break;
		case 2:
		NextStaticPattern[0]|=1;
		//PrevStaticPattern[0]|=2;
		//NextFieldMotion[0]&=-2;
		//PrevFieldMotion[0]&=-3;
		NextFieldMotion[0]&=~(1<<bitpos);
		break;
		case 3:
		if(NextDiff-PrevDiff>(PrevDiff>>5)){
			NextStaticPattern[0]|=2;
			//PrevStaticPattern[0]|=1;
			//NextFieldMotion[0]&=-3;
			//PrevFieldMotion[0]&=-2;
		...
		if(PrevDiff-NextDiff>(NextDiff>>5)){
			NextStaticPattern[0]|=1;
			//PrevStaticPattern[0]|=2;
			//NextFieldMotion[0]&=-2;
			//PrevFieldMotion[0]&=-3;
	...
	unsigned char *p22Detected, unsigned int *p22Sequence)
{
	...
	*Violate=0;
	if((NextStaticPattern&3)==1){
		...
		*p22Sequence=(fw_DI_3A) ? 1 : 2;
		*film_index=FILM22;
	}else if((NextStaticPattern&3)==3 && !is_known_24to25){
		*ExactMatch=0;
		//*Violate=1;
		*p22Detected=0;
		*p22Sequence=0;
...
void scalerVIP_film_FWmode_Quick_detection(
	...
	if(z32enable){
		scalerVIP_film_FWmode_detection3(
			FrameStaticPattern, FramePairPattern, NextStaticPattern,
			ExactMatch322, Violate322,index322,
			p32Detected2, p32Sequence2
		);
		*p32Detected1=*p32Detected2;
		*p32Sequence1=*p32Sequence2;
		*ExactMatch321=*ExactMatch322;
		*Violate321=*Violate322;
		*index321=*index322;
	}
	//*/
...
int fw_film22_pair;
int framestatic2224;
void scalerVIP_film_FWmode_detection2224(unsigned int FrameStaticPattern,
	unsigned char *ExactMatch, unsigned char *Violate, unsigned int *film_index,
	unsigned char *p22Detected, unsigned int *p22Sequence
){
	if(!framestatic2224){return;}
...
typedef struct {
	int x_gmv_static_th;
	int x_gmv_moving_th;
	int y_lowpass_th;// 40a0 15 8
	int y_framemotion_th;// 40a0 7 0
	int teethth_teeth_lb;
	int teethth_moving_lb;
	int teethth_prog_lb;
}C_film_by_GMV_Y;

C_film_by_GMV_Y fw_film_by_GMV_table[5]={// follow gFilmInitTable.length
	{90,74,14,10,4,4,4},
	{90,74,14,10,4,4,4},
	{90,74,14,10,4,4,4},
	{90,74,14,10,4,4,4},
...
C_film_by_GMV_Y fw_film_by_GMV_y;
...
void scalerVIP_film_by_GMV(void)
{
	...
	int z_film_timing=0;// todo : Write the id of the current film table using.
	C_film_by_GMV_Y* z_film_by_GMV_Y=NULL;
	DRV_film_table_t2* z_film_original=NULL;
	SLR_VIP_TABLE *gVip_Table = NULL;
	unsigned char idx=0;
	di_im_di_si_film_final_result_RBUS im_di_si_film_final_result_reg;
	di_im_di_film_sawtooth_filmframe_th_RBUS di_im_di_film_sawtooth_filmframe_th_q;

	gVip_Table = fwif_colo_get_AP_vip_table_gVIP_Table();
	if (gVip_Table == NULL || RPC_system_info_structure_table ==NULL) {
		return;
	}

	z_film_timing = RPC_system_info_structure_table->VIP_source;

	/*get pqa table index need check source first. because ap flow or table index will get incorrect table*/

	idx = gVip_Table->VIP_QUALITY_Coef[z_film_timing][VIP_QUALITY_FUNCTION_Film];

	z_film_by_GMV_Y=&(fw_film_by_GMV_table[idx]);
	z_film_original=&(gFilmInitTable[idx]);

	switch(z_film_timing)
	{
		case VIP_QUALITY_HDMI_480I:
		fw_film_GMV_ratio=scalerVIP_DI_MiddleWare_MCNR_Get_GMV_Ratio();
		if(fw_film_GMV_ratio<=z_film_by_GMV_Y->x_gmv_moving_th){
			fw_film_by_GMV_y.y_lowpass_th=z_film_original->film_row_px_field.film_SawtoothThl;
			fw_film_by_GMV_y.y_framemotion_th=z_film_original->film_row_px_frame.film_FrMotionThl;
			fw_film_by_GMV_y.teethth_teeth_lb=z_film_original->film_row_px_field.film_teethth_teeth_lb;
			fw_film_by_GMV_y.teethth_moving_lb=z_film_original->film_row_px_field.film_teethth_moving_lb;
			fw_film_by_GMV_y.teethth_prog_lb=z_film_original->film_row_px_field.film_teethth_prog_lb;
		}else if(fw_film_GMV_ratio<z_film_by_GMV_Y->x_gmv_static_th){
			int za=z_film_by_GMV_Y->x_gmv_moving_th-fw_film_GMV_ratio;
			int zb=z_film_by_GMV_Y->x_gmv_static_th-z_film_by_GMV_Y->x_gmv_moving_th;
			fw_film_by_GMV_y.y_lowpass_th=z_film_by_GMV_Y->y_lowpass_th+(z_film_original->film_row_px_field.film_SawtoothThl-z_film_by_GMV_Y->y_lowpass_th)*za/zb;
			fw_film_by_GMV_y.y_framemotion_th=z_film_by_GMV_Y->y_framemotion_th+(z_film_original->film_row_px_frame.film_FrMotionThl-z_film_by_GMV_Y->y_framemotion_th)*za/zb;
			fw_film_by_GMV_y.teethth_teeth_lb=z_film_by_GMV_Y->y_framemotion_th+(z_film_original->film_row_px_field.film_teethth_teeth_lb-z_film_by_GMV_Y->teethth_teeth_lb)*za/zb;
			fw_film_by_GMV_y.teethth_moving_lb=z_film_by_GMV_Y->y_framemotion_th+(z_film_original->film_row_px_field.film_teethth_moving_lb-z_film_by_GMV_Y->teethth_prog_lb)*za/zb;
			fw_film_by_GMV_y.teethth_prog_lb=z_film_by_GMV_Y->y_framemotion_th+(z_film_original->film_row_px_field.film_teethth_prog_lb-z_film_by_GMV_Y->teethth_prog_lb)*za/zb;
		}else{
			fw_film_by_GMV_y.y_lowpass_th=z_film_by_GMV_Y->y_lowpass_th;
			fw_film_by_GMV_y.y_framemotion_th=z_film_by_GMV_Y->y_framemotion_th;
			fw_film_by_GMV_y.teethth_teeth_lb=z_film_by_GMV_Y->teethth_teeth_lb;
			fw_film_by_GMV_y.teethth_moving_lb=z_film_by_GMV_Y->teethth_moving_lb;
			fw_film_by_GMV_y.teethth_prog_lb=z_film_by_GMV_Y->teethth_prog_lb;
		}

		//*
		im_di_si_film_final_result_reg.regValue = rtd_inl(DI_IM_DI_SI_FILM_FINAL_RESULT_reg);
		if(im_di_si_film_final_result_reg.regValue&0x800){
			printk(KERN_EMERG
				"\n[fw_film_GMV_ratio]GMV_ratio=%d VIP_source=%d idx=%d y_lowpass_th=%d y_framemotion_th=%d[/fw_film_GMV_ratio]\t"
				,fw_film_GMV_ratio
				,z_film_timing
				,idx
				,fw_film_by_GMV_y.y_lowpass_th
				,fw_film_by_GMV_y.y_framemotion_th
			);
		}
		//*/
		di_im_di_film_sawtooth_filmframe_th_q.regValue=rtd_inl(DI_IM_DI_FILM_SAWTOOTH_FILMFRAME_TH_reg);
		di_im_di_film_sawtooth_filmframe_th_q.film_sawtooththl=fw_film_by_GMV_y.y_lowpass_th;
		di_im_di_film_sawtooth_filmframe_th_q.film_frmotionthl=fw_film_by_GMV_y.y_framemotion_th;
		rtd_outl(DI_IM_DI_FILM_SAWTOOTH_FILMFRAME_TH_reg,di_im_di_film_sawtooth_filmframe_th_q.regValue);
			break;
...
FILM_FWMode DETECTION_scalerVIP_film_FWmode()
{
	...
	im_di_film_new_function_main_reg.regValue = rtd_inl(DI_IM_DI_FILM_NEW_FUNCTION_MAIN_reg);
	{
		...
		fw_film22_pair = (im_di_si_film_final_result_reg.regValue&0x2000)?1:0;
		framestatic2224 = (im_di_si_film_final_result_reg.regValue&0x1000)?1:0;
		scalerVIP_film_by_GMV();
		fw_film_suspend=0;
		//fw_film_GMV_ratio=scalerVIP_DI_MiddleWare_MCNR_Get_GMV_Ratio();
		//if(im_di_si_film_final_result_reg.regValue&0x800)printk(KERN_EMERG "\n[fw_film_GMV_ratio]%d[/fw_film_GMV_ratio]\t",scalerVIP_DI_MiddleWare_MCNR_Get_GMV_Ratio());
	}
	...
	z_film_scenechange_32=z_film_scenechange_32>0?z_film_scenechange_32-1:0;
...
#endif
	hw_RTNR_Interlace_Force2D_En =0; //if bringup is ok , please remove it.
	...
	if(
		(StatusFrameStaticPattern&0x02000001)==0x02000001 &&
		(StatusFrameStaticPattern&0x01ffe000)!=0x01ffe000 &&
		(StatusFrameStaticPattern&0x00001ffe)!=0x00001ffe &&
		(((StatusFrameStaticPattern&0x1555554)>>1)==(StatusFrameStaticPattern&0xaaaaaa))
	)
	{
		z_24to25=z_24to25|1;
	}
	if( (z_24to25&0x02000001)==0x02000001 && (StatusFrameStaticPattern&0x03ffffff)!=0x03ffffff )
	{
		is_24to25=99;
		filmQuick_en = 1;
	}
	if(DynamicOptimizeSystem[175]==1)
		rpt_di_film_spear2_hifreq =0;// Merlin4, back to general case
	else
	...
	if(fw_di_film_sawtooth_comparePrevNext){
		int i,j,k;
		int reg_vc_di_film22_observe_many_frames=3;// tunable, 0~3
		if( (StatusFrameStaticPattern&0x21)!=0x21 )
		{
			for(i=7;i>0;i--)
			{
				a_film_framediff_hist[i]=a_film_framediff_hist[i-1];
			}
			a_film_framediff_hist[0]=FrameDiff;
			a_film_framediff_hist[8]=0;
			for(i=0;i<8;i++)
			{
				a_film_framediff_hist[8]=a_film_framediff_hist[8]+a_film_framediff_hist[i];
			}
			if(FrameDiff*4>a_film_framediff_hist[8]
				&& (StatusFrameStaticPattern&15)!=14
				&& (StatusFrameStaticPattern&30)!=28
				&& (StatusFrameStaticPattern&0x1ff)!=0x19
				&& (StatusFrameStaticPattern&0x3fe)!=0x32
			)
			{
				if( film_index!=FILM55 && film_index!=FILM64 && film_index!=FILM87 )
				{
					z_film_scenechange_32=4;
				}
			}
		}
		...
		for(i=0;i<8;i++){// region number
			...
			A22_StatusPrevDiffSum[i][0]=(p22_StatusPrevDiffSum[i][0]);
			A22_StatusNextDiffSum[i][0]=(p22_StatusNextDiffSum[i][0]);
			p22_StatusPrevDiffSum[i][0]=(1<<reg_vc_di_film22_observe_many_frames)>>1;
			p22_StatusNextDiffSum[i][0]=(1<<reg_vc_di_film22_observe_many_frames)>>1;
			for(j=0;j<8;j+=2){
				p22_StatusPrevDiffSum[i][0]+=A22_StatusPrevDiffSum[i][j]+A22_StatusNextDiffSum[i][j|1];
				p22_StatusNextDiffSum[i][0]+=A22_StatusNextDiffSum[i][j]+A22_StatusPrevDiffSum[i][j|1];
			}
			p22_StatusPrevDiffSum[i][0]=p22_StatusPrevDiffSum[i][0]>>reg_vc_di_film22_observe_many_frames;
			p22_StatusNextDiffSum[i][0]=p22_StatusNextDiffSum[i][0]>>reg_vc_di_film22_observe_many_frames;
			if(p22_StatusPrevDiffSum[i][0]>FrameDiff){p22_StatusPrevDiffSum[i][0]=FrameDiff;}
			if(p22_StatusNextDiffSum[i][0]>FrameDiff){p22_StatusNextDiffSum[i][0]=FrameDiff;}
		}
		// detection 22
	...
	if( z_film_scenechange_32>0 && is_24to25>0 )
	{
		is_24to25=0;
	}
	...
	if( fw_film_is_dtv )
	{
		...
		{
			printk(KERN_EMERG, "[fw_film_big_dtv]\t");
		}
	}
	else if(FrameDiff>(PrevDiff<<7)&&FrameDiff>(NextDiff<<7)){
		if(im_di_si_film_final_result_reg.regValue&0x400)
		{
			printk(KERN_EMERG, "[fw_film_big_framediff]\t");
		}
		...
		if(im_di_si_film_final_result_reg.regValue&0x400)
		{
			printk(KERN_EMERG, "[fw_film_big_prevdiff]\t");
		}
		...
		if(im_di_si_film_final_result_reg.regValue&0x400)
		{
			printk(KERN_EMERG, "[fw_film_big_nextdiff]\t");
		}
	...
	else
	{
		if(im_di_si_film_final_result_reg.regValue&0x400)
			{
			printk(KERN_EMERG, "[fw_film_big_not]\t");
		}
	}
	if(fw_film22_pair){
		scalerVIP_film22_pair(
			StatusFramePairPattern,PrevDiff,NextDiff
			,&StatusNextStaticPattern_film22
			,7
			,&hw_StatusNextFieldMotion_film22_All
			,&hw_StatusPrevFieldMotion_film22_All
		);
		scalerVIP_film22_pair(
			StatusFramePairPattern_H1,PrevDiff_H1,NextDiff_H1
			,&StatusNextStaticPattern_film22_H1
			,6
			,&hw_StatusNextFieldMotion_film22_All
			,&hw_StatusPrevFieldMotion_film22_All
		);
		scalerVIP_film22_pair(
			StatusFramePairPattern_H2,PrevDiff_H2,NextDiff_H2
			,&StatusNextStaticPattern_film22_H2
			,5
			,&hw_StatusNextFieldMotion_film22_All
			,&hw_StatusPrevFieldMotion_film22_All
		);
		scalerVIP_film22_pair(
			StatusFramePairPattern_H3,PrevDiff_H3,NextDiff_H3
			,&StatusNextStaticPattern_film22_H3
			,4
			,&hw_StatusNextFieldMotion_film22_All
			,&hw_StatusPrevFieldMotion_film22_All
		);
		scalerVIP_film22_pair(
			StatusFramePairPattern_V1,PrevDiff_V1,NextDiff_V1
			,&StatusNextStaticPattern_film22_V1
			,3
			,&hw_StatusNextFieldMotion_film22_All
			,&hw_StatusPrevFieldMotion_film22_All
		);
		scalerVIP_film22_pair(
			StatusFramePairPattern_V2,PrevDiff_V2,NextDiff_V2
			,&StatusNextStaticPattern_film22_V2
			,2
			,&hw_StatusNextFieldMotion_film22_All
			,&hw_StatusPrevFieldMotion_film22_All
		);
		scalerVIP_film22_pair(
			StatusFramePairPattern_V3,PrevDiff_V3,NextDiff_V3
			,&StatusNextStaticPattern_film22_V3
			,1
			,&hw_StatusNextFieldMotion_film22_All
			,&hw_StatusPrevFieldMotion_film22_All
		);
		scalerVIP_film22_pair(
			StatusFramePairPattern_C,PrevDiff_C,NextDiff_C
			,&StatusNextStaticPattern_film22_C
			,0
			,&hw_StatusNextFieldMotion_film22_All
			,&hw_StatusPrevFieldMotion_film22_All
		);
...
*/
	...
	if(!filmQuick_en){
	scalerVIP_film_FWmode_detection2224(StatusFrameStaticPattern,
		&ExactMatch22, &Violate22,&index22,
	...
	if( Status22Detected )
	{
		StatusQuickDetected_cnt++;
	}
	else
	if( is_known_24to25 && is_24to25>0 && StatusQuickDetected_cnt>0 )
	{
		if( StatusQuickDetected_cnt>15 )
		{
			StatusQuickDetected_cnt=15;
		}
		StatusQuickDetected_cnt--;
	}
	else
	{
		StatusQuickDetected_cnt=0;
	}
	if( StatusQuickDetected_cnt>0 )
	{
		Status22Detected=1;
	}
	else
	...
	if(im_di_si_film_final_result_reg.regValue&0x400){
		rtd_pr_vpq_emerg("\n[fw_film_disconfort]hw22=%d fw22=%d frame=%d prev=%d next=%d[/fw_film_disconfort]\t"
			,hw_film_Status22Detected
			,Status22Detected
			,FrameDiff
			,PrevDiff
			,NextDiff
	...
	if(im_di_si_film_final_result_reg.regValue&0x400){
		rtd_pr_vpq_emerg("\n[fw_film_suspend]32_1=%d 32_2=%d 22=%d FrameStatic=%d[/fw_film_suspend]\t"
			,Status32Detected1
			,Status32Detected2
			,Status22Detected
			,StatusFrameStaticPattern
		);
	}
	if(!Status32Detected1&&!Status32Detected2&&Status22Detected&&(StatusFrameStaticPattern&1))
	{
		fw_film_suspend=1;
	}
	// decision
...
#define bad_edit_test 0
	...
	*/
	...
	fw_StatusFilmSequence = filmQuick_en?StatusFilmSequence_tmp:hw_film_StatusFilmSequence;
...
void APPLY_scalerVIP_film_FWmode(FILM_FWMode DetInformation)
{
	...
	di_im_di_film_topbotline3_RBUS im_di_film_topbotline3_reg;
        di_im_di_si_film_bound_RBUS im_di_si_film_bound_reg;

#ifdef fix_compile_error_MEMC
	unsigned char *FilmInfo = Scaler_MEMC_GetCadence();
#else
	unsigned char *FilmInfo = NULL;
#endif
	im_di_film_new_function_main_reg.regValue = rtd_inl(DI_IM_DI_FILM_NEW_FUNCTION_MAIN_reg);
	...
	im_di_film_topbotline3_reg.regValue = rtd_inl(DI_IM_DI_FILM_TopBotLine3_reg);
        im_di_si_film_bound_reg.regValue = rtd_inl(DI_IM_DI_SI_FILM_BOUND_reg);
	...
	*/
	im_di_film_new_function_main_reg.film_bad_edit_en = 0;	//yushiang5053 bad edit enable not need enable to Merlin4 film flow.
	...
	if(fw_film_suspend == 1)
	{
		//im_di_film_new_function_main_reg.fw_film_en = 0;
		im_di_film_new_function_main_reg.fw_film_filmdetected = 0;
	}

        if(DynamicOptimizeSystem[211]==1)
        {
                im_di_film_new_function_main_reg.fw_film_en=0;
                im_di_si_film_bound_reg.film22_quick_decision_en =2;
                DynamicOptimizeSystem[211]=0;
        }
        else
        {
                im_di_si_film_bound_reg.film22_quick_decision_en =0;
	...
	rtd_outl(DI_IM_DI_FILM_TopBotLine3_reg, im_di_film_topbotline3_reg.regValue);
        ...
        rtd_outl(DI_IM_DI_SI_FILM_BOUND_reg,im_di_si_film_bound_reg.regValue);
