...
int drvif_Hdmi_GetEmpPacket(vfe_hdmi_em_type_e type, unsigned char* pkt_buf, unsigned short len)
{
	...
	case KADP_VFE_HDMI_EMP_SBTM:
		return (len >= EM_SBTM_INFO_LEN) ? newbase_hdmi_get_sbtm_emp(port, pkt_buf) : FALSE;

	case KADP_VFE_HDMI_EMP_RSV0:
		return (len >= RSV0_EM_LEN) ? newbase_hdmi_get_rsv0_emp(port, pkt_buf) : FALSE;

	case KADP_VFE_HDMI_EMP_RSV1:
		return (len >= RSV1_EM_LEN) ? newbase_hdmi_get_rsv1_emp(port, pkt_buf) : FALSE;
