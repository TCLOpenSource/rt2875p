...
const struct hdmi_dfe_reg_st *dfe[1] =
{
...
const struct hdmi21_aphy_rstb_reg_st *hd21_aphy_rstb[2] = {
...
//	}
//Need Record DFE parameters value:LE TAP0() //work around for RL6641-1346 
#if 0
	for (lane=0; lane< lane_mode; lane++) {
		...
		if (lemax[lane] > DFE_P0_REG_DFE_EN_L0_P0_get_le_min_lane0_p0(hdmi_in(DFE_REG_DFE_EN_L0_reg)))
		{
			phy_st[nport].dfe_t[lane].le = DFE_P0_REG_DFE_EN_L0_P0_get_le_min_lane0_p0(hdmi_in(DFE_REG_DFE_EN_L0_reg));
		}
		else 
		{
			phy_st[nport].dfe_t[lane].le = lemax[lane];
		}
...
{
	unsigned char le_0,le_1,le_2;
	signed char tap2_0,tap2_1,tap2_2;
	...
	if (b_clk > 2700) {
		...
		le_0 = phy_st[nport].dfe_t[0].le ;
		le_1 = phy_st[nport].dfe_t[1].le ;
		le_2 = phy_st[nport].dfe_t[2].le ;
...
{   
		...
		if ((lane_mode == HDMI_3LANE)&& (b_clk > 1380) && (b_clk < 2700)) {
			min_le = MIN(lib_hdmi_dfe_get_le(nport, 0), MIN(lib_hdmi_dfe_get_le(nport, 1), lib_hdmi_dfe_get_le(nport, 2)));
