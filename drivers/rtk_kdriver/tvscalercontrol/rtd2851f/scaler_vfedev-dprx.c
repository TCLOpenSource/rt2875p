...
#ifdef CONFIG_DPRX_REPORT_ZERO_TIMING

static DEFINE_SPINLOCK(DPRX_ZERO_TIMING_CTRL);

unsigned char DPRX_Reply_Zero_Timing = FALSE;
static unsigned char DPRX_Reply_Zero_Timing_mute_cnt = 0;
static unsigned int DPRX_ZERO_REPORT_RECORD_TIME = 0;//when ap polling the timing and the zero timing flag is ture. Record the time
...
void Set_Dprx_Reply_Zero_Timing_Flag(unsigned char flag)
{
    ...
    spin_lock_irqsave(&DPRX_ZERO_TIMING_CTRL, flags);
    DPRX_Reply_Zero_Timing = flag;
    spin_unlock_irqrestore(&DPRX_ZERO_TIMING_CTRL, flags);
    rtd_pr_vfe_info("##func: %s DPRX value:(%d %d)##\r\n",__FUNCTION__, flag, DPRX_Reply_Zero_Timing);
}
...
static int dprx_vfe_ctrl_handler(unsigned long arg)
{
    ...
    case DPRX_VFE_CTRL_CONNECT:
        ...
        if (DPRX_Global_Status != SRC_OPEN_DONE)
        {
            DPRX_VFE_ERR("DPRX_VFE_CTRL_CONNECT Fail. Source is not SRC_OPEN_DONE (fsm=%d)\n", DPRX_Global_Status);
            return -1;//Fail Source is not SRC_INIT_DONE
        }
    ...
    case DPRX_VFE_CTRL_DISCONNECT:
        ...
        {
            DPRX_VFE_ERR("DPRX_VFE_CTRL_DISCONNECT Fail. Source is not SRC_CONNECT_DONE\n");
            return -1;//Fail Source is not SRC_INIT_DONE
        }
    ...
    if(attr != NULL)
    {
        ...
        case DPRX_VFE_ATTR_PORT_SET_WAKE_UP_MODE:
            if (attr->data_array==0)
            {
                ...
                ret = vfe_dprx_drv_set_wakeup_mode(set_wakeup_mode);
            }
            break;
    ...
    case VFE_IOC_DPRX_VIDEO_GET_ATTR:
        ...
        case DPRX_VIDEO_ATTR_VIDEO_TIMING:
            ...
            if (attr.data_array==1 && attr.data_len==sizeof(KADP_VFE_DPRX_TIMING_INFO_T) && attr.p_data)
            {
...
#ifdef CONFIG_DPRX_REPORT_ZERO_TIMING
                if (DPRX_Reply_Zero_Timing  & REPORT_ZERO_TIMING)
                {
                    Set_Dprx_Reply_Zero_Timing_Flag(CLEAR_ZERO_FLAG);
                    DPRX_Reply_Zero_Timing_mute_cnt = DPRX_REPLY_ZERO_TIMING_COUNT;
                    DPRX_ZERO_REPORT_RECORD_TIME = rtk_timer_misc_90k_ms();

                    DPRX_VFE_ERR("DPRX_Reply_Zero_Timing  force report zero timing = DPRX_Reply_Zero_Timing_mute_cnt=%d\n",
                                DPRX_Reply_Zero_Timing_mute_cnt);
                }

                if (DPRX_Reply_Zero_Timing_mute_cnt == ACTUAL_DETECT_RESULT)
                {
#endif
                ...
                {
                    vfe_dprx_drv_get_port_timing_info(attr.port, &get_timing_info); // read to trigger detect
                        
                    if (DPRX_Reply_Zero_Timing_mute_cnt == CHECK_TIME_PERIOD)
                    {
                        if (!check_polling_time_period(DPRX_ZERO_REPORT_RECORD_TIME, rtk_timer_misc_90k_ms(), DPRX_TIME_PERIOD_THRESHOULD))
                        {
                            DPRX_Reply_Zero_Timing_mute_cnt = ACTUAL_DETECT_RESULT;
                            DPRX_VFE_ERR("DPRX_Reply_Zero_Timing_mute_cnt expired, restart actual timing\n",
                                DPRX_Reply_Zero_Timing_mute_cnt);
                        }
                    }
                    else
                    {
                        // FixMe, 20190919
                        DPRX_VFE_ERR("DPRX_Reply_Zero_Timing_mute_cnt=%d > 0, return zero timing\n", DPRX_Reply_Zero_Timing_mute_cnt);

                        DPRX_Reply_Zero_Timing_mute_cnt--;

                        if (DPRX_Reply_Zero_Timing_mute_cnt > DPRX_REPLY_ZERO_TIMING_COUNT)
                        {
                            // FixMe, 20190919
                            DPRX_VFE_ERR("Something wrong, DPRX_Reply_Zero_Timing_mute_cnt %d should less than %d !!!!!!!!\n",
                            DPRX_Reply_Zero_Timing_mute_cnt, DPRX_REPLY_ZERO_TIMING_COUNT);
                            BUG();
                        }
                    }
