...
{	/* 2_0048, 2_0050, 2_0054, 2_005c */
	...
	if (on_off == ON) {
		...
		tv_sb1_ana_vdadc_pow_ctl_reg.reg_ldo100ma_pow = 1;	/* the major power of vdadc */
		...
		tv_sb1_ana_vdadc_pow_ctl_reg.reg_ldo100ma_pow = 0;	/* 2_005c[1]=1, enable LDO power */
#endif
#endif
...
void drvif_module_vdc_CheckInputStatus(unsigned char ucInputSrc, unsigned char ucCheckStart)
{
	...
	if (fpga_test_reg1_reg.regValue != VDreg_NoAck)
	{
		if (fpga_test_reg1_reg.factory_flag)
		{
			...
			rtd_pr_vdc_debug("VDTOP_FPGA_TEST_REG1_reg==%x\n", fpga_test_reg1_reg.regValue);
...
//			UINT8 ucTestDmaMode = (fpga_test_reg1_reg.fw_dma_mode & 0x0f);
		...
		rtd_pr_vdc_info("(dead==>)VDTOP_FPGA_TEST_REG1_reg==%x\n", fpga_test_reg1_reg.regValue);
//		return;
	}
...
//					}
			...
			if (module_vdc_read_burst_win_error_flag() == 1) {
				...
				if (vdc_NoiseStatus > 0x70 && drvif_video_status_reg(VDC_Lines_625_detected) == 0) { //fix fluke<30db mode dectet and no color, RL
					...
					rtd_pr_vdc_emerg("weak_signal case6 improve\n");
				}
#endif
			}
#endif
...
bool drvif_module_vdc_OutputStable(void)
{
	...
	if (((drvif_module_vpq_GetYcSep() & 0xF) == VDC_YCSEPARATE_3D) 	&& (uc3dNoSupport == 0)){
		...
		if ((ucVstate == 3) && (avd_drv_vdpq_dma_3d_ready() == 1) && (g_uiBuffer_Ready == 1)){
		...
		if (drvif_module_vdc_Read_Vstate() == 3 && g_uiBuffer_Ready == 1) {
			VDC_DEBUG_MSG(VDC_MSG_TVCHCH, "VD ready !! Vstate=3 && g_uiBuffer_Ready == 1\n")
			//M6PRTANPM-765 improve chroma lock speed ryanliao 20190226 [start]
			if(_SRC_CVBS == Scaler_AVD_Get_g_ucSrcType()){
				module_vdc_Set_lose_chromalock_level(0x05); /* set to default ref jj*/
			}else{ //TV
...
void set_vdc_resetTV_Count(unsigned char vdc_resettv_count){
	vdc_resetTV_Count = vdc_resettv_count;
}
void set_vdc_AV_modech(bool vdc_av_modech){
	vdc_AV_modech = vdc_av_modech;
}
