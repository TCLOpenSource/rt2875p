...
#endif
void module_vpq_Get_PrjIDFrom_VpqPrjID(void)
{
	static bool bInitPrjId = false; // Just for RTICE debug
	if(bInitPrjId == false)
	{
		switch(fwif_VIP_get_Project_ID())
		{
			case VIP_Project_ID_TV001:
				ucVdpqPrjId = 0;
				break;
			case VIP_Project_ID_TV002:
				ucVdpqPrjId = 1;
				break;
			case VIP_Project_ID_TV006:
				ucVdpqPrjId = 2;
				break;
			case VIP_Project_ID_TV010:
				ucVdpqPrjId = 3;
				break;
			default:
				ucVdpqPrjId = 0;
...
void module_vpq_IC_hardware_reset(void)
{
	...
	vdcf_outl(0xb801999c, 0x00000a00);
	...
	//vdcf_outl(0xb8019b34, 0x00000000); // yc delay related register
	...
	vdcf_outl(0xb8019e54, 0x00000444);
	vdcf_outl(0xb8019e58, 0x00000000);
	vdcf_outl(0xb8019e68, 0x0000000e);
	vdcf_outl(0xb8019e74, 0x00000000);
	vdcf_outl(0xb8019ee8, 0x00000000);
	vdcf_outl(0xb8019eec, 0x00000000);
	vdcf_outl(0xb8019ef0, 0x00040808);
	vdcf_outl(0xb8019ef4, 0x0c0c0e0e);
	vdcf_outl(0xb8019ef8, 0x10101010);
	vdcf_outl(0xb8019efc, 0x000ff880);
	vdcf_outl(0xb8019f00, 0x00000003);
	vdcf_outl(0xb8019f04, 0x23141d03);
	...
	//vdcf_outl(0xb8019f5c, 0x00000000); // read only register
	//vdcf_outl(0xb8019f60, 0x00000000); // read only register
	//vdcf_outl(0xb8019f64, 0x00000000); // read only register
	vdcf_outl(0xb8019f68, 0x000a0000);
	vdcf_outl(0xb8019f6c, 0x00000000);
	vdcf_outl(0xb8019f7c, 0x00001031);
	vdcf_outl(0xb8019f70, 0x00041400);
	vdcf_outl(0xb8019f74, 0x04010314);
	vdcf_outl(0xb8019f78, 0x00000403);
	vdcf_outl(0xb8019f80, 0x00000000);
	vdcf_outl(0xb8019f84, 0x00000082); // demod region control, defualt disable to avoid burst disappear and cgain overflow
	//vdcf_outl(0xb8019f88, 0x00000000); // yc delay related register
	//vdcf_outl(0xb8019f8c, 0x70000000); // yc delay related register
	//vdcf_outl(0xb8019f90, 0x00000000); // yc delay related register
	//vdcf_outl(0xb8019fa4, 0x00000000); // yc delay related register
	vdcf_outl(0xb8019fc0, 0x00000000);
	vdcf_outl(0xb8019fc4, 0x00000000);
	vdcf_outl(0xb8019fc8, 0x00000000);
	vdcf_outl(0xb8019fcc, 0x00000000);
	vdcf_outl(0xb8019fd0, 0x00000000);
	vdcf_outl(0xb8019fd4, 0x00000000);
	vdcf_outl(0xb8019fd8, 0x00000000);
	vdcf_outl(0xb8019fdc, 0x00000000);
	vdcf_outl(0xb8019fe0, 0x00000000);
	vdcf_outl(0xb8019fe4, 0x00000000);
	vdcf_outl(0xb8019fe8, 0x00000000);
	vdcf_outl(0xb8019fec, 0x00000000);
}
...
void module_vpq_dma_check_alive(unsigned char ucWrtClearTimes, unsigned char *ucRetResult, unsigned char *ucRetFIFOStatus )
{
	...
	//tvsb1_tv_sb1_dcu1_arb_cr_RBUS tvsb1_tv_sb1_dcu1_arb_cr_reg;

	*ucRetResult = 2;
	*ucRetFIFOStatus = 0;

	vdpq_dma_ctrl5_reg.regValue = vdcf_inl(VDPQ_DMA_CTRL5_reg);
	//tvsb1_tv_sb1_dcu1_arb_cr_reg.regValue = rtd_inl(TVSB1_TV_SB1_DCU1_ARB_CR_reg);

	if( vdpq_dma_ctrl5_reg.dma_reset == 1 || vdpq_dma_ctrl5_reg.mem_off == 1 /*|| ((tvsb1_tv_sb1_dcu1_arb_cr_reg.arb_en & 0x1) == 0)*/ )
	{
		rtd_pr_vpq_info("[%s] Mem/Arb (%d %d) is off, cannot check\n",
			__func__, vdpq_dma_ctrl5_reg.dma_reset, vdpq_dma_ctrl5_reg.mem_off/*, (tvsb1_tv_sb1_dcu1_arb_cr_reg.arb_en & 0x1)*/);
		*ucRetResult = 1;
		*ucRetFIFOStatus = 0;
	}
	else
