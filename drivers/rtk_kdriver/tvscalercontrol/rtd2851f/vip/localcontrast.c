...
{
	color_d_vc_global_ctrl_0_RBUS color_d_vc_global_ctrl_0;
	color_d_vc_global_ctrl_0.regValue = IoReg_Read32(COLOR_D_VC_Global_CTRL_0_reg);
#ifdef For_BringUp_Disable
	color_d_vc_global_ctrl_0.m_lc_comp_en = 0;
#else
	color_d_vc_global_ctrl_0.m_lc_comp_en = enable;
#endif
	color_d_vc_global_ctrl_0.write_enable_12 = 1;
	IoReg_Write32(COLOR_D_VC_Global_CTRL_0_reg, color_d_vc_global_ctrl_0.regValue);
}

void drvif_color_set_LC_DB(unsigned char en, unsigned char sel, unsigned char apply)
{
	lc_lc_global_ctrl1_RBUS blu_lc_global_ctrl1;
	blu_lc_global_ctrl1.regValue = IoReg_Read32(LC_LC_Global_Ctrl1_reg);

	blu_lc_global_ctrl1.lc_db_en = en;			// ptr->lc_db_en;
	blu_lc_global_ctrl1.lc_db_apply = apply;		// ptr->lc_db_apply;
	blu_lc_global_ctrl1.lc_db_read_level = sel;	// ptr->lc_db_read_level;
...
void drvif_color_set_LC_Global_Ctrl(DRV_LC_Global_Ctrl *ptr)
{
	...
	//blu_lc_global_ctrl1.lc_db_en = 1;			// ptr->lc_db_en;
	//blu_lc_global_ctrl1.lc_db_apply = 1;		// ptr->lc_db_apply;
	//blu_lc_global_ctrl1.lc_db_read_level = 1;	// ptr->lc_db_read_level;
...
void drvif_color_set_LC_BL_Profile_Interpolation_Table(unsigned int *ptr,unsigned char mode,unsigned char tab_size)
{
	...
	if (tab_size >= LC_Backlight_Profile_Interpolation_table_num)
		tab_size = LC_Backlight_Profile_Interpolation_table_num-1;
...
char drvif_color_get_LC_Enable(void)
{
	color_d_vc_global_ctrl_0_RBUS color_d_vc_global_ctrl_0;
	color_d_vc_global_ctrl_0.regValue = IoReg_Read32(COLOR_D_VC_Global_CTRL_0_reg);
	return color_d_vc_global_ctrl_0.m_lc_comp_en;
}
...
{
/* // only master //mac7p pq compile fix
	...
	if (color_d_vc_global_ctrl_0_reg.m_lc_comp_en & 0x1) {
		if (PPOVERLAY_MAIN_Active_H_Start_End_get_mh_act_sta(IoReg_Read32(PPOVERLAY_MAIN_Active_H_Start_End_reg)) &&
			PPOVERLAY_MAIN_Active_V_Start_End_get_mv_act_sta(IoReg_Read32(PPOVERLAY_MAIN_Active_V_Start_End_reg))) {
			color_d_vc_global_ctrl_0_reg.m_lc_comp_en = 0x3; //use reserved brhavior to keep main enable status
		} else
			color_d_vc_global_ctrl_0_reg.m_lc_comp_en = 0x1;

		color_d_vc_global_ctrl_0_reg.write_enable_12 = 1;
...
unsigned char drvif_color_get_LC_blk_hnum(void)
{
	lc_lc_global_ctrl2_RBUS blu_lc_global_ctrl2;
	blu_lc_global_ctrl2.regValue = IoReg_Read32(LC_LC_Global_Ctrl2_reg);
	rtd_pr_vpq_info("get_LC_blk_hnum=%d\n",blu_lc_global_ctrl2.lc_blk_hnum);
...
unsigned char drvif_color_get_LC_blk_vnum(void)
{
	lc_lc_global_ctrl2_RBUS blu_lc_global_ctrl2;
	blu_lc_global_ctrl2.regValue = IoReg_Read32(LC_LC_Global_Ctrl2_reg);
	rtd_pr_vpq_info("get_LC_blk_vnum=%d\n",blu_lc_global_ctrl2.lc_blk_vnum);
...
void drvif_color_get_LC_APL(unsigned int *ptr_out, unsigned int bufferSz)
{
	...
	if(bufferSz < read_times*2)
	{
		VIPprintf("[%s] Input APL Buffer Size %d error, should more than %d\n", __func__, bufferSz, read_times*2);
		memset(ptr_out, 0, sizeof(unsigned int)*bufferSz);
		return;
	}

	lc_global_ctrl0.lc_apl_en = 1;
	lc_apl_read_addr.lc_apl_mod = 0;
	lc_apl_read_addr.lc_apl_addr = 0;
