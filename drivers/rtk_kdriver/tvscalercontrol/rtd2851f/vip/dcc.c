...
void drvif_color_dcc_on(unsigned char enable)
{
	...
	d_vc_global_ctrl_0_Reg.regValue = rtd_inl(COLOR_D_VC_Global_CTRL_0_reg);

	if (enable == 0) {
		d_vc_global_ctrl_0_Reg.ms_ctrl_dcc = 0;/*dcc curve off*/
		d_vc_global_ctrl_0_Reg.write_enable_1 =1;
	} else {
		d_vc_global_ctrl_0_Reg.ms_ctrl_dcc = 1;/*dcc curve on*/
		d_vc_global_ctrl_0_Reg.write_enable_1 =1;
	}
	rtd_outl(COLOR_D_VC_Global_CTRL_0_reg, d_vc_global_ctrl_0_Reg.regValue);
}
...
void drvif_color_dcc_Curve_init(unsigned char display)
{
	...
	color_d_vc_global_ctrl_0_reg.regValue = rtd_inl(COLOR_D_VC_Global_CTRL_0_reg);
	color_dcc_d_dcc_db_ctrl_reg.regValue = rtd_inl(COLOR_DCC_D_DCC_DB_CTRL_reg);
	...
	if (display == SLR_MAIN_DISPLAY) {
		...
		//color_dcc_d_dcc_ctrl_reg.dcc_main_en = 1; //mac6p removed
	...
	color_d_vc_global_ctrl_0_reg.ms_ctrl_dcc = 1;
	color_d_vc_global_ctrl_0_reg.write_enable_1 = 1;

	#ifdef TV010_PQ_STYLE
	color_d_vc_global_ctrl_reg.icm_dcc_swap = 1 ;
	color_d_vc_global_ctrl_reg.write_enable_9 = 1;
	IoReg_SetBits(PPOVERLAY_DTG_dummy_reg_1_reg, _BIT0); //[JIRA] RL6543-1724, Mac6p ECO patch : when icm_dcc_swap = 1, must set 0xb80287ac[0] = 1
	#endif

	scaler_rtd_outl(COLOR_D_VC_Global_CTRL_reg, color_d_vc_global_ctrl_reg.regValue);
	scaler_rtd_outl(COLOR_D_VC_Global_CTRL_0_reg, color_d_vc_global_ctrl_0_reg.regValue);
}
