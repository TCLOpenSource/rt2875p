...
#else
	nr_mosquito_ctrl_RBUS_reg.mosquito_detect_en = (Scaler_Get_DI_SNR_Borrow_Sram_Mode() == SNR_DI_Sram_VE_USE)?(0):(SNR_mosquitoNR_set_tbl[value][0]);
#ifdef CONFIG_SNR_CLOCK_HW_ISSUE
	nr_mosquito_ctrl_RBUS_reg.mosquito_detect_en = (Scaler_Get_DI_SNR_Borrow_Sram_Mode() == SNR_DI_Sram_VE_USE)?(0):(1);
...
{
	nr_dch1_cp_ctrl_RBUS dch1_cp_ctrl_reg;
	//nr_dch1_local_var2_RBUS dch1_Local_Var2_Reg; /*mac8p rm*/
	nr_mosquito_ctrl_RBUS nr_mosquito_ctrl_RBUS_reg;
...
void drvif_color_DRV_SNR_Spatial_NR_Y(DRV_SNR_Spatial_NR_Y *ptr)
{
	...
	dch1_cp_ctrl_reg.cp_zoranweighty_sel = ptr->SNR_zoranweighty_sel;// Mac8p add
...
void drvif_color_DRV_SNR_Spatial_NR_C(DRV_SNR_Spatial_NR_C *ptr)
{
	...
	dch1_cp_ctrl_reg.cp_zoranweightc_sel = ptr->SNR_zoranweightc_sel;// Mac8p add
...
void drvif_color_DRV_SNR_Mosquito_NR_En(unsigned char En_flag)	/* Mosquito NR control is in general control*/
{
	...
	nr_mosquito_ctrl_RBUS_reg.mosquito_detect_en = 0;
#else
	nr_mosquito_ctrl_RBUS_reg.mosquito_detect_en = (Scaler_Get_DI_SNR_Borrow_Sram_Mode() == SNR_DI_Sram_VE_USE)?(0):(En_flag);	/* the spec is MosquitoNR Enable is controlled by MPEG NR UI */
#ifdef CONFIG_SNR_CLOCK_HW_ISSUE
	nr_mosquito_ctrl_RBUS_reg.mosquito_detect_en = (Scaler_Get_DI_SNR_Borrow_Sram_Mode() == SNR_DI_Sram_VE_USE)?(0):(1);
#endif //CONFIG_SNR_CLOCK_HW_ISSUE
#endif
...
void drvif_IESM_format_convert(void)// Merlin4 add
{
	...
	if((Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP) == FALSE) && ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)||(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_DP)))// spnr cbypass mode
	{
...
void drvif_IESM_frontporch_set(unsigned char game_mode_check_flag)
{
	...
	front_porch_num = Scaler_DispGetInputInfo(SLR_INPUT_V_LEN)- Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN_PRE)- Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_STA);
	...
	if(game_mode_check_flag ==1){
		edgesmooth_ex_ctrl_reg.lbuf_disable =1;
	}else{
