...
//extern unsigned char scalerVIP_DI_MiddleWare_MCNR_Get_GMV_Ratio(void); //mac6 no implement
...
void scalerVIP_film_by_GMV(void)
{
	...
	DRV_film_table_t* z_film_original=NULL;
	...
	switch(z_film_timing)
	{
		...
		if(fw_film_GMV_ratio<=z_film_by_GMV_Y->x_gmv_moving_th){
			fw_film_by_GMV_y.y_lowpass_th=z_film_original->film_SawtoothThl;
			fw_film_by_GMV_y.y_framemotion_th=z_film_original->film_FrMotionThl;
			...
			fw_film_by_GMV_y.y_lowpass_th=z_film_by_GMV_Y->y_lowpass_th+(z_film_original->film_SawtoothThl-z_film_by_GMV_Y->y_lowpass_th)*za/zb;
			fw_film_by_GMV_y.y_framemotion_th=z_film_by_GMV_Y->y_framemotion_th+(z_film_original->film_FrMotionThl-z_film_by_GMV_Y->y_framemotion_th)*za/zb;
		}else{
...
FILM_FWMode DETECTION_scalerVIP_film_FWmode(void)
{
...

	{
		...
		//scalerVIP_film_by_GMV(); // mac6 no MCNR
		//fw_film_GMV_ratio=scalerVIP_DI_MiddleWare_MCNR_Get_GMV_Ratio();
	...
	hw_RTNR_Interlace_Force2D_En = 0; //im_di_rtnr_control_reg.cp_rtnr_interlace_force2d_en; //mac6p removed
...
void APPLY_scalerVIP_film_FWmode(FILM_FWMode DetInformation)
{
	...
	di_im_di_si_film_22_result_a_RBUS im_di_si_film_22_result_a_reg;
	...
	if(film_index==FILM22)
		im_di_film_new_function_main_reg.fw_film_filmdetected = (DetInformation.Fw_StatusFilmDetected)&&(im_di_si_film_22_result_a_reg.film_detected22!=0);
	else
	...
	*/
	...
	if(DetInformation.fw_Status22Detected)
	{
		di_im_di_si_film_hb_cout_RBUS reg_di_im_di_si_film_hb_cout_reg;
		reg_di_im_di_si_film_hb_cout_reg.regValue = rtd_inl(DI_IM_DI_SI_FILM_HB_COUT_reg);
		reg_di_im_di_si_film_hb_cout_reg.di_film_nonfilmregion_force = 1;
		rtd_outl(DI_IM_DI_SI_FILM_HB_COUT_reg, reg_di_im_di_si_film_hb_cout_reg.regValue);
	}
	else
	{
		di_im_di_si_film_hb_cout_RBUS reg_di_im_di_si_film_hb_cout_reg;
		reg_di_im_di_si_film_hb_cout_reg.regValue = rtd_inl(DI_IM_DI_SI_FILM_HB_COUT_reg);
		reg_di_im_di_si_film_hb_cout_reg.di_film_nonfilmregion_force = 0;
		rtd_outl(DI_IM_DI_SI_FILM_HB_COUT_reg, reg_di_im_di_si_film_hb_cout_reg.regValue);
	}
