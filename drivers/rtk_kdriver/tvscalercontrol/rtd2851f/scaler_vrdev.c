...
#define VR_SEQCAP_BURSTLEN 512 //32*128/8
#define VR_BLKCAP_BURSTLEN 512 //64*8byte
...
unsigned char drvif_vr_dc2h_swmode_inforpc(unsigned int onoff,unsigned int buffernumber,unsigned int width,unsigned int length){
    ...
    if(get_VR_Pixel_Format() == VR_CAP_NV21 || get_VR_Pixel_Format() == VR_CAP_NV12 || get_VR_Pixel_Format() == VR_CAP_NV16)
        swmode_infoptr->YbufferSize = vr_bufAddr_align(vr_cap_frame_max_width*vr_cap_frame_max_height);
    else if(get_VR_Pixel_Format() == VR_CAP_RGB888 || get_VR_Pixel_Format() == VR_CAP_ARGB8888 || get_VR_Pixel_Format() == VR_CAP_RGBA8888 || get_VR_Pixel_Format() == VR_CAP_ABGR8888)
        swmode_infoptr->YbufferSize = CaptureCtrl_VR.cap_buffer[0].size;
    else
    {
        rtd_pr_vr_emerg("fun:%s,unknow pixel format\n",__FUNCTION__);
        return FALSE;
    }
	...
	if(Get_PANEL_VFLIP_ENABLE())
	{
        ...
        else if(get_VR_Pixel_Format() == VR_CAP_RGB888)
        {
            for (i = 0; i < MAX_CAP_BUF_NUMS; i++)
                swmode_infoptr->cap_buffer[i] = (((UINT32)CaptureCtrl_VR.cap_buffer[i].phyaddr) ? ((UINT32)CaptureCtrl_VR.cap_buffer[i].phyaddr + vr_cap_frame_max_width*vr_cap_frame_max_height*3 - vr_cap_frame_max_width*3) : (0));
        }
        else if(get_VR_Pixel_Format() == VR_CAP_ARGB8888 || get_VR_Pixel_Format() == VR_CAP_RGBA8888 || get_VR_Pixel_Format() == VR_CAP_ABGR8888)
        {
            for (i = 0; i < MAX_CAP_BUF_NUMS; i++)
                swmode_infoptr->cap_buffer[i] = (((UINT32)CaptureCtrl_VR.cap_buffer[i].phyaddr) ? ((UINT32)CaptureCtrl_VR.cap_buffer[i].phyaddr + vr_cap_frame_max_width*vr_cap_frame_max_height*4 - vr_cap_frame_max_width*4) : (0));
        }
        else
        {
            rtd_pr_vr_notice("fun:%s,unknow pixel format\n",__FUNCTION__);
            return FALSE;
        }
	...
	if (0 != (ret = Scaler_SendRPC(SCALERIOC_SET_DC2H_SWMODE_ENABLE,0,0))){
		rtd_pr_vr_notice("[VR][dc2h]ret=%d, SCALERIOC_SET_DC2H_SWMODE_ENABLE RPC fail !!!\n", ret);
		return FALSE;
	}
	return TRUE;
}

DC2H_Enable_rgb2yuv vr_colorformattransfer_enable(DC2H_IN_SEL dc2h_in_sel)
{
    ...
    rtd_pr_vt_notice("[VR]get_VR_Pixel_Format()=%x, dc2h_in_sel=%x, dc2h_vr_en_rgb2yuv=%d\n",get_VR_Pixel_Format(), dc2h_in_sel,dc2h_enrgb2yuv_state);
...
void drvif_vr_color_colorspaceyuv2rgbtransfer(DC2H_IN_SEL dc2h_in_sel)
{
	...
	dc2h_rgb2yuv_ctrl_reg.en_rgb2yuv = vr_colorformattransfer_enable(dc2h_in_sel);
...
void set_vr_dc2h_capture_sdnr_in_clock(unsigned char enable)
{
...
void set_DC2H_rgb2yuv_ctrl_reg_refer_dump_location_vr(KADP_VR_DUMP_LOCATION_TYPE_T in_position, SIZE *in_size, unsigned char *dc2h_in_sel)
{
    ...
    else if(in_position == KADP_VR_SCALER_INPUT)
    {
        *dc2h_in_sel = _Idomain_Sdnr_input;
        ...
        set_vr_dc2h_capture_sdnr_in_clock(TRUE);
    }
    else
    {
...
void drvif_vr_dc2h_seqmode_conifg(unsigned char enable, int capWid, int capLen, int capSrc)
{
	...
	if (enable == TRUE)
	{
		...
		drvif_vr_dc2h_input_overscan_config(0, 0, pdc2h_Vrhdl->in_size.nWidth, pdc2h_Vrhdl->in_size.nLength);
...
void vr_reset_dc2h_hw_setting(void)
{
	...
	set_vr_dc2h_capture_sdnr_in_clock(FALSE);
}
		...
		if (boffon == TRUE) {
			if((get_VR_Pixel_Format() == VR_CAP_NV12)||(get_VR_Pixel_Format() == VR_CAP_NV16)||(get_VR_Pixel_Format() == VR_CAP_NV21)){
				...
				if(get_VrBufferNum()==5) {
					...
					dc2h_dma_dc2h_cap_l3_start_RBUS dc2h_cap_l3_start_Reg;
					unsigned int lastcapbufferaddr = 0;
                    unsigned int seq_vflip_offset = (get_VR_Pixel_Format() == VR_CAP_RGB888) ? (vr_cap_frame_max_width*vr_cap_frame_max_height*3 - vr_cap_frame_max_width*3) : (vr_cap_frame_max_width*vr_cap_frame_max_height*4 - vr_cap_frame_max_width*4);
                    ...
                    if(Get_PANEL_VFLIP_ENABLE())
                    {
                        if(dc2h_cap_l1_start_Reg.regValue == (CaptureCtrl_VR.cap_buffer[0].phyaddr + seq_vflip_offset)){
                            lastcapbufferaddr = (CaptureCtrl_VR.cap_buffer[4].phyaddr + seq_vflip_offset);
                        }else if (dc2h_cap_l1_start_Reg.regValue == CaptureCtrl_VR.cap_buffer[1].phyaddr + seq_vflip_offset){
                            lastcapbufferaddr = (CaptureCtrl_VR.cap_buffer[0].phyaddr + seq_vflip_offset);
                        }else if (dc2h_cap_l1_start_Reg.regValue == CaptureCtrl_VR.cap_buffer[2].phyaddr + seq_vflip_offset){
                            lastcapbufferaddr = (CaptureCtrl_VR.cap_buffer[1].phyaddr + seq_vflip_offset);
                        }else if (dc2h_cap_l1_start_Reg.regValue == CaptureCtrl_VR.cap_buffer[3].phyaddr + seq_vflip_offset){
                            lastcapbufferaddr = (CaptureCtrl_VR.cap_buffer[2].phyaddr + seq_vflip_offset);
                        }else if (dc2h_cap_l1_start_Reg.regValue == CaptureCtrl_VR.cap_buffer[4].phyaddr + seq_vflip_offset){
                            lastcapbufferaddr = (CaptureCtrl_VR.cap_buffer[3].phyaddr + seq_vflip_offset);
                        }

                        if(lastcapbufferaddr == dc2h_cap_l3_start_Reg.regValue){
                            if(lastcapbufferaddr == CaptureCtrl_VR.cap_buffer[0].phyaddr + seq_vflip_offset){
                                lastcapbufferaddr = (CaptureCtrl_VR.cap_buffer[4].phyaddr + seq_vflip_offset);
                            }else if (lastcapbufferaddr == CaptureCtrl_VR.cap_buffer[1].phyaddr + seq_vflip_offset){
                            	lastcapbufferaddr = (CaptureCtrl_VR.cap_buffer[0].phyaddr + seq_vflip_offset);
                            }else if (lastcapbufferaddr == CaptureCtrl_VR.cap_buffer[2].phyaddr + seq_vflip_offset){
                            	lastcapbufferaddr = (CaptureCtrl_VR.cap_buffer[1].phyaddr + seq_vflip_offset);
                            }else if (lastcapbufferaddr == CaptureCtrl_VR.cap_buffer[3].phyaddr + seq_vflip_offset){
                            	lastcapbufferaddr = (CaptureCtrl_VR.cap_buffer[2].phyaddr + seq_vflip_offset);
                            }else if (lastcapbufferaddr == CaptureCtrl_VR.cap_buffer[4].phyaddr + seq_vflip_offset){
                            	lastcapbufferaddr = (CaptureCtrl_VR.cap_buffer[3].phyaddr + seq_vflip_offset);
                            }
					    }
...
void vr_set_DC2H_cap_boundaryaddr(unsigned int VrBufferNum)
{
	...
	else
	{
		if((get_VR_Pixel_Format() == VR_CAP_NV12) || (get_VR_Pixel_Format() == VR_CAP_NV16)||(get_VR_Pixel_Format() == VR_CAP_NV21)){
			if(5 == VrBufferNum)
			{
				dc2h_cap_boundaryaddr1_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[0].size - VR_BLKCAP_BURSTLEN);
				...
				dc2h_cap_boundaryaddr3_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[1].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[1].size - VR_BLKCAP_BURSTLEN);
				...
				dc2h_cap_boundaryaddr5_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[2].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[2].size - VR_BLKCAP_BURSTLEN);
				...
				dc2h_cap_boundaryaddr7_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[3].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[3].size -VR_BLKCAP_BURSTLEN);
				...
				dc2h_cap_boundaryaddr9_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[4].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[4].size - VR_BLKCAP_BURSTLEN);
			...
			else if(4 == VrBufferNum)
			{
				...
				dc2h_cap_boundaryaddr9_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[3].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[3].size - VR_BLKCAP_BURSTLEN);
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr9_reg, dc2h_cap_boundaryaddr9_reg.regValue);
			...
			else if(3 == VrBufferNum)
			{
				...
				dc2h_cap_boundaryaddr7_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[2].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[2].size - VR_BLKCAP_BURSTLEN);
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr7_reg, dc2h_cap_boundaryaddr7_reg.regValue);
				dc2h_cap_boundaryaddr8_reg.regValue =  (UINT32)CaptureCtrl_VR.cap_buffer[2].phyaddr;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr8_reg, dc2h_cap_boundaryaddr8_reg.regValue);

				dc2h_cap_boundaryaddr9_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[2].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[2].size - VR_BLKCAP_BURSTLEN);
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr9_reg, dc2h_cap_boundaryaddr9_reg.regValue);
			...
			else if(2 == VrBufferNum)
			{
				...
				dc2h_cap_boundaryaddr5_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[1].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[1].size - VR_BLKCAP_BURSTLEN);
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr5_reg, dc2h_cap_boundaryaddr5_reg.regValue);
				dc2h_cap_boundaryaddr6_reg.regValue =  (UINT32)CaptureCtrl_VR.cap_buffer[1].phyaddr;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr6_reg, dc2h_cap_boundaryaddr6_reg.regValue); 

				dc2h_cap_boundaryaddr7_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[1].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[1].size - VR_BLKCAP_BURSTLEN);
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr7_reg, dc2h_cap_boundaryaddr7_reg.regValue);
				dc2h_cap_boundaryaddr8_reg.regValue = (UINT32)CaptureCtrl_VR.cap_buffer[1].phyaddr;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr8_reg, dc2h_cap_boundaryaddr8_reg.regValue);

				dc2h_cap_boundaryaddr9_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[1].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[1].size - VR_BLKCAP_BURSTLEN);
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr9_reg, dc2h_cap_boundaryaddr9_reg.regValue);
			...
			else //1 == VrBufferNum
			{
				...
				dc2h_cap_boundaryaddr3_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[0].size - VR_BLKCAP_BURSTLEN);
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr3_reg, dc2h_cap_boundaryaddr3_reg.regValue);
				dc2h_cap_boundaryaddr4_reg.regValue =  (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr4_reg, dc2h_cap_boundaryaddr4_reg.regValue);

				dc2h_cap_boundaryaddr5_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[0].size - VR_BLKCAP_BURSTLEN);
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr5_reg, dc2h_cap_boundaryaddr5_reg.regValue);
				dc2h_cap_boundaryaddr6_reg.regValue =  (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr6_reg, dc2h_cap_boundaryaddr6_reg.regValue); 

				dc2h_cap_boundaryaddr7_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[0].size - VR_BLKCAP_BURSTLEN);
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr7_reg, dc2h_cap_boundaryaddr7_reg.regValue);
				dc2h_cap_boundaryaddr8_reg.regValue = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr8_reg, dc2h_cap_boundaryaddr8_reg.regValue);

				dc2h_cap_boundaryaddr9_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[0].size - VR_BLKCAP_BURSTLEN);
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr9_reg, dc2h_cap_boundaryaddr9_reg.regValue);
		...
		else
		{
			...
			//dc2h_cap_boundaryaddr1_reg.regValue =  ((UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[0].size - VR_SEQCAP_BURSTLEN);// sub burst len
			//rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr1_reg, dc2h_cap_boundaryaddr1_reg.regValue);
			//dc2h_cap_boundaryaddr2_reg.regValue =  (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr;
			//rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr2_reg, dc2h_cap_boundaryaddr2_reg.regValue);
		...
		else
		{
            ...
            if(Get_PANEL_VFLIP_ENABLE())
            {
                if(get_VR_Pixel_Format() == VR_CAP_RGB888)
                    dc2h_cap_l1_start_reg.regValue = ((UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr + vr_cap_frame_max_width*vr_cap_frame_max_height*3 - vr_cap_frame_max_width*3);
                else if(get_VR_Pixel_Format() == VR_CAP_ARGB8888 || get_VR_Pixel_Format() == VR_CAP_RGBA8888 || get_VR_Pixel_Format() == VR_CAP_ABGR8888)
                    dc2h_cap_l1_start_reg.regValue = ((UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr + vr_cap_frame_max_width*vr_cap_frame_max_height*4 - vr_cap_frame_max_width*4);
                else
                    rtd_pr_vr_emerg("[error]unknow pixel format\n");
            }
            else
...
{	
	if(0 == buf_cnt) /* Release buffers by setting buf_cnt = 0*/
	{
		if((vr_capture_location == KADP_VR_DISPLAY_OUTPUT) || (vr_capture_location == KADP_VR_SCALER_OUTPUT) 
			|| (vr_capture_location == KADP_VR_OSDVIDEO_OUTPUT) || (vr_capture_location == KADP_VR_SCALER_INPUT))
		{
...
unsigned char HAL_VR_GetVideoFrameOutputDeviceLimitation(KADP_VR_VIDEO_WINDOW_TYPE_T videoWindowID, KADP_VR_VIDEO_FRAME_OUTPUT_DEVICE_LIMITATION_INFO_T *pVideoFrameOutputDeviceLimitationInfo)
{
	...
	down(&VR_Semaphore);
	pVideoFrameOutputDeviceLimitationInfo->maxResolution = (KADP_VR_RECT_T){0,0,VR_CAP_FRAME_WIDTH_4K2K,VR_CAP_FRAME_HEIGHT_4K2K};
...
unsigned char HAL_VR_SetVideoFrameOutputDeviceOutputRegion(KADP_VR_VIDEO_WINDOW_TYPE_T videoWindowID, KADP_VR_DUMP_LOCATION_TYPE_T dumpLocation, KADP_VR_RECT_T *pOutputRegion)
{
	...
	if(get_vr_function() == TRUE) {
		if((dumpLocation==VRDumpLocation) && (pOutputRegion->w == curCapInfo.capWid) && (pOutputRegion->h == curCapInfo.capLen)) {
			...
			if(KADP_VR_SCALER_OUTPUT == dumpLocation || KADP_VR_SCALER_INPUT == dumpLocation)
				VFODStateVR.bAppliedPQ = FALSE;
			else
				VFODStateVR.bAppliedPQ = TRUE;
...
unsigned char vr_dump_data_to_file(unsigned int idx, unsigned int dump_w,unsigned int dump_h)
{
	...
	if((get_VR_Pixel_Format() == VR_CAP_NV12) || (get_VR_Pixel_Format() == VR_CAP_NV16) || (get_VR_Pixel_Format() == VR_CAP_NV21))
	{
		if (pVirStartAdrr != 0)
		{
			...
			if (filp == NULL)
			{
				rtd_pr_vr_notice("[VR]file open Y fail\n");
			...
			if (filp == NULL)
			{
				rtd_pr_vr_notice("[VR]file open fail\n");
				dvr_unmap_memory(pVirStartAdrr, size);
				return FALSE;
			}
			...
			rtd_pr_vr_notice("[VR]dump_finish\n");
			g_ulFileCount++;
			dvr_unmap_memory(pVirStartAdrr, size);
			return TRUE;
		}
...
ssize_t vivr_write(struct file *filep, const char *buffer, size_t count, loff_t *offp)
{
			...
			&& (cmd_buf[5] == '='))
	{
		...
		if(((cmd_buf[6]-0x30) >= VR_CAP_RGB888) && ((cmd_buf[6]-0x30) <= VR_CAP_NV16))
		{
