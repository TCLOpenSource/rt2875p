 ...
 ************************************************************************/
		...
		case VPQ_EXTERN_IOC_SET_Gamma_byParameters:
		{
			...
			if (copy_from_user(&hal_VPQ_DATA_EXTERN, (int __user *)arg, sizeof(HAL_VPQ_DATA_EXTERN_T))) {
				...
				// length = 4：set by one parameter(gain)
				// version：0=inv-gamma, 1=gamma, 2=inv-out-gamma, 3=out-gamma
				// wId：0=R, 1=G, 2=B, 3=all
				if ((hal_VPQ_DATA_EXTERN.length == sizeof(int)) || (hal_VPQ_DATA_EXTERN.version == 2) ||(hal_VPQ_DATA_EXTERN.wId == GAMMA_CHANNEL_B)) {
					Data_addr = hal_VPQ_DATA_EXTERN.pData;
					if(copy_from_user(&DEM_arg, (int __user *)Data_addr, hal_VPQ_DATA_EXTERN.length))
					{
	...
	case VPQ_EXTERN_IOC_SET_SLR_HDR_TABLE:
	{
		...
		// if this cmd success, will call Scaler_color_copy_HDR_table_from_AP() and get current EOTF、OETF table from .c panel luminance
		// on the contrary, if cmd fail, we will call Scaler_color_Set_EOTF_OETF_Table to get current EOTF、OETF table from .c panel luminance
		...
		if(ptr_HDR_bin != NULL)
		{
			if (copy_from_user(&hdr_table_size, (void __user *)arg, sizeof(unsigned int))) { //size check
				rtd_pr_vpq_err("kernel copy VPQ_EXTERN_IOC_SET_SLR_HDR_TABLE fail\n");
  				...
  				rtd_pr_vpq_err("VPQ_EXTERN_IOC_SET_SLR_HDR_TABLE struct size, AP=%d, driver=%zu!!\n", hdr_table_size, sizeof(SLR_HDR_TABLE));
				if (hdr_table_size != sizeof(SLR_HDR_TABLE)) {
					rtd_pr_vpq_err("VPQ_EXTERN_IOC_SET_SLR_HDR_TABLE struct size miss match, AP=%d, driver=%zu!!\n",
						hdr_table_size, sizeof(SLR_HDR_TABLE));
					...
					Scaler_color_Set_EOTF_OETF_Table();
						...
						rtd_pr_vpq_info("kernel copy VPQ_EXTERN_IOC_SET_SLR_HDR_TABLE success\n");
