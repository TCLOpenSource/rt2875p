...
static void scalerip_bypass(unsigned char byPass)
{
	...
	if(byPass){ // bypass
		if(display){
			...
			IoReg_SetBits(DI_IM_DI_WEAVE_WINDOW_CONTROL_reg, DI_IM_DI_WEAVE_WINDOW_CONTROL_force2d_mask);
		}
	}else{
		if(display){
			//data_path_select_reg.xcdi_in_sel = 1; 
...
{
#if 1	/* since DI use rolling line mode, can't disable DI */
	...
	if (DI_IP_En_Lv2 == 0) {
		...
		rtd_pr_vbe_info("DI_Enable PQCMode=%d,\n", drv_memory_di_Get_Compression_Mode(0));
	} else {
#if 1
		// set force 2D enable if flow need to disable DI for saving BW
		di_im_di_weave_window_control_reg.regValue = IoReg_Read32(DI_IM_DI_WEAVE_WINDOW_CONTROL_reg);
	...
	if (diflag && Scaler_DispGetStatus(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY),SLR_DISP_THRIP)) {
		if(DI_IM_DI_CONTROL_get_ma_3aenable(IoReg_Read32(DI_IM_DI_CONTROL_reg))) {		// 3A
			set_di_wait_frame_num(3);
		} else {	// 5A
			set_di_wait_frame_num(5);
		}
	} else
		set_di_wait_frame_num(0);
	set_di_record_timer_counter(IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg));//record current 90k counter
	return 0;

#else
	unsigned char timeout = 0,  frame_num=0;
...
{
//	unsigned short temp;
//	extern char DI_PQC_CRC_Check_reset_cnt;//mac7p pq compile fix
	...
	//	if (!Scaler_Get_DisplayRatio_Change_Flag())
		...
		if(display == _CHANNEL1)
		{
			...
			if(vgip_chn1_ctrl_reg.ch1_digital_mode) //digital mode need to set porch
			{
				...
				if(Scaler_InputSrcGetType(display) == _SRC_VO){
					if ((vsc_get_adaptivestream_flag(display))&& ((Scaler_DispGetInputInfo(SLR_INPUT_H_LEN) == VO_FIXED_4K2K_HTOTAL) ||(Scaler_DispGetInputInfo(SLR_INPUT_H_LEN) == VO_FIXED_2K1K_HTOTAL)))
					{
						...
						h_porch_num = Scaler_DispGetInputInfo(SLR_INPUT_H_LEN) - Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID_PRE) - VGIP_H_PORCH_MARGIN;
						...
						vgip_chn1_misc_reg.ch1_hporch_num = h_porch_num;
					}
					else{
				...
				else//hdmi case
				{
					...
					if(Scaler_DispGetInputInfo(SLR_INPUT_H_LEN) > (Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID_PRE) + VGIP_H_PORCH_MARGIN)){
						...
						if(vgip_chn1_misc_reg.ch1_hporch_num < Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_STA_PRE))
						{
							vgip_chn1_misc_reg.ch1_hporch_num = Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_STA_PRE);
							rtd_pr_vbe_notice("The H porch number is smaller than act start!!!!\n");
						}
					}
					else{
						vgip_chn1_misc_reg.ch1_hporch_num = Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_STA_PRE);
...
//		scalerip_modify_rdstart();
		...
		drvif_color_rtnr_progressive_check(Scaler_DispGetStatus((SCALER_DISP_CHANNEL)display,SLR_DISP_THRIP),Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL), 0);
		...
		if (Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY) {
            ...
            //Scaler_color_set_RTNR_H_Rotate(FALSE);

			rtnr_new_Ctrl.regValue = IoReg_Read32(DI_IM_DI_RTNR_NEW_CONTROL_reg);
			//if(Scaler_DispGetInputInfo(SLR_INPUT_DI_WID)>1920 && (Scaler_color_access_RTNR_H_Rotate_Flag(0, 0) == 0)){ //mac7p pq compile fix
...
{
#ifndef BUILD_QUICK_SHOW //wait pq share code ready then move
	unsigned short display;
	display = Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY);
	drvif_IESM_format_convert(); //drvif_IESM_format_convert() have to before drvif_IESM_frontporch_set()
...
#ifndef BUILD_QUICK_SHOW
	if(isEnable ==1){	//set line buffer mode for game mode to reduce delay time
		drvif_IESM_frontporch_set(1); //disable snr line buffer mode for game mode
		drvif_scalerdisplay_set_sharpness_line_mode(); //sharpness 1 line buffer mode for game mode
	}else{
		drvif_IESM_frontporch_set(0); //set snr line buffer mode
		drvif_scalerdisplay_set_sharpness_line_mode(); //sharpness line buffer mode
	}
#else
	if(isEnable ==1){	//set line buffer mode for game mode to reduce delay time
		drvif_IESM_frontporch_set(1); //disable snr line buffer mode for game mode
		...
		drvif_IESM_frontporch_set(0); //set snr line buffer mode
		drvif_scalerdisplay_set_sharpness_line_mode_quick_show(0); //sharpness line buffer mode
	}
#endif
}
...
void set_di_record_timer_counter(unsigned int time)
{
	record_timer_counter = time;
}
