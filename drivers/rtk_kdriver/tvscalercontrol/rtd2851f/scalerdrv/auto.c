...
static void drvif_get_h_boundary(unsigned char display)
{
	...
	if(get_ADC_Input_Source() == _SRC_VGA){
		...
		ulparam2 = (unsigned long long)((p_dispinfo->IVFreq * p_dispinfo->IHTotal * p_dispinfo->IVTotal)/1000000);
		//do_div(ulparam2, 10000000); // clock unit MHz
	...
	rtd_pr_adc_notice("Auto H boundary = %d\n",stAUTO_PARA.usAutoHStart);
	...
	rtd_pr_adc_notice("Auto H boundary1 = %d\n",stAUTO_PARA.usAutoHStart);
}
...
MEASURE_V_POS:
	...
	*v_end = smartfit_auto_result_vsta_end_reg.vx_end;
	...
	if((*v_end== 0x0000)||(*v_end<=*v_start))
	{
		...
		rtd_pr_adc_notice("#####[%s(%d)]0xb8022004=0x%08x, 0xb8022008=0x%08x\n", __func__, __LINE__, IoReg_Read32(0xb8022004),IoReg_Read32(0xb8022008));
		rtd_pr_adc_notice("#####[%s(%d)]0xb802200c=0x%08x, 0xb8022010=0x%08x\n", __func__, __LINE__, IoReg_Read32(0xb802200c),IoReg_Read32(0xb8022010));
		rtd_pr_adc_notice("#####[%s(%d)]0xb8022014=0x%08x, 0xb8022018=0x%08x\n", __func__, __LINE__, IoReg_Read32(0xb8022014),IoReg_Read32(0xb8022018));
		if(vcnt++ > 5){//timeout
...
//	adc_adc_dcrestore_ctrl_RBUS adc_adc_dcrestore_ctrl_reg;
	...
	color_d_vc_global_ctrl_reg.ms_vc_en = 0;
	...
	color_d_vc_global_ctrl_reg.regValue = IoReg_Read32(COLOR_D_VC_Global_CTRL_reg);
	color_d_vc_global_ctrl_reg.ms_vc_en = 1;
	color_d_vc_global_ctrl_reg.write_enable_11 =1;
	IoReg_Write32(COLOR_D_VC_Global_CTRL_reg, color_d_vc_global_ctrl_reg.regValue);
...
void drvif_self_awb_initial_setting(void)//Auto color initial seeting at beginning
{
	...
	dds_pll_pllphase_ctrl_reg.pllsph0path = 1; //merlin5 c vrsion (0: short path, 1: long path)
	...
	adc_adc_vbias_reg.adc_vbias0_0 = 1;/*ADC Sample 011 */
									   /* LDO voltage select 011:1.1v bit[7:5]=101 bit[4:2]=000 bit0=1*/
	adc_adc_vbias_reg.adc_vbias0_1 = 0;
	adc_adc_vbias_reg.adc_vbias0_4_2 = YPbPr_LDO_Trim()?(YPbPr_LDO_Trim_vbias_4_3()<<1):0;
	adc_adc_vbias_reg.adc_vbias0_7_5 = YPbPr_LDO_Trim()?(YPbPr_LDO_Trim_vbias_5()|0x4):5;
	adc_adc_vbias_reg.adc_vbias1_3_2 = 3;//BIT[3:2]SF driving, 0:840uA,1:960uA,2:1080uA,3:1200uA
...
void OTP_Initial_Setting(void)
{
    ...
    IoReg_Write32(ADC_ADC_VBIAS_reg, YPbPr_LDO_Trim()?((YPbPr_LDO_Trim_vbias_5()<<5)|(YPbPr_LDO_Trim_vbias_4_3()<<3)|0x01b3cca1):0x01b3cca1);
...
EXPORT_SYMBOL(drvif_set_smartfit_boundary_with_no_hs_mode);
...
EXPORT_SYMBOL(drvif_set_smartfit_auto_field_ctrl_no_hs_mode);
...
static void smartfit_no_hs_mode(unsigned char display, int *hstart, int *hend, int *vstart, int *vend)
{
...
EXPORT_SYMBOL(smartfit_no_hs_mode);
