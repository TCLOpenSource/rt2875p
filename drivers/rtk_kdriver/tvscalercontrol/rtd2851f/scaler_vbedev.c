...
//VBE_DISP_PANEL_EDP_SUSPEND_RESUME_T vbe_disp_panel_edp_suspend_resume;
...
void vbe_disp_setmute(unsigned char bOnOff)
{
	...
	if(bOnOff){
		//rtd_pr_vbe_notice("vbe_disp_setmute bOnOff = %d\n", bOnOff);
...
unsigned char scaler_game_mode_dynamic_change_clk_flag = 0;
unsigned char vbe_disp_get_game_mode_dynamic_change_clk_flag(void)
{
	return scaler_game_mode_dynamic_change_clk_flag;
}
void vbe_disp_set_game_mode_dynamic_change_clk_flag(unsigned char enable)
{
	scaler_game_mode_dynamic_change_clk_flag = enable;
}

unsigned char (*Get_Val_game_mode_dynamic_change_clk_flag_impl)(void) = vbe_disp_get_game_mode_dynamic_change_clk_flag;

void (*Set_Val_game_mode_dynamic_change_clk_flag_impl)(unsigned char) = vbe_disp_set_game_mode_dynamic_change_clk_flag;

unsigned int vbe_disp_check_support_game_mode_dynamic_change_clk(void)
{	
	extern UINT16 panel_support_max_frame_rate;
	
	if(((Get_DISPLAY_PANEL_TYPE() == P_EDP_4K2K) ||
		(Get_DISPLAY_PANEL_TYPE() == P_EDP_4K2K_2PORT) ||
		(Get_DISPLAY_PANEL_TYPE() == P_EDP_3K1K) ||
		(Get_DISPLAY_PANEL_TYPE() == P_EDP_4K1K)) && (panel_support_max_frame_rate >= 144))
		return TRUE;
	else
		return FALSE;
}
...
void vbe_disp_set_game_mode_dynamic_change_settings(unsigned char display)
{
	...
	unsigned int mul_dbuf_cnt = 0;

	rtd_pr_vbe_notice("vbe_disp_set_game_mode_dynamic_change_settings start!!\n");

	modestate_decide_dtg_m_modeByDisp(display);
	modestate_decide_fractional_framesyncByDisp(display);
	...
	*/
	...
	drvif_clock_dynamic_reset_dclk();//reset dclk

	GDMA_Clk_Select(0, true);//reset GDMA clock
	...
	while(!((dtg_frame_cnt1_reg.uzudtg_output_frame_cnt == uzu_valid_framecnt) &&
		...
		(PPOVERLAY_new_meas2_linecnt_real_get_uzudtg_dly_line_cnt_rt(rtd_inl(PPOVERLAY_new_meas2_linecnt_real_reg)) < 1270)) && --count){
		dtg_frame_cnt1_reg.regValue = rtd_inl(PPOVERLAY_DTG_FRAME_CNT1_reg);
	}
	if(count == 0)
	...
	if(vbe_disp_get_game_mode_dynamic_change_clk_flag()){
		if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= 235) &&
			(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) <= 605))
			vbe_disp_config_dynamic_edp_interface(0);//set EDP mode 0
		else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= 995) &&
			(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) <= 1205))
			vbe_disp_config_dynamic_edp_interface(1);//set EDP mode 1
		else
			vbe_disp_config_dynamic_edp_interface(2);//set EDP mode 2
	}
	else
		vbe_disp_config_dynamic_edp_interface(2);//set EDP mode 2

	rtd_pr_vbe_notice("vbe_disp_set_game_mode_dynamic_change_settings finish!!\n");
}

void vbe_disp_config_dynamic_edp_interface(unsigned char mode)
{
	if(mode == 0){
		rtd_pr_vbe_notice("vbe_disp_config_dynamic_edp_interface use mode 0\n");
		/* For input 24~60 frame rate */
		//SFG
		IoReg_Write32(0xb802D950, 0x05B40014);   // DV_Den_end,DV_Den_sta, (follow DTG timing) 
		IoReg_Write32(0xb802D954, 0x0A300030);   // DH_Den_end,DH_Den_sta, (follow DTG timing) 
		IoReg_Write32(0xb802D958, 0x051F14AA);   // DV_width,Dh_width,Dh_total (follow DTG timing) 
		IoReg_Write32(0xb802D964, 0x00000001);   // SFG Normal Mode, MSO=dis, 1.5 packet mode disable, H-Flip disable, 1-SEG, 2Port(1MAC),
		IoReg_Write32(0xb802D968, 0x00000000);   // SEG0/SEG1 Start Address,
		////--------------------------------TX0_mac--------------------------------
		IoReg_Write32(0xb8066324, 0x00001BDA);      // [23:0]M_vid=0x1BDA
		//MSA
		IoReg_Write32(0xb8066338, 0x00000040);      // [7] msa_db=0 [6]=1 just before MSA
		IoReg_Write32(0xb806633C, 0x00000020);      // [14]:color format follow misc1.6, [13]ycc_col=601, [12]VESA, [11]Async,[10:9] no stereo, [8]interlace odd num,
		                                       // [7:5]:color bit=8 bit [4:3]format=RGB,
		IoReg_Write32(0xb8066340, 0x00000B40);      // [15:0]:Htot = 2880 
		IoReg_Write32(0xb8066344, 0x00000030);      // [15:0]:Hsta = 0x30 = 48
		IoReg_Write32(0xb8066348, 0x00000A00);      // [15:0]:Hwidth = 2560
		IoReg_Write32(0xb806634C, 0x0000001F);      // [15]HSP=low, [14:0]:Hsw = 0x1F = 31
		IoReg_Write32(0xb8066350, 0x00000AA0);      // [15:0]:Vtot = 2720
		IoReg_Write32(0xb8066354, 0x0000000D);      // [15:0]:Vsta = 0xD = 13
		IoReg_Write32(0xb8066358, 0x000005A0);      // [15:0]:Vhight = 0x5A0 = 1440
		IoReg_Write32(0xb806635C, 0x00000005);      // [15]VSP=low, [14:0]:Vsw = 5
		//// ARBITER
		IoReg_Write32(0xb806636C, 0x00000780);      // [15:0]vdata_per_line=0x0780 (1920)
			                                       // Hwidth * BPC (bit per component) * 3 / 8 (byte) = lane_num * v_data_per_line (byte)
		IoReg_Write32(0xb8066374, 0x00000150);      // [9:3]tu_data_size=0x1D, [2:0]=0.2=0x0
                                       				// tu_data_size =-> SCLK(freq) * BPC * 3 /8 (byte)= (TU_DATA_SIZE/TU) * LCLK(freq) * LANE_NUM (byte)
		IoReg_Write32(0xb8066378, 0x0000015d);      // [15:0]:hdelay  
	}
	else if(mode == 1){
		rtd_pr_vbe_notice("vbe_disp_config_dynamic_edp_interface use mode 1\n");
		/* For input 100~120 frame rate */
		//SFG
		IoReg_Write32(0xb802D950, 0x05B40014);   // DV_Den_end,DV_Den_sta, (follow DTG timing) 
		IoReg_Write32(0xb802D954, 0x0A300030);   // DH_Den_end,DH_Den_sta, (follow DTG timing) 
		IoReg_Write32(0xb802D958, 0x051F0AA0);   // DV_width,Dh_width,Dh_total (follow DTG timing) 
		IoReg_Write32(0xb802D964, 0x00000001);   // SFG Normal Mode, MSO=dis, 1.5 packet mode disable, H-Flip disable, 1-SEG, 2Port(1MAC),
		IoReg_Write32(0xb802D968, 0x00000000);   // SEG0/SEG1 Start Address,
		////--------------------------------TX0_mac--------------------------------
		IoReg_Write32(0xb8066324, 0x00001CA5);      // [23:0]M_vid=0x1CA5
		//MSA
		IoReg_Write32(0xb8066338, 0x00000040);      // [7] msa_db=0 [6]=1 just before MSA
		IoReg_Write32(0xb806633C, 0x00000020);      // [14]:color format follow misc1.6, [13]ycc_col=601, [12]VESA, [11]Async,[10:9] no stereo, [8]interlace odd num,
		                                       // [7:5]:color bit=8 bit [4:3]format=RGB,
		IoReg_Write32(0xb8066340, 0x00000AA0);      // [15:0]:Htot = 0xAA0 = 2720
		IoReg_Write32(0xb8066344, 0x00000030);      // [15:0]:Hsta = 48
		IoReg_Write32(0xb8066348, 0x00000A00);      // [15:0]:Hwidth = 2560
		IoReg_Write32(0xb806634C, 0x0000001F);      // [15]HSP=low, [14:0]:Hsw = 0x1F = 31
		IoReg_Write32(0xb8066350, 0x000005C9);      // [15:0]:Vtot = 0x5C9 = 1481
		IoReg_Write32(0xb8066354, 0x0000000D);      // [15:0]:Vsta = 0xD = 13
		IoReg_Write32(0xb8066358, 0x000005A0);      // [15:0]:Vhight = 0x5A0 = 1440
		IoReg_Write32(0xb806635C, 0x00000005);      // [15]VSP=low, [14:0]:Vsw = 5
		//// ARBITER
		IoReg_Write32(0xb806636C, 0x00000780);      // [15:0]vdata_per_line=0x0780 (1920)
			                                       // Hwidth * BPC (bit per component) * 3 / 8 (byte) = lane_num * v_data_per_line (byte)
		IoReg_Write32(0xb8066374, 0x00000158);      // [9:3]tu_data_size=0x1D, [2:0]=0.2=0x0
                                       				// tu_data_size =-> SCLK(freq) * BPC * 3 /8 (byte)= (TU_DATA_SIZE/TU) * LCLK(freq) * LANE_NUM (byte)
		IoReg_Write32(0xb8066378, 0x00000153);      // [15:0]:hdelay 
	}
	else if(mode == 2){
		rtd_pr_vbe_notice("vbe_disp_config_dynamic_edp_interface use mode 2\n");
		/* Original settings */
		//SFG
		IoReg_Write32(0xb802D950, 0x05B40014);   // DV_Den_end,DV_Den_sta, (follow DTG timing) 
		IoReg_Write32(0xb802D954, 0x0A300030);   // DH_Den_end,DH_Den_sta, (follow DTG timing) 
		IoReg_Write32(0xb802D958, 0x051F0A50);   // DV_width,Dh_width,Dh_total (follow DTG timing) 
		IoReg_Write32(0xb802D964, 0x00000001);   // SFG Normal Mode, MSO=dis, 1.5 packet mode disable, H-Flip disable, 1-SEG, 2Port(1MAC),
		IoReg_Write32(0xb802D968, 0x00000000);   // SEG0/SEG1 Start Address,
		////--------------------------------TX0_mac--------------------------------
		IoReg_Write32(0xb8066324, 0x000025D7);      // [23:0]M_vid=0x25D7
		//MSA
		IoReg_Write32(0xb8066338, 0x00000040);      // [7] msa_db=0 [6]=1 just before MSA
		IoReg_Write32(0xb806633C, 0x00000020);      // [14]:color format follow misc1.6, [13]ycc_col=601, [12]VESA, [11]Async,[10:9] no stereo, [8]interlace odd num,
		                                       // [7:5]:color bit=8 bit [4:3]format=RGB,
		IoReg_Write32(0xb8066340, 0x00000A50);      // [15:0]:Htot = 0xA50 = 2640
		IoReg_Write32(0xb8066344, 0x00000030);      // [15:0]:Hsta = 0x30 = 48
		IoReg_Write32(0xb8066348, 0x00000A00);      // [15:0]:Hwidth = 2560
		IoReg_Write32(0xb806634C, 0x0000003F);      // [15]HSP=low, [14:0]:Hsw = 0x3F = 63
		IoReg_Write32(0xb8066350, 0x000005BA);      // [15:0]:Vtot = 0x5BA = 1466
		IoReg_Write32(0xb8066354, 0x0000000D);      // [15:0]:Vsta = 0xD = 13
		IoReg_Write32(0xb8066358, 0x000005A0);      // [15:0]:Vhight = 0x5A0 = 1440
		IoReg_Write32(0xb806635C, 0x00000005);      // [15]VSP=low, [14:0]:Vsw = 5
		//// ARBITER
		IoReg_Write32(0xb806636C, 0x00000780);      // [15:0]vdata_per_line=0x0780 (1920)
			                                       // Hwidth * BPC (bit per component) * 3 / 8 (byte) = lane_num * v_data_per_line (byte)
		IoReg_Write32(0xb8066374, 0x000001D6);      // [9:3]tu_data_size=0x34<<3=1A6, [2:0]=0.8=0x6
			                                       // tu_data_size =-> SCLK(freq) * BPC * 3 /8 (byte)= (TU_DATA_SIZE/TU) * LCLK(freq) * LANE_NUM (byte)
		IoReg_Write32(0xb8066378, 0x000000C8);      // [15:0]:hdelay
	}
}
...
unsigned int VSYNCGEN_get_vodma_clk_0(void)
{
	...
	rtd_pr_vbe_emerg("[vo] vsyncgen busclk: %d/%d/%d, divider_num=%d, ratio_n_off=%d\n", nMCode, nPrediv, busclk, divider_num, ratio_n_off);

	return vodmapll;
}

#ifndef BUILD_QUICK_SHOW
unsigned int vbe_get_VDEC4K_run_datafs_condition_impl(void){
	if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC)
		&& (get_panel_res() != PANEL_RES_FHD)
		&& (get_panel_res() != PANEL_RES_HD)
...
unsigned int (*vbe_get_VDEC4K_run_datafs_condition)(void) = vbe_get_VDEC4K_run_datafs_condition_impl;
...
unsigned int Scaler_get_uzu_vfreqX10(unsigned char display)
{
    ...
    */
    ...
    if(Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_FRAC_FS_REMOVE_RATIO) != 0)
    {
        vfreqX10 /= Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_FRAC_FS_REMOVE_RATIO);
    }
    else
    {
        rtd_pr_vbe_info("[ERR]SLR_INPUT_FRAC_FS_REMOVE_RATIO is 0 !!!\n");
    }
    vfreqX10 /= uzuRemove;
...
#endif
extern unsigned char main_vsc_connect_ready;
void HAL_VBE_DISP_SetDisplayOutput(BOOLEAN bOnOff)
{
	...
	if(bOnOff)
	{
	     if(display_timing_ctrl1_reg.disp_en == 0) {
	...
	else
	{
		...
		#endif
            if(!((main_vsc_connect_ready==TRUE) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) != _MODE_STATE_SEARCH))) {
...
//- If percent is 110, driver set SSC percent set 1.1 %.
//- If Period is 25300, driver set SSC percent set 25.3 KHz.
...
#ifdef CONFIG_ORBIT_PIXEL_SHIFT_ALGO
	enable = vbe_disp_oled_orbit_enable;
#else
	enable = FALSE;
#endif
...
void vbe_disp_panel_suspend(void)
{
    ...
    vbe_disp_suspend_resume.SFG_DISP_IF_CTRL_00_S_R =                IoReg_Read32(SFG_DISP_IF_CTRL_00_reg);
    vbe_disp_suspend_resume.SFG_DISP_IF_CTRL_02_S_R =                IoReg_Read32(SFG_DISP_IF_CTRL_02_reg);
    vbe_disp_suspend_resume.SFG_DISP_IF_CTRL_03_S_R =                IoReg_Read32(SFG_DISP_IF_CTRL_03_reg);
    vbe_disp_suspend_resume.SFG_DISP_IF_CTRL_04_S_R =                IoReg_Read32(SFG_DISP_IF_CTRL_04_reg);
    vbe_disp_suspend_resume.SFG_DISP_IF_CTRL_05_S_R =                IoReg_Read32(SFG_DISP_IF_CTRL_05_reg);
    vbe_disp_suspend_resume.SFG_PIF_FIFO_CTRL_08_S_R =               IoReg_Read32(SFG_PIF_FIFO_CTRL_08_reg);
    vbe_disp_suspend_resume.SFG_PIF_FIFO_CTRL_09_S_R =               IoReg_Read32(SFG_PIF_FIFO_CTRL_09_reg);
    vbe_disp_suspend_resume.SFG_PIF_FIFO_CTRL_10_S_R =               IoReg_Read32(SFG_PIF_FIFO_CTRL_10_reg);
...
void vbe_disp_panel_resume(void)
{
    ...
    IoReg_Write32(SFG_DISP_IF_CTRL_00_reg,					  vbe_disp_suspend_resume.SFG_DISP_IF_CTRL_00_S_R);
    IoReg_Write32(SFG_DISP_IF_CTRL_02_reg,					  vbe_disp_suspend_resume.SFG_DISP_IF_CTRL_02_S_R);
    IoReg_Write32(SFG_DISP_IF_CTRL_03_reg,					  vbe_disp_suspend_resume.SFG_DISP_IF_CTRL_03_S_R);
    IoReg_Write32(SFG_DISP_IF_CTRL_04_reg,					  vbe_disp_suspend_resume.SFG_DISP_IF_CTRL_04_S_R);
    IoReg_Write32(SFG_DISP_IF_CTRL_05_reg,					  vbe_disp_suspend_resume.SFG_DISP_IF_CTRL_05_S_R);
    IoReg_Write32(SFG_PIF_FIFO_CTRL_08_reg,					  vbe_disp_suspend_resume.SFG_PIF_FIFO_CTRL_08_S_R);
    IoReg_Write32(SFG_PIF_FIFO_CTRL_09_reg,					  vbe_disp_suspend_resume.SFG_PIF_FIFO_CTRL_09_S_R);
    IoReg_Write32(SFG_PIF_FIFO_CTRL_10_reg,					  vbe_disp_suspend_resume.SFG_PIF_FIFO_CTRL_10_S_R);
	...
	{
		//eDP Dphy
		IoReg_Write32(0xb802df04, 0x000000f0); // lane assign
		IoReg_Write32(0xb802df10, 0x03020100); // lane assign
		IoReg_Write32(0xb802df14, 0x03020100); // lane assign

		//---------------------------------------------------------------------------------------------------------------------------------
		//--------------------------------TX0_Dphy--------------------------------
		IoReg_Write32(0xb8066200, 0x0000000C);  //[7:0]: 4 lane
		IoReg_Write32(0xb8066204, 0x00000042);  //[7:0]: Video pattern, DB enable
		                                   //[31:8]: Reserved
		IoReg_Write32(0xb8066234, 0x00000015);  //insert_en=1, skew_en, normal scb, scb_en
		IoReg_Write32(0xb8066238, 0x0000001B);  //[7:0] lane swap, lane0=0,lane1=1,lane2=2,lane3=3
		IoReg_Write32(0xb806623C, 0x00000010);  //[7:0] sr_insert_per_MSB(total=512), tst_mode=0;
		IoReg_Write32(0xb8066240, 0x000000FF);  //[7:0] sr_insert_per_LSB(total=512)

		IoReg_Write32(0xb8066244, 0x00000000);  //[7:0] lane1/lane0: skew no delay
		IoReg_Write32(0xb8066248, 0x00000000);  //[7:0] lane3/lane2: skew no delay
		...
		//---------------------------------------------------------------------------------------------------------------------------------
		////--------------------------------TX0_mac--------------------------------
		if(Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_EDP_P0_QHD_165_638M) {
			IoReg_Write32(0xb8066300, 0x000000C7);      // [7:0]:MSO=4 lane, enhance mode=EN, 4-lane
			IoReg_Write32(0xb8066304, 0x00000000);      // [7:0]:Pg_rst=0,msa_rst=0
			IoReg_Write32(0xb8066308, 0x00000000);      // [7:0]:debug sel
			IoReg_Write32(0xb806630C, 0x00000000);      // [7:0]:irq disable
			IoReg_Write32(0xb806631C, 0x00000000);      // IE disable
			IoReg_Write32(0xb8066320, 0x00000000);      // [7] MN_vid auto=dis, [6]:db, [0]:popup
			IoReg_Write32(0xb8066324, 0x000025D7);      // [23:0]M_vid=0x25D7
			IoReg_Write32(0xb8066328, 0x00002000);      // [23:0]N_vid=0x2000
			IoReg_Write32(0xb8066330, 0x00000000);      // [7:0] N_mul=0;
			IoReg_Write32(0xb8066320, 0x00000040);      // DB
			...
			//// VBID
			IoReg_Write32(0xb8066360, 0x00000000);      // [2]audiomute_flag [1]novideo_flag
			IoReg_Write32(0xb8066364, 0x00000000);      // [1]interlace=0 [0]vbid_fw_ctrl=HW mode

			//// ARBITER
			IoReg_Write32(0xb8066368, 0x00000012);      // [6]idel switch by VS, [5]db=0, [4]video_idle_db=vact_edge[3]Video_idle=0, [2] Hwid_div2=0,[1]vactive_md=new mode [0]arbiter_en=0
			IoReg_Write32(0xb806636C, 0x00000780);      // [15:0]vdata_per_line=0x0780 (1920)
			                                       // Hwidth * BPC (bit per component) * 3 / 8 (byte) = lane_num * v_data_per_line (byte)
			IoReg_Write32(0xb8066370, 0x00000040);      // [7]TU_size<1,[6:0]:tu_size=0x40 (64)
			IoReg_Write32(0xb8066374, 0x000001D6);      // [9:3]tu_data_size=0x34<<3=1A6, [2:0]=0.8=0x6
			                                       // tu_data_size =-> SCLK(freq) * BPC * 3 /8 (byte)= (TU_DATA_SIZE/TU) * LCLK(freq) * LANE_NUM (byte)
			IoReg_Write32(0xb8066378, 0x000000C8);      // [15:0]:hdelay
			IoReg_Write32(0xb806637C, 0x00000064);      // [7:0]:sec_end_cnt LSB[7:0]
			IoReg_Write32(0xb80663AC, 0x00000046);      // [7:0]:sec_end_cnt MSB[15:8]
			IoReg_Write32(0xb8066380, 0x00000080);      // sec_end db apply
		}
		else if(Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_EDP_P0_4K2K_60_530D64M) {
			IoReg_Write32(0xb8066300, 0x000000C7);	   // [7:0]:MSO=4 lane, enhance mode=EN, 4-lane
			IoReg_Write32(0xb8066304, 0x00000000);	   // [7:0]:Pg_rst=0,msa_rst=0
			IoReg_Write32(0xb8066308, 0x00000000);	   // [7:0]:debug sel
			IoReg_Write32(0xb806630C, 0x00000000);	   // [7:0]:irq disable
			IoReg_Write32(0xb806631C, 0x00000000);	   // IE disable
			IoReg_Write32(0xb8066320, 0x00000000);	   // [7] MN_vid auto=dis, [6]:db, [0]:popup
			IoReg_Write32(0xb8066324, 0x00001F72);	   // [23:0]M_vid=0x1F72
			IoReg_Write32(0xb8066328, 0x00002000);	   // [23:0]N_vid=0x2000
			IoReg_Write32(0xb8066330, 0x00000000);	   // [7:0] N_mul=0;
			IoReg_Write32(0xb8066320, 0x00000040);	   // [7]=0 FW manually set M/N, [6]=1 double buffer apply

			//// MSA
			IoReg_Write32(0xb8066338, 0x00000040);	   // [7] msa_db=0 [6]=1 just before MSA
			IoReg_Write32(0xb806633C, 0x00000020);	   // [14]:color format follow misc1.6, [13]ycc_col=601, [12]VESA, [11]Async,[10:9] no stereo, [8]interlace odd num,
												   // [7:5]:color bit=8 bit [4:3]format=RGB,
			IoReg_Write32(0xb8066340, 0x00000FB4);	   // [15:0]:Htot = 4020
			IoReg_Write32(0xb8066344, 0x00000058);	   // [15:0]:Hsta = 88
			IoReg_Write32(0xb8066348, 0x00000F00);	   // [15:0]:Hwidth = 3840
			IoReg_Write32(0xb806634C, 0x00000020);	   // [15]HSP=low, [14:0]:Hsw = 0x20 = 32
			IoReg_Write32(0xb8066350, 0x00000898);	   // [15:0]:Vtot = 2200
			IoReg_Write32(0xb8066354, 0x00000014);	   // [15:0]:Vsta = 20
			IoReg_Write32(0xb8066358, 0x00000870);	   // [15:0]:Vhight = 0x0870 = 2160
			IoReg_Write32(0xb806635C, 0x00000005);	   // [15]VSP=low, [14:0]:Vsw = 5
			...
			//// ARBITER
			IoReg_Write32(0xb8066368, 0x00000012);	   // [6]idel switch by VS, [5]db=0, [4]video_idle_db=vact_edge[3]Video_idle=0, [2] Hwid_div2=0,[1]vactive_md=new mode [0]arbiter_en=0
			IoReg_Write32(0xb806636C, 0x00000B40);	   // [15:0]vdata_per_line=0x0B40 (2880)
												   // Hwidth * BPC (bit per component) * 3 / 8 (byte) = lane_num * v_data_per_line (byte)
			IoReg_Write32(0xb8066370, 0x00000040);	   // [7]TU_size<1,[6:0]:tu_size=0x40 (64)
			IoReg_Write32(0xb8066374, 0x0000017A);	   // [9:3]tu_data_size=0x1D, [2:0]=0.2=0x0
												   // tu_data_size =-> SCLK(freq) * BPC * 3 /8 (byte)= (TU_DATA_SIZE/TU) * LCLK(freq) * LANE_NUM (byte)
			IoReg_Write32(0xb8066378, 0x0000015e);	   // [15:0]:hdelay
			IoReg_Write32(0xb806637C, 0x00000064);	   // [7:0]:sec_end_cnt LSB[7:0]
			IoReg_Write32(0xb80663AC, 0x00000046);	   // [7:0]:sec_end_cnt MSB[15:8]
			IoReg_Write32(0xb8066380, 0x00000080);	   // sec_end db apply
		}
		else if(Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_EDP_P0_4K2K_60_533D28M) {
			IoReg_Write32(0xb8066300, 0x000000C7);      // [7:0]:MSO=4 lane, enhance mode=EN, 4-lane
			IoReg_Write32(0xb8066304, 0x00000000);      // [7:0]:Pg_rst=0,msa_rst=0  
			IoReg_Write32(0xb8066308, 0x00000000);      // [7:0]:debug sel
			IoReg_Write32(0xb806630C, 0x00000000);      // [7:0]:irq disable
			IoReg_Write32(0xb806631C, 0x00000000);      // IE disable
			IoReg_Write32(0xb8066320, 0x00000000);      // [7] MN_vid auto=dis, [6]:db, [0]:popup  
			IoReg_Write32(0xb8066324, 0x00001F9A);      // [23:0]M_vid=0x1F9A  
			IoReg_Write32(0xb8066328, 0x00002000);      // [23:0]N_vid=0x2000 
			IoReg_Write32(0xb8066330, 0x00000000);      // [7:0] N_mul=0; 
			IoReg_Write32(0xb8066320, 0x00000040);      // [7]=0 FW manually set M/N, [6]=1 double buffer apply 
			...
			IoReg_Write32(0xb806633C, 0x00000020);      // [14]:color format follow misc1.6, [13]ycc_col=601, [12]VESA, [11]Async,[10:9] no stereo, [8]interlace odd num,
							       // [7:5]:color bit=8 bit [4:3]format=RGB, 
			IoReg_Write32(0xb8066340, 0x00000FA0);      // [15:0]:Htot = 4000   
			IoReg_Write32(0xb8066344, 0x00000050);      // [15:0]:Hsta = 80
			IoReg_Write32(0xb8066348, 0x00000F00);      // [15:0]:Hwidth = 3840
			IoReg_Write32(0xb806634C, 0x00000020);      // [15]HSP=low, [14:0]:Hsw = 0x20 = 32
			IoReg_Write32(0xb8066350, 0x000008AE);      // [15:0]:Vtot = 2222	 
			IoReg_Write32(0xb8066354, 0x00000036);      // [15:0]:Vsta = 54
			IoReg_Write32(0xb8066358, 0x00000870);      // [15:0]:Vhight = 0x0870 = 2160       
			IoReg_Write32(0xb806635C, 0x00000005);      // [15]VSP=low, [14:0]:Vsw = 5	 
			...
			IoReg_Write32(0xb806636C, 0x00000B40);      // [15:0]vdata_per_line=0x0B40 (2880)
							       // Hwidth * BPC (bit per component) * 3 / 8 (byte) = lane_num * v_data_per_line (byte)  
			IoReg_Write32(0xb8066370, 0x00000040);      // [7]TU_size<1,[6:0]:tu_size=0x40 (64)	  
			IoReg_Write32(0xb8066374, 0x0000017C);      // [9:3]tu_data_size=0x1D, [2:0]=0.2=0x0
							       // tu_data_size =-> SCLK(freq) * BPC * 3 /8 (byte)= (TU_DATA_SIZE/TU) * LCLK(freq) * LANE_NUM (byte) 
			IoReg_Write32(0xb8066378, 0x00000154);      // [15:0]:hdelay 				 
			IoReg_Write32(0xb806637C, 0x00000064);      // [7:0]:sec_end_cnt LSB[7:0]
			IoReg_Write32(0xb80663AC, 0x00000046);      // [7:0]:sec_end_cnt MSB[15:8]  
			IoReg_Write32(0xb8066380, 0x00000080);      // sec_end db apply
		}
		else if(Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_EDP_P0_4K1K_60_319D896M) {
			IoReg_Write32(0xb8066300, 0x000000C7);	   // [7:0]:MSO=4 lane, enhance mode=EN, 4-lane
			IoReg_Write32(0xb8066304, 0x00000000);	   // [7:0]:Pg_rst=0,msa_rst=0	
			IoReg_Write32(0xb8066308, 0x00000000);	   // [7:0]:debug sel
			IoReg_Write32(0xb806630C, 0x00000000);	   // [7:0]:irq disable
			IoReg_Write32(0xb806631C, 0x00000000);	   // IE disable
			IoReg_Write32(0xb8066320, 0x00000000);	   // [7] MN_vid auto=dis, [6]:db, [0]:popup  
			IoReg_Write32(0xb8066324, 0x000012F4);	   // [23:0]M_vid=0x12F4  
			IoReg_Write32(0xb8066328, 0x00002000);	   // [23:0]N_vid=0x2000 
			IoReg_Write32(0xb8066330, 0x00000000);	   // [7:0] N_mul=0; 
			IoReg_Write32(0xb8066320, 0x00000040);	   // [7]=0 FW manually set M/N, [6]=1 double buffer apply 
			...
			//// MSA								  
			IoReg_Write32(0xb8066338, 0x00000040);	   // [7] msa_db=0 [6]=1 just before MSA 
			IoReg_Write32(0xb806633C, 0x00000040);	   // [14]:color format follow misc1.6, [13]ycc_col=601, [12]VESA, [11]Async,[10:9] no stereo, [8]interlace odd num,
												   // [7:5]:color bit=10 bit [4:3]format=RGB, 
			IoReg_Write32(0xb8066340, 0x00000E10);	   // [15:0]:Htot = 3600   
			IoReg_Write32(0xb8066344, 0x00000050);	   // [15:0]:Hsta = 80
			IoReg_Write32(0xb8066348, 0x00000D70);	   // [15:0]:Hwidth = 3440
			IoReg_Write32(0xb806634C, 0x00000020);	   // [15]HSP=low, [14:0]:Hsw = 0x20 = 32
			IoReg_Write32(0xb8066350, 0x000005C9);	   // [15:0]:Vtot = 1481	 
			IoReg_Write32(0xb8066354, 0x0000001C);	   // [15:0]:Vsta = 28
			IoReg_Write32(0xb8066358, 0x000005A0);	   // [15:0]:Vhight = 0x05A0 = 1440 	  
			IoReg_Write32(0xb806635C, 0x0000000A);	   // [110]VSP=low, [14:0]:Vsw = 10    
			...
			//// ARBITER							  
			IoReg_Write32(0xb8066368, 0x00000012);	   // [6]idel switch by VS, [5]db=0, [4]video_idle_db=vact_edge[3]Video_idle=0, [2] Hwid_div2=0,[1]vactive_md=new mode [0]arbiter_en=0
			IoReg_Write32(0xb806636C, 0x00000C99);	   // [15:0]vdata_per_line=0x0C99 (3225)
												   // Hwidth * BPC (bit per component) * 3 / 8 (byte) = lane_num * v_data_per_line (byte)  
			IoReg_Write32(0xb8066370, 0x00000040);	   // [7]TU_size<1,[6:0]:tu_size=0x40 (64)	  
			IoReg_Write32(0xb8066374, 0x0000011D);	   // [9:3]tu_data_size=0x1D, [2:0]=0.2=0x0
												   // tu_data_size =-> SCLK(freq) * BPC * 3 /8 (byte)= (TU_DATA_SIZE/TU) * LCLK(freq) * LANE_NUM (byte) 
			IoReg_Write32(0xb8066378, 0x000001e0);	   // [15:0]:hdelay 								 
			IoReg_Write32(0xb806637C, 0x00000064);	   // [7:0]:sec_end_cnt LSB[7:0]
			IoReg_Write32(0xb80663AC, 0x00000046);	   // [7:0]:sec_end_cnt MSB[15:8]  
			IoReg_Write32(0xb8066380, 0x00000080);	   // sec_end db apply
		}
		...
		////TX_TOP
		IoReg_Write32(0xb806638C, 0x00000000);      // I2V_first_blank_filter[1], idel2Vdieo switch=cnt to 8191
		IoReg_Write32(0xb8066390, 0x00000000);      // RSV1
		IoReg_Write32(0xb8066394, 0x00000000);      // RSV2

		////Arbiter
		IoReg_Write32(0xb8066398, 0x00008000);      // (Read_only) [15]wclr [14:0]min_h_blank_wid
		IoReg_Write32(0xb806639C, 0x00000000);      // IE disable

		////Vesa
		IoReg_Write32(0xb80663A0, 0x00000000);      // align Vsync and Hsync = disable
		IoReg_Write32(0xb80663B0, 0x00000013);      // [3] FW M/N code [0] using source's field signal
		IoReg_Write32(0xb80663B8, 0x000000FF);      // [0]=1 Ref BE to gen VBID[0]

		////DB apply
		IoReg_Write32(0xb8066338, 0x000000C0);      // [7] msa_db=1 [6]=0 just before MSA
		IoReg_Write32(0xb8066368, 0x00000013);      // [6]idel switch by VS, [5]db=0, [4]video_idle_db=vact_edge[3]Video_idle=0, [2] Hwid_div2=0,[1]vactive_md=new mode
		                                       // [0]arbiter_en=1
	}
...
unsigned char drvif_factory_set_lvds_current(KADP_VBE_DISP_LVDS_T *lvds_info)
{
	...
	if(len > 0)
	{
		...
		phy_ctrl_1_0_reg.pif_tx0_idrv_sel = lvds_info->current_val[index];//get bit0-bit5
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_0_reg, phy_ctrl_1_0_reg.regValue);
		if(lvds_info->current_val[index] & BIT6)//get bit6,notice:spec no write this bit
			IoReg_SetBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_66_reg, _BIT4);
		else
			IoReg_ClearBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_66_reg, _BIT4);
		index++;
		len--;
	}
	...
	if(len > 0)
	{
		...
		if(lvds_info->current_val[index] & BIT6)//get bit6,notice:spec no write this bit
			IoReg_SetBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_70_reg, _BIT4);
		else
			IoReg_ClearBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_70_reg, _BIT4);
		index++;
		len--;
	}
	...
	if(len > 0)
	{
		...
		if(lvds_info->current_val[index] & BIT6)//get bit6,notice:spec no write this bit
			IoReg_SetBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_74_reg, _BIT4);
		else
			IoReg_ClearBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_74_reg, _BIT4);
		index++;
		len--;
	}
	...
	if(len > 0)
	{
		...
		if(lvds_info->current_val[index] & BIT6)//get bit6,notice:spec no write this bit
			IoReg_SetBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_78_reg, _BIT4);
		else
			IoReg_ClearBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_78_reg, _BIT4);
		index++;
		len--;
	}
	...
	if(len > 0)
	{
		...
		if(lvds_info->current_val[index] & BIT6)//get bit6,notice:spec no write this bit
			IoReg_SetBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_82_reg, _BIT4);
		else
			IoReg_ClearBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_82_reg, _BIT4);
		index++;
		len--;
	}
	...
	if(len > 0)
	{
		...
		if(lvds_info->current_val[index] & BIT6)//get bit6,notice:spec no write this bit
			IoReg_SetBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_86_reg, _BIT4);
		else
			IoReg_ClearBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_86_reg, _BIT4);
		index++;
		len--;
	}
	...
	if(len > 0)
	{
		...
		if(lvds_info->current_val[index] & BIT6)//get bit6,notice:spec no write this bit
			IoReg_SetBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_90_reg, _BIT4);
		else
			IoReg_ClearBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_90_reg, _BIT4);
		index++;
		len--;
	}
	...
	if(len > 0)
	{
		...
		if(lvds_info->current_val[index] & BIT6)//get bit6,notice:spec no write this bit
			IoReg_SetBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_94_reg, _BIT4);
		else
			IoReg_ClearBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_94_reg, _BIT4);
		index++;
		len--;
	}
	...
	if(len > 0)
	{
		...
		if(lvds_info->current_val[index] & BIT6)//get bit6,notice:spec no write this bit
			IoReg_SetBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_98_reg, _BIT4);
		else
			IoReg_ClearBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_98_reg, _BIT4);
		index++;
		len--;
	}
	...
	if(len > 0)
	{
		...
		if(lvds_info->current_val[index] & BIT6)//get bit6,notice:spec no write this bit
			IoReg_SetBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_102_reg, _BIT4);
		else
			IoReg_ClearBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_102_reg, _BIT4);
		index++;
		len--;
	}
	...
	if(len > 0)
	{
		...
		if(lvds_info->current_val[index] & BIT6)//get bit6,notice:spec no write this bit
			IoReg_SetBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_106_reg, _BIT4);
		else
			IoReg_ClearBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_106_reg, _BIT4);
		index++;
		len--;
	}
	...
	if(len > 0)
	{
		...
		if(lvds_info->current_val[index] & BIT6)//get bit6,notice:spec no write this bit
			IoReg_SetBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_110_reg, _BIT4);
		else
			IoReg_ClearBits(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_110_reg, _BIT4);
		index++;
		len--;
	}
...
unsigned char drvif_factory_get_lvds_current(KADP_VBE_DISP_LVDS_T *lvds_info)
{
	...
	addTemp = PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_66_reg;
	for(i = 0;i < 12; i++)
	{
		regValue[i] |= (IoReg_Read32(addTemp) & _BIT4) << 2;
		addTemp += 0x10;
	}
...
unsigned char drvif_factory_set_pre_emphasis_current(KADP_VBE_DISP_Emphasis_T *emp_info)
{
	...
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_25_RBUS		phy_ctrl_1_25_reg;
	...
	if(len > 0)
	{
		phy_ctrl_1_25_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_25_reg);
		phy_ctrl_1_25_reg.pif_tx6_iem_sel = emp_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_25_reg, phy_ctrl_1_25_reg.regValue);
		len--;
	}
...
unsigned char drvif_factory_get_pre_emphasis_current(KADP_VBE_DISP_Emphasis_T *emp_info)
{
	...
	regValue[index++] = phy_ctrl_1_25_reg.pif_tx6_iem_sel;
...
void vbe_disp_orbit_vo_overscan()
{
	...
	memset(&input_size, 0, sizeof(StructSrcRect));
	memset(&input_timing, 0, sizeof(StructSrcRect));
	memset(&output_dispwin, 0, sizeof(StructSrcRect));
	memset(&orbit_shift, 0, sizeof(ORBIT_PIXEL_SHIFT_STRUCT));
...
VBE_DISP_VRR_FRACTIONAL_TIMING_MODE_T vbe_disp_get_VRR_timing_fractional_mode(void)
{
	...
	return VBE_DISP_VRR_1_1;//VRR always run VBE_DISP_VRR_1_1
}
...
long vbe_ioctl(struct file *file, unsigned int cmd,  unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case VBE_IOC_GETBOEVERSION:
		{
			...
			memset(&vbeDispBoeVer, 0, sizeof(KADP_VBE_DISP_BOE_VERSION_T));
			memset(&stRomVer, 0, sizeof(KADP_DISP_BOE_VERSION_T));
			memset(&stFwVer, 0, sizeof(KADP_DISP_BOE_VERSION_T));
