...
/*==================== Variables ================== */
static unsigned char v_pos_adjust_full_range = FALSE;//if True. v position can adjust 0-100. change some UI scale and video chnage once.
...
void VGA_Clamp_Ctrl(unsigned char output_enable, ADC_DATA_PATH adc_path, ADC_SOY_PATH soy_path)
{
	...
	adc_adc_clamp_ctrl2_reg.adc_sog_cmp_2_0 = soy_path;
...
//	adc_adc_dcrestore_ctrl_RBUS adc_adc_dcrestore_ctrl_reg;
	...
	adc_adc_vbias_reg.adc_vbias0_1 = 0;/* LDO voltage select 011:1.1v bit[7:5]=101 bit[4:2]=000 bit0=1*/
	...
	adc_adc_vbias_reg.adc_vbias1_3_2 = 0;//BIT[3:2]SF driving, 0:600uA,1:720uA,2:840uA,3:960uA
	...
	dds_pll_plldiv_reg.apll_reserve_1_0 = 0;//k5l bring up need check
	dds_pll_plldiv_reg.apll_reserve_3_2 = 0;//k5l bring up need check
	dds_pll_plldiv_reg.apll_reserve_5_4 = 0;//k5l bring up need check
	dds_pll_plldiv_reg.apll_reserve_6 = 1;//k5l bring up need check
	dds_pll_plldiv_reg.apll_reserve_7 = 0;//floating input now (reserve) 01: test V11
	IoReg_Write32(DDS_PLL_PLLDIV_reg, dds_pll_plldiv_reg.regValue);

	//[scaler][code sync]
	adc_smartfit_adc_auto_field_reg.regValue = 0;
	adc_smartfit_adc_auto_field_reg.adj_source = 0;//pll_setting mux. 0: from ADC smfit, other value is no signal
	...
	adc_adc_ctl_rgb12_reg.adc_ctl_rgb2_10 = 0;
	adc_adc_ctl_rgb12_reg.adc_ctl_rgb2_11= 0;
...
void VGA_SyncProc_Init(void)
{
	...
	if ((_ENABLE == Scaler_Get_VGA_Support_SOG()) || (_ENABLE == Scaler_Get_VGA_Support_Composite_Sync())) {
		...
		case SOG_MODE:/* SOG sync type */
			...
			rtd_pr_adc_info("In VGA_SyncProc_Init, switch to SOG type!\n");
			break;
		...
		case CS_ON_H_MODE:/* Composite sync type */
			...
			if (SYNC_PROCESSOR1 == VGA_SP_PATH) {
				...
				if (CS_ON_H_MODE == VGADetecSyncTypeMode) {
					...
					rtd_pr_adc_info("In VGA_SyncProc_Init, switch to Composite Sync type!\n");
					...
					rtd_pr_adc_info("In VGA_SyncProc_Init, switch to Separate Sync type!\n");
				}
...
void VGA_ADCPowerControl(unsigned char on_off)
{
	...
	if (TRUE == on_off) {
		...
		IoReg_Write32(ADC_ADC_CLAMP_CTRL3_reg, adc_clamp_ctrl3_reg.regValue);
#endif /* #ifdef CONFIG_DUAL_CHANNEL */
		rtd_pr_adc_info("#####rzhen#####[%s(%d)] HSYNC PAD power on\n", __func__, __LINE__);
		...
		rtd_pr_adc_info("#####rzhen#####[%s(%d)] HSYNC PAD power down\n", __func__, __LINE__);
...
void VGA_SP_ModeSetting(void)
{
	...
	if (_ENABLE == Scaler_Get_VGA_Support_SOG()) {
		...
		if (SOG_MODE == VGADetecSyncTypeMode) {
			...
			rtd_pr_adc_notice("#####[%s(%d)]usHighPulsePeriod Pre= 0x%08x\n", __func__, __LINE__, usHighPulsePeriod);
			...
			rtd_pr_adc_notice("%s dv restore ctrl disable\n", __FUNCTION__);
	...
	if (_ENABLE == Scaler_Get_VGA_Enable_Asjudt_ClampPulse_In_SS_Type()) {
		...
		if (SEPARATE_MODE == VGADetecSyncTypeMode) {
			...
			rtd_pr_adc_notice("VGA SS Type, Clamp Pulse Start : 0x%08x, End : 0x%08x\n", usClampHStart, usClampHEnd);
...
unsigned char VGA_DetectTiming(void)
{
	...
	pll_ctrl_reg.apllldosel = YPbPr_LDO_Trim()?YPbPr_LDO_Trim_apllldosel():1; ///LDO reference voltage setting  01:1.1V(default);
	...
	VGA_Clamp_Ctrl(_DISABLE, (((VGA_ADC_DATA_PATH *)ptOptions->path)->DATA_INPUT), (((VGA_ADC_DATA_PATH *)ptOptions->path)->SOY_INPUT));
	...
	if(VGA_TimingDetectStage == VGA_HVSYNC_STATE){
		...
		rtd_pr_adc_notice("#####[%s(%d)] VGA_HVSYNC_STATE success\n", __FUNCTION__, __LINE__);
		...
		if (result == _MODE_SUCCESS) {
			...
			rtd_pr_adc_notice("#####[%s(%d)] VGA_SIGNALTYPE_STATE success\n", __FUNCTION__, __LINE__);
			return _MODE_SUCCESS;
		} else {
			...
			rtd_pr_adc_notice("#####[%s(%d)] VGA_SIGNALTYPE_STATE fail \n", __FUNCTION__, __LINE__);
			return result;
		}
	}else{
		...
		if (SignalMode != _MODE_SUCCESS) {
			...
			SyncProc_FunctionDisable(_SRC_VGA);
			rtd_pr_adc_info("##### [%s(%d)] VGA_OFFLINE_STATE fail ##\n", __func__, __LINE__);
			return SignalMode;
		...
		else if(check_sp_cap_unlock())//sp unlock
		{
			...
			vga_good_timing_ready = FALSE;
			SyncProc_FunctionDisable(_SRC_VGA);
			rtd_pr_adc_info("##### [%s(%d)] check_sp_cap_unlock fail ##\n", __func__, __LINE__);
			return _MODE_NOSIGNAL;
...
void VGA_Setup(void){
	...
	drvif_adc_ctl_rgb12_check(_SRC_VGA);

	VGA_ADC_LPF_BW_Setting(p_dispinfo);

	drvif_adc_power_control(ADC_POWER_VGA_ONLY_DETECT_CONTROL,__func__,__LINE__);
	...
	if (_ENABLE == get_adc_abl_function()) {
		...
		rtd_pr_adc_notice("#####[%s(%d)]usABL_Hstart = %d\n", __func__, __LINE__, usABL_Hstart);
...
{//check vga current timing already run auto config or not. return FALSE: need calibration 
	...
	if(p_timinginfo)
	{
		...
		if(stScreenModeCurrInfo->Hsync_Polarity_Indicator != get_adc_hsync_polarity_indicator()){
			rtd_pr_adc_emerg("#####[%s(%d)]Need auto, Hsync_Polarity_Indicator=%d #####\n", __func__, __LINE__, stScreenModeCurrInfo->Hsync_Polarity_Indicator);
			rtd_pr_adc_emerg("#####[%s(%d)]Need auto, get_adc_hsync_polarity_indicator =%d #####\n", __func__, __LINE__, get_adc_hsync_polarity_indicator());
			return FALSE;
		}

		if((p_timinginfo->ModeCurr == _MODE_720x480_60HZ)||(p_timinginfo->ModeCurr == _MODE_VGA_576P)||(p_timinginfo->ModeCurr == _MODE_1600x900_60HZ_RB))
			return FALSE;
...
unsigned char Scaler_AdjustPCAuto(unsigned char display)
{
	...
	rtd_pr_adc_notice("#####[%s(%d)]VGA auto adjust begin!!!#####\n", __func__, __LINE__);
	rtd_pr_adc_notice("#####[%s(%d)]display=%d #####\n", __func__, __LINE__, display);
	...
	if(p_dispinfo){
		if(((p_dispinfo->IPH_ACT_WID_PRE == 4095)&&(p_dispinfo->IPV_ACT_LEN_PRE == 4095))
			...
			rtd_pr_adc_err("#####[%s(%d)]NoSignal and Unsupport no need auto adjust!!!#####\n", __func__, __LINE__);
			return FALSE;
		}
	}else
		return FALSE;
	...
	if(Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE)
	{
		...
		if(Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE)
		{
			if(vga_clock_check_onlinemeasure_status(display) == FALSE)
			{
				...
				rtd_pr_adc_err("#####[%s(%d)]VGA auto adjust fail!!!#####\n", __func__, __LINE__);
				return FALSE;
			}
		...
		if(usResult==ERROR_SUCCEED){
			up(get_adc_detectsemaphore());
			rtd_pr_adc_notice("#####[%s(%d)]VGA auto adjust success!!!#####\n", __func__, __LINE__);
			stScreenModeCurrInfo->Open_Count = 1;
			stScreenModeCurrInfo->Hsync_Polarity_Indicator = get_adc_hsync_polarity_indicator();
			return TRUE;
		}else{
			up(get_adc_detectsemaphore());
			rtd_pr_adc_notice("#####[%s(%d)]VGA auto adjust fail!!!#####\n", __func__, __LINE__);
			return FALSE;
		}
	}
	up(get_adc_detectsemaphore());
...
unsigned char Scaler_GetHPosition(void)
{
	...
	if(p_timinginfo)
	{
		stScreenModeCurrInfo = get_vga_auto_data(p_timinginfo->ModeCurr);
		if(!stScreenModeCurrInfo){
			rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust] #####\n", __func__, __LINE__);
			return 0;
		}
	}
	else{
...
unsigned char Scaler_SetHPosition(unsigned char value)
{
	...
	stScreenModeCurrInfo->H_Position = value < 100 ? value : 100;
	rtd_pr_adc_notice("#####[%s]value=%d,hpos=%d,IPH_ACT_STA=%d#####\n", __func__, value, stScreenModeCurrInfo->H_Position, p_dispinfo->IPH_ACT_STA );
	...
	rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust]0xb8022418[29:16]=0x%04x#####\n", __func__, __LINE__,dma_vgip_act_hsta_width_reg.dma_ih_act_sta);
	rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust]0xb8022420[29:16]=0x%04x#####\n", __func__, __LINE__,dma_vgip_dma_vgip_delay_reg.dma_ihs_dly);
#else
	...
	rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust]0xb8022218[29:16]=0x%04x#####\n", __func__, __LINE__,vgip_chn1_act_hsta_reg.ch1_ih_act_sta);
	rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust]0xb8022220[29:16]=0x%04x#####\n", __func__, __LINE__,vgip_chn1_delay_reg.ch1_ihs_dly);
#endif
	Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY),SLR_INPUT_IPH_ACT_STA ,h_start);
...
unsigned char Scaler_SetVPosition(unsigned char value)
{
...
#endif
	unsigned int scale_numerator = 50;//
	unsigned int scale_denominator = 50;//	
	...
	stScreenModeCurrInfo->V_Position = value < 100 ? value : 100;
	rtd_pr_adc_notice("#####rzhen##### [%s] value=%d vpos:%d#####\n", __func__, value, stScreenModeCurrInfo->V_Position);

	v_start = FIX_VGIP_V_START;
	v_delay = p_dispinfo->IPV_ACT_STA - v_start;
	if(v_pos_adjust_full_range)
	{
		if(v_delay < 50)
			scale_numerator = v_delay; // max adjust range 
	}
	if(0x32 >= stScreenModeCurrInfo->V_Position)
	{
		if(v_pos_adjust_full_range)
			v_delay = v_delay + (0x32 - stScreenModeCurrInfo->V_Position) * scale_numerator / scale_denominator;//calculate final value 
		else
	...
	else
	{
		if(v_pos_adjust_full_range)
		{
			if(v_delay >= ((stScreenModeCurrInfo->V_Position - 0x32) * scale_numerator / scale_denominator))
				v_delay = v_delay - ((stScreenModeCurrInfo->V_Position - 0x32) * scale_numerator / scale_denominator);//calculate final value 
			else
				v_delay = 0;
		}
		else
	...
	IoReg_Write32(VGIP_VGIP_CHN1_DELAY_reg, vgip_chn1_delay_reg.regValue);
#endif
	Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY),SLR_INPUT_IPV_ACT_STA , v_start);
...
void Scaler_SetPhase(unsigned char value)
{
	...
	rtd_pr_adc_notice("#####rzhen##### [%s(%d)] value=%d #####\n", __func__, __LINE__, value);
...
void Scaler_SetClock(unsigned char value)
{
	...
	rtd_pr_adc_notice("#####rzhen##### [%s(%d)] truth_clk=%d #####\n", __func__, __LINE__, truth_clk);
	rtd_pr_adc_notice("#####rzhen##### [%s(%d)] INPUT_ADC_CLOCK=%d #####\n", __func__, __LINE__, p_dispinfo->INPUT_ADC_CLOCK);
...
//	adc_smartfit_adc_auto_phase_ctrl0_RBUS auto_phase_ctrl0_reg;
	...
	unsigned int cnt=0,sta_time=0,end_time=0;

	sta_time = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
	for(cnt=0;cnt<10000;cnt++){
		if((IoReg_Read32(ADC_ADC_RGB_DATA_reg) & 0x0fff) > 1024)//check ADC G Value
			break;
	}
	end_time = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
	rtd_pr_adc_info("#####[%s(%d)] read register cost time: %d ms,cnt=%d!!!\n",__func__,__LINE__,(end_time-sta_time)/90,cnt);

	rtd_pr_adc_notice("#####[%s(%d)]display = %x#####\n", __func__, __LINE__,display);
	//rtd_pr_adc_notice("#####[%s(%d)]m_stScreenModeCurrInfo[index].H position = %d#####\n", __func__, __LINE__,m_stScreenModeCurrInfo[index].H_Position));
	//rtd_pr_adc_notice("#####[%s(%d)]m_stVGAModeCurrInfo[mode].H_Position = %d#####\n", __func__, __LINE__,m_stVGAModeCurrInfo[mode].H_Position);
	if(drv_auto_pre_check(&select_color)!=ERROR_SUCCEED)
		return ERROR_ABORT;

	rtd_pr_adc_notice("#####[%s(%d)]select_color = %x#####\n", __func__, __LINE__,select_color);
	...
	//---------Auto_Clock-----------
	while((IoReg_Read32(0xb8020088) & 0x00000fff)<0x400 && (timeout--))
	{
		msleep(1);
	}
	...
	{
		rtd_pr_adc_err("#####[%s(%d)] onine status error#####\n", __func__, __LINE__);
		return ERROR_ABORT;
	...
	p_screenInfo->Hsync_Polarity_Indicator = get_adc_hsync_polarity_indicator();
	SET_AUTO_FINISH();
