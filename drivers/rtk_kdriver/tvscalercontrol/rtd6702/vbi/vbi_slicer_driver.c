...
unsigned int vbi_status_get_size(VBI_TYPE_T type)
{
	...
	switch (type)
	{
		...
		default:
		{
			rtd_pr_vbi_debug("vbi type is not correct:%d",type);
			break;
		}
	}
	return size;
...
#endif
unsigned long video_send_vbi_slicer_rpc(unsigned long para1, unsigned long para2)
{
	...
	switch (para1)
	{
		case VBI_PAL_TELETEXT:
	 	{
		    ...
		    if(vbi_buf == NULL) {rtd_pr_vbi_err("error! vbi_buf = null \n"); return FALSE;}
		    vbi_buf_idx = (unsigned short *)(((unsigned char *)vbi_buf) + (VBI_TTX_BUF_LENGTH*PPR_PTNGEN_PKT_LEN_BYTES));
		    if(vbi_buf_idx == NULL) {rtd_pr_vbi_err("error! vbi_buf_idx = null \n"); return FALSE;}
			...
			// rtd_pr_vbi_debug("start:%d,end:%d\n",start_idx,end_idx);
			while (1)
			{
				if(vbi_slicer_status[VBI_PAL_TELETEXT]!=VBI_DATA_COLLECTING)
				{
					rtd_pr_vbi_debug("don't handle ttx data\n");
					break;
				}
		...
		case VBI_PAL_WSS:
		{
			if(vbi_slicer_status[VBI_PAL_WSS]!=VBI_DATA_NON)
			{
				...
				rtd_pr_vbi_notice("kernel recevie video vbi_pal_wss=%x\n",vbi_pal_wss);
			}
			break;
		}
		case VBI_PAL_VPS:
		...
		//	rtd_pr_vbi_debug("start:%d,end:%d\n",start_idx,end_idx);
		    while (1)
		    {
		    	...
		    	{
		    		rtd_pr_vbi_debug("don't handle vps data\n");
		    		break;
		    	}
		        if ( start_idx == end_idx )
		            break;
...
void init_vbi_rpc(void)
{
	...
	if (register_kernel_rpc(RPC_VCPU_ID_0x101_VBI, video_send_vbi_slicer_rpc) == 1)
		rtd_pr_vbi_notice("Register RPC init_vbi_rpc failed!\n");
...
unsigned int Scaler_share_vbimem(VBI_TYPE_T vbi_status)
{
	...
	else if(vbi_status==VBI_PAL_TELETEXT)
	{
		...
		rtd_pr_vbi_debug("ttxphyaddr=%lx\n",m_ttxBufPhyAddr);
	}
	else if(vbi_status==VBI_PAL_VPS)
	{
		...
		rtd_pr_vbi_debug("vpsphyaddr=%lx\n",m_vpsBufPhyAddr);
	}
	return true;
}
...
void Scaler_VbiVPSSetBuf(unsigned int* buf_addr)
{
	if(vbi_vps_buffer_need_set==0){
		...
		{
			rtd_pr_vbi_debug("ret=%d, set buffer address for VBI VPS fail !!!\n", ret );
			return;
...
void Scaler_VbiTtxSetBuf(unsigned int* buf_addr)
{
	if(vbi_tt_buffer_need_set==0) {
		...
		{
			rtd_pr_vbi_debug("ret=%d, set buffer address for VBI TTx fail !!!\n", ret );
			return;
...
void Scaler_VbiTtInit(void)
{
	...
	rtd_pr_vbi_debug("=========== Scaler_TestTtx_SlicerData ====================\n");
...

	{
		...
		{
			rtd_pr_vbi_debug("ret=%d, init VBI TT fail !!!\n", ret);
		}
	}
	...
	if (vbi_slicer_status[VBI_PAL_VPS] ==VBI_DATA_NON) {//ttx driver setting
		...
		up(get_VBI_vps_semaphore());
		vbi_tt_vbi_control_register_Reg.regValue= vdcf_inl(VBI_TT_VBI_Control_Register_reg);
		vbi_tt_vbi_control_register_Reg.tt_vbi_st_err_ignored=1;
		vdcf_outl(VBI_TT_VBI_Control_Register_reg, vbi_tt_vbi_control_register_Reg.regValue);
	}
...
void Scaler_VbiTtDeinit(void)
{
...

	{
		...
		if (0 != (ret = Scaler_SendRPC(SCALERIOC_VBI_ENABLE,0,0)))
		{
			rtd_pr_vbi_debug("ret=%d, Deinit VBI TT fail !!!\n", ret);
		}
	}
#endif
...
void Scaler_VbiCcInit(void) // 0: from av/sv/atv, 1: from ypbpr
{
...

	{
		...
		if (0 != (ret = Scaler_SendRPC(SCALERIOC_VBI_ENABLE,0,0)))
		{
			rtd_pr_vbi_debug("ret=%d, init VBI CC fail !!!\n", ret);
			//return;
...
/*fix me	if (Scaler_GetDev() == -1) {
		rtd_pr_vbi_debug("open /dev/scaler fail, cannot deinit VBI CC\n");
		return;
	}

	if ((ret = ioctl(Scaler_GetDev(), SCALERIOC_VBI_ENABLE, &param)) < 0) {
		rtd_pr_vbi_debug("ret=%d, deinit VBI CC fail !!!\n", ret);
		return;
	}*/
...
void Scaler_VbiCcStop(unsigned char from) // 0: from av/sv/atv, 1: from ypbpr
{
	...
	{
		rtd_pr_vbi_debug("ret=%d, stop VBI CC fail !!!\n", ret);
		return;
	}
...
void Scaler_VbiCcPlay(unsigned char from) // 0: from av/sv/atv, 1: from ypbpr
{
	...
	{
		rtd_pr_vbi_debug("ret=%d, play VBI CC fail !!!\n", ret);
		return;
	}
}
...
void Scaler_VbiCcSetBuf(unsigned int* buf_addr)
{
	if(vbi_cc_buffer_need_set==0){
		...
		{
			rtd_pr_vbi_debug("ret=%d, set buffer address for VBI CC fail !!!\n", ret );
			return;
...
bool  Scaler_VbiTtSetGetProperty(TTX_VBI_IOCTL_PARAM *param)
{
...

    {
        ...
        {
            rtd_pr_vbi_debug("ret=%d, Set VBI TT Function fail !!!\n", ret);
            return FALSE;
        }
        else
        {
...
void Scaler_vbi_cgms_init(CGMS_SOURCE source)
{
	...
	rtd_pr_vbi_debug("=========== Scaler_vbi_cgms_init:%d ====================\n",source);
...

	{
		...
		{
			rtd_pr_vbi_debug("ret=%d, init VBI CGMS fail !!!\n", ret);
		}
	}
	...
	vdcf_maskl(ADC_ADC_CLKGEN_RGB_Capture_reg, ~ADC_ADC_CLKGEN_RGB_Capture_vbi_clkx2_sel_mask, 0);// reset vbi clk to default (x1 //Remain ADC register for Mark2
...
void Scaler_vbi_cgms_Uninit(CGMS_SOURCE source)
{
...

	{
		...
		if (0 != (ret = Scaler_SendRPC(SCALERIOC_VBICGMSENABLE,0,0)))
		{
			rtd_pr_vbi_debug("ret=%d, uninit VBI CGMS fail !!!\n", ret);
			//return;
	...
	while(cnt){
		...
		if (wssData != vbi_preWss)
		{
			...
			if ((vbi_pal_wss!=vbi_preWss) &&(vbi_wss_change_flag == 1)) {
				...
				vbi_slicer_status[VBI_PAL_WSS]=VBI_DATA_COLLECTED;
				rtd_pr_vbi_notice("kernelvbi_pal_wss=%x\n",vbi_pal_wss);
				break;
			}
		}
		cnt--;
...
extern KADP_VFE_AVD_CC_TTX_STATUS_T VBI_support_type_get(void);
extern unsigned char VBI_Support_Enable(void);
void vbi_just_576i_wss_init(void)
{
	...
	if ((VBI_Support_Enable()) && (VBI_support_type_get()==KADP_VFE_AVD_CC_IS_SUPPORTED))
	{
		// Do nothing
	}
	else
...
{
	unsigned int regData;
	if (vdcf_inl(VBI_TT_VPS_DATA3_reg) & VBI_TT_VPS_DATA3_tt_wss_rdy_mask) {
		regData = vdcf_inl(VBI_TT_VBI_CCWSS_DATA_reg);
		vbi_pal_wss = ((regData>>8)&0xff00)|(regData>>24&0xff);
		vdcf_maskl(VBI_TT_VBI_DATA_STATUS_reg,  ~(VBI_TT_VPS_DATA3_tt_wss_rdy_mask|VBI_TT_VPS_DATA3_tt_vps_rdy_mask|VBI_TT_VPS_DATA3_tt_cc_rdy_mask|VBI_TT_VPS_DATA3_tt_tt_rdy_mask), VBI_TT_VPS_DATA3_tt_wss_rdy_mask);
		rtd_pr_vbi_debug("vbi_pal_wss=%x\n",vbi_pal_wss);
	} else {
		wss_data_clear();
	}
}
...
void cgms_data_receive(void)
{
	...
	if (vdcf_inl(VBI_CGMS_C4_reg) & VBI_CGMS_C4_cgms_c4_cgms_vld_mask) {
		...
		rtd_pr_vbi_debug("vbi_cgms_data=%x\n",vbi_cgms_data);
	} else {
		cgms_data_clear();
	}
}
