...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
   ...
   // i3ddma_hsd_hor_delta1_reg.regValue	= IoReg_Read32(H3DDMA_HSD_I3DDMA_HSD_Hor_Delta1_reg);
	...
	if (in_Size->nLength > out_Size->nLength) {
		...
		//o---------------- fill coefficients into access port--------------------o
		coef_pt = tScale_Down_Coef_Repeat;// tScaleDown_COEF_TAB[SDFilter];
...
//			ich2_uzd_Ctrl0_REG.buffer_mode = 0x02;
		...
		if ( out_Size->nLength == 0 ) {
			...
			nFactor = (unsigned long long)(( (unsigned long long)(in_Size->nLength) * 1024 * 1024)) / (out_Size->nLength);
		}
		//nFactor = SHR(nFactor+1, 1); //rounding
...
void I3DDMA_ultrazoom_set_scale_down_for_NN(SIZE* in_Size, SIZE* out_Size, unsigned char panorama)
{
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
   ...
   // i3ddma_hsd_hor_delta1_reg.regValue	= IoReg_Read32(H3DDMA_HSD_I3DDMA_HSD_Hor_Delta1_reg);
	...
	if (in_Size->nLength > out_Size->nLength) {
		...
		//o---------------- fill coefficients into access port--------------------o
		coef_pt = tScale_Down_Coef_Repeat;
...
//			ich2_uzd_Ctrl0_REG.buffer_mode = 0x02;
		...
		i3ddma_vsd_initial_value_reg.ver_ini = 0x80;
		//nFactor = (unsigned int)((ptInSize->nLength << 20) - (Vini<<12)) / (ptOutSize->nLength - 1);
		//frank@0108 add for code exception
		if ( out_Size->nLength == 0 ) {
			rtd_pr_i3ddma_debug("output length = 0 !!!\n");
...
{
#ifdef CONFIG_ENABLE_HDMI_NN
	rtd_pr_i3ddma_emerg("Don't do I3DDMA_ultrazoom_config_scaling_down\n");
	return;
#else
...
void I3DDMA_ultrazoom_config_scaling_down_for_NN(SIZE* in_Size, SIZE* out_Size, unsigned char panorama)
{
	// Scale down setup for Channel1
	h3ddma_hsd_i3ddma_hsd_ctrl0_RBUS i3ddma_hsd_ctrl0_reg;
	h3ddma_hsd_i3ddma_vsd_ctrl0_RBUS i3ddma_vsd_ctrl0_reg;
	...
	//i3ddma_hsd_ctrl0_reg.h_zoom_en = (in_Size->nWidth > out_Size->nWidth);
	// Test, force uzd
	if((g_enSupCapMode == I3DDMA_UZD_CAP_ONLY) || (g_enSupCapMode == I3DDMA_CAP_MIX))
	{
		i3ddma_hsd_ctrl0_reg.h_zoom_en = (in_Size->nWidth > out_Size->nWidth);
	...
	else //I3DDMA_CROP_CAP_ONLY mode
	{
	...
	i3ddma_vsd_ctrl0_reg.cutout_en = ((g_enSupCapMode == I3DDMA_CROP_CAP_ONLY) ? 1 : 0);
	IoReg_Write32(H3DDMA_HSD_I3DDMA_VSD_Ctrl0_reg, i3ddma_vsd_ctrl0_reg.regValue);
	IoReg_Write32(H3DDMA_HSD_I3DDMA_HSD_Ctrl0_reg, i3ddma_hsd_ctrl0_reg.regValue);

	I3DDMA_ultrazoom_set_scale_down_for_NN(in_Size, out_Size, panorama);
