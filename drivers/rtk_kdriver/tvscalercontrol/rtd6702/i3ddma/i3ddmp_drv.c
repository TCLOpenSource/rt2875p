...
void drv_I3DDMA_ApplySetting(unsigned char display)
{
	...
	else if((i3ddmaCtrl.ptx_timing->h_act_len ==1440) && (DMA_VGIP_DMA_VGIP_CTRL_get_dma_in_sel(rtd_inl(DMA_VGIP_DMA_VGIP_CTRL_reg)) == VGIP_SRC_ATV))
	{
		...
		&& ((DMA_VGIP_DMA_VGIP_CTRL_get_dma_in_sel(rtd_inl(DMA_VGIP_DMA_VGIP_CTRL_reg)) == VGIP_SRC_HDMI1) || (DMA_VGIP_DMA_VGIP_CTRL_get_dma_in_sel(rtd_inl(DMA_VGIP_DMA_VGIP_CTRL_reg)) == VGIP_SRC_HDMI2)))
	{
...
void scaler_mhl_i3ddma_config(void)
{
	...
	source = get_hdmi_vgip_source_type(Get_DisplayMode_Port(SLR_SUB_DISPLAY));
...
void I3DDMA_Set_HDMI3D_Vgip(void)
{
	...
	unsigned char src = get_hdmi_vgip_source_type(Get_DisplayMode_Port(SLR_MAIN_DISPLAY));
...
void fw_i3ddma_set_vgip(unsigned char src, unsigned char mode)
{
	...
	if(mode == VGIP_MODE_ANALOG)
	{
		...
		ctrl_i_clock_2p_sel(_DAM_VGIP_PATH, 0);
	}
	else if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI))	//hdmi
	{
		...
		{//hdmi case
				ctrl_i_clock_2p_sel(_DAM_VGIP_PATH, check_hdmi_enabe_2p_mode(Get_DisplayMode_Port(SLR_MAIN_DISPLAY)));
		}
	}
	else if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) ||(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_JPEG))	//vdec 8k 60
	{
		ctrl_i_clock_2p_sel(_DAM_VGIP_PATH, VODMA_VODMA_CLKGEN_get_vodma_clk_2x_2p_sel(IoReg_Read32(VODMA_VODMA_CLKGEN_reg))? 0 : 1);
	...
	if((get_i3ddma_4k120_flag() == TRUE) || (get_hdmi_4k_hfr_mode()))
	{
	...
	if(dvrif_i3ddma_compression_get_enable()) {
		if(src == VGIP_SRC_ATV){// || (src == VGIP_SRC_ADC)) {
			...
			// if(dma_vgip_chn1_act_hsta_width_reg.dma_ih_act_sta > 32) {
				dma_vgip_chn1_delay_reg.regValue = IoReg_Read32(DMA_VGIP_DMA_VGIP_DELAY_reg);
				dma_vgip_chn1_delay_reg.dma_ihs_dly = dma_vgip_chn1_delay_reg.dma_ihs_dly >> 1;
				IoReg_Write32(DMA_VGIP_DMA_VGIP_DELAY_reg, dma_vgip_chn1_delay_reg.regValue);
				// dma_vgip_chn1_act_hsta_width_reg.dma_ih_act_sta = dma_vgip_chn1_act_hsta_width_reg.dma_ih_act_sta - 32;
				// IoReg_Write32(DMA_VGIP_DMA_VGIP_ACT_HSTA_Width_reg, dma_vgip_chn1_act_hsta_width_reg.regValue);
			// }
		}
		else if((src == VGIP_SRC_HDMI1) || (src == VGIP_SRC_HDMI2)) {
			dma_vgip_chn1_delay_reg.regValue = IoReg_Read32(DMA_VGIP_DMA_VGIP_DELAY_reg);
...
{//for avd i3ddma start compensation, because final state maybe need compensation
	...
	if((avd_info->IPH_ACT_STA_PRE != (2*dma_vgip_chn1_delay_reg.dma_ihs_dly + dma_vgip_act_hsta_width_reg.dma_ih_act_sta)) ||
		...
		)
	{
		...
		if(avd_info->IPH_ACT_STA_PRE < dma_vgip_chn1_delay_reg.dma_ihs_dly){
			...
			dma_vgip_act_hsta_width_reg.dma_ih_act_sta = _H_ACT_STA_PRE_DEF - 2*dma_vgip_chn1_delay_reg.dma_ihs_dly;
		}else
			dma_vgip_act_hsta_width_reg.dma_ih_act_sta = avd_info->IPH_ACT_STA_PRE - 2*dma_vgip_chn1_delay_reg.dma_ihs_dly;
