...
#define VT_CAP_FRAME_WIDTH_8k4k     7680
#define VT_CAP_FRAME_HEIGHT_8k4k    4320
#define VT_CAP_ORBIT_WIDTH_8K4K     7808
#define VT_CAP_ORBIT_HEIGHT_8K4K	4384
#define VT_CAP_ORBIT_WIDTH_4K2K     3904
#define VT_CAP_ORBIT_HEIGHT_4K2K	2192
...
void drvif_dc2h_input_overscan_config(unsigned int x, unsigned int y, unsigned int w, unsigned int h)
{
	...
	//dc2h_rgb2yuv_dc2h_3dmaskto2d_ctrl_Reg.dc2h_3dmaskto2d_3dformat = 4;/*overscan > reserved*/
...
//	unsigned int width96align;
	...
	/*if((TRUE == force_disp_s1_s2_same_clk()) && (is_4K2K_capture() == TRUE))
...
void set_DC2H_rgb2yuv_ctrl_reg_refer_dump_location(KADP_VT_DUMP_LOCATION_TYPE_T in_position, SIZE *in_size, unsigned char *dc2h_in_sel)
{
	...
	if(in_position == KADP_VT_SCALER_OUTPUT)
	{
		...
		else
		{
			...
			{
				in_size->nWidth = VT_CAP_ORBIT_WIDTH_8K4K;
				in_size->nLength = VT_CAP_ORBIT_HEIGHT_8K4K;
			}
			else//orbit 4k 120
			{
				in_size->nWidth = VT_CAP_ORBIT_WIDTH_4K2K;
				in_size->nLength = VT_CAP_ORBIT_HEIGHT_4K2K;
			}
		}
...
unsigned char judge_dc2h_inout_size_valid(KADP_VT_DUMP_LOCATION_TYPE_T location, unsigned int capW, unsigned int capH)
{
	...
	if(location == KADP_VT_SCALER_INPUT)
	{
		rtd_pr_vt_notice("dc2h source capture for extension\n");
		return FALSE;
	}
	else if (location == KADP_VT_SCALER_OUTPUT)
	{
		ppoverlay_main_active_h_start_end_RBUS main_active_h_start_end_Reg;
		ppoverlay_main_active_v_start_end_RBUS main_active_v_start_end_Reg;
		main_active_h_start_end_Reg.regValue = IoReg_Read32(PPOVERLAY_MAIN_Active_H_Start_End_reg);
	...
	rtd_pr_vt_notice("dc2h input size(%d,%d)\n",dc2h_inputW, dc2h_inputH);
	if(dc2h_inputW < capW || dc2h_inputH < capH)
	{
		rtd_pr_vt_emerg("invalid,dc2h out size larger than input size\n");
		return FALSE;
	}
	if(ppoverlay_uzudtg_orbit_main_ctrl0_reg.orbit_m_overscan_en && location == KADP_VT_SCALER_OUTPUT)
	{
		...
		{
			rtd_pr_vt_notice("orbit on,dc2h capture uzu,max output 4k\n");
			return FALSE;
		}
	}
...
void drvif_dc2h_seqmode_conifg(unsigned char enable, int capWid, int capLen, int capSrc)
{
	...
	if((pdc2h_hdl->out_size.nWidth) % 2)
	{/*if dc2h uzd output is odd, uzd output repeat last point, dc2h dma pack even Width*/
		pdc2h_hdl->out_size.nWidth = pdc2h_hdl->out_size.nWidth + 1;
	}

	if(judge_dc2h_inout_size_valid(capSrc,capWid,capLen) == FALSE)
	{
		rtd_pr_vt_emerg("%s,dc2h unsupport in size smaller than out size\n", __FUNCTION__);
		return;
	}
	...
	else
	{
		if((get_VT_Pixel_Format() == VT_CAP_NV12) || (get_VT_Pixel_Format() == VT_CAP_NV16)||(get_VT_Pixel_Format() == VT_CAP_NV21)){
			if(5 == VtBufferNum)
			{
				...
				dc2h_cap_boundaryaddr3_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[1].phyaddr +  (UINT32)CaptureCtrl_VT.cap_buffer[1].size;
				...
				dc2h_cap_boundaryaddr5_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[2].phyaddr +  (UINT32)CaptureCtrl_VT.cap_buffer[2].size;
				...
				dc2h_cap_boundaryaddr7_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[3].phyaddr +  (UINT32)CaptureCtrl_VT.cap_buffer[3].size;
				...
				dc2h_cap_boundaryaddr9_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[4].phyaddr +  (UINT32)CaptureCtrl_VT.cap_buffer[4].size;
			...
			else if(4 == VtBufferNum)
			{
				...
				dc2h_cap_boundaryaddr9_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[3].phyaddr +  (UINT32)CaptureCtrl_VT.cap_buffer[3].size;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr9_reg, dc2h_cap_boundaryaddr9_reg.regValue);
			...
			else if(3 == VtBufferNum)
			{
				...
				dc2h_cap_boundaryaddr7_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[2].phyaddr +  (UINT32)CaptureCtrl_VT.cap_buffer[2].size;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr7_reg, dc2h_cap_boundaryaddr7_reg.regValue);
				dc2h_cap_boundaryaddr8_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[2].phyaddr;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr8_reg, dc2h_cap_boundaryaddr8_reg.regValue);
				
				dc2h_cap_boundaryaddr9_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[2].phyaddr +  (UINT32)CaptureCtrl_VT.cap_buffer[2].size;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr9_reg, dc2h_cap_boundaryaddr9_reg.regValue);
			...
			else if(2 == VtBufferNum)
			{
				...
				dc2h_cap_boundaryaddr5_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[1].phyaddr +  (UINT32)CaptureCtrl_VT.cap_buffer[1].size;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr5_reg, dc2h_cap_boundaryaddr5_reg.regValue);
				dc2h_cap_boundaryaddr6_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[1].phyaddr;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr6_reg, dc2h_cap_boundaryaddr6_reg.regValue); 
				
				dc2h_cap_boundaryaddr7_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[1].phyaddr +  (UINT32)CaptureCtrl_VT.cap_buffer[1].size;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr7_reg, dc2h_cap_boundaryaddr7_reg.regValue);
				dc2h_cap_boundaryaddr8_reg.regValue = (UINT32)CaptureCtrl_VT.cap_buffer[1].phyaddr;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr8_reg, dc2h_cap_boundaryaddr8_reg.regValue);
				
				dc2h_cap_boundaryaddr9_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[1].phyaddr +  (UINT32)CaptureCtrl_VT.cap_buffer[1].size;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr9_reg, dc2h_cap_boundaryaddr9_reg.regValue);
			...
			else //1 == VtBufferNum
			{
				...
				dc2h_cap_boundaryaddr3_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[0].phyaddr +  (UINT32)CaptureCtrl_VT.cap_buffer[0].size;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr3_reg, dc2h_cap_boundaryaddr3_reg.regValue);
				dc2h_cap_boundaryaddr4_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[0].phyaddr;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr4_reg, dc2h_cap_boundaryaddr4_reg.regValue);

				dc2h_cap_boundaryaddr5_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[0].phyaddr +  (UINT32)CaptureCtrl_VT.cap_buffer[0].size;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr5_reg, dc2h_cap_boundaryaddr5_reg.regValue);
				dc2h_cap_boundaryaddr6_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[0].phyaddr;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr6_reg, dc2h_cap_boundaryaddr6_reg.regValue); 
				
				dc2h_cap_boundaryaddr7_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[0].phyaddr +  (UINT32)CaptureCtrl_VT.cap_buffer[0].size;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr7_reg, dc2h_cap_boundaryaddr7_reg.regValue);
				dc2h_cap_boundaryaddr8_reg.regValue = (UINT32)CaptureCtrl_VT.cap_buffer[0].phyaddr;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr8_reg, dc2h_cap_boundaryaddr8_reg.regValue);
				
				dc2h_cap_boundaryaddr9_reg.regValue =  (UINT32)CaptureCtrl_VT.cap_buffer[0].phyaddr +  (UINT32)CaptureCtrl_VT.cap_buffer[0].size;
				rtd_outl(DC2H_DMA_dc2h_Cap_BoundaryAddr9_reg, dc2h_cap_boundaryaddr9_reg.regValue);
...
unsigned char do_vt_reqbufs(unsigned int buf_cnt)
{
	...
	if(0 == buf_cnt) /* Release buffers by setting buf_cnt = 0*/
	{
		if((vfod_capture_location == KADP_VT_SCALER_OUTPUT) && (get_video_do_overscan_state()) && (get_src_cap_enable_status() == TRUE))
		{
			...
			|| (vfod_capture_location == KADP_VT_OSDVIDEO_OUTPUT))
		{
		...
		if ((get_vt_EnableFRCMode()==FALSE))
		{
			...
			if(get_film_mode_parameter().enable == _DISABLE){
				...
				if(0 /*(Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID_PRE) > 2560) &&
					...
					if(FALSE == force_disp_s1_s2_same_clk()) /* input 120 memc always bypass */
						Scaler_MEMC_MEMC_CLK_OnOff(1, 1, 1); //not 4k 50/60Hz, enable MEMC clock with mute on
				}
				if(FALSE == force_disp_s1_s2_same_clk()){
					Scaler_MEMC_outMux(_ENABLE,_ENABLE); //db on, mux on
				}
...
unsigned int VT_get_input_height(VT_CAP_SRC capSrc)
{
	...
	if((get_vt_VtCaptureVDC())){
		...
		if(capSrc == VT_CAP_SRC_VIDEO_NOPQ) {
//for uzu position could not support 2k1k(when 2step enable,uzu mux size will be less than 2k1k)			
			if(ppoverlay_uzudtg_control1_reg.third_uzu_mode ==1) {
