...
static unsigned char ADC_Connect_SRC = _SRC_MAX;
...
{
	extern unsigned char ypp_good_timing_ready;
#ifdef CONFIG_SUPPORT_SRC_VGA
	extern unsigned char vga_good_timing_ready;
	vga_good_timing_ready = FALSE;
#endif//#ifdef CONFIG_SUPPORT_SRC_VGA
	ypp_good_timing_ready = FALSE;
	ADC_Connect_SRC = _SRC_MAX;
	rtd_pr_adc_info("#####[%s(%d)]######\r\n",__func__,__LINE__);
}
...
unsigned char adc_drv_timing_ready(void)
{
	extern unsigned char ypp_good_timing_ready;
	//extern unsigned char vga_good_timing_ready;
	if(ADC_Connect_SRC == _SRC_YPBPR){
		return ypp_good_timing_ready;
	}
#ifdef CONFIG_SUPPORT_SRC_VGA
	else if(ADC_Connect_SRC == _SRC_VGA){
		return FALSE;
	}
#endif
	else
...
{
	// Does not disable CRT !!!!!!
	if(enable)
		CRT_CLK_OnOff(SCALER_ADC_CLK_1, CLK_ON, NULL );
}
...
//	adc_power_RBUS adc_adc_power_reg;
	...
	if (on_off) {
		...
		dds_pll_pll_ctrl_reg.apllldosel = YPbPr_LDO_Trim()?YPbPr_LDO_Trim_apllldosel():1;	/* LDO 1.05V */
...
void ADC_Initial_Setting(void)
{
	...
	adc_apll_adc_vadc_idaco_ctrl1_RBUS adc_apll_adc_vadc_idaco_ctrl1_reg;
...
#ifdef ADC_UP_DOWN_CLAMP
	adc_adc_offset0_reg.adc_off_grn = 0;
	adc_adc_offset0_reg.adc_off_red = 0;
#else
...
#ifdef ADC_UP_DOWN_CLAMP
	adc_adc_offset1_reg.adc_off_blu = 0;
#else
	...
	adc_adc_clamp_ctrl3_reg.adc_clamp_ctrl5_8_7 = 1; /* BIT [8:7]:SOG_ADC VREF select (00:600mv 01:640mv 10:680mv 11:720mv) */
	...
	dds_pll_pll_phase_interpolation_reg.pllsphrs = 7; //RL6702-5663, 0x18020110[5] = 0x1
	...
	dds_pll_p_code_mapping_methods_reg.g_value = 0x3; //RL6702-5663, 0x18020114[7:4] = 0x3
	...
	dds_pll_plldiv_reg.apll_reserve_1_0 = 1; /* default thermal delay ypp 0x3 */
	...
	adc_adc_vbias_reg.adc_vbias1_0 = 1; //RL6702-5663, 0x1802000C[8] = 0x1
	...
	adc_adc_vbias_reg.adc_vbias2_4 = 0; //ADC Gain Calibration power control, 0: off, 1:on
	adc_adc_vbias_reg.adc_vbias2_5 = 0; //ADC Gain Calibration, Always Enable Clamp Signal
	adc_adc_vbias_reg.adc_vbias2_7 = 0; //ADC Gain Calibration, 0: normal, 1:calibration
	...
	adc_adc_clock_reg.adc_2x_sample_4 = 1;//RL6702-5663, 0x18020010[20] = 0x1
	...
	// VIN345P connect to internal path
	adc_apll_adc_vadc_idaco_ctrl1_reg.regValue = 0;
	adc_apll_adc_vadc_idaco_ctrl1_reg.reg_ypp_vd_share = 1; //RL6702-5663, 0x18020078[0] = 0x1
	IoReg_Write32(ADC_APLL_ADC_VADC_IDACO_CTRL1_reg, adc_apll_adc_vadc_idaco_ctrl1_reg.regValue);
...
#else
	adc_adc_clamp_ctrl1_reg.adc_clamp_ctrl4_6 = 0;
	adc_adc_clamp_ctrl1_reg.adc_clamp_ctrl4_5 = 0;
	...
	adc_adc_clamp_ctrl2_reg.regValue = IoReg_Read32(ADC_ADC_CLAMP_CTRL2_reg);
	adc_adc_clamp_ctrl2_reg.adc_sog_cmp_7_6 = YPbPr_LDO_Trim()?YPbPr_LDO_Trim_sog_cmp_7_6():0;
	IoReg_Write32(ADC_ADC_CLAMP_CTRL2_reg, adc_adc_clamp_ctrl2_reg.regValue);

#ifdef ADC_UP_DOWN_CLAMP
	adc_adc_test_reg.regValue = 0;
	adc_adc_test_reg.adc_test_2 = 1; // bit2 up/dn source selcet from(0:VDC 1:YPP)
...
#define REGISTER_START_ADC_3 (0xb8020078)
#define REGISTER_SIZE_ADC_3 (3)/* b8020078~b8020080 */
