...
unsigned char ypp_good_timing_ready = FALSE;//when YPP get good timing,it will be true
unsigned char ypp_debug_msg_print = TRUE;
...
{/* Do YPbPr coneect initial setting */
	...
	adc_adc_clamp_ctrl2_reg.adc_sog_cmp_2_0 = 7;/* SOG1 input MUX select (offline) 111:GND */
	adc_adc_clamp_ctrl2_reg.adc_sog_cmp_5_4 = 1;/* SOG0 input MUX select (online)   01:VIN4P */
...
unsigned char YPbPr_DetectTiming(void)
{
	...
	sync_processor_ef_ctrl_RBUS mv_ctrl_reg;
	...
	unsigned char ret = _MDOE_UNDKOWN;
	...
	if(YPbPr_TimingDetectStage != YPBPR_OFFLINE_STATE) {
		...
		ypp_good_timing_ready = FALSE;
	}
	mv_ctrl_reg.regValue = IoReg_Read32(SYNC_PROCESSOR_Ef_CTRL_reg);
	mv_ctrl_reg.s1_ef_enable = 0;//disable macrovision
	IoReg_Write32(SYNC_PROCESSOR_Ef_CTRL_reg,  mv_ctrl_reg.regValue);
	
	mv_ctrl_reg.regValue = IoReg_Read32(SYNC_PROCESSOR_Ef_CTRL_reg);
	mv_ctrl_reg.s1_ef_occur = 0;//clear macrovision status
	IoReg_Write32(SYNC_PROCESSOR_Ef_CTRL_reg,  mv_ctrl_reg.regValue);

	IoReg_Write32(SYNC_PROCESSOR_Ef_CTRL_reg,  0x720);
	...
	adc_adc_dcrestore_ctrl_reg.adc_ctl_rgb_5 = 1; //off pad
	...
	adc_adc_dcrestore_ctrl_reg.regValue = IoReg_Read32(ADC_ADC_DCRESTORE_CTRL_reg);
	adc_adc_dcrestore_ctrl_reg.adc_ctl_rgb_5 = 0; //pad connect
	IoReg_Write32(ADC_ADC_DCRESTORE_CTRL_reg, adc_adc_dcrestore_ctrl_reg.regValue);
	...
	if (YPbPr_TimingDetectStage == YPBPR_SOY_STATE) {/* Around 90ms */
		...
		if (YPbPr_DetectTiming_PadCheck() == FALSE) {
			...
			if(ypp_debug_msg_print)
				rtd_pr_adc_notice("#####[%s(%d)] Pad no signal!!!\n", __FUNCTION__, __LINE__);

			auto_soy_soych0_a0_reg.regValue = IoReg_Read32(AUTO_SOY_SOYCH0_A0_reg);
			auto_soy_soych0_a0_reg.reg_auto_mode = 0;//manual mode
		...
		if (_ENABLE == Scaler_Get_YPbPr_Auto_Soy_Enable()) {
			if (FALSE == YPbPr_AutoSOY_Detect()) {/* Auto restart setting (AutoSOY()) */
				...
				IoReg_Write32(AUTO_SOY_SOYCH0_A0_reg, auto_soy_soych0_a0_reg.regValue);
				if(ypp_debug_msg_print)
					rtd_pr_adc_notice("#####[%s(%d)]SOY fail, no signal!!!\n", __FUNCTION__, __LINE__);
		...
		if (ret == _MODE_SUCCESS) {
			...
			rtd_pr_adc_notice("#####[%s(%d)] 0xB8020300=0x%08x\n", __FUNCTION__, __LINE__,IoReg_Read32(AUTO_SOY_SOYCH0_A0_reg));
			return _MODE_SUCCESS;
		} else {
			rtd_pr_adc_notice("#####[%s(%d)] ret=0x%08x\n", __FUNCTION__, __LINE__, ret);
	...
	#ifdef ADC_PROTECT_PATCH //for WOSQRTK-11230
		adc_adc_dcrestore_ctrl_reg.regValue = IoReg_Read32(ADC_ADC_DCRESTORE_CTRL_reg);
		sync_processor_sp1_autorst_reg.regValue = IoReg_Read32(SYNC_PROCESSOR_SP1_AutoRst_reg);

		if((adc_adc_dcrestore_ctrl_reg.adc_dcrestore_ctrl_5 == 0)
			||(sync_processor_sp1_autorst_reg.s1_auto_result_rdy ==0)){
			rtd_pr_adc_info("#####[%s(%d)]re-do timing check,0xB8020018=0x%08x,0xB8021004=0x%08x\n",__func__, __LINE__,IoReg_Read32(ADC_ADC_DCRESTORE_CTRL_reg),IoReg_Read32(SYNC_PROCESSOR_SP1_AutoRst_reg));
			rtd_pr_adc_info("#####[%s(%d)]re-do timing check,0xB8020024=0x%08x,0xB8021070=0x%08x\n",__func__, __LINE__,IoReg_Read32(ADC_ADC_CLAMP_CTRL2_reg),IoReg_Read32(SYNC_PROCESSOR_SP_PAD_STATUS_reg));
			SyncProc_FunctionDisable(_SRC_YPBPR);
			YPbPr_TimingDetectStage = YPBPR_SOY_STATE;/* To do Auto SOY */
			ypp_good_timing_ready = FALSE;
			return ret;
		...
		if (ret != _MODE_SUCCESS) {
...
#ifdef YPBPR_SHOW_COSTTIME
			after_time = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg)/90;/* Detect success to get after time */
			rtd_pr_adc_notice("#####func:%s code time=%d ms######\r\n", __FUNCTION__, (after_time-before_time));
#endif
			ypp_good_timing_ready = FALSE;
			ypp_debug_msg_print = TRUE;

			//rtd_pr_adc_notice("YPbpr ANALOG_MODE_MEASUREMENT\n");
			rtd_pr_adc_notice("HSYNCPULSECOUNT:%d\n", p_timinginfo->IHSyncPulseCount);
			rtd_pr_adc_notice("IHCount:%d,IVCount:%d\n", p_timinginfo->IHCount, p_timinginfo->IVCount);
			rtd_pr_adc_notice("IHFreq:%d,IVFreq:%d\n", p_timinginfo->IHFreq, p_timinginfo->IVFreq);
			rtd_pr_adc_notice("**HS=%d,HW=%d,VS=%d,VW=%d\n",
				p_dispinfo->IPH_ACT_STA,
				p_dispinfo->IPH_ACT_WID,
				p_dispinfo->IPV_ACT_STA,
				p_dispinfo->IPV_ACT_LEN);
			rtd_pr_adc_notice("**IHFreq=%d,IVFreq=%d,IHTotal=%d,IVTotal=%d,ADC_CLOCK:%d\n",
				p_dispinfo->IHFreq,
				p_dispinfo->IVFreq,
				p_dispinfo->IHTotal,
				p_dispinfo->IVTotal,
				p_dispinfo->INPUT_ADC_CLOCK);
			//rtd_pr_adc_notice("YPbPr mode table index = %d\n", p_dispinfo->ucMode_Table_Index);
			...
			if(!ypp_good_timing_ready)
				ypp_good_timing_ready = TRUE;
			Set_Reply_Zero_Timing_Flag(VSC_INPUTSRC_ADC, SOURCE_GOOD_TIMING_READY);
#ifdef YPBPR_SHOW_COSTTIME
			before_time = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg)/90;/* Reset before time */
#endif
		}
...
void YPbPr_DetectFail_NoSignal_Handler(void)
{
    ...
    if (ypp_debug_msg_print){
		ypp_debug_msg_print = FALSE;
		rtd_pr_adc_info("#####[%s(%d)]0xB8020018=0x%08x\n",__func__, __LINE__,IoReg_Read32(ADC_ADC_DCRESTORE_CTRL_reg));
		rtd_pr_adc_info("#####[%s(%d)]0xB8021100=0x%08x\n",__func__, __LINE__,IoReg_Read32(OFFMS_SP_MS3Ctrl_reg));
		rtd_pr_adc_info("#####[%s(%d)]0xB8021118=0x%08x\n",__func__, __LINE__,IoReg_Read32(OFFMS_SP_MS3Stus_reg));
		rtd_pr_adc_info("#####[%s(%d)]0xB8020300=0x%08x\n",__func__, __LINE__,IoReg_Read32(AUTO_SOY_SOYCH0_A0_reg));
		rtd_pr_adc_info("#####[%s(%d)]0xB802030C=0x%08x\n",__func__, __LINE__,IoReg_Read32(AUTO_SOY_SOYCH0_A3_reg));
    }
}
...
void YPbPr_ADCPowerControl(unsigned char on_off)
{
	...
	if (TRUE == on_off) {
		...
		dds_pll_pll_ctrl_reg.apllldosel = YPbPr_LDO_Trim()?YPbPr_LDO_Trim_apllldosel():1;	/* LDO 1V */
		...
		dds_pll_pll_phase_interpolation_reg.pllsphrs = 6;/* default */
...
void YPbPr_Clamp_Ctrl(unsigned char output_enable, ADC_DATA_PATH adc_path, ADC_SOY_PATH soy_path)
{
	...
	adc_adc_clamp_ctrl2_reg.regValue =  IoReg_Read32(ADC_ADC_CLAMP_CTRL2_reg);
	adc_adc_clamp_ctrl2_reg.adc_sog_cmp_5_4 = soy_path;
	adc_adc_clamp_ctrl2_reg.adc_sog_cmp_2_0 = 7; //111:GND
	IoReg_Write32(ADC_ADC_CLAMP_CTRL2_reg, adc_adc_clamp_ctrl2_reg.regValue);
	...
	if(output_enable){
		...
		adc_adc_clamp_ctrl3_reg.regValue = IoReg_Read32(ADC_ADC_CLAMP_CTRL3_reg);
		adc_adc_clamp_ctrl3_reg.adc_clamp_ctrl5_15 = 1; /* SOG1 DCrestore enable(0:disable,1:enable)*/
		IoReg_Write32(ADC_ADC_CLAMP_CTRL3_reg, adc_adc_clamp_ctrl3_reg.regValue);
	}
...
void YPbPr_AutoSOY_DAC_LUT_FILL_Fast(int a_lOffset)
{
	...
	for (i = AUTO_SOY_CALIBRATION_INPUT_LEVEL_MAX; i >= 0; i--) {
		...
		rtd_pr_adc_notice("addr=%d, value=%d\n", i, ucAdcValue);
#endif
...
//	auto_soy_soych0_b0_RBUS auto_soy_soych0_b0_reg;
	...
	rtd_pr_adc_notice("Input DAC level : Y=%d\n", CurDacValue);
	...
	rtd_pr_adc_notice("CurDacValue=%d, ManualCompLevelStart=%d, ManualCompLevelEnd=%d, maxrange=%d\n", CurDacValue, ManualCompLevelStart, ManualCompLevelEnd, maxrange);
	...
	for (i = CurDacValue; CurDacShift <= maxrange; CurDacShift++) {
		if (CurDacShift == 0) {
			...
			if (0 == j) {
				...
				rtd_pr_adc_notice("ADC Compare Level Toggle fail at : %x \n", i); 
				continue;
			}
		...
		if ((i - CurDacShift) >= ManualCompLevelStart) {
			...
			if (0 == j) {
				/* this compare level no toggle */
				rtd_pr_adc_notice("##ADC Compare Level Toggle fail at : 0x%x=%d \n", i,i); 
				continue;
			}
...
void YPbPr_AutoSOY_Calibration_Fast(void)
{
	...
	if (YPbPr_Need_SOY_Calibration == 1) {
		rtd_pr_adc_info("Do fast AutoSOY_DAC_Calibration\n");
		...
		adc_adc_clamp_ctrl2_reg.regValue = IoReg_Read32(ADC_ADC_CLAMP_CTRL2_reg);
		original_sog_path = adc_adc_clamp_ctrl2_reg.adc_sog_cmp_5_4;
		adc_adc_clamp_ctrl2_reg.adc_sog_cmp_5_4 = ADC_SOY_MUX_CMP4_5_VSOG_CAL;
		...
		for (i = AUTO_SOY_CALIBRATION_INPUT_LEVEL_MIDDLE; i >= 0; i--) {
			...
			rtd_pr_adc_info("i=%d, lCurAdcValue=%d (line:%d)\n", i, lCurAdcValue, __LINE__);
		...
		adc_adc_clamp_ctrl2_reg.regValue = IoReg_Read32(ADC_ADC_CLAMP_CTRL2_reg);
		adc_adc_clamp_ctrl2_reg.adc_sog_cmp_5_4 = original_sog_path;
		adc_adc_clamp_ctrl2_reg.adc_sog_cmp1_7 = 1;
		IoReg_Write32(ADC_ADC_CLAMP_CTRL2_reg, adc_adc_clamp_ctrl2_reg.regValue);
...
#ifdef READ_AUTO_SOY_ADC_LUT_RESULT
	rtd_pr_adc_info("======No hit list and fill it by interpolation========\n");
#endif
...
void YPbPr_AutoSOY_DAC_LUT(int CurDacValue)
{
	...
	/* rtd_pr_adc_debug("CurDacValue=%d, ManualCompLevelStart=%d, ManualCompLevelEnd=%d, maxrange=%d\n", CurDacValue, ManualCompLevelStart, ManualCompLevelEnd, maxrange); */

	for (i = CurDacValue; CurDacShift <= maxrange; CurDacShift++) {
		/* rtd_pr_adc_debug("i=%d, CurDacShift=%d\n", i, CurDacShift); */
		...
		if (CurDacShift == 0) {
			...
			if (0 == j) {
				/* this compare level no toggle */
				/*rtd_pr_adc_debug("ADC Compare Level Toggle fail at : %x \n", i); */
				continue;
			}
		...
		if ((i - CurDacShift) >= ManualCompLevelStart) {
			...
			if (0 == j) {
				/* this compare level no toggle */
				/* rtd_pr_adc_debug("##ADC Compare Level Toggle fail at : 0x%x=%d \n", i,i); */
				continue;
			}
...
void YPbPr_AutoSOY_Calibration(void)
{
	...
	if (YPbPr_Need_SOY_Calibration == 1) {
		...
		sync_processor_sp_pad_status_RBUS sync_processor_sp_pad_status_reg;
		LUT_ADC_ENTRY64 =  0;
		rtd_pr_adc_info("Do AutoSOY_Calibration\n");
		YPbPr_Power_ADCRGB(TRUE);/* Need ADC full power */
		/* 1. Disable Auto SOY function */
		auto_soy_soych0_a0_reg.regValue = IoReg_Read32(AUTO_SOY_SOYCH0_A0_reg);
		auto_soy_soych0_a0_reg.reg_auto_mode = _DISABLE;
...
//	dds_pll_pllphase_ctrl_RBUS dds_pll_pllphase_ctrl_reg;
	...
	autosoy_soych0_a0_reg.reg_auto_mode = _DISABLE;
...
void YPbPr_AutoSOY_ReStart(unsigned char ReStart, unsigned int CompareLevel)/* input First DC Level */
{
	...
	/* rtd_pr_adc_debug("CompareLevel=%x\n",CompareLevel); */
	if (TRUE == ReStart) {
		/* Disable Auto SOY */
...
unsigned char YPbPr_AutoSOY_Detect(void)
{
	...
	for (i = 0; i < 21; i++) {
		  ...
		  if (autosoy_no_trigger_flag_reg.no_trig_r) {/* check notrig flag */
				...
				if (FALSE == AutoSOYRestart) {/* Run (False Restart) if notriger=0 in the first time */
					/* rtd_pr_adc_debug( "AutoSOY detect success 1st, time:%x, val:%x\n", i, CompareLevel); */
					YPbPr_AutoSOY_ReStart(TRUE, DCLevel + AUTO_SOY_COMPARE_LEVEL_OFFSET); /* first auto's comparelevel+0x5 Run AutoSOY restart again */
					rtd_pr_adc_notice("#####[%s(%d)] setting DCLevel level=0x%x,trigger i=%d\n",__func__,__LINE__, DCLevel + AUTO_SOY_COMPARE_LEVEL_OFFSET, i);
					AutoSOYRestart = TRUE;  /* change to TRUE */
					goto RESTART_AUTO_SOY;
				} else {
					rtd_pr_adc_notice("AutoSOY detect success, time:%d, CompareLevel:0x%x,DCLevel:0x%x,CompareLevelMax:0x%x,Min_r:0x%x\n", i, CompareLevel, DCLevel, CompareLevelMax, autosoy_soych0_a4_reg.min_r);
					return TRUE;
				}
	...
	rtd_pr_adc_notice("AutoSOY detect fail, CompareLevel:%d,DCLevel=%x\n", CompareLevel, DCLevel);
	return FALSE;
}
...
#endif
	unsigned char ucVCO_Divider4 = 0, ucDanger_tolerance = 6, ctrl, ucDanger_position =0;
//	unsigned int ulRate;
...
//	dds_pll_pllphase_ctrl_RBUS dds_pll_pllphase_ctrl_reg;
	...
	rtd_pr_adc_notice("auto phase=%d uc64phase_limited:%d\n", phase, uc64phase_limited);
#if 0
	...
	if (Scaler_InputSrcGetMainChType() == _SRC_VGA){
		...
		rtd_pr_adc_notice("pixelclock=%d", pixelclock);
	}
	...
	if (Scaler_InputSrcGetMainChType() == _SRC_VGA) {
		...
		switch(p_dispinfo->ucMode_Curr){
			...
			case _MODE_1080P50:
			case _MODE_1080P60:
				fix_phase_value = 0x34;

				M_VALUE = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_RESULT_PHASE_M_reg)&0x03FF;
				L_VALUE = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_RESULT_PHASE_L_reg);
				if(M_VALUE < 1 && L_VALUE && L_VALUE < 0x20000){
			...
			case _MODE_1080I25:
			case _MODE_1080I30:
				fix_phase_value = 0x14;

				M_VALUE = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_RESULT_PHASE_M_reg)&0x03FF;
				L_VALUE = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_RESULT_PHASE_L_reg);
				if(M_VALUE < 1 && L_VALUE < 0x20000){
					phase = fix_phase_value;
				}
	...
	rtd_pr_adc_notice("####[%s(%d)]M_VALUE:0x%08x L_VALUE:0x%08x A_temp1:0x%08x A_temp2:0x%08x####\r\n",__func__,__LINE__,M_VALUE,L_VALUE,A_temp1,A_temp2);
	/* Calculate the absolute value from the selected phase to transition */
	rtd_pr_adc_notice("fine-tune phase=%d, PhaseMax=%d\n", phase, PhaseMax);
	...
	rtd_pr_adc_notice( "Danger_distance=%d\n", ucDanger_distance);
#endif

	rtd_pr_adc_notice( "Danger_position=%d, ucDanger_tolerance=%d\n", ucDanger_position, ucDanger_tolerance);
	rtd_pr_adc_notice( "ctrl=%d\n", ctrl);
	...
	if (SP_AUTO_SOY_CLK_SEL_INIT_STATE == state) {
		...
		case _MODE_576P:
			...
			if(dispCurMode == _MODE_576I)
				...
				autosoy_soych0_a2_reg.reg_max_window = (_MODE_480I == dispCurMode)?1:COMPARE_LEVEL_NORMAL_SD_SEEK_MAX_WINDOW;
			...
			sync_processor_sp1_coastftbk_reg.s1_coast_win2_end = 2;
			sync_processor_sp1_coastftbk_reg.s1_coast_front= 2;
		...
		default:
			...
			sync_processor_sp1_coastftbk_reg.s1_coast_win2_end = 2;
			sync_processor_sp1_coastftbk_reg.s1_coast_front= 2;
			sync_processor_sp1_coastftbk_reg.s1_coast_back = 1;
			IoReg_Write32(SYNC_PROCESSOR_SP1_CoastFtBk_reg, sync_processor_sp1_coastftbk_reg.regValue);
		break;
		}
...
void YPbPr_SetSOYCompareLevel(unsigned char soy_path, unsigned char value)
{
	...
	if (SP_VHS_SOURCE_SOY0 == soy_path) {
		...
		autosoy_soych1_a0_reg.regValue = IoReg_Read32(AUTO_SOY_SOYCH1_A0_reg);
...
void YPbPr_Apll_Mode_Setting(unsigned int CurModeIdx)
{
	...
	case _MODE_480I:
		...
		dds_pll_p_code_mapping_methods_reg.g_value = 0x4;
	...
	case _MODE_1080P50:
		dds_pll_dds_mix_w1_reg.regValue = IoReg_Read32(DDS_PLL_DDS_MIX_W1_reg);
		dds_pll_dds_mix_w1_reg.new_mode_en = 0;/*enable new mode tracking*/
		dds_pll_dds_mix_w1_reg.lock_th = 3;
		IoReg_Write32(DDS_PLL_DDS_MIX_W1_reg, dds_pll_dds_mix_w1_reg.regValue);
...
void YPbPr_SyncProc_TimingModeSetting(unsigned int CurModeIdx)
{
	...
	switch (CurModeIdx) {
		...
		case _MODE_480P:
			...
			dds_pll_pll_set_reg.regValue = IoReg_Read32(DDS_PLL_PLL_SET_reg);
			dds_pll_pll_set_reg.pllssi = 2;/* PLL Charger Pump Current IchDpll */
			IoReg_Write32(DDS_PLL_PLL_SET_reg, dds_pll_pll_set_reg.regValue);
			break;
		...
		case _MODE_720P60:
			...
			dds_pll_pll_set_reg.regValue = IoReg_Read32(DDS_PLL_PLL_SET_reg);
			dds_pll_pll_set_reg.pllssi = 5;/* PLL Charger Pump Current IchDpll */
			IoReg_Write32(DDS_PLL_PLL_SET_reg, dds_pll_pll_set_reg.regValue);
			break;
...
{/* This api is setting after detect timing success  mode setting */
	...
	rtd_pr_adc_info("#####[%s(%d)]0xB8020018=0x%08x",__func__, __LINE__,IoReg_Read32(ADC_ADC_DCRESTORE_CTRL_reg));

	drvif_adc_ctl_rgb12_check(_SRC_YPBPR);
...
void drvif_adc_ctl_rgb12_check(int input)
{	//input: _SRC_YPBPR, _SRC_AV
	adc_adc_ctl_rgb12_RBUS adc_ctl_rgb12_reg;
	extern unsigned short get_AVD_Input(void);

	adc_ctl_rgb12_reg.regValue = IoReg_Read32(ADC_ADC_CTL_RGB12_reg);

	if(input == _SRC_VGA){
		adc_ctl_rgb12_reg.adc_ctl_rgb1_7 = 1;
		adc_ctl_rgb12_reg.adc_ctl_rgb1_5_4 = 0x0;
		adc_ctl_rgb12_reg.adc_ctl_rgb1_3_2 = 0x0;
		adc_ctl_rgb12_reg.adc_ctl_rgb1_1_0 = 0x0;
	}else if(input == _SRC_YPBPR){
		adc_ctl_rgb12_reg.adc_ctl_rgb1_8 = 0x1;
		adc_ctl_rgb12_reg.adc_ctl_rgb1_5_4 = 0x1;
		adc_ctl_rgb12_reg.adc_ctl_rgb1_3_2 = 0x1;
		adc_ctl_rgb12_reg.adc_ctl_rgb1_1_0 = 0x1;
	}else{
		adc_ctl_rgb12_reg.adc_ctl_rgb1_11_10 = (_AV_INPUT3 == get_AVD_Input())?0x1:0x2; //0008[11:10],1:VIN1P,2:VIN4P
	}

	IoReg_Write32(ADC_ADC_CTL_RGB12_reg, adc_ctl_rgb12_reg.regValue);
}
...
{
	int i;
	rtd_pr_adc_debug( "[YPbPr]: Content of DAC calibration result:\n");
	rtd_pr_adc_debug( "unsigned char dac_result[%d]={\n", AUTO_SOY_CALIBRATION_INPUT_LEVEL_MAX);
	for (i = 10; i >= 0; i--) {
		/* rtd_pr_adc_debug("0x%02x,",DAC_result[i]); */
		rtd_pr_adc_debug( "%3d,", DAC_result[i]);
	}
	for (i = 0; i <= 63; i++) {
		/* rtd_pr_adc_debug("0x%02x,",DAC_result[i]); */
		rtd_pr_adc_debug( "%3d,", DAC_result[i]);
		if (i == AUTO_SOY_CALIBRATION_INPUT_LEVEL_MAX) {
			rtd_pr_adc_debug(KERN_DEBUG "\b};\n\n");
			break;
		}
		if ((i % 16) == 15) {
			rtd_pr_adc_debug(KERN_DEBUG "\n");
		}
	}
	for (i = 0; i <= 63; i++) {
		/* rtd_pr_adc_debug("0x%02x,",DAC_result[i]); */
		rtd_pr_adc_debug( "%3d,", DAC_result[AUTO_SOY_CALIBRATION_INPUT_LEVEL_MAX-i]);
		if (i == AUTO_SOY_CALIBRATION_INPUT_LEVEL_MAX) {
			rtd_pr_adc_debug( "\b};\n\n");
			break;
		}
...
{//True : Need DDS checking. False : already disable DDS check
	...
	result = autosoy_soych0_a4_reg.max_r - autosoy_soych0_a4_reg.min_r;
	if (result < 25) {
		rtd_pr_adc_info("########Small_signal Disable DDS check########\r\n");
		dds_status_reg.regValue = IoReg_Read32(DDS_PLL_DDS_status_reg);
...
unsigned char YPbPr_LDO_Trim(void)
{
	//0xb80381E0[31]=1, will be do LDO trim
	return (IoReg_Read32(EFUSE_DATAO_120_reg) & _BIT31)>>31;
}

unsigned char YPbPr_LDO_Trim_vbias_5_3(void)
{
	//a. 0xb802000C[5:3] = 0xb80381E0[26:24]
	return (IoReg_Read32(EFUSE_DATAO_120_reg) & (_BIT26|_BIT25|_BIT24))>>24;
}

unsigned char YPbPr_LDO_Trim_apllldosel(void)
{
	//b. 0xb8020104[17:16] = 0xb80381E0[28:27]
	return (IoReg_Read32(EFUSE_DATAO_120_reg) & (_BIT28|_BIT27))>>27;
}

unsigned char YPbPr_LDO_Trim_sog_cmp_7_6(void)
{
	//c. 0xb8020024[31:30] = 0xb80381E0[30:29]
	return (IoReg_Read32(EFUSE_DATAO_120_reg) & (_BIT30|_BIT29))>>29;
}
...
unsigned char check_macrovision(unsigned int timing)
{
	...
	auto_soy_soych0_a3_RBUS autosoy_soych0_a3_reg;
	mv_ctrl_reg.regValue = IoReg_Read32(SYNC_PROCESSOR_Ef_CTRL_reg);
	mv_ctrl_reg.s1_ef_enable = 1;//enable macrovision
	IoReg_Write32(SYNC_PROCESSOR_Ef_CTRL_reg,  mv_ctrl_reg.regValue);

	msleep(10);

	//rtd_pr_adc_notice("mv_ctrl_reg.s1_mv_occur = %d\n", mv_ctrl_reg.s1_mv_occur);
	
	mv_ctrl_reg.regValue = IoReg_Read32(SYNC_PROCESSOR_Ef_CTRL_reg);
	if(mv_ctrl_reg.s1_ef_occur){
