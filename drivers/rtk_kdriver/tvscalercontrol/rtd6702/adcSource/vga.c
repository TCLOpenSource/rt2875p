...
static StructScreenModeInfo vga_auto_data[_MODE_PC_TIMING_END + 1];
...
//	adc_adc_dcrestore_ctrl_RBUS adc_adc_dcrestore_ctrl_reg;
	...
	adc_adc_vbias_reg.adc_vbias0_2_1 = 0;/* LDO voltage select 011:1.1v bit[7:5]=101 bit[4:2]=010 bit0=1*/
...
void VGA_ADCPowerControl(unsigned char on_off)
{
	...
	if (TRUE == on_off) {
		...
		ahs_smt_ctrl_reg.hsync0_3en = 1; /* 1: Power on */
		...
		ahs_smt_ctrl_reg.hsync0_3en = 0; /* 0: Power down */
		IoReg_Write32(ADC_AHS_SMT_CTRL_reg, ahs_smt_ctrl_reg.regValue);
	}
