...
#ifdef CONFIG_ENABLE_HDMITX // [MARK2] HDMITX output path
...
unsigned char fll_tracking_pcr_vs_flag=0;
...
void modestate_set_fll_tracking_pcr_vs_flag(unsigned char bEnable)
{
    if(fll_tracking_pcr_vs_flag != bEnable){
        rtd_pr_vbe_notice("### fll_tracking_pcr_vs_flag (%d) ###\n", bEnable);
        fll_tracking_pcr_vs_flag = bEnable;
    }
}

unsigned char modestate_get_fll_tracking_pcr_vs_flag(void)
{
    return fll_tracking_pcr_vs_flag;
}
...
#if 0	//Mark HW limit : frac a/b only support 1:1 or 1:2
...
//#ifndef CONFIG_MEMC_BYPASS
	...
	if(Get_DISPLAY_REFRESH_RATE() == 144){
		...
		memc_goldenvs_delay_line = DTGM_2_GOLDEN_VSYNC_DELAY_LINE_8K;
		memc_goldenvs_delay_pixel = DTGM_2_GOLDEN_VSYNC_DELAY_PIXEL_8K;
	}
	else{
...
*/
		...
		if(Get_TIMING_PROTECT_PANEL_TYPE()){
			...
			if((mode == 1)||(mode == 2)||(mode == 3)){
				...
				}else if (!((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) && vsc_get_adaptivestream_flag(SLR_MAIN_DISPLAY))){
					needSlow_tunning = 1;
				}
			}
		}
...
#if 0//mark2 no orbit line  delay
...
{
	unsigned int s1ip_delay_line =DTG_DEFAULT_S1IP_LINE_DELAY , s1ip_delay_line_value = 0;
	unsigned int time_borrow =scaler_disp_dtg_get_time_borrow_value();
	unsigned int b_isMEMCypass = (PPOVERLAY_MEMC_MUX_CTRL_get_memc_outmux_sel(IoReg_Read32(PPOVERLAY_MEMC_MUX_CTRL_reg))==0)?1:0;

	if((_DISP_WID == 7680) && (_DISP_LEN == 4320)){//8K panel
		s1ip_delay_line_value = DTG_DEFAULT_S1IP_LINE_DELAY_8K;
	}
	else{
		s1ip_delay_line_value = DTG_DEFAULT_S1IP_LINE_DELAY;
	}
	...
	if((drv_memory_get_game_mode() == _ENABLE) || vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())
	{
		if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) &&	Scaler_MEMC_Lib_UltraLowDelayMode_Judge()){
			...
			//normal game mode
			s1ip_delay_line = s1ip_delay_line_value;
		}else
	...
	{
		if((drv_memory_get_game_mode()) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) &&	Scaler_MEMC_Lib_UltraLowDelayMode_Judge()){
			// ultra-lowdelay game mode, no need to change.
		}else{
			//oled panel need to keep fixed value 0x16 between s1ip delay and timeborrow
			s1ip_delay_line = s1ip_delay_line_value - DTG_DEFAULT_TIME_BORROW_V + time_borrow;
		}
	}
	else
		s1ip_delay_line = s1ip_delay_line_value;
...
{
	int s1ip_uzu3rd_delay_line = 0;
	ppoverlay_uzudtg_control3_RBUS ppoverlay_uzudtg_control3_reg;
	ppoverlay_uzudtg_control5_RBUS ppoverlay_uzudtg_control5_reg;
	ppoverlay_uzudtg_stage1_time_borrow_RBUS ppoverlay_uzudtg_stage1_time_borrow_reg;
	ppoverlay_double_buffer_ctrl2_RBUS ppoverlay_double_buffer_ctrl2_reg;
	ppoverlay_double_buffer_ctrl2_reg.regValue = IoReg_Read32(PPOVERLAY_Double_Buffer_CTRL2_reg);

	if((_DISP_WID == 7680) && (_DISP_LEN == 4320)){//8K panel
		s1ip_uzu3rd_delay_line = DTG_DEFAULT_S1IP_UZU3RD_LINE_DELAY_8K;
	}
	else{
		s1ip_uzu3rd_delay_line = 0;
	}

	ppoverlay_uzudtg_stage1_time_borrow_reg.regValue = IoReg_Read32(PPOVERLAY_uzudtg_stage1_time_borrow_reg);
	ppoverlay_uzudtg_stage1_time_borrow_reg.stage1_time_borrow_v = scaler_disp_dtg_get_time_borrow_value();
//	ppoverlay_uzudtg_stage1_time_borrow_reg.stage1_time_borrow_sw_mode = 0;
	...
	rtd_pr_vbe_notice("stage1_time_borrow=%d\n", ppoverlay_uzudtg_stage1_time_borrow_reg.stage1_time_borrow_v);

	ppoverlay_uzudtg_control3_reg.regValue = IoReg_Read32(PPOVERLAY_uzudtg_control3_reg);
	ppoverlay_uzudtg_control5_reg.regValue = IoReg_Read32(PPOVERLAY_uzudtg_control5_reg);

	ppoverlay_uzudtg_control3_reg.s1ip_max_line_delay = scaler_disp_dtg_get_s1ip_delay_value();
	ppoverlay_uzudtg_control5_reg.s1ip_uzu3rd_max_line_delay = s1ip_uzu3rd_delay_line;

	IoReg_Write32(PPOVERLAY_uzudtg_control3_reg, ppoverlay_uzudtg_control3_reg.regValue);
	IoReg_Write32(PPOVERLAY_uzudtg_control5_reg, ppoverlay_uzudtg_control5_reg.regValue);
	if(bEnable_DB)
	{
		ppoverlay_double_buffer_ctrl2_reg.memc_dtgreg_dbuf_set =1;
		IoReg_Write32(PPOVERLAY_Double_Buffer_CTRL2_reg, ppoverlay_double_buffer_ctrl2_reg.regValue);
...
//	ppoverlay_uzudtg_control1_RBUS ppoverlay_uzudtg_control1_reg;
    ...
    if(ScalerHdmiTx_Get_Timing_Status() != TX_TIMING_INIT_STATE){
        // HDMITX setting
        ;
        // bypass dispd setting when HDMITX output is enabled
        rtd_pr_vbe_info("[HDMITX] HDMITX IS RUNNING[%d]@%s\n", ScalerHdmiTx_Get_Timing_Status(), __FUNCTION__);
    }
#endif
...
void Scaler_dynamic_change_output_timing(void)
{
	//update pif settings for output timing change
	//Photo display 8k30hz
	static unsigned int pre_output_timing_mode = OUTPUT_TIMING_MODE_MAX;
	if((pre_output_timing_mode != OUTPUT_8K4K60HZ) && (Get_Dynamic_Panel_Output_Timing_Mode() == OUTPUT_8K4K60HZ)){
		rtd_pr_vbe_notice("[PIF] Change output timing 8K4K60HZ!\n");
		panel_vby1_8k60hz_420_frc_tx();	//if pif setting is not 8K60Hz, need change to 8K60 setting
	}
	else if((pre_output_timing_mode != OUTPUT_8K4K30HZ) && (Get_Dynamic_Panel_Output_Timing_Mode() == OUTPUT_8K4K30HZ)){
		rtd_pr_vbe_notice("[PIF] Change output timing 8K4K30HZ!\n");
		panel_vby1_8k30hz_hsplit_frc_tx();	//if pif setting is not 8K30Hz, need change to 8K30Hz setting
	}
	else if((pre_output_timing_mode != OUTPUT_4K2K120HZ) && (Get_Dynamic_Panel_Output_Timing_Mode() == OUTPUT_4K2K120HZ)){
		rtd_pr_vbe_notice("[PIF] Change output timing 4K2K120HZ!\n");
		panel_vby1_4k120hz_frc_tx();	//if pif setting is not 4K120Hz, need change to 4K120Hz setting
	}
	pre_output_timing_mode = Get_Dynamic_Panel_Output_Timing_Mode();
}
...
*/
	...
	if(Get_DISPLAY_REFRESH_RATE() >= 120){
		if((Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ) >= 995)
	...
	//		vbe_dtg_memec_bypass_switch_set(1);
			...
			//Scaler_MEMC_Bypass_CLK_OnOff(0);
		}else if ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1))
		{
	...
	if((Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI)){
		...
		if(Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC)){
			...
			if ((vbe_disp_get_adaptivestream_fs_mode() == 1) &&
				...
				if(drvif_scaler_get_display_mode() != DISPLAY_MODE_FRAME_SYNC){
					rtd_pr_vbe_notice("[%s][line:%d] data fs case run timing fs.\n", __FUNCTION__, __LINE__);
					fwif_color_safe_od_enable(0);
					drvif_scaler_set_display_mode(DISPLAY_MODE_FRAME_SYNC);
					Scaler_Set_Display_Timing_Mode(DISPLAY_MODE_FRAME_SYNC);
					fwif_color_safe_od_enable(1);
				...
				modestate_set_fll_running_flag(_DISABLE);
			//}else if(get_current_driver_pattern(DRIVER_FREERUN_PATTERN) == 1){
				...
				if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())){
					rtd_pr_vbe_notice("[%s][line:%d] VRR/Freesync run timing fs.\n", __FUNCTION__, __LINE__);
					modestate_set_fll_running_flag(_DISABLE);
				...
				else if(vbe_get_HDMI_run_timing_framesync_condition()&& (scaler_vsc_get_gamemode_go_datafrc_mode()==FALSE)){
						rtd_pr_vbe_notice("[%s][line:%d] HDMI 8k default run timing fs.\n", __FUNCTION__, __LINE__);
				...
				else if(get_new_game_mode_small_window_condition()&&drv_memory_get_low_delay_game_mode_dynamic()){
					rtd_pr_vbe_notice("[%s][line:%d]game mode small window condition, set timing fs.\n", __FUNCTION__, __LINE__);
	/*
					//use hw_ctrl4 for panel timing protect. (iv2dv large to small)
	...
	*/
		...
		if(Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_FRC_LG_8K60HZ){
			Scaler_dynamic_change_output_timing();
		}
	...
	if(drv_memory_get_low_delay_game_mode_dynamic())	//WOSQRTK-10203 need to do this early	(patch for QA stage)
        ...
        //add subscaleup_coef4t256p10b
        fwif_color_set_subscaleup_coef4t256p10b();
        //-----------------------------------------        
		down(get_forcebg_semaphore());
...
unsigned int Get_dispd_stage1_f2p_data_rate(void){
	...
	if(sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel == 0)
		clk_div = 1;
	else if(sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel == 1)
		clk_div = 2;
	else if(sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel == 2)
		clk_div = 4;
	else
		clk_div = 8;

	return Get_DISPLAY_CLOCK_TYPICAL() * 2 / clk_div;
}
...
//#endif
		...
		{
			vertical_total = ((disp_stage1_data_rate/((Get_DISP_HORIZONTAL_TOTAL()/100)*vFreq))*removeRatio)/multipleRatio;
		}else
		{
				if((Get_DISPLAY_REFRESH_RATE() >= 120) &&
		...
		rtd_pr_vbe_notice("[%s] protect vtotal=%d\n", __FUNCTION__, vertical_total);
		...
		if((Get_DISPLAY_PANEL_CUSTOM_INDEX() != P_FRC_LG_8K60HZ) && (vertical_total <= Get_PANEL_DISP_VERTICAL_TOTAL_60Hz_MIN()))
		{
...
unsigned int scaler_disp_smooth_get_memc_protect_vtotal_By_100(void)
{
	...
	if(((vFreq>2350)&&(vFreq<=2450)) || //24hz
	   ...
	   ((vFreq>5950)&&(vFreq<=6050)))   //60hz
	{
		temp = div64_u64((unsigned long long)disp_stage1_data_rate,(Get_DISP_HORIZONTAL_TOTAL()/100));
                temp = temp*100;
    ...
    else
	{
		if((Get_DISPLAY_REFRESH_RATE() >= 120) &&
			...
			((vFreq>14350)&&(vFreq<=14450))))	//144hz
		{
			temp = div64_u64((unsigned long long)disp_stage1_data_rate,(Get_DISP_HORIZONTAL_TOTAL()/10));
			temp = temp*10;
			vertical_total = div64_u64(div64_u64(temp,vFreq)*removeRatio,multipleRatio);
		}
		else{
	...
	rtd_pr_vbe_notice("[%s] protect vtotalx100=%d\n", __FUNCTION__, vertical_total);
	return vertical_total;
...
void HAL_VBE_DISP_SetSpreadSpectrum(BOOLEAN bEnable, UINT16 u16Percent, UINT16 u16Period)
{
	...
	UINT32 m_code, f_code, n_code_ssc, f_code_ssc;
	...
	if((ulFreq>=800*1000000) && (ulFreq <= 1400*1000000)){
		div = 1;
	}else if((ulFreq>=400*1000000) && (ulFreq <= 700*1000000)){
		div = 1;
	...
	//n_code = 0;//n_code bypass
	dclk_Temp = ((ulFreq) /(27*100));
	...
	temp= 0;
	dpll_clk = (((m_code+3)*1000000 + (f_code*1000000)/2048)*27);
	...
	rtd_pr_vbe_debug("m_code:%d, f_code:%d, n_code_ssc:%d, f_code_ssc:%d\n", m_code , f_code, n_code_ssc, f_code_ssc);
	rtd_pr_vbe_debug("\n step:%d, gran_est:%d\n", step , gran_est);
...
void vbe_disp_panel_suspend(void)
{
	...
	#endif
	#if 0 //fix merlin7 compile error
	...
	vbe_disp_suspend_resume.SFG_PIF_FIFO_CTRL2B_S_R =                IoReg_Read32(SFG_PIF_FIFO_CTRL2B_reg);
	vbe_disp_suspend_resume.SFG_PIF_FIFO_CTRL2C_S_R =                IoReg_Read32(SFG_PIF_FIFO_CTRL2C_reg);
	...
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_A_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_A_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_B_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_B_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_C_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_C_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_D_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_D_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_E_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_E_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_F_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_F_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_G_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_G_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_H_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_H_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_I_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_I_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_J_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_J_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_K_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_K_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_L_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_L_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_M_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_M_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_N_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_N_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_O_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_O_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_P_S_R           = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_P_reg);
	if(IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_0_A_reg)!=0){
		vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_0_28_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_0_A_reg);
	}
	...
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60A_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60A_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60B_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60B_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60C_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60C_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60D_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60D_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60E_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60E_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60F_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60F_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60G_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60G_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60H_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60H_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60I_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60I_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60J_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60J_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60K_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60K_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60L_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60L_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60M_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60M_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60N_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60N_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60O_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60O_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60P_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60P_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61A_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61A_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61B_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61B_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61C_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61C_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61D_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61D_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61E_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61E_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61F_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61F_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61G_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61G_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61H_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61H_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61I_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61I_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61J_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61J_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61K_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61K_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61L_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61L_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61M_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61M_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61N_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61N_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61O_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61O_reg);
	vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61P_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61P_reg);
	...
	if((Get_DISPLAY_PANEL_TYPE() == P_VBY1_4K2K)  ||
		(Get_DISPLAY_PANEL_TYPE() == P_VBY1_8K4K)  ||
...
void vbe_disp_panel_resume(void)
{
	...
	IoReg_Write32(SFG_PIF_FIFO_CTRL2B_reg,					  vbe_disp_suspend_resume.SFG_PIF_FIFO_CTRL2B_S_R);
	IoReg_Write32(SFG_PIF_FIFO_CTRL2C_reg,					  vbe_disp_suspend_resume.SFG_PIF_FIFO_CTRL2C_S_R);
	...
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_A_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_A_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_B_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_B_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_C_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_C_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_D_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_D_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_E_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_E_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_F_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_F_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_G_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_G_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_H_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_H_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_I_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_I_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_J_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_J_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_K_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_K_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_L_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_L_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_M_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_M_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_N_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_N_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_O_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_O_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_P_reg,             vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDS_PHY_CTRL_P_S_R);
#if 0 //don't control lane power here
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_0_A_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_0_28_S_R);
#endif
	...
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60A_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60A_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60B_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60B_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60C_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60C_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60D_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60D_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60E_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60E_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60F_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60F_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60G_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60G_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60H_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60H_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60I_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60I_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60J_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60J_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60K_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60K_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60L_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60L_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60M_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60M_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60N_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60N_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60O_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60O_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60P_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_60P_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61A_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61A_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61B_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61B_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61C_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61C_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61D_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61D_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61E_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61E_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61F_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61F_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61G_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61G_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61H_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61H_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61I_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61I_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61J_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61J_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61K_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61K_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61L_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61L_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61M_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61M_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61N_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61N_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61O_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61O_S_R);
	IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61P_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_61P_S_R);
...
//	vbe_disp_suspend_resume.PPOVERLAY_memcdtg_FREE_DH_TOTAL_S_R = IoReg_Read32(PPOVERLAY_memcdtg_FREE_DH_TOTAL_reg);
	...
	vbe_disp_suspend_resume.CRT_DCLK_GATE_SEL2_S_R = IoReg_Read32(SYS_REG_SYS_DCLK_GATE_SEL2_reg);
...
//	IoReg_Write32(PPOVERLAY_memcdtg_FREE_DH_TOTAL_reg, vbe_disp_suspend_resume.PPOVERLAY_memcdtg_FREE_DH_TOTAL_S_R );
	...
	IoReg_Write32(SYS_REG_SYS_DCLK_GATE_SEL2_reg, vbe_disp_suspend_resume.CRT_DCLK_GATE_SEL2_S_R);
...
unsigned char vbe_disp_orbit_set_position_justscan(int x, int y)
{
	...
	ppoverlay_display_timing_ctrl2_RBUS ppoverlay_display_timing_ctrl2_reg;
	ppoverlay_memcdtg_control_5_RBUS 	ppoverlay_memcdtg_control_5_reg;
	//SCALER_ORBIT_SHIFT_INFO scalerOrbitMemInfo = {0};
	...
	ppoverlay_memcdtg_control_5_reg.regValue = IoReg_Read32(PPOVERLAY_MEMCDTG_CONTROL_5_reg);
	if(!(x%2)){//2n
	    if(x ==0){
            sfg_sfg_ctrl_1_reg.seg0_start = 0;
            sfg_sfg_dummy_ctrl_0a_reg.regValue = 0;
            ppoverlay_memcdtg_control_5_reg.dtgm2orbitvs_pixel_sft = -1*x;
	    }
		if(x < 0){
            sfg_sfg_ctrl_1_reg.seg0_start = 0;
            ppoverlay_memcdtg_control_5_reg.dtgm2orbitvs_pixel_sft = -1*x;
            ...
            sfg_sfg_dummy_ctrl_0a_reg.regValue = 0;
			ppoverlay_memcdtg_control_5_reg.dtgm2orbitvs_pixel_sft = x;
			ppoverlay_memcdtg_control_5_reg.dtgm2orbitvs_pixel_sft |= _BIT12; 	//sign bit
		}
	}
    else{
        if(x < 0){// 2n+1
			...
			ppoverlay_memcdtg_control_5_reg.dtgm2orbitvs_pixel_sft = -1*(x+1);
		}else{
            sfg_sfg_dummy_ctrl_0a_reg.regValue = 0x44440000;
			ppoverlay_memcdtg_control_5_reg.dtgm2orbitvs_pixel_sft = x -1;
			ppoverlay_memcdtg_control_5_reg.dtgm2orbitvs_pixel_sft |= _BIT12; 	//sign bit
		}
    }
	if(y >=0){
	 ppoverlay_memcdtg_control_5_reg.dtgm2orbitvs_line_sft = y;
	}else{
	 ppoverlay_memcdtg_control_5_reg.dtgm2orbitvs_line_sft = (-1*y);
	 ppoverlay_memcdtg_control_5_reg.dtgm2orbitvs_line_sft |= _BIT12;	 //sign bit
	}
    ...
    {
    	IoReg_Write32(PPOVERLAY_MEMCDTG_CONTROL_5_reg, ppoverlay_memcdtg_control_5_reg.regValue);
...
void vbe_disp_orbit_reset_position_justscan(void)
{
	...
	ppoverlay_memcdtg_control_5_reg.regValue = IoReg_Read32(PPOVERLAY_MEMCDTG_CONTROL_5_reg);
	ppoverlay_memcdtg_control_5_reg.dtgm2orbitvs_line_sft = 0;
	ppoverlay_memcdtg_control_5_reg.dtgm2orbitvs_pixel_sft = 0;
	IoReg_Write32(PPOVERLAY_MEMCDTG_CONTROL_5_reg, ppoverlay_memcdtg_control_5_reg.regValue);
...
{
	pinmux_lvdsphy_lvds_phy_ctrl_0_a_RBUS pinmux_lvdsphy_lvds_phy_ctrl_0_a_reg;

	if(IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_0_A_reg)!=0){
		vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_0_28_S_R = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_0_A_reg);
	}
	pinmux_lvdsphy_lvds_phy_ctrl_0_a_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_0_A_reg);
	pinmux_lvdsphy_lvds_phy_ctrl_0_a_reg.regValue = 0;
	IoReg_Write32(PINMUX_LVDSPHY_LVDS_PHY_CTRL_0_A_reg, pinmux_lvdsphy_lvds_phy_ctrl_0_a_reg.regValue);
...
UINT32 vbe_disp_get_VRR_timing_real_vtotal(void)
{
	...
	if(vrr_timingMode != VBE_DISP_VRR_MAX){
		...
		if(vrr_timingMode == VBE_DISP_VRR_1_1){
			vertical_total = Get_dispd_stage1_f2p_data_rate()/(Get_DISP_HORIZONTAL_TOTAL() * vsyncFreq);
		}else if(vrr_timingMode == VBE_DISP_VRR_1_2){
			vertical_total = Get_dispd_stage1_f2p_data_rate()/(Get_DISP_HORIZONTAL_TOTAL() * vsyncFreq * 2);
		}
	}
...
#define DEMURA_DMA_BURST_LENGTH 0x10
void vbe_disp_vby1_chip_info_setting(unsigned char benable, unsigned int a_ulAddr, unsigned int a_ulSize, unsigned char a_ucMode)
{//current for 4k120 setting
	ppoverlay_meta_vs_ctrl_RBUS ppoverlay_meta_vs_ctrl_reg;
	vby1_meta_ctl_RBUS vby1_meta_ctl_reg;
	vby1_meta_ctl_1_RBUS vby1_meta_ctl_1_reg;
	vby1_meta_data_chipinfo_0_RBUS vby1_meta_data_chipinfo_0_reg;
	demura_demura_dma_share_control_RBUS demura_demura_dma_share_control_reg;
	demura_demura_dma_wrap_addr_RBUS demura_demura_dma_wrap_addr_reg;
	...
	ppoverlay_meta_vs_ctrl_reg.regValue = IoReg_Read32(PPOVERLAY_META_VS_CTRL_reg);
	// HW or SW mode
	ppoverlay_meta_vs_ctrl_reg.meta_vs_en = benable;
	ppoverlay_meta_vs_ctrl_reg.meta_vs_mode = a_ucMode;
	ppoverlay_meta_vs_ctrl_reg.meta_vs_cnt_f = Get_DISP_DEN_STA_VPOS() - 1;
	ppoverlay_meta_vs_ctrl_reg.meta_vs_cnt_r = Get_DISP_DEN_END_VPOS() + 1;
	IoReg_Write32(PPOVERLAY_META_VS_CTRL_reg, ppoverlay_meta_vs_ctrl_reg.regValue);

	vby1_meta_ctl_reg.regValue = IoReg_Read32(VBY1_META_CTL_reg);
	//htotal=4400, 4k60 vby1 16 lane, each lane=4400/16=275 pixel => each line=275 pixel=0x113
	// meta_start delay line > (meta_vs_cnt_r - Get_DISP_DEN_END_VPOS())
	vby1_meta_ctl_reg.meta_start = 0x113 + 16; // 1 line + 16 pixel
	IoReg_Write32(VBY1_META_CTL_reg, vby1_meta_ctl_reg.regValue);

	vby1_meta_ctl_1_reg.regValue = IoReg_Read32(VBY1_META_CTL_1_reg);
	// 0: den end failing
	// 1: vsync
	vby1_meta_ctl_1_reg.meta_sel = 0;
	IoReg_Write32(VBY1_META_CTL_1_reg, vby1_meta_ctl_1_reg.regValue);

	vby1_meta_data_chipinfo_0_reg.regValue = IoReg_Read32(VBY1_META_DATA_CHIPINFO_0_reg);
	vby1_meta_data_chipinfo_0_reg.metabuf_en = benable;
	vby1_meta_data_chipinfo_0_reg.metadata_num = a_ulSize / 16 - 1;
	IoReg_Write32(VBY1_META_DATA_CHIPINFO_0_reg, vby1_meta_data_chipinfo_0_reg.regValue);

	//Set Demura DMA (48k byte)
	demura_demura_dma_share_control_reg.regValue = IoReg_Read32(DEMURA_DEMURA_DMA_SHARE_CONTROL_reg);
	demura_demura_dma_share_control_reg.demura_dma_share_req_en = 1;
	demura_demura_dma_share_control_reg.demura_dma_share_dma_en = 1;
	IoReg_Write32(DEMURA_DEMURA_DMA_SHARE_CONTROL_reg, demura_demura_dma_share_control_reg.regValue);

	// Demura start addr
	demura_demura_dma_wrap_addr_reg.regValue = IoReg_Read32(DEMURA_DEMURA_DMA_WRAP_ADDR_reg);
	demura_demura_dma_wrap_addr_reg.sta_addr = a_ulAddr / 16;
	IoReg_Write32(DEMURA_DEMURA_DMA_WRAP_ADDR_reg, demura_demura_dma_wrap_addr_reg.regValue);

	// Demura burst length
	demura_demura_dma_wrap_len_reg.regValue = IoReg_Read32(DEMURA_DEMURA_DMA_WRAP_LEN_reg);
	demura_demura_dma_wrap_len_reg.burst_len = DEMURA_DMA_BURST_LENGTH;
	...
	demura_demura_dma_wrap_word_reg.frame_burst_num = a_ulSize / 16; //0xc00;  48k byte=3000 x 16byte(unit 128bits)=0xbb8 x 16byte
	IoReg_Write32(DEMURA_DEMURA_DMA_WRAP_WORD_reg, demura_demura_dma_wrap_word_reg.regValue);


}

void vbe_disp_vby1_chip_info_sw_mode(unsigned char a_ucChipInfoReady)
{
	ppoverlay_meta_vs_ctrl_RBUS ppoverlay_meta_vs_ctrl_reg;

	ppoverlay_meta_vs_ctrl_reg.regValue = IoReg_Read32(PPOVERLAY_META_VS_CTRL_reg);
	ppoverlay_meta_vs_ctrl_reg.meta_vs_sw_set = a_ucChipInfoReady;
	IoReg_Write32(PPOVERLAY_META_VS_CTRL_reg, ppoverlay_meta_vs_ctrl_reg.regValue);
}
