...
VIP_TABLE_TIMIMG_ENUM xGetInputVipTiming(VIP_SOURCE_TIMING srcTiming)
{
	...
	case VIP_QUALITY_PVR_4k2kP_60:
		return VIP_TIMIMG_4K2K;
		break;
#ifdef Add_NewSrcIndex	 
	case VIP_QUALITY_DolbyHDR_HDMI_4k2kP_30:
	case VIP_QUALITY_DolbyHDR_HDMI_4k2kP_60:
	case VIP_QUALITY_DolbyHDR_HDMI_4k2k120:
	case VIP_QUALITY_DolbyHDR_DTV_4k2kP_30:
	case VIP_QUALITY_DolbyHDR_DTV_4k2kP_60:
	case VIP_QUALITY_DolbyHDR_DTV_4k2k120:
	case VIP_QUALITY_DolbyHDR_PVR_4k2kP_30:
	case VIP_QUALITY_DolbyHDR_PVR_4k2kP_60:
	case VIP_QUALITY_DolbyHDR_PVR_4k2k120:    
#else
...
void xLoadDefaultPicModeTable()
{
	...
	for (iCnt = 0; iCnt < m_iIndexSourceCnt[VIP_TABLE_PICMODE]; iCnt ++) {	//iCnt = source type
		...
		if (VIP_TABLE_INDEX_BY_SOURCE == m_indexType[VIP_TABLE_PICMODE]) {
			...
			m_picModeTableIdx[iCnt].source = (int)iCnt;
			...
			if (pIndexTableItem != NULL) {
				if (pIndexTableItem->timing != VIP_TIMIMG_DEFAULT || pIndexTableItem->color != VIP_COLORSTD_DEFAULT) {
					...
					if (pTableData != NULL) {
						...
						m_picModeTableIdx[iTableIdx].source = (int)xGetSourceTypeFromVipSource((VIP_TABLE_SRC_ENUM)pIndexTableItem->source);
...
void xLoadDefaultColorDataTable()
{
	...
	for (iCnt = 0; iCnt < m_iIndexSourceCnt[VIP_TABLE_COLORDATA]; iCnt ++) {
		...
		if (VIP_TABLE_INDEX_BY_SOURCE == m_indexType[VIP_TABLE_COLORDATA]) {
			...
			m_colorDataTableIdx[iCnt].source = iCnt;
			...
			if (pIndexTableItem != NULL) {
				if (pIndexTableItem->timing != VIP_TIMIMG_DEFAULT || pIndexTableItem->color != VIP_COLORSTD_DEFAULT) {
					...
					if (pTableData != NULL) {
						...
						m_colorDataTableIdx[iTableIdx].source = (int)xGetSourceTypeFromVipSource((VIP_TABLE_SRC_ENUM)pIndexTableItem->source);
...
void xLoadDefaultColorTempTable()
{
	...
	for (iCnt = 0; iCnt < m_iIndexSourceCnt[VIP_TABLE_COLORTEMP]; iCnt ++) {
		...
		if (VIP_TABLE_INDEX_BY_SOURCE == m_indexType[VIP_TABLE_COLORTEMP]) {
			SourceOption src;
			if (xGetSourceByIndex(&src, iCnt) == true)
				m_colorTempDataIdx[iCnt].source = (int)src;
			else // browser
				m_colorTempDataIdx[iCnt].source = SOURCE_PLAYBACK;
		} else
			m_colorTempDataIdx[iCnt].source = iCnt;
			...
			if (pIndexTableItem != NULL) {
				if (pIndexTableItem->timing != VIP_TIMIMG_DEFAULT || pIndexTableItem->color != VIP_COLORSTD_DEFAULT) {
					...
					if (pTableData != NULL) {
						...
						m_colorTempDataIdx[iTableIdx].source = (int)xGetSourceTypeFromVipSource((VIP_TABLE_SRC_ENUM)pIndexTableItem->source);
...
void xLoadDefaultColorFacModeTable()
{
	...
	for (iCnt = 0; iCnt < m_iIndexSourceCnt[VIP_TABLE_COLORFAC]; iCnt ++) {
		...
		if (VIP_TABLE_INDEX_BY_SOURCE == m_indexType[VIP_TABLE_COLORFAC]) {
			...
			m_colorFacModeTableIdx[iCnt].source = (int)iCnt;
			...
			if (pIndexTableItem != NULL) {
				if (pIndexTableItem->timing != VIP_TIMIMG_DEFAULT || pIndexTableItem->color != VIP_COLORSTD_DEFAULT) {
					...
					if (pTableData != NULL) {
						...
						m_colorFacModeTableIdx[iTableIdx].source = (int)xGetSourceTypeFromVipSource((VIP_TABLE_SRC_ENUM)pIndexTableItem->source);
