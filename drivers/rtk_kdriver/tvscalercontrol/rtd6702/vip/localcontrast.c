...
unsigned char drvif_color_set_SLC_Curve(unsigned int *pCurve, unsigned char Blk)
{
	...
	for(i=0; i<3; i++)
	{
		...
		if(i<2)
		{
			...
			// mark2 curve data numbers: total=35, separate to 12, 12, 11
			for(j=0; j<6; j++)
			{
				WriteValue = ((pCurve[i*12+j*2+1] & 0xfff) << 16) + (pCurve[i*12+j*2] & 0xfff);
		...
		else
		{
			...
			IoReg_Write32(LC_D_SLC_tab_wr_data10_reg, pCurve[i*12+10]);
		}
...
short drvif_SLC_DMA_Err_Check(unsigned char showMSG)
{
	...
	err_TBL[0] = SRNN_3DLUT_SRNN_3DLUT_Table1_ErrStatus2Irq_get_table1_err_status(IoReg_Read32(SRNN_3DLUT_SRNN_3DLUT_Table1_ErrStatus2Irq_reg));

	if (err_TBL[0]  != 0) {
		ret_val = ret_val | 0x01;
		if (showMSG == 1)
			rtd_pr_vpq_emerg("SLC_DMA_Err_Check, TBL1=%x\n", err_TBL[0]);
	}
...
{
	// SLC only use table1
	IoReg_SetBits(SRNN_3DLUT_SRNN_3DLUT_Table1_ErrStatus2Irq_reg, SRNN_3DLUT_SRNN_3DLUT_Table1_ErrStatus2Irq_table1_err_clr_mask);
}
...
char drvif_SLC_Wait_for_DMA_Apply_Done(void)
{
	...
	if (apply_done_check <= 0) {
		rtd_pr_vpq_emerg("SLC DMA, Wait DMA finish fail\n");
		ret_val = -1;
	}
	return ret_val;
}
...
void drvif_color_Set_SLC_By_DMA( unsigned int addr, unsigned int* vir_addr, unsigned int *pTBL){
	...
	//unsigned int curve[35]={0,32,84,120,180,270,370,480,600,720,840,960,1080,1200,1370,1540,1710,1880,2048,2218,2388,2558,2728,2890,3040,3160,3280,3400,3520,3640,3740,3840,3920,4020,4095}; 
	//unsigned int SLC_curve[256][35];
	int tbl,i;
	int data_count_byte=0;
	int pt=0,regvalue=0;
	int k = 0;
	/* SLC register */
	lc_slc_global_ctrl0_RBUS lc_slc_global_ctrl0_reg;
	...
	/* DMAtoSRNN register */
	srnn_3dlut_srnn_3dlut_table1_format0_RBUS srnn_3dlut_srnn_3dlut_table1_format0_reg; //table1_num_x; table1_num_y;table1_bitwidth;table1_dma_en
	srnn_3dlut_srnn_3dlut_table1_format1_RBUS srnn_3dlut_srnn_3dlut_table1_format1_reg; //table1_num_z; table1_burst_len; table1_id
	srnn_3dlut_srnn_3dlut_table1_addr_RBUS srnn_3dlut_srnn_3dlut_table1_addr_reg; //table1_mstart
	srnn_3dlut_srnn_3dlut_table1_burst_RBUS srnn_3dlut_srnn_3dlut_table1_burst_reg; //brust num
	srnn_3dlut_srnn_3dlut_table1_dma_RBUS srnn_3dlut_srnn_3dlut_table1_dma_reg; //remain
	srnn_3dlut_srnn_3dlut_table1_status_RBUS srnn_3dlut_srnn_3dlut_table1_status_reg; //table1_wdone
	srnn_3dlut_srnn_3dlut_db_ctl_RBUS srnn_3dlut_srnn_3dlut_db_ctl_reg;
	...
	srnn_3dlut_srnn_3dlut_table1_format0_reg.regValue = IoReg_Read32(SRNN_3DLUT_SRNN_3DLUT_Table1_Format0_reg);
	srnn_3dlut_srnn_3dlut_table1_format1_reg.regValue = IoReg_Read32(SRNN_3DLUT_SRNN_3DLUT_Table1_Format1_reg);
	srnn_3dlut_srnn_3dlut_table1_burst_reg.regValue = IoReg_Read32(SRNN_3DLUT_SRNN_3DLUT_Table1_Burst_reg);
	srnn_3dlut_srnn_3dlut_table1_dma_reg.regValue = IoReg_Read32(SRNN_3DLUT_SRNN_3DLUT_Table1_DMA_reg);
	srnn_3dlut_srnn_3dlut_table1_status_reg.regValue = IoReg_Read32(SRNN_3DLUT_SRNN_3DLUT_Table1_Status_reg);
	srnn_3dlut_srnn_3dlut_db_ctl_reg.regValue = IoReg_Read32(SRNN_3DLUT_SRNN_3DLUT_db_ctl_reg);
	srnn_3dlut_srnn_3dlut_table1_addr_reg.regValue = IoReg_Read32(SRNN_3DLUT_SRNN_3DLUT_Table1_Addr_reg);

	//phy addr
	srnn_3dlut_srnn_3dlut_table1_addr_reg.table1_mstart = addr;
	IoReg_Write32(SRNN_3DLUT_SRNN_3DLUT_Table1_Addr_reg,addr);
	...
	rtd_pr_vpq_info(" SLC phy_addr = %x,SLC_virt = %p,\n", addr, pVirtAddr);
	
	vir_addr_8 = (unsigned char*)vir_addr;

	//arrange 12bit curve data to 8bit ddr	
	for (  tbl = 0; tbl < 256; tbl++)
	{
		for( i=0;i<35;i++)
		{
			...
			{
				regvalue=regvalue+pt;
				k=0;
				
				*vir_addr_8=regvalue>>16;
				vir_addr_8++;
				
				*vir_addr_8=(regvalue>>8)&0xff;
				vir_addr_8++;

				*vir_addr_8=regvalue&0xff;
				vir_addr_8++;

				data_count_byte=data_count_byte+3;
			}
		}
	}	
	
	rtd_pr_vpq_info("SLC data arrangement done, total num:%d (3byte)\n", data_count_byte);
	rtd_pr_vpq_info("SLC phy_addr = %x,SLC_virt = %p,\n", addr, pVirtAddr);
	
	
	
	//set SLC_dma_enable=1
	lc_slc_global_ctrl0_reg.regValue = IoReg_Read32(LC_SLC_Global_Ctrl0_reg);
	lc_slc_global_ctrl0_reg.slc_dma_enable = 1;
	IoReg_Write32(LC_SLC_Global_Ctrl0_reg,lc_slc_global_ctrl0_reg.regValue);
	
	//set burst num=70
	srnn_3dlut_srnn_3dlut_table1_burst_reg.regValue = IoReg_Read32(SRNN_3DLUT_SRNN_3DLUT_Table1_Burst_reg);
	srnn_3dlut_srnn_3dlut_table1_burst_reg.table1_burst_num = burst_num;
	IoReg_Write32(SRNN_3DLUT_SRNN_3DLUT_Table1_Burst_reg,srnn_3dlut_srnn_3dlut_table1_burst_reg.regValue);
	
	//set remain=0
	srnn_3dlut_srnn_3dlut_table1_dma_reg.regValue = IoReg_Read32(SRNN_3DLUT_SRNN_3DLUT_Table1_DMA_reg);
	srnn_3dlut_srnn_3dlut_table1_dma_reg.table1_remain = remain;
	IoReg_Write32(SRNN_3DLUT_SRNN_3DLUT_Table1_DMA_reg,srnn_3dlut_srnn_3dlut_table1_dma_reg.regValue);
	
	//set dma_en; bit_width=27; y=1; x=2532
	srnn_3dlut_srnn_3dlut_table1_format0_reg.regValue = IoReg_Read32(SRNN_3DLUT_SRNN_3DLUT_Table1_Format0_reg);
	srnn_3dlut_srnn_3dlut_table1_format0_reg.table1_bitwidth = bitwidth;
	srnn_3dlut_srnn_3dlut_table1_format0_reg.table1_num_x = num_x ;
	srnn_3dlut_srnn_3dlut_table1_format0_reg.table1_num_y = num_y;
	srnn_3dlut_srnn_3dlut_table1_format0_reg.table1_dma_en = 1;
	IoReg_Write32(SRNN_3DLUT_SRNN_3DLUT_Table1_Format0_reg,srnn_3dlut_srnn_3dlut_table1_format0_reg.regValue);

	//set id; burst_len; z
	srnn_3dlut_srnn_3dlut_table1_format1_reg.regValue = IoReg_Read32(SRNN_3DLUT_SRNN_3DLUT_Table1_Format1_reg);
	srnn_3dlut_srnn_3dlut_table1_format1_reg.table1_id = 0xd;
	srnn_3dlut_srnn_3dlut_table1_format1_reg.table1_burst_len = burst_len;
	srnn_3dlut_srnn_3dlut_table1_format1_reg.table1_num_z = num_z ;
	IoReg_Write32(SRNN_3DLUT_SRNN_3DLUT_Table1_Format1_reg,srnn_3dlut_srnn_3dlut_table1_format1_reg.regValue);
...
unsigned char drvif_color_get_SLC_Curve(unsigned int *pCurve, unsigned char Blk)
{
	...
	for(i=0; i<3; i++)
	{
		...
		// mark2 curve data numbers: total=35, separate to 11, 12, 12
		if(i==0)
		{
			for(j=0; j<5; j++)
			{
			...
			ReadValue = IoReg_Read32(LC_D_SLC_tab_rd_data10_reg);
			pCurve[10] = ((ReadValue >>  0) & 0xfff );
		}
		else if(i==1)
		{
			for(j=0; j<6; j++)
			{
				ReadValue = IoReg_Read32(LC_D_SLC_tab_rd_data0_reg+j*4);
				pCurve[11+j*2  ] = ((ReadValue >>  0) & 0xfff );
				pCurve[11+j*2+1] = ((ReadValue >> 16) & 0xfff );
			}
		}
		else
		{
			...
			{
				ReadValue = IoReg_Read32(LC_D_SLC_tab_rd_data0_reg+j*4);
				pCurve[23+j*2  ] = ((ReadValue >>  0) & 0xfff );
				pCurve[23+j*2+1] = ((ReadValue >> 16) & 0xfff );
			}
		}
