...
#ifdef DISABLE_ALL_ONLINE_FUNCTION
...
#define YPP_ONLINE_ENABLE_MASK  (0)//YPP
#define VGA_ONLINE_ENABLE_MASK  (0)//VGA
...
#else//#ifdef DISABLE_ALL_ONLINE_FUNCTION
#ifdef HDMI_HS_MEAS_OVER_RANGE_WD_ENABLE
#define HDMI_4K120_VRR_ONLINE_ENABLE_MASK (_BIT4 | _BIT5 | _BIT10  | _BIT11 | _BIT13 | _BIT21 | _BIT22)//HDMI 4k120 vrr
#define HDMI_4K120_FREESYNC_ONLINE_ENABLE_MASK (_BIT4 | _BIT5 | _BIT10  | _BIT13 | _BIT21 | _BIT22)//HDMI 4k120 free sync
#define HDMI_4K120_ONLINE_ENABLE_MASK (_BIT4 | _BIT5 | _BIT7 | _BIT8 | _BIT9 | _BIT10 | _BIT11 | _BIT13 | _BIT15 | _BIT21 | _BIT22)//HDMI 4k120
#define HDMI_VRR_ONLINE_ENABLE_MASK  (_BIT2 | _BIT4 | _BIT5 | _BIT8 | _BIT9 | _BIT11 | _BIT13 | _BIT21 | _BIT22)//HDMI VRR
#define HDMI_FREESYNC_ONLINE_ENABLE_MASK  (_BIT4 | _BIT5 | _BIT8 | _BIT9 | _BIT13 | _BIT21 | _BIT22)//HDMI Freesync
#define HDMI_NORMAL_ONLINE_ENABLE_MASK  (_BIT2 | _BIT4 | _BIT5 | _BIT7 | _BIT8 | _BIT9 | _BIT10 | _BIT11 | _BIT13 | _BIT15 | _BIT21 | _BIT22)//HDMI normal
#else
...
#define YPP_ONLINE_ENABLE_MASK  (_BIT3 | _BIT8 | _BIT9 | _BIT11 | _BIT13 | _BIT15 | _BIT21)//YPP
#define VGA_ONLINE_ENABLE_MASK  (_BIT3 | _BIT8 | _BIT9 | _BIT10 | _BIT11 | _BIT13 | _BIT14 | _BIT15 | _BIT21)//VGA
#define VO_CP_FS_ONLINE_ENABLE_MASK  (_BIT8 | _BIT9 | _BIT11 | _BIT13)//VO adaptive FS
#define VO_CP_FRC_ONLINE_ENABLE_MASK  (_BIT8 | _BIT9 | _BIT11 | _BIT13 | _BIT14 | _BIT15)//VO adaptive FS
#define VO_FS_ONLINE_ENABLE_MASK  (_BIT8 | _BIT9 | _BIT11 | _BIT13)//VO FS
#define VO_FRC_ONLINE_ENABLE_MASK  (_BIT8 | _BIT9 | _BIT11 | _BIT13)//VO FRC
#endif
...
unsigned char get_scaler_run_hdmi_hfr_mode(void)
{
	return Scaler_Pre_Go_4K120_Mode;
}

void set_scaler_run_hdmi_hfr_mode(unsigned char mode)
		...
		if(_CHANNEL1 == channel)
		{
			...
			if (src == _SRC_HDMI) {
				{// Only for HDMI source, check particular device or debugging command
					...
					{
						rtd_pr_onms_info("[drvif_mode_dma_onlinemeasure_setting][HDMI] channel: 1, Update onms wdg from %x to %x\n", original_value,  onms_onms1_watchdog_en_reg.regValue);
					}
				}
			}
		...
		else// if(_CHANNEL2 == channel)
		{
			...
			if (src == _SRC_HDMI) {
				{// Only for HDMI source, check particular device or debugging command
					...
					{
						rtd_pr_onms_info("[drvif_mode_dma_onlinemeasure_setting][HDMI] channel: 2, Update onms wdg from %x to %x\n", original_value,  onms_onms2_watchdog_en_reg.regValue);
					}
				}
			}
		...
		if(_CHANNEL1 == channel)
		{
			...
			if (src == _SRC_HDMI) {
				{// Only for HDMI source, check particular device or debugging command
					...
					{
						rtd_pr_onms_info("[drvif_mode_dma_onlinemeasure_setting][HDMI] channel: 1, Update onms int from %x to %x\n", original_value,  onms_onms1_interrupt_en_reg.regValue);
					}
				}
			}
		...
		else// if(_CHANNEL2 == channel)
		{
			...
			if (src == _SRC_HDMI) {
				{// Only for HDMI source, check particular device or debugging command
					...
					{
						rtd_pr_onms_info("[drvif_mode_dma_onlinemeasure_setting][HDMI] channel: 2, Update onms int from %x to %x\n", original_value,  onms_onms2_interrupt_en_reg.regValue);
					}
				}
			}
...
//						onms_onms3_hsync_meas_ctrl_reg.on3_htotal_max_delta_new = 1;
	...
	if(_ENABLE == wdgenable)
	{
		...
		if (src == VSC_INPUTSRC_HDMI) {
			{// Only for HDMI source, check particular device or debugging command
				...
				{
					rtd_pr_onms_info("[drvif_mode_dma_onlinemeasure_setting][HDMI]  Update onms wd from %x to %x\n", original_value,  onms_onms3_watchdog_en_reg.regValue);
				}
			}
		}
...
#if 1			/* always enable watchdog to backgrould */
			ppoverlay_main_display_control_rsv_reg.main_wd_to_background = _ENABLE;//Enable force BG
#else
	...
	if(_ENABLE == interruptenable)
	{
			...
			if (src == VSC_INPUTSRC_HDMI) {
				{// Only for HDMI source, check particular device or debugging command
					...
					{
						rtd_pr_onms_info("[drvif_mode_dma_onlinemeasure_setting][HDMI]  Update onms int from %x to %x\n", original_value,  onms_onms3_interrupt_en_reg.regValue);
					}
				}
			}
	...
	if ((src == VSC_INPUTSRC_HDMI)&&(wdgenable || interruptenable)) {
		...
		pr_info("[drvif_mode_dma_onlinemeasure_setting][HDMI]  Update current_hdmi_onms_watchdog_setting=%x, current_hdmi_onms_is_i3ddma=%x\n", current_hdmi_onms_watchdog_setting,  current_hdmi_onms_is_i3ddma);
	}
}
#endif
			...
			else
			{
				...
				{//hdmi
...
#endif
					{
						...
						//11: 16383                                //11: 32768
						onms_onms1_demode_vs_uf_reg.on1_de_vsper_uf_th = 0;
						if(Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ)) {
				...
				{//vdec
						...
						if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_JPEG){
							if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_V_FREQ))
								onms_onms1_demode_vs_of_reg.on1_de_vsper_of_th = ((unsigned int)CONFIG_ONLINE_XTAL_FREQ*(unsigned int)11)/Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_V_FREQ);
							else
								onms_onms1_demode_vs_of_reg.on1_de_vsper_of_th = ((unsigned int)CONFIG_ONLINE_XTAL_FREQ*(unsigned int)11)/600;
						}else{
...
{//Return FALSE: Online measure has error status.
	...
	if(channel == _CHANNEL1)
	{
		...
		if(src == _SRC_HDMI)//HDMI RX,skip onms status check when work on specified device
		{
			...
			{
				rtd_pr_onms_info("[drvif_mode_check_onlinemeasure_status][HDMI] channel: 1, Update onms status from %x to %x\n", original_value,  onms_onms1_status_reg.regValue);
			}
		}
	...
	else
	{
		...
		if(src == _SRC_HDMI)//HDMI RX,skip onms status check when work on specified device
		{
			...
			{
				rtd_pr_onms_info("[drvif_mode_check_onlinemeasure_status][HDMI] channel: 2, Update onms status from %x to %x\n", original_value,  onms_onms2_status_reg.regValue);
			}
		}
...
{//Return FALSE: Online measure has error status.
	...
	if(src == VSC_INPUTSRC_HDMI)//HDMI RX,skip onms status check when work on specified device
	{
		...
		{
			rtd_pr_onms_info("[drvif_mode_dma_onlinemeasure_setting][HDMI] channel: dma, Update onms status from %x to %x\n", original_value,  onms_onms3_status_reg.regValue);
		}
	}
...
//	sub_vgip_vgip_chn2_act_hsta_width_RBUS vgip_chn2_act_hsta_width_reg;
	...
	//Enable Cap In1_force_all_rst
	drv_memory_MCapForceAllRst(display, TRUE);
	...
	if(SLR_MAIN_DISPLAY == display)
	{
		...
		ctrl_i_clock_2p_sel(_MAIN_VGIP_PATH, 0);//reset 2p
...
*/
		...
		ctrl_i_clock_2p_sel(_DAM_VGIP_PATH, 0);//reset 2p
	...
	else// if(_CHANNEL2 == GET_INPUT_CHANNEL(display))
	{
		...
		ctrl_i_clock_2p_sel(_SUB_VGIP_PATH, 0);//reset 2p
