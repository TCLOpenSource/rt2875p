...
//#include <rbus/ptol_reg.h> //Mark2 removed
...
//	ppoverlay_memcdtg_control3_RBUS ppoverlay_memcdtg_control3_reg;
	...
	ppoverlay_uzudtg_control1_reg.third_uzu_mode = tsuzu_en; // uzu2 = 0:disable, 1:enable
...
void zoom_imd_smooth_color_ultrazoom_config_scaling_up(unsigned char display)
{
	...
	//unsigned char V8tap_mode = 0;
	if (imdSmoothInfo[display].IMD_SMOOTH_MEM_ACT_WID<=3840) {
...
//	smooth_toggle_Scaler_AdjustPathAndInfo(display);	// Move position
	...
	else
	{
		...
		ich2_uzd_Ctrl0_REG.sort_fmt = ((imdSmoothInfo[display].IMD_SMOOTH_DISP_422CAP)? 0 : 1);
...
//	imdSmoothInfo[display].IMD_SMOOTH_DISP_MA3A = FALSE;//need review
	...
	if ( (Scaler_InputSrcGetType(display) != _SRC_VO) ||
    	 ((Scaler_InputSrcGetType(display) == _SRC_VO) && (Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_VO_SOURCE_TYPE) == 1) )
	   )
	{
	    ...
	    unsigned int  disp_hsize, unsigned int  disp_vsize    )
{
		...
		zoom_info.source_type =Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_VO_SOURCE_TYPE);
...
unsigned char Scaler_Set_VOsmoothenable(unsigned char display)
{
	...
	{
		rtd_pr_smt_info("_rk[zoom]SCALERIOC_SET_VOsmoothenable fail. ret=%d\n",ret);
		return FALSE;
	}
...
void zoom_checkresetflag(unsigned char display, unsigned char zoomfunc)
{
	...
	{//if source changed, reset zoom_first_crop,zoom_fist_dis, to prevent wrong crop/disp size by rika 20140630
		...
		if(Scaler_InputSrcGetType(display) ==_SRC_VO){
			pre_volinein = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_VO_SOURCE_TYPE);
		}
	...
	{ //under vo->vo case, need to check if directvo<->dtv
		...
		current_volinein = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_VO_SOURCE_TYPE);
...
//for SDNR overscan
	...
	if(SLR_MAIN_DISPLAY == display){
...

		{
			...
			IoReg_Write32(SCALEDOWN_ICH1_UZD_DB_CTRL_reg,ich2_uzd_db_ctrl_reg.regValue);
