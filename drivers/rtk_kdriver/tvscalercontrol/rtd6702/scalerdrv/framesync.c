...
UINT32 Scaler_Get_FixLastLine_DVTotal_Limit_50Hz_Typical(void)
{
	if(Scaler_FixLastLine_DVTotal_Limit_50Hz_Typical != 0)
		...
		return (Get_dispd_stage1_f2p_data_rate() / (Get_DISP_HORIZONTAL_TOTAL()*50));
}
...
//	verticalTotal = ((Get_DISPLAY_CLOCK_TYPICAL()/Get_DISP_HORIZONTAL_TOTAL())*100)/outputFrameRate;
	verticalTotal = ((disp_stage1_data_rate/Get_DISP_HORIZONTAL_TOTAL())*200)/outputFrameRate; // multiple 100 and shift 1 for rounding
	...
	if(!((Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_FRC_LG_8K60HZ) && (Get_Dynamic_Panel_Output_Timing_Mode() == OUTPUT_4K2K120HZ))
		&& !((Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_FRC_LG_8K60HZ) && (Get_Dynamic_Panel_Output_Timing_Mode() == OUTPUT_8K4K30HZ))){
...
void framesync_set_vtotal_by_vodma_freerun_period(void){
	...
	verticalTotal = ((Get_dispd_stage1_f2p_data_rate()/Get_DISP_HORIZONTAL_TOTAL())*200)/outputFrameRate; // multiple 100 and shift 1 for rounding
	verticalTotal = ((verticalTotal + 1) >> 1) & 0xFFFFFFFE;
...
unsigned int framesync_get_video_line_buffer_delay(void)
{
	...
	if(((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) ==  VSC_INPUTSRC_VDEC)&& vsc_get_adaptivestream_flag(SLR_MAIN_DISPLAY)) ||
		...
		if(iv2dv_line < (pVOInfo->v_start + _VO_GATTING_OFFSET) - (Get_DISP_DEN_STA_VPOS()-_DTG_TIME_BORROW_LINE)){
		...
		iv2dv_line = DEFAULT_FS_IV2DV_DELAY;
	}
#else
	iv2dv_line = DEFAULT_FS_IV2DV_DELAY;
#endif
		...
		if(drv_memory_get_low_delay_game_mode_dynamic()){//if game mode case, need to use calculation result @Crixus 20170712
			...
			if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_FRAMESYNC)){
				...
				if((Get_DISPLAY_REFRESH_RATE() == 120) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())&& ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DISP_LEN)== _DISP_LEN)){
					...
					if(iv2dv_delay<DEFAULT_FRC_IV2DV_DELAY){
						iv2dv_delay = DEFAULT_FRC_IV2DV_DELAY;
						iv2dv_delay = DEFAULT_FRC_IV2DV_DELAY;
					}
					fs_iv_dv_fine_tuning5_reg.iv2dv_line = iv2dv_delay;
					fs_iv_dv_fine_tuning5_reg.iv2dv_line_2 = iv2dv_delay;
...
unsigned char drvif_framesync_detect(void)
{
	...
	//Currently only JPEG source will enter this protection flow.
	//check ibuff status when full gatting enabled and forcebg disabled
	if ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_FRAMESYNC) == TRUE) && (IoReg_Read32(VODMA_VODMA_CLKGEN_reg) & VODMA_VODMA_CLKGEN_en_fifo_full_gate_mask)
...
//CSW+ 0961214 new code for frame sync by tyan
	...
	//rtd_pr_vbe_info("Read DPLL_M=%x, div=%x in FastDo\n", mcode, div);
...
unsigned int drvif_framesync_get_vo_clk(void)
{
	...
	vo_clk = 27000000*((nMCode+4)/(nNCode+1))/(result);
	rtd_pr_vbe_emerg("\n\n [%s][%d][%d,%d,%d] vo_clk : %ld \n\n", __FUNCTION__, __LINE__,nMCode, nNCode, nOCode, vo_clk);
	return (unsigned int)vo_clk;
}
...
unsigned int VODMA_get_datafs_increased_clock(unsigned int orig_ratio_n_off)
{
	...
	vodmapll = drvif_framesync_get_vo_clk();//VODMA PLL 1188M
    stage1_dclk = Get_dispd_stage1_f2p_data_rate() * (sys_dispclksel_reg.dclk_fract_a + 1) / (sys_dispclksel_reg.dclk_fract_b + 1);

	if(stage1_dclk > vodmapll){//stage1 dclk > 1188M, 8K50/60Hz case
		vodmapll *= 2;
	}

	if ((vodmapll / 64 * 63) > stage1_dclk) {
        //[KTASKWBS-8584] If i3ddma source and not input fast case, do not increase vo clock
        ratio_n_off = 64 - ((stage1_dclk/10000) * 64 / (vodmapll / 10000));
        if ((ratio_n_off >= 1) && (((vodmapll / 64) * (64 - ratio_n_off)) < stage1_dclk)) {
            ratio_n_off -= 1;
        }
    }
	rtd_pr_vbe_emerg("[%s] vodmapll = %d, stage1_dclk = %d, ratio_n_off = %d\n", __FUNCTION__, vodmapll, stage1_dclk, ratio_n_off);

#if 0
    if ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) != VSC_INPUTSRC_VDEC) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE) > DATAFS_DISABLE_GATTING_WIDTH) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) > 1080)){
			ratio_n_off = orig_ratio_n_off;
			pixel_clock = ((vodmapll/64) * (64 - ratio_n_off))/1000000;
			if((pixel_clock >= 230) && (pixel_clock < 250)){	//4K24Hz
				ratio_n_off -= 1;
			}
			else if((pixel_clock >= 250) && (pixel_clock < 300)){//4K25Hz
				ratio_n_off -= 3;
			}
			else if((pixel_clock >= 470) && (pixel_clock <= 594))
			{
				ratio_n_off = 32;	//594M
			}
			else{//8K case
				if((ratio_n_off > 1) &&((Scaler_DispGetInputInfoByDisp(0,SLR_INPUT_V_FREQ) > 235) &&(Scaler_DispGetInputInfoByDisp(0,SLR_INPUT_V_FREQ) < 245))){	//8K24Hz
					ratio_n_off -= 1;
				}
				else
					ratio_n_off = 0;
