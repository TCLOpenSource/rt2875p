...
//	UINT32 DH_total_Temp; // for Scaler_Get_EnterFRC_LastLine_DHtotal_Adjust_Enable_Flag()
	...
	if ( default_freq != 0 ) {
		temp_len = Scaler_DispGetInputInfo(SLR_INPUT_V_LEN);
		if (temp_len == 0)
			IFrameRate = 60;
		else
		{
			IFrameRate = niHFreq * 100 / temp_len;
		}

		if ( IFrameRate > 62)  //frame rate over 60 + miss error
		{
			rtd_pr_vbe_debug("Warning !! frame rate = %d  over 62",IFrameRate);
		...
		if ( niVActLen == 0 ) {
			rtd_pr_vbe_debug("ERROR!! [drvif_clock_set_dclk] get niVActLen = 0, set to default 60 MHz\n");
			niHFreq = 157;  // Set DClk to 60 MHz timing
			niVActLen = 231;
		}
...
//			if((TRUE == Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_FRAMESYNC)) && (TRUE == fwif_scaler_decide_display_cut4line(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY))))
		...
		{
			ulFreq = ulFreq<<1;
			rtd_pr_vbe_debug("drvif_clock_set_dclk, Double DVS need Double Dclk!\n");
	        }
		Scaler_Set_FRC_Open_Fix_Lastline_2(TRUE);
	}
...
//#else
	...
	if((Get_DISPLAY_REFRESH_RATE() >= 120) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 990)){
		dclk_fract_a = 0;
		dclk_fract_b = 0;
	}

	//Mark2 need set uzu 300M in 24Hz source
	if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ)>235)&&(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ)<245)){
		rtd_pr_vbe_notice("input 24HZ, Hack clock 300M\n");
		dclk_fract_a = 0;
		dclk_fract_b = 1;
	}
...
//#else
    ...
    if(Get_DISPLAY_REFRESH_RATE() >= 120){
		if((Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_REMOVE_RATIO) == 1) && (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_MULTIPLE_RATIO) == 5))
			dclk_fract_a = Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_REMOVE_RATIO) * 2 - 1;
		else
			dclk_fract_b = Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_MULTIPLE_RATIO) / 2 - 1;
    }
	...
	//Mark2 need set uzu 300M in 24Hz source
	if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ)>235)&&(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ)<245)){
		rtd_pr_vbe_notice("60HZ panel, input 24HZ, Hack clock 300M\n");
		dclk_fract_a = 0;
		dclk_fract_b = 1;
	}
...
//#ifndef CONFIG_MEMC_BYPASS
	...
	if((Get_DISPLAY_REFRESH_RATE() >= 120) && (Get_DISPLAY_PANEL_CUSTOM_INDEX() != P_FRC_LG_8K60HZ)){
		sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f1p_gate_sel = 0;
		if((Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY) && ((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) || force_disp_s1_s2_same_clk()))
			sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel = 1;
		else
			sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel = 2;
		...
		sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f4p_gate_sel_nouse = 0;
		...
		sys_reg_sys_dclk_gate_sel1_reg.dclk_mc_ippre2x_gate_sel = (sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel)?(sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel-1):0;
		sys_reg_sys_dclk_gate_sel1_reg.dclk_mc_gate_sel = sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f2p_gate_sel;
		sys_reg_sys_dclk_gate_sel1_reg.dclk_s2_f2p_div2_gate_sel_nouse= 1;
		sys_reg_sys_dclk_gate_sel1_reg.dclk_s2_420_2p_gate_sel = 1;
		sys_reg_sys_dclk_gate_sel2_reg.dclk_osd_m_disp_sub_1p_gate_sel = 2;
		sys_reg_sys_dclk_gate_sel2_reg.dclk_osd_m_disp_sub_2p_gate_sel = 3;
		...
		if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_8K4K){//8K60
			sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f1p_gate_sel = 0;
			sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel = 0;
			sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f1p_gate_sel = 0;
			sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f2p_gate_sel = 0;
			if((Get_DISPLAY_PANEL_CUSTOM_INDEX() == VBY_ONE_PANEL_8K4K_60HZ_OSD2K_PIP2K_1S_32L)
				|| (Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_FRC_LG_8K60HZ))
			{
				sys_reg_sys_dclk_gate_sel0_reg.dclk_osd_f1p_gate_sel = 3;
				sys_reg_sys_dclk_gate_sel0_reg.dclk_osd_f2p_gate_sel = 4;
			}
			else{
			...
			sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f4p_gate_sel_nouse = 0;
			sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f4p_gate_sel = 1;
			sys_reg_sys_dclk_gate_sel1_reg.dclk_memc_in_if_gate_sel = sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel;
			sys_reg_sys_dclk_gate_sel1_reg.dclk_memc_out_if_gate_sel = sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f2p_gate_sel+1;
			sys_reg_sys_dclk_gate_sel1_reg.dclk_mc_ippre_gate_sel = sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel;
			sys_reg_sys_dclk_gate_sel1_reg.dclk_mc_ippre2x_gate_sel = (sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel)?(sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel-1):0;
			sys_reg_sys_dclk_gate_sel1_reg.dclk_mc_gate_sel = sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f2p_gate_sel;
			sys_reg_sys_dclk_gate_sel1_reg.dclk_s2_f2p_div2_gate_sel_nouse= 1;
			sys_reg_sys_dclk_gate_sel1_reg.dclk_s2_420_2p_gate_sel = 0;
			if((Get_DISPLAY_PANEL_CUSTOM_INDEX() == VBY_ONE_PANEL_8K4K_60HZ_OSD4K_PIP2K_1S_32L)
				|| (Get_DISPLAY_PANEL_CUSTOM_INDEX() == VBY_ONE_PANEL_8K4K_60HZ_OSD2K_PIP2K_1S_32L)
				|| (Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_FRC_LG_8K60HZ)){
				sys_reg_sys_dclk_gate_sel2_reg.dclk_osd_m_disp_sub_1p_gate_sel = 3;
				sys_reg_sys_dclk_gate_sel2_reg.dclk_osd_m_disp_sub_2p_gate_sel = 4;
			}
			else{
				sys_reg_sys_dclk_gate_sel2_reg.dclk_osd_m_disp_sub_1p_gate_sel = 2;
				sys_reg_sys_dclk_gate_sel2_reg.dclk_osd_m_disp_sub_2p_gate_sel = 3;
			...
			if(Get_DISPLAY_CLOCK_TYPICAL() == 1188*1000000){//dpll 1.2G
				sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f1p_gate_sel = 1;
				sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel = 2;
				sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f1p_gate_sel = 1;
				sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f2p_gate_sel = 2;
				if(Get_DISPLAY_PANEL_CUSTOM_INDEX() == VBY_ONE_PANEL_4K2K_OSD2K2K_to_H5X_SPLIT){//OSD split 2K2K
					sys_reg_sys_dclk_gate_sel0_reg.dclk_osd_f1p_gate_sel = 2;
					sys_reg_sys_dclk_gate_sel0_reg.dclk_osd_f2p_gate_sel = 3;
				}
				else{
				...
				sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f4p_gate_sel_nouse = 0;
				sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f4p_gate_sel = 3;
				sys_reg_sys_dclk_gate_sel1_reg.dclk_memc_in_if_gate_sel = sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel;
				sys_reg_sys_dclk_gate_sel1_reg.dclk_memc_out_if_gate_sel = sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f2p_gate_sel+1;
				sys_reg_sys_dclk_gate_sel1_reg.dclk_mc_ippre_gate_sel = sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel;
				sys_reg_sys_dclk_gate_sel1_reg.dclk_mc_ippre2x_gate_sel = (sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel)?(sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel-1):0;
				sys_reg_sys_dclk_gate_sel1_reg.dclk_mc_gate_sel = sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f2p_gate_sel;
				sys_reg_sys_dclk_gate_sel1_reg.dclk_s2_f2p_div2_gate_sel_nouse= 2;
				sys_reg_sys_dclk_gate_sel1_reg.dclk_s2_420_2p_gate_sel = 2;
				sys_reg_sys_dclk_gate_sel2_reg.dclk_osd_m_disp_sub_1p_gate_sel = 2;
				sys_reg_sys_dclk_gate_sel2_reg.dclk_osd_m_disp_sub_2p_gate_sel = 3;
			}
			else{//dpll 594M
...
//#ifndef CONFIG_MEMC_BYPASS
	if(get_MEMC_bypass_status_refer_platform_model() == FALSE){
	    ...
	    if((Get_DISPLAY_REFRESH_RATE() >= 120) && (Get_DISPLAY_PANEL_CUSTOM_INDEX() != P_FRC_LG_8K60HZ)){
	        ...
	        if(low_delay_en == 1)
	            sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel = 1;
	        else if((Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY) && ((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) || force_disp_s1_s2_same_clk()))
				sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel = 1;
			else
				sys_reg_sys_dclk_gate_sel0_reg.dclk_s1_f2p_gate_sel = 2;
			sys_reg_sys_dclk_gate_sel0_reg.dclk_s2_f1p_gate_sel = 0;
