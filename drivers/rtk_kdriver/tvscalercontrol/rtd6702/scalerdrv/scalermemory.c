...
#define MAIN_FIFO_GATE_TH 1700;//8K60Hz dolby cert work around
...
unsigned int calc_comp_line_sum_bit(unsigned int comp_wid, unsigned int frame_limit_bit, unsigned char cmp_alpha_en);
...
unsigned char m_domain_memory_from_type(void)
{//4k memory size borrow type
	unsigned int carveout_width;
	unsigned int carveout_len;
	extern UINT8 scaler_vsc_get_gamemode_go_datafrc_mode(void);
	if(allocate_memory_for_8k_platform()) {
		// 8k model + 8k disp solution => uzd output 4k, buffersize=4k 
		carveout_width = 4096;//M_DOMAIN_WIDTH_CARVEOUT_4K; //tocheck >4k120 source
		carveout_len = M_DOMAIN_LEN_CARVEOUT_4K;
	}
	else
	{
		carveout_width = M_DOMAIN_WIDTH_CARVEOUT_2K;
		carveout_len = M_DOMAIN_LEN_CARVEOUT_2K;
	}
	...
	else if(scaler_vsc_get_gamemode_go_datafrc_mode() && (is_game_mode_set_line_buffer() == false))
		return BORROW_FROM_MEMC;//from memc
	else if((Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID) > carveout_width) && (Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN) > carveout_len)
...
{//3k 4k use data frc need borrow memory
	...
	if(allocate_memory_for_8k_platform()) {
		// 8k model + 8k disp solution => uzd output 4k, buffersize=4k 
		carveout_width = M_DOMAIN_WIDTH_CARVEOUT_4K;
		carveout_len = M_DOMAIN_LEN_CARVEOUT_4K;
    }
	else
	{
	...
	if ((!Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC))
			&& ((unsigned int)(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE) * Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE)) >  ((unsigned int)carveout_width * carveout_len)
...
//for sub cap/disp use in DCU2
	...
	mdomain_cap_ddr_outstanding_ctrl_main_cap_reg.in1_max_outstanding = _MAX_CAP_OUTSTANDING;
...
unsigned int memory_get_capture_line_size_compression(unsigned char compression_bits)
{
	...
	TotalSize = calc_comp_line_sum_bit(capWid, compression_bits, 0);
...
unsigned int memory_get_capture_frame_size_compression(unsigned char compression_bits,unsigned int capWid,unsigned int capLen)
{
	unsigned int TotalSize;
	unsigned int ulLineSumBit = 0;


	// cmp_line_sum_bit unit:128bits=16byte
	ulLineSumBit = calc_comp_line_sum_bit(capWid, compression_bits, 0);

	// DDR space
	//PQDC frame mode size=Image_H*line_sum_bit + Image_W* First_line_more_bit
	// First_line_more_bit=0x20
	TotalSize = (ulLineSumBit * 16) * capLen + capWid * 0x20 / 8;
...
unsigned long get_query_start_address(unsigned char idx){
	...
	unsigned int i3ddma_addr_offset = 0;
	unsigned int sub_addr_offset = 0;
	...
	if(idx == QUERY_IDX_MDOMAIN_MAIN){
		...
		if ((is_m_domain_data_frc_need_borrow_memory() && m_domain_memory_from_type() == BORROW_FROM_MEMC)) {
		...
		//how to calculate i3ddma_addr_offset? main_addr + main_buf_size*3?
		sub_addr_offset = CARVEOUT_FOR_MAIN_SIZE_16BITS;
#ifdef CONFIG_OPTEE_SECURE_SVP_PROTECTION
		size = (unsigned int)carvedout_buf_query_secure(CARVEDOUT_SCALER_MDOMAIN, (void **)&addr) ;
#else
	 	size = (unsigned int)carvedout_buf_query(CARVEDOUT_SCALER_MDOMAIN, (void **)&addr) ;
#endif
		if((size == 0) || (size < (sub_addr_offset+CARVEOUT_FOR_SUB_SIZE_12BITS))){
			BUG();
	 	}
		addr = addr + sub_addr_offset;
	} else if(idx == QUERY_IDX_I3DDMA) {
		//how to calculate i3ddma_addr_offset? main_addr + main_buf_size*3?
		i3ddma_addr_offset = CARVEOUT_FOR_MAIN_SIZE_16BITS + CARVEOUT_FOR_SUB_SIZE_12BITS;
#ifdef CONFIG_OPTEE_SECURE_SVP_PROTECTION
		size = (unsigned int)carvedout_buf_query_secure(CARVEDOUT_SCALER_MDOMAIN, (void **)&addr) ;
#else
	 	size = (unsigned int)carvedout_buf_query(CARVEDOUT_SCALER_MDOMAIN, (void **)&addr) ;
#endif
		if((size == 0) || (size < (i3ddma_addr_offset+CARVEOUT_FOR_I3DDMA_SIZE_20BITS))){
			BUG();
	 	}
		addr = addr + i3ddma_addr_offset;
	}
	if (idx != QUERY_IDX_VIP_DMAto3DTABLE)
		rtd_pr_scaler_memory_info("[%s]idx = %d, addr = 0x%lx\n", __FUNCTION__, idx, addr);
	return addr;
}
...
static void memory_set_sub_for_pip(unsigned char display)
{
	...
	if((SLR_SUB_DISPLAY == display) && ((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) || get_hdmi_vrr_4k60_mode())){
		fifoLen = _FIFOLENGTH;
	...
	if((get_hdmi_vrr_4k60_mode() || (get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1)) || ((get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_ROTATE_0)&&(get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_DEFAULT)))
	{
	...
	if(get_hdmi_vrr_4k60_mode())
		IoReg_Write32(MDOMAIN_DISP_DDR_Sub3rdAddr_reg, (drvif_memory_get_block_addr(MEMIDX_MAIN_THIRD) + ((ulLineStep + ulLineStep%2) << 3))&0x7ffffff0);
	else
...
//	unsigned int  TotalSizeFrame=0;
	...
	if((vsc_get_adaptivestream_flag(SLR_MAIN_DISPLAY)) ||(m_dispwin.srcy <= 100)) {
		ddr_mainprevstart_reg.main_pre_rd_v_start = drv_Calculate_m_pre_read_value();//MDOM_PREREAD_START;
	} else {
	...
	mdomain_disp_ddr_mainlinenum_reg.main_line_number = (Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_LEN) & 0x1FFF);
	...
	else if((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) || ((get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_ROTATE_0)&&(get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_DEFAULT)))
	{
		mdomain_disp_ddr_mainlinestep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainLineStep_reg);
...
//	unsigned int  TotalSizeFrame=0;
	...
	if(m_dispwin.srcy <= 100) {
		ddr_mainprevstart_reg.main_pre_rd_v_start = drv_Calculate_m_pre_read_value();
	} else {
		ddr_mainprevstart_reg.main_pre_rd_v_start = (m_dispwin.srcy-10);
	}
	IoReg_Write32(MDOMAIN_DISP_DDR_MainPreVStart_reg, ddr_mainprevstart_reg.regValue);
...
unsigned int drv_get_caveout_buffer_size_by_platform(unsigned int pixel_bits)
{
	...
	if(allocate_memory_for_8k_platform()) {
		// 8k model + 8k disp solution => uzd output 4k, buffersize=4k 
		width = M_DOMAIN_WIDTH_CARVEOUT_4K;
		len = M_DOMAIN_LEN_CARVEOUT_4K;
    }
	else {
		// 8k/4k model + 4k disp solution => uzd output 2k, buffersize=2k ; 
		// 2k model buffersize = 2k
		width = M_DOMAIN_WIDTH_CARVEOUT_2K;
		len = M_DOMAIN_LEN_CARVEOUT_2K;
	}
...
#else
    buffersize = memory_get_capture_frame_size_compression(pixel_bits, width, len);//64bits
    buffersize = (buffersize+ FINAL_BURST_LENTH);
#endif //CONFIG_BW_96B_ALIGNED
	return buffersize;
}

unsigned int drv_get_borrow_buffer_size_by_platform(unsigned int pixel_bits)
{
	unsigned int width;
	unsigned int len;
	unsigned int buffersize = 0;
	width = M_DOMAIN_WIDTH_MAX_SIZE;
	len = M_DOMAIN_LEN_MAX_SIZE;
#ifdef CONFIG_BW_96B_ALIGNED
	buffersize = (drvif_memory_get_data_align((width * pixel_bits + 256) / 8, DMA_SPEEDUP_ALIGN_VALUE) * len + FINAL_BURST_LENTH);
#else
 //   buffersize = memory_get_capture_line_size_compression_by_width(width, pixel_bits);//64bits
 //   buffersize = ((buffersize*64 / 8) * len + FINAL_BURST_LENTH);
    buffersize = memory_get_capture_frame_size_compression(pixel_bits, width, len);//64bits
    buffersize = (buffersize+ FINAL_BURST_LENTH);
...
//	unsigned char lineRepeatEnable = FALSE;
	...
	if(dvrif_memory_compression_get_enable(display) == TRUE)
	{
		if(dvrif_memory_get_compression_mode(display) == MEMCAPTYPE_LINE)
		{
			...
			TotalSize = memory_get_capture_frame_size_compression(dvrif_memory_get_compression_bits(display), Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_CAP_WID), Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_CAP_LEN));
		}
	}
	else
...
//		,Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY), SLR_INPUT_CAP_LEN), drvif_scaler3d_decide_3d_SG_data_FRC());
	...
	if (display == SLR_MAIN_DISPLAY)	 {
...

		{
			...
			if(get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1)
			{
				buffer_size = drv_get_borrow_buffer_size_by_platform(MDOMAIN_16BITS);//using pc mode size
				buffer_size = buffer_size/2 + FINAL_BURST_LENTH/2;//4k1k for 4k120 case
			}
			// 4k60 vrr/game mode
			else if ((src == VSC_INPUTSRC_HDMI)&& is_m_domain_data_frc_need_borrow_memory()) {
				buffer_size = drv_get_borrow_buffer_size_by_platform(MDOMAIN_16BITS);//using pc mode size
				rtd_pr_scaler_memory_emerg("[borrow memory] memory buffer_size=%d (MDOMAIN_MAIN_BUFFER_SIZE_16BITS)\n", buffer_size);
			...
			align_addr = query_addr; //dvr_memory_alignment((unsigned long)query_addr, buffer_size);
			#ifdef CONFIG_ARM64
			...
			MemTag[MEMIDX_MAIN_SEC].StartAddr = align_addr;
			...
			else
			{
				...
				if (is_m_domain_data_frc_need_borrow_memory() && m_domain_memory_from_type() == BORROW_FROM_CMA) {//4k frc and need borrow memory from cma
			...
			rtd_pr_scaler_memory_info("MemTag[MEMIDX_MAIN].StartAddr=%lx, MemTag[MEMIDX_MAIN_SEC].StartAddr=%lx, MemTag[MEMIDX_MAIN_THIRD].StartAddr=%lx, buffer_size=%x\n", MemTag[MEMIDX_MAIN].StartAddr, MemTag[MEMIDX_MAIN_SEC].StartAddr, MemTag[MEMIDX_MAIN_THIRD].StartAddr, buffer_size);
			...
			rtd_pr_scaler_memory_emerg("PAGE_ALIGN(buffer_size)=%x\n", PAGE_ALIGN(buffer_size));
			...
			#endif
			{
				unsigned long low_addr =  min(MemTag[MEMIDX_MAIN_THIRD].StartAddr & 0x7ffffff0,
						MemTag[MEMIDX_MAIN_SEC].StartAddr & 0x7ffffff0);

				unsigned long high_addr = max((MemTag[MEMIDX_MAIN].StartAddr+buffer_size) & 0x7ffffff0
						, (MemTag[MEMIDX_MAIN_THIRD].StartAddr+buffer_size) & 0x7ffffff0);
	...
	drvif_memory_free_block(MEMIDX_MAIN_THIRD);
		...
		if(allocate_memory_for_8k_platform()) {
			buffer_size = drv_get_caveout_buffer_size_by_platform(MDOMAIN_12BITS);//MDOMAIN_SUB_BUFFER_SIZE;
			query_addr = get_query_start_address(QUERY_IDX_MDOMAIN_SUB);
			buffer_size = drvif_memory_get_data_align(buffer_size, DMA_SPEEDUP_ALIGN_VALUE);
			align_addr = dvr_memory_alignment((unsigned long)query_addr, buffer_size);
			
			//16 bytes alignment
			MemTag[MEMIDX_SUB_THIRD].StartAddr = align_addr& 0x7ffffff0;
			MemTag[MEMIDX_SUB_SEC].StartAddr = drvif_memory_get_data_align(MemTag[MEMIDX_SUB_THIRD].StartAddr+buffer_size, DMA_SPEEDUP_ALIGN_VALUE) & 0x7ffffff0;
			MemTag[MEMIDX_SUB].StartAddr = drvif_memory_get_data_align(MemTag[MEMIDX_SUB_SEC].StartAddr+buffer_size, DMA_SPEEDUP_ALIGN_VALUE) & 0x7ffffff0;
			...
			rtd_pr_scaler_memory_info("MemTag[MEMIDX_SUB].StartAddr=%lx, MemTag[MEMIDX_SUB_SEC].StartAddr=%lx, MemTag[MEMIDX_SUB_THIRD].StartAddr=%lx, buffer_size=%x\n", MemTag[MEMIDX_SUB].StartAddr, MemTag[MEMIDX_SUB_SEC].StartAddr, MemTag[MEMIDX_SUB_THIRD].StartAddr, buffer_size);
...
void memory_set_memory_size(unsigned char bDouble, eMemCapAccessType Cap_type)
{
	...
	if (Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY){
...

		{
			...
			//rtd_pr_scaler_memory_emerg("MemTag[MEMIDX_MAIN].StartAddr=%x, MemTag[MEMIDX_MAIN_SEC].StartAddr=%x, MemTag[MEMIDX_MAIN_THIRD].StartAddr=%x, buffer_size=%x\n", MemTag[MEMIDX_MAIN].StartAddr, MemTag[MEMIDX_MAIN_SEC].StartAddr, MemTag[MEMIDX_MAIN_THIRD].StartAddr, buffer_size);
...
//-		rtd_pr_scaler_memory_debug("\n Allocate Sub Disp Memory !\n");
		...
		if(allocate_memory_for_8k_platform()) {
			...
			query_addr = get_query_start_address(QUERY_IDX_MDOMAIN_SUB);
			buffer_size = drvif_memory_get_data_align(buffer_size, DMA_SPEEDUP_ALIGN_VALUE);
			nStartAddr = dvr_memory_alignment((unsigned long)query_addr, buffer_size);
		} else {
			drvif_memory_free_block(MEMIDX_SUB);	// free block befor getting a new one
			drvif_memory_free_block(MEMIDX_SUB_SEC);	// free block befor getting a new one
			drvif_memory_free_block(MEMIDX_SUB_THIRD);	// free block befor getting a new one
			...
			if(get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1)
			{
				buffer_size = ((drv_get_borrow_buffer_size_by_platform(MDOMAIN_16BITS) + FINAL_BURST_LENTH)/2); //MDOMAIN_SUB_TWO_PIXEL_BUFFER_SIZE_PC_MODE
			}
			else
				buffer_size = drv_get_caveout_buffer_size_by_platform(MDOMAIN_12BITS);
		#ifdef ENABLE_3_BUFFER_DELAY_MODE
...
void dvrif_memory_handler(unsigned char display)
{
	...
	if (display == SLR_MAIN_DISPLAY) {	// Main
		...
		else
		{
			...
			{
				//rtd_pr_scaler_memory_debug("c.....\n");
				memory_set_main(TRUE,display);		//  CSW 0961105 change single buffer to double to slove 1080i moving picture not smooth
			}
		}
		if((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) && (Get_DISPLAY_REFRESH_RATE() >= 120))
...
void drv_memory_disable_di_write_req(unsigned char disableDiWr)
{
	...
	di_dma_multi_req_num_reg.rdma_req_num = 6; // 3;//(enable? 3: 0);	// change to 6, 20201120 recommend by jimmy xu in mer6
	di_dma_multi_req_num_reg.wdma_req_num = (disableDiWr? 0: 3); // (disableDiWr? 0: 2);	// change to 3, 20210805 recommend by jimmy xu in mk2
...
//	unsigned char cmp_alpha_en = 0;
	...
	rtd_pr_scaler_memory_info("function=%s, line=%d\n", __FUNCTION__, __LINE__);
	rtd_pr_scaler_memory_info("sub_pixel_num=%x, cmp_line_sum_bit_pure_rgb=%x, cmp_line_sum_bit_pure_a=%x, cmp_line_sum_bit_32_dummy=%x, cmp_line_sum_bit_128_dummy=%x\n", sub_pixel_num, cmp_line_sum_bit_pure_rgb, cmp_line_sum_bit_pure_a, cmp_line_sum_bit_32_dummy, cmp_line_sum_bit_128_dummy);
	rtd_pr_scaler_memory_info("cmp_line_sum_bit_real=0x%x\n", cmp_line_sum_bit_real);
	rtd_pr_scaler_memory_info("cmp_line_sum_bit=0x%x, frame_limit_bit=%d\n", cmp_line_sum_bit, frame_limit_bit);
...
void dvrif_memory_comp_setting(UINT8 enable_compression, UINT16 comp_wid, UINT16 comp_len, UINT8 comp_ratio){
	...
	mdomain_cap_cap_pq_cmp_bit_llb_RBUS mdomain_cap_cap_pq_cmp_bit_llb_reg;
	mdomain_cap_cap_pq_cmp_st_RBUS mdomain_cap_cap_pq_cmp_st_reg;
	...
	if(enable_compression == TRUE){
		...
		*/
		...
		if(dvrif_memory_get_compression_mode(SLR_MAIN_DISPLAY) == COMPRESSION_LINE_MODE){
			mdomain_cap_cap_pq_cmp_bit_reg.frame_limit_bit = comp_ratio*4;
			...
			mdomain_cap_cap_pq_cmp_bit_reg.frame_limit_bit = comp_ratio*4;
			//mdomain_cap_cap_pq_cmp_bit_reg.line_limit_bit = comp_ratio + M_DOMAIN_PQC_LINE_MARGIN;//Mark2 removed
		}
		...
		rtd_pr_scaler_memory_debug("sub_pixel_num=%x, cmp_line_sum_bit_pure_rgb=%x, cmp_line_sum_bit_pure_a=%x, cmp_line_sum_bit_32_dummy=%x, cmp_line_sum_bit_128_dummy=%x\n", sub_pixel_num, cmp_line_sum_bit_pure_rgb, cmp_line_sum_bit_pure_a, cmp_line_sum_bit_32_dummy, cmp_line_sum_bit_128_dummy);
		rtd_pr_scaler_memory_debug("cmp_line_sum_bit_real=%x\n", cmp_line_sum_bit_real);
		rtd_pr_scaler_memory_debug("cmp_line_sum_bit=%x\n", cmp_line_sum_bit);

		// Both line mode & frame mode need to set cmp_line_sum_bit;
		mdomain_cap_cap_pq_cmp_pair_reg.cmp_line_sum_bit = cmp_line_sum_bit;
		...
		if(Get_PANEL_VFLIP_ENABLE() || get_hdmi_vrr_4k60_mode() ||  dvrif_memory_get_compression_mode(SLR_MAIN_DISPLAY) == COMPRESSION_LINE_MODE)
		...
		//mdomain_cap_cap_pq_cmp_pair_reg.cmp_dic_mode_en = 1;//Mark2 removed
		mdomain_cap_cap_pq_cmp_pair_reg.cmp_jump4_en = 1;
		mdomain_cap_cap_pq_cmp_pair_reg.cmp_jump6_en = 1;
		...
		mdomain_cap_cap_pq_cmp_bit_llb_reg.regValue = IoReg_Read32(MDOMAIN_CAP_CAP_PQ_CMP_BIT_LLB_reg);
		mdomain_cap_cap_pq_cmp_bit_llb_reg.llb_x_blk_sta = llb_x_blk_sta;
		mdomain_cap_cap_pq_cmp_bit_llb_reg.llb_init = Llb_init;
		mdomain_cap_cap_pq_cmp_bit_llb_reg.llb_step = Llb_step;
		IoReg_Write32(MDOMAIN_CAP_CAP_PQ_CMP_BIT_LLB_reg, mdomain_cap_cap_pq_cmp_bit_llb_reg.regValue);

		//compression st setting
		mdomain_cap_cap_pq_cmp_st_reg.regValue = IoReg_Read32(MDOMAIN_CAP_CAP_PQ_CMP_ST_reg);
		//mdomain_cap_cap_pq_cmp_st_reg.cmp_readro_sel = //default;
		IoReg_Write32(MDOMAIN_CAP_CAP_PQ_CMP_ST_reg, mdomain_cap_cap_pq_cmp_st_reg.regValue);
		...
		if(dvrif_memory_get_compression_mode(SLR_MAIN_DISPLAY) == COMPRESSION_LINE_MODE){
			mdomain_cap_cap_pq_cmp_enable_reg.first_line_more_en = 0;
		}
		else{
			mdomain_cap_cap_pq_cmp_enable_reg.first_line_more_en = 1;
		}
		...
		mdomain_cap_cap_pq_cmp_enable_reg.overcurve_lossy_en = 1;
		//mdomain_cap_cap_pq_cmp_enable_reg.master_qp_en = 1;//fix work around need to check by dic
		...
		mdomain_cap_cap_pq_cmp_allocate_reg.dynamic_allocate_line = 2;
		...
		if(comp_ratio == COMPRESSION_10_BITS){// minmal compression bit setting
			...
			//mdomain_cap_cap_pq_cmp_blk0_add0_reg.dic_mode_tolerance = 0;//Mark2 removed
		}
		else{
		...
		//mdomain_cap_cap_pq_cmp_smooth_reg.dic_mode_entry_th = 15;//Mark2 removed
		...
		//mdomain_cap_cap_pq_cmp_guarantee_reg.dic_mode_qp_th_g = 1;//Mark2 removed
		//mdomain_cap_cap_pq_cmp_guarantee_reg.dic_mode_qp_th_rb = 1;//Mark2 removed
		...
		// Both line mode & frame mode need to set cmp_line_sum_bit;
		mdomain_disp_dispm_main_pq_decmp_pair_reg.decmp_line_sum_bit = cmp_line_sum_bit;
		...
		//mdomain_disp_dispm_main_pq_decmp_pair_reg.decmp_dic_mode_en = 1; //Mark2 removed
		mdomain_disp_dispm_main_pq_decmp_pair_reg.decmp_jump4_en = 1;
		mdomain_disp_dispm_main_pq_decmp_pair_reg.decmp_jump6_en = 1;
		...
		//mdomain_disp_dispm_main_pq_decmp_sat_en_reg.decmp_ctrl_para_1 = //reserved;
		//mdomain_disp_dispm_main_pq_decmp_sat_en_reg.decmp_ctrl_para_0 = //reserved;
		mdomain_disp_dispm_main_pq_decmp_sat_en_reg.saturation_type = 0;
...
void dvrif_memory_comp_setting_sub(UINT8 enable_compression, UINT16 comp_wid, UINT16 comp_len, UINT8 comp_ratio){
	...
	mdomain_cap_smooth_tog_ctrl_0_RBUS mdomain_cap_smooth_tog_ctrl_0_reg;
	mdomain_cap_caps_pq_cmp_bit_llb_RBUS mdomain_cap_caps_pq_cmp_bit_llb_reg;
	...
	if(enable_compression == TRUE){
		...
		*/
		...
		if(dvrif_memory_get_compression_mode(SLR_SUB_DISPLAY) == COMPRESSION_LINE_MODE){
			mdomain_cap_caps_pq_cmp_bit_reg.s_frame_limit_bit = comp_ratio*4;
			//mdomain_cap_caps_pq_cmp_bit_reg.s_line_limit_bit = comp_ratio;//Mark2 removed
		}
		else{//frame mode, line limit bit need to add margin to avoid the peak bandwidth
			mdomain_cap_caps_pq_cmp_bit_reg.s_frame_limit_bit = comp_ratio*4;
        ...
        // Both line mode & frame mode need to set cmp_line_sum_bit;
        mdomain_cap_caps_pq_cmp_pair_reg.s_cmp_line_sum_bit = s_cmp_line_sum_bit;
		if(dvrif_memory_get_compression_mode(SLR_SUB_DISPLAY) == COMPRESSION_LINE_MODE){
			//use line mode
		...
		if(Get_PANEL_VFLIP_ENABLE() || get_hdmi_vrr_4k60_mode() || dvrif_memory_get_compression_mode(SLR_SUB_DISPLAY) == COMPRESSION_LINE_MODE)
		...
		mdomain_cap_caps_pq_cmp_pair_reg.s_cmp_jump6_en = 1;
		...
		Llb_init = frame_limit_bit * 4 + llb_tolerance * 32;
		Llb_step = (Llb_init *16 - frame_limit_bit * 64) / cmp_width_div32;
		mdomain_cap_caps_pq_cmp_bit_llb_reg.regValue = IoReg_Read32(MDOMAIN_CAP_CAPS_PQ_CMP_BIT_LLB_reg);
		mdomain_cap_caps_pq_cmp_bit_llb_reg.s_llb_x_blk_sta = llb_x_blk_sta;
		mdomain_cap_caps_pq_cmp_bit_llb_reg.s_llb_init = Llb_init;
		mdomain_cap_caps_pq_cmp_bit_llb_reg.s_llb_step = Llb_step;
		IoReg_Write32(MDOMAIN_CAP_CAPS_PQ_CMP_BIT_LLB_reg, mdomain_cap_caps_pq_cmp_bit_llb_reg.regValue);

		//compression PQC constrain @Crixus 20170626
		mdomain_cap_caps_pq_cmp_enable_reg.regValue = IoReg_Read32(MDOMAIN_CAP_CAPS_PQ_CMP_ENABLE_reg);
		if(dvrif_memory_get_compression_mode(SLR_SUB_DISPLAY) == COMPRESSION_LINE_MODE){
			mdomain_cap_caps_pq_cmp_enable_reg.s_first_line_more_en = 0;
		}
		else{
			mdomain_cap_caps_pq_cmp_enable_reg.s_first_line_more_en = 1;
		}
		//RGB444 setting for PQC @Crixus 20170718
		if ((Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP) == FALSE)){
			if(pc_mode_run_422())
		...
		mdomain_cap_caps_pq_cmp_enable_reg.s_overcurve_lossy_en = 1;
		//mdomain_cap_caps_pq_cmp_enable_reg.s_master_qp_en = 1;//fix work around need to check by dic
		...
		mdomain_cap_caps_pq_cmp_allocate_reg.s_dynamic_allocate_line = 2;
		...
		mdomain_disp_dispm_sub_pq_decmp_pair_reg.s_decmp_data_color = 1;
        // Both line mode & frame mode need to set cmp_line_sum_bit;
        mdomain_disp_dispm_sub_pq_decmp_pair_reg.s_decmp_line_sum_bit = s_cmp_line_sum_bit;
		if(dvrif_memory_get_compression_mode(SLR_SUB_DISPLAY) == COMPRESSION_LINE_MODE){
			//default use line mode
		...
		mdomain_disp_dispm_sub_pq_decmp_pair_reg.s_decmp_jump6_en = 1;
		...
		mdomain_disp_dispm_sub_pq_decmp_sat_en_reg.s_saturation_type = 0;
...
//#ifdef VPQ_COMPILER_ERROR_ENABLE_MERLIN6
	...
	unsigned short cmp_line_sum_bit, sub_pixel_num;
	unsigned int cmp_line_sum_bit_pure_rgb, cmp_line_sum_bit_pure_a, cmp_line_sum_bit_32_dummy, cmp_line_sum_bit_256_dummy, cmp_line_sum_bit_real;
	unsigned char PQC_Mode;
	unsigned char PQC_Bit_Limit, PQC_Bit_Limit_reg, llb_init;
...
//	di_di_pq_cmp_qp_st_RBUS di_di_pq_cmp_qp_st_reg;
	di_di_pq_cmp_bit_llb_RBUS di_di_pq_cmp_bit_llb_reg;
...
//		di_di_dma_multi_req_num_reg.wdma_addr_rolling = 0;
	...
	PQC_Bit_Limit_reg = PQC_Bit_Limit<<2;
	/* set flag*/
	...
	di_di_pq_cmp_pair_reg.cmp_jump6_en = 1;	// mk2 add
	//di_di_pq_cmp_pair_reg.cmp_dic_mode_en = 1;	// mk2 remove
	...
	//Cal line sum bit
	sub_pixel_num = (cmp_alpha_en)? 4 : 3;
	cmp_line_sum_bit_pure_rgb = PQC_Width * PQC_Bit_Limit;
	cmp_line_sum_bit_pure_a = (cmp_alpha_en)?(PQC_Width*PQC_Bit_Limit):(0);
	cmp_line_sum_bit_32_dummy = (6+1) * 32 * sub_pixel_num;
	cmp_line_sum_bit_256_dummy = 0;
	cmp_line_sum_bit_real = cmp_line_sum_bit_pure_rgb + cmp_line_sum_bit_pure_a + cmp_line_sum_bit_32_dummy + cmp_line_sum_bit_256_dummy;
	cmp_line_sum_bit = (cmp_line_sum_bit_real+127)>>7;	// ceil((cmp_line_sum_bit_real)/128);
...
#else
	di_di_pq_cmp_bit_reg.frame_limit_bit = PQC_Bit_Limit_reg;
	//di_di_pq_cmp_bit_reg.line_limit_bit = PQC_Bit_Limit + 4;	/*recommand by skyred for mer3*/
	di_di_pq_cmp_bit_reg.block_limit_bit = 0x3f;	/*recommand by skyred for mer3*/
	...
	di_di_pq_decmp_pair_reg.decmp_jump6_en = 1;
	...
	di_di_pq_decmp_sat_en_reg.saturation_type = 0;
	...
	di_di_pq_cmp_bit_llb_reg.regValue = IoReg_Read32(DI_DI_PQ_CMP_BIT_LLB_reg);
	di_di_pq_cmp_bit_llb_reg.llb_x_blk_sta = 0; 	// mk2
	llb_init = (PQC_Bit_Limit_reg<<2) + 32; 	// mk2
	di_di_pq_cmp_bit_llb_reg.llb_init = llb_init;		// mk2
	di_di_pq_cmp_bit_llb_reg.llb_step = ((llb_init<<4) - (PQC_Bit_Limit_reg<<6))/(PQC_Width >> 5);		// mk2
	IoReg_Write32(DI_DI_PQ_CMP_BIT_LLB_reg, di_di_pq_cmp_bit_llb_reg.regValue);
...
//fixme:vip
	...
	*/
	...
	if (access == SLR_DI_5A_mode) {
		...
		data_r.water_level = 196; // new val for mk2
		data_r.length = 60; // new val for mk2
		
		data_w.water_level = 120;	 // new val for mk2
		data_w.length = 120;	 // new val for mk2
		
		info_r.water_level = 64;	 // new val for mk2
		info_r.length = 64;  // new val for mk2
		
		info_w.water_level = 64;	 // new val for mk2
		info_w.length = 64;  // new val for mk2
		...
		data_r.water_level = 392;	 // new val for mk2
		data_r.length = 120;	 // new val for mk2
		...
		data_r.water_level = 648;	 // new val for mk2
		data_r.length = 120;	 // new val for mk2
	...
	//rtd_pr_scaler_memory_debug("\n Info number = %d, length = %d, remain = %d, water_level = %d in setting DI Read FIFO", info_r.number, info_r.length, info_r.remain, info_r.water_level);
	//rtd_pr_scaler_memory_debug("\n Info number = %d, length = %d, remain = %d, water_level = %d in setting DI Write FIFO", info_w.number, info_w.length, info_w.remain, info_w.water_level);
	ddr_rdatdma_wtlennum.regValue	= 	IoReg_Read32(DI_DDR_RDATDMA_WTLENNUM_reg);
    ddr_wdatdma_wtlennum.regValue	=   IoReg_Read32(DI_DDR_WDATDMA_WTLENNUM_reg);
    ddr_rinfdma_wtlennum.regValue	=   IoReg_Read32(DI_DDR_RINFDMA_WTLENNUM_reg);
    ddr_winfdma_wtlennum.regValue	=   IoReg_Read32(DI_DDR_WINFDMA_WTLENNUM_reg);
    ddr_datdma_rm.regValue			=   IoReg_Read32(DI_DDR_DATDMA_RM_reg);
    ddr_infdma_rm.regValue			=   IoReg_Read32(DI_DDR_INFDMA_RM_reg);
	di_im_di_mpeg_sequence_app_reg.regValue = IoReg_Read32(DI_IM_DI_MPEG_SEQUENCE_APP_reg);

	ddr_rdatdma_wtlennum.dat_water_r	= data_r.water_level&0xFF;
	ddr_rdatdma_wtlennum.dat_len_r		= data_r.length;
	ddr_rdatdma_wtlennum.dat_num_r		= data_r.number&0xFFFF;
	if (isCompression_Mode != PQC_Disable) { // enable outstanding in pqc on
		ddr_rdatdma_wtlennum.dma_outstanding_en = 1;
		di_im_di_mpeg_sequence_app_reg.wdma_outstanding_en = 1;
	} else {
		ddr_rdatdma_wtlennum.dma_outstanding_en = 0;
		di_im_di_mpeg_sequence_app_reg.wdma_outstanding_en = 0;
	}
	ddr_wdatdma_wtlennum.dat_water_w	= data_w.water_level&0xFF;
	ddr_wdatdma_wtlennum.dat_len_w		= data_w.length;
	ddr_wdatdma_wtlennum.dat_num_w		= data_w.number&0xFFFF;
	...
	// for water/len MSB
	ddr_datdma_rm.dat_water_r_h = (data_r.water_level&0x300)>>8;
	ddr_datdma_rm.dat_num_r_h = (data_r.number&0x30000)>>16;
	ddr_datdma_rm.dat_water_w_h = (data_w.water_level&0x100)>>8;
	ddr_datdma_rm.dat_num_w_h = (data_w.number&0x30000)>>16;
...
//		value =(info->IPV_ACT_LEN_PRE *info->IPH_ACT_WID_PRE * bitNum)>>4;
	...
	rtd_pr_scaler_memory_info("DI_BND=%x,%x, DI add base=%x,%x,\n",
		IoReg_Read32(DI_DI1_WRRD_BoundaryAddr1_reg), IoReg_Read32(DI_DI1_WRRD_BoundaryAddr2_reg), data_addr_reg, info_addr_reg);
...
unsigned int drv_memory_MA_SNR_Config(unsigned char access ,unsigned int phyAddr, unsigned char DI_MEM_IDX)
{
...
#endif
	/* SNR part*/
	iedge_smooth_mvf_dma_dat_size_RBUS iedge_smooth_mvf_dma_dat_size_reg;
	...
	iedge_smooth_mvf_wr_ddr_dma_wtlennum_RBUS iedge_smooth_mvf_wr_ddr_dma_wtlennum_reg;
	iedge_smooth_mvf_wr_ddr_datdma_rm_RBUS iedge_smooth_mvf_wr_ddr_datdma_rm_reg;
	iedge_smooth_mvf_rd_ddr_dma_wtlennum_RBUS iedge_smooth_mvf_rd_ddr_dma_wtlennum_reg;
	iedge_smooth_mvf_rd_ddr_datdma_rm_RBUS iedge_smooth_mvf_rd_ddr_datdma_rm_reg;
	iedge_smooth_edsm_db_ctrl_RBUS iedge_smooth_edsm_db_ctrl_reg;

	unsigned int height = Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN);
	unsigned int width = Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID);
	//unsigned int pixel_total = height * width;
	unsigned int frame_size;	// 128b
	unsigned short water, len, remain, num;
	unsigned int mem[4], mem_aligned[4], mem_end;
	unsigned int buffer, buffer_aligned;
	unsigned short aligned_val;
	unsigned short i;
	unsigned char info_bit;
	unsigned short blk_h, blk_w;	
	char ip_status = 0; //0:disable, 1:masnr, 2:blk mnr
	unsigned int width_4N = (width%4)?((width/4+1)*4):(width);

	ip_status = drv_memory_MA_SNR_Enable_Check(access, phyAddr, DI_MEM_IDX, width, height);
	
	if (ip_status == MVF_Disable) {
		...
		*/
		...
		drv_memory_Set_MASNR_Mem_Ready_Flag(DI_MEM_IDX, MVF_Disable);
		return phyAddr;
	}
...
#endif
	/* SNR part*/
	iedge_smooth_mvf_dma_dat_size_reg.regValue = IoReg_Read32(IEDGE_SMOOTH_MVF_DMA_Dat_Size_reg);
	iedge_smooth_mvf_dma_dat_format_reg.regValue = IoReg_Read32(IEDGE_SMOOTH_MVF_DMA_Dat_format_reg);
	...
	iedge_smooth_mvf_wr_ddr_dma_wtlennum_reg.regValue = IoReg_Read32(IEDGE_SMOOTH_MVF_WR_DDR_DMA_WTLENNUM_reg);
	iedge_smooth_mvf_wr_ddr_datdma_rm_reg.regValue = IoReg_Read32(IEDGE_SMOOTH_MVF_WR_DDR_DATDMA_RM_reg);
	iedge_smooth_mvf_rd_ddr_dma_wtlennum_reg.regValue = IoReg_Read32(IEDGE_SMOOTH_MVF_RD_DDR_DMA_WTLENNUM_reg);
	iedge_smooth_mvf_rd_ddr_datdma_rm_reg.regValue = IoReg_Read32(IEDGE_SMOOTH_MVF_RD_DDR_DATDMA_RM_reg);

	if(ip_status==1)//masnr
	{
		//buffer = drv_memory_Get_MA_SNR_Info_Size_Static(access, width_4N, height, DI_MEM_IDX);
		buffer = drv_memory_Get_MA_SNR_Info_Size_Static(access, width, height, DI_MEM_IDX);
		info_bit = 2;	// 2 bit per pixel.
	}
	else if(ip_status==2)//blk mnr
	{
		if(Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_THRIP) == TRUE)//i mode
		blk_h = ((height<<1)%8)?(((height<<1)>>3)+1):((height<<1)>>3);
		else
		blk_h = (height%8)?((height>>3)+1):(height>>3);
		blk_w = (width%8)?((width>>3)+1):(width>>3);
		buffer = drv_memory_Get_blk_MNR_Info_Size_Static(blk_h * blk_w);
		info_bit = 27;	// 27 bit per block //mk2
	}
    ...
    drvif_color_iEdgeSmooth_Addr_Boundary_ini(mem_aligned[0], (unsigned int)dvr_memory_alignment((unsigned long)mem_end, buffer_aligned));
	...
	water = len; //mk2 
	frame_size = (width_4N * height * info_bit + 127)>>7;
	num = frame_size / len;	/* number*lenth+remain = pixel number*2/128, 128b per num */
	remain = frame_size - (len * num);
...
#endif
	iedge_smooth_mvf_wr_ddr_dma_wtlennum_reg.wr_dat_len = len;
	iedge_smooth_mvf_wr_ddr_dma_wtlennum_reg.wr_dat_water = water;
	iedge_smooth_mvf_wr_ddr_dma_wtlennum_reg.wr_dat_num = num;
	iedge_smooth_mvf_wr_ddr_datdma_rm_reg.wr_dat_remain = remain;
	//iedge_smooth_mvf_wr_ddr_datdma_rm_reg.wr_dma_remain_en = 1;
	iedge_smooth_mvf_rd_ddr_dma_wtlennum_reg.rd_dat_len = len;
	iedge_smooth_mvf_rd_ddr_dma_wtlennum_reg.rd_dat_water = 128-len;//water; // mk2
	iedge_smooth_mvf_rd_ddr_dma_wtlennum_reg.rd_dat_num = num;
	iedge_smooth_mvf_rd_ddr_datdma_rm_reg.rd_dat_remain = remain;
	iedge_smooth_mvf_rd_ddr_datdma_rm_reg.rd_dma_remain_en = 1;	
	...
	iedge_smooth_mvf_dma_dat_size_reg.mvf_height = height;
	iedge_smooth_mvf_dma_dat_size_reg.mvf_width = width;
...
#endif	
	/* SNR part*/
	IoReg_Write32(IEDGE_SMOOTH_MVF_DMA_Dat_Size_reg, iedge_smooth_mvf_dma_dat_size_reg.regValue);
	IoReg_Write32(IEDGE_SMOOTH_MVF_DMA_Dat_format_reg, iedge_smooth_mvf_dma_dat_format_reg.regValue);
	...
	IoReg_Write32(IEDGE_SMOOTH_MVF_WR_DDR_DMA_WTLENNUM_reg, iedge_smooth_mvf_wr_ddr_dma_wtlennum_reg.regValue);
	IoReg_Write32(IEDGE_SMOOTH_MVF_WR_DDR_DATDMA_RM_reg, iedge_smooth_mvf_wr_ddr_datdma_rm_reg.regValue);
	IoReg_Write32(IEDGE_SMOOTH_MVF_RD_DDR_DMA_WTLENNUM_reg, iedge_smooth_mvf_rd_ddr_dma_wtlennum_reg.regValue);
	IoReg_Write32(IEDGE_SMOOTH_MVF_RD_DDR_DATDMA_RM_reg, iedge_smooth_mvf_rd_ddr_datdma_rm_reg.regValue);
...
char drv_memory_MA_SNR_Enable_Check(unsigned char access ,unsigned int phyAddr, unsigned char DI_MEM_IDX, unsigned int DI_Width, unsigned int DI_Height)
{
	...
	if (RPC_system_info_structure_table == NULL) {
			rtd_pr_scaler_memory_emerg("drv_memory_MA_SNR_Enable_Check Fail, RPC_system_info_structure_table == NULL\n");
			return 0;
	}
	...
	MVF_Force_Mode = RPC_system_info_structure_table->MASNR_Ctrl.MVF_Force_Mode_;
	/* diable MA-SNR */
	/*1.DI is disable, */
	...
	if (MVF_Force_Mode == 0) {
		if ((decide_by_AP == 0) || ((vsc_src == VSC_INPUTSRC_VDEC) && (adaptvie_stream_flag >= ADAPTIVE_STREAM_ON)) || ((DISP_THRIP  == FALSE) && (DISP_RTNR == FALSE))) {
			ret = MVF_Disable;
		} else if( (height<2160) && (width<3840) ){ //<=4k, MASNR. (h5x masnr only support <=4k)
			ret = MVF_MASNR;
		} else {
			ret = MVF_MNR;
		}
	} else {	// for tool debug
		if (MVF_Force_Mode == MVF_MASNR)
			ret = MVF_MASNR;
		else if (MVF_Force_Mode == MVF_MNR)
			ret = MVF_MNR;
		else
			ret = MVF_Disable;
	}

	rtd_pr_scaler_memory_info("MA-SNR, decide_by_AP=%d, vsc_src=%d, adaptvie_stream_flag=%d, DISP_THRIP=%d, DISP_RTNR=%d,MVF_Force_Mode=%d,\n",
		decide_by_AP, vsc_src, adaptvie_stream_flag, DISP_THRIP, DISP_RTNR, MVF_Force_Mode);
...
unsigned int  drv_memory_Get_MA_SNR_Info_Size_Static(unsigned char access, unsigned int I_Width, unsigned int I_Height, unsigned char DI_MEM_IDX)
{
	...
	data_bit = 2;
	/*if (access == SLR_DI_progress_mode) {
		width = I_Width>>1;		//p mode=>delay=0=>max at 5 line mode memory size,
		height = I_Height;
	} else*/ {
		width = I_Width;
		height = I_Height;
...
unsigned int  drv_memory_Get_blk_MNR_Info_Size_Static(unsigned int blk_num)
{
	unsigned int buffer_size;
	unsigned int offset = 1024;	/* avoid trash other memory*/
	...
	data_bit = 26;
	buffer_size = (blk_num * data_bit)>>3;
	buffer_size = buffer_size + offset;

	rtd_pr_scaler_memory_info("blk_MNR_size_cal, buffer_size=%d,\n", buffer_size);
		...
		do{
			...
			pre_cap_idx = MDOMAIN_CAP_DDR_In2Status_get_in2_block_select(IoReg_Read32(MDOMAIN_CAP_DDR_In2Status_reg));
            		...
            		if(MDOMAIN_CAP_DDR_In2Status_get_in2_block_select(IoReg_Read32(MDOMAIN_CAP_DDR_In2Status_reg)) == pre_cap_idx)
            		{
                		...
                		if(count == 0)
                    		rtd_pr_scaler_memory_err("M domain sub timeout2 !\n");
            		}
			wait_frame--;
		}while(wait_frame != 0);
	}
...
void drv_memory_MCapForceAllRst(unsigned char display, unsigned char a_bEnable)
{
	mdomain_cap_ddr_outstanding_ctrl_main_cap_RBUS mdomain_cap_ddr_outstanding_ctrl_main_cap_reg;
	mdomain_cap_ddr_outstanding_ctrl_sub_cap_RBUS mdomain_cap_ddr_outstanding_ctrl_sub_cap_reg;

	rtd_pr_scaler_memory_info("drv_memory_MCapForceAllRst, display=%d, a_bEnable=%d\n", display, a_bEnable);
	if (SLR_MAIN_DISPLAY == display)
	{
		mdomain_cap_ddr_outstanding_ctrl_main_cap_reg.regValue = IoReg_Read32(MDOMAIN_CAP_DDR_Outstanding_Ctrl_Main_Cap_reg);
		mdomain_cap_ddr_outstanding_ctrl_main_cap_reg.in1_force_all_rst = a_bEnable;
		IoReg_Write32(MDOMAIN_CAP_DDR_Outstanding_Ctrl_Main_Cap_reg, mdomain_cap_ddr_outstanding_ctrl_main_cap_reg.regValue);
	}
	else
	{
		mdomain_cap_ddr_outstanding_ctrl_sub_cap_reg.regValue = IoReg_Read32(MDOMAIN_CAP_DDR_Outstanding_Ctrl_Sub_Cap_reg);
		mdomain_cap_ddr_outstanding_ctrl_sub_cap_reg.in2_force_all_rst = a_bEnable;
...
unsigned char drv_memory_get_low_delay_game_mode_dynamic(void)
{
	...
	low_delay_game = (drv_memory_get_game_mode_dynamic() || ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())));
...
#define IDOMAIN_IP_DELAY_MARGIN 20	//I-domain max delay 20 line
#define MDOMAIN_COMPRESSION_MARGIN 1	//DMA latency
#define MDOMAIN_SCALEDOWN_MARGIN 5	//Scale down and then scale up case need to add extra lines for iv2dv
...
unsigned int drv_GameMode_iv2dv_delay_compress_margin(unsigned int iv2dv_ori){
	...
	active_size = ich1_ibuff_ctrl_BUFF_CTRL.ibuf_h_size;//DIC suggest use m-domain h-active to calculate
	//margin = (fifo depth * burst length) / compress bit / den sizes
	...
	if((m_dispwin.src_height < _DISP_LEN) && (is_caveout_memory_buffer_source(SLR_MAIN_DISPLAY)))//when scale down to 2k memory, need to add extra delay
		iv2dv_ori += MDOMAIN_SCALEDOWN_MARGIN;
...
//	UINT32 orbit_front;
	...
	if(scaler_input_hdmi)
	{
		if((hdmi_htotal != 0) && (hdmi_vtotal != 0) && (hdmi_vfreq != 0)){
	...
	if(!VODMA_VODMA_CLKGEN_get_vodma_clk_1x_2p_sel(IoReg_Read32(VODMA_VODMA_CLKGEN_reg)))
		voClock *= 2;

	dHtotal = Get_DISP_HORIZONTAL_TOTAL();
	dCLK = Get_dispd_stage1_f2p_data_rate();
	//den shift with time borrow, we consider worse case.
	dDen_start =  Get_DISP_DEN_STA_VPOS()-scaler_disp_dtg_get_time_borrow_value();
	m_dispwin = Scaler_DispWindowGet();
...
#if 0
	if((Get_DISPLAY_REFRESH_RATE() >= 120) /*&& (PPOVERLAY_uzudtg_control1_get_uzu_4k_120_mode(rtd_inl(PPOVERLAY_uzudtg_control1_reg))) mark2 removed*/){
		// 120hz panel run 2pixel mode, dCLK need multiple 2 for calculate.
		dCLK *= 2;
	}
#endif

	//DCLK use uzudtg dclk so need consider frac_a/b
	frac_a = (SYS_REG_SYS_DISPCLKSEL_get_dclk_fract_a(IoReg_Read32(SYS_REG_SYS_DISPCLKSEL_reg))+1);
	frac_b = (SYS_REG_SYS_DISPCLKSEL_get_dclk_fract_b(IoReg_Read32(SYS_REG_SYS_DISPCLKSEL_reg))+1);
	dCLK = (dCLK/frac_b)*frac_a;
	rtd_pr_scaler_memory_emerg("dCLK = %u, frac_a = %d, frac_b = %d\n", dCLK, frac_a, frac_b);
	...
	*/
	//rtd_pr_scaler_memory_emerg("[ben]snrVstart = %d, voHtotal = %d, voVactive = %d, voVstart = %d ,VoClock = %u\n", snrVstart, voHtotal, voVactive, voVstart, voClock);
	//rtd_pr_scaler_memory_emerg("[ben]dHtotal = %d, dCLK = %u, mdisp_preRead = %d, IDOMAIN_IP_DELAY_MARGIN = %d\n", dHtotal, dCLK,
...
#if 0 //when we calculate iv2dv delay we have use full vact after sdnr to calculate, the final result doesn't need to multiple 2 again
	if(Scaler_DispGetStatus(SLR_MAIN_DISPLAY,SLR_DISP_INTERLACE))
...
void drv_GameMode_iv2dv_delay(unsigned char enable){
	...
	if(enable == _ENABLE){
		...
		//iv2dv delay
		id2dv_delay = DEFAULT_FRC_IV2DV_DELAY;//default setting
	}
...
#ifdef CONFIG_SCALER_RUN_ON_ZEBU // skip iv2dv delay apply done status
	...
	if(drv_memory_get_game_mode_iv2dv_slow_enable() == TRUE){
		...
		if(id2dv_delay != DEFAULT_FRC_IV2DV_DELAY){
...
#if 1//#ifdef CONFIG_REALTEK_2K_MODEL_ENABLED
/*======================M-domain Multi-Buffer Controlling=====================*/
//static UINT8 Mdomain_multibuffer_enable = false;
//static UINT8 Mdomain_capture_multibuffer_enable = false;
//static UINT8 Mdomain_multibuffer_number = 0;
...
void drv_memory_cap_limit_boundary_setting(unsigned char display, unsigned int buffer_size){
	...
	if(display == SLR_MAIN_DISPLAY){
		...
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK0_reg, (MemTag[MEMIDX_MAIN].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0x7ffffff0);//need cut burstLen
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK1_reg, (MemTag[MEMIDX_MAIN_SEC].StartAddr & 0x7ffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK1_reg, (MemTag[MEMIDX_MAIN_SEC].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0x7ffffff0);//need cut burstLen
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK2_reg, (MemTag[MEMIDX_MAIN_THIRD].StartAddr & 0x7ffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK2_reg, (MemTag[MEMIDX_MAIN_THIRD].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0x7ffffff0);//need cut burstLen
	}
	else if(display == SLR_SUB_DISPLAY){
		...
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK0_reg, (MemTag[MEMIDX_SUB].StartAddr & 0x7ffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK0_reg, (MemTag[MEMIDX_SUB].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0x7ffffff0);//need cut burstLen
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK1_reg, (MemTag[MEMIDX_SUB_SEC].StartAddr & 0x7ffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK1_reg, (MemTag[MEMIDX_SUB_SEC].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0x7ffffff0);//need cut burstLen
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK2_reg, (MemTag[MEMIDX_SUB_THIRD].StartAddr & 0x7ffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK2_reg, (MemTag[MEMIDX_SUB_THIRD].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0x7ffffff0);//need cut burstLen
	}
}
...
void drv_memory_disp_limit_boundary_setting(unsigned char display){
	...
	if(display == SLR_MAIN_DISPLAY){
		...
		if(get_hdmi_vrr_4k60_mode()){
			//4K60Hz VRR flow
			IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK0_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK0_reg) & 0x7ffffff0));
			IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK0_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK0_reg) & 0x7ffffff0));
			IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK1_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK1_reg) & 0x7ffffff0));
			IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK1_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK1_reg) & 0x7ffffff0));
			IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK2_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK2_reg) & 0x7ffffff0));
			IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK2_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK2_reg) & 0x7ffffff0));
		}
#if 0
...
{//return true: use
    if((get_scaler_run_hdmi_hfr_mode()== HDMI_4K120_2_1) && (get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1))
...
unsigned int drv_Calculate_m_pre_read_value(void)
{
	...
	rtd_pr_scaler_memory_notice("[drv_Calculate_m_pre_read_value]m_pre_read=%d!!\n",m_pre_read);
