...
unsigned char set_di_doublebuf_write_TV006(unsigned char diflag,unsigned char _2d_flag)
{
	...
	rtd_pr_vbe_info("DI_Enable in lv2=%d, diflag=%d, h_flip=%d,\n", DI_IP_En_Lv2, diflag, Scaler_color_access_RTNR_H_Rotate_Flag(0, 0));
...
//					if(Scaler_DispGetInputInfo(SLR_INPUT_CAP_WID) >= 960)
		...
		if(Scaler_DispGetInputInfo(SLR_INPUT_DI_WID)>0x7FF){
			di_im_di_active_window_msb_control_reg.hsize_msb= (Scaler_DispGetInputInfo(SLR_INPUT_DI_WID)>>11);
		}else{
			di_im_di_active_window_msb_control_reg.hsize_msb= 0;
		}

		if(Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN)>0x3FF){
			di_im_di_active_window_msb_control_reg.vsize_msb= (Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN)>>10);
		}else{
			di_im_di_active_window_msb_control_reg.vsize_msb= 0;
		}

		if((htotal - Scaler_DispGetInputInfo(SLR_INPUT_DI_WID))>0x3FF){
			di_im_di_active_window_msb_control_reg.hblksize_msb = ((htotal - Scaler_DispGetInputInfo(SLR_INPUT_DI_WID))>>10);
		}else{
			di_im_di_active_window_msb_control_reg.hblksize_msb = 0;
		}
		IoReg_Write32(DI_IM_DI_ACTIVE_WINDOW_MSB_CONTROL_reg, di_im_di_active_window_msb_control_reg.regValue);
...
//		scalerip_modify_rdstart();
		...
		if (Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY) {
			Scaler_color_set_RTNR_H_Rotate(get_rotate_hflip_flag(SLR_MAIN_DISPLAY));
			rtnr_new_Ctrl.regValue = IoReg_Read32(DI_IM_DI_RTNR_NEW_CONTROL_reg);
			if(Scaler_DispGetInputInfo(SLR_INPUT_DI_WID)>3840 && (Scaler_color_access_RTNR_H_Rotate_Flag(0, 0) == 0)){
				rtnr_new_Ctrl.rtnr_8k_en = 0x1; 	//8k support
			}else{
				rtnr_new_Ctrl.rtnr_8k_en = 0x0;
			}
