...
#define SB2_HD_SEM_NEW_5_reg 0xB801A634
#define Get_HD_SEM_NEW_5() while(!IoReg_Read32(SB2_HD_SEM_NEW_5_reg)){rtd_pr_adc_info("#@# %s wait SB2_HD_SEM_NEW_5_reg ready\n", moduleName); msleep(20);}
...
#define _Y75UV75_COMPONENT_Y_TARGET (720)//lg target value is 720
#define _Y75UV75_COMPONENT_U_TARGET (772)//lg target value is 772
#define _Y75UV75_COMPONENT_V_TARGET (816)//lg target value is 816
...
//VGA
#define _AUTO_VGA_DEFAULT_GAIN_R		(276)
#define _AUTO_VGA_DEFAULT_GAIN_G		(257)
#define _AUTO_VGA_DEFAULT_GAIN_B		(272)
...
//YPP
#define _AUTO_YPP_DEFAULT_GAIN_PR		(275)
#define _AUTO_YPP_DEFAULT_GAIN_Y		(457)
#define _AUTO_YPP_DEFAULT_GAIN_PB		(265)
...
//VGA
#define _AUTO_SELF_VGA_LEVEL_TARGET_R	(432)
#define _AUTO_SELF_VGA_LEVEL_TARGET_G	(443)
#define _AUTO_SELF_VGA_LEVEL_TARGET_B	(432)

//YPP
#define KXL_AUTO_SELF_YPP_LEVEL_TARGET_PR	(480)//with 75% color bar
#define KXL_AUTO_SELF_YPP_LEVEL_TARGET_Y	(384)//with 75% color bar
#define KXL_AUTO_SELF_YPP_LEVEL_TARGET_PB	(484)//with 75% color bar

//YPP
#define KXLP_AUTO_SELF_YPP_LEVEL_TARGET_PR	(480)//with 75% color bar
#define KXLP_AUTO_SELF_YPP_LEVEL_TARGET_Y	(384)//with 75% color bar
#define KXLP_AUTO_SELF_YPP_LEVEL_TARGET_PB	(484)//with 75% color bar
...
void Reset_YPP_Auto_Phase_SmartFit_Setting(void)
{
    ...
    auto_phase_ctrl0_reg.para_ap_en = 0;//Reset autophase. rzhen@2017-01-06
...
void EnableIPTGInSXGA(void)
{
	// Set APLL for SXGA
#if 1
	IoReg_Write32(DDS_PLL_PLL_SET_reg, 0x03030115);	// M=24,N=3,K=0
	IoReg_Write32(DDS_PLL_PLLDIV_reg, 0x00000697);	// Divider=1688
	IoReg_Write32(ADC_SMARTFIT_ADC_PLLPHASE_CTRL_reg, 0x00430000);	// ADC clk=(1/2)*VCO clk
	IoReg_Write32(DDS_PLL_PLL_PHASE_INTERPOLATION_reg, 0x00000220);	// PLL phase interpolation control load
	IoReg_Write32(DDS_PLL_P_CODE_mapping_methods_reg, 0x00000030);	// G value=16, P code over flow 2 bit
	IoReg_Write32(DDS_PLL_PE_tracking_method_reg, 0x00000002);	// Adaptive Tracking (Default value)
	IoReg_Write32(DDS_PLL_DDS_MIX_W1_reg, 0x3026b433);	// P code max=1268252 and new mode disable
	IoReg_Write32(DDS_PLL_DDS_MIX_W2_reg, 0x000002a1);	// Delay chain select , Free Run enable
	IoReg_Write32(DDS_PLL_PLL_DIV_CTRL_reg, 0x00000008);	// DDS tracking HS positive edge (Default value)
	IoReg_Write32(DDS_PLL_I_CODE_reg, 0x00000508);	// I code=1288
	IoReg_Write32(DDS_PLL_P_CODE_reg, 0x000000f7);	// P code=126825, over flow 2 bits
	IoReg_Write32(DDS_PLL_PFD_CALIBRATED_RESULTS_reg, 0x00000000);	// Calibration disable (Default value)
	IoReg_Write32(DDS_PLL_PE_MEARSURE_reg, 0x00000000);	// PE measure disable (Default value)
	IoReg_Write32(DDS_PLL_PE_MAX_MEASURE_reg, 0x00000000);	// PE Max measure disable (Default value)
	IoReg_Write32(DDS_PLL_FAST_PLL_CTRL_reg, 0x00000040);	// enable double buffer load
	IoReg_Write32(DDS_PLL_PLL_CTRL_reg, 0x00050608);	// M=24,N=3,K=0
	...
	IoReg_Write32(ADC_ADC_POWER_reg, 0x000001ff);

	IoReg_Write32(PATTERNGEN_PTG_TOTALHV_reg, 0x0698042A);   // Total pixel = 1688*1066
	IoReg_Write32(PATTERNGEN_PTG_ACT_H_Start_Width_reg, 0x01680500);   // Horizontal active start = 204, active width = 1280
	IoReg_Write32(PATTERNGEN_PTG_ACT_V_Start_Length_reg, 0x00290400);   // Vertical active start = 21, active width = 1024
	IoReg_Write32(PATTERNGEN_PTG_TG_reg, 0x00000001);   // Enable timing generator
	IoReg_Write32(PATTERNGEN_PTG_L_PAT_HSTA_Width_reg, 0x01680500);   // Pattern Gen display Hstart & Hactive
	IoReg_Write32(PATTERNGEN_PTG_L_PAT_VSTA_Length_reg, 0x00290400);   // Pattern Gen display Vstart & Vactive
	...
	IoReg_Write32(VGIP_VGIP_CHN1_CTRL_reg, 0x90000001);	//001:ADC/YPbPr (incluing Pattern Generator),analog mode
	//rtd_outl(0xb8022210, 0x10000001);	//001:ADC/YPbPr (incluing Pattern Generator),digital mode

	IoReg_Write32(VGIP_VGIP_CHN1_ACT_HSTA_Width_reg, 0x01680500);	//27:16==>Ch1_ih_act_sta/11:0==>Ch1_ih_act_wid
	IoReg_Write32(VGIP_VGIP_CHN1_ACT_VSTA_Length_reg, 0x00290400);	//27:16==>Ch1_iv_act_sta/11:0==>Ch1_iv_act_len
	IoReg_Write32(VGIP_VGIP_CHN1_DELAY_reg, 0x80000000);	//27:16==>Ch1_IHS_DLY/11:0==>Ch1_IVS_DLY      CH1_hs_width	31:30 need to be 10
	IoReg_Write32(VGIP_Data_Path_Select_reg, 0x00000008);	//from vgip/ch1/DI bypass
...
static void  drvif_set_smartfit_boundary(void)
{
	...
	rtd_pr_adc_notice("[ADC]: HS=0x%3x=%4d HE=0x%3x=%4d VS=0x%3x=%4d VE=0x%3x=%4d\n",smartfit_auto_h_boundary_reg.hb_sta,smartfit_auto_h_boundary_reg.hb_sta,smartfit_auto_h_boundary_reg.hb_end,smartfit_auto_h_boundary_reg.hb_end,smartfit_auto_v_boundary_reg.vb_sta,smartfit_auto_v_boundary_reg.vb_sta,smartfit_auto_v_boundary_reg.vb_end,smartfit_auto_v_boundary_reg.vb_end);
}
...
static void drvif_get_h_boundary(unsigned char display)
{
	...
	if(get_ADC_Input_Source() == _SRC_VGA){
		...
		ulparam1 = (unsigned long long)(CONFIG_CPU_XTAL_FREQ/1000000);
		rtd_pr_adc_emerg("#####[%s(%d)] ulparam1 = %lld\n", __FUNCTION__, __LINE__, ulparam1);

		ulparam2 = (unsigned long long)(p_dispinfo->IVFreq * p_dispinfo->IHTotal);
		ulparam2*= (unsigned long long)(p_dispinfo->IVTotal);
		do_div(ulparam2, 1000000); // clock unit MHz
		rtd_pr_adc_emerg("#####[%s(%d)] ulparam2 = %lld\n", __FUNCTION__, __LINE__, ulparam2);

		if(get_adc_hsync_polarity_indicator()){
			IHSyncPulseCount = p_timinginfo->IHSyncPulseCount;
		}else{
			IHSyncPulseCount = p_timinginfo->IHCount - p_timinginfo->IHSyncPulseCount;
		}

		IHSyncPulseCount = IHSyncPulseCount * (unsigned short)ulparam2 / (unsigned short)ulparam1 / 10;
		rtd_pr_adc_emerg("#####[%s(%d)] IHSyncPulseCount = %d\n", __FUNCTION__, __LINE__, IHSyncPulseCount);

	}else{

		IHSyncPulseCount = p_dispinfo->IHSyncPulseCount;
	}

	stAUTO_PARA.usAutoHStart   = (unsigned int)stAUTO_PARA.usAutoHEnd * IHSyncPulseCount / p_dispinfo->Hsync + 0x10;   // Clock number in HSYNC pulse
...
unsigned char drv_auto_pre_check(unsigned char *select_result)
{
		...
		Result = drvif_measure_v_position(SLR_MAIN_DISPLAY, 0x40, 0x40, 0x40, &ver_start, &ver_end,color_path);

		if ((ERROR_SUCCEED != Result)||((ver_start + p_dispinfo->IPV_ACT_LEN) > p_dispinfo->IVTotal))
		{
			rtd_pr_adc_notice("#####[%s(%d)]ERROR_ABORT Result=0x%02x, ver_start=%d,ver_end=%d#####\n", __func__, __LINE__, Result, ver_start,ver_end);
			rtd_pr_adc_notice("#####[%s(%d)]ERROR_ABORT IPV_ACT_LEN=%d\n!!! #####\n", __func__, __LINE__, p_dispinfo->IPV_ACT_LEN);
			rtd_pr_adc_notice("#####[%s(%d)]ERROR_ABORT IVTotal=%d\n!!! #####\n", __func__, __LINE__, p_dispinfo->IVTotal);
			return ERROR_ABORT;
		}

		Result = drvif_measure_h_position(SLR_MAIN_DISPLAY, 0x40, &hor_start, &hor_end,color_path);

		if ((ERROR_SUCCEED != Result)||((hor_start + p_dispinfo->IPH_ACT_WID) > p_dispinfo->IHTotal))
		{
			rtd_pr_adc_notice("#####[%s(%d)]ERROR_ABORT Result=0x%02x, hor_start=%d,hor_end=%d#####\n", __func__, __LINE__, Result, hor_start,hor_end);
			rtd_pr_adc_notice("#####[%s(%d)]ERROR_ABORT IPH_ACT_WID=%d\n!!! #####\n", __func__, __LINE__, p_dispinfo->IPH_ACT_WID);
			rtd_pr_adc_notice("#####[%s(%d)]ERROR_ABORT IHTotal=%d\n!!! #####\n", __func__, __LINE__, p_dispinfo->IHTotal);
			return ERROR_ABORT;
		}
...
MEASURE_V_POS:
	//msleep(150);//wait for mspg content stable, ensure smartfit can measure timing
	smartfit_auto_adj_reg.regValue = IoReg_Read32(SMARTFIT_AUTO_ADJ_reg);
	...
	{
		rtd_pr_adc_notice("#####[%s(%d)]Error 1 result = 0x%02x#####\n", __func__, __LINE__, result);
		return result;
	}
	...
	*v_end = smartfit_auto_result_vsta_end_reg.vx_end;
	rtd_pr_adc_notice("#####[%s(%d)]*v_start=%d #####\n", __func__, __LINE__, *v_start);
	rtd_pr_adc_notice("#####[%s(%d)]*v_end=%d #####\n", __func__, __LINE__, *v_end);

	if((*v_end== 0x0000)||(*v_end<=*v_start)){
		rtd_pr_adc_notice("#####[%s(%d)]Error 2 usVer_End == 0x0000#####\n", __func__, __LINE__);
		rtd_pr_adc_notice("#####[%s(%d)]0xb8022004=0x%08x, 0xb8022008=0x%08x\n", __func__, __LINE__, IoReg_Read32(SMARTFIT_AUTO_H_BOUNDARY_reg),IoReg_Read32(SMARTFIT_AUTO_V_BOUNDARY_reg));
		rtd_pr_adc_notice("#####[%s(%d)]0xb802200c=0x%08x, 0xb8022010=0x%08x\n", __func__, __LINE__, IoReg_Read32(SMARTFIT_AUTO_R_G_B_MARGIN_reg),IoReg_Read32(SMARTFIT_AUTO_ADJ_reg));
		rtd_pr_adc_notice("#####[%s(%d)]0xb8022014=0x%08x, 0xb8022018=0x%08x\n", __func__, __LINE__, IoReg_Read32(SMARTFIT_AUTO_RESULT_VSTA_END_reg),IoReg_Read32(SMARTFIT_AUTO_RESULT_HSTA_END_reg));
		if(vcnt++ > 5){//timeout
			rtd_pr_adc_notice("#####[%s(%d)]ABORT due to vcnt=%d\n", __func__, __LINE__, vcnt);
			return  ERROR_ABORT;
		}
...
MEASURE_H_POS:
	...
	*h_end = smartfit_auto_result_hsta_end_reg.hx_end;
	...
	//rtd_pr_adc_notice("#####[%s(%d)]hx_sta=%d, hx_end=%d\n", __func__, __LINE__, smartfit_auto_result_hsta_end_reg.hx_sta, smartfit_auto_result_hsta_end_reg.hx_end);
	rtd_pr_adc_notice("#####[%s(%d)]*h_start=%d, *h_end=%d\n", __func__, __LINE__, *h_start, *h_end);

	if((*h_end == 0x0000)||(*h_end <= *h_start)){
		rtd_pr_adc_notice("#####[%s(%d)]Error 2 h_end == 0x0000#####\n", __func__, __LINE__);

		if(hcnt++ > 5){//timeout
			rtd_pr_adc_notice("#####[%s(%d)]ABORT due to hcnt=%d\n", __func__, __LINE__, hcnt);
			return  ERROR_ABORT;
		}
	...
	unsigned char inc ,unsigned short *pGain)
{
	...
	usCurGain = *pGain;
	rtd_pr_adc_debug("color=%d, inc=%d, OriGain=%d\n", color, inc, *pGain);
	switch(color)
	{
		case SELECT_RED:
...
static void drvif_self_awb_changegain(UINT8 color, UINT16 delta, UINT8 inc, int src, ADCGainOffset * ptADC_info)
{
	...
	if(delta >= 2)
	{
	    if (color & SELECT_RED) {//forster modified 050728
			rtd_pr_adc_notice("drvif_self_awb_changegain RED delta=%d, inc=%d,  ptADCGainOffset->Gain_R=%d\n", delta, inc, ptADC_info->Gain_R);
	    ...
	    if (color & SELECT_GREEN) {//forster modified 050728
			rtd_pr_adc_notice("drvif_self_awb_changegain GREEN delta=%d, inc=%d, ptADCGainOffset->Gain_G=%d\n", delta, inc,  ptADC_info->Gain_G);
	    ...
	    if (color & SELECT_BLUE) {//forster modified 050728
			rtd_pr_adc_notice("drvif_self_awb_changegain BLUE delta=%d, inc=%d, ptADCGainOffset->Gain_B=%d\n", delta, inc,  ptADC_info->Gain_B);
...
void drvif_SelfAWB_gettargetvalue(UINT16* target, UINT8 color, UINT8 src)
{
	...
	switch(src) 	// weihao 960308
	{
		case _SRC_VGA: 	// weihao 960308
			...
			#endif
			//rtd_pr_adc_info("@@@@@@@@max_value=0x%x=%d\n",*max_value,*max_value);
			rtd_pr_adc_debug("target=0x%x=%d\n\n",*target,*target);
			break;

		case _SRC_YPBPR: 	// weihao 960308
			platform = get_platform();//Get platform
			rtd_pr_adc_info("###[%s(line: %d)]platform=%d###\n\n", __func__, __LINE__, platform);
			...
			{
				*target = target_y;
				rtd_pr_adc_debug("Y target=0x%x=%d\n\n",*target,*target);
			}
			else if(color == SELECT_RED)
			{
				*target =  target_pr;
				rtd_pr_adc_debug("Pr target=0x%x=%d\n\n",*target,*target);
			}
			else
			{
				*target = target_pb;
				rtd_pr_adc_debug("Pb target=0x%x=%d\n\n",*target,*target);
			}
			#if 0	//compile error
			abl_Reserved_reg.regValue = IoReg_Read32(ABL_Reserved_reg);//leochen
...
UINT8 drvif_SelfAWB_measurecolor(UINT8 color, UINT8 margin, UINT16* voltagevalue)
{
	...
	*voltagevalue = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_RESULT_PHASE_L_reg) & 0x03ff;
	...
	rtd_pr_adc_notice("#####[%s(%d)]SelfAWB measurecolor Measure Color=%x Margin = %d\n", __FUNCTION__, __LINE__, color, *voltagevalue);
	return ERROR_SUCCEED;
...
unsigned char drvif_self_awb_calculate_target_diff(int src , UINT8 ucColor)
{
	...
	if(src == _SRC_VGA){
		if(drvif_auto_measurecolor(SLR_MAIN_DISPLAY, ucColor, _COLOR_MAX, &measureresult) != ERROR_SUCCEED){
			rtd_pr_adc_notice("#####[%s(%d)][Get Target Diff] Error	Gain ABORT 0 %c", __FUNCTION__, __LINE__, '!');
			return FALSE;
		}
	}else{
	...
	//rtd_pr_adc_info("[Get Target Diff] HIGH=%3d ",temp_high);
	...
	if(src == _SRC_VGA){
		if(drvif_auto_measurecolor(SLR_MAIN_DISPLAY, ucColor, _COLOR_MAX, &measureresult) != ERROR_SUCCEED){
			rtd_pr_adc_notice("#####[%s(%d)][Get Target Diff] Error Gain ABORT 1 %c", __FUNCTION__, __LINE__, '!');
			return FALSE;
		}
	}else{
		if(drvif_SelfAWB_measurecolor(ucColor, _COLOR_MAX, &measureresult) != ERROR_SUCCEED){
	...
	//rtd_pr_adc_info("LOW=%3d Diff=%3d\n",temp_low,temp_high-temp_low);

	//rtd_pr_adc_info("color=%x temp_high=%x temp_low=%x\n", ucColor,temp_high,temp_low);
	...
	case 1:
		if(src == _SRC_VGA)
			rtd_pr_adc_notice("#####[%s(%d)][Get Target Diff] Red HIGH=%3d LOW=%3d Diff=0x%3x=%3d ", __FUNCTION__, __LINE__, temp_high,temp_low,code_diff,code_diff);
		else
			rtd_pr_adc_notice("#####[%s(%d)][Get Target Diff] Pr HIGH=%3d LOW=%3d Diff=0x%3x=%3d ", __FUNCTION__, __LINE__, temp_high,temp_low,code_diff,code_diff);
	...
	case 2:
		if(src == _SRC_VGA)
			rtd_pr_adc_notice("#####[%s(%d)][Get Target Diff] Grn HIGH=%3d LOW=%3d Diff=0x%3x=%3d ", __FUNCTION__, __LINE__, temp_high,temp_low,code_diff,code_diff);
		else
			rtd_pr_adc_notice("#####[%s(%d)][Get Target Diff] Y HIGH=%3d LOW=%3d Diff=0x%3x=%3d ", __FUNCTION__, __LINE__, temp_high,temp_low,code_diff,code_diff);
	...
	case 4:
		if(src == _SRC_VGA)
			rtd_pr_adc_notice("#####[%s(%d)][Get Target Diff] Blu HIGH=%3d LOW=%3d Diff=0x%3x=%3d ", __FUNCTION__, __LINE__, temp_high,temp_low,code_diff,code_diff);
		else
			rtd_pr_adc_notice("#####[%s(%d)][Get Target Diff] Pb HIGH=%3d LOW=%3d Diff=0x%3x=%3d ", __FUNCTION__, __LINE__, temp_high,temp_low,code_diff,code_diff);

		g_ulAWBTargetDiff_B = code_diff;;
		break;

	default:
		rtd_pr_adc_notice("#####[%s(%d)][Get Target Diff] color error!\n", __FUNCTION__, __LINE__);
		break;
	}
	//IoReg_Write32(ABL_Reserved_reg, abl_Reserved_reg.regValue);//leochen
	return TRUE;
...
UINT8 drvif_SelfAWB_allchannel_measure_voltagevaule(UINT8 margin, unsigned char r_ready,  unsigned char g_ready, unsigned char b_ready, UINT16 *r_voltage_diffresult, UINT16 *g_voltage_diffresult, UINT16 *b_voltage_diffresult)
{
	...
	if(r_ready == FALSE)
	{
	#if 0 //interim patch for avoid flash red dot case, rzhen@20190523
		*r_voltage_diffresult = (((IoReg_Read32(ADC_ADC_RGB_DATA_reg)>>16) & 0x0fff) >> 2);
	#else
		*r_voltage_diffresult = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_RESULT_PHASE_M_reg) & 0x03ff;
	#endif
		if(*r_voltage_diffresult == 0)
		{
			rtd_pr_adc_notice("#####[%s(%d)]*r_voltage_diffresult is 0. Error Abort#######\r\n", __FUNCTION__, __LINE__);
			return ERROR_ABORT;
		}
	}
	if(g_ready == FALSE)
	{
	#if 0 //interim patch for avoid flash red dot case, rzhen@20190523
		*g_voltage_diffresult = ((IoReg_Read32(ADC_ADC_RGB_DATA_reg) & 0x0fff) >> 2);
	#else
		*g_voltage_diffresult = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_RESULT_PHASE_L_reg) & 0x03ff;
	#endif
		if(*g_voltage_diffresult == 0)
		{
			rtd_pr_adc_notice("#####[%s(%d)]*g_voltage_diffresult is 0. Error Abort#######\r\n", __FUNCTION__, __LINE__);
			return ERROR_ABORT;
		}
	}
	if(b_ready == FALSE)
	{
	#if 0 //interim patch for avoid flash red dot case, rzhen@20190523
		*b_voltage_diffresult = (((IoReg_Read32(ADC_ADC_RGB_DATA2_reg)>>16) & 0x0fff) >> 2);
	#else
		*b_voltage_diffresult = (IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_RESULT_PHASE_L_reg)>>16) & 0x03ff;
	#endif
		if(*b_voltage_diffresult == 0)
		{
			rtd_pr_adc_notice("#####[%s(%d)]*b_voltage_diffresult is 0. Error Abort#######\r\n", __FUNCTION__, __LINE__);
			return ERROR_ABORT;
		}
	}
	...
	rtd_pr_adc_notice("#####[%s(%d)]Measure success!!!\n", __FUNCTION__, __LINE__);
	...
	UINT16 *r_voltage_diffresult, UINT16 *g_voltage_diffresult, UINT16 *b_voltage_diffresult, int src, ADCGainOffset * ptADC_info)
{
	...
	{
		rtd_pr_adc_notice("#####[%s(%d)][SelfAWB] Error  Gain ABORT 0 %c", __FUNCTION__, __LINE__, '!');
		return ERROR_ABORT;
	}
	...
	rtd_pr_adc_notice("#####[%s(%d)]HIGH=r_ready=%d,g_ready=%d,b_ready=%d\n ", __FUNCTION__, __LINE__, r_ready, g_ready, b_ready);
	rtd_pr_adc_notice("#####[%s(%d)]HIGH=r(%3d) g(%3d) b(%3d)\r\n ", __FUNCTION__, __LINE__, temp_r_high, temp_g_high, temp_b_high);
	...
	if(drvif_SelfAWB_allchannel_measure_voltagevaule(_COLOR_MAX, r_ready, g_ready, b_ready, r_voltage_diffresult, g_voltage_diffresult, b_voltage_diffresult) != ERROR_SUCCEED)
	{
		rtd_pr_adc_notice("#####[%s(%d)][SelfAWB] Error Gain ABORT 1 %c", __FUNCTION__, __LINE__, '!');
		return ERROR_ABORT;
	}
	...
	rtd_pr_adc_notice("#####[%s(%d)]LOW=r_ready=%d,g_ready=%d,b_ready=%d\n ", __FUNCTION__, __LINE__, r_ready, g_ready, b_ready);
	rtd_pr_adc_notice("#####[%s(%d)]LOW=r(%3d) g(%3d) b(%3d)\n", __FUNCTION__, __LINE__, temp_r_low, temp_g_low, temp_b_low);
	...
	rtd_pr_adc_notice("#####[%s(%d)]Diff=r_ready=%d,g_ready=%d,b_ready=%d\n ", __FUNCTION__, __LINE__, r_ready, g_ready, b_ready);
	rtd_pr_adc_notice("#####[%s(%d)]Diff=r(%3d) g(%3d) b(%3d)\n", __FUNCTION__, __LINE__, *r_voltage_diffresult, *g_voltage_diffresult, *b_voltage_diffresult);
	...
	rtd_pr_adc_notice("#####[%s(%d)]R diffresult=%d, ucMax_Target=%d\n", __func__, __LINE__, *r_voltage_diffresult, difftarget_r);
	rtd_pr_adc_notice("#####[%s(%d)]G diffresult=%d, ucMax_Target=%d\n", __func__, __LINE__, *g_voltage_diffresult, difftarget_g);
	rtd_pr_adc_notice("#####[%s(%d)]B diffresult=%d, ucMax_Target=%d\n", __func__, __LINE__, *b_voltage_diffresult, difftarget_b);
	...
	while(color)
	{
		if((color == SELECT_RED) && (r_ready == FALSE))
		{
		    if(*r_voltage_diffresult > difftarget_r)
		    {
				...
				rtd_pr_adc_notice("#####[%s(%d)]SELECT_RED Increase Gain   temp=%d\n", __func__, __LINE__, delta);
				...
				rtd_pr_adc_notice("#####[%s(%d)]SELECT_RED Decrease Gain  temp=%d\n", __func__, __LINE__, delta);
		...
		else if((color == SELECT_GREEN) && (g_ready == FALSE))
		{
		    if(*g_voltage_diffresult > difftarget_g)
		    {
				...
				rtd_pr_adc_notice("#####[%s(%d)]SELECT_GREEN Increase Gain   temp=%d\n", __func__, __LINE__, delta);
				drvif_self_awb_changegain(color, delta, 1, src, ptADC_info);                 // Increse Gain; Decrease Contrast

			}else if(*g_voltage_diffresult < difftarget_g){
				delta  = difftarget_g - *g_voltage_diffresult;
				rtd_pr_adc_notice("#####[%s(%d)]SELECT_GREEN Decrease Gain  temp=%d\n", __func__, __LINE__, delta);
				// Non-zero return value of auto_adcgainchange() means ADC gain reaches minimum.
				drvif_self_awb_changegain(color, delta, 0, src, ptADC_info);                   // Decrease Gain; Increse Contrast
		...
		else if((color == SELECT_BLUE) && (b_ready == FALSE))
		{
		    if(*b_voltage_diffresult > difftarget_b)
		    {
				...
				rtd_pr_adc_notice("#####[%s(%d)]SELECT_BLUE Increase Gain   temp=%d\n", __func__, __LINE__, delta);
				drvif_self_awb_changegain(color, delta, 1, src, ptADC_info);                 // Increse Gain; Decrease Contrast

			}else if(*b_voltage_diffresult < difftarget_b){
				delta  = difftarget_b - *b_voltage_diffresult;
				rtd_pr_adc_notice("#####[%s(%d)]SELECT_BLUE Decrease Gain  temp=%d\n", __func__, __LINE__, delta);
				// Non-zero return value of auto_adcgainchange() means ADC gain reaches minimum.
				drvif_self_awb_changegain(color, delta, 0, src, ptADC_info);                   // Decrease Gain; Increse Contrast
...
UINT8 drvif_self_awb_gain_adjust_proc(UINT8 ucColor, UINT16 *voltage_diffresult, UINT16 ucMax_Target, int src, ADCGainOffset * ptADC_info)
{
	...
	rtd_pr_adc_debug("HIGH=%3d ",temp_high);
...
//Change Low
	...
	rtd_pr_adc_debug("LOW=%3d Diff=%3d\n",temp_low,temp_high-temp_low);
    ...
    *voltage_diffresult = temp_high - temp_low;
    ...
    if(*voltage_diffresult > ucMax_Target)
    {
		...
		rtd_pr_adc_debug("Decrease Gain  temp=%d\n", temp);
		// Non-zero return value of auto_adcgainchange() means ADC gain reaches minimum.
...
unsigned char auto_waitivs(unsigned char display)
{
	...
	do {
		...
		{
		#ifdef CONFIG_FORCE_RUN_I3DDMA
			flag =  IoReg_BusyCheckRegBit(DMA_VGIP_DMA_VGIP_STATUS_reg, _BIT26,0x1FFFF);
		#else
		...
		{
			flag =	IoReg_BusyCheckRegBit(SUB_VGIP_VGIP_CHN2_STATUS_reg, _BIT26,0x1FFFF);
		}
	#endif
...
{//This API is for Auto clock to run phase check
	...
	if(_ENABLE ==Scaler_Get_YPbPr_AutoPhase_Flag())
	{
		...
		rtd_pr_adc_info("auto_field_reg.rgb_in_one_en=%x\n", auto_field_reg.rgb_in_one_en);
...
unsigned int get_sod_software_auto_phase(unsigned short phase)
{
	...
	// IoReg_Write32(ADC_SMARTFIT_ADC_AUTO_FIELD_reg, smartfit_auto_field_reg.regValue);
	rtd_pr_adc_notice("#####[%s(%d)]auto_field_reg.rgb_in_one_en=%x\n",__func__,__LINE__,smartfit_auto_field_reg.rgb_in_one_en);
	...
	//rtd_pr_adc_info("#####[%s(%d)]@@Software_Auto_Phase",__func__,__LINE__);

	drv_auto_set_phase(phase);
	...
	do{
		smartfit_auto_adj_reg.regValue = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_ADJ_reg);
		if(smartfit_auto_adj_reg.now_af == 0){
			rtd_pr_adc_info("#####[%s(%d)] finished, timeout=%d!\n",__func__,__LINE__,timeout);
			break;
		}

		msleep(1);//can't remove, ensure phase finish
	}while(--timeout);

	if(timeout == 0){
		rtd_pr_adc_info("#####[%s(%d)] timeout error, 0xb8020410=0x%08x\n",__func__,__LINE__,IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_ADJ_reg));
	}else{
		SOD_VALUE = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_RESULT_PHASE_L_reg);
	}

	rtd_pr_adc_notice("#####[%s(%d)]phase:0x%02x SOD:0x%08x####\n",__func__,__LINE__,phase,SOD_VALUE);
...
{
	unsigned int timing;
	sync_processor_sp1_coastftbk_RBUS sync_processor_sp1_coastftbk_reg;
	adc_smartfit_adc_auto_r_g_b_margin_RBUS smartfit_auto_r_g_b_margin_reg;
	adc_smartfit_adc_auto_phase_ctrl0_RBUS smartfit_auto_phase_ctrl0_reg;
	adc_smartfit_adc_auto_adj_RBUS smartfit_auto_adj_reg;
	adc_smartfit_adc_auto_field_RBUS smartfit_auto_field_reg;
	auto_soy_soych0_a0_RBUS auto_soy_soych0_a0_reg;//0xB8020300
	auto_soy_soych0_a2_RBUS auto_soy_soych0_a2_reg;//0xB8020308
	auto_soy_soych0_a3_RBUS auto_soy_soych0_a3_reg;//0xB802030C
	...
	unsigned char pll_read_cnt = 0;
	unsigned char soy_sett_cnt = 0;
	...
	switch(timing){
		...
		case _MODE_480I:
			if(check_macrovision(timing) == TRUE){
				tmp_lvl_value = 0x17;//480i and macrovision case. 20160218.
				macrovision_480i_enable = 1;
				//rtd_pr_adc_emerg("480i and macrovision case!!\n");

				sync_processor_sp1_coastftbk_reg.regValue = IoReg_Read32(SYNC_PROCESSOR_SP1_CoastFtBk_reg);
				sync_processor_sp1_coastftbk_reg.s1_coast_win2_end = 0x16;
				sync_processor_sp1_coastftbk_reg.s1_coast_front= 0x5;
				sync_processor_sp1_coastftbk_reg.s1_coast_back = 0xf;
	...
	if(macrovision_480i_enable){
		auto_soy_soych0_a0_reg.reg_auto_mode = 0;
	}else{
		auto_soy_soych0_a0_reg.reg_auto_mode = 1;
	}

	auto_soy_soych0_a0_reg.reg_auto_ini_dc = tmp_lvl_value;
	...
	if((timing == _MODE_1080P50)||(timing ==_MODE_1080P60)){
		while(++pll_read_cnt){

			dds_pll_dds_mix_w1_reg.regValue = IoReg_Read32(DDS_PLL_DDS_MIX_W1_reg);
			if(dds_pll_dds_mix_w1_reg.pll_lock){
				pll_lock_cnt++;
			}

			if(pll_read_cnt <= 10){
				rtd_pr_adc_notice("[%s(%d)]pll_lock_cnt=%d, pll_read_cnt=%d\n", __func__, __LINE__, pll_lock_cnt, pll_read_cnt);
				if(pll_lock_cnt >= 5)
					break;
			}else{

				if(soy_sett_cnt > 1){
					rtd_pr_adc_notice("[%s(%d)]soy_sett_cnt >2 break\n", __func__, __LINE__);
					break;
				}
				...
				// Write 0x18020308[5:0]=0x1, delay
				auto_soy_soych0_a2_reg.regValue=IoReg_Read32(AUTO_SOY_SOYCH0_A2_reg);
				auto_soy_soych0_a2_reg.reg_min_diff = 1;
				IoReg_Write32(AUTO_SOY_SOYCH0_A2_reg, auto_soy_soych0_a2_reg.regValue);
				msleep(2);

				// Write 0x1803030C[3:0]=0xD, delay
				auto_soy_soych0_a3_reg.regValue = IoReg_Read32(AUTO_SOY_SOYCH0_A3_reg);
				auto_soy_soych0_a3_reg.reg_dc_out_mode = (soy_sett_cnt == 0)?0xD:0xB;
				IoReg_Write32(AUTO_SOY_SOYCH0_A3_reg, auto_soy_soych0_a3_reg.regValue);
				msleep(2);

				// Write 0x18030308[5:0]=0x4, delay
				auto_soy_soych0_a2_reg.regValue=IoReg_Read32(AUTO_SOY_SOYCH0_A2_reg);
				auto_soy_soych0_a2_reg.reg_min_diff = 4;
				IoReg_Write32(AUTO_SOY_SOYCH0_A2_reg, auto_soy_soych0_a2_reg.regValue);
				msleep(2);
	...
	smartfit_auto_field_reg.regValue = 0;//IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_FIELD_reg);
	smartfit_auto_field_reg.auto_balance_rgb = 2;
	// rtd_pr_adc_notice("auto_field_reg.rgb_in_one_en=%x\n", smartfit_auto_field_reg.rgb_in_one_en);
	IoReg_Write32(ADC_SMARTFIT_ADC_AUTO_FIELD_reg, smartfit_auto_field_reg.regValue);
	...
	if(HW_SW_sel == Hardware_Auto_Phase){
		...
		smartfit_auto_phase_ctrl0_reg.para_ap_en = 1;	//New ap for hardware auto phse mode
	#else
		smartfit_auto_phase_ctrl0_reg.autophase_en = HW_SW_sel;	 //Hardware / Software Auto Phase Switch
		smartfit_auto_phase_ctrl0_reg.para_ap_en = 0; 	//New ap for hardware auto phse mode
		...
		smartfit_auto_phase_ctrl0_reg.autosod_step_num = Sel_Step_Num;
	#endif

		IoReg_Write32(ADC_SMARTFIT_ADC_AUTO_PHASE_CTRL0_reg, smartfit_auto_phase_ctrl0_reg.regValue);
		rtd_pr_adc_info("#####[%s(%d)]Hardware_Auto_Phase, 0xb8020424=0x%08x",__func__,__LINE__,smartfit_auto_phase_ctrl0_reg.regValue);

	}else if(HW_SW_sel == Software_Auto_Phase){
		smartfit_auto_phase_ctrl0_reg.autophase_en = HW_SW_sel;
		IoReg_Write32(ADC_SMARTFIT_ADC_AUTO_PHASE_CTRL0_reg, smartfit_auto_phase_ctrl0_reg.regValue);
		rtd_pr_adc_info("#####[%s(%d)]Software_Auto_Phase, 0xb8020424=0x%08x",__func__,__LINE__,smartfit_auto_phase_ctrl0_reg.regValue);
	...
	smartfit_auto_adj_reg.force_flip_ = 1;
	smartfit_auto_adj_reg.sum_max = 1;
	smartfit_auto_adj_reg.pulse_det_en = Sel_Mode?1:0;
	smartfit_auto_adj_reg.autophase_sel_tri = Sel_Mode?0:1;
	smartfit_auto_adj_reg.diff_en = 1;
	smartfit_auto_adj_reg.now_af = 1;
	IoReg_Write32(ADC_SMARTFIT_ADC_AUTO_ADJ_reg, smartfit_auto_adj_reg.regValue);
	rtd_pr_adc_info("#####[%s(%d)]Sel_Mode=%d,0xb8020410=0x%08x",__func__,__LINE__,Sel_Mode,smartfit_auto_adj_reg.regValue);
	...
	//egger 20120822 modify cmp level updating threshold to get right phase and cmp level mapping
	auto_soy_soych0_a2_reg.regValue=IoReg_Read32(AUTO_SOY_SOYCH0_A2_reg);
	auto_soy_soych0_a2_reg.reg_min_diff = 0;
	IoReg_Write32(AUTO_SOY_SOYCH0_A2_reg, auto_soy_soych0_a2_reg.regValue);
...
static unsigned char ypp_auto_phase_waitfinish(unsigned char display)
{
	...
	do {
		smartfit_auto_adj_reg.regValue = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_ADJ_reg);
		if(smartfit_auto_adj_reg.now_af == 0){
			rtd_pr_adc_info("#####[%s(%d)]auto phase finished, Wait_Time_Cnt=%d!\n",__func__,__LINE__,Wait_Time_Cnt);
			break;
		}

		if(get_ADC_Input_Source() == _SRC_VGA){
			if(vga_clock_check_onlinemeasure_status(display) == FALSE){
				rtd_pr_adc_info("#####[%s(%d)]VGA ERROR_ABORT##########\n",__func__,__LINE__);
				return ERROR_ABORT;
			}
		}else{
			if(judge_scaler_break_case(display)){
				rtd_pr_adc_info("#####[%s(%d)]ERROR_ABORT##########\n",__func__,__LINE__);
				return ERROR_ABORT;
			}
		}
		msleep(10);//can't remove, keep enough time wait auto phase finish
	} while (--Wait_Time_Cnt);

	if(Wait_Time_Cnt == 0){
		rtd_pr_adc_notice("#####[%s(%d)]auto phase timeout, 0xb8020410=0x%08x\n",__func__,__LINE__,IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_ADJ_reg));
    }
...
unsigned char auto_phasesearch_WaitFinish(unsigned char *a_pucPhaseResult)
{
	...
	//Wait auto phase finish, approach to 200ms
	Result = ypp_auto_phase_waitfinish(Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL));

	if (ERROR_SUCCEED != Result){
		rtd_pr_adc_notice("#####[%s(%d)]0xb8020410=0x%08x\n",__func__,__LINE__,IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_ADJ_reg));
		rtd_pr_adc_notice("#####[%s(%d)]0xb8020424=0x%08x\n",__func__,__LINE__,IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_PHASE_CTRL0_reg));
		// return ERROR_ABORT;
		msleep(10);
	}
	...
	*a_pucPhaseResult = smartfit_auto_phase_ctrl0_reg.autophase_max_step;
	rtd_pr_adc_notice("#####[%s(%d)]autophase_max_step=0x%02x\n",__func__,__LINE__,smartfit_auto_phase_ctrl0_reg.autophase_max_step);
...
//	StructDisplayInfo *p_dispinfo = Get_ADC_Dispinfo();
	...
	//rtd_pr_adc_info("****drvif_ypbpr_auto_phasedo_start****\n\n");
	...
	if(ERROR_SUCCEED != Result)
	{
		rtd_pr_adc_debug("[ypbpr auto phase start]YPbPr AutoPhase E1\n");
		return Result;
	}
	...
	if(ERROR_SUCCEED != Result)
	{
		rtd_pr_adc_debug("[ypbpr auto phase start]YPbPr AutoPhase E2\n");
		return Result;
	}

	if((h_start == 0) || (h_end == 0))
	{
		rtd_pr_adc_debug("[ypbpr auto phase start]YPbPr AutoPhase E3\n");
		return ERROR_ABORT;
	}
	...
	//rtd_pr_adc_notice("[ypbpr auto phase start]phase_flag = %x", phase_flag);
	drv_auto_set_phase(0);
	drv_auto_set_difference_threshold(YPBPR_AUTO_PHASE_DIFF_TH);

#if 0	//new auto phase algorithm no need setting step,num,start, rzhen@2017-01-06
...
#else	//old phase algorithm
	#if 1
	if(phase_flag){
		Result  = auto_phasesearch_start(Hardware_Auto_Phase, display, Pulse_Detection_Enable, _COLORS_GREEN, HWAUTO_STEP_2, HWAUTOSTEPNUM(0x3f), HWAUTOSTEPSTART(0), &phase_tmp,Low_Pass_Filter_0);
	}else{
		Result  = auto_phasesearch_start(Hardware_Auto_Phase, display, Pulse_Detection_Enable, _COLORS_GREEN, HWAUTO_STEP_1, HWAUTOSTEPNUM(0x3f), HWAUTOSTEPSTART(0), &phase_tmp,Low_Pass_Filter_0);
	}
	#else
	...
	if(phase_flag)
	{
		...
		if((timing == _MODE_1080I25) || (timing == _MODE_1080I30) || (timing == _MODE_480I)) {
			...
			rtd_pr_adc_emerg("[ypbpr 720p 50/60 auto phase start]Result = %x", Result);
	...
	//if(ERROR_SUCCEED != Result)
	//	rtd_pr_adc_info("[ypbpr auto phase start]*******YPbPr AutoPhase auto_phasesearch_start  fail.\n");
	//else
	//	rtd_pr_adc_info("[ypbpr auto phase start]YPbPr AutoPhase auto_phasesearch_start  success.\n");
...
unsigned char drvif_ypbpr_auto_phasedo_WaitFinish(unsigned char display)
{
	...
	if (ERROR_SUCCEED == Result)
		rtd_pr_adc_info("#####[%s(%d)] auto_phasesearch_WaitFinish success\n",__func__,__LINE__);
	else
		rtd_pr_adc_info("#####[%s(%d)] auto_phasesearch_WaitFinish  fail\n",__func__,__LINE__);


	rtd_pr_adc_notice("2.phase_flag = %x", phase_flag);
	rtd_pr_adc_notice("2.best phase = 0x%x=%d\n", best, best);

	YPbPr_SetPhase(display, best, TRUE); //forster modified 071211

	//rtd_pr_adc_notice("[ypbpr auto phase]YPBPR Adjust Phase SUCCEED=%d!!!!\n",best);
...
void Scaler_PrintADCGainInRegister(void)
{
	...
	rtd_pr_adc_notice("[ADC Gain   in register]: R=0x%x=%d  G=0x%x=%d  B=0x%x=%d\n",
...
void Scaler_PrintADCOffsetInRegister(void)
{
	...
	adc_offset0_reg.regValue = IoReg_Read32(ADC_ADC_OFFSET0_reg);
	adc_offset1_reg.regValue = IoReg_Read32(ADC_ADC_OFFSET1_reg);
	rtd_pr_adc_notice("[ADC Offset in register]: R=0x%x=%d  G=0x%x=%d  B=0x%x=%d\n",
...
{
	//rtd_pr_adc_info("=========In Write_SmatFif=========\n");
	adc_smartfit_adc_auto_h_boundary_RBUS smartfit_auto_h_boundary_reg;
	adc_smartfit_adc_auto_v_boundary_RBUS smartfit_auto_v_boundary_reg;
	adc_smartfit_adc_auto_r_g_b_margin_RBUS smartfit_auto_r_g_b_margin_reg;
	adc_smartfit_adc_auto_adj_RBUS smartfit_auto_adj_reg;
...
unsigned short drvif_auto_smartfit_finddominant( unsigned short HStart, unsigned short HEnd, unsigned short VStart, unsigned short VEnd, unsigned char HistCh)
{
	...
	do{
		...
		//rtd_pr_adc_notice("#####[%s(%d)]smartfit_auto_adj_reg.now_af=%d\n", __FUNCTION__, __LINE__, smartfit_auto_adj_reg.now_af);
		...
		if(smartfit_auto_adj_reg.now_af==0)//if smart-fit done
		{
			...
			SUM = ( ( (unsigned long long)(smartfit_auto_result_phase_m_reg.acc_41_32) ) <<32) + (unsigned long long)(smartfit_auto_result_phase_l_reg.acc_31_0);//Need check
			PxlNum = (HEnd-HStart+1)*(VEnd-VStart+1);
			do_div(SUM, PxlNum);
			PEAK = SUM;
			#if 0
			rtd_pr_adc_info("##%s(line: %d)##HStart=%d, HEnd=%d\n",__func__, __LINE__, HStart,HEnd);
			rtd_pr_adc_info("##%s(line: %d)##VStart=%d, VEnd=%d\n",__func__, __LINE__, VStart,VEnd);
			rtd_pr_adc_info("##%s(line: %d)##PEAK=%d\n",__func__, __LINE__, (unsigned short)PEAK);
			#endif

			break;
		};

		//rtd_pr_adc_info("FindDominant_SmartFit_Dragon cnt = %d\n",cnt);
...
unsigned short drvif_adc_gainadjust(unsigned short StartH,unsigned short EndH, unsigned short StartV, unsigned short EndV, unsigned short Target, unsigned short ADC_Data, unsigned char RGB_ADC, unsigned char HistCh)
{
    ...
    if(RGB_ADC == G_ADC_Dragon)
    	rtd_pr_adc_notice("Y ADC_gain_adjust:  Beginning Gain=%d\n", ADC_Data);
	else if(RGB_ADC == B_ADC_Dragon)
    	rtd_pr_adc_notice("Pb(U) ADC_gain_adjust: Beginning Gain=%d\n", ADC_Data);
	else if(RGB_ADC == R_ADC_Dragon)
    	rtd_pr_adc_notice("Pr(V) ADC_gain_adjust: Beginning Gain=%d\n", ADC_Data);
	...
	do
	{
		...
		//rtd_pr_adc_info("#######counter:%d Target=%d,ADC_Data=%d,PEAK_Level=%d,diff=%d######\r\n",counter,Target,ADC_Data,PEAK_Level,diff);
		...
		else if (PEAK_Level < Target)
		{
			max = ADC_Data;
			ADC_Data = ( max + min ) / 2;
			rtd_pr_adc_notice("ADC_adjust:Decrease gain count:%d Tar=%d PEAK=%d NewGain=%d\n",counter, Target, PEAK_Level, ADC_Data);
			drvif_write_adc_gainvalue(ADC_Data, RGB_ADC);
		}
		else if (PEAK_Level >Target)
		{
			min = ADC_Data;
			ADC_Data = ( max + min) / 2;
		    rtd_pr_adc_notice("ADC_adjust:Increase gain count:%d Tar=%d PEAK=%d NewGain=%d\n",counter, Target, PEAK_Level, ADC_Data);
			drvif_write_adc_gainvalue(ADC_Data, RGB_ADC);
		}
...
unsigned char drvif_auto_autocolor_gain_adjust_loop(ADCGainOffset *pt_info, unsigned short GainTarget, unsigned short *GainPos, unsigned short GainData, unsigned char RGB_idx, unsigned char Channel)
{
	...
	rtd_pr_adc_info("Adjust_Loop start\n");
...
#if 0 //for debugging
		rtd_pr_adc_info("Adjust_Loop Check_idx=%d\n", Check_idx);
		rtd_pr_adc_info("GainTarget=%d, OffsetTarget=%d\n", GainTarget, OffsetTarget);
		rtd_pr_adc_info("Gain_Check  =%d, Offset_Check  =%d\n", Gain_Check, Offset_Check);
#endif
		Check_idx++;
	...
	if(GainData < 511 && GainData > 20)//for AWB //Success
	{
		...
		switch(Channel)
		{
			case AutoColor_Y:
				...
				rtd_pr_adc_info("###%s AutoColor_Y success#####\r\n",__FUNCTION__);
			break;

			case AutoColor_U:
				pt_info->Gain_B = GainData;
				rtd_pr_adc_info("###%s AutoColor_U success#####\r\n",__FUNCTION__);
			break;

			case AutoColor_V:
				pt_info->Gain_R = GainData;
				rtd_pr_adc_info("###%s AutoColor_V success#####\r\n",__FUNCTION__);
			break;
		}
	}
	else //fail
	{
		...
		{
			case AutoColor_Y:
				rtd_pr_adc_info("###%s AutoColor_Y fail#####\r\n",__FUNCTION__);
			break;

			case AutoColor_U:
				rtd_pr_adc_info("###%s AutoColor_U fail#####\r\n",__FUNCTION__);
			break;

			case AutoColor_V:
				rtd_pr_adc_info("###%s AutoColor_V fail#####\r\n",__FUNCTION__);
			break;
		}
	}

	rtd_pr_adc_notice("diff_gain : %x, Check_idx : %x\n", diff_gain, Check_idx);
...
//	adc_adc_dcrestore_ctrl_RBUS adc_adc_dcrestore_ctrl_reg;
	...
	rtd_pr_adc_info("==================YPbPr_Adjustment start==================\n\n");
	...
	if (bartype == _Y100UV100) {
		rtd_pr_adc_info("Pattern: _Y100UV100\n");
		...
		rtd_pr_adc_info("Pattern: _Y75UV75\n");
		...
		rtd_pr_adc_info("Pattern: _Y100UV75\n");
	...
	//20070713 VIP/Jason
	rtd_pr_adc_info("<<< Y adjustment: >>>add\n");
	...
	rtd_pr_adc_notice("#####[%s(%d)] result=%d\n", __FUNCTION__, __LINE__, result);

#ifdef CONFIG_WATCHDOG_ENABLE
	IoReg_Write32(TC_WR_TR_VADDR, 1);//tick system WD, to avoid system reset
	IoReg_Write32(TC_WR_TR_VADDR, 0); //kick watchdog
#endif
	rtd_pr_adc_info("<<< U adjustment: >>>\n");
	...
	rtd_pr_adc_info("================== YPbPr_Adjustment end==================\n\n");
	if (result) {
		rtd_pr_adc_info("YPbPr auto color success~!\n");
	} else {
		rtd_pr_adc_info("YPbPr auto color Fail!\n");
	}
	return result;
...
void drvif_self_awb_initial_setting(void)//Auto color initial seeting at beginning
{
	...
	adc_adc_ctl_rgb12_reg.adc_ctl_rgb2_10 = 1;
	...
	//adc_adc_ctl_rgb12_reg.adc_ctl_rgb1_11_10 = 0; don't set for ADC
	...
	dds_pll_pll_phase_interpolation_reg.pllsphrs = 7;//default
	...
	dds_pll_pll_set_reg.pllscp = 0;//2.8p(default)
	dds_pll_pll_set_reg.pllscs = 3;// 47p(default)
	dds_pll_pll_set_reg.pllsrs = 3;// 12k
	...
	dds_pll_pll_ctrl_reg.apllldosel = YPbPr_LDO_Trim()?YPbPr_LDO_Trim_apllldosel():1;//LDO reference voltage setting  01:1.1V(default);
	...
	dds_pll_plldiv_reg.apll_reserve_1_0 = 3; //default thermal delay ypp 0x3
	...
	dds_pll_pllphase_ctrl_reg.pllsph0path = 1; //0: short path, 1: long path
	...
	adc_adc_vbias_reg.adc_vbias0_2_1 = 0;
	adc_adc_vbias_reg.adc_vbias0_5_3 = YPbPr_LDO_Trim()?YPbPr_LDO_Trim_vbias_5_3():4;
	adc_adc_vbias_reg.adc_vbias0_7_6 = 2;
	//adc_adc_vbias_reg.adc_vbias1_3_1 = 6;//Merlin6 reserved
	adc_adc_vbias_reg.adc_vbias1_5_4 = 0;
	...
	adc_adc_clock_reg.adc_clock_6 = 0; //RGB Output Clock Divider, 0:Normal, 1:Inverted
	adc_adc_clock_reg.adc_2x_sample_0 = 1;//Online Auto SOY ADC clock select 1: CKAPLL<1:0>
	adc_adc_clock_reg.adc_2x_sample_5 = 1;//RGB 1X Output Clock (ADC_CLK_1X) Polarity, 0:Negative, 1:Positive
	IoReg_Write32(ADC_ADC_CLOCK_reg, adc_adc_clock_reg.regValue);
...
unsigned char ypbpr_self_awb_offset_calibration(ADCGainOffset * ptADC_info)
{
	...
	rtd_pr_adc_info("[SelfAWB_Offset] Self ABL start!\t");
	...
	else
	{
		...
		rtd_pr_adc_info("[SelfAWB] SelfAWB offset success for YPP!\t");

		rtd_pr_adc_notice("Result: offsetR=0x%x=%d  offsetG=0x%x=%d  offsetB=0x%x=%d\n",ptADC_info->Offset_R,ptADC_info->Offset_R,ptADC_info->Offset_G,ptADC_info->Offset_G,ptADC_info->Offset_B,ptADC_info->Offset_B);
...
PROCESS_ERROR:
		...
		sync_processor_sp1_ctrl_reg.regValue = SP1_Ctrl ;
		IoReg_Write32(SYNC_PROCESSOR_SP1_CTRL_reg,sync_processor_sp1_ctrl_reg.regValue);
		rtd_pr_adc_info("[SelfAWB] SelfAWB Offset ABORT\n");
...
//	adc_adc_clamp_ctrl3_RBUS adc_adc_clamp_ctrl3_reg;
	...
	do
	{
		...
		rtd_pr_adc_notice("[SelfAWB]Gain_Tune %2d reday:%d %d %d\t", tune_count, pr_ready, y_ready, pb_ready);
		...
		if(pr_ready == FALSE)
		{
			rtd_pr_adc_notice("Pr: voltage_diff=%d, target_diff=%d, diff=%d\n", diffvalue_pr, difftarget_pr, ABS(diffvalue_pr, difftarget_pr));
			if(ABS(diffvalue_pr, difftarget_pr) < 2)// && (ABS(result1, usMin_Value) < 2))
			{
				rtd_pr_adc_notice("[SelfAWB] SelfAWB OK for Pr, Diff = %d, Target=%d count=%d\n",diffvalue_pr,difftarget_pr, tune_count);
				pr_ready = TRUE;
			}
			else
			{
				if(diffvalue_pr < difftarget_pr)
					rtd_pr_adc_notice("Pr Decrease gain => gain_new=%d\n",ptADC_info->Gain_R);
				else
					rtd_pr_adc_notice("Pr Increase gain => gain_new=%d\n",ptADC_info->Gain_R);
			}
		}

		if(y_ready == FALSE)
		{
			rtd_pr_adc_notice("Y: voltage_diff=%d, target_diff=%d, diff=%d\n", diffvalue_y, difftarget_y, ABS(diffvalue_y, difftarget_y));
			if(ABS(diffvalue_y, difftarget_y) < 2)// && (ABS(result1, usMin_Value) < 2))
			{
				rtd_pr_adc_notice("[SelfAWB] SelfAWB OK for Y, Diff = %d, Target=%d count=%d\n",diffvalue_y, difftarget_y, tune_count);
				y_ready = TRUE;
			}
			else
			{
				if(diffvalue_y < difftarget_y)
					rtd_pr_adc_notice("Y Decrease gain => gain_new=%d\n",ptADC_info->Gain_G);
				else
					rtd_pr_adc_notice("Y Increase gain => gain_new=%d\n",ptADC_info->Gain_G);
			}
		}

		if(pb_ready == FALSE)
		{
			rtd_pr_adc_notice("Pb: voltage_diff=%d, target_diff=%d, diff=%d\n", diffvalue_pb, difftarget_pb, ABS(diffvalue_pb, difftarget_pb));
			if(ABS(diffvalue_pb, difftarget_pb) < 2)// && (ABS(result1, usMin_Value) < 2))
			{
				rtd_pr_adc_notice("[SelfAWB] SelfAWB OK for Pb, Diff = %d, Target=%d count=%d\n",diffvalue_pb,difftarget_pb, tune_count);
				pb_ready = TRUE;
			}
			else
			{
				if(diffvalue_pb < difftarget_pb)
					rtd_pr_adc_notice("Pb Decrease gain => gain_new=%d\n",ptADC_info->Gain_B);
				else
					rtd_pr_adc_notice("Pb Increase gain => gain_new=%d\n",ptADC_info->Gain_B);
			}
		}
		count--;
	...
	{
		rtd_pr_adc_notice("[SelfAWB] Reday Fail %d %d %d", pr_ready, y_ready, pb_ready);
		error = true;//Not all reday
	}

	if((ptADC_info->Gain_R == 0x00) || (ptADC_info->Gain_G== 0x00) || (ptADC_info->Gain_B == 0x00) || error)
	{
		rtd_pr_adc_notice("[SelfAWB] SelfAWB gain fail error:%d!\n", error);
		rtd_pr_adc_notice("[SelfAWB] SelfAWB fail!\n");
		goto PROCESS_ERROR;
	}
	else
	{
...
PROCESS_ERROR:
		...
		rtd_pr_adc_notice("#####[%s(%d)][SelfAWB] SelfAWB gain Gain ABORT\n", __FUNCTION__, __LINE__);
...
unsigned char ypbpr_self_awb_gain_calibration(ADCGainOffset * ptADC_info)
{
	...
	while(_TRUE)
	{
		...
		do
		{
			...
			if(color == SELECT_BLUE)			// 2 : Blue or PB
				rtd_pr_adc_debug("[SelfAWB] SelfAWB_Tune YPP Pb  %2d\t",tune_count);
			else if(color == SELECT_GREEN)		// 1: Green or Y
				rtd_pr_adc_debug("[SelfAWB] SelfAWB_Tune YPP Y   %2d\t",tune_count);
			else								// 0: Red or PR
				rtd_pr_adc_debug("[SelfAWB] SelfAWB_Tune YPP Pr  %2d\t",tune_count);
			...
			rtd_pr_adc_debug("result0=%d, usMax_Value=%d, diff=%d\n", result0, difftarget, ABS(result0, difftarget));
			...
			{
				rtd_pr_adc_debug("[SelfAWB] Target=%d Diff-Target=%d\t=> ",difftarget,result0-difftarget);
				if(color == SELECT_BLUE)			// 2 : Blue or PB
				{
					if((result0-difftarget)<0)
						rtd_pr_adc_debug("Decrease gain => gain_new=%d\n",ptADC_info->Gain_B);
					else
						rtd_pr_adc_debug("Increase gain => gain_new=%d\n",ptADC_info->Gain_B);
				}
				else if(color == SELECT_GREEN)		// 1: Green or Y
				{
					if((result0-difftarget)<0)
	...
	if((ptADC_info->Gain_R == 0x00) || (ptADC_info->Gain_G== 0x00) || (ptADC_info->Gain_B == 0x00) || error)
	{
		...
		rtd_pr_adc_info("[SelfAWB] SelfAWB fail!\n");
		goto PROCESS_ERROR;
	}
	else
	{
		...
		rtd_pr_adc_info("Result: gainR=0x%x=%d  gainG=0x%x=%d  gainB=0x%x=%d\n",ptADC_info->Gain_R,ptADC_info->Gain_R,ptADC_info->Gain_G,ptADC_info->Gain_G,ptADC_info->Gain_B,ptADC_info->Gain_B);
		drvif_get_default_gainvalue(&gain_value_r, &gain_value_g, &gain_value_b ,_SRC_YPBPR);
		rtd_pr_adc_debug("\t\t\t\t\tIdeal:  gainR=0x%3x=%3d  gainG=0x%3x=%3d  gainB=0x%3x=%3d\n",gain_value_r,gain_value_r,gain_value_g,gain_value_g,gain_value_b,gain_value_b);
		rtd_pr_adc_debug("\t\t\t\t\tDiff:   diffR=%9d  diffG=%9d  diffB=%9d\n",gain_value_r-ptADC_info->Gain_R,gain_value_g-ptADC_info->Gain_G,gain_value_b-ptADC_info->Gain_B);
...
//	adc_adc_clamp_ctrl3_RBUS adc_adc_clamp_ctrl3_reg;
	...
	rtd_pr_adc_notice("#####[%s(%d)]####0xb8020018 = 0x%08x ####\r\n",__FUNCTION__, __LINE__,IoReg_Read32(ADC_ADC_DCRESTORE_CTRL_reg));
	rtd_pr_adc_notice("#####[%s(%d)]####0xb802001c = 0x%08x ####\r\n",__FUNCTION__, __LINE__,IoReg_Read32(ADC_ADC_CLAMP_CTRL0_reg));
	rtd_pr_adc_notice("#####[%s(%d)]####0xb8020020 = 0x%08x ####\r\n",__FUNCTION__, __LINE__,IoReg_Read32(ADC_ADC_CLAMP_CTRL1_reg));
	rtd_pr_adc_notice("#####[%s(%d)][Get Target Diff] Initial ADC Gain   = %d,%d,%d\n", __FUNCTION__, __LINE__, ptADC_info->Gain_R, ptADC_info->Gain_G, ptADC_info->Gain_B);
	...
	if(result == FALSE)
	{
		rtd_pr_adc_err("#####[%s(%d)][Get Target Diff] fail error!\n", __FUNCTION__, __LINE__);
		goto PROCESS_ERROR;
	}
	else
	{
		...
		adc_adc_vbias_reg.adc_vbias2_7 = 0;  // Calibration
   		adc_adc_vbias_reg.adc_vbias2_4 = 0;  //Gain_Cal_DAC3B
		IoReg_Write32(ADC_ADC_VBIAS_reg, adc_adc_vbias_reg.regValue);
		rtd_pr_adc_notice("#####[%s(%d)][Get Target Diff]Target Pr:%d Y:%d Pb:%d!\n\t", __FUNCTION__, __LINE__, g_ulAWBTargetDiff_R, g_ulAWBTargetDiff_G, g_ulAWBTargetDiff_B);
		rtd_pr_adc_notice("#####[%s(%d)][Get Target Diff]success!!!\n\t", __FUNCTION__, __LINE__);
...
PROCESS_ERROR:
		...
		adc_adc_clamp_ctrl0_reg.adc_clamp_ctrl1_8 = 0;
		IoReg_Write32(ADC_ADC_CLAMP_CTRL0_reg, adc_adc_clamp_ctrl0_reg.regValue);
		rtd_pr_adc_err("#####[%s(%d)][Get Target Diff] Fail ABORT\n", __FUNCTION__, __LINE__);
		down(get_adc_detectsemaphore());
		if (get_ADC_Global_Status() == SRC_OPEN_DONE) {//Recover open status
			p_timing = Get_ADC_Dispinfo();
			p_timing->IPH_ACT_WID_PRE = 0;
...
unsigned char External_ADC_Calibration_proc(int src)
{
	...
	rtd_pr_adc_notice("#####rzhen##### [%s(%d)] src=%d #####\n", __func__, __LINE__, src);
	...
	if(src == _SRC_YPBPR)
	{
		...
		rtd_pr_adc_notice("#####[%s(%d)] usResult=%d\n", __FUNCTION__, __LINE__, usResult);
...
void Reset_ADC_Gain_Offset(int src)
{
	//YPP
	...
	drvif_set_gain_from_table(src);
	drvif_set_offset_from_table(src);
}
...
void OTP_Initial_Setting(void)
{
    ...
    IoReg_Write32(ADC_ADC_VBIAS_reg, YPbPr_LDO_Trim()?((YPbPr_LDO_Trim_vbias_5_3()<<3)|0x01b3cca1):0x01b3cca1);
    //rtd_pr_adc_notice("#####[%s(%d)]0xB802000C=0x%08x\n", __func__,__LINE__,IoReg_Read32(ADC_ADC_VBIAS_reg));
    ...
    IoReg_Write32(ADC_ADC_CLAMP_CTRL2_reg, YPbPr_LDO_Trim()?((YPbPr_LDO_Trim_sog_cmp_7_6()<<30)|0x17200000):0x17200000);
    //rtd_pr_adc_notice("#####[%s(%d)]0xB8020024=0x%08x\n", __func__,__LINE__,IoReg_Read32(ADC_ADC_CLAMP_CTRL2_reg));
	...
	IoReg_Write32(DDS_PLL_FAST_PLL_CTRL_reg, 0x00000040);	// enable double buffer load
	IoReg_Write32(DDS_PLL_PLL_CTRL_reg, YPbPr_LDO_Trim()?((YPbPr_LDO_Trim_apllldosel()<<17)|0x00050608):0x00050608);	// enable APLL & dual clk ,LDO
	//rtd_pr_adc_notice("#####[%s(%d)]0xB8020104=0x%08x\n", __func__,__LINE__,IoReg_Read32(DDS_PLL_PLL_CTRL_reg));
}
...
void Get_CodeDiff(unsigned char DacLevel1, unsigned char DacLevel2, unsigned short GainValueR, unsigned short GainValueG, unsigned short GainValueB, unsigned char gCalibrationSource)
{
	...
	//rtd_pr_adc_notice"Get_CodeDiff\n");
	...
	for (ii = 0; ii < Avg_count + 1; ii++) {
		...
		if (0 != ii) {
			if((20 < (ABS(new_temp[0],old_temp[0]))) || (20 < (ABS(new_temp[1], old_temp[1]))) || (20 < (ABS(new_temp[2], old_temp[2])))) {
				ii = ii-1;
				//rtd_pr_adc_notice"Get_CodeDiff error YPP_abs(new_temp[0],old_temp[0])=%d,YPP_abs(new_temp[1],old_temp[1])=%d,YPP_abs(new_temp[2],old_temp[2])=%d,%d\n",
					//(20 < (ABS(new_temp[0], old_temp[0]))), (20 < (ABS(new_temp[1], old_temp[1]))), (20 < (ABS(new_temp[2], old_temp[2]))),
					//(20 < ABS(new_temp[0], old_temp[0])) || (20 < ABS(new_temp[1], old_temp[1])) || (20 < ABS(new_temp[2], old_temp[2])));
				//rtd_pr_adc_notice"Get_CodeDiff error new_temp[0]=%d,old_temp[0]=%d,new_temp[1]=%d,old_temp[1]=%d,new_temp[2]=%d,old_temp[2]=%d \n",
	...
	for(ii = 0; ii < Avg_count + 1; ii++) {
		...
		if (0 != ii) {
			if((20 < (ABS(new_temp[3], old_temp[3]))) || (20 < (ABS(new_temp[4], old_temp[4]))) || (20 < (ABS(new_temp[5], old_temp[5])))) {
				ii--;
				rtd_pr_adc_notice("Get_CodeDiff error new_temp[3]=%d,old_temp[3]=%d,new_temp[4]=%d,old_temp[4]=%d,new_temp[5]=%d,old_temp[5]=%d \n",
	...
	rtd_pr_adc_notice("Currently, Gain=%d,%d,%d, code=%d,%d,%d,%d,%d,%d,%d,%d,%d\n", GainValueR, GainValueG, GainValueB,
...
unsigned char drvif_auto_measurecolor(unsigned char display, unsigned char color, unsigned char margin, unsigned short* rev)
{
	...
	/*
	rtd_pr_adc_info("Active_Region[0]=%d\n",Active_Region[0]);
	rtd_pr_adc_info("Active_Region[1]=%d\n",Active_Region[1]);
	rtd_pr_adc_info("Active_Region[2]=%d\n",Active_Region[2]);
	rtd_pr_adc_info("Active_Region[3]=%d\n",Active_Region[3]);
	rtd_pr_adc_info("Active_Region=%d,%d,%d,%d\n\n",Active_Region[0],Active_Region[0]+Active_Region[1],Active_Region[2],Active_Region[2]+Active_Region[3]);
	*/
	...
	/*
	rtd_pr_adc_info("HStart=%4d=0x%3x\n",HStart,HStart);
	rtd_pr_adc_info("HEnd  =%4d=0x%3x\n",HEnd,HEnd);
	rtd_pr_adc_info("VStart=%4d=0x%3x\n",VStart,VStart);
	rtd_pr_adc_info("VEnd=  %4d=0x%3x\n",VEnd,VEnd);
	*/
	...
	{
		rtd_pr_adc_err("#####[%s(%d)]Measure Error Abort!!!\n", __FUNCTION__, __LINE__);
		return ERROR_ABORT;
	}
	...
	*rev = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_RESULT_PHASE_L_reg) & 0x03ff;
	...
	rtd_pr_adc_notice("#####[%s(%d)]Measure Color=0x%08x, Margin=0x%08x\n", __FUNCTION__, __LINE__,color,*rev);
	return ERROR_SUCCEED;
}
...
unsigned char drvif_auto_tunedevicegain(unsigned char display, unsigned char ucColor, unsigned short *pMargin, unsigned short ucMax_Target)
{
	...
	{
		rtd_pr_adc_err("#####[%s(%d)]Error ABORT!!!\n", __FUNCTION__, __LINE__);
		return ERROR_ABORT;
	}

	if(*pMargin < VGA_AWB_CHECK_LEVEL)
	{
		rtd_pr_adc_err("#####[%s(%d)]*pMargin < VGA_AWB_CHECK_LEVEL\n", __FUNCTION__, __LINE__);
		return ERROR_ABORT;
	}
...
unsigned char drvif_auto_tunedeviceoffset(unsigned char display, unsigned char ucColor, unsigned short *pMargin, unsigned short ucMin_Target)
{
	...
	{
		rtd_pr_adc_notice("#####[%s(%d)]Error ABORT!!!\n", __FUNCTION__, __LINE__);
		return ERROR_ABORT;
	}
...
unsigned char drvif_adc_autobalance_fast(unsigned char display)
{
	...
	rtd_pr_adc_info("[AutoColor] VGA Auto color progressing! fast\n\n");
...
#ifdef _VIDEO_AUTO_WHITE_BLANCE
	...
	rtd_pr_adc_notice("#####[%s(%d)][AutoColor] Blue measure start!\n", __FUNCTION__, __LINE__);
	...
	while(TRUE){
		...
		do{
			...
			if(color == SELECT_BLUE)			// 2 : Blue or PB
				rtd_pr_adc_notice("#####[%s(%d)][AutoColor] AutoColor_Tune VGA Blu %2d\n", __FUNCTION__, __LINE__, drvif_AutoColor_tunedevicegain_count);
			else if(color == SELECT_GREEN)		// 1: Green or Y
				rtd_pr_adc_notice("#####[%s(%d)][AutoColor] AutoColor_Tune VGA Grn %2d\n", __FUNCTION__, __LINE__, drvif_AutoColor_tunedevicegain_count);
			else								// 0: Red or PR
				rtd_pr_adc_notice("#####[%s(%d)][AutoColor] AutoColor_Tune VGA Red %2d\n", __FUNCTION__, __LINE__, drvif_AutoColor_tunedevicegain_count);
		...
		if(0 == count){
			...
			rtd_pr_adc_notice("#####[%s(%d)][AutoColor]QUIT for VGA color=%d, Result = %4d, Target=%4d Diff=%d\n", __FUNCTION__, __LINE__,color,result0,usMax_Value,result0-usMax_Value);
		}
		
		if(color == SELECT_BLUE){				// 4: Blue

			color = SELECT_GREEN;
			rtd_pr_adc_notice("#####[%s(%d)][AutoColor]Grn measure start!\n", __FUNCTION__, __LINE__);
			drvif_AutoColor_tunedevicegain_count=0;
		}else if(color == SELECT_GREEN){		// 2: Green

			color = SELECT_RED;
			rtd_pr_adc_notice("#####[%s(%d)][AutoColor]Red measure start!\n", __FUNCTION__, __LINE__);
			drvif_AutoColor_tunedevicegain_count=0;
		}else{									// 1: Red								
	...
	{
		rtd_pr_adc_notice("#####[%s(%d)][AutoColor]VGA Auto color fail!\n", __FUNCTION__, __LINE__);
		return ERROR_ABORT;
	}else{
		rtd_pr_adc_notice("#####[%s(%d)][AutoColor]Success: gainR=%d, gainG=%d, gainB=%d\n", __FUNCTION__, __LINE__,ptADCGainOffset->Gain_R,ptADCGainOffset->Gain_G,ptADCGainOffset->Gain_B);
		return ERROR_SUCCEED;
	}
}
...
unsigned char drvif_adc_autobalance_standard(unsigned char display)
{
	...
	rtd_pr_adc_notice("#####[%s(%d)][AutoColor] Auto color progressing standard\n", __FUNCTION__, __LINE__);
	...
	rtd_pr_adc_notice("#####[%s(%d)][AutoColor]VGA_AWB_CHECK_LEVEL=0x%08x\n", __FUNCTION__, __LINE__, VGA_AWB_CHECK_LEVEL);

	if (rev < VGA_AWB_CHECK_LEVEL){

		rtd_pr_adc_err("#####[%s(%d)][AutoColor]ERROR_ABORT rev < 0x180 !!!\n", __FUNCTION__, __LINE__);
		goto PROCESS_ERROR;
	}

	rtd_pr_adc_notice("#####[%s(%d)][AutoColor]Grn check ok !!!\n", __FUNCTION__, __LINE__);
	...
	rtd_pr_adc_notice("#####[%s(%d)][AutoColor]Blu check ok !!!\n", __FUNCTION__, __LINE__);
	...
	rtd_pr_adc_notice("#####[%s(%d)][AutoColor]Red check ok !!!\n", __FUNCTION__, __LINE__);
	...
	rtd_pr_adc_notice("#####[%s(%d)][AutoColor]Blu measure start !!!\n", __FUNCTION__, __LINE__);
	...
	while (TRUE){
		...
		do{
			...
			{
				rtd_pr_adc_notice("#####[%s(%d)][AutoColor]result0:%x, usMax_Value:%x\n", __FUNCTION__, __LINE__, result0, usMax_Value);
				rtd_pr_adc_notice("#####[%s(%d)][AutoColor]result1:%x, usMin_Value:%x\n", __FUNCTION__, __LINE__, result1, usMin_Value);
				break;
			}
...
void Interpolate_GainValue(unsigned short iCode, unsigned char iADCSelect, short  *iNewGain, unsigned char iCalibrationSource )
{
	...
	rtd_pr_adc_notice"Interpolate_GainValue ratio=%d \n", ratio_temp1);
	...
	for(i1 = 0; i1< GAINSLICE; i1++) {
		for (i2 = 0; i2 < CODESLICE; i2++){
			...
			//rtd_pr_adc_notice"Interpolate_GainValue new ADC_Calib[%d][%d]=%d \n", i1, i2, ADC_Calib[i1][i2]);
		}
	}
	...
	rtd_pr_adc_notice"Interpolate_GainValue interpolate_zone=%d,iNewGain=%d\n", interpolate_zone, *iNewGain);
}

void Build_Self_AWB_ADC_Curve(unsigned char bCalibrationSource)
{
	int count, i, ii;
	rtd_pr_adc_notice"Build_Self_AWB_ADC_Curve source=%d\n", bCalibrationSource);
	OTP_Initial_Setting();
	...
	count = CODESLICE;
	//rtd_pr_adc_notice"Build_Self_AWB_ADC_Curve count=%d\n",count);
	for(i = 0; i < count; i++) {
	...
	rtd_pr_adc_notice"ADC_Calib source=%d, Gain Slice=%d,%d,%d,%d,%d,%d,%d,%d,%d\n" ,bCalibrationSource, AWB_ADC_Gain_Slice[bCalibrationSource][0],
...
void SAWB_GainCalculate(unsigned char sCalibrationSource)
{
	...
	//Step3. Caculate the Gain for GetTagertValue and the final Gain=
	int i;
	rtd_pr_adc_notice"SAWB_GainCalculate source=%d\n", sCalibrationSource);
	OTP_Initial_Setting();
	//IoReg_Mask32(ADC_ADC_DCRESTORE_CTRL_reg, 0xffd0fe00, _BIT21); //Enable Calib mode OFFPAD,LPF=10M
	//IoReg_Mask32(ADC_ADC_CLAMP_CTRL1_reg, 0xfffffe00, _BIT8|_BIT6|_BIT5|_BIT3|_BIT2|_BIT0); //VClamping postive Enable
...
unsigned char drvif_auto_phasedo(StructScreenModeInfo *p_screenInfo, unsigned char display, unsigned char NM, unsigned char color)
{
	...
	rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust]Adjust Phase Start !!!!\n", __func__, __LINE__);
...
unsigned char drvif_auto_clockdo(StructScreenModeInfo *p_screenInfo, unsigned char display, unsigned char NM,unsigned char color)
{
	...
	rtd_pr_adc_notice("#####[%s(%d)]usTempClock = %d#####\n", __func__, __LINE__, usTempClock);
	rtd_pr_adc_notice("#####[%s(%d)]INPUT_ADC_CLOCK = %d#####\n", __func__, __LINE__, p_dispinfo->INPUT_ADC_CLOCK);
...
//sync auto adjust flow
	...
	do
	{
		...
		if (ERROR_SUCCEED != Result)
		{
			rtd_pr_adc_notice("#####[%s(%d)]Result = 0x%02x#####\n", __func__, __LINE__, Result);
			return Result;
		}
		rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust]Active actual h_start = %d#####\n", __func__, __LINE__, h_start);
		rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust]Active actual h_end = %d#####\n", __func__, __LINE__, h_end);
		h_end   = h_end  - h_start + 1;
		rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust]Active actual h_end = %d#####\n", __func__, __LINE__, h_end);
		rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust]IPH_ACT_WID_PRE = %d#####\n", __func__, __LINE__, IPH_ACT_WID_PRE);
		if(h_end < IPH_ACT_WID_PRE)
		{
			if((IPH_ACT_WID_PRE - h_end) >= delta)
			{
				rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust]1-delta = %d#####\n", __func__, __LINE__, delta);
				stop    = 1;
			}
			else
			{
				h_start = (unsigned int)(IPH_ACT_WID_PRE - h_end) * (usTempClock) /(unsigned int)h_end;
				rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust]h_start = %d#####\n", __func__, __LINE__, h_start);
   				if(h_start <= 2)    break;
...
//[Code Sync][AlanLi][0980525][1][end]
       	...
       	|| (h_end < IPH_ACT_WID_PRE && (IHTotal - usTempClock) == 4))
	{
		...
		Result    = drvif_measure_h_position(display, 0X40, &h_start, &h_end,color);
		rtd_pr_adc_notice("#####[%s(%d)]Result=0x%02x#####\n", __func__, __LINE__, Result);
	...
	if(!((h_end == IPH_ACT_WID_PRE)&&(p_screenInfo->Clock == 50)))
	{
		...
		//rtd_pr_adc_info("[auto adjust]final clock= %d\n", stVGAModeCurrInfo[ucMode_Curr].Clock);
	...
	rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust]Adjust Clock SUCCEED count =%x#####\n", __func__, __LINE__, count);
	return ERROR_SUCCEED;
...
unsigned char drvif_auto_positiondo(StructScreenModeInfo *p_screenInfo, unsigned char display, unsigned char NM,unsigned char color)
{
	...
	rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust]Adjust Position Start !!!!\n", __func__, __LINE__);

    Scaler_DispSetAutoPosition(TRUE);

	Result = drvif_measure_v_position(display, 0x80, 0x80, 0x80, &v_start, &v_end,color); //darwin, mag
	if (ERROR_SUCCEED != Result){
		rtd_pr_adc_notice("#####[%s(%d)]ERROR_ABORT Result=0x%02x, v_start=%d,v_end=%d#####\n", __func__, __LINE__, Result, v_start,v_end);
		return Result;
	}

	if(!check_v_auto_position_result(v_start, v_end, p_dispinfo))
	{
		rtd_pr_adc_notice("#####confirm v position result error v_start=%d,v_end=%d#####\n", v_start,v_end);
		return ERROR_ABORT;
	}
	...
	rtd_pr_adc_notice("#####[%s(%d)][Auto Adjust]Adjust Position SUCCEED !!!!\n", __func__, __LINE__);
...
static void  drvif_set_smartfit_boundary_with_no_hs_mode(unsigned char display)
{
	smartfit_auto_h_boundary_RBUS smartfit_auto_h_boundary_reg;
	smartfit_auto_v_boundary_RBUS smartfit_auto_v_boundary_reg;

	smartfit_auto_h_boundary_reg.regValue = IoReg_Read32(SMARTFIT_AUTO_H_BOUNDARY_reg);
	smartfit_auto_h_boundary_reg.hb_sta = 0;
	smartfit_auto_h_boundary_reg.hb_end = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_IPH_ACT_WID);
	IoReg_Write32(SMARTFIT_AUTO_H_BOUNDARY_reg, smartfit_auto_h_boundary_reg.regValue);

	smartfit_auto_v_boundary_reg.regValue = IoReg_Read32(SMARTFIT_AUTO_V_BOUNDARY_reg);
	smartfit_auto_v_boundary_reg.vb_sta = 0;
	smartfit_auto_v_boundary_reg.vb_end = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_IPV_ACT_LEN);
	IoReg_Write32(SMARTFIT_AUTO_V_BOUNDARY_reg, smartfit_auto_v_boundary_reg.regValue);

	rtd_pr_adc_notice("[ADC]: HS=0x%3x=%4d HE=0x%3x=%4d VS=0x%3x=%4d VE=0x%3x=%4d\n",
			smartfit_auto_h_boundary_reg.hb_sta, smartfit_auto_h_boundary_reg.hb_sta,
			smartfit_auto_h_boundary_reg.hb_end, smartfit_auto_h_boundary_reg.hb_end,
			smartfit_auto_v_boundary_reg.vb_sta, smartfit_auto_v_boundary_reg.vb_sta,
			smartfit_auto_v_boundary_reg.vb_end, smartfit_auto_v_boundary_reg.vb_end);
}

static void  drvif_set_smartfit_auto_field_ctrl_no_hs_mode(unsigned char display)
{
	smartfit_auto_field_RBUS smartfit_auto_field_reg;
	smartfit_auto_adj_RBUS smartfit_auto_adj_reg;
...
#endif
 	/*
	 * 0 : Select source after VGIP. H delay need to be 2.
	 * 1 : Select source before VGIP. H delay need to be 3 
	 */
	smartfit_auto_field_reg.smartfit_src_sel = 0;
	smartfit_auto_field_reg.no_hs_mode = 1;
	IoReg_Write32(SMARTFIT_AUTO_FIELD_reg, smartfit_auto_field_reg.regValue);
...
static void smartfit_start_and_wait(void)
{
	int timeoutcnt;
	smartfit_auto_adj_RBUS smartfit_auto_adj_reg;
	...
	do {
		msleep(1);
		smartfit_auto_adj_reg.regValue = IoReg_Read32(SMARTFIT_AUTO_ADJ_reg);
	} while ((smartfit_auto_adj_reg.now_af) && (--timeoutcnt));
}

__maybe_unused static void smartfit_no_hs_mode(unsigned char display, int *hstart, int *hend, int *vstart, int *vend)
{
	smartfit_auto_result_hsta_end_RBUS auto_result_hsta_end_reg;
	smartfit_auto_result_vsta_end_RBUS auto_result_vsta_end_reg;

	drvif_set_smartfit_auto_field_ctrl_no_hs_mode(display);
	drvif_set_smartfit_boundary_with_no_hs_mode(display);
	smartfit_start_and_wait();
