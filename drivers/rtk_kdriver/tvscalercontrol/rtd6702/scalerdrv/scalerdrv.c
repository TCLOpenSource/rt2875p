...
unsigned char panel_output_timing_mode = OUTPUT_TIMING_MODE_MAX;
...
/*================================== Function  ==============================*/
void rpc_send_hdmi_2_1_flag(unsigned char flag)
{//send SCALERIOC_HDMI21_FLAG to video fw
	static unsigned char hdmi21_flag = FALSE;//default not hdmi2.1
	int ret =0;
	if(hdmi21_flag != flag)
	{
		if (0 != (ret = Scaler_SendRPC(SCALERIOC_HDMI21_FLAG,flag,1)))//
		{
			rtd_pr_vsc_err("ret=%d, SCALERIOC_HDMI21_FLAG RPC fail !!! flag:%d \n", ret, flag);
			return;
		}
		else
			rtd_pr_vsc_info("SCALERIOC_HDMI21_FLAG RPC flag:%d \n", flag);
		hdmi21_flag = flag;
	}
}
...
{/*TRUE: memc bypass; FALSE: memc on */
	...
	if(Scaler_MEMC_RbusAccess_Check() == 0){
        ...
        }else if((get_panel_res() == PANEL_RES_WQHD)||(get_panel_res() == PANEL_RES_FHD) || (get_panel_res() == PANEL_RES_HD)){
		flg = TRUE;  //2k default memc bypass
		...
		flg = FALSE; //4k/8k default memc on
	}else{
		flg = FALSE;
	}
#endif
...
#if 0 //IC before Mark2 use this flow
	if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)
	{
		unsigned int pixel_clock = (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IHLEN)/10) * (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVLEN)/10) * (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ)/10);
...
unsigned char get_hdmi_4k_hfr_mode(void)
{
	...
	return scaler_hmdi_hfr_mode;// will decide at scaler start
#endif
	return HDMI_NON_4K120; //due to Mark2 vo, hdr,dispi support 2pixel mode, so always return non, not go sepcial 4k120 flow
}

unsigned char get_hdmi_vrr_4k60_mode(void){
#if 0	//Mark2 no need to go VRR 4K60 mode
	if((Get_DISPLAY_REFRESH_RATE() >= 120)
...
//[RL6702-2770] decide output timing mode
void Set_Dynamic_Panel_Output_Timing_Mode(void)
{
	if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_JPEG){	//photo output 8K30Hz
		panel_output_timing_mode = OUTPUT_8K4K30HZ;
	}else if(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= 605){	//framerate > 60 output 4K120Hz
		panel_output_timing_mode = OUTPUT_4K2K120HZ;
	}else{	//normal mode output 8K60Hz
		panel_output_timing_mode = OUTPUT_8K4K60HZ;
	}
}

unsigned int Get_Dynamic_Panel_Output_Timing_Mode(void)
{
	return panel_output_timing_mode;
}
...
//	vgip_chn1_3d_right_act_vsta_length_RBUS vgip_chn1_3d_right_act_vsta_length_reg;
	...
	if((SCALER_DISP_CHANNEL)_CHANNEL1 == (SCALER_DISP_CHANNEL)display)
	{
		...
		if ((Scaler_InputSrcGetType(display)==_SRC_HDMI)&&(Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPV_ACT_STA) >6))
		{
			...
			else if((source_len!=480)&&(source_len!=576)&&(source_len!=768))// 480p and 576p need set protch by DIC suggester, others need set vdelay be 4
			{
				...
				if(Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ) > 120){//input 144Hz front porch very small, set VGIP delay to 0
				...
				else if(Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ) > 605){
	...
	else// if(_CHANNEL2 == GET_INPUT_CHANNEL(info->display))
	{
		...
		{
			sub_vgip_vgip_chn2_delay_reg.regValue = IoReg_Read32(SUB_VGIP_VGIP_CHN2_DELAY_reg);
			sub_vgip_vgip_chn2_delay_reg.ch2_ihs_dly = ucIHSDelay >> 1;
	...
	if(((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_AVD) || (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_ADC) || (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)) &&
		(Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display, SLR_INPUT_IPV_ACT_LEN) <= 1080) &&
			...
			(H3DDMA_I3DDMA_enable_get_uzd_mux_sel(IoReg_Read32(H3DDMA_I3DDMA_enable_reg)))){
		{
			vgip_chn1_ctrl_reg.regValue = IoReg_Read32(VGIP_VGIP_CHN1_CTRL_reg);
			vgip_chn1_ctrl_reg.ch1_field_inv = _ENABLE;
			IoReg_Write32(VGIP_VGIP_CHN1_CTRL_reg, vgip_chn1_ctrl_reg.regValue);
...
void fw_scaler_set_vgip(unsigned char channel, unsigned char src, unsigned char mode)
{
	...
	sub_vgip_vgip_chn2_ctrl_RBUS vgip_chn2_ctrl_reg;
#endif
	scaledown_ich1_uzd_ctrl1_RBUS scaledown_ich1_uzd_ctrl1_reg;
#ifdef CONFIG_VGIP_DIGITAL_MODE
		if(src == VGIP_SRC_HDR1 || src == VGIP_SRC_HDR2 || src == VGIP_SRC_HDMI1 || src == VGIP_SRC_HDMI2)
	...
	if ( _CHANNEL1 == channel )
	{
		...
		scaledown_ich1_uzd_ctrl1_reg.regValue = IoReg_Read32(SCALEDOWN_ICH1_UZD_Ctrl1_reg);
		if(vgip_chn1_ctrl_reg.ch1_digital_mode)
		{//digital mode
			...
			scaledown_ich1_uzd_ctrl1_reg.sdnr_htotal = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_H_LEN) - 1;//set h toal
		}
		else
		{
			...
			scaledown_ich1_uzd_ctrl1_reg.sdnr_htotal = 0;//set h toal
		}
		...
		IoReg_Write32(SCALEDOWN_ICH1_UZD_Ctrl1_reg, scaledown_ich1_uzd_ctrl1_reg.regValue);

		rtd_pr_vbe_debug("[IDMA] Src, VgipM=%d, %d\n", src, vgip_chn1_ctrl_reg.ch1_in_sel);
		if(mode == VGIP_MODE_ANALOG)
		{//analog
			ctrl_i_clock_2p_sel(_MAIN_VGIP_PATH, 0);//control iclock is 2p or not
		}
		else
		{//digital
			if(src == VGIP_SRC_HDR2)
			{//vo dma2
				ctrl_i_clock_2p_sel(_MAIN_VGIP_PATH, 0);//control iclock is 2p or not
			}
			else if(src == VGIP_SRC_HDR1)
			{//vodma1
				//if VODMA_VODMA_CLKGEN_get_vodma_clk_2x_2p_sel(IoReg_Read32(VODMA_VODMA_CLKGEN_reg)): 01 1p
				ctrl_i_clock_2p_sel(_MAIN_VGIP_PATH, VODMA_VODMA_CLKGEN_get_vodma_clk_2x_2p_sel(IoReg_Read32(VODMA_VODMA_CLKGEN_reg))? 0 : 1);
			}
			else
			{//hdmi case
				ctrl_i_clock_2p_sel(_MAIN_VGIP_PATH, check_hdmi_enabe_2p_mode(Get_DisplayMode_Port(SLR_MAIN_DISPLAY)));
			}
		}
	}
#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
	else	// if ( _CHANNEL2 == channel )
	{
		...
		if(mode == VGIP_MODE_ANALOG)
		{//analog
			ctrl_i_clock_2p_sel(_SUB_VGIP_PATH, 0);//control iclock is 2p or not
		}
		else
		{//digital
			if(src == VGIP_SRC_HDR2)
			...
			{//vodma1
				//if VODMA_VODMA_CLKGEN_get_vodma_clk_2x_2p_sel(IoReg_Read32(VODMA_VODMA_CLKGEN_reg)): 01 1p
				ctrl_i_clock_2p_sel(_SUB_VGIP_PATH, VODMA_VODMA_CLKGEN_get_vodma_clk_2x_2p_sel(IoReg_Read32(VODMA_VODMA_CLKGEN_reg))? 0 : 1);
			}
			else
			{//hdmi case
				ctrl_i_clock_2p_sel(_SUB_VGIP_PATH, check_hdmi_enabe_2p_mode(Get_DisplayMode_Port(SLR_SUB_DISPLAY)));
			}
		}
	}
#endif
...
void vbe_disp_set_VRR_set_display_vtotal_60Hz(void){
	...
	if(Get_DISPLAY_REFRESH_RATE() == 144)
		vertical_total = 2186;
	else if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_8K4K)
	{
		if(((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 245) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 255))
...
void drvif_scaler_set_display_mode(UINT8 timing_mode)
{
		...
		{
			display_timing_ctrl1_reg.regValue = IoReg_Read32(PPOVERLAY_Display_Timing_CTRL1_reg);
			display_timing_ctrl1_reg.fix_last_line_vsync_sel = 0; // 1:pcr_tracking vsync(A)(B) counter_vs, 0:uzd_vs (original)
			if(_ENABLE == Scaler_Get_FreeRun_To_FrameSync_By_HW_Enable())
			{
				if(_DISABLE == display_timing_ctrl1_reg.disp_fsync_en)
				{
					// FLL tracking PCR VS disable
					...
					ppoverlay_fix_last_line_pcr_ctrl_reg.pcr_vs_count_en = 0;
...
//#ifndef CONFIG_MEMC_BYPASS
				if(get_MEMC_bypass_status_refer_platform_model() == FALSE){
					...
					rtd_pr_vbe_debug("turn off me4.5 ok\n");
					#endif
				}
//#endif
...
//#ifndef CONFIG_MEMC_BYPASS
				if(get_MEMC_bypass_status_refer_platform_model() == FALSE){
					...
					rtd_pr_vbe_debug("turn on me4.5 ok\n");
					#endif
				}
//#endif
			...
			unsigned short frame_rate = Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ);
			ppoverlay_fix_last_line_pcr_ctrl_RBUS ppoverlay_fix_last_line_pcr_ctrl_reg;
			ppoverlay_fix_last_line_pcr_ctrl_reg.regValue = rtd_inl(PPOVERLAY_fix_last_line_pcr_ctrl_reg);
			ppoverlay_fix_last_line_pcr_ctrl_reg.pcr_vs_count_en = (modestate_get_fll_tracking_pcr_vs_flag()? 1: 0);
			...
			display_timing_ctrl1_reg.fix_last_line_vsync_sel = (modestate_get_fll_tracking_pcr_vs_flag()? 1: 0); // 1:pcr_tracking vsync(A)(B) counter_vs, 0:uzd_vs (original)
...
#define INCREASE_VTOTAL_60HZ_8K 0x16ee
#define INCREASE_VTOTAL_50HZ_8K 0x15F9
...
#define OLED_PANEL_DEN_PROTECT_PORCH 38
...
unsigned int drvif_scaler_get_speedUp_vtotal(unsigned int inVFreq)
{
	...
	speedUp_Vtotal = Get_dispd_stage1_f2p_data_rate() / (Get_DISP_HORIZONTAL_TOTAL()*speedUp_VFreq);
...
//	unsigned int timeout = 0;
    ...
    if(DVtotal==0){
        ...
        DVtotal = Get_dispd_stage1_f2p_data_rate()/(Get_DISP_HORIZONTAL_TOTAL()
			*vFreq);
    }
...
void drvif_scaler_enable_frc2fsync_HW_speedup_vtotal(UINT8 bEnable)
{
	...
	if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_8K4K){
		if((vFreq>245)&&(vFreq<=255)){ // 25hz
			speedUp_vtotal = INCREASE_VTOTAL_50HZ_8K;
		}else if((vFreq>495)&&(vFreq<=505)){
			speedUp_vtotal = INCREASE_VTOTAL_50HZ_8K;
		}else if((vFreq>295)&&(vFreq<=305)){
			speedUp_vtotal = INCREASE_VTOTAL_50HZ_8K;
		}else{
			speedUp_vtotal = INCREASE_VTOTAL_60HZ_8K;
		}
	}
	else{
		if((vFreq>245)&&(vFreq<=255)){ // 25hz
			...
			if(vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag()){
				...
				if(vFreq<=FRC2FSYNC_VRR_SPECIAL_FREQ){
					if(vFreq >= FRC2FSYNC_VRR_MIN_SUPPORT_FREQ){
						speedUp_vtotal = Get_dispd_stage1_f2p_data_rate()/(Get_DISP_HORIZONTAL_TOTAL() * (vFreq-FRC2FSYNC_VRR_SPEEDUP_OFFSET)/10);
					}else{
						speedUp_vtotal = Get_dispd_stage1_f2p_data_rate()/(Get_DISP_HORIZONTAL_TOTAL() * (FRC2FSYNC_VRR_MIN_SUPPORT_FREQ+FRC2FSYNC_VRR_SPEEDUP_OFFSET)/10);
					}
				}else{
					speedUp_vtotal = INCREASE_VTOTAL_60HZ;
				}
...
void drvif_scaler_set_display_mode_frc2fsync_by_hw_speedup_Mcode_HW_CTRL_3(void)
{
		...
		fs_iv_dv_fine_tuning5_reg.iv2dv_line = DEFAULT_FRC_IV2DV_DELAY;
		fs_iv_dv_fine_tuning5_reg.iv2dv_line_2 = DEFAULT_FRC_IV2DV_DELAY;
#else
		fs_iv_dv_fine_tuning5_reg.iv2dv_line = DEFAULT_FRC_IV2DV_DELAY;
		fs_iv_dv_fine_tuning5_reg.iv2dv_line_2 = DEFAULT_FRC_IV2DV_DELAY;
#endif
...
//#define _VRR_MAX_SUPPORT_FRAMERATE 1200
...
//#endif
		...
		if(vbe_disp_decide_uzuclock_mapping_vo()){
			if((Scaler_DispGetInputInfoByDisp(0,SLR_INPUT_V_FREQ) > 235) && (Scaler_DispGetInputInfoByDisp(0,SLR_INPUT_V_FREQ) < 245))
			{
				...
				// x = 2700
				if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_8K4K)
					ppoverlay_frc_to_frame_sync_den_end_ctrl_reg.dv_den_cnt_target = Get_DISP_DEN_END_VPOS() + 1000;
				else
			...
			else if((Scaler_DispGetInputInfoByDisp(0,SLR_INPUT_V_FREQ) > 245) && (Scaler_DispGetInputInfoByDisp(0,SLR_INPUT_V_FREQ) < 255))
			{
				...
				// x = 2616
				if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_8K4K)
					ppoverlay_frc_to_frame_sync_den_end_ctrl_reg.dv_den_cnt_target = Get_DISP_DEN_END_VPOS() + 810;
				else
...
#define VODMA_1_PIXEL_24HZ_OFFSET 6
...
void drvif_scaler_set_Vodma_Delay_Target(unsigned char bEnable)
{
	...
	if(bEnable){
		...
		u32_delay_target = (((dVtotal * (mDtg_mul-1) + frame_sync_den_target)*dHTotal*dclk_frac_a)/(dclk_frac_b*b_2pixel_mode*uzuHTotal)) - ((mDtg_rem-1)*uzuVTotal);
#if 0
...
#endif
		rtd_pr_vbe_notice("u32_delay_target:%d, frame_sync_den_target:%d,frac_a:%d,frac_b:%d,2px=%d, mDtg_rem=%d, mDtg_mul=%d, pnlVTotal=%d, dVtotal=%d, dHTotal=%d, uzuVTotal=%d, uzuHTotal=%d\n",
			u32_delay_target, frame_sync_den_target, dclk_frac_a, dclk_frac_b, b_2pixel_mode, mDtg_rem, mDtg_mul, pnlVTotal, dVtotal, dHTotal, uzuVTotal, uzuHTotal);
	}else{
		u32_delay_target = 0;
	}
...
void drvif_scaler_set_Vodma_reset_target_line_count(void)
{
	...
	rtd_pr_vbe_notice("[voreset] frac_a:%d, frac_b:%d, b_2pixel_mode:%d, frame_sync_den_target:%d \n", frac_a, frac_b,b_2pixel_mode,frame_sync_den_target);
	rtd_pr_vbe_notice("[voreset] offset:%d, reset target:%d \n", offset, reset_target);
}
...
void drvif_scaler_run_Reset_VODMA(void)
{
	...
	while(Scaler_Get_Reset_VODMA_Flag())
    {
        ...
        if(TimeOutCounter > TimeOutCounterThreshold)
        {
            rtd_pr_vbe_emerg("VODMA reset flow wait timeout......\n");
            break;
        }
    }
...
#if 0	//move to vo vsync reset done
	//enable timing fsync
	display_timing_ctrl1_reg.regValue = IoReg_Read32(PPOVERLAY_Display_Timing_CTRL1_reg);
	display_timing_ctrl1_reg.disp_fsync_en = _ENABLE;
	display_timing_ctrl1_reg.disp_fix_last_line_new= 0;
...
void drvif_scaler_set_display_mode_frc2fsync_by_I2D_CTRL(void)
{
...
#endif
    {
        ...
        rtd_pr_vsc_notice("drvif_scaler_set_display_mode_frc2fsync_by_I2D_CTRL = %x \n", IoReg_Read32(PPOVERLAY_I2D_CTRL_1_reg));
        ...
        IoReg_Write32(PPOVERLAY_I2D_CTRL_0_reg, ppoverlay_i2d_ctrl_0_reg.regValue);
        // rtd_pr_vsc_notice("PPOVERLAY_I2D_CTRL_0_reg = %x \n", IoReg_Read32(PPOVERLAY_I2D_CTRL_0_reg));
        ...
        while(ppoverlay_i2d_ctrl_0_reg.i2d_special_dly_en == 1)
        {
			...
			{
				rtd_pr_vbe_notice("gamemode rerun scaler, break\n");
				//Disable I2D
				ppoverlay_i2d_ctrl_0_reg.regValue = IoReg_Read32(PPOVERLAY_I2D_CTRL_0_reg);
				ppoverlay_i2d_ctrl_0_reg.i2d_special_dly_en = 0;
				IoReg_Write32(PPOVERLAY_I2D_CTRL_0_reg, ppoverlay_i2d_ctrl_0_reg.regValue);
...
unsigned char allocate_memory_for_8k_platform(void)
{//retunr TRUE memory is i3ddma:8k mdomain 4k+4k  retunr  false: m domain 2k the same with merlin7
	if((get_platform_model() == PLATFORM_MODEL_8K)&& (get_display_resolution()==DISPLAY_RESOLUTION_8K)) {
		return TRUE;//4k memory size
    }
	return FALSE;
}

bool is_caveout_memory_buffer_source(SCALER_DISP_CHANNEL display)
{//if true: main m domain use carveout size. using decide scaling down or not
	...
	if(allocate_memory_for_8k_platform()) {
		if (display == SLR_MAIN_DISPLAY) {
			if ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)
				&& ( ((scaler_vsc_get_gamemode_go_datafrc_mode() == TRUE) && (is_game_mode_set_line_buffer() == false)&& (Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID) > 4096)
				&& (Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN) > M_DOMAIN_LEN_CARVEOUT_4K))
					|| (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())
					|| ((Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID) > 4096)
						&& (Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN) > M_DOMAIN_LEN_CARVEOUT_4K)
						&& (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 990))//input size > 4K & HFR
					))
				ret= false;
		}
		...
		if ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)
			&& ((get_hdmi_4k_hfr_mode() != HDMI_NON_4K120)
				||  ((scaler_vsc_get_gamemode_go_datafrc_mode() == TRUE) && (is_game_mode_set_line_buffer() == false)&& (Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID) > M_DOMAIN_WIDTH_CARVEOUT_2K)
...
//frank@02012013 Change below code to solve set sub display window BUG --
...
#endif
	{
		if ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && get_hdmi_vrr_4k60_mode()) {
		...
		else if ((((display == SLR_MAIN_DISPLAY) && (Scaler_get_data_framesync(display) == FALSE) && (is_caveout_memory_buffer_source(display))) ||(display == SLR_SUB_DISPLAY)))
		{////decide mdoamin size to caveout size
			...
			if (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_FRAMESYNC) == FALSE) {
				if ((Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DISP_WID)> carveout_width)
						&&(Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID)>carveout_width)) {
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_CAP_WID,carveout_width);
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_MEM_ACT_WID, carveout_width);
				}
				if ((Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DISP_LEN)> carveout_len)
					&&(Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_LEN)>carveout_len)) {
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_CAP_LEN,carveout_len);
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_MEM_ACT_LEN, carveout_len);
				}
			}
#endif
		}
	...
	if((display == SLR_MAIN_DISPLAY) && ((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) || (get_hdmi_4k_hfr_mode() == HDMI_4K120_2_0) ||(get_hdmi_vrr_4k60_mode())))
...
//	tRPCShareMem->ulAddr[EMF_COEF_MEM_IDX] = g_atShareMemInfo[SCALERIOC_SETEMF].ulOffset + g_ulPhyAddr + 0xa0000000;
	...
	tRPCShareMem->ulAddr[SCALER_BSP_ERROR_REPORT_INFO_BUFFER_IDX] = g_atShareMemInfo[SCALERIOC_BSP_ERROR_REPORT_INFO_BUFFER].ulOffset + g_ulPhyAddr + 0xa0000000;////bsp error share memory
	...
	tRPCShareMem->ulAddr[VO_TRACK_I3_ENABLE_IDX] = g_atShareMemInfo[SCALERIOC_VO_TRACK_I3_ENABLE].ulOffset + g_ulPhyAddr + 0xa0000000;
	tRPCShareMem->ulAddr[I3_NN_SHARE_BUF_STATUS_IDX] = g_atShareMemInfo[SCALERIOC_I3_NN_SHARE_BUF_STATUS].ulOffset + g_ulPhyAddr + 0xa0000000;
	tRPCShareMem->ulAddr[I3DDMA_VODMA_COMP_ATTR_IDX] = g_atShareMemInfo[SCALERIOC_SET_I3DDMA_VODMA_COMP_ATTR].ulOffset + g_ulPhyAddr + 0xa0000000;
	tRPCShareMem->ulAddr[SCALERIOC_GAMEMODE_DATA_IDX] = g_atShareMemInfo[SCALERIOC_GAMEMODE_DATA].ulOffset + g_ulPhyAddr + 0xa0000000;

	// HDMITX
	tRPCShareMem->ulAddr[TX_TIMING_READY_IDX] = g_atShareMemInfo[HDMITX_TIMING_READY].ulOffset + g_ulPhyAddr + 0xa0000000;
	tRPCShareMem->ulAddr[VO_SET_GATING_IDX] = g_atShareMemInfo[VO_SET_GATING_READY].ulOffset + g_ulPhyAddr + 0xa0000000;
	tRPCShareMem->ulAddr[VO_IV_CHANGE] = g_atShareMemInfo[IV_SRC_SEL_READY].ulOffset + g_ulPhyAddr + 0xa0000000;
	tRPCShareMem->ulAddr[VO_ENABLE_GATING_IDX] = g_atShareMemInfo[VO_ENABLE_GATING].ulOffset + g_ulPhyAddr + 0xa0000000;
...
//#endif
		...
		/* === HDMITX ========== */
		SetRPCShareMem(HDMITX_TIMING_READY, sizeof(unsigned int));
		SetRPCShareMem(VO_SET_GATING_READY, sizeof(unsigned int));
		SetRPCShareMem(IV_SRC_SEL_READY, sizeof(unsigned int));
		SetRPCShareMem(VO_ENABLE_GATING, sizeof(unsigned int));
		/* === HDMITX End ========== */
...
void scaler_Rotate_VI_init_send_rpc(void)
{
	...
	rotate_data->rotatemode =  (unsigned int)get_rotate_mode(SLR_MAIN_DISPLAY);//rotate mode
...
unsigned char get_rotate_hflip_flag(unsigned char display)
{
...
void Scaler_Set_Cinema_Mode(unsigned int cinema_mode_enable, unsigned int run_vo_cinema)
{
	...
	//rtd_pr_vbe_emerg("Scaler_Set_Cinema_Mode %d/%d !!!\n", cinema_mode_enable, run_vo_cinema);
...
void modestate_decide_dtg_m_mode(void)
{
	...
	else if((Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_FRC_LG_8K60HZ) && (Get_Dynamic_Panel_Output_Timing_Mode() == OUTPUT_8K4K30HZ)){
		Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 30);	//FRC application photo source output 8K30Hz
		rtd_pr_vbe_notice("[FRC_LG_8K60HZ]output=%d \n", Scaler_DispGetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ));
		return;
	}
...
void modestate_decide_dtg_m_modeByDisp(unsigned char _channel)
{
	...
	if((Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI)){
		...
		Scaler_DispSetInputInfoByDisp(channel,SLR_INPUT_DTG_MASTER_V_FREQ, 30);	//FRC application photo source output 8K30Hz
		rtd_pr_vbe_notice("[FRC_LG_8K60HZ]output=%d \n", Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_DTG_MASTER_V_FREQ));
		return;
	}
...
void modestate_decide_fractional_framesync(void)
{
	...
	if((Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_FRC_LG_8K60HZ) && (Get_Dynamic_Panel_Output_Timing_Mode() == OUTPUT_8K4K30HZ)){
		Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_REMOVE_RATIO, 1);
		Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_MULTIPLE_RATIO, 1);
		rtd_pr_vbe_notice("[FRC_LG_8K60HZ] Photo force fractional 1:1 \n");
	}
	else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 235) &&
		(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 245))	//24Hz
	{
		if(Get_DISPLAY_REFRESH_RATE() >= 120){
			...
			(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) <= 605))	//60hz
	{
		...
		if(Get_DISPLAY_REFRESH_RATE() >= 120){
			if(((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= 995) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) <= 1005))
				...
				rtd_pr_vbe_debug("1/1, 1pixel (input 100Hz/120Hz) \n");
				Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_REMOVE_RATIO, 1);
				Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_MULTIPLE_RATIO, 1);
			}
			else{
...
void modestate_decide_fractional_framesyncByDisp(unsigned char _channel)
{
	...
	else if((Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) > 235) &&
			(Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 245))	//24Hz
	{
		if(Get_DISPLAY_REFRESH_RATE() >= 120){
			rtd_pr_vbe_debug("10/2, 2pixel \n");
...
#define HTOTAL_MAPPING_MEMC_CLOCK_INCREASE_24Hz 5500
#define HTOTAL_MAPPING_MEMC_CLOCK_INCREASE_25Hz 5280
...
//#endif
	...
	if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_8K4K){
		if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ)>235)&&(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ)<245)){
			...
			rtd_pr_vbe_notice("Output 8K and input 24HZ, Hack htotal as %d.\n", HTOTAL_MAPPING_MEMC_CLOCK_INCREASE_24Hz*2);
			uzudtg_dh_total_reg.uzudtg_dh_total = (HTOTAL_MAPPING_MEMC_CLOCK_INCREASE_24Hz*2-1);
			uzudtg_dh_total_reg.uzudtg_dh_total_last_line = (HTOTAL_MAPPING_MEMC_CLOCK_INCREASE_24Hz*2-1);
		}
		else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ)>=245)&&(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ)<255)){
			//In order to keep framerate not change, we will make each cost time of vertical line is the same.
			// (1/240M) * 4400 = (1/300M) * 5500
			rtd_pr_vbe_notice("Output 8K and input 25HZ, Hack htotal as %d.\n", HTOTAL_MAPPING_MEMC_CLOCK_INCREASE_25Hz*2);
			uzudtg_dh_total_reg.uzudtg_dh_total = (HTOTAL_MAPPING_MEMC_CLOCK_INCREASE_25Hz*2-1);
			uzudtg_dh_total_reg.uzudtg_dh_total_last_line = (HTOTAL_MAPPING_MEMC_CLOCK_INCREASE_25Hz*2-1);
		}
	}
	else{
		if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ)>235)&&(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ)<245)){
			//In order to keep framerate not change, we will make each cost time of vertical line is the same.
			// (1/240M) * 4400 = (1/300M) * 5500
			rtd_pr_vbe_notice("Output 4K and input 24HZ, Hack htotal as %d.\n", HTOTAL_MAPPING_MEMC_CLOCK_INCREASE_24Hz);
			uzudtg_dh_total_reg.uzudtg_dh_total = (HTOTAL_MAPPING_MEMC_CLOCK_INCREASE_24Hz-1);
			uzudtg_dh_total_reg.uzudtg_dh_total_last_line = (HTOTAL_MAPPING_MEMC_CLOCK_INCREASE_24Hz-1);
		}
		else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ)>=245)&&(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ)<255)){
			//In order to keep framerate not change, we will make each cost time of vertical line is the same.
			// (1/240M) * 4400 = (1/300M) * 5500
			rtd_pr_vbe_notice("Output 4K and input 25HZ, Hack htotal as %d.\n", HTOTAL_MAPPING_MEMC_CLOCK_INCREASE_25Hz);
			uzudtg_dh_total_reg.uzudtg_dh_total = (HTOTAL_MAPPING_MEMC_CLOCK_INCREASE_25Hz-1);
			uzudtg_dh_total_reg.uzudtg_dh_total_last_line = (HTOTAL_MAPPING_MEMC_CLOCK_INCREASE_25Hz-1);
		}
	}
...
//#endif
	...
	if((Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_FRC_LG_8K60HZ) && (Get_Dynamic_Panel_Output_Timing_Mode() == OUTPUT_8K4K30HZ))
		uzudtg_countrol_reg.uzudtg_h2x_mode = 1;//output 8K30Hz, need set h2x_mode
	else
		uzudtg_countrol_reg.uzudtg_h2x_mode = 0;
...
//#else // memcdtg start position should sync to MEMC HW
		if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_8K4K){
			if((Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_FRC_LG_8K60HZ) && Get_Dynamic_Panel_Output_Timing_Mode() == OUTPUT_4K2K120HZ){
...
//#endif
	...
	if((Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_FRC_LG_8K60HZ) && (Get_Dynamic_Panel_Output_Timing_Mode() == OUTPUT_8K4K30HZ))
		memcdtg_control_reg.memcdtg_h2x_mode = 1;//output 8K30Hz, need set h2x_mode
	else
		memcdtg_control_reg.memcdtg_h2x_mode = 0;
	if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI_MEMC_FREERUN){
		memcdtg_control_reg.memcdtg_line_cnt_sync = 0;
		memcdtg_control_reg.memcdtg_fsync_en =0;
		//memcdtg_control_reg.memcdtg_free_vh_total_en = 1;//mark2 removed
	}else
	...
	ppoverlay_dctl_iv2dv_ihs_ctrl_reg.iv2dv_ih_slf_width = Scaler_DispGetInputInfo(SLR_INPUT_H_LEN) / 2;	//2pixel mode divide 2
...
unsigned char drivf_i3ddma_config(unsigned char display, unsigned char src)
{
	...
	if(src != VGIP_SRC_HDMI1 &&  src != VGIP_SRC_HDMI2 && src != VGIP_SRC_ADC && src != VGIP_SRC_ATV  && src != VGIP_SRC_HDR1)
	{
...
// timing dectect done
...
#ifndef CONFIG_RTK_KDRV_DV_IDK_DUMP
	if((get_i3ddma_4k120_flag() == TRUE) || get_nn_force_i3ddma_enable())

#else
	if (1)
#endif
	{
		rtd_pr_vbe_emerg("enable 4k120 or 4k120 enable nn flag\n");
	...
	else
	{
        ...
        if(get_vsc_run_pc_mode() && ((src == VGIP_SRC_HDMI1) || (src == VGIP_SRC_HDMI2) || (src == VGIP_SRC_ADC))){
            rtd_pr_vbe_notice("[%s][%d] i3ddma COMPRESSION_20_BITS. \n",__FUNCTION__,__LINE__);
		...
		if((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE) <= 1920) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) <= 1080)){
			dvrif_i3ddma_set_compression_bits(COMPRESSION_24_BITS);
		}
		if((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE) <= 3840) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) <= 2160)){
			if(get_vsc_run_pc_mode()||drv_memory_get_game_mode()||DbgSclrFlgTkr.hdmi_dolby_vision_flag){
				dvrif_i3ddma_set_compression_bits(COMPRESSION_20_BITS);
			}else
				dvrif_i3ddma_set_compression_bits(COMPRESSION_16_BITS);
		}else{//8k
			if(get_vsc_run_pc_mode() && ((src == VGIP_SRC_HDMI1) || (src == VGIP_SRC_HDMI2))){
				dvrif_i3ddma_set_compression_bits(COMPRESSION_20_BITS);
			}else if(get_vsc_run_pc_mode()||drv_memory_get_game_mode()||DbgSclrFlgTkr.hdmi_dolby_vision_flag){
				dvrif_i3ddma_set_compression_bits(COMPRESSION_12_BITS);
			}
			else{
				dvrif_i3ddma_set_compression_bits(COMPRESSION_10_BITS);
			}
		}

		if(allocate_memory_for_8k_platform()) {
			if ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE) <= M_DOMAIN_WIDTH_CARVEOUT_4K) &&
					(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) <= M_DOMAIN_LEN_CARVEOUT_4K) &&
			...
			if ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE) <= M_DOMAIN_WIDTH_CARVEOUT_2K) &&
					(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) <= M_DOMAIN_LEN_CARVEOUT_2K) &&
					(get_current_driver_pattern(DRIVER_FREERUN_PATTERN) == 0) && (Get_PANEL_VFLIP_ENABLE() == 0))
	        	dvrif_i3ddma_set_compression_mode(I3DDMA_COMP_FRAME_MODE);
			else
	        dvrif_i3ddma_set_compression_mode(I3DDMA_COMP_LINE_MODE);
    ...
    && ((I3DDMA_COLOR_DEPTH_T) Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_COLOR_DEPTH) == I3DDMA_COLOR_DEPTH_8B))
{
	...
	if((get_i3ddma_4k120_flag() == TRUE) || (get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1))
	{
	...
	//drvif_I3DDMA_Set_Input_Info_Vaule(I3DDMA_INFO_FRAMERATE_X2, true);
	if((src == VGIP_SRC_HDMI1) || (src == VGIP_SRC_HDMI2) || (src == VGIP_SRC_HDR1))
		mode = VGIP_MODE_DIGITAL;
	else
		mode = VGIP_MODE_ANALOG;
...
int drvif_i3ddma_triplebuf_flag(void)
{
	...
	if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)
	{
		...
		{//120 panel
			...
			{
				//HDMI CTS framerate range : -0.6%~+0.5
				if(((Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) >= 994) && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) <= 1005)) ||
					((Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) >= 1191) && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) <= 1206))||
					((Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) >= 1435) && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) <= 1445)))
				{
					return FALSE;//go 100 or 120 hz
				}
				else if((Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) > 601) && (!((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag()))))//60.2Hz~99.4Hz //to be compatible with OLED panel
				{
					if(vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag()){
				...
				else if((Get_DISPLAY_PANEL_OLED_TYPE() == TRUE) && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) > 240) && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) <= 245))//to be compatible with OLED panel
				{
					rtd_pr_vsc_info("[OLED] input frame bigger than 24, VO force to 24hz\n");
					return TRUE;//i domain go 24
				}
				else if((Get_DISPLAY_PANEL_OLED_TYPE() == TRUE) && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) > 300) && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) <= 305))//to be compatible with OLED panel
				{
		...
		else
		{
			if( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE) <= HDMI_TO_VO_FRC_60_MAX_WIDTH_60_PANEL
				&& Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) <= HDMI_TO_VO_FRC_60_MAX_LENTH_60_PANEL )//60.2Hz~99.4Hz //to be compatible with OLED panel
			{
				if((Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) > 601) && (!((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag()))))//60.2Hz~99.4Hz //to be compatible with OLED panel
				{
					if(vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag()){
						rtd_pr_vsc_info("vrr input frame big than 60, dont use triple buffer.\n");
						return FALSE;
...
//#ifdef CONFIG_MEMC_BYPASS
			if(get_MEMC_bypass_status_refer_platform_model() == TRUE){
            //else if(Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ) <= 310 && (get_current_driver_pattern(DRIVER_FREERUN_PATTERN)))
...
unsigned char get_vsc_connect_ready(unsigned char display)
{//when vsc conenct finish. we set main_vsc_connect_ready is true
	if(display == SLR_MAIN_DISPLAY)
	{
		return main_vsc_connect_ready;
	}
#ifdef CONFIG_DUAL_CHANNEL
	else
	{
		return sub_vsc_connect_ready;
	}
#endif
	return main_vsc_connect_ready;
}
...
void hdmi_vtop_hfr_control(void)
{
	...
	if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)
	{
		...
		{//4k2k input
			...
			{
				dm_input_format_reg.dm_in_sel = 3;//2pixel 444
			}
			else
			{
...
void scaler_hdr_mode_setting(unsigned char display, unsigned char hdrtype)
{
	...
	if(display == SLR_MAIN_DISPLAY)
	{
		switch(hdrtype)
		{
			...
			case HDR_DM_SDR_MAX_RGB:
				...
				Scaler_LGE_HDR10_Enable_newflow(display, 2/*BT2020*/);
				drvif_DM2_OETF_EOTF_Path_Sel(1);
				drvif_DM2_B0203_noSram_Enable(1, 0);
				fwif_color_set_HDR_Dolby_mode_Enable(display, hdrtype);
				drvif_color_set_BBC_Enable(0);
				fwif_color_set_HDR_TV006();
				Scaler_Set_HDR_YUV2RGB(display, HAL_VPQ_HDR_MODE_SDR_MAX_RGB, HAL_VPQ_COLORIMETRY_MAX);
				drvif_color_HDR10_DM_setting();
				...
				fwif_color_set_HDR10_runmode(display);
			...
			case HDR_DM_HDR10:
				...
				fwif_color_set_HDR_Dolby_mode_Enable(display, hdrtype);
				fwif_color_set_HDR_TV006();
				Scaler_Set_HDR_YUV2RGB(display, HAL_VPQ_HDR_MODE_HDR10, HAL_VPQ_COLORIMETRY_BT2020);
				drvif_color_HDR10_DM_setting();
				fwif_color_set_HDR10_runmode(display);
				drvif_color_set_HDR_RGB_swap(1);
				fwif_color_TC_Flow(0);
			...
			case HDR_DM_HLG:
				...
				drvif_color_set_BBC_Enable(1);
				fwif_color_set_HDR_TV006();
				Scaler_Set_HDR_YUV2RGB(display, HAL_VPQ_HDR_MODE_HLG, HAL_VPQ_COLORIMETRY_BT2020);
				drvif_color_HDR10_DM_setting();
				fwif_color_set_HDR10_runmode(display);
				drvif_color_set_HDR_RGB_swap(1);
				fwif_color_TC_Flow(0);
			...
			case HDR_DM_TECHNICOLOR:
				/* enable HDR10*/
				hdmi_vtop_hfr_control();//for two pixel mode setting
				Scaler_LGE_HDR10_Enable_newflow(display, 1);
				drvif_DM2_OETF_EOTF_Path_Sel(1);
				drvif_DM2_B0203_noSram_Enable(1, 0);
				fwif_color_set_HDR_Dolby_mode_Enable(display, hdrtype);
				drvif_color_set_BBC_Enable(0);
			...
			case HDR_DM_ST2094:
				...
				fwif_color_set_HDR_TV006();
				Scaler_Set_HDR_YUV2RGB(display, HAL_VPQ_HDR_MODE_ST2094, HAL_VPQ_COLORIMETRY_BT2020);
				drvif_color_HDR10_DM_setting();
				fwif_color_set_HDR10_runmode(display);
				drvif_color_set_HDR_RGB_swap(1);
				fwif_color_TC_Flow(0);
	...
	else
	{
		switch(hdrtype)
		{
			case HDR_DM_MODE_NONE:
			{
				Scaler_LGE_HDR10_Enable_newflow(display, 0/*BT2020*/);
			...
			case HDR_DM_SDR_MAX_RGB:
				// HDR1_2, HDR2
				Scaler_LGE_HDR10_Enable_newflow(display, 2/*BT2020*/);
				drvif_DM2_HDR1_2_OETF_EOTF_Path_Sel(1);
...
unsigned char driver_timinginfo_check(unsigned int input_width,unsigned int input_length,unsigned int h_freq,
										unsigned int v_freq,unsigned int h_total,unsigned int v_total)
{
	if(input_width == 0)
	{
		return FALSE;
...
INPUT_TIMING_INDEX get_current_pattern_index(void)
{
	...
	else if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)
	{
		...
		extern unsigned char Hdmi_GetRawTimingInfo_for_hdmi_ptg(HDMI_TIMING_T* timing_info);
#endif
		...
		rtd_pr_vbe_debug("[driver config]hdmi current index=%d\n",index);
		return index;
	}
	else if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC)
	{
...
unsigned int get_current_driver_pattern(DRIVER_LIST_INFO info)
{
	...
	switch(info)
	{
		...
		case DRIVER_FREERUN_PATTERN:
			...
			{				
				if (allocate_memory_for_8k_platform()) { // 4k timing go data frc for 8k resolution 
...
{
#ifndef CONFIG_ENABLE_MARK2_ZEBU_BRING_UP  // skip dump this message in ZEBU debug platform to speed up system boot up speed
	INPUT_TIMING_INDEX i = HDMI_2K1K_30;
	char *timing_string[] = {
		"HDMI_2K1K_30",
		"HDMI_2K1K_60",
	...
	rtd_pr_vbe_emerg("**************************************************\n");
	//driver_config_data_to_video();
#endif // #ifndef CONFIG_ENABLE_MARK2_ZEBU_BRING_UP)  // skip dump this message in ZEBU debug platform to speed up system boot up speed
}
...
{//for 4k 120/ 2 pixel mode
#if 0//no need to control  becasue no 4k120 mode 
	...
	if(enable)
	{
		...
		// vgip_data_path_select_reg.main_2pxl_en = 1; //fixed me. Mark2 no main_2pxl_en
	...
	{
		 //fixed me. Mark2 no main_2pxl_en
		vgip_data_path_select_reg.regValue = IoReg_Read32(VGIP_Data_Path_Select_reg);
...
void get_vtop_input_info(unsigned char *twopixel_mode, unsigned char *hdmi_input, unsigned char *color_format)
{
	...
	*hdmi_input = ((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) || (get_hdmi_4k_hfr_mode() == HDMI_3K120))? TRUE: FALSE;
...
unsigned char force_disp_s1_s2_same_clk(void)
{//TRUE: use stage1, stage2 same dclock   2k 120 case
	if(get_MEMC_bypass_status_refer_platform_model()) {
		if(Get_DISPLAY_REFRESH_RATE() >= 120)
			return TRUE;
	}
...
void scaler_set_full_gatting_rpc(unsigned int enable)
{
	...
	if(1 == enable) { /* wait video set gatting bit done */
		do{
			...
			if(0 == wait_timeoutcnt && !get_vo_change_flag(SLR_MAIN_DISPLAY)) {
...
unsigned char get_hdmi_vgip_source_type(unsigned char connect_port)
{//return VGIP_SRC_HDMI1 or VGIP_SRC_HDMI2
    HDMI_OUTPUT_CHANNEL_T hdm_output_ch = drvif_Hdmi_GetHDMIOutputChannel(connect_port);
    rtd_pr_vbe_err("[HDMI] get_hdmi_vgip_source_type(connect_port=%d), output_channel=%d\n", connect_port, hdm_output_ch);
	return (hdm_output_ch==HDMI_OUTPUT_CHANNEL_2) ? VGIP_SRC_HDMI2 : VGIP_SRC_HDMI1;
}

#ifdef CONFIG_ENABLE_HDMI_PATTERN
void Enable_HDMI_PTG(void)
{//4k 60 yuv444 pattern
#ifdef CONFIG_ENABLE_HDMI_PATTERN_4K
//#include "./i_domain/hdmi/HDMIPLL_594M.tbl"
//***digital start***
//rtd_outl(0x1800d414,0x00000001);//SCLKG_PLL_DIVS=1 (/2)
rtd_outl(0xb800d414,0x00000000);//SCLKG_PLL_DIVS=0 (/1)
rtd_outl(0xb8000310,0x1007002D);//pllhdmi vpll
//rtd_outl(0x18000310,0x10070033);//hdmi_dsc, vpll*2/3=hdmi_dsc when O=0
//***digital end***

//***analog start***
//-------HDMI_DPLL-------------------------
rtd_outl(0xb800d404,0x00061604);//DPLL power off,rst low
#if 0//HDMI 297M
rtd_outl(0xb800d400,0x4a80207f);//M=86,O=0,N=2,27*(86+2)/4=594MHz
#else//HDMI 594M
rtd_outl(0xb800d400,0x15800274);//M=86,O=0,N=2,27*(86+2)/4=594MHz
#endif
//RS=b'111,IP=b'100,CP=0

//toggle DBDCB to let M/N/O setting activate
rtd_outl(0xb800d410,0x00000003);//input clock=crystal cLk, output clock=PLL, double buffer M, N, O code
rtd_outl(0xb800d410,0x00000013);//double buffer=1
//HDMI Rx VPLL setting
//rtd_part_outl(0x1800D404,12,12,0x1);//DPLL_LDO_POW=1
rtd_outl(0xb800d404,0x00061604);//DPLL_LDO_POW=1,CP=0

//rtd_part_outl(0x1800D404,0,0,0x1);//DPLL_POW=1  
rtd_outl(0xb800d404,0x00061605);//DPLL_POW=1
msleep(1);//at least > 100us
//rtd_part_outl(0x1800D404,1,1,0x1);//DPLL_VCORSTB=1
rtd_outl(0xb800d404,0x00061607);//DPLL_VCORSTB=1
rtd_outl(0xb800d404,0x00061617);//DPLL_RSTB=1
msleep(1);//at least > 200us
//rtd_part_outl(0x1800D404,2,2,0);// DPLL_FREEZE = 0
rtd_outl(0xb800d404,0x00061613);//DPLL_FREEZE=0

rtd_part_outl(0xb800d1a8, 3, 3, 1);//hdmi_out_sel = 1, HDMI_PTG

//***analog end***

//***pin setting start***
rtd_outl(0xb80008f0,0x00001000);	//<15:8>testpin_mux=0x10(crt)
rtd_outl(0xb8000818,0xE0F0F0E0);	//measure <31:28>pad_gpio_46_io(test_en_bit0), 
									//measure <7:4>pad_gpio_49_io(test_en_bit3)
//#include "./i_domain/hdmi/HDMI_4K.tbl"

rtd_outl(0xb800D800, 0x113008C9);   // Total pixel = 4400*2250
rtd_outl(0xb800D804, 0x01180F00);   // Horizontal active start = 280, active width = 3840
rtd_outl(0xb800D808, 0x002D0870);   // Vertical active start = 45, active width = 2160
rtd_outl(0xb800D80C, 0x00000000);   // Disable timing generator

// 0x18022110 status read
rtd_outl(0xb800D814, 0x01180F00);   // Pattern Gen display Hstart & Hactive
rtd_outl(0xb800D818, 0x002D0870);   // Pattern Gen display Vstart & Vactive
rtd_outl(0xb800D81C, 0x00000003);   // PTG control, select pattern = veritcal color bar
//rtd_outl(0x1800D81C, 0x00000080);   // PTG control, select pattern = Random generator
//rtd_outl(0x1800D81C, 0x00000004);   // PTG control, select pattern = horizontal color bar
//rtd_outl(0x1800D81C, 0x00000006);   // PTG control, select pattern = horizontal & vertical gradient

rtd_part_outl(0xb800D894, 31, 31, 0x1); // ptg_1pxl_mode=1 for date rate <= 600MHz
rtd_part_outl(0xb800D894, 30, 30, 0x0); // rpt_2pxl_mode=0 for 1pixel mode

rtd_outl(0xb800D820, 0x000001E0);   // Color bar width  =59
//rtd_outl(0x1800D824, 0x00FFFFFF);   // color set 0 W: White
//rtd_outl(0x1800D828, 0x0000FF00);   // color set 1 R: Red
//rtd_outl(0x1800D82C, 0x00FF0000);   // color set 2 G: Green
//rtd_outl(0x1800D830, 0x0000FF00);   // color set 3 B: Blue
//rtd_outl(0x1800D834, 0xFF00FF00);   // color set 4 C: Cyan
//rtd_outl(0x1800D838, 0x00FFFF00);   // color set 5 M: Megnta
//rtd_outl(0x1800D83C, 0xFFFF00AA);   // color set 6 Y: Yellow
//rtd_outl(0x1800D840, 0x00000000);   // color set 7 K: Black
//rtd_outl(0x1800D844, 0x07000fff);   // color set (copy from defualt value)

rtd_outl(0xb800D80c, 0x00000001);   // Enable timing generator
#else//#ifdef CONFIG_ENABLE_HDMI_PATTERN_8k
//#include "./i_domain/hdmi/HDMIPLL_1188M.tbl"
//***digital start***
//rtd_outl(0x1800d414,0x00000001);//SCLKG_PLL_DIVS=1 (/2)
rtd_outl(0xb800d414,0x00000000);//SCLKG_PLL_DIVS=0 (/1)
rtd_outl(0xb8000310,0x1007002D);//pllhdmi vpll
...
//-------HDMI_DPLL-------------------------
rtd_outl(0xb800d404,0x00061604);//DPLL power off,rst low
rtd_outl(0xb800d400,0x15800074);//M=86,O=0,N=2,27*(86+2)/2=1188Mhz
//RS=b'111,IP=b'100,CP=0
...
// /16
//*** pin setting end***
//#include "./i_domain/hdmi/HDMI_8K.tbl"
rtd_outl(0xb800D800, 0x22601193);   // Total pixel = 8800*4500
rtd_outl(0xb800D804, 0x02301E00);   // Horizontal active start = 560, active width = 7680
rtd_outl(0xb800D808, 0x005A10E0);   // Vertical active start = 90, active width = 4320
rtd_outl(0xb800D80C, 0x00000000);   // Disable timing generator

// 0x18022110 status read
rtd_outl(0xb800D814, 0x02301E00);   // Pattern Gen display Hstart & Hactive 
rtd_outl(0xb800D818, 0x005A10E0);   // Pattern Gen display Vstart & Vactive
rtd_outl(0xb800D81C, 0x00000003);   // PTG control, select pattern = veritcal color bar
//rtd_outl(0x1800D81C, 0x00000080);   // PTG control, select pattern = Random generator
//rtd_outl(0x1800D81C, 0x00000004);   // PTG control, select pattern = horizontal color bar
//rtd_outl(0x1800D81C, 0x00000006);   // PTG control, select pattern = horizontal & vertical gradient

rtd_part_outl(0xb800D894, 31, 31, 0x0); // ptg_1pxl_mode 
rtd_part_outl(0xb800D894, 30, 30, 0x1); // rpt_2pxl_mode 


rtd_outl(0xb800D820, 0x000003C0);   // Color bar width  = 960
//rtd_outl(0x1800D824, 0x00FFFFFF);   // color set 0 W: White
//rtd_outl(0x1800D828, 0x0000FF00);   // color set 1 R: Red
//rtd_outl(0x1800D82C, 0x00FF0000);   // color set 2 G: Green
//rtd_outl(0x1800D830, 0x0000FF00);   // color set 3 B: Blue
...
void set_scaler_info_for_hdmi_ptg(void)
{
#ifdef CONFIG_ENABLE_HDMI_PATTERN_4K
	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IVSTARTPOS,(180));
	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IHSTARTPOS, (38));
	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IVHEIGHT, (2160));
	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IHWID, (3840));

	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IVLEN, (2250));
	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IHLEN, (4400));

	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IVCOUNT, (2250));
	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IHCOUNT, (4400));

	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IVFREQ, (60000));
	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IHFREQ, (135000));
#else
	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IVSTARTPOS,(360));
	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IHSTARTPOS, (76));
	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IVHEIGHT, (4320));
	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IHWID, (7680));

	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IVLEN, (4500));
	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IHLEN, (8800));

	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IVCOUNT, (4500));
	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IHCOUNT, (8800));

	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IVFREQ, (60000));
	Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_IHFREQ, (270000));
#endif
}



unsigned char Hdmi_GetRawTimingInfo_for_hdmi_ptg(HDMI_TIMING_T* timing_info)
{
    if(timing_info)
	{//fill info
#ifdef CONFIG_ENABLE_HDMI_PATTERN_4K
		timing_info->v_act_len = 2160;
		timing_info->v_act_sta = 41;
		timing_info->h_act_len = 3840;
		timing_info->h_act_sta = 192;

		timing_info->h_total = 4400;
		timing_info->v_total = 2250;

		timing_info->mode_id = 111;
		timing_info->modetable_index = 9;

		timing_info->polarity = 0;

		timing_info->h_freq = 135000;
		timing_info->v_freq = 60000;
		timing_info->colorspace = COLOR_YUV444;


		timing_info->colordepth = HDMI_COLOR_DEPTH_8B;
		timing_info->colorimetry = 0;

		timing_info->IVSyncPulseCount = 0;

		timing_info->is_interlace = 0;
#else
		timing_info->v_act_len = 4320;
		timing_info->v_act_sta = 82;
		timing_info->h_act_len = 7680;
		timing_info->h_act_sta = 384;

		timing_info->h_total = 8800;
		timing_info->v_total = 4500;
...
unsigned char check_hdmi_enabe_2p_mode(unsigned char hdmi_port)
{//hdmi source to set crt i clcok 2 p mode the control  means 1p clock is the same with 2p clock
	sys_reg_sys_clksel_RBUS sys_reg_sys_clksel_reg;
	sys_reg_sys_clksel_reg.regValue = IoReg_Read32(SYS_REG_SYS_CLKSEL_reg);

	if(get_hdmi_vgip_source_type(hdmi_port) == VGIP_SRC_HDMI1)
	{//hdmi 1
		if(sys_reg_sys_clksel_reg.hdmirx_2p_clk_sel)
			return 0;
		else
			return 1;
	}
	...
	{//hdmi2
		if(sys_reg_sys_clksel_reg.hdmirx2_2p_clk_sel)
			return 0;
		else
			return 1;
	}
...
void ctrl_i_clock_2p_sel(unsigned char path, unsigned char enable)
{//path is dma vgip or main vgip or sub vgip. enable is enable or not
	...
	{
		case _DAM_VGIP_PATH:
			sys_reg_sys_dispclksel_reg.i3_srcpll2p_en = enable;
			IoReg_Write32(SYS_REG_SYS_DISPCLKSEL_reg, sys_reg_sys_dispclksel_reg.regValue);
			break;

		case _MAIN_VGIP_PATH:
			sys_reg_sys_dispclksel_reg.i1_srcpll2p_en = enable;
			IoReg_Write32(SYS_REG_SYS_DISPCLKSEL_reg, sys_reg_sys_dispclksel_reg.regValue);
			break;

		case _SUB_VGIP_PATH:
			sys_reg_sys_dispclksel_reg.i2_srcpll2p_en = enable;
			IoReg_Write32(SYS_REG_SYS_DISPCLKSEL_reg, sys_reg_sys_dispclksel_reg.regValue);
			break;
...
unsigned char watermark_payload_type = WATERMARK_PAYLOAD_NONE;//payload result

unsigned int watermark_payload_result[15] = {0};//payload result 1x use 8  2x use 15


static WATERMARK_MIDPOINT midpointList[4] = {0};//max 4 value and number 1x:2 value  2x: 4 value
static DEFINE_SPINLOCK(watermark_status_ctrl_spinlock);/*control watermark_status*/

unsigned char watermark_status = WATERMARK_IDLE;//record water mark running status;

void ctrl_watermark_status(unsigned char status)
{//change water mark driver status
	unsigned long flags;//for spin_lock_irqsave
	spin_lock_irqsave(&watermark_status_ctrl_spinlock, flags);
	switch(status)
	{
		case WATERMARK_IDLE:
			watermark_status = WATERMARK_IDLE;
			break;

		case WATERMARK_INIT:
			if(watermark_status == WATERMARK_IDLE)
				watermark_status = WATERMARK_INIT;
			break;

		case WATERMARK_COLLECT_2ND_PIXEL:
			if(watermark_status == WATERMARK_INIT)
				watermark_status = WATERMARK_COLLECT_2ND_PIXEL;
			break;

		case WATERMARK_DETECT:
			if(watermark_status == WATERMARK_DETECT)
				watermark_status = WATERMARK_DETECT;
			break;
		
	}
	spin_unlock_irqrestore(&watermark_status_ctrl_spinlock, flags);
}

unsigned char get_watermark_status(void)
{//get water mark driver status
	return watermark_status;
}


void calculate_set_midpointForWatermark(void) 
{//vote to get the midpoint
	unsigned char i = 0, j = 0, k = 0;
	unsigned char secondPixelList[16] = {0};// copy fro4 register 
	unsigned char midpoint_vote_array[256] = {0};//for vote calculate
	unsigned char isExist = 0;//current vlaue is already max 4 number 
	
	unsigned int regValue;

	// there are 16-value distribute in 4-register
	for(i = 0; i < 4; i++) {
		regValue = IoReg_Read32(WATERMARK_WATERMARK_RUN_IN_TO_SW_1_to_4_reg+(i*4));

		secondPixelList[j++] = (regValue >> 24) & 0xFF;
		secondPixelList[j++] = (regValue >> 16) & 0xFF;
		secondPixelList[j++] = (regValue >> 8) & 0xFF;
		secondPixelList[j++] = (regValue) & 0xFF;
	}

	for(i = 0; i < 16; i++) {
		midpoint_vote_array[secondPixelList[i]]++;

		isExist = 0;
		for(k = 0; k < 4; k++) {
			if(midpointList[k].value == secondPixelList[i]) {
				midpointList[k].num++;
				isExist = 1;
				break;
			}
		...
		if(isExist) {
			//it already exist the list, just sort to keep [0] is minimum
			for(j = k; j < 3; j++) {
				if(midpointList[j].num > midpointList[j+1].num) {
					WATERMARK_MIDPOINT temp;
					temp.value = midpointList[j+1].value;
					temp.num = midpointList[j+1].num;
					midpointList[j+1].value = midpointList[j].value;
					midpointList[j+1].num = midpointList[j].num;
					midpointList[j].value = temp.value;
					midpointList[j].num = temp.num;
				}

			}
		}
		else {
			// it will keep [0] as the minimun value of 4 midpoints
			if(midpoint_vote_array[secondPixelList[i]] > midpointList[0].num) {
				midpointList[0].num = midpoint_vote_array[secondPixelList[i]];
				midpointList[0].value = secondPixelList[i];
...
void set_midpointForWatermark(void) 
{//set the midpoint to register
	watermark_watermark_1x_RBUS watermark_watermark_1x_reg;
	watermark_watermark_2x_RBUS watermark_watermark_2x_reg;
	watermark_watermark_2x_cont_RBUS watermark_watermark_2x_cont_reg;
	watermark_watermark_slice_setting_RBUS watermark_watermark_slice_setting_reg;

	// set 1X midpoint and threshold
	watermark_watermark_1x_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_1X_reg);
	watermark_watermark_1x_reg.midpoint_1x_0 = midpointList[3].value;
	watermark_watermark_1x_reg.midpoint_1x_1 = midpointList[2].value;
	watermark_watermark_1x_reg.threshold_1x_0 = 3; //DIC suggest
	watermark_watermark_1x_reg.threshold_1x_1 = 3; //DIC suggest
	IoReg_Write32(WATERMARK_WATERMARK_1X_reg, watermark_watermark_1x_reg.regValue);

	// set 2X midpoint and threshold
	watermark_watermark_2x_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_2X_reg);
	watermark_watermark_2x_reg.midpoint_2x_00 = midpointList[3].value;
	watermark_watermark_2x_reg.midpoint_2x_01 = midpointList[2].value;
	watermark_watermark_2x_reg.midpoint_2x_10 = midpointList[1].value;
	watermark_watermark_2x_reg.midpoint_2x_11 = midpointList[0].value;
	IoReg_Write32(WATERMARK_WATERMARK_2X_reg, watermark_watermark_2x_reg.regValue);

	watermark_watermark_2x_cont_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_2X_CONT_reg);
	watermark_watermark_2x_cont_reg.threshold_2x_00 = 3; //DIC suggest
	watermark_watermark_2x_cont_reg.threshold_2x_01 = 3; //DIC suggest
	watermark_watermark_2x_cont_reg.threshold_2x_10 = 3; //DIC suggest
	watermark_watermark_2x_cont_reg.threshold_2x_11 = 3; //DIC suggest
	IoReg_Write32(WATERMARK_WATERMARK_2X_CONT_reg, watermark_watermark_2x_cont_reg.regValue);

	watermark_watermark_slice_setting_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_SLICE_SETTING_reg);
	watermark_watermark_slice_setting_reg.slice_point_1x_0_1 = (midpointList[3].value + midpointList[2].value) / 2;
	// fix me, it need to depend on the color depth of source to set the threshold
	// reference to the table from Watermark_design_spec.ppt
	watermark_watermark_slice_setting_reg.slice_point_2x_00_01 = (16+89)/2;
	watermark_watermark_slice_setting_reg.slice_point_2x_01_10 = (89+162)/2;
	watermark_watermark_slice_setting_reg.slice_point_2x_10_11 = (162+235)/2;
	IoReg_Write32(WATERMARK_WATERMARK_SLICE_SETTING_reg, watermark_watermark_slice_setting_reg.regValue);

}



void drvif_watermark_initial(unsigned char display) 
{//water mark intitial setting
	unsigned int wid = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_IPH_ACT_WID);
	watermark_watermark_setting_1_RBUS watermark_watermark_setting_1_reg;
	watermark_watermark_setting_2_RBUS watermark_watermark_setting_2_reg;

	if(get_watermark_status() != WATERMARK_IDLE)
	{
		rtd_pr_vsc_err("%s fail status[%d].\n", __FUNCTION__, get_watermark_status());
		return;
	}
	watermark_watermark_setting_1_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_SETTING_1_reg);
	watermark_watermark_setting_1_reg.watermark_src_sel = 1; //0: before VGIP, 1: after rgb2yuv
	if(display == SLR_MAIN_DISPLAY)
	{
		if(get_force_i3ddma_enable(display))
			watermark_watermark_setting_1_reg.watermark_path_sel = 2; //0: Main VGIP, 1: Sub VGIP, 2: DMA VGIP //dma vgip
		else
			watermark_watermark_setting_1_reg.watermark_path_sel = 0; //0: Main VGIP, 1: Sub VGIP, 2: DMA VGIP // main vgip 
	}
	else
	{
		watermark_watermark_setting_1_reg.watermark_path_sel = 1; //0: Main VGIP, 1: Sub VGIP, 2: DMA VGIP //sub vgip
	}
	watermark_watermark_setting_1_reg.over_range_threshold = 10; //DIC suggest 10
	IoReg_Write32(WATERMARK_WATERMARK_SETTING_1_reg, watermark_watermark_setting_1_reg.regValue);

	watermark_watermark_setting_2_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_SETTING_2_reg);
	watermark_watermark_setting_2_reg.division_quotient = wid / 240;
	watermark_watermark_setting_2_reg.division_quotient = wid % 240;
	IoReg_Write32(WATERMARK_WATERMARK_SETTING_2_reg, watermark_watermark_setting_2_reg.regValue);
	ctrl_watermark_status(WATERMARK_INIT);

}

void drvif_watermark_start_midpoint(void)
{//water mark start to get midpoint
	watermark_watermark_enable_RBUS watermark_watermark_enable_reg;
	unsigned char timeout = 5;
	if(get_watermark_status() != WATERMARK_INIT)
	{
		rtd_pr_vsc_err("%s fail status[%d].\n", __FUNCTION__, get_watermark_status());
		return;
	}

	//enable 16_symbol_2nd_pxl_en to obtain 16-2nd value
	watermark_watermark_enable_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_ENABLE_reg);
	watermark_watermark_enable_reg.sym16_2nd_pxl_en = 1;
	IoReg_Write32(WATERMARK_WATERMARK_ENABLE_reg, watermark_watermark_enable_reg.regValue);

	// wait until hw_clr
	watermark_watermark_enable_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_ENABLE_reg);
	while((watermark_watermark_enable_reg.sym16_2nd_pxl_en) && timeout) {
		watermark_watermark_enable_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_ENABLE_reg);
		timeout--;
		msleep(10);
	}
	if(timeout == 0) {
		rtd_pr_vsc_err("[Watermark] sym16_2nd_pxl_en enable fail.\n");
		return;
	}
}


void drvif_watermark_detect(void) {
	watermark_watermark_enable_RBUS watermark_watermark_enable_reg;
	watermark_watermark_interrupt_RBUS watermark_watermark_interrupt_reg;
	watermark_watermark_status_RBUS watermark_watermark_verification_status_reg;

	//clear status
	watermark_watermark_verification_status_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_STATUS_reg);
	watermark_watermark_verification_status_reg.watermark_process_done = 1;//clear status
	IoReg_Write32(WATERMARK_WATERMARK_STATUS_reg, watermark_watermark_verification_status_reg.regValue);

	
	//enable watermark detection
	watermark_watermark_enable_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_ENABLE_reg);
	watermark_watermark_enable_reg.watermark_detect_en = 1;//enable detect 
	IoReg_Write32(WATERMARK_WATERMARK_ENABLE_reg, watermark_watermark_enable_reg.regValue);


	//enable for interrupt
	watermark_watermark_interrupt_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_INTERRUPT_reg);
	watermark_watermark_interrupt_reg.watermark_interrupt_en = 0;//need to check 
	IoReg_Write32(WATERMARK_WATERMARK_INTERRUPT_reg, watermark_watermark_interrupt_reg.regValue);
}


void drvif_watermark_get_payload_result(void) 
{
 	unsigned char timeout = 5;
	unsigned int i = 0;
	watermark_watermark_verification_status_RBUS watermark_watermark_verification_status_reg;
	while(!WATERMARK_WATERMARK_STATUS_get_watermark_process_done(IoReg_Read32(WATERMARK_WATERMARK_STATUS_reg)) && timeout)
	{
		msleep(10);
		timeout--;
	}
	if(timeout == 0)
	{
		rtd_pr_vsc_err("[Watermark] drvif_watermark_detect wait timeout.\n");
		return;
	}

	watermark_watermark_verification_status_reg.regValue = IoReg_Read32(WATERMARK_WATERMARK_VERIFICATION_STATUS_reg);
	if(watermark_watermark_verification_status_reg.concentration_1x_pass)
	{//1x payload
		watermark_payload_type = WATERMARK_PAYLOAD_1X;
		for(i= 0 ; i < 8 ; i++)
		{
			watermark_payload_result[i] = IoReg_Read32(WATERMARK_WATERMARK_1X_PAYLOAD_1_reg + i*4);
		}
		
	}
	else if(watermark_watermark_verification_status_reg.concentration_2x_pass)
	{//2x payload
		watermark_payload_type = WATERMARK_PAYLOAD_2X;
		for(i= 0 ; i < 15 ; i++)
		{
			watermark_payload_result[i] = IoReg_Read32(WATERMARK_WATERMARK_2X_PAYLOAD_1_reg + i*4);
		}
	}
	else
	{//no payload
		watermark_payload_type = WATERMARK_PAYLOAD_NONE;
		for(i= 0 ; i < 15 ; i++)
		{
			watermark_payload_result[i] = 0;
		}
	}
	ctrl_watermark_status(WATERMARK_IDLE);
...
//dada access
#define BACKGROUND_DATA_ACCESS_TIMEOUT 8000 //8 second
#define BACKGROUND_DATA_ACCESS_START_CONTER 11 //10 second 11 times//1.5 min
...
{//enable data access
	...
	{//main
		...
		{
			if(background_data_access_start_conter && drvif_wait_timeout_check_by_counter(data_access_basetime, BACKGROUND_DATA_ACCESS_TIMEOUT))//7second
			{
	...
	{//sub
		...
		{
			if(sub_background_data_access_start_conter && drvif_wait_timeout_check_by_counter(sub_data_access_basetime, BACKGROUND_DATA_ACCESS_TIMEOUT))//7second
			{
