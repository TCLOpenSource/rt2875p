...
#include <vo/rtk_vo.h>	//for VODMA_COLOR_YUV444
...
void scalerdisplay_set_main_act_window_byOrbit(UINT16 DispHSta, UINT16 DispHEnd, UINT16 DispVSta, UINT16 DispVEnd)
{
    extern ORBIT_PIXEL_SHIFT_STRUCT Get_Orbit_Shift_Data(void);

    extern ORBIT_PIXEL_SHIFT_STRUCT orbit_move_range[_VBE_PANEL_OREBIT_MODE_MAX];
    extern unsigned char vbe_disp_get_orbit_mode(void);
	...
	ppoverlay_main_active_h_start_end_new_RBUS main_active_h_start_end_new_reg;
    static int pre_shiftx=-1,pre_shifty=-1;
    static int pre_width=-1,pre_height=-1;
    int orbit_enable,orbit_mode,shiftx,shifty,shift_range_x,shift_range_y;
	...
	// Main Active H pos before UZU3RD
	main_active_h_start_end_new_reg.regValue = IoReg_Read32(PPOVERLAY_MAIN_Active_H_Start_End_New_reg);
    orbit_enable = vbe_disp_get_orbit();
    orbit_mode = vbe_disp_get_orbit_mode();
    shiftx = Get_Orbit_Shift_Data().x;
    shifty = Get_Orbit_Shift_Data().y;
    shift_range_x = orbit_move_range[vbe_disp_get_orbit_mode()].x*2;
    shift_range_y = orbit_move_range[vbe_disp_get_orbit_mode()].y*2;

    DispHSta += Get_DISP_ACT_STA_BIOS();
    DispHEnd += Get_DISP_ACT_STA_BIOS();

    if((orbit_enable!=1 || orbit_mode!=1)
        || (DispHSta == 0
        && DispVSta == 0))
    {
        pre_shiftx = -1;
    ...
    if(pre_width != (DispHEnd - DispHSta)
        || pre_height != (DispVEnd - DispVSta))
    {
        pre_shiftx = -1;
        pre_shifty = -1;
    }

    if(pre_shiftx == -1 && pre_shifty == -1)//first time
    {
        pre_width = DispHEnd - DispHSta;
        pre_height = DispVEnd - DispVSta;
        if(DispHSta != 0)
        {
            main_active_h_start_end_reg.mh_act_sta = DispHSta + (shiftx - (shift_range_x/2));
            main_active_h_start_end_reg.mh_act_end = DispHEnd + (shiftx - (shift_range_x/2));
            IoReg_Write32(PPOVERLAY_MAIN_Active_H_Start_End_reg, main_active_h_start_end_reg.regValue);
			main_active_h_start_end_new_reg.mh_act_sta_new = DispHSta + (shiftx - (shift_range_x/2));
			main_active_h_start_end_new_reg.mh_act_end_new = DispHEnd + (shiftx - (shift_range_x/2));
			IoReg_Write32(PPOVERLAY_MAIN_Active_H_Start_End_New_reg, main_active_h_start_end_new_reg.regValue);
            pre_shiftx =shiftx;
        }

        if(DispVSta != 0)
        {
            main_active_v_start_end_reg.mv_act_sta = DispVSta - (shifty - (shift_range_y/2));
            main_active_v_start_end_reg.mv_act_end = DispVEnd - (shifty - (shift_range_y/2));
        ...
        rtd_pr_vbe_warn("[%s] first set main_active_h=%x, main_active_v=%x\n", __FUNCTION__,main_active_h_start_end_reg.regValue,main_active_v_start_end_reg.regValue);
        return;
    }
    if(DispHSta != 0)
    {
        main_active_h_start_end_reg.mh_act_sta = DispHSta + (shiftx - pre_shiftx);
        main_active_h_start_end_reg.mh_act_end = DispHEnd + (shiftx - pre_shiftx);
        IoReg_Write32(PPOVERLAY_MAIN_Active_H_Start_End_reg, main_active_h_start_end_reg.regValue);
        pre_shiftx =shiftx;
    }
    ...
    if(DispHSta != 0)
    {
        main_active_v_start_end_reg.mv_act_sta = DispVSta - (shifty - pre_shifty);
        main_active_v_start_end_reg.mv_act_end = DispVEnd - (shifty - pre_shifty);
        IoReg_Write32(PPOVERLAY_MAIN_Active_V_Start_End_reg, main_active_v_start_end_reg.regValue);
        pre_shifty = shifty;
    }
    ...
    drvif_scalerdisplay_set_dbuffer_en(SLR_MAIN_DISPLAY, FALSE);
    up(get_forcebg_semaphore());/*semaphore must added for not effect mute function when setting b8028300*/
    rtd_pr_vbe_warn("[%s] set main_active_h=%x, main_active_v=%x\n", __FUNCTION__,main_active_h_start_end_reg.regValue,main_active_v_start_end_reg.regValue);
...
void scalerdisplay_set_main_act_window(UINT16 DispHSta, UINT16 DispHEnd, UINT16 DispVSta, UINT16 DispVEnd)
{
	...
	rtd_pr_vbe_debug("[DISP] set main=H=%x/%x, V=%x/%x\n", DispHSta, DispHEnd, DispVSta, DispVEnd);
	...
	// Main Active H pos before UZU3RD
	main_active_h_start_end_new_reg.regValue = IoReg_Read32(PPOVERLAY_MAIN_Active_H_Start_End_New_reg);
	main_active_h_start_end_new_reg.mh_act_sta_new = (DispHSta & 0x1fff);
	main_active_h_start_end_new_reg.mh_act_end_new = (DispHEnd & 0x1fff);
	IoReg_Write32(PPOVERLAY_MAIN_Active_H_Start_End_New_reg, main_active_h_start_end_new_reg.regValue);
...
void scalerdisplay_vo_ratio_change(VO_VIDEO_PLANE plane)
 {
	...
	if(panel_ratio > source_ratio){//Change panel width
	    ...
	    if (input_height == 0 || pVOInfo->pixelAR_ver == 0) {
		rtd_pr_vbe_err("[scaler][error] Change panel width with INVALID VALUE!!!\n");
		return;
	    }
	    ...
	    if (pVOInfo->pixelAR_hor == 0 || (unsigned int)Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID_PRE) == 0) {
		rtd_pr_vbe_err("[scaler][error] Change panel height with INVALID VALUE!!!\n");
		return;
	    }
...
static void scalerdisplay_set_main_ctrl(void)
{
	...
	if(_TRUE == Scaler_Get_AspectRatio_Enter_Normal_Mode())
	{
		...
		rtd_pr_vbe_debug("scalerdisplay_set_main_ctrl, Normal Mode : %x\n", ratio_type);
	}
	...
	if(_TRUE == Scaler_Get_AspectRatio_Enter_DotByDot_Mode())
	{
		...
		rtd_pr_vbe_debug("scalerdisplay_set_main_ctrl, DotByDot Mode : %x\n", ratio_type);
	}
	...
	if(_TRUE == Scaler_Get_AspectRatio_Enter_Native_Mode())
	{
		...
		rtd_pr_vbe_debug("scalerdisplay_set_main_ctrl, Native Mode : %x\n", ratio_type);
	}
#endif //#ifdef CONFIG_ENABLE_NATIVE_DISPLAY_RATIO_MODE

	if ( (SLR_RATIO_POINTTOPOINT == ratio_type)/* && (!Scaler_PipGetInfo(SLR_PIP_ENABLE)) */) {
...
*/
	...
	if((Get_GOSUBandVODMA()==TRUE)&&((get_rotate_mode(SLR_MAIN_DISPLAY)==(DIRECT_VO_FRAME_ORIENTATION)ROTATE_MODE_90)||(get_rotate_mode(SLR_MAIN_DISPLAY)==(DIRECT_VO_FRAME_ORIENTATION)ROTATE_MODE_270))&&(Scaler_InputSrcGetMainChType() == _SRC_VO))
	{
    ...
    rtd_pr_vbe_warn("[%s] first set Hstart=%x, Vstart=%x\n", __FUNCTION__,Hstart,Vstart);

	scalerdisplay_set_main_act_window_byOrbit(
			(unsigned short)Get_DISP_ACT_STA_HPOS() + HRatioOffset + HStartOffset,
			(unsigned short)Get_DISP_ACT_STA_HPOS() + Scaler_DispGetInputInfo(SLR_INPUT_DISP_WID) + HRatioOffset + HStartOffset ,
			(unsigned short)Get_DISP_ACT_STA_VPOS() + VRatioOffset + VStartOffset,
			(unsigned short)Get_DISP_ACT_STA_VPOS() + Scaler_DispGetInputInfo(SLR_INPUT_DISP_LEN) + VRatioOffset + VStartOffset );
...
#if 0
rtd_pr_vbe_info("HRatioOffset=%d\n",HRatioOffset);
rtd_pr_vbe_info("HStartOffset=%d\n",HStartOffset);
rtd_pr_vbe_info("VRatioOffset=%d\n",VRatioOffset);
rtd_pr_vbe_info("VStartOffset=%d\n",VStartOffset);
rtd_pr_vbe_info("Get_DISP_ACT_STA_HPOS=%d\n",Get_DISP_ACT_STA_HPOS());
rtd_pr_vbe_info("SLR_INPUT_DISP_WID=%d\n",Scaler_DispGetInputInfo(SLR_INPUT_DISP_WID));
rtd_pr_vbe_info("Get_DISP_ACT_STA_VPOS=%d\n",Get_DISP_ACT_STA_VPOS());
rtd_pr_vbe_info("SLR_INPUT_DISP_LEN=%d\n",Scaler_DispGetInputInfo(SLR_INPUT_DISP_LEN));
#endif //#if 0
...
//#ifdef CONFIG_MEMC_BYPASS
	if(get_MEMC_bypass_status_refer_platform_model() == TRUE){
		main_den_h_start_end_reg.mh_den_sta = (DispHSta & 0x1FFF)/(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) +1);
		main_den_h_start_end_reg.mh_den_end = (DispHEnd & 0x1FFF)/(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) +1);
		main_background_h_start_end_reg.mh_bg_sta = ((DispHSta + Border) & 0x1FFF)/(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) +1);
		main_background_h_start_end_reg.mh_bg_end = ((DispHEnd - Border) & 0x1FFF)/(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) +1);
//#else
	}else{
	if(Get_MEMC_Enable_Dynamic() == 0){
		...
		rtd_pr_vbe_debug("##### scalerdisplay_set_main_display_window_H : DispHEnd = %x\n", DispHEnd);
		main_den_h_start_end_reg.mh_den_sta = (DispHSta & 0x1FFF);
		main_den_h_start_end_reg.mh_den_end = DispHEnd & 0x1FFF;
		main_background_h_start_end_reg.mh_bg_sta = ((DispHSta + Border) & 0x1FFF);
		main_background_h_start_end_reg.mh_bg_end = ((DispHEnd - Border) & 0x1FFF);
		}
	}
//#endif
...
void drvif_scalerdisplay_panel_porch_protect(void)
{
		...
		||(dv_den_start_end_reg.dv_den_end > Get_DISP_DEN_END_VPOS()))
	{
		...
		dv_den_start_end_reg.dv_den_end = Get_DISP_DEN_END_VPOS();
		IoReg_Write32(PPOVERLAY_DV_DEN_Start_End_reg, dv_den_start_end_reg.regValue);
		rtd_pr_vbe_debug("Enter Panel Porch Protect!\n");
	}
...
//#endif
	...
	rtd_pr_vbe_debug("DispInit, type/idx/3D=%d/%d/%d[PR/SG=%d/%d]\n", Get_DISPLAY_PANEL_TYPE(), Get_DISPLAY_PANEL_CUSTOM_INDEX(),
...
//				lvds_ctrl3.lvds_dport_en = 1;
		...
		IoReg_Write32(PINMUX_GPIO_EMMC_CFG_1_reg, 0x00000000);	// LVDS A-Port F/G/H Pair PinShare
		IoReg_Write32(PINMUX_GPIO_EMMC_CFG_2_reg, 0x00000000);	// LVDS A-Port C/D/E Pair PinShare
		...
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_0_0_reg, 0x03001810);	// PI_A1 = Phase0, PI_A2 = Phase0, CK7X = 1X Current, PI_Clock = 300~600MHz,
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_0_1_reg, 0x03001810);	// PI_B1 = Phase0, PI_B2 = Phase0, CK7X = 1X Current, PI_Clock = 300~600MHz,
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_0_2_reg, 0x03040000);	// PI_AB = Phase0, CK7X = 1X Current, CK5X Duty Adjust Disable, PI_Clock = 300~600MHz,
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_0_3_reg, 0x04153700);	// Choose legacy charge-pump, DIV248 = 2, VCO = 74.25*7*DIV248 = 1.0395GHz,
							// Small Kvco, CK1X Not Inverse, PLL_VCO = 2V,
							// Icp = 12.5uA, Cp = 9.5pF, Rs = 12Kohm,
							// PI_A Disble, PI_B Disable ==> PI_AB Disble
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_3_2_reg, 0x90040000);	// P0 A-Pair LVDS, Use PI_A1, SR Disable, internal resistor 100ohm,
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_2_4_reg, 0x90040000);	// P0 B-Pair LVDS, Use PI_A1, SR Disable, internal resistor 100ohm,
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_0_4_reg, 0x90040000);	// P0 C-Pair LVDS, Use PI_A1, SR Disable, internal resistor 100ohm,
		IoReg_Write32(0xb8000C1C, 0x91040000);	// P0 D-Pair LVDS, Use PI_A2, SR Disable, internal resistor 100ohm,
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_2_3_reg, 0x90040000);	// P0 E-Pair LVDS, Use PI_A1, SR Disable, internal resistor 100ohm,
		...
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_0_18_reg, 0x00000000);	// P1 Not Adjudt CK7X Delay, CK7X Not Inverse, PI Not Lag 0.5UI,
		...
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_2_5_reg, 0xFFFF0000);	// Lane Power Enable for non-Vby1-Mode
...
#endif
	rtd_pr_vbe_debug("Initial Display is completed. \n");
...
void drvif_scalerdisplay_set_bg_color(unsigned char display, eDISPLAYBORDERTYPE BorderType, unsigned int ColorR, unsigned int ColorG, unsigned int ColorB)
{
	...
	if(SLR_MAIN_DISPLAY == display)
	{
		if(BorderType == _DISPLAY_BG)
		{
			...
			if (Get_display_forcebg_mask(SLR_MAIN_DISPLAY, SLR_FORCE_BG_TYPE_INNER_PATTERN) == FALSE) {
				...
				{
					rtd_pr_vbe_debug("#########Main DISPLAY_BG Color is the same\n");
					return;
				}
		...
		else if(BorderType == _DISPLAY_EB)
		{
			...
			{
				rtd_pr_vbe_debug("#########Main DISPLAY_EB Color is the same\n");
				return;
			}
		...
		else if(BorderType == _DISPLAY_HB)
		{
			...
			main_border_highlight_border_color1_reg.regValue = IoReg_Read32(PPOVERLAY_Main_Border_Highlight_Border_Color1_reg);
			main_border_highlight_border_color2_reg.regValue = IoReg_Read32(PPOVERLAY_Main_Border_Highlight_Border_Color2_reg);

			if((main_border_highlight_border_color1_reg.m_hbd_g == (ColorG&0x3fff)) && (main_border_highlight_border_color1_reg.m_hbd_b == (ColorB&0x3fff))
				&& (main_border_highlight_border_color2_reg.m_hbd_r == (ColorR&0x3fff)))
			{
				rtd_pr_vbe_debug("#########Main DISPLAY_HB Color is the same\n");
				return;
			}

			main_border_highlight_border_color1_reg.m_hbd_g = ColorG&0x3fff;
			main_border_highlight_border_color1_reg.m_hbd_b = ColorB&0x3fff;
			main_border_highlight_border_color2_reg.m_hbd_r = ColorR&0x3fff;
			IoReg_Write32(PPOVERLAY_Main_Border_Highlight_Border_Color1_reg, main_border_highlight_border_color1_reg.regValue);
			IoReg_Write32(PPOVERLAY_Main_Border_Highlight_Border_Color2_reg, main_border_highlight_border_color2_reg.regValue);
		}
		else
			return;
	...
	else// if(SLR_SUB_DISPLAY == display)
	{
		...
		if(BorderType == _DISPLAY_BG)
		{
			...
			{
				rtd_pr_vbe_debug("#########Sub DISPLAY_BG Color is the same\n");
				return;
			}
		...
		else if(BorderType == _DISPLAY_EB)
		{
			...
			{
				rtd_pr_vbe_debug("#########Sub DISPLAY_EB Color is the same\n");
				return;
			}
		...
		else if(BorderType == _DISPLAY_HB)
		{
			...
			{
				rtd_pr_vbe_debug("#########Sub DISPLAY_HB Color is the same\n");
				return;
			}
...
void fw_scalerdisplay_get_display_size(unsigned char display)
{
	...
	if ( display == SLR_MAIN_DISPLAY ) {
		...
		if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) == PIXEL_MODE_2_PIXEL){
			...
			rtd_pr_vbe_debug("Rtkbypass 2 pixel mode, width /2 : %x\n", dispWid);
		}
#else
		if(Get_MEMC_Enable_Dynamic() == 0){
			dispLen =Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DISP_LEN);
...
//		))	// yychao+ : for JPEG source
		...
		{
			ratio_type = Scaler_Get_Normal_AspectRatio_RatioType();
			rtd_pr_vbe_debug("fw_scalerdisplay_get_display_size, Normal Mode : %x\n", ratio_type);
		}
#endif //#ifdef CONFIG_ENABLE_NORMAL_DISPLAY_RATIO_MODE
		...
		{
			ratio_type = Scaler_Get_DotByDot_AspectRatio_RatioType();
			rtd_pr_vbe_debug("fw_scalerdisplay_get_display_size, DotByDot Mode : %x\n", ratio_type);
		}
#endif //#ifdef CONFIG_ENABLE_DOT_BY_DOT_DISPLAY_RATIO_MODE
		...
		{
			ratio_type = Scaler_Get_Native_AspectRatio_RatioType();
			rtd_pr_vbe_debug("fw_scalerdisplay_get_display_size, Native Mode : %x\n", ratio_type);
		}
#endif //#ifdef CONFIG_ENABLE_NATIVE_DISPLAY_RATIO_MODE
						...
						switch(Scaler_DispGetInputInfo(SLR_INPUT_AFD)){
							...
							default:
									{ //playback
										...
										panel_ratio = (unsigned int)input_height*pVOInfo->pixelAR_ver*dispWid;
										source_ratio = (unsigned int)Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID_PRE)*pVOInfo->pixelAR_hor*dispLen;
									//frank@0704 mark this condition to do qwer verify	if((source_ratio*9/dispLen)<(panel_ratio*16/dispWid)+1)  //input source aspect raito <=16/9
										{
											if(panel_ratio > source_ratio){//Change panel width
		...
		if((Get_GOSUBandVODMA()==TRUE)&&((Get_Rotate_Mode(SLR_MAIN_DISPLAY)==ROTATE_MODE_90)||(Get_Rotate_Mode(SLR_MAIN_DISPLAY)==ROTATE_MODE_270))&&(Scaler_InputSrcGetMainChType() == _SRC_VO))
		{
...
//		doOverscanFlag = TRUE;
	...
	rtd_pr_vbe_debug("[ZOOM]after overscan %d %d %d %d\n",scan_data.HStart,scan_data.HWidth,scan_data.VStart,scan_data.VHeight);
	...
	*/
	...
	if(Scaler_InputSrcGetMainChType()== _SRC_HDMI && drvif_IsHDMI())
	{
		...
		{
			rtd_pr_vbe_debug("hdmi even odd swap\n");
...
StructSrcRect Scaler_intputRegion_base_scalerTiming[MAX_DISP_CHANNEL_NUM];
StructSrcRect Scaler_intputRegion[MAX_DISP_CHANNEL_NUM];
StructSrcRect Scaler_intputTiming[MAX_DISP_CHANNEL_NUM];
...
void set_input_for_crop(unsigned char display, StructSrcRect input)
{
	if (display < MAX_DISP_CHANNEL_NUM) {
		Scaler_intputRegion[display] = input;
	}else{
		rtd_pr_vbe_emerg("set_input_for_crop err display=%d\n", display);
	}
}

StructSrcRect get_input_for_crop(unsigned char display)
{
	return Scaler_intputRegion[display];
}

void set_input_timing_for_crop(unsigned char display, StructSrcRect input)
{
	if (display < MAX_DISP_CHANNEL_NUM) {
		Scaler_intputTiming[display] = input;
	}
	else{
		rtd_pr_vbe_emerg("set_input_timing_for_crop err display=%d\n", display);
	}

}

StructSrcRect get_input_timing_for_crop(unsigned char display)
{
	return Scaler_intputTiming[display];
}

StructSrcRect cal_new_input_base_on_display_win(unsigned char display)
{
	//StructSrcRect m_dispwin = Scaler_DispWindowGet();
	StructSrcRect m_input_size = Scaler_AdaptiveStream_InputWindowGet(display);
	...
	//rtd_pr_vbe_info("[pool test]input info:x:%d,y:%d,wid:%d,len:%d\n",m_input_size.srcx,m_input_size.srcy,m_input_size.src_wid,m_input_size.src_height);
	if(Scaler_DispGetStatus(display, SLR_DISP_INTERLACE))
		pre_len *= 2;
	//check 4k2k input and output ,overflow
	if(_DISP_WID && _DISP_LEN){
		NewInput2OldInput.srcx = ((unsigned long long)Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPH_ACT_WID_PRE) * (unsigned long long)m_input_size.srcx * 100 / (unsigned long long)_DISP_WID + 50) / 100;
		NewInput2OldInput.srcy = (pre_len * m_input_size.srcy *100/_DISP_LEN + 50)/100;
		NewInput2OldInput.src_wid = ((unsigned long long)Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPH_ACT_WID_PRE) * (unsigned long long)m_input_size.src_wid * 100 / (unsigned long long)_DISP_WID + 50) / 100;
	...
	if((NewInput2OldInput.srcx + NewInput2OldInput.src_wid) > Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPH_ACT_WID_PRE)){
		rtd_pr_vbe_info("[scaler][WARNING]new adaptive stream, wid is over size\n");
	...
	if((NewInput2OldInput.srcy + NewInput2OldInput.src_height) > pre_len){
		rtd_pr_vbe_info("[scaler][WARNING]new adaptive stream, len is over size\n");
		NewInput2OldInput.src_height = pre_len - NewInput2OldInput.srcy;
	}
	//rtd_pr_vbe_info("[pool test]PRE:wid:%d,len:%d\n",Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPH_ACT_WID_PRE),Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPV_ACT_LEN_PRE));
	//rtd_pr_vbe_info("[pool test]ACT:x:%d,y:%d,wid:%d,len:%d\n",NewInput2OldInput.srcx,NewInput2OldInput.srcy,NewInput2OldInput.src_wid,NewInput2OldInput.src_height);

	return NewInput2OldInput;
}

StructSrcRect cal_new_inputRegion_base_on_hal_orgInput(unsigned char display)
{
	StructSrcRect inputRegion = get_input_for_crop(display);
	StructSrcRect inputTiming = get_input_timing_for_crop(display);
	StructSrcRect NewInput2OldInput;
	if ((Get_DisplayMode_Src(display) == VSC_INPUTSRC_AVD) && (SLR_MAIN_DISPLAY == display))
	{
		inputTiming.src_wid = 720;// for ATV
		if(VD_27M_HSD960_DI5A == fw_video_get_27mhz_mode(display)){// for AV
			inputTiming.src_wid = 720;
		}else if(VD_27M_HSD1440_DI3A== fw_video_get_27mhz_mode(display)){
			inputTiming.src_wid = 1440;
		}
		rtd_pr_vbe_emerg("[AVD source]inputTiming info:x:%d,y:%d,wid:%d,len:%d\n",inputTiming.srcx,inputTiming.srcy,inputTiming.src_wid,inputTiming.src_height);
	}

	NewInput2OldInput = inputRegion;
	...
	if((NewInput2OldInput.srcx + NewInput2OldInput.src_wid) > Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_WID)){
		rtd_pr_vbe_err("[scaler][WARNING]new adaptive stream, wid is over size\n");
		NewInput2OldInput.src_wid = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_WID) - NewInput2OldInput.srcx;
	}
	...
	if((NewInput2OldInput.srcy + NewInput2OldInput.src_height) > Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_LEN)){
		rtd_pr_vbe_err("[scaler][WARNING]new adaptive stream, len is over size\n");
		NewInput2OldInput.src_height = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_LEN) - NewInput2OldInput.srcy;
	}
	//rtd_pr_vbe_info("[pool test]PRE:wid:%d,len:%d\n",Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPH_ACT_WID_PRE),Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPV_ACT_LEN_PRE));
	//rtd_pr_vbe_info("[pool test]ACT:x:%d,y:%d,wid:%d,len:%d\n",NewInput2OldInput.srcx,NewInput2OldInput.srcy,NewInput2OldInput.src_wid,NewInput2OldInput.src_height);
	Scaler_intputRegion_base_scalerTiming[display] = NewInput2OldInput;
	return NewInput2OldInput;
}
...
unsigned char fw_scaler_cal_sdnr_capture_for_hal_orgInput(unsigned char display)
{
	unsigned char src;
	unsigned char colorspace=0;

	StructSrcRect newInput_main = {0, 0, 0, 0};
	StructSrcRect newInput_sub = {0, 0, 0, 0};
	unsigned int ucSDNR_H;
	unsigned int ucSDNR_X;

	src = Scaler_InputSrcGetMainChType();

	if(SLR_MAIN_DISPLAY == display){
		newInput_main = cal_new_inputRegion_base_on_hal_orgInput(display);
	}else{
		newInput_sub = cal_new_inputRegion_base_on_hal_orgInput(display);
	}

	#if 1
	//rtd_pr_vbe_emerg("file=%s, function=%s, line=%d\n", __FILE__, __FUNCTION__, __LINE__);
	if(SLR_MAIN_DISPLAY == display)
		rtd_pr_vbe_emerg("[new hal orgInput] newInput_main:x:%d, y:%d, wid:%d, len:%d\n", newInput_main.srcx, newInput_main.srcy, newInput_main.src_wid, newInput_main.src_height);
	else
		rtd_pr_vbe_emerg("[new hal orgInput] newInput_sub:x:%d, y:%d, wid:%d, len:%d\n", newInput_sub.srcx, newInput_sub.srcy, newInput_sub.src_wid, newInput_sub.src_height);
	#endif

	Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA, Scaler_intputRegion_base_scalerTiming[display].srcx);
	Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_STA, Scaler_intputRegion_base_scalerTiming[display].srcy);
	Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, Scaler_intputRegion_base_scalerTiming[display].src_wid);
	Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_LEN, Scaler_intputRegion_base_scalerTiming[display].src_height);

	rtd_pr_vbe_debug("file=%s, function=%s, line=%d\n", __FILE__, __FUNCTION__, __LINE__);
	rtd_pr_vbe_debug("[new hal orgInput]display:%d,inpu base scalerTiming:x:%d, y:%d, wid:%d, len:%d\n",display,
		Scaler_intputRegion_base_scalerTiming[display].srcx, Scaler_intputRegion_base_scalerTiming[display].srcy,
		Scaler_intputRegion_base_scalerTiming[display].src_wid,Scaler_intputRegion_base_scalerTiming[display].src_height);

	if (0)//Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC))
	{	//
		...
		{
			// K4LG-336  workaround to avoid issue
			// SDNR Hor_back only has 11bit so that it need to cut for over 4094 width
			ucSDNR_H = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID);
			ucSDNR_X = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA);
			if (ucSDNR_H+ucSDNR_X > REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD)
			{
				if(ucSDNR_H > REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD)
				{
				    ucSDNR_H = REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD;
				}
				else
				{
					ucSDNR_H = REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD - ucSDNR_X;
				}
				Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, ucSDNR_H);
				rtd_pr_vbe_err("[warning] sub doesn't have di, but size is over 4096\n");
			}
		}
		else// SLR_MAIN_DISPLAY
		{
			// Corrected crop region W&X if HSD enable
			// K4LG-336  workaround to avoid issue
			// SDNR Hor_back only has 11bit so that it need to cut for over 4094 width
			if (Scaler_Get_di_hsd_flag()&&(SLR_MAIN_DISPLAY==display)){
				...
				if ((VD_27M_HSD960_DI5A == fw_video_get_27mhz_mode(display))&&(Get_DisplayMode_Src(display) == VSC_INPUTSRC_AVD)&&
					...
					unsigned int temp_x = (inputRegion.srcx * uDI_WID * 100 /  inputTiming.src_wid + 55 ) / 100;
					unsigned int temp_w = (inputRegion.src_wid* uDI_WID * 100 / inputTiming.src_wid + 55 ) / 100;
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA, temp_x);
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, temp_w);
					rtd_pr_vbe_notice("[HSD] new sdnr size:x :%d, wid:%d\n",temp_x,temp_w);
				}
			} else {
				...
				if (ucSDNR_H+ucSDNR_X > REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD){
					...
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, ucSDNR_H);
					rtd_pr_vbe_err("[warning] di and rtnr off, but size is over 4096\n");
				}
			}
		}
		if (Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC)) {
			...
			Scaler_DispSetInputInfo(SLR_INPUT_SDNRV_ACT_STA, 0);
		}
	}
	...
	/*protected 4096 width for sdnr regiter not support so bug value*/
	if (Scaler_DispGetInputInfoByDisp(display,SLR_INPUT_IPH_ACT_WID) > REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD)
		Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD);

	//protect data area
	if ((Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA)+Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID)) >  Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_WID)) {
		Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA, Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_WID)-Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID));
	}
	...
	if(Scaler_DispGetStatus((SCALER_DISP_CHANNEL)display,SLR_DISP_422CAP) && Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC))
	{
		Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA, (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA) & ~0x1));
		Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID) & ~0x1));
	}
#if 1
	//rtd_pr_vbe_emerg("function=%s, line=%d\n",__FUNCTION__, __LINE__);
	rtd_pr_vbe_emerg("SLR_INPUT_DI_WID=%d\n", Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_WID));
	rtd_pr_vbe_emerg("SLR_INPUT_DI_LEN=%d\n", Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_LEN));
	rtd_pr_vbe_emerg("SLR_INPUT_SDNRH_ACT_STA=%d\n", Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA));
	rtd_pr_vbe_emerg("SLR_INPUT_SDNRH_ACT_WID=%d\n", Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID));
	rtd_pr_vbe_emerg("SLR_INPUT_SDNRV_ACT_STA=%d\n", Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_STA));
	rtd_pr_vbe_emerg("SLR_INPUT_SDNRV_ACT_LEN=%d\n", Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_LEN));
#endif
	return TRUE;
}
...
unsigned char fw_scaler_cal_sdnr_capture(unsigned char display)
{
	...
	unsigned char colorspace=0;
	unsigned int uDI_WID = 0x00;
	unsigned int uINPUT_WID = 0x00;
	StructSrcRect newInput = Scaler_AdaptiveStream_InputWindowGet(display);
#ifdef NEW_INPUT_REGION_BASE_ON_HAL_ORIGINAL_INPUT
	if(Scaler_InputRegionType_Get(display) == INPUT_BASE_ON_HAL_ORIGINAL_INPUT)
		return fw_scaler_cal_sdnr_capture_for_hal_orgInput(display);
#endif

	src = Scaler_InputSrcGetMainChType();

	rtd_pr_vbe_debug("file=%s, function=%s, line=%d\n", __FILE__, __FUNCTION__, __LINE__);
	rtd_pr_vbe_debug("Main_InputRegion_w_temp=%d, Main_InputRegion_x_temp=%d\n", Main_InputRegion_w_temp, Main_InputRegion_x_temp);
	#if 0
	rtd_pr_vbe_emerg("SUB VD 27M mode=%d\n", fw_video_get_27mhz_mode((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY)));
	rtd_pr_vbe_emerg("SUB Display=%d\n", (SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY));
	#endif

	if(Scaler_InputRegionType_Get(display) == INPUT_BASE_ON_DISP_WIN){
		newInput = cal_new_input_base_on_display_win(display);
		rtd_pr_vbe_err("[pool test] cal vgip: newInput:(%d,%d,%d,%d)\n",newInput.srcx,newInput.srcy,newInput.src_wid,newInput.src_height);
	}else{
		if (display == SLR_MAIN_DISPLAY) {
			...
			newInput.srcx = Sub_InputRegion_x_temp;
			newInput.srcy = Sub_InputRegion_y;
			newInput.src_wid = Sub_InputRegion_w_temp;
			newInput.src_height = Sub_InputRegion_h;
		}


		if (newInput.src_wid == 0) {
			if (Scaler_Get_di_hsd_flag()&&(SLR_MAIN_DISPLAY==display) &&(VD_27M_HSD960_DI5A == fw_video_get_27mhz_mode(display))&&(Get_DisplayMode_Src(display) == VSC_INPUTSRC_AVD)&&
				(Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPH_ACT_WID) == AVD_27M_VGIP_WIDTH))	{
				// AV function need to convert to  original source
				newInput.src_wid = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,AVD_27M_VGIP_WIDTH/2);
				newInput.srcx = 0;
			} else {
				newInput.src_wid = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_WID);
				newInput.srcx = 0;
			}
		}
		if (newInput.src_height == 0) {
			newInput.src_height = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DI_LEN);
			newInput.srcy = 0;
		}
	}
	...
	{		// SLR_MAIN_DISPLAY
		...
		uINPUT_WID = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPH_ACT_WID);
		...
		if (Scaler_Get_di_hsd_flag()&&(SLR_MAIN_DISPLAY==display))	{
			if ((VD_27M_HSD960_DI5A == fw_video_get_27mhz_mode(display))&&(Get_DisplayMode_Src(display) == VSC_INPUTSRC_AVD)&&
				...
				//uRetVal = ((inVal * (unsigned int)uOutW * 100 / (unsigned int)uInW) + 55 )/100 ;
				if(uINPUT_WID > 0){
					newInput.srcx = (newInput.srcx * uDI_WID * 100 /  uINPUT_WID + 55 ) / 100;
					newInput.src_wid = (newInput.src_wid* uDI_WID * 100 /  uINPUT_WID + 55 ) / 100;
				}
			}
		}
		rtd_pr_vbe_emerg("newInput.srcx=%x, newInput.src_wid=%x\n", newInput.srcx, newInput.src_wid);
		rtd_pr_vbe_emerg("newInput.srcy=%x, newInput.src_height=%x\n", newInput.srcy, newInput.src_height);

		if (Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE)) {
		#ifdef CONFIG_ENABLE_VD_27MHZ_MODE
			if(VD_27M_HSD960_DI5A == fw_video_get_27mhz_mode(display))
			{
				newInput.srcx = Scaler_CalAVD27MWidth((SCALER_DISP_CHANNEL)display, newInput.srcx);
				newInput.src_wid = Scaler_CalAVD27MWidth((SCALER_DISP_CHANNEL)display, newInput.src_wid);
			}
		#endif
		...
		// SDNR Hor_back only has 11bit so that it need to cut for over 4094 width
		{
			unsigned int ucSDNR_W = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID);
			unsigned int ucSDNR_X = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA);
			if (ucSDNR_W+ucSDNR_X > REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD)
			{
				if(ucSDNR_W > REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD)
				{
					ucSDNR_W = REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD;
				}
				else
				{
					ucSDNR_W = REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD - ucSDNR_X;
				}
			}
			Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, ucSDNR_W);
		}
	}else {
		if ((SCALER_DISP_CHANNEL)display == SLR_SUB_DISPLAY)
		{
		    if(Get_sub_from_DI_mux_path()){
				...
				Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA, newInput.srcx);
  				...
  				if (newInput.src_wid > Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_DI_WID))/*Not reasonable input*/
	  				Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_DI_WID));
  				else
	  				Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, newInput.src_wid);

  				if (newInput.src_height > Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_DI_LEN))/*Not reasonable input*/
	  				Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_LEN, Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_DI_LEN));
  				else
	  				Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_LEN, newInput.src_height);
			}else {
			#if 1
				...
				Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA, newInput.srcx);
				Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, newInput.src_wid);
				if ((Scaler_DispGetStatus(SLR_SUB_DISPLAY, SLR_DISP_INTERLACE)) && (Scaler_DispGetStatus(SLR_SUB_DISPLAY, SLR_DISP_THRIP) == 0))
				{//sub interlace,bypass di
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_STA, newInput.srcy/2);
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_LEN, newInput.src_height/2);
				}else{
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_STA, newInput.srcy);
					Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRV_ACT_LEN, newInput.src_height);
				}
 ...
 * This function is used to handle some auto aspect ratio, including movies & subtitle */
/* =====================================================*/
unsigned char fw_scaler_cal_vgip_capture_for_hal_orgInput(unsigned char display)
{
	//unsigned char src;
//	unsigned char colorspace=0;
	...
	rtd_pr_vbe_emerg("SLR_INPUT_IPH_ACT_STA=%d\n", Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPH_ACT_STA));
	rtd_pr_vbe_emerg("SLR_INPUT_IPH_ACT_WID=%d\n", Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPH_ACT_WID));
	rtd_pr_vbe_emerg("SLR_INPUT_IPV_ACT_STA=%d\n", Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPV_ACT_STA));
	rtd_pr_vbe_emerg("SLR_INPUT_IPV_ACT_LEN=%d\n", Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPV_ACT_LEN));
...
unsigned char fw_scaler_cal_vgip_capture(unsigned char display)
{
...
#ifdef NEW_INPUT_REGION_BASE_ON_HAL_ORIGINAL_INPUT
	if(Scaler_InputRegionType_Get(display) == INPUT_BASE_ON_HAL_ORIGINAL_INPUT)
		return fw_scaler_cal_vgip_capture_for_hal_orgInput(display);
#endif
	...
	#if 1
	rtd_pr_vbe_emerg("file=%s, function=%s, line=%d\n", __FILE__, __FUNCTION__, __LINE__);
	rtd_pr_vbe_emerg("Main_InputRegion_w_temp=%d, Main_InputRegion_x_temp=%d\n", Main_InputRegion_w_temp, Main_InputRegion_x_temp);
	rtd_pr_vbe_emerg("SUB VD 27M mode=%d\n", fw_video_get_27mhz_mode((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY)));
	rtd_pr_vbe_emerg("SUB Display=%d\n", (SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY));
	#endif
	...
	if ((SCALER_DISP_CHANNEL)display == SLR_SUB_DISPLAY) {
		...
		if (Get_sub_from_DI_mux_path())
		{
			Scaler_DispSetInputInfo(SLR_INPUT_IPH_ACT_STA, Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_STA_PRE));
			Scaler_DispSetInputInfo(SLR_INPUT_IPH_ACT_WID, Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE));
			Scaler_DispSetInputInfo(SLR_INPUT_IPV_ACT_STA, Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_STA_PRE));
			Scaler_DispSetInputInfo(SLR_INPUT_IPV_ACT_LEN, Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE));
...
//#ifdef CONFIG_SDNR_CROP//sunray add for WOSQRTK-8296 and WOSQRTK-8469
		...
		//else
		{
			...
			Scaler_DispSetInputInfo(SLR_INPUT_IPV_ACT_LEN, Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN_PRE));
  #endif
			newInput = Scaler_AdaptiveStream_InputWindowGet(display);;
			if(Scaler_InputRegionType_Get(display) == INPUT_BASE_ON_DISP_WIN){
				newInput = cal_new_input_base_on_display_win(display);
			}else{
				newInput.srcx = Main_InputRegion_x_temp;
			...
			if (Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE)) {
				...
				if ((Main_InputRegion_h == 0) || (Main_InputRegion_h > (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_IPV_ACT_LEN_PRE)<<1)))/*Not reasonable input*/
  				...
  				if(Scaler_InputSrcGetMainChType() == _SRC_VO && vsc_get_adaptivestream_flag(display)){
					StructSrcRect m_dispwin = Scaler_DispWindowGet();
					StructSrcRect m_input_size = Scaler_AdaptiveStream_InputWindowGet(display);

					//rtd_pr_vbe_info("[pool test]input info:x:%d,y:%d,wid:%d,len:%d\n",m_input_size.srcx,m_input_size.srcy,m_input_size.src_wid,m_input_size.src_height);
					//rtd_pr_vbe_info("[pool test]diplay info:x:%d,y:%d,wid:%d,len:%d\n",m_dispwin.srcx,m_dispwin.srcy,m_dispwin.src_wid,m_dispwin.src_height);
...
{//for 0xb8028028
	...
	if (display == SLR_MAIN_DISPLAY) {
		...
		if(En) {
			IoReg_SetBits(PPOVERLAY_Double_Buffer_CTRL_reg, PPOVERLAY_Double_Buffer_CTRL_dmainreg_dbuf_en_mask | PPOVERLAY_Double_Buffer_CTRL_dmainreg_dbuf_set_mask);
		} else {
			IoReg_Mask32(PPOVERLAY_Double_Buffer_CTRL_reg, ~PPOVERLAY_Double_Buffer_CTRL_dmainreg_dbuf_en_mask, 0);
		}
		spin_unlock_irqrestore(get_DBreg_spinlock(SLR_MAIN_DISPLAY), flags);//unlock double buffer spinlock
	} else {
		spin_lock_irqsave(get_DBreg_spinlock(SLR_MAIN_DISPLAY), flags);//lock double buffer spinlock //also use the main path spinlock because also control 0xb8028028
		if(En) {
			IoReg_SetBits(PPOVERLAY_Double_Buffer_CTRL_reg, PPOVERLAY_Double_Buffer_CTRL_dsubreg_dbuf_en_mask  | PPOVERLAY_Double_Buffer_CTRL_dsubreg_dbuf_set_mask);
		} else {
			IoReg_Mask32(PPOVERLAY_Double_Buffer_CTRL_reg, ~PPOVERLAY_Double_Buffer_CTRL_dsubreg_dbuf_en_mask, 0);
		}
		spin_unlock_irqrestore(get_DBreg_spinlock(SLR_MAIN_DISPLAY), flags);//unlock double buffer spinlock //also use the main path spinlock because also control 0xb8028028
	}
}
...
/*api which have protect by forcebg_semaphore*/
void drvif_scalerdisplay_set_forcebg_dbuffer(SCALER_DISP_CHANNEL display, int En)//only for forcebg
{//disable main forcebg double buffer, becasue the double buffer is changed
	...
	if (display == SLR_MAIN_DISPLAY) {
		...
		if(En) {
			IoReg_SetBits(PPOVERLAY_Double_Buffer_CTRL3_reg, PPOVERLAY_Double_Buffer_CTRL3_dmainreg_other_dbuf_en_mask | PPOVERLAY_Double_Buffer_CTRL3_dmainreg_other_dbuf_set_mask);
		} else {
			IoReg_Mask32(PPOVERLAY_Double_Buffer_CTRL3_reg, ~PPOVERLAY_Double_Buffer_CTRL3_dmainreg_other_dbuf_en_mask, 0);
		}
		spin_unlock_irqrestore(get_DBreg_spinlock(SLR_MAIN_DISPLAY), flags);//unlock double buffer spinlock
	} else {
		spin_lock_irqsave(get_DBreg_spinlock(SLR_MAIN_DISPLAY), flags);//lock double buffer spinlock //also use the main path spinlock because also control 0xb8028028
...
void drvif_scalerdisplay_set_dclk_sel(DISP_DMACLK_SELECT dma_clk_sel, DISP_DCLK_SELECT dclk_sel)
{
	...
	rtd_pr_vbe_info("[SLR] set_dclk_sel=%x\n", sys_dispclksel_reg.regValue);
	fw_scaler_dtg_double_buffer_apply();
...
static void scaler_finetune_tcon_alog_do(unsigned int dh_total, unsigned int dv_total, unsigned int dh_start, unsigned int dv_start )
{
	...
	if(drvif_scaelr3d_decide_is_3D_display_mode() == TRUE){
		...
		//================================
		WaitFor_DEN_STOP();
		IoReg_Write32(COLORMAP_D_CMRGB2RGB_Tab1_Data_4_reg, 0x00401FA3);  // Cap setting
		IoReg_Write32(COLORMAP_D_CMRGB2RGB_Tab2_Data_3_reg, 0x1FA32020);  // Disp setting
		IoReg_Write32(COLORMAP_D_CMRGB2RGB_Tab2_Out_offset_0_reg, 0x00000000);  // Line step
		IoReg_Write32(COLORMAP_D_CMRGB2RGB_Tab2_Data_2_reg, 0x00078040);  // Pixel num
		IoReg_Write32(COLORMAP_D_CMRGB2RGB_Tab2_Out_offset_1_reg, 0x00000438);  // Line num

		msleep(50);
		WaitFor_DEN_STOP();
		IoReg_Write32(COLORMAP_D_CMRGB2RGB_Tab1_Data_1_reg, 0x00000040);  // Compression 8 bits

		// Cap and Disp en
		IoReg_Write32(COLORMAP_D_CMRGB2RGB_Tab1_Data_3_reg, 0x01092020);  // Cap enable
		IoReg_Write32(COLORMAP_D_CMRGB2RGB_Tab2_Data_0_reg, 0x00000003);  // Disp enable

		WaitFor_DEN_STOP();
		IoReg_Write32(COLORMAP_D_CMRGB2RGB_Tab1_Data_In_Offset_1_reg, 0x20000000);  // OD enable
...
void  fw_scaler_4k2k_color_config(void)
{
	...
	//if((IoReg_Read32(0xB801bc3c) & _BIT15))
	{
		...
		//IoReg_Write32(0xb8023600,0x00000200); //I-Dither
		    ...
		    IoReg_Write32(OSD_SR_OSD_SR_1_Scaleup_Ctrl1_reg,0x00000000);//mark for PQ setting for M3 0723// disable SPNR
		...
		//check 0xb8029004[31:30]=0  //disable adaptive uzu
		    ...
		    IoReg_Write32(SCALEUP_DM_UZU_12tap_dering_reg,0x00000000);//mark for PQ setting for M3 0723 // disable supk
		    IoReg_Write32(SCALEUP_D_UZU_PATGEN_FRAME_TOGGLE3_reg,0x00000000); //mark for PQ setting for M3 0723// disable unsharp mask

		    //rtd_outl(COLOR_D_VC_Global_CTRL_reg,0x4000000C); //modify for PQ setting for M3 0723
		    IoReg_Write32(COLOR_D_VC_Global_CTRL_reg,0x4000000C); //disable sharpness,modify for PQ setting for M3 0723
                    IoReg_Write32(DITHER_DITHERING_CTRL_reg,0x00000000); // disable dither

		//IoReg_Write32(COLOR_D_VC_Global_CTRL_reg,0x40000009);// disable vv color (0717_DarwinPQ_CMI_setting, DCC_on, ICM_on) //IoReg_Write32(0xb802a000,0x00000000); // disable vv color
		...
		if(fw_scaler_check_color_space_in_rgb()){
			...
			//rtd_pr_vbe_info("[4k2k][RGB], src/hdmi/color=%d/%d/%d\n", Scaler_InputSrcGetMainChType(), drvif_IsHDMI(), drvif_Hdmi_GetColorSpace());
		}
		rtd_pr_vbe_debug("[4k2k] Vip disabled!!\n");
	...
	if(modestate_get_isForceEnableRTNR()){
		...
		di_im_di_control_RBUS_reg.di444to422lowpass = 0;
...
void drvif_scalerdisplay_set_sharpness_line_mode(void)
{
	...
	//if((Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP) == FALSE) && (get_scaler_connect_source(SLR_MAIN_DISPLAY) == _SRC_HDMI))	// RGB444 and HDMI input
	if(get_vsc_run_pc_mode() && (get_scaler_connect_source(SLR_MAIN_DISPLAY) == _SRC_HDMI))	// RGB444 and HDMI input
		display_timing_ctrl2_reg.shpnr_line_mode_sel = 0;
	else
		display_timing_ctrl2_reg.shpnr_line_mode_sel = 1;
	//if(_DISP_WID < 1920){
		//display_timing_ctrl2_reg.shpnr_line_mode = 1;
	//}else{
...
/*
    ...
    if (m_TCON_I2C_DEVICE_ADDR) {
	    if (I2C_Write_EX(m_TCON_I2C_MASTER, m_TCON_I2C_DEVICE_ADDR, sizeof(dat), dat, 0) != S_OK) {
	        rtd_pr_vbe_info("[I2CSettingToMillet] I2C Write fail 1\n");
	    }
    }
...
*/
    ...
    if (m_TCON_I2C_DEVICE_ADDR) {
		//if (I2C_Write_EX(m_TCON_I2C_MASTER, m_TCON_I2C_DEVICE_ADDR, sizeof(dat), dat, 0) != S_OK) {
		//    rtd_pr_vbe_info("[I2CSettingToMillet] I2C Write fail 2\n");
		//}

		for (int i=0; i<10; i++) {
	    if (I2C_Write_EX(m_TCON_I2C_MASTER, m_TCON_I2C_DEVICE_ADDR, sizeof(dat), dat, 0) != S_OK) {
				rtd_pr_vbe_info("\033[1;33m %s %s %d: I2C_Write_EX NG, try again --%d\033[m\n", __FILE__, __FUNCTION__, __LINE__, i);
				msleep(40);	//re-try no need busy wait 	//important!! one write cycle must delay 5~10ms, must not be removed
				communicateToWatchDog();
				if (i==9) {
					rtd_pr_vbe_info("\033[1;31m %s %s %d: [I2CSettingToMillet] I2C Write fail 2\033[m\n", __FILE__, __FUNCTION__, __LINE__);
				}
			}
			else {
				//S_OK, leave re-try
...
UINT32 drvif_scalerdisplay_MilletI2C_Read(UINT16 addr)
{
    ...
    if (m_TCON_I2C_DEVICE_ADDR) {
	    if (I2C_Read_EX(m_TCON_I2C_MASTER, m_TCON_I2C_DEVICE_ADDR, 6, dat, 5, readData, 0) != S_OK) {
	        rtd_pr_vbe_info("[I2CSettingToMillet] I2C Read fail\n");
	    }
    }
   ...
   if(	readData[4] == (dat[2]^dat[3]^dat[4]^dat[5]^readData[0] ^readData[1] ^readData[2]^readData[3]) )
   	rtd_pr_vbe_info("Check Sun OK!\n");
   else
   	rtd_pr_vbe_info("Check Sun Fail!\n");

    rtd_pr_vbe_info("Addr (%x), readData (%x.%x.%x.%x), ret : (%x) \n", addr, readData[0], readData[1], readData[2], readData[3], ret);
...
UINT32 drvif_scalerdisplay_MilletI2C_Write(UINT16 addr, UINT32 data)
{
    ...
    if (m_TCON_I2C_DEVICE_ADDR) {
	    if (I2C_Write_EX(m_TCON_I2C_MASTER, m_TCON_I2C_DEVICE_ADDR, sizeof(dat), dat, 0) != S_OK) {
	        rtd_pr_vbe_info("[I2CSettingToMillet] I2C Write fail\n");
		 ret = 0;
	    }
    }
    ret = 1;
...
{//This is no used
	...
	rtd_pr_vbe_debug("[Setting Millet3] step1...\n");

	scalerdisplay_set_main_ctrl();
	scalerdisplay_set_main_display_window(0, _DISP_WID, 0, _DISP_LEN, 0);

	rtd_pr_vbe_debug("[Setting Millet3] step2...\n");
...

	{
		...
		if(display_timing_ctrl1_reg.disp_fsync_en == 0){
			...
			rtd_pr_vbe_debug("last line need equal to h_total total \n");
			dh_total_last_line_length_reg.regValue = IoReg_Read32(PPOVERLAY_DH_Total_Last_Line_Length_reg);
			...
			&& (drvif_I3DDMA_Get_Input_Info_Vaule(I3DDMA_INFO_INPUT_SRC_TYPE) == _SRC_VO) && (Scaler_Get3DMode((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY)) != SLR_3DMODE_2D_CVT_3D))
		{
			...
			rtd_pr_vbe_debug("[3D][VO] reset h_total\n");
			dh_total_last_line_length_reg.regValue = IoReg_Read32(PPOVERLAY_DH_Total_Last_Line_Length_reg);
			dh_total_last_line_length_reg.dh_total = (Get_DISP_HORIZONTAL_TOTAL()- 1)& 0x00001fff;
			dh_total_last_line_length_reg.dh_total_last_line = dh_total_last_line_length_reg.dh_total;
			IoReg_Write32(PPOVERLAY_DH_Total_Last_Line_Length_reg, dh_total_last_line_length_reg.regValue);
	...
	rtd_pr_vbe_debug("[Setting Millet3] step3...\n");
	/*WaitFor_DEN_STOP();*/
...
void scaler_dtg_reconfig(void)
{
	...
	spin_unlock_irqrestore(get_DBreg_spinlock(SLR_MAIN_DISPLAY), flags);//unlock double buffer spinlock
	up(get_forcebg_semaphore());/*semaphore must added for not effect mute function when setting b8028300*/
	rtd_pr_vbe_debug("[scaler_dtg_reconfig] dtg 0xb802800c ~ 0xb8028018\n");
...
//#ifdef VBY_ONE_PANEL
	...
	if(Get_PANEL_3D_SG_OUTPUT_120HZ_ENABLE() &&
		...
		if(sr_mode == SR_H2_V1){	// 4k2k 120Hz
			rtd_pr_vbe_info("[scaler_dtg_reconfig] Hx2 Vx1 re-config dvtotal\n");
...

	{
		...
		{
			rtd_pr_vbe_debug("Get_DISP_DEN_STA_VPOS : %d \n",Get_DISP_DEN_STA_VPOS());
...
{//This is no used.
	...
	rtd_pr_vbe_debug("[scaler_dtg_reconfig] mtg 0xb8028110 ~ 0xb8028124\n");
	{	//0xb8028110, 0xb8028118
		main_den_h_start_end_reg.regValue = IoReg_Read32(PPOVERLAY_MAIN_DEN_H_Start_End_reg);
...
void scaler_PixelWorks_PA168_set_LvdsInutFmt(PA168_LVDS_INPUT_FMT lvdsFmt)
{
	...
	rtd_pr_vbe_info("[PA168] Set LVDS Fmt=%d\n", lvdsFmt);
...
void scaler_PixelWorks_PA168_captureDisable(unsigned char mode)
{
	...
	msg[1] = mode;
	I2C_Write_EX(3, PA168_I2C_SLAVE_ID, 2, msg, 0);
	rtd_pr_vbe_info("[PA168] Cap OFF=%d\n", mode);
	return;
}
...
void scaler_PixelWorks_PA168_lvdsDisable(unsigned char mode)
{
	...
	if(mode){
		...
		rtd_pr_vbe_info("[PA168] Lvds Down\n");
		msleep(50);
	}else{
		// LVDS resume
	...
	//if(lastTconMode != curTconMode)
	{
		...
		if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 230) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 310) && !drvif_scaelr3d_decide_is_3D_display_mode()){
			if(_DISP_WID > 1920){
				...
				rtd_pr_vbe_info("[PA168][WARN] SHOULD NOT BE HERE!!\n");
			}
		}
		...
		rtd_pr_vbe_info("[PA168] step3...\n");
		msleep(200);
	}

	WaitFor_DEN_STOP();
	IoReg_Write32(PIF_LVDS_CTRL3_reg, lvds_ctrl3_reg.regValue);
	rtd_pr_vbe_info("[PA168] CH=%d->%d\n", lastTconMode, curTconMode);
	lastTconMode = curTconMode;
...
unsigned int scalerdisplay_get_vtotal_by_vfreq(UINT16 vsyncFreq)
{
	...
	unsigned int h_total = Get_DISP_HORIZONTAL_TOTAL();
	unsigned int disp_stage1_data_rate = 0;
	extern unsigned int Get_dispd_stage1_f2p_data_rate(void);

	rtd_pr_vbe_debug("######## vsyncFreq:%d ###############\n",vsyncFreq);

	//fix me
	if(vsyncFreq == 0){
		rtd_pr_vbe_emerg("######## vsyncFreq:%d, protect to 60Hz ###############\n",vsyncFreq);
		vsyncFreq = 60;
	}
	...
	if(vbe_disp_get_adaptivestream_fs_mode()){
		rtd_pr_vbe_notice("### scaler_disp_smooth_set_memc_protect_vtotal !!! ###\n");
		vertical_total = scaler_disp_smooth_get_memc_protect_vtotal();
	}
#endif

	disp_stage1_data_rate = Get_dispd_stage1_f2p_data_rate();

//	if(vertical_total==0){
	if (0 != h_total)
		vertical_total = disp_stage1_data_rate / (h_total * vsyncFreq);
	else
		rtd_pr_vbe_err("DISP_HORIZONTAL_TOTAL = 0\n");

	if ((Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI))
		vertical_total &= 0xFFFFFFFE;

	rtd_pr_vbe_debug("######## vertical_total:%d ###############\n",
		vertical_total);

	return vertical_total;
}

unsigned int scalerdisplay_get_DTG_MASTER_vtotal_by_vfreq(void)
{
	return scalerdisplay_get_vtotal_by_vfreq(Scaler_DispGetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ));
}
...
//		registerAddr = PPOVERLAY_D3DTG_DVS_CNT_VADDR;
	       ...
	       rtd_pr_vbe_debug("[ScalerDisplay] Get frame rate type error (%d) \n", VsyncType);
		return 0;
	}
	...
	rtd_pr_vbe_debug("[ScalerDisplay] Get Frame rate (type: %d, value : %d) \n", VsyncType, vsyncFreq);
	return vsyncFreq;
}
...
void scalerdisplay_set_PWM_Tuning(void)
{
	...
	if((display_timing_ctrl1_reg.disp_frc_fsync == 1)&&(display_timing_ctrl1_reg.disp_fsync_en == 1)){	//framesync timing
		...
		if(display_timing_ctrl1_reg.double_dvs_en == 1){
			...
			rtd_pr_vbe_info("[scalerdisplay_set_PWM_Tuning] Double DVS\n");
		}
		rtd_pr_vbe_info("[scalerdisplay_set_PWM_Tuning] FrameSync : vSyncFreq (%d)\n", vSyncFreq);
	}
		...
		rtd_pr_vbe_info("[scalerdisplay_set_PWM_Tuning] FRC : vSyncFreq (%d)\n", vSyncFreq);
	}
...
{
	rtd_pr_vbe_debug("scalerdisplay_set_disp2tve_flow : mode => %d\n", mode);
...
void scalerdisplay_change_timing_to_disp2tve(unsigned char mode){
	...
	*/
	...
	rtd_pr_vbe_debug("[crixus_test]scalerdisplay_change_timing_to_disp2tve\n");
	...
	switch(resolution_type){
		...
		default:
			...
			rtd_pr_vbe_debug("[Error] Unhandle Mode !!!!\n");
			DH_Total = 2200;
			DV_Total = 1125;
			DH_Wid = 1920;
			DV_Len = 1080;
...
void TPVSetWBGrayPattern(char gray, bool Enable)
{
    ...
    rtd_pr_vbe_debug("function=%s, line=%d, enable=%x\n", __FUNCTION__, __LINE__, Enable);
	down(get_forcebg_semaphore());/*semaphore must added for not effect mute function when setting b8028300*/
	//Wait Display Data enable end period
...
//void Scaler_MEMC_output_force_bg_enable(UINT8 enable,char const* pfunc,UINT32 line);
//#endif
...
void drvif_scalerdisplay_wait_d0_apply(unsigned char display)
{
	...
	if(SLR_MAIN_DISPLAY == display){
		...
		ppoverlay_double_buffer_ctrl3_reg.regValue = IoReg_Read32(PPOVERLAY_Double_Buffer_CTRL3_reg);
		while((timeoutcount) && (ppoverlay_double_buffer_ctrl3_reg.dmainreg_other_dbuf_set)){
			timeoutcount--;
			ppoverlay_double_buffer_ctrl3_reg.regValue = IoReg_Read32(PPOVERLAY_Double_Buffer_CTRL3_reg);
		}
		if(timeoutcount == 0){
			rtd_pr_vbe_debug("[FBG MAIN]timeout error!!!\n");
			drvif_scalerdisplay_set_forcebg_dbuffer(SLR_MAIN_DISPLAY, FALSE);
		}
	}else{//for sub
...
static StructSrcRect sub_mutewin = {0, 0, 0, 0};//for sub mute window area

void set_sub_mutewin(StructSrcRect winRect)
{
	sub_mutewin.srcx = winRect.srcx;
	sub_mutewin.srcy = winRect.srcy;
	sub_mutewin.src_wid = winRect.src_wid;
	sub_mutewin.src_height = winRect.src_height;
}
...
void scalerdisplay_force_bg_enable(unsigned int channel, unsigned char enable)
{
...
#ifndef ENABLE_IMD_PST_SMOOTHTOGGLE
...
#endif
	extern void set_pst_active_state(unsigned char display, unsigned short state);
	rtd_pr_vbe_notice("%s,Disp[%d]=%d\n", __FUNCTION__, channel, enable);
	...
	if (!enable) {
		...
		{
			rtd_pr_vbe_notice("\r### scaler(%d) is not active, so can not disable bg ###\r\n", channel);
			return;
		}
	}
  ...
  if(enable) //different algo. enable:new   disable:old
  {
	...
	if(channel == SLR_MAIN_DISPLAY){//main path
		...
		#if IS_ENABLED(CONFIG_RTK_AI_DRV) // lesley 1022
		extern bool bg_flag;
		...
		ppoverlay_double_buffer_ctrl3_reg.regValue = IoReg_Read32(PPOVERLAY_Double_Buffer_CTRL3_reg);
		read_level = ppoverlay_double_buffer_ctrl3_reg.dmainreg_other_dbuf_read_sel;
		ppoverlay_double_buffer_ctrl3_reg.dmainreg_other_dbuf_read_sel = 0;
		IoReg_Write32(PPOVERLAY_Double_Buffer_CTRL3_reg, ppoverlay_double_buffer_ctrl3_reg.regValue);
		...
		if (enable == main_display_control_rsv_reg.m_force_bg) {
			ppoverlay_double_buffer_ctrl3_reg.regValue = IoReg_Read32(PPOVERLAY_Double_Buffer_CTRL3_reg);
			ppoverlay_double_buffer_ctrl3_reg.dmainreg_other_dbuf_read_sel = read_level;
			IoReg_Write32(PPOVERLAY_Double_Buffer_CTRL3_reg, ppoverlay_double_buffer_ctrl3_reg.regValue);
		...
		rtd_pr_vbe_notice("####RTK Enable Main ForceBG(Black screen)!!!####\r\n");
//#ifdef CONFIG_ORBIT_PIXEL_SHIFT_ALGO
...
//#endif
		...
		if(sub_mutewin.src_wid && sub_mutewin.src_height)
		{//enable sub window
			PipmpSetSubMuteWindow(sub_mutewin.srcx, sub_mutewin.src_wid, sub_mutewin.srcy, sub_mutewin.src_height);
		...
		drvif_scalerdisplay_set_forcebg_dbuffer(SLR_SUB_DISPLAY, FALSE);
  ...
  else
  {
	if(channel == SLR_MAIN_DISPLAY){//main path
		...
		if (enable == main_display_control_rsv_reg.m_force_bg) {
			...
			ppoverlay_double_buffer_ctrl3_reg.dmainreg_other_dbuf_read_sel = read_level;
			IoReg_Write32(PPOVERLAY_Double_Buffer_CTRL3_reg, ppoverlay_double_buffer_ctrl3_reg.regValue);
			rtd_pr_vbe_info("n####[FBG MAIN]Already disable forceBG, Scaler_MEMC_Get_CheckMEMC_Outbg=%d####\r\n",Scaler_MEMC_Get_CheckMEMC_Outbg());
		...
		if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC)
			rtd_pr_vbe_notice("####RTK Disable Main ForceBG avsyn:%d film:%d di_pause:%d!!!####\r\n", get_vo_avsync_flag(0), get_film_detect_done_flag(0), VODMA_vodma_i2rnd_m_flag_get_i2rnd_m_nxt_rpt_flag_fw(IoReg_Read32(VODMA_vodma_i2rnd_m_flag_reg)));
		else
			rtd_pr_vbe_notice("####RTK Disable Main ForceBG!!!####\r\n");
		enable_data_access(SLR_MAIN_DISPLAY, TRUE);
	}
#ifdef CONFIG_DUAL_CHANNEL
	else if(channel == SLR_SUB_DISPLAY){//sub path
		...
		if (!enable) {
			for (i = 0; i < SLR_FORCE_BG_TYPE_MAX; i++) {
				if (display_forcebg_mask[channel][i]) {
					mp_layout_force_to_background_reg.s_force_bg = TRUE;
					IoReg_Write32(PPOVERLAY_MP_Layout_Force_to_Background_reg, mp_layout_force_to_background_reg.regValue);
					drvif_scalerdisplay_set_forcebg_dbuffer(SLR_SUB_DISPLAY, FALSE);
					rtd_pr_vbe_notice("\r\n### mask(%d)(%d) is enable, so sub change to force bg###\r\n", channel, i);
					return;
				}
			}
		}
		...
		if(1){//Revert
			do {
				...
				IoReg_Write32(PPOVERLAY_DTG_LC_reg, dtg_lc_reg.regValue);
				if((IoReg_Read32(PPOVERLAY_DTG_pending_status_2_reg) & _BIT4)) {
					IoReg_Write32(PPOVERLAY_MP_Layout_Force_to_Background_reg, mp_layout_force_to_background_reg.regValue);
					break;
				}
				timeoutcount--;
		...
		if(Get_DisplayMode_Src(SLR_SUB_DISPLAY) == VSC_INPUTSRC_VDEC)
			rtd_pr_vbe_notice("####RTK Disable Sub ForceBG avsyn:%d !!!####\r\n", get_vo_avsync_flag(1));
		else
			rtd_pr_vbe_notice("####RTK Disable Sub ForceBG !!!####\r\n");
		enable_data_access(SLR_SUB_DISPLAY, TRUE);
	}
#endif
	...
	if(!enable)
	{
		...
		dtg_pending_status_2_reg.dtg_vlc_status_2 = 1;
        ...
        rtd_pr_vbe_debug("[%s][%d]disable FBG!!\n", __FUNCTION__, __LINE__);
	}
    else
        rtd_pr_vbe_debug("[%s][%d]enable FBG!!\n", __FUNCTION__, __LINE__);
...
void set_display_forcebg_mask_by_isr(unsigned char display, unsigned char masktype, unsigned char maskvalue)
{//use spin lock for ISR
#ifdef CONFIG_SCALER_BRING_UP
	if ( masktype != SLR_FORCE_BG_TYPE_VSC)
		maskvalue = FALSE;
#endif
...
{//use spin_lock_irqsave and spin_unlock_irqrestore for normal task
	unsigned long flags;//for spin_lock_irqsave
#ifdef CONFIG_SCALER_BRING_UP
	if ( masktype != SLR_FORCE_BG_TYPE_VSC)
		maskvalue = FALSE;
...
#endif
	if(display == SLR_MAIN_DISPLAY)
		spin_lock_irqsave(&ForceBG_Main_Mask_Spinlock, flags);
	else
		spin_lock_irqsave(&ForceBG_Sub_Mask_Spinlock, flags);
	if (masktype >= SLR_FORCE_BG_TYPE_MAX) {
		if(display == SLR_MAIN_DISPLAY)
			spin_unlock_irqrestore(&ForceBG_Main_Mask_Spinlock, flags);
		else
			spin_unlock_irqrestore(&ForceBG_Sub_Mask_Spinlock, flags);
		rtd_pr_vbe_debug("\r\n###set_display_forcebg_mask type(%d) error###\r\n", masktype);
		return;
	}
	if(display_forcebg_mask[display][masktype] == maskvalue)
	{//value is the same
		if(display == SLR_MAIN_DISPLAY)
			spin_unlock_irqrestore(&ForceBG_Main_Mask_Spinlock, flags);
		else
	...
	if(masktype != SLR_FORCE_BG_TYPE_VSC)
	{//internal mute mask control
		rtd_pr_vbe_info("###internal set_display_forcebg_mask display(%d) type(%d) value(%d)###\r\n", display, masktype, maskvalue);
	}
}
...
{//TRUE: Enable force bg. FALSE: Disable force bg. Only check AP mask
...
unsigned char Check_ForceBG_Mask_Enable(unsigned char display, unsigned char status_print)
{//TRUE: Enable force bg. FALSE: Disable force bg. Check all mask
	int i;
	for (i = 0; i < SLR_FORCE_BG_TYPE_MAX; i++) {
		if (display_forcebg_mask[display][i]) {
			if(status_print)
				rtd_pr_vbe_notice("\r\n###Check_ForceBG_Mask_Enable mask(%d)(%d) is enable ###\r\n", display, i);
			return TRUE;
		}
	}
	return FALSE;
...
void check_abnormal_mute_on_condition(unsigned char display)
{//this is for debug use. when all mask is disable. print any abnormal case

}

void mute_control(unsigned int display, unsigned char enable)
{//use semaphore and call mute on/ off
	down(get_forcebg_semaphore());
	scalerdisplay_force_bg_enable(display, enable);
...
void update_dtgm2uzu_delay(void)
{//chnage 0xb802850c and 0xb8028510 delay
	ppoverlay_uzudtg_control2_RBUS ppoverlay_uzudtg_control2_reg;
	unsigned char need_update = FALSE;
	if(Get_DISPLAY_REFRESH_RATE() >= 120)
	{// only 120 panel need to change the setting
		if(force_disp_s1_s2_same_clk() || (get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1))
		{
			ppoverlay_uzudtg_control2_reg.regValue = IoReg_Read32(PPOVERLAY_uzudtg_control2_reg);
			if(ppoverlay_uzudtg_control2_reg.dtgm2uzu_s1_vs_pixel != 0x98)
			{
				ppoverlay_uzudtg_control2_reg.dtgm2uzu_s1_vs_pixel = 0x98;
				IoReg_Write32(PPOVERLAY_uzudtg_control2_reg, ppoverlay_uzudtg_control2_reg.regValue);
				need_update = TRUE;
			}
			if(ppoverlay_uzudtg_control2_reg.dtgm2uzu_s1_vs_line != 0x86)
			{
				ppoverlay_uzudtg_control2_reg.dtgm2uzu_s1_vs_line = 0x86;
				IoReg_Write32(PPOVERLAY_uzudtg_control2_reg, ppoverlay_uzudtg_control2_reg.regValue);
				need_update = TRUE;
			}
		}
		...
		{
			ppoverlay_uzudtg_control2_reg.regValue = IoReg_Read32(PPOVERLAY_uzudtg_control2_reg);
			if(ppoverlay_uzudtg_control2_reg.dtgm2uzu_s1_vs_pixel != 0x4e)
			{
				ppoverlay_uzudtg_control2_reg.dtgm2uzu_s1_vs_pixel = 0x4e;
				IoReg_Write32(PPOVERLAY_uzudtg_control2_reg, ppoverlay_uzudtg_control2_reg.regValue);
				need_update = TRUE;
			}
			if(ppoverlay_uzudtg_control2_reg.dtgm2uzu_s1_vs_line != 0x7f)
			{
				ppoverlay_uzudtg_control2_reg.dtgm2uzu_s1_vs_line = 0x7f;
				IoReg_Write32(PPOVERLAY_uzudtg_control2_reg, ppoverlay_uzudtg_control2_reg.regValue);
				need_update = TRUE;
			}
		}
		if(need_update)
		{
			IoReg_SetBits(PPOVERLAY_Double_Buffer_CTRL2_reg, _BIT16);
		}
	}
}


//for RTKREQ-823 application
static ppoverlay_main_den_h_start_end_RBUS main_den_h_start_end_reg_backup;
static ppoverlay_main_den_v_start_end_RBUS main_den_v_start_end_reg_backup;
static ppoverlay_main_background_h_start_end_RBUS ppoverlay_main_background_h_start_end_reg_backup;
static ppoverlay_main_background_v_start_end_RBUS ppoverlay_main_background_v_start_end_reg_backup;
static ppoverlay_main_background_color1_RBUS ppoverlay_main_background_color1_reg_backup;
static ppoverlay_main_background_color2_RBUS ppoverlay_main_background_color2_reg_backup;
static ppoverlay_main_border_highlight_border_color2_RBUS ppoverlay_main_border_highlight_border_color2_reg_backup;
static ppoverlay_main_border_highlight_border_color3_RBUS ppoverlay_main_border_highlight_border_color3_reg_backup;
static unsigned char inner_pattern_enable = FALSE;

unsigned char get_inner_pattern_enable(void)
{
	return inner_pattern_enable;
}


void ctrl_inner_pattern(unsigned char on_off, inner_pattern_winbox_win_attr *inner_info_array)
{
	...
	ppoverlay_main_background_color1_RBUS ppoverlay_main_background_color1_reg;
	ppoverlay_main_background_color2_RBUS ppoverlay_main_background_color2_reg;
	ppoverlay_main_border_highlight_border_color2_RBUS ppoverlay_main_border_highlight_border_color2_reg;
	ppoverlay_main_border_highlight_border_color3_RBUS ppoverlay_main_border_highlight_border_color3_reg;

	if(on_off)
	{
		down(get_forcebg_semaphore());
		if(inner_pattern_enable == FALSE)
		{
			inner_pattern_enable = TRUE;
			main_den_h_start_end_reg_backup.regValue = IoReg_Read32(PPOVERLAY_MAIN_DEN_H_Start_End_reg);
			main_den_v_start_end_reg_backup.regValue = IoReg_Read32(PPOVERLAY_MAIN_DEN_V_Start_End_reg);

			ppoverlay_main_background_h_start_end_reg_backup.regValue = IoReg_Read32(PPOVERLAY_MAIN_Background_H_Start_End_reg);
			ppoverlay_main_background_v_start_end_reg_backup.regValue = IoReg_Read32(PPOVERLAY_MAIN_Background_V_Start_End_reg);

			ppoverlay_main_background_color1_reg_backup.regValue = IoReg_Read32(PPOVERLAY_Main_Background_Color1_reg);
			ppoverlay_main_background_color2_reg_backup.regValue = IoReg_Read32(PPOVERLAY_Main_Background_Color2_reg);

			ppoverlay_main_border_highlight_border_color2_reg_backup.regValue = IoReg_Read32(PPOVERLAY_Main_Border_Highlight_Border_Color2_reg);
			ppoverlay_main_border_highlight_border_color3_reg_backup.regValue = IoReg_Read32(PPOVERLAY_Main_Border_Highlight_Border_Color3_reg);

			set_display_forcebg_mask(SLR_MAIN_DISPLAY, SLR_FORCE_BG_TYPE_INNER_PATTERN, 1);
		}
		//set board
		main_den_h_start_end_reg.regValue = IoReg_Read32(PPOVERLAY_MAIN_DEN_H_Start_End_reg);
		main_den_h_start_end_reg.mh_den_sta = (inner_info_array[0].x % 2)? inner_info_array[0].x - 1: inner_info_array[0].x;
		main_den_h_start_end_reg.mh_den_end = ((inner_info_array[0].w + inner_info_array[0].x) % 2)? (inner_info_array[0].w + inner_info_array[0].x) - 1: (inner_info_array[0].w + inner_info_array[0].x);
		IoReg_Write32(PPOVERLAY_MAIN_DEN_H_Start_End_reg, main_den_h_start_end_reg.regValue);

		
		main_den_v_start_end_reg.regValue = IoReg_Read32(PPOVERLAY_MAIN_DEN_V_Start_End_reg);
		main_den_v_start_end_reg.mv_den_sta = (inner_info_array[0].y % 2)? inner_info_array[0].y - 1: inner_info_array[0].y;
		main_den_v_start_end_reg.mv_den_end = ((inner_info_array[0].h + inner_info_array[0].y) % 2)? (inner_info_array[0].h + inner_info_array[0].y) - 1: (inner_info_array[0].h + inner_info_array[0].y);
		IoReg_Write32(PPOVERLAY_MAIN_DEN_V_Start_End_reg, main_den_v_start_end_reg.regValue);

		ppoverlay_main_border_highlight_border_color2_reg.regValue = IoReg_Read32(PPOVERLAY_Main_Border_Highlight_Border_Color2_reg);
		ppoverlay_main_border_highlight_border_color2_reg.m_bd_b = inner_info_array[0].fill_B << 4;
		IoReg_Write32(PPOVERLAY_Main_Border_Highlight_Border_Color2_reg, ppoverlay_main_border_highlight_border_color2_reg.regValue);

		
		ppoverlay_main_border_highlight_border_color3_reg.regValue = IoReg_Read32(PPOVERLAY_Main_Border_Highlight_Border_Color3_reg);
		ppoverlay_main_border_highlight_border_color3_reg.m_bd_r = inner_info_array[0].fill_R << 4;
		ppoverlay_main_border_highlight_border_color3_reg.m_bd_g = inner_info_array[0].fill_G << 4;
		IoReg_Write32(PPOVERLAY_Main_Border_Highlight_Border_Color3_reg, ppoverlay_main_border_highlight_border_color3_reg.regValue);

		//set forcebg
		ppoverlay_main_background_h_start_end_reg.regValue = IoReg_Read32(PPOVERLAY_MAIN_Background_H_Start_End_reg);
		ppoverlay_main_background_h_start_end_reg.mh_bg_sta = (inner_info_array[1].x % 2)? inner_info_array[1].x - 1: inner_info_array[1].x;
		ppoverlay_main_background_h_start_end_reg.mh_bg_end = ((inner_info_array[1].w + inner_info_array[1].x) % 2)? (inner_info_array[1].w + inner_info_array[1].x) - 1: (inner_info_array[1].w + inner_info_array[1].x);
		...
		ppoverlay_main_background_v_start_end_reg.mv_bg_sta = (inner_info_array[1].y % 2)? inner_info_array[1].y - 1: inner_info_array[1].y;
		ppoverlay_main_background_v_start_end_reg.mv_bg_end = ((inner_info_array[1].h + inner_info_array[1].y) % 2)? (inner_info_array[1].h + inner_info_array[1].y) - 1: (inner_info_array[1].h + inner_info_array[1].y);
		IoReg_Write32(PPOVERLAY_MAIN_Background_V_Start_End_reg, ppoverlay_main_background_v_start_end_reg.regValue);

		ppoverlay_main_background_color1_reg.regValue = IoReg_Read32(PPOVERLAY_Main_Background_Color1_reg);
		ppoverlay_main_background_color1_reg.m_bg_b = inner_info_array[1].fill_B << 4;
		ppoverlay_main_background_color1_reg.m_bg_g = inner_info_array[1].fill_G << 4;
		IoReg_Write32(PPOVERLAY_Main_Background_Color1_reg, ppoverlay_main_background_color1_reg.regValue);

		ppoverlay_main_background_color2_reg.regValue = IoReg_Read32(PPOVERLAY_Main_Background_Color2_reg);
		ppoverlay_main_background_color2_reg.m_bg_r = inner_info_array[1].fill_R << 4;
		IoReg_Write32(PPOVERLAY_Main_Background_Color2_reg, ppoverlay_main_background_color2_reg.regValue);

		IoReg_SetBits(PPOVERLAY_Double_Buffer_CTRL_reg, _BIT0);

		scalerdisplay_force_bg_enable(SLR_MAIN_DISPLAY, 1);
		up(get_forcebg_semaphore());

	}
	else
	{
		if(inner_pattern_enable)
		{
			down(get_forcebg_semaphore());
			inner_pattern_enable = FALSE;
			set_display_forcebg_mask(SLR_MAIN_DISPLAY, SLR_FORCE_BG_TYPE_INNER_PATTERN, 0);
			//set board
			main_den_h_start_end_reg.regValue = main_den_h_start_end_reg_backup.regValue;
			IoReg_Write32(PPOVERLAY_MAIN_DEN_H_Start_End_reg, main_den_h_start_end_reg.regValue);
			
			main_den_v_start_end_reg.regValue = main_den_v_start_end_reg_backup.regValue;
			IoReg_Write32(PPOVERLAY_MAIN_DEN_V_Start_End_reg, main_den_v_start_end_reg.regValue);
			
			ppoverlay_main_border_highlight_border_color2_reg.regValue = ppoverlay_main_border_highlight_border_color2_reg_backup.regValue;
			IoReg_Write32(PPOVERLAY_Main_Border_Highlight_Border_Color2_reg, ppoverlay_main_border_highlight_border_color2_reg.regValue);

			ppoverlay_main_border_highlight_border_color3_reg.regValue = ppoverlay_main_border_highlight_border_color3_reg_backup.regValue;
			IoReg_Write32(PPOVERLAY_Main_Border_Highlight_Border_Color3_reg, ppoverlay_main_border_highlight_border_color3_reg.regValue);

			//set forcebg
			ppoverlay_main_background_h_start_end_reg.regValue = ppoverlay_main_background_h_start_end_reg_backup.regValue;
			IoReg_Write32(PPOVERLAY_MAIN_Background_H_Start_End_reg, ppoverlay_main_background_h_start_end_reg.regValue);
			
			ppoverlay_main_background_v_start_end_reg.regValue = ppoverlay_main_background_v_start_end_reg_backup.regValue;
			IoReg_Write32(PPOVERLAY_MAIN_Background_V_Start_End_reg, ppoverlay_main_background_v_start_end_reg.regValue);
			
			ppoverlay_main_background_color1_reg.regValue = ppoverlay_main_background_color1_reg_backup.regValue;
			IoReg_Write32(PPOVERLAY_Main_Background_Color1_reg, ppoverlay_main_background_color1_reg.regValue);
			
			ppoverlay_main_background_color2_reg.regValue = ppoverlay_main_background_color2_reg_backup.regValue;
			IoReg_Write32(PPOVERLAY_Main_Background_Color2_reg, ppoverlay_main_background_color2_reg.regValue);

			IoReg_SetBits(PPOVERLAY_Double_Buffer_CTRL_reg, _BIT0);
			
			scalerdisplay_force_bg_enable(SLR_MAIN_DISPLAY, 0);
			up(get_forcebg_semaphore());
		}
	}
