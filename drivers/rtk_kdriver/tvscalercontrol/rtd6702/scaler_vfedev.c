...
#define HDMI_PORT_NUM 4//hdmi physical port number
...
unsigned char HDMI_Reply_Zero_Timing[HDMI_PORT_NUM] = {FALSE, FALSE, FALSE, FALSE};/*If true, let webos get zero timing according physical port*/
static unsigned char HDMI_Reply_Zero_Timing_mute_cnt[HDMI_PORT_NUM] = {0, 0, 0, 0};
static unsigned int HDMI_ZERO_REPORT_RECORD_TIME[HDMI_PORT_NUM] = {0, 0, 0, 0};//when ap polling the timing and the zero timing flag is ture. Record the time
...
void Set_HDMI_Reply_Zero_Timing_Flag(unsigned char connect_port, unsigned char flag)
{//only for hdmi
	unsigned long flags;//for spin_lock_irqsave
	if(connect_port >= HDMI_PORT_NUM)
	{
		rtd_pr_vfe_info("##func:%s port :%d error##\r\n",__FUNCTION__, connect_port);
		return;
	}
	if((flag == SOURCE_GOOD_TIMING_READY) && (HDMI_Reply_Zero_Timing[connect_port] & SOURCE_GOOD_TIMING_READY))
		return;
	spin_lock_irqsave(&HDMI_ZERO_TIMING_CTRL, flags);
	if(flag == REPORT_ZERO_TIMING){
		if(HDMI_Reply_Zero_Timing[connect_port] & SOURCE_GOOD_TIMING_READY)
			HDMI_Reply_Zero_Timing[connect_port] = HDMI_Reply_Zero_Timing[connect_port] | REPORT_ZERO_TIMING;
	} else {
		HDMI_Reply_Zero_Timing[connect_port] = flag;
	}
	spin_unlock_irqrestore(&HDMI_ZERO_TIMING_CTRL, flags);
	rtd_pr_vfe_info("##func:%s HDMI value:(%d %d) port:%d ##\r\n",__FUNCTION__, flag, HDMI_Reply_Zero_Timing[connect_port], connect_port);
...
int VFE_AVD_ATV_CONNECT(unsigned short PortNumber)
{
	...
	{
		rtd_pr_vfe_debug("VFE_AVD_ATV_CONNECT demodType error. DemodType=%d\n", tDemodType);
	}
...
{
	if (vbi_clk_get_enable() && pal_wss_flag) {
		palwss_data_send();
		pal_wss_flag=0;
	}
}
...
void VBI_Suspend(void)
{
	...
	if (vbi_connect_flag&(VBI_NTSC_CC_BIT|VBI_PALN_CC_BIT)) {/* cc open */
		...
		if (VBI_CC_Register_Backup) {
			...
			VBI_CC_Register_Backup[index++] = vdcf_inl(ADC_ADC_CLKGEN_RGB_Capture_reg);//Remain ADC register for Mark2
	...
	if (vbi_connect_flag & VBI_AV_CGMS_BIT) {/* cgms_cvbs open */
		...
		if (VBI_CC_Register_Backup) {
			...
			if (VBI_CGMS_Register_Backup) {
				...
				VBI_CGMS_Register_Backup[index++] = vdcf_inl(ADC_ADC_CLKGEN_RGB_Capture_reg);//Remain ADC register for Mark2
				VBI_CGMS_Register_Backup[index++] = vdcf_inl(VBI_CGMS_C0_reg);
				VBI_CGMS_Register_Backup[index++] = vdcf_inl(VBI_CGMS_C1_reg);
				VBI_CGMS_Register_Backup[index++] = vdcf_inl(VBI_CGMS_C2_reg);
				VBI_CGMS_Register_Backup[index++] = vdcf_inl(VBI_CGMS_C3_reg);
...
void VBI_Resume(void)
{
	...
	if (VBI_TTX_Register_Backup) {
		...
		if (IoReg_Read32(STB_SC_VerID_reg) == 0x65050000) {//version A
	...
	if (VBI_CC_Register_Backup) {
		...
		IoReg_Mask32(ADC_ADC_CLKGEN_RGB_Capture_reg, ~_BIT2,((VBI_CC_Register_Backup[index++])&_BIT2));//Remain ADC register for Mark2
	...
	if (VBI_CGMS_Register_Backup) {
		...
		if (VBI_CC_Register_Backup) {
			...
			IoReg_Mask32(ADC_ADC_CLKGEN_RGB_Capture_reg, ~_BIT2,((VBI_CGMS_Register_Backup[index++])&_BIT2));//Remain ADC register for Mark2
			vdcf_outl(VBI_CGMS_C0_reg, VBI_CGMS_Register_Backup[index++]);
			vdcf_outl(VBI_CGMS_C1_reg, VBI_CGMS_Register_Backup[index++]);
			vdcf_outl(VBI_CGMS_C2_reg, VBI_CGMS_Register_Backup[index++]);
			vdcf_outl(VBI_CGMS_C3_reg, VBI_CGMS_Register_Backup[index++]);
...
//	abl_reserved_RBUS abl_Reserved_reg;//for debug test
	...
	vfe_hdmi_hdcp_data_apply_t hdcp_data_apply = {0, ((PBYTES) 0),0};
	/* ====================== */
	...
	switch(cmd)
	{
		...
		case VFE_IOC_AVD_INIT:
			...
			else
			{
				...
				//rtd_pr_vfe_debug("kernel VFE_IOC_AVD_INIT get parameter success\n");
		...
		case VFE_IOC_AVD_AV_CONNECT:
			...
			{
				rtd_pr_vfe_info("AV port=%d\n", port);

				// Original
			if(port == 0)
				{
					AVD_Input = _AV_INPUT1;  // Orignal, Vin=10, A0N
				}
				// CVBS, YPP use the same Y
			else if(port == 1)
				{
					AVD_Input = _AV_INPUT2;  // Vin=9, A0N
				}
				// CVBS, YPP use separate Y
			else if(port == 2)
				{
					AVD_Input = _AV_INPUT4;  // Vin=13, A0N
				}
			else
				return -1;//Port number is wrong
			...
			if (0 == Scaler_AVD_AV_Connect(AVD_Input))
			{
				...
				//up(&VDC_DetectSemaphore);
				ret = 0;
				rtd_pr_vfe_debug("kernel VFE_IOC_AVD_AV_CONNECT and Scaler_AVD_AV_Connect() success\n");
			}
			else
			{
				ret = -1;
		...
		case VFE_IOC_AVD_SCART_CONNECT:
			...
			if (0 == Scaler_AVD_Scart_Connect(AVD_Input))
			{
				...
				//up(&VDC_DetectSemaphore);
				ret = 0;
				rtd_pr_vfe_debug("kernel VFE_IOC_AVD_SCART_CONNECT and Scaler_AVD_Scart_Connect() success\n");
			}
			else
			{
				ret = -1;
		...
		case VFE_IOC_AVD_SET_COLOR_SYSTEM:
		{
			...
			//rtd_pr_vfe_info("0   ColorSystem=%x\n", ColorSystem);
		...
		case VFE_IOC_AVD_GET_TIMINGINFO:
			...
			//rtd_pr_vfe_info("kernel VFE_IOC_AVD_GET_TIMINGINFO start\n");
			if (AVD_Global_Status == SRC_CONNECT_DONE) {
				...
				if (0 == ret)
				{
					...
					{
						rtd_pr_vfe_debug("Kernel VFE_IOC_AVD_GET_TIMINGINFO fail\n");
						ret = -1;
					}
					else
					{
		...
		case VFE_IOC_AVD_GET_SCART_ID:
		{
			...
			{
				rtd_pr_vfe_debug("Kernel VFE_IOC_AVD_GET_SCART_ID Fail\n");
				ret = -1;
			}
		...
		case VFE_IOC_AVD_GET_SCART_FAST_BLANKING_STATUS:
		{
			...
			{
				rtd_pr_vfe_debug("Kernel VFE_IOC_AVD_GET_SCART_FAST_BLANKING_STATUS Fail\n");
				ret = -1;
			}
			down(get_adc_power_semaphore());
			if (ADC_Global_Status == SRC_NOTHING || ADC_Global_Status == SRC_INIT_DONE)
		...
		case VFE_IOC_AVD_ATV_SET_CH_CHANGE:
		{
			...
			{
				rtd_pr_vfe_debug("Kernel VFE_IOC_AVD_ATV_SET_CH_CHANGE Fail\n");
				ret = -1;
			}
			break;
		}
		...
		case VFE_IOC_HDMI_DISCONNECT:
			...
			if (vfe_hdmi_drv_disconnect(port) != 0)
			{
				...
				Set_HDMI_Reply_Zero_Timing_Flag(port, CLEAR_ZERO_FLAG);
				up(&SetSource_Semaphore);
			}
			break;
			...
			else
			{
				...
				if (HDMI_Global_Status == SRC_CONNECT_DONE)
				{
					if (vfe_hdmi_drv_is_current_channel(hdmitiminginfo.port))
					{
						...
						if((hdmitiminginfo.port >= 0) && (hdmitiminginfo.port < HDMI_PORT_NUM) && (HDMI_Reply_Zero_Timing[hdmitiminginfo.port] & REPORT_ZERO_TIMING))
						{
							Set_HDMI_Reply_Zero_Timing_Flag(hdmitiminginfo.port, CLEAR_ZERO_FLAG);
							HDMI_Reply_Zero_Timing_mute_cnt[hdmitiminginfo.port] = HDMI_REPLY_ZERO_TIMING_COUNT;
							HDMI_ZERO_REPORT_RECORD_TIME[hdmitiminginfo.port] = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
							// FixMe, 20190919
						...
						if((hdmitiminginfo.port < 0) || (hdmitiminginfo.port >= HDMI_PORT_NUM))
						{
						...
						else if (HDMI_Reply_Zero_Timing_mute_cnt[hdmitiminginfo.port] == ACTUAL_DETECT_RESULT)
						{
							// get timing from HDMI VFE
							hdmiport = hdmitiminginfo.port;
							if (vfe_hdmi_drv_get_port_timing_info(&hdmitiminginfo) == 0)
						...
						{
							if(HDMI_Reply_Zero_Timing_mute_cnt[hdmitiminginfo.port] == CHECK_TIME_PERIOD)
							{
								if(!check_polling_time_period(HDMI_ZERO_REPORT_RECORD_TIME[hdmitiminginfo.port], IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg), HDMI_TIME_PERIOD_THRESHOULD))
								{//Next time return actually timing
									HDMI_Reply_Zero_Timing_mute_cnt[hdmitiminginfo.port] = ACTUAL_DETECT_RESULT;
								}
							}
							else
							{
								...
								HDMI_Reply_Zero_Timing_mute_cnt[hdmitiminginfo.port] = HDMI_Reply_Zero_Timing_mute_cnt[hdmitiminginfo.port] -1 ;

								if (HDMI_Reply_Zero_Timing_mute_cnt[hdmitiminginfo.port] > HDMI_REPLY_ZERO_TIMING_COUNT)
								{
									// FixMe, 20190919
		...
		case VFE_IOC_HDMI_READ_MULTI_VSIF:
		{
			...
			else
			{
				...
				if (p_vsif==NULL) {
					rtd_pr_vfe_debug("[ERR] allocate memory failed\n");
					ret = EFAULT;
				}
				else
				{
		...
		case VFE_IOC_HDMI_GET_EM_PACKET:
		{
			...
			{
				unsigned char *tmp_emp = NULL;
				int len = 0;
				// FixMe, 20190919
				//tmp_emp = (unsigned char*) kmalloc(MAX_EM_HDR_INFO_LEN, GFP_KERNEL);
				if (tmp_emp==NULL)
				{
					rtd_pr_vfe_err("[ERR] VFE_IOC_HDMI_GET_EM_PACKET memeory fail \n");
					ret = -EFAULT;
				}
				else
				{
		...
		case VFE_IOC_HDMI_READ_EDID_EX:
			...
			{
				if ((edid_rw.len != 256) && (edid_rw.len != 384) && (edid_rw.len !=512))
				{
					rtd_pr_vfe_err("[ERR] read edid to HDMI-%d failed, invalid size - %d\n", edid_rw.port,  edid_rw.len);
					ret = -EFAULT;
				}
				else
				{
		...
		case VFE_IOC_HDMI_WRITE_EDID_EX:
			...
			else
			{
				...
				if ((edid_rw.len != 256) && (edid_rw.len != 384) && (edid_rw.len !=512))
				{
					rtd_pr_vfe_err("[ERR] write edid to HDMI-%d failed, invalid size - %d\n", edid_rw.port, edid_rw.len);
					ret = -EFAULT;
				}
				else
				{
					...
					if (p_edid==NULL ||
						...
						|| vfe_hdmi_drv_write_edid(edid_rw.port, p_edid, edid_rw.len)!=0
					)
					{
		...
		case VFE_IOC_HDMI_WRITE_HDCP:
			...
			{
				char *p_ksv = kmalloc(325, GFP_KERNEL);
				char *p_device_key = p_ksv + 5;

				if (p_ksv==NULL || copy_from_user(p_ksv, to_user_ptr(hdcp_data.pksv), 5) ||
		...
		case VFE_IOC_HDMI_WRITE_HDCP_APPLY:
			...
			else
			{
				...
				unsigned int key_size = (hdcp_data_apply.version) ? 878 : 325;

				tmp_kernel_buff_hdcp = kmalloc(key_size, GFP_KERNEL);//kernal space
				...
				if (copy_from_user(tmp_kernel_buff_hdcp,to_user_ptr(hdcp_data_apply.pksv), key_size))
				{
					rtd_pr_vfe_debug("[ERR] copy_from_user error\n");
					ret = EFAULT;
				}
				...
				{
					/* excute driver api */
					if (vfe_hdmi_drv_write_hdcp_apply(hdcp_data_apply.version, tmp_kernel_buff_hdcp, hdcp_data_apply.apply) != 0)
						ret = -1;
				}
		...
		case VFE_IOC_HDMI_GET_DATA:
			...
			else
			{
				...
				if (p_data==NULL)
				{
					rtd_pr_vfe_err("[ERR] VFE_IOC_HDMI_GET_DATA memeory fail \n");
					ret = EFAULT;
				}
				else
				{
		...
		case VFE_IOC_AVD_INIT_VBI_MODULE:
		{
			...
			else
			{
				...
				{
					ret =-EFAULT;
					rtd_pr_vfe_debug("init vbi module fail:%d\n",vbi_status.vbi_support_type);
				}
			}
			break;
		}
...
static char *vfe_devnode(struct device *dev, umode_t *mode)
{
	return NULL;
}
...
{
	rtd_pr_vfe_notice("Online measure main Error Ack\n");
...
#endif
			if	(HDMI_Global_Status == SRC_CONNECT_DONE) {
				Set_HDMI_Reply_Zero_Timing_Flag(Get_DisplayMode_Port(SLR_MAIN_DISPLAY), REPORT_ZERO_TIMING);/*Let Webos get zero timing*/
...
#endif
			if	(HDMI_Global_Status == SRC_CONNECT_DONE) {
				Set_HDMI_Reply_Zero_Timing_Flag(Get_DisplayMode_Port(SLR_SUB_DISPLAY), REPORT_ZERO_TIMING);/*Let Webos get zero timing*/
				HDMI_set_detect_flag(TRUE);
			}
...
{//delete vbi_detect_tsk
	...
	if (vbi_tsk_running_flag) {
		if(p_vbi_tsk){
			...
			pr_err("vbi_detect_tsk thread stopped already\n");
		}
	}
}
...
static void delete_srcdetect_tsk(void)
{
	...
	if (adc_detect_tsk_running_flag) {
		if(p_adc_detect_task){
			...
			rtd_pr_vfe_debug("adc_detect_tsk thread stopped already\n");
		}
	}
#endif
	if (avd_detect_tsk_running_flag) {
		if(p_avd_detect_task){
			...
			rtd_pr_vfe_debug("avd_detect_tsk thread stopped already\n");
		}
	}
	...
	if (hdmi_detect_tsk_running_flag) {
		if(p_hdmi_detect_task){
			...
			rtd_pr_vfe_debug("hdmi_detect_tsk thread stopped already\n");
		}
	}
}
		...
		case _SRC_CVBS:
			...
			if (0 == Scaler_AVD_Init(KADP_VFE_AVD_CC_AND_TTX_ARE_SUPPORTED)){
				...
				if (0 == VFE_AVD_OPEN()){
					...
					Scaler_AVD_SetLGEColorSystem(0x7f);
...
{//delete video_onlinemeasure_tsk
	...
	if (onms_tsk_running_flag) {
		if(p_onms_tsk){
			...
			pr_err("video_onlinemeasure_tsk thread stopped already\n");
		}
	}
}
