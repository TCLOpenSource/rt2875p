...
#endif
	rtd_outl(TV_SB1_ANA_VDAC_SRC_SEL_reg, 0x00000033); // [5] REG_VDACDIV_RSTB
	// [4]REG_VDAC_CLK_POLAR, [2:0]REG_VDAC_SRC_SEL, 0:from VD, 3:from ADC2X_DOUT
...
void drvif_vdac_tvdac_power_on(unsigned char enable)
{
	...
	if(enable) {
		...
		VDAC_TV_CTRL1_reg.reg_vid_powmbias = 1;
	} else {
		VDAC_TV_CTRL1_reg.reg_vid_powvdac = 0;
		VDAC_TV_CTRL1_reg.reg_vid_powmbias = 0;
	}
