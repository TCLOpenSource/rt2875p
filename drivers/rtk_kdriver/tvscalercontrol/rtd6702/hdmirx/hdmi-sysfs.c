...
unsigned char default_common_port = 0;
...
extern unsigned int fw_b_clock;
#endif
    ...
    )
{
    ...
    if (strcmp(attr->attr.name, "debug_ctrl")==0)
    {
        ...
        // default_common_port
        //---------------------------------------------------
        n = scnprintf(ptr, count, "default_common_port=%d\n", default_common_port);
        ptr+=n; count-=n;
        ...
        //---------------------------------------------------
        // force_FW clock detection
        //---------------------------------------------------
#ifdef FAKE_FW_CLOCK_DETECT
        n = scnprintf(ptr, count, "fw_b_clock=%d  (0: disable, others : fw b_clock (294MHz=2815))\n",
            fw_b_clock);
        ptr+=n; count-=n;
#endif
                   ...
                   const char *buf, size_t count)
{
    ...
    if (strcmp(attr->attr.name, "debug_ctrl")==0)
    {
        ...
        if (sscanf(buf, "default_common_port=%5d", &val)==1)
        {
            default_common_port = (val) ? 1 : 0;
        }
        else if (sscanf(buf, "bypass_detect=%5d", &val)==1)
        {
...
#ifdef FAKE_FW_CLOCK_DETECT
        else if (sscanf(buf, "fw_b_clock=%d", &val)==1)
        {
            fw_b_clock = val;
        }
#endif
    ...
    )
{
    ...
    {
        HDMI_TIMING_T tm, tm2;
        unsigned char port = newbase_hdmi_get_current_display_port();
        unsigned char frl_mode = newbase_rxphy_get_frl_mode(port);

        memset(&tm, 0, sizeof(HDMI_TIMING_T));
        memset(&tm2, 0, sizeof(HDMI_TIMING_T));

        if(lib_hdmims_offline_measure_native(&tm, frl_mode))
        {
            unsigned char measure_result = TRUE;
            unsigned char measure_result2 = TRUE;

            measure_result2 &= lib_hdmims_offline_measure(&tm2, frl_mode, OFFMS_ONE_SHOT);
            ...
            measure_result &= lib_hdmims_calc_freq(&tm);
            measure_result2 &= lib_hdmims_calc_freq(&tm2);
            ...
            ptr+=n; count-=n;
            memset(&tm, 0, sizeof(HDMI_TIMING_T));
            lib_hdmims_onms_set_watchdog_int_en(0, 0);
            measure_result &= lib_hdmims_online1_measure(&tm);

            memset(&tm2, 0, sizeof(HDMI_TIMING_T));
            lib_hdmims_onms_set_watchdog_int_en(2, 0);
            measure_result2 &= lib_hdmims_online3_measure(&tm2);
            n = scnprintf(ptr, count, "[ONMS1_3] (ONMS1 SRC:%d /ONMS3 SRC:%d)\n", lib_hdmims_online1_get_ms_src_sel(), lib_hdmims_online3_get_ms_src_sel());
            ptr+=n; count-=n;
            ...
            ptr+=n; count-=n;
            memset(&tm, 0, sizeof(HDMI_TIMING_T));
            measure_result &= lib_hdmims_offline_measure_demode(&tm, frl_mode);
            ...
            ptr+=n; count-=n;
            memset(&tm, 0, sizeof(HDMI_TIMING_T));
            measure_result &= lib_hdmims_online3_demode_measure(&tm);
                   ...
                   const char *buf, size_t count)
{
    ...
    if (strcmp(attr->attr.name, "meas")==0)
    {
        ...
        {// 0: VGIP input, 1: VGIP output
            lib_hdmims_online1_set_ms_src_sel((val) ?1:0);
        }
        else if (sscanf(buf, "ONMS3_SRCSEL=%1d",&val)==1)
        {// 0: VGIP input, 1: VGIP output
            lib_hdmims_online3_set_ms_src_sel((val) ?1:0);
        }
