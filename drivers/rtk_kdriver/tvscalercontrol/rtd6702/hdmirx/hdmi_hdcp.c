...
void newbase_hdmi_save_customer_hdcp2p2(unsigned char* hdcp_2p2, unsigned int len )
{
	...
	{
		HDMI_EMG("ap burn key fail p=%x,len=%d\n",(unsigned int)hdcp_2p2,len);
		return;
	}
...
void newbase_hdmi_hdcp14_write_ksv_fifo(unsigned char port, unsigned char* p_ksv_fifo, unsigned int len)
{
    unsigned char bit0_tmp;
    unsigned long flags;
    unsigned int  wait_ksv_fifo_free = 0;
    unsigned int  i;
    ...
    if (p_ksv_fifo && len)
    {
        if (len > MAX_KSV_FIFO_DEPTH)
        {
            HDCP_WARN("KSV fifo length (%d) should not more than 640. the rest bytes will be ignored\n", len);
            len = MAX_KSV_FIFO_DEPTH;
        }
    }
    else
    {
        HDCP_WARN("NULL ksv list, clear KSV FIFO list\n");
        len = MAX_KSV_FIFO_DEPTH;
    }

    // make sure TX is not reading KSV fifo
    while(HDMI_STB_HDCP_AP_get_ksvfifo_status(hdmi_in(HDMI_STB_HDCP_AP_reg))==1)
    {
        mdelay(1);
        if ((wait_ksv_fifo_free++ & 0xF)==0xF)
            HDCP_WARN("KSV FIFO is being read ....waiting for the reading compelte\n");
    }

    // write data to KSV FIFO (0x43)
    lock_hdcp_sram(flags);
    ...
    for (i=0; i< len; i++)
    {
        hdmi_out(HDMI_STB_HDCP_AP_reg, HDMI_STB_HDCP_AP_dp_ksvfifo_ptr(i)|HDMI_STB_HDCP_AP_ap1(0x43));
        hdmi_out(HDMI_STB_HDCP_DP_reg, (p_ksv_fifo) ? *(p_ksv_fifo++) : 0);
    }
