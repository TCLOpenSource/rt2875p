...
void lib_hdmi_dsc_crt_reset(unsigned char dscd_port)
{
    unsigned long flags;
    ...
    case 1:
        // reset DSC
        // DO CRT reset (I):  RST = L
        hdmi_out(SYS_REG_SYS_SRST4_reg, SYS_REG_SYS_SRST4_rstn_dscd2_mask);
        udelay(5);
        // DO CRT reset (I):  CLK = OFF
        hdmi_out(SYS_REG_SYS_CLKEN4_reg, SYS_REG_SYS_CLKEN4_clken_dscd2_mask);
        udelay(5);
        // DO CRT reset (I):  CLK = ON
        hdmi_out(SYS_REG_SYS_CLKEN4_reg, (SYS_REG_SYS_CLKEN4_clken_dscd2_mask|
...
unsigned char lib_hdmi_dsc_get_crt_on(unsigned char dscd_port)
{
    switch(dscd_port){
    case 0:
        return ((hdmi_in(SYS_REG_SYS_CLKEN0_reg) & SYS_REG_SYS_CLKEN0_clken_dscd_mask) >> SYS_REG_SYS_CLKEN0_clken_dscd_shift);
    case 1:
        return ((hdmi_in(SYS_REG_SYS_CLKEN4_reg) & SYS_REG_SYS_CLKEN4_clken_dscd2_mask) >> SYS_REG_SYS_CLKEN4_clken_dscd2_shift);
    }
    return 0;
}

void lib_hdmi_dsc_set_fw_pps(unsigned char dscd_port, unsigned char pps[DSC_PPS_LEN])
{
...
void lib_hdmi_dsc_get_cur_pps(unsigned char dscd_port, unsigned char pps[DSC_PPS_LEN])
{
    unsigned char* p_pps = pps;
    int i;
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
...
unsigned char lib_hdmi_dsc_get_dscd_p_n_s_not_finish(unsigned char dscd_port)
{
    return DSCD_INTS_DSCD_get_p_n_s_not_finish_even_irq(hdmi_in(DSCD_INTS_DSCD_reg));
}

void lib_hdmi_dsc_clr_dscd_p_n_s_not_finish(unsigned char dscd_port)
{//write 1 clear
    hdmi_mask(DSCD_INTS_DSCD_reg, ~DSCD_INTS_DSCD_p_n_s_not_finish_even_irq_mask, DSCD_INTS_DSCD_p_n_s_not_finish_even_irq_mask);
}

unsigned char lib_hdmi_dsc_get_dscd_out_pic_finish(unsigned char dscd_port)
{
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
...
void lib_hdmi_dsc_clr_dscd_out_pic_finish(unsigned char dscd_port)
{//write 1 clear
...
void lib_hdmi_dsc_double_buffer_enable(unsigned char dscd_port, unsigned char enable)
{
    return hdmi_mask(DSCD_DB_CTRL_reg, ~DSCD_DB_CTRL_db_en_mask, (enable) ? DSCD_DB_CTRL_db_en_mask : 0);
}

void lib_hdmi_dsc_enable(unsigned char dscd_port, unsigned char enable)
{
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
...
unsigned char lib_hdmi_get_dsc_enable(unsigned char dscd_port)
{
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
...
void lib_hdmi_dsc_fw_mode_enable(unsigned char dscd_port, unsigned char enable)
{
    return hdmi_mask(DSCD_CTRL_reg, ~DSCD_CTRL_pps_fw_mode_mask, (enable) ? DSCD_CTRL_pps_fw_mode_mask : 0);
}

void lib_hdmi_dsc_set_crc_en(unsigned char dscd_port, unsigned char enable)
{
...
unsigned int lib_hdmi_dsc_get_dscd_out_crc(unsigned char dscd_port)
{
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
...
unsigned int lib_hdmi_dsc_get_dscd_in_crc(unsigned char dscd_port)
{
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
...
unsigned int lib_hdmi_dsc_get_dscd_pps_crc(unsigned char dscd_port)
{
...
void lib_hdmi_dsc_set_output_hporch(unsigned char dscd_port, unsigned int value)
{
    if(value <20)
    {
...
unsigned char lib_hdmi_dsc_get_output_error_status(unsigned char dscd_port)
{
...
void lib_hdmi_dsc_clr_output_error_status(unsigned char dscd_port)
{
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
...
void lib_hdmi_dscd_timing_monitor_en(unsigned char dscd_port, unsigned char enable)
{
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
...
unsigned char lib_hdmi_dscd_is_timing_monitor_en(unsigned char dscd_port)
{
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
...
void lib_hdmi_dsc_set_timing_monitor_sta0(unsigned char dscd_port, DSCD_TIMING_MONITOR_STA_END_TYPE sta_end_type)
{
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
...
void lib_hdmi_dsc_set_timing_monitor_end0(unsigned char dscd_port, DSCD_TIMING_MONITOR_STA_END_TYPE sta_end_type)
{
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
...
void lib_hdmi_dsc_set_timing_monitor_sta1(unsigned char dscd_port, DSCD_TIMING_MONITOR_STA_END_TYPE sta_end_type)
{
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
...
void lib_hdmi_dsc_set_timing_monitor_end1(unsigned char dscd_port, DSCD_TIMING_MONITOR_STA_END_TYPE sta_end_type)
{
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
...
void lib_hdmi_dsc_set_timing_monitor_sta2(unsigned char dscd_port, DSCD_TIMING_MONITOR_STA_END_TYPE sta_end_type)
{
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
        HDMI_EMG("[lib_hdmi_dsc_set_timing_monitor_sta1] DSCD CRT not enable!! DSCD can not work!\n");
...
void lib_hdmi_dsc_set_timing_monitor_end2(unsigned char dscd_port, DSCD_TIMING_MONITOR_STA_END_TYPE sta_end_type)
{
    if(!lib_hdmi_dsc_get_crt_on(dscd_port))
    {
        HDMI_EMG("[lib_hdmi_dsc_set_timing_monitor_end1] DSCD CRT not enable!! DSCD can not work!\n");
...
unsigned int lib_hdmi_dsc_get_timing_monitor_result(unsigned char dscd_port, DSCD_TIMING_MONITOR_RESULT_INDEX result_index)
...
const char* _hdmi_hd21_dsc_fsm_str(HDMI_DSCD_FSM_T fsm)
{
    ...
    default:                      return "DSCD FSM UNKNOW";
    }
}
...
int newbase_hdmi_hd21_dsc_get_dscd_port(unsigned char port, unsigned char* p_dscd_port)
{
    if (port >= HDMI_PORT_TOTAL_NUM || newbase_hdmi_get_current_display_port() != port || p_dscd_port==NULL)
        return -1;

    return newbase_hdmi_get_common_port(port, p_dscd_port);
}

unsigned char newbase_hdmi_hd21_dsc_pps_measure(unsigned char port, HDMI_TIMING_T *tm)
{
    unsigned char dscd_port;
    unsigned char hw_pps_buf[DSC_PPS_LEN];
    CVTEM_INFO info;
    DSC_PPS dscd_hw_pps;

    if (newbase_hdmi_hd21_dsc_get_dscd_port(port, &dscd_port)<0)
    {
        HDMI_EMG("[newbase_hdmi_hd21_dsc_pps_measure] get dscd port failed!!\n");
        return FALSE;
    }
    ...
    lib_hdmi_dsc_get_cur_pps(dscd_port, hw_pps_buf);
...
HDMI_TIMING_T* newbase_hdmi_hd21_dsc_get_current_pps_timing(void)
{
...
{
    unsigned char dscd_port = 0;
    if (newbase_hdmi_hd21_dsc_get_dscd_port(port, &dscd_port)<0)
    {
        return;
    }

    if(GET_FLOW_CFG(HDMI_FLOW_CFG_GENERAL, HDMI_FLOW_CFG0_SUPPORT_DSC) &&(hdmi_rx[port].cvtem_emp_cnt>0) &&  !lib_hdmi_dsc_get_crt_on(dscd_port))
    {//  Start DSC handler when detected CVTEM
        ...
        lib_hdmi_dsc_crt_reset(dscd_port);
    }
    ...
    {
        if (lib_hdmi_dsc_get_output_error_status(dscd_port))
        {
            ...
            lib_hdmi_dsc_clr_output_error_status(dscd_port); //clear error status and check again   
    ...
    case DSCD_FSM_INIT:
        ...
        memset(&dscd_fsm_st.pps_timing, 0, sizeof(HDMI_TIMING_T));
        lib_hdmi_dsc_set_crc_en(dscd_port, 1);
        lib_hdmi_dsc_fw_mode_enable(dscd_port, 0); //disable pps fw mode
        lib_hdmi_dscd_timing_monitor_en(dscd_port, FALSE);
    ...
    case DSCD_FSM_WAIT_CVTEM:
    {
        ...
        else
        {
            ...
            //Set timing monitor
            lib_hdmi_dsc_set_timing_monitor_sta0(dscd_port, DSCD_TM_VS_FALLING);
            lib_hdmi_dsc_set_timing_monitor_end0(dscd_port, DSCD_TM_VACT_RISING);
            lib_hdmi_dscd_timing_monitor_en(dscd_port, TRUE);
    ...
    case DSCD_FSM_SET_FW_PPS:
    {
        ...
        lib_hdmi_dsc_fw_mode_enable(dscd_port, 1);
        lib_hdmi_dsc_set_fw_pps(dscd_port, cvtem_pps_buf);
        SET_DSCD_FSM(DSCD_FSM_WAIT_PPS_READY);
        break;
    }
    ...
    {
        DSC_PPS dscd_hw_pps;
        unsigned int current_pps_crc = lib_hdmi_dsc_get_dscd_pps_crc(dscd_port);

        lib_hdmi_dsc_get_cur_pps(dscd_port, (unsigned char*)&dscd_hw_pps);
    ...
    case DSCD_FSM_PPS_MEASURE:
    {
        ...
        current_pps_crc =  lib_hdmi_dsc_get_dscd_pps_crc(dscd_port);
        ...
        else
        {
            ...
            if((dsc_output_hporch>=DSCD_OUTPUT_HPORCH_MIN) && (dsc_output_hporch< dscd_fsm_st.pps_timing.h_act_len) && (dscd_fsm_st.pps_timing.h_act_len >0) && (dscd_fsm_st.pps_timing.v_act_len >0))
            {
                ...
                lib_hdmi_dsc_set_output_hporch(dscd_port, dsc_output_hporch>>2);
            ...
            {
                HDMI_DSC_INFO("DSCD FSM:%s, Invalid dsc_output_hporch, h_total=%d, h_act_len=%d, pps_crc=0x%x\n",  _hdmi_hd21_dsc_fsm_str(GET_DSCD_FSM()), dscd_fsm_st.pps_timing.h_total, dscd_fsm_st.pps_timing.h_act_len, lib_hdmi_dsc_get_dscd_pps_crc(dscd_port));
                dscd_fsm_st.wait_pps_cnt = 0;
                SET_DSCD_FSM(DSCD_FSM_WAIT_PPS_READY);
            }
        }
    ...
    {
        unsigned out_crc = lib_hdmi_dsc_get_dscd_out_crc(dscd_port);
        unsigned in_crc = lib_hdmi_dsc_get_dscd_in_crc(dscd_port);
        unsigned pps_crc = lib_hdmi_dsc_get_dscd_pps_crc(dscd_port);
        ...
        DSCD_FLOW_PRINTF("DSCD FSM:%s,(Current,DSC) port=(%d,%d), dscd_en=%d, (out, in,pps)=(%08x,%08x,%08x), PPS hactive:%d, vactive:%d, output_vback:%d, INTS=%x, INTS_2=%x\n",
            ...
            lib_hdmi_get_dsc_enable(dscd_port),
...
void newbase_hdmi_hd21_dsc_enanble(unsigned char port, unsigned char enable)
{
    ...
    common_port = dscd_port;        // for vpll setting
    ...
    if (enable)
    {
        ...
        lib_hdmi_dsc_enable(dscd_port, 1);                    // enable DSC
        udelay(100);
    }
    else
    {
        lib_hdmi_dsc_enable(dscd_port, 0);                     // disable DSC
    ...
    HDMI_WARN("[newbase_hdmi_hd21_dsc_enanble] HDMI2.1 port =%d, dscd_port=%d, enable=%d\n", port, dscd_port, enable);
}
...
unsigned int newbase_hdmi_dscd_current_output_vbackporch(void)
{
    ...
    {
        HDMI_WARN("[newbase_hdmi_dscd_tm_vbackporch] get current vback proch failed, HDMI[p%d] is not a hdmi2.1 port\n", dscd_fsm_st.run_dscd_port);
        return 0;
    }

    if (newbase_hdmi_hd21_dsc_get_dscd_port(dscd_fsm_st.run_dscd_port, &dscd_port)<0) {
        HDMI_WARN("[newbase_hdmi_dscd_tm_vbackporch] get current vback proch failed, HDMI[p%d] has no associated dscd port\n", dscd_fsm_st.run_dscd_port);
        return 0;
    }

    tm_result_clk_cycle = lib_hdmi_dsc_get_timing_monitor_result(dscd_port, DSCD_TM_RESULT_1);
 ...
 *------------------------------------------------------------------*/
static unsigned char _dscd_vaild_crc_check(unsigned int crc)
...
unsigned char newbase_hdmi_check_dscd_output_valid(void)
{
    ...
    lib_hdmi_dsc_clr_output_error_status(dscd_port);
    lib_hdmi_dsc_clr_dscd_out_pic_finish(dscd_port);    //clear dscd output status

    do
    {
        unsigned int out_pic_finish = lib_hdmi_dsc_get_dscd_out_pic_finish(dscd_port);    // only support for merlin6
        unsigned int out_crc = lib_hdmi_dsc_get_dscd_out_crc(dscd_port);
        unsigned int in_crc = lib_hdmi_dsc_get_dscd_in_crc(dscd_port);
        unsigned int pps_crc = lib_hdmi_dsc_get_dscd_pps_crc(dscd_port);
        unsigned char buf[128];
        unsigned char check_output_crc_vaild = _dscd_vaild_crc_check(pps_crc);
        unsigned char is_dscd_no_error = !lib_hdmi_dsc_get_output_error_status(dscd_port);
        lib_hdmi_dsc_get_cur_pps(dscd_port, buf);
        ...
        HDMI_WARN("DSCD FSM:%s, dscd_port=%d, count=%d, dscd_en=%d, Check DSCD (gen_finish, crc_vaild, is_no_err)=(%d,%d,%d),(out, in,pps)=(%08x,%08x,%08x),INTS=%x, INTS_2=%x\n",
            _hdmi_hd21_dsc_fsm_str(GET_DSCD_FSM()), dscd_fsm_st.run_dscd_port, 
           wait_count,  lib_hdmi_get_dsc_enable(dscd_port), out_pic_finish, check_output_crc_vaild, is_dscd_no_error,
    ...
    while(wait_count>0 && lib_hdmi_dsc_get_crt_on(dscd_port));
