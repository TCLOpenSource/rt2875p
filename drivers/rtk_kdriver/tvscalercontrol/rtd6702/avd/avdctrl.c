...
unsigned short g_usCurVHaldTotal=0;
...
unsigned char avd_drv_timing_ready(void)
{
	if (g_ucSrcType == _SRC_CVBS){
		return (avd_good_timing_ready && g_ucDetectStage);
	}else if (g_ucSrcType == _SRC_TV){
...
//		|| (g_tSupportedColorSys & KADP_COLOR_SYSTEM_PAL_60) == KADP_COLOR_SYSTEM_PAL_60
	...
	if (DETECT_STAGE_FINAL != g_ucDetectStage) {
		...
		if ( _SRC_TV == g_ucSrcType)
		{
			...
			//rtd_pr_avd_info("DETECT_STAGE_FINAL != g_ucDetectStage(%d), can't do SetAVDLGETimingInfo()\n", g_ucDetectStage);
...
{
	rtd_pr_avd_info("#####[%s(%d)] atv_channel_change_start=%d\n",__func__,__LINE__,atv_channel_change_start);
