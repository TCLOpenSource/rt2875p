...
void AVD_Memory_Free(void)
{
	...
	rtd_pr_avd_info("%s done", __func__);
}
...
//			ClearAVDLGETimingInfo();
	...
	if (TRUE == check_dma_3d_stable){
		...
		if((vdpq_dma_ctrl4_reg.dma_3d_on_off) && (drvif_module_vdc_Read_Vstate()==3)){
			...
			if( (drvif_module_vdc_Read_Vstate()==3) && (vdpq_yc_sep_mode_ctrl_reg.yc_sep_mode == 0) )
			{
				check_dma_3d_Count = 0;
				vdpq_dma_3d_ready =TRUE;
			}else{
...
void Scaler_AVD_Set_DetTiming_In_AutoScan_Flag(unsigned char a_ucFlag)
{
	...
	//rtd_pr_avd_emerg("Scaler_AVD_Set_DetTiming_In_AutoScan_Flag  g_ucDetTimingInAutoScan=%x\n", g_ucDetTimingInAutoScan);
}
...
{
	// int cnt_chroma_lock;
	unsigned short wait_vstate_cnt = 30;
...
//	unsigned char mn_flag = 0xff;
	...
	if((avd_good_timing_ready == FALSE) && (_SRC_CVBS == g_ucSrcType) && (DETECT_STAGE_INIT == g_ucDetectStage)){
	...
	if (g_ucSrcType == _SRC_TV) {
		...
		//VDC_DEBUG_MSG(VDC_MSG_TVCHCH, "Vstate=%x,time=%d\n",drvif_module_vdc_Read_Vstate(),vdcf_inl(VDTOP_VSTATELOCK_TIME_POLLING_reg));
...
//						g_ucVDMode = VDC_MODE_PALM;
				...
				VDC_DEBUG_MSG(VDC_MSG_TVCHCH_DETAIL, "set mode 2=%x, 94a0=%x, 94a4=%x\n", g_ucVDMode,vdcf_inl(VDTOP_MANUAL_MODE_reg),vdcf_inl(VDTOP_MODE_DETECTION_STATUS_reg));
...
//			drvif_module_vdc_Set_lose_chromalock_count(0x0b);	/* set to default */
			...
			if(avd_good_timing_ready == false){
				rtd_pr_avd_warn("#####[%s(%d)] 0xb8019008=0x%08x,0xb801900c=0x%08x,0xb8019010=0x%08x\n", __func__, __LINE__, vdcf_inl(VDTOP_VIDEO_STATUS_REG1_reg),vdcf_inl(VDTOP_VIDEO_STATUS_REG2_reg),vdcf_inl(VDTOP_VIDEO_STATUS_REG3_reg));
			...
			rtd_pr_avd_info("#####[%s(%d)] DETECT_STAGE_INIT, g_ucVDMode=%x, g_ucVDPreMode=%x, g_bChangeSystem=%x, 94a4=%x\n", __func__, __LINE__, g_ucVDMode, g_ucVDPreMode, g_bChangeSystem,vdcf_inl(VDTOP_MODE_DETECTION_STATUS_reg));
			...
			rtd_pr_avd_info("#####[%s(%d)] INIT g_ucCurColorStd=%x, g_ucPreColorStd=%x\n",__func__,__LINE__, g_ucCurColorStd, g_ucPreColorStd);
			if(g_ucPreColorStd != g_ucCurColorStd){
				rtd_pr_avd_info("#####[%s(%d)] g_ucCurColorStd=%d, g_ucPreColorStd=%d\n",__func__,__LINE__,g_ucCurColorStd,g_ucPreColorStd);
				g_bChangeSystem = TRUE;
			}

			if (ZAUTO == g_ucCurColorStd){
				rtd_pr_avd_info("#####[%s(%d)] g_ucCurColorStd=%d\n",__func__,__LINE__,g_ucCurColorStd);
				g_bChangeSystem = TRUE;
			}
			...
			// wait_vstate_cnt *= (vdcf_inl(0xB80198DC) < 5) ? ((vdcf_inl(0xB80198DC) > 1) ? vdcf_inl(0xB80198DC): 1) : 0;
			while(drvif_module_vdc_Read_Vstate() < 3){
				if(0 == wait_vstate_cnt)
					break;
				// rtd_pr_avd_info("####[%s(%d)]vstate = %d######[wait_vstate_cnt=%d]\n",__func__,__LINE__,drvif_module_vdc_Read_Vstate(),wait_vstate_cnt);
				msleep(10); // wait_vstate_cnt = 10*1, 10*2, 10*3, 10*4
				wait_vstate_cnt --;
			}
			rtd_pr_avd_info("####[%s(%d)]vstate = %d######[wait_vstate_cnt=%d]\n",__func__,__LINE__,drvif_module_vdc_Read_Vstate(),wait_vstate_cnt);
		...
		case DETECT_STAGE_FINAL:
			...
			if(avd_good_timing_ready == false){
				rtd_pr_avd_warn("#####[%s(%d)] 0xb8019008=0x%08x,0xb801900c=0x%08x,0xb8019010=0x%08x,vdc_TV_reset=%d\n", __func__, __LINE__, vdcf_inl(VDTOP_VIDEO_STATUS_REG1_reg),vdcf_inl(VDTOP_VIDEO_STATUS_REG2_reg),vdcf_inl(VDTOP_VIDEO_STATUS_REG3_reg),vdc_TV_reset);
...
//					VDC_DEBUG_MSG(VDC_MSG_TVCHCH_DETAIL, "ATSC mode set == detect, 94a4=%x\n", vdcf_inl(VDTOP_MODE_DETECTION_STATUS_reg));
		...
		module_vdc_Set_bgate_Setting(1); //[Start]ryanliao 20190108 bgate_Setting
		g_ulInitStageCount = 0;
		//rtd_pr_avd_emerg("*****VDC HV no lock vdc_LockStatus=%x\n", vdc_LockStatus);
			...
			if (((ptAVDScalerDispInfo->ucMode_Curr) == 0 )|| avd_good_timing_ready)
			{
				if((atv_channel_change || atv_channel_change_start || !avd_good_timing_ready)
					...
					if((ATV_no_signal_Count > 20) || drvif_module_vdc_read_scan_flag()){//if atv is no signal it will timeout and set scaler timing info
						// prevent scaler structure get zero start value
					...
					if(VSC_INPUTSRC_AVD == Get_DisplayMode_Src(SLR_MAIN_DISPLAY)){
						rtd_pr_avd_info("%s line=%d ATV_no_signal_Count=%d atv_ch_change=%d atv_change_start=%d avd_good_timing_ready=%d,ucRet=%d\n",__FUNCTION__,__LINE__,ATV_no_signal_Count,atv_channel_change,atv_channel_change_start,avd_good_timing_ready,ucRet);
					}
...
//		drvif_module_vdc_ClockOn();
	...
	rtd_pr_avd_info("** 0xb8000204=%x\n", IoReg_Read32(SYS_REG_SYS_CLKSEL_reg));
	/*============YPP_VD_ADC=================================*/
	if (_SRC_CVBS == g_ucSrcType || (_SRC_TV == g_ucSrcType && KADP_AVD_EXTERNAL_DEMOD == g_tAVDDemodType))
	{
		for (i = 0  ; i < REGISTER_SIZE_YPP_VD_ADC ; i++) {
