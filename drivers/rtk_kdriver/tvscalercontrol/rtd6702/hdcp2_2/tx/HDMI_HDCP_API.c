		...
		UINT8* m, INT32 mSize)
{
	...
	if(file==NULL)
	{
		rtd_pr_hdcp_debug("open RX_INFO_STORED_FILE ERROR!\n");
		return ;
	}
	fs = get_fs();
	set_fs(KERNEL_DS);
	...
	*(temp_buf+rIDSize+kmSize+ekhkmSize+mSize+1) = '\0';
    ...
    kernel_write(file, temp_buf, rIDSize+kmSize+ekhkmSize+mSize+1, &pos);
#else
		...
		UINT8* m, INT32 mSize)
{
	...
	if(pRID == NULL){
		rtd_pr_hdcp_debug("kcalloc failed, pRID is NULL\n");
		goto ERR;
	}
	if(line == NULL){
		rtd_pr_hdcp_debug("kcalloc failed, line is NULL\n");
		goto ERR;
	}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
	while ((read = kernel_read(file, line, len, &pos)) != -1)
#else
