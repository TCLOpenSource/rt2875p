...
struct semaphore VPQ_DM_HDR_1_2_DMA_TBL_Semaphore;
...
unsigned char vpq_set_Gamut_3D_Lut_LGDB(GAMUT_3D_LUT_17x17x17_T *pLut, unsigned int iEnable, unsigned char bOnlySetEnable)
{
	...
	if (bOnlySetEnable && iEnable) {
		...
		if (iEnable == 1)
			ret = fwif_color_set_DDomain_3dLUT_16_TV006(pArray, 1);
		else
			fwif_color_set_DDomain_3dLUT_16_TV006(NULL, 0);
#if 0
...
void od_boot_ini(void)
{

	rtd_pr_vpq_info("VPQ_OD, boot\n");
...
void vpq_boot_init(void)
{
	...
	if (get_MEMC_bypass_status_refer_platform_model() == FALSE) {
		...
		if(MEMC_First_Run_Done == 0){
			...
			rtd_pr_vpq_emerg("[MEMC]initial in vpq_boot_init!!\n");
		}
	}
#endif
...
#if 1
	int u8_k, u8_i;
	rtd_pr_memc_notice("[jerry_SE_FilmDetect][SE_FilmDtect_Init]\n");
...
//	rtd_pr_memc_notice("[jerry_film_mode_detect_tsk_01]\n");
	...
	while (1)
	{
		...
		if(SE_status != 1){ // sleep when buffer empty
			if(cnt%1000 == 1 && CadenceDetect_dbg_en){
				rtd_pr_memc_notice("[%s][status is wrong !!][SE_rdPtr,%d,][status,%d,]\n", __FUNCTION__, SE_rdPtr, SE_status);
			}
...
//		rtd_pr_memc_notice("[jerry_film_mode_detect_tsk_02]\n");
		ScalerVIP_SE_Proc();
...
static int VPQ_detect_tsk(void *p)
{
    ...
    while (1)
    {
		...
		Scaler_color_set_HDR_VPQ_TSK();
		fwif_color_NNSR_model_TBL_update_VPQTSK();
		//rtd_pr_vpq_info("VPQ_detect_tsk\n");
		if(blending_ratio_ctrl[0]==1)
			fwif_color_NNSR_change_blending_ratio();
 		if (freezing(current))
		{
			try_to_freeze();
		}
...
UINT32 TM_table32[ToneMapping_size] = {0};
UINT16 TM_table12[ToneMapping_size] = {
	...
	case VPQ_IOC_SET_BLACK_LEVEL:
	{
		...
		if (copy_from_user(&blackLv, (int __user *)arg, sizeof(HAL_VPQ_SET_BLACK_LEVEL))) {
			...
			if (get_support_vo_force_v_top(support_sdr_max_rgb)) {
				Scaler_Set_HDR_YUV2RGB(SLR_MAIN_DISPLAY, PQModeInfo_flag[0], PQModeInfo_flag[1]);
			} else {
	...
	case VPQ_IOC_SET_LOWDELAY_MODE:
		{
			...
			if (Get_Factory_SelfDiagnosis_Mode()) {
				...
				if (copy_from_user(&lowdelaymode, (int __user *)arg, sizeof(bool))) {
					...
					if(lowdelaymode) {
						//rtd_pr_memc_emerg("[crixus]VPQ_IOC_SET_LOWDELAY_MODE1\n");
						...
						//rtd_pr_memc_emerg("[crixus]VPQ_IOC_SET_LOWDELAY_MODE0\n");
						//game mode dynamic, check in vsc task @Crixus 20151228
						drv_memory_set_game_mode_dynamic(_DISABLE);
						rtd_pr_memc_notice("[%s][%d][LowDelayMode]", __FUNCTION__, __LINE__);
...
int vpq_v4l2_ai_sqm_ctrl(unsigned char aipq_sqm_mode)
{
	...
	if(aipq_sqm_mode == 0)
	{
		...
		ret = scalerAI_pq_mode_ctrl(aipq_mode);
	}
...
int vpq_v4l2_ai_face_ctrl(unsigned char stereo_face)
{
	...
	if(status == -1)
	{
		printk("aipq v4l2 setting not ready: stereo %d\n", stereo_face);
		return ret;
	}
...
UINT8 v4l2_vpq_ire = V4L2_EXT_VPQ_INNER_PATTERN_DISABLE;
...
int vpq_v4l2_main_ioctl_s_ext_ctrls(struct file *file, void *fh, struct v4l2_ext_controls *ctrls)
{
...
#else
			if (get_support_vo_force_v_top(support_sdr_max_rgb)) {
				Scaler_Set_HDR_YUV2RGB(SLR_MAIN_DISPLAY, vip_sysInfo->HDR_flag, HAL_VPQ_COLORIMETRY_MAX);
			} else {
                                if ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE) && (Check_AP_Set_Enable_ForceBG(SLR_MAIN_DISPLAY) == FALSE))
                                {
                                        Scaler_Set_HDR_YUV2RGB(SLR_MAIN_DISPLAY, vip_sysInfo->HDR_flag, HAL_VPQ_COLORIMETRY_BT2020);
                                        if (isBlack_LvDiff_Flag == 1) {
                                                Scaler_SetDataFormatHandler();
                                        } else {
                                                /* seamless. do seamless process in isr (void Scaler_hdr_setting_SEAMLESS()), avoid transient noise*/
				...
				else
				{
					...
					else if( odExtInfo.ext_type == 1 ) /*LODC TblSize 17x17 Region 4x4*/
					{
						...
						else
						{
							unsigned char* pLUT = NULL;
							sys_reg_sys_clken3_RBUS sys_reg_sys_clken3_reg;
							pLUT = (unsigned char *)dvr_malloc(CMD_LENGTH*sizeof(unsigned char));
						...
						else
						{
							...
							else
							{
								...
								else
								{
									...
									for(i=0; i<17; i++)
									{
										pcidRowBnd[i] = (*(pLUT + sizeof(unsigned char)*CMD_SINGLE_TBL_LEN*10 + i)) << 4;
										pcidColBnd[i] = (*(pLUT + sizeof(unsigned char)*CMD_SINGLE_TBL_LEN*10 + sizeof(unsigned char)*17 + i)) << 4;
									}
									...
									for(i=0;i<10;i++)
									{
										fwif_color_pcid_Transpose_TV006(pLUT+(i*CMD_SINGLE_TBL_LEN*sizeof(unsigned char)), 19, TransTbl);
										fwif_color_set_pcid_RgnTableValue(TransTbl, i, _PCID_COLOR_ALL);
									}
									drvif_color_set_Tcon_pcid1_pcid2_LGPOD_fcic_enable(0,1,0,0);
									ret = 0;
	...
	break;
		...
		{
#ifdef isSupport_HDR_3dLUT_24x24x24	// need to wait for 3D LUT ready, table change from 24x24x24 to 17x17x17
			if(!ext_control.ptr)
			{
				rtd_pr_vpq_emerg("func:%s line:%d [error] ext_control.ptr is null\r\n",__FUNCTION__, __LINE__);
				ret = -EFAULT;
			...
			else
			{
				...
				{
#if 0
					rtd_pr_vpq_emerg("rord Oetf[0]=%d,Oetf[1]=%d,Oetf[1022]=%d,Oetf[1023]=%d \n",
					Oetf_table16[0],
					Oetf_table16[1],
					Oetf_table16[1022],
					Oetf_table16[1023]
                ...
                case V4L2_CID_EXT_LED_DB_DATA:
                {
                        ...
                        else
                        {
                                ...
                                else
                                {
                                        ...
                                        rtd_pr_vpq_emerg(" sizeof(v4l2_ext_led_db_lut)=%d ,LED_DB_LUT=%d ,sizeof(KADP_LED_DB_LUT_T)=%d\n",sizeof(v4l2_ext_led_db_lut),sizeof(LED_DB_LUT),sizeof(KADP_LED_DB_LUT_T));
				...
				return -1;
                        ...
                        else
                        {
                                ...
                                else
                                {
                                        ...
                                        rtd_pr_vpq_emerg("rord bitMask=%d,ctrlValue=%d \n",stRealInfo.bitMask,stRealInfo.ctrlValue);
                                        ret = 0;
	...
	if (vpq_v4l2_main_get_stop_run(cmd))
        ...
        switch(cmd)
        {
			...
			if (Get_Factory_SelfDiagnosis_Mode()) {
				...
				rtd_pr_memc_notice("[%s][%d][LowDelayMode][memc,%d,vpq,%d,]", __FUNCTION__, __LINE__, fV4l2_memc_lowdeay, v4l2_vpq_lowdelaymode);
				ret = 0;
			}
		}
                break;
			...
			rtd_pr_vpq_info("V4L2 LD_LUT=%d,\n", vpq_led_LD_lutTableIndex);
                ...
                case V4L2_CID_EXT_VPQ_STEREO_FACE_CTRL:
				{
                        ...
                        //rtd_pr_vpq_emerg("aipq V4L2_CID_EXT_VPQ_STEREO_FACE_CTRL mode: %d, eBackLight %d\n", controls.value, webos_tooloption.eBackLight);
						rtd_pr_vpq_emerg("aipq V4L2_CID_EXT_VPQ_STEREO_FACE_CTRL mode: %d, eBackLight %s\n", controls.value, webos_strToolOption.eBackLight);

						ret = vpq_v4l2_ai_face_ctrl(controls.value);// k8 ai_face
...
// 0622 lsy, tmp mark
                /*case V4L2_CID_EXT_AIPQ_SQM_MODE:
				{
						...
						rtd_pr_vpq_emerg("aipq V4L2_CID_EXT_AIPQ_SQM_MODE mode: %d, eBackLight %s\n", controls.value, webos_strToolOption.eBackLight);
...
int vpq_module_init(void)
{
	...
	sema_init(&VPQ_DM_HDR_1_2_DMA_TBL_Semaphore, 1);
