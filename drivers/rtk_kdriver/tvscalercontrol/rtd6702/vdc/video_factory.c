...
void drvif_module_vdc_factory_TestDMA_Setting(void)
{
	...
	/* stop Auto MA */
	rtdf_clearBits(VGIP_VGIP_CHN1_CTRL_reg, _BIT25);	/* disable auto ma */
...
#ifdef kernel_space_dumpdma
unsigned long m_vd_pCasheStartAddr = 0;
unsigned int m_vd_PhyStartAddr = 0;
unsigned long m_vd_pNonCacheStartAddr = 0;
#else
unsigned char *m_vd_pCasheStartAddr = NULL;
unsigned long m_vd_PhyStartAddr;
unsigned long *m_vd_pNonCacheStartAddr;
#endif
...
int exe_remount_usb_storage(void)
{
	...
	int ret = -1;
	char path[] = "/bin/mount";
	char *argv[] = {path, "-t", "vfat","-o","remount,rw","/dev/block/sda1","/tmp/", NULL};
	char *envp[] = {NULL};

	ret = call_usermodehelper(path, argv, envp, UMH_WAIT_PROC);
	VDC_DEBUG_MSG(VDC_MSG_FACT, "EXE %s and return value = %d\n",__FUNCTION__, ret);
	if(ret < 0)
...
void xDumpVDRawData(unsigned int Length)
{
	...
	memset(PathBuffer[3], 0, sizeof(PathBuffer[3]));
	rtk_get_usb_path(PathBuffer[3], sizeof(PathBuffer[3]));

	IoReg_SetBits(0xb8027220, _BIT1);
	iStartAddr = (unsigned char *)m_vd_pNonCacheStartAddr;
	ptr = (UINT32*) iStartAddr;
	VDC_DEBUG_MSG(VDC_MSG_FACT, "%x %x %x %x\n", *ptr, *(ptr + 1), *(ptr + 2), *(ptr + 3));
	...
	if (iStartAddr != 0 && iSize != 0) {
		...
		for(i=0; i<4; i++)
		{
			snprintf(sFilePath, 128, "%s/%s%s%s", PathBuffer[i], "DMA_", sFileName, ".bin"); //Dump to USB
			pFile = filp_open(sFilePath, O_CREAT | O_RDWR, 0600);
			if(IS_ERR(pFile))
			{
				VDC_DEBUG_MSG(VDC_MSG_FACT, "try to open %s fail\n", sFilePath);
			}
			else
			{
				VDC_DEBUG_MSG(VDC_MSG_FACT, "try to open %s success, start to dump\n", sFilePath);
				break;
			}
		}
		...
		if( !IS_ERR(pFile) )
		{
			...
			while (iCnt < iSize) {
				...
				if (iBufIdx >= 128) {
					kernel_write(pFile, (const char *)bufArray, 128 * 4, &offset);
					iBufIdx = 0;
				}
		...
		{
			VDC_DEBUG_MSG(VDC_MSG_FACT, "fail to dump, leave\n");
		}
	}
	exe_sync();
...
unsigned int drvif_module_vdc_factory_TestDMA_Enable(unsigned int Length)
{
	...
	VDC_DEBUG_MSG(VDC_MSG_FACT, "release all memory done!! DMA Go~\n");

#ifdef kernel_space_dumpdma
	VDC_DEBUG_MSG(VDC_MSG_FACT, "try to allocate memory with size 0x%x\n", Length);
	m_vd_pCasheStartAddr = (unsigned long)dvr_malloc_uncached_specific(Length, GFP_DCU2_FIRST,(void **)&m_vd_pNonCacheStartAddr);
	if(m_vd_pCasheStartAddr == 0)
	{
		VDC_DEBUG_MSG(VDC_MSG_FACT, "%s.%d\n", __FUNCTION__, __LINE__);
		VDC_DEBUG_MSG(VDC_MSG_FACT, "[VD DMA] alloc idma sequence memory failed[%lx]\n", m_vd_pCasheStartAddr);
		return FALSE;
	}
	nAddr = (unsigned int)dvr_to_phys((void *)m_vd_pCasheStartAddr);
	VDC_DEBUG_MSG(VDC_MSG_FACT, "cache:0x%lx,noncache:0x%lx,phy:0x%x\n",m_vd_pCasheStartAddr,m_vd_pNonCacheStartAddr,nAddr);
#endif
	...
	video_dma_debug_ctrl_reg.dma_debugmode_sel = 0;
