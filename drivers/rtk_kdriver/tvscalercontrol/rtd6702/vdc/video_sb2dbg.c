...
#define show_vfe_reg_value(addr) \
(unsigned int)({ \
	unsigned int _0 = (unsigned int)(addr); \
	unsigned int _r = IoReg_Read32(_0); \
	rtd_pr_vfe_emerg("\t%08x: %08x, bitmap: %*pbl\r\n", _0, \
		_r, (unsigned int)sizeof(_r)*BITS_PER_BYTE, &_r); \
	(unsigned int)_r; \
})

#define show_vfe_reg_bank(tbl) \
(void)({ \
	int _i = 0; \
	rtd_pr_vfe_emerg(__stringify(tbl)"\r\n"); \
	rtd_pr_vfe_emerg("------------[ cut here ]------------\r\n"); \
	for (_i = 0; _i < TABLE_SIZE(tbl); ++_i) \
		show_vfe_reg_value(tbl[_i]); \
	(void)0; \
	rtd_pr_vfe_emerg("------------[ end dump ]------------\r\n"); \
})

#define show_vdc_reg_value(addr) \
(unsigned int)({ \
	unsigned int _0 = (unsigned int)(addr); \
	unsigned int _r = IoReg_Read32(_0); \
	rtd_pr_vdc_emerg("\t%08x: %08x, bitmap: %*pbl\r\n", _0, \
		_r, (unsigned int)sizeof(_r)*BITS_PER_BYTE, &_r); \
	(unsigned int)_r; \
})

#define show_vdc_reg_bank(tbl) \
(void)({ \
	int _i = 0; \
	rtd_pr_vdc_emerg(__stringify(tbl)"\r\n"); \
	rtd_pr_vdc_emerg("------------[ cut here ]------------\r\n"); \
	for (_i = 0; _i < TABLE_SIZE(tbl); ++_i) \
		show_vdc_reg_value(tbl[_i]); \
	(void)0; \
	rtd_pr_vdc_emerg("------------[ end dump ]------------\r\n"); \
})
...
static inline enum RBUS_AT show_rbus_info(void)
{
	...
	switch (r.range.at) {
	case RBUS_AT_VFE:
		rtd_pr_vfe_emerg("rbus timeout @0x%08lx, isTimeout %x\r\n", r.info.timeout_reg, r.info.isTimeout);
		break;
	case RBUS_AT_VDC:
		rtd_pr_vdc_emerg("rbus timeout @0x%08lx, isTimeout %x\r\n", r.info.timeout_reg, r.info.isTimeout);
	break;
	case RBUS_AT_VIP:
		rtd_pr_vdc_emerg("rbus timeout @0x%08lx, isTimeout %x\r\n", r.info.timeout_reg, r.info.isTimeout);
		break;
...
void show_vdc_rbus_timeout_info(void)
{
	...
	if (isSb2Timeout("Video Decorder")) {
		...
		switch (at) {
		case RBUS_AT_VFE:
			show_vfe_reg_bank(vfe_monitor_bank);
			break;

		case RBUS_AT_VDC:
			show_vdc_reg_bank(vdc_monitor_bank);
			break;

		case RBUS_AT_VIP:
			show_vdc_reg_bank(vip_monitor_bank);
			break;
...
void module_video_register_sb2_timeout_callback(void)
{
	...
	rtd_pr_vdc_emerg("register vdc sb2 timeout callback\r\n");
}
...
void module_video_unregister_sb2_timeout_callback(void)
{
	...
	rtd_pr_vdc_emerg("unregister vdc sb2 timeout callback\r\n");
}
