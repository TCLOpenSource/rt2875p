...
#include <rbus/adc_reg.h>//Remain ADC register for Mark2
...
#define ucFH_bias_high_thl_NTSC	0xa3a0
#define ucFH_bias_low_thl_NTSC	0xa2e0
#define ucFH_bias_high_thl_PAL	0x6800
#define ucFH_bias_low_thl_PAL	0x6690
#define ucFH_bias_high_thl_PAL60 0x8240
#define ucFH_bias_low_thl_PAL60 0x80d0
#endif
...
bool vdc_needopenpowerflag = TRUE;//True now vd power off/ saving need to set on
unsigned char vdc_resetTV_Count = 0x0;
#ifdef CONFIG_AVD_FHFV_BIAS
bool vdc_YC1D_check_set_1d = FALSE;
#endif
...
{	/* 2_0048, 2_0050, 2_0054, 2_005c */
#if 1 //Remain ADC register for Mark2
	...
	extern void drvif_adc_ctl_rgb12_check(int input);
	...
	if (on_off == ON) {
		...
		if (_SRC_TV != a_ucInputSrc) {
			vdadc_pow_ctl_reg.vdadc_pow = 1;	/* 2_005c[0] VDADC & PGA Power */
		...
		vdadc_pow_ctl_reg.vdadc_pow = 1;	/* 2_005c[0] VDADC & PGA Power */
#else
		vdadc_pow_ctl_reg.vdadc_pow = 0;	/* 2_005c[0] VDADC & PGA Power */
		...
		vd_adc_clamp_ctl_1_reg.vdadc_clamp_ctrl1_14 = 0x0;
		vd_adc_clamp_ctl_1_reg.vdadc_clamp_ctrl1_13 = 0x0;
		vd_adc_clamp_ctl_1_reg.vdadc_clamp_ctrl1_7 = 0;
		vd_adc_clamp_ctl_1_reg.vdadc_clamp_ctrl1_6 = 0x0;
		vd_adc_clamp_ctl_1_reg.vdadc_clamp_ctrl1_5 = 0x0;
...
#ifdef CONFIG_AVD_FHFV_BIAS
	unsigned char ucFHBiasForce1D = 0;
	unsigned int FrameBufferF2Status = 0;
...
#ifdef CONFIG_AVD_FHFV_BIAS
	FrameBufferF2Status = VDPQ_FRAME_BUFFER_F2_STATUS_get_f2_cennum(vdcf_inl(VDPQ_FRAME_BUFFER_F2_STATUS_reg));
	VD_mode = drvif_module_vdc_ReadMode(VDC_SET);
	//rtd_pr_vdc_emerg("FrameBufferF2Status:%x,VD_mode:%d\n",FrameBufferF2Status,VD_mode);
	if((VD_mode==VDC_MODE_NTSC ||VD_mode==VDC_MODE_NTSC443 ||VD_mode==VDC_MODE_PALM)&&((FrameBufferF2Status>ucFH_bias_high_thl_NTSC)||(FrameBufferF2Status<ucFH_bias_low_thl_NTSC)))
		ucFHBiasForce1D = true;
	else if((VD_mode==VDC_MODE_PALI ||VD_mode==VDC_MODE_PALN)&&((FrameBufferF2Status>ucFH_bias_high_thl_PAL)||(FrameBufferF2Status<ucFH_bias_low_thl_PAL)))
		ucFHBiasForce1D = true;
	else if((VD_mode==VDC_MODE_PAL60)&&((FrameBufferF2Status>ucFH_bias_high_thl_PAL60)||(FrameBufferF2Status<ucFH_bias_low_thl_PAL60)))
		ucFHBiasForce1D = true;
#endif
	#if 1	//jj_test_flow_b
...
#ifdef CONFIG_AVD_FHFV_BIAS
unsigned char drvif_module_vdc_GetCVideoYC1D(void)
{
	return vdc_YC1D_check_set_1d;
}

bool drvif_module_vdc_FHFV_Bias_Query(void)
{
	unsigned char VD_mode = 0;
	unsigned short FieldVTotal = 0;
	VD_mode = drvif_module_vdc_ReadMode(VDC_SET);
	drvif_module_vdc_GetVHalfTotalLen(&FieldVTotal);
	rtd_pr_vdc_info("drvif_module_vdc_FHFV_Bias_Query VD_mode:%d, FieldVTotal:%d\n",VD_mode,FieldVTotal);
	if((VD_mode==VDC_MODE_PALI ||VD_mode==VDC_MODE_PALN)&& (FieldVTotal>314||FieldVTotal<311) && FieldVTotal!=0)
		return true;
	else if((VD_mode==VDC_MODE_NTSC ||VD_mode==VDC_MODE_NTSC443 ||VD_mode==VDC_MODE_PALM)&& (FieldVTotal>264||FieldVTotal<261) && FieldVTotal!=0)
		return true;
	else if((VD_mode==VDC_MODE_PAL60)&& (FieldVTotal>0)) {
		if((FieldVTotal ==263)&&(0== drvif_module_vdc_GetCVideoYC1D()))
			return false;
		return true;
	}
	else
