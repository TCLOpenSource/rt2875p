...
unsigned char HDR_force_1_2 = 0;	// 0: no force, 1:HDR1_1, 2:HDR1_2
/*NNSR*/
int nnsr_size = 0;
...
extern int scalerAI_pq_mode_ctrl(RTK_AI_PQ_mode ai_pq_mode);
...
#endif
unsigned int SLC_curve[35]={
	0,32,84,120,180,270,370,480,600,720,840,960,1080,1200,1370,1540,1710,1880,2048,2218,2388,2558,2728,2890,
	3040,3160,3280,3400,3520,3640,3740,3840,3920,4020,4095
}; 
...
//		memcpy(buf, (unsigned char *) &(smartPic_clue->Adapt_DCC_Info.S_RPC_DCC_LAYER_Array.Apply_Curve), size);
	...
	case TOOLACCESS_MEMC_CHANGE_SIZE: /*1256 get*/
		...
		MEMC_RTICE_GetParam((MEMC_SIZE_PARAM*)pUiTemp);
	...
	case TOOLACCESS_AI_TUNE_DCC: /*1264 get*/
		...
		pwUINT16Temp = (unsigned short *)vip_malloc(sizeof(DRV_AI_Tune_DCC_table));

		if(pwUINT16Temp == NULL)
			return -ID_SIZE_ERR;

		drivef_ai_tune_dcc_get((DRV_AI_Tune_DCC_table*) pwUINT16Temp);
	...
	case TOOLACCESS_AI_DCC_USER_CURVE: /*1265 get*/
		if (size > sizeof(unsigned short)*129)
			return -ID_SIZE_ERR;

		pwUINT16Temp = (unsigned short *)vip_malloc(sizeof(unsigned short)*129);

		if(pwUINT16Temp == NULL)
			return -ID_SIZE_ERR;

		drivef_ai_dcc_user_curve_get((unsigned short*) pwUINT16Temp);
		fwif_color_ChangeUINT16Endian(pwUINT16Temp, size>>1, 1);
	...
	case TOOLACCESS_AI_PQMASK_Control: /*1275 get*/
		...
		if(rtice_param.mode0 == 0) { // performance
			...
			if (size > 20*sizeof(unsigned int))
				return -ID_SIZE_ERR;
			...
			ofst++;
			/* 16 */
			u32tmpValue = pPQMaskColorMgr->Param.ScFusionRatio_Pre;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 17 */
			u32tmpValue = pPQMaskColorMgr->Param.ScFusionRatio_Cur;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			...
			fwif_color_ChangeUINT32Endian((unsigned int *)buf, size/sizeof(unsigned int), 1);
#endif
		} else if(rtice_param.mode0 == 8) {
			extern AI_CAPTURE_IMG_RECORD_T AiCapImgRec;

			if (size > sizeof(AI_CAPTURE_IMG_RECORD_T))
				return -ID_SIZE_ERR;

			memcpy(buf, &AiCapImgRec, size);
	...
	case TOOLACCESS_AI_PQMASK_SQM_TABLE:
	{
		...
		if (size+start_addr > sizeof(AI_DepthWeightTbl_SQM_TBL))
			return -ID_SIZE_ERR;

		if (size > 0) {
			memcpy(buf, (unsigned short *)AI_DepthWeightTbl_SQM_TBL+start_addr, size);
	...
	case TOOLACCESS_NNSR_Weight_Tbl:
		...
		if(rtice_param.mode0 ==0)
			nnsr_size = SRNN_Weight_Num_mode0;
		else if(rtice_param.mode0 ==1)
			nnsr_size = SRNN_Weight_Num_mode1;
		else if(rtice_param.mode0 ==2)
		...
		if (pwUINT16Temp == NULL)
			return -ID_SIZE_ERR;
		fwif_color_srnn_m_disp_en_ctrl(0);		
		drvif_srnn_model_get(pwUINT16Temp,rtice_param.mode0);
		fwif_color_srnn_m_disp_en_ctrl(1);
	   ...
	   case TOOLACCESS_NNSR_Wr_Weight_debug:
		...
		if(rtice_param.mode0 ==0)
			...
			nnsr_size = SRNN_Weight_Num_mode2;	
		else if(rtice_param.mode0 ==4)/*480p*/
			nnsr_size = SRNN_Weight_Num_mode1;
		else if(rtice_param.mode0 ==5)/*720p*/
			nnsr_size = SRNN_Weight_Num_mode1;		
		...
		pUINT8Temp = &weight_blending_ctrl[0];
		...
		*num_type = VIP_UINT8;		
		...
		//pUINT8Temp = NULL;
            ...
            break;
	case TOOLACCESS_NNSR_weight_blend_ctrl_debug:
		if(size>sizeof(unsigned char)*5)
			 return -ID_SIZE_ERR;
		...
		pUINT8Temp = &blending_ratio_ctrl[0];
		memcpy(buf, pUINT8Temp, size);
		*num_type = VIP_UINT8;	
		break;
...
*/
	...
	{
		if (size > sizeof(unsigned int)*(35))
			return -ID_SIZE_ERR;

		pwUINT32Temp = (unsigned int *)vip_malloc(35 * sizeof(int));
		if (pwUINT32Temp == NULL)
			return -ID_SIZE_ERR;
	...
	case TOOLACCESS_DM_HDR_3D_Lut_TBL:
		...
		if (HDR_force_1_2 == 1)
			rtice_param.mode0 = 0;
		else if (HDR_force_1_2 == 2)
			rtice_param.mode0 = 1;
#endif

		//fwif_color_get_Read_DM_HDR_3dLUT(buf2_tmp, pwUINT32Temp);
		fwif_color_rtice_DM2_3D_17x17x17_LUT_Get(rtice_param.mode0, pwUINT16Temp, HDR_3DLUT_getFromReg);
	...
	case TOOLACCESS_DM2_HDR_force_1_2:
		if (size > (sizeof(char)))
			return -ID_SIZE_ERR;
	...
	case TOOLACCESS_VIP_MVF_Force_Mode:
		if (size > (sizeof(char)))
			return -ID_SIZE_ERR;		

		*buf = RPC_system_setting_info->MASNR_Ctrl.MVF_Force_Mode_;
	...
	case TOOLACCESS_SLC_TBL:
	{
		if (size > (sizeof(int)*35))
			return -ID_SIZE_ERR;

		// need SLC read driver
		memcpy(buf, SLC_curve, size);
		fwif_color_ChangeUINT32Endian((unsigned int*)buf, size/sizeof(int), 1);
...
*/
	...
	case TOOLACCESS_VIP_LEA:
		...
		pwINT32Temp = (int *)vip_malloc(16 * sizeof( int));
		if (pwINT32Temp == NULL)
			return -ID_SIZE_ERR;
		...
		pwINT32Temp[12] = (unsigned int)(((unsigned long)LD_virAddr) & 0xffffffff);
...
int rtice_set_vip_table(ap_data_param rtice_param, unsigned char *buf)
{
	...
	unsigned int * pUiTemp = NULL;
	unsigned int size, start_addr,temp_size;//, end_addr,temp_size;
	_system_setting_info *system_setting_info = (_system_setting_info *)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_system_info_structure);
	_RPC_system_setting_info *RPC_system_setting_info = (_RPC_system_setting_info *)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_RPC_system_info_structure);
	unsigned char HY,VY,HC,VC;
...
*/
	...
	case TOOLACCESS_MEMC_CHANGE_SIZE: /*1256 set*/
		...
		MEMC_RTICE_SetParam((MEMC_SIZE_PARAM*)pUiTemp);
		MEMC_RTICE_Trigger();
	...
	case TOOLACCESS_AI_TUNE_DCC: /*1264 set*/
		...
		fwif_color_ChangeUINT16Endian(( unsigned short*)buf,size>>1, 1);

		memcpy((unsigned char *)pwUINT16Temp, (unsigned char *)buf, size);

		drivef_ai_tune_dcc_set((DRV_AI_Tune_DCC_table *) pwUINT16Temp, 1);
	...
	case TOOLACCESS_AI_PQMASK_Control: /*1275 set*/
		...
		if(rtice_param.mode0 == 1) { // set some color mgr setting
			...
			/* 16 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorMgr->Param.ScFusionRatio_Pre = u32tmpValue & 0xff;
			ofst++;
			/* 17 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorMgr->Param.ScFusionRatio_Cur = u32tmpValue & 0xff;
			ofst++;
			// image histogram by depth threshold
			/* 18 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			...
			/* 2 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorMgr->Status.ForceCurFrm = u32tmpValue;
			ofst++;
	...
	case TOOLACCESS_AI_PQMASK_SQM_TABLE:
	{
		...
		if (size > 0) {
			fwif_color_ChangeINT16Endian((short*)buf, size/sizeof(short), 1);
			memcpy((unsigned short *)AI_DepthWeightTbl_SQM_TBL+start_addr, buf, size);
	...
	if (size)
		drvif_srnn_bbbsr_table((DRV_srnn_table *)&gVip_Table->VIP_SRNN_Table[buf[0]]);
	...
	case TOOLACCESS_NNSR_Weight_Table_Idx:
	{
		extern unsigned int srnn_model_para[17][1324];
	...
	case TOOLACCESS_NNSR_Weight_Tbl:
		...
		//fwif_color_set_NNSR_model_TBL(rtice_param.mode0, pwUINT16Temp);

		drvif_srnn_model_set(pwUINT16Temp,rtice_param.mode0);
	...
	case TOOLACCESS_NNSR_weight_blend_ctrl_debug:
		...
		memcpy(pUINT8Temp, buf, size);
		memcpy(blending_ratio_ctrl, pUINT8Temp, size);
...
//			Scaler_SetMA_Chroma_Error(buf[12]); // merlin4 remove
	...
	case TOOLACCESS_DM_HDR_3D_Lut_TBL:
		...
		fwif_color_rtice_DM2_3D_17x17x17_LUT_Set(rtice_param.mode0, 1, pwUINT16Temp);
	...
	case TOOLACCESS_VIP_PQ_Power_Saving:
        ...
        case TOOLACCESS_SLC_TBL:
			...
			fwif_color_ChangeUINT32Endian((unsigned int*)buf, size/sizeof(int), 1);
			memcpy(SLC_curve, buf, size);
			fwif_color_set_SLC_TBL(&SLC_curve[0]);
	...
	case TOOLACCESS_VIP_MVF_Force_Mode:
		...
		RPC_system_setting_info->MASNR_Ctrl.MVF_Force_Mode_ = *buf;
		break;
...
*/
	...
	case TOOLACCESS_VIP_LEA:
		...
		pwINT32Temp = (int *)vip_malloc(sizeof(int)*16);
	...
	if (isSet_Flag == 1) {	/* write */
		if (rtice_param.id == TOOLACCESS_DM2_HDR_EOTF_TBL) {
			...
			if (size == (sizeof(unsigned int) * (EOTF_size * 3))) {
				...
				fwif_color_rtice_DM2_EOTF_Set(mode0, pwUINT32Temp + (EOTF_size * 0), pwUINT32Temp + (EOTF_size * 1), pwUINT32Temp + (EOTF_size * 2));
			}
			...
			if (size == (sizeof(short) * (OETF_size * 3))) {
				...
				fwif_color_rtice_DM2_OETF_Set(mode0, pwUINT16Temp + (OETF_size * 0), pwUINT16Temp + (OETF_size * 1), pwUINT16Temp + (OETF_size * 2));
			}
			...
			if (size == (sizeof(short) * (ToneMapping_size * 3))) {
				...
				ret = fwif_color_rtice_DM2_ToneMapping_Set(mode0, pwINT16Temp + (ToneMapping_size * 0), pwINT16Temp + (ToneMapping_size * 1), pwINT16Temp + (ToneMapping_size * 2));
			}
			...
			if (size == (sizeof(short) * (HDR_24x24x24_size * 3))) {
				...
				ret = fwif_color_rtice_DM2_3D_24x24x24_LUT_Set(mode0, 1, pwUINT16Temp);
			}
		...
		if (rtice_param.id == TOOLACCESS_DM2_HDR_EOTF_TBL) {
			...
			if (size == (sizeof(unsigned int) * (EOTF_size * 3))) {
				fwif_color_rtice_DM2_EOTF_Get(mode0, pwUINT32Temp + (EOTF_size * 0), pwUINT32Temp + (EOTF_size * 1), pwUINT32Temp + (EOTF_size * 2));
				fwif_color_ChangeUINT32Endian(pwUINT32Temp, EOTF_size*3, 1);
			...
			*num_type = VIP_UINT32;
			...
			if (size == (sizeof(short) * (OETF_size * 3))) {
				fwif_color_rtice_DM2_OETF_Get(mode0, pwUINT16Temp + (OETF_size * 0), pwUINT16Temp + (OETF_size * 1), pwUINT16Temp + (OETF_size * 2));
				fwif_color_ChangeUINT16Endian(pwUINT16Temp, OETF_size*3, 1);
			...
			*num_type = VIP_UINT16;
			...
			if (pwINT16Temp != NULL && (size == (sizeof(short) * (ToneMapping_size * 3))) && ret >= 0) {
				ret = fwif_color_rtice_DM2_ToneMapping_Get(mode0, pwINT16Temp + (ToneMapping_size * 0), pwINT16Temp + (ToneMapping_size * 1), pwINT16Temp + (ToneMapping_size * 2));
				fwif_color_ChangeUINT16Endian(pwINT16Temp, ToneMapping_size*3, 1);
				memcpy(buf, (char*) pwINT16Temp, size);
				dummy++;
			}
			*num_type = VIP_UINT16;
			...
			if (size == (sizeof(short) * (HDR_24x24x24_size * 3))) {
				ret = fwif_color_rtice_DM2_3D_24x24x24_LUT_Get(mode0, pwUINT16Temp, HDR_3DLUT_getFromReg);
				fwif_color_ChangeUINT16Endian(pwUINT16Temp, (HDR_24x24x24_size * 3), 1);
				memcpy(buf, (char*) pwUINT16Temp, size);
			}
			*num_type = VIP_UINT16;
...
int  rtice_SetGet_DeMura(ap_data_param *rtice_param, unsigned char *num_type, unsigned char *buf, _system_setting_info *system_setting_info, unsigned char isSet_Flag)
{
	...
	if (isSet_Flag == 1) {
		...
		if (rtice_param->id == TOOLACCESS_VIP_DeMura_TBL) {
			...
			memcpy(pDemura_TBL_tmp , vir_addr_8, sizeof(char) * tbl_frame_size);
