...
{
#if 0 //old rgb/offset before gamma
...
#elif defined(CONFIG_RTK_SW_OUTGAMMA_CT)
	...
	if(Scaler_VOFromJPEG(Scaler_Get_CurVoInfo_plane()) == 1)
		fwif_color_set_rtk_colortemp_output_gamma(!Scaler_Get_CinemaMode_PQ(), 2048, 2048, 2048, 0, 0, 0, WaitSync);
	else
		fwif_color_set_rtk_colortemp_output_gamma(!Scaler_Get_CinemaMode_PQ(), colorTemp->redGain<<2, colorTemp->greenGain<<2, colorTemp->blueGain<<2,
		(colorTemp->redOffset-512)*4, (colorTemp->greenOffset-512)*4, (colorTemp->blueOffset-512)*4, WaitSync);
#else //use color temp after gamma
	unsigned char WaitSync = 1;
	if ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) != _MODE_STATE_ACTIVE) || Check_AP_Set_Enable_ForceBG(SLR_MAIN_DISPLAY))
		WaitSync = 0;
