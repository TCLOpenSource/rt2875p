			...
			for (h = h_sta; h <= h_end; h++) {
				...
				switch(HSI)
				{
					case _HUE_ELEM:
						...
						//printk("[MHH]offset = %d, h = %d, hue_Adj = %d, hue_Cur = %d, sat_Cur = %d\n", offset, h, hue_Adj, hue_Cur, sat_Cur);
						//printk("[MHH]buf[(((i - i_sta) * slen + (s - s_sta)) * hlen + (h - h_sta)) * 2 + 0] = %d\n", buf[(((i - i_sta) * slen + (s - s_sta)) * hlen + (h - h_sta)) * 2 + 0]);
		...
		for (i = i_sta; i <= i_end; i++) {
			for (h = h_sta; h <= h_end; h++) {
				...
				if(nOffset<0)
				{
					...
					//printk("check3, (h,s,i) = %d,%d,%d, sat_Bnd = %d, sat_Cur = %d, nOffset = %d, offset_tmp = %d", h, s, i, sat_Bnd, sat_Cur, nOffset, offset_tmp);
					//if(offset_final < offset_tmp) //. find max
						//offset_final = offset_tmp;
					if(h != h_sta && h != h_end)
					{
						...
						int s_sta, int s_end, unsigned char s_total, int i_sta, int i_end, unsigned char i_total)
{
	...
	if(nOffset<0 )
		...
		offset_final = 1024;
	printk("check2, nOffset = %d", nOffset);
	/* find max/min offset */
	for (i = i_sta; i <= i_end; i++) {
		for (s = s_sta; s <= s_end; s++) {
			for (h = h_sta; h <= h_end; h++) {
				...
				if(nOffset<0)
				{
					...
					//printk("check3, (h,s,i) = %d,%d,%d, itn_Bnd = %d, itn_Cur = %d, nOffset = %d, offset_tmp = %d", h, s, i,itn_Bnd, itn_Cur, nOffset, offset_tmp);
					if(h != h_sta && h != h_end)
					{
						if(offset_final < offset_tmp ) //. find max
							offset_final = offset_tmp;
...
// pillar:		0...	         		   ...47
	...
	if(h_sta>h_end) //. hue R
	{
		...
		if(buf1 == NULL)
		{
			printk("drvif_color_icm_Adjust_Block, memory allocate fail\n");
			return -1;
		}
