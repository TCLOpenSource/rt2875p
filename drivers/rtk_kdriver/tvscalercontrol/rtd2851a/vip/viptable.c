...
//TCON
DRV_od_table_mode_t			m_pOD_setting_table[VIP_QUALITY_SOURCE_NUM];
DRV_od_table_t              m_pOD_table;
unsigned char 				m_pod_table_test[OD_table_length];
VIP_DeMura_TBL				m_pDeMura_TBL;
unsigned char 				m_pVALC_Table[6][17][17];
DRV_pcid_valc_t  			m_pVALCprotectSetting[2];
unsigned short 				m_pLEDOutGamma[257];
VIP_Output_InvOutput_Gamma 	m_pOutput_InvOutput_Gamma[Output_InvOutput_Gamma_TBL_MAX];
...
void VipTableInit()
{
	...
	memset(m_pOD_setting_table, 0, sizeof(DRV_od_table_mode_t) * VIP_QUALITY_SOURCE_NUM);
	//memset(m_pOD_table, 0, sizeof(DRV_od_table_t));
	memset(m_pod_table_test, 0, sizeof(unsigned char) * OD_table_length);
	//memset(m_pDeMura_TBL, 0, sizeof(VIP_DeMura_TBL));
	memset(m_pVALC_Table, 0, sizeof(unsigned char) * 6 * 17 * 17);
	memset(m_pVALCprotectSetting, 0, sizeof(DRV_pcid_valc_t) * 2);
	memset(m_pLEDOutGamma, 0, sizeof(unsigned short) * 257);
	memset(m_pOutput_InvOutput_Gamma, 0, sizeof(VIP_Output_InvOutput_Gamma) * Output_InvOutput_Gamma_TBL_MAX);
...
bool Init()
{
	...
	if (NULL == m_pictureMode) {
		...
		if(m_pictureMode == NULL)
			{
				rtd_pr_vpq_err("[%s:%d] can't alloc memory\n", __FILE__, __LINE__);
				return false;
			}
...
void* GetColorTempLevel(UINT32 source, UINT32 timing, UINT32 color)
{
	...
	if (mode >= PICTURE_MODE_MAX)
		mode = PICTURE_MODE_USER;
...
//	SourceOption sourceOption = xGetCurSource();
	...
	if (curPictureMode >= PICTURE_MODE_MAX)
		curPictureMode = PICTURE_MODE_USER;
...
void* GetQualityCoef(VIP_SOURCE_TIMING source_timing)
{
	...
	return (void *)(((UINT8 *)m_pVipTable->VIP_QUALITY_Coef) + source_timing * VIP_QUALITY_FUNCTION_TOTAL_CHECK);
}
...
void* GetQualityCoefExtend(VIP_SOURCE_TIMING source_timing)
{
	...
	return (void *)(((UINT8 *)m_pVipTable->VIP_QUALITY_Extend_Coef) + source_timing * VIP_QUALITY_FUNCTION_TOTAL_CHECK);
}
...
void* GetQualityCoefExtend2(VIP_SOURCE_TIMING source_timing)
{
	...
	return (void *)(((UINT8 *)m_pVipTable->VIP_QUALITY_Extend2_Coef) + source_timing * VIP_QUALITY_FUNCTION_TOTAL_CHECK);
}
...
void* GetQualityCoefExtend3(VIP_SOURCE_TIMING source_timing)
{
	...
	return (void *)(((UINT8 *)m_pVipTable->VIP_QUALITY_Extend3_Coef) + source_timing * VIP_QUALITY_FUNCTION_TOTAL_CHECK);
}
...
void* GetQualityCoefExtend4(VIP_SOURCE_TIMING source_timing)
{
	...
	return (void *)(((UINT8 *)m_pVipTable->VIP_QUALITY_Extend4_Coef) + source_timing * VIP_QUALITY_FUNCTION_TOTAL_CHECK);
}
...
void* GetQualityCoef3Dmaode(VIP_3DSOURCE_TIMING source_timing)
{
	...
	return (void *)(((UINT8 *)m_pVipTable->VIP_QUALITY_3Dmode_Coef) + source_timing * VIP_QUALITY_3DFUNCTION_TOTAL_CHECK);
}
...
SLR_COLORTEMP_DATA* GetCurColorTempData()
{
	...
	sourceOption = xGetCurSource();
	curTiming = xGetCurVipTiming();
	//rtd_pr_vpq_debug("\033[1;40;31m ************bug sourceOption not send to GetColorTempLevel which need input fmt*******\033[m\n");
...
SLR_PICTURE_MODE_DATA* GetCurPictureModeData(UINT32 picMode)
{
	...
	if (curPictureMode >= PICTURE_MODE_MAX)
		curPictureMode = PICTURE_MODE_USER;
	return &m_picModeTable[iIdx].picMode[curPictureMode];
}
...
// we need to change sourceOption value .
	...
	//rtd_pr_vpq_debug("planck:::GetCurPictureModeData:::sourceOption=%d\n", sourceOption);
	//rtd_pr_vpq_debug("planck:::GetCurPictureModeData:::curTiming=%d\n", curTiming);
	//rtd_pr_vpq_debug("planck:::GetCurPictureModeData:::curPictureMode=%d\n", curPictureMode);
	//rtd_pr_vpq_debug("planck:::GetCurPictureModeData:::vipPictureMode=%d\n", vipPictureMode);
...
void SetValueToAllPictureModeTable(unsigned char which ,unsigned char val)
{
	...
	rtd_pr_vpq_debug("m_iIndexSourceCnt[VIP_TABLE_PICMODE];:%d\n",m_iIndexSourceCnt[VIP_TABLE_PICMODE]);
...
bool xSourceControl_GetSourceByIdx(SourceOption *pRetSource, int index)
{
	...
	//rtd_pr_vpq_debug("VipTable::SourceControl_GetSourceByIdx error! index = %d\n", index);
...
void xLoadDefaultColorTempTable()
{
	...
	if (NULL == m_colorTempData) {
		rtd_pr_vpq_err("[%s:%d] alloc memory fail\n", __FILE__, __LINE__);
		return;
	}
...
void xLoadDefaultTCONTable()
{
	memcpy(&m_pOD_setting_table, OD_setting_table, sizeof(DRV_od_table_mode_t) * VIP_QUALITY_SOURCE_NUM);
}
