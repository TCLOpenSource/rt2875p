...
extern void drv_auto_set_phase(unsigned char phase);
...
void YPbPr_DetectFail_NoSignal_Handler(void)
{
    ...
    if (ypp_debug_msg_print){
		...
		rtd_pr_adc_info("#####[%s(%d)]0xB8020018=0x%08x\n",__func__, __LINE__,IoReg_Read32(0xB8020018));
		rtd_pr_adc_info("#####[%s(%d)]0xB8021100=0x%08x\n",__func__, __LINE__,IoReg_Read32(0xB8021100));
		rtd_pr_adc_info("#####[%s(%d)]0xB8021118=0x%08x\n",__func__, __LINE__,IoReg_Read32(0xB8021118));
		rtd_pr_adc_info("#####[%s(%d)]0xB8020300=0x%08x\n",__func__, __LINE__,IoReg_Read32(0xB8020300));
		rtd_pr_adc_info("#####[%s(%d)]0xB802030C=0x%08x\n",__func__, __LINE__,IoReg_Read32(0xB802030C));
    }
}
	...
	for (i = 0; i < 21; i++) {
		  ...
		  if (autosoy_no_trigger_flag_reg.no_trig_r) {/* check notrig flag */
				...
				if (FALSE == AutoSOYRestart) {/* Run (False Restart) if notriger=0 in the first time */
					...
					rtd_pr_adc_notice("setting DCLevel level=0x%x,trigger i=%d\n", DCLevel + AUTO_SOY_COMPARE_LEVEL_OFFSET, i);
...
//	dds_pll_pllphase_ctrl_RBUS dds_pll_pllphase_ctrl_reg;
	...
	rtd_pr_adc_notice("#####[%s(%d)]auto phase=%d, uc64phase_limited:%d\n",__func__, __LINE__, phase, uc64phase_limited);
#if 0
	...
	if (Scaler_InputSrcGetMainChType() == _SRC_VGA) {
		...
		switch(p_dispinfo->ucMode_Curr){
			...
			case _MODE_1080I25:
			case _MODE_1080I30:
				fix_phase_value = 0x18;

				if(phase < fix_phase_value){
					M_VALUE = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_RESULT_PHASE_M_reg)&0x03FF;
					L_VALUE = IoReg_Read32(ADC_SMARTFIT_ADC_AUTO_RESULT_PHASE_L_reg);
					if(M_VALUE < 1 && L_VALUE < 0x1000000){
						phase = fix_phase_value;
					}
				}
	...
	rtd_pr_adc_notice("####[%s(%d)]fine-tune phase=%d, PhaseMax=%d\n",__func__,__LINE__, phase, PhaseMax);
...
{/* This api is setting after detect timing success  mode setting */
	...
	rtd_pr_adc_info("#####[%s(%d)]0xB8020018=0x%08x",__func__, __LINE__,IoReg_Read32(0xB8020018));
...
{
	//0xB80380BC[31]=1, will be do LDO trim
	return (IoReg_Read32(EFUSE_DATAO_47_reg) & _BIT31)>>31;
}

unsigned char YPbPr_LDO_Trim_vbias_4_3(void)
{
	//a. 0xb802000C[5:3] = 0xB80380BC[26:24]
	return (IoReg_Read32(EFUSE_DATAO_47_reg) & (_BIT25|_BIT24))>>24;
}

unsigned char YPbPr_LDO_Trim_vbias_5(void)
{
	//a. 0xb802000C[5:3] = 0xB80380BC[26:24]
	return (IoReg_Read32(EFUSE_DATAO_47_reg) & _BIT26)>>26;
}

unsigned char YPbPr_LDO_Trim_apllldosel(void)
{
	//b. 0xb8020104[17:16] = 0xB80380BC[28:27]
	return (IoReg_Read32(EFUSE_DATAO_47_reg) & (_BIT28|_BIT27))>>27;
}

unsigned char YPbPr_LDO_Trim_sog_cmp_7_6(void)
{
	//c. 0xb8020024[31:30] = 0xB80380BC[30:29]
	return (IoReg_Read32(EFUSE_DATAO_47_reg) & (_BIT30|_BIT29))>>29;
}
