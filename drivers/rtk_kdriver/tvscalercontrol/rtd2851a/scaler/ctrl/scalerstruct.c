...
StructSourceInput *	src_info = NULL;
...
static SCALER_VSC_INPUT_REGION_TYPE	scaler_InputRegionType[MAX_DISP_CHANNEL_NUM];
static StructSrcRect		adaptivestream_inputwin[MAX_DISP_CHANNEL_NUM];
...
DEBUG_SCALER_VSC_STRUCT DbgSclrFlgTkr=
{
	...
	TRUE,	//	char Main_Scaler_Stop_flag;
	...
	FALSE,	//    char Sub_smooth_toggle_set_flag;
	...
	0,  //smoothtoggle_start_flag
	0,  //run_dtgM2GoldenVsync_slow_tuning_flag
	FALSE,	//	char Main_run_smooth_toggle_from_mute_off;
	0,//sub_adaptive_flag
	0, //run_dtgM2UzuDelay_slow_tuning_flag;
}
; // debug scaler flag tracker

unsigned char Get_Val_DbgSclrFlgTkr(unsigned int index)
{
    switch(index)
    {
        case MAIN_SCALER_STOP_IDX://Main_Scaler_Stop_flag
        return DbgSclrFlgTkr.Main_Scaler_Stop_flag;
        break;

        case MAIN_RUN_SCALER_IDX://Main_Run_Scaler_flag
        return DbgSclrFlgTkr.Main_Run_Scaler_flag;
        break;

        case MAIN_RESET_MODE_IDX://Main_Reset_Mode_flag
        return DbgSclrFlgTkr.Main_Reset_Mode_flag;
        break;

        case MAIN_SMT_SET_IDX://Main_smooth_toggle_set_flag
        return DbgSclrFlgTkr.Main_smooth_toggle_set_flag;
        break;

        case MAIN_SMT_IS_FROM_MUTEOFF_IDX://Main_run_smooth_toggle_from_mute_off
        return DbgSclrFlgTkr.Main_run_smooth_toggle_from_mute_off;
        break;

        case MAIN_REQUEST_SMT_FROM_MUTEOFF_IDX://Main_SetOutput_request_muteoff_smooth_toggle_set_flag
        return DbgSclrFlgTkr.Main_SetOutput_request_muteoff_smooth_toggle_set_flag;
        break;

        case MAIN_OUTPUT_SET_IDX://Main_Output_Set_flag
        return DbgSclrFlgTkr.Main_Output_Set_flag;
        break;

        case SUB_SCALER_STOP_IDX://Sub_Scaler_Stop_flag
        return DbgSclrFlgTkr.Sub_Scaler_Stop_flag;
        break;

        case SUB_RUN_SCALER_IDX://Sub_Run_Scaler_flag
        return DbgSclrFlgTkr.Sub_Run_Scaler_flag;
        break;

        case SUB_RESET_MODE_IDX://Sub_Reset_Mode_flag
        return DbgSclrFlgTkr.Sub_Reset_Mode_flag;
        break;

        case SUB_SMT_SET_IDX://Sub_smooth_toggle_set_flag
        return DbgSclrFlgTkr.Sub_smooth_toggle_set_flag;
        break;

        case SUB_OUTPUT_SET_IDX://Sub_Output_Set_flag
        return DbgSclrFlgTkr.Sub_Output_Set_flag;
        break;
...
void Set_Val_DbgSclrFlgTkr(unsigned int index, unsigned char value)
{
    switch(index)
    {
        case MAIN_SCALER_STOP_IDX://Main_Scaler_Stop_flag
        DbgSclrFlgTkr.Main_Scaler_Stop_flag = value;
        break;

        case MAIN_RUN_SCALER_IDX://Main_Run_Scaler_flag
        DbgSclrFlgTkr.Main_Run_Scaler_flag = value;
        break;

        case MAIN_RESET_MODE_IDX://Main_Reset_Mode_flag
        DbgSclrFlgTkr.Main_Reset_Mode_flag = value;
        break;

        case MAIN_SMT_SET_IDX://Main_smooth_toggle_set_flag
        DbgSclrFlgTkr.Main_smooth_toggle_set_flag = value;
        break;

        case MAIN_SMT_IS_FROM_MUTEOFF_IDX://Main_smooth_toggle_set_flag
        DbgSclrFlgTkr.Main_run_smooth_toggle_from_mute_off = value;
        break;

        case MAIN_REQUEST_SMT_FROM_MUTEOFF_IDX://Main_SetOutput_request_muteoff_smooth_toggle_set_flag
        DbgSclrFlgTkr.Main_SetOutput_request_muteoff_smooth_toggle_set_flag = value;
        break;

        case MAIN_OUTPUT_SET_IDX://Main_Output_Set_flag
        DbgSclrFlgTkr.Main_Output_Set_flag = value;
        break;

        case SUB_SCALER_STOP_IDX://Sub_Scaler_Stop_flag
        DbgSclrFlgTkr.Sub_Scaler_Stop_flag = value;
        break;

        case SUB_RUN_SCALER_IDX://Sub_Run_Scaler_flag
        DbgSclrFlgTkr.Sub_Run_Scaler_flag = value;
        break;

        case SUB_RESET_MODE_IDX://Sub_Reset_Mode_flag
        DbgSclrFlgTkr.Sub_Reset_Mode_flag = value;
        break;

        case SUB_SMT_SET_IDX://Sub_smooth_toggle_set_flag
        DbgSclrFlgTkr.Sub_smooth_toggle_set_flag = value;
        break;

        case SUB_OUTPUT_SET_IDX://Sub_Output_Set_flag
        DbgSclrFlgTkr.Sub_Output_Set_flag = value;
        break;
...
unsigned char get_main_scaler_stop_flag(void)
{
	return DbgSclrFlgTkr.Main_Scaler_Stop_flag;
}

unsigned char judge_main_mute_off_trigger_smoothtoggle(void)
{//judge main trigger smooth from mute off or not
	return (Get_Val_DbgSclrFlgTkr(MAIN_SMT_IS_FROM_MUTEOFF_IDX)|| Get_Val_DbgSclrFlgTkr(MAIN_REQUEST_SMT_FROM_MUTEOFF_IDX));
}

void set_main_scaler_stop_flag(char enable)
{
	DbgSclrFlgTkr.Main_Scaler_Stop_flag = enable;
}

unsigned char Get_Val_main_adaptive_stream_flag(void)
{
	return DbgSclrFlgTkr.adaptive_stream;
}

void Set_Val_main_adaptive_stream_flag(unsigned char enable)
{
	DbgSclrFlgTkr.adaptive_stream = enable;
}

unsigned char Get_Val_sub_adaptive_stream_flag(void)
{
	return DbgSclrFlgTkr.sub_adaptive_flag;
}

void Set_Val_sub_adaptive_stream_flag(unsigned char enable)
{
	DbgSclrFlgTkr.sub_adaptive_flag = enable;
}
...
static SLR_VOINFO 			vo_info[VO_VIDEO_PLANE_V3];			// VO info from kernel space
static SLR_VOINFO 			pre_vo_info[VO_VIDEO_PLANE_V3];
...
static unsigned char sub_from_di_mux_path_flag = FALSE;

void Set_sub_from_DI_mux_path(unsigned char value)
{
	sub_from_di_mux_path_flag = value;
}

unsigned char Get_sub_from_DI_mux_path(void)
{
	return sub_from_di_mux_path_flag;
}
...
void Scaler_update_src_type(void)
{
	...
	{
		rtd_pr_vsc_debug("ret = %d, set src_type to driver fail !!!\n", ret);
	}
...
void Scaler_SET_VSCDispinfo_WithVFEDispinfo(unsigned char display, StructDisplayInfo* VFEDispInfo)
{
    if(VFEDispInfo)
    {
        ...
        Scaler_DispSetInputInfoByDisp(display, SLR_INPUT_VODMA_PLANE, VFEDispInfo->vodma_plane);
        Scaler_DispSetInputInfoByDisp(display, SLR_INPUT_VO_SRC_FRAMERATE, VFEDispInfo->SrcIVFreq);
...
unsigned char Scaler_TVSetChannelInfo(SLR_TV_CHANNEL_INFO *chInfo)
{
	...
	{
		rtd_pr_vsc_debug("ret=%d, set TV_scan_mode flag to driver fail !!!\n", ret);
		return FALSE;
	}
...
unsigned char Scaler_TVSetScanMode(unsigned char a_ucScanMode)
{
	...
	if (TRUE == a_ucScanMode)
	{
		...
		rtd_pr_vsc_debug("3D->2D finish count=%x\n", i);
	}
	else
		rtd_pr_vsc_debug("ATV auto scan End\n");
...
void Scaler_TVSetExitScan(bool m_exit)
{
	 ...
	 rtd_pr_vsc_debug("\033[1;31m ##### %s%d %s:tvExitScanflag:%d\033[m\n",__FILE__, __LINE__,__FUNCTION__,tvExitScanflag );
}
...
extern void vbe_set_HDMI_dynamic_film_mode_flag(unsigned int status);
...
void Scaler_DispSetInputInfoByDisp(SCALER_DISP_CHANNEL channel, SLR_INPUT_INFO infoList, unsigned short value)
{
	switch (infoList)
	{
...
#ifndef BUILD_QUICK_SHOW     
			if((channel == SLR_MAIN_DISPLAY) && (value == _MODE_STATE_SEARCH)) {
				set_hdr_semaless_active(FALSE);

				// Reset HDMI film mode flag
				vbe_set_HDMI_dynamic_film_mode_flag(FALSE);

                BSP_ERROR_BUFFER_RESET();//reset bsp error report status
			}
#ifdef NEW_INPUT_OUTPUT_BUFFER_FLOW_CONFIG
			if(value == _MODE_STATE_SEARCH)
				set_pst_active_state(channel, FALSE);//reste activer state
#endif
#endif
			break;
		...
		case SLR_INPUT_VODMA_PLANE:
			disp_info[channel].vodma_plane = value;
			break;
		case SLR_INPUT_VO_SRC_FRAMERATE:
			disp_info[channel].SrcIVFreq = value;
			break;
		default:
			rtd_pr_vsc_debug("[disp_info] Don't know how to set disp_info!!!\n");
			break;
	}
...
unsigned short Scaler_DispGetInputInfoByDisp(unsigned char channel, SLR_INPUT_INFO infoList)
{
	...
	if ( channel > SLR_SUB_DISPLAY) {
		rtd_pr_vsc_debug("[disp_info] Invalid channel\n");
		return 0;
	}
// frank@1017 add below code to solve scan tool warning
#ifndef CONFIG_DUAL_CHANNEL
	...
	switch (infoList)
	{
		...
		case SLR_INPUT_VODMA_PLANE:
			...
			//livezoom or magnifier use the same plane, do not force plane @Crixus 20170110
			if(Get_Live_zoom_mode() == LIVE_ZOOM_ON){
				return disp_info[channel].vodma_plane;
			}else{
				//i2r case, main force to use V1, sub force to use plane v2 @Crixus 20170110
...
#endif
		case SLR_INPUT_VO_SRC_FRAMERATE:
			return disp_info[channel].SrcIVFreq;
		default:
			rtd_pr_vsc_debug("[disp_info] Don't know how to get disp_info!!!\n");
			return 0;
	}
}
...
void Scaler_Disp3dSetInfo(SCALER_DISP_CHANNEL channel, SLR_DISP_3D_INFO infoList, unsigned int value)
{
	...
	switch (infoList)
	{
		...
		case SLR_DISP_3D_MEM_LEN_PRE:
			...
			break;
		default:
			rtd_pr_vsc_debug("[disp3d_info] Don't know how to set disp3d_info!!!\n");
			break;
	}
...
unsigned int Scaler_Disp3dGetInfo(SCALER_DISP_CHANNEL channel, SLR_DISP_3D_INFO infoList)
{
	...
	switch (infoList)
	{
		...
		default:
			rtd_pr_vsc_debug("[disp3d_info] Don't know how to get disp3d_info!!!\n");
			return 0;
	}
}
...
// frank@1017 add below code to solve scan tool warning
	...
	if (index == SLR_DISP_COMP) {
		if ((!(disp_info[channel].disp_status & SLR_DISP_THRIP)) && (disp_info[channel].disp_status & SLR_DISP_INTERLACE))
			return TRUE;
		else
			return FALSE;
	}
...
SLR_RATIO_TYPE Scaler_DispGetRatioMode(void)
{
  ...
  // 4k2k photo mode ratio type only support 16:9
        ...
        if((Scaler_InputSrcGetMainChType() == _SRC_VO) && Scaler_VOFromJPEG(Scaler_Get_CurVoInfo_plane())){
            //rtd_pr_vsc_debug("%s line:%d 4k2k photo mode need to return 16:9 ratio\n", __FILE__, __LINE__);
        ...
        if(Scaler_GetHdmiDmaCapture_Enable() && (Scaler_GetHdmiDmaCapture_Status() >= HDMI_DMA_4K2K_STATE_DMA_ENABLE)){
            //rtd_pr_vsc_debug("%s line:%d Hdmi 4k2k need to return 16:9 ratio\n", __FILE__, __LINE__);
            return SLR_RATIO_16_9;
        }
    #endif
  #endif
...
void Scaler_InputRegionType_Set(unsigned char display,SCALER_VSC_INPUT_REGION_TYPE inputType)
{
	if(inputType > INPUT_BASE_ON_HAL_ORIGINAL_INPUT){
		rtd_pr_vsc_err("[error] wrong input region type:%d\n",inputType);
		inputType = INPUT_BASE_ON_TIMING;
	}
	scaler_InputRegionType[display] = inputType;
}
SCALER_VSC_INPUT_REGION_TYPE Scaler_InputRegionType_Get(unsigned char display)
{
	return scaler_InputRegionType[display];
}

void Scaler_AdaptiveStream_InputWindowSet(StructSrcRect m_inputwin, unsigned char display)
{
	adaptivestream_inputwin[display] = m_inputwin;
	return;
}

StructSrcRect Scaler_AdaptiveStream_InputWindowGet(unsigned char display)
{
	if((adaptivestream_inputwin[display].src_height == 0) ||(adaptivestream_inputwin[display].src_wid == 0)){
		...
		adaptivestream_inputwin[display] = Scaler_DispWindowGet();//don't overscan
	}
	return (StructSrcRect)adaptivestream_inputwin[display];
}
...
{
	//rtd_pr_vsc_debug("@@@@@@@@@@@@@@@@@@[%s]m_dispwin.wid:%x, m_dispwin.height:%x\n", __FUNCTION__, m_dispwin.src_wid, m_dispwin.src_height);
...
StructSrcRect Scaler_DispWindowGet(void)
{
	...
	if(Get_PANEL_TYPE_IS_MILLET3_4K2K_SERIES()){
		rtd_pr_vsc_debug("output_2k1k_timing:%d, Get_DISPLAY_2k1k_output_mode_status() : %d \n", output_2k1k_timing, Get_DISPLAY_2k1k_output_mode_status());
		if(output_2k1k_timing != Get_DISPLAY_2k1k_output_mode_status()){
			if(Get_DISPLAY_2k1k_output_mode_status() == 0){
				rtd_pr_vsc_debug("Scaler_DispWindowGet: By2\n");
				...
				rtd_pr_vsc_debug("Scaler_DispWindowGet : Divide 2\n");
				custom_dispwin.srcx /= 2;
				custom_dispwin.srcy /= 2;
				custom_dispwin.src_height/= 2;
				custom_dispwin.src_wid/= 2;
...
StructSrcRect* Get_Val_Scaler_DispWindowPnt(void)
{
	return &custom_dispwin;
}
...
unsigned char Scaler_Set3DEnable(unsigned char enable)
{
	...
	{
		rtd_pr_vsc_debug("ret=%d, set b3DEnablable flag to driver fail !!!\n", ret);
		return FALSE;
	}
...
void Scaler_SetVoRdyVersion(void)
{
	...
	//rtd_pr_vsc_debug("Scaler_SetVoRdyVersion  g_ulVoRdyVersion=%d\n", g_ulVoRdyVersion);
}
...
unsigned int Scaler_GetVoRdyVersion(void)
{
	...
	//rtd_pr_vsc_debug("Scaler_GetVoRdyVersion  g_ulVoRdyVersion=%d\n", g_ulVoRdyVersion);
}
...
/*fix me@frankcheng	int ret, ack;
	...
	if ((ret = ioctl(Scaler_GetDev(), SCALERIOC_SEND_DISP_READY_IRC, &ack)) < 0) {
		rtd_pr_vsc_debug("ret=%d, set Display Ready IRC to ROS fail !!!\n", ret);
		return FALSE;
	}*/
...
#ifdef CONFIG_I2RND_ENABLE
	//[ANDROIDTV-3549] i2run SDR+HDR case need to use original port info. @Crixus 20171110
	if(Scaler_I2rnd_get_display() == I2RND_MAIN_S0_TABLE)
		port = Get_DisplayMode_Port(0);
	else if(Scaler_I2rnd_get_display() == I2RND_SUB_S1_TABLE)
		port = Get_DisplayMode_Port(1);
	else
		port = Get_DisplayMode_Port(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY));
#else
	port = Get_DisplayMode_Port(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY));
#endif
...
SLR_VOINFO* Scaler_VOInfoPointer(unsigned char plane)
{
    if (plane>=VO_VIDEO_PLANE_V3)
    {
        plane = 0;
    }
	return &vo_info[plane];
}
...
unsigned char Scaler_DO_VO_timing_Equal(VO_VIDEO_PLANE plane)
{
	...
	*/
	...
	{
		rtd_pr_vsc_debug("ret=%d, get SCALERIOC_GETVOINFOl from driver fail@%s !!!\n", ret, __FUNCTION__);
	}
	...
	if (memcmp(pPreVoInfo, pVOInfo, sizeof(SLR_VOINFO)- (sizeof(SLR_AFD_TYPE) + (sizeof(unsigned int)*4))) == 0) //don't compare voinfo AFD, PixelAR and launcher
	{
		...
		#ifdef CONFIG_ARM64
		if((memcmp(pPreVoInfo, pVOInfo,((unsigned long)&(pVOInfo->afd)-(unsigned long)&(pVOInfo->mode))) == 0) && (!Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC))) //only compare input timing not afd/hv ratio
		#else
...
// FixMe, 20140928
	...
	// FixMe, 20140928
	rtd_pr_vsc_debug("Scaler_GetRoiInfo I3D[%d], mode/wid/len/vFreq=%d/%d/%d/%d\n", drvif_scaler3d_get_IDMA3dMode(), mode, width, height, ivFreq);
	...
	switch (srcType) {
		case _SRC_HDMI:
			if (drvif_IsHDMI()) {
				rtd_pr_vsc_debug("Scaler_GetRoiInfo IsHDMI = 1\n");
			...
			{
				rtd_pr_vsc_debug("Scaler_GetRoiInfo IsHDMI = 0, current DVI mode\n");
		    }
	...
	rtd_pr_vsc_debug("Scaler_GetRoiInfo hz = %d\n", hz);
...
bool Scaler_GetTimingIsHD(void) //peterpan1017,20130301: Get timing is SD ro HD
{
	...
	switch(Scaler_DispGetInputInfo(SLR_INPUT_MODE_CURR))
	{
		...
		//case _MODE_1440_480P:
			//rtd_pr_vsc_debug("\033[1;31m ##### %s%d %s\033[m\n",__FILE__, __LINE__,__FUNCTION__ );
			return FALSE;
		case _MODE_720P50:
		case _MODE_720P60:
		case _MODE_1080I25:
		...
		case _MODE_1080PM50:
		case _MODE_1080PM60:
			//rtd_pr_vsc_debug("\033[1;34m ##### %s%d %s\033[m\n",__FILE__, __LINE__,__FUNCTION__ );
			return TRUE;
		default:
			rtd_pr_vsc_debug("\033[1;31m [not Modestate Timing] %s%d %s\033[m\n",__FILE__, __LINE__,__FUNCTION__ );
