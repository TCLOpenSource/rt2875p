...
// refactor sta ==================================================>
#define GLOBAL_UC_VD_COUNT_2  2
#define GLOBAL_UC_VD_COUNT_15 15
#define GLOBAL_UC_VD_COUNT_20 20
#define GLOBAL_UC_VD_COUNT_30 30
#define GLOBAL_DELAY_TIMES_10 10
#define GLOBAL_DELAY_TIMES_20 20

void Scaler_AVD_Power_Setting_Check(void)
{
	if (FALSE == drvif_module_vdc_check_pll27x_setting()  || FALSE == module_vdc_check_vd_clock()) {
		...
		if(get_AVD_Global_Status() == SRC_CONNECT_DONE)
		{
			...
			if ((_SRC_TV == Scaler_AVD_Get_g_ucSrcType())
				&& (KADP_AVD_INTERNAL_DEMOD == Scaler_AVD_GetDemodType())) {
...
void Scaler_AVD_Timer_Status_Check(void)
{
	if ((TRUE == g_bTimerInitDone) && (TRUE == g_ucStopVDTimer)
		&& (get_AVD_Global_Status() == SRC_CONNECT_DONE)) {
		mod_timer(&VDTimerList, jiffies + HZ/100);
...
void Scaler_AVD_VD_Set_Mode_Check(unsigned char ucVdCount)
{
	unsigned char j=0;
	g_ucTimerCount = 0x00;
...
unsigned char Scaler_AVD_Signal_Consist_Check(unsigned char ucVdCount, unsigned char delayTime)
{
	unsigned char j=0;
...
void Scaler_AVD_Color_System_Check(void)
{
	if ((DETECT_STAGE_INIT == Scaler_AVD_GetDetectStage())
		&& (true == Scaler_AVD_Get_g_bStartChangeSystem())) {
...
void Scaler_AVD_Signal_Status_Check(void)
{
...
void Scaler_AVD_Cpeak_Status_Check(void)
{
	unsigned char vd_power_check;

	// av source, hs set-topbox display abnormal, gain data overflow
	...
	} else { //vd_cpeak_diable
		cpeak_check_flag =0;
	}
}

void Scaler_AVD_Noise_Status_Check(void)
{
...
void Scaler_AVD_Search_State_Check(void)
{
...
void Scaler_AVD_g_ucVDMode_Check(unsigned char uc625_flag, unsigned char *pVdChange)
{
...
unsigned char Scaler_AVD_TV_DVBT_Check(unsigned char *pVdChange)
{
	if (drvif_module_vdc_Get625flag() == 1) {
		...
		if ((drvif_module_vdc_ReadMode(VDC_SET) == VDC_MODE_PALI)
			&& (drvif_module_vdc_ReadMode(VDC_DETECT) == VDC_MODE_SECAM)) { // PALI => SECAM
			VDC_DEBUG_MSG(VDC_MSG_TVCHCH_DETAIL, "PAL to SECAM, 94a4=%x\n", vdcf_inl(0xb80194a4));
			Set_Val_AVD_g_ucVDMode(VDC_MODE_SECAM);
			*pVdChange = 1;
		} else if ((drvif_module_vdc_ReadMode(VDC_SET) == VDC_MODE_SECAM)
			&& (drvif_module_vdc_ReadMode(VDC_DETECT) == VDC_MODE_PALI)) { // SECAM => PALI
			VDC_DEBUG_MSG(VDC_MSG_TVCHCH_DETAIL, "SECAM to PAL, 94a4=%x\n", vdcf_inl(0xb80194a4));
			Set_Val_AVD_g_ucVDMode(VDC_MODE_PALI);
			*pVdChange = 1;
			...
			&& (drvif_module_vdc_ReadMode(VDC_DETECT) == VDC_MODE_PALI)) { // NTSC50 => PALI
		...
		if ((drvif_module_vdc_ReadMode(VDC_DETECT) == VDC_MODE_PALI)
			|| (drvif_module_vdc_ReadMode(VDC_DETECT) == VDC_MODE_SECAM)) { // set PALI or SECAM
			VDC_DEBUG_MSG(VDC_MSG_TVCHCH_DETAIL, "Detect NTSC, reset !! 94a4=%x\n", vdcf_inl(0xb80194a4));
			Scaler_AVD_ClearTiming();
			return FALSE;
		} else if (drvif_module_vdc_ReadMode(VDC_SET) != VDC_MODE_NTSC) { // set to NTSC
			VDC_DEBUG_MSG(VDC_MSG_TVCHCH_DETAIL, "SET NTSC, 94a4=%x\n", vdcf_inl(0xb80194a4));
			Set_Val_AVD_g_ucVDMode(VDC_MODE_NTSC);
			*pVdChange = 1;
		}
	}
...
unsigned char Scaler_AVD_TV_Colombia_Check(unsigned char *pVdChange)
{
	if (drvif_module_vdc_Get625flag() == 0) {
		if (Scaler_AVD_Get_g_ucMode_Curr() != _MODE_480I) {
		...
		if ((drvif_module_vdc_ReadMode(VDC_SET) == VDC_MODE_NTSC)
			&& (drvif_module_vdc_ReadMode(VDC_DETECT) == VDC_MODE_PALM)) { // NTSC => PAL_M
			Set_Val_AVD_g_ucVDMode(VDC_MODE_PALM);
			*pVdChange = 1;
		} else if ((drvif_module_vdc_ReadMode(VDC_SET) == VDC_MODE_PALM)
			&& (drvif_module_vdc_ReadMode(VDC_DETECT) == VDC_MODE_NTSC)) { // PAL_M => NTSC
			Set_Val_AVD_g_ucVDMode(VDC_MODE_NTSC);
			*pVdChange = 1;
		} else if ((drvif_module_vdc_ReadMode(VDC_SET) == VDC_MODE_NTSC443)
			&& (drvif_module_vdc_ReadMode(VDC_DETECT) == VDC_MODE_NTSC)) { // NTSC443 => NTSC
			Set_Val_AVD_g_ucVDMode(VDC_MODE_NTSC);
			*pVdChange = 1;
		} else if ((drvif_module_vdc_ReadMode(VDC_SET) == VDC_MODE_NTSC443)
			&& (drvif_module_vdc_ReadMode(VDC_DETECT) == VDC_MODE_PALM)) { // NTSC443 => PALM
			Set_Val_AVD_g_ucVDMode(VDC_MODE_PALM);
			*pVdChange = 1;
		...
		if ((drvif_module_vdc_ReadMode(VDC_DETECT) == VDC_MODE_NTSC)
			|| (drvif_module_vdc_ReadMode(VDC_DETECT) == VDC_MODE_PALM)) { // set PALI or SECAM
			VDC_DEBUG_MSG(VDC_MSG_TVCHCH_DETAIL, "Detect PALN, reset !! 94a4=%x\n", vdcf_inl(0xb80194a4));
			Scaler_AVD_ClearTiming();
			return FALSE;
		} else if (drvif_module_vdc_ReadMode(VDC_SET) != VDC_MODE_PALN) {
			Set_Val_AVD_g_ucVDMode(VDC_MODE_PALN);
			*pVdChange = 1;
		}
	}
...
unsigned char Scaler_AVD_ATV_No_Signal_Check(StructDisplayInfo* a_ptDispInfo, ModeInformationType* a_ptTimingInfo)
{
	if ((1 == drvif_module_vdc_GetLockStatus()) || (_SRC_TV != Scaler_AVD_Get_g_ucSrcType()))
		return TRUE;
...
#endif
	{
		if ((Get_Val_AVD_ucCurColorStd() == ZNTSC)
			...
			|| (Get_Val_AVD_ucCurColorStd()  ==  ZPAL_M)) { // 480i NTSC
			drvif_module_vdc_FreeRun(VDC_FREERUN_525);
			...
			if ((Get_Val_AVD_ucCurColorStd() == 0xff)
				...
				&& ((g_tSupportedColorSys & KADP_COLOR_SYSTEM_NTSC_M) == KADP_COLOR_SYSTEM_NTSC_M
					...
					|| (g_tSupportedColorSys & KADP_COLOR_SYSTEM_PAL_M) == KADP_COLOR_SYSTEM_PAL_M)) { // 480i NTSC
	...
	if (((a_ptDispInfo->ucMode_Curr) == 0)|| avd_good_timing_ready) {
		...
		drvif_inprocvideo_setup_NoSignal(Scaler_AVD_Get_g_ucSrcType(), a_ptDispInfo, a_ptTimingInfo, Get_Val_AVD_ucCurColorStd());
	}
...
unsigned char Scaler_AVD_DetectTiming(void)
{
	...
	unsigned char ucCheckStart = FALSE;
	...
	Scaler_AVD_Power_Setting_Check();

	Scaler_AVD_Timer_Status_Check();
	...
	if (1 == drvif_module_vdc_GetLockStatus()) {
		...
		ucCheckStart = (DETECT_STAGE_FINAL == Scaler_AVD_GetDetectStage())?TRUE:FALSE;
		drvif_module_vdc_CheckInputStatus(Scaler_AVD_Get_g_ucSrcType(), ucCheckStart);
		set_freezable();
		...
		case DETECT_STAGE_INIT:
			...
			Scaler_AVD_g_ucVDMode_Check(uc625_flag, &ucVdChange);
			...
			if (g_ucVDMode != VDC_MODE_AUTO) {
				Scaler_AVD_VD_Set_Mode_Check(GLOBAL_UC_VD_COUNT_15);
			}
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				// 300mS
				if(Scaler_AVD_Signal_Consist_Check(GLOBAL_UC_VD_COUNT_30, GLOBAL_DELAY_TIMES_20) == FALSE)
					return ucRet;
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			// 20mS
			if(Scaler_AVD_Signal_Consist_Check(GLOBAL_UC_VD_COUNT_2, GLOBAL_DELAY_TIMES_10) == FALSE)
				return ucRet;
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				// 300mS
				if (drvif_module_vdc_ReadMode(VDC_SET) == VDC_MODE_NTSC) {
					if(Scaler_AVD_Signal_Consist_Check(GLOBAL_UC_VD_COUNT_30, GLOBAL_DELAY_TIMES_10) == FALSE)
						return ucRet;
				}
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			...
			if (g_ucVDMode == VDC_MODE_AUTO) {
			// 200mS
				if(Scaler_AVD_Signal_Consist_Check(GLOBAL_UC_VD_COUNT_20, GLOBAL_DELAY_TIMES_10) == FALSE)
					return ucRet;
			...
			set_freezable();
			if (0 == drvif_module_vdc_GetLockStatus()) {
				rtd_pr_avd_info("#####(%d) Signal is no good return!\n", __LINE__);
				Scaler_AVD_Signal_Status_Check();
				return ucRet;
			}
			...
			} else { // _SRC_SCART, _SRC_SCART_RGB
			...
			if ((_SRC_TV == Scaler_AVD_Get_g_ucSrcType())||(_SRC_CVBS == Scaler_AVD_Get_g_ucSrcType()))
			{
				...
				if (drvif_module_vdc_ReadMode(VDC_DETECT) != drvif_module_vdc_ReadMode(VDC_SET))
				{
					...
					case TV_AllMode:
						...
						break;
					case TV_DVBT:     // NTSC, PAL I, SECAM
						if(Scaler_AVD_TV_DVBT_Check(&ucVdChange) == FALSE)
							return ucRet;
						break;
					case TV_Colombia: // NTSC, PAL_NC_358, PAL_M
						if(Scaler_AVD_TV_Colombia_Check(&ucVdChange) == FALSE)
							return ucRet;
						break;
					case TV_ATSC:     // NTSC
...
//					VDC_DEBUG_MSG(VDC_MSG_TVCHCH_DETAIL, "ATSC mode set == detect, 94a4=%x\n", vdcf_inl(0xb80194a4));
		...
		if(Scaler_AVD_ATV_No_Signal_Check(ptAVDScalerDispInfo, ptAVDScalerTimingInfo) == FALSE)
			return ucRet;
