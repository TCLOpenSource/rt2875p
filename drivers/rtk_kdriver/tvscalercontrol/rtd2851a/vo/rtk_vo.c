...
int vo_module_init(void)
{
    ...
    if (register_kernel_rpc(RPC_VCPU_ID_0x117_QOS_MODE, rpcSetQOSMode) == 1)
        rtd_pr_vo_crit("Register RPC HDR_FRAMEINFO failed!\n");
...
int VBM_BorrowMemChunk(unsigned long *buffer_addr, unsigned long *buffer_size, UINT32 reqSize, UINT32 type)
{
	...
	rtd_pr_vo_info("[vo] VBM_BorrowMemChunk: get addr %lx, size %ld\n", (unsigned long)*buffer_addr, (unsigned long)*buffer_size);
	if (*buffer_addr == 0) {
...
unsigned char checkConnectedSource(unsigned int ch)
{
...
unsigned long rpcVoReady(unsigned long para1, unsigned long para2)
{
	...
	if (ch == 0) { //dtv only run at main VO.
	...
	if ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) &&
		...
		(pVOInfo->h_width <= 1920) &&
		(pVOInfo->h_width <= 1080) &&
#endif
	...
	/*
	para1 bit4:display;
	bit3:smooth toggle fail flag;
	bit2-1:smooth toggle state decide open smooth toggle or not;
	bit0:curr timing go smooth toggle or not */
