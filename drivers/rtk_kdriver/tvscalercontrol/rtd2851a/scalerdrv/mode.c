...
void drvif_mode_onms_ctrl_setting(unsigned char channel, int src)
{
	...
	if(_CHANNEL1 == channel) {//main case new error status
		...
		if(onms_onms1_ctrl_reg.on1_start_ms) {
			...
			if (src == _SRC_YPBPR || src == _SRC_VGA) {//for K3LG-963 rzhen@2016-09-09
			...
			else if ((src == _SRC_HDMI) || (src == _SRC_VO)) {
#ifdef HDMI_HS_MEAS_OVER_RANGE_WD_ENABLE
				...
				if(src == _SRC_HDMI) {
					...
					onms_onms1_hsync_meas_ctrl_reg.on1_target_measure = onms_onms1_hs_period_reg.on1_de_hs_period;
					onms_onms1_hsync_meas_ctrl_reg.on1_average_line = 3;
...
{//This function is to set onlinemeasure watchdog and interrupt setting
	...
	if(_ENABLE == wdgenable)
	{
		if(_CHANNEL1 == channel)
		{
			...
			if (((src == _SRC_YPBPR) || (src == _SRC_VGA) || (src == _SRC_CVBS) || (src == _SRC_HDMI))&&(!Get_tv006_wb_pattern())) {
		...
		else// if(_CHANNEL2 == channel)
		{
			...
			if(drvif_Hdmi_GetVRREnable())//vrr case input is hdmi 2.1 need HDMI provide the api
			{
...
//				onms_onms2_interrupt_en_reg.on2_ie_vtotalde_chg = _ENABLE;//on2_vtotalde_chg interrupt enable Bit7
				...
				onms_onms2_interrupt_en_reg.on2_ie_hgclk_gate_over_range = _ENABLE;//bit22
...
void drvif_mode_dma_onms_ctrl_setting(VSC_INPUT_TYPE_T src)
{
...
unsigned int drvif_mode_dma_onms_int_setting(VSC_INPUT_TYPE_T src)
{
	unsigned int online_wd_int_setting = 0;
...
{//This function is to set onlinemeasure watchdog and interrupt setting
	...
	if(wdgenable || interruptenable) {	//dma online measure new status
		drvif_mode_dma_onms_ctrl_setting(src);
		//decide wd and int setting
		online_wd_int_setting = drvif_mode_dma_onms_int_setting(src);
	}
	...
	if(_ENABLE == wdgenable) {
		...
		if (((src == VSC_INPUTSRC_ADC) || (src == VSC_INPUTSRC_AVD) || (src == VSC_INPUTSRC_HDMI))&&(!Get_tv006_wb_pattern())) {
			ppoverlay_main_display_control_rsv_reg.main_wd_to_background = _ENABLE;//Enable force BG
		} else {
			ppoverlay_main_display_control_rsv_reg.main_wd_to_background = _DISABLE;//Disable forced bg for LGE//_ENABLE;	//Main Watch Dog Enable (Force-to-Background)
		}
...
void drvif_mode_clear_onms_error_status(unsigned char src)
{
	onms_onms1_status_RBUS onms_onms1_status_reg;
...
void drvif_mode_enableonlinemeasure(unsigned char channel)
{
	...
	rtd_pr_onms_debug("#####Enable onlinemeasure channel:%d######\r\n",channel);
	...
	//if(_CHANNEL1 == channel)
	{
			...
			else
			{
				...
				{//hdmi
					...
					if(drvif_Hdmi_GetVRREnable())//vrr case input is hdmi 2.1 need HDMI provide the api
					{
						...
						onms_onms1_demode_vs_uf_reg.on1_de_vsper_uf_th = ((unsigned int)CONFIG_ONLINE_XTAL_FREQ*(unsigned int)9)/600;//60 hz max 24300000 = 27000000*0.9 //HD SD suggest
		...
		else if(src == _SRC_YPBPR || src == _SRC_VGA) {//ADC case setting is different
...
#ifdef HDMI_HS_MEAS_OVER_RANGE_WD_ENABLE
		else if(src == _SRC_HDMI) {//HDMI case setting is different
			onms_onms1_ctrl_reg.on1_sycms_clk = 1;
			onms_onms1_ctrl_reg.on1_vtotal_max_delta = 5;//only for demode
			switch(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_MODE_CURR)) {
				case _MODE_480I:
...
#ifdef _CLEAR_ERROR_STATUS
		drvif_mode_clear_onms_error_status(src);
#endif //#ifdef _CLEAR_ERROR_STATUS
	}
#if 0 // mac7p no onms
#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
...
//#endif
...
#if 0 //def HDMI_VRR_ONLINEMEASURE_SUPPORT
	...
	if(src == VSC_INPUTSRC_HDMI)
	{
		...
		if(drvif_Hdmi_GetVRREnable())//vrr case input is hdmi 2.1 need HDMI provide the api
		{
			onms_onms3_demode_vs_uf_reg.on3_de_vsper_uf_th = ((unsigned int)CONFIG_ONLINE_XTAL_FREQ*(unsigned int)9)/600;//60 hz max 24300000 = 27000000*0.9 //HD SD suggest
...
//	sub_vgip_vgip_chn2_act_hsta_width_RBUS vgip_chn2_act_hsta_width_reg;
	...
	if (display == SLR_MAIN_DISPLAY) {
		if(Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_SEARCH){
			...
			rtd_pr_vsc_info("[cirxus][%s][%d]disable s1 clock!!\n", __FUNCTION__, __LINE__);
	...
	drvif_memory_free_block(MEMIDX_MAIN_THIRD);
	 ...
	 if(get_platform_model() == PLATFORM_MODEL_2K) {
		if(Get_Val_domain_2k_memory_addr()) {
			rtd_pr_vsc_info("M-domian free memory start address %lx\n", Get_Val_domain_2k_memory_addr());
...
{//return false: error
	...
	wait_ONMS_ready(display, offline_frame_rate);//wait onlne can pop result
	switch(online_path)
	{
		case _DAM_VGIP_PATH:
		{
