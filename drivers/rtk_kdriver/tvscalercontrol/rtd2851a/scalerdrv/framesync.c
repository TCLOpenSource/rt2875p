...
#ifdef CONFIG_DATA_FRAMESYNC_FORCE_422_CAP
	//VDEC 4k2k go non-gatting flow, do not check framesync error
	if((Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC) == TRUE) &&
		(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) &&
		(!((Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID_PRE) > 1920) &&
		(Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN_PRE) > 1080))))
		return TRUE;
#endif
