...
#define CONFIG_STR_HPD_LOW_SPEEDUP_FROM_BOOTCODE    0
...
{    
	...
	//HDMI_EMG("vfe_hdmi_drv_get_port_timing_info timing_info->mode=%d, t1.mode=%d, newbase_hdmi_get_hdmi_mode_reg=%d\n",
	//	timing_info->mode, tm.mode, newbase_hdmi_get_hdmi_mode_reg(port));
...
int vfe_hdmi_drv_get_display_timing_info(vfe_hdmi_timing_info_t *timing_info, unsigned char vsc_ch)
{
	...
	//HDMI_EMG("vfe_hdmi_drv_get_display_timing_info timing_info->mode=%d, t1.mode=%d, newbase_hdmi_get_hdmi_mode_reg=%d\n",
	//	timing_info->mode, t1.mode, newbase_hdmi_get_hdmi_mode_reg(port));
	...
	timing_info->vidoe_fmt = (vfe_hdmi_vsi_video_format_e)t1.hvf;
	...
	timing_info->color_depth  = (enum vfe_hdmi_color_depth_e)t1.colordepth;
	timing_info->color_imetry = (enum vfe_hdmi_color_imetry_e)t1.colorimetry;
	...
	timing_info->dvi_hdmi = ((newbase_hdmi_get_hdmi_mode(port)==0)? 0 : 1);
	//rtd_pr_hdmi_emerg("Timing=%dp%d\n", timing_info->active.h, timing_info->v_freq);


	HDMI_DBG("[- HDMI TIMING]\n");
	HDMI_DBG("==========================================\n");
	HDMI_DBG("timing_info->h_freq    = %d\n", timing_info->h_freq);
	HDMI_DBG("timing_info->v_freq    = %d\n", timing_info->v_freq);
	HDMI_DBG("timing_info->h_total   = %d\n", timing_info->h_total);
	HDMI_DBG("timing_info->v_total   = %d\n", timing_info->v_total);
	HDMI_DBG("timing_info->active.x  = %d\n", timing_info->active.x);
	HDMI_DBG("timing_info->active.y  = %d\n", timing_info->active.y);
	HDMI_DBG("timing_info->active.w  = %d\n", timing_info->active.w);
	HDMI_DBG("timing_info->active.h  = %d\n", timing_info->active.h);
	HDMI_DBG("timing_info->scan_type = %d\n", timing_info->scan_type);
	HDMI_DBG("timing_info->x_offset         = %d\n", timing_info->x_offset);
	HDMI_DBG("timing_info->y_offset         = %d\n", timing_info->y_offset);
	HDMI_DBG("timing_info->mode_id          = %d\n", timing_info->mode_id);
	HDMI_DBG("timing_info->mode_table_index = %d\n", timing_info->mode_table_index);
	HDMI_DBG("timing_info->polarity         = %d\n", timing_info->polarity);
	HDMI_DBG("timing_info->color_space      = %d\n", timing_info->color_space);
	HDMI_DBG("timing_info->color_depth      = %d\n", timing_info->color_depth);
	HDMI_DBG("timing_info->color_imetry     = %d\n", timing_info->color_imetry);
	HDMI_DBG("timing_info->full_3d_timing   = %d\n",
			 timing_info->full_3d_timing);
	HDMI_DBG("timing_info->vidoe_fmt(eVideoFormat) = %d\n",
			 timing_info->vidoe_fmt);
	HDMI_DBG("timing_info->struct_3d(e3DStructure) = %d\n",
			 timing_info->struct_3d);
	HDMI_DBG("==========================================\n");
...
void vfe_hdmi_drv_handle_on_line_measure_error(unsigned char vfe_call)
{
	...
	//Error Count Reset
	lib_hdmi_fw_char_err_rst(port,1);//reset fw ced det


	link_stable = newbase_hdmi_check_data_link_status(port);
	if (link_stable==FALSE) {
	...
	HDMI_EMG("[vfe_hdmi_drv_handle_on_line_measure_error %d] starts, vfe_call =%d, link_stable=%d\n", __LINE__, vfe_call, link_stable);
}
...
int vfe_hdmi_drv_set_hpd(vfe_hdmi_hpd_t* hpd_value)
{
	...
	if(hpd_value->hpd_ctl) {
		msleep(300);    // [NUMDEV-1002][HDMI][Dias] switch between Enhanced and Standard format cause No Signal, need to remove after code sync from merlin7
		lib_hdmi_z0_set(port, LN_ALL, 1);   // Q10235 PS4 toggle HPD only, it causes noise
		newbase_hdmi_set_hpd(port, 1);
	} else {
...
int vfe_hdmi_drv_get_diagnostic_data(vfe_hdmi_diagnostic_data* data,unsigned char* pdata)
{
	...
	case GENERAL_INFO:
		...
		if (newbase_hdmi_audio_get_audio_status(port, &audio_status)==0)
		{
			dagnostics_info.audio.ChannelStatusBits= audio_status.channel_num;// like 2ch -2, 5.1ch - 6, 7.1ch - 8
			dagnostics_info.audio.LayoutBitValue= (audio_status.channel_mode)?1:0;
	...
	case GET_ERR_INFO:
		...
		if(newbase_hdmi_gcp_err_det(port, DIAGNOSTIC_GCP_ERROR_THD, 0xff ,50))//GCP_ERROR
		{
			...
			if(lib_hdmi_hdcp22_is_onoff(port) && lib_hdmi_hdcp14_is_onoff(port)==0)//hdcp 2p2 enable
				dagnostics_error |= 0x02;//KADP_HDMI_ERROR_HDCP22_REAUTH;
				HDMI_EMG("Diagnostics HDCP22_REAUTH error\n");
		}
