                   ...
                   const char *buf, size_t count)
{
    ...
    if (strcmp(attr->attr.name, "debug_ctrl")==0)
    {
        ...
        {
            lib_hdmi_set_hdmi_bypass_phy_ps_mode((val) ? 1 : 0);
        }
                   ...
                   const char *buf, size_t count)
{
    ...
    if (strcmp(attr->attr.name, "flow_cfg")==0)
    {
        if (sscanf(buf, "G%2dE%2d=%5d", &group_id, &entry_id, &val)==3)
        {
            ...
            {
                rtd_pr_hdmi_debug("[HDMISYS]%s=%d, set successful!!!\n",
            ...
            {
                rtd_pr_hdmi_debug("[HDMISYS]%s=%d, set fail! Out of Range!\n",
                GET_FLOW_CFG_NAME(group_id, entry_id),
                val);
            }
        }
    ...
    )
{
    ...
    if (strcmp(attr->attr.name, "meas")==0)
    {
        ...
        if(lib_hdmims_offline_measure_native(&tm, frl_mode))
        {
            ...
            measure_result2 &= lib_hdmims_offline_measure(&tm2, frl_mode);
            ...
            n = scnprintf(ptr, count, "[OFMS] IHFreq: %d / %d (0.1KHz)\n", tm.h_freq, tm2.h_freq);
            ptr+=n; count-=n;
            n = scnprintf(ptr, count, "[OFMS] IVFreq: %d / %d (0.1Hz)\n", tm.v_freq, tm2.v_freq);
            ptr+=n; count-=n;
            ...
            ptr+=n; count-=n;
            n = scnprintf(ptr, count, "[ONMS3_DE] IVFreq: %d (0.1Hz)\n", tm.v_freq);
            ptr+=n; count-=n;
