...
void _hdmi_z0_power_on_delay_work_func(struct delayed_work *delayed_work);
void _hdmi_z0_power_off_delay_work_func(struct delayed_work *delayed_work);
#endif
#endif // UT_flag
...
char newbase_hdmi_get_pcb_earc_port(void)
{
	...
	for (hdmi_port=0; hdmi_port < HDMI_RX_PORT_MAX_NUM; hdmi_port++)
	{
		...
		{
			HDMI_EMG("%s [HDMI][ERR]Get PCB Parameter fail name = %s is not existed !\n" , __func__  , name );
...
#ifndef BUILD_QUICK_SHOW
void newbase_hdmi_qs_init(void)
{
	unsigned char j;
...
const HDMI_DETECT_EVENT_TABLE_ST hdmi_detect_event_table[] =
{
	...
	{HDMI_DETECT_ONMS_QUICK_RESET, HDMI_EVENT_ACTION_RESET_TO_WAIT_SYNC, "ONMS_QUICK_RESET"},
...
unsigned char newbase_hdmi_wait_sync(unsigned char port, unsigned char frl_mode)
{
	...
	if (frl_mode != MODE_TMDS)
	{
	    // Not support HDMI 2.1, just return false...		
		return FALSE;
	}
	else
	{
		unsigned int curr_clk = newbase_rxphy_get_clk(port);
		//use tmds_clk_diff and tmds_clk_tolerance  to Reduce complexity
		unsigned int tmds_clk_diff = HDMI_ABS(hdmi_rx[port].timing_t.tmds_clk_b, curr_clk);
		unsigned int tmds_clk_tolerance = TMDS_CLOCK_TOLERANCE(port);
		if ((curr_clk < VALID_MIN_CLOCK) ||
			(tmds_clk_diff > tmds_clk_tolerance))
		{
...
unsigned char newbase_hdmi_setup_video_pll(unsigned char port, unsigned char frl_mode)
{
	...
	{
		SET_H_RUN_VRR(port, emp_vtem.VRR_EN | newbase_hdmi_get_freesync_enable(port));
	}
	else
	{
	...
	HDMI_INFO("[setup_video_pll] port [%d], bIsHDMIDVI: %d, PixelRepeat: %d, ColorDepth: %s, ColorSpace: %s, BCH error 2bit(detect,cont,total) = (%d,%d,%d)\n",
		port,
		GET_H_MODE(port),
	...
	HDMI_INFO("[setup_video_pll] port =%d, p_phy_st->frl_mode =%d\n", port, frl_mode);
...
unsigned char newbase_hdmi_vic_correction(unsigned char port, HDMI_TIMING_T *tm, unsigned char force_assign_tm)
{
	...
	HDMI_INFO("newbase_hdmi_vic_correction[2.0][Port %d] force_assign_tm =%d\n", port, force_assign_tm);
	...
	else
	{
		...
		{
			HDMI_WARN("Table,\t HActive: %5d, VActive: %5d, HStart:%3d, VStart:%3d, h_total: %5d, v_total: %5d, hfreq:%4d, vfreq:%3d\n",
			...
			|| (force_assign_tm == TRUE))
			{
				...
				{//YUV420
					HDMI_WARN("OFMS,\t HActive: %5d, VActive: %5d, HStart:%3d, VStart:%3d, h_total: %5d, v_total: %5d, hfreq:%4d, vfreq:%3d\n",
						tm ->h_act_len, tm ->v_act_len, tm ->h_act_sta, tm ->v_act_sta, tm ->h_total, tm ->v_total, tm->h_freq, tm->v_freq);
					...
					HDMI_WARN("Final,\t HActive: %5d, VActive: %5d, HStart:%3d, VStart:%3d, h_total: %5d, v_total: %5d, hfreq:%4d, vfreq:%3d\n",
						tm ->h_act_len, tm ->v_act_len, tm ->h_act_sta, tm ->v_act_sta, tm ->h_total, tm ->v_total, tm->h_freq, tm->v_freq);

				}
				else
				{
					HDMI_INFO("Not YUV420!! No correction. Use OFMS originally result, HActive: %d, VActive: %d\n", tm ->h_act_len, tm ->v_act_len);
					is_correction = FALSE;
				}
			}
			else
...
unsigned char newbase_hdmi_measure(unsigned char port, unsigned char frl_mode)
{
	...
	// RL6641-1363 : fix issue of video broken on 480i/240p/576i/288p
...
unsigned char newbase_hdmi_check_data_link_status(unsigned char port)
{
    ...
    {//do nothing when running VRR/ALLM/DSC/ Enter Power saving 
        #ifndef BUILD_QUICK_SHOW
        HDMI_INFO("check_data_link_status failed, HDMI[p%d], VRR_EN=%d, ALLM=%d, Dolby mode=%d\n", port, newbase_hdmi_get_vrr_enable(port), is_allm, get_HDMI_Dolby_VSIF_mode());
        #endif
        return FALSE;
    }
...
unsigned char newbase_hdmi_check_mode(void)
{
	...
	if (hdmi_rx[port].hdmi_2p1_en==FALSE && !(lib_hdmi_get_fw_debug_bit(DEBUG_23_BITERR_DET)))
	{
...
HDMI_INT_PRO_CHECK_RESULT newbase_hdmi_get_is_interlace_reg(unsigned char port, unsigned char frl_mode, HDMI_MS_T ms_mode)
{
	...
	if(frl_mode == MODE_TMDS)
	{
		...
		case HDMI_MS_ONESHOT:
			...
			if (result!= HDMI_IPCHECK_INTERLACE)
			{
				if(lib_hdmi_vsync_checked(port)) // wait one more frame if no even odd detected
				{
					result = lib_hdmi_get_is_interlace_reg(port);
					lib_hdmi_clear_interlace_reg(port);     // clear interlace status
				}
...
void newbase_hdmi_tmds_channel_status_error_handler(unsigned char port, unsigned char* rest_phy)
{
    ...
    else
    {
        ...
        {
            ERP_WARN("HDMI[p%d] TMDS async fifo error, RGB unstable = R=%d, G=%d, B=%d (RGB stable=%d -> 0, async_fifo_error=%d, timeout_cnt=%d)\n",
                port,
                HDMI_TMDS_CTRL_get_rcd(rgb_status),
                HDMI_TMDS_CTRL_get_gcd(rgb_status),
                HDMI_TMDS_CTRL_get_bcd(rgb_status),
                hdmi_rx[port].rgb_detect, hdmi_rx[port].rgb_async_fifo_error, hdmi_rx[port].rgb_async_fifo_error_timeout);
...
void newbase_hdmi_vsync_detection_error_handler(unsigned char port)
{
    unsigned char is_allm = FALSE;
    unsigned char is_vrr = FALSE;
...
void newbase_hdmi_interlace_detection_error_handler(unsigned char port)
{
...
void newbase_hdmi_bch_error_detection_error_handler(unsigned char port, unsigned char frl_mode, unsigned char* rest_phy)
{
    if (lib_hdmi_get_bch_2bit_error(port))
    {
    ...
    if (time_after(jiffies, hdmi_rx[port].err_msg_display_timeout))
    {
        ...
        HDCP_INFO("HDMI[p%d][BCH] HDCP mode:%s, RxStatus(%02x, %02x), hdcp_enc=%d, bch err 1bit=%d, 2bit err (detect,duration,cnt)=(%d, %d, %d), bch_pkt_value=%d, I2c_status=%d\n", 
            ...
            hdmi_rx[port].bch_err_cnt_continue,
            hdmi_rx[port].bch_err_cnt,
            lib_hdmi_get_debug_bch_err_pkt(port), HDMI_STB_P0_HDCP_PCR_get_iic_st(hdmi_in(HDMI_STB_HDCP_PCR_reg)));
    ...
    if ((hdmi_rx[port].rgb_de_align_cnt > 10) && hdmi_rx[port].no_pkt_continue > 30 && hdmi_rx[port].video_t.vs_valid<5) // no any of info packet received and vsync not stable
    {
        ...
        {
            unsigned int max_recover_round = ((port == newbase_hdmi_get_current_display_port()) && newbase_hdmi_is_dispsrc(0)) ? 0xFFFFFFFF : 16;
...
void newbase_hdmi_error_handler(unsigned char port)
{
    ...
    {//For TMDS only
        ...
        if(GET_FLOW_CFG(HDMI_FLOW_CFG_ERR_DET, HDMI_FLOW_CFG4_CHRGB_CHECK_EN)&& is_wait_enough_time)
        {
            ...
            *--------------------------------------------------------------*/
            newbase_hdmi_tmds_channel_status_error_handler(port, &rest_phy);
        }
     ...
     *--------------------------------------------------------------*/
     newbase_hdmi_vsync_detection_error_handler(port);


    /*--------------------------------------------------------------
     * Interlace detection
     *--------------------------------------------------------------*/
     newbase_hdmi_interlace_detection_error_handler(port);
     ...
     * BCH Error Detection
     *--------------------------------------------------------------*/
     newbase_hdmi_bch_error_detection_error_handler(port, frl_mode, &rest_phy);
...
unsigned char newbase_hdmi_check_stable_before_display(void)
{
	...
	if(GET_FLOW_CFG(HDMI_FLOW_CFG_GENERAL, HDMI_FLOW_CFG0_FAST_DETECT_ENABLE) ==  HDMI_PCB_FORCE_ENABLE)
	{
		HDMI_WARN("HDMI[%d] Fast detect mode, bypass wait_hdcp_data_sync\n", port);
	}
	else if(GET_HDMI_DOLBY_VSIF_MODE() != 0)//DOLBY_HDMI_VSIF_DISABLE
	{
		HDMI_WARN("HDMI[%d] Detected Dolby mode=%d,  bypass wait_hdcp_data_sync\n", port, GET_HDMI_DOLBY_VSIF_MODE());
	}
	else
	{
		if (newbase_hdmi_wait_hdcp_data_sync(port)==0)
		{
			HDMI_EMG("HDMI[%d] Wait HDCP data sync failed\n", port);
	...
	avi_state = newbase_hdmi_get_avi_infoframe(port, &avi_info);
	if(avi_state == 0){
		HDMI_INFO("HDMI is ready to display, NO Avi INFO!\n");
	}else{
		HDMI_INFO("HDMI is ready to display, (Get vic, avi_info.VIC=%d)\n", avi_info.VIC);
	}
	...
	newbase_hdmi_get_vtem_info(port, &emp_vtem);
	#ifndef BUILD_QUICK_SHOW
	HDMI_INFO("Port=%d, RUN_VRR=%d (VRR_EN=%d, AMD_FREESYNC_en=%d), ALLM=%d, Dolby mode=%d, VIC=%d(avi_state:%d)\n",
		port,
		hdmi_rx[port].timing_t.run_vrr,
		emp_vtem.VRR_EN,
		newbase_hdmi_get_freesync_enable(port),
		is_allm,
		get_HDMI_Dolby_VSIF_mode(),
		avi_info.VIC,
		avi_state);
	#endif
...
unsigned char newbase_hdmi_gcp_err_det(unsigned char port, unsigned int err_threshold , unsigned char period ,unsigned char det_times)
{
	unsigned int before_err_count = hdmi_rx[port].bch_err_cnt;
	unsigned int after_err_count = 0;

	msleep(5 * det_times);

	after_err_count = hdmi_rx[port].bch_err_cnt - before_err_count;

	if((hdmi_rx[port].bch_err_cnt == 0) && (before_err_count>0))
	{
		HDMI_WARN("[newbase_hdmi_gcp_err_det port:%d] bch_err_cnt is clear by other thread!before_err_count =%d,\n", port, before_err_count);
		return 0;
	}

	if (after_err_count > err_threshold)
	{
		HDMI_WARN("[newbase_hdmi_gcp_err_det port:%d][bch_2bit] error count =%d\n", port, after_err_count);
		return 1;
	}
...
void lib_hdmi_delay_display_action(unsigned char nport)
{
