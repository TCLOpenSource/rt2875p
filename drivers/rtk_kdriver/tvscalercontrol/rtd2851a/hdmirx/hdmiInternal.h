...
enum HDMI_MODE_RESULT
{
    HDMI_MODE_NOSIGNAL  = 0xFF,
    HDMI_MODE_NOSUPPORT = 0xFE,
    HDMI_MODE_NOCABLE       = 0xFD,
    HDMI_MODE_SUCCESS       = 0xFC,
    HDMI_MODE_DETECT        = 0xFB,
    HDMI_MDOE_UNDKOWN   = 0xFA,
};
...
#define HDMI_FIX_RL6641_1363        //fix issue of video broken on 480i/240p/576i/288p
...
//for TV043 Ultra 4640R, if TV DC off/on, some times it will HDCP 2.2 handshake error
//and can not output
//If DC off/on, some times it will output HDCP 1.4
#ifndef BUILD_QUICK_SHOW
#define HDCP_STATUS_CHECK_ENABLE		1
#else
#define HDCP_STATUS_CHECK_ENABLE		0
#endif
#define HDCP_STATUS_CHECK_TOGGLE_MS	10000

#define HDCP_STATUS_CHECK_ROKU_ULTRA_4640R_HPD_MS	600

#define HPD_DEFAULT_LOW_MS          700     //KTASKWBS-8520,TV006 asked to adjust HPD Low status duration.
...
#define DIAGNOSTIC_GCP_ERROR_THD	30
#define DIAGNOSTIC_BCH_ERROR_THD 30
...
#define HDMI_LINK_STABLE_VSYNC_CHECK_THD    5
...
extern const char* _hdmi_hd21_frl_mode_str(unsigned char frl_mode);
...
typedef struct {
	...
	unsigned char hdmi_timing_ready;
	HDMI_PHY_PARAM_T phy_param_t;

	HDMI_TIMING_T timing_t;     // real video timing
...
extern HDMI_DETECT_CONFIG_ST g_detect_fsm_status[HDMI_PORT_TOTAL_NUM];
...
// -------------------------------------  hdmiHdmi ----------------------------------------------//
extern void newbase_hdmi_qs_init(void);
...
extern unsigned char newbase_hdmi_check_measure_v_status(unsigned char port);
...
extern unsigned char newbase_hdmi_compatibility_match(unsigned char port);
extern unsigned char newbase_hdmi_compatibility_run_action(unsigned char port);
...
extern unsigned char newbase_hdmi_gcp_err_det(unsigned char port, unsigned int err_threshold , unsigned char period ,unsigned char det_times);
...
extern void lib_hdmi_delay_display_action(unsigned char port);
#endif //#if HDMI_DISPLAY_DELAY_ENABLE
...
extern unsigned char lib_hdmims_measure(HDMI_TIMING_T *tm, unsigned char frl_mode);
...
extern unsigned char lib_hdmims_calc_freq(HDMI_TIMING_T *tm);;
extern unsigned char lib_hdmims_offline_measure(HDMI_TIMING_T *tm, unsigned char frl_mode);
extern unsigned char lib_hdmims_offline_measure_demode(HDMI_TIMING_T *tm, unsigned char frl_mode);
extern unsigned char lib_hdmims_offline_measure_native(HDMI_TIMING_T *tm, unsigned char frl_mode);
...
extern unsigned char lib_hdmims_online1_measure(HDMI_TIMING_T *tm);
...
extern unsigned char lib_hdmims_online3_measure(HDMI_TIMING_T *tm);
extern unsigned char lib_hdmims_online3_demode_measure(HDMI_TIMING_T *tm);
...
// new api for hdmiHdmi.c internal （newbase_hdmi_detect_mode）
extern unsigned char video_fsm_hdmi_wait_sync_proccess(unsigned char port, unsigned char frl_mode);
extern unsigned char video_fsm_hdmi_setup_video_pll_proccess(unsigned char port, unsigned char frl_mode);
extern unsigned char video_fsm_hdmi_display_on_proccess(unsigned char port, unsigned char frl_mode);
extern unsigned char video_fsm_hdmi_measure_proccess(unsigned char port, unsigned char frl_mode);
extern unsigned char video_fsm_hdmi_video_ready_proccess(unsigned char port, unsigned char frl_mode);
