...
void newbase_hdmi_qms_handler(unsigned char port)
{
    ...
    current_vfreq = lib_hdmims_calc_vfreq(hdmi_rx[port].video_t.vs2vs_cnt); // not lib_hdmims_get_onms3_vfreq, because rtd2851c and rtd2851a don't support DEmode

    if(g_qms_debug_print)
    {//for debugging
    HDMI_EMG("[QMS] port=%d,QMS_FSM:%d, current_qms_en=%d, m_const=(%d,%d), current_next_tfr=%d, thread_stop=%d, oringinal_vfreq=%d, new_vfreq=%d, qms_vs_cnt=%d\n",
        port,
    ...
    {
        HDMI_WARN("[newbase_hdmi_qms_handler port:%d] vs_cnt=%d, QMS en from %d -> %d, current_vfreq=%d, current_m_const=%d, current_next_tfr=%d\n",
            port, m_qms_status[port].qms_vs_cnt, m_qms_status[port].pre_qms_en, current_qms_en, current_vfreq, current_m_const, current_next_tfr);
    ...
    case HDMI_QMS_FSM_NOT_SUPPORT:
        if(newbase_hdmi_get_qms_enable(port))
        {
            ...
            {// 0 -> 1, start QMS
                ...
                HDMI_WARN("[QMS] port=%d,  (pre, curr) = (%d,%d) in HDMI_QMS_FSM_NOT_SUPPORT, go to READY, current_vfreq=%d, NEXT_TFR=%d\n", 
                    port, m_qms_status[port].pre_m_const, m_qms_status[port].current_m_const, current_vfreq, current_next_tfr);
            ...
            {// 0 -> 0, keep the same state
                ...
                m_qms_status[port].current_m_const = current_m_const;
                m_qms_status[port].pre_vfreq = current_vfreq; 
            }
        }
    ...
    case HDMI_QMS_FSM_VFREQ_CHANGE_START:
    {
        ...
        {
            HDMI_WARN("[QMS] port=%d,  (pre, curr) = (%d,%d) in VFREQ_CHANGE_START, current_vfreq=%d, NEXT_TFR=%d\n", 
                port, m_qms_status[port].pre_m_const, m_qms_status[port].current_m_const, current_vfreq, current_next_tfr);
        }
    }
        break;
    case HDMI_QMS_FSM_VFREQ_CHANGE_RUNNING:
        ...
        {
            HDMI_WARN("[newbase_hdmi_qms_handler port:%d] Current QMS_FSM:%d, Unexpected current_m_const=%d, pre_m_const=%d, current_vfreq=%d, NEXT_TFR=%d\n",
                port, m_qms_status[port].qms_fsm, m_qms_status[port].current_m_const, m_qms_status[port].pre_m_const, current_vfreq, current_next_tfr);
        }
    ...
    case HDMI_QMS_FSM_VFREQ_CHANGE_END:
        ...
        else
        {
            ...
            HDMI_WARN("[newbase_hdmi_qms_handler port:%d] Current QMS_FSM:%d, Unexpected2 current_m_const=%d, pre_m_const=%d, current_vfreq=%d, NEXT_TFR=%d\n",
                port,  m_qms_status[port].qms_fsm, m_qms_status[port].current_m_const, m_qms_status[port].pre_m_const, current_vfreq, current_next_tfr);
        }
    ...
    case HDMI_QMS_FSM_READY:
        ...
        {//  QMS_EN=1, m_const 1->0, abnormal
            m_qms_status[port].pre_m_const = m_qms_status[port].current_m_const;
            m_qms_status[port].pre_vfreq = m_qms_status[port].current_vfreq; // record before vfreq
            HDMI_WARN("[newbase_hdmi_qms_handler port:%d] Current QMS_FSM:%d, QMS V freq change!! current_m_const=%d, pre_m_const=%d, current_vfreq=%d, NEXT_TFR=%d\n",
                port, m_qms_status[port].qms_fsm, m_qms_status[port].current_m_const, m_qms_status[port].pre_m_const, current_vfreq, current_next_tfr);
            SET_QMS_FSM(port,  HDMI_QMS_FSM_VFREQ_CHANGE_RUNNING);
        }
