...
void lib_hdmi_hdcp22_reset_aes_engine(unsigned char nport)
{
    hdmi_out(HDMI_SOFT_1_reg, 0);    // clock off
    udelay(5);    
    hdmi_out(HDMI_SOFT_0_reg, 0);    // rst low
    udelay(5);                            
    hdmi_out(HDMI_SOFT_0_reg, 1);    // rst high
    udelay(5);
    hdmi_out(HDMI_SOFT_1_reg, 1);    // clock on
}
...
void lib_hdmi_hdcp22_send_msg_command_optee(unsigned char nport)
{
    ...
    //if(Get_Hdmi_hdcp_2_2_Read_Message())
    {
...
#endif
        {
            ...
            {
                count--;
                //rtd_pr_hdmi_info("wait hprime  count =%x !!!!!!!!",count);
            ...
            if (count == 0)
                rtd_pr_hdmi_emerg("[HDCP2.2][ERR] Tx read hprime timeout\n");
#endif
...
void lib_hdmi_hdcp22_get_msg_command(unsigned char nport)
{
    ...
    if (lib_hdmi_hdcp22_get_write_msg(nport))  //GET TX mesage
    {
        ...
        case AKE_INIT:
            ...
            hdcp2_load_from_flash(); //reload to void mircast hdcp2 key conflict
        ...
        case SKE_Send_Eks:
            bLen =24;
            p_hdcp->hdcp2_state = HDCP2_SEND_EKS;
            HDCP_INFO("HDMI[p%d] HDCP2 receives Eks, authen successed (round=%d) (STB_HDCP_2p2_CR=%08x, STB_HDCP_2p2_SR0=%08x), Bit Error(%d,%d)\n",
                nport, p_hdcp->hdcp2_auth_count, hdmi_in(HDMI_STB_HDCP_2p2_CR_reg), hdmi_in(HDMI_STB_HDCP_2p2_SR0_reg),
                hdmi_rx[nport].bch_1bit_err_cnt,
                hdmi_rx[nport].bch_err_cnt);
            break;
