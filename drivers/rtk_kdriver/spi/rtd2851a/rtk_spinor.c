...
{
	rtd_pr_spi_nor_info("Not support FCIC interface!\r\n");
	return -1;
}
...
#ifndef CONFIG_CUSTOMER_TV006	
	// 	DIO_SPI_WP		PAD_GPIO_83_IO	pad_gpio_83
	// 	DIO_SPI_DI		PAD_GPIO_84_IO	pad_gpio_84, 0x18000894[19] =0x1(internal pull high)
	//	DIO_SPI_CSN		PAD_GPIO_85_IO	pad_gpio_85
	//	DIO_SPI_HOLD		PAD_GPIO_86_IO	pad_gpio_86
	//  REG32(0x18000894) = (REG32(0x18000894) & 0x0f0f0f0f) | 0x10181010;
	regval1 = (rtd_inl(0xB8000894) & 0x0f0f0f0f) | 0x10181010;
	rtd_outl(0xB8000894, regval1);

	//	DIO_SPI_SCLK		PAD_GPIO_87_IO	pad_gpio_87
	//	DIO_SPI_DO		PAD_GPIO_88_IO	pad_gpio_88
	//  REG32(0x18000898) = (REG32(0x18000898) & 0x0f0fffff) | 0x10100000;
	regval2 = (rtd_inl(0xB8000898) & 0x0f0fffff) | 0x10100000;
	rtd_outl(0xB8000898, regval2);
#endif

	regval1 = rtd_inl(0xB8000894);
	regval2 = rtd_inl(0xB8000898);

	rtd_pr_spi_nor_info("SPI Flash pinmux setting check, reg-0x%x: 0x%x , reg-0x%x: 0x%x!\r\n", \
		0xB8000894, regval1, 0xB8000898, regval2);
...
static __maybe_unused  void sfc_read_md(struct spinor_chip_t *spinor_chip, loff_t from, size_t len, unsigned char *buf)
{
	...
	ret = smd_fdma_copy(from, buf, len, SET_DMA_LEN_512, FLASH_2_DDR);
	if(ret == ENXIO){
		sfc_memcpy((unsigned long)buf, (SPI_BASE_ADDR + (unsigned long)from), len);
	}
...
static __maybe_unused int sfc_write_md(struct spinor_chip_t *spinor_chip, loff_t to, size_t len, unsigned char *buf)
{
	...
	ret = smd_fdma_copy(to, buf, len, SET_DMA_LEN_512, DDR_2_FLASH);
	if(ret == ENXIO){
		sfc_memcpy((SPI_BASE_ADDR + (unsigned long)to), (unsigned long)buf, len);
	}
...
static int sfc_set_clk(struct spinor_chip_t *spinor_chip, unsigned int clk)
{
	...
	if(clk_div <= support_max_div && clk_div >= support_min_div) {
		...
		for(delay_sel=0; delay_sel< DELAY_SEL_STEP_TOTAL_CNT; delay_sel++)
		{
			...
			rtd_pr_spi_nor_info("[SPINOR-INFO] ID: 0x%x, clk_div:0x%x, delay_sel:0x%x %s !\n", \
				id, clk_div, delay_sel, valid_delay_sel ? "valid": "invalid"); 
		}		

		rtd_pr_spi_nor_info("[SPINOR-INFO]find available delay_sel_flag(0x%x) for clock(0x%x)\n", delay_sel_flag, clk_div);
...
static int sfc_init_early(struct spinor_chip_t *spinor_chip, RHAL_SPI_INIT_PARAM_T  param)
{
	...
	rtd_maskl(SB2_SFC_CE_reg, ~SB2_SFC_CE_tdt_mask, SB2_SFC_CE_tdt(0x39));			/*HW suggest:set tdt value to 0x39*/
...
retry_sacn:
	...
	if (i == DEV_SIZE_S) {
		rtd_pr_spi_nor_err("[SPINOR-INFO]Cannot find any match ID!!!, ID: 0x%x clk_div:0x%x, delay_sel:0x%x\n", id, spinor_chip->clock_div, spinor_chip->delay_sel);
		goto end;
	}
	else
	{
