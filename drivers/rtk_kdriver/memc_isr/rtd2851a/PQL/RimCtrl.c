...
VOID RimDet_by_KMC_YUV_Meter(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	unsigned int right_sketchy_line = 0;
	unsigned int right_precise_line = 0;
	unsigned int rdbk_histo00_cnt = 0,sum_rdbk_histo01to31_cnt = 0;
	unsigned int u32_i;
	unsigned short rim_cnt = 2160,h_line_total = 3840;//4k
	//static unsigned short H_boundary_search_range = 16;//4k
	static unsigned short H_line_search_start = 0, H_line_search_end_check1= 0, H_line_search_end_check2 = 0;
	static unsigned int H_line_search_position = 0,H_line_search_position_pre = 0;
	static unsigned short status_machine = 0;
	static unsigned short H_line_search_position_max = 0;
	...
	ReadRegister(HARDWARE_HARDWARE_43_reg,0,31, &u32_rb_val_flag);/*0xB809B03C*/		
#endif 
	...
	if(pOutput->u4_outResolution_pre == _PQL_OUT_1920x1080){// 2k
		if((right_sketchy_line<1903)&&(right_precise_line<1903))
			return;
	}else{//4//4k
		if((right_sketchy_line<3823)&&(right_precise_line<3823))
			return;
	}
	...
	if(u32_rb_val_flag&0x4)
	{
		...
		rtd_pr_memc_err("[sum_rdbk_histo01to31_cnt]>>[%d]\n",sum_rdbk_histo01to31_cnt);
		rtd_pr_memc_err("[rdbk_histo00_cnt]>>[%d]\n",rdbk_histo00_cnt);
	}
#endif			
	...
	if(status_machine == 0){//init
		...
		H_line_search_start = h_line_total-17;
		H_line_search_end_check1 = h_line_total-17;
		H_line_search_end_check2 = h_line_total-1;
		...
		WriteRegister(KMC_METER_TOP_KMC_METER_TOP_00_reg, 16,28,H_line_search_end_check2);
		status_machine = 1;
		...
		if(sum_apl == 0){
			...
			if(H_line_search_position!=H_line_search_position_pre){
				...
				if( sum_rdbk_histo01to31_cnt >(rim_cnt<<2)){
					status_machine = 2;
				}else{
						if((sum_rdbk_histo01to31_cnt>0)&&(sum_rdbk_histo01to31_cnt<(rim_cnt<<2))){
							status_machine = 4;
							H_line_search_position_max = H_line_search_position;
						}else{
							H_line_search_position=_CLIP_(H_line_search_position,h_line_total-17,h_line_total-1);
							if((H_line_search_end_check2-H_line_search_end_check1)<2){
								H_line_search_end_check1 = H_line_search_position;
								H_line_search_end_check2 = H_line_search_position+4;
							}
							status_machine = 3;
						}
					}
		...
		else if(status_machine==2){//positon < h_rim

			H_line_search_position = H_line_search_position+4;
			H_line_search_position=_CLIP_(H_line_search_position,h_line_total-17,h_line_total-1);

			WriteRegister(KMC_METER_TOP_KMC_METER_TOP_00_reg, 2,14,H_line_search_position);
			WriteRegister(KMC_METER_TOP_KMC_METER_TOP_00_reg, 16,28,H_line_search_end_check2);
				
			pOutput->u12_out_resolution[_RIM_RHT] = H_line_search_position;
			H_line_search_position_max=H_line_search_position;
			status_machine=1;
...
#if 1
	if(u32_rb_val_flag&0x4){// bit 2
		rtd_pr_memc_err("===== [%s], [%d] =========\n",__FUNCTION__, __LINE__);
		rtd_pr_memc_err("[status_machine]>>[%d]\n",status_machine);
		rtd_pr_memc_err("[H_line_search_position]>>[%d]\n",H_line_search_position);
		rtd_pr_memc_err("[H_line_search_end_check1]>>[%d]\n",H_line_search_end_check1);
		rtd_pr_memc_err("[H_line_search_end_check2]>>[%d]\n",H_line_search_end_check2);
		rtd_pr_memc_err("[rim_cnt]>>[%d]\n",rim_cnt);
		...
		//rtd_pr_memc_err("[cursor_color_y0,cursor_color_y1]>>[%d,%d]\n",cursor_color_y0,cursor_color_y1);
		rtd_pr_memc_err("[sum_apl,sum_apl_pre]>>[%d,%d]\n",sum_apl,sum_apl_pre);
...
//			                  (u8_k == _RIM_LFT? (u12_BBD_roughRim[u8_k] * _Rim_Hor_Bin_) : (u12_BBD_roughRim[u8_k] * _Rim_Hor_Bin_ - 1));
	...
	if(u32_rb_val&0x4){// bit 2
		...
		rtd_pr_memc_err("[u1_BBD_fineValid][T,B,L,R]>>[%d,%d,%d,%d]\n",u1_BBD_fineValid[_RIM_TOP],u1_BBD_fineValid[_RIM_BOT],u1_BBD_fineValid[_RIM_LFT],u1_BBD_fineValid[_RIM_RHT]);
		...
		rtd_pr_memc_err("[u12_BBD_roughRim][T,B,L,R]>>[%d,%d,%d,%d]\n",u12_BBD_roughRim[_RIM_TOP],u12_BBD_roughRim[_RIM_BOT],u12_BBD_roughRim[_RIM_LFT],u12_BBD_roughRim[_RIM_RHT]);
	...
	*/
	...
	if(log_en){
		rtd_pr_memc_notice("[Rim_check_protect_MC][%d][,%d,%d,][,%d,%d,][,%d,%d,%d,%d,][apl,%d,][flash_flg,%d,]\n\r", __LINE__, 
			pOutput->u12_Rim_Target[_RIM_LFT], pOutput->u12_Rim_Result[_RIM_LFT], u12_det_Rim[_RIM_LFT], u12_rim_bound[_RIM_LFT],
			u1_BBD_fineValid[_RIM_LFT], u12_BBD_fineRim[_RIM_LFT], u1_BBD_roughValid[_RIM_LFT], u12_BBD_roughRim[_RIM_LFT],
...
//	const _PQLCONTEXT *s_pContext = GetPQLContext();
	...
	unsigned int log_en = 0, test_val_2 = 0;
	ReadRegister(SOFTWARE2_SOFTWARE2_61_reg, 30, 30, &log_en);
