...
unsigned int vpq_project_id;
unsigned int vpqex_project_id;
///////////////////////////////////////////////////////////////////////////////////////////
...
VOID Dh_closeSignal_Detect(const _PARAM_DH_CLOSE *pParam, _OUTPUT_DH_CLOSE *pOutput)
{
    ...
    unsigned int  special_pattern_227_flag = 0, special_pattern_49_flag = 0;
	...
	if(pParam->u1_fblvl_en == 1)
	{
		...
		//	 rtd_pr_memc_info("u8_dh_fbLvl=%d u8_fbLvl=%d u8_fbLvl_Th0=%d u8_fbLvl_Th1=%d\n",u8_dh_fbLvl,u8_fbLvl, pParam->u8_fbLvl_Th0,pParam->u8_fbLvl_Th1);
	...
	if (pParam->u1_meAllDtl_en == 1)
	{
		...
		//unsigned char special_flag_insect = 0;
		...
		//ReadRegister(KME_ME1_BG1_ME_BG_DUMMY_0_reg,20, 20, &special_flag_insect);
		//ReadRegister(KME_ME1_BG1_ME_BG_DUMMY_0_reg, 20, 20, &condition_insect);
	...
	if (pParam->u1_lbmc_switch_en == 1)
	{
		...
		if((((pOutput->u8_lbmc_mode == _MC_NORMAL_) && (s_pContext->_output_mc_lbmcswitch.u8_lf_lbmcMode == _MC_SINGLE_UP_)) ||
		   ((pOutput->u8_lbmc_mode == _MC_NORMAL_) && (s_pContext->_output_mc_lbmcswitch.u8_lf_lbmcMode == _MC_SINGLE_DOWN_)))&& (!special_pattern_eagle_flag)&&(!special_pattern_transporter_flag)&&(!dhbypass_rotterdam_30s)&&(!special_pattern_huapi2_flag)&&(!special_pattern_227_flag))
		{
	...
	if(1)
	{
		...
		ReadRegister(KME_ME1_BG1_ME_BG_DUMMY_0_reg, 18, 18, &special_pattern_transporter_flag);
		u1_is_inFormat_PureVideo_dh = ((s_pContext->_output_frc_phtable.u1_is_inFormat_PureVideo) &&  (special_pattern_transporter_flag!=1));
