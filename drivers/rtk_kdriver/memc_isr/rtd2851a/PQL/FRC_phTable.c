...
VOID FRC_phTable_set_LineModeCtrl(VOID)
{
	...
	if(log_en){
		...
		__FUNCTION__, u1_isLowDelayMode, u1_adaptivestream_flag, MEMC_Lib_get_DisplayMode_Src(), MEMC_Lib_get_Vpq_LowDelay_to_MEMC_flag());
	}

	if(((in_fmRate == _PQL_IN_60HZ) && (out_fmRate == _PQL_OUT_120HZ)) || ((in_fmRate == _PQL_IN_50HZ) && (out_fmRate == _PQL_OUT_100HZ))){
			//if((u1_adaptivestream_flag == 1 && (u1_callback_subscibe == 1 || u1_direct_media_flag == 1)) || (u1_isLowDelayMode == 1 && MEMC_Lib_get_DisplayMode_Src() == 2/*VSC_INPUTSRC_HDMI*/ && MEMC_Lib_get_Vpq_LowDelay_to_MEMC_flag() == 1)){
		if((u1_isLowDelayMode == 1 && (u1_adaptivestream_flag == 1 || (MEMC_Lib_get_DisplayMode_Src() == 2/*VSC_INPUTSRC_HDMI*/ && MEMC_Lib_get_Vpq_LowDelay_to_MEMC_flag() == 1)))){
			if(status != 1){
...
VOID FRC_phTable_StateMachine(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
	...
	test_BFI_en = 0; // fwif_color_get_BFI_En();
...
VOID FRC_phTable_Generate(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
	...
	if(pOutput->u1_generate_video_en == 1 || pOutput->u1_generate_film_en == 1)
	{
		...
		VTotal = 0; // scalerdisplay_get_DTG_MASTER_vtotal_by_vfreq();
...
unsigned int DynamicGenPhaseTable(void)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	unsigned int Motion;
	unsigned char u8_i, CadLen = 0;
	PQL_INPUT_FRAME_RATE in_FrmRate;
	PQL_OUTPUT_FRAME_RATE out_FrmRate;
	static unsigned int lastmot = 0;

	if(s_pContext->_output_filmDetectctrl.u32_Mot_sequence[_FILM_ALL]==lastmot) {
		return 0;
	}

	if( (s_pContext->_output_filmDetectctrl.u32_Mot_sequence[_FILM_ALL]&0x1) == 0x1) {
		Motion = s_pContext->_output_filmDetectctrl.u32_Mot_sequence[_FILM_ALL] >> 1;
		CadLen = 1;
		...
		in_FrmRate = s_pContext->_external_data._input_frameRate;
		out_FrmRate = s_pContext->_external_data._output_frameRate;

		if(in_FrmRate==_PQL_IN_60HZ&&out_FrmRate==_PQL_OUT_120HZ){
			CadLen = 2*CadLen;
		}
		else if(in_FrmRate==_PQL_IN_30HZ&&out_FrmRate==_PQL_OUT_120HZ){
			CadLen = 4*CadLen;
		}
		else{
			CadLen = 0;
	...
	lastmot=s_pContext->_output_filmDetectctrl.u32_Mot_sequence[_FILM_ALL];
