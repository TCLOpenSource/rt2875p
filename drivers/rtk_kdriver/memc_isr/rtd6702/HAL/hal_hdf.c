...
unsigned char HAL_HDF_AddressCheck(unsigned int u32_address)
{
	...
	if(u32_address == (MC_DMA_MC_LF_DMA_WR_Rule_check_up_reg-MEMC_REG_BASE_OFFSET)
		...
		|| u32_address == (MC_DMA_MC_LFH_DMA_WR_Rule_check_up_reg-MEMC_REG_BASE_OFFSET)
		|| u32_address == (MC_DMA_MC_LFH_I_DMA_RD_Rule_check_up_reg-MEMC_REG_BASE_OFFSET)
		|| u32_address == (MC_DMA_MC_LFH_P_DMA_RD_Rule_check_up_reg-MEMC_REG_BASE_OFFSET))
	{
		...
		|| u32_address == (MC_DMA_MC_LFH_DMA_WR_Rule_check_low_reg-MEMC_REG_BASE_OFFSET)
		|| u32_address == (MC_DMA_MC_LFH_I_DMA_RD_Rule_check_low_reg-MEMC_REG_BASE_OFFSET)
		|| u32_address == (MC_DMA_MC_LFH_P_DMA_RD_Rule_check_low_reg-MEMC_REG_BASE_OFFSET))
	{
	...
	if(HAL_HDF_AddressCheck(u32Page_Reg_Addr) == _ADDR_CASE_MC_UP_BOUND){
		if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_8K4K){
			...
			if(Get_DISPLAY_REFRESH_RATE() <= 60){
				*u32Page_Reg_Val = (memc_dma_address_info->MC00_start_addr + KMC_TOTAL_SIZE_8buf_8bit_8K);
			}else{
				*u32Page_Reg_Val = (memc_dma_address_info->MC00_start_addr + KMC_TOTAL_SIZE_8buf_10bit_8K);
			}
			#else
			*u32Page_Reg_Val = (memc_dma_address_info->MC00_start_addr + KMC_TOTAL_SIZE_6buf_10bit_8K);
			#endif
		}else{
			//MC memory boundary
			#if CONFIG_MC_8_BUFFER
			if(Get_DISPLAY_REFRESH_RATE() <= 60){
				*u32Page_Reg_Val = (memc_dma_address_info->MC00_start_addr + KMC_TOTAL_SIZE_8buf_8bit);
			}else{
		...
		rtd_pr_memc_notice("[HAL_HDF_LimitCheck][_ADDR_CASE_MC_UP_BOUND][%x = %x]\n\r", u32Page_Reg_Addr, *u32Page_Reg_Val);
	}
		...
		*u32Page_Reg_Val = (memc_dma_address_info->MC00_start_addr);
		rtd_pr_memc_notice("[HAL_HDF_LimitCheck][_ADDR_CASE_MC_LOW_BOUND][%x = %x]\n\r", u32Page_Reg_Addr, *u32Page_Reg_Val);
	}
		...
		*u32Page_Reg_Val = (memc_dma_address_info->ME_start_addr + ME_TOTAL_SIZE);
		rtd_pr_memc_notice("[HAL_HDF_LimitCheck][_ADDR_CASE_ME_UP_BOUND][%x = %x]\n\r", u32Page_Reg_Addr, *u32Page_Reg_Val);
	}
	else if(HAL_HDF_AddressCheck(u32Page_Reg_Addr) == _ADDR_CASE_ME_LOW_BOUND){
		*u32Page_Reg_Val = (memc_dma_address_info->ME_start_addr);
		rtd_pr_memc_notice("[HAL_HDF_LimitCheck][_ADDR_CASE_ME_LOW_BOUND][%x = %x]\n\r", u32Page_Reg_Addr, *u32Page_Reg_Val);
	}
		...
		*u32Page_Reg_Val = (memc_dma_address_info->ME_start_addr + ME_TOTAL_SIZE);
		rtd_pr_memc_notice("[HAL_HDF_LimitCheck][_ADDR_CASE_ME_SHARE_UP_BOUND][%x = %x]\n\r", u32Page_Reg_Addr, *u32Page_Reg_Val);
	}
	else if(HAL_HDF_AddressCheck(u32Page_Reg_Addr) == _ADDR_CASE_ME_SHARE_LOW_BOUND){
		//u32Page_Reg_Val = (memc_dma_address_info->ME_start_addr + ME_TOTAL_SIZE);
		*u32Page_Reg_Val = (memc_dma_address_info->ME_start_addr);
		rtd_pr_memc_notice("[HAL_HDF_LimitCheck][_ADDR_CASE_ME_SHARE_LOW_BOUND][%x = %x]\n\r", u32Page_Reg_Addr, *u32Page_Reg_Val);
	}
		...
		*u32Page_Reg_Val = (memc_dma_address_info->ME_start_addr + ME_TOTAL_SIZE);
		rtd_pr_memc_notice("[HAL_HDF_LimitCheck][_ADDR_CASE_MV_INFO_UP_BOUND][%x = %x]\n\r", u32Page_Reg_Addr, *u32Page_Reg_Val);
	}
	else if(HAL_HDF_AddressCheck(u32Page_Reg_Addr) == _ADDR_CASE_MV_INFO_LOW_BOUND){
		//u32Page_Reg_Val = (memc_dma_address_info->ME_start_addr + ME_TOTAL_SIZE + ME_SHARE_TOTAL_SIZE);
		*u32Page_Reg_Val = (memc_dma_address_info->ME_start_addr);
		rtd_pr_memc_notice("[HAL_HDF_LimitCheck][_ADDR_CASE_MV_INFO_LOW_BOUND][%x = %x]\n\r", u32Page_Reg_Addr, *u32Page_Reg_Val);
	}
...
VOID HAL_HDF_DMA_Disable(unsigned int u32Page_Reg_Addr, unsigned int *u32Page_Reg_Val)
{
	...
	if( u32_address == MC_DMA_MC_LF_DMA_WR_Ctrl_reg		|| u32_address == MC_DMA_MC_HF_DMA_WR_Ctrl_reg    ||
		...
		u32_address == MC_DMA_MC_LFH_DMA_WR_Ctrl_reg	|| u32_address == MC_DMA_MC_LFH_I_DMA_RD_Ctrl_reg || u32_address == MC_DMA_MC_LFH_P_DMA_RD_Ctrl_reg ||
		...
		if( *u32Page_Reg_Val & 1 ) {
			*u32Page_Reg_Val = *u32Page_Reg_Val & 0b11111111111111111111111111111110;	// setting bit0 to 0
			rtd_pr_memc_notice("[MEMC][%s][%d] Disable the DMA %x\n\r", __FUNCTION__, __LINE__, u32_address);
		}
	}
	...
	if( u32_address == KME_DM_TOP0_KME_00_AGENT_reg		|| u32_address == KME_DM_TOP0_KME_01_AGENT_reg ||
		...
		if( *u32Page_Reg_Val >> 20 & 1 ) {
			*u32Page_Reg_Val = *u32Page_Reg_Val & 0b11111111111011111111111111111111;	// setting bit20 to 0
			rtd_pr_memc_notice("[MEMC][%s][%d] Disable the DMA %x\n\r", __FUNCTION__, __LINE__, u32_address );
		}
	}
...
VOID HAL_HDF_Load(VOID)
{
	...
	for(u32Page_Idx = 0; u32Page_Idx < u32PageNum; u32Page_Idx++)
	{
		...
		rtd_pr_memc_emerg("[%s][%d][%x,][i,%x,][,%d,%x,%d,%x,]\n", __FUNCTION__, __LINE__, u32PageNum, u32Page_Idx,
			u32Page_Offset, u32Page_Addr, u32PageSize, u32Page_Reg_Addr);

		if(u32Page_Idx == u32PageNum - 1)
		{
			rtd_pr_memc_emerg("Load hdf done... Total %d pages\r\n", u32PageNum);
		}
	}
#else
	unsigned int u32Page_Idx = 0;
	...
	for(u32Page_Idx = 0; u32Page_Idx < u32PageNum; u32Page_Idx++) {
		if( u32Page_Idx == _PAGE_ID_PQL0 || u32Page_Idx == _PAGE_ID_PQL1 || u32Page_Idx == _PAGE_ID_Software1 ) {
			continue;
		}
...
VOID HAL_HDF_Load_Page_u8HDF(unsigned int page)
{
	...
	unsigned int u32Page_Reg_Val = 0;
	unsigned int u32PageNum = (u8HDF_Array[7] << 24) + (u8HDF_Array[6] << 16) + (u8HDF_Array[5] << 8) + u8HDF_Array[4];
	unsigned long u32Page_Offset = FILE_HEADER_SIZE;
	...
	{
		if(page>0x4d){
		rtd_pr_memc_emerg("[%s][%d][%x,%x,][i,%d,][,%d,%x,%d,%x,]\n", __FUNCTION__, __LINE__, page, u32PageNum, i,
			u32Page_Offset, u32Page_Addr, u32PageSize, u32Page_Reg_Addr);
		}
		// check the page id
		if( page != u8HDF_Array[u32Page_Offset + PAGE_ID_OFFSET ] ) {
		//if( u8HDF_Array[u32Page_Offset + PAGE_ID_OFFSET ] < page ) {
			u32Page_Offset = u32Page_Offset + PAGE_HEADER_SIZE + u32PageSize;
			continue;
		}
		u32Page_Addr = (u8HDF_Array[u32Page_Offset + PAGE_ADDRESS_OFFSET + 3] << 24) +
		...
		rtd_pr_memc_emerg("[%s][%d][%x,%x,][,%d,%x,%d,%x,]\n", __FUNCTION__, __LINE__, page, u32PageNum, 
			u32Page_Offset, u32Page_Addr, u32PageSize, u32Page_Reg_Addr);
		break;
	}
#endif
