...
VOID _PQLSERVERDLL PQL_Info(const unsigned char InfoID,const _PQL_Info*pInfo)
{
	if(pInfo)
	{
		...
		case PQL_INFO_MIRACAST:
			...
			break;
		case PQL_INFO_FORBES:
			PQL_INFO_Forbes(&(pInfo->u1_Forbes_flg));
			break;
		case PQL_INFO_STEP_HIST_SUM_RATIO:
			PQL_INFO_DTL_Step_Hist_sum_ratio(&(pInfo->DTL_Step_Hist_sum_ratio[0]));
			break;	
		case PQL_INFO_SOURCE_TYPE:
			PQL_INFO_SourceType(&(pInfo->u16_source_type));
			break;
		case PQL_INFO_VIP_PROJECT_ID:
			PQL_INFO_VIP_project_ID(&(pInfo->u8_VIP_Project_ID));
			break;		
		case PQL_INFO_TRIBALL_Patch:
			PQL_INFO_triballPatch(&(pInfo->u8_mTriBall_holdfrm));				
			break;		
		default:	
			break;
		}
