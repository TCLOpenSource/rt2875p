...
VOID FRC_PhaseTable_Init(_OUTPUT_FRC_PH_TABLE *pOutput)
{
	...
	rtd_pr_memc_notice("FRC_PhaseTable_Init Called!!\r\n");
...
///////////////////////////end
	...
	GMV_avg = (GMV_avg>>5);
#endif
	//rtd_pr_memc_notice("(%d)gmv_record = %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d..(%d,%d,%d)\n",
...
//////////////
	...
	//static unsigned int u32_DBM_en_flag = 0;
	...
	#if 0
	WriteRegister(SOFTWARE1_SOFTWARE1_22_reg, 31, 31, 0x0);	// YE Test for DejudderByMotion switch
	WriteRegister(SOFTWARE1_SOFTWARE1_22_reg, 0, 0, 0x0);		// YE Test for test_bit0
	WriteRegister(SOFTWARE1_SOFTWARE1_22_reg, 1, 1, 0x0);		// YE Test for test_bit1
	WriteRegister(SOFTWARE1_SOFTWARE1_22_reg, 2, 2, 0x0);		// YE Test for test_bit2
	WriteRegister(SOFTWARE1_SOFTWARE1_22_reg, 3, 3, 0x0);		// YE Test for test_bit3
	WriteRegister(SOFTWARE1_SOFTWARE1_22_reg, 4, 4, 0x0);		// YE Test for test_bit4


	WriteRegister(SOFTWARE1_SOFTWARE1_45_reg, 24, 31, 0x0);	// YE Test for dejudder_0
	WriteRegister(SOFTWARE1_SOFTWARE1_46_reg, 0, 7, 0x0);		// YE Test for dejudder_1
	WriteRegister(SOFTWARE1_SOFTWARE1_46_reg, 8, 15, 0x0);	// YE Test for dejudder_2
	WriteRegister(SOFTWARE1_SOFTWARE1_46_reg, 16, 23, 0x0);	// YE Test for dejudder_3
	WriteRegister(SOFTWARE1_SOFTWARE1_46_reg, 24, 31, 0x0);	// YE Test for dejudder_4

	WriteRegister(SOFTWARE1_SOFTWARE1_58_reg, 0, 15,  0x0);	// YE Test for sad_0
	WriteRegister(SOFTWARE1_SOFTWARE1_58_reg, 16, 31,0x0);	// YE Test for Sad_1
	WriteRegister(SOFTWARE1_SOFTWARE1_59_reg, 0, 15,  0x0);	// YE Test for Sad_2
	WriteRegister(SOFTWARE1_SOFTWARE1_59_reg, 16, 31,0x0);	// YE Test for Sad_3
	WriteRegister(SOFTWARE1_SOFTWARE1_60_reg, 0, 15,  0x0);	// YE Test for Sad_4
	#endif
...
VOID FRC_phTable_set_LineModeCtrl(VOID)
{
	...
	if(log_en){
		rtd_pr_memc_notice("[%s][u1_isLowDelayMode,%d,][u1_adaptivestream_flag,%d,][MEMC_Lib_get_DisplayMode_Src,%d,][Vpq_LowDelay_to_MEMC,%d,]\n", 
		__FUNCTION__, u1_isLowDelayMode, u1_adaptivestream_flag, MEMC_Lib_get_DisplayMode_Src(), Scaler_MEMC_Get_VpqLowDelayToMEMC_Flag());
	}
...
VOID FRC_phTable_StateMachine(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
	...
	unsigned char u1_LowDelay_inframe_chg = (in_fmRate == pOutput->u1_lowdelay_inframe) ? 0 : 1;
	...
	// synthesize to generate change signal, and merged into phase table control signal
	{
		...
		unsigned char u1_direct_media_flag_chg = 0;
...
//		u32_low_delay = (u32_low_delay>>4) & 0x00000ff;
		...
		if((u1_outMode_chg == 1) || (u1_n2m_switch == 1) || (u1_in3dFormat_chg == 1) || (u1_out3dFormat_chg == 1)
			...
			|| (N2M_memcdtg_dv_total_check==1 && u1_n2m_switch==0)
			...
			if(u32_log_flag==1)
			{
				rtd_pr_memc_emerg("[%s][u1_generate_video_en](%d)(%d)(%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)\n\r", __FUNCTION__,
					u8_LowDelay_cnt, u1_outMode_chg, u1_n2m_switch, u1_in3dFormat_chg, u1_out3dFormat_chg, pOutput->u8_system_start_lock,
					u1_LowDelayMode_chg, DC_off_on_flg, u1_LowDelay_inframe_chg, u1_source_type_chg, VR360_en_chg, u1_isLowDelayMode,
	...
	//*******************************//
		...
		if(pOutput->u1_generate_video_en == 1){
			rtd_pr_memc_emerg("[u1_generate_video_en](%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d)\n\r",
	...
	if(MEMC_Lib_UltraLowDelayMode_Judge() == TRUE){
	...
	//Q-16730
	if(log_en){
		rtd_printk(KERN_NOTICE, TAG_NAME_MEMC, "[MEMC][%s][,%d,%d,]\n", __FUNCTION__, u8_MEMCMode, MEMC_LibGetMEMCMode());
	}
...
VOID FRC_phTable_Generate(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
	...
	if(pOutput->u1_generate_video_en == 1 || pOutput->u1_generate_film_en == 1)
	{
		...
		rtd_printk(KERN_NOTICE, TAG_NAME_MEMC, "memcdtgVTotal = [,%d,]\r\n", scalerdisplay_get_DTG_MASTER_vtotal_by_vfreq());
