...
VOID  patch_BluceMic_detect(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	...
	if((s_pContext->_output_rimctrl.u12_Rim_Result[_RIM_TOP] == 0) &&
		(s_pContext->_output_rimctrl.u12_Rim_Result[_RIM_BOT] == (s_pRimParam->s16_resolution_height-1)) &&
		(s_pContext->_output_rimctrl.u12_Rim_Result[_RIM_LFT] == 0) &&
		(s_pContext->_output_rimctrl.u12_Rim_Result[_RIM_RHT] == (s_pRimParam->s16_resolution_width-1)) &&	// no rim
...
VOID  patch_Soccer_action(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	...
	if(pParam->u1_sportscene_en && s_pContext->_output_me_sceneAnalysis.u1_SportScene){
		...
		WriteRegister(IPPRE_IPPRE_04_reg, 16, 23, 0x20); //16

		WriteRegister(MC_MC_B4_reg, 16, 23, 0xE0);
		WriteRegister(MC_MC_B4_reg, 24, 31, 0xF0);
	}
...
VOID  patch_tri_ball_Issue_action(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	...
	mc_mc_b4_reg.mc_bld_mvd_y_max = mvd_y_max;
	mc_mc_b4_reg.mc_bld_mvd_y_min = mvd_y_min;
...
//			rtd_setbits(0xb809d410, _BIT13);
		...
		if(freeze_cnt == 2){
			Scaler_MEMC_Set_MEMC_FreezeStatus_Flag(0);
			rtd_pr_memc_notice("$$$   [Scaler_MEMC_Set_MEMC_FreezeStatus_Flag][%d]    $$$\n\r",freeze_cnt);
		}
