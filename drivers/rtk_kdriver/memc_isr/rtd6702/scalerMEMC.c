...
void Scaler_MEMC_DMA_DB_Check(void)
{
	...
	rtd_pr_memc_emerg("[MEMC][DMA_DB_Check_2]=[,%x,%x,%x,%x,%x]\n",rtd_inl(MC_DMA_MC_WDMA_DB_CTRL_reg),rtd_inl(MC_DMA_MC_RDMA_DB_CTRL_reg),rtd_inl(PQC_PQDC_MC_PQ_DECMP_DB_CTRL_reg),rtd_inl(PQC_PQDC_MC_PQ_CMP_DB_CTRL_reg),rtd_inl(LBMC_LBMC_EC_reg));
...
void Scaler_MEMC_Set_MC_MEMBoundary(unsigned int  nDownlimit,unsigned int nUplimit)
{
	...
	rtd_pr_memc_emerg("[%s][up,%x,down,%x,]\n\r", __FUNCTION__, nUplimit, nDownlimit);
	rtd_pr_memc_emerg("[%s][LFH_W,%x,LFH_R_I,%x,LFH_R_I,%x,]\n\r", __FUNCTION__, rtd_inl(MC_DMA_MC_LFH_DMA_WR_Ctrl_reg), rtd_inl(MC_DMA_MC_LFH_I_DMA_RD_Ctrl_reg), rtd_inl(MC_DMA_MC_LFH_P_DMA_RD_Ctrl_reg));
}
...
#if 0
unsigned char MEMC_Lib_case_check(VOID)
{

	UINT32 MEMC_case_sel = 0;// 4k60

	if(Scaler_Get_MEMC_SR_path() == SLR_DISPD_MEMC_SR2){
		MEMC_case_sel = 2;// 4k60
	}else if((Get_DISPLAY_PANEL_TYPE() == P_VBY1_8K4K) && (Get_DISPLAY_REFRESH_RATE() == 120)) {
		MEMC_case_sel = 3;// 8k120
	}else if(((Get_DISPLAY_PANEL_TYPE() == P_VBY1_8K4K) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_8K4K)) && (Get_DISPLAY_REFRESH_RATE() == 60)) {
		MEMC_case_sel = 1;// 8k60
	}else{
		MEMC_case_sel = MEMC_case;
	}

	RTD_LOG_DEBUG("[jerry_test][MEMC_Lib_case_check]=%d,%d,%d\n\r",Scaler_Get_MEMC_SR_path(),Get_DISPLAY_PANEL_TYPE(),Get_DISPLAY_REFRESH_RATE());
	RTD_LOG_DEBUG("[%s][%d][%d]\n\r",__FUNCTION__, __LINE__, MEMC_case_sel);

	MEMC_Lib_CASE_SET(MEMC_case_sel);

	return MEMC_case_sel;
...
//	MEMC_Lib_LoadBringUpScript_Enable_DMA();
	...
	if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_8K4K){
		Scaler_MEMC_Set_ME_MEMBoundary(ME_start_addr, (ME_start_addr+KME_TOTAL_SIZE_8K));
		#if CONFIG_MC_8_BUFFER
		if(Get_DISPLAY_REFRESH_RATE() <= 60){
			Scaler_MEMC_Set_MC_MEMBoundary(MC00_start_addr,(MC00_start_addr + KMC_TOTAL_SIZE_8buf_8bit_8K));	
		}else{
			Scaler_MEMC_Set_MC_MEMBoundary(MC00_start_addr,(MC00_start_addr + KMC_TOTAL_SIZE_8buf_10bit_8K));	
		}
		#else
		Scaler_MEMC_Set_MC_MEMBoundary(MC00_start_addr,(MC00_start_addr + KMC_TOTAL_SIZE_6buf_10bit_8K));	
		#endif
	}else{
...
unsigned char Scaler_MEMC_Dejudder_Level_Mapping(unsigned char inLevel)
{
	...
	rtd_pr_memc_notice("[MEMC][%s] inLevel = %d, outLevel = %d\n", __FUNCTION__, inLevel, outLevel);
	return outLevel;
#else
	unsigned char outLevel = 0;
...
void Scaler_MEMC_SetMotionComp(unsigned char blurLevel, unsigned char judderLevel, SCALER_MEMC_TYPE_T motion){
	...
	if(motion == MEMC_TYPE_OFF){
		...
		//rtd_printk(KERN_NOTICE, TAG_NAME_MEMC, "[MEMC] judderLevel=%d, blurLevel=%d\n", judderLevel, blurLevel);
...
//	rtd_pr_memc_notice("$$$   scalerMEMC_LowDelay_(%d)   $$$\n\r", type);
	...
	//rtd_printk(KERN_NOTICE, TAG_NAME_MEMC, "[MEMC]vdec_direct = %d\n", drv_memory_get_vdec_direct_low_latency_mode());
	...
	if(type == 1){
		...
		//	MEMC_LibSetMEMCFrameRepeatEnable(1, 0);
		//}
		rtd_printk(KERN_NOTICE, TAG_NAME_MEMC, "[MEMC][%s][CC][%d,%d]\n",__FUNCTION__, pre_type,type);
		if(type != pre_type){
			rtd_printk(KERN_NOTICE, TAG_NAME_MEMC, "[MEMC][%s][DD][%d,%d,%d]\n",__FUNCTION__, pre_type,last_mode,MEMC_LibGetMEMCMode());
...
unsigned int Scaler_MEMC_GetAVSyncDelay(void)
{
	...
	unsigned int repeat_mode = 0, pz_sel = 0;
