...
BOOL Mid_MEMC_SetPullDownMode(INPUT_FRAMERATE enInputFrameRate, OUTPUT_FRAMERATE enOutputFrameRate, unsigned char bTruMotionMode, unsigned char enRealCinema)
{
	...
	else if(enInputFrameRate == INPUT_FRAMERATE_60HZ && (enOutputFrameRate == OUTPUT_FRAMERATE_60HZ)\
		&& bTruMotionMode == TRUE && bEnRealCinema == TRUE)
	{
		mPullDownMode = _PQL_IN60HZOUT60HZ_TRUMOTON_CINEMAON;
	}
	#endif
