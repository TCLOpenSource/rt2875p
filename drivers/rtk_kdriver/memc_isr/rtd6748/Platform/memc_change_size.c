...
unsigned int GS_image_pre_htotal=0;
// Global paramter for internal calculation
unsigned int GSC_me_end_addr = 0;
unsigned int GSC_mc_end_addr = 0;
unsigned int GS_MCBuffer_Size = 6;
...
void MEMC_ChangeSize_init(const unsigned char MEMC_SZ_SEL)
{
	...
	GS_ddr_128n_align = 0;
	GS_image_pre_htotal = gMEMC_SizeParamTable[MEMC_SZ_SEL].ImageHTotal;
	rtd_pr_memc_notice("[%s][%d]GS_image_pre_htotal=%d\n\r", __FUNCTION__, __LINE__,GS_image_pre_htotal);
...
void MEMC_ChangeSize_main(const unsigned char MEMC_SZ_SEL, unsigned int ME_base_addr, MEMC_INOUT_MODE emom, MEMC_OUTPUT_FRAMERATE Out_FrameRate,unsigned int MEMC_LoadSizeParam_Type)
{
	...
	if((MEMC_LoadSizeParam_Type == MEMC_LoadSizeParam_ByScaler) && (Scaler_MEMC_Get_DISP_DHTOTAL() != 0)){
		...
		if(Get_DISP_HORIZONTAL_TOTAL()!=0){
			GS_image_htotal=Get_DISP_HORIZONTAL_TOTAL();
		}
	}
	GS_image_pre_htotal=GS_image_htotal;
	rtd_pr_memc_notice("[%s][%d]GS_image_pre_htotal=%d\n\r", __FUNCTION__, __LINE__,GS_image_pre_htotal);
	...
	rtd_pr_memc_notice("[%s][%d][%d,%d,%d][htotal,%d,]\n\r", __FUNCTION__, __LINE__, GS_MC_8_buffer, GS_MCBuffer_Size, GS_pqc_selection, Get_DISP_HORIZONTAL_TOTAL());
...
void MEMC_ChangeSize_LOGO(void)
{
	...
	if(kme_logo0_kme_logo0_00_reg.km_logo_v_fetch_mode == 1){
	...
	else if(kme_logo0_kme_logo0_00_reg.km_logo_v_fetch_mode == 2){
		...
		kme_logo0_kme_logo0_08_reg.km_logo_v0_ration = 21;
		kme_logo0_kme_logo0_08_reg.km_logo_v1_ration = 43;
		kme_logo0_kme_logo0_0c_reg.km_logo_v2_ration = 43;
		kme_logo0_kme_logo0_0c_reg.km_logo_v3_ration = 21;
	}
	else{ //4k only use logo_h0~h3, 8k use log h0~h7
...
void MEMC_ChangeSize_MC2(void)
{
	...
	mc2_mc2_a8_reg.mc_pix_logo_db_rim_bot = 540;
