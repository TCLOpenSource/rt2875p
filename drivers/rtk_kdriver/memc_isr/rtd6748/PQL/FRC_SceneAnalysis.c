...
VOID FRC_Good_Scene_Detect(const _PARAM_FRC_SceneAnalysis *pParam, _OUTPUT_FRC_SceneAnalysis *pOutput)
{
	...
	unsigned char u8_TopRim_th = 4*MEMC_PARAM_TABLE.RIM_Param[output_Resolution].RimCtrl_Param.u8_scale_V; // 4k: u8_TopRim_th=16
	unsigned char u8_LftRim_th = 6*MEMC_PARAM_TABLE.RIM_Param[output_Resolution].RimCtrl_Param.u8_scale_H; // 4k: u8_LftRim_th=24
...
VOID FRC_Dynamic_SADDiffTh(const _PARAM_FRC_SceneAnalysis *pParam, _OUTPUT_FRC_SceneAnalysis *pOutput)
{
	...
	unsigned int u32_RB_debug, dubug_info = 0;

	unsigned int u32_RB_me1_sc_status = 0;
	ReadRegister(SOFTWARE2_SOFTWARE2_00_reg, 3, 3, &u32_RB_debug);
	...
	if(pParam->u1_dyn_SAD_Diff_en)
	{
		...
		if(log_en){//d904[24]
			rtd_pr_memc_notice("[%s],%d,info,%d,%d,%d,%d,rmv_cnt_avg,%d,%d,%d,totalNum,%d,sad_diff,%d,%d,%d,sc=,%d,\n", __FUNCTION__, dubug_info, 
...
VOID MEMC_me1_bgmask_newalgo_Proc(const _PARAM_FRC_SceneAnalysis *pParam, _OUTPUT_FRC_SceneAnalysis *pOutput)
{
    ...
    int top3 ,bot3, left3 ,right3 ;
    int top4 ,bot4 , left4 ,right4 ;
    int condhalobefore_1,condhalobefore_2,condhalobefore_3;
    static int holdframe_halobefore,video_halobefore;

    //int left11,right11,top11,bot11,left21,right21,top21,bot21,left31,right31,top31,bot31,left41,right41,top41,bot41;
...
if(fg_apl_sum>0 && bg_apl_sum>0){
    ...
    //cond374_9 = bgcnt_array[12]<600 && bgcnt_array[14]<600 && bgcnt_array[15]<600 && gmv_bgcnt>23000 && gmv_bgx<20 && gmv_fgx<20 &&_ABS_(gmv_fgy)<2 && _ABS_(gmv_bgy)<2;
    ...
    //cond374_11 = bgcnt_array[11]==1020 && bgcnt_array[12]>700 && bgcnt_array[16]>900 && bgcnt_array[24]>900  && bgcnt_array[12]<1020 && bgcnt_array[16]<1020 && bgcnt_array[24]<1020;
    ...
    //cond374_sp = cond374_6 && cond374_8 && cond374_9 && bgcnt_array[16]==1020 && bgcnt_array[15]<400 && rgn_apli[0]<14000 && rgn_apli[0]>13000 && Dtl_array[15]>28000 && Dtl_array[15]<31000 && Dtl_array[24]>25000 && Dtl_array[24]<27000;
    ...
    //rtd_pr_memc_info("%d %d %d %d %d %d %d %d\n",rgn_aplcnt,rgn_apli[0],rgn_apli[1],bgcnt_array[5],bgcnt_array[6],bgcnt_array[7],bgcnt_array[14],bgcnt_array[15]);
    //rtd_pr_memc_info("%d %d %d %d\n", bgcnt_array[11],bgcnt_array[12],bgcnt_array[16],bgcnt_array[24]);
...
if(fg_apl_sum>0 && bg_apl_sum>0){    
	...
	if(Scaler_MEMC_GetPanelSizeByDisp() == MEMC_SIZE_2K)//if(Get_DISPLAY_PANEL_TYPE() == P_LVDS_2K1K)
	{
		ReadRegister(KME_DEHALO_KME_DEHALO_E0_reg,0,8, &left1 ); // Dehalo rim.
		ReadRegister(KME_DEHALO_KME_DEHALO_E0_reg,9,17, &right1 );
		ReadRegister(KME_DEHALO_KME_DEHALO_E4_reg,0,8, &top1 );
		ReadRegister(KME_DEHALO_KME_DEHALO_E4_reg,9,17, &bot1 );
	
		ReadRegister(KME_DEHALO_KME_DEHALO_18_reg,0,8, &left2 ); // Dehalo retimer rim.
		ReadRegister(KME_DEHALO_KME_DEHALO_18_reg,9,17, &right2 );
		ReadRegister(KME_DEHALO_KME_DEHALO_18_reg,18,26, &top2 );
		ReadRegister(KME_DEHALO_KME_DEHALO_1C_reg,0,8, &bot2 );
	
		ReadRegister(KME_DEHALO_KME_DEHALO_1C_reg,9,17, &left3 ); // Dehalo retimer1 rim.
		ReadRegister(KME_DEHALO_KME_DEHALO_1C_reg,18,26, &right3 );
		ReadRegister(KME_DEHALO_KME_DEHALO_20_reg,0,8, &top3 );
		ReadRegister(KME_DEHALO_KME_DEHALO_20_reg,9,17, &bot3 );
	
		ReadRegister(KME_DEHALO_KME_DEHALO_20_reg,18,26, &left4 ); // Dehalo retimer2 rim.
		ReadRegister(KME_DEHALO_KME_DEHALO_24_reg,0,8, &right4 );
		ReadRegister(KME_DEHALO_KME_DEHALO_24_reg,9,17, &top4 );
		ReadRegister(KME_DEHALO_KME_DEHALO_24_reg,18,26, &bot4 );
	
		WriteRegister(KME_DEHALO_KME_DEHALO_E0_reg,0,8, left1>>1 ); // Dehalo rim.
		if(right1<240)
        {
            WriteRegister(KME_DEHALO_KME_DEHALO_E0_reg,9,17, right1);
        }
        else
        {
            WriteRegister(KME_DEHALO_KME_DEHALO_E0_reg,9,17, right1>>1 );
        }
		WriteRegister(KME_DEHALO_KME_DEHALO_E4_reg,0,8, top1>>1 );
		WriteRegister(KME_DEHALO_KME_DEHALO_E4_reg,9,17, bot1>>1 );
	
		WriteRegister(KME_DEHALO_KME_DEHALO_18_reg,0,8, left2>>1 ); // Dehalo retimer rim.
		WriteRegister(KME_DEHALO_KME_DEHALO_18_reg,9,17, right2>>1 );
		WriteRegister(KME_DEHALO_KME_DEHALO_18_reg,18,26, top2>>1 );
		WriteRegister(KME_DEHALO_KME_DEHALO_1C_reg,0,8, bot2>>1 );

		WriteRegister(KME_DEHALO_KME_DEHALO_1C_reg,9,17, left3>>1 ); // Dehalo retimer1 rim.
		WriteRegister(KME_DEHALO_KME_DEHALO_1C_reg,18,26, right3>>1 );
		WriteRegister(KME_DEHALO_KME_DEHALO_20_reg,0,8, top3 >>1 );
		WriteRegister(KME_DEHALO_KME_DEHALO_20_reg,9,17, bot3>>1 );
	
	
		WriteRegister(KME_DEHALO_KME_DEHALO_20_reg,18,26, left4>>1 ); // Dehalo retimer2 rim.
		WriteRegister(KME_DEHALO_KME_DEHALO_24_reg,0,8, right4>>1 );
		WriteRegister(KME_DEHALO_KME_DEHALO_24_reg,9,17, top4>>1 );
		WriteRegister(KME_DEHALO_KME_DEHALO_24_reg,18,26, bot4>>1 );
	}
	...
	*/
    ...
    if(readval10)
    {
		...
		rtd_pr_memc_info("bg:%d %d fg:%d %d %d dtlsum:%d\n",bg_apl_seg7_cnt*100/bg_apl_sum,(bg_apl_seg6_cnt+bg_apl_seg7_cnt)*100/bg_apl_sum,fg_apl_seg0_cnt*100/fg_apl_sum,(fg_apl_seg0_cnt+fg_apl_seg1_cnt)*100/fg_apl_sum,(fg_apl_seg0_cnt+fg_apl_seg1_cnt+fg_apl_seg2_cnt)*100/fg_apl_sum,dtlsum);
	...
	if(readval12)
    {
        ...
        rtd_pr_memc_info("bgapl:%d %d\n",(bg_apl_seg5_cnt+bg_apl_seg6_cnt+bg_apl_seg7_cnt)*1000/(bg_apl_sum),(bg_apl_seg6_cnt+bg_apl_seg7_cnt)*1000/(bg_apl_sum));
	}
	if(readval13)
    {
    ...
    if(readval14)
    {
        ...
        rtd_pr_memc_info("-----------start----------\n");
        rtd_pr_memc_info("0-7bg:%d %d %d %d %d %d %d %d\n", bgcnt_array[0] ,bgcnt_array[1],bgcnt_array[2],bgcnt_array[3],bgcnt_array[4],bgcnt_array[5],bgcnt_array[6],bgcnt_array[7]);
        rtd_pr_memc_info("8-15bg:%d %d %d %d %d %d %d %d\n", bgcnt_array[8] ,bgcnt_array[9],bgcnt_array[10],bgcnt_array[11],bgcnt_array[12],bgcnt_array[13],bgcnt_array[14],bgcnt_array[15]);
        rtd_pr_memc_info("16-23bg:%d %d %d %d %d %d %d %d\n", bgcnt_array[16] ,bgcnt_array[17],bgcnt_array[18],bgcnt_array[19],bgcnt_array[20],bgcnt_array[21],bgcnt_array[22],bgcnt_array[23]);
        rtd_pr_memc_info("24-31bg:%d %d %d %d %d %d %d %d\n", bgcnt_array[24] ,bgcnt_array[25],bgcnt_array[26],bgcnt_array[27],bgcnt_array[28],bgcnt_array[29],bgcnt_array[30],bgcnt_array[31]);
...
printk("mv8:%d %d %d %d %d %d %d %d\n",rmvx[8],rmvx[9],rmvx[10],rmvx[11],rmvx[12],
             rmvx[13],rmvx[14],rmvx[15]);
    ...
    {                     
        ...
        //rtd_pr_memc_info("%d %d\n",cond26_1,cond26_2);
        printk("dtl1:%d %d %d %d\n",Dtl_array[0],Dtl_array[1],Dtl_array[6],Dtl_array[7]);
        printk("dtl2:%d %d %d %d\n",Dtl_array[8],Dtl_array[9],Dtl_array[14],Dtl_array[15]);
        printk("dtl3:%d %d %d %d\n",Dtl_array[16],Dtl_array[17],Dtl_array[22],Dtl_array[23]);
        printk("dtl4:%d %d %d %d\n",Dtl_array[24],Dtl_array[25],Dtl_array[30],Dtl_array[31]);
    ...
    if(readval14)
    {
        ...
        //rtd_pr_memc_info("bg:%d %d %d %d\n",bgcnt_array[20],bgcnt_array[21],bgcnt_array[22],bgcnt_array[23]);
        //rtd_pr_memc_info("bg:%d %d %d %d\n",bgcnt_array[28],bgcnt_array[29],bgcnt_array[30],bgcnt_array[31]);
        printk("apl1:%d %d %d %d\n",rgn_apli[0],rgn_apli[1],rgn_apli[6],rgn_apli[7]);
        printk("apl2:%d %d %d %d\n",rgn_apli[8],rgn_apli[9],rgn_apli[14],rgn_apli[15]);
        printk("apl3:%d %d %d %d\n",rgn_apli[16],rgn_apli[17],rgn_apli[22],rgn_apli[23]);
        printk("apl4:%d %d %d %d\n",rgn_apli[24],rgn_apli[25],rgn_apli[30],rgn_apli[31]);
    ...
    if(readval16)
    {
        ...
        rtd_pr_memc_info("bgcnt:5:%d 6:%d 7:%d 11:%d 12:%d 14:%d 15:%d 16:%d 24:%d\n",bgcnt_array[5],bgcnt_array[6],bgcnt_array[7],bgcnt_array[11],bgcnt_array[12],bgcnt_array[14],bgcnt_array[15],bgcnt_array[16],bgcnt_array[24]);
    }  
*/
}
VOID MEMC_dehalo_newalgo_bgmaskBase_Proc(const _PARAM_FRC_SceneAnalysis *pParam, _OUTPUT_FRC_SceneAnalysis *pOutput)
{
  ...
  //  dirctionCond =(( rgnmvy_fg[maxIdx]>>31) != ( rgnmvy_fg[minIdx]>>31) )||((( rgnmvy_fg[maxIdx]>>31) == ( rgnmvy_fg[minIdx]>>31) ); 
   ...
   if(_ABS_DIFF_(gmv_fgx,gmv_bgx) > 35) // fg bg mvx diff is big
    ...
    if(((ABS(gmv_fgy,gmv_bgy)-ABS(gmv_fgx,gmv_bgx))>0) || (((ABS(gmv_fgx,gmv_bgx)-ABS(gmv_fgy,gmv_bgy))< 8) && (ABS(gmv_fgy,gmv_bgy) > 15)))  //ori
...
if(fg_apl_sum>0 && bg_apl_sum>0){
	...
	if(u8_dh_condition_dhbypass_en)
    {
        if(((avg_bg_diff_cond == 1)||(glb_bgcnt_large==1) || (dark_flash_big_fg == 1) || (bad_light_off == 1) || (tiny_bgcnt_flag == 1) || (smallcnt_y_big_flag == 1) || 
            ...
            ||(pOutput->u8_dh_condition_26_bypass == 1)||(pOutput->u8_dh_condition_1841_bypass == 1)||(pOutput->u8_dh_condition_207_bypass == 1)||(pOutput->u8_dh_condition_333_bypass == 1)||(pOutput->u8_me1_condition_195_face_broken==1)||(pOutput->u8_dh_condition_car_bypass==1)||(pOutput->u8_me1_condition_apl_suddenchange_2k_flag == 1)||(pOutput->u8_me1_condition_rotatebroken_2k_flag == 1)
            ||(pOutput->u8_dh_condition_412_bypass == 1)||(pOutput->u8_dh_condition_35_bypass == 1)||(pOutput->u8_dh_condition_215_bypass==1)||(pOutput->u8_dh_condition_2271_bypass==1)||(pOutput->u8_dh_condition_410_scgarbage == 1)||(pOutput->u8_dh_condition_smallbgcnt_ver_bypass == 1)||( pOutput->u8_me1_condition_bgmidstable_fgmotion_2k_flag ==1)||( pOutput->u8_me1_condition_random_bgmid_2k_flag ==1)
            ||(pOutput->u8_dh_condition_393_scgarbage == 1)||(pOutput->u8_dh_condition_182_bypass == 1)||(pOutput->u8_dh_condition_343_bypass == 1)||(pOutput->u8_dh_condition_416_bypass == 1)||(	pOutput->u8_dh_condition_lager_dtlcomparison_bypass == 1)||(pOutput->u8_dh_condition_smallbgcnt_slant_bypass == 1)) && (pOutput->u8_me1_condition_halobefore_flag ==0)&&
            (fwcontrol_13 == 0) && (fwcontrol_214 == 0) && (fwcontrol_185 == 0) && (fwcontrol_192 == 0) && (fwcontrol_299 == 0) && (fwcontrol_189 == 0) && (pOutput->u8_dh_condition_229 == 0) && (fwcontrol_156 == 0) && (fwcontrol_306 == 0) && (fwcontrol_227 == 0)&&(fwcontrol_153 == 0)&&(fwcontrol_298 == 0 )&&(pOutput->u8_dh_condition_214==0)&&(pOutput->u8_me1_condition_fixgmv_2k_flag == 0)&&(pOutput->u8_dh_condition_insect_status!= 1)&&(pOutput->u8_dh_condition_transporter_status != 1)
             &&(pOutput->u8_dh_condition_eagle_status!=1)&&(pOutput->u8_dh_condition_huapi2_status!=1)&&(pOutput->u8_dh_condition_dance_status!=1)&&(pOutput->u8_dh_condition_rotterdam_status!=1)&&(pOutput->u8_dh_condition_dance_2_status!=1)&&(pOutput->u8_dh_condition_skating_status!=1)&&(pOutput->u8_dh_conditio_0026_1_status!=1)&&(dh_bypass_forrotterdam_33s==0)&&(pOutput->u8_dh_condition_IDT_flag == 0)&&(flag_avenger==0)&&(flag_IDT==0)&&(flag_filmj==0))
        {
	...
	if(readval16)
    {
		...
		rtd_pr_memc_info("on:%d %d %d %d :%d %d %d %d :%d %d %d %d %d\n", fwcontrol_13 ,fwcontrol_214,fwcontrol_185,fwcontrol_299 ,fwcontrol_192 ,fwcontrol_189,pOutput->u8_dh_condition_229,fwcontrol_156 ,fwcontrol_306,fwcontrol_227 ,fwcontrol_153,fwcontrol_298,pOutput->u8_dh_condition_214);
	}  
...
VOID MEMC_dehalo_newalgo_bgmaskBase_Proc_DHBYPASS(const _PARAM_FRC_SceneAnalysis *pParam, _OUTPUT_FRC_SceneAnalysis *pOutput)
{
    ...
    int sum_apl=0,aplsum32rgn=0;
    int bgcnt_sum1,bgcnt_sum2,apl_sum1,apl_sum2,apl_sum2_avr;
    int halfapl_array_length =8;
    static  int halfapl_array[8]={0};
    static  int halfapl_array_left[8]={0};

    static  int halfbg_array[8]={0};
    ...
    int apl_change_cnt_left = 0,apl_change_jumpcnt_left = 0,apl_sum1_avr = 0;
    int flag1,flag2 ;
    ...
    static int aplsum32rgn_pre = 0,middtl_sum_pre = 0;
    int middtl_sum = 0;
    ...
    updateGMVArray(halfapl_array,halfapl_array_length,apl_sum2_avr); //right
    for( m=0;m<7;m++)
    {
        if(_ABS_DIFF_(halfapl_array[m+1],halfapl_array[m])>250)//2500 //300
            apl_change_cnt++;
        if((halfapl_array[m+1]-halfapl_array[m])>0)
            apl_change_jumpcnt++;
    }

    updateGMVArray(halfapl_array_left,halfapl_array_length,apl_sum1_avr); //left
    for( m=0;m<7;m++)
    {
        if(_ABS_DIFF_(halfapl_array_left[m+1],halfapl_array_left[m])>300)//2500
            apl_change_cnt_left++;
        if((halfapl_array_left[m+1]-halfapl_array_left[m])>0)
            apl_change_jumpcnt_left++;
    }

    updateGMVArray(halfbg_array,halfapl_array_length,bgcnt_sum2);
    for( m=0;m<7;m++)
    {
        if(_ABS_DIFF_(halfbg_array[m+1],halfbg_array[m])>3000)
            bg_change_cnt++;
    }
  ...
  // if((!SCFlag_holdflag) && _ABS_(u11_gmv_mvx)<=50  && _ABS_(u10_gmv_mvy)<10 && _ABS_(gmv_bgx)<=50 && _ABS_(gmv_bgy)<10 && apl_change_cnt>=2 && apl_change_jumpcnt>=1 && apl_change_jumpcnt<=7 &&apl_sum2_avr<15000 && (fg_apl_seg6_cnt+fg_apl_seg7_cnt*100)/fg_apl_sum<15)
   if((!SCFlag_holdflag) && _ABS_(u11_gmv_mvx)<=50  && _ABS_(u10_gmv_mvy)<10 && _ABS_(gmv_bgx)<=50 && _ABS_(gmv_bgy)<10 && apl_change_cnt>=2 && apl_change_jumpcnt>=1 && apl_change_jumpcnt<=7 &&apl_sum2_avr<25000 && (!cond_whthor_flag)&&(!blkhor_flag))
   {
    ...
    //printk("mv:%d %d %d %d cnt:%d %d %d %d \n",u11_gmv_mvx,u10_gmv_mvy,gmv_bgx,gmv_bgy,apl_change_cnt,apl_change_jumpcnt,apl_sum2_avr, (fg_apl_seg6_cnt+fg_apl_seg7_cnt*100)/fg_apl_sum);

    
    
   if( _ABS_(u11_gmv_mvx)<=5  && _ABS_(u10_gmv_mvy)<5 && _ABS_(gmv_bgx)<=5 && _ABS_(gmv_bgy)<5 && apl_change_cnt_left>=2 && (apl_change_jumpcnt_left>=1 && apl_change_jumpcnt_left<=7) &&apl_sum1_avr<6000)
   {
        video_light_left = 1;
        light_left = 1;
    }
    else
    {
     ...
     flag1 = apl_change_cnt>=2 && (apl_change_jumpcnt>=1 && apl_change_jumpcnt<=7);
     flag2 = apl_change_cnt_left>=2 && (apl_change_jumpcnt_left>=1 && apl_change_jumpcnt_left<=7);
   //printk("right:%d %d %d left:%d %d %d\n",apl_change_cnt,apl_change_jumpcnt,apl_sum2_avr,apl_change_cnt_left,apl_change_jumpcnt_left,apl_sum1_avr);
    //printk("%d %d %d\n",flag1,flag2,apl_sum1_avr);
    ...
    if(flag_light_cond)
   {
    ...
    {
        //WriteRegister(KPOST_TOP_KPOST_TOP_A0_reg, 20, 23, 0x9);
        flag_sencehange =1;
    ...
    {
        //WriteRegister(KPOST_TOP_KPOST_TOP_A0_reg, 20, 23, 0x4);
        flag_tinydtl =1;
    }
    else
    {
...
if(0){
    if((lightflag == 1) ||(lightflag_cond == 1) ||(rotateflag == 1)||(fgmvmutationflag == 1)||(light_left == 1) ||(flag_sencehange == 1)||(flag_tinydtl == 1)||(aplsumchange == 1))
 	{
        ...
        else if(aplsumchange == 1)
        {
            if( s_pContext->_external_data._output_frameRate == _PQL_OUT_120HZ)
            {
                pOutput->u8_dh_condition_bypass_check_add = (pOutput->u8_dh_condition_dhbypass_hold_cnt_add > 0) ? pOutput->u8_dh_condition_bypass_check_add : 7;
                pOutput->u8_dh_condition_dhbypass_hold_cnt_add = (pOutput->u8_dh_condition_dhbypass_hold_cnt_add > 0) ? pOutput->u8_dh_condition_dhbypass_hold_cnt_add : 0;
            }
            else
            {
...
//	FRC_RgPanning_Detect(pParam, 	pOutput);
	...
	MEMC_dehalo_newalgo_bgmaskBase_Proc_DHBYPASS(pParam, pOutput);
...
void FRC_is_not_natural_video(const _PARAM_FRC_SceneAnalysis *pParam, _OUTPUT_FRC_SceneAnalysis *pOutput)
{
	...
	ReadRegister(0xb809d9b4,16, 16,&logtest);
	if(logtest)
	...
	&& 4400000>=aplp && aplp>=4300000*/ && 15>=gmvx && gmvx>=10 && gmvy==0){
			...
			WriteRegister(0xb809d5e8,16,16,1);//force panning 1
			WriteRegister(0xb809ebd8,22,22,1);
			WriteRegister(0xb809cd10,0,0,0);
		...
		else if(nIdentifiedNum_preProcess_a[1] == 1 )
		{
			...
			WriteRegister(0xb809d5e8,16,16,0);//force panning 1
			WriteRegister(0xb809ebd8,22,22,0);
	...
	/*
	//vertrez24p
	if(600>=score && score>=550 && 2100000>=dtl && dtl>= 1900000 && ((6600000>=apli && apli>=6000000) || (4400000>=apli && apli>=4200000)) && gmvy==0)
	{
		s_pContext->_output_wrt_comreg.u1_is_test_video=1;
		WriteRegister(SOFTWARE1_SOFTWARE1_62_reg, 2, 2, 1);
		WriteRegister(SOFTWARE2_SOFTWARE2_01_reg, 0, 7, 0xf0);
		u5_blend_holdfrm_preProcess_a[2] = 30;
	...
	else if(u5_blend_holdfrm_preProcess_a[2] >0)
	{
		...
		WriteRegister(SOFTWARE1_SOFTWARE1_62_reg, 2, 2, 1);
		WriteRegister(SOFTWARE2_SOFTWARE2_01_reg, 0, 7, 0xf0);
		rtd_pr_memc_emerg("vertrez24p");
		u5_blend_holdfrm_preProcess_a[2]--;
	}
	else if(nIdentifiedNum_preProcess_a[2] == 2 )
	{
		s_pContext->_output_wrt_comreg.u1_is_test_video=0;
		WriteRegister(SOFTWARE1_SOFTWARE1_62_reg, 2, 2, 0);
		WriteRegister(SOFTWARE2_SOFTWARE2_01_reg, 0, 7, 0x00);
		nIdentifiedNum_preProcess_a[2] = 0;
	}
	//movZonePlateCenter
	if(880>=score && score>=850 && 3650000>=dtl && dtl>= 3600000 && 4200000>=apli && apli>=4000000
&& 4200000>=aplp && aplp>=4000000 && 30>=gmvx && gmvx>=0 && gmvy==0){
		WriteRegister(SOFTWARE1_SOFTWARE1_62_reg, 2, 2, 1);
		WriteRegister(SOFTWARE2_SOFTWARE2_01_reg, 0, 7, 0xff);
	...
	else if(u5_blend_holdfrm_preProcess_a[3]>0)
	{
		WriteRegister(SOFTWARE1_SOFTWARE1_62_reg, 2, 2, 1);
		WriteRegister(SOFTWARE2_SOFTWARE2_01_reg, 0, 7, 0xff);
		rtd_pr_memc_emerg("movZonePlateCenter");
