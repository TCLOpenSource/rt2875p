...
VOID ReadComReg_Init(_OUTPUT_ReadComReg *pOutput)
{
	...
	rtd_pr_memc_info("[%s] %d, %d, %d, %d, %d, %d, %d\n", __FUNCTION__, g_ME1_VBUF_GAIN, g_ME2_VBUF_GAIN, g_PLOGO_VBUF_GAIN, g_BLOGO_VBUF_GAIN, g_ME2_PFV_GAIN, g_HEIGHT_GAIN, g_WIDTH_GAIN);
}
...
VOID ReadComReg_Proc_inputInterrupt_Hsync(const _PARAM_ReadComReg *pParam, _OUTPUT_ReadComReg *pOutput)
{
	...
	if(log_en){
		rtd_pr_memc_info("[phasetable][%s][ME1_idx,%d,%d,%d,ME2_idx,%d,%d,][MC_idx,%d,%d,%d,][phase,%d,%d,][Time,%d,]\n\r", __FUNCTION__,
		...
		for(u8_i = 0; u8_i < _CAD_CHANGE_QUE_BUF_FRM; u8_i++){
			rtd_pr_memc_info("[%s][g_cadence_change_que][%d][u8_i,%d,][mc_wrt_index,%d,][state,%d,][type,%d,]\n\r",
...
VOID ReadComReg_Proc_outputInterrupt(const _PARAM_ReadComReg *pParam, _OUTPUT_ReadComReg *pOutput)
{
	...
	if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_3K1K){
		u8_TopRim_th = 17;
