...
VOID Dh_closeSignal_Detect(const _PARAM_DH_CLOSE *pParam, _OUTPUT_DH_CLOSE *pOutput)
{
	...
	unsigned int  special_pattern_421_flag = 0;
	...
	if(pParam->u1_fblvl_en == 1)
	{
		...
		if((pOutput->u1_fblvl_1frm_close == 1)&&(!special_pattern_transporter_flag))
		{
			u1_fblvl_dhClose          = 1;
	...
	if (pParam->u1_lbmc_switch_en == 1)
	{
		...
		if((((pOutput->u8_lbmc_mode == _MC_NORMAL_) && (s_pContext->_output_mc_lbmcswitch.u8_lf_lbmcMode == _MC_SINGLE_UP_)) ||
		   ((pOutput->u8_lbmc_mode == _MC_NORMAL_) && (s_pContext->_output_mc_lbmcswitch.u8_lf_lbmcMode == _MC_SINGLE_DOWN_)))&& (!special_pattern_eagle_flag)&&(!special_pattern_transporter_flag)&&(!dhbypass_rotterdam_30s)&&(!special_pattern_huapi2_flag))
		{
	...
	if(printlog_en){
		if(pOutput->u1_closeDh_sig == 1){//if(pOutput->u1_closeDh_sig == 1)		
			...
			*/
            ...
            if(pOutput->u1_fblvl_close)
                rtd_pr_memc_emerg("[johning_dai]u1_fblvl_close:%d\n",pOutput->u1_fblvl_close);
            if(pOutput->u1_meAllDtl_close)
