...
VOID  patch_Soccer_action(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	...
	if(pParam->u1_sportscene_en && s_pContext->_output_me_sceneAnalysis.u1_SportScene){
		...
		WriteRegister(IPPRE_IPPRE_04_reg, 16, 23, 110); //16
...
VOID  patch_tri_ball_Issue_action(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	...
	if(pParam->u1_tri_ball_Issue_en == 0)//d5dc[15]
		return;
...
//	unsigned int test_val, test_val2, test_val3, test_val4;
	...
	if(memc_switch_en == 1 && (g_memc_switch_state == 1)){
		if(Scaler_MEMC_Get_BypassToEnable_Flag() == 1){
		...
		else if(memc_switch_frm_cnt == (frm_num+checkcnt-1)){//else if(memc_switch_frm_cnt == frm_num-1){[ML7CETOP-707]
			if(VR360_en == 1){
#if IC_K5LP // merlin4_blend_mode
				WriteRegister(MC_MC_B0_reg, 0, 2, 0x1);
#elif (IC_K6LP || IC_K8LP)
			...
			if(memc_switch_frm_cnt>frm_num-2){
				rtd_pr_memc_notice("[ML7CETOP-707]MEMC repeat recover checking for ME clk status(%d)",memc_switch_frm_cnt);
			}
		...
		if(DTV_PowerSaving_stage == 1){
			//set Repeat Mode
			Mid_Mode_SetMEMCFrameRepeatEnable(1);
			rtd_pr_memc_notice("[PowerSaving_MEMC_bypass_switch][ME_CLK_off][BB][,%x,%x,%x,%x,%x,%x,%x,%x,]\n", rtd_inl(MC_DMA_MC_RDMA_DB_CTRL_reg), rtd_inl(MC_MC_B0_reg)&0x7, rtd_inl(MC2_MC2_50_reg)&0x3, (rtd_inl(MC2_MC2_98_reg)>>8)&0x1, rtd_inl(MC_DMA_MC_LF_P_DMA_RD_Ctrl_reg), rtd_inl(MC_DMA_MC_HF_P_DMA_RD_Ctrl_reg), (rtd_inl(KME_ME1_TOP1_ME1_SCENE1_00_reg)>>9)&0x1, (rtd_inl(MC_MC_28_reg)>>14)&0x1);
...
VOID Patch_detect(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	...
	if(pParam->u1_tri_ball_Issue_en == 1)//d5dc[15]
	{
