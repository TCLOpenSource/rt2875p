...
VOID FBTCCurve_ME(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
	...
	ReadRegister(HARDWARE_HARDWARE_58_reg, 17, 17, &u32_RB_val);//bit 17
	ReadRegister(SOFTWARE3_SOFTWARE3_06_reg, 7, 7, &log_en);
	...
	if(log_en){//d918[7]
		rtd_pr_memc_crit( "bad_reg_cnt,%d\n", u8_BadRegion_cnt);
	}
...
VOID FBLevelCalc(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
	...
	#endif
	if(u32_RB_val){
		rtd_pr_memc_emerg("[%s]4.FBLevel=%d[%d,%d]!\n",__FUNCTION__,pOutput->u8_FBLevel,pOutput->u8_FBResult,pOutput->u27_FBTempConsis);
...
VOID FBSystemCtrl_Proc(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
	...
	unsigned char  u8_flbk_lvl_CMR_pattern = 0;
	...
	if(s_pContext->_output_me_sceneAnalysis.u1_CMR_pattern_apply > 0)
...
//		rtd_pr_memc_debug("[jerry_MEMC_DisableMute_01][%d, %d](90K = %d)\n", memc_mute_cnt, memc_mute_state,rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
		...
		if(ES_Test_24==1)
		{
			rtd_pr_memc_info("[MEMC_FLAG] ,RFB_ID=%d, ES_Test_0 =%d ,ES_Test_1 =%d,rmv_cond=%d, \n",u32_RFB_ID,ES_Test_0,ES_Test_1 ,rmv_cond);
			if(u32_RFB_ID>0){
...
if(ES_Test_4==1)	  //YE Test to control ES Enable/Disable
	...
	if(((log_en3>>10)&0x1) == 1){
			rtd_pr_memc_info("[u8_rFB_alpha][,%d,%d,%d,%d,%d,%d,%d,%d,][,%d,%d,%d,%d,%d,%d,%d,%d,][,%d,%d,%d,%d,%d,%d,%d,%d,][,%d,%d,%d,%d,%d,%d,%d,%d,]\n",
	...
	if(((log_en3>>10)&0x1) == 1){
			rtd_pr_memc_info("[u8_rFB_level][,%d,%d,%d,%d,%d,%d,%d,%d,][,%d,%d,%d,%d,%d,%d,%d,%d,][,%d,%d,%d,%d,%d,%d,%d,%d,][,%d,%d,%d,%d,%d,%d,%d,%d,]\n",
