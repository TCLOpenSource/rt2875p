...
else if(pParam->u1_rFB_show_en == 0)
{
	...
	if(1){
		u32_wrt_data = s_pContext->_output_frc_sceneAnalysis.u1_Zoom_true  |
				   (( s_pContext->_output_wrt_comreg.u1_IP_preProcess_true | s_pContext->_output_wrt_comreg.u1_IP_wrtAction_true | s_pContext->_output_wrt_comreg.u1_Pattern_Update_true )<< 1) |
...
VOID scCtrl_wrtAction(const _PARAM_WRT_COM_REG *pParam, _OUTPUT_WRT_COM_REG *pOutput)
{
	...
	if(s_pContext->_external_data._output_mode != _PQL_OUT_PC_MODE)
	{
		...
		if(u32_RB_debug == 1){//d764[31]
			rtd_pr_memc_emerg("[%s][%d]case>>%d pan:%d fb:%d purevideo:%d mixmode:%d Voseek:%d holdcnt:%d sc:%d,%d,%d,%d,%d,%d,%d",
				__FUNCTION__, __LINE__,case_apply,
				s_pContext->_output_me_sceneAnalysis.u2_panning_flag, s_pContext->_output_frc_sceneAnalysis.u1_specialscfb_true,
...
VOID sc_enCtrl_wrtAction(const _PARAM_WRT_COM_REG *pParam, _OUTPUT_WRT_COM_REG *pOutput)
{
	...
	if(pParam->u1_scholdCtrl_wrt_en == 0)
		return;
	...
	else
	{
		...
		{
			kme_me1_top1_me1_scene1_02_reg.me1_sc_me1_hold_cnt=0x1;
			kme_me1_top1_me1_scene1_02_reg.me1_sc_mc_hold_cnt = 0x6;//kme_me1_top1_me1_scene1_02_reg.me1_sc_me1_hold_cnt = 0x6;//write to default setting, MEMC_SetRTKhdf()
		}
		else//code sync from k8 , but it will effect sad value.
		{
...
VOID fbCtrl_wrtAction(const _PARAM_WRT_COM_REG *pParam, _OUTPUT_WRT_COM_REG *pOutput)
{
	...
	unsigned int NewLowDelay_en = 0, NewLowDelay_FB_close = 0, force_FB_lvl = 0;;
...
VOID ME_dynSR_wrtRegister(_str_ME_dynSR_table *ME_dynSR_table)
{
	...
	/*rtd_pr_memc_emerg("vbuf_mvx_range=%d,vbuf_mvy_range=%d,kme_me2_vbuf_mvx_clip_range=%d,kme_me2_vbuf_mvy_clip_range=%d"
	,kme_vbuf_top_kme_vbuf_top_3c_reg.vbuf_mvx_range,kme_vbuf_top_kme_vbuf_top_3c_reg.vbuf_mvy_range
	,kme_me2_vbuf_top_kme_me2_vbuf_top_e0_reg.kme_me2_vbuf_mvx_clip_range,kme_me2_vbuf_top_kme_me2_vbuf_top_e0_reg.kme_me2_vbuf_mvy_clip_range );*/
...
VOID ME_dynSR_wrtAction(const _PARAM_WRT_COM_REG *pParam, _OUTPUT_WRT_COM_REG *pOutput)
{
	...
	{	
		/*rtd_pr_memc_emerg("table_level=%d,%d,%d,%d,%d",table_level,s_pContext->_output_frc_sceneAnalysis.u1_still_frame,s_pContext->_output_Patch_Manage.u1_patch_highFreq_zmv_true
		,s_pContext->_output_wrt_comreg.u1_24P_judder_test,s_pContext->_output_wrt_comreg.u1_is_test_video);*/
	...
	//rtd_pr_memc_emerg("table_level=%d",table_level);
...
//
	...
	{CTRL_NONE,CTRL_NONE,CTRL_NONE,CTRL_NONE,CTRL_NONE,CTRL_NONE,CTRL_NONE,CTRL_NONE,0,CTRL_NONE,CTRL_NONE,CTRL_NONE,CTRL_NONE,CTRL_NONE,CTRL_NONE,CTRL_NONE,CTRL_NONE,0,CTRL_NONE,CTRL_NONE,CTRL_NONE,1,CTRL_NONE,CTRL_NONE,CTRL_NONE,1,0,0},	/*9 small object*/
};
...
VOID  ME_Vbuf_LFSR_updStgh_wrtRegister(_str_ME_Vbuf_LFSR_updStgh_table *ME_Vbuf_LFSR_updStgh_table)
{
	...
	kme_vbuf_top_kme_vbuf_top_0c_reg.vbuf_ip_1st_lfsr_mask0_x = ME_Vbuf_LFSR_updStgh_table->vbuf_ip_1st_lfsr_mask0_x;
	kme_vbuf_top_kme_vbuf_top_0c_reg.vbuf_ip_1st_lfsr_mask0_y = ME_Vbuf_LFSR_updStgh_table->vbuf_ip_1st_lfsr_mask0_y;
	
	kme_vbuf_top_kme_vbuf_top_0c_reg.vbuf_ip_1st_lfsr_mask1_x = ME_Vbuf_LFSR_updStgh_table->vbuf_ip_1st_lfsr_mask1_x;
	kme_vbuf_top_kme_vbuf_top_0c_reg.vbuf_ip_1st_lfsr_mask1_y = ME_Vbuf_LFSR_updStgh_table->vbuf_ip_1st_lfsr_mask1_y;
	
	kme_vbuf_top_kme_vbuf_top_0c_reg.vbuf_ip_1st_lfsr_mask2_x = ME_Vbuf_LFSR_updStgh_table->vbuf_ip_1st_lfsr_mask2_x;
	kme_vbuf_top_kme_vbuf_top_0c_reg.vbuf_ip_1st_lfsr_mask2_y = ME_Vbuf_LFSR_updStgh_table->vbuf_ip_1st_lfsr_mask2_y;
	
	kme_vbuf_top_kme_vbuf_top_0c_reg.vbuf_ip_1st_lfsr_mask3_x = ME_Vbuf_LFSR_updStgh_table->vbuf_ip_1st_lfsr_mask3_x;
	kme_vbuf_top_kme_vbuf_top_0c_reg.vbuf_ip_1st_lfsr_mask3_y = ME_Vbuf_LFSR_updStgh_table->vbuf_ip_1st_lfsr_mask3_y;
	...
	kme_vbuf_top_kme_vbuf_top_5c_reg.vbuf_pi_1st_lfsr_mask0_x = ME_Vbuf_LFSR_updStgh_table->vbuf_pi_1st_lfsr_mask0_x;
	kme_vbuf_top_kme_vbuf_top_5c_reg.vbuf_pi_1st_lfsr_mask0_y = ME_Vbuf_LFSR_updStgh_table->vbuf_pi_1st_lfsr_mask0_y;
	
	kme_vbuf_top_kme_vbuf_top_5c_reg.vbuf_pi_1st_lfsr_mask1_x = ME_Vbuf_LFSR_updStgh_table->vbuf_pi_1st_lfsr_mask1_x;
	kme_vbuf_top_kme_vbuf_top_5c_reg.vbuf_pi_1st_lfsr_mask1_y = ME_Vbuf_LFSR_updStgh_table->vbuf_pi_1st_lfsr_mask1_y;
	
	kme_vbuf_top_kme_vbuf_top_5c_reg.vbuf_pi_1st_lfsr_mask2_x = ME_Vbuf_LFSR_updStgh_table->vbuf_pi_1st_lfsr_mask2_x;
	kme_vbuf_top_kme_vbuf_top_5c_reg.vbuf_pi_1st_lfsr_mask2_y = ME_Vbuf_LFSR_updStgh_table->vbuf_pi_1st_lfsr_mask2_y;
	
	kme_vbuf_top_kme_vbuf_top_5c_reg.vbuf_pi_1st_lfsr_mask3_x = ME_Vbuf_LFSR_updStgh_table->vbuf_pi_1st_lfsr_mask3_x;
	kme_vbuf_top_kme_vbuf_top_5c_reg.vbuf_pi_1st_lfsr_mask3_y = ME_Vbuf_LFSR_updStgh_table->vbuf_pi_1st_lfsr_mask3_y;
...
VOID ME_ipme_flp_alp_wrtAction(const _PARAM_WRT_COM_REG *pParam, _OUTPUT_WRT_COM_REG *pOutput)
{
	...
	_str_ME_ipme_flp_alp_table *tmmmp_ME_ipme_flp_alp_table = NULL;
	...
	tmmmp_ME_ipme_flp_alp_table = fwif_MEMC_get_ME_ipme_flp_alp_table(0);
	...
	if(log_en == 1){
		rtd_pr_memc_emerg("[g_Hist_Y_Mean_Value][bb][,%d,][,%d,%d,][,%d,%d,]\n", s_pContext->_output_me_sceneAnalysis.u1_Brightness_Chg_Cnt, tmp_ME_ipme_flp_alp_table->ipme_h_flp_alp0, tmp_ME_ipme_flp_alp_table->ipme_h_flp_alp1, tmmmp_ME_ipme_flp_alp_table->ipme_h_flp_alp0, tmmmp_ME_ipme_flp_alp_table->ipme_h_flp_alp1);
	}

	if(s_pContext->_output_me_sceneAnalysis.u1_Brightness_Chg_Cnt > 0){
		ME_ipme_flp_alp_wrtRegister(tmmmp_ME_ipme_flp_alp_table);
	}
	else{
...
static _str_Dh_Local_FB_table  Dh_Local_FB_table[24]={
	...
	{0x18,0x48,0x0,0x0},		/*22*/	// level 10
	{0x0,0x0,0x0,0x0},		/*23*/	// level 10
};
...
{ 
 ...
 if(s_pContext->_output_frc_sceneAnalysis.u8_me1_condition_195_face_broken == 1&&(!video192))
   ...
   else if(video195)
   {
	...
	//rtd_pr_memc_info("gmv:%d %d rmv:%d %d\n",u11_gmv_mvx,u10_gmv_mvy,rmvx[15],rmvy[15]);w
...
VOID MEMC_me1_bgmask_newalgo_WrtAction(const _PARAM_WRT_COM_REG *pParam ,_OUTPUT_WRT_COM_REG *pOutput)
{
    ...
    BSC = 1;
    path2 = 0x0 ;
    ReadRegister(SOFTWARE3_SOFTWARE3_25_reg,21 , 21 , &me1all);//d964
         ...
         /*   else
            ...
            else if(video172)
            {
			 ...
			 if(readval==0)
			 {
				 ...
				 if(Scaler_MEMC_GetPanelSizeByDisp() == MEMC_SIZE_2K)//if(Get_DISPLAY_PANEL_TYPE() == P_LVDS_2K1K)	  
				 {
					MEMC_me1_bgmask_2k_normal_setting(pParam  ,pOutput) ;
					 MEMC_me1_bgmask_2k_special_setting(pParam	,pOutput) ;
				 }			 
			 }
...
VOID MEMC_dehalo_newalgo_bgmaskBase_WrtAction_golf(const _PARAM_WRT_COM_REG *pParam,_OUTPUT_WRT_COM_REG *pOutput)
{
    const _PQLCONTEXT *s_pContext = GetPQLContext();
    int flag_IDT=0;
    static int v_IDT;
    //T add
    int flag_CGF=0,flag_Aeon=0,flag_skating=0,flag_filmj=0,flag_fof=0,flag_Flyingdagger=0,flag_FILM_JUDDER30s=0,flag_closer=0;
    static int v_CGF,v_Aeon,v_skating,v_skating_test,v_filmj,v_fof,v_Flyingdagger,v_FILM_JUDDER30s,v_closer;
    int srmvx,srmvy,u11_gmv_mvx,u10_gmv_mvy;
    int rgnIdx,rmvx[32]={0},rmvy[32]={0};
    int readval23=0,readvalobme=0;
    for(rgnIdx =0 ; rgnIdx< 32; rgnIdx++)
    {
...
//gongban T add 
    ...
    ReadRegister(SOFTWARE3_SOFTWARE3_25_reg,22 , 22 , &readvalobme);//d964
if(readval23==0)
{
...
//ed
	...
	//WriteRegister(0xb809ee40,31,31,1);//pfv cor en
	 ...
	 //WriteRegister(KPOST_TOP_KPOST_TOP_A0_reg, 16, 19, 8);
        WriteRegister(KME_DEHALO5_PHMV_FIX_0_reg,13,13,0x0); 
        WriteRegister(KME_DEHALO5_PHMV_FIX_23_reg,16,16,0x0); 
        WriteRegister(KME_DEHALO5_PHMV_FIX_5_reg,15,15,0);//pred_old_diff_cut
        WriteRegister(KME_DEHALO5_PHMV_FIX_6_reg,7,7,0);//bad_pred_cut
...
{
//obme
        WriteRegister(KME_VBUF_TOP_KME_VBUF_TOP_8C_reg, 18, 19, 0);//FRC_TOP__KME_VBUF_TOP__reg_vbuf_ip_dc_obme_mode
        WriteRegister(KME_VBUF_TOP_KME_VBUF_TOP_8C_reg, 20, 21, 0);//FRC_TOP__KME_VBUF_TOP__reg_vbuf_pi_dc_obme_mode
        WriteRegister(KME_VBUF_TOP_KME_VBUF_TOP_8C_reg, 22, 22, 1);//FRC_TOP__KME_VBUF_TOP__vbuf_ip_dc_obme_mode_sel
        WriteRegister(KME_VBUF_TOP_KME_VBUF_TOP_8C_reg, 23, 23, 1);//FRC_TOP__KME_VBUF_TOP__vbuf_pi_dc_obme_mode_sel
        WriteRegister(KME_VBUF_TOP_KME_VBUF_TOP_8C_reg, 24, 25, 2);//FRC_TOP__KME_VBUF_TOP__reg_vbuf_ip_ac_obme_mode
        WriteRegister(KME_VBUF_TOP_KME_VBUF_TOP_8C_reg, 26, 27, 2);//FRC_TOP__KME_VBUF_TOP__reg_vbuf_pi_ac_obme_mode
        WriteRegister(KME_ME1_TOP0_KME_ME1_TOP0_00_reg, 8, 8, 0);//me1_ip_ac_flt_sel
        WriteRegister(KME_ME1_TOP0_KME_ME1_TOP0_40_reg, 8, 8, 0);//me1_pi_ac_flt_sel

        WriteRegister(KME_ME2_VBUF_TOP_KME_ME2_VBUF_TOP_D8_reg, 28, 29, 0);//kme_me2_vbuf_1st_dc_obme_mode
        WriteRegister(KME_ME2_VBUF_TOP_KME_ME2_VBUF_TOP_D8_reg, 30, 31, 0);//kme_me2_vbuf_2nd_dc_obme_mode
        WriteRegister(KME_ME2_VBUF_TOP_KME_ME2_VBUF_TOP_DC_reg, 0, 1, 0);//kme_me2_vbuf_fsearch_dc_obme_mode
        WriteRegister(KME_ME2_VBUF_TOP_KME_ME2_VBUF_TOP_DC_reg, 2, 2, 1);//kme_me2_vbuf_1st_dc_obme_mode_sel
        WriteRegister(KME_ME2_VBUF_TOP_KME_ME2_VBUF_TOP_DC_reg, 3, 3, 1);//kme_me2_vbuf_2nd_dc_obme_mode_sel
        WriteRegister(KME_ME2_VBUF_TOP_KME_ME2_VBUF_TOP_DC_reg, 4, 4, 1);//kme_me2_vbuf_fsearch_dc_obme_mode_sel
        WriteRegister(KME_ME2_VBUF_TOP_KME_ME2_VBUF_TOP_D8_reg, 21, 22, 2);//kme_me2_vbuf_1st_ac_obme_mode
        WriteRegister(KME_ME2_VBUF_TOP_KME_ME2_VBUF_TOP_D8_reg, 23, 24, 2);//kme_me2_vbuf_1st_ac_obme_mode	
        WriteRegister(KME_ME2_VBUF_TOP_KME_ME2_VBUF_TOP_D8_reg, 25, 26, 2);//kme_me2_vbuf_1st_ac_obme_mode
        WriteRegister(KME_ME2_CALC0_KME_ME2_CALC0_9C_reg, 7, 7, 0);//me2_ph_ac_flt_sel    
...
// HS merlin7
	...
	else if( u32_RB_val == 1 )	// 15 regions
	{
		if(pParam->u1_logo_dhRgnProc_en)
		{
			...
			{
				WriteRegister(KME_DEHALO6_dh_logo_ctrl_1_rga_reg, 26, 31, 20);	// dh_logo_mv_d_diff_th_rga
				WriteRegister(KME_DEHALO6_dh_logo_ctrl_1_rge_reg, 26, 31, 20);	// dh_logo_mv_d_diff_th_rge

				if(s_pContext->_output_FRC_LgDet.u1_logo_PurePanning_status == 7){
					WriteRegister(KME_DEHALO2_KME_DEHALO2_84_reg, 0, 5, 20);	// dh_logo_mv_d_diff_th_rg4	
					WriteRegister(KME_DEHALO6_dh_logo_ctrl_1_rg4_reg, 26, 31, 20);	// dh_logo_mv_d_diff_th_rg4		
				}
				dynamic_tunning_case1 = 1;
			...
			else if( s_pContext->_output_FRC_LgDet.u1_logo_PurePanning_status == 10 || s_pContext->_output_FRC_LgDet.u1_logo_PurePanning_status == 2 )
			{
				...
				WriteRegister(KME_DEHALO6_dh_logo_ctrl_2_rg9_rga_reg, 24, 31, 12);		//mv_t_th_rga
				WriteRegister(KME_DEHALO6_dh_logo_ctrl_2_rgb_rgc_reg, 8, 15, 12);		//mv_t_th_rgb
				WriteRegister(KME_DEHALO6_dh_logo_ctrl_2_rgd_rge_reg, 8, 15, 12);		//mv_t_th_rgd
				WriteRegister(KME_DEHALO6_dh_logo_ctrl_2_rgd_rge_reg, 24, 31, 12);		//mv_t_th_rge
			...
			{
				WriteRegister(KME_DEHALO6_dh_logo_ctrl_2_rg9_rga_reg, 16, 23, 80);
				WriteRegister(KME_DEHALO6_dh_logo_ctrl_2_rgd_rge_reg, 16, 23, 80);
				dynamic_tunning_case1 = 1;
			}
...
{	
	...
	if(Pro_tv006)//(  vpq_project_id == 0x00060000 || vpqex_project_id == 0x00060000 )
	...
	if(1)//(fwif_VIP_get_Project_ID() == VIP_Project_ID_TV030)	//if(Pro_tv030)
...
{
	unsigned int mc_dma_value=0;//,me_dma_value=0,mv_dma_value=0,me_info_value=0;//,mv_info_value=0;

	//ReadRegister(KME_DM_TOP2_KME_ME_SHR_ST_reg, 0, 1, &me_info_value);
	//ReadRegister(MVINFO_DMA_KMVINFO_DM_reg, 0, 7, &mv_info_value);
	//ReadRegister(ME_SHARE_DMA_KME_DMA_STATUS_reg, 0, 1, &me_dma_value);
	//ReadRegister(MVINFO_DMA_KMVINFO_STATUS_reg, 1, 1, &mv_dma_value);
	ReadRegister(LBMC_LBMC_E8_reg, 0, 31, &mc_dma_value);

	if(mc_dma_value!=0){//if(me_info_value!=0 || me_dma_value!=0 || mv_dma_value!=0 || mc_dma_value!=0){
		return 1;
	}
	else
		return 0;
...
VOID MEMC_DisableMute(const _PARAM_WRT_COM_REG *pParam, _OUTPUT_WRT_COM_REG *pOutput)
{
	...
	static unsigned int memc_dma_error_cnt=0;
	unsigned int memc_dma_cnt=61;//experiment results
...
VOID Wrt_ComReg_Proc_outputInterrupt(const _PARAM_WRT_COM_REG *pParam, _OUTPUT_WRT_COM_REG *pOutput)
{
	...
	*/
	...
	if(MEMC_RepeatMode==0){
		...
		MEMC_dehalo_newalgo_bgmaskBase_WrtAction_golf (pParam,pOutput);
