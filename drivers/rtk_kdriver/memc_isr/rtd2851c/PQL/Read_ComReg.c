...
VOID ReadComReg_Proc_inputInterrupt(const _PARAM_ReadComReg *pParam, _OUTPUT_ReadComReg *pOutput)
{
	...
	pOutput->s11_me_mv_max_vx_rb = ((u32_RB_val >> 10) &1) == 0? ((u32_RB_val>>0)& 0x7ff):(((u32_RB_val>>0)& 0x7ff)	- (1<<11)); 
	pOutput->s10_me_mv_max_vy_rb =((u32_RB_val >> 20) &1) == 0? ((u32_RB_val >> 11) & 0x3ff) : (((u32_RB_val >> 11) & 0x3ff)  - (1<<10));

	ReadRegister(KME_ME1_BG1_ME_BG_INFO_MIN_MV_reg ,0,31,&u32_RB_val);	 //fg rgn30 and fg rgn31  
	pOutput->s11_me_mv_min_vx_rb = ((u32_RB_val >> 10) &1) == 0? ((u32_RB_val>>0)& 0x7ff):(((u32_RB_val>>0)& 0x7ff)	- (1<<11)); 
	pOutput->s10_me_mv_min_vy_rb =((u32_RB_val >> 20) &1) == 0? ((u32_RB_val >> 11) & 0x3ff) : (((u32_RB_val >> 11) & 0x3ff)  - (1<<10));
...
VOID ReadComReg_Proc_outputInterrupt(const _PARAM_ReadComReg *pParam, _OUTPUT_ReadComReg *pOutput)
{
	...
	/*
	ReadRegister(KME_ME1_BG1_ME_BG_INFO_GMV01_reg,10,20, &RB_val);
	pOutput->dh_gmv_fgx=RB_val;
	ReadRegister(KME_ME1_BG1_ME_BG_INFO_GMV01_reg,0,9, &RB_val);
	pOutput->dh_gmv_fgy=RB_val;
	ReadRegister(KME_ME1_BG1_ME_BG_INFO_GMV00_reg,16,26, &RB_val);
	pOutput->dh_gmv_bgx=RB_val; 
	ReadRegister(KME_ME1_BG1_ME_BG_INFO_GMV01_reg,21,30, &RB_val);	 
	pOutput->dh_gmv_bgy=RB_val;
	*/
	//  pOutput->s11_me_GMV_1st_vx_rb = ((u32_RB_val >> 10) &1) == 0? (u32_RB_val		 & 0x7ff) : ((u32_RB_val		 & 0x7ff)  - (1<<11));
	//  pOutput->s10_me_GMV_1st_vy_rb = ((u32_RB_val >> 20) &1) == 0? ((u32_RB_val >> 11) & 0x3ff) : (((u32_RB_val >> 11) & 0x3ff)  - (1<<10));
    ...
    for(rgnIdx =0 ; rgnIdx< 32; rgnIdx++)
    {
        ...
        pOutput->s11_bg_rMV_x_rb[rgnIdx]=rmvx_bg;
        pOutput->s10_bg_rMV_y_rb[rgnIdx]=rmvy_bg;
    }
