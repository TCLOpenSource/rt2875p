...
static unsigned int u32ME1_BlockNum = 240;
static unsigned int u32ME1_RowNum = 135;

static unsigned int u32IPME2_HSize = 960;
static unsigned int u32IPME2_VSize = 540;
static unsigned int u32ME2_BlockNum = 480;
static unsigned int u32ME2_RowNum = 270;
...
static unsigned int u32PLogo_HSize = 960;
static unsigned int u32PLogo_VSize = 540;

static unsigned int u32BLogo_BlkNum = 240;
static unsigned int u32BLogo_RowNum = 135;
...
VOID Mid_Mode_EnableMEshareDMA(VOID)
{
	...
	rtd_outl(ME_SHARE_DMA_KME_RD_client_en_reg,0x00000007); //0xb809B7B0 [5:0], dma_off me_new	
...
VOID Mid_Mode_MEDEN_Checking(VOID)
{
	...
	if( ( ME_shareDMA_W==1 && ME_shareDMA_R==1 ) && ( KME_DEN_CNT==0 )){
...
VOID Mid_Mode_SetMEMCFrameRepeatEnable(BOOL bEnable)
{
	...
	if(bEnable)
	{
		...
		if(VR360_en == 1){
			HAL_DM_SetKmc00WriteEnable(FALSE);
			HAL_DM_SetKmc01WriteEnable(FALSE);
			HAL_DM_SetKmc04ReadEnable(FALSE);
			HAL_DM_SetKmc05ReadEnable(FALSE);
			HAL_DM_SetKmc06ReadEnable(FALSE);
			HAL_DM_SetKmc07ReadEnable(FALSE);
		...
		if(u8_pulldown_mode == _PQL_IN24HZ_TRUMOTOFF_CINEMAOFF ||
