...
unsigned char u8_SlowIn_MaxShiftX=0;
unsigned char u8_SlowIn_MaxShiftY=0;
...
{
	unsigned int u32_rb_val = 0, sum_apl = 0, avg_pos = 0, log_en = 0;
	...
	static char hold_cnt = 0;
 	unsigned int u1_Algo_en;
	ReadRegister(HARDWARE_HARDWARE_44_reg, 15, 15, &u1_Algo_en);

	if(!u1_Algo_en) { //d5b0[15]
		u16_RightRim_16px = 0;
		line_pos[0]=0;
		return;
	}
	...
	ReadRegister(HARDWARE_HARDWARE_44_reg,7,7, &log_en);/*0xB809D5B0*/
	...
	if(pOutput->u4_outResolution_pre == _PQL_OUT_1920x1080){// 2k
		...
		WriteRegister(KMC_METER_TOP_KMC_METER_TOP_04_reg, 16,28,0x437);
		h_line_total = 1920;
	}else{//4//4k
		WriteRegister(KMC_METER_TOP_KMC_METER_TOP_04_reg, 0,12,0x0);
		WriteRegister(KMC_METER_TOP_KMC_METER_TOP_04_reg, 16,28,0x86f);
		h_line_total = 3840;
	}
...
unsigned short RimCtrl_Get_HW_rim(unsigned char rimID, _OUTPUT_RimCtrl *pOutput)
{
	...
	ReadRegister(HARDWARE_HARDWARE_44_reg, 8, 8, &log_en);/*0xB809D5B0*/
	...
	else if(rimID==_RIM_RHT && u16_RightRim_16px != 0){
		u12_HW_rim = u16_RightRim_16px;
	}
	else{
		u12_HW_rim = u1_BBD_fineValid == 1? u12_BBD_fineRim :
	...
	if(log_en){// bit 8
		rtd_pr_memc_err("[%s] RimID:%d	Result:%d	FV:%d	FR:%d	RV:%d	RR:%d\n",__FUNCTION__,
...
unsigned short RimCtrl_Get_Rim_Bound(unsigned char u2_Rim_id, _OUTPUT_RimCtrl *pOutput)
{
...
VOID RimCtrl_Boundary_Protect(unsigned short *u12_Rim_source,unsigned short *u12_Rim_Result, _OUTPUT_RimCtrl *pOutput)
{
	...
	unsigned char  apl_avg = 0, u8_k = 0;
	unsigned int u1_Protect_disable, log_en;
	ReadRegister(HARDWARE_HARDWARE_43_reg, 3, 3, &u1_Protect_disable);
	ReadRegister(HARDWARE_HARDWARE_44_reg, 13, 13, &log_en);
	...
	if(u1_Protect_disable) { //d5ac[3]
		...
		for (u8_k = 0; u8_k < _RIM_NUM; u8_k++){
			u12_rim_bound[u8_k] = RimCtrl_Get_Rim_Bound(u8_k, pOutput);
		}
...
VOID RimCtrl_HW_extract(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	unsigned short u12_det_Rim[_RIM_NUM], u12_rim_bound[_RIM_NUM];
	unsigned char  u8_k;
...
#if	1
	unsigned int u32_rb_val = 0, log_en = 0, log_en2 = 0, log_en3 = 0, log_en4 = 0, u1_pan_speedup;
	ReadRegister(HARDWARE_HARDWARE_43_reg,0,31, &u32_rb_val);/*0xB809D5AC*/		
	ReadRegister(HARDWARE_HARDWARE_44_reg,2,2, &log_en2);
	ReadRegister(HARDWARE_HARDWARE_44_reg,4,4, &log_en3);
	ReadRegister(HARDWARE_HARDWARE_44_reg,9,9, &log_en4);
	ReadRegister(SOFTWARE2_SOFTWARE2_61_reg, 30, 30, &log_en);	
	ReadRegister(HARDWARE_HARDWARE_44_reg,14,14, &u1_pan_speedup);
#endif
	...
	if(u1_pan_speedup && u1_Panning_flg && _ABS_DIFF_(u11_gmv_mvx, u10_gmv_mvy)>=4) {
		if(_ABS_(u10_gmv_mvy)>_ABS_(u11_gmv_mvx)) {
			if(u10_gmv_mvy>0) {
	...
	//rtd_pr_memc_warn("mvx:%d	mvy:%d	panning:%d\n", u11_gmv_mvx, u10_gmv_mvy, u1_Panning_flg);
	...
	if(log_en2){// bit 2
		rtd_pr_memc_err("===== [%s], [%d] =========\n",__FUNCTION__, __LINE__);
		rtd_pr_memc_err("SC>>%d Panning>>%d Gmv:%d,%d\n",u1_Hist_SC_flg,u1_Panning_flg,u11_gmv_mvx,u10_gmv_mvy);
...
/*	const _PQLCONTEXT *s_pContext = GetPQLContext();*/
	...
	unsigned char u8_RimBiggerTh = u1_Panning_flg==true ? (pParam->u8_RimBiggerTh>>1) : pParam->u8_RimBiggerTh;
	ReadRegister(SOFTWARE2_SOFTWARE2_61_reg, 30, 30, &log_en);
	ReadRegister(HARDWARE_HARDWARE_44_reg, 11, 11, &log_en2);
	...
	if(log_en2){ //bit 11
		rtd_pr_memc_crit("[%s][T,B,L,R] Target>>[%d,%d,%d,%d] Pre>>[%d,%d,%d,%d] Cnt>>[%d,%d,%d,%d] Th>>%d\n", __FUNCTION__,
			...
			pOutput->s8_BiggerCnt[_RIM_TOP], pOutput->s8_BiggerCnt[_RIM_BOT], pOutput->s8_BiggerCnt[_RIM_LFT], pOutput->s8_BiggerCnt[_RIM_RHT], pParam->u8_RimBiggerTh);
	}
}
...
//	const _PQLCONTEXT *s_pContext = GetPQLContext();
	...
	unsigned int log_en = 0, log_en2 = 0, test_val_2 = 0, u1_limit_speed = 0;
	ReadRegister(SOFTWARE2_SOFTWARE2_61_reg, 30, 30, &log_en);
	ReadRegister(SOFTWARE2_SOFTWARE2_61_reg, 8, 15, &test_val_2);
	ReadRegister(HARDWARE_HARDWARE_44_reg, 12, 12, &log_en2);
	ReadRegister(HARDWARE_HARDWARE_44_reg, 17, 17, &u1_limit_speed);
...
//		if (s_pContext->_external_info.u1_Hist_SC_flg)
		...
		if (pOutput->s8_SmallerCnt[i] >= u8_dyn_thr /*pParam->u8_RimSmallerTh*/)
		{
			...
			if(u1_limit_speed){
				...
				{
					case _RIM_TOP :
						pOutput->u12_Rim_Result[i] = _MIN_(pOutput->u12_Rim_Result[i], pOutput->u12_Rim_Pre[i]+u8_SlowIn_MaxShiftY);
					break;
					case _RIM_BOT :
						pOutput->u12_Rim_Result[i] = _MAX_(pOutput->u12_Rim_Result[i], (short)pOutput->u12_Rim_Pre[i]-u8_SlowIn_MaxShiftY);
					break;
					case _RIM_LFT :
						pOutput->u12_Rim_Result[i] = _MIN_(pOutput->u12_Rim_Result[i], pOutput->u12_Rim_Pre[i]+u8_SlowIn_MaxShiftX);
					break;
					case _RIM_RHT :
						pOutput->u12_Rim_Result[i] = _MAX_(pOutput->u12_Rim_Result[i], (short)pOutput->u12_Rim_Pre[i]-u8_SlowIn_MaxShiftX);
					break;
				}
			}
			pOutput->u1_RimChange      = 1;
	...
	if(log_en2){//bit 12
		rtd_pr_memc_crit("[%s][T,B,L,R] Target>>[%d,%d,%d,%d] Result>>[%d,%d,%d,%d] Cnt>>[%d,%d,%d,%d] SC>>%d,%d Th>>%d,%d\n", __FUNCTION__,
			pOutput->u12_Rim_Target[_RIM_TOP], pOutput->u12_Rim_Target[_RIM_BOT], pOutput->u12_Rim_Target[_RIM_LFT], pOutput->u12_Rim_Target[_RIM_RHT],
			pOutput->u12_Rim_Result[_RIM_TOP], pOutput->u12_Rim_Result[_RIM_BOT], pOutput->u12_Rim_Result[_RIM_LFT], pOutput->u12_Rim_Result[_RIM_RHT],
			...
			sc_cnt, u8_sc_cnt, pParam->u8_RimSmallerSCTh, pParam->u8_RimSmallerTh);
	}
...
/*		const _PQLCONTEXT *s_pContext = GetPQLContext();*/
	unsigned char  u4_Res_H_Divide = (pOutput->u4_outResolution_pre == _PQL_OUT_1920x1080 || pOutput->u4_outResolution_pre == _PQL_OUT_1920x540) ? 2 : 
    	...
    	unsigned char  u4_Res_V_Divide = (pOutput->u4_outResolution_pre == _PQL_OUT_1920x1080 || pOutput->u4_outResolution_pre == _PQL_OUT_3840x1080) ? 1 : 
...
{
	int i=0;
	unsigned int Res_H = (pOutput->u4_outResolution_pre == _PQL_OUT_1920x1080 || pOutput->u4_outResolution_pre == _PQL_OUT_1920x540) ? 1920 : 3840;
...
VOID Rim_check_protect_Film(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	unsigned char  Res_H_Divide = (pOutput->u4_outResolution_pre == _PQL_OUT_1920x1080 || pOutput->u4_outResolution_pre == _PQL_OUT_1920x540) ? 2 : 
		                     			(pOutput->u4_outResolution_pre == _PQL_OUT_3840x2160 || pOutput->u4_outResolution_pre == _PQL_OUT_3840x1080 ||pOutput->u4_outResolution_pre == _PQL_OUT_3840x540) ? 3 : 0;
    unsigned char  Res_V_Divide = (pOutput->u4_outResolution_pre == _PQL_OUT_1920x1080 || pOutput->u4_outResolution_pre == _PQL_OUT_3840x1080) ? 1 : 
	                         			(pOutput->u4_outResolution_pre == _PQL_OUT_3840x2160) ? 2 : 0;  // Fot RimCtrl_Film_Rim_Convert
...
VOID RimCtrl_Init(_OUTPUT_RimCtrl *pOutput)
{
	...
	for(u8_i=0; u8_i<_RIM_NUM;u8_i++)
	{
		...
		pOutput->u12_Rim_Target[u8_i] = (u8_i==_RIM_TOP ||u8_i==_RIM_LFT)?0:((u8_i==_RIM_BOT)?2159:3839);		// rim target of fast-in and slow-out
...
VOID RimCtrl_DynamicBlackTh(VOID)
{
	...
	#if 1
	unsigned int u32_rb_val = 0, log_en = 0;
	...
	if(log_en){
		rtd_pr_memc_notice("[Rim_check_protect_MC][%d][u8_darklevel,%d,]\n\r", __LINE__, u8_darklevel);
	}
#if 0
...
VOID RimCtrl_Rim_Comp(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	ReadRegister(HARDWARE_HARDWARE_43_reg,0,31, &u32_rb_val);/*0xB809D5AC*/
	ReadRegister(HARDWARE_HARDWARE_44_reg,6,6, &log_en);/*0xB809D5B0*/
...
VOID RimCtrl_SC_rim(_OUTPUT_RimCtrl *pOutput)
{
	...
	ReadRegister(HARDWARE_HARDWARE_44_reg,10,10, &log_en);/*0xB809D5B0*/
	ReadRegister(HARDWARE_HARDWARE_44_reg,16,16, &u1_Algo_en);

	if(!u1_Algo_en){//bit 16
		u8_SC_cnt = 0;
		return;
	}
	u8_SC_cnt = (u1_Hist_SC_flg) ? u8_SC_cnt+1 : 0;

	if(log_en){// bit10
		rtd_pr_memc_err("[%s][%d] cnt>>%d  SC>>[%u, %u]\n",	__FUNCTION__, __LINE__,	u8_SC_cnt, g_HisDiff, g_HueDiff);
	}
	...
	if(u8_SC_cnt>=2){
		...
		if(log_en){// bit10
			rtd_pr_memc_err("[%s][%d] HW RIM>>[%d, %d, %d, %d] Set RIM>>[%d, %d, %d, %d]\n", __FUNCTION__, __LINE__,
...
VOID RimCtrl_Proc(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	ReadRegister(HARDWARE_HARDWARE_44_reg, 28, 28, &log_Result_en[_RIM_TOP]);
	ReadRegister(HARDWARE_HARDWARE_44_reg, 29, 29, &log_Result_en[_RIM_BOT]);
	ReadRegister(HARDWARE_HARDWARE_44_reg, 30, 30, &log_Result_en[_RIM_LFT]);
	ReadRegister(HARDWARE_HARDWARE_44_reg, 31, 31, &log_Result_en[_RIM_RHT]);

	ReadRegister(HARDWARE_HARDWARE_44_reg, 24, 24, &log_Target_en[_RIM_TOP]);
	ReadRegister(HARDWARE_HARDWARE_44_reg, 25, 25, &log_Target_en[_RIM_BOT]);
	ReadRegister(HARDWARE_HARDWARE_44_reg, 26, 26, &log_Target_en[_RIM_LFT]);
	ReadRegister(HARDWARE_HARDWARE_44_reg, 27, 27, &log_Target_en[_RIM_RHT]);
	...
	if (pParam->u1_RimCtrl_en == 1)
	{
		...
		u1_Panning_flg = s_pContext->_output_me_sceneAnalysis.u2_panning_flag==0 ? false : true;
	...
	//	if((u32_rb_val)&0x40) return;
		...
		if(log_Target_en[_RIM_TOP]){//d5b0[24]
			rtd_pr_memc_crit("[%s][TOP][Target] HW>>%d	Comp:%d FastOut>>%d SlowIn>>%d	SC>>%d\n", __FUNCTION__, Rim_info_Target[0][_RIM_TOP],
								Rim_info_Target[1][_RIM_TOP], Rim_info_Target[2][_RIM_TOP], Rim_info_Target[3][_RIM_TOP], Rim_info_Target[4][_RIM_TOP]);
		}
		if(log_Target_en[_RIM_BOT]){//d5b0[25]
			rtd_pr_memc_crit("[%s][BOT][Target] HW>>%d	Comp:%d FastOut>>%d SlowIn>>%d	SC>>%d\n", __FUNCTION__, Rim_info_Target[0][_RIM_BOT],
								Rim_info_Target[1][_RIM_BOT], Rim_info_Target[2][_RIM_BOT], Rim_info_Target[3][_RIM_BOT], Rim_info_Target[4][_RIM_BOT]);
		}
		if(log_Target_en[_RIM_LFT]){//d5b0[26]
			rtd_pr_memc_crit("[%s][LFT][Target] HW>>%d	Comp:%d FastOut>>%d SlowIn>>%d	SC>>%d\n", __FUNCTION__, Rim_info_Target[0][_RIM_LFT],
								Rim_info_Target[1][_RIM_LFT], Rim_info_Target[2][_RIM_LFT], Rim_info_Target[3][_RIM_LFT], Rim_info_Target[4][_RIM_LFT]);
		}
		if(log_Target_en[_RIM_RHT]){//d5b0[27]
			rtd_pr_memc_crit("[%s][RHT][Target] HW>>%d	Comp:%d FastOut>>%d SlowIn>>%d	SC>>%d\n", __FUNCTION__, Rim_info_Target[0][_RIM_RHT],
								Rim_info_Target[1][_RIM_RHT], Rim_info_Target[2][_RIM_RHT], Rim_info_Target[3][_RIM_RHT], Rim_info_Target[4][_RIM_RHT]);
		}
		if(log_Result_en[_RIM_TOP]){//d5b0[28]
			rtd_pr_memc_crit("[%s][TOP][Result]	HW>>%d	Comp:%d	FastOut>>%d	SlowIn>>%d	SC>>%d\n", __FUNCTION__, Rim_info_Result[0][_RIM_TOP],
								Rim_info_Result[1][_RIM_TOP], Rim_info_Result[2][_RIM_TOP], Rim_info_Result[3][_RIM_TOP], Rim_info_Result[4][_RIM_TOP]);
		}
		if(log_Result_en[_RIM_BOT]){//d5b0[29]
			rtd_pr_memc_crit("[%s][BOT][Result]	HW>>%d	Comp:%d	FastOut>>%d	SlowIn>>%d	SC>>%d\n", __FUNCTION__, Rim_info_Result[0][_RIM_BOT],
								Rim_info_Result[1][_RIM_BOT], Rim_info_Result[2][_RIM_BOT],	Rim_info_Result[3][_RIM_BOT], Rim_info_Result[4][_RIM_BOT]);
		}
		if(log_Result_en[_RIM_LFT]){//d5b0[30]
			rtd_pr_memc_crit("[%s][LFT][Result]	HW>>%d	Comp:%d	FastOut>>%d	SlowIn>>%d	SC>>%d\n", __FUNCTION__, Rim_info_Result[0][_RIM_LFT],
		...
		if(log_Result_en[_RIM_RHT]){//d5b0[31]
			rtd_pr_memc_crit("[%s][RHT][Result]	HW>>%d	Comp:%d	FastOut>>%d	SlowIn>>%d	SC>>%d\n", __FUNCTION__, Rim_info_Result[0][_RIM_RHT],
