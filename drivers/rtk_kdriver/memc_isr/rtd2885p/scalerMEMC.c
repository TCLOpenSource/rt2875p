...
void Scaler_MEMC_Set_MC_MEMBoundary(unsigned int  nDownlimit,unsigned int nUplimit)
{
	...
	pr_emerg("[MEMC_BRINGUP_TEST][CCC][9458=,%x,]\n", nUplimit);
...
#ifdef BUILD_QUICK_SHOW
#if QUICK_SHOW_LOWDEALY
	//Scaler_MEMC_LowDelayMode(true);
	//rtd_pr_memc_info("qs LowDelayMode\n");
#endif
#endif
...
#else
	unsigned char level_map[11] = { 0, 14, 28, 42, 56, 70, 84, 96, 107, 118, 128 }; //updated 20211013 by L_G Ahn
	//{0, 14, 28, 41, 55, 68, 82, 95, 106, 117, 128};	//confirmed by L_G Ann
...
unsigned char Scaler_MEMC_Deblur_Level_Mapping(unsigned char inLevel)
{
	...
	rtd_pr_memc_info("[MEMC][%s] inLevel = %d, outLevel = %d\n", __FUNCTION__, inLevel, outLevel);
#else
	unsigned char level_map[11] = {0, 14, 28, 41, 55, 68, 82, 95, 106, 117, 128};	//confirmed by L_G Ann
...
extern unsigned char drv_memory_get_game_mode_dynamic(void);
...
unsigned int Scaler_MEMC_GetAVSyncDelay(void)
{
	...
	ReadRegister(SOFTWARE3_SOFTWARE3_12_reg, 30, 30, &log_en2);
