...
static VOID LbmcMode_Detect_onlyCnt(const _PARAM_MC_LBMC_SWITCH *pParam, _OUTPUT_MC_LBMC_SWITCH *pOutput, unsigned char band_Type)
{
	...
	ReadRegister(SOFTWARE1_SOFTWARE1_26_reg, 17,  17, &u32_ID_Log_en2);//YE Test debug dummy register
	...
	ReadRegister(SOFTWARE2_SOFTWARE2_50_reg, 0, 0, &log_en1);
	...
	if(u32_ID_Log_en2==1)
		{
			if(u8_lbmc_mode==1)
			rtd_pr_memc_debug("[MEMC YE_RMV],8to23_mean_rmv=%d, rmv_00_mvy=%d,rmv_01_mvy=%d,rmv_02_mvy=%d,rmv_03_mvy=%d,rmv_04_mvy=%d ,rmv_05_mvy=%d,rmv_06_mvy=%d,rmv_07_mvy=%d \
	...
	if (band_Type == _LBMC_LF_) // LF
	{
		...
		{
			if(u8_lbmc_mode==1)
			rtd_pr_memc_debug("[MEMC YE_Band_LF],band_Type=%d,smv_posCnt=%d,smv_negCnt=%d,Lmv_posCnt=%d,Lmv_negCnt=%d,Ovlap_s2n_cntTh=%d,Lmv_s2n_cntTh=%d,Ovlap_n2s_cntTh=%d,Lmv_n2s_cntTh=%d,mode=%d, \n ",
		...
		*/
		...
		if(u32_ID_Log_en==1)
			{
				//if(u8_lbmc_mode==1)
				rtd_pr_memc_debug("[MEMC_YE_Mode_LF],band_Type=%d, cnt_totalNum=%d,rb_smv_posCnt=%d,rb_smv_negCnt=%d,rb_Lmv_posCnt=%d,rb_Lmv_negCnt =%d, \n",
	...
	else // Hf
	{
		...
		{
			if(u8_lbmc_mode==1)
			rtd_pr_memc_debug("[MEMC YE_Band_HF],lbmc_mode=%d,band_Type=%d,smv_posCnt=%d,smv_negCnt=%d,Lmv_posCnt=%d,Lmv_negCnt=%d,Ovlap_s2n_cntTh=%d,Lmv_s2n_cntTh=%d,Ovlap_n2s_cntTh=%d,Lmv_n2s_cntTh=%d,mode=%d, \n ",
...
if(0)
{
...
#else
		rmv_diff[i]=ABS(u16_8to23_mean_rmv,u10_rmv_mvy[i]);
		rmv_diff[j]=ABS(u16_8to23_mean_rmv,u10_rmv_mvy[j]);
#endif
			...
			{
				if(u8_lbmc_mode==1)
					rtd_pr_memc_debug("[MEMC YE_RMV_DIFF],8to23_mean_rmv=%d,i=%d,j=%d,0to7_flag=%d,reduce_8to60_flag=%d,24to31_flag=%d,reduce_under60_flag=%d,rmv_diff_i= %d,rmv_diff_j= %d,8to23_mean_rmv=%d,rmv_mvy[i]=%d,rmv_mvy[j]=%d,diff_th=%d,rb_smv_negCnt=%d,rb_Lmv_negCnt=%d,u16_rb_smv_posCnt=%d,u16_rb_Lmv_posCnt=%d, \n",
	...
	// mode switch
	{
		...
		if (band_Type == _LBMC_LF_)
		{
			...
			{
				//if(u8_lbmc_mode==1)
				rtd_pr_memc_debug("[MEMC YE_Mode_LF],band_Type=%d, n2su_en=%d, su2n_en=%d,n2sd_en=%d,sd2n_en=%d,smv_near0_cnt=%d,cnt_totalNum=%d,rb_smv_posCnt=%d,rb_smv_negCnt=%d,ovlap_posCnt=%d,rb_Lmv_posCnt=%d,ovlap_negCnt=%d,rb_Lmv_negCnt =%d, \n",
		...
		else
		{
			...
			{
				if(u8_lbmc_mode==1)
				rtd_pr_memc_debug("[MEMC YE_Mode_HF],band_Type=%d, n2su_en=%d, su2n_en=%d,n2sd_en=%d,sd2n_en=%d,smv_near0_cnt=%d,cnt_totalNum=%d,rb_smv_posCnt=%d,rb_smv_negCnt=%d,ovlap_posCnt=%d,rb_Lmv_posCnt=%d,ovlap_negCnt=%d,rb_Lmv_negCnt =%d, \n",
					band_Type,u1_n2su_en,u1_su2n_en,u1_n2sd_en,u1_sd2n_en,u16_smv_near0_cnt,u16_cnt_totalNum,u16_rb_smv_posCnt,u16_rb_smv_negCnt,u16_ovlap_posCnt,u16_rb_Lmv_posCnt,u16_ovlap_negCnt,u16_rb_Lmv_negCnt);
		...
		if(u32_ID_Log_en2==1)
			{
				if(u8_lbmc_mode==1)
					rtd_pr_memc_debug("[MEMC YE_lbmc_mode] , u8_lbmc_mode=%d , \n", u8_lbmc_mode  );
...
#if RTK_MEMC_Performance_tunging_from_tv001
	if(log_en1){
		rtd_pr_memc_notice( "[%s] type,%d, lf,%d, hf,%d,\n", __FUNCTION__, band_Type, pOutput->u8_lf_lbmcMode, pOutput->u8_hf_lbmcMode);
	}
#endif
