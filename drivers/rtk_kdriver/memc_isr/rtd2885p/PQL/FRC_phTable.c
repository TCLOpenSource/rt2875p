...
extern bool Get_NewGameMode_Reset_PhaseTable_Flag(void);
extern void Set_NewGameMode_Reset_PhaseTable_Flag(bool enable);
extern bool Get_NewGameMode_To_Noraml_Reset_PhaseTable_Flag(void);
extern void Set_NewGameMode_To_Noraml_Reset_PhaseTable_Flag(bool enable);
...
///////////////////////////end
	...
	GMV_avg = (GMV_avg>>5);
#endif
	//rtd_pr_memc_debug("(%d)gmv_record = %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d..(%d,%d,%d)\n",
...
//////////////
	...
	u32_DBM_table[0][0]  =(u32_dejudder_0)&0x000000FF; 		//dejuddefr for L_G control point 1;
	u32_DBM_table[0][1] = (u32_dejudder_1)&0x000000FF; 		//dejuddefr for L_G control point 1
	u32_DBM_table[0][2] = (u32_dejudder_2)&0x000000FF; 		//dejuddefr for L_G control point 2
	u32_DBM_table[0][3] = (u32_dejudder_3)&0x000000FF; 		//dejuddefr for L_G control point 3
	u32_DBM_table[0][4] = (u32_dejudder_4)&0x000000FF; 		//dejuddefr for L_G control point 4
	//u32_DBM_table[0][5] = u32_dejudder_Value_Max;
	u32_DBM_table[1][0]  =(u32_gmv_0)&0x0000FFFF;			//gmv for for L_G control point 1
	u32_DBM_table[1][1] = (u32_gmv_1)&0x0000FFFF;			//gmv for for L_G control point 1
	u32_DBM_table[1][2] = (u32_gmv_2)&0x0000FFFF;			//gmv for for L_G control point 2
	u32_DBM_table[1][3] = (u32_gmv_3)&0x0000FFFF;			//gmv for for L_G control point 3
	u32_DBM_table[1][4] = (u32_gmv_4)&0x0000FFFF;			//gmv for for L_G control point 4
	//u32_DBM_table[1][5] = u32_GMV_Value_Max;

	u32_dejudder_0  =u32_DBM_table[0][0];					//dejuddefr for L_G control point 1;
	u32_dejudder_1  =u32_DBM_table[0][1];		//dejuddefr for L_G control point 1
	u32_dejudder_2  =u32_DBM_table[0][2];		//dejuddefr for L_G control point 2
	u32_dejudder_3  =u32_DBM_table[0][3];		//dejuddefr for L_G control point 3
	u32_dejudder_4  =u32_DBM_table[0][4];		//dejuddefr for L_G control point 4
	//u32_DBM_table[0][5] = u32_dejudder_Value_Max;
	u32_gmv_0  =  u32_DBM_table[1][0];			//gmv for for L_G control point 1
	u32_gmv_1  =  u32_DBM_table[1][1];			//gmv for for L_G control point 1
	u32_gmv_2  =  u32_DBM_table[1][2];			//gmv for for L_G control point 2
	u32_gmv_3  =  u32_DBM_table[1][3];			//gmv for for L_G control point 3
	u32_gmv_4  =  u32_DBM_table[1][4];			//gmv for for L_G control point 4
	...
	//rtd_pr_memc_debug("%d, dejudder_lvl = %d\n",u32_DBM_dejudder,dejudder_lvl);
	//debug print
...
VOID FRC_phTable_set_LineModeCtrl(VOID)
{
	...
	#if !(IC_K24)
	static unsigned char  LbmcTrig_status = 0;
	...
	unsigned char u1_vdec_direct_lowdelay_flag = drv_memory_get_vdec_direct_low_latency_mode(SLR_MAIN_DISPLAY) | drv_memory_get_vdec_direct_low_latency_mode(SLR_SUB_DISPLAY);
...
unsigned int FRC_phTable_UltraLowDelayMode_Target(VOID)//by jerry
{
...
VOID FRC_phTable_StateMachine(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
...
#else
	unsigned char u1_UI_GameMode = (MEMC_Lib_get_Vpq_LowDelay_to_MEMC_flag() == TRUE) ? 1 : 0;
#endif
	
	unsigned char u1_UI_GameMode_chg = (u1_UI_GameMode == pOutput->u1_UI_GameMode_state) ? 0 : 1;
	//unsigned char u1_inframe_chg = (in_fmRate == pOutput->u1_lowdelay_inframe) ? 0 : 1;//k23
	...
	bool u1_QS_PCMode = MEMC_Lib_Get_QS_PCmode_Flg();
	static bool u1_QS_PCMode_pre = false;
	bool u1_PCMode_chg = (u1_QS_PCMode == u1_QS_PCMode_pre) ? 0 : 1;
	...
	unsigned int log_en = 0;
	static unsigned int time_0 = 0;
	static unsigned int frm_cnt = 0;
	unsigned int test_en = 0, test_en2 = 0, test_en3 = 0;
	unsigned int force_lowdelay_en = 0, force_lowdelay_type = 0;


	unsigned int force_repeat_en = 0, force_repeat_type = 0;
	...
	ReadRegister(SOFTWARE2_SOFTWARE2_63_reg, 29, 29, &test_en2);
	ReadRegister(SOFTWARE2_SOFTWARE2_63_reg, 28, 28, &test_en3);
	ReadRegister(SOFTWARE1_SOFTWARE1_45_reg, 0, 31, &u32_value);
...
#if RTK_MEMC_Performance_tunging_from_tv001 //newgamemode_config	
	if(Get_NewGameMode_To_Noraml_Reset_PhaseTable_Flag()==true){
		Set_NewGameMode_To_Noraml_Reset_PhaseTable_Flag(false);
	}
#endif
...
#else
	u1_direct_media_flag = 0;//u1_direct_media_flag = get_scaler_run_cloud_game();
	u1_vdec_direct_lowdelay_flag = drv_memory_get_vdec_direct_low_latency_mode();
#endif
...
#if RTK_MEMC_Performance_tunging_from_tv001 //newgamemode_config
			if(Get_NewGameMode_Reset_PhaseTable_Flag()==true){
				Set_NewGameMode_Reset_PhaseTable_Flag(false);
			}
#endif
		}
	...
	// Q-16919 test
	if((rtd_inl(TIMER_SCPU_CLK90K_LO_reg) - time_0) >= 90000){
		static unsigned char status = 0;
		time_0 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
		if(test_en){
			if(frm_cnt % 120 < 60){
				LowDelay_mode = 1;
				u1_LowDelayMode_chg = (u1_isLowDelayMode == pOutput->u1_lowdelaystate) ? 0 : 1;
			}
			else{
				LowDelay_mode = 0;
				u1_LowDelayMode_chg = (u1_isLowDelayMode == pOutput->u1_lowdelaystate) ? 0 : 1;
...
//		ReadRegister(SOFTWARE1_SOFTWARE1_63_reg, 0, 31, &u32_low_delay);
//		if(((u32_low_delay >> 1) & 1) == 1){
//			pOutput->u1_lowdelay_regen = 1;
//			WriteRegister(SOFTWARE1_SOFTWARE1_63_reg, 1, 1, 0);
//		}
//		u32_low_delay = (u32_low_delay>>4) & 0x00000ff;
		...
		if(u1_inframe_chg == 1){
			if(u32_log_flag==1)
			{
				rtd_pr_memc_emerg("$$$   hi_LowDelay_InFrameRate_Chg = (%d)(%d)   $$$\n\r", in_fmRate, pOutput->u1_inframe_pre);
			}
		}
		// u1_source_type change
		if(u1_source_type_chg == 1){
			rtd_pr_memc_emerg("$$$   hi_source_type_chg = (%d)(%d)   $$$\n\r", u8_source_type, pOutput->u8_source_type_pre);
		}
...
#else
		pOutput->u1_generate_PC_en    =  (u1_outMode_chg || u1_n2m_switch || pOutput->u8_system_start_lock == 0 || (u1_LowDelayMode_chg == 1) || (u1_UI_GameMode_chg == 1) || u1_PCMode_chg)? 1 : 0;
...
#else		
			rtd_pr_memc_emerg("[u1_generate_video_en](%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)(%d)(%d)\n\r",
				pOutput->u1_lowdelay_regen, u1_outMode_chg, u1_n2m_switch, u1_in3dFormat_chg,
				u1_out3dFormat_chg, pOutput->u8_system_start_lock, u1_LowDelayMode_chg, u1_inframe_chg,
				u8_LowDelay_cnt, in_fmRate, out_fmRate, u1_isLowDelayMode,
				LowDelayGen_cnt, u1_source_type_chg, VR360_en_chg, u1_adaptivestream_flag_chg,
				u1_lowdelay_state_chg_flg, u1_direct_media_flag_chg, u1_UI_GameMode_chg, u1_vdec_direct_lowdelay_flag_chg,
				timing_unstable_flag, QMS_24hz_state_chg, qms_chg);
...
#if 1//[V]#if RTK_MEMC_Performance_tunging_from_tv001//[-]//henry
	...
	// update states
	{
		...
		//pOutput->u1_lowdelay_inframe = in_fmRate;//k23
...
VOID FRC_phTable_Generate(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
...
#if 1//#if RTK_MEMC_Performance_tunging_from_tv001//xj///寫法不同但通用
...
#ifdef new_delay
	unsigned int u32BP, u32SysDly = 0;
#if 1//#if RTK_MEMC_Performance_tunging_from_tv001//xj//寫法不同但通用
	unsigned int inVtotal = 0;
#endif
	...
	if(pOutput->u1_generate_video_en == 1 || pOutput->u1_generate_film_en == 1)
	{
		...
		unsigned int VTotal;
#ifdef new_delay
		unsigned int inVtotal;
#endif
		//RTKReadRegister(PPOVERLAY_memcdtg_DV_TOTAL_reg, &VTotal);		
		rtd_pr_memc_notice( "memcdtgVTotal = [,%d,]\r\n", scalerdisplay_get_DTG_MASTER_vtotal_by_vfreq());
