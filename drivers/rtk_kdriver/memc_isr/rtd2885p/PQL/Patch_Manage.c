...
VOID  patch_BluceMic_detect(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	...
	for(i = 0; i < 32; i++)
	{
		...
		(s_pContext->_output_rimctrl.u12_Rim_Result[_RIM_BOT] == (MEMC_PARAM_TABLE.RIM_Param[output_Resolution].RimCtrl_Param.s16_resolution_height-1)) &&
		(s_pContext->_output_rimctrl.u12_Rim_Result[_RIM_LFT] == 0) &&
		(s_pContext->_output_rimctrl.u12_Rim_Result[_RIM_RHT] == (MEMC_PARAM_TABLE.RIM_Param[output_Resolution].RimCtrl_Param.s16_resolution_width-1)) &&	// no rim
...
VOID  patch_HighFreqPlayerOSD_detect(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	...
	unsigned char u1_mc_rdtl_true = 0, mc_rdtl_cnt = 0, u1_me_sad_true = 0, me_sad_cnt = 0, i=0, j=0;
	unsigned int mc_rdtl_cnt_th = 0, nImageHsize = 0, nImageVsize = 0;
	unsigned int me_rAPL_rb_cnt = 0;
	unsigned int H_start = 0, H_end = 8, V_start = 0, V_end = 4;
	unsigned int H_res = rtd_inl(PPOVERLAY_MAIN_DEN_H_Start_End_reg);
	unsigned int V_res = rtd_inl(PPOVERLAY_MAIN_DEN_V_Start_End_reg);
	...
	if(H_res != 0)
	{
		H_start = (ppoverlay_main_active_h_start_end_reg.mh_act_sta+(H_res>>3)/2)/(H_res>>3);
		H_end = (ppoverlay_main_active_h_start_end_reg.mh_act_end+(H_res>>3)/2)/(H_res>>3);
	}
	if(V_res != 0)
	{
		V_start = (ppoverlay_main_active_v_start_end_reg.mv_act_sta+(V_res>>2)/2)/(V_res>>2);
		V_end = (ppoverlay_main_active_v_start_end_reg.mv_act_end+(V_res>>2)/2)/(V_res>>2);
	}
	for (j=V_start;j<V_end;j++)
	{
		...
		{
			me_rAPL_rb_cnt ++;
			if(s_pContext->_output_read_comreg.u10_mc_VHdtl_rb[8*j+i]<0x3)
				mc_rdtl_cnt++;
		}
	}
	mc_rdtl_cnt_th = (me_rAPL_rb_cnt*22)>>5;
	if(mc_rdtl_cnt>mc_rdtl_cnt_th)
		u1_mc_rdtl_true = 1;
...
#if 0
{
	...
	if(ip29)
	rtd_pr_memc_debug("sad %d %d.. %x %x %x %x %x %x %x %x .. %x %x %x %x %x %x %x %x.. %x %x %x %x %x %x %x %x..%x %x %x %x %x %x %x %x \n", mc_rdtl_cnt, me_sad_cnt, 
	...
	if(ip28)
	rtd_pr_memc_debug("dtl %d %d.. %x %x %x %x %x %x %x %x .. %x %x %x %x %x %x %x %x.. %x %x %x %x %x %x %x %x..%x %x %x %x %x %x %x %x \n", mc_rdtl_cnt, me_sad_cnt, 
	...
	if(ip27)
	rtd_pr_memc_debug("apl %d %d.. %x %x %x %x %x %x %x %x .. %x %x %x %x %x %x %x %x.. %x %x %x %x %x %x %x %x..%x %x %x %x %x %x %x %x \n", mc_rdtl_cnt, me_sad_cnt, 
...
VOID  patch_Soccer_action(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	...
	if(pParam->u1_patch_soccer_en == 0 && pParam->u1_sportscene_en == 0)
		return;
	
	if(pParam->u1_tri_ball_Issue_en ==0)
		return;
	
	if(pParam->u1_sportscene_en && s_pContext->_output_me_sceneAnalysis.u1_SportScene){
		WriteRegister(IPPRE_IPPRE_04_reg, 4, 11, 0xFF);  //255
		WriteRegister(IPPRE_IPPRE_04_reg, 16, 23, 110); //32

		WriteRegister(MC_MC_B4_reg, 16, 23, 0xe0);
		WriteRegister(MC_MC_B4_reg, 24, 31, 0xe0);
	}
	else if (pParam->u1_patch_soccer_en && s_pContext->_external_info.u8_Soccer_holdfrm > 0)
	{
...
//		WriteRegister(FRC_TOP__KME_DEHALO2__reg_dh_pfv_check_dif_gain_p_ADDR, 8, 14, 0x0);
	...
	{
		WriteRegister(IPPRE_IPPRE_04_reg, 4, 11, 0xFF); // 0xFF
		WriteRegister(IPPRE_IPPRE_04_reg, 16, 23, 0x20); // 0x20
...
VOID  patch_tri_ball_Issue_detect(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	...
	unsigned int log_en;
	ReadRegister(HARDWARE_HARDWARE_55_reg, 14, 14, &log_en);
	...
	if(u8_cadence_Id == _CAD_VIDEO && (in_fmRate == _PQL_IN_50HZ || in_fmRate == _PQL_IN_60HZ) && (out_fmRate == _PQL_OUT_100HZ || out_fmRate == _PQL_OUT_120HZ)){
		if( (s_pContext->_external_info.u8_triball_holdfrm > 0) && ( _ABS_(u11_gmv_mvx)>20 || _ABS_(u10_gmv_mvy)>20 || (_ABS_(u11_gmv_mvx)+_ABS_(u10_gmv_mvy))>=28) && (u32_gmv_cnt>=20000)){
	...
	{				
		if(s_pContext->_external_info.u8_triball_holdfrm > 0){
			if((_ABS_(u11_gmv_mvx)+_ABS_(u10_gmv_mvy))>=35){
				pOutput->u1_tri_ball_Detected = 5;
			}else{
	...
	if(log_en) { //d5dc[14]
		//rtd_pr_memc_emerg("[%s][%d] case>>%d\n", __FUNCTION__, __LINE__, pOutput->u1_tri_ball_Detected);
	}
}
...
VOID  patch_tri_ball_Issue_action(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
...
#if 0//[V]#if RTK_MEMC_Performance_tunging_from_tv001//xj//m7,k8參數不同
	
		switch(pOutput->u1_tri_ball_Detected){
			case 1:
				mvd_y_max = 224;
				mvd_y_min = 192;
				mc_mc_b8_reg.mc_sobj_en = 0; // K25804 : golfking_RTK_SmallBall
				break;
			case 2:
				mvd_y_max = 224;
				mvd_y_min = 128;	
				mc_mc_b8_reg.mc_sobj_en = 0;
				break;
			case 3:
				mvd_y_max = 224;
				mvd_y_min = 192;	
			...
			default:
				mvd_y_max = 224;
				mvd_y_min = 128;
				mc_mc_b8_reg.mc_sobj_en = 1; // K25804 : #288
				break;
		}
	...
	rtd_outl(MC_MC_B8_reg, mc_mc_b8_reg.regValue);
...
extern unsigned char Scaler_MEMC_Get_BypassToEnable_Flag(VOID);
extern VOID Scaler_MEMC_Set_EnableToBypass_Flag(unsigned char u1_enable);
extern unsigned char Scaler_MEMC_Get_EnableToBypass_Flag(VOID);
extern VOID Scaler_MEMC_Set_MEMC_FreezeStatus_Flag(unsigned char u1_enable);
extern unsigned char Scaler_MEMC_Get_MEMC_FreezeStatus_Flag(VOID);
#endif
...
//	unsigned int test_val, test_val2, test_val3, test_val4;
...
#if RTK_MEMC_Performance_tunging_from_tv001
	if(((Scaler_MEMC_Get_BypassToEnable_Flag() == 1) || (memc_switch_frm_cnt >= 2)) && (DTV_PowerSaving_stage == 0)){
		g_memc_switch_state = 1;
		checkcnt = 10;
	}else if((Scaler_MEMC_Get_EnableToBypass_Flag() == 1) || (DTV_PowerSaving_stage != 0)){
	...
	if(memc_switch_en == 1 && (g_memc_switch_state == 1))
	{
...
#if RTK_MEMC_Performance_tunging_from_tv001
		if(Scaler_MEMC_Get_BypassToEnable_Flag() == 1)
#else
...
#endif
		{
...
#if RTK_MEMC_Performance_tunging_from_tv001
			memc_switch_frm_cnt = frm_num + checkcnt;
			Scaler_MEMC_Set_BypassToEnable_Flag(0);
#else	
			...
			WriteRegister(MC_MC_28_reg, 14, 14, 0x1); //rtd_setbits(MC__MC_28_ADDR, _BIT14); //0xb8099628
...
#if RTK_MEMC_Performance_tunging_from_tv001
		else if(memc_switch_frm_cnt == (frm_num+checkcnt-1))
#else
...
#endif
		{
			if(VR360_en == 1){
				...
				if((u8_MEMCMode == 0) || (u8_MEMCMode == 4 && dejudder == 0) || (me_clk_state == 0)){
					...
					WriteRegister(MC_MC_28_reg, 14, 14, 0x0); //rtd_setbits(MC__MC_28_ADDR, _BIT14); //0xb8099628
...
#if RTK_MEMC_Performance_tunging_from_tv001
		else if(memc_switch_frm_cnt == frm_num-2 || (memc_switch_frm_cnt < (frm_num+checkcnt-1) && (me_clk_state == 1)))//[ML7CETOP-707]
#else
...
#if RTK_MEMC_Performance_tunging_from_tv001
		if((memc_switch_frm_cnt > 2) && (memc_switch_frm_cnt <= (frm_num+checkcnt)))
#else
...
#if RTK_MEMC_Performance_tunging_from_tv001
			Scaler_MEMC_Set_MEMC_FreezeStatus_Flag(0);
#else
...
#if RTK_MEMC_Performance_tunging_from_tv001
		Scaler_MEMC_Set_EnableToBypass_Flag(0);
#else
