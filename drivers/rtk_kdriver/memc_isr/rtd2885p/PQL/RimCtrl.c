...
extern unsigned int u32BLogo_RowNum;
unsigned short g_RightRim_16px = 0;
unsigned char g_RightRim_invalid_width = 0;
...
unsigned char u8_SlowIn_MinShiftX=0;
unsigned char u8_SlowIn_MinShiftY=0;
...
_Param_By_Resolution_RimCtrl Param_By_Resolution[_PQL_OUT_RESOLUTION_MAX] = {
	//                      width height    	bound(T,B,R,L)         divide	slowin_min_shift   RimDiffTh_cof	apl_size	scale(H,V)
	...
	/*_PQL_OUT_2560x1440*/	{2560, 1440,	{240, 1200, 2132,  428},	2, 1, 		16, 16, 			1, 			64800,		3, 3},
};

#else
const _Param_By_Resolution_RimCtrl Param_By_Resolution[_PQL_OUT_RESOLUTION_MAX] = {
	...
	/*_PQL_OUT_1280x720*/	{1280,  720,	{120,  600, 1066,  214},	1, 0, 		 8,  8, 			1, 			64800,		1, 1},
	/*_PQL_OUT_7680x4320*/	{7680, 4320,	{720, 3600, 6400, 1280},	4, 3, 		48, 48, 			2, 			64800,		8, 8},
	/*_PQL_OUT_2560x1440*/	{2560, 1440,	{240, 1200, 2100,  420},	3, 2, 		24, 24, 			1, 			64800,		3, 3},
};
...
VOID RimCtrl_OutResolution_Parse(_OUTPUT_RimCtrl *pOutput)
{
	...
	if (outResolution_cur != (PQL_OUTPUT_RESOLUTION)pOutput->u4_outResolution_pre)
	{
...
#if RTK_MEMC_Performance_tunging_from_tv001
		g_resolution_width = MEMC_PARAM_TABLE.RIM_Param[outResolution_cur].RimCtrl_Param.s16_resolution_width;
		g_resolution_height = MEMC_PARAM_TABLE.RIM_Param[outResolution_cur].RimCtrl_Param.s16_resolution_height;
		g_H_Divide = MEMC_PARAM_TABLE.RIM_Param[outResolution_cur].RimCtrl_Param.u8_H_Divide;
		g_V_Divide = MEMC_PARAM_TABLE.RIM_Param[outResolution_cur].RimCtrl_Param.u8_V_Divide;
		g_RightRim_invalid_width = (g_resolution_width==3840&&g_resolution_height==2160) ? 3 : 1;
#else
...
#if RTK_MEMC_Performance_tunging_from_tv001
		g_rim_bound[_RIM_LFT] = MEMC_PARAM_TABLE.RIM_Param[outResolution_cur].RimCtrl_Param.s16_rim_bound[_RIM_LFT];
		g_rim_bound[_RIM_RHT] = MEMC_PARAM_TABLE.RIM_Param[outResolution_cur].RimCtrl_Param.s16_rim_bound[_RIM_RHT];
		g_rim_bound[_RIM_TOP] = MEMC_PARAM_TABLE.RIM_Param[outResolution_cur].RimCtrl_Param.s16_rim_bound[_RIM_TOP];
		g_rim_bound[_RIM_BOT] = MEMC_PARAM_TABLE.RIM_Param[outResolution_cur].RimCtrl_Param.s16_rim_bound[_RIM_BOT];
		
		g_SlowIn_MinShiftX = MEMC_PARAM_TABLE.RIM_Param[outResolution_cur].RimCtrl_Param.u8_SlowIn_MinShiftX;
		g_SlowIn_MinShiftY = MEMC_PARAM_TABLE.RIM_Param[outResolution_cur].RimCtrl_Param.u8_SlowIn_MinShiftY;
		g_RimDiffTh_cof = MEMC_PARAM_TABLE.RIM_Param[outResolution_cur].RimCtrl_Param.u8_RimDiffTh_cof;
				g_apl_size = MEMC_PARAM_TABLE.RIM_Param[outResolution_cur].RimCtrl_Param.u32_apl_size;
#else
		...
		rtd_pr_memc_err("[%s][%d][%d,%d][%d,%d]\n", __FUNCTION__, outResolution_cur, g_resolution_width, g_resolution_height, g_H_Divide, g_V_Divide);
	}
}

unsigned int SetSearchLine(unsigned int H_line_search_position, unsigned short MinVal, unsigned short MaxVal)
{
	unsigned int u1_YUV_Self_Calib = 0;
	ReadRegister(SOFTWARE2_SOFTWARE2_09_reg, 25, 25, &u1_YUV_Self_Calib);

	H_line_search_position=_CLIP_(H_line_search_position,MinVal,MaxVal);
	if(u1_YUV_Self_Calib==0){
...
VOID RimDet_by_KMC_YUV_Meter(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	ReadRegister(HARDWARE_HARDWARE_43_reg,7,7, &log_en); /*0xB809D5AC*/
...
#else
		//ReadRegister(KMC_METER_TOP_KMC_METER_TOP_80_reg + 4 * u32_i, 0, 24, &u32_rb_val);
		u32_rb_val = s_pContext->_output_read_comreg.u32_yuv_meter[u32_i];
#endif
...
#else
		apl_pre[u32_i] =  s_pContext->_output_read_comreg.u20_me_rAPLp_rb[u32_i];
		apl[u32_i] =  s_pContext->_output_read_comreg.u20_me_rAPLi_rb[u32_i];
		sum_apl = _ABS_DIFF_(apl_pre[u32_i], apl[u32_i])+sum_apl;
#endif
	}
...
#else
	//statis histogram is invaild at rightest two lines in 4k
	g_RightRim_16px = (g_RightRim_16px>=h_line_total-g_RightRim_invalid_width) ? h_line_total-1 : g_RightRim_16px;
#endif
...
VOID RimCtrl_Boundary_Protect(unsigned short *u12_Rim_source, unsigned short *u12_Rim_Result, _OUTPUT_RimCtrl *pOutput)
{
	...
	unsigned char  apl_avg = 0, u8_k = 0, dark_score = 0;
	unsigned int u1_Protect_disable = 0, log_en = 0;

#if RTK_MEMC_Performance_tunging_from_tv001
	unsigned char u8_darklevel = 0;
	unsigned int u1_apl_protect_en;
	ReadRegister(HARDWARE_HARDWARE_43_reg,19,19, &u1_apl_protect_en);
#endif

	ReadRegister(HARDWARE_HARDWARE_43_reg, 3, 3, &u1_Protect_disable);
	ReadRegister(HARDWARE_HARDWARE_43_reg, 4, 4, &log_en);
	dark_score = s_pContext->_output_me_sceneAnalysis.u1_Brightness_Chg_Cnt;
...
#else
		if (apl_avg < 0x10 && dark_score > 10) // use dark_score for #231
#endif	
		{
			u12_Rim_Result[_RIM_LFT] =  pOutput->u12_out_resolution[_RIM_LFT];
			u12_Rim_Result[_RIM_RHT] =  pOutput->u12_out_resolution[_RIM_RHT];
...
#else
	rtd_pr_memc_err("[%s][T,B,L,R] Src>>[%d,%d,%d,%d] Result>>[%d,%d,%d,%d] bound>>[%d,%d,%d,%d] apl>>%d\n", __FUNCTION__,
					u12_Rim_source[_RIM_TOP], u12_Rim_source[_RIM_BOT], u12_Rim_source[_RIM_LFT], u12_Rim_source[_RIM_RHT],
					u12_Rim_Result[_RIM_TOP], u12_Rim_Result[_RIM_BOT], u12_Rim_Result[_RIM_LFT], u12_Rim_Result[_RIM_RHT],
					u12_rim_bound[_RIM_TOP], u12_rim_bound[_RIM_BOT], u12_rim_bound[_RIM_LFT], u12_rim_bound[_RIM_RHT], apl_avg);
#endif
...
VOID RimCtrl_HW_extract(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	unsigned short u12_det_Rim[_RIM_NUM], u12_rim_bound[_RIM_NUM], u12_det_Rim_ori[2], u12_shrink_value;
...
#if RTK_MEMC_Performance_tunging_from_tv001
	unsigned short u16_RimDiffTh = pParam->u12_RimDiffTh; //pParam->u12_RimDiffTh * g_RimDiffTh_cof;
#else
	...
	unsigned int log_en2 = 0, log_en3 = 0, log_en4 = 0, log_en5 = 0, u1_pan_protect = 0, debug_en = 0, u8_unconf_th, debug_val[4];

#if RTK_MEMC_Performance_tunging_from_tv001	
	unsigned int log_en = 0;	
	ReadRegister(SOFTWARE2_SOFTWARE2_61_reg, 30, 30, &log_en);	
	ReadRegister(SOFTWARE2_SOFTWARE2_58_reg, 0, 15, &debug_val[_RIM_TOP]);
	ReadRegister(SOFTWARE2_SOFTWARE2_58_reg, 16, 31, &debug_val[_RIM_BOT]);
	ReadRegister(SOFTWARE2_SOFTWARE2_59_reg, 0, 15, &debug_val[_RIM_RHT]);
	ReadRegister(SOFTWARE2_SOFTWARE2_59_reg, 16, 31, &debug_val[_RIM_LFT]);
#else
	ReadRegister(SOFTWARE2_SOFTWARE2_58_reg, 0, 15, &debug_val[0]); // TOP
	ReadRegister(SOFTWARE2_SOFTWARE2_58_reg, 16, 31, &debug_val[1]); // BOT
	ReadRegister(SOFTWARE2_SOFTWARE2_59_reg, 0, 15, &debug_val[2]); // RHT
	ReadRegister(SOFTWARE2_SOFTWARE2_59_reg, 16, 31, &debug_val[3]); // LFT
#endif
	...
	ReadRegister(SOFTWARE2_SOFTWARE2_56_reg, 1, 4, &debug_en);
	ReadRegister(SOFTWARE2_SOFTWARE2_56_reg,16,19, &u8_unconf_th);
...
#if RTK_MEMC_Performance_tunging_from_tv001
		if(((debug_en>>u8_k)&0x1) == 1){
			u12_det_Rim[u8_k] = debug_val[u8_k];
		}
		
		if(u8_k == _RIM_RHT){
			/*if((u12_det_Rim[_RIM_RHT]>=3823 && u12_rim_bound[_RIM_RHT] == 3200)
				|| (u12_det_Rim[_RIM_RHT]>=1903 && u12_rim_bound[_RIM_RHT] == 1600))*/
	...
	if(u1_pan_protect && u2_Panning_flg!=Pan_Dir_None && s_pContext->_output_read_comreg.u12_me_GMV_1st_unconf_rb<u8_unconf_th) {
		if(u2_Panning_flg==Pan_Dir_X) {
			u12_shrink_value = (g_resolution_width+512)>>10; //3840:4, 1920:2
			if(u11_gmv_mvx>0) {
				//u12_det_Rim[_RIM_LFT] = (u12_det_Rim[_RIM_LFT] < 1) ? 0 : u12_det_Rim[_RIM_LFT]-1;
				u12_det_Rim[_RIM_RHT] = u12_det_Rim[_RIM_RHT] - u12_shrink_value;
			}
			else {
				u12_det_Rim[_RIM_LFT] = u12_det_Rim[_RIM_LFT] + u12_shrink_value;
			...
			u12_shrink_value = (g_resolution_height+512)>>10; //2160:2, 1080:1
			if(u10_gmv_mvy>0) {
				//u12_det_Rim[_RIM_TOP] = (u12_det_Rim[_RIM_TOP] < 1) ? 0 : u12_det_Rim[_RIM_TOP]-1;
				u12_det_Rim[_RIM_BOT] = u12_det_Rim[_RIM_BOT] - u12_shrink_value;
			}
			else {
				u12_det_Rim[_RIM_TOP] = u12_det_Rim[_RIM_TOP] + u12_shrink_value;
				// 2_det_Rim[_RIM_BOT] = (u12_det_Rim[_RIM_BOT]+1 < u12_rim_bound[_RIM_BOT]) ? u12_rim_bound[_RIM_BOT] : u12_det_Rim[_RIM_BOT]+1;
			}
			g_MC_Shrink_V = 4;
			g_MC_Shrink_H = 0;
		}
	...
	*/
	...
	// Hortizontal
	/*
	if ((pOutput->u12_out_resolution[_RIM_RHT] - u12_det_Rim[_RIM_RHT]) > (u12_det_Rim[_RIM_LFT] + u16_RimDiffTh))
	{
		u12_det_Rim[_RIM_RHT] = (pOutput->u12_out_resolution[_RIM_RHT] - u12_det_Rim[_RIM_LFT]);
	}
	else if (u12_det_Rim[_RIM_LFT] > (pOutput->u12_out_resolution[_RIM_RHT] - u12_det_Rim[_RIM_RHT] + u16_RimDiffTh))
	{
		u12_det_Rim[_RIM_LFT] = (pOutput->u12_out_resolution[_RIM_RHT] - u12_det_Rim[_RIM_RHT]);
	}
	*/
	...
	if(log_en3){//d5ac[8]
		...
		rtd_pr_memc_err("res_RHT_size >> %d,%d,%d\n", u12_det_Rim[_RIM_RHT],pOutput->u12_out_resolution[_RIM_RHT], (pOutput->u12_out_resolution[_RIM_RHT] - u12_det_Rim[_RIM_RHT]));
		rtd_pr_memc_err("===== rim_cur =========\n");
		rtd_pr_memc_err("u12_Rim_Cur >> [%d,%d,%d,%d]\n", pOutput->u12_Rim_Cur[_RIM_TOP], pOutput->u12_Rim_Cur[_RIM_BOT], pOutput->u12_Rim_Cur[_RIM_RHT], pOutput->u12_Rim_Cur[_RIM_LFT]);
		rtd_pr_memc_err("u12_Rim_Pre >> [%d,%d,%d,%d]\n", pOutput->u12_Rim_Pre[_RIM_TOP], pOutput->u12_Rim_Pre[_RIM_BOT], pOutput->u12_Rim_Pre[_RIM_RHT], pOutput->u12_Rim_Pre[_RIM_LFT]);
#endif
...
#else
		rtd_pr_memc_err("[%s][T,B,L,R] Target>>[%d,%d,%d,%d] Result>>[%d,%d,%d,%d] Cnt>>[%d,%d,%d,%d] SC>>%d,%d Th>>%d,%d,%d	flash>>%d Pan>>%d diff>>%d\n", __FUNCTION__,
			pOutput->u12_Rim_Target[_RIM_TOP], pOutput->u12_Rim_Target[_RIM_BOT], pOutput->u12_Rim_Target[_RIM_LFT], pOutput->u12_Rim_Target[_RIM_RHT],
			pOutput->u12_Rim_Result[_RIM_TOP], pOutput->u12_Rim_Result[_RIM_BOT], pOutput->u12_Rim_Result[_RIM_LFT], pOutput->u12_Rim_Result[_RIM_RHT],
			pOutput->s8_SmallerCnt[_RIM_TOP], pOutput->s8_SmallerCnt[_RIM_BOT], pOutput->s8_SmallerCnt[_RIM_LFT], pOutput->s8_SmallerCnt[_RIM_RHT],
			sc_cnt, u8_sc_cnt, u8_dyn_thr, pParam->u8_RimSmallerSCTh, pParam->u8_RimSmallerTh, k23755_flash_flg, u2_Panning_flg, s16_rim_diff);
#endif
...
#endif
	{
		if(me1_blkV_Resolution > pOutput->u12_me1_vbuf_blk_rim[_RIM_BOT]){
			pOutput->u12_me1_vbuf_meander_blk_rim[_RIM_TOP] = me1_blkV_Resolution - pOutput->u12_me1_vbuf_blk_rim[_RIM_BOT];
		}else{
			pOutput->u12_me1_vbuf_meander_blk_rim[_RIM_TOP] = 0;
		}
...
#endif
	{
		if(me1_pixV_Resolution > pOutput->u12_me1_vbuf_pix_rim[_RIM_BOT]){		
			pOutput->u12_me1_vbuf_meander_pix_rim[_RIM_TOP] = me1_pixV_Resolution - pOutput->u12_me1_vbuf_pix_rim[_RIM_BOT];

		}else{
			pOutput->u12_me1_vbuf_meander_pix_rim[_RIM_TOP] = 0;
...
VOID RimCtrl_ME2_Rim_Convert(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
...
#if RTK_MEMC_Performance_tunging_from_tv001
	if(Scaler_MEMC_GetPanelSizeByDisp() == MEMC_SIZE_2K && pOutput->u12_me2_vbuf_pix_rim[_RIM_TOP] >= 4)
#else
	if(Get_DISPLAY_PANEL_TYPE() == P_LVDS_2K1K && pOutput->u12_me2_vbuf_pix_rim[_RIM_TOP] >= 4)
#endif	
	{
		pOutput->u12_me2_vbuf_pix_rim[_RIM_TOP] = pOutput->u12_me2_vbuf_pix_rim[_RIM_TOP] - 4;
	}
	...
	if(Scaler_MEMC_GetPanelSizeByDisp() == MEMC_SIZE_2K)
#else
	if(Get_DISPLAY_PANEL_TYPE() == P_LVDS_2K1K)//for K24905 #304 	
#endif
	{ 
		pOutput->u12_me2_vbuf_ph_blk_rim[_RIM_TOP] = (pOutput->u12_Rim_Result[_RIM_TOP]+4) >> pOutput->u8_me2_BlkV_sft_bit;
	}else{
...
#if 0//#if RTK_MEMC_Performance_tunging_from_tv001//xj //effect check Rim boundary test
	pOutput->u8_LogoRim_Blk[_RIM_BOT] = _CLIP_(pOutput->u8_LogoRim_Blk[_RIM_BOT], 0, 132);
#else
	pOutput->u8_LogoRim_Blk[_RIM_BOT] = _CLIP_(pOutput->u8_LogoRim_Blk[_RIM_BOT], 0, u32BLogo_RowNum-1);
#endif
...
VOID RimCtrl_Output(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	if (pOutput->u1_RimChange == 1)
	{
		...
		//rtd_pr_memc_debug("u8_me1_pixH_sft_bit = %d, u8_me1_pixV_sft_bit = %d\r\n",pOutput->u8_me1_pixH_sft_bit,pOutput->u8_me1_pixV_sft_bit);
		//rtd_pr_memc_debug("u8_me1_blkH_sft_bit = %d, u8_me1_blkV_sft_bit = %d\r\n",pOutput->u8_me1_blkH_sft_bit,pOutput->u8_me1_blkV_sft_bit);
		//rtd_pr_memc_debug("u8_me2_pixH_sft_bit = %d, u8_me2_pixV_sft_bit = %d\r\n",pOutput->u8_me2_pixH_sft_bit,pOutput->u8_me2_pixV_sft_bit);
		//rtd_pr_memc_debug("u8_me2_BlkH_sft_bit = %d, u8_me2_BlkV_sft_bit = %d\r\n",pOutput->u8_me2_BlkH_sft_bit,pOutput->u8_me2_BlkV_sft_bit);
		//rtd_pr_memc_debug("u8_dh_BlkH_sft_bit = %d, u8_dh_BlkV_sft_bit = %d\r\n",pOutput->u8_dh_BlkH_sft_bit,pOutput->u8_dh_BlkV_sft_bit);
		//rtd_pr_memc_debug("u8_logo_pixH_sft_bit = %d, u8_logo_pixV_sft_bit = %d\r\n",pOutput->u8_logo_pixH_sft_bit,pOutput->u8_logo_pixV_sft_bit);
		//rtd_pr_memc_debug("u8_logo_blkH_sft_bit = %d, u8_logo_blkV_sft_bit = %d\r\n",pOutput->u8_logo_blkH_sft_bit,pOutput->u8_logo_blkV_sft_bit);
#endif
...
VOID Rim_check_protect(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	//rtd_pr_memc_debug("[Rim_check_protect] u1_RimChange  = %d\n",pOutput->u1_RimChange);
...
VOID RimCtrl_Init(_OUTPUT_RimCtrl *pOutput)
{
	...
	else if(Scaler_MEMC_GetPanelSizeByDisp() == MEMC_SIZE_4K){//else if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_4K2K){
		u16_Rim_hsize = 3839;
		u16_Rim_vsize = 2159;
	}
	else{ //8k
	...
	else if(/*Get_DISPLAY_PANEL_TYPE() == P_VBY1_3K1K*/0)
	{
		u16_Rim_hsize = 2560;
		u16_Rim_vsize = 1440;
	}
...
/*
	...
	unsigned int Res_V = (pOutput->u4_outResolution_pre == _PQL_OUT_1920x1080 || pOutput->u4_outResolution_pre == _PQL_OUT_3840x1080) ? 1080 : 
					(pOutput->u4_outResolution_pre == _PQL_OUT_1920x540 || pOutput->u4_outResolution_pre == _PQL_OUT_3840x540) ? 540 : 
					(pOutput->u4_outResolution_pre == _PQL_OUT_768X480) ? 480 : 
					(pOutput->u4_outResolution_pre == _PQL_OUT_1280X720) ? 720 : 
					(pOutput->u4_outResolution_pre == _PQL_OUT_7680X4320) ? 4320 : 
					(pOutput->u4_outResolution_pre == _PQL_OUT_2560x1440) ? 1440 : 2160;		
*/
	unsigned char BOT_Comp_value = 0, TOP_Comp_value = 0, u8_info_id = 0;
	static unsigned char sc_cnt = 0;
	unsigned int LFT_ratio = 0, RHT_ratio = 0, u1_Algo_en = 0, L_diff = 0, R_diff = 0;
	static unsigned char Comp_R_cnt = 0, Comp_L_cnt = 0;
#if 1
	...
	ReadRegister(HARDWARE_HARDWARE_43_reg, 6, 6, &log_en);/*0xB809D5B0*/
	ReadRegister(HARDWARE_HARDWARE_43_reg, 18, 18, &u1_Algo_en);/*0xB809D5B0*/
#endif
	...
	u32_ratio_temp_min = u32_ratio_temp_min>>g_V_Divide; //2160: g_V_Divide=2, 1080: g_V_Divide=1
	u32_ratio_temp_max = u32_ratio_temp_max>>g_V_Divide; 
	TOP_ratio = ((_ABS_DIFF_( pOutput->u12_Rim_Cur[_RIM_TOP] , pOutput->u12_out_resolution[_RIM_TOP]))<<10)>>(9+g_V_Divide); //2160: g_V_Divide=2, 1080: g_V_Divide=1
	...
	LFT_ratio = (_ABS_DIFF_( pOutput->u12_Rim_Cur[_RIM_LFT] , pOutput->u12_out_resolution[_RIM_LFT]))*100/pOutput->u12_out_resolution[_RIM_RHT];
	RHT_ratio = (_ABS_DIFF_( pOutput->u12_Rim_Cur[_RIM_RHT] , pOutput->u12_out_resolution[_RIM_RHT]))*100/pOutput->u12_out_resolution[_RIM_RHT];
	L_diff = (_ABS_DIFF_( pOutput->u12_Rim_Cur[_RIM_LFT] , pOutput->u12_out_resolution[_RIM_LFT]));
	R_diff = (_ABS_DIFF_( pOutput->u12_Rim_Cur[_RIM_RHT] , pOutput->u12_out_resolution[_RIM_RHT]));
...
#else
		if((_ABS_(ratio_diff)>4 && (TOP_ratio>BOT_ratio) && ((TOP_ratio<50 && TOP_ratio>30) && (BOT_ratio<50 && BOT_ratio>30)))|| pOutput->u1_RimTwoSide_balancing == 0){ // fine tune threshold for #109
			pOutput->u1_RimCompflag = 1;
			Comp_flag_cnt = 5;
			u8_info_id = 1;
		}else if(pOutput->u1_RimTwoSide_balancing == 2){
	...
	if(RHT_ratio > LFT_ratio){ // for #35 & K-23755
		if(Comp_R_cnt < 9)
			Comp_R_cnt++;
	}else{
		if(Comp_R_cnt > 0)
	...
	if(log_en){ //d5ac[6]
		...
		rtd_pr_memc_err("cur_V_size >> %d,%d,%d\n",_ABS_DIFF_(pOutput->u12_Rim_Cur[_RIM_BOT] , pOutput->u12_Rim_Cur[_RIM_TOP]),pOutput->u12_Rim_Cur[_RIM_BOT],pOutput->u12_Rim_Cur[_RIM_TOP]);
		rtd_pr_memc_err("cur_H_size >> %d,%d,%d\n",_ABS_DIFF_(pOutput->u12_Rim_Cur[_RIM_RHT] , pOutput->u12_Rim_Cur[_RIM_LFT]),pOutput->u12_Rim_Cur[_RIM_RHT],pOutput->u12_Rim_Cur[_RIM_LFT]);
		rtd_pr_memc_err("===== v ratio check =========\n");
		rtd_pr_memc_err("cur_Rim_ratio >> %d,%d,%d\n", TOP_ratio, center_V_ratio, BOT_ratio);
		rtd_pr_memc_err("pre_Rim_ratio >> %d,%d,%d\n", pre_TOP_ratio, pre_center_V_ratio, pre_BOT_ratio);
		rtd_pr_memc_err("===== h ratio check =========\n");
		rtd_pr_memc_err("H_Rim_ratio >> %d,%d,,%d,,%d\n", LFT_ratio, RHT_ratio, L_diff, Comp_R_cnt);
		rtd_pr_memc_err("===== rim result(FW) =========\n");	
		rtd_pr_memc_err("result_V_size >> %d,%d,%d\n",_ABS_DIFF_(pOutput->u12_Rim_Result[_RIM_BOT] , pOutput->u12_Rim_Result[_RIM_TOP]),pOutput->u12_Rim_Result[_RIM_BOT],pOutput->u12_Rim_Result[_RIM_TOP]);
		rtd_pr_memc_err("result_H_size >> %d,%d,%d\n",_ABS_DIFF_(pOutput->u12_Rim_Result[_RIM_RHT] , pOutput->u12_Rim_Result[_RIM_LFT]),pOutput->u12_Rim_Result[_RIM_RHT],pOutput->u12_Rim_Result[_RIM_LFT]);
		rtd_pr_memc_err("===== final check =========\n");		
		rtd_pr_memc_err("final_comp_rim >> %d, %d,%d, %d,%d, %d,%d\n", u8_info_id, pOutput->u1_RimCompflag, Comp_flag_cnt, TOP_Comp_value, BOT_Comp_value, pParam->u8_lbme_pixRim_comp,pOutput->u1_RimTwoSide_balancing);
		rtd_pr_memc_err("===== value write for register =========\n");
		rtd_pr_memc_err("mc_rim1_V_size >> %d,%d,%d\n",_ABS_DIFF_(pOutput->u12_mc_rim1[_RIM_BOT] , pOutput->u12_mc_rim1[_RIM_TOP]),pOutput->u12_mc_rim1[_RIM_BOT],pOutput->u12_mc_rim1[_RIM_TOP]);
		rtd_pr_memc_err("mc_rim1_H_size >> %d,%d,%d\n",_ABS_DIFF_(pOutput->u12_mc_rim1[_RIM_RHT] , pOutput->u12_mc_rim1[_RIM_LFT]),pOutput->u12_mc_rim1[_RIM_RHT],pOutput->u12_mc_rim1[_RIM_LFT]);
#endif
	...
	if(Comp_R_cnt >= 5 && u1_Algo_en == 1){
		pOutput->u12_Rim_Target[_RIM_RHT] = _MIN_( pOutput->u12_out_resolution[_RIM_RHT] - L_diff, pOutput->u12_out_resolution[_RIM_RHT]);
	}
	if(Comp_L_cnt >= 5 && u1_Algo_en == 1){
		pOutput->u12_Rim_Target[_RIM_LFT] = _MAX_( R_diff, pOutput->u12_out_resolution[_RIM_LFT]);
	}
...
VOID RimCtrl_SC_rim(_OUTPUT_RimCtrl *pOutput)
{
	...
	ReadRegister(HARDWARE_HARDWARE_43_reg, 13, 13, &log_en);/*0xB809D5B0*/
	...
	if(log_en){//d5ac[13]
#if RTK_MEMC_Performance_tunging_from_tv001
		rtd_pr_memc_debug(KERN_ERR, TAG_NAME_MEMC, "[%s][%d] SC_cnt>>%d check_rim_cnt>>%d SC>>[%u, %u]\n", __FUNCTION__, __LINE__, u8_SC_cnt, u8_check_rim_cnt, g_HisDiff, g_HueDiff);
#else
		rtd_pr_memc_debug(KERN_ERR, TAG_NAME_MEMC, "[%s][%d] SC_cnt>>%d check_rim_cnt>>%d	SC>>[%u, %u]	flash>>%d\n",	__FUNCTION__, __LINE__, u8_SC_cnt, u8_check_rim_cnt, g_HisDiff, g_HueDiff, k23755_flash_flg);
#endif	
	}
		...
		if(u8_check_rim_cnt<5){
			for (u8_k = 0; u8_k < _RIM_NUM; u8_k ++) {
				...
				if(_ABS_DIFF_(u12_Rim_HW[u8_k], u12_Rim_HW_check[u8_k])>16){
					u8_check_rim_cnt=0;
					if(log_en){//d5ac[13]
						rtd_pr_memc_debug(KERN_ERR, TAG_NAME_MEMC, "[%s][%d][T,B,L,R] HW RIM>>[%d,%d,%d,%d] RIM CHECK>>[%d,%d,%d,%d]\n", __FUNCTION__, __LINE__,
			...
			if(log_en){//d5ac[13]
				rtd_pr_memc_debug(KERN_ERR, TAG_NAME_MEMC, "[%s][%d] HW RIM>>[%d, %d, %d, %d] Set RIM>>[%d, %d, %d, %d]\n", __FUNCTION__, __LINE__,
					u12_Rim_HW[_RIM_TOP], u12_Rim_HW[_RIM_BOT], u12_Rim_HW[_RIM_LFT], u12_Rim_HW[_RIM_RHT],
					pOutput->u12_Rim_Result[_RIM_TOP], pOutput->u12_Rim_Result[_RIM_BOT], pOutput->u12_Rim_Result[_RIM_LFT], pOutput->u12_Rim_Result[_RIM_RHT]);
			}
		}
...
VOID RimCtrl_Proc(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	if (pParam->u1_RimCtrl_en == 1)
	{
		...
		if(log_Result_en[_RIM_RHT]){//d5b0[31]
			rtd_pr_memc_crit("[%s][u12_Rim_Pre][T,B,L,R]	>> [%d,%d,%d,%d]\n", __FUNCTION__, 
								pOutput->u12_Rim_Pre[_RIM_TOP], pOutput->u12_Rim_Pre[_RIM_BOT], pOutput->u12_Rim_Pre[_RIM_LFT], pOutput->u12_Rim_Pre[_RIM_RHT]);
		}
		...
		#if 0
		rtd_pr_memc_debug(KERN_NOTICE, TAG_NAME_MEMC, "[RimCtrl_Proc]horizatal_speed = %d,%d\r\n",s_pContext->_output_read_comreg.s11_me_GMV_1st_vx_rb);
		rtd_pr_memc_debug(KERN_NOTICE, TAG_NAME_MEMC, "[RimCtrl_Proc]vertical_speed = %d,%d\r\n",s_pContext->_output_read_comreg.s10_me_GMV_1st_vy_rb);		
		#endif
