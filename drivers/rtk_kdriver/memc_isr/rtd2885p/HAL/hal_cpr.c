...
{
	//reg_hact
	//WriteRegister(PQC_PQDC_MC_LF_PQ_CMP_PAIR_reg,0,11,u32Hactive);	// 2 port
	//reg_vact
	//WriteRegister(PQC_PQDC_MC_LF_PQ_CMP_PAIR_reg,16,27,u32Vactive);	// hf/lf row count
}
...
VOID HAL_CPR_Set444Enable(BOOL bEnable)//modify from merlin4, using RTK pqc
{
	...
	pqc_pqdc_mc_lf_pq_decmp_sat_en_RBUS pqc_pqdc_mc_lf_pq_decmp_sat_en_reg;
	...
	kme_pqc_pqdc_kme_pq_cmp_qp_st_RBUS kme_pqc_pqdc_kme_pq_cmp_qp_st_reg;
	kme_pqc_pqdc_kme_pq_cmp_fifo_st1_RBUS kme_pqc_pqdc_kme_pq_cmp_fifo_st1_reg;
	...
	pqc_pqdc_mc_lf_pq_decmp_sat_en_reg.regValue = rtd_inl(PQC_PQDC_MC_LF_PQ_DECMP_SAT_EN_reg);
...
#endif	
	rtd_outl(PQC_PQDC_MC_LF_PQ_DECMP_SAT_EN_reg,pqc_pqdc_mc_lf_pq_decmp_sat_en_reg.regValue);
...
#else
	kme_pqc_pqdc_kme_pq_cmp_enable_reg.cmp_ctrl_para0 = 0x10;
#endif	
...
#else
	kme_pqc_pqdc_kme_pq_decmp_sat_en_reg.decmp_ctrl_para = 0x4;
#endif
	rtd_outl(KME_PQC_PQDC_KME_PQ_DECMP_SAT_EN_reg, kme_pqc_pqdc_kme_pq_decmp_sat_en_reg.regValue);
	
	kme_pqc_pqdc_kme_pq_cmp_qp_st_reg.regValue = rtd_inl(KME_PQC_PQDC_KME_PQ_CMP_QP_ST_reg);
	kme_pqc_pqdc_kme_pq_cmp_qp_st_reg.cmp_ctrl_para1 = 0x44;
	kme_pqc_pqdc_kme_pq_cmp_qp_st_reg.cmp_ctrl_para2 = 0xd0;
	rtd_outl(KME_PQC_PQDC_KME_PQ_CMP_QP_ST_reg, kme_pqc_pqdc_kme_pq_cmp_qp_st_reg.regValue);
	
	kme_pqc_pqdc_kme_pq_cmp_fifo_st1_reg.regValue = rtd_inl(KME_PQC_PQDC_KME_PQ_CMP_FIFO_ST1_reg);
	kme_pqc_pqdc_kme_pq_cmp_fifo_st1_reg.cmp_ctrl_para3 = 0x0;
	rtd_outl(KME_PQC_PQDC_KME_PQ_CMP_FIFO_ST1_reg, kme_pqc_pqdc_kme_pq_cmp_fifo_st1_reg.regValue);
	
	if(bEnable){
		//rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_PAIR_reg,0xfffffffc,0x00000000); //444(bit:0)
		//rtd_maskl(PQC_PQDC_MC_HF_PQ_CMP_PAIR_reg,0xfffffffc,0x00000000); //444(bit:0)
		//rtd_maskl(PQC_PQDC_MC_LF_PQ_DECMP_PAIR_reg,0xfffffffc,0x00000000); //444(bit:0)
		...
		if(MEMC_Lib_get_INPUT_COLOR_SPACE_Mode()== 1){ //I3DDMA_COLOR_SPACE_T
			rtd_clearbits(PQC_PQDC_MC_LF_PQ_CMP_PAIR_reg, 0x11); //cmp_data_color(bit:0 & 4)
			rtd_clearbits(PQC_PQDC_MC_HF_PQ_CMP_PAIR_reg, 0x13); //cmp_data_color(bit:0 & 4)
			rtd_clearbits(PQC_PQDC_MC_LF_PQ_DECMP_PAIR_reg, 0x11); //cmp_data_color(bit:0 & 4)
			rtd_clearbits(PQC_PQDC_MC_HF_PQ_DECMP_PAIR_reg, 0x13); //cmp_data_color(bit:0 & 4)

			rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_ENABLE_reg,0xffffff7f,0x00000000); //first_line(bit:7)
			rtd_maskl(PQC_PQDC_MC_HF_PQ_CMP_ENABLE_reg,0xffffff7f,0x00000000); //first_line(bit:7)

			rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_ENABLE_reg,0xffffffcf,0x00000010); //444(bit:4~5)
			rtd_maskl(PQC_PQDC_MC_HF_PQ_CMP_ENABLE_reg,0xffffffcf,0x00000010); //444(bit:4~5)

			rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_ENABLE_reg,0xffe0ffff,0x000e0000); //gratio(bit:16~20)
			rtd_maskl(PQC_PQDC_MC_HF_PQ_CMP_ENABLE_reg,0xffe0ffff,0x000e0000); //gratio(bit:16~20)

			rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_ALLOCATE_reg,0x0000ffff,0x100d0000); //gratiomaxmin (bit:16~20, 24~28)
			...
			rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_ENABLE_reg,0xffffffcf,0x00000020); //444(bit:4~5)
			rtd_maskl(PQC_PQDC_MC_HF_PQ_CMP_ENABLE_reg,0xffffffcf,0x00000020); //444(bit:4~5)

			rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_ENABLE_reg,0xffe0ffff,0x000c0000); //gratio(bit:16~20)
			rtd_maskl(PQC_PQDC_MC_HF_PQ_CMP_ENABLE_reg,0xffe0ffff,0x000c0000); //gratio(bit:16~20)

			rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_ALLOCATE_reg,0x0000ffff,0x0d090000); //gratiomaxmin (bit:16~20, 24~28)
			rtd_maskl(PQC_PQDC_MC_HF_PQ_CMP_ALLOCATE_reg,0x0000ffff,0x0d090000); //gratiomaxmin (bit:16~20, 24~28)
		}
		rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_POOR_reg,0xffffff00,0x00000003); //LF_PQ_CMP_POOR
		rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_POOR_reg,0xffff00ff,0x00000600); //LF_PQ_CMP_POOR
		rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_POOR_reg,0xff00ffff,0x000c0000); //LF_PQ_CMP_POOR
		rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_POOR_reg,0x00ffffff,0x14000000); //LF_PQ_CMP_POOR
		...
		//== org setting===
		//rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_BIT_reg,0xffff0000,0x00001412); //flb (bit:0~5, 8~13)
		//rtd_maskl(PQC_PQDC_MC_HF_PQ_CMP_BIT_reg,0xffff0000,0x00001614); //flb (bit:0~5, 8~13)
		//== new setting  for BW saving==
		//rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_BIT_reg,0xffff0000,0x0000100e); //flb (bit:0~5, 8~13)
		//rtd_maskl(PQC_PQDC_MC_HF_PQ_CMP_BIT_reg,0xffff0000,0x0000110f); //flb (bit:0~5, 8~13)
		rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_BIT_reg,0xc0ff0000,0x08001a14); //flb (bit:0~5, 8~13)
	...
	else// if(GS_MC_8_buffer == 0)//Not 444, but 420 or 422
	{
		...
		rtd_setbits(PQC_PQDC_MC_LF_PQ_CMP_PAIR_reg, 0x11); //cmp_data_color(bit:0 & 4)
		...
		WriteRegister(PQC_PQDC_MC_LF_PQ_DECMP_PAIR_reg, 0, 0, 1);
		WriteRegister(PQC_PQDC_MC_LF_PQ_DECMP_PAIR_reg, 4, 4, 1);
		...
		rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_ENABLE_reg,0xffffff7f,0x00000080); //first_line(bit:7)
		rtd_maskl(PQC_PQDC_MC_HF_PQ_CMP_ENABLE_reg,0xffffff7f,0x00000080); //first_line(bit:7)

		rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_ENABLE_reg,0xffffffcf,0x00000000); //444(bit:4~5)
		rtd_maskl(PQC_PQDC_MC_HF_PQ_CMP_ENABLE_reg,0xffffffcf,0x00000000); //444(bit:4~5)

		rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_ENABLE_reg,0xffe0ffff,0x00100000); //gratio(bit:16~20)
		rtd_maskl(PQC_PQDC_MC_HF_PQ_CMP_ENABLE_reg,0xffe0ffff,0x00100000); //gratio(bit:16~20)

	#if (IC_K7LP || IC_K8LP || IC_K24)
		WriteRegister(PQC_PQDC_MC_LF_PQ_CMP_POOR_reg, 0, 4, 3);
		WriteRegister(PQC_PQDC_MC_LF_PQ_CMP_POOR_reg, 8, 12, 6);
		WriteRegister(PQC_PQDC_MC_LF_PQ_CMP_POOR_reg, 16, 20, 12);
		WriteRegister(PQC_PQDC_MC_LF_PQ_CMP_POOR_reg, 24, 28, 20);
		pqc_pqdc_mc_hf_pq_cmp_poor_reg.regValue = rtd_inl(PQC_PQDC_MC_HF_PQ_CMP_POOR_reg);
		pqc_pqdc_mc_hf_pq_cmp_poor_reg.poor_limit_th_qp1 = 3;
		pqc_pqdc_mc_hf_pq_cmp_poor_reg.poor_limit_th_qp2 = 6;
		pqc_pqdc_mc_hf_pq_cmp_poor_reg.poor_limit_th_qp3 = 12;
	...
	#else //K6LP
		WriteRegister(PQC_PQDC_MC_LF_PQ_CMP_POOR_reg, 0, 4, 1);
		WriteRegister(PQC_PQDC_MC_LF_PQ_CMP_POOR_reg, 8, 12, 3);
		WriteRegister(PQC_PQDC_MC_LF_PQ_CMP_POOR_reg, 16, 20, 6);
		WriteRegister(PQC_PQDC_MC_LF_PQ_CMP_POOR_reg, 24, 28, 12);
		pqc_pqdc_mc_hf_pq_cmp_poor_reg.regValue = rtd_inl(PQC_PQDC_MC_HF_PQ_CMP_POOR_reg);
		...
		if(MEMC_Lib_Get_JP_DTV_4K_flag())// PQC 8 bit
		{
			if((GS_MC_8_buffer == 0) || (GS_MC_8_buffer == 1 && MEMC_Lib_get_DisplayRefreshRate() > 60)){
				...
				WriteRegister(PQC_PQDC_MC_HF_PQ_DECMP_PAIR_reg, 0, 1, 3);
				WriteRegister(PQC_PQDC_MC_HF_PQ_DECMP_PAIR_reg, 4, 4, 1); 	
				rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_BIT_reg,0xc0ff0000,0x08000c09); //flb (bit:0~5, 8~13)
				rtd_maskl(PQC_PQDC_MC_HF_PQ_CMP_BIT_reg,0xc0ff0000,0x08000906); //flb (bit:0~5, 8~13)		
			}
		}
		else// PQC 10 bit
		{
			if(((GS_MC_8_buffer == 0) || (GS_MC_8_buffer == 1 && MEMC_Lib_get_DisplayRefreshRate() > 60)) && (get_product_type() != PRODUCT_TYPE_DIAS)){
				WriteRegister(PQC_PQDC_MC_HF_PQ_CMP_PAIR_reg, 0, 1, 1);
				WriteRegister(PQC_PQDC_MC_HF_PQ_CMP_PAIR_reg, 4, 4, 1);
				WriteRegister(PQC_PQDC_MC_HF_PQ_DECMP_PAIR_reg, 0, 1, 1);
				WriteRegister(PQC_PQDC_MC_HF_PQ_DECMP_PAIR_reg, 4, 4, 1);
				rtd_maskl(PQC_PQDC_MC_LF_PQ_CMP_BIT_reg,0xc0ff0000,0x20000e09); //flb (bit:0~5, 8~13)
		...
		//DB apply
		//WriteRegister(PQC_PQDC_MC_PQ_CMP_DB_CTRL_reg, 2, 2, 1);
#if IS_ENABLED(CONFIG_RTK_KDRV_TP)
	...
	//rtd_inl(PQC_PQDC_MC_LF_PQ_CMP_BIT_reg);
	MEMC_LibSet_PQC_DB_apply();
	
	rtd_pr_memc_notice( "[%s]lf_cmp_data_format=%d, lf_decmp_data_format=%d\n", __FUNCTION__, (rtd_inl(PQC_PQDC_MC_LF_PQ_CMP_PAIR_reg)&0x3), (rtd_inl(PQC_PQDC_MC_LF_PQ_DECMP_PAIR_reg)&0x3));
...
VOID HAL_CPR_SetBudgetPacket(unsigned int u32Budget_Packet_Lf, unsigned int u32Budget_Packet_Hf)
{
	...
	//WriteRegister(PQC_PQDC_MC_LF_PQ_CMP_ALLOCATE_reg,0,7,u32Budget_Packet_Lf);	
	//reg_hf_budget_packet
	//WriteRegister(PQC_PQDC_MC_LF_PQ_CMP_ALLOCATE_reg,16,23,u32Budget_Packet_Hf);		
}
...
{
	//reg_lr_inv
	//WriteRegister(PQC_PQDC_MC_LF_PQ_CMP_BIT_reg,12,12,(unsigned int)bEnable);	
}
