...
VOID Mid_CRTC_SetFRCTiming(OUTPUT_FORMAT emof, PANEL_PARAMETERS_ST stPanelPara)
{
	...
	unsigned int u32DEHALO_Vtrig_dly = 0;
	unsigned int u32MC_Vtrig_dly = 0;
	unsigned int u32FRC_Vtrig_dly = 0;
	unsigned int u32Val = 0;
	unsigned int u32RowSize = 0;
	#if 1  // K4Lp
	unsigned int u32DHRowSize = 0, u32ME2Extra_active;
...
VOID Mid_CRTC_SetPanelTiming(OUTPUT_FORMAT emof, PANEL_PARAMETERS_ST stPanelPara)
{
	...
	unsigned int u32ME_Vtrig_dly = 0;
	unsigned int u32MC_Vtrig_dly = 0;
	unsigned int u32Out_Vtrig_dly = 0;
								...
								OUTPUT_FORMAT enOutputFormat)
{
	...
	switch(enInputFrameRate)
	{
		case INPUT_FRAMERATE_24HZ:
		{
			switch(enOutputFrameRate)
			{
				...
				case OUTPUT_FRAMERATE_125HZ:
				{
					return FALSE;
				}
				case OUTPUT_FRAMERATE_144HZ:
				{
					u32N = 1;
					u32M = 6;
					break;
	...
	if((u32M % 2 != 0) && (enOutputFrameRate > OUTPUT_FRAMERATE_60HZ) && !(enInputFrameRate == INPUT_FRAMERATE_24HZ && enOutputFrameRate == OUTPUT_FRAMERATE_120HZ && get_scaler_qms_mode_flag() == TRUE))
	{
		bDouble = TRUE;
	}
...
//
	rtd_pr_memc_notice( "Mid crtc set N2M = %d:%d\r\n", u32N, u32M);
...
{
#if 0 // IC_K7LP
	unsigned char Normal_LF[4][9] = {{0x24, 0x24, 0x24, 0x24, 0x24, 0x24, 0x24, 0x24, 0x24},	// 1920x1080
	...
	unsigned char Normal_HF[4][4] = {{0x35, 0x35, 0x35, 0x36},		// 1920x1080
							 {0x46, 0x46, 0x46, 0x46},		// 3840x2160
							 {0x46, 0x46, 0x46, 0x46},		// 768x480
							 {0x46, 0x46, 0x46, 0x46}};		// 3840x2160

	unsigned char Single_LF[4][9] = {{0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11}, 	// 1920x1080
							 {0x44, 0x44, 0x44, 0x44, 0x44, 0x40, 0x36, 0x30, 0x28},	// 3840x2160
							 {0x44, 0x44, 0x44, 0x44, 0x44, 0x40, 0x36, 0x30, 0x28},	// 768x480
							 {0x44, 0x44, 0x44, 0x44, 0x44, 0x40, 0x36, 0x30, 0x28}};	// 3840x2160

	unsigned char Single_HF[4][4] = {{0x32, 0x32, 0x32, 0x32},		// 1920x1080
							 {0x46, 0x46, 0x46, 0x46},		// 3840x2160
							 {0x46, 0x46, 0x46, 0x46},		// 768x480
							 {0x46, 0x46, 0x46, 0x46}};		// 3840x2160
 ...
 * @retval VOID
*/
VOID Mid_CRTC_Set3DLR(BOOL bEnable, unsigned int u32Cycle, unsigned int u32Pattern)
{
...
VOID Mid_CRTC_Set_LBMC_neg_en(unsigned char u1_enable)
{
#if (IC_K24)
