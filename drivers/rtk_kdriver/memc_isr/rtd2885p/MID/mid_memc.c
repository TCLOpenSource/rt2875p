...
#ifdef BUILD_QUICK_SHOW
	s_pContext_m->_external_data.u8_deblur_lvl = (unsigned char)u16Level;
#else
...
#ifdef BUILD_QUICK_SHOW
	s_pContext_m->_external_data.u8_dejudder_lvl = (unsigned char)u16Level;
#else
...
#ifdef BUILD_QUICK_SHOW
	s_pContext_m->_external_data._stc_memc_module_ratio = Command._stc_memc_module_ratio;
#else
...
BOOL Mid_MEMC_SetMEMCFBLevel(unsigned short u16Level)
{
	...
	_PQL_Command Command;
#ifdef BUILD_QUICK_SHOW
	s_pContext_m->_external_data.u8_flbk_lvl = (unsigned char)u16Level;
#else
...
#ifdef BUILD_QUICK_SHOW
	s_pContext_m->_external_data.u8_pulldown_mode = mPullDownMode;
#else
	Command.u8_pulldown_mode= mPullDownMode;
//	rtd_pr_memc_debug("[MEMC]mPullDownMode = %d (InFR=%d, OutFR=%d, isMemcON=%d, isCinemaON=%d)\r\n",
