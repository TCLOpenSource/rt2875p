...
{
	HAL_POST_SetBlackInsertLineModeEnable(bEnable);//not used
}
...
{
	MEMC_instanboot_resume_check[5]=1;
	if(bEnable)
	{
		HAL_POST_SetOutINTSel(7);
		HAL_POST_SetOutINTSource(0);
	}
	HAL_POST_SetOutINTEnable((unsigned int)enIntType, bEnable);//b809D008, bit 5:8
...
{
	HAL_KPOST_TOP_SetOutINTWclr(enIntType, FALSE);//b809D008, bit 1:4
