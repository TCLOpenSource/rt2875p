...
static CFIFODMAStatus g_DMAStatus = {DMA_STATUS_MAX_IP,0};
/*------------------------------------------------------------------
                ...
                if(subip){
                    ...
                    if(i==DMA_STATUS_TVSB2){
                        if(j==TVSB2_SUB_IP_DI_W){
                            subip->_subCount = 2;
                            subip->_pDataHead = &tvsb2_diw_date_overflow;
                        }else if(j==TVSB2_SUB_IP_DI_R){
                            subip->_subCount = 2;
                            subip->_pDataHead = &tvsb2_dir_date_overflow;
                        ...
                        if(j==TVSB1_SUB_IP_VD){
                            subip->_subCount = 1;
                            subip->_pDataHead = &tvsb1_vd_over_under_flow;
                        }else if(j==TVSB1_SUB_IP_Audio1){
                            subip->_subCount = 1;
                            subip->_pDataHead = &tvsb1_audio_overflow;
                        }else if(j==TVSB1_SUB_IP_Audio2){
                            subip->_subCount = 1;
                            subip->_pDataHead = &tvsb1_audio2_overflow_1;
                        }else if(j==TVSB1_SUB_IP_VBI){
                            subip->_subCount = 1;
                            subip->_pDataHead = &tvsb1_vbi_over_unnder_flow;
                        ...
                        if(j==SB1_SUB_IP_EMMC){
                            subip->_subCount = 1;
                            subip->_pDataHead = &sb1_emmc_over_under_flow;
                        }else{}
                    }
                        ...
                        if(j==TVSB4_SUB_IP_OD_W){
                            ...
                            subip->_subCount = 1;
                            subip->_pDataHead = &tvsb4_dc2h_over_underflow;
                        }
                        else if(j==TVSB4_SUB_IP_SUBTITLE){
                            subip->_subCount = 1;
                            subip->_pDataHead = &tvsb4_subtitle_underflow;
                        }
                        else if(j==TVSB4_SUB_IP_OSD1_3){
                            subip->_subCount = 1;
                            subip->_pDataHead = &tvsb4_osd_1_3_underflow;
                        }
...
int release_dma_status(void)
{
    unsigned int i=0,j=0;
    CFIFOIpStatus * pip = 0;
    CFIFOSubIpStatus * subip=0;
    struct CFIFODataInfo * pd = 0,*pnxt=0;
    for(i=0;i<g_DMAStatus._subCount;i++){
        //ip
...
int parse_dma_status(unsigned char *cmd_data,unsigned int cmd_len)
{
    ...
    CFIFOIpStatus * pip = 0;
    CFIFOSubIpStatus * psub = 0;
    struct CFIFODataInfo * pd = 0;
    bool isFind = false;
...
int update_dma_status(void)
{
    ...
    for(i=0;i<g_DMAStatus._subCount;i++){
        pip = g_DMAStatus._ips[i];
        if(pip != 0 && pip->_isChoosen){
            /*RTICE_DEBUG("[%s:%d]update ip=%d\r\n",__func__,__LINE__,pip->_id);*/
            for(j=0;j<SubsCountOfIP[i];j++){
                psub = pip->_subIps[j];
                if(psub != 0 && psub->_isChoosen){
...
int rtice_dma_status_tool_init(void)
{
    rtice_register_command_handler(&dma_status_cmd_handler,\
		sizeof(dma_status_cmd_handler) / sizeof(rtice_command_handler_t));
    rtd_pr_rtice_info("rtice_register_command_handler() has been executed for DMA_STATUS.\n");
 ...
 * Retn : 0 : success, others fail
 *------------------------------------------------------------------*/
void rtice_dma_status_tool_exit(void)
{
    rtice_unregister_command_handler(&dma_status_cmd_handler,\
		sizeof(dma_status_cmd_handler) / sizeof(rtice_command_handler_t));
    rtd_pr_rtice_info("rtice_unregister_command_handler() has been executed for DMA_STATUS.\n");
    release_dma_status();
}
