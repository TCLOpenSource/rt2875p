...
#include <rtd_log/rtd_module_log.h>	/* DBG_PRINT() */
...
/*#define DBG_PRINT(s, args...) rtd_pr_rtice_info(s, ## args)*/
...
static char *rtice_devnode(struct device *dev, umode_t *mode)
{
	return NULL;
}
...
int rtice_init_module(void)
{
	...
	rtd_pr_rtice_info("rtice2ap init module major number = %d\n", rtice_major);
