...
irqreturn_t AudioHw_Earc_Isr(int irq, void *dev_id)
{
    ...
    if (CMDC_IRQ_STATUS.heartbeat_ack_irq != 0) {
        if (CMDC_IRQ_STATUS.heartbeat_ack_irq == 1) {
            ...
            if (RunCapFlag == 1) {
#if 0  //old read cap method                
                earc_earc_cmdc_req_RBUS CMDC_Tx_Req;
                earc_earc_cmdc_control_1_RBUS CMDC_PayLoad;

                CMDC_PayLoad.regValue = AIO_ReadRegister(EARC_eARC_CMDC_CONTROL_1_reg);
                CMDC_PayLoad.device_id = AIO_EARC_CAP_DEVICE_ID;
                CMDC_PayLoad.offset = AIO_EARC_CAP_OFFSET;
                AIO_WriteRegister(EARC_eARC_CMDC_CONTROL_1_reg, CMDC_PayLoad.regValue);
...
int32_t Audio_HW_Earc_Get_Rx_Status(AIO_AUDIO_EARC_RX_STATUS_T *RxState)
{
    if (g_Earc_State.ConnectState == AIO_EARC_STATE_EARC) {
        if ((AioEarcGetIrqFlag() & AIO_EARC_START_CHNG_IRQ) != 0) {
            ...
            g_Earc_State.ReadLatencyReady = false;
	    alog_debug("latency chg detected\n");
        }
    ...
    if (g_Earc_State.ConnectState == AIO_EARC_STATE_EARC &&
            ...
            if (g_Earc_State.ReadCapReady == false && g_Earc_State.TransactionState == AIO_AUDIO_EARC_TRAN_READ_CAP_PROCESS) {
                ...
                for (i = 1; i <= 12; i++) {
                    ...
                    if (g_Earc_State.ReadCapReady == true && g_Earc_State.TransactionState == AIO_AUDIO_EARC_TRAN_READ_CAP_FINISH) {
                        //memcpy(g_CapabilityData,Info->ReadData , 256);
	    ...
	    RunCapFlag=1;
            while (1) {               //Sync with Ack
                ...
                if (read_cap_to_count > 50000) { //500ms timeout
...
int32_t Audio_HW_Earc_Write_Latency(uint8_t WriteData)
{
    ...
    if (g_Earc_State.ConnectState == AIO_EARC_STATE_EARC &&
        ...
        earc_earc_cmdc_req_RBUS CMDC_Tx_Req;
        earc_earc_cmdc_control_1_RBUS CMDC_PayLoad;
        earc_earc_cmdc_control_2_RBUS Write_Data;
        unsigned long flags;
        int i = 0;
        ...
        Write_Data.regValue = AIO_ReadRegister(EARC_eARC_CMDC_CONTROL_2_reg);
        Write_Data.write_data = WriteData;
        AIO_WriteRegister(EARC_eARC_CMDC_CONTROL_2_reg, Write_Data.regValue);
...
static int32_t  AutoReadCap(unsigned char *Data)
{
    unsigned int i = 0, data_len = 0, ret = AIO_OK;
    unsigned int ValidFlag = 0, FinIsrFlag = 0, ErrorFlag = 0;
    earc_earc_cmdc_control_2_RBUS eARC_CMDC_CONTROL_2;
    ...
    eARC_CMDC_CONTROL_2.regValue = AIO_ReadRegister(EARC_eARC_CMDC_CONTROL_2_reg);
    ...
    AIO_WriteRegister(EARC_eARC_CMDC_CONTROL_2_reg, eARC_CMDC_CONTROL_2.regValue);
    ...
    if (ErrorFlag == 1) {
        ...
        for (i = 0; i < data_len; i += 4) {
            ...
            Data[i + 3] = ((ReadData.regValue >> 24) & 0xFF);
            Data[i + 2] = ((ReadData.regValue >> 16) & 0xFF);
            Data[i + 1] = ((ReadData.regValue >> 8) & 0xFF);
            Data[i] = (ReadData.regValue & 0xFF);
...
void Audio_HW_Earc_SetAPHY(void)
{
    ...
    if (EARC_OTP.Done_bit == 1) {
        alog_err("EARC OTP Found..\n");
        EARC_ANA_2.regValue = AIO_ReadRegister(EARCMISC_earc_ana_2_reg);
        EARC_ANA_2.df_ib_sel = EARC_OTP.Df_Ib_Sel;
        AIO_WriteRegister(EARCMISC_earc_ana_2_reg, EARC_ANA_2.regValue);
        alog_err("EARC OTP df_ib_sel=%x\n", EARC_OTP.Df_Ib_Sel);

        EARC_ANA_1.regValue = AIO_ReadRegister(EARCMISC_earc_ana_1_reg);
        EARC_ANA_1.r300_sel = EARC_OTP.R300_Sel;
        EARC_ANA_1.r60_sel = EARC_OTP.R60_Sel;
        EARC_ANA_1.cm_ib_sel = EARC_OTP.Cm_Ib_Sel;
        AIO_WriteRegister(EARCMISC_earc_ana_1_reg, EARC_ANA_1.regValue);
        alog_err("EARC OTP R300_Sel=%x\n", EARC_OTP.R300_Sel);
        alog_err("EARC OTP R60_Sel=%x\n", EARC_OTP.R60_Sel);
        alog_err("EARC OTP Cm_Ib_Sel=%x\n", EARC_OTP.Cm_Ib_Sel);
    }
    PreHpdState = 0;
    Pre5vState = 0;
    //
