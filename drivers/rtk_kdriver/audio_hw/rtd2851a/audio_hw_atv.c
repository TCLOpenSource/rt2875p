...
//static int g_UserBTSCoverModulationModeFlag = 1;	//[Fix me]
...
static ATV_A2_TH_TAB A2_Th_MN_Table[101] = {														/* ATV_MODE_MAX_TH_0 */
	...
	{ATV_LPF_COEF_SEL_14, ATV_LPF_COEF_SEL_14, ATV_MODE_HI_TH_2P00T, ATV_MODE_LO_TH_1P50T, ATV_A2_DEBOUNCE_0P1S, 0x60000,	0x0c0000,	20},	//11
	...
	{ATV_LPF_COEF_SEL_14, ATV_LPF_COEF_SEL_14, ATV_MODE_HI_TH_2P00T, ATV_MODE_LO_TH_1P50T, ATV_A2_DEBOUNCE_0P1S, 0x60000,	0x190000,	20},	//100
};
...
#define AUDIO_SIF_SEL_TYPE	(ATV_AUDIO_SIF_BTSC_SELECT | \
	...
	AtvSetFwCarrierFreq(ATV_ENABLE, 0);
        //AtvSetFwCarrierFreq(ATV_DISABLE, 0, 0, 0);  //Merlin4
...
uint32_t AtvGetHWStd(void)
{
	...
	if ((AtvCheckAGCFlag() != 1) && (g_Std_msd_result.hw_detected_std != ATV_SOUND_STD_UNKNOWN)) {
		...
		//	if( (g_AudioTVControlFlag & ENABLE_PS_RATION_MORETHAN_30DB_MASK) == ENABLE_PS_RATION_MORETHAN_30DB )
		{
			...
			if (g_Std_msd_status.shift_value == 0) 
			{
	...
	if (g_Std_msd_result.hw_detected_std == ATV_SOUND_STD_FM_MONO_NO_I) {if ((g_Std_energy_6 < g_Std_energy_55) ||
...
#define NICAM_SOUNDMODE_OUT_CNT 3
			...
			if (g_NicamSignalStable == 1) { // unstable , can detected
				...
				if (AtvIsPlayDigitalState()) {
					...
					//[Fix me] Add optional API for customization
...
// Merlin4 main trunk
// [Fix me] refine and think which method is better
/*
		if (g_UserBTSCoverModulationModeFlag == 1){
			AtvBTSCoverModulationMode(1);	//reg_std 0x00 MN
		}
		else{	
	...
	if (main_sys == real_main_sys || main_sys == ATV_SOUND_AUTO_SYSTEM) {
		...
		*pToneSNR = ToneMag;    // Return SIF magnitude
		...
		if (AtvCheckVideoTiming(real_main_sys) != 1) {
			*pToneSNR = 0;
			alog_info("Audio_HwpSIFGetMainToneSNR.......fake main tone, return SNR 0\n");
		}
#endif
...
void CleanSIFEQOverflow(void)
{
	...
	// Step3: Disable Main power of SIF_EQ
	eq_tvd_bp_control.regValue = AIO_ReadRegister(AUDIO_EQ_tvd_bp_Control_reg);
	...
	// Step4: Disable Sub power of SIF_EQ
	eq_tvd_bp_control.regValue = AIO_ReadRegister(AUDIO_EQ_tvd_bp_Control_reg);
	...
	// Step5: Write back the parameters of EQ/Loudness
	AIO_WriteRegister(AUDIO_EQ_tvd_lp_reg, eq_tvd_lp.regValue);
	AIO_WriteRegister(AUDIO_EQ_tvd_bp1_1_reg, eq_tvd_bp1_1.regValue);
	AIO_WriteRegister(AUDIO_EQ_tvd_bp1_2_reg, eq_tvd_bp1_2.regValue);
	AIO_WriteRegister(AUDIO_EQ_tvd_bp2_1_reg, eq_tvd_bp2_1.regValue);
	AIO_WriteRegister(AUDIO_EQ_tvd_bp2_2_reg, eq_tvd_bp2_2.regValue);
	alog_info("CleanSIFEQLoudnessOverflow step5 : Write back\n");
	audio_hw_usleep(10);

	// Step6: Enable Sub power of EQ & Loudness
	eq_tvd_bp_control.regValue = AIO_ReadRegister(AUDIO_EQ_tvd_bp_Control_reg);
	...
	alog_info("CleanSIFEQLoudnessOverflow step6 : Enable Sub power of SIF_EQ %x\n", eq_tvd_bp_control_bk.regValue);

	// Step7: Enable Main power of EQ & Loudness
	eq_tvd_bp_control.regValue = AIO_ReadRegister(AUDIO_EQ_tvd_bp_Control_reg);
...
int32_t AtvCheckVideoTiming(ATV_SOUND_STD_MAIN_SYSTEM main_sys)
{
	...
	//unsigned int noise_status, noise_status_max = 0, i;
	//unsigned int VD_BURST_TH = 0, burst_noise_diff = 0;
	...
	if (main_sys == ATV_SOUND_UNKNOWN_SYSTEM)
		{
			alog_info("AtvCheckVideoTiming error, return SNR 0\n");
...
#if 0
	for (i = 0; i < 10; i++) {
		noise_status_reg.regValue = AIO_ReadRegister(VDTOP_NOISE_STATUS_reg);
		noise_status = noise_status_reg.status_noise;
		if (noise_status > noise_status_max)
			noise_status_max = noise_status;
	}

	if (noise_status_max > 0x35){
		VD_BURST_TH = BURST_TH_LOW;
	}else{
		VD_BURST_TH = BURST_TH;
	}
	...
	alog_info("443_358_burst_stauts = %x %x\n", sep_443_burst, sep_358_burst);
	alog_info("443_358_burst_Hsync_stauts = %x %x\n", sep_443_burst_hsync, sep_358_burst_hsync);

	switch (main_sys) {//HKTTANDP-12, RL
	case ATV_SOUND_MN_SYSTEM:
		//case1
		if (!(sep_358_burst == 0xFF && sep_443_burst == 0xFF)){
			if (sep_358_burst < BURST_TH){ //358<TH

				if (noise_status_max > sep_443_burst_hsync){
					burst_noise_diff = 0;
					alog_info("normal case 443 Hsync <TH, noise too big \n");
				}else{
					burst_noise_diff = (sep_443_burst_hsync-noise_status_max);
				}

				if (burst_noise_diff  > VD_BURST_TH){ //443 Hsync >TH
					timing_match_flag = 1;
					//alog_info("%s, %d", __FUNCTION__, __LINE__);
				}else{//443 Hsync <TH
					timing_match_flag = 0;
					alog_info("normal case 443 Hsync <TH, but not match \n");
				}			
			}else{ //358>TH

				if (noise_status_max > sep_358_burst){
					burst_noise_diff = 0;
					alog_info("normal case 358<TH, noise too big \n");
				}else{
					burst_noise_diff = (sep_358_burst-noise_status_max);
				}	

				if(burst_noise_diff < VD_BURST_TH){
							timing_match_flag = 0;
							alog_info("normal case 358<TH, but not match \n");						
				}else{
		...
		}else{ //0xff case, check 443 Hsync

			if (noise_status_max > sep_443_burst_hsync){
				burst_noise_diff = 0;
				alog_info("0xff case check 443 Hsync for MN, noise too big \n");
			}else{
				burst_noise_diff = (sep_443_burst_hsync-noise_status_max);
			}
			...
			if (burst_noise_diff > VD_BURST_TH){
						...
						timing_match_flag = 0;
						alog_info("0xff case check 443 Hsync for MN, but not match \n");	
					}
	...
	case ATV_SOUND_L_SYSTEM:
		//case2
		if (!(sep_358_burst == 0xFF && sep_443_burst == 0xFF)){
			if (sep_443_burst < BURST_TH){ //443<TH
				if (sep_358_burst < BURST_TH){//358<TH
					timing_match_flag = 1;
					//alog_info("%s, %d", __FUNCTION__, __LINE__);
				}else{ //358>TH
				...
				if (noise_status_max > sep_443_burst){
					burst_noise_diff = 0;
					alog_info("normal case 443>TH, noise too big \n");
				}else{
					burst_noise_diff = (sep_443_burst-noise_status_max);
				}
			
				if (burst_noise_diff < VD_BURST_TH){
							timing_match_flag = 0;
							alog_info("normal case 443>TH, but not match \n");						
						}
				else{
							timing_match_flag = 1;
							//alog_info("%s, %d", __FUNCTION__, __LINE__);
			...
			if (noise_status_max > sep_443_burst_hsync){
				burst_noise_diff = 0;
				alog_info("0xff case check 443 Hsync, noise too big \n");
			}else{
				burst_noise_diff = (sep_443_burst_hsync-noise_status_max);
			}
			...
			if (burst_noise_diff > VD_BURST_TH){
					timing_match_flag = 0;
					alog_info("0xff case check 443 Hsync, but not match \n");	
				}else{ 
...
{
	AIO_WriteRegister(0xb8033100, 0x00010202);//IFADC_CTRL1
	AIO_WriteRegister(0xb8033104, 0x00000240);//IFADC_CTRL2

	AIO_WriteRegister(0xb8033118, 0x30200010);//IQADC_CTRL0

	AIO_WriteRegister(0xb8033120, 0x00440001);//IQADC_CTRL2
	AIO_WriteRegister(0xb8033124, 0x00001111);//IQADC_CTRL3
	AIO_WriteRegister(0xb8033128, 0x10210022);//IQADC_CTRL4
	AIO_WriteRegister(0xb803312c, 0x00000000);//IQADC_CTRL5

	AIO_WriteRegister(0xb8033134, 0x22000022);//IQADC_CTRL7
...
void Audio_Auto_Verify_Set_Path(void)
{
	...
	if (g_user_config == AUDIO_USER_CONFIG_BRING_UP)
	{
		...
		AIO_WriteRegister(0xb8000104,0x000F8000);
		AIO_WriteRegister(0xb8000104, VERIFY_BIT19|VERIFY_BIT18|VERIFY_BIT17|VERIFY_BIT16|VERIFY_BIT15|VERIFY_BIT0);

		AIO_WriteRegister(0xb800011c,0x000003FE);
		AIO_WriteRegister(0xb800011c, VERIFY_BIT9|VERIFY_BIT8|VERIFY_BIT7|VERIFY_BIT6|VERIFY_BIT5|VERIFY_BIT4
			|VERIFY_BIT3|VERIFY_BIT2|VERIFY_BIT1|VERIFY_BIT0);

	  	AIO_WriteRegister(0xb8000114,0x00038000);
	  	AIO_WriteRegister(0xb8000114, VERIFY_BIT17|VERIFY_BIT16|VERIFY_BIT15|VERIFY_BIT0);
	 
	  	AIO_WriteRegister(0xb8000118,0x00009000);
	  	AIO_WriteRegister(0xb8000118, VERIFY_BIT15|VERIFY_BIT12|VERIFY_BIT0);

		AIO_WriteRegister(0xb80004d4,0x88085501);//Enable Audio PLL related LDO, PLL_POW_LDO11V & PLL_POW_LDO
		AIO_WriteRegister(0xb80004e4,0x00000019);//Enable PLLAUD_CK196M/CK295M output
		AIO_WriteRegister(0xb80004b4,0x00000003);//Enable PLL512Fs output*/
		AIO_WriteRegister(0xb80004ec,0x0000000C);//PSAUD_EN, PSAUD_PSEN 0 --> 1

		//audio path init
		audio_hw_usleep(1000); //ideal 80us, wait 1ms.
		
		WriteMaskRegister(0xb8006004, ~(VERIFY_BIT31|VERIFY_BIT30|VERIFY_BIT29|VERIFY_BIT28), 
			VERIFY_BIT31|VERIFY_BIT30|VERIFY_BIT29|VERIFY_BIT28);//SDRAM on
		WriteMaskRegister(0xb8006004, ~(VERIFY_BIT20), VERIFY_BIT20);//SIF ADC CLOCK on
		WriteMaskRegister(0xb8006004, ~(VERIFY_BIT13|VERIFY_BIT12|VERIFY_BIT5|VERIFY_BIT4), 
			VERIFY_BIT13|VERIFY_BIT5);//PLL AUDIO

		WriteMaskRegister(0xb8006b5c, ~(VERIFY_BIT3|VERIFY_BIT2), VERIFY_BIT3|VERIFY_BIT2);//enable output normal mode FOR HP/AIO Frank

		WriteMaskRegister(0xb8006b58, ~(VERIFY_BIT10|VERIFY_BIT9|VERIFY_BIT8|VERIFY_BIT2|VERIFY_BIT1), 
		...
		/*
		WriteMaskRegister(0xb8006004, 0x0FFFFFFF, _BIT31|_BIT30|_BIT29|_BIT28);//SDRAM on
		WriteMaskRegister(0xb8006004, 0xFFEFFFFF, _BIT20);//SIF ADC CLOCK on
		WriteMaskRegister(0xb8006004, 0xFFFFCFCF, _BIT13|_BIT5);//PLL AUDIO

		WriteMaskRegister(0xb8006b5c, 0xFFFFFFF3, _BIT3|_BIT2);//enable output normal mode FOR HP/AIO Frank

		WriteMaskRegister(0xb8006b58, 0xFFFFF8F9, _BIT10|_BIT2|_BIT1);//enable HPO/AIO enable depop
		
		WriteMaskRegister(0xb8033000, ~(_BIT24),_BIT24);// turn on "REG_VDAC_POWMBIAS" for BB audio
		*/
		//ryanliao turn on audio power
		WriteMaskRegister(0xb8006b44, ~(VERIFY_BIT11|VERIFY_BIT10|VERIFY_BIT9|VERIFY_BIT8|VERIFY_BIT7|
		...
		WriteMaskRegister(0xb8006038, ~(VERIFY_BIT8|VERIFY_BIT7|VERIFY_BIT6|VERIFY_BIT5|VERIFY_BIT4), 
			VERIFY_BIT8|VERIFY_BIT7|VERIFY_BIT6|VERIFY_BIT5|VERIFY_BIT4);// enable tv_demod, i2s_out, spdif_out functions

		AIO_WriteRegister(0xb8006700, 0x00000000);

		AIO_WriteRegister(0xb8006b48, 0x00023000);// HPO mode
		AIO_WriteRegister(0xb8006b4c, 0xD5D517D5);// HPO bias control (11: 20u)
		AIO_WriteRegister(0xb8006b10, 0x000C0003);//DAC_0_1 setting PB(modify)
		AIO_WriteRegister(0xb8006b38, 0x80180005);//DAC0_HPO DAC1_AIO1 unmute L channel
		AIO_WriteRegister(0xb8006b3c, 0x00180005);//DAC0_HPO DAC1_AIO1 unmute R channel
		#if 0
		//K4L
		alog_err("\n[SIF_ADC]IFD_SIF_ADC_Initial_Flow\n");
		tv_sb1_ana_ifadc_ctrl1_RBUS ifadc_ctrl1;
		...
		//SIF ADC init
		//register default
		WriteRegister(0xb8033100,0x00010202);//PGA IB20U select <bit7:0> = 0x02(10u),IFD MBIAS power on(bit16=1), ADC2X_BG_SEL= 10(1.221V)
		WriteRegister(0xb8033104,0x00000040);//IFADC Comparator bias select, <bit6:4>= 100(50uF), IQADC input cm power off <bit24> = 0  
		WriteRegister(0xb8033118,0x30200010);//IQ ADC calibration input swing <bit30:28> = 0x11, REG_IQ_CAL_DIV<bit21:20> = 0x10, REG_CKOUT_SEL_Q: the default is "1:I_CKOUT" on AIF spec
		WriteRegister(0xb8033120,0x00440000);//REG_IQCMP_ISEL <bit22:20> = 0x100(50uA), REG_IQCAL_ISEL <bit18:16> = 0x100(50uA), IF ADC GAIN FIX EN <bit0> = 0(SIF mode)
		WriteRegister(0xb8033124,0x00000000);//PGA/ADC Power Off
		WriteRegister(0xb8033124,0x00000101);//I_ADC PGA/ADC Power on
		WriteRegister(0xb8033128,0x00000022);//Default setting(10=20uA), IQ PGA CURRENT  select(same with Mac5P)
			
		//IFADC+IFPGA power on
		WriteMaskRegister(0xb8033100, 0xFEEEFFFF, 0x00010000);	//turn on MBIAS
		WriteMaskRegister(0xb8033104, 0xFEFFFFFF, 0x01000000);	//REG_ADC2X_DIFPOW, IFADC input cm power on
		WriteMaskRegister(0xb8033128, 0x8fc8fccc, 0x00020022);	//I PGA input select = IFD input, Q PGA input select = No input, REG_IB20U_IPGA_SEL = 10(20uA)

		//IQADC+IQPGA power on
		WriteRegister(0xb803312C,0x00000000);//default setting
		WriteRegister(0xb8033130,0x00000000);//default setting
		WriteRegister(0xb8033134,0x22000022);//REG_IADC_LDO_SEL = 010, REG_QADC_LDO_SEL = 010, IADC, QADC_META_POW = turn off, REG_IADC_VREF_SEL = 10, REG_QADC_VREF_SEL = 10

		//Set IFPGA GAIN = 2dB
		WriteMaskRegister(0xb8033120, 0xfffffffe, 0x00000001);//IF ADC GAIN FIX EN <bit0>= 1(Internal Dedmo IQ mode)
		WriteMaskRegister(0xb803312c, 0xffffcc00, 0x00000022);//Q_CKIN_Sel = 00, I_CKIN_Sel = 00: From PLL27X_IFADC, REG_IPGA_Q and REG_IPGA_D GAIN set to 0010(2dB)

		// SIF Audio related setting
		WriteRegister(0xb8033118,0x30200000); // REG_CKOUT_SEL_Q--> 0:Q_CKOUT
		WriteRegister(0xb8033128,0x40000022); // REG_QPGA_INPUT_SEL  100: SIF input
		WriteRegister(0xb8033134,0x00001000); // QADC META control -->1
		WriteRegister(0xb8033120,0x40000001); 

		WriteRegister(0xb803312C, 0x00002022); // REG_Q_CKIN_SEL 10: CK_24M From Digital
		WriteRegister(0xb8033124, 0x00001010); // REG_POW_Q -->Q channel ADC power on, Q channel PGA power on
		
		//sif path init
		WriteRegister(0xb8006180, 0x000b0180); //SIF HW AGC enable
		WriteRegister(0xb8006350, 0x80000000); //from SIF_ADC

		//Audio MSD setting
		WriteRegister(0xb8006200, 0x30C80617); //HW msd_en all hw scan
		WriteRegister(0xb8006300, 0x00000004); //HW md_en
		WriteRegister(0xb8006554, 0x00007650);
		#else
		audio_sif_adc_initial_flow();
		#endif
		WriteMaskRegister(0xb800635c, 0xF7FFFFFF, 0); //disable SIF DVC
		WriteMaskRegister(0xb80067bc, ~(VERIFY_BIT31|VERIFY_BIT15), 0);
	}
	else
	{
...
void AtvCheckA2DK3rdTone(void)
{
	   ...
	   g_Std_msd_result.hw_detected_std == ATV_SOUND_STD_A2_DK3)
	{
		...
		if(sound_mode == ATV_SOUND_MODE_MONO)
		{
			...
			{
				g_Std_msd_result.hw_detected_std = ATV_SOUND_STD_DK_MONO;
				alog_err("Fake A2DK, change to DK MONO\n");
			}
		}
		else
		{
			if(g_Std_msd_result.hw_detected_std == ATV_SOUND_STD_DK_MONO)
			{
				...
				{
					g_Std_msd_result.hw_detected_std = ATV_SOUND_STD_A2_DK1;
					alog_err("Fake DK MONO, change to A2DK1\n");
				}
				else if(force_a2_dk_sel == ATV_FORCE_A2_DK2)
				{
					g_Std_msd_result.hw_detected_std = ATV_SOUND_STD_A2_DK2;
					alog_err("Fake DK MONO, change to A2DK2\n");
				}
				else
				{
					g_Std_msd_result.hw_detected_std = ATV_SOUND_STD_A2_DK3;
					alog_err("Fake DK MONO, change to A2DK3\n");
				}
			}
		}
	}
