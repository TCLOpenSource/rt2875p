...
unsigned char g_writecmd_flag=0,g_writecmd=0,g_cap_update=0,g_latency_update=0;
			...
			if (EARC_READ_COUNT > 3)
			{
				...
				else
				{
					...
					if(g_cap_update==1)
					{
						for (i=0; i < 255; ++i)
					{
							Para.capability[i]=Para2.capability[i];
							//alog_err("[%d]%x ",i, Para.capability[i]);
						}
							//alog_err("\nPara.capability done\n");						
						g_cap_update=0;
					}
