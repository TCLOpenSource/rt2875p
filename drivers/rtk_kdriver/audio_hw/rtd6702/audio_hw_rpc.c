...
{	
...
#endif
	p = dma_alloc_coherent(audio_driver_data->clsdev, RPC_MEMORY_MAP_SIZE, &dat, GFP_KERNEL | __GFP_DMA);
	...
	info = p;
	info_audio = (AUDIO_RPC_SET_ATV_CLOCK *)((ulong) dat);

	res = (struct RPCRES *)
		((((unsigned long)p + sizeof(AUDIO_RPC_SET_ATV_CLOCK) + 8) & ALIGN4));
	res_audio = (struct RPCRES *)
		(((((unsigned int) dat) + sizeof(AUDIO_RPC_SET_ATV_CLOCK) + 8) &
			0xFFFFFFFC));
	...
	if (audio_send_rpc_command(RPC_AUDIO,
		...
		dma_free_coherent(audio_driver_data->clsdev, RPC_MEMORY_MAP_SIZE, p, dat);
		return -1;
	}
...
{	
	...
	vir_audioConfig = p;
	phy_audioConfig = (AUDIO_CONFIG_COMMAND *)((ulong) dat);

	res = (struct RPCRES *)
		((((unsigned long)p + sizeof(AUDIO_CONFIG_COMMAND) + 8) & ALIGN4));
	res_audio = (struct RPCRES *)
		(((((unsigned int) dat) + sizeof(AUDIO_CONFIG_COMMAND) + 8) &
			0xFFFFFFFC));
