...
void  Audio_HwCheckNoSignal(void)
{
	...
	if ((Tone45 < NoSignalTH) && (Tone55 < NoSignalTH)
		 && (Tone60 < NoSignalTH) && (Tone65 < NoSignalTH))
	{
		g_NoSignalCount ++;
	}
	else
...
void AtvChageA2SoundMode(ATV_SOUND_MODE sound_mode)
{
	...
	if(g_CurSifType == ATV_AUDIO_SIF_KOREA_A2_SELECT){
		...
		if(g_A2_soundmode ==  ATV_SOUND_MODE_STEREO && g_UserTVA2Language == 0){
			//alog_info("1Compenate.....A2 =%d %d\n", g_sif_offset, g_UserTVA2Language); 
			HwpSetVolume_SIF(g_sif_offset+32);

		}else if(g_A2_soundmode ==  ATV_SOUND_MODE_DUAL && g_UserTVA2Language == 0){//main sound
			//alog_info("2Compenate.....A2 =%d %d\n", g_sif_offset, g_UserTVA2Language); 
			HwpSetVolume_SIF(g_sif_offset+32);

		}else if(g_A2_soundmode ==  ATV_SOUND_MODE_DUAL && g_UserTVA2Language == 1){//sub sound
			//alog_info("22Compenate.....A2 =%d %d\n", g_sif_offset, g_UserTVA2Language); 
			HwpSetVolume_SIF(g_sif_offset+32);

		}else{
			//alog_info("3Compenate.....A2 =%d\n", g_sif_offset); 
			HwpSetVolume_SIF(g_sif_offset+24); //mono
		}
...
{
	int32_t sif_vol=0;
	sif_vol = g_sif_offset+g_sif_highdev_offset+g_sif_AMcomp_offset+g_sif_BTSC_FMradio_comp;
	HwpSetVolume_SIF(sif_vol);
	alog_info("[%s] g_sif_offset= %d, g_sif_highdev_offset= %d, g_sif_AMcomp_offset= %d, g_sif_BTSC_FMradio_comp= %d\n", __FUNCTION__, g_sif_offset, g_sif_highdev_offset,g_sif_AMcomp_offset,g_sif_BTSC_FMradio_comp);
}
