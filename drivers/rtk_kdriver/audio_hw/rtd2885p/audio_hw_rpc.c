...
// can't edit this size , this is also depedence on kernel and fw's map (LINUX_KERNEL_MEMORY_MAP_SIZE amw_kernel_rpc.c )
// and RPC_DVR_MALLOC_UNCACHED_SIZE(audio_rpc.c)
// RPC_MEMORY_MAP_SIZE(audio_hw_rpc.c)
#define RPC_MEMORY_MAP_SIZE (4096) // can't edit this size
	...
	unsigned long *retvalue)
{
	...
	command == ENUM_KERNEL_RPC_GET_MUTE_N_VOLUME) {
		...
		if (*retvalue != S_OK || DUMMY_ntohl(audio_ret->result) != S_OK) {
			alog_err("[ALSA] RPC S_OK fail\n");
			alog_err("[ALSA] retvalue %lx, result %x, command %lx\n",
				*retvalue, DUMMY_ntohl(audio_ret->result), command);
			ret = -1;
		}
	}
...
int32_t rtk_snd_set_atv_clock(AUDIO_RPC_SET_ATV_CLOCK *atv_clk_info)
{
	...
	vir_addr = dvr_malloc_uncached_specific(RPC_MEMORY_MAP_SIZE, GFP_DCU1, &unvir_addr);
	if (!vir_addr) {
	...
	memset(unvir_addr, 0, RPC_MEMORY_MAP_SIZE);

	info = unvir_addr;
	info_audio = (AUDIO_RPC_SET_ATV_CLOCK *)((ulong) phy_addr);

	res = (struct RPCRES *)	((((unsigned long) unvir_addr + sizeof(AUDIO_RPC_SET_ATV_CLOCK) + 8) & ALIGN4));
	res_audio = (struct RPCRES *) (((((unsigned int) phy_addr) + sizeof(AUDIO_RPC_SET_ATV_CLOCK) + 8) & 0xFFFFFFFC));

	alog_debug("[atv_clk_info] %ld %d", atv_clk_info->instanceID, atv_clk_info->atv_type);

	/* send atv clock info */
	info->instanceID = DUMMY_htonl(atv_clk_info->instanceID); //endian swap
	info->atv_type = DUMMY_htonl(atv_clk_info->atv_type);	//endian swap
...
int32_t rtk_snd_set_decoder_mute(bool mute, ENUM_AI_PATH path_sel)
{
	...
	vir_audioConfig = unvir_addr;
	phy_audioConfig = (AUDIO_CONFIG_COMMAND *)((ulong) phy_addr);

	res = (struct RPCRES *) ((((unsigned long)unvir_addr + sizeof(AUDIO_CONFIG_COMMAND) + 8) & ALIGN4));
	res_audio = (struct RPCRES *) (((((unsigned int) phy_addr) + sizeof(AUDIO_CONFIG_COMMAND) + 8) & 0xFFFFFFFC));

	/* send sw decoder info */
	vir_audioConfig->msgID = DUMMY_htonl(AUDIO_CONFIG_CMD_SET_AI_VOLUME);
	vir_audioConfig->value[0] = DUMMY_htonl(true); //always enable SET_AI_VOLUME function
	vir_audioConfig->value[1] = DUMMY_htonl(path_sel);

	//if mute = 1, volume = ENUM_AUDIO_DVOL_MUTE, ENUM_AUDIO_DVOL_K0p0DB AIN_VOL_0DB
	for(ch=0;ch<8;ch++)
	{
		vir_audioConfig->value[2 + ch] = DUMMY_htonl(mute?ENUM_AUDIO_DVOL_MUTE:ENUM_AUDIO_DVOL_K0p0DB);
	}
...
int32_t rtk_snd_enhance_btsc_separation(bool enable)
{
	...
	alog_info("[atv_btsc_enhance_info] %d", enable);
