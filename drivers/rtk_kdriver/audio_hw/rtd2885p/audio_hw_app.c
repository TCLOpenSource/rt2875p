...
{
#if 0 //2851c/2885p no support ts dvc
	audio_lrrr_digital_volume_control_0_RBUS	DVOL_CTRL0_REG;
...
static int set_loud_coef(unsigned int band_id)
{
	...
	audio_loudness_status_register loudness_status_register;
...
//fix coverity: untrusted loop bound
               while(base<MAX_LOUD_VALUE)
               {
			if(input > interval)
			{
			input -= interval;
...
{
	audio_system_int_en_RBUS      system_int_en_RBUS;
	audio_eq_status_register eq_status_register;
	unsigned int                  A1Address,
				      A2Address,
				      H0Address;
	int                           A1TargetData,
...
int32_t Audio_HwpSetEQValue(bool para, APP_EQ_VAL val_eq)
{
	audio_eq_control_register_RBUS EQ_CTRL_REG;
	audio_hw_process_control_RBUS  HWP_CTRL_REG;
	//audio_eq_status_register EQ_STATUS_REG;
