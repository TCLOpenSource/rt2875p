...
typedef union
{
    ...
    struct{
        RBus_UInt32  res1:21;
        RBus_UInt32  eq_band11_of:1;        
        RBus_UInt32  eq_band10_of:1;        
...
}audio_loudness_status_register;
typedef enum {
	APP_ID_CAPABILITY		= 0,
	// bit-mapping setting
	// [ 0:15] for HWP
...
typedef union{
	RBus_UInt32 regValue;
	struct {
		unsigned int EARC_Reserved:14;
		unsigned int Done_bit:1;
		unsigned int ADC_LR_bit:1;
		unsigned int ADC_gain:2;	
		unsigned int DAC0_LR_bit:1;	
		unsigned int DAC0_gain:2;	
		unsigned int DAC1_LR_bit:1;	
		unsigned int DAC1_gain:2;				
		unsigned int Reserved:8;		
	} ;
}APP_AUDIO_OTP_T;

typedef enum {
    APP_AVC2_CH_ID_CH01 = 0,
    APP_AVC2_CH_ID_CH23,
    APP_AVC2_CH_ID_CH45,
    APP_AVC2_CH_ID_CH67,
    APP_AVC2_CH_ID_MAX_NUM,
} APP_AVC2_CH_ID;
 ...
 * @ingroup lib_audio
 */
int32_t Audio_HwpSetTreble2Value(bool para, APP_TREBLE_VAL val_treble);
 ...
 * @ingroup lib_audio
 */
int32_t Audio_HwpSetTreble2Value_48K(bool para, APP_TREBLE_VAL val_treble);
...
/**
 * Setup max output level (DAC1 limiter)
 *
...
/**
 * Setup max output level (I2S0 limiter)
 *
