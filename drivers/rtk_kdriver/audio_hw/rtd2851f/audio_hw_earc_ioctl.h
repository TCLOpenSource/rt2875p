...
typedef struct {
    ...
    unsigned char erx_readcap_ready;    
} AUDIOHW_EARC_INFO_T;
...
typedef union{
	...
	struct {
		...
		unsigned int df_trf_sel:2;		
		unsigned int cm_trf_sel:3;		
		unsigned int df_duty_sel:3;		
		unsigned int reserved_aphy:8;	
		unsigned int vol_level_extra:2;
	} ;
}AUDIOHW_EARC_APHY_T;
/*
 * audio earc commands
...
#define AUDIO_IOC_EARC_SET_APHY					_IOW(AUDIO_IOC_EARC_MAGIC, 5, AUDIOHW_EARC_APHY_T)

#define AUDIO_IOC_EARC_MAXNR					6
