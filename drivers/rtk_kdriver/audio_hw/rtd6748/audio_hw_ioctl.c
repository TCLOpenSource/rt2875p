...
static long atv_set_apvr_enable_flag(void __user *arg)
{
	uint8_t d;

	data_from_user(d, arg);
	Audio_HwSetAPVREnable_Flag(d);
	return 0;
}
...
static long atv_ioctl_Audio_Hw_Verify_Set_Path(void __user *arg)
{
...
static long audio_hw_atv_ioctl_handler(unsigned int cmd, void __user *arg)
{
	...
	case AUDIO_IOC_ATV_SET_FM_OUT_DOWN_GAIN:
		...
		break;
	case AUDIO_IOC_ATV_HW_SET_APVR_ENABLE_FLAG:
		ret = atv_set_apvr_enable_flag(arg);
		break;
	...
	case AUDIO_IOC_ATV_VERIFY_SET_PATH:
		ret = atv_ioctl_Audio_Hw_Verify_Set_Path(arg);
		break;
...
static long app_hwp_set_avc2(void __user *arg)
{
	...
	data_from_user(d, arg);
	if (d.p_cfg_avc2.avc2_channel == APP_AVC2_CH_ID_CH01) {
	    if (Audio_HwpSetAVC2(d.para, &d.p_cfg_avc2)) {
		return -ENOTTY;
	    }
	} else if (d.p_cfg_avc2.avc2_channel == APP_AVC2_CH_ID_CH23) {
	    if (Audio_HwpSetCH23AVC2(d.para, &d.p_cfg_avc2)) {
		return -ENOTTY;
	    }
	}
...
static long app_hwp_set_limit_output_dac0(void __user *arg)
{
    APP_HWP_SET_LIMIT_OUTPUT_T d;

    data_from_user(d, arg);
    Audio_HwpSetLimitOuptut_DAC0(d.para, d.maxLevelThresdhold);
    return 0;
}

static long app_hwp_set_limit_output_dac1(void __user *arg)
{
    APP_HWP_SET_LIMIT_OUTPUT_T d;

    data_from_user(d, arg);
    Audio_HwpSetLimitOuptut_DAC1(d.para, d.maxLevelThresdhold);
    return 0;
}

static long app_hwp_set_limit_output_i2s0(void __user *arg)
{
    APP_HWP_SET_LIMIT_OUTPUT_T d;

    data_from_user(d, arg);
    Audio_HwpSetLimitOuptut_I2S0(d.para, d.maxLevelThresdhold);
    return 0;
}
...
static long app_hwp_set_treble_value(void __user *arg)
{
	...
	if(arg==NULL)//coverity
		return -ENOTTY;	
	data_from_user(d, arg);
	if(&d.p_val_treble!=NULL) //coverity check
		return -ENOTTY;
...
static long app_hwp_set_treble2_value(void __user *arg)
{
	...
	if(arg==NULL)//coverity
		return -ENOTTY;			
	data_from_user(d, arg);
	if(&d.p_val_treble2 !=NULL)//coverity check
		return -ENOTTY;
...
static long app_hwp_set_treble2_value_48k(void __user *arg)
{
	...
	if(arg==NULL) //coverity check
		return -ENOTTY;	
	data_from_user(d, arg);
	if(&d.p_val_treble2_48k!=NULL) //coverity check
		return -ENOTTY;	
...
static long app_hwp_set_eq_parameter(void __user *arg)
{
	...
	{
		alog_err("null eq_parameter\n");
		return -ENOTTY;
	}else
	{
		data_from_user(d, arg);
		if((65536<d.eq_coef.a1) || (65536<d.eq_coef.a2) || (65536<d.eq_coef.h0))
			{
				alog_err("null eq_parameter\n");
				return -ENOTTY;
			}		
...
static long app_hwp_set_loud_parameter(void __user *arg)
{
	...
	{
		data_from_user(d, arg);
		if(d.loud_coef.a1>65536 || d.loud_coef.a2>65536 || d.loud_coef.h0>65536)
		{
			alog_err("null eq_parameter\n");
			return -ENOTTY;
		}
...
static long app_hwp_set_eq_dir_parameter(void __user *arg)
{
	APP_HWP_SET_EQ_PARAMETER_T d;
	if(!arg) //tained data sanitize, fix coverity check
	{
		alog_err("null eq_parameter\n");
		return -ENOTTY;
	}else
...
static long app_hwp_set_loud_dir_parameter(void __user *arg)
{
	APP_HWP_SET_LOUD_PARAMETER_T d;
	if(!arg) //tained data sanitize, fix coverity check
	{
	...
	{
		data_from_user(d, arg);
		if (Audio_HwpSetLoudParameter(d.band_id, d.loud_coef))
			return -ENOTTY;
	}
	return 0;
}
...
static long app_hwp_set_bass_value_48k(void __user *arg)
{
	...
	if(arg==NULL)//coverity
		return -ENOTTY;		
	data_from_user(d, arg);
	if(&d.p_val_bass_48k!=NULL)//coverity
		return -ENOTTY;	
...
static long app_hwp_set_eq_value(void __user *arg)
{
	...
	if(arg==NULL)//coverity check
		return -ENOTTY;		
	data_from_user(d, arg);
	if(&d.p_val_eq!=NULL)//coverity check
		return -ENOTTY;
...
static long app_hwp_set_treble_value_48k(void __user *arg)
{
	...
	if(arg==NULL)//coverity check
		return -ENOTTY;	
	data_from_user(d, arg);
	if(&d.p_val_treble_48k!=NULL)//coverity check
		return -ENOTTY;
...
static long audio_hw_app_ioctl_handler(unsigned int cmd, void __user *arg)
{
	...
	case AUDIO_IOC_APP_HWP_SET_BALANCE:
		...
		break;
	case AUDIO_IOC_APP_HWP_SET_LIMIT_OUTPUT_DAC0:
		ret = app_hwp_set_limit_output_dac0(arg);
		break;
	case AUDIO_IOC_APP_HWP_SET_LIMIT_OUTPUT_DAC1:
		ret = app_hwp_set_limit_output_dac1(arg);
		break;
	case AUDIO_IOC_APP_HWP_SET_LIMIT_OUTPUT_I2S0:
		ret = app_hwp_set_limit_output_i2s0(arg);
		break;
	case AUDIO_IOC_APP_HWP_SET_EQ_DIRECT_PARAMETER:
		ret = app_hwp_set_eq_dir_parameter(arg);
		break;
	case AUDIO_IOC_APP_HWP_SET_LOUD_DIRECT_PARAMETER:
		ret = app_hwp_set_loud_dir_parameter(arg);
		break;		
	default:
		ret = -ENOIOCTLCMD;
		break;
...
#ifdef SUPPORT_EARC
static long Aio_Audio_HW_Earc_Init(void __user *arg)
{
	if (Audio_HW_Earc_Init() != 0)
		return -ENOTTY;
	return 0;
}

static long Aio_Audio_HW_Earc_Deinit(void __user *arg)
{
	if (Audio_HW_Earc_Deinit() != 0)
		return -ENOTTY;
	return 0;
}

static long Aio_Audio_HW_Earc_Get_Connect_State(void __user *arg)
{
	AIO_EARC_STATE_T d;
	
	if (Audio_HW_Earc_Get_Connect_State(&d) != 0)
		return -ENOTTY;
...
static long Aio_Audio_HW_Earc_Get_Rx_Status(void __user *arg)
{
	AIO_AUDIO_EARC_RX_STATUS_T d;
	
	if (Audio_HW_Earc_Get_Rx_Status(&d) != 0)
		return -ENOTTY;
...
static long Aio_Audio_HW_Earc_Read_Transaction(void __user *arg)
{
	#if 0
	AIO_AUDIO_EARC_CMDC_READ_T d;
	if(!arg) //tained data sanitize, fix coverity check
	{
		alog_err("null Aio_Audio_HW_Earc_Read_Transaction parameter\n");
		return -ENOTTY;
	}else
	data_from_user(d, arg);

	if (Audio_HW_Earc_Read_Transaction(&d) != 0)
		return -ENOTTY;

	data_to_user(d, arg);
	#endif
	alog_err("null Aio_Audio_HW_Earc_Read_Transaction\n");
	return 0;
}

static long Aio_Audio_HW_Earc_Write_Latency(void __user *arg)
{
	uint8_t d;

	data_from_user(d, arg);
	if (Audio_HW_Earc_Write_Latency(d) != 0)
		return -ENOTTY;
...
static long Aio_Audio_HW_Earc_ARC_Switch(void __user *arg)
{
	AIO_AUDIO_EARC_ARC_SWITCH_T d;

	data_from_user(d, arg);
	if (Audio_HW_Earc_Arc_Switch(d) != 0)
		return -ENOTTY;
...
static long Aio_Audio_AQ_Loopback_Verify(void __user *arg)
{
	AIO_HWP_SET_DMA_T d;

	data_from_user(d, arg);
	if (Audio_AQLoopBackVerify(d)!=0)
		return -ENOTTY;
	return 0;
}
...
static long audio_hw_aio_ioctl_handler(unsigned int cmd, void __user *arg)
{
	...
	case AUDIO_IOC_AIO_SET_OPT_SRC_I2S:
		...
		break;
	#ifdef SUPPORT_EARC
	case AUDIO_IOC_AIO_EARC_INIT:
		ret = Aio_Audio_HW_Earc_Init(arg);
		break;
	case AUDIO_IOC_AIO_EARC_DEINIT:
		ret = Aio_Audio_HW_Earc_Deinit(arg);
		break;
	case AUDIO_IOC_AIO_EARC_GET_CONNECT_STATE:
		ret = Aio_Audio_HW_Earc_Get_Connect_State(arg);
		break;
	case AUDIO_IOC_AIO_EARC_GET_RX_STATUS:
		ret = Aio_Audio_HW_Earc_Get_Rx_Status(arg);
		break;
	case AUDIO_IOC_AIO_EARC_READ_TRANSACTION:
		ret = Aio_Audio_HW_Earc_Read_Transaction(arg);
		break;
	case AUDIO_IOC_AIO_EARC_WRITE_LATENCY:
		ret = Aio_Audio_HW_Earc_Write_Latency(arg);
		break;
	case AUDIO_IOC_AIO_EARC_ARC_SWITCH:
		ret = Aio_Audio_HW_Earc_ARC_Switch(arg);
		break;
	case AUDIO_IOC_AIO_AQ_LOOPBACK_VERIFY:
		ret = Aio_Audio_AQ_Loopback_Verify(arg);
		break;		
	#endif
	default:
		ret = -ENOIOCTLCMD;
