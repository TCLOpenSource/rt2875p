...
irqreturn_t AudioHw_Earc_Isr(int irq, void *dev_id)
{
	...
	AIO_WriteRegister(SB2_CPU_SW_INT_2_reg,eARC_FROM_ACPU_mask); //clear with write_data bit 0.
       /* put your handler */
       //alog_info("%s: get interrupt swint(0x%x), cpu_time(%llx) \n", __func__, swint, sched_clock())
	   //alog_err("earc_isr=%x\n",swint);
...
//    EARC_ANA_2.earc_test_mode = 0; //Disable ARC Internal Pattern//mask it, due to acpu already control it
...
void CMRx_KOffset_Flow(void)
{
	earcmisc_earc_db_ctrl_RBUS EARC_DB_CTRL;
	Aphy_reserved_extra_T EARC_CMRx_CTRL;
	earcmisc_earc_koff_RBUS EARC_KOFF_CTRL;
	unsigned int Koff_to=200;

	alog_err("EARC KOFF s\n");	
	EARC_DB_CTRL.regValue = AIO_ReadRegister(EARCMISC_earc_db_ctrl_reg);
	EARC_CMRx_CTRL.regValue=EARC_DB_CTRL.aphy_reserved_extra;
	EARC_CMRx_CTRL.CMRx_KOFF_En=1;
	EARC_CMRx_CTRL.CMRx_Vref_Sel=1;//KOFF DAC	
	EARC_DB_CTRL.aphy_reserved_extra=EARC_CMRx_CTRL.regValue;
	AIO_WriteRegister(EARCMISC_earc_db_ctrl_reg, EARC_DB_CTRL.regValue);

	EARC_KOFF_CTRL.regValue = AIO_ReadRegister(EARCMISC_earc_koff_reg);
	EARC_KOFF_CTRL.offset_divisor=0xf;
	EARC_KOFF_CTRL.offset_stable_cnt=0x2;
	EARC_KOFF_CTRL.offset_delay_cnt=1;
	EARC_KOFF_CTRL.offset_pc=0;
	EARC_KOFF_CTRL.offset_en=1;
	AIO_WriteRegister(EARCMISC_earc_koff_reg, EARC_KOFF_CTRL.regValue);

	while(1)
	{
		audio_hw_usleep(10);
		EARC_KOFF_CTRL.regValue = AIO_ReadRegister(EARCMISC_earc_koff_reg);		
		if(EARC_KOFF_CTRL.z0_ok==1)
		{
			EARC_DB_CTRL.regValue = AIO_ReadRegister(EARCMISC_earc_db_ctrl_reg);
			EARC_CMRx_CTRL.regValue=EARC_DB_CTRL.aphy_reserved_extra;
			EARC_CMRx_CTRL.CMRx_KOFF_Offset_Code=4;  // 4*15=60mV
			EARC_DB_CTRL.aphy_reserved_extra=EARC_CMRx_CTRL.regValue;
			AIO_WriteRegister(EARCMISC_earc_db_ctrl_reg, EARC_DB_CTRL.regValue);		
			alog_err("EARC KOFF e\n");	
			break;
		}
		Koff_to--;
		if(Koff_to==0)
		{
			alog_err("EARC KOFF timeout, back to hysteresis window\n");
			EARC_DB_CTRL.regValue = AIO_ReadRegister(EARCMISC_earc_db_ctrl_reg);
			EARC_CMRx_CTRL.regValue=EARC_DB_CTRL.aphy_reserved_extra;
			EARC_CMRx_CTRL.CMRx_KOFF_En=0;
			EARC_CMRx_CTRL.CMRx_Vref_Sel=0;//RC filter
			EARC_CMRx_CTRL.CMRx_Hy_Wd_Sel=0x2; // 70mV
			EARC_DB_CTRL.aphy_reserved_extra=EARC_CMRx_CTRL.regValue;
			AIO_WriteRegister(EARCMISC_earc_db_ctrl_reg, EARC_DB_CTRL.regValue);
			break;
...
void Audio_HW_Earc_APHY_Update(AIO_FS_ID freq)
{
	...
	}else
	{
		...
		alog_debug("freq=%x, back to default=%x\n",freq,EARC_ANA_1.reserved_aphy);	
	}	
}
...
void Audio_HW_Earc_OTPCheck(void)
{
	...
	EARC_OTP_Z0.regValue = AIO_ReadRegister(AIO_EARC_OTP_Z0_REG);
	...
	EARC_OTP.regValue = AIO_ReadRegister(AIO_EARC_OTP_REG);	
	...
	if(EARC_OTP.Hy_Wd_Done==1)	
	{
		alog_err("EARC OTP Found:Wd = %d\n",EARC_OTP.Hy_Wd_Sel);		
		EARC_DB_CTRL.regValue = AIO_ReadRegister(EARCMISC_earc_db_ctrl_reg);
		EARC_CMRx_CTRL.regValue=EARC_DB_CTRL.aphy_reserved_extra;
		EARC_CMRx_CTRL.CMRx_Hy_Wd_Sel=EARC_OTP.Hy_Wd_Sel;  // 30x:90mv,2:70mv,1:60mv,0:50mv
		EARC_DB_CTRL.aphy_reserved_extra=EARC_CMRx_CTRL.regValue;
		AIO_WriteRegister(EARCMISC_earc_db_ctrl_reg, EARC_DB_CTRL.regValue);
	}
}
...
void Audio_HW_Earc_SetAPHY(void)
{
	...
	EARC_ANA_0.regValue = AIO_ReadRegister(EARCMISC_earc_ana_0_reg);
	EARC_ANA_0.df_trf_sel = 0x0;//rising time for compatibility
	...
	EARC_ANA_1.cm_ib_sel = 0x7;//0x6;
	EARC_ANA_1.r_en = 0xf;
	AIO_WriteRegister(EARCMISC_earc_ana_1_reg, EARC_ANA_1.regValue);

	EARC_ANA_2.regValue = AIO_ReadRegister(EARCMISC_earc_ana_2_reg);
	EARC_ANA_2.df_ib_sel = 0x7;//more swing for compatibility
	EARC_ANA_2.cm_trf_sel = 0x3;
	EARC_ANA_2.df_duty_sel = 0x7;
	EARC_ANA_2.arc_res_sel = 0x7;
	EARC_ANA_2.vol_level_extra = 0x3;//rising time for compatibility
	EARC_ANA_2.vol_level_cm = 0x0;
	AIO_WriteRegister(EARCMISC_earc_ana_2_reg, EARC_ANA_2.regValue);

	EARC_DB_CTRL.regValue = AIO_ReadRegister(EARCMISC_earc_db_ctrl_reg);
	EARC_CMRx_CTRL.regValue=EARC_DB_CTRL.aphy_reserved_extra;
	EARC_CMRx_CTRL.CMRx_Hy_Wd_Sel=0;  // 30x:90mv,2:70mv,1:60mv,0:50mv
	EARC_DB_CTRL.aphy_reserved_extra=EARC_CMRx_CTRL.regValue;
	AIO_WriteRegister(EARCMISC_earc_db_ctrl_reg, EARC_DB_CTRL.regValue);
	...
	CMDC_TIMING0.tx_disconn = 0x10; //6dc0[30]=1, turn on pramble-1 patch
