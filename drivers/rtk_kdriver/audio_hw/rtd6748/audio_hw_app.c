...
#define AUDIO_OTP_REG	(0xb8038188)
...
int32_t Audio_HwpSetMuteInternal(bool para, uint32_t ch_id, uint32_t mute_id)
{
	...
	if (para == AIO_ENABLE) {
		...
		for (i = 0; i < APP_MUTE_NUM; i++){
			mask |= m_mute[i];
			alog("[AUDIO][APP] Unmute mute[%d] = %d, mask = %d\n", i, m_mute[i], mask);
		}
...
static int set_mute_ts(unsigned int para, unsigned int ch_id)
{
	...
	if (ch_id&APP_CH_ID_CH0) {
		...
		TS_DVOL_CTRL2_REG.hpf_lr_enable = AIO_ENABLE; //AIO_ENABLE for RTD2874
	...
	if (ch_id&APP_CH_ID_CH1) {
		...
		TS_DVOL_CTRL2_REG.hpf_rr_enable = AIO_ENABLE; //AIO_ENABLE for RTD2874
		AIO_WriteRegister(AUDIO_LRRR_Digital_Volume_Control_2_reg, TS_DVOL_CTRL2_REG.regValue);
	}
...
int32_t Audio_HwpSetAVC(bool para, APP_AVC_CFG *p_cfg_avc)
{
	...
	/* --------------------------------------------------*/
	/* ?â????? ?â????? ?â????? ?â???????
	 * ?AVC1  ?â??¤dvol  ?â??¤AVC2  ?â??¤HW-limiter  ?
	 * ?â????? ?â????? ?â????? ?â???????
	 * --------------------------------------------------*/
	/* ryanliao
...
static int set_loud_coef(unsigned int band_id)
{
	...
	// [Clayton] Fix coverity issue, a1shift must be 1
...
// [Clayton] Fix coverity issue, h0shift must be 0
/*
...
int32_t Audio_HwpSetBassValue_48K(bool para, APP_BASS_VAL val_bass)
{
	audio_loudness_control_register_RBUS LOUD_CTRL_REG;
	audio_hw_process_control_RBUS        HWP_CTRL_REG;
	APP_BASS_VAL                         u_val_bass,*p_val_bass;

	//if (p_val_bass == NULL)
	//	return AIO_ERR_APP_INVALID_BASS_VOL;
	p_val_bass=&val_bass; //coverity
	if (para == AIO_ENABLE) {
		// 2016/6/2 Clayton
		// Always Follow standard flow to set EQ parameters
		// [start]
...
int32_t Audio_HwpSetTrebleValue(bool para, APP_TREBLE_VAL val_treble)
{
	audio_loudness_control_register_RBUS LOUD_CTRL_REG;
	audio_hw_process_control_RBUS        HWP_CTRL_REG;
	APP_TREBLE_VAL                       u_val_treble,*p_val_treble;

	//if (p_val_treble == NULL)
	//	return AIO_ERR_APP_INVALID_TREBLE_VOL;
	p_val_treble=&val_treble;//coverity
	if (para == AIO_ENABLE) {
		// 2016/6/2 Clayton
		// Always Follow standard flow to set EQ parameters
		// [start]
...
int32_t Audio_HwpSetTreble2Value(bool para, APP_TREBLE_VAL val_treble2)
{
	audio_loudness_control_register_RBUS LOUD_CTRL_REG;
	audio_hw_process_control_RBUS        HWP_CTRL_REG;
	APP_TREBLE_VAL                       u_val_treble2,*p_val_treble2;

	//if (p_val_treble2 == NULL)
	//	return AIO_ERR_APP_INVALID_TREBLE_VOL;
	p_val_treble2=&val_treble2;//coverity
	if (para == AIO_ENABLE) {
		// 2016/6/2 Clayton
		// Always Follow standard flow to set EQ parameters
		// [start]
...
int32_t Audio_HwpSetTreble2Value_48K(bool para, APP_TREBLE_VAL val_treble2)
{
	...
	p_val_treble2=&val_treble2;//for coverity issue
	if (para == AIO_ENABLE) {
		// 2016/6/2 Clayton
		// Always Follow standard flow to set EQ parameters
		// [start]
...
int audio_drv_set_eq_val(APP_EQ_VAL *p_eq_val)
{
	...
	if (band_idx < MAX_EQ_BAND_NUM) {
		...
		if (base == MAX_EQ_VALUE)
			input = 0;
		else if(base >MAX_EQ_VALUE)
		{
			alog("base out of range");
			return AIO_ERROR;		
		}
...
#elif  (MAX_EQ_BAND_NUM ==  13)
	for (band_idx = APP_EQ_BAND_ID1; band_idx <= APP_EQ_BAND_ID11; band_idx++)		
...
int32_t Audio_HwpSetEQValue_48K(uint32_t Band_enable, APP_EQ_VAL *p_val_eq)
{
	...
	if (Band_enable != 0x0) {
		...
		if (Band_enable != curr_band_enable) {
			...
			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_hp_enable    = (((Band_enable >> 1)&0x0001) == 1) ? true : false;
				i++; }

			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_band1_enable = (((Band_enable >> 2)&0x0001) == 1) ? true : false;
				i++; }

			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_band2_enable = (((Band_enable >> 3)&0x0001) == 1) ? true : false;
				i++; }

			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_band3_enable = (((Band_enable >> 4)&0x0001) == 1) ? true : false;
				i++; }

			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_band4_enable = (((Band_enable >> 5)&0x0001) == 1) ? true : false;
				i++; }

			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_band5_enable = (((Band_enable >> 6)&0x0001) == 1) ? true : false;
				i++; }

			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_band6_enable = (((Band_enable >> 7)&0x0001) == 1) ? true : false;
				i++; }

			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_band7_enable = (((Band_enable >> 8)&0x0001) == 1) ? true : false;
				i++; }
#if  (MAX_EQ_BAND_NUM > 9)
			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_band8_enable = (((Band_enable >> 9)&0x0001) == 1) ? true : false;
				i++; }

			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_band9_enable = (((Band_enable >> 8)&0x0001) == 1) ? true : false;
				i++; }
#if (MAX_EQ_BAND_NUM > 11)
			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_band10_enable = (((Band_enable >> 9)&0x0001) == 1) ? true : false;
				i++; }

			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_band11_enable = (((Band_enable >> 10)&0x0001) == 1) ? true : false;
				i++; }
#if (MAX_EQ_BAND_NUM > 13)
			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_band12_enable = (((Band_enable >> 11)&0x0001) == 1) ? true : false;
				i++; }

			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_band13_enable = (((Band_enable >> 12)&0x0001) == 1) ? true : false;
				i++; }

			if (i < MAX_EQ_BAND_NUM) {EQ_CTRL_REG.eq_band14_enable = (((Band_enable >> 13)&0x0001) == 1) ? true : false;
				i++; }
...
/*
	...
	*/
    DVOL_CTRL_0_REG.regValue = AIO_ReadRegister(AUDIO_Digital_Volume_Control_0_reg);
    DVOL_CTRL_0_REG.dvcm = 1; //Zero cross
    AIO_WriteRegister(AUDIO_Digital_Volume_Control_0_reg, DVOL_CTRL_0_REG.regValue);
	...
	//alog("[AUDIO][APP] vol=%d, a=%d,b=%d,c=%d,ch=%d\n",vol,a,b,c,ch_id);
...
void  Audio_HwpSetLimitOuptut_DAC1(bool para, uint32_t maxLevelThresdhold)
{

    audio_dac0_1_src_sel_RBUS dac0_1_src_sel_RBUS_reg;

    dac0_1_src_sel_RBUS_reg.regValue = AIO_ReadRegister(AUDIO_dac0_1_src_sel_reg);
    dac0_1_src_sel_RBUS_reg.dac1_hw_out_limiter_en  = para;
    dac0_1_src_sel_RBUS_reg.dac1_hw_out_limiter     = maxLevelThresdhold;
    AIO_WriteRegister(AUDIO_dac0_1_src_sel_reg, dac0_1_src_sel_RBUS_reg.regValue);
...
int32_t Audio_HwpSetEQ_Smooth_H0(APP_EQ_ALL_BAND band_id, APP_EQ_COEF eq_coef)
{
	int32_t ret=0;
#ifdef USE_SMOOTHEQ_THREAD
	if((band_id>=APP_EQ_ALL_LPF_BAND1) && (band_id<=APP_EQ_ALL_BPF_BAND14))
	{
		//while(g_HwpSetEQ_Smooth_H0_En[band_id]==1)//avoid to block by smoothEQ_run func
		//{audio_hw_usleep(10);}		
		alog_err("eq cmd id=%d\n",band_id);
		d[band_id].band_id=band_id;
		d[band_id].eq_coef=eq_coef;			
		g_HwpSetEQ_Smooth_H0_En[band_id]=1;
	}
#else
	ret=Audio_HwpSetEQ_Smooth_H0_Run(band_id,eq_coef);
#endif	
	return ret;
}
...
int32_t Audio_HwpSetEQ_Smooth_H0_Run(APP_EQ_ALL_BAND band_id, APP_EQ_COEF eq_coef)
{
...
#ifdef USE_SMOOTHEQ_THREAD	
	g_HwpSetEQ_Smooth_H0_En[band_id]=0;
#endif
	//if (idx_check_step ==0)
	//{
		alog_info("[Audio][EQ]*band_id=%d tar_a1=%x, tar_a2=%x, tar_h0=%x\n", band_id, tar_a1, tar_a2, tar_h0);
		...
		for (j =0; j < eq_num; j++) //check tar_idx
		{
			...
			if (BPF_PARA[j][2] == (tar_h0&0xFFFF)){
				tar_idx = j;
				//alog_err("[Audio][EQ]**tar_idx_1=%x\n", tar_idx);
				break;
			}
			else if ((!(tar_h0&0x8000) && (BPF_PARA[j][2] < (tar_h0&0xFFFF)))||
				((tar_h0&0x8000) && (j>160) && (BPF_PARA[j][2] < (tar_h0&0xFFFF)))){
				tar_idx = j;
				if (((j==161)&&((0x10000-(tar_h0&0xFFFF)) < ((tar_h0&0xFFFF)-BPF_PARA[j][2]))) || 
					((j>0)&&((BPF_PARA[j-1][2]-(tar_h0&0xFFFF)) < ((tar_h0&0xFFFF)-BPF_PARA[j][2]))))
					tar_idx = j-1;
				GoRealTarget = true;
				//alog_err("[Audio][EQ]**tar_idx_2=%x\n", tar_idx);
				break;
			}
		}
		...
		{	
			...
			if (BPF_PARA[i][2] == (tH0&0xFFFF)){
				current_idx = i;
				//alog_err("[Audio][EQ]***current_idx_1=%x\n", current_idx);
				break;
			}
			else if ((!(tH0&0x8000) && (BPF_PARA[i][2] < (tH0&0xFFFF)))||
				((tH0&0x8000) && (i>160) && (BPF_PARA[i][2] < (tH0&0xFFFF)))){
				current_idx = i;
				if (((i==161)&&((0x10000-(tH0&0xFFFF)) < ((tH0&0xFFFF)-BPF_PARA[i][2]))) || 
					((i>0)&&((BPF_PARA[i-1][2]-(tH0&0xFFFF)) < ((tH0&0xFFFF)-BPF_PARA[i][2]))))
					current_idx = i-1;
					//alog_err("[Audio][EQ]***current_idx_2=%x\n", current_idx);
				break;
			}
		}

		if((current_idx != i && current_idx != (i-1)) || (tar_idx != j && tar_idx != (j-1)))	
				alog_err(" [Audio][EQ]......@@......current_idx and tar_idx check error\n");
		if (tar_idx >= eq_num)
			tar_idx = eq_num - 1;
		//else if (tar_idx < 0)//unsigned compare, fix coverity
		//	tar_idx = 0;
		//idx_check_step = 1;
			
	//}

		alog_info("[Audio][EQ]****tA1=%x, tA2=%x, tH0=%x\n", tA1, tA2, tH0);
		alog_info("[Audio][EQ]*****check current_idx=%x, tar_idx=%x Address=%x\n", current_idx, tar_idx, Audio_HwpReadRegister(H0Address));
	...
	do{
		...
		if(g_HwpSetEQ_Smooth_H0_En[band_id]==1)
		{
			tar_a1 = d[band_id].eq_coef.a1;
			tar_a2 = d[band_id].eq_coef.a2;  // dummy for LPF,HPF
			tar_h0 = d[band_id].eq_coef.h0;	
			for (j =0; j < eq_num; j++) //check tar_idx
			{			
				if (BPF_PARA[j][2] == (tar_h0&0xFFFF)){
					tar_idx = j;
					alog_info("Update New [Audio][EQ]**tar_idx=%x\n", tar_idx);
					break;
				}
				else if ((!(tar_h0&0x8000) && (BPF_PARA[j][2] < (tar_h0&0xFFFF)))|| ((tar_h0&0x8000) && (j>160) && (BPF_PARA[j][2] < (tar_h0&0xFFFF)))){
						tar_idx = j;
						if (((j==161)&&((0x10000-(tar_h0&0xFFFF)) < ((tar_h0&0xFFFF)-BPF_PARA[j][2]))) || ((j>0)&&((BPF_PARA[j-1][2]-(tar_h0&0xFFFF)) < ((tar_h0&0xFFFF)-BPF_PARA[j][2])))){
							tar_idx = j-1;
						}
						GoRealTarget = true;
				//alog_err("[Audio][EQ]**tar_idx_2=%x\n", tar_idx);
			...
			alog_info("Old [Audio][EQ] check current_idx=%x, tar_idx=%x, H0Address=%x\n", current_idx, tar_idx, Audio_HwpReadRegister(H0Address));			
			alog_info("Update New [Audio][EQ]*band_id=%d tar_a1=%x, tar_a2=%x, tar_h0=%x\n", band_id, tar_a1, tar_a2, tar_h0);
			state = COMPARE;
			tmp_state = 0;			
			g_HwpSetEQ_Smooth_H0_En[band_id]=0;
		}
		switch(state){
			...
			case P2P: {
				alog_info("[Audio][EQ]P2P\n");
				tmp_state = P2P;
				state = GOZERO;
			}break;
			
			case N2N: {
				alog_info("[Audio][EQ]N2N\n");
				tmp_state = N2N;
				state = GOZERO;
			}break;
			
			case GOZERO: { 
				alog_info("[Audio][EQ]GOZERO\n");
				if(current_idx == 0xa0){
			...
			case SETNEWVAL2: {
				if ((tar_a1 == tA1 && tar_a2 == tA2)||(tar_a1 == tA1 && band_id == 0)){
					alog_info("[Audio][EQ]SETNEWVAL2_0\n");
					state = GOTARGET;
				}else{
					alog_info("[Audio][EQ]SETNEWVAL2_1\n");
			...
			case GOTARGET: {
				alog_info("[Audio][EQ]GOTARGET\n");
				alog_info("GoRealTarget=%x, tar_h0=%x\n",GoRealTarget,tar_h0);
				...
				if ((GoRealTarget == true) && (current_idx == tar_idx))
				{
					if (set_eq_reg(H0Address, (tar_h0&0xFFFF), h0shift) != AIO_OK)
					{
						up(&audio_sem_app);
						return AIO_ERROR;
					}
					//alog_err("[Audio][EQ]*******111*******check current_idx=%x, tar_idx=%x, target_value=%x, Address=%x\n", current_idx, tar_idx, tar_h0, Audio_HwpReadRegister(H0Address));
					if(AIO_ReadRegister(AUDIO_EQ_Status_Register_reg) >0 ) alog_err(" [Audio][Overflow][GOTARGET] %x\n",AIO_ReadRegister(AUDIO_EQ_Status_Register_reg));//Detect oveflow
					audio_hw_usleep(t2);
					GoRealTarget = false;
				}
			...
			case N2P: {alog_info("[Audio][EQ]N2P\n"); state = INCREASE;} break;
			case P2N: {alog_info("[Audio][EQ]P2N\n"); state = DECREASE;}break;
			
			case DECREASE :{ //P2N
				alog_info("[Audio][EQ]DECREASE\n");
				if(current_idx == 0xa0){
			...
			case SETNEWVAL:{
				if ((tar_a1 == tA1 && tar_a2 == tA2)||(tar_a1 == tA1 && band_id == 0)){
					alog_info("[Audio][EQ]SETNEWVAL_0\n");
					state = tmp_state;
				}else{
					alog_info("[Audio][EQ]SETNEWVAL_1\n");
					if (band_id == 0){//LPF PARA
						if (set_eq_reg(A1Address, 0, a1shift) != AIO_OK)
						{
							up(&audio_sem_eq);
			...
			case FINISH:{state = COMPARE; Finish = true; alog_info("[Audio][EQ][Finish]%x\n", current_idx);}break;
		}
	}while(!Finish);


alog_info("[Audio][EQ][Smooth]OKOKOKOKOKOK******\n");
up(&audio_sem_eq);
return AIO_OK;
...
int32_t Audio_HwpSetLoud_Smooth_H0(APP_LOUD_ALL_BAND band_id, APP_EQ_COEF loud_coef)
{
	int32_t ret=0;
#ifdef USE_SMOOTHEQ_THREAD
	if((band_id<=APP_LOUD_ALL_BPF_BAND2) && (band_id>=APP_LOUD_ALL_LPF_BAND1))
	{
		//while(g_HwpSetLoud_Smooth_H0_En[band_id]==1)//avoid to block by smoothEQ_run func
		//{audio_hw_usleep(10);}
		alog_err("loud cmd id=%d\n",band_id);
		k[band_id].band_id=band_id;
		k[band_id].loud_coef=loud_coef;		
		g_HwpSetLoud_Smooth_H0_En[band_id]=1;	
	}
#else
	ret=Audio_HwpSetLoud_Smooth_H0_Run(band_id,loud_coef);
#endif	
	return ret;
}
int32_t Audio_HwpSetLoud_Smooth_H0_Run(APP_LOUD_ALL_BAND band_id, APP_EQ_COEF loud_coef)
{
...
#ifdef USE_SMOOTHEQ_THREAD	
	g_HwpSetLoud_Smooth_H0_En[band_id]=0;	
#endif

	//if (idx_check_step ==0)
	//{
		alog_info("[Audio][Loud]*band_id=%d tar_a1=%x, tar_a2=%x, tar_h0=%x\n", band_id, tar_a1, tar_a2, tar_h0);
		...
		for (j =0; j < eq_num; j++) //check tar_idx
		{
			...
			if (BPF_PARA[j][2] == (tar_h0&0xFFFF)){
				tar_idx = j;
				//alog_err("[Audio][Loud]**tar_idx_1=%x\n", tar_idx);
				break;
			}
			else if ((!(tar_h0&0x8000) && (BPF_PARA[j][2] < (tar_h0&0xFFFF)))||
				((tar_h0&0x8000) && (j>160) && (BPF_PARA[j][2] < (tar_h0&0xFFFF)))){
				...
				if (((j==161)&&((0x10000-(tar_h0&0xFFFF)) < ((tar_h0&0xFFFF)-BPF_PARA[j][2]))) || 
					...
					tar_idx = j-1;
				GoRealTarget = true;
				//alog_err("[Audio][Loud]**tar_idx_2=%x\n", tar_idx);
				break;
			}
		}
		...
		{	
			...
			if (BPF_PARA[i][2] == (tH0&0xFFFF)){
				current_idx = i;
				//alog_err("[Audio][Loud]***current_idx_1=%x\n", current_idx);
				break;
			}
			else if ((!(tH0&0x8000) && (BPF_PARA[i][2] < (tH0&0xFFFF)))||
				((tH0&0x8000) && (i>160) && (BPF_PARA[i][2] < (tH0&0xFFFF)))){
				...
				if (((i==161)&&((0x10000-(tH0&0xFFFF)) < ((tH0&0xFFFF)-BPF_PARA[i][2]))) || 
					((i>0)&&((BPF_PARA[i-1][2]-(tH0&0xFFFF)) < ((tH0&0xFFFF)-BPF_PARA[i][2]))))
					current_idx = i-1;
					//alog_err("[Audio][Loud]***current_idx_2=%x\n", current_idx);
				break;
			}
		}

		if((current_idx != i && current_idx != (i-1)) || (tar_idx != j && tar_idx != (j-1)))	
				alog_err(" [Audio][Loud]......@@......current_idx and tar_idx check error\n");
		if (tar_idx >= eq_num)
			tar_idx = eq_num - 1;
		//else if (tar_idx < 0) //unsigned compare, fix coverity
		//	tar_idx = 0;
	...
	//}
		...
		alog_info("[Audio][Loud]****tA1=%x, tA2=%x, tH0=%x\n", tA1, tA2, tH0);
		alog_info("[Audio][Loud]*****check current_idx=%x, tar_idx=%x Address=%x\n", current_idx, tar_idx, Audio_HwpReadRegister(H0Address));
		if(AIO_ReadRegister(AUDIO_EQ_Status_Register_reg) >0 )	alog_err(" [Audio][Overflow]do start %x\n",AIO_ReadRegister(AUDIO_EQ_Status_Register_reg));//Detect oveflow

	do{
		if(g_HwpSetLoud_Smooth_H0_En[band_id]==1)
		{
			tar_a1 = k[band_id].loud_coef.a1;
			tar_a2 = k[band_id].loud_coef.a2;  // dummy for LPF,HPF
			tar_h0 = k[band_id].loud_coef.h0;	
			for (j =0; j < eq_num; j++) //check tar_idx
			{			
				if (BPF_PARA[j][2] == (tar_h0&0xFFFF)){
					tar_idx = j;
					alog_info("Update New [Audio][Loud]**tar_idx=%x\n", tar_idx);
					break;
				}
			}			
			alog_info("Old [Audio][Loud] check current_idx=%x, tar_idx=%x, H0Address=%x\n", current_idx, tar_idx, Audio_HwpReadRegister(H0Address));			
			alog_info("Update New [Audio][Loud]*band_id=%d tar_a1=%x, tar_a2=%x, tar_h0=%x\n", band_id, tar_a1, tar_a2, tar_h0);
			state = COMPARE;
			tmp_state = 0;			
			g_HwpSetLoud_Smooth_H0_En[band_id]=0;
		}		
		...
		switch(state){
			...
			case P2P: {
				alog_info("[Audio][Loud]P2P\n");
				tmp_state = P2P;
				state = GOZERO;
			}break;
			
			case N2N: {
				alog_info("[Audio][Loud]N2N\n");
				tmp_state = N2N;
				state = GOZERO;
			}break;
			
			case GOZERO: { 
				alog_info("[Audio][Loud]GOZERO\n");
				if(current_idx == 0xa0){
			...
			case SETNEWVAL2: {
				if ((tar_a1 == tA1 && tar_a2 == tA2)||(tar_a1 == tA1 && band_id == 0)){
					alog_info("[Audio][Loud]SETNEWVAL2_0\n");
					state = GOTARGET;
				}else{
					alog_info("[Audio][Loud]SETNEWVAL2_1\n");
					if (band_id == 0){//LPF PARA
						if (set_eq_reg(A1Address, 0, a1shift) != AIO_OK)
						{
							up(&audio_sem_loud);
			...
			case GOTARGET: {
				...
				if ((GoRealTarget == true) && (current_idx == tar_idx))
				{
					...
					//alog_err("[Audio][Loud]*******111*******check current_idx=%x, tar_idx=%x, target_value=%x, Address=%x\n", current_idx, tar_idx, tar_h0, Audio_HwpReadRegister(H0Address));
					if(AIO_ReadRegister(AUDIO_EQ_Status_Register_reg) >0 ) alog_err(" [Audio][Overflow][GOTARGET] %x\n",AIO_ReadRegister(AUDIO_EQ_Status_Register_reg));//Detect oveflow
					audio_hw_usleep(t2);
					GoRealTarget = false;
				}
			...
			case N2P: {alog_info("[Audio][Loud]N2P\n"); state = INCREASE;} break;
			case P2N: {alog_info("[Audio][Loud]P2N\n"); state = DECREASE;}break;
			
			case DECREASE :{ //P2N
				alog_info("[Audio][Loud]DECREASE\n");
				if(current_idx == 0xa0){
			...
			case SETNEWVAL:{
				if ((tar_a1 == tA1 && tar_a2 == tA2)||(tar_a1 == tA1 && band_id == 0)){
					alog_info("[Audio][Loud]SETNEWVAL_0\n");
					state = tmp_state;
				}else{
					alog_info("[Audio][Loud]SETNEWVAL_1\n");
					if (band_id == 0){//LPF PARA
						if (set_eq_reg(A1Address, 0, a1shift) != AIO_OK)
						{
							up(&audio_sem_loud);
			...
			case FINISH:{state = COMPARE; Finish = true; alog_info("[Audio][Loud][Finish]%x\n", current_idx);}break;
		}
	}while(!Finish);


alog_info("[Audio][Loud][Smooth]OKOKOKOKOKOK******\n");
up(&audio_sem_loud);
return AIO_OK;
...
{	
	...
	tA1 = Audio_HwpReadRegister(A1Address);
	// [Clayton] Fix corerity, a1shift must be 1
	//if (a1shift == AIO_ENABLE)
	tA1 = (tA1&0xFFFF0000) >> 16;
	/*
	else
	...
	tH0 = Audio_HwpReadRegister(H0Address);
	// [Clayton] Fix corerity, h0shift must be 0
	/*
	if (h0shift == AIO_ENABLE)
		tH0 = (tH0&0xFFFF0000) >> 16;
	else
...
{	
	...
	csw_loud_bp1_1_reg.regValue = AIO_ReadRegister(AUDIO_CSW_Loud_bp1_1_reg);
	csw_loud_bp1_2_reg.regValue = AIO_ReadRegister(AUDIO_CSW_Loud_bp1_2_reg);
	csw_loud_bp2_1_reg.regValue = AIO_ReadRegister(AUDIO_CSW_Loud_bp2_1_reg);
	csw_loud_bp2_2_reg.regValue = AIO_ReadRegister(AUDIO_CSW_Loud_bp2_2_reg);
#endif
...
int32_t Audio_HwpSetCH23AVC2(bool para, APP_AVC2_CFG *p_cfg_avc2)
{
    ...
    if (para == AIO_ENABLE) {
        ...
        AVC2_CTRL1_REG.avc2_zc_win_lsrs = t_cfg_avc2->avc2_mon_win;
