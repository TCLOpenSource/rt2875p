...
// FIXME: USB storage driver won't provide this API anymore.
static int get_usb_storage_mount_path_one(char *buf, size_t size){
	return 0;
}
#else
...
//	mode = drvif_module_vdc_factory_debug_get_dma_mode();
	strncpy(prefix, Audio_dma_name_table[mode], 32);
}
...
#ifdef kernel_space_dumpdma
ulong m_audio_pCasheStartAddr = 0;
//unsigned int m_audio_PhyStartAddr = 0;
ulong m_audio_pNonCacheStartAddr = 0;
#else
...
//	int iFileNum;
	...
	//IoReg_SetBits(0xb8027220, _BIT1);  // cap1 freeze enable //need to fix by ryanlan

	iStartAddr = (ulong *)m_audio_pNonCacheStartAddr;
	if(!iStartAddr) //null check, fix coverity
		return;
	else{
	...
	if (iStartAddr != 0 && iSize != 0) {
		...
		sprintf(sFilePath, "%s", "chmod 777 /mnt/usbmounts/sda1/*");
		sprintf(sFilePath, "%s/%s%s%s", "/mnt/usbmounts/sda1", "DMA_", sFileName, ".bin"); //Dump to USB
	#else//------NFS----------//
		system("mount -t nfs -o tcp,nolock 172.21.15.135:/home/RTDOMAIN/jenniferchang/nfs/ /nfs");
		sprintf(sFilePath, "%s/%s%s%s", "/nfs", "VdDma_", sFileName, ".bin"); //Dump to NFS
	#endif
#endif //kernel_space_dumpdma
		alog_info("%s\n", sFilePath);
...
//			alog_info(".");
			...
			if (iBufIdx >= 128) {
				...
				loff_t tmp_ofst = offset;
				rc = kernel_write(pFile, (const char *)bufArray, 128 * 4, &tmp_ofst);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
			loff_t tmp_ofst = offset;
			kernel_write(pFile, (const char *)bufArray, iBufIdx * 4, &tmp_ofst);
#else
	...
	//IoReg_ClearBits(0xb8027220, _BIT1); // cap1 freeze disable //need to fix by ryanlan
	#ifdef kernel_space_dumpdma
	kfree(buf);
	#endif
		}	
...
#ifdef kernel_space_dumpdma
	m_audio_pCasheStartAddr = (ulong)dvr_malloc_uncached_specific(Length, GFP_DCU2_FIRST,(void **)&m_audio_pNonCacheStartAddr);
	if(m_audio_pCasheStartAddr == 0)	{
		alog_info("%s.%d\n", __FUNCTION__, __LINE__);
		alog_info("alloc idma sequence memory failed[%lx]\n", m_audio_pCasheStartAddr);
		return FALSE;
	}
	nAddr = (ulong)dvr_to_phys((void *)m_audio_pCasheStartAddr);
	alog_info("cache:0x%lx,noncache:0x%lx,phy:0x%x\n",m_audio_pCasheStartAddr,m_audio_pNonCacheStartAddr,nAddr);
#else
	#if 0
...
int audio_hw_dbg_EXECUTE(const int cmd_index, char **cmd_pointer)
{
	...
	case AUDIO_HW_DBG_CMD_EARC_DEBUG:
	{
		#ifdef SUPPORT_EARC
		int32_t EARC_DEBUG_EN = 0;
		
		if (*cmd_pointer == NULL) {
			alog_err("earc debug command is incorrect.\n");
			return -1;
		}

		token = strsep(cmd_pointer, delim);
		if (token == NULL || kstrtoint(token, 0, &EARC_DEBUG_EN) != 0) {
			alog_err("failed to parse the earc debug enable flag.\n");
			return -1;
		}

		AioEarcSetDebugFlag(EARC_DEBUG_EN);
		#else
		alog_err("[AudioHw]Not Support eARC\n");
		#endif
		break;
	}
	default:
