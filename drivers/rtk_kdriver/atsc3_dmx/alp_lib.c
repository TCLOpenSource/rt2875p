...
void atsc_memcpy(unsigned char* dst, unsigned char* src, int len)
{
        memcpy(dst, src, len);
}
...
)
{
        ...
        if (alp == NULL || alp_len < 1) {
                ATSC3_DMX_WARN("Parse ALP failed - invalid parameters (alp_len = %d) ken\n", alp_len);
                return -1;
        }
        ...
        if (ALP_PACKET_TYPE(alp) == ALP_PACKET_TYPE_ID_MPEG_2_TS) {
                ...
                if(payload_size < 3) {
                        ATSC3_DMX_WARN("payload_size=%d\n", payload_size);
                        return -1;
                }

                //ATSC3_DMX_WARN("alp_get_alp_size, alp {%02x %02x}\n", alp[0], alp[1]);
                ...
                //ATSC3_DMX_WARN("alp_get_alp_size, Type=(Generic), payload_size=%d, length=%d\n", ret, payload_size);
                return ret;
        }
...
)
{
        ...
        if (p_info == NULL || alp == NULL || alp_data_len == 0) {
                ATSC3_DMX_WARN("Parse ALP failed - invalid parameters\n");
                return -1;
        }
        ...
        switch(packet_type) {
                ...
                case ALP_PACKET_TYPE_ID_LINK_LAYER_SIGINALING_PACKET:
                case ALP_PACKET_TYPE_ID_PACKET_TYPE_EXTENSION:
                        ATSC3_DMX_WARN("Parse ALP failed - unsupported packet type - %d (%s)\n",
