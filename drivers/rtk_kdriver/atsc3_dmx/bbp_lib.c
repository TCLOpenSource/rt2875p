...
)
{
        ...
        if (bbp[0] & 0x80) {
                ...
                switch (p_info->ofi) {
                        ...
                        case 3:
                                p_info->ext.type = bbp[2] >> 5;
                                p_info->ext.len  = (bbp[3] << 5) + (bbp[2] & 0x1F);
        ...
        if (p_info->payload_start > bbp + bbp_len) {    // out of bound
                ATSC3_DMX_WARN("bbp_parse_bbp failed, payload out of bound\n");
                return -1;
        }

        p_info->payload_size = bbp + bbp_len - p_info->payload_start;

        ATSC3_DMX_DBG("bbp_parse_bbp, pointer=%u, payload_size =%u\n",
                p_info->pointer, p_info->payload_size );

        // parse payload ..............
        if ( p_info->payload_size > 0 && p_info->payload_size <= 8191) {  // has ALPs
                ...
                if (p_info->pointer != 0) { // has ALP tail
                        ...
                        if (p_info->pointer == 8191) {
                                p_info->alps[0].size = p_info->payload_size;
                        } else {
                                p_info->alps[0].size  = p_info->pointer;
                        }
                        alp += p_info->alps[0].size;
                        payload_size -= p_info->alps[0].size;
                        alp_cnt++;
                }
                ...
                while(payload_size) {
                        ...
                        ATSC3_DMX_DBG("ALP[%d] data=%p, size=%u, flags=%x\n",
                                alp_cnt,
...
end_proc:
        //printf("bbp_get_header_size HDR = {%02x %02x %02x %02x } size=%d\n",
...
void bbp_dump_bbp_info(bbp_info* p_info)
{
        ...
        ATSC3_DMX_INFO("BBP CTX, pointer=%d, ext={%p, %u}, payload={%p, %u}, n_alp=%u\n",
                 p_info->pointer,
                 p_info->ext.data,
                 p_info->ext.len,
        ...
        for (i = 0; i < p_info->n_alps; i++) {
                ATSC3_DMX_INFO("BBP ALP[%d]={%p, %u, %x}\n",
                         i,
                         p_info->alps[i].data,
                         p_info->alps[i].size,
                         p_info->alps[i].flags);
