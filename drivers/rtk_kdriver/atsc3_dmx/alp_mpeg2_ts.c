...
)
{
        ...
        if (alp == NULL || alp_len < 2) {
                ATSC3_DMX_WARN("get mpeg2 ts alp size failed - invalid parameters\n");
                return -1;
        }

        if (ALP_PACKET_TYPE(alp) != ALP_PACKET_TYPE_ID_MPEG_2_TS) {
                ATSC3_DMX_WARN("get mpeg2 ts alp size failed - type is not correct\n");
                return -1;
        }
...
)
{
        ...
        if (alp == NULL || alp_len < 1 || p_info == NULL ||
            ALP_PACKET_TYPE(alp) != ALP_PACKET_TYPE_ID_MPEG_2_TS) {
                ATSC3_DMX_WARN("alp_parse_mpeg2_ts_alp failed, invalid parameters\n");
                return -1;
        }
        ...
        if ((alp[0] & 0x1)) {
                if (alp_len < 2) {
                        ATSC3_DMX_WARN("alp_parse_mpeg2_ts_alp failed, alp_len %d < 2\n", alp_len);
                        return -1;
                }
        ...
        ATSC3_DMX_DBG("p_info->mepg2_ts.hdm=%x\n", p_info->mepg2_ts.hdm);
        ...
        if (alp_len < p_info->packet_size) {     // no enough data size
                ATSC3_DMX_WARN("alp_parse_mpeg2_ts_alp failed, alp_len %d < %d\n", alp_len, p_info->packet_size);
...
)
{
        ...
        for (i = 0; i < ntsp; i++) {
                ...
                ATSC3_DMX_DBG("TSP[%d]={%02x %02x %02x %02x }...\n", i, tsp[0], tsp[1], tsp[2], tsp[3]);
                ...
                if (TSP_PID(tsp) == 0x1FFF && p_info->mepg2_ts.dpm) {
                        ...
                        ATSC3_DMX_DBG("p_info->mepg2_ts.hdm=%02x\n", p_info->mepg2_ts.hdm);

                        if (p_info->mepg2_ts.hdm) {
                                if (p_info->mepg2_ts.n_tsp == 0 || ref_hdr == NULL) {
                                        ref_hdr = data;
                                        next_cc = NEXT_CC(ref_hdr[2]);
                                } else {
                                        ATSC3_DMX_DBG("HDR Check : Curr=%02x%02x%02x, Ref=%02x%02x%02x\n",
                                        ...
                                        if ((data[0] != ref_hdr[0]) ||
                                            ...
                                            (TSP_CC(tsp) != next_cc)) {
                                                ...
                                                ATSC3_DMX_DBG("header changed, end of chaining\n");
                                                break;
                                        }
                                        ...
                                        ATSC3_DMX_DBG("header detected, remove header : next cc = %d, chained count=%d\n",
                        ...
                        ATSC3_DMX_DBG("tsp_list[%d]= data=%p, len=%d { %02x %02x %02x %02x}\n",
        ...
        for (i = 0; i < num_ts; i++) {
                *p_buff++ = 0x47;
                ...
                if (hdm) {
                        ...
                        // generate TSP
                        memcpy(p_buff, alp, 187);
                        ATSC3_DMX_DBG("000 TSP[%d] %02x %02x %02x %02x\n", i, p_buff[0], p_buff[1], p_buff[2], p_buff[3]);
