...
#define CW_B4TL(b1, b2, b3, b4)        ( (b1) + ((b2) << 8) + ((b3) << 16) + ((b4) << 24))
...
static INT32 Set_CW(UINT8 cw_index,  UINT8 *content, int len)
{
    ...
    for(i = 0; i < len && i < TPK_CW_LENGTH; i++)
        buf[i] = content[i];
    data[1] = CW_B4TL(buf[0], buf[1], buf[2], buf[3]);
    data[0] = CW_B4TL(buf[4], buf[5], buf[6], buf[7]);
                                       ...
                                       char *module_name, UINT8 *index_array, int len)
{
    ...
    if (cw_node == NULL) {
        ...
        strncpy(cw_node->module_name, module_name, CW_MODULE_NAME_LEN - 1);
...
static int single_cpu_set_continous_cws(RTK_CW_MANAGER *p_this, char *module_name, UINT8 start_index, UINT8 *content, int len)
{
    ...
    if(p_this == NULL || p_this->private == NULL)
        return -1;
    key_mm = (RTK_CW_KEY_REGION_MM *)p_this->private;
    if(module_name == NULL || start_index >= CW_AREA_LEN
       || content == NULL || len <= 0)
        return ret;
...
static int single_cpu_dump_cws_status(RTK_CW_MANAGER *p_this, char *buf)
{
    ...
    if(buf == NULL)
        return 0;
    buf[0] = 0;
    snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "%s", "******The state of the CW space******\n");
    READ_LOCK(key_mm->cw_mm_lock, flags);
    if (!list_empty(&key_mm->cw_key_region_list)) {
        int i = 0;
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "[%s]: ", "ALL");
        for(i = 0; i < CW_AREA_LEN; i++) {
            if(test_bit(i, key_mm->cw_map)) {
                snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "%d ", i);
            }
        }
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "%s", "\n");
    }
    list_for_each(plist, &key_mm->cw_key_region_list) {
        int i = 0;
        RTK_CW_KEY_REGION *tmp = list_entry(plist, RTK_CW_KEY_REGION, node);
        snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf), "[%s]: ", tmp->module_name);
        for(i = 0; i < CW_AREA_LEN; i++) {
...
int __init rtk_single_cpu_cw_module_init(void)
{
...
void rtk_single_cpu_cw_module_exit(void)
{
