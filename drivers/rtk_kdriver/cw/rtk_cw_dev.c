...
static long cw_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    if((cw_ope_data.index_array != NULL && cw_ope_data.array_len > 128)
    	|| (cw_ope_data.data != NULL && cw_ope_data.data_len > 1024))
        return -EFAULT;
    ...
    cw_ope_data.module_name[CW_MODULE_NAME_LEN - 1] = 0;
...
static long cw_compat_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    if((compat_ptr(cw_ope_data.index_array) != NULL && cw_ope_data.array_len > 128)
    	|| (compat_ptr(cw_ope_data.data) != NULL && cw_ope_data.data_len > 1024))
        return -EFAULT;
...
static ssize_t cw_debug_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    if (strcmp(attr->attr.name, "set_cw_dbg_en") == 0) {
        return snprintf(buf, PAGE_SIZE, "set_cw_dbg_en = %d\n", set_cw_dbg_en);
    } else if (strcmp(attr->attr.name, "get_cw_dbg_en") == 0) {
        return snprintf(buf, PAGE_SIZE, "get_cw_dbg_en = %d\n", get_cw_dbg_en);
    }
    return 0;
}
...
extern int __init rtk_single_cpu_cw_module_init(void);
