...
#define DEMUX_TP2_BUFF_SIZE        (16*1024*1024)
...
typedef struct {
	...
	unsigned char    ecp_pes_flag;   /* for ecp pes filter flag */
	...
	int    totalDroppingPin;

	TPK_HARDWARE_INFO_T   tpInfo ;
	DEMUX_CW_INDEX_INFO_T cwInfo[MAX_DESCRAMBLER_INDEX_NUM] ;

    unsigned int     usedSVPPollSize ;
    DEMUX_BUF_T      svpBufPoll ; /* buffer reserved for es and ib buffer */
