...
int DEMUX_BUF_AllocBuffer_Ext(DEMUX_BUF_T *pBuf, int size, int isCache, int useZone)
{
	isCache = demux_device->disableCache ? 0 : isCache ;
	pBuf->allocSize = size < 0x1000 ? 0x1000 : size ;

	pBuf->phyAddr = pli_malloc(pBuf->allocSize, useZone ? GFP_DCU1 : GFP_DCU2_FIRST);
	if( INVALID_VAL == pBuf->phyAddr ) {
		dmx_err("[%s] pli_malloc failed. phyAddr:%lx, Size:%x, isCache:%d, useZone:%d \n",__func__, (unsigned long)pBuf->phyAddr, size, isCache, useZone);
		return -EFAULT;
	}

	if(isCache){
		pBuf->cachedaddr = dvr_remap_cached_memory(pBuf->phyAddr, pBuf->allocSize, __builtin_return_address(0));
		if (pBuf->cachedaddr == NULL)
			return -EFAULT;
	}else{
		pBuf->nonCachedaddr = dvr_remap_uncached_memory(pBuf->phyAddr, pBuf->allocSize, __builtin_return_address(0));
		if (pBuf->nonCachedaddr == NULL)
			return -EFAULT;
	}
	...
	dmx_notice("[%s] useZone cachedAddr %lx, uncachedAddr %lx, phyAddr %lx, allocSize %x\n",__func__,
		(unsigned long)(uintptr_t)pBuf->cachedaddr,	(unsigned long)(uintptr_t)pBuf->nonCachedaddr, (unsigned long)pBuf->phyAddr, pBuf->allocSize);
	return 0 ;
}

int DEMUX_BUF_AllocBuffer(DEMUX_BUF_T *pBuf, int size, int isCache, int useZone)
{
	int zone_bit_0 = useZone & 0x1;
	int zone_bit_1 = useZone & 0x2;
	...
	if (isCache) {
		if (zone_bit_1) {
			pBuf->cachedaddr = (UINT8 *)dvr_malloc_specific(pBuf->allocSize, GFP_DCU1_LIMIT);
		} else {
			pBuf->cachedaddr = (UINT8 *)dvr_malloc_specific(pBuf->allocSize, zone_bit_0 ? GFP_DCU1 : GFP_DCU2_FIRST);
		}
	} else {
		void* uncached;
		...
		if (zone_bit_1) {
			pBuf->cachedaddr = (UINT8 *)dvr_malloc_uncached_specific(
				pBuf->allocSize,
				GFP_DCU1_LIMIT, &uncached);
		} else {
			pBuf->cachedaddr = (UINT8 *)dvr_malloc_uncached_specific(
				pBuf->allocSize,
				zone_bit_0 ? GFP_DCU1 : GFP_DCU2_FIRST,
				&uncached
				);
		}
...
#endif
	dmx_mask_print(DEMUX_BUFFER_DEBUG, "useZone cachedAddr %lx, uncachedAddr %lx, phyAddr %lx, allocSize %x\n",
		(unsigned long)(uintptr_t)pBuf->cachedaddr,	(unsigned long)(uintptr_t)pBuf->nonCachedaddr, (unsigned long)pBuf->phyAddr, pBuf->allocSize);
	return 0 ;
}

int DEMUX_BUF_FreeBuffer_Ext(DEMUX_BUF_T *pBuf)
{
	...
	if (pBuf->phyAddr && !pBuf->fromPoll) {

		if(pBuf->isCache){
			dvr_unmap_memory(pBuf->cachedaddr, pBuf->allocSize);
		}else{
			dvr_unmap_memory(pBuf->nonCachedaddr, pBuf->allocSize);
		}

		pli_free(pBuf->phyAddr);
...
int DEMUX_BUF_AllocFromPoll(DEMUX_BUF_T *pBuf, int size, DEMUX_BUF_T *pSrcBuf, int isCache)
{
	...
	dmx_mask_print(DEMUX_BUFFER_DEBUG, "Poll: cachedAddr %lx, uncachedAddr %lx, phyAddr %lx, allocSize %x\n",
		(unsigned long)(uintptr_t)pBuf->cachedaddr,	(unsigned long)(uintptr_t)pBuf->nonCachedaddr,	(unsigned long)pBuf->phyAddr, pBuf->allocSize);
	return 0 ;
}
...
int DEMUX_BUF_SetupPSIBuffer(demux_channel *pCh, unsigned long phyAddr, int size)
{
	...
	pBuf->fromPoll = 0;
	pBuf->virAddr = pBuf->nonCachedaddr;
	dmx_crit("[%s %d]CH%d setup PSI buf header = 0x%lx\n", __func__, __LINE__, pCh->tunerHandle, (unsigned long)pBuf->phyAddr);
	...
	dmx_crit("[%s %d]CH%d setup psi buf = 0x%lx\n", __func__, __LINE__, pCh->tunerHandle, (unsigned long)pBuf->phyAddr);
	return 0;
}
int DEMUX_BUF_ReturnRefClockBuffer(demux_channel *pCh)
{
	...
	dmx_notice("[%s %d]CH%d return reference clock = 0x%lx\n", __func__, __LINE__, pCh->tunerHandle, (unsigned long)pBuf->phyAddr);
...
int DEMUX_BUF_ReturnPSIBuffer(demux_channel *pCh)
{
	...
	dmx_crit("[%s %d]CH%d return PSI buf = 0x%lx\n", __func__, __LINE__, pCh->tunerHandle, (unsigned long)pBuf->phyAddr);
	if (pBuf->phyAddr) {
		dvr_unmap_memory(pBuf->isCache ? (void *)pBuf->cachedaddr : (void *)pBuf->nonCachedaddr, pBuf->size);
		DEMUX_BUF_ZeroOutBuffer(pBuf);
	}
	...
	pBuf = &pCh->psiBufferH;
	dmx_crit("[%s %d]CH%d return PSI buf Header = 0x%lx\n", __func__, __LINE__, pCh->tunerHandle, (unsigned long)pBuf->phyAddr);
	if (pBuf->phyAddr) {
		dvr_unmap_memory(pBuf->isCache ? (void *)pBuf->cachedaddr : (void *)pBuf->nonCachedaddr, pBuf->size);
		DEMUX_BUF_ZeroOutBuffer(pBuf);
	}
...
int DEMUX_BUF_SetupDecBuffer(demux_channel *pCh, int pin, unsigned long bsPhyAddr, int bsSize, unsigned long ibPhyAddr, int ibSize)
{
	...
	dmx_crit("[%s %d]CH%d setup pin(%d) BS buf header = 0x%lx\n", __func__, __LINE__, pCh->tunerHandle, pin, (unsigned long)pBufBSH->phyAddr);
	...
	dmx_crit("[%s %d]CH%d setup pin(%d) BS buf = 0x%lx, size = %d\n", __func__, __LINE__, pCh->tunerHandle, pin, (unsigned long)pBufBS->phyAddr, pBufBS->size);
	...
	if (ibPhyAddr && ibSize) {
		...
		dmx_crit("[%s %d]CH%d setup pin(%d) IB buf header = 0x%lx\n", __func__, __LINE__, pCh->tunerHandle, pin, (unsigned long)pBufIBH->phyAddr);
		...
		dmx_crit("[%s %d]CH%d setup pin(%d) IB buf = 0x%lx, size = %d\n", __func__, __LINE__, pCh->tunerHandle, pin, (unsigned long)pBufIB->phyAddr, pBufIB->size);
...
int DEMUX_BUF_ReturnDecBuffer(demux_channel *pCh, int pin)
{
	...
	if (pBufBS->phyAddr) {
		dmx_crit("[%s %d]CH%d return pin %d BS buf = 0x%lx  pBufBS->cachedaddr=0x%lx  pBufBS->virAddr = 0x%lx\n",
			__func__, __LINE__, pCh->tunerHandle, pin, (unsigned long)pBufBS->phyAddr, (unsigned long)(uintptr_t)pBufBS->cachedaddr, (unsigned long)(uintptr_t)pBufBS->virAddr);
		if (pCh->bUseSelfBuf) {
			dvr_unmap_memory(pBufBS->isCache ? (void *)pBufBS->cachedaddr : (void *)pBufBS->nonCachedaddr, pBufBS->size);
			pli_free(pBufBS->phyAddr);
		}
		else
			dvr_unmap_memory(pBufBS->isCache ? (void *)pBufBS->cachedaddr : (void *)pBufBS->nonCachedaddr, pBufBS->size);
	...
	if (pBufBSH->phyAddr && !pCh->bUseSelfBuf) {
		dmx_crit("[%s %d]CH%d return pin %d BS buf header = 0x%lx\n", __func__, __LINE__, pCh->tunerHandle, pin, (unsigned long)pBufBSH->phyAddr);
	...
	/* unmap ib buffer */
	if (pBufIB->phyAddr) {
		if( (pin == VIDEO_PIN) && (rtkdemux_dbg_check_inband_en == 1) )
		{
#if IS_ENABLED(CONFIG_RTK_KDRV_DC_MEMORY_TRASH_DETCTER)
			rtkdemux_dbg_remove_DCMT(pCh->pinInfo[pin].dcmt_number);
			dmx_emerg("[%s %d](CH%d) Pin%d Already remove the DCMT on mem%d\n", __func__, __LINE__, pCh->tunerHandle, pin, pCh->pinInfo[pin].dcmt_number);
			pCh->pinInfo[pin].dcmt_number = 0xffffffff;
#endif
		}

		dmx_crit("[%s %d]CH%d return pin %d IB buf = 0x%lx\n", __func__, __LINE__, pCh->tunerHandle, pin, (unsigned long)pBufIB->phyAddr);
		if (pCh->bUseSelfBuf)
	...
	if (pBufIBH->phyAddr && !pCh->bUseSelfBuf) {
		dmx_crit("[%s %d]CH%d return pin %d IB buf header = 0x%lx\n", __func__, __LINE__, pCh->tunerHandle, pin, (unsigned long)pBufIBH->phyAddr);
...
void DEMUX_BUF_UpdatePinInfo(demux_channel *pCh)
{
	...
	for (i = VIDEO_PIN; i < NUMBER_OF_PINS; i++) {
		...
		if (pCh->pinInfo[i].bufStatus == BUF_STAT_FREE) {
			if (pCh->isECP){
				...
				if(IS_VIDEO_PIN(i) && pBufBS->virAddr == NULL)
				{
					...
					if(pBufBS->isCache){
						...
						pBufBS->nonCachedaddr = dvr_remap_uncached_memory(pBufBS->phyAddr, pBufBS->size, __builtin_return_address(0));
			...
			if (pCh->isECP){
				dmx_notice("[%s %d] BUF_STAT_NEW \n", __func__, __LINE__);
				DEMUX_TA_ECP_UpdatePininfo(pCh, i, DEMUX_STREAM_TYPE_IB, 1, (unsigned long)pBufIBH - (unsigned long)pCh, (unsigned long)pBufIB - (unsigned long)pCh);
				DEMUX_TA_ECP_UpdatePininfo(pCh, i, DEMUX_STREAM_TYPE_BS, 1, (unsigned long)pBufBSH - (unsigned long)pCh, (unsigned long)pBufBS - (unsigned long)pCh);
				...
				if(IS_VIDEO_PIN(i) && pBufBS->virAddr !=NULL)
				{
					...
					if(pBufBS->isCache){
						...
						pBufBS->virAddr = pBufBS->nonCachedaddr = NULL;
					}
					RHAL_TP_EnableMemProtect(pCh->tunerHandle, MEMORY_PRISON_VIDEO_ES, pBufBS->phyAddr,  pBufBS->phyAddr+pBufBS->size);
				}
			}
			...
			dmx_emerg("[%s %d](CH%d) Pin%d IB PhysicalLower=0x%p PhysicalUpper=0x%p\n", __func__, __LINE__, pCh->tunerHandle,i, pPinIBInfo->pBufferLower, pPinIBInfo->pBufferUpper);

			if((i == VIDEO_PIN) && (rtkdemux_dbg_check_inband_en == 1))
			{
#if IS_ENABLED(CONFIG_RTK_KDRV_DC_MEMORY_TRASH_DETCTER)
				pCh->pinInfo[i].dcmt_number = rtkdemux_dbg_add_DCMT(pPinIBInfo->pBufferLower, pPinIBInfo->pBufferUpper);
				dmx_emerg("[%s %d](CH%d) Pin%d Already setting the DCMT on mem\n", __func__, __LINE__, pCh->tunerHandle,i);
#endif
			}
...
int DEMUX_BUF_PrepareSelfDecBuf(demux_channel *pCh, int pin, int bsSize, int ibSize)
{
	...
	if (pBufBS->virAddr == 0 && DEMUX_BUF_AllocBuffer_Ext(pBufBS, bsSize, 0, 1) < 0) {
		dmx_crit("[%s %d]CH%d out of memory: allocate bs buffer fail. bsSize = %d\n", __func__, __LINE__, pCh->tunerHandle, bsSize);
		return -1;
	}
	DEMUX_BUF_ResetRBHeader(pBufBSH, pBufBS, RINGBUFFER_STREAM,  1);
	dmx_crit("[%s] pin=%d pBufBS->cache=%d cachedAddr %lx, uncachedAddr %lx, phyAddr %lx, Size %x\n",
			__func__, pin, pBufBS->isCache, (unsigned long)(uintptr_t)pBufBS->cachedaddr, (unsigned long)(uintptr_t)pBufBS->nonCachedaddr, (unsigned long)pBufBS->phyAddr, pBufBS->size);
	...
	if (ibSize > 0) {
		...
		if (pBufIB->virAddr == 0 && DEMUX_BUF_AllocBuffer(pBufIB, ibSize, 0, (LIMIT | 1)) < 0) {
		...
		dmx_crit("[%s]	pin=%d pBufIB->cache=%d  cachedAddr %lx, uncachedAddr %lx, phyAddr %lx, allocSize %x\n",
			__func__, pin, pBufIB->isCache, (unsigned long)(uintptr_t)pBufIB->cachedaddr, (unsigned long)(uintptr_t)pBufIB->nonCachedaddr, (unsigned long)pBufIB->phyAddr, pBufIB->allocSize);
	}
...
int DEMUX_PESBuffer_Alloc(DEMUX_PES_MAP_T *pMap, int size)
{
	...
	pMap->pBufferLower  = (unsigned char *)(unsigned long)pRBH->beginAddr;
			...
			if (DEMUX_PESBuffer_Free(pMap) < 0){
				...
				if (pCh->isECP){
					if (DEMUX_TA_ECP_UpdatePESBufferinfo(pCh, index, 0)<0){
						dmx_err("%s_%d ECP RELEASE BUFFER FAIL !!!!\n", __func__, __LINE__);
					}
				}
		...
		if (pMap->bufferStatus == BUFFER_NEW){
			...
			if (pCh->isECP){
				if (DEMUX_TA_ECP_UpdatePESBufferinfo(pCh, index, 1)<0)
					dmx_err("%s_%d ECP SETUP  BUFFER FAIL !!!!\n", __func__, __LINE__);
			}
...
int DEMUX_BUF_AllocTPBuffer(demux_channel * pCh)
{
	...
	if (pCh->bIsSDT) {
		if (DEMUX_BUF_AllocBuffer_Ext(&pCh->tpBuffer, SDT_CH_BUFF_SIZE, 0, 0) < 0)
		{
	...
	if(TP_TP2 == pCh->tunerHandle){
		if (DEMUX_BUF_AllocBuffer_Ext(&pCh->tpBuffer, DEMUX_TP2_BUFF_SIZE, 0, 0) < 0)
		{
			dmx_err("%s_%d: ch:%d DEMUX_BUF_AllocBuffer\n!!!!!", __func__, __LINE__,pCh->tunerHandle );
			goto ALLOCATE_FAIL;
		}
...
#ifdef USE_STATIC_RESERVED_BUF
	{
		...
		unsigned long startPhyAddr = 0;
		size = DEMUX_TP_BUFF_SIZE;
		totalSize = carvedout_buf_query(CARVEDOUT_TP, (void**)&startPhyAddr);
		dmx_info("startPhyAddr:0x%x chNum:%d totalSize:%d size:%d\n",startPhyAddr,pCh->tunerHandle,totalSize,size);
		if (totalSize > 0) {
			phyAddr = startPhyAddr + pCh->tunerHandle * size;
			if ((phyAddr+size) <= (startPhyAddr + totalSize)) {
				dmx_info("Using Reserved Buffer phyAddr: 0x%x\n",phyAddr);
				pBuf->phyAddr = phyAddr;
				pBuf->allocSize = size < 0x1000 ? 0x1000 : size ;
				pBuf->cachedaddr = dvr_remap_cached_memory(pBuf->phyAddr, pBuf->allocSize, __builtin_return_address(0));
				pBuf->nonCachedaddr = dvr_remap_uncached_memory(pBuf->phyAddr, pBuf->allocSize, __builtin_return_address(0));
				if( (pBuf->cachedaddr == 0) || (pBuf->nonCachedaddr == 0) ) {
					dmx_err("%s_%d: ch:%d ERROR!! dvr_rempa fail cachedaddr=0x%lx nonCachedaddr=0x%lx\n!!!!!", __func__, __LINE__,pCh->tunerHandle, (unsigned long)(uintptr_t)pBuf->cachedaddr, (unsigned long)(uintptr_t)pBuf->nonCachedaddr);
					goto ALLOCATE_FAIL;
				}
				pBuf->isCache = 0;
				...
				dmx_notice("[%s]TP[%d] cachedAddr 0x%lx, uncachedAddr 0x%lx, phyAddr 0x%lx, allocSize 0x%x\n",
					__func__, pCh->tunerHandle, (unsigned long)(uintptr_t)pBuf->cachedaddr,
					(unsigned long)(uintptr_t)pBuf->nonCachedaddr,	(unsigned long)pBuf->phyAddr, pBuf->allocSize);
				goto ALLOCATE_SUCCESS;
			} else {
...
#endif
	{
		if (DEMUX_BUF_AllocBuffer_Ext(pBuf, DEMUX_TP_BUFF_SIZE, 0, 0) < 0)
		{
			dmx_err("%s_%d: ch:%d DEMUX_BUF_AllocBuffer\n!!!!!", __func__, __LINE__,pCh->tunerHandle );
			goto ALLOCATE_FAIL;
		}
...
int DEMUX_BUF_ConfigTPBuffer(demux_channel * pCh)
{
	...
	if (tpStatus.tp_src == MTP && pCh->mtpBuffer.allocSize > 0) {
		...
		dmx_crit("[%s %d] mtpBuffer(phyAddr:0x%lx, virAddr:%p,Size: %d), timeStamp:%d, uploadUnitSize:%d \n",
			__func__, __LINE__, (unsigned long)pCh->mtpBuffer.phyAddr, pCh->mtpBuffer.virAddr, pCh->mtpBuffer.size, ((tpStatus.tp_param.ts_in_tsp_len == byte_192) ? 192 : 188) ,mtpAlignment);
	}
...
int  DEMUX_BUF_MTPBuffer_Create(demux_channel *pCh)
{
	...
	dmx_crit("%s_%d: MTP share TP buffer: tp (phy:%lx, vir:%p, size:%d/%d), mtp (phy:%lx, vir:%p, size:%d/%d)\n",
		__func__, __LINE__, (unsigned long)pTpBuf->phyAddr, pTpBuf->virAddr, pTpBuf->size, pTpBuf->allocSize,
		(unsigned long)pCh->mtpBuffer.phyAddr, pCh->mtpBuffer.virAddr, pCh->mtpBuffer.size, pCh->mtpBuffer.allocSize);
	DEMUX_BUF_ResetRBHeader(&pCh->tpBufferH, &pCh->tpBuffer, RINGBUFFER_STREAM, 0);
