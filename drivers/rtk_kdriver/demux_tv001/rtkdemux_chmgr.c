...
int CHMGR_AcquireChannel(DEMUX_CH_ACQUIRE_T* pInfo)
{
	...
	pInfo->owner[15] = '\0';
	if(pInfo->owner[0] != '\0')
	    demux_device->pChUsage[chIndex].pOwner = kstrdup(pInfo->owner, GFP_KERNEL);
	else
	    demux_device->pChUsage[chIndex].pOwner = NULL;
...
int CHMGR_TryAcquireChannel(DEMUX_CH_ACQUIRE_PRIORITY priority)
{
	...
	/*since TP2 not start code search,
	  For some special case, hard code first return TP2
	*/
	if(priority == DEMUX_CH_CH2_FIRST){
		if(demux_device->pChUsage[2].bUsed == false)
			return 2;
		else
			return liveCh;
	}
