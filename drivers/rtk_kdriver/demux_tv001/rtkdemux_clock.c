...
EXPORT_SYMBOL(CLOCK_GetPTS);

int64_t CLOCK_Get_TP_INTERNAL(void)
{
	unsigned int ptrlo = rtd_inl(TP_TP_INTERNAL_CNT_LOW_reg) ;
	unsigned int ptrhi = rtd_inl(TP_TP_INTERNAL_CNT_HIGH_reg) ;
	int64_t ret = ptrlo | (((int64_t)ptrhi) << 32) ;
...
EXPORT_SYMBOL(CLOCK_GetAVSyncPTS);
