...
{
    dmx_info("[%s] video_thr [%ld] audio_thr [%ld] audio_wait [%ld] caller [%pF]\n", __func__, videoFreeRunThr, audioFreeRunThr, audioFreeRunThrToWait, __builtin_return_address(0));
...
{
    dmx_info("[%s] audio_thr [%ld] audio_wait [%ld] caller [%pF]\n", __func__, audioFreeRunThr, audioFreeRunThrToWait, __builtin_return_address(0));
    pRefClock->audioFreeRunThreshold = DEMUX_reverseInteger(audioFreeRunThr);
    pRefClock->audioFreeRunThresholdToWait = DEMUX_reverseInteger(audioFreeRunThrToWait);
}
void REFCLK_SetVideoFreeRunThreshold(REFCLOCK *pRefClock, long videoFreeRunThr)
{
    long curretVideoFreeRunThr;
    curretVideoFreeRunThr = DEMUX_reverseInteger(pRefClock->videoFreeRunThreshold);
    if(curretVideoFreeRunThr == videoFreeRunThr)
        return;
    dmx_info("[%s] video_thr [%ld] caller [%pF]\n", __func__, videoFreeRunThr, __builtin_return_address(0));
    pRefClock->videoFreeRunThreshold = DEMUX_reverseInteger(videoFreeRunThr);
}
