...
static INT32 TP_RequestSecBufferCB(void *pContext, UINT8 **ppBuf, UINT16 size)
{
	...
	ch    = ((long)pContext) >> 8 ;
	index = ((long)pContext) & 0x3f ;  /* MAX_SECTION_FILTER_NUM(64) */
...
/*	dmx_info("%s called, line %d, idxR %d, idxW %d, rd 0x%x, wr 0x%x\n", __func__, __LINE__, idxR, idxW, rd, wr) ; */
	if (rd > wr && idxR == idxW) {
		...
		space = SEC_FILTER_BUFFER_UNIT_SIZE - ((wr - (unsigned long)pSec->pBufferLower)%SEC_FILTER_BUFFER_UNIT_SIZE);
		if (space < (8+sz)) { /* search next unit */
			if (demux_device->secMap[idxW].nextBufIndex != idxR) { /* done */
				...
				space = (rd - (unsigned long)pSec->pBufferLower)%SEC_FILTER_BUFFER_UNIT_SIZE;
...
{
	DEMUX_CHANNEL_T ch = (DEMUX_CHANNEL_T)(((long)pContext) >> 8);
	demux_channel *pCh = &demux_device->pChArray[ch];
	int index = ((long)pContext) & 0x3f ;  /* MAX_SECTION_FILTER_NUM(64) */
...
*/
	...
	*pSec->pWrPtr = ((long)pSecData->p_data + pSec->phyAddrOffset + pSecData->len + 3) & ~0x3 ;
...
int IOCTL_InitModule (void)
{
	...
	if (DEMUX_BUF_AllocBuffer(&demux_device->bufPoll, size, 0, (LIMIT | 1))   < 0)
		goto demux_init_fail;
...
EXPORT_SYMBOL(IOCTL_GetBufferInfo);
...
int IOCTL_GetPESHeader(DEMUX_PES_HEADER_INFO_T* header)
{
	int i = 0, pid = -1, target = -1;
	char *headerbuf = NULL;
	demux_channel *pCh;
	DEMUX_CHECK_CH(header->ch);
	DEMUX_CHECK_PID(header->PID);
	pCh = &demux_device->pChArray[header->ch];

	for (i = 0; i < DEMUX_NUM_OF_TARGETS; i++)
	{
		pid = pCh->activeTarget[i].pid;
		if(pid != -1 && pid == header->PID)
		{
			target = i;
			dmx_info("func %s, line %d, get pid %d target %d\n", __func__, __LINE__, header->PID, target);
			break;
		}
	}
	...
	{
		header->Valid_Len = 0;
		dmx_err("func %s, line %d, miss pid %d\n", __func__, __LINE__, header->PID);
		return -1;
	}

	headerbuf = pCh->demuxTargetInfo[target].PesHeaderParseInfo.pesHeaderBuf;

	if(!(headerbuf[0] == 0x0 && headerbuf[1] == 0x0 && headerbuf[2] == 0x1))
	{
		header->Valid_Len = 0;
		dmx_err("func %s, line %d, pid %d, incorrect header\n", __func__, __LINE__, header->PID);
		return -1;

	}

	memcpy(header->PES_INFO, headerbuf, PES_HEADER_BUF_SIZE);
	header->Valid_Len = PES_HEADER_BUF_SIZE;
...
int IOCTL_AddSectionFilter(DEMUX_ADD_SEC_FILTER_T *pInfo)
{
	...
	if (ret >= 0) {
		...
		pInfo->param.pContext    = (void *)(unsigned long)((pInfo->ch << 8) | ret);
...
*/
		...
		if (i < 0 || RHAL_AddSectionFilter((TPK_TP_ENGINE_T)pInfo->ch, pInfo->param, (void *)DEFAULT_FILE_HANDLE, &pSec->handle) != TPK_SUCCESS) {
			if (i >= 0) {
			...
			pSec->pBufferLower  = (unsigned char *)(unsigned long)pRBH->beginAddr;
...
int IOCTL_CancelSectionFilter(DEMUX_CANCEL_SEC_FILTER_T *pInfo)
{
	...
	if (RHAL_RemoveSectionFilter((UINT8)pInfo->ch, pCh->secListCB[pInfo->secfIndex].handle) == TPK_SUCCESS) {
...
int IOCTL_GetPCRAdaptionField(DEMUX_PCR_ADAPTION_FIELD_T *pInfo)
{
	demux_channel *pCh;
	if( NULL == pInfo )
	    return -EFAULT;
	...
	DEMUX_CHECK_CH(pInfo->ch);
	pCh = &demux_device->pChArray[pInfo->ch];
	if (RHAL_GetPCRAdaptionField(pCh->tunerHandle, pInfo->pid,pInfo->data,&pInfo->size) < 0) {
	    return -EFAULT;
	}
	return 0;
}

int IOCTL_CompensatePCROffset(DEMUX_COMPENSATE_PCROFFSET_T *pInfo)
{
	demux_channel *pCh;
	DEMUX_PCRSYNC_T *pPcrProcesser;
	DEMUX_CHECK_CH(pInfo->ch);
	...
	mutex_lock(&pCh->mutex);
	pPcrProcesser = &pCh->pcrProcesser;
	pPcrProcesser->compensateFlag = (bool)pInfo->compensateFlag;
	pPcrProcesser->compensatePcrOffset = pInfo->compensatePcrOffset;
	// Notify pcrsync processor which will re-calculate pcrOffset
	dmx_crit("[%s %d] Compensate pcr offset and re-trigger to adjust it!!!\n", __func__, __LINE__);
...
int IOCTL_ResetFilters(DEMUX_RESET_FILTERS_T *pInfo)
{
	...
	if(bVideo == true ) {
		PCRSYNC_FlushVideoPidOrDest(pPcrProcesser, false);
		pCh->activeTarget[DEMUX_TARGET_VIDEO].pid     = -1;
		pCh->activeTarget[DEMUX_TARGET_VIDEO_2ND].pid = -1;
	}
	...
	if (bSec) {
		for (i = 0; i < MAX_SECTION_FILTER_NUM; i++) {
			if (pCh->secListCB[i].used) {
				RHAL_RemoveSectionFilter((UINT8)pInfo->ch, pCh->secListCB[i].handle);
...
int IOCTL_InitDescrambler(DEMUX_INIT_DESCRAMBLER_T *pInfo)
{
	...
	return (RHAL_SetDescrambleAlgorithm((UINT8)pInfo->ch, pInfo->alg) == TPK_SUCCESS ? 0 : -1);
}
int IOCTL_SetDescramblerMode(DEMUX_DESCRAMBLER_MODE_T *pInfo)
{
	dmx_function_call();
	DEMUX_CHECK_CH(pInfo->ch);
	return (RHAL_DescrambleControl((UINT8)pInfo->ch, pInfo->mode) == TPK_SUCCESS ? 0 : -1);
}
...
int IOCTL_SetCW(DEMUX_SET_CW_T *pInfo)
{
	...
	if (pInfo->keyType != DEMUX_SET_CW_ONLY_IV) {
		return (RHAL_SetCW((UINT8)pInfo->ch, keySel, pInfo->key_id, pInfo->Key, pIV) == TPK_SUCCESS ? 0 : -1);
	} else {
		return 0;
	}
}
...
int IOCTL_MTPWriteBuffer(DEMUX_MTP_WRITE_T *pInfo)
{
	...
	if (RHAL_WriteMTPBuffer(mtp_id, (UINT8 *)(unsigned long)pInfo->writeAddr, pInfo->writeSize) != TPK_SUCCESS)
		return -1;
...
int IOCTL_MTPGetWriteStatus(DEMUX_MTP_WRITE_STATUS_T *pInfo)
{
	...
	pInfo->writeAddr = (UINT32)(unsigned long)pWritePhyAddr;
...
int IOCTL_GetESBufStatus(DEMUX_ES_BUF_STATUS_T *pInfo)
{
	demux_channel *pCh;
	//UINT32 bufSize;
	UINT32 dataSize = 0;
	...
	if (DEMUX_GetESBufferData(pCh, &dataSize) != TPK_SUCCESS)
		return -1;
...
int IOCTL_SetPrivateInfo(DEMUX_PRIVATE_INFO_T *pInfo)
{
	...
	case DEMUX_PRIVATEINFO_AUDIO_FORMAT:
		{
			dmx_notice("(%s-%d): ch=%d DEMUX_PRIVATEINFO_AUDIO_FORMAT: dest:%d \n", __FILE__, __LINE__, pCh->tunerHandle, pInfo->dest);
			if (pInfo->dest == DEMUX_DEST_ADEC0) {
				...
				memcpy(pCh->audioPrivateInfo, &pInfo->data[2], sizeof(UINT32)*8);
			} else {
				...
				memcpy(pCh->audioPrivateInfo2, &pInfo->data[2], sizeof(UINT32)*8);
			}
			break;
		}
...
EXPORT_SYMBOL(IOCTL_SetPrivateInfo);
...
int IOCTL_GetBufferInfoEX(DEMUX_BUF_INFO_EX_T *pInfo)
{
	...
	if(demux_device->secFilterBuffer.size == 0) {
...
{
	if (RHAL_TPOUT_SetDataSource(pInfo->tpout_id, pInfo->tpout_src) != TPK_SUCCESS)
		return -1;
...
{
	if (RHAL_TPOUT_SetClockSpeed(pInfo->tpout_id, pInfo->pre_div, pInfo->post_div, 0) != TPK_SUCCESS)
		return -1;
...
{
	int tp_id = 0;
	UINT8 sync_byte[MAX_TP_P_COUNT] = {0x47, 0x48, 0x49};
	demux_channel *pCh;
	TPK_TP_STATUS_T tpStatus;
#ifdef CONFIG_RTK_TP_WITH_MIO
	TPK_TPOUT_TS_PARAM_T tpo_status;
#endif
	UINT32 tpo_mode = TPO_MODE_TWOBIT;
	...
	if (pInfo->inputType == TPK_INPUT_SERIAL)
	{
		...
		tpStatus.tp_param.two_bits          = 0 ;
		tpo_mode = TPO_MODE_SERIAL;
	...
	else if (pInfo->inputType == TPK_INPUT_PARALLEL)
	{
		...
		if(pInfo->casType!=TPK_WITHOUT_CAS){
			tpStatus.tp_param.two_bits          = 1;
			tpo_mode = TPO_MODE_TWOBIT;
		}else{
			tpStatus.tp_param.two_bits          = 0;
		}
		dmx_notice("[%s %d] - Set TP%d to Parallel mode!\n", __func__, __LINE__, tp_id) ;
	}else if (pInfo->inputType == TPK_INPUT_TWO_BIT)
	{
		tpStatus.tp_param.two_bits          = 1;
		tpo_mode = TPO_MODE_TWOBIT;
		dmx_notice("[%s %d] - Set TP%d to two bit mode!\n", __func__, __LINE__, tp_id) ;
#endif		
	}
	...
	*/
	...
	if(pInfo->casType!=TPK_WITHOUT_CAS)
	{
		...
		{
			//**************************************
			// FOR Non CI+1.4 Case ,enable tpp0 
			//**************************************
			tpp_id = 0;
		}
		...
		if(tpout_src == TPK_TPP_TO_TPOUT){
			...
			if (RHAL_TPP_SetSyncReplace(tpp_id, sync_byte[tpp_id]) != TPK_SUCCESS)
				return TPK_FAIL;
			...
			if (RHAL_TPP_OutEnable(tpp_id, 1) != TPK_SUCCESS)
				return TPK_FAIL;
	
			dmx_notice("%s Set CI channel to TPP%d! tp%d_sync_byte=%X tpp_out_sync_byte=%X\n", __func__, tpp_id, tp_id, tpStatus.tp_param.sync_byte, sync_byte[tpp_id]);
			dmx_notice("%s Set TPO src(%d) remux from %d!(1:MEM,2:MUX)\n", __func__, TPK_TPP_TO_TPOUT, tpp_src);
	
		}

#ifdef CONFIG_RTK_TP_WITH_MIO
		/******************************/
		/* for mark2/mio 2 bit settting */
		if (RHAL_TPOUT_GetMode(0, &tpo_status) != TPK_SUCCESS)
			return TPK_FAIL;
		if(tpo_status.out_2bit != tpo_mode){
			if(tpo_mode == TPO_MODE_SERIAL){
				dmx_notice("%s mark2/mio use serial mode\n", __func__);
				tpo_status.out_serial = 1;
				RHAL_MIO_SetIOMode(1);
			}else{
				dmx_notice("%s mark2/mio use two-bit mode\n", __func__);
				tpo_status.out_serial = 0;
				RHAL_MIO_SetIOMode(0);
			}
			tpo_status.out_2bit = tpo_mode;
			if (RHAL_TPOUT_SetMode(0, tpo_status) != TPK_SUCCESS)
				return TPK_FAIL;
		}
		/******************************/
#endif
		...
		//set tpo source
		if (RHAL_TPOUT_SetDataSource(0, tpout_src) != TPK_SUCCESS)
			return TPK_FAIL;
...
((end) + ((end) >= (start) ? 0 : limit - base) - (start))
	...
	{
		if(0 == demux_device->pChArray[iChannel].isInit)
			continue;
		if (!((long)filp->private_data & (1 << iChannel)))
			continue;
		...
		for ( iItem = 0; iItem < MAX_PID_FILTER_CB_NUM; iItem++)
		{
			...
			demux_channel *pCh=  &demux_device->pChArray[iChannel];
			if (demux_device->pChArray[iChannel].pidFilterListCB[iItem].used) {
				...
				u32 *pWrPtr  = pPesMap->pWrPtr;
				if(pCh->isECP){
					pWrPtr = (unsigned int *)&((RINGBUFFER_HEADER *)pPesMap->rbHeader.virAddr)->writePtr;
				}
				if (pRBH && (pRBH->reserve2 == DEMUX_PESBUFFER_OP_INITIALIZED))
					if(BS_DISTANCE_TO_READ(pRBH->readPtr[1], *pWrPtr, pPesMap->pBufferLower, pPesMap->pBufferUpper) > 4)
						return true;
			}
		}
	}
