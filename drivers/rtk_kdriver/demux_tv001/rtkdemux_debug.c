...
static const char *rtkdemux_cmd_str[] = {
	...
	"check_inband=",	/* DEMUX_DBG_CMD_CHECK_INBAND */
...
unsigned char rtkdemux_dbg_check_inband_en = 0;
...
*/
	...
	for (i = 0; i < demux_device->chNum; i++) {
		...
		if (demux_device->isInit && pCh->isInit) {
			...
			UINT32 tp_buffer_size;
			UINT32 pDataSize;
			UINT32 free_size;
			RHAL_GetTPBufferStatus((TPK_TP_ENGINE_T) i, &tp_buffer_size, &pDataSize);
			free_size = tp_buffer_size - pDataSize;
			...
			for (j = 0 ; j < demux_device->numberOfVD ; j++) {
				...
				DMX_DBG_LOG(DGB_COLOR_GREEN"    Video%d BS: %uB allocated, %uB size, %uB used, %uB free"DGB_COLOR_NONE_N,
					j, pCh->bsDecBuf[pin].allocSize, pCh->bsDecBuf[pin].size, pCh->bsDecBuf[pin].size - free_size, free_size);
				total_allocated += pCh->bsDecBuf[pin].allocSize;
				...
				DMX_DBG_LOG(DGB_COLOR_GREEN"    Video%d IB: %uB allocated, %uB size, %uB used, %uB free"DGB_COLOR_NONE_N,
					j, pCh->ibDecBuf[pin].allocSize, pCh->ibDecBuf[pin].size, pCh->ibDecBuf[pin].size - free_size, free_size);
				total_allocated += pCh->ibDecBuf[pin].allocSize;
...
static void rtkdemux_dbg_dump_pts_start(int tp_index, const char* filename)
{
	...
	/* first line */
	snprintf(buffer, 128, "VideoPTS,AudioPTS,Video1PTS,Audio1PTS,PLL,PCR,TP INTERNAL\n");
...
void rtkdemux_dbg_dump_pts(demux_channel *pCh, int pin_index, int64_t pts)
{
	static int64_t notify_clock[DEMUX_CH_NUM] = {0};
	int tp_index = pCh->tunerHandle;

	rtkdemux_dump_pts[tp_index].pts[pin_index] = pts;

	if (rtkdemux_dump_pts[tp_index].enable) {
		REFCLOCK * const refClock = DEMUX_GetReferenceClock(pCh);
		int64_t pll = CLOCK_GetAVSyncPTS(refClock);
		int64_t tp_internal = CLOCK_Get_TP_INTERNAL();
		int64_t clock;
		char buffer[256];
		memset(buffer, 0, sizeof(buffer));
		snprintf(buffer, 256, "%lld,%lld,%lld,%lld,%lld,%lld,%lld\n",
			...
			pll, pCh->pcrProcesser.pcr, tp_internal);
		DUMP_WriteFile(rtkdemux_dump_pts[tp_index].fp, buffer, strlen(buffer), &rtkdemux_dump_pts[tp_index].pos);
...
static void rtkdemux_dbg_print_pts_start(int tp_index, const char* filename)
{
	...
	snprintf(buffer, 128, "CHANNEL, TSP TIMESTAMP, PTS, RCD, PLL, PCR, TP INTERNAL, WP\n");
...
void rtkdemux_dbg_print_pts(demux_channel *pCh, NAVBUF *pNavBuffer, unsigned char* WP)
{
	int tp_index = pCh->tunerHandle;

	if (rtkdemux_print_pts[tp_index].enable) {
		char buffer[256];
		int pin_index = pNavBuffer->data.pinIndex;
		static char* pinName[] = {"VIDEO","AUDIO","VIDEO1","AUDIO1"};
		REFCLOCK * const refClock = DEMUX_GetReferenceClock(pCh);
		int64_t pts = pNavBuffer->data.pts;
		int64_t rcd = REFCLK_GetRCD(refClock);
		int64_t pll = CLOCK_GetAVSyncPTS(refClock);
		int64_t tp_internal = CLOCK_Get_TP_INTERNAL();
		unsigned char *pStart = pNavBuffer->data.payloadData - (pCh->tsPacketSize - pNavBuffer->data.payloadSize);
		unsigned long tsp_timestamp = DEMUX_reverseInteger(*(unsigned long *)pStart);

		memset(buffer, 0, sizeof(buffer));
		snprintf(
			buffer, 256,
			"[Ch%d][%s], %lu, %lld, %lld, %lld, %lld, %lld, 0x%lx\n",
			tp_index, pinName[pin_index], tsp_timestamp, pts, rcd, pll, pCh->pcrProcesser.pcr, tp_internal, (unsigned long)(uintptr_t)WP
		);

		DMX_DBG_LOG(DGB_COLOR_CYAN"[Ch%d][%s], %lu, %lld, %lld, %lld, %lld, %lld, 0x%lx\n"DGB_COLOR_NONE, tp_index, pinName[pin_index], tsp_timestamp, pts, rcd, pll, pCh->pcrProcesser.pcr, tp_internal, (unsigned long)(uintptr_t)WP);
		DUMP_WriteFile(rtkdemux_print_pts[tp_index].fp, buffer, strlen(buffer), &rtkdemux_print_pts[tp_index].pos);
...
int rtkdemux_dbg_add_DCMT(unsigned char *pBufferLower, unsigned char *pBufferUpper)
{
    DC_MT_DESC mt_desc;

    memset(&mt_desc, 0, sizeof(mt_desc));
    mt_desc.type = MEMORY_TYPE_SERIAL;
    mt_desc.start = (unsigned long)pBufferLower;
    mt_desc.end = (unsigned long)pBufferUpper;
    mt_desc.mode = 1;
    mt_desc.ip_entry[0].module = MODULE_SB2_VCPU;//MODULE_ANY;
    mt_desc.ip_entry[0].prot = PROT_MODE_WO;
    
    return dcmt_set_monitor_auto(&mt_desc);
}

void rtkdemux_dbg_remove_DCMT(unsigned int entry)
{
...
static inline void rtkdemux_dbg_EXECUTE(const int cmd_index, char **cmd_pointer)
{
	...
	case DEMUX_DBG_CMD_CHECK_INBAND:
		{
			demux_channel *pCh = NULL;
			int check_inband_en;

			if (*cmd_pointer == NULL) {
				DMX_DBG_LOG(DGB_COLOR_RED"rtkdemux_debug:%d check inband command is incorrect.\n"DGB_COLOR_NONE, __LINE__);
				break;
			}
            
			token = strsep(cmd_pointer, delim);
			if (token == NULL || kstrtoint(token, 0, &check_inband_en) != 0) {
				DMX_DBG_LOG(DGB_COLOR_RED"rtkdemux_debug:%d failed to parse the inband command enable flag.\n"DGB_COLOR_NONE, __LINE__);
				break;
			}
			...
			if (demux_device && demux_device->pChArray)
				pCh = &demux_device->pChArray[tp_index];

			if (pCh != NULL) {
				if (pCh->pinInfo[VIDEO_PIN].bufStatus == BUF_STAT_DONE) {
					DEMUX_BUF_PIN_T *pPinIBInfo;
					pPinIBInfo = &pCh->pinInfo[VIDEO_PIN].bufInfo[DEMUX_STREAM_TYPE_IB];

#if IS_ENABLED(CONFIG_RTK_KDRV_DC_MEMORY_TRASH_DETCTER)
					if(check_inband_en == 1) {
						pCh->pinInfo[VIDEO_PIN].dcmt_number = rtkdemux_dbg_add_DCMT(pPinIBInfo->pBufferLower, pPinIBInfo->pBufferUpper);
						dmx_emerg("[%s %d](CH%d) Pin%d Already setting the DCMT on mem%d\n", __func__, __LINE__, pCh->tunerHandle, VIDEO_PIN, pCh->pinInfo[VIDEO_PIN].dcmt_number);
				    } else {
						rtkdemux_dbg_remove_DCMT(pCh->pinInfo[VIDEO_PIN].dcmt_number);
						dmx_emerg("[%s %d](CH%d) Pin%d Already remove the DCMT on mem%d\n", __func__, __LINE__, pCh->tunerHandle, VIDEO_PIN, pCh->pinInfo[VIDEO_PIN].dcmt_number);
						pCh->pinInfo[VIDEO_PIN].dcmt_number = 0xffffffff;
					}
#else
                    dmx_emerg("[%s %d](CH%d) Pin%d setting the DCMT Fail, CONFIG_RTK_KDRV_DC_MEMORY_TRASH_DETCTER not config\n", __func__, __LINE__, pCh->tunerHandle, VIDEO_PIN);
#endif
					rtkdemux_dbg_check_inband_en = check_inband_en;
				}
			}

			DMX_DBG_LOG("tp=%d rtkdemux_dbg_check_inband_en=%d\n", tp_index, rtkdemux_dbg_check_inband_en);
			break;
		}
	default:
...
static ssize_t rtkdemux_proc_write(
	struct file *file,
	...
	loff_t *ppos)
{
	...
	if (copy_from_user(str, buf, RTKDEMUX_MAX_CMD_LENGTH - 1)) {
		DMX_DBG_LOG("rtkdemux_debug:%d copy_from_user failed! (buf=%p, count=%ld)\n",
			__LINE__, buf, (unsigned long)count);
		return -EFAULT;
	}
...
static ssize_t rtkdemux_proc_read(
	struct file *file,
	char __user *buf,
	size_t count,
	loff_t *ppos)
...
static struct proc_ops  rtkdemux_proc_fops = {
        .proc_write  = rtkdemux_proc_write,
        .proc_read  = rtkdemux_proc_read,
};
#else
