...
int IOCTLEX_InitChannelImpl(int chIndex)
{
	...
	/* PCRSYNC variables of compensation pcroffset*/
	pCh->pcrProcesser.compensateFlag = false;
	pCh->pcrProcesser.compensatePcrOffset = 0;

	RHAL_SetTPRefClockMode(Demux_REFCLK_SetClockMode);
...
int IOCTLEX_UninitChannel(DEMUX_CHANNEL_T *pInfo)
{
	...
	result = IOCTLEX_UninitChannelImpl(pCh);

	for(i = 0; i< DEMUX_CH_NUM ; i++) {
		pCh = &demux_device->pChArray[i];
		if(pCh->isInit != 0)
			break;
	}
...
int IOCTLEX_UninitChannelImpl(demux_channel *pCh)
{
	...
	for(i=0; i<MAX_PID_FILTER_CB_NUM; i++) {
		if(DEMUX_PESBuffer_Free(&pCh->pesMap[i]) < 0) {
			dmx_err(DGB_COLOR_RED"FAIL!!!!: fail to free pes buffer\n");
		}
		else {
			pCh->pesMap[i].bufferStatus = BUFFER_OFFLINE;
		}
	}
...
EXPORT_SYMBOL(IOCTLEX_ReleaseBuffer);
...
int IOCTLEX_SetBufOwner(DEMUX_BUF_OWNER_T *pInfo)
{
	...
	dmx_notice("[%s %d] bUseSelfBuf = %d !\n", __func__, __LINE__, bUseSelfBuf);
...
int IOCTLEX_GetDemuxPts(DEMUX_PTS_T *pInfo)
{
	demux_channel *pCh;
	DEMUX_CHECK_CH(pInfo->ch);
	pCh = &demux_device->pChArray[pInfo->ch];
	
	pInfo->lastDemuxAPTS 	= pCh->pcrProcesser.lastDemuxAudioPTS;
	pInfo->lastDemuxVPTS 	= pCh->pcrProcesser.lastDemuxVideoPTS;
	pInfo->lastDemuxSubAPTS	= pCh->pcrProcesser.lastDemuxSubAudioPTS;
...
long IOCTLEX_Handle(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == DEMUX_IOC_SET_BUF_OWNER || cmd == DEMUX_IOC_SET_BUF_OWNER_COMPAT) {
		...
		DEMUX_CHANNEL_T data = DEMUX_CH_NUM;

		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_CHANNEL_T)))
			ret = -EFAULT;
		else
			if(data < DEMUX_CH_NUM)
		...
		memset(&data, 0, sizeof(DEMUX_CH_ACQUIRE_T));
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_CH_ACQUIRE_T))) {          
			ret = -EFAULT;
		} else {
			if( CHMGR_AcquireChannel(&data) < 0 ||  copy_to_user((void __user *)arg, (void *)&data, sizeof(DEMUX_CH_ACQUIRE_T))) {
				ret = -EFAULT;
				dmx_err("ioctl DEMUX_IOC_ACQUIRE_CH failed !!!!!!!!!!!!!!!\n");
			} else {
				filp->private_data =  (void*)((long)(filp->private_data) | (1 << data.chIndex));
		...
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(int))) {
			...
			filp->private_data = (void*)((long)(filp->private_data) & (~(1 << data)));
	...
	} else if (cmd == DEMUX_IOC_GET_PTS) {
		DEMUX_PTS_T data;
		ret = 0;
		if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(DEMUX_PTS_T))
		|| IOCTLEX_GetDemuxPts(&data) < 0
		|| copy_to_user((void __user *)arg, (void *)&data, sizeof(DEMUX_PTS_T))) {
			ret = -EFAULT;
			dmx_err("ioctl IOCTLEX_GetPcr failed !!!!!!!!!!!!!!!\n");
		}
	} else {
