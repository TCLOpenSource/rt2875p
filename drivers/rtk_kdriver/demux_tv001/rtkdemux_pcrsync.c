...
void PCRSYNC_ResetVariables(DEMUX_PCRSYNC_T *pPcrProcesser)
{
	...
	pPcrProcesser->isUnderFreeRunMode_audio = false;
	...
	pPcrProcesser->streamFirstDemuxAudioPTS = 0;
	pPcrProcesser->streamFirstDemuxVideoPTS = 0;
	pPcrProcesser->lastDemuxSubAudioPTS = -1;
	pPcrProcesser->audio_pts_gap = 0;
	pPcrProcesser->video_pts_gap = 0;
	...
	pPcrProcesser->preferAVPTS = AV_NONE;
	pPcrProcesser->referenceAVPTS = AV_NONE;
	...
	pPcrProcesser->videoFreeRunCheckCount= 0;
...
void PCRSYNC_SmoothPCR(DEMUX_PCRSYNC_T *pPcrProcesser , NAVDTVPCRINFO *pPcrinfo)
{
	static int64_t last_stc = 0;
	static int64_t last_pcr = 0;
	static int64_t last_prepcr = 0;
	int64_t diff_stc_tmp = 0;
	int64_t diff_pcr_tmp = 0;
	int64_t diff_pred_pcr_tmp = 0;
	int64_t diff_pcr_tmp_1 = 0;
	int64_t diff_pred_pcr_tmp_1 = 0;
	...
	if (pPcrinfo->stc != pPcrProcesser->realSTC) {
		...
		if (pPcrinfo->pcr != pPcrProcesser->realPCR) {
			...
			if(dmx_check_log_mask(DEMUX_PCRSYC_DEBUG_EX1))
			{
				diff_stc_tmp = pPcrinfo->stc - last_stc;
				diff_pcr_tmp = pPcrinfo->pcr - last_pcr;
				diff_pred_pcr_tmp = tmp_pcr - last_prepcr;
				diff_pcr_tmp_1 = diff_pcr_tmp - diff_stc_tmp;
				diff_pred_pcr_tmp_1 = diff_pred_pcr_tmp - diff_stc_tmp;
			}

			if (pPcrinfo->pcr > 0 && tmp_pcr > 0 && abs(pPcrinfo->pcr - tmp_pcr) > 100 )
				pPcrinfo->pcr = tmp_pcr;

			dmx_mask_print_info(DEMUX_PCRSYC_DEBUG_EX1, "[%s %d] diff [%lld] stc [%lld] pcr [%lld] pred [%lld] stc_diff [%lld] pred_diff_stc [%lld] pcr_diff_stc [%lld]\n",
					 __func__ , __LINE__, abs(pPcrinfo->pcr - tmp_pcr), pPcrinfo->stc, pPcrinfo->pcr, tmp_pcr, diff_stc_tmp, diff_pred_pcr_tmp_1, diff_pcr_tmp_1);

			if(dmx_check_log_mask(DEMUX_PCRSYC_DEBUG_EX1))
			{
				last_pcr = pPcrinfo->pcr;
				last_prepcr = tmp_pcr;
				last_stc = pPcrinfo->stc;
			}
		...
		*/
		...
		if ((pPcrProcesser->bAdjusted == false) && (pPcrProcesser->rcd != -1) && (pPcrProcesser->referenceAVPTS == beforeReferenceAVPTS)) {
			...
			*/
			...
			if (pPcrProcesser->compensateFlag) {
				dmx_mask_print(DEMUX_PCRSYC_DEBUG,"[dtv(%d)] Compensation pcr offset(%lld)!!! \n", (int)pPcrProcesser->tunerHandle, pPcrProcesser->compensatePcrOffset);
				pPcrProcesser->pcrDefaultOffset += pPcrProcesser->compensatePcrOffset;
			}
			...
			*/
			...
			/* at first use DTV_DELAY_TIME to calc mpts, if apts less than mpts, should enlarge pcr offset */
				...
				*/
				...
				if (pPcrProcesser->compensateFlag) {
					dmx_mask_print(DEMUX_PCRSYC_DEBUG,"[dtv(%d)] Compensation pcr offset(%lld)!!! \n", (int)pPcrProcesser->tunerHandle, pPcrProcesser->compensatePcrOffset);
					pPcrProcesser->pcrOffset += pPcrProcesser->compensatePcrOffset;
				}
			}
		...
		if ((pPcrProcesser->bAdjusted == false)
		  ...
		  || ((pPcrProcesser->referenceAVPTS == VIDEO_TYPE) && (mpts < pPcrProcesser->lastDemuxVideoPTS))) {
			...
			else if (pPcrProcesser->bNoVideoPts == false && pPcrProcesser->isUnderFreeRunMode == false)
				REFCLK_SetVideoFreeRunThreshold(pPcrProcesser->pRefClock, MAX_FREE_RUN_THRESHOLD);
			if (pPcrProcesser->bNoAudioPts == false && pPcrProcesser->isUnderFreeRunMode_audio == false)
...
void PCRSYNC_AdjustRCD(DEMUX_PCRSYNC_T *pPcrProcesser, NAVDTVPCRINFO pcrinfo)
{
	...
	if( pPcrProcesser->lastDemuxAudioPTS != pPcrProcesser->prevAudioDemuxPTS )
		pPcrProcesser->audio_pts_gap = abs(pPcrProcesser->lastDemuxAudioPTS - pPcrProcesser->prevAudioDemuxPTS);
	if( pPcrProcesser->lastDemuxVideoPTS != pPcrProcesser->prevVideoDemuxPTS )
		pPcrProcesser->video_pts_gap = abs(pPcrProcesser->lastDemuxVideoPTS - pPcrProcesser->prevVideoDemuxPTS);
			...
			if ((currPTS - pPcrProcesser->resetPTS) > 2*DEMUX_CLOCKS_PER_SEC) {
				...
				if (((pPcrProcesser->referenceAVPTS == AUDIO_TYPE)
				  ...
				  || ((pPcrProcesser->referenceAVPTS == VIDEO_TYPE)
					...
					if (pPcrProcesser->isUnderFreeRunMode_audio == false)
						REFCLK_SetAudioFreeRunThreshold(pPcrProcesser->pRefClock, DEFAULT_FREE_RUN_THRESHOLD, DEFAULT_FREE_RUN_THRESHOLD);
					...
					else if (pPcrProcesser->isUnderFreeRunMode == false)
						REFCLK_SetVideoFreeRunThreshold(pPcrProcesser->pRefClock, DEFAULT_FREE_RUN_THRESHOLD);
					//printf("%s,%s,%d, set FreeRunThreshold %d.\n", __FILE__, __func__, __LINE__, DEFAULT_FREE_RUN_THRESHOLD);
				}
			}
				...
				if((pPcrProcesser->bNoVideoPts == false) && (pPcrProcesser->lastDemuxVideoPTS > 0) && (mpts > pPcrProcesser->lastDemuxVideoPTS)) {
					if(pPcrProcesser->referenceAVPTS == VIDEO_TYPE) {
						...
						if(mpts > pPcrProcesser->lastDemuxVideoPTS){
							if(pPcrProcesser->lastDemuxVideoPTS != pPcrProcesser->prevVideoDemuxPTS){
								pPcrProcesser->videoFreeRunCheckCount++;
							}
							if(pPcrProcesser->videoFreeRunCheckCount >= 5)
								REFCLK_SetVideoFreeRunThreshold(pPcrProcesser->pRefClock, MIN_FREE_RUN_THRESHOLD);
						}
					}
...
void PCRSYNC_CheckAVPTSStable(DEMUX_PCRSYNC_T *pPcrProcesser)
{
	...
	if(!pPcrProcesser->isUnderFreeRunModePTS){
		...
		{
			if(AFreerunThresholdValue == MIN_FREE_RUN_THRESHOLD && (false == pPcrProcesser->isUnderFreeRunMode_audio) && ( mpts < pPcrProcesser->lastDemuxAudioPTS) ){
				REFCLK_SetAudioFreeRunThreshold(pPcrProcesser->pRefClock, DEFAULT_FREE_RUN_THRESHOLD, DEFAULT_FREE_RUN_THRESHOLD);
...
void PCRSYNC_CheckPTSStable(DEMUX_PCRSYNC_T *pPcrProcesser)
{
	...
	if((pPcrProcesser->bNoAudioPts != bNoAudioPts) || (pPcrProcesser->bNoVideoPts != bNoVideoPts))
	{
		...
		if((pPcrProcesser->referenceAVPTS > AV_NONE) && (pPcrProcesser->preferAVPTS != pPcrProcesser->referenceAVPTS) && (pPcrProcesser->preferAVPTS != beforePreferAVPTS))
			pPcrProcesser->checkPcrOffset = PREPARE_TO_GET_OFFSET;
	}
}
...
void PCRSYNC_UpdateFullnessStatus(DEMUX_PCRSYNC_T *pPcrProcesser)
{
	...
	if (pPcrProcesser->bNoAudioPts == false && pPcrProcesser->lastDemuxAudioPTS != -1 && pPcrProcesser->lastAudioSystemPTS != -1) {
		...
		if (pPcrProcesser->bVirtualPcrMode == false && (pPcrProcesser->isTrackingSteady == 1 || pPcrProcesser->isTrackingSteady == 3 || pPcrProcesser->bHighWaterLevelMode == false) && ((pPcrProcesser->bNoAudioOutputPts == false && audioUnderflow != 0) || (pPcrProcesser->referenceAVPTS == AUDIO_TYPE && (mpts > (pPcrProcesser->lastDemuxAudioPTS + pPcrProcesser->audio_pts_gap))))) {
			pPcrProcesser->audioFullnessLowCount++;
			dmx_mask_print(DEMUX_PCRSYC_DEBUG,"[dtv(%d) #%d]: PCRSYNC_UpdateFullnessStatus %ld %d %lld %lld %lld\n", pPcrProcesser->tunerHandle, __LINE__, audioUnderflow, mpts > pPcrProcesser->lastDemuxAudioPTS, mpts, pPcrProcesser->lastDemuxAudioPTS, pPcrProcesser->audio_pts_gap);
		}
...
void PCRSYNC_CheckFullness(DEMUX_PCRSYNC_T *pPcrProcesser)
{
	if (pPcrProcesser->bHighWaterLevelMode == false && ((pPcrProcesser->audioFullnessLowCount > 100) || (pPcrProcesser->videoFullnessLowCount > 100))) {
		...
		if (pPcrProcesser->bVirtualPcrMode == false && pPcrProcesser->isTrackingSteady == 3 && pPcrProcesser->preferAVPTS == AV_NONE) {
...
void PCRSYNC_UpdateRCDStableStatus(DEMUX_PCRSYNC_T *pPcrProcesser, NAVDTVPCRINFO pcrinfo)
{
	...
	if (rcdMonitor->bIsArrayWrapAround) {
		...
		rcdMonitor->audio_rcdUnstableTotal -= rcdMonitor->audio_array[rcdMonitor->index];
	}
	...
	if (rcd_diff >= RCD_UNSTABLE_AUDIO_LEVEL_S) {
		rcdMonitor->audio_array[rcdMonitor->index] = RCD_UNSTABLE_WEIGHTS_S;
	} else if (rcd_diff >= RCD_UNSTABLE_AUDIO_LEVEL_A) {
		rcdMonitor->audio_array[rcdMonitor->index] = RCD_UNSTABLE_WEIGHTS_A;
	} else if (rcd_diff >= RCD_UNSTABLE_AUDIO_LEVEL_B) {
		rcdMonitor->audio_array[rcdMonitor->index] = RCD_UNSTABLE_WEIGHTS_B;
	} else if (rcd_diff >= RCD_UNSTABLE_AUDIO_LEVEL_C) {
		rcdMonitor->audio_array[rcdMonitor->index] = RCD_UNSTABLE_WEIGHTS_C;
	} else {
		rcdMonitor->audio_array[rcdMonitor->index] = RCD_UNSTABLE_WEIGHTS_STABLE;
	}
...
void PCRSYNC_CheckRCDStable(DEMUX_PCRSYNC_T *pPcrProcesser)
{
	int rcdUnstableValue = 0, audio_rcdUnstableValue = 0, rcdStabilityCount = 0;
	...
	REFCLK_GetFreeRunThreshold(pPcrProcesser->pRefClock,&VFreerunThresholdValue,&AFreerunThresholdValue,&AFreerunThrToWaitValue);
	audio_rcdUnstableValue = (pPcrProcesser->rcdStabilityMonitor.audio_rcdUnstableTotal*100)/rcdStabilityCount;
	dmx_mask_print_info(DEMUX_PCRSYC_DEBUG_EX1, "RCD v_value [%d] a_value [%d]\n", rcdUnstableValue, audio_rcdUnstableValue);
	...
	if(rcdUnstableValue >= RCD_UNSTABLE_THRESHOLD_HIGH){
		...
		if(!pPcrProcesser->isUnderFreeRunModePTS && !pPcrProcesser->isUnderFreeRunMode_audio){
	...
	if(audio_rcdUnstableValue >= RCD_UNSTABLE_THRESHOLD_HIGH){
		if(!pPcrProcesser->isUnderFreeRunMode_audio){
		    dmx_mask_print(DEMUX_PCRSYC_DEBUG,"[dtv(%d)]:RCD[%d/%d] RCD not stable, set audio to free run!!!\n", pPcrProcesser->tunerHandle, audio_rcdUnstableValue, RCD_UNSTABLE_THRESHOLD_HIGH);
		    REFCLK_SetAudioFreeRunThreshold(pPcrProcesser->pRefClock, MIN_FREE_RUN_THRESHOLD, MIN_FREE_RUN_THRESHOLD);
		    pPcrProcesser->isUnderFreeRunMode_audio = true;
		    //pPcrProcesser->checkPcrOffset = PREPARE_TO_GET_OFFSET;
		}
	}else{
		if(pPcrProcesser->isUnderFreeRunMode_audio){
		    dmx_mask_print(DEMUX_PCRSYC_DEBUG,"[dtv(%d)]:RCD[%d/%d] RCD is stable, set audio to pcr master!!!\n", pPcrProcesser->tunerHandle, audio_rcdUnstableValue, RCD_UNSTABLE_THRESHOLD_HIGH);
		    REFCLK_SetAudioFreeRunThreshold(pPcrProcesser->pRefClock, DEFAULT_FREE_RUN_THRESHOLD, DEFAULT_FREE_RUN_THRESHOLD);
		    pPcrProcesser->isUnderFreeRunMode_audio = false;
		    //pPcrProcesser->checkPcrOffset = PREPARE_TO_GET_OFFSET;
		}
...
void PCRSYNC_UpdateDemuxPTS(DEMUX_PCRSYNC_T *pPcrProcesser, bool bVideo, bool bAudio, int64_t pts, bool bIsSubAudio)
{
	if (bVideo) {
		...
		if (pPcrProcesser->startVideoPts == -1) {
			...
			pPcrProcesser->streamFirstDemuxVideoPTS = pts;
		...
		if( pPcrProcesser->lastDemuxVideoPTS != -1 && pts < (pPcrProcesser->lastDemuxVideoPTS - 500*DEMUX_CLOCKS_PER_MS))
			pPcrProcesser->streamFirstDemuxVideoPTS = pts;
		...
		if (!bIsSubAudio) { /*main Audio*/
			...
			if (pPcrProcesser->startAudioPts == -1) {
				...
				pPcrProcesser->streamFirstDemuxAudioPTS = pts;
			...
			if(pPcrProcesser->lastDemuxAudioPTS != -1 && pts < (pPcrProcesser->lastDemuxAudioPTS - 500*DEMUX_CLOCKS_PER_MS) )
				pPcrProcesser->streamFirstDemuxAudioPTS = pts;
			...
			/*add for TCL2851M-1244: mpts > apts*/
			if( (pPcrProcesser->rcd != -1) && (pPcrProcesser->lastDemuxAudioPTS != -1) && (pPcrProcesser->lastDemuxVideoPTS != -1)
				&& (pPcrProcesser->bNoAudioPts == false) && (pPcrProcesser->bNoVideoPts == false)
				&& ( (RCD + pll) > pPcrProcesser->lastDemuxAudioPTS )){
					long madiff = RCD + pll - pPcrProcesser->lastDemuxAudioPTS;
					//dmx_mask_print(DEMUX_PCRSYC_DEBUG,"rcd = %lld pll=%lld audio_pts = %lld\n", RCD, pll, pPcrProcesser->lastDemuxAudioPTS);
					if(madiff < DTV_DELAY_TIME){
						pPcrProcesser->pcrOffset = 2*DTV_DELAY_TIME;
						dmx_mask_print(DEMUX_PCRSYC_DEBUG,"[dtv(%d)]: MPTS > APTS!!! Increase pcroffset. MPTS:%lld, demux APTS:%lld, demux VPTS:%lld",
							(int)pPcrProcesser->tunerHandle, RCD + pll, pPcrProcesser->lastDemuxAudioPTS, pPcrProcesser->lastDemuxVideoPTS);
					}
			}
