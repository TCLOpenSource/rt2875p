...
#include "rtkdemux_ecp_ta.h"
#include <VideoRPC_System.h>
#include <vdec/common/rtk_vdec_svp_common.h>
...
#else
extern void rtkvdec_vtm_state(void)
{
	dmx_crit("rtkvdec_vtm_state doesn't work due to vdec disable\n");
}
#endif
...
{
    dmx_crit("rtkaudio_send_string doesn't work due to audio disabled\n");
}
#endif
...
void DEMUX_IPCCopyMemory(char *des, char *src, unsigned long len)
{
	...
	if ((((long)src & 0x3) != 0) || (((long)des & 0x3) != 0) || ((len & 0x3) != 0))
...
#ifdef ENABLE_ECP_SERVICE
void DEMUX_check_ecp_status(demux_channel *pCh)
{
	bool ecp_status;

	ecp_status = (bool)RHAL_TP_CheckProtectStatus(TP_TP0);

	if (ecp_status == dmx_ecp_status){
		return;
	}
	if (ecp_status) {
		dmx_crit("%s_%d: ECP ON\n", __func__, __LINE__);
...
void Demux_Enter_ECP(demux_channel *pCh)
{
	int pin = 0;

	if(pCh->isECP == 1){
		dmx_notice("[%s %d] ECP already exit!!\n", __func__, __LINE__);
		return;
	}
	...
	DEMUX_TA_ECP_INIT(pCh);
	set_dtv_securestatus(TRUE);

	//unmap tp , set protection
	if (pCh->tpBuffer.phyAddr){
		DEMUX_BUF_T *pBuf  = &pCh->tpBuffer;
		dmx_notice("[%s] TP pBuf->cachedaddr=%lx pBuf->virAddr=%lx pBuf->nonCachedaddr=%lx pBuf->phyAddr=%lx\n",
			__func__, (unsigned long)(uintptr_t)pBuf->cachedaddr, (unsigned long)(uintptr_t)pBuf->virAddr, (unsigned long)(uintptr_t)pBuf->nonCachedaddr, (unsigned long)pBuf->phyAddr);
        if(pBuf->isCache){
                dvr_unmap_memory((void *)pBuf->cachedaddr, pBuf->allocSize);
        }
		rtkvdec_svp_cma_memory_init(pBuf->phyAddr, pBuf->allocSize);  //replace unmap_kernel_line_addr(pBuf->phyAddr, pBuf->allocSize);
		pBuf->cachedaddr =	pBuf->virAddr = NULL;
		//set protection
		RHAL_TP_EnableMemProtect(pCh->tunerHandle, MEMORY_PRISON_TP, pCh->tpBuffer.phyAddr,  pCh->tpBuffer.phyAddr + pBuf->allocSize);
	}
	//unmap video es. set protection
	for (pin=0;pin< NUMBER_OF_PINS;pin++){
		if(IS_VIDEO_PIN(pin) && pCh->bsDecBuf[pin].phyAddr && pCh->bsDecBuf[pin].virAddr != NULL)
		{
			DEMUX_BUF_T *pBuf  = &pCh->bsDecBuf[pin];
			dmx_notice("[%s %d] get pin=%d pBuf->isCache=%d dec buffer cache=%lx, nocache=%lx\n", __func__, __LINE__, pin, pBuf->isCache, (unsigned long)(uintptr_t)pBuf->cachedaddr, (unsigned long)(uintptr_t)pBuf->nonCachedaddr);
			dvr_unmap_memory(pBuf->isCache ? (void *)pBuf->cachedaddr : (void *)pBuf->nonCachedaddr, pBuf->size);
			rtkvdec_svp_cma_memory_init(pBuf->phyAddr, pBuf->allocSize);  //replace unmap_kernel_line_addr(pBuf->phyAddr, pBuf->allocSize);
			if(pBuf->isCache){
				pBuf->virAddr = pBuf->cachedaddr = NULL;
			}
			else{
				pBuf->virAddr = pBuf->nonCachedaddr = NULL;
			}
			//TODO, SEC PROTECTION
			RHAL_TP_EnableMemProtect(pCh->tunerHandle, MEMORY_PRISON_VIDEO_ES, pBuf->phyAddr,  pBuf->phyAddr+pBuf->size);
		}
	}
	dmx_notice("[%s_%d] enter ecp successful!!!!!\n", __func__, __LINE__);
}

void Demux_Exit_ECP(demux_channel *pCh)
{
	int pin = 0;
	...
	DEMUX_TA_ECP_UNINIT(pCh);

	if (pCh->tpBuffer.phyAddr){
		DEMUX_BUF_T *pBuf	= &pCh->tpBuffer;
		dmx_notice("[%s][ch%d] cancel TP buffer protect!\n",__func__,  pCh->tunerHandle);
		RHAL_TP_DisableMemProtect(pCh->tunerHandle, MEMORY_PRISON_TP);
		rtkvdec_svp_cma_memory_free(pBuf->phyAddr, pBuf->allocSize); //replace remap_kernel_line_addr(pBuf->phyAddr, pBuf->allocSize);
		if(pBuf->isCache){
	...
	for (pin=0;pin< MAX_VDEC_NUM;pin++){
		if(IS_VIDEO_PIN(pin) && pCh->bsDecBuf[pin].phyAddr && pCh->bsDecBuf[pin].virAddr == NULL)
		{
			DEMUX_BUF_T *pBufBS  = &pCh->bsDecBuf[pin];
			dmx_notice("[%s][ch%d] cancel ES buffer protect!\n",__func__,  pCh->tunerHandle);
			RHAL_TP_DisableMemProtect(pCh->tunerHandle, MEMORY_PRISON_VIDEO_ES);
			rtkvdec_svp_cma_memory_free(pBufBS->phyAddr, pBufBS->allocSize); //replace remap_kernel_line_addr(pBufBS->phyAddr, pBufBS->allocSize);
			if(pBufBS->isCache){
				pBufBS->cachedaddr = dvr_remap_cached_memory(pBufBS->phyAddr, pBufBS->allocSize, __builtin_return_address(0));
				pBufBS->virAddr = pBufBS->cachedaddr;
			}
			else{
				pBufBS->nonCachedaddr = dvr_remap_uncached_memory(pBufBS->phyAddr, pBufBS->allocSize, __builtin_return_address(0));
				pBufBS->virAddr = pBufBS->nonCachedaddr;
			}
			dmx_notice("[%s] pBuf->cachedaddr=%lx pBuf->virAddr=%lx pBuf->nonCachedaddr=%lx pBuf->phyAddr=%lx\n",
				__func__, (unsigned long)(uintptr_t)pBufBS->cachedaddr, (unsigned long)(uintptr_t)pBufBS->virAddr, (unsigned long)(uintptr_t)pBufBS->nonCachedaddr, (unsigned long)pBufBS->phyAddr);
		}
	}
	...
	for (pin =0; pin < MAX_PID_FILTER_CB_NUM; pin++){
		if (pCh->pesMap[pin].occupied || pCh->pesMap[pin].rbHeader.virAddr){
			pCh->pesMap[pin].pWrPtr 	   = (unsigned int *)&((RINGBUFFER_HEADER *)pCh->pesMap[pin].rbHeader.virAddr)->writePtr;
			pCh->pesMap[pin].pRdPtr 	   = (unsigned int *)&((RINGBUFFER_HEADER *)pCh->pesMap[pin].rbHeader.virAddr)->readPtr[0];
		}
		pCh->pesMap[pin].phyAddrOffset = (long) pCh->pesMap[pin].phyAddr - (long) pCh->pesMap[pin].virAddr;
	}
	set_dtv_securestatus(FALSE);
	pCh->isECP = 0;
	dmx_notice("[%s_%d] exit ecp successful!!!!!\n", __func__, __LINE__);
}
#endif
...
inline int VerifyInBandCommandData(INBAND_CMD_TYPE type, UINT32 size)
{
	UINT32 command_size;
	switch(type)
	{   
		case INBAND_CMD_TYPE_PTS:
			command_size = sizeof(PTS_INFO);
			break;
		default:
			return 1;
			break;
...
inline int CheckInBandCommandData(DEMUX_BUF_PIN_T *pPinInfo)
{
	unsigned char *MonitorRP;
	unsigned char *MonitorRP_temp;
	unsigned char *MonitorWP;
	UINT32 MonitorType;
	UINT32 MonitorSize;

	MonitorRP = (unsigned char *)(unsigned long)DEMUX_reverseInteger(*pPinInfo->pRdPtr);
	MonitorWP = (unsigned char *)(unsigned long)DEMUX_reverseInteger(*pPinInfo->pWrPtr);
	MonitorRP_temp = MonitorRP;

	while( MonitorRP_temp!=MonitorWP )
	{
		MonitorType = DEMUX_reverseInteger(*((UINT32*)(MonitorRP_temp - pPinInfo->phyAddrOffset)));
		MonitorRP_temp = MonitorRP_temp + 4;
		if(MonitorRP_temp >= pPinInfo->pBufferUpper)
			MonitorRP_temp = MonitorRP_temp - (pPinInfo->pBufferUpper - pPinInfo->pBufferLower);
		MonitorSize = DEMUX_reverseInteger(*((UINT32*)(MonitorRP_temp - pPinInfo->phyAddrOffset)));

		if(VerifyInBandCommandData(MonitorType, MonitorSize) == 0)
		{
			unsigned char *temp;
			dmx_crit("InBandCommand DataError Lower=0x%p Upper=0x%p RP=0x%p WP=0x%p RP_Happen=%p Type=%d size=%d\n", pPinInfo->pBufferLower, pPinInfo->pBufferUpper, MonitorRP, MonitorWP, MonitorRP_temp - 4, MonitorType, MonitorSize);
			MonitorRP_temp = MonitorRP;
			while(MonitorRP_temp != MonitorWP)
			{
				temp = MonitorRP_temp - pPinInfo->phyAddrOffset;
				dmx_crit("0x%p -> 0x%02x%02x%02x%02x\n", MonitorRP_temp, temp[0], temp[1], temp[2], temp[3]);
				MonitorRP_temp = MonitorRP_temp + 4;
				if(MonitorRP_temp >= pPinInfo->pBufferUpper)
					MonitorRP_temp = MonitorRP_temp - (pPinInfo->pBufferUpper - pPinInfo->pBufferLower);
			}
		...
		MonitorRP_temp = MonitorRP_temp + MonitorSize - 4;
		if(MonitorRP_temp >= pPinInfo->pBufferUpper)
			MonitorRP_temp = MonitorRP_temp - (pPinInfo->pBufferUpper - pPinInfo->pBufferLower);
	}
	return 0 ;
...
inline int DeliverInBandCommand(DEMUX_BUF_PIN_T *pPinInfo, unsigned char *Data, long size)
{
	...
	if (phyWr + size > (unsigned long)pPinInfo->pBufferUpper) { /* wrap-around */
		unsigned long space = (unsigned long)pPinInfo->pBufferUpper - phyWr ;
...
{
    void *context = (void *)(unsigned long)((channel->tunerHandle << 8) | index) ;
    mutex_lock(&channel->section_mutex);
...
int DEMUX_GetSecRingBuffer(void *pContext, size_t count)
{
    ...
    if ((demux_device->secBufCnt + count) > SEC_FILTER_BUFFER_UNIT_CNT) {
        ...
        dmx_err("[%s %d] section buffer is enough(used %d, new %ld)!!\n", __func__, __LINE__, demux_device->secBufCnt, (unsigned long)count);
        return -1;
    }
...
int DeliverPrivateInfo(demux_channel *pCh, DEMUX_BUF_PIN_T *pPinInfo, int infoId, char *pInfo, int length, DEMUX_BUF_PIN_T *pBSPinInfo)
{
	...
	if (pCh->isECP)
	{
		res = DEMUX_TA_ECP_DeliverPrivateInfo(pCh, infoId, pInfo, length, ((long)pPinInfo - (long)pCh), ((long)pBSPinInfo - (long)pCh));
		return res;
	}
...
*/
		...
		if (infoId == PRIVATEINFO_AUDIO_END_OF_FILE) {
			...
			memset(&cmd, 0, sizeof(EOS));
			cmd.header.type = INBAND_CMD_TYPE_SEQ_END;
			cmd.header.size = sizeof(EOS);
			cmd.wPtr = DEMUX_reverseInteger(*pBSPinInfo->pWrPtr);
			cmd.eventID = *(u32 *)pInfo;
			res =  DeliverInBandCommand(pPinInfo, (unsigned char *)&cmd, cmd.header.size);
...
{
	int64_t time_start = 0, time_end = 0, time_diff = 0;

	time_start = CLOCK_GetPTS();
	...
	time_end = CLOCK_GetPTS();

	time_diff = time_end - time_start;

	//If the sleep time exceed 60 ms, show warning
	if(time_diff > 60 * 90)
		dmx_warn("sleep too long !! target (%ld ms) but (%lld / 90) ms \n", pNavBuffer->wait.waitTime, time_diff);
...
int SyncWritePointers(demux_channel *pCh, unsigned int WP_update_flags, unsigned char *WP[])
{
	...
	for (i = 0 ; i < NUMBER_OF_PINS ; i++) {
		...
		if ((WP_update_flags & (1<<i)) != 0) {
			...
			unsigned char   *pWritePointer = (unsigned char *)(unsigned long)DEMUX_reverseInteger(*pPinInfo->pWrPtr) ;
			if (!pCh->bUseDDRCopy) {
...
#endif
			*pPinInfo->pWrPtr = DEMUX_reverseInteger((u32)(unsigned long)WP[i]) ;
		}
	}
...
int DEMUX_GetESBufferData(demux_channel *pCh, unsigned int *bufferDataSize)
{
	...
	for (i = 0 ; i < NUMBER_OF_PINS ; i++) {
		...
		writableSize = BS_DISTANCE(phyWr, phyRd, pPinInfo->pBufferLower, pPinInfo->pBufferUpper) - 1 ;
		*bufferDataSize += pPinInfo->pBufferUpper - pPinInfo->pBufferLower - writableSize;
		}
	//dmx_err("toddy DEMUX_GetESBufferData *bufferDataSize=%u \n", *bufferDataSize);
	return 0 ;
...
int DEMUX_DeliverData(demux_channel *pCh, NAVDEMUXOUT *pDemuxOut)
{
	...
	if (pCh->isECP){
		ret = DEMUX_TA_ECP_DeliverData(pCh, (unsigned long)pDemuxOut - (unsigned long)pCh, pCh->pcrProcesser.pRefClock);
		return ret;
	}
	...
	for (i = 0 ; i < pDemuxOut->numBuffers ; i++) {
		...
		if (pNavBuffer->data.payloadSize > 0) {
			...
			if (pNavBuffer->data.pts >= 0) {
				if (pCh->tp_src == MTP && rtkdemux_dbg_esbufmonitor_en) {
					if (IS_AUDIO_PIN(pNavBuffer->data.pinIndex) || pCh->pcrProcesser.lastDemuxAudioPTS == -1) {
						if (PVR_CheckESBufStable(pCh)) {
							NAV_SYNC_WRITE_POINTERS();
							pDemuxOut->pNavBuffers = &pDemuxOut->pNavBuffers[i];
							pDemuxOut->numBuffers -= i;
							return -1;
						} else if ( pCh->pvrSpeed ==0) {
							dmx_mask_print(DEMUX_PVR_ES_MONITOR_DEBUG, "ch%d, deliverdata in stepMode: deliverNum:%d \n", pCh->tunerHandle,pDemuxOut->numBuffers );
						}
					}
				}
				...
				NAV_SYNC_WRITE_POINTERS();
				rtkdemux_dbg_dump_pts(pCh, pNavBuffer->data.pinIndex, pNavBuffer->data.pts);
	...
	if (pCh->bUpdateDemuxTarget) {
		...
		for (i = 0 ; i < pCh->pidActiveCount ; i++) {
			...
			if (!got) {
				...
				/*        dmx_info("Remove PID line %d, rm_count %d, %d, %d, %d\n", __LINE__, rm_count, got, true, false) ;*/
		...
		for (i = 0 ; i < pCh->pidFilterCount ; i++) {
			...
			if (target >= 0) {
				...
				memset(pCh->demuxTargetInfo[target].PesHeaderParseInfo.pesHeaderBuf, 0, PES_HEADER_BUF_SIZE) ;
			}
		}
...
void DEMUX_HandleCriticalEvents(demux_channel *pCh)
{
	...
	if (HAS_FLAG(pCh->status, STATUS_FLUSH_MTP)) {
		...
		dmx_notice("[%s %d] handle STATUS_FLUSH_MTP\n", __func__, __LINE__);
		DEMUX_EVENT_FlushTPBuffer(pCh);
...
#ifdef ENABLE_ECP_SERVICE
		//---- Set ECP protection---
		if (HAS_FLAG(pCh->events, EVENT_ENTER_ECP_MODE) && (pCh->isECP == 0)) {
			Demux_Enter_ECP(pCh);
			RESET_FLAG(pCh->events, EVENT_EXIT_ECP_MODE | EVENT_ENTER_ECP_MODE);
		}
	
		if (HAS_FLAG(pCh->events, EVENT_EXIT_ECP_MODE)) {
			Demux_Exit_ECP(pCh);
			RESET_FLAG(pCh->events, EVENT_EXIT_ECP_MODE | EVENT_ENTER_ECP_MODE);
		}
#endif
...
static int DEMUX_GetWaitTime(demux_channel *pCh)
{
	...
	if (byteRate) {
        ...
        if (pPinInfo->pRBH != NULL && pPinInfo->pRBH->reserve2 ==DEMUX_PESBUFFER_OP_INITIALIZED) {
...
int DEMUX_EVENT_AudioNewFormat(demux_channel *pCh, int pin, DEMUX_AUDIO_DEC_T audioFormat, s32 privateInfo[8])
{
	...
	cmd.audioType   = (AUDIO_DEC_TYPE)audioFormat ;
...
int DEMUX_ReadData(demux_channel *pCh, NAVBUF *pBuffer, int isProhibited)
{
	...
	if (pCh->pTPRelase) {
		...
		pCh->tpBuffer.virAddr = (unsigned char *)( (long)pCh->tpBuffer.phyAddr - pCh->phyAddrOffset );
		if (pLastReadAddr >= (pCh->tpBuffer.virAddr + pCh->tpBuffer.size))
	...
	if (pLastReadAddr && pBuffer->data.payloadSize > 0 && pLastReadAddr != pBuffer->data.payloadData)
		dmx_crit("[%s %d] ERROR: read ptr shift : last = 0x%p, now = 0x%p, tp = 0x%p/0x%lx\n", __func__, __LINE__, pLastReadAddr, pBuffer->data.payloadData, pCh->tpBuffer.nonCachedaddr, (unsigned long)pCh->tpBuffer.phyAddr);
	...
	if (ret == TPK_BUFFER_FULL) {
		...
		if(pCh->isECP){
			DEMUX_TA_ECP_inv_range(pCh, (long)pCh->pTPRelase, pCh->TPReleaseSize);
		}
		else{
...
int DEMUX_WritePesPrefixData(demux_channel *pCh, int pesIndex, int length)
{
	...
	pCurWr = (unsigned char *)(unsigned long)(*pMap->pWrPtr);
...
bool ParseTemiInfo(demux_channel *pCh, int target, TEMI_INFO *pTemiInfo, unsigned char * pPacketStart, int phyAddrOffset)
{
	...
	//if ((af_ex_flag & 0x10)==1)//bitwise comparison always evaluates to false
		//return false;
		...
		{ 
			...
			if (af_descr_flag_has_timeStamp)
			{
				...
				if (af_descr_flag_has_timeStamp == 1)
					...
					pTemiInfo->u64MediaTimestamp = ((uint64_t)pData[0]<<24)|((uint64_t)pData[1]<<16)|((uint64_t)pData[2]<<8)|((uint64_t)pData[3]);
					pTemiInfo->u64MediaTimestamp= (pTemiInfo->u64MediaTimestamp <<32)|((uint64_t)pData[4]<<24)|((uint64_t)pData[5]<<16)|((uint64_t)pData[6]<<8)|((uint64_t)pData[7]);
				}
			}
...
bool ParsePacketCB(demux_channel *pCh, int target, unsigned char *pPacketStart,  int phyAddrOffset)
{
	...
	if (HAS_AF(pPacketStart)) {
		...
		{
			TEMI_INFO sTemiInfo;
			unsigned char * pCurWr = (unsigned char *)(unsigned long)*pMap->pWrPtr;
...
int Demux_Parse(demux_channel *pCh, NAVDEMUXIN *pDemuxIn, NAVDEMUXOUT *pDemuxOut)
{
	...
	if (pCh->isECP){
		int ret = -1;
		ret =  DEMUX_TA_ECP_Parse(pCh, (unsigned long)pDemuxIn -(unsigned long) pCh, (unsigned long)pDemuxOut - (unsigned long)pCh);
		if(demux_device->ecp_pes_flag == 1){
			demux_device->ecp_pes_flag = 0;
			DEMUX_wakeUpPoll();
		}
		return ret;
	}
	...
	while (pDemuxIn->pCurr+188 <= pDemuxIn->pEnd) {
		...
		if (IS_OFF_SYNC(pDemuxIn->pCurr)) {
			...
			/* dmx_info("pid %d, target %d targetCB %d\n", pid, target, targetCB) ; */
...
int DEMUX_ThreadProc(demux_channel *pCh)
{
	...
	while (!kthread_should_stop()) {
...
#ifdef ENABLE_ECP_SERVICE
		/*check ecp status, If open, enter ecp. Only DVB need ECP function*/
		if(RHAL_Delivery_GetMode(pCh->tunerHandle) == TP_DELIVERY_MODE_TS && pCh->tunerHandle == 0){
			DEMUX_check_ecp_status(pCh);
		}
#endif
		...
		if (pCh->startStreaming && !pCh->bIsSDT && DEMUX_HasReferenceClock(pCh)) {
			...
			const  int64_t current__ = CLOCK_GetPTS();
			const  int64_t passed__  = abs64(current__ - last__);
			long pVideoFreeRunThr = 0, pAudioFreeRunThr = 0, pAudioFreeRunThrToWait = 0;


			if( 5*DEMUX_CLOCKS_PER_SEC <= passed__ ) {

				REFCLK_GetFreeRunThreshold((REFCLOCK *)refClock, &pVideoFreeRunThr, &pAudioFreeRunThr, &pAudioFreeRunThrToWait);

				dmx_mask_print(DEMUX_NOTICE_PRINT|DEMUX_PCRSYC_DEBUG,"(CH%d)RTKDEMUX: RCD:%lld, MPTS: %lld, mastership mode %d, s %d, v %d, a %d, st %d, vPTS %lld, aPTS %lld, bitRate %x, VideoFreeRunThr %ld, AudioFreeRunThr %ld, AudioFreeRunThrToWait %ld\n"
					, pCh->tunerHandle
					...
					, pCh->bitRate
					, pVideoFreeRunThr, pAudioFreeRunThr, pAudioFreeRunThrToWait);
				last__ = current__;
			}
...
long Demux_REFCLK_SetClockMode(int ch, long clockMode, int type){
	demux_channel *pCh;
	long clk_src_ch=-1;

	pCh = &demux_device->pChArray[ch];
	if(type == 0){
		if(pCh != NULL && (1 == pCh->bPcrTrackEnable)) {
			/*get refclock mode*/
			clk_src_ch = REFCLK_GetClockMode(DEMUX_GetReferenceClock(pCh));
		}
	}else{
		if(pCh != NULL && (1 == pCh->bPcrTrackEnable)) {
			REFCLK_SetClockMode(DEMUX_GetReferenceClock(pCh), clockMode);
		}
	}
	return clk_src_ch;
}
