	...
	for (i = 0; i < num; i++) {
		...
		if (i%RPC_NR_PAIR == 1) {
			if (rpc_kthread[i/RPC_NR_PAIR] == 0)
				rpc_kthread[i/RPC_NR_PAIR] = kthread_run(rpc_kernel_thread, (void *)i, "%s", rpc_kthread_name[i/RPC_NR_PAIR]);
		}
	}
		...
		if (i%RPC_NR_PAIR == 1) {
			if (rpc_kthread[(i+RPC_NR_KERN_DEVS)/RPC_NR_PAIR] == 0)
				rpc_kthread[(i+RPC_NR_KERN_DEVS)/RPC_NR_PAIR] = kthread_run(rpc_kernel_thread,
						(void *)(i+RPC_NR_KERN_DEVS), "%s", rpc_kthread_name[(i+RPC_NR_KERN_DEVS)/RPC_NR_PAIR]);
		}
	}
#endif
	if (!is_init) {
...
static int rpc_kernel_thread(void *p)
{
	char readbuf[sizeof(RPC_STRUCT)+3*sizeof(unsigned long)] = {0};
	RPC_KERN_Dev *dev;
	RPC_STRUCT *rpc;
	unsigned long *tmp;
			...
			||(htonl(rpc->programID) == VIDEO_AGENT) || (htonl(rpc->programID)== KERNELID)) //RPC data
		{
			...
			*(tmp+1) = htonl(ret);
			...
			if(countack == LOOPCOUNT){
				...
				dev->ringOut = dev->ringIn;
				up(&dev->ptrSync->readSem);
				rtd_pr_rpc_err("kernel rpc ack: program:%lu version:%lu procedure:%lu taskID:%lu sysPID:%lx size:%lu context:%lx",
...
void dump_kernel_rpc_data(int opt)
{
	...
	if(kernel_rpc_debug_flag[RPC_KERN_DEV_V1S_ID3/RPC_NR_PAIR] == 0){
		...
		my_memcpy((int *)&record_timeout_readbuf[opt], (int *)devread, sizeof(record_timeout_readbuf));
		my_memcpy((int *)&record_timeout_writebuf[opt], (int *)devwrite, sizeof(record_timeout_writebuf));
	}
    ...
    unsigned long *retvalue)
{
	...
	if( opt == RPC_VIDEO2)
	{
		...
		{
			rtd_pr_rpc_err("RPCkern: VCPU2 not finished init RPC ...\n");
			up(&send_kernel_rpc_sem[opt]);
			return RPC_VCPU_NOT_READY;
		}
	}
...
//	rtd_pr_rpc_info(" aaa: %x bbb: %x \n", sendbuf, tmp);
	...
	*(tmp+2) = htonl(param2);
	...
	if (rpc_kern_write(opt, sendbuf, sizeof(sendbuf)) != sizeof(sendbuf)) {
		rtd_pr_rpc_err("ERROR in send kernel RPC to %s : command:%lx, parm1: %lx, parm2: %lx ...\n", rpc_kthread_name[opt],
	...
	if(send_mode & KRPC_BLOCK_MODE) //for block mode
   	{
	    ...
	    if (!wait_event_timeout(rpc_wq[opt], info.reply, TIMEOUT)) {
			rtd_pr_rpc_err("[krpc timeout v1.2]kernel rpc to %s wait ack timeout ->disable %s thread:  command:%lx, parm1: %lx, parm2: %lx ...\n", rpc_kthread_name[opt],
			...
			if(krpc_handle_command_count[opt] != 0 )
				...
				show_stack(rpc_kthread[opt], NULL, KERN_ERR);
#else
