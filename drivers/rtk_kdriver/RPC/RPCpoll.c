...
int rpc_poll_open(struct inode *inode, struct file *filp)
{
    ...
    PDEBUG("RPC poll open with minor number: %d\n", minor);
        ...
        if (!filp->private_data) {
				...
				rtd_pr_rpc_err("%s: no such device ! \n", __func__);
				kfree(proc);
				return -ENXIO;
			}
...
int rpc_poll_release(struct inode *inode, struct file *filp)
{
        ...
        if(extra->currProc == proc){
			...
			||minor == 14
                        ...
                        if(!rpc_done(extra)){
                                rtd_pr_rpc_err("%s: previous rpc hasn't finished, force clear!! ringOut %p => %p\n", __func__,
                ...
                loff_t *f_pos)
{
        ...
        if (rpc_poll_is_paused) {
#ifndef CONFIG_ARM64
		rtd_pr_rpc_err("[rpc_poll_write] write rpc_kern_device: caller%x, *buf:0x%x \n", (unsigned int) __read_32bit_caller_register(), *(unsigned int *)buf);
#endif
                rtd_pr_rpc_err("RPCpoll: someone access rpc poll during the pause...\n");
                msleep(1000);
                return -EAGAIN;
...
static void show_block_user_rpc(int index)
{
    ...
    list_for_each_entry(rrpc, &extra->blockrpc_list, list){
        rtd_pr_rpc_info("%s: program:%lu version:%lu procedure:%lu taskID:%lu sysPID:%lx size:%lu context:%lx, count:%d\n",
...
void show_rpc_buffer(RPC_POLL_Dev *pdev)
{
	...
	for(j=0;j<RPC_RING_SIZE ;j++)
	{
		if (j%16 == 0) {
			...
			rtd_pr_rpc_info(" ");
		rtd_pr_rpc_info("%02x",(unsigned char)*(dev->ringStart +j));
		}
	rtd_pr_rpc_info("\n");
	rtd_pr_rpc_info("=============================================\n");
	...
	const char *buf, size_t count, loff_t *f_pos)
{
	...
	if ((cmd_buf[0] == 'd') && (cmd_buf[1] == 'u') && (cmd_buf[2] == 'm') && (cmd_buf[3] == 'p')) {
		...
		if(send_rpc_command(RPC_KCPU,RPC_SCPU_DEBUG_COMMAND, 0, 0, &rpc_ret) == RPC_OK)
			rtd_pr_rpc_info("send debug kernel rpc to kcpu success(%ld).\n", rpc_ret);
		else
			rtd_pr_rpc_info("send debug kernel rpc to kcpu failed(%ld).\n", rpc_ret);
	}
#endif
	return ret;
}
