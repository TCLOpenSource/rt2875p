	...
	if (itr & (RPC_INT_AS | RPC_INT_VS |RPC_INT_V2S
		...
		while (itr & (RPC_INT_AS | RPC_INT_VS |RPC_INT_V2S
			...
			if (itr & RPC_INT_V2S) {
				...
				//writel(RPC_INT_VS, (void *)REG_SB2_CPU_INT);
#ifdef CONFIG_DEBUG_KERNEL_RPC
				record_isr_time[RPC_VIDEO2] = jiffies;//current_kernel_time().tv_sec; //jiffies;
#endif
...
#ifdef CONFIG_DEBUG_KERNEL_RPC
					record_isr_user_wake_up_time[RPC_VIDEO2] = jiffies;
#endif
...
#ifdef CONFIG_DEBUG_KERNEL_RPC
					record_isr_wake_up_time[RPC_VIDEO2] = jiffies;//current_kernel_time().tv_sec; //jiffies;
#endif
...
#ifdef CONFIG_DEBUG_KERNEL_RPC
				record_isr_time[RPC_KCPU] = jiffies;//current_kernel_time().tv_sec; //jiffies;
#endif
...
#ifdef CONFIG_DEBUG_KERNEL_RPC
					record_isr_user_wake_up_time[RPC_KCPU] = jiffies;
#endif
...
#ifdef CONFIG_DEBUG_KERNEL_RPC
					record_isr_wake_up_time[RPC_KCPU] = jiffies;//current_kernel_time().tv_sec ;//jiffies;
#endif
...
//		rtd_pr_rpc_info("Not RPC interrupt...\n");
		return IRQ_NONE;
	}
...
//      PDEBUG("   ***ptr[%d] = 0x%8x\n", i, (int)ptr);
    ...
    if (request_irq(gic_irq_find_mapping(IRQ_CPU2CPU), rpc_isr, IRQF_SHARED, "rpc", (void *)RPC_ID) != 0)
	rtd_pr_rpc_err("Can't get assigned irq...\n");

    venus_rpc_devs = platform_device_register_simple("Venus_RPC", -1, NULL, 0);
// +++ cyhuang (2011/03/23)
    if ((result = platform_driver_register(&venus_rpc_driver)) != 0) {
// +++ cyhuang (2011/03/23)
        rtd_pr_rpc_err("Can't register RPC device driver...\n");
        goto fail;
    }
