...
u32 shuttle_keycode     = 0;
u32 shuttle_received    = 0;
#endif
...
//      IR_DBG( "[%08X] is detected.\n", symbol);
    ...
    if((symbol != lastSrValue) || (symbol == lastSrValue && get_take_time_ns(lastRecvTick, get_cur_system_counter()) > ir_repeat_interval * NS_PER_MS)) {
        IR_DBG( "[%08x] is detected. %lluns\n", symbol, get_take_time_ns(lastRecvTick, get_cur_system_counter()));
