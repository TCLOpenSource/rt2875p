                          ...
                          unsigned long flags)
{
    ...
    while (len) {
        ...
        if (ret <= 0) {
            ...
            /*work around. share pin io_write will cause tp can't recevie Falling edge signal. */
            if (g_fifo_func_use_hw_mode) {
                SET_PCMCIA_CTRL(GET_PCMCIA_CTRL() | PC_CE2_CARD2);
                SET_PCMCIA_CTRL(GET_PCMCIA_CTRL() & ~PC_CE2_CARD2);
            }
            /*==============================*/
#endif
...
#ifdef RTK_PCMCIA_CD_DEBOUNCE
        rtd_outl(PCMCIA_MIS_PCMCIA_CD_DEB1_reg, (rtd_inl(PCMCIA_MIS_PCMCIA_CD_DEB1_reg)) | (p_this->reg_bits.CD_DEB_INSERT_ISREN | p_this->reg_bits.CD_DEB_REMOVE_ISREN));
#else
...
int rtk_pcmcia_cd_debounce_enable(RTK_PCMCIA *p_this, unsigned char On)
{
    ...
    if (On) {
        ...
        rtd_outl(PCMCIA_MIS_PCMCIA_CD_DEB1_reg, ret);
    }
    else {
        ret &= (~p_this->reg_bits.CD_DEB_EN);
        rtd_outl(PCMCIA_MIS_PCMCIA_CD_DEB1_reg, ret);
...
unsigned int rtk_pcmcia_cd_manual_mode_set(RTK_PCMCIA *p_this, unsigned int value)
{
    ...
    ret = rtd_inl(PCMCIA_MIS_PCMCIA_CD_DEB1_reg);
    if (g_cd_seperate2pins) {
        ret |= (PCMCIA_MIS_PCMCIA_CD_DEB1_cd0_manual_mode(value) | PCMCIA_MIS_PCMCIA_CD_DEB1_cd1_manual_mode(value));
    }
    else {
        switch (p_this->id) {
            case 0:
                ret |= PCMCIA_MIS_PCMCIA_CD_DEB1_cd0_manual_mode(value);
                break;
            case 1:
                ret |= PCMCIA_MIS_PCMCIA_CD_DEB1_cd1_manual_mode(value);
                break;
        }
    }
    rtd_outl(PCMCIA_MIS_PCMCIA_CD_DEB1_reg, ret);
...
unsigned int rtk_pcmcia_cd_manual_value_set(RTK_PCMCIA *p_this, unsigned int value)
{
    ...
    ret = rtd_inl(PCMCIA_MIS_PCMCIA_CD_DEB1_reg);
    if (g_cd_seperate2pins) {
        ret |= (PCMCIA_MIS_PCMCIA_CD_DEB1_cd0_manual_value(value) | PCMCIA_MIS_PCMCIA_CD_DEB1_cd1_manual_value(value));
    }
    else {
        switch (p_this->id) {
            case 0:
                ret |= PCMCIA_MIS_PCMCIA_CD_DEB1_cd0_manual_value(value);
                break;
            case 1:
                ret |= PCMCIA_MIS_PCMCIA_CD_DEB1_cd1_manual_value(value);
                break;
        }
    }
    rtd_outl(PCMCIA_MIS_PCMCIA_CD_DEB1_reg, ret);
...
int __init rtk_pcmcia_module_init(void)
{
    ...
    if ( rtk_is_pcmcia_disable(PCMCIA_INTERAL) ) {
        PCMCIA_WARNING("pcmcia 0 otp disabled\n");
        return -ENODEV;
    }
