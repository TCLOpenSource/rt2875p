...
//#define RTK_PCMCIA_CD_DEBOUNCE
#endif

#if defined(CONFIG_RTK_KDRV_MIO)
#define RTK_PCMCIA_DIRECT_READ_MIO
#endif
...
typedef enum
{
    PCMCIA_OFF = 0,
    PCMCIA_ON =1,
    PCMCIA_SHAREPIN_SW_MODE=2,
    PCMCIA_SHAREPIN_HW_MODE=3,
    PCMCIA_UNDEFINED,
} RTK_PCMCIA_BOOT_STATUS;

typedef enum
{
    PCMCIA_INTERAL = 0, /*local pcmcia*/
    PCMCIA_MIO = 1, /*mio pcmcia*/
} RTK_PCMCIA_NUM;
...
#define RTK_PCMCIA_EVENT_IREQ_ASSERT           0x2
    ...
    struct
    {
        ...
        unsigned long CD_DEB_EN;
        unsigned long CD_DEB_INSERT_ISREN;
        unsigned long CD_DEB_REMOVE_ISREN;
        unsigned long CD_DEB_OUT;
        unsigned long CD_DEB_INSERT_FLAG;
        unsigned long CD_DEB_REMOVE_FLAG;
#endif
    } reg_bits;
...
/**********************PCMCIA CFG3 format************************
*  D63:6   : reserved
*  D22     : pcmcia0_cd_gpio_index_enable      (0: disable PCMCIA_GPIO_CD Index, 1:enable PCMCIA_GPIO_CD Index)
*  D21     : pcmcia0_cd_gpio_index_polarity     (0: negative, 1: positive)
*  D20     : pcmcia0_cd_gpio_index_type         (0: MISC, 1: ISO)
*  D12-19  : pcmcia0_cd_gpio_index                 (GPIO detect for PCMCIA0)
*  D9-11   : reserved
*  D8      : pcmcia use 2CD pins for one socket (0: disable, 1: enable)
*  D7-6    : reserved   
...
#define PCMCIA_GPI_PIN_CD1_ENABLE           (1<<22)
#define PCMCIA_GPI_CD1_ENABLE(x)            ((x&PCMCIA_GPI_PIN_CD1_ENABLE)?1:0)

#define PCMCIA_GPI_PIN_CD1_TYPE             (1<<20)
#define PCMCIA_GPI_CD1_TYPE(x)              ((x&PCMCIA_GPI_PIN_CD1_TYPE)?1:0)

#define PCMCIA_GPI_PIN_CD1_INDEX(x)	        ((x>>12)&0xFF)					
...
#ifdef PCMCIA_DBG_EN
#define PCMCIA_DBG(fmt, args...)            rtd_pr_pcmcia_debug("[DBG] " fmt, ## args)
#else

#define PCMCIA_DBG(fmt, args...)
#endif

#define PCMCIA_INFO(fmt, args...)       rtd_pr_pcmcia_info("[Info] " fmt, ## args)
#define PCMCIA_WARNING(fmt, args...)    rtd_pr_pcmcia_warn("[Warn] " fmt, ## args)
#define PCMCIA_ERROR(fmt, args...)      rtd_pr_pcmcia_emerg("[ERROR] " fmt, ## args)
