...
#define HDCP2_CMD_SELECT_STOREDKM               0x26
#define HDCP2_CMD_WRITE_STOREDKM                0x27

#define HDCP2_CMD_TV002_LOAD_KEY                0x26
#define HDCP2_CMD_DEMOBOARD_LOAD_KEY            0x27
#define HDCP2_CMD_AP_BURN_KEY                   0x28
#define HDCP2_CMD_GET_RECEIVER_ID               0x29
...
#define HDCP2_CMD_REPEATER_SET_ENABLE           0x60
#define HDCP2_CMD_REPEATER_SET_RXINFO_RIDLIST   0x61

typedef enum {
    HDMI_HDCP2_MODE,
    DP_OPTEE_HDCP2_MODE,
} HDMI_DP_HDCP2_MODE_T;

extern int optee_hdcp2_init(void);
extern void optee_hdcp2_deinit(void);
extern spinlock_t *get_hdcp2_optee_spinlock(void);
extern void optee_hdcp2_main(HDMI_DP_HDCP2_MODE_T hdmi_dp, unsigned char id, unsigned char nport, unsigned char *send_buf, unsigned int send_size, unsigned char *rev_buf, unsigned int rev_size);
...
extern void optee_hdcp2_widi_Select_StoredKm(unsigned char id, int index);
extern void optee_hdcp2_widi_Write_StoredKm(unsigned char id, int index);
extern bool optee_hdcp14_compute_km(unsigned char encrypted_bkeys[320], unsigned char aksv[5], unsigned char km[7]);
extern void optee_hdcp2_repeater_set_enable(unsigned char id, unsigned char nport, unsigned char enable);
extern void optee_hdcp2_repeater_set_rxinfo_ridlist(unsigned char id, unsigned char nport, unsigned short rxinfo, unsigned char *ridlist, unsigned int size);
extern void optee_hdcp2_get_receiver_id(unsigned char id, unsigned char *data, unsigned int len);
#endif
