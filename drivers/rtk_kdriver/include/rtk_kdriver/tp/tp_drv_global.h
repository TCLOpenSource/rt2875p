...
static DEFINE_MUTEX(tp_update_buf_rp_mutex);
...
extern struct mutex tp_dump_mutex;
...
#define _tp_dump_lock()        mutex_lock(&tp_dump_mutex)
#define _tp_dump_unlock()      mutex_unlock(&tp_dump_mutex)
...
#define _tp_update_buf_rp_lock() \
        mutex_lock(&tp_update_buf_rp_mutex); \
        //Tp_Locker_SetLockFunction("_tp_buf_lock", __FUNCTION__)

#define _tp_update_buf_rp_unlock() \
        mutex_unlock(&tp_update_buf_rp_mutex); \
        //Tp_Locker_ClearLockFunction("_tp_buf_lock", __FUNCTION__)
...
typedef enum
{
	TP_DELIVERY_MODE_ATSC30_BBF = 0,
	TP_DELIVERY_MODE_ATSC30_ALP	= 1,
	TP_DELIVERY_MODE_JAPAN4K 	= 2,
	TP_DELIVERY_MODE_TS     	= 3,  //delivery TS from TP engine
	TP_DELIVERY_MODE_CI20       = 4,  //delivery TS from TPP engine for USBCAM
	TP_DELIVERY_MODE_MAX
}TP_DELIVERY_MODE_T;
...
typedef struct {
        ...
        INT32 tpi_rate_cnt[MAX_TPI_COUNT];
        struct task_struct * tpi_rate_cnt_t;
        UINT8 tpi_rate_cnt_t_start;
        TPK_TPI_LOG_T tpi_log_print[MAX_TP_P_COUNT];

        /* tp status */
        TPK_TP_STATUS_T tp[MAX_TP_COUNT];

        /*tp atsc status */
        TPK_TP_ATSC_STATUS_T tp_atsc[MAX_TP_COUNT];

        /*tp japan status */
        TPK_TP_JAPAN_STATUS_T tp_japan[MAX_TP_COUNT];
        ...
        UADDRESS tp_input_monitor_virt_addr;
        UADDRESS tp_input_monitor_phys_addr;
#endif
        UINT8 deliveryMode[MAX_TP_COUNT];
        REFERENCE_CLOCK_CALLBACK set_get_referenc_clock;
        UINT8 useECP[MAX_TP_COUNT];
} TP_DRV_T;
...
extern UINT32 check_tpp_buffer_interval_ms;
...
extern UINT32 tp_tpoclk_dbg_div;
...
#define TP_PROC_PARAM_CHANNEL_SCAN          "tp_channel_scan="          /*  dump PAT/PMT data if deliver to demux */
#define TP_PROC_PARAM_TPO_CLK_RADOM_GEN     "tp_out_clk_random_gen"     /*  random output tpout clk */
#define TP_PROC_PARAM_RECORD_OIF_EN         "record_oif_en="            /*  start & stop record oif from TP ring buffer (0: stop record; 1: start record) */
#define TP_PROC_PARAM_RECORD_BBP_EN         "record_bbp_en="            /*  start & stop record bbp from TP ring buffer (0: stop record; 1: start record) */
#define TP_PROC_PARAM_RECORD_TLV_EN         "record_tlv_en="            /*  start & stop record tlv from TP or TLV(0: stop record; 1: start record) */
#define TP_PROC_PARAM_RECORD_FILTER_TLV_EN  "record_filtertlv_en="      /*  start & stop record tlv after filtered (0: stop record; 1: start record) */

#define DUMP_DBG_MSG_BUFFER_LENGTH 65536
...
typedef enum {
        ...
        TP_PROC_CHANNEL_SCAN,            /* dump section PAT/PMT data if deliver to demux */
        TP_PROC_TPO_CLK_RANDOM_GEN,      /* random output tpout clk */
        TP_PROC_RECORD_OIF_EN,           /* start & stop record OIF from TP ring buffer (0: stop record; 1: start record) */
        TP_PROC_RECORD_BBP_EN,           /* start & stop record BBP from TP ring buffer (0: stop record; 1: start record) */
        TP_PROC_RECORD_TLV_EN,           /* start & stop record TLV from TP ring buffer (0: stop record; 1: start record) */
        TP_PROC_RECORD_FILTER_TLV_EN,
