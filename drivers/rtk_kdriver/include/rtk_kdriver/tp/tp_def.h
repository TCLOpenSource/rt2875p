...
#include "atsc3_dmx/atsc3_dmx.h"
#include "atsc3_dmx/ring.h"
...
#define CHIP_VERSION                              RTD287X_TP
...
#define TP_INFO_PACK_BUFFER_SIZE                  (4096 * 8)
...
#define TP1_SEC_FILTER_COUNT                      34
#define TP2_SEC_FILTER_COUNT                      32  /* for CI+1.4, receive Trust SDT from TP3 */
#define TP3_SEC_FILTER_COUNT                      0
#endif

#define TP_TLV_FILTER_NUM           32
...
#define TAIL_STATUS_CRC_STATUS_TOO_SHORT_FOR_CRC_CHECK          _BIT3
#define TAIL_STATUS_CRC_STATUS_CRC_ERR                          _BIT2
...
#define TP_P_BUFFER_SIZE                          (TP_P_BUFFER_ALIGNMENT * 800) /* LCM(188,192,8) x 200 */
...
#define OIF_RINGBUFFER_SIZE (4*1024*1024)

#define BBP_ARRAY_COUNT                           512
#define BBP_ARRAY_SIZE                            8192
#define BBFRAME_GET_SIZE                          1000*188

#define TLV_RINGBUFFER_SIZE (12 * 1024 * 1024)

#define START_CODE_SEARCH_INFO_PACK_LENGTH        16
#define MAX_ADAPTION_FIELD_LENGTH                 184

#define ATSC30_PLP_MAX                            64
#define ATSC30_STC_MONITOR_CNT                    10

#define TLV_DESCRIPTOR_CNT  128
#define TLV_DESCRIPTOR_BUFFER_SIZE 0x1F000
#define TLV_SYNC_BYTE       0x7f
#define TLV_SYNC_EN         0x00001000
#define TLV_STUFF_DATA_SIZE 8
#define TLV_PAKET_MMTP_PAYLAD_MPU 0x0
#define TLV_PAKET_MMTP_PAYLAD_SI  0x2

#define ECP_CTRL_BIT 0x00001000
#define ECP_CTRL_REG 0xB8014F14
...
// Merlin4 support CI
#define TP_SUPPORT_CI
// Merlin4 support TP3
#define TP_SUPPORT_TP3

// TPI support
// Merlin4 support TPI1 and TPI2
#define TP_SUPPORT_TPI1
#define TP_SUPPORT_TPI2

//#define TP_TIMESTAMP_USE_27M
/*****************************************
...
typedef enum {
        ...
        TPK_PORT_IP2,
        TPK_PORT_IP3,
        TPK_PORT_CI_BYPASSED_TP_INPUT,
        TPK_PORT_CI_DEMUX_OUT_CI_CH0,
        TPK_PORT_CI_DEMUX_OUT_CI_CH1,
        TPK_PORT_CI_DEMUX_OUT_CI_CH2,
        TPK_PORT_CI_DEMUX_OUT_CI_CH3,
        TPK_PORT_NUM,
...
typedef enum {
        TPK_WITHOUT_CAS = 0,
        TPK_WITH_CAS,
        TPK_WITH_CIP_CAS        /* CI+1.4 */
} TPK_CAS_TYPE_T;

/* delivery source Type     */
typedef enum {
        TPK_DELI_SRC_TYPE_NONE,
        TPK_DELI_SRC_TYPE_TLV_HW0 = 1,  /* HW TLV 0*/
        TPK_DELI_SRC_TYPE_TLV_HW1,      /* HW TLV 1*/
        TPK_DELI_SRC_TYPE_TLV_SW0,      /* raw mode TLV 0*/
        TPK_DELI_SRC_TYPE_ATSC30_HW0,   /* HW atsc3.0 0*/
        TPK_DELI_SRC_TYPE_ATSC30_SW0,   /* raw mode atsc3.0 0*/
        TPK_DELI_SRC_TYPE_TS,           /* raw mode TS */
        TPK_DELI_SRC_TYPE_MAX           /* max */
} TPK_DELIVERY_SRC_TYPE_T;

/* delivery tlv filter type valid enum    */
typedef enum {
        TPK_DELI_TLV_FILTER_ALL_INVALID      = 0x00,
        TPK_DELI_TLV_FILTER_PKT_ID_VALID     = 0x20,
        TPK_DELI_TLV_FILTER_PYLD_TYPE_VALID  = 0x10,
        TPK_DELI_TLV_FILTER_IPSRC_PORT_VALID = 0x08,
        TPK_DELI_TLV_FILTER_IPSRC_ADDR_VALID = 0x04,
        TPK_DELI_TLV_FILTER_IPDST_PORT_VALID = 0x02,
        TPK_DELI_TLV_FILTER_IPDST_ADDR_VALID = 0x01,
        TPK_DELI_TLV_FILTER_MACX
} TPK_DELIVERY_TLV_FILTER_TYPE_T;
...
typedef enum {
        TP_FRC_EX_KEPP = 0,
        TP_FRC_EX_CNT_90K,
        TP_FRC_EX_CNT_27M = TP_FRC_EX_CNT_90K,
        TP_FRC_EX_META_DATA,
        TP_FRC_EX_NO_PREFIX
} TPK_FRC_PREFIX_EX_T;

typedef enum {
        TPI_LOG_NONE = 0,               /* nothing to print */
        TPI_LOG_RF,                     /* RF-channel and have PID filter */
        TPI_LOG_CAM,                    /* RF-channel and plug-in CAM */
        TPI_LOG_RF_CLK_CHANGE           /* RF-channel and demod clock changed */
} TPK_TPI_LOG_T;
...
typedef enum {
        TLV_NTP_SET_PORT = 0,
        TLV_NTP_SET_IP,
        TLV_NTP_SET_PORT_AND_IP
} TLV_NTP_SET_T;

typedef enum {
        MEMORY_PRISON_VIDEO_ES = 0,
        MEMORY_PRISON_AUDIO_ES,
        MEMORY_PRISON_TP,
} TP_TA_MEMORY_PRISON_TYPE_T;
/***** struct type *****/
typedef struct {
        UINT8 inited;
        UADDRESS BasePhy;
...
typedef struct {
        ...
        UINT8 casType: 2;                  /* 0: without cas, 1: with cas, 2: with ci+1.4 cas */
        ...
        UINT8 sync_byte_cip14;                /* default please set to 0x47 */
} TPK_TP_TS_PARAM_T;
...
typedef struct {
        ...
        UINT8 out_2bit: 1;          /* for mark2/mio */
#endif
...
typedef struct {
        UINT16  pid;            /* PID of section */
        UINT32  p_data;         /* start address of section */
        INT32   buf_idx;
        UINT16  len;            /* length of section in byte */
        UINT8   crc_err;        /* crc status */

} TPK_SEC_DATA_COMPAT_T;
...
typedef struct {
        ...
        UINT16 pcr_irq_cnt;
        UINT8 adaption_field[MAX_ADAPTION_FIELD_LENGTH];
...
typedef struct {
        struct dmx_callback cb;
        struct atsc3_dmx* dmx;
        struct atsc3_ring* dmx_in;
        struct atsc3_ring* p_in;

        DELIVERY_BB_FRAME_T *bbp_array;
        unsigned int bbp_array_size;
        UADDRESS bbp_array_phy;
        unsigned char *bbp_array_cached;

        UINT32 bbp_array_write_index;
        UINT32 bbp_array_read_index;
        UINT32 bbp_array_read_index_count;
        UINT8 last_bbp_cc[ATSC30_PLP_MAX];
        UINT32 plp_cnt[ATSC30_PLP_MAX];
        UINT32 plp_size[ATSC30_PLP_MAX];
        UINT32 error_cnt;

        UINT8 isATSC30;

        dma_addr_t     dring_phyAddr;
        UINT8 *dring_virAddr;
        UINT8 *dring_cachedaddr;
        UINT8 *dring_nonCachedaddr;
        unsigned int   dring_size;

        dma_addr_t     iring_phyAddr;
        UINT8 *iring_virAddr;
        UINT8 *iring_cachedaddr;
        UINT8 *iring_nonCachedaddr;
        unsigned int   iring_size;
        unsigned int   open_count;
        UINT8  pTimeInfo[9];
        UINT8  clock_recovery;
        UINT8  wall_clock;
        UINT64 wall_clock_value;
        ...
        UINT8  leap59;
        UINT8  leap61;
        UINT64 wallclock_stc_offset;

        // STC
        UINT64 stc_monitor[ATSC30_STC_MONITOR_CNT];
        UINT8  stc_monitor_index;

        TPK_INPUT_PORT_T input_port;

        // Debug
        UINT32 atsc3_dmx_callback_cnt;
        UINT64 for_debug_stc_value;
        UINT64 for_debug_stc_local_value;
        UINT32 for_debug_first_bbf_in_time;
        UINT32 for_debug_first_valid_bbf_in_time;
        UINT32 for_debug_first_get_bbf_time;
        UINT32 for_debug_first_release_bbf_time;
        UINT32 for_debug_atsc_request_bbf_time;
        UINT32 for_debug_atsc_cancel_bbf_time;
        UINT32 for_debug_bbf_total_size;
        UINT32 for_debug_ring_buffer_full_cnt;
} TPK_TP_ATSC_STATUS_T;

typedef enum {
        TP_ATSC_STREAM_STOP = 0,          /* Stop streaming */
        TP_ATSC_STREAM_START ,            /* Start streming */
} TPK_TP_ATSC_STREAM_CTRL_T;

typedef struct {
        struct japan4k_dmx_callback cb;
        struct japan4k_dmx* dmx;

        UINT8 isJAPAN4K;
        TPK_TP_BUF_T mass_buffer;
        TPK_TP_BUF_T tlv_descripor_buffer[TLV_DESCRIPTOR_CNT];

        unsigned int   open_count;
        UINT8  clock_recovery;
        UINT8  wall_clock;

        UINT8   filter_table_idx[TP_TLV_FILTER_NUM];
        ...
        // Debug
        UINT32 japan4k_dmx_callback_cnt;
        UINT32 for_debug_tlv_total_size;
        UINT32 for_debug_ring_buffer_full_cnt;
        UINT32 tlv_size;
        UINT32 timestamp_range1;
        UINT32 timestamp_range2;
        UINT32 timestamp_max;
        UINT32 for_debug_first_tlv_in_time;
} TPK_TP_JAPAN_STATUS_T;
...
typedef struct {
        ...
        UINT64 pkt_per_second;  /* packet cnt per second for checking CAM input bit rate */
        ...
        struct __kernel_old_timeval saved_time;
        UINT64 stc_from_isr;
        UINT64 pcr_from_isr;
        ...
        UINT32 tp_mc_start_address;
        UINT32 tp_mc_end_address;
        UINT32 vbm_mc_start_address;
        UINT32 vbm_mc_end_address;
        UINT32 video_es_mc_start_address;
        UINT32 video_es_mc_end_address;
        UINT32 audio_es_mc_start_address;
        UINT32 audio_es_mc_end_address;
} TPK_TP_STATUS_T;
...
typedef struct
{
    UINT8  en_nag_pid_update_bit:1;
    UINT8  msc_sel:1;
    UINT8  data_src;
}TPK_NAGXX_Conrol_Data;
...
typedef struct {
        ...
        UINT8 data_swap;
        UINT8 data_order;
} TPK_MIO_CTRL_PARAM_T;

/* MIO T2P parameter */
typedef struct {
        TPK_MIO_CTRL_PARAM_T T2P_ctrl_param;

        UINT8 out_clk_high;
        UINT8 out_clk_low;

        UINT8 out_val_pol: 1;       /*Polarity of output TS*/
        UINT8 out_sync_pol: 1;      /*Polarity of output TS*/
        UINT8 out_clk_pol: 1 ;      /*Polarity of output TS*/

        UINT8 in_val_pol: 1 ;      /*Polarity of output TS*/
        UINT8 in_sync_pol: 1;      /*Polarity of output TS*/
        UINT8 in_clk_pol: 1 ;      /*Polarity of output TS*/
} TPK_MIO_T2P_PARAM_T;

/* MIO P2T parameter */
typedef struct {
        TPK_MIO_CTRL_PARAM_T P2T_ctrl_param;

        UINT8 out_sync_duration: 1;
