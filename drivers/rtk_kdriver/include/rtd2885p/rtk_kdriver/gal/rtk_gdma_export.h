...
#ifdef CONFIG_RTK_KDRV_GDMA_RLINK
#define USE_RLINK_INTERFACE 1
#else
#define USE_RLINK_INTERFACE 0
#endif

#define ENABLE_VSYNC_NOTIFY (1)
...
typedef enum {
	...
	GDMA_DBG_CMD_SHOW_INT_STATUS = 13,
	GDMA_DBG_CMD_OSD_CLONE_EN = 14,
	GDMA_DBG_CMD_OSD_BW_GEN_EN = 15,
	GDMA_DBG_CMD_OSD_SYNC_EN = 16,
	GDMA_DBG_CMD_STOP_UPDATE= 17,
	GDMA_DBG_CMD_SPLIT_MODE = 18,
	GDMA_DBG_CMD_OSD_RLINK = 19,
	GDMA_DBG_CMD_ES_OSD_ADDR = 20,
	GDMA_DBG_CMD_OSD_GAL_AI_BLOCK = 21,	
	GDMA_DBG_CMD_OSD_GET_GOLBAL_ALPHA = 22,
	GDMA_DBG_CMD_OSD_SET_OSD_GAIN = 23,
	GDMA_DBG_CMD_OSD_CAP_FB = 24,
	GDMA_DBG_CMD_OSD_GAL_AI_BLOCK_AFBC = 25,	
	GDMA_DBG_CMD_OSD_SET_OSD_ALPHA_DETECT = 26,
	GDMA_DBG_CMD_OSD_GET_OSD_ALPHA_RATIO = 27,
	GDMA_DBG_CMD_OSD_SET_LOGLEVEL = 28,
	GDMA_DBG_CMD_OSD_MONITOR_CTL = 29,
	GDMA_DBG_CMD_OSD_RUNTIME_CTL = 30,
	GDMA_DBG_CMD_OSD_HW_ORBIT = 31,
	GDMA_DBG_CMD_OSD_YUV422 = 32,
	GDMA_DBG_CMD_NUMBER
...
typedef struct {
	...
	int h_shift_pixel;		/* position value: shift right, negative: shfit left */
	int v_shift_pixel;		/* position value: shift down, negative: shfit up */
	int ini_w;				/* original width */
	int ini_h;				/* original height */
	int ini_dst_w;			/* original dst width*/
	int ini_dst_h;			/* original dst height*/
	unsigned int ini_addr;	/* original address */
	int crop_left;
	int crop_up;
	unsigned int apply_enable;
	unsigned int apply_afbc_x_sta;
	unsigned int apply_afbc_y_sta;
	int apply_dst_w;		/* apply dst width*/
	int apply_dst_h;		/* apply dst height*/
	unsigned int apply_hor_factor;
	unsigned int apply_ver_factor;
	unsigned int apply_hor_ini;
	unsigned int apply_ver_ini;
} CONFIG_OSDSHIFT_UPSCALE_STRUCT;
...
typedef struct {
	...
	int v_move_range;
	int apply;
} CONFIG_OSDSHIFT_UPSCALE_PARA;
	
typedef struct {
	unsigned int levelval;
	unsigned int on_off;
	unsigned int size;
} CONFIG_OSD_GAIN_INFO;

typedef struct {
    uint64_t syncstamp;                        /*  syncstamp number */
...
typedef struct {
	...
	unsigned int h_move_range;
	unsigned int v_move_range;
...
typedef struct {
	u_char	frame_interval;
	u_short	sta_x_th;
	u_short	sta_y_th;
	u_short	end_x_th;
	u_short	end_y_th;
} GDMA_OSD_DETECTION_PARAMETER_T;
...
int GDMA_GetFenceInfo           (GDMA_FENCE_INFO *);

int GDMA_SignalGetReleaseFenceInfo           (GDMA_FENCE_INFO *);
int GDMA_SignalGetSignalFenceInfo           (GDMA_FENCE_INFO *);


int GDMA_Display                (GDMA_DISPLAY *);
int GDMA_OSD1OnTop              (unsigned char);
int GDMA_Config3D               (unsigned char);
int GDMA_ConfigVFlip            (unsigned char);
...
int GDMA_ConfigInputSrc_4k2k (unsigned char);
int GDMA_Config3dUi (unsigned char);
int GDMA_ConfigBoxSrcSize (int size);
int GDMA_ConfigBoxDstSize (int size);
int GDMA_ConfigOSDEnable (unsigned char);
...
int GDMA_ConfigNonBlocking (unsigned char);
