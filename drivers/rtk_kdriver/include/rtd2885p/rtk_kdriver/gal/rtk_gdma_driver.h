...
#define GDMA_SUPPORT_MONITOR_CTL 1

#define GDMA_IRQ IRQ_DDOMAIN
#define USR_WORKQUEUE_UPDATE_BUFFER
#if defined(CONFIG_ARCH_RTK288O) || defined(CONFIG_ARCH_RTK2875) || defined(CONFIG_ARCH_RTK2885M) || defined(CONFIG_ARCH_RTK6748) || defined(CONFIG_ARCH_RTK2885P)
#define THRESHOLD_DETECT 1
#endif
...
#else
//#define DBG_PRINT(s, args...) printk(KERN_WARNING"" s, ## args)
...
#define GDMA_PRINT(level,fmt, arg...) do { \
	if( unlikely(gDebugGDMA_loglevel >= level) ) { pr_crit("gdma "  fmt, ##arg ); } \
   } while(0) 
...
#define VSYNC_PRINT(level,fmt, arg...) do { \
		if( unlikely(gDebugFence_loglevel >= level) ) { pr_crit("vsync TT:%lld "  fmt, ktime_to_us(ktime_get()), ##arg ); } \
	   } while(0) 
...
#define OSD_DETECTION_SUPPORT 1
#define OSD_DETECTION_ALPHA_SUPPORT 1

#define ESTICKER_778 0
...
typedef struct _TFBC_FORMAT {
    unsigned int reserved1:16;
    unsigned int tfbc_argb_order:5;
    unsigned int tfbc_req_lossy:2;
    unsigned int reserved0:1;
    unsigned int tfbc_req_format:7;
    unsigned int reserved:1;
} TFBC_FORMAT;

typedef struct _AFBC_FORMAT {
    unsigned int afbc_argb_order:5;
    unsigned int reserved1:11;
    unsigned int afbc_yuv_trans:1;
    unsigned int reserved0:7;
    unsigned int afbc_fmt_1st:1;
    unsigned int reserved:7;
} AFBC_FORMAT;

typedef struct _AFBC_TOTAL_TILE {
    unsigned int x_total_tile:16;
    unsigned int y_total_tile:16;
} AFBC_TOTAL_TILE;

typedef struct _AFBC_PIC_PIXEL_XY {
    unsigned int x_pic_pixel:16;
    unsigned int y_pic_pixel:16;
} AFBC_PIC_PIXEL_XY;

typedef struct _AFBC_START_XY {
...
#if defined(CONFIG_ARCH_RTK287O) || defined(CONFIG_ARCH_RTK288O) || defined(CONFIG_ARCH_RTK2875) || defined(CONFIG_ARCH_RTK2885M) || defined(CONFIG_ARCH_RTK6748) || defined(CONFIG_ARCH_RTK2885P)
...
typedef struct {
	...
	gdma_osd1_ctrl_RBUS osd_ctrl_reg;
	gdma_osd1_RBUS osd_reg;
	gdma_osd1_wi_RBUS osd_wi_reg;
	gdma_osd1_wi_3d_RBUS osd_wi_3d_reg;
	gdma_osd1_size_RBUS osd_size_reg;
	gdma_osd1_clear1_RBUS osd_clear1_reg;
	gdma_osd1_clear2_RBUS osd_clear2_reg;
	...
	osd_sr_osd_sr_1_filter_ctrl0_RBUS osd_sr_filter_ctrl_reg;
...
typedef struct {
	...
	struct semaphore sem_get_fence;      /* mutual exclusion semaphore     */
    ...
    /* for registers */
    gdma_osd1_RBUS GDMA_osd;
    gdma_osd1_ctrl_RBUS GDMA_osd_ctrl;
    ...
    void (*GDMA_CallBack) (void *handler, unsigned char isRightEye);
    void (*GDMA_CookieCallBack) (void *handler, int cookie);
#if IS_ENABLED(CONFIG_DRM_REALTEK)
	void (*GDMA_IrqCallBack)(void * data);
