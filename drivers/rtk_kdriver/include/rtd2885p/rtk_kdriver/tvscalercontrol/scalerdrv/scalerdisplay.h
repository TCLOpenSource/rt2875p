...
#define _DISP_DEFAULT_WID (Get_DEFAULT_DISP_ACT_END_HPOS() - Get_DEFAULT_DISP_ACT_STA_HPOS())
#define _DISP_DEFAULT_LEN  (Get_DEFAULT_DISP_ACT_END_VPOS()  - Get_DEFAULT_DISP_ACT_STA_VPOS())
/*===================================  Types ================================*/
...
typedef enum _SLR_FORCE_BG_TYPE {
	...
	SLR_FORCE_BG_TYPE_SIG_ARC_READY,//for ARC ready mask
	SLR_FORCE_BG_TYPE_ZERO_NULL_ARC_MUTE,//for ARC null or zero need to mute
	SLR_FORCE_BG_TYPE_ROTATE,
	SLR_FORCE_BG_TYPE_DTV_CHANNEL_CHANGE_HDR_SEAMLESS,//for Japan DTV switch  seamless case
	SLR_FORCE_BG_TYPE_HDMI_AVMUTE_MASK,//for HDMI AV packet mask
	SLR_FORCE_BG_TYPE_INNER_PATTERN,//for calman inner pattern requirement
	SLR_FORCE_BG_TYPE_HIDDEN,//for miracast sub hidden
	SLR_FORCE_BG_TYPE_ORBIT_READY,//for orbit ready
	SLR_FORCE_BG_TYPE_LB_MODE_GATTING_NOT_READY, //14 gatting not ready for line buffer modes
	SLR_FORCE_BG_TYPE_DOLBY,
    SLR_FORCE_BG_TYPE_DLG,
...
void set_input_for_crop(unsigned char display, StructSrcRect input);/*For set input region*/
...
void set_display_forcebg_mask_by_isr(unsigned char display, unsigned char masktype, unsigned char maskvalue);//for isr use
...
void drvif_scalerdisplay_set_panel_bg_color(unsigned int ColorR, unsigned int ColorG, unsigned int ColorB);//set panel forcebg color
