...
typedef struct {
	...
	unsigned char horicomth;
	unsigned char teethDiff_en;
...
typedef struct {
	unsigned char correct_en;
	unsigned char energy_bias;
	unsigned char pre_diff_th;
	unsigned char nxt_diff_th;
	unsigned char weight1;
	unsigned char weight2;
	unsigned char weight3;
	unsigned char weight4;
	unsigned char energy_th1;
	unsigned char energy_th2;
	unsigned char energy_th3;
} DRV_di_ma_hmc_mv_correct;

typedef struct {
	unsigned char energy_check_en;
	unsigned char check_pre_th;
	unsigned char check_nxt_th;
	unsigned char check_pre_gain;
	unsigned char check_nxt_gain;
} DRV_di_ma_hmc_mv_energy_check;

typedef struct {
	unsigned char allTeethCheck_en;
	unsigned char all_teeth_th;
	unsigned char notTeethRecover_en;
	unsigned char notTeeth_Hline_th;
	unsigned char notTeeth_Dline_th;
} DRV_di_ma_hmc_mv_teeth_check;

typedef struct {
	unsigned char hmc_vlpf_en;
	unsigned char hmc_vlpf_hori_protect_en;
	unsigned char hmc_vlpf_vhf_en;
	unsigned char hmc_teeth_follow_ma;
	unsigned char choose_vlpf;
	unsigned char verti_min_th;
	unsigned char hori_min_th;
	unsigned char hori_max_th;
	unsigned char vhf_center_th;
	unsigned char vhf_margin_th;
	// following not in hmc table
	unsigned char vdiff_th;
	unsigned short line_start1;
	unsigned short line_end1;
	unsigned short line_cnt1; // read only
	unsigned short teeth_cnt1; // read only
	unsigned short line_start2;
	unsigned short line_end2;
	unsigned short line_cnt2; // read only
	unsigned short teeth_cnt2; // read only
} DRV_di_ma_hmc_vlpf;

typedef struct {
	unsigned char cmp_tmv_en;
	unsigned char cmp_tmv_th;
	unsigned char cmp_tmvY_th_h;
	unsigned char cmp_tmvY_th_m;
	unsigned char cmp_tmvY_th_s;
} DRV_di_ma_hmc_cmp_tmv;

typedef struct {
	unsigned char bMA_en;
	unsigned char bMA_gain_en;
	unsigned char bMA_gain_factor;
	unsigned char bMA_gain;
	unsigned char bMA_line_t_en;
	unsigned char bMA_line_t_cnt;
	unsigned char bMA_line_t_factor;

} DRV_di_ma_hmc_blending_with_MA;
...
typedef struct {
	...
	DRV_di_ma_hmc_blending_with_MA di_ma_hmc_blending_with_MA;		/* 7*/
...
typedef struct {
	...
	DRV_di_ma_hmc_dynamic_control di_ma_hmc_dynamic_control;		/* 7*/
	DRV_di_ma_hmc_mv_correct di_ma_hmc_mv_correct;					/* 11*/
	DRV_di_ma_hmc_mv_energy_check di_ma_hmc_mv_energy_check;		/* 5*/
	DRV_di_ma_hmc_mv_teeth_check di_ma_hmc_mv_teeth_check;			/* 5*/
	DRV_di_ma_hmc_vlpf di_ma_hmc_vlpf;								/* 10*/
	DRV_di_ma_hmc_cmp_tmv di_ma_hmc_cmp_tmv;						/* 5*/
} DRV_di_ma_hmc_table;
...
typedef struct
{
	...
	unsigned char smd_gst_compensate_avg_en;				//di_di_smd_resurrect_ctrl_RBUS

	// chen 180308
	unsigned char smd_mv0_no_intra_en;				//di_di_smd_ma_mc_weight_ctr_RBUS
	unsigned char smd_still_ma_en;
	// end chen 180308
...
typedef struct
{
	...
	unsigned char smd_mv_teeth_detect_en;						//di_di_smd_resurrect_ctrl_RBUS
	unsigned char smd_notgmv_res_en;								//di_di_smd_resurrect_ctrl_RBUS
	unsigned char smd_teeth_res_en;									//di_di_smd_resurrect_ctrl_RBUS
	unsigned char smd_teeth_detect_method;					//di_di_smd_resurrect_ctrl_RBUS
...
// chen 180308
typedef struct {
	unsigned char smd_tmvx_diff_th1;		//di_di_smd_tmv_similarity_RBUS
	unsigned char smd_tmvy_diff_th1;
	unsigned char smd_tmvx_diff_th2;
	unsigned char smd_tmvy_diff_th2;
	unsigned char smd_tmvx_diff_th3;
	unsigned char smd_tmvy_diff_th3;
	unsigned char smd_tmv_acc1;
	unsigned char smd_tmv_acc2;
	unsigned char smd_tmv_acc3;
	unsigned char smd_dt_count_th1;
	unsigned char smd_dt_count_th2;
	unsigned char gst_static_tmvdiff_th;
	unsigned char smd_db_tmv_shift;			//di_di_smd_ma_mc_weight_ctr_RBUS
	unsigned char smd_db_tmv_out;
} DRV_Smd_tmv_relative_setting;

typedef struct {
	unsigned char smd_teeth_vlpf_en;			//di_di_smd_teeth_processing_RBUS
  unsigned char smd_acc_teeth_count_en;
	unsigned char smd_c_teeth_disable;		//di_di_smd_tmv_similarity_RBUS
  unsigned char smd_mv_pixel_acc_th5;		//di_di_smd_teeth_processing_RBUS
  unsigned char smd_mad_th_tf;
  unsigned char smd_teeth_th_tf;
  unsigned char smd_teeth_pos;
  unsigned char smd_hori_diff_th;
  unsigned char smd_hori_diff_gain;
  unsigned char smd_spatialmv_method; //di_di_smd_ma_mc_weight_ctr_RBUS
} DRV_Smd_teeth_by_vlpf;

typedef struct {
	     unsigned char smd_gmvctr_fw;			//di_di_smd_gmv_fw_counter1_RBUS
       unsigned char smd_gmv1maxctr_fw;
       unsigned char smd_gmv2maxctr_fw; //di_di_smd_gmv_fw_counter2_RBUS
       unsigned char smd_gmv3maxctr_fw;
       unsigned char smd_gmv1_x_fw;			//di_di_smd_gmv1_value_fw_RBUS
       unsigned char smd_gmv1_y_fw;
       unsigned char smd_gmv2_x_fw;			//di_di_smd_gmv2_value_fw_RBUS
       unsigned char smd_gmv2_y_fw;
} DRV_Smd_FW_GMV_info;


typedef struct {
	     unsigned char smd_spatialmv_sad_th;			//di_di_smd_choosecandidate_RBUS
	     unsigned char smd_reliablerange_sad_th;
	     unsigned char smd_resurrect_localcurr_num;	//di_di_smd_refinemv2_RBUS
	     unsigned char smd_leftweave_gap_th;				//di_di_smd_final_adjustment1_RBUS
	     unsigned char smd_rightweave_gap_th;
	     unsigned char smd_line_continous_en;				//di_di_smd_final_adjustment2_RBUS
       unsigned char smd_compensate_teeth_th;			//di_di_smd_compensate_RBUS
	     unsigned char smd_compensate_avg;
} DRV_Smd_control_o;
// end chen 180308
...
// chen 180308
	DRV_Smd_tmv_relative_setting				Smd_tmv_relative_setting;
	DRV_Smd_teeth_by_vlpf								Smd_teeth_by_vlpf;
	DRV_Smd_FW_GMV_info									Smd_FW_GMV_info;
	DRV_Smd_control_o										Smd_control_o;
	// end chen 180308
...
void drvif_color_set_DI_detection_mode(unsigned char sw_detection_mode);
void drvif_color_Set_DI_RTNR_DMA_Setting_hold(unsigned char hold);
void drvif_color_Set_RTNR_I7R_First_Frame(unsigned char isFirstFrame, unsigned char apply_db);
