...
#define AUTO_BRINGUP_SOURCE _SRC_HDMI //_SRC_VGA   _SRC_TV  _SRC_CVBS _SRC_HDMI
...
//#define CONFIG_ENABLE_HDMI_PATTERN_4K//enable hdmi internal pattern 4k
//#define CONFIG_ENABLE_HDMI_PATTERN_8K//enable hdmi internal pattern 8k
#if defined(CONFIG_ENABLE_HDMI_PATTERN_8K) || defined(CONFIG_ENABLE_HDMI_PATTERN_4K)
#define CONFIG_ENABLE_HDMI_PATTERN//enable hdmi internal pattern
#endif
#define DISABLE_ALL_ONLINE_FUNCTION//doesn not  use online measure
#endif
//#define CONFIG_VBM_NOTREADY//vbm borrow memory not ready
//#define CONFIG_FORCE_RUN_I3DDMA   //not vo source force go i3ddma flow
//#define CONFIG_VODRIVER_KERNEL//i3ddma vo setting at kernel
...
typedef enum PANEL_OUTPUT_TIMING_MODE{
	OUTPUT_4K2K120HZ = 0,
	OUTPUT_8K4K30HZ = 1,
	OUTPUT_8K4K60HZ = 2,
	OUTPUT_TIMING_MODE_MAX,
}PANEL_OUTPUT_TIMING_MODE;

typedef struct PANEL_DYNAMIC_PARAMETER_SETTING{
	unsigned int Panel_Typical_Clock;
	unsigned int Panel_H_Total;
	unsigned int Panel_V_Total;
	unsigned int Panel_H_DEN_START;
	unsigned int Panel_H_DEN_END;
	unsigned int Panel_V_DEN_START;
	unsigned int Panel_V_DEN_END;
	unsigned int Panel_H_ACT_START;
	unsigned int Panel_H_ACT_END;
	unsigned int Panel_V_ACT_START;
	unsigned int Panel_V_ACT_END;
	unsigned int Panel_REFRESH_RATE;
}PANEL_DYNAMIC_PARAMETER_SETTING;

typedef struct WATERMARK_MIDPOINT {
	unsigned char value;//peak index vlaue
	unsigned char num;//peak counter number
}WATERMARK_MIDPOINT;

typedef enum  {
	WATERMARK_IDLE= 0,//not use
	WATERMARK_INIT,//for initi setting
	WATERMARK_COLLECT_2ND_PIXEL,//collect and calculete data
	WATERMARK_DETECT,//start detect
}WATERMARK_RUNNING_STATE;

typedef enum  {
	WATERMARK_PAYLOAD_NONE = 0,//not payload
	WATERMARK_PAYLOAD_1X,//payload is 1x
	WATERMARK_PAYLOAD_2X,//payload is 2x
}WATERMARK_PAYLOADTYPE;

#define WATERMARK_2ND_PIXEL_COLLECT_FRAME_NUM 1//
...
void rpc_send_hdmi_2_1_flag(unsigned char flag);//send SCALERIOC_HDMI21_FLAG to video fw
...
unsigned char force_disp_s1_s2_same_clk(void);//TRUE: use two step UZU not normal uzu
...
unsigned char get_hdmi_vgip_source_type(unsigned char connect_port);//return VGIP_SRC_HDMI1 or VGIP_SRC_HDMI2
...
unsigned char allocate_memory_for_8k_platform(void);//retunr TRUE memory is i3ddma:8k mdomain 4k+4k  retunr  false: m domain 2k the same with merlin7
void ctrl_i_clock_2p_sel(unsigned char path, unsigned char enable);//path is dma vgip or main vgip or sub vgip. enable is enable or not
unsigned char check_hdmi_enabe_2p_mode(unsigned char hdmi_port);//hdmi source to set crt i clcok 2 p mode 
