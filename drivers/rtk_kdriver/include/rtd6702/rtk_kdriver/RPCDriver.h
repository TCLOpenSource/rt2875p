...
#define CONFIG_REALTEK_RPC_VCPU2
#define CONFIG_REALTEK_RPC_VCPU3
...
#define PDEBUGD(fmt, args...)       if (rpc_pr_dispatch >0) rtd_pr_rpc_alert ("RPC: " fmt, ## args) /*dispatch*/
#define DISPDEBUG(fmt, args...)     if (rpc_pr_dispatch_more >0) rtd_pr_rpc_alert ("RPC: " fmt, ## args) /*dispatch more info*/
#define PDEBUGR(fmt, args...)	    if (rpc_pr_read >0) rtd_pr_rpc_alert ("RPC: " fmt, ## args) /*print read pointer*/
#define PDEBUGW(fmt, args...)       if (rpc_pr_write >0) rtd_pr_rpc_alert ("RPC: " fmt, ## args) /*print write pointer*/
#else
...
#endif
#define PDEBUG_LOG(fmt, args...) rpc_log(fmt, ## args)
extern int rpc_pr_pid_debug;
#define PID_DEBUG(fmt, args...)       if (rpc_pr_pid_debug >0) rtd_pr_rpc_emerg("wrpc:" fmt, ## args)
#define EMERG_DEBUG(fmt, args...)     rtd_pr_rpc_emerg("wrpc:" fmt, ## args)


#define ACPU_CORE_ID            0x3

#define SYNC_FLAG_ACPU			0x000000d0
#define SYNC_FLAG_V1CPU 		0x000000d4
#define SYNC_FLAG_V2CPU 		0x000000d8
#define SYNC_FLAG_V3CPU		0x00000064
...
#define RPC_INT_WRITE_1 1
#define RPC_WRITE_ENABLE_VCPU3 (1<<5)
#define RPC_WRITE_ENABLE_VCPU2 (1<<4)
#define RPC_WRITE_ENABLE_SCPU (1<<3)
#define RPC_WRITE_ENABLE_VCPU (1<<2)
#define RPC_WRITE_ENABLE_ACPU (1<<1)

#define RPC_INT_SCPU_A (1 << 2)
#define RPC_INT_SCPU_V (1 << 3)
#define RPC_INT_SCPU_V2 (1<<4)


#define RPC_INT_VCPU_S (1 << 1)
#define RPC_INT_VCPU2_S (1 << 1)
#define RPC_INT_ACPU_S (1 << 1)
...
#ifdef CONFIG_ARM64
#define CONFIG_ARM64_RPC
extern void __iomem *rpc_base;
#define get_rpc_addr(x)     (rpc_base + ((long)(x) - 0x1b100000))
#define rpc_pa(x)           ((u64)x-(u64)(rpc_base-0x1b100000))
#define __read_caller_register()      \
    ({ int __res;                               \
        __asm__ __volatile__(                   \
        "mov\t%0, x30\n\t"                   \
        : "=r" (__res));                    \
...
#ifdef CONFIG_REALTEK_RPC_VCPU3
	RPC_POLL_DEV_SV3_ID6,
	RPC_POLL_DEV_V3S_ID7,
#endif
	RPC_POLL_DEV_TOTAL
}NUM_POLL_DEV;
...
#ifdef CONFIG_REALTEK_RPC_VCPU3
	RPC_INTR_DEV_SV3_ID6,
	RPC_INTR_DEV_V3S_ID7,
#endif
	RPC_INTR_DEV_TOTAL
}NUM_INTR_DEV;
...
#endif
#ifdef CONFIG_REALTEK_RPC_VCPU3
	RPC_KERN_DEV_SV3_ID4,
	RPC_KERN_DEV_V3S_ID5,
#endif
...
/* Memory is 4-byte aligned on 32bit systems and is set for compatibility on 64bit systems */
#pragma pack(push)
#pragma pack(4)
/*
// read : nonblocking
...
typedef struct RPC_POLL_Dev {
	u32 ringBuf;		/* buffer for interrupt mode */
	u32 ringStart;	/* pointer to start of ring buffer */
	u32 ringEnd;		/* pointer to end   of ring buffer */
	u32 ringIn;		/* pointer to where next data will be inserted  in   the ring buffer */
	u32 ringOut;		/* pointer to where next data will be extracted from the ring buffer */
...
static inline char *get_ring_data(const char *func, RPC_COMMON_Dev *dev,
				  char *out, char *buf, int datasize)
{

	int size = get_ring_data_size((char *)get_rpc_addr(dev->ringStart), (char *)get_rpc_addr(dev->ringEnd), (char *)get_rpc_addr(dev->ringIn), out);
	int tail = (char *)get_rpc_addr(dev->ringEnd) - out;

	if (size < datasize) {
		rtd_pr_rpc_err("%s: Size not enough %d < %d\n", func, size, datasize);
		return NULL;
	}
	...
	if (tail >= datasize) {
		...
		my_memcpy((int *)(buf + tail), (int *)get_rpc_addr(dev->ringStart),
			  datasize - tail);
		out = get_rpc_addr(dev->ringStart) + datasize - tail;
	}
...
{
	PDEBUG
	    ("%s: program:%u version:%u procedure:%u taskID:%u sysPID:%x size:%u context:%x %s\n",
	     func, rpc->programID, rpc->versionID, rpc->procedureID,
	     rpc->taskID, rpc->sysPID, rpc->parameterSize, rpc->mycontext,
	     in_atomic() ? "atomic" : "");
}
...
static inline void peek_rpc_struct(const char *func, RPC_COMMON_Dev *dev)
{
	...
	unsigned long pid;
	unsigned long arg;
	char *out = (char *)get_rpc_addr(dev->ringOut);
	...
	if (rpc.programID == AUDIO_AGENT && rpc.versionID == 0) {
		...
		out =
		    get_ring_data(func, dev, out, (char *)&arg,
				  sizeof(unsigned long));
		if (out == NULL)
			return;
		arg = ntohl(arg);
		if (rpc.procedureID == 1)
			rtd_pr_rpc_info("%s: alloc %lu bytes\n", func, arg);
		else
			rtd_pr_rpc_info("%s: free addr %lx\n", func, arg);
	} else if (rpc.programID == REPLYID && rpc.versionID == REPLYID) {
		out =
		    get_ring_data(func, dev, out, (char *)&pid,
				  sizeof(unsigned long));
		if (out == NULL)
			return;
		pid = ntohl(pid);
		rtd_pr_rpc_info("%s: pid:%lu\n", func, pid);
	}
}
...
{
	long tmp = (long)nextRpc;
	tmp = ((tmp + 3) & 0xfffffffffffffffc);
	if (tmp == (long)get_rpc_addr(extra->dev->ringEnd))
		extra->nextRpc = (char *)get_rpc_addr(extra->dev->ringStart);
	else
...
static inline int rpc_done(RPC_DEV_EXTRA *extra)
{
	...
	return (char *)get_rpc_addr(dev->ringOut) == extra->nextRpc;
}

static inline int ring_empty(RPC_COMMON_Dev *dev)
{
	return (char *)get_rpc_addr(dev->ringOut) == (char *)get_rpc_addr(dev->ringIn);
}
#else
...
static inline void show_rpc_struct(const char *func, RPC_STRUCT *rpc)
{
	PDEBUG(KERN_NOTICE "%s: program:%u version:%u procedure:%u taskID:%u sysPID:%u size:%u context:%x %s\n",
		func, rpc->programID, rpc->versionID, rpc->procedureID, rpc->taskID, rpc->sysPID, rpc->parameterSize,
		(unsigned long)rpc->mycontext, in_atomic() ? "atomic" : "");
}
...
static inline void show_queued_rpc(RPC_DEV_EXTRA *extra, char *out)
{
	...
	if (out) {
		...
		/*rtd_pr_rpc_err("%s-rpc(%p),program:%u version:%u procedure:%u taskID:%u sysPID:%u size:%u context:%x %s\n",
...
static inline void peek_rpc_struct(const char *func, RPC_COMMON_Dev *dev)
{
	...
	if(rpc.programID == AUDIO_AGENT && rpc.versionID == 0){
		...
		arg = ntohl(arg);
		if(rpc.procedureID == 1)
			rtd_pr_rpc_err("%s: alloc %lu bytes\n", func, arg);
		else
			rtd_pr_rpc_err("%s: free addr %lx\n", func, arg);
	}else if(rpc.programID == REPLYID && rpc.versionID == REPLYID){
		out = get_ring_data(func, dev, out, (char *)&pid, sizeof(unsigned long));
		if(out == NULL)
			return;
		pid = ntohl(pid);
		rtd_pr_rpc_notice("%s: pid:%lu\n", func, pid);
		...
		rtd_pr_rpc_err ("%s: command:%lu, para1:%lu, para2: %lu\n", func, command[0], command[1],command[2]);
...
static inline int is_taskID_alive(unsigned long taskID)
{
	...
	if(unlikely(taskID > PID_MAX_DEFAULT)){
		//rtd_pr_rpc_err("invalid taskID:%lu > pid_max:%d\n", taskID, PID_MAX_DEFAULT);
		return -1;
	}

	if(unlikely(current == NULL || current->nsproxy == NULL /*|| current->nsproxy->pid_ns == NULL*/)){
		//rtd_pr_rpc_err("%s:%d strange happened current:%p nsproxy:%p\n", __func__, __LINE__, current, current->nsproxy);
		return -1;
	}

	rcu_read_lock();

	task= pid_task(find_vpid(taskID), PIDTYPE_PID);
...
static inline int update_thread_list(RPC_PROCESS *proc)
{
	...
	if(thread == NULL){
		rtd_pr_rpc_err("%s: failed to allocate RPC_THREAD\n", __func__);
		return -ENOMEM;
	}
...
static inline void record_block_rpc(unsigned long data, RPC_COMMON_Dev *dev)
{
    ...
    char *out = get_rpc_addr(dev->ringOut);
    ...
    if((rerpc.taskID !=0)&&(user_rpc_queue_count < MAX_DEBUG_QUEUE)) //block rpc
    {
        ...
        if(rrpc == NULL){
            rtd_pr_rpc_err("%s: failed to allocate DEBUG_RECORD_RPC\n", __func__);
            return;
        }
...
static inline void release_block_rpc(RPC_DEV_EXTRA *extra, unsigned long taskID)
{
    ...
    list_for_each_entry_safe(rrpc, tmprpc, &extra->blockrpc_list, list){
        rrpc->tcount++;
        //rtd_pr_rpc_debug("yan_wang debug the taskID == %d \n", taskID);
...
#define RPC_VCPU_ID_0x109_rtkvdec_set_command	0x109
...
#define RPC_VCPU_ID_0x115_VO_EOS	0x115
#define RPC_VCPU_ID_0x116_FILM_DETECT_DONE 0x116

#define RPC_VCPU_ID_0x117_MEMC_BYPASS_MODE     0x117
...
#define RPC_VCPU_SET_EXPORTCRC_MEMORY 0x160
#define RPC_VCPU_SET_IMPORTCRC_MEMORY 0x161
...
#define RPC_ACPU_AUDIO_CONFIG   0x204
#define RPC_ACPU_AUDIO_HALT     0x205
...
#define RPC_ACPU_SET_HDMITX_FORMAT     0x305   // ACPU->SCPU
