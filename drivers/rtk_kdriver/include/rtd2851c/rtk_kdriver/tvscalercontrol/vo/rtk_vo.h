...
#define ENABLE_DUMP_FRAME_INFO //for bring up
...
#define VO_IOC_SETPICTUREINFO   _IOW(VO_IOC_MAGIC, 5, int)
#define VO_IOC_MAXNR            5
...
typedef struct {
	...
	unsigned char   eos[VODMA_DEV_NUM];
	UPDATE_SEQ_CTRL	updare_seq_ctrl[VODMA_DEV_NUM];
	unsigned char   filmDetectDone[VODMA_DEV_NUM];
    unsigned int    photoChannel;
    unsigned long   dmabuf_hndl[VODMA_DEV_NUM][VODMA_HALBUF_NUM]; // for hal_phy_addr[][]
} VO_DATA;
...
typedef enum {
  ...
  typedef struct VO_MALLOC_STRUCT {
  	unsigned long Memory;
  	unsigned long PhyAddr ;
  	unsigned long VirtAddr ;
  } VO_MALLOC_STRUCT;

  typedef struct VO_DUMP_STRUCT {
  ...
  struct VO_RPC_DEBUG_MEMORY {
  	int videoFirmwareVersion ;
  } ;
  typedef struct VO_RPC_DEBUG_MEMORY VO_RPC_DEBUG_MEMORY;
  #endif

  typedef struct VO_DUMP_BUFFER_HEADER {
  	unsigned int magic;
  	unsigned int size;
  	unsigned int rd;
  	unsigned int wr;
  } VO_DUMP_BUFFER_HEADER;
  // -------------------------------
#endif
...
unsigned long rpcVoReady(unsigned long, unsigned long);
unsigned long rpcVoNoSignal(unsigned long, unsigned long);
unsigned long rpcVoAVSync(unsigned long, unsigned long);
unsigned long rpcVoHdrFrameInfo(unsigned long, unsigned long);
unsigned long rpcSetQOSMode(unsigned long, unsigned long);
...
unsigned char Get_Val_get_force_change_vo_flag(unsigned char);
...
extern SLR_VOINFO (*Get_Val_VOInfoFromShareMem)(void);
extern VO_DATA * (*GetVODMAData)(void);
extern seamless_change_sync_info *(*GetSeamlessInfo)(void);
extern unsigned char (*Get_Val_IsVOInfoSameTiming)(VO_DATA *, SLR_VOINFO *,unsigned char);

unsigned char get_vo_change_flag(unsigned char ch);
void set_vo_change_flag(unsigned char ch, unsigned char flag);
void Set_Val_vodma1_vdec_source(unsigned int type);//set vodma1 source type
unsigned int Get_Val_vodma1_vdec_source(void);//get vodam1 source type not
