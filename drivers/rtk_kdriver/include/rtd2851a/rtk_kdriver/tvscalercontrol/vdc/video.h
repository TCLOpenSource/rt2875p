...
//#define CONFIG_ENABLE_VDC_BIG_SIGNAL_CHECK
...
typedef enum {
	...
	VDC_MSG_INT_CH_FLAG = _BIT26,		/* internal ch change flag */
...
{										\
	...
	{									\
		asm("or $5,$31,$0");				\
		rtd_pr_vdc_emerg("###### VDC API called at %x --- ");	\
	}									\
}
...
{												\
	...
	{											\
		rtd_pr_vdc_debug("[INIT]" string, ##args);		\
	}											\
	if(vdc_level==VDC_MSG_RESET)				\
	{										\
		rtd_pr_vdc_debug("[RESET]" string, ##args);		\
	}										\
	if(vdc_level==VDC_MSG_ERROR)				\
	{										\
		rtd_pr_vdc_debug("[error]" string, ##args);				\
	}										\
	if(((g_vdc_msg_level&vdc_level)!=0)) {		\
		if(vdc_level==VDC_MSG_STATUS)			\
		{										\
			rtd_pr_vdc_debug("[status]" string, ##args);		\
		}										\
		if(vdc_level==VDC_MSG_DGAIN)			\
		{										\
			rtd_pr_vdc_emerg("[DG]" string, ##args);		\
		}										\
		if(vdc_level==VDC_MSG_CHROMA)			\
		{										\
			rtd_pr_vdc_emerg("[C]" string, ##args);			\
		}										\
		if(vdc_level==VDC_MSG_HDTO)				\
		{										\
			rtd_pr_vdc_emerg("[HDTO]" string, ##args);		\
		}										\
		if(vdc_level==VDC_MSG_YCSEP)			\
		{										\
			rtd_pr_vdc_emerg("[yc]" string, ##args);			\
		}										\
		if(vdc_level==VDC_MSG_SCART)			\
		{										\
			rtd_pr_vdc_emerg("[SCART]" string, ##args);		\
		}										\
		if(vdc_level==VDC_MSG_FACT)				\
		{										\
			rtd_pr_vdc_emerg("[FACTORY]" string, ##args);	\
		}										\
		if(vdc_level==VDC_MSG_TVSCAN)			\
		{										\
			rtd_pr_vdc_emerg("[SCAN]" string, ##args);		\
		}										\
		if(vdc_level==VDC_MSG_DEBUG1)			\
		{										\
			rtd_pr_vdc_emerg("[D1]" string, ##args);			\
		}										\
		if(vdc_level==VDC_MSG_DEBUG2)			\
		{										\
			rtd_pr_vdc_emerg("[D2]" string, ##args);		\
		}										\
		if(vdc_level==VDC_MSG_DEBUG3)			\
		{										\
			rtd_pr_vdc_emerg("[D3]" string, ##args);		\
		}										\
		if(vdc_level==VDC_MSG_TVCHCH)			\
		{										\
			rtd_pr_vdc_emerg("[tch]" string, ##args);		\
		}										\
		if(vdc_level==VDC_MSG_TVCHCH_DETAIL)			\
		{										\
			rtd_pr_vdc_emerg("[tchD]" string, ##args);		\
		}										\
		if(vdc_level==VDC_MSG_DEBUG_P)			\
		{										\
			rtd_pr_vdc_emerg("[Debug]" string, ##args);		\
		}										\
	}											\
}
...
#define	Fn5_min	2782461128UL	//2782461128
#define	Fn5_max	2904463118UL	//2891661468
#define	Fn4_min	Fn5_max		//2917264768
#define	Fn4_max	3037679748UL	//3021366820
#define	Fn3_min	Fn4_max		//3053992676
#define	Fn3_max	3169706334UL	//3166554564
#define	Fn2_min	Fn3_max		//3172858104
#define	Fn2_max	3314726950UL	//3292869692
#define	Fn1_min	Fn2_max		//3336584208
#define	Fn1_max	3463100254UL	//3438580728
#define	F0_min	Fn1_max		//3487619780
#define	F0_max	3595962604UL	//3559596216
#define	Fp1_min	F0_max		//3632328992
#define	Fp1_max	3726953618UL	//3697302080
#define	Fp2_min	Fp1_max		//3756605156
#define	Fp2_max	3875779526UL	//3845372156
#define	Fp3_min	Fp2_max		//3906186896
#define	Fp3_max	4007628186UL	//4000482488
#define	Fp4_min	Fp3_max		//4014773884
#define	Fp4_max	4166952088UL	//4146018088
#define	Fp5_min	Fp4_max		//4187886088
#define	Fp5_max	4275799704UL	//4275799704
...
typedef enum VDC_AVD_AV_COEF {
	VDC_AVD_AV_CONFIG_0 = 0,	//for resistor   75 ohm & capacitor   47 nF
	VDC_AVD_AV_CONFIG_1,		//for resistor 110 ohm & capacitor 100 nF
} VDC_AVD_AV_COEF;
...
#define COLOR_STD_OUT_RANGE 0xff
...
extern void drvif_module_vdc_set_CbCrDelay(unsigned char ucCb_delay, unsigned char ucCr_delay);
...
extern bool drvif_module_vdc_ReadDisablemodeEN(void);
...
void drvif_module_vdc_Set_NewY1Cb1Cr1_delay(bool bEnable, unsigned char InYcsep, unsigned char InSource, unsigned char InMode);
...
extern unsigned char drvif_module_vdc_CheckHVisLocked_TVScan(void);
#endif
...
///////////////////////////////////////////////////////////////////////////////////////
extern void drvif_module_vdc_set_ColorMode(unsigned char ucMode);
...
extern unsigned char drvif_video_ChromaLockStatus(void);
...
extern unsigned int drvif_module_vdc_read_hstate_vsync_error_unbalance_check(void);
...
#if 1
extern unsigned char drvif_module_vdc_read_internal_auto_scan_flag(void);
extern void drvif_module_vdc_set_internal_auto_scan_flag(unsigned char on_off);
extern unsigned char drvif_module_vdc_read_internal_ch_change_flag(void);
...
void drvif_module_vdc_ADC_Vrefp_Setting(VDC_AVD_AV_COEF config);
void drvif_module_vdc_FDF_Table_Select(VDC_AVD_AV_COEF config);
...
#ifdef CONFIG_CUSTOMER_TV010
//for nonstandard
typedef enum ATV_NON_STANDARD_FLAG{
	ATVNONSTANDFLAG_0 = 0x0001,
	ATVNONSTANDFLAG_1 = 0x0002,
	ATVNONSTANDFLAG_2 = 0x0004,
	ATVNONSTANDFLAG_3 = 0x0008,
	ATVNONSTANDFLAG_4 = 0x0010,
	ATVNONSTANDFLAG_5 = 0x0020,
	ATVNONSTANDFLAG_6 = 0x0040,
	ATVNONSTANDFLAG_7 = 0x0080,
	ATVNONSTANDFLAG_8 = 0x0100,
	ATVNONSTANDFLAG_9 = 0x0200,
}ATV_NON_STANDARD_FLAG;

typedef enum AV_NON_STANDARD_FLAG{
	AVNONSTANDFLAG_0 = 0x00010000,
	AVNONSTANDFLAG_1 = 0x00020000,
	AVNONSTANDFLAG_2 = 0x00040000,
	AVNONSTANDFLAG_3 = 0x00080000,
	AVNONSTANDFLAG_4 = 0x00100000,
	AVNONSTANDFLAG_5 = 0x00200000,
	AVNONSTANDFLAG_6 = 0x00400000,
	AVNONSTANDFLAG_7 = 0x00800000,
	AVNONSTANDFLAG_8 = 0x01000000,
	AVNONSTANDFLAG_9 = 0x02000000,
}AV_NON_STANDARD_FLAG;
#endif
