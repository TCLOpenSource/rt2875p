...
#define REG_SHARE_CLOCK             0xb8060034
#define SHARE_REG_CLOCK_BIT         10
#define SHARE_REG_WOL_EVENT         0xb8060450
...
#define ENABLE_WOL		1
#define ENABLE_WOL_WPD		1
/*
*  bit7: wake detect enable
*  bit6: wake reason is effect or not
*  bit5~0: wake type(1:magic packet , 2:mdns )
*/
#define ETH_WAKE_DUMMY_REG 0xb8060144
//#define ENABLE_WOW		1
...
#define NUM_WOL_PATTERN     1
#define WOL_MASK_SIZE       32
...
u8 packet_pattern[NUM_WOL_PATTERN][PACKET_PATTERN_SIZE] = 
{
    //case 1 :: mdns query
    {
	...
	0x00, 0x00, 0x14, 0xe9, 0x14, 0xe9, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
...
static int g_is_new_wol_flow = 1;
...
static void rtl8168_xmii_enhance_performance(struct rtl8168_private *tp, u32 enable)
{
	...
	if(tapbin_val) {
		...
		rtl8168_xmii_set_page(tp, 0x0bc0);
...
static void rtl_read_snr_val(struct rtl8168_private *tp)
{
	...
	rtl8168_xmii_set_page(tp, 0x0aa0);
	for(i = 0; i < 3; i++) {
...
static void rtl_get_phy_status(struct rtl8168_private *tp)
{
	...
	/* page 0xa40, reg 0 */
	phyStatus->phy_set = rtl_readphy(tp, 0x00);
	...
	phyStatus->adv_ability = rtl_readphy(tp, 0x04);

	/* phy status, page 0xa42, reg16 */
	rtl8168_xmii_set_page(tp, 0x0a42);
	phyStatus->phy_status = rtl_readphy(tp, 0x10);
	...
	phyStatus->pcs_state = rtl_readphy(tp, 0x10);
...
static void rtl_set_wol_pattern(struct rtl8168_private *tp)
{
	...
	for(i = 0; i < NUM_WOL_PATTERN; i  = i + 4) {
		...
		r8168_mac_ocp_write(tp, 0xC130 + i / 2, tmp);
	}
	...
	for(i = 0; i < (WOL_MASK_SIZE * 2); i++)
	{
		...
		for(j = 0; j < NUM_WOL_PATTERN; j = j + 4) {
			if((i % 2) == 0) {
				...
				r8168_mac_ocp_write(tp, 0x7C00 + i * 16 + j / 2, tmp);
			} else {
				int n;
				int k;
				u16 tmp = 0;
	...
	{
		if(i < 8)
			r8168_mac_ocp_write(tp, 0xc020 + i * 2, wol_pattern[i]);
		else
			r8168_mac_ocp_write(tp, 0xc100 + (i -8) * 2, wol_pattern[i]);
	}
	...
	{
		int j = 0;
		r8168_mac_ocp_write(tp, 0x6A00 + i * 138, packet_start_code[i]);
		for(j = 0; j  < (PACKET_PATTERN_SIZE / 2); j++) {
			r8168_mac_ocp_write(tp, 0x6A02 + i * 138 +  j * 2, 
...
static void rtl_set_wol_pattern(struct rtl8168_private *tp)
{
    ...
    for(i=0;i<16;i++)
    {
        ...
        eth_info("write %d mask2=0x%02x mask1=0x%02x mask0=0x%02x\n"
                "                      temp=0x%08x\n"
                        ,i
                        ,wol_mask[2][i]
                        ,wol_mask[1][i]
...
static void rtl_wol_set(struct rtl8168_private *tp, u32 enable)
{
	...
	if (enable == RTL8168_WOL_ENABLE)
	{
		...
		if(g_is_new_wol_flow) {
		    rtd_outl(ETH_WAKE_DUMMY_REG, rtd_inl(ETH_WAKE_DUMMY_REG) | 0x80);
		    rtl_wpd_set(tp);
		}
#endif
...
{
	if (BMCR_PDOWN & rtl_readphy(tp, MII_BMCR))
		return true;
	else
		return false;
}
		...
		char *buf)
{
    ...
    return sprintf(buf, "wol_enable = %d, wol_interrupt = 0x%08x	\
				...
				wol_status & BIT(SHARE_REG_WOL_EVENT_BIT)?
				        "Receive magic packet!!":"Doesn't receive magic packet!!");
}
        ...
        char *buf)
{
    ...
    for(i=0;i<WOL_MASK_SIZE;i++)
    {
        memset(temp,0,sizeof(temp));
        count+=sprintf(temp,"%d mask2=0x%x mask1=0x%x mask=0x%x\n "
                             "addr[0x%03x]=0x%08x\n"
                    ,i
                    ,wol_mask[2][i]
                    ,wol_mask[1][i]
                    ,wol_mask[0][i]
                    ,i*8
                    ,rtl_eri_read(tp, i*8, ERIAR_EXGMAC)
        );
        strcat(buf,temp);
    }
    memset(temp,0,sizeof(temp));
    count+=sprintf(temp,"case 1/2 wol pattern addr[0x80]=0x%08x\n",rtl_eri_read(tp, 0x80, ERIAR_EXGMAC));
    strcat(buf,temp);

    memset(temp,0,sizeof(temp));
    count+=sprintf(temp,"case 3   wol pattern addr[0x84]=0x%08x\n",rtl_eri_read(tp, 0x84, ERIAR_EXGMAC));
    strcat(buf,temp);
    return count;
#else
        ...
        char *buf)
{
	...
	if(rtd_inl(ETH_WAKE_DUMMY_REG) & BIT(6))
	    wake_reason = rtd_inl(ETH_WAKE_DUMMY_REG) & 0x3F;
...
static ssize_t rtl8168_new_wol_flow_enable_show(struct kobject *kobj, struct kobj_attribute *attr,
        char *buf)
{
	return snprintf(buf, PAGE_SIZE, "new_wol_flow:%d\n", g_is_new_wol_flow);
}
...
static void rtl8168_net_suspend(struct net_device *dev)
{
	...
	rtd_outl(ETH_WAKE_DUMMY_REG, rtd_inl(ETH_WAKE_DUMMY_REG) & (~0xFF));
