...
struct bus_type rtk_cec_bus_type = {
    .name = "rtk_cec",
    .match = cec_bus_match,
    /* cyhuang (2011/11/24) +++ */
#ifdef CONFIG_PM
...
void cec_drv_shutdown(struct device *dev)
{
    ...
    if (p_drv->set_cec_global_interrupt)
        p_drv->set_cec_global_interrupt(p_dev, 0);
}
 ...
 *------------------------------------------------------------------*/
int __init cec_core_init(void)
{
    cec_warn("%s, register cec_bus %p\n", __FUNCTION__, &rtk_cec_bus_type);

    return bus_register(&rtk_cec_bus_type); /* register cec bus type */
}
 ...
 * Retn : 0 : success, others fail
 *------------------------------------------------------------------*/
void __exit cec_core_exit(void)
{
    bus_unregister(&rtk_cec_bus_type);  /* unregister cec bus */
}
