...
static const char *g_switch_name_list[] = {
    "PIN_SWITCH_WOV", "PIN_HP_OUT_JD", NULL
};
...
int rtk_gpio_switch_register(RTK_GPIO_ID gid, char *name,unsigned char invert)
{
    int ret = 0;
    int state = 0;
    struct gpio_switch_data *switch_data = NULL;
    ...
    if(!name)
        snprintf(switch_data->sdev.name, sizeof(switch_data->sdev.name), "%s-%d", gpio_type(gpio_group(gid)), gpio_idx(gid));
    else{
        if(strcmp("jd",name) == 0){
            snprintf(switch_data->sdev.name, sizeof(switch_data->sdev.name), "%s",  "h2w");
            switch_data->gpio_active_low = (invert? 0: 1);
            switch_data->type = HP_OUT_JD;
        }
        else{
            snprintf(switch_data->sdev.name, sizeof(switch_data->sdev.name), "%s",  name);
    ...
    if((NULL != name) && (strcmp("jd",name) == 0)){
        state = rtk_gpio_input(switch_data->gpio);
        if (switch_data->gpio_active_low)
            state = !state;
        if(state == 1)
            state = PLUG_IN_HEADSET_ANLG;
...
static int rtk_switch_gpio_parse_and_register_dev(char *name)
{
    ...
    RTK_SWITCH_WARN(" g_gpio_invert :%d\n",g_gpio_invert);   
    return rtk_gpio_switch_register(gid, dev_name,g_gpio_invert);
