...
void haltbtc_8822e_set_wl_trx_mask(struct btc_coexist *btc, u8 path, u8 group, u32 val)
{
        btc->btc_set_rf_reg(btc, path, 0xef, MASK20BITS, 0x20000);
        btc->btc_set_rf_reg(btc, path, 0x33, MASK20BITS, group);
        btc->btc_set_rf_reg(btc, path, 0x3f, MASK20BITS, val);
        btc->btc_set_rf_reg(btc, path, 0xef, MASK20BITS, 0x0);
}
...
void halbtc8822e_cfg_init(struct btc_coexist *btc)
{
	...
	/* WLS0_EN_GNT_CHECK = 0: Let WLS0 Tx when BTS1 Tx (GNT_BT/GNT_WL = 1/1) */
	if (btc->board_info.btdm_ant_num == 1)
		btc->btc_set_rf_reg(btc, BTC_RF_A, 0x2, 0x1, 0x0);

	if (btc->board_info.btdm_ant_num == 1) {
		/* set WL Tx thru in TRX mask table if GNT_WL=0 && BT_S1=ss group */
		haltbtc_8822e_set_wl_trx_mask(btc, RF_PATH_A, BTC_BT_SS_GROUP_SHARED_ANT, 0x5ff);
		haltbtc_8822e_set_wl_trx_mask(btc, RF_PATH_B, BTC_BT_SS_GROUP_SHARED_ANT, 0x5ff);
		/* set path-A(S0) tx masked by BT rx if GNT_WL=0 && BT_S1=rx group */
		haltbtc_8822e_set_wl_trx_mask(btc, RF_PATH_A, BTC_BT_RX_GROUP_SHARED_ANT, 0x5df);
	}
