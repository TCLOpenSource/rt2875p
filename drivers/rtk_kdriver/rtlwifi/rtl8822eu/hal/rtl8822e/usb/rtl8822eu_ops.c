 ...
 *
 *****************************************************************************/
#define _RTL8822EU_OPS_C_

#include <drv_types.h>			/* PADAPTER, basic_types.h and etc. */
#include <hal_intf.h>			/* struct hal_ops */
#include <hal_data.h>			/* HAL_DATA_TYPE */
#include "../../hal_halmac.h"		/* register define */
#include "../rtl8822e.h"		/* 8822e hal common define. rtl8822eu_init_default_value ...*/
#include "rtl8822eu.h"			/* 8822eu functions */

#ifdef CONFIG_SUPPORT_USB_INT
static void rtl8822eu_interrupt_handler(PADAPTER padapter, u16 pkt_len, u8 *pbuf)
{	
	}
...
{
	pdvobj->HardwareType = HARDWARE_TYPE_RTL8822EU;
...
static u8 sethwreg(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_SET_RPWM:
#ifdef CONFIG_LPS_LCLK
		{
			...
			if ((ps_state & BIT(0)) && (LPS_PG == pwrpriv->lps_level))
				ps_state |= BIT(4);
			#endif
			rtw_write8(padapter, REG_USB_HRPWM_8822E, ps_state);
		}
#endif
		break;
	case HW_VAR_AMPDU_MAX_TIME:
		rtw_write8(padapter, REG_AMPDU_MAX_TIME_V1_8822E, 0x70);
		break;
	...
	case HW_VAR_SET_DRV_ERLY_INT:
		...
		break;
	default:
		ret = rtl8822e_sethwreg(padapter, variable, val);
		break;
	}
...
static void gethwreg(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_CPWM:
#ifdef CONFIG_LPS_LCLK
		*val = rtw_read8(padapter, REG_USB_HCPWM_8822E);
		/* RTW_INFO("##### REG_USB_HCPWM(0x%02x) = 0x%02x #####\n", REG_USB_HCPWM_8822E, *val); */
#endif /* CONFIG_LPS_LCLK */
		break;
	case HW_VAR_RPWM_TOG:
#ifdef CONFIG_LPS_LCLK
		*val = rtw_read8(padapter, REG_USB_HRPWM_8822E);
		*val &= BIT_TOGGLE_8822E;
#endif /* CONFIG_LPS_LCLK */
		break;

	default:
		rtl8822e_gethwreg(padapter, variable, val);
		break;
	}
...
static u8 sethaldefvar(PADAPTER padapter, HAL_DEF_VARIABLE eVariable, void *pValue)
{
	...
	switch (eVariable) {
	default:
		rtl8822e_sethaldefvar(padapter, eVariable, pValue);
		break;
	}
...
static u8 gethaldefvar(PADAPTER	padapter, HAL_DEF_VARIABLE eVariable, void *pValue)
{
	...
	case HW_VAR_MAX_RX_AMPDU_FACTOR:
		*(HT_CAP_AMPDU_FACTOR *)pValue = MAX_AMPDU_FACTOR_64K;
		break;
	default:
		bResult = rtl8822e_gethaldefvar(padapter, eVariable, pValue);
		break;
	}
...
static void rtl8822eu_init_default_value(PADAPTER	padapter)
{
	rtl8822e_init_default_value(padapter);
}
...
static u8 read_adapter_info(PADAPTER padapter)
{
	 ...
	 * 1. Read Efuse/EEPROM to initialize
	 */
	if (rtl8822e_read_efuse(padapter) != _SUCCESS)
		goto exit;
...
void rtl8822eu_set_hal_ops(PADAPTER padapter)
{
	...
	err = rtl8822eu_halmac_init_adapter(padapter);
	if (err) {
		RTW_INFO("%s: [ERROR]HALMAC initialize FAIL!\n", __func__);
		return;
	}

	rtl8822e_set_hal_ops(padapter);

	ops = &padapter->hal_func;

	ops->hal_init = rtl8822eu_init;
	ops->hal_deinit = rtl8822eu_deinit;

	ops->inirp_init = rtl8822eu_inirp_init;
	ops->inirp_deinit = rtl8822eu_inirp_deinit;

	ops->init_xmit_priv = rtl8822eu_init_xmit_priv;
	ops->free_xmit_priv = rtl8822eu_free_xmit_priv;

	ops->init_recv_priv = rtl8822eu_init_recv_priv;
	ops->free_recv_priv = rtl8822eu_free_recv_priv;
#ifdef CONFIG_RTW_SW_LED
	ops->InitSwLeds = rtl8822eu_initswleds;
	ops->DeInitSwLeds = rtl8822eu_deinitswleds;
#endif

	ops->init_default_value = rtl8822eu_init_default_value;
	ops->intf_chip_configure = rtl8822eu_interface_configure;
	...
	ops->hal_xmit = rtl8822eu_hal_xmit;
	ops->mgnt_xmit = rtl8822eu_mgnt_xmit;
