 ...
 *
 *****************************************************************************/
#define _RTL8822EU_RECV_C_

#include <drv_types.h>			/* PADAPTER, rtw_xmit.h and etc. */
#include <hal_data.h>			/* HAL_DATA_TYPE */
#include "../../hal_halmac.h"		/* RX desc */
#include "../rtl8822e.h"		/* rtl8822e_query_rx_desc, rtl8822e_c2h_handler_no_io() */

int rtl8822eu_init_recv_priv(PADAPTER padapter)
{
	return usb_init_recv_priv(padapter, INTERRUPT_MSG_FORMAT_LEN);
}

void rtl8822eu_free_recv_priv(PADAPTER padapter)
{
...
static u8 recvbuf2recvframe_proccess_c2h(PADAPTER padapter, u8 *pbuf, s32 transfer_len)
{
	...
	rtl8822e_c2h_handler_no_io(padapter, pbuf, transfer_len);
...
(PADAPTER padapter, u8 *pbuf, struct rx_pkt_attrib *pattrib, union recv_frame *precvframe, _pkt *pskb)
{
	...
	if (check_fwstate(&padapter->mlmepriv, WIFI_MONITOR_STATE) == _FALSE) {
		if (rtl8822e_rx_fcs_appended(padapter))
...
int recvbuf2recvframe(PADAPTER padapter, void *ptr)
{
	...
	do {
		...
		rtl8822e_query_rx_desc(precvframe, pbuf);
