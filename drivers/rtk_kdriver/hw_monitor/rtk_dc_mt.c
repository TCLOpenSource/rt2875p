...
int dump_memtrash_log(DCID dc_id, unsigned int entry, MT_SYS_ID sys_id, int mode); //DC1/2 MT/MTEX DS_SYS1/2/3
...
int kernel_trap_warning = DCMT_INTR_MODE_PANIC;  // change default to 1
...
#define SET_ENTRY_NAME(_entry, _STR)  strncpy(_entry.name,_STR, 20)
...
int dump_memtrash_log(DCID dc_id, unsigned int entry, MT_SYS_ID sys_id, int mode) //DC1/2 MT/MTEX DS_SYS1/2/3
{
    ...
    DCMT_LOG(DCMT_LOG_ISR, KERN_ERR, FRED_START "\n\n\n\n\n\n"
             "[Memory trash][KEY] DC%d-SYS%d @ mem%d ,module:%s(0x%x) trash phy_addr:0x%08x,access_type:%s,access_len:%d"
             "\n\n\n\n\n\n" FCOLOR_END,
             dc_id + 1,
             sys_id + 1,
             entry,
...
static int _dcmt_set_monitor(unsigned int entry, DC_MT_DESC * pDesc, int update)
{
    ...
    */
    ...
    for(id = DCID_1; id < DC_NUM; ++id)
    {
        ...
        if((dc_addr_start > dc_addr_end)) //when region offset is 0,dc2 end < start
        {
...
static void dcmt_monitor_over_range(void) //use mem4(mem4 is mtex-0)
{
    ...
    mt_desc.type = MEMORY_TYPE_SERIAL_BLOCK;
    mt_desc.start = total_ddr_size;
    mt_desc.end = total_ddr_size+0x1000-1;
...
int dcmt_monitor_carvedout_mem(char * key)
{
    ...
    else if(isStrFullMatch(key,"K_OS",0) == 0)
    {
        ...
        pDesc->ip_entry[1].prot = PROT_MODE_RW;
        //pDesc->ip_entry[2].module = MODULE_SB1_TP2;
        //pDesc->ip_entry[2].prot = PROT_MODE_RW;
        pDesc->ip_entry[2].module = MODULE_SB1_EMMC_KC;
        pDesc->ip_entry[2].prot = PROT_MODE_RW;

#if defined(CONFIG_ARCH_RTK6748) || defined(CONFIG_ARCH_RTK6702)
        pDesc->ip_entry[3].module = MODULE_SB1_MD_KC;
#elif defined(CONFIG_ARCH_RTK2851A)
        pDesc->ip_entry[3].module = MODULE_SB3_MD_KC;
#endif

        pDesc->ip_entry[3].prot = PROT_MODE_RW;
        //pDesc->ip_entry[4].module = MODULE_SB2_KCPU;
    ...
    else if(isStrFullMatch(key,"SCALER_MDOMAIN",0) == 0)
    {
        ...
        pDesc->ip_entry[3].prot = PROT_MODE_RW;
        pDesc->ip_entry[4].module = MODULE_TVSB2_SNR_R;
        pDesc->ip_entry[4].prot = PROT_MODE_RW;
        pDesc->ip_entry[5].module = MODULE_TVSB4_DMATO3DLUT;
        pDesc->ip_entry[5].prot = PROT_MODE_RW;
    }
...
{
	return 0;
    dump_memtrash_log(id, entry, sys_id, 1);
#if 0
...
static irqreturn_t dc_mt_isr(int this_irq, void* dev_id)
{
    ...
    //dump logs
    is_in_dcmt_isr = 1;
    dump_memtrash_log(id, entry, sys_id, 0);
