#ifndef __DC_MT_COMMON_H__
#define __DC_MT_COMMON_H__
...
typedef enum //defined by spec
{
        DC1_MODE = 0,
        DC2_MODE = 1,
        IB_1_1_MODE = 2,
        IB_1_1_SCRMBLE_MODE = 3,
        IB_1_2_MODE = 4,
        IB_2_1_MODE = 5,
} IB_MODE;
...
enum DCMT_DFT_MEM
{
        ...
        DCMT_DFT_MEM_VCPU3,
#ifdef CONFIG_ARM64
        DCMT_DFT_MEM_MMUTABLE,
#endif
        DCMT_DFT_MEM_KASAN,
        DCMT_DFT_MEM_OVERRANGE2 = DC_MT_ENTRY_COUNT-1
};
...
#ifdef  CONFIG_RTK_KDRV_SUPPORT_VCPU2
	#define DC_INIT_VCPU2_ROUTING_EN (BIT(27))
#endif
...
#define DCMT_IB_SLICE_BASIC (128)

#define DC_SYS_ADDR_MAX (0x7fffffff)
...
// APIs
//////////////////////////////////////////////////////////////////
extern void _dc_mt_unset_mem_region(int i, unsigned char is_resume_store);
extern int  _dc_mt_set_parameter(unsigned char i, DC_MT_DESC* p_desc);
...
#define DCMT_LOG(flag, level, fmt, args...) \
        do { \
                ...
                { \
                        rtd_pr_dcmt_print(level,fmt, ## args); \
                } \
        }while(0) \
...
#define DC_MT_DBG(fmt, args...)     rtd_pr_dcmt_debug(" DBG, " fmt, ## args)
#else
#define DC_MT_DBG(fmt, args...)
#endif

#if 1
#define DC_MT_INFO(fmt, args...)    rtd_pr_dcmt_info(fmt, ## args)
#define DC_MT_WARNING(fmt, args...) rtd_pr_dcmt_warn(fmt, ## args)
#define DC_MT_ERR(fmt, args...)     rtd_pr_dcmt_err(fmt, ## args)
#define DC_MT_NOTICE(fmt,args...) rtd_pr_dcmt_notice(fmt, ## args)
#else
...
#define DCMT_DEBUG_INFO_REGISTER(name,func)  \
        do{     \
                atomic_notifier_chain_register(&dcmt_notifier_list, &DCMT_##name );\
                atomic_notifier_chain_register(&dcmt_notifier_list_cat, &DCMT_cat_##name );\
        }while(0)
#define DCMT_DEBUG_INFO_UNREGISTER(name,func)  \
        do{     \
                atomic_notifier_chain_unregister(&dcmt_notifier_list, &DCMT_##name );\
