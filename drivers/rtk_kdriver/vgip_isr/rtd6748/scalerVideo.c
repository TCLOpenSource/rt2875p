...
//#include <rbus/adc_reg.h> //ADC register spec out
...
void scalerVideo_UpdateDebugInfo()
{
	...
	if( scalerVideo_BitWiseOperation(vdpq_vdpq_fw_used2_reg.regValue, 23, 23, 0, VD_BIT_GET) != 0 )
	{
		...
		else
		{
			...
			if( FoundFlag == FALSE && (unsigned int)ScalerVideo_Debug.DisableSpecificSetBufferIdx < VD_SET_NUM )
			{
...
unsigned char scalerVideo_ApplyVDSetting(unsigned int SetNumber)
{
    ...
    switch(SetNumber)
    {
        ...
        case VDPQ_MOTION:
        {
			...
			if(ID_NO[6])
				vdpq_yc_3d_f4_pair_ctrl_6_reg.global_diff_tolerance = 7;
			else
...
{
	/* 0xb801a624 */
	rtd_outl(SB2_HD_SEM_NEW_5_reg,0);
	//VP_Delay(1);//~ 11 us
	//udelay(10);
}
...
unsigned char AVD_Clock_Flag_detection()
{
	...
	rstn_vdec2_flag = ((rtd_inl(SYS_REG_SYS_SRST1_reg)>>21)&0x01) ; /* b800_0104[21] , for VD SW reset , ,  0xb8000104[21]=1*/
	clken_atvin_vd_flag = ((rtd_inl(SYS_REG_SYS_CLKEN2_reg)>>27)&0x01); /* b800_0118[27], for VD clock reset ,  0xb8000118[27]=1*/
	//AVD_Reset_Enable_flag = ((rtd_inl(0xB801A604))&0x01); /*B801A604 [0], for AVD reset start, 0xB801A604[0]=1*/
	IFD_freq_change_flag = ((rtd_inl(SB2_DUMMY_1_reg)>>4)&0x01);/*B801A604 [4], for IFD_freq_change start, */
