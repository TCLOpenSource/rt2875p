...
extern void dynamic_change_hdr_setting_hdmi_finish(unsigned char hdr_enable, I3DDMA_COLOR_SPACE_T color_format, unsigned char hdr_type);
	...
	//unsigned char bEnable_main, bEnable_sub;//mac7p pq compile fix
			...
			if(((g_vGipChn1_Act_Hsta_Width != vGipChn1_Act_Hsta_Width ) && g_vGipChn1_Act_Hsta_Width != 0 ) ||
				((g_vGipChn1_Act_Vsta_Length != vGipChn1_Act_Vsta_Length) && g_vGipChn1_Act_Vsta_Length != 0 )) {
					...
					if(str_status == 0){
						...
						minute = time.tm_min;
						second = time.tm_sec;
						printk(KERN_NOTICE "####[David] notify FSC on vGipChn1_Act_Hsta_Width: 0x%x, vGipChn1_Act_Vsta_Length: 0x%x, jiffies: %lu, rtc[ms]: %lu, sys-time: %02d:%02d:%02d.%03d" , vGipChn1_Act_Hsta_Width,vGipChn1_Act_Vsta_Length,jiffies,jiffies*1000/HZ,hour,minute,second,millisecond);
					}else{
						printk(KERN_NOTICE "####[David] notify FSC on vGipChn1_Act_Hsta_Width: 0x%x, vGipChn1_Act_Vsta_Length: 0x%x, jiffies: %lu, rtc[ms]: %lu, str_status: %01d", vGipChn1_Act_Hsta_Width,vGipChn1_Act_Vsta_Length,jiffies,jiffies*1000/HZ,str_status);
					}
#else
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	if (handle_ch1_sta_flag == 1 && SysInfo != NULL && Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE) {
		//fwif_color_DM2_Hist_AutoMode_ch1_sta_ISR(); //mac7p pq compile fix
...
//<<<End 20171228, pinyen modify for Dolby HDMI to avoid pink gabage when the begining from SDR to DolbyHDR
 ...
 if((get_HDMI_Dolby_VSIF_mode() == DOLBY_HDMI_VSIF_LL)&& handle_ch1_end_flag)
{
#ifdef CONFIG_RTK_KDRV_DV
#ifdef	CONFIG_DOLBY_IDK_1_6_1_1
							//dolby_adapter_dolby_trigger_timer6();
