...
typedef	struct {
		...
		unsigned long	FIFO_VALID;           /* Smartcard valid TX/RX fifo byte ([bit0-bit3] map to fifo[byte0-byte3]) Register */
#endif
} SC_REG;
...
extern unsigned long long scd_interface_value(scd *p_this);
