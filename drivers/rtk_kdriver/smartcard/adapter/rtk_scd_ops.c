...
static int  ops_interface_value(scd_device *dev,unsigned long long *interface_value)
{
	scd *p_this = (scd *) scd_get_drvdata(dev);
...
{
	smc_irq = platform_get_irq(pdev, 0);
	if (smc_irq < 0){
		SC_ERR("rtk_smc_probe get_irq failed \n");
		return smc_irq;
	}
    return 0;
}
...
{
	SC_INFO("rtk_smc_remove\n");
    return 0;
}

static const struct of_device_id of_rtk_smc_ids[] = {
    { .compatible = "realtek,rtk_smartcard" },
    {}
};
MODULE_DEVICE_TABLE(of, of_rtk_smc_ids);

static struct platform_driver scd_platform_drv =
{
	.probe      = rtk_smc_probe,
	.remove     = rtk_smc_remove,
	.driver   = {
		.name = "rtk-smc",
		.of_match_table = of_match_ptr(of_rtk_smc_ids),
	}
};
 ...
 *------------------------------------------------------------------*/
int __init scd_module_init(void)
{
	SC_INFO("realtek scd module init\n");

	platform_driver_register(&scd_platform_drv);
 ...
 * Retn : 0 : success, others fail
 *------------------------------------------------------------------*/
void __exit scd_module_exit(void)
{
	...
	platform_driver_unregister(&scd_platform_drv);
