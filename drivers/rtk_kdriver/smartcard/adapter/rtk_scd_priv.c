...
#define SET_FIFO_VALID(adp, val)     rtd_outl(adp->reg.FIFO_VALID,  val)
#endif

#define GET_MIS_ISR(adp)             rtd_inl(adp->reg.ISR)
/* refer to smart card spec */
#define TX_FIFO_LENGTH      128
#define RX_FIFO_LENGTH      128
...
#ifdef CONFIG_RTK_KDRV_MIO_SMARTCARD
#define GET_SCFP(adp)                rtd_inl(0xB801BF40)
#else
...
#ifdef CONFIG_RTK_KDRV_MIO_SMARTCARD
#define GET_SC_RXLENR(adp)           (rtd_inl(adp->reg.RXLENR) & 0xFF)
#else
...
static void dump_sc_tx_log(scd *p_this,int flag)
{
    ...
    {
        if(flag)
            SC_ERR("[%08lu] event=%s, fifo_len=%ld, tx_status=%lx, SCCR=%08lx, IRSR=%08lx, IRER=%08lx, MIS_ISR=%08lx\n", 
            ...
            SC_WARNING("[%08lu] event=%s, fifo_len=%ld, tx_status=%lx, SCCR=%08lx, IRSR=%08lx, IRER=%08lx, MIS_ISR=%08lx\n", 
                tx_event_log[i].time_stamp, 
                TxEvent(tx_event_log[i].event),
                tx_event_log[i].tx_fifo_len, 
                tx_event_log[i].tx_status, 
...
void scd_xmit_tx_fifo(scd *p_this, unsigned char *tx_buff, unsigned int tx_len)
{
    unsigned int i = 0;
...
#ifdef CONFIG_ARCH_RTK6702
    SET_FIFO_VALID(p_this,  SC_FIFO_VALID0);
    for (i = 0; i < tx_len; i++){
    ...
    if(tx_len >= 4){
        SET_FIFO_VALID(p_this,  SC_FIFO_VALID0 | SC_FIFO_VALID1 | SC_FIFO_VALID2 | SC_FIFO_VALID3);
        while(tx_len >= 4){
            fifo_data = MISC_SMARTCARD1_SC_TXFIFO_dat0(tx_buff[i])
                  | MISC_SMARTCARD1_SC_TXFIFO_dat1(tx_buff[i+1])
                  | MISC_SMARTCARD1_SC_TXFIFO_dat2(tx_buff[i+2])
                  | MISC_SMARTCARD1_SC_TXFIFO_dat3(tx_buff[i+3]);
            SET_SC_TXFIFO(p_this, fifo_data);
            i += 4; tx_len -= 4;
        }
    }

    if(3 == tx_len){
        SET_FIFO_VALID(p_this,  SC_FIFO_VALID0 | SC_FIFO_VALID1 | SC_FIFO_VALID2);
        fifo_data = MISC_SMARTCARD1_SC_TXFIFO_dat0(tx_buff[i])
                | MISC_SMARTCARD1_SC_TXFIFO_dat1(tx_buff[i+1])
                | MISC_SMARTCARD1_SC_TXFIFO_dat2(tx_buff[i+2]);
        SET_SC_TXFIFO(p_this, fifo_data);
    }else if(2 == tx_len){
        SET_FIFO_VALID(p_this,  SC_FIFO_VALID0 | SC_FIFO_VALID1);
        fifo_data = MISC_SMARTCARD1_SC_TXFIFO_dat0(tx_buff[i])
                | MISC_SMARTCARD1_SC_TXFIFO_dat1(tx_buff[i+1]);
        SET_SC_TXFIFO(p_this, fifo_data);
    }else if(1 == tx_len){
...
unsigned int scd_read_rx_fifo(scd *p_this, unsigned char *rx_buff, unsigned int rx_len)
{
    unsigned int i = 0;
    ...
    for (i = 0; i < rx_len; i++){
        if( rx_buff )
            rx_buff[i] = GET_SC_RXFIFO(p_this);
    }
#else
    unsigned int fifo_data;
    if(rx_len >= 4){
        SET_FIFO_VALID(p_this,  SC_FIFO_VALID0 | SC_FIFO_VALID1 | SC_FIFO_VALID2 | SC_FIFO_VALID3);
        while(rx_len >= 4){
            fifo_data = GET_SC_RXFIFO(p_this);
            if(rx_buff){
                rx_buff[i++] = MISC_SMARTCARD1_SC_RXFIFO_get_dat0(fifo_data);
                rx_buff[i++] = MISC_SMARTCARD1_SC_RXFIFO_get_dat1(fifo_data);
                rx_buff[i++] = MISC_SMARTCARD1_SC_RXFIFO_get_dat2(fifo_data);
                rx_buff[i++] = MISC_SMARTCARD1_SC_RXFIFO_get_dat3(fifo_data);
            }
            rx_len -= 4;
        }
    }
...
void scd_fsm_reset(scd *p_this)
{
	...
	if (event & SC_RCV_INT) {
		...
		if (p_this->atr.length < 0) {
			...
			i = scd_read_rx_fifo(p_this, NULL, rx_len);
			goto end_proc;
		}
		/* receive atr :*/
		while ((rx_len = GET_SC_RXLENR(p_this)) > 0) {
			if (p_this->atr.length >= MAX_ATR_SIZE)
				goto err_atr_failed;

			i = scd_read_rx_fifo(p_this, &(p_this->atr.data[p_this->atr.length]), rx_len);
...
void scd_fsm_active(scd *p_this)
{
	...
	unsigned char len;
	unsigned char buff[RX_FIFO_LENGTH];
	int tx_data_len;
	...
	if (event & SC_RCV_INT) {
		...
		if (len) {
			...
			(void)scd_read_rx_fifo(p_this, buff, len);
	...
	if (event & SC_TXEMPTY_INT) {
		...
		tx_data_len = kfifo_len(&p_this->tx_fifo);	/* get number of data to xmit*/

		if (tx_data_len && (p_this->tx_status & SC_TX_ABORT) == 0) {
			unsigned char tx_buff[TX_FIFO_LENGTH];
			...
			scd_xmit_tx_fifo(p_this, tx_buff, tx_data_len);
...
int scd_set_clock(scd *p_this, unsigned long clk)
{
	...
	SC_INFO("SYSTEM_CLK:%d, div:0x%x, clk_div:0x%x, pre_clock_div:0x%x\n", SYSTEM_CLK, div, p_this->clock_div, p_this->pre_clock_div);
	SET_SCFP(p_this, val);
...
{
	unsigned int baud_div2[4] = {31, 32, 39,1};
	return (id<4) ? baud_div2[id]: baud_div2[0];
}
...
int scd_set_etu(scd *p_this, unsigned long etu)
{
    ...
    SC_INFO("ETU = %lu. Baud Div2=%lu (%d), Div1 = %ld, ClkDiv=%d\n", 
			etu/p_this->clock_div, 
			p_this->baud_div2,
...
int scd_reset(scd *p_this)
{
	...
	wait_event_interruptible_timeout(p_this->wq, p_this->fsm != IFD_FSM_RESET, (HZ));	/*  wait 1000 ms  */
...
int scd_xmit(scd *p_this, unsigned char *p_data, unsigned int len)
{
	...
	while(kfifo_len(&p_this->tx_fifo)) {
		...
		spin_lock_irqsave(&p_this->lock, flag);
#ifdef CONFIG_RTK_KDRV_MIO_SMARTCARD
        /*not support one byte by one byte xmit*/
#else
		// load data to fifo ...
...
#else
	while (len) {
		unsigned char tx_len = (len > TX_FIFO_LENGTH) ? TX_FIFO_LENGTH : len;

		SET_SCIRSR(p_this, SC_TXEMPTY_INT);	/* clear tx empty...*/

		scd_xmit_tx_fifo(p_this, p_data, tx_len);
		  ...
		  unsigned long timeout)
{
	...
	if ((p_this->rx_status & SC_RX_ABORT)) {
		length = kfifo_out_locked(&p_this->rx_fifo, data, len, &p_this->rx_fifo_lock);	/* read out data that already received*/
		SC_ERR("Read Aborted\n");
		return SC_ERR_USR_ABORT;
	}

	return kfifo_out_locked(&p_this->rx_fifo, data, len,
				&p_this->rx_fifo_lock);
}
...
static void scd_load_gpio_config(scd *p_this)
{
	...
	/* PinMux for debug Mark2 with MIO Board */
	rtd_outl(PINMUX_MIO_ST_GPIO_MIO_CFG_20_reg, (rtd_inl(PINMUX_MIO_ST_GPIO_MIO_CFG_20_reg) & ~(0xF << 12)) | (0x8 << 12)); /* CD 0x18060c50[15:12] 8: SC_CD_src0 B5*/
	rtd_outl(PINMUX_MIO_ST_GPIO_MIO_CFG_20_reg, (rtd_inl(PINMUX_MIO_ST_GPIO_MIO_CFG_20_reg) & ~(0x1 << 10)) | (0x1 << 10)); /* CD 0x18060c50[15:12] 8: SC_CD pull down*/

	//rtd_outl(PINMUX_MIO_Pin_Mux_Ctrl0_reg, (rtd_inl(PINMUX_MIO_Pin_Mux_Ctrl0_reg) & ~(0x3 << 23)) | (0x0 << 23)); /* CD src in input mux, select src0 */

	rtd_outl(PINMUX_MIO_ST_GPIO_MIO_CFG_15_reg, (rtd_inl(PINMUX_MIO_ST_GPIO_MIO_CFG_15_reg) & ~(0xF << 20)) | (0x8 << 20)); /* IO 0x18060c3c[23:20]; 8: SC_DATA_src0 G10*/
	//rtd_outl(PINMUX_Pin_Mux_Ctrl2_reg, (rtd_inl(PINMUX_Pin_Mux_Ctrl2_reg) & ~(0x3 << 4)) | (0x0 << 4)); /* IO data in input mux, select src0 */

	rtd_outl(PINMUX_MIO_ST_GPIO_MIO_CFG_21_reg, (rtd_inl(PINMUX_MIO_ST_GPIO_MIO_CFG_21_reg) & ~(0xF << 4)) | (0x8 << 4)); /* RST 0x18060c54[7:4] 8: SC_RST C9*/
	rtd_outl(PINMUX_MIO_ST_GPIO_MIO_CFG_18_reg, (rtd_inl(PINMUX_MIO_ST_GPIO_MIO_CFG_18_reg) & ~(0xF << 20)) | (0x8 << 20)); /* CLK 0x18060c48[23:20] 8: SC_SCLK D12*/

	rtd_outl(PINMUX_MIO_ST_GPIO_MIO_CFG_9_reg, rtd_inl(PINMUX_MIO_ST_GPIO_MIO_CFG_9_reg) | (0xF << 20));    /*PWR SEL  : MIO GPIO 37 M6*/
	rtd_outl(PINMUX_MIO_ST_GPIO_MIO_CFG_21_reg, rtd_inl(PINMUX_MIO_ST_GPIO_MIO_CFG_21_reg) | (0xF << 20));    /* CMD VCC : MIO GPIO 85 A9*/

 	val = 0x8C5525;
#else
	// PinMux for debug Merlin6 Board
	rtd_outl(PINMUX_GPIO_RIGHT_CFG_10_reg, (rtd_inl(PINMUX_GPIO_RIGHT_CFG_10_reg) & ~(0xF << 4)) | (0x8 << 28)); /* CD 0xB8000894[31:28] 8: SC_CD_src0 AB27*/
	rtd_outl(PINMUX_Pin_Mux_Ctrl2_reg, (rtd_inl(PINMUX_Pin_Mux_Ctrl2_reg) & ~(0x3 << 8)) | (0x0 << 8)); /* CD src in input mux, select src0 */

	rtd_outl(PINMUX_GPIO_RIGHT_CFG_4_reg, (rtd_inl(PINMUX_GPIO_RIGHT_CFG_4_reg) & ~(0xF << 4)) | (0x8 << 4)); /* IO 0xB800087C[7:4]; 8: SC_DATA_src0 Y22*/
	rtd_outl(PINMUX_Pin_Mux_Ctrl2_reg, (rtd_inl(PINMUX_Pin_Mux_Ctrl2_reg) & ~(0x3 << 4)) | (0x0 << 4)); /* IO data in input mux, select src0 */

	rtd_outl(PINMUX_GPIO_RIGHT_CFG_11_reg, (rtd_inl(PINMUX_GPIO_RIGHT_CFG_11_reg) & ~(0xF << 20)) | (0x8 << 20)); /* RST 0xB8000898[23:20] 8: SC_RST Y26*/
	rtd_outl(PINMUX_GPIO_RIGHT_CFG_7_reg, (rtd_inl(PINMUX_GPIO_RIGHT_CFG_7_reg) & ~(0xF << 4)) | (0x8 << 4)); /* CLK 0xB8000888[7:4] 8: SC_SCLK AE26*/

	rtd_outl(PINMUX_GPIO_LEFT_CFG_1_reg, rtd_inl(PINMUX_GPIO_LEFT_CFG_1_reg) | (0xF << 20));    /*PWR SEL  : MISC GPIO 62 AA9*/
	rtd_outl(PINMUX_GPIO_RIGHT_CFG_10_reg, rtd_inl(PINMUX_GPIO_RIGHT_CFG_10_reg) | (0xF << 4));    /* CMD VCC : MISC GPIO 126 AA26*/
...
#ifdef CONFIG_RTK_KDRV_MIO_SMARTCARD
	p_this->pin_cmd_vcc =
	    rtk_gpio_id(MIO_GPIO, (val >> 8) & 0xFF);
#else
	p_this->pin_cmd_vcc =
 ...
 unsigned long long scd_interface_value(scd *p_this)
{
	...
	SC_PRINT_AWAYS
	    (" \n SC%d : \n\n        interface value=%llx\n\n        cmd_vcc(en=%d, polarity=%d, pin=%s_GPIO[%d] ,pin_dir=%d(0:in,1:out) ,pin_value=%d)\n\n        pwr_sel(en=%d,polarity=%d,pin=%s_GPIO[%d] ,pin_dir=%d(0:in,1:out) ,pin_value=%d ) \n\n \n",
	     ...
	     gpio_idx(this.pin_pwr_sel),rtk_gpio_get_dir(p_this->pin_cmd_vcc),rtk_gpio_output_get(p_this->pin_pwr_sel));
...
#ifdef CONFIG_RTK_KDRV_MIO_SMARTCARD
			CRT_CLK_OnOff(SMC_MIO, CLK_ON, NULL);
#endif
		}
...
#else
		if (smc_irq < 0){
			SC_ERR("get_irq failed \n");
			return NULL;
		}
		SC_INFO("Request IRQ #%d\n", smc_irq);
		if (request_irq(smc_irq, scd_isr, SA_SHIRQ, "REALTEK SCD",(void *)p_this) < 0) {
			SC_ERR("scd : open %s scd failed, unable to request irq#%d\n",IFD_MODOLE, smc_irq);
...
#ifdef CONFIG_RTK_KDRV_MIO_SMARTCARD
	CRT_CLK_OnOff(SMC_MIO, CLK_OFF, NULL);
#endif
	return 0;
}
