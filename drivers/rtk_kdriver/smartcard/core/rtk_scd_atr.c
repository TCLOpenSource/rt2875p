...
int is_atr_complete(scd_atr *p_atr)
{
	...
	if (T0 & 0x80) {
		...
		TD1 = p_atr->data[Idx - 1];
		Idx += ((TD1 >> 4) & 0x01) +
		    ((TD1 >> 5) & 0x01) +
		    ((TD1 >> 6) & 0x01) + ((TD1 >> 7) & 0x01);
	...
	if (TD1 & 0x80) {
		...
		TD2 = p_atr->data[Idx - 1];
		Idx += ((TD2 >> 4) & 0x01) +
		    ((TD2 >> 5) & 0x01) +
		    ((TD2 >> 6) & 0x01) + ((TD2 >> 7) & 0x01);
	}
	...
	/*-------- TX3 complete ------------    */
	if (TD2 & 0x80) {
		/* TD3 exists - find length of TD4*/
		TD3 = p_atr->data[Idx - 1];
		Idx += ((TD3 >> 4) & 0x01) +
		    ((TD3 >> 5) & 0x01) +
		    ((TD3 >> 6) & 0x01) + ((TD3 >> 7) & 0x01);
	}
	...
	/*-------- TX4 & header complete ------------  */
	if (TD3 & 0x80) {
		/* TD4 exists - find length of TD5*/
		TD4 = p_atr->data[Idx - 1];
		Idx += ((TD4 >> 4) & 0x01) +
		    ((TD4 >> 5) & 0x01) +
		    ((TD4 >> 6) & 0x01) + ((TD4 >> 7) & 0x01);
	}
	...
	case 15:
		...
		if (p_atr->length < Idx + nHistoryByte)	/* without TCK*/
			return 0;

		return 1;

	default:
		if (p_atr->length < 1 + Idx + nHistoryByte)	/* with TCK*/
			return 0;
...
int decompress_atr(scd_atr *p_atr, scd_atr_info *p_info)
{
	...
	p_info->nHistoryBytes = p_info->T0 & 0xF;
	...
	for (i = 0; i < 4; i++)
		if (p_info->T3[3] & (0x10 << i))
			p_info->T4[i] = p_atr->data[id++];
