#ifndef __RTK_SCD_H__
#define __RTK_SCD_H__

#include <linux/device.h>
#include "rtk_scd_debug.h"
#include "rtk_scd_atr.h"
#include "rtk_scd_buff.h"
...
typedef enum
{
	SC_INTERAL = 0, /*local smartcard*/
	SC_MIO = 1, /*mio smartcard*/
} SC_NUM;
...
typedef struct {
	...
	int (*set_param) (scd_device *dev,
			  unsigned long param_id, unsigned long val);

	int (*get_param) (scd_device *dev,
			  unsigned long param_id, unsigned long *p_val);
	...
	int (*abort) (scd_device *dev);	/* terminate currect IO immediately*/
	...
	int  (*get_interface_value)(scd_device *dev,unsigned long long *interface_value);
