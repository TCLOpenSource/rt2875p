...
static void rtk_omxdev_clock_acquire_lock(rtk_omxdev_client_t *client,
  ...
  if (client->group != INVALID_GROUP_ID) {
    rtd_pr_omx_err("[omx] acquire : client already has clock\n");
    req->status = RTK_OMXDEV_STATUS_ERROR;
    return;
  }

  if (param->group == INVALID_GROUP_ID) {
    rtd_pr_omx_err("[omx] acquire : invalid group id\n");
    req->status = RTK_OMXDEV_STATUS_ERROR;
    return;
  }
  ...
  if (i == MAX_CLOCK_COUNT) {
    if (clock_count == 0) {
      rtd_pr_omx_err("[omx] acquire : no clock available\n");
      req->status = RTK_OMXDEV_STATUS_ERROR;
      return;
    }
  ...
  rtd_pr_omx_info("[omx] %p acquire clock %08lx %08x %02u\n", client, clock->phyaddr,
  ...
  if (client->group != INVALID_GROUP_ID) {
    for (i = 0; i < MAX_CLOCK_COUNT; i++) {
      ...
      if (clock->group == client->group) {
        rtd_pr_omx_info("[omx] %p release clock %08lx %08x %02u\n", client,
  ...
  if (_device_context) {
    for (i = 0; i < MAX_CLOCK_COUNT; i++) {
      ...
      if (clock->refclock) {
        ...
        rtd_pr_omx_info("[omx] finalize refclock %u %08lx\n", i, clock->phyaddr);
        clock->refclock = NULL;
      }
    }
    rtd_pr_omx_info("[omx] finalize context %p\n", _device_context);
...
static int rtk_omxdev_context_create(void) {
  ...
  if (!_device_context) {
    rtd_pr_omx_err("[omx] failure to create content");
  ...
  for (i = 0; i < MAX_CLOCK_COUNT; i++) {
    ...
    if (!clock->refclock) {
      rtd_pr_omx_err("[omx] failure to refclock");
    ...
    clock->num_client = 0U;
    clock->time = 0U;
    rtd_pr_omx_info("[omx] create clock %u %08lx\n", i, clock->phyaddr);
  }
  mutex_unlock(&_device_lock);
...
static long _rtk_omxdev_ioctl_unknown(rtk_omxdev_client_t *client,
  ...
  rtd_pr_omx_err("[omx] ioctl    : invalid command %d\n", req->cmd);
  return 0;
}
...
static long _rtk_omxdev_unlocked_ioctl(struct file *filp, unsigned int cmd,
  ...
  if (cmd == RTK_OMXDEC_IOC_REQUEST) {
    ...
    if (!ret) {
      ...
      rtd_pr_omx_warn("[omx] ioctl    : %p command %d, ret %ld\n", client, req.cmd,
                 ret);
    }
  }
  return ret;
...
static const struct proc_ops rtk_omxdev_proc_status_fops = {
    .proc_open = rtk_omxdev_proc_status_open,
    .proc_read = seq_read,
...
static int rtk_omxdev_proc_init(void) {
  if (rtk_omxdev_proc_dir == NULL && rtk_omxdev_proc_entry == NULL) {
    ...
    if (rtk_omxdev_proc_dir == NULL) {
      rtd_pr_omx_err("[omx] failure to create proc dir (%s)\n", RTK_OMXDEC_PROC_DIR);
    ...
    if (rtk_omxdev_proc_entry == NULL) {
      rtd_pr_omx_err("[omx] failure to create proc log entry\n");
...
static int __init rtk_omxdev_init(void) {
  ...
  if (ret) {
    rtd_pr_omx_err("[omx] failure to allocate context\n");
    return ret;
  }
  ret = misc_register(&rtk_omxdev_miscdev);
  if (ret) {
    rtd_pr_omx_err("[omx] failure to misc_register\n");
    return ret;
  }
  ret = rtk_omxdev_proc_init();
  if (ret) {
    rtd_pr_omx_err("[omx] failure to rtk_omxdev_proc_init\n");
    return ret;
  }

  rtd_pr_omx_info("[omx] init\n");
