...
#define VERSION "$Id: RTK_289X 2016-06-13 20:10 $"
...
extern void emmc_write_api_register(s32 (*func)(u8*, u64, u32));
...
/************************************************************************
 * Note: These setting should same with rtkemmc_299x.c()
 ************************************************************************/
#define RAW_PAD_HS50_1 ((0x12UL<<26)|(0x12UL<<17)|(0x12UL<<8))
#define RAW_PAD_HS50_2 ((0x12UL<<26)|(0x12UL<<17)|(0x12UL<<8))
#define RAW_PAD_HS50_3 ((0x12UL<<26)|(0x12UL<<17)|(0x12UL<<8))
#define RAW_PAD_HS50_4 ((0x12UL<<26)|(0x12UL<<17)|(0x12UL<<8))
#define RAW_PAD_HS50_5 ((0x12UL<<26))
...
void raw_em_set_IO_driving(void)
{
    ...
    */
    cr_writel(0x11100000,RTK_IOPAD_CFG1);	//rst,clk,cmd,
    cr_writel(0x11111111,RTK_IOPAD_CFG2);	//ds,d5,d3,d4,d0,d1,d2,d7
    cr_writel(0x10000000,RTK_IOPAD_CFG3);	//d6


	io_pad_setting1 = RAW_PAD_HS50_1;
	io_pad_setting2 = RAW_PAD_HS50_2;
	io_pad_setting3 = RAW_PAD_HS50_3;
	io_pad_setting4 = RAW_PAD_HS50_4;
...
int raw_em_set_base_clk(void)
{
	...
	/* Phase Setting */
	reginfo = cr_readl(PLL27X_REG_EMMC_PLLPHASE_CTRL_reg) & ~((0x3fUL<<20)|(0x3fUL<<10)|(0x3fUL<<0));
	reginfo |= ((DEFAULT_C_PHASE<<20)|(DEFAULT_R_PHASE<<10)|(DEFAULT_W_PHASE<<0));
	cr_writel(reginfo,PLL27X_REG_EMMC_PLLPHASE_CTRL_reg);
...
int raw_em_TriggerXferCmd(unsigned int cmd_flag, struct sd_cmd_pkt *cmd_info)
{
	...
	cr_writel(cr_readl(EM_INT_STATUS)|(BIT(1)|BIT(2)),
					  EM_INT_STATUS);
	if(cmd_flag & BIT(10))
		wr_bit = BIT(1);
	else
...
static s32 raw_dw_em_init(void)
{
	...
	mmcraw("raw dw eMMC initial 2018/04/29 13:30\n");
   	mmcraw("(RTK287O plateform)\n");
...
CMD0_RETRY:
	...
	while (timeend--){
		...
		if(resp[0] == 0xffffffff){
			EM_ERR("resp error (0x%08x)\n",resp[0]);
			resp[0] = 0;
		}

		if (resp[0] & MMC_CARD_BUSY){
			if( resp[0] & MMC_SECTOR_ADDR ) {
				EM_ERR("sector addressing card\n");
			}else{
				EM_ERR("byte addressing card\n");
			}
			break;
		}
...
{
	if (mmcoops)
		EM_WARNING("Overriding previously set MMCOOPS ops\n");
...
static int __init lg1k_ll_mmc_init(void)
{
	...
	emmc_write_api_register(export_raw_dw_em_write);
