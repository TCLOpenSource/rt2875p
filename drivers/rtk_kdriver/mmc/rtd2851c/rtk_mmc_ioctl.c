...
extern bool get_mmc_tuning_50_done(struct mmc_host *host);
extern void set_mmc_tuning_50_done(struct mmc_host *host,bool val);
extern bool get_mmc_tuning_400_done(struct mmc_host *host);
extern void set_mmc_tuning_400_done(struct mmc_host *host,bool val);
...
void mmc_execute_tuning_50(struct mmc_host *host)
{
    ...
    if (get_mmc_tuning_400_done(host) && get_mmc_tuning_50_done(host) ){
    ...
    if (err)
        ...
        set_mmc_tuning_50_done(host,true);
    }
...
void mmc_execute_tuning_400(struct mmc_host *host)
{
    ...
    if (err){
        ...
        if(!get_mmc_tuning_400_done(host)){
        ...
        set_mmc_tuning_400_done(host,false);
    }
    else {
        EM_ERR("%s: ds tuning susccess\n", mmc_hostname(host));
        set_mmc_tuning_400_done(host,true);
    }
    return;
}
